...
public class MYSQLDBRegisterTableTests : RegisterTableTests
{
    ...
    protected override SQLObjectDatabase GetDatabaseV2 =>
        (SQLObjectDatabase) ObjectDatabase.GetObjectDatabase(ConnectionType.DATABASE_MYSQL,
            MySQLDBSetUp.ConnectionString);
    ...
    public void TestTableWithBrokenPrimaryKey()
    {
        ...
        Database.ExecuteNonQuery(string.Format("DROP TABLE IF EXISTS `{0}`",
            AttributesUtils.GetTableName(typeof(TestTableWithBrokenPrimaryV1))));
        // Create Table
        Database.RegisterDataObject(typeof(TestTableWithBrokenPrimaryV1));
        // Break Primary Key
        Database.ExecuteNonQuery(string.Format("ALTER TABLE `{0}` DROP PRIMARY KEY",
            AttributesUtils.GetTableName(typeof(TestTableWithBrokenPrimaryV1))));
