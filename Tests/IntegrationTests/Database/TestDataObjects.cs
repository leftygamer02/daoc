...
{
    [Relation(LocalField = "Test_TableRelation_ID", RemoteField = "Test_TableRelationEntry_ID", AutoLoad = true,
        AutoDelete = true)]
...
{
    [Relation(LocalField = "Test_TableRelations_ID", RemoteField = "ForeignTestField", AutoLoad = true,
        AutoDelete = true)]
...
{
    private string m_foreignTestField;

    [DataElement(Varchar = 255, Index = true)]
    public string ForeignTestField
    {
        get => m_foreignTestField;
        set
        {
            Dirty = true;
            m_foreignTestField = value;
        }
    }
...
{
    private string m_strUniquePart;

    [DataElement(UniqueColumns = "IntUniquePart")]
    public string StrUniquePart
    {
        get => m_strUniquePart;
        set
        {
            Dirty = true;
            m_strUniquePart = value;
        }
    }

    private int m_intUniquePart;

    [DataElement]
    public int IntUniquePart
    {
        get => m_intUniquePart;
        set
        {
            Dirty = true;
            m_intUniquePart = value;
        }
    }
...
{
    [Relation(LocalField = "Test_TableRelationsNoAutoload_ID", RemoteField = "ForeignTestField", AutoLoad = false,
        AutoDelete = true)]
    public TestTableRelationsEntries[] Entries;
...
{
    [Relation(LocalField = "Test_TableRelationsNoAutodelete_ID", RemoteField = "ForeignTestField", AutoLoad = true,
        AutoDelete = false)]
    public TestTableRelationsEntries[] Entries;
...
{
    [Relation(LocalField = "Test_TableRelationsPrecache_ID", RemoteField = "ForeignTestField", AutoLoad = true,
        AutoDelete = true)]
...
{
    [Relation(LocalField = "Test_TableRelationsPrecachePrimary_ID",
        RemoteField = "Test_TableRelationsEntryPrecached_ID", AutoLoad = true, AutoDelete = true)]
...
{
    [ReadOnly] [DataElement] public string ReadOnly { get; set; }
...
/// </summary>
[DataTable(TableName = "Test_TableBaseView", ViewName = "Test_TableAsView",
    ViewAs = "SELECT *, 'Weird Indeed' as `WeirdValue` FROM {0} WHERE `ViewValue` != 'HIDDEN'")]
public class TestTableAsView : TestTableBaseView
{
    [DataElement] public string WeirdValue { get; set; }
}

/// <summary>
/// Basic Table being a View of Base View and Implementing a Relation based on existing field.
/// </summary>
[DataTable(TableName = "Test_TableBaseView", ViewName = "Test_TableAsViewWithRelations",
    ViewAs = "SELECT * FROM {0}")]
...
{
    private string m_precachedValue;

    [DataElement]
    public string PrecachedValue
    {
        get => m_precachedValue;
        set
        {
            Dirty = true;
            m_precachedValue = value;
        }
    }
