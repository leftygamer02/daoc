...
[TestFixture]
internal class Test_DataQuest
{
    private DBDataQuest dbDataQuest = new();
    ...
    public void ParseQuest_GivenDBDataQuest_CompareStepData()
    {
        ...
        for (var i = 1; i <= stepCount; i++)
        {
            dataQuest.Step = i;
            var index = i - 1;
            Assert.AreEqual(new List<byte>() {0, 4, 6, 6, 5}[index], (byte) dataQuest.StepType,
                "Failed at Step " + dataQuest.Step);
            ...
            Assert.AreEqual(new string[] {"storyA", "", "storyC", "storyD", "storyEnd"}[index],
                dataQuest.SpySourceText);
            Assert.AreEqual(new string[] {"step1", "step2", "step3", "step4", "step5"}[index],
                dataQuest.StepTexts[index]);
            Assert.AreEqual(new string[] {"item1", "item2", "item3", "item4", "item5"}[index],
                dataQuest.SpyStepItemTemplate);
        }
    }
