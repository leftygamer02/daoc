...
public class SetUpTests
{
    ...
    public static void CreateGameServerInstance()
    {
        ...
        var CodeBase =
            new FileInfo(new Uri(Assembly.GetExecutingAssembly().Location).LocalPath).Directory;
        Console.WriteLine("Code Base: " + CodeBase.FullName);
        var FakeRoot = CodeBase.Parent;
    ...
    private class GameServerWithDefaultDB : GameServer
    {
        ...
        protected override void CheckAndInitDB()
        {
            ...
            {
                m_database =
                    ObjectDatabase.GetObjectDatabase(Configuration.DBType, Configuration.DBConnectionString);
                ...
                assembly.GetTypes().AsParallel().ForAll(type =>
                {
                    ...
                    if (attrib.Any()) m_database.RegisterDataObject(type);
                });

                var loadQuestsProp =
                    m_database.SelectObject<ServerProperty>(DB.Column("Key").IsEqualTo("load_quests"));
    ...
    public virtual void Init()
    {
        ...
        if (!GameServer.Instance.IsRunning)
        {
            ...
            if (!GameServer.Instance.Start()) Console.WriteLine("Error init GameServer");
        }
        else
        {
