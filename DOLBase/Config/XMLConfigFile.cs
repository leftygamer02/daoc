...
public class XMLConfigFile : ConfigElement
{
    ...
    {
        var badName = IsBadXMLElementName(name);
        ...
        if (element.HasChildren)
        {
            ...
            foreach (var entry in element.Children) SaveElement(writer, entry.Key, entry.Value);
    ...
    public static XMLConfigFile ParseXMLFile(FileInfo configFile)
    {
        ...
        using (var reader = new XmlTextReader(configFile.OpenRead()))
        {
            while (reader.Read())
                ...
                {
                    if (reader.Name != "root") current = current.Parent;
                }
        }
