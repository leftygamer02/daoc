...
public class MPK
{
    ...
    /// </summary>
    private readonly Dictionary<string, MPKFile> _files = new();
    ...
    /// </summary>
    public int DirectorySize => _sizeDir;
    ...
    public MPKFile this[string fname]
    {
        get
        {
            if (_files.ContainsKey(fname)) return _files[fname];
    ...
    public void Write(string fname)
    {
        ...
        using (var filemem = new MemoryStream())
        {
            using (var wrtr = new BinaryWriter(filemem, Encoding.UTF8))
            {
                ...
                for (byte i = 0; i < 16; i++) buf[i] ^= i;
            }
        }

        using (var fileStream = File.Open(fname, FileMode.Create, FileAccess.Write))
        {
            using (var wrtr = new BinaryWriter(fileStream, Encoding.UTF8))
            {
                ...
                foreach (var file in files) wrtr.Write(file.CompressedData);
            }
        }
    }
    ...
    public void Extract(string dirname)
    {
        ...
        if (!dirname.EndsWith(Path.DirectorySeparatorChar.ToString())) dirname += Path.DirectorySeparatorChar;

        foreach (var file in _files.Values) file.Save(dirname);
    }
    ...
    public void Read(string fname)
    {
        ...
        {
            if (rdr.ReadUInt32() != Magic) throw new Exception("Invalid MPK file");
    ...
    private void ReadArchive(BinaryReader rdr)
    {
        ...
        {
            var pos = rdr.BaseStream.Position;
            var len = rdr.BaseStream.Seek(0, SeekOrigin.End);
            ...
            using (var files = new MemoryStream(buf))
            {
                ...
                if (crc.Value != CRCValue) throw new Exception("Invalid or corrupt MPK");
    ...
    private static byte[] ReadDirectory(BinaryReader rdr, ref long totalin)
    {
        ...
        while (!inf.IsFinished)
        {
            ...
            {
                int count;
                if ((count = rdr.Read(input, 0, 1024)) <= 0) throw new Exception("EOF");
    ...
    public void Display()
    {
        ...
        Console.WriteLine("**************************************************************************");
        foreach (var file in _files.Values) file.Display();
    }

    private void OnInvalidFile(MPKFile file)
    {
        if (InvalidFile != null) InvalidFile(file, new EventArgs());
    }
