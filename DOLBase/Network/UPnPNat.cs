...
public class UPnPNat
{
    ...
    private static string _GetServiceUrl(string resp)
    {
        ...
        if (node == null)
            return null;
        var eventnode = desc.SelectSingleNode(
    ...
    public bool Discover()
    {
        ...
        do
        {
            ...
            if (resp.Contains("upnp:rootdevice"))
            {
                ...
                if (!string.IsNullOrEmpty(_serviceUrl = _GetServiceUrl(resp))) return true;
            }
        } while (length > 0);
    ...
    public List<PortForwading> ListForwardedPort()
    {
        ...
        var forwadedPort = new List<PortForwading>();
        for (var index = 0;; ++index)
            try
            {
                var xdoc = _GetSOAPRequest(
                    _serviceUrl,
                ...
                nsMgr.AddNamespace("tns", "urn:schemas-upnp-org:device-1-0");
                var port = new PortForwading();
                var node = xdoc.SelectSingleNode("//NewInternalClient/text()", nsMgr);
                if (node != null)
