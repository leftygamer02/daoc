...
public class FTPConnection
{
    ...
    public bool LogMessages
    {
        ...
        set
        {
            if (!value) _messageList = new List<string>();
    ...
    public List<string> GetDirectories(string mask)
    {
        ...
        var aRowList = table.Select("Name LIKE '" + mask + "'", "", DataViewRowState.CurrentRows);
        tmpList = new List<string>();
        for (var i = 0; i < aRowList.Length; i++) tmpList.Add((string) aRowList[i]["Name"]);
    ...
    public List<string> GetDirectories()
    {
        ...
        lock (_tcpClient)
        {
            ...
            returnValue = GetMessageReturnValue(tempMessageList[0]);
            if (!(returnValue == 150 || returnValue == 125)) throw new Exception(tempMessageList[0]);

            if (_mode == FTPMode.Active) client = listener.AcceptTcpClient();
            ...
            if (!(returnValue == 226)) throw new Exception(returnValueMessage);

            networkStream.Close();
            client.Close();

            if (_mode == FTPMode.Active) listener.Stop();
        }
    ...
    public virtual void DeleteFile(string remoteFileName)
    {
        lock (_tcpClient)
        {
            ...
            returnValue = GetMessageReturnValue(tempMessageList[0]);
            if (returnValue != 250) throw new Exception(tempMessageList[0]);
        }
    }
    ...
    private void SetMode(string mode)
    {
        ...
        lock (_tcpClient)
        {
            ...
            returnValue = GetMessageReturnValue(tempMessageList[0]);
            if (returnValue != 200) throw new Exception(tempMessageList[0]);
        }
    }
    ...
    private void SetDataPort(int portNumber)
    {
        var tempMessageList = new List<string>();
        var returnValue = 0;
        var iPortHigh = portNumber >> 8;
        var iPortLow = portNumber & 255;
    ...
    public virtual void CreateDirectory(string directoryName)
    {
        ...
        lock (_tcpClient)
        {
            ...
            returnValue = GetMessageReturnValue(tempMessageList[0]);
            if (returnValue != 257) throw new Exception(tempMessageList[0]);
        }
    }
    ...
    {
        while (_activeConnectionsCount != 0) Thread.Sleep(100);

        _activeConnectionsCount++;

        var cmdBytes = Encoding.ASCII.GetBytes((command + "\r\n").ToCharArray());
        var aStream = _tcpClient.GetStream();
    ...
    private List<string> Read()
    {
        ...
        var tempMessage = ReadLines(stream);
    ...
    private static List<string> ReadLines(NetworkStream stream)
    {
        ...
        while (read)
        {
            ...
            if (bytes < buffer.Length) read = false;
        }

        var mess = tmpMes.Split(seperator);
        ...
        lock (_tcpClient)
        {
            switch (_mode)
            {
                ...
                case FTPMode.Passive:
                    ...
                    var index2 = message.IndexOf(",", index1 + 1);
                    var index3 = message.IndexOf(",", index2 + 1);
                    var index4 = message.IndexOf(",", index3 + 1);
                    var index5 = message.IndexOf(",", index4 + 1);
                    var index6 = message.IndexOf(")", index5 + 1);
    ...
    {
        if (_logMessages) _messageList.AddRange(messages);
    }
