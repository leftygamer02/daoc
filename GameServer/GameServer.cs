...
public class GameServer : BaseServer
{
    ...
    protected virtual IServerRules ServerRulesImpl
    {
        get
        {
            if (Instance.m_serverRules == null)
            {
                ...
                {
                    if (log.IsErrorEnabled) log.Error("ServerRules null on access and failed to create.");
                }
            }
    ...
    {
        get => Configuration.SaveInterval;
        set
        {
    ...
    /// </summary>
    public bool IsRunning => _listen != null;

    /// <summary>
    /// Gets the number of millisecounds elapsed since the GameServer started.
    /// </summary>
    public int TickCount => Environment.TickCount - m_startTick;
    ...
    public static void CreateInstance(GameServerConfiguration config)
    {
        ...
        if (!logConfig.Exists) ResourceUtil.ExtractResource("logconfig.xml", logConfig.FullName);
    ...
    /// </summary>
    protected Socket UDPSocket => m_udpListen;

    /// <summary>
    /// Gets the UDP buffer of this server instance
    /// </summary>
    protected byte[] UDPBuffer => m_udpBuf;
    ...
    protected bool StartUDP()
    {
        ...
        try
        {
            ...
            if (Configuration.UDPOutEndpoint != null) m_udpOutSocket.Bind(Configuration.UDPOutEndpoint);
    ...
    protected void RecvFromCallback(IAsyncResult ar)
    {
        ...
        if (s != null)
        {
            ...
            {
                // Handle the packet
                var read = s.EndReceiveFrom(ar, ref tempRemoteEP);
                if (read == 0)
                {
                ...
                {
                    var pakCheck = (server.UDPBuffer[read - 2] << 8) | server.UDPBuffer[read - 1];
                    ...
                    if (calcCheck != pakCheck)
                    {
                        if (log.IsWarnEnabled)
                            log.WarnFormat(
                                "Bad UDP packet checksum (packet:0x{0:X4} calculated:0x{1:X4}) -> ignored",
                                pakCheck, calcCheck);
                        if (log.IsDebugEnabled)
                            log.Debug(Marshal.ToHexDump("UDP buffer dump, received " + read + "bytes",
                                server.UDPBuffer));
                    }
                    else
                    {
                        ...
                        if (client != null)
                        {
                            ...
                            if (client.UdpEndPoint.Equals(sender)) client.PacketProcessor.HandlePacket(pakin);
                        }
    ...
    private bool BeginReceiveUDP(Socket s, GameServer server)
    {
        ...
        try
        {
            s.BeginReceiveFrom(server.UDPBuffer, 0, MAX_UDPBUF, SocketFlags.None, ref tempRemoteEP,
                m_udpReceiveCallback,
                server);
            ret = true;
        }
        catch (SocketException e)
        {
            log.Fatal(
                string.Format(
                    "Failed to resume receiving UDP packets. UDP is DEAD now. (code: {0}  socketCode: {1})",
                    e.ErrorCode,
                    e.SocketErrorCode), e);
        }
    ...
    public void SendUDP(byte[] bytes, int count, EndPoint clientEndpoint, AsyncCallback callback)
    {
        ...
        var took = Environment.TickCount - start;
    ...
    public override bool Start()
    {
        try
        {
            ...
            log.Info(
                $"Default ThreadPoool minworkthreads {minWorkerThreads} minIOCThreads {minIOCThreads} maxworkthreads {maxWorkerThreads} maxIOCThreads {maxIOCThreads}");
            ...
            if (!InitComponent(() => WorldManager = new WorldManager(this),
                    "Instancied World Manager Initialization"))
                return false;
            ...
            {
                log.InfoFormat("Not Loading Quest Manager : Obeying Server Property <load_quests> - {0}",
                    Properties.LOAD_QUESTS);
            }
            ...
            if (!InitComponent(GameLoop.Init(), "GameLoop Init")) return false;

            //Start Aux GameLoop
            if (!InitComponent(AuxGameLoop.Init(), "AuxGameLoop Init")) return false;
            ...
            if (Properties.DISCORD_ACTIVE && !string.IsNullOrEmpty(Properties.DISCORD_WEBHOOK_ID))
            {
                ...
                var message = new DiscordMessage(
                    "",
                    "Atlas GameServer",
                    "https://cdn.discordapp.com/avatars/924819091028586546/656e2b335e60cb1bfaf3316d7754a8fd.webp",
                    false,
                    new[]
                    {
                        new DiscordMessageEmbed(
                            ...
                            thumbnail: new DiscordMessageEmbedThumbnail(
                                "https://cdn.discordapp.com/emojis/865577034087923742.png")
                        )
                    }
                );
    ...
    {
        var scriptDirectory = Path.Combine(Configuration.RootDirectory, "scripts");
        ...
        if (!Configuration.EnableCompilation)
        {
            ...
            if (File.Exists(Configuration.ScriptCompilationTarget))
                ...
                log.WarnFormat(
                    "Compilation Disabled - Could not find pre-compiled Assembly : {0} - Server starting without Scripts Assembly!",
                    Configuration.ScriptCompilationTarget);

            compiled = true;
        }
        else
        {
            compiled = ScriptMgr.CompileScripts(false, scriptDirectory, Configuration.ScriptCompilationTarget,
                Configuration.AdditionalScriptAssemblies);
        }
        ...
        if (compiled)
        {
            ...
            try
            {
                ...
                foreach (var type in asm.GetTypes())
                {
                    ...
                    var attrib = type.GetCustomAttributes(typeof(DataTable), false);
                    if (attrib.Length > 0)
                    {
                        if (log.IsInfoEnabled)
    ...
    protected bool StartScriptComponents()
    {
        try
        {
            ...
            {
                GameEventMgr.RegisterGlobalEvents(asm, typeof(GameServerStartedEventAttribute),
                    GameServerEvent.Started);
                GameEventMgr.RegisterGlobalEvents(asm, typeof(GameServerStoppedEventAttribute),
                    GameServerEvent.Stopped);
    ...
    protected bool StartKeepManager()
    {
        ...
        foreach (var type in script.GetTypes())
        {
            ...
            try
            {
                var objs = type.GetCustomAttributes(typeof(KeepManagerAttribute), false);
        ...
        if (keepManager != null)
            try
            {
                var manager = Activator.CreateInstance(keepManager, null) as IKeepManager;
    ...
    protected virtual bool UpdateDatabase()
    {
        ...
        try
        {
            ...
            foreach (var type in asm.GetTypes())
            {
                ...
                var attributes = type.GetCustomAttributes(typeof(DatabaseUpdateAttribute), false);
    ...
    public override void Stop()
    {
        ...
        if (log.IsWarnEnabled)
        {
            var stacks = PacketProcessor.GetConnectionThreadpoolStacks();
            if (stacks.Length > 0)
            {
    ...
    private object m_packetBufPoolLock = new();

    public int MaxPacketPoolSize => Configuration.MaxClientCount * 3;
    ...
    {
        var count = MaxPacketPoolSize;

        lock (m_packetBufPoolLock)
        {
            m_packetBufPool = new Queue<byte[]>(count);

            for (var i = 0; i < count; i++) m_packetBufPool.Enqueue(new byte[BUF_SIZE]);
        }
            ...
            try
            {
                ...
                foreach (var assembly in AppDomain.CurrentDomain.GetAssemblies())
                    ...
                    assembly.GetTypes().AsParallel().ForAll(type =>
                    {
                        ...
                        if (attrib.Any())
                        {
                            if (log.IsInfoEnabled) log.InfoFormat("Registering table: {0}", type.FullName);
