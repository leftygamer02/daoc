...
//		}
    ...
    {
        return new GameSpellAndImmunityEffect(this, (int) CalculateEffectDuration(target, effectiveness), 0,
            effectiveness);
    }

    // constructor
    public HereticImmunityEffectSpellHandler(GameLiving caster, Spell spell, SpellLine spellLine) : base(caster,
        spell, spellLine)
    {
    }
}
...
public class HereticSpeedDecreaseSpellHandler : HereticImmunityEffectSpellHandler
{
    ...
    protected virtual void OnAttacked(DOLEvent e, object sender, EventArgs arguments)
    {
        ...
        if (attackArgs.AttackData.SpellHandler is StyleBleeding ||
            attackArgs.AttackData.SpellHandler is DoTSpellHandler ||
            attackArgs.AttackData.SpellHandler is HereticDoTSpellHandler)
        {
            var affect = FindEffectOnTarget(living, this);
            if (affect != null)
                affect.Cancel(false);
        }
    }
    ...
    public HereticSpeedDecreaseSpellHandler(GameLiving caster, Spell spell, SpellLine line) : base(caster, spell,
        line)
    {
        TIMER_PROPERTY = this;
    }
