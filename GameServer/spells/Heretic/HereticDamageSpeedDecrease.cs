...
public class HereticDamageSpeedDecrease : HereticSpeedDecreaseSpellHandler
{
    ...
    {
        var speclevel = 1;
        if (m_caster is GamePlayer) speclevel = ((GamePlayer) m_caster).GetModifiedSpecLevel(m_spellLine.Spec);

        min = 1;
        max = 1;

        if (target.Level > 0) min = 0.5 + (speclevel - 1) / (double) target.Level * 0.5;

        if (speclevel - 1 > target.Level)
        {
            var overspecBonus = (speclevel - 1 - target.Level) * 0.005;
            min += overspecBonus;
            max += overspecBonus;
        }
    ...
    public override void OnEffectPulse(GameSpellEffect effect)
    {
        ...
        if (m_caster.Mana < Spell.PulsePower) RemoveEffect();
        ...
        if (!m_caster.IsAlive || !effect.Owner.IsAlive || m_caster.Mana < Spell.PulsePower ||
            !m_caster.IsWithinRadius(effect.Owner, Spell.Range) || m_caster.IsMezzed || m_caster.IsStunned ||
        ...
        {
            var powerPerTarget = (double) (effect.Spell.PulsePower / m_focusTargets.Count);

            var powerUsed = (int) powerPerTarget;
    ...
    protected virtual void OnSpellResist(GameLiving target)
    {
        ...
        if (target is GameNPC)
        {
            ...
            if (brain != null)
            {
                ...
                if (owner != null)
                    MessageToLiving(owner, "Your " + target.Name + " resists the effect!",
