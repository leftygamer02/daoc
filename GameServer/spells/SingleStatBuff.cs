...
public abstract class SingleStatBuff : PropertyChangingSpell
{
    ...
    {
        var specLevel = Caster.GetModifiedSpecLevel(m_spellLine.Spec);
                ...
                if (playerCaster.CharacterClass.ClassType == eClassType.ListCaster)
                {
                    ...
                    if (playerCaster.UseDetailedCombatLog && m_caster.GetModified(eProperty.DebuffEffectivness) > 0)
                        playerCaster.Out.SendMessage(
                            $"debuff effectiveness: {m_caster.GetModified(eProperty.DebuffEffectivness)}",
        ...
        else if (Caster is NecromancerPet necroPetCaster && necroPetCaster.Owner is GamePlayer playerOwner &&
                 Spell.Target == "Enemy")
        {
        ...
        if (Spell.Target != "Enemy")
        {
            ...
            if (Caster is GamePlayer gamePlayer && gamePlayer.UseDetailedCombatLog &&
                m_caster.GetModified(eProperty.BuffEffectiveness) > 0)
                gamePlayer.Out.SendMessage(
                    $"buff effectiveness: {m_caster.GetModified(eProperty.BuffEffectiveness)}",
                    eChatType.CT_DamageAdd, eChatLoc.CL_SystemWindow);
        }
        else
        {
    ...
    {
        var critMod = 1.0;
        var critChance = Caster.DotCriticalChance;
        ...
        if (playerCaster?.UseDetailedCombatLog == true && critChance > 0)
            playerCaster.Out.SendMessage($"Debuff crit chance: {Caster.DotCriticalChance}", eChatType.CT_DamageAdd,
        ...
        if (Util.Chance(critChance))
        {
            ...
            playerCaster?.Out.SendMessage(
                $"Your {Spell.Name} critically debuffs the enemy for {Math.Round(critMod - 1, 3) * 100}% additional effect!",
...
public class ArmorFactorBuff : SingleStatBuff
{
    public override eBuffBonusCategory BonusCategory1
    {
        get
        {
            if (Caster is GamePlayer c && (c.CharacterClass is ClassRanger || c.CharacterClass is ClassHunter) &&
                (SpellLine.KeyName.ToLower().Equals("beastcraft") ||
                 SpellLine.KeyName.ToLower().Equals("pathfinding"))) return eBuffBonusCategory.BaseBuff;
