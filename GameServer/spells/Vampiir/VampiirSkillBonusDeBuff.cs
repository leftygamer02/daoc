...
public class VampiirSkillBonusDeBuff : SpellHandler
{
    ...
    public override void OnEffectStart(GameSpellEffect effect)
    {
        ...
        if (effect.Owner is GamePlayer)
        {
            ...
            for (var i = (int) eProperty.Skill_First; i <= (int) eProperty.Skill_Last; i++)
                if (player.GetModifiedSpecLevel(SkillBase.GetPropertyName((eProperty) i)) != 0)
                    player.BaseBuffBonusCategory[i] =
                        -player.GetModifiedSpecLevel(SkillBase.GetPropertyName((eProperty) i));
