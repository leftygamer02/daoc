...
public class VampiirBoltSpellHandler : SpellHandler
{
    ...
    {
        foreach (var targ in SelectTargets(target)) DealDamage(targ);
    ...
    protected class BoltOnTargetAction : RegionECSAction
    {
        ...
        public BoltOnTargetAction(GameLiving actionSource, GameLiving boltTarget,
            VampiirBoltSpellHandler spellHandler)
            : base(actionSource)
        {
            if (boltTarget == null)
        ...
        protected override int OnTick(ECSGameTimer timer)
        {
            ...
            if (target == null || target.CurrentRegionID != caster.CurrentRegionID ||
            ...
            if (target is GameNPC || target.Mana > 0)
            {
                if (target is GameNPC)
                    power = (int) Math.Round((double) target.Level * (double) m_handler.Spell.Value * 2 /
                                             100);
                else
                ...
                {
                    target.Mana -= power;
                    ((GamePlayer) target).Out.SendMessage(caster.Name + " takes " + power + " power!",
                        eChatType.CT_YouWereHit, eChatLoc.CL_SystemWindow);
                }

                if (caster is GamePlayer)
                    ((GamePlayer) caster).Out.SendMessage(
                        "You receive " + power + " power from " + target.Name + "!", eChatType.CT_Spell,
                        eChatLoc.CL_SystemWindow);
            }
            else
            {
                ((GamePlayer) caster).Out.SendMessage("You did not receive any power from " + target.Name + "!",
