...
public class RampingDamageFocus : SpellHandler
{
    ...
    public RampingDamageFocus(GameLiving caster, Spell spell, SpellLine spellLine) : base(caster,
        new FocusSpell(spell), spellLine)
    {
...
// 				}
    ...
    public override void OnDirectEffect(GameLiving target, double effectiveness)
    {
        ...
        foreach (var t in targets)
        {
            ...
            if (Spell.Value > 0) snareSubSpell.StartSpell(t);
        }
    }
    ...
    protected virtual void DealDamage(GameLiving target, double effectiveness)
    {
        ...
        var growthPercent = Spell.LifeDrainReturn * 0.01;
        var growthCapPercent = Spell.AmnesiaChance * 0.01;
        var damageIncreaseInPercent = Math.Min(pulseCount * growthPercent, growthCapPercent);
        ...
        get
        {
            ...
            if (Spell.Damage != 0)
                list.Add(p != null
                    ...
                    : LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE, "DelveInfo.Damage",
                        Spell.Damage.ToString("0.###;0.###'%'")));
            list.Add(p != null
                ? LanguageMgr.GetTranslation(p.Client, "DelveInfo.Target", Spell.Target)
                : LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE, "DelveInfo.Target",
                    Spell.Target));
            if (Spell.Range != 0)
                list.Add(p != null
                    ? LanguageMgr.GetTranslation(p.Client, "DelveInfo.Range", Spell.Range)
                    : LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE, "DelveInfo.Range",
                        Spell.Range));
            if (Spell.Duration != 0)
                list.Add(p != null
                    ? LanguageMgr.GetTranslation(p.Client, "DelveInfo.Duration") + " " +
                      (Spell.Duration / 1000).ToString("0' sec';'Permanent.';'Permanent.'")
                    : LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE, "DelveInfo.Duration") +
            ...
            if (Spell.Frequency != 0)
                list.Add(p != null
                    ...
                    : LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE, "DelveInfo.Frequency",
                        (Spell.Frequency * 0.001).ToString("0.0")));
            if (Spell.Power != 0)
                list.Add(p != null
                    ? LanguageMgr.GetTranslation(p.Client, "DelveInfo.PowerCost", Spell.Power.ToString("0;0'%'"))
                    : LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE, "DelveInfo.PowerCost",
            ...
            list.Add(p != null
                ...
                : LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE, "DelveInfo.CastingTime",
                    (Spell.CastTime * 0.001).ToString("0.0## sec;-0.0## sec;'instant'")));
            if (Spell.Radius != 0)
                list.Add(p != null
                    ? LanguageMgr.GetTranslation(p.Client, "DelveInfo.Radius", Spell.Radius)
                    : LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE, "DelveInfo.Radius",
                        Spell.Radius));
            if (Spell.DamageType != eDamageType.Natural)
                list.Add(p != null
                    ...
                    : LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE, "DelveInfo.Damage",
                        GlobalConstants.DamageTypeToName(Spell.DamageType)));
            list.Add(" "); //empty line
            list.Add("Repeating direct damage spell that starts at " + Spell.Damage + " " + Spell.DamageType +
                     " damage and increase by " + Spell.LifeDrainReturn + "% every tick up to a maximum of " +
                     Spell.AmnesiaChance + "%.");
            list.Add(" "); //empty line
            if (Spell.Value > 0)
    ...
    private ISpellHandler CreateSnare()
    {
        ...
        var subSpell = new Spell(dbSpell, Spell.Level);
