...
public class DamageReductionAndPowerReturnSpellHandler : SpellHandler
{
    ...
    public override void OnEffectStart(GameSpellEffect effect)
    {
        ...
        var toOther = Spell.Pulse == 0 ? eChatType.CT_System : eChatType.CT_Spell; ///Pulse;
...
//         Log.DebugFormat("sender:{0} res:{1} IsMelee:{2} Type:{3}", living.Name, ad.AttackResult, ad.IsMeleeAttack, ad.AttackType);
        ...
        var damagereduction = living.TempProperties.getProperty<int>(Damage_Reduction);
        ...
        if (ad.Damage > 0)
            MessageToLiving(ad.Target, string.Format("The damage reduction absorbs {0} damage!", damageAbsorbed),
                eChatType.CT_Spell);
        MessageToLiving(ad.Attacker,
            string.Format("A damage reduction absorbs {0} damage of your attack!", damageAbsorbed),
            eChatType.CT_Spell);
        if (damageAbsorbed > 0)
