...
public class DamageAddSpellHandler : AbstractDamageAddSpellHandler
{
    ...
    public void EventHandler(DOLEvent e, object sender, EventArgs arguments, double effectiveness)
    {
        ...
        var dpsCap = DPSCap(attacker.Level);
        var dps = IgnoreDamageCap ? Spell.Damage : Math.Min(Spell.Damage, dpsCap);
        double damage = Util.Random((int) (minVariance * dps * atkArgs.AttackData.WeaponSpeed * 0.1),
            (int) (maxVariance * dps * atkArgs.AttackData.WeaponSpeed * 0.1));
        ; // attack speed is 10 times higher (2.5spd=25)
        var damageResisted = damage * target.GetResist(Spell.DamageType) * -0.01;
        ...
        if (ad.Attacker is GameNPC)
        {
            ...
            if (brain != null)
            {
                ...
                if (owner != null)
                    MessageToLiving(owner,
                        string.Format(
                            LanguageMgr.GetTranslation(owner.Client,
                                "DamageAddAndShield.EventHandlerDA.YourHitFor"), ad.Attacker.Name,
                            target.GetName(0, false), ad.Damage), eChatType.CT_Spell);
            }
        }
        else
        {
            ...
            if (attackerClient != null)
                MessageToLiving(attacker,
                    string.Format(
                        LanguageMgr.GetTranslation(attackerClient, "DamageAddAndShield.EventHandlerDA.YouHitExtra"),
                        target.GetName(0, false), ad.Damage), eChatType.CT_Spell);
        }
        ...
        if (targetClient != null)
            MessageToLiving(target,
                string.Format(
                    LanguageMgr.GetTranslation(targetClient, "DamageAddAndShield.EventHandlerDA.DamageToYou"),
                    attacker.GetName(0, false), ad.Damage), eChatType.CT_Spell);
    ...
    // constructor
    public DamageAddSpellHandler(GameLiving caster, Spell spell, SpellLine spellLine) : base(caster, spell,
        spellLine)
    {
    }
}
...
public class DamageShieldSpellHandler : AbstractDamageAddSpellHandler
{
    ...
    public override void EventHandler(DOLEvent e, object sender, EventArgs arguments)
    {
        var args = arguments as AttackedByEnemyEventArgs;
        if (args == null) return;
        ...
        var attacker = sender as GameLiving; //sender is target of attack, becomes attacker for damage shield
        if (attacker == null) return;
        if (attacker.ObjectState != GameObject.eObjectState.Active) return;
        if (attacker.IsAlive == false) return;
        var target = args.AttackData.Attacker; //attacker becomes target for damage shield
        if (target == null) return;
        if (target.ObjectState != GameObject.eObjectState.Active) return;
        if (target.IsAlive == false) return;

        var spread = m_minDamageSpread;
        spread += Util.Random(50);
        var damage = Spell.Damage * target.AttackSpeed(target.ActiveWeapon) * spread * 0.00001;
        var damageResisted = damage * target.GetResist(Spell.DamageType) * -0.01;
            ...
            if (brain != null)
            {
                ...
                if (owner != null && owner.ControlledBrain != null && ad.Attacker == owner.ControlledBrain.Body)
                    MessageToLiving(owner,
                        string.Format(
                            LanguageMgr.GetTranslation(owner.Client,
                                "DamageAddAndShield.EventHandlerDS.YourHitFor"), ad.Attacker.Name,
                            target.GetName(0, false), ad.Damage), eChatType.CT_Spell);
            }
        }
        else if (attackerClient != null)
        {
            MessageToLiving(attacker,
                string.Format(
                    LanguageMgr.GetTranslation(attackerClient, "DamageAddAndShield.EventHandlerDS.YouHitFor"),
                    target.GetName(0, false), ad.Damage), eChatType.CT_Spell);
        }
    ...
    // constructor
    public DamageShieldSpellHandler(GameLiving caster, Spell spell, SpellLine spellLine) : base(caster, spell,
        spellLine)
    {
    }
}
...
public abstract class AbstractDamageAddSpellHandler : SpellHandler
{
    ...
    public override bool StartSpell(GameLiving target)
    {
        ...
        {
            var lineSpec = Caster.GetModifiedSpecLevel(m_spellLine.Spec);
    ...
    public override void OnEffectStart(GameSpellEffect effect)
    {
        ...
        if (Spell.Pulse == 0) chatType = eChatType.CT_Spell;

        var upperCase = Spell.Message2.StartsWith("{0}");
    ...
    public override int OnEffectExpires(GameSpellEffect effect, bool noMessages)
    {
        if (!noMessages && Spell.Pulse == 0)
        {
            ...
            var upperCase = Spell.Message4.StartsWith("{0}");
    ...
    // constructor
    public AbstractDamageAddSpellHandler(GameLiving caster, Spell spell, SpellLine spellLine) : base(caster, spell,
        spellLine)
    {
    }
}