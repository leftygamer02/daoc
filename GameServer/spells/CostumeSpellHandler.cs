...
internal class CostumeSpellHandler : SpellHandler
{
    ...
    private void OnZone(DOLEvent e, object sender, EventArgs arguments)
    {
        if (sender is GamePlayer)
            switch ((sender as GamePlayer).CurrentRegionID)
            {
                ...
                default:
                    var costume = FindEffectOnTarget(sender as GamePlayer, this);
                    if (costume != null)
                        costume.Cancel(true);
                    return;
            }
    ...
    public override int OnEffectExpires(GameSpellEffect effect, bool noMessages)
    {
        ...
        if (player != null)
        {
            ...
            GameEventMgr.RemoveHandler((GamePlayer) effect.Owner, GamePlayerEvent.RegionChanged,
                new DOLEventHandler(OnZone));
        }
