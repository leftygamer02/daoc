...
public abstract class SummonSpellHandler : SpellHandler
{
    ...
    public override void FinishSpellCast(GameLiving target)
    {
        foreach (GamePlayer player in m_caster.GetPlayersInRadius(WorldMgr.INFO_DISTANCE))
            if (player != m_caster)
                player.Out.SendMessage(
                    LanguageMgr.GetTranslation(player.Client.Account.Language, "GameObject.Casting.CastsASpell",
                        m_caster.GetName(0, true)), eChatType.CT_Spell, eChatLoc.CL_SystemWindow);
        ...
        {
            if (m_isSilent == false)
                MessageToCaster(string.Format("The {0} is now under your control.", m_pet.Name),
                    eChatType.CT_Spell);
        }
        else
        {
    ...
    {
        var point = Caster.GetPointFromHeading(Caster.Heading, 64);
        x = point.X;
        y = point.Y;
        z = Caster.Z;
    ...
    public override void ApplyEffectOnTarget(GameLiving target, double effectiveness)
    {
        ...
        if (template.ClassType != null && template.ClassType.Length > 0)
        {
            ...
            if (brain == null && log.IsWarnEnabled)
                log.Warn(
                    $"ApplyEffectOnTarget(): ClassType {template.ClassType} on NPCTemplateID {template.TemplateId} not found, using default ControlledBrain");
        }
        ...
        CreateECSEffect(new ECSGameEffectInitParams(m_pet, CalculateEffectDuration(target, effectiveness),
            effectiveness, this));
    ...
    protected virtual void OnNpcReleaseCommand(DOLEvent e, object sender, EventArgs arguments)
    {
        ...
        foreach (var ability in pet.effectListComponent.GetAbilityEffects())
            if (ability is InterceptECSGameEffect interceptEffect && interceptEffect.InterceptSource == pet &&
                interceptEffect.InterceptTarget == petOwner)
