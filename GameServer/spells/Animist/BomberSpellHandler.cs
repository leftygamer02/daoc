...
public class BomberSpellHandler : SummonSpellHandler
{
    ...
    private const string BOMBERSPAWNTICK = "bomberspawntick";
    ...
    public override void ApplyEffectOnTarget(GameLiving target, double effectiveness)
    {
        ...
        {
            var subspell = SkillBase.GetSpellByID(m_spell.SubSpellID);
        ...
        {
            m_pet.Level =
                m_pet.Owner?.Level ?? (byte) 1; // No bomber class to override SetPetLevel() in, so set level here
            ...
            m_pet.Follow(target, 5,
                Spell.Range * 5); // with Toa bonus, if the bomber was fired > Spell.Range base, it didnt move..
        }
    }
    ...
    private void BomberArriveAtTarget(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var subspell = SkillBase.GetSpellByID(m_spell.SubSpellID);
        var living = m_pet.TempProperties.getProperty<object>(BOMBERTARGET, null) as GameLiving;
        ...
        {
            var spellhandler =
                ScriptMgr.CreateSpellHandler(Caster, subspell, SkillBase.GetSpellLine(SpellLine.KeyName));
