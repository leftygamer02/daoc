...
public class SummonAnimistMainPet : SummonAnimistPet
{
        ...
        if (Caster is GamePlayer && Caster.ControlledBrain != null)
        {
            MessageToCaster(
                LanguageMgr.GetTranslation((Caster as GamePlayer).Client,
                    "SummonAnimistPet.CheckBeginCast.AlreadyHaveaPet"), eChatType.CT_SpellResisted);
            return false;
        }
    ...
    {
        if (Spell.DamageType == 0) return new TurretMainPetCasterBrain(owner);

        //[Ganrod] Nidel: Spell.DamageType : 1 for tank pet
        if (Spell.DamageType == (eDamageType) 1) return new TurretMainPetTankBrain(owner);
