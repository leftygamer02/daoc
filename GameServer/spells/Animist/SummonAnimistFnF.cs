...
public class SummonAnimistFnF : SummonAnimistPet
{
    ...
    public override bool CheckBeginCast(GameLiving selectedTarget)
    {
        ...
        var rgn = WorldMgr.GetRegion(Caster.CurrentRegion.ID);
            ...
            if (Caster is GamePlayer)
                MessageToCaster(
                    LanguageMgr.GetTranslation((Caster as GamePlayer).Client,
                        "SummonAnimistFnF.CheckBeginCast.NoGroundTarget"), eChatType.CT_SpellResisted);
            return false;
        }
        ...
        foreach (AbstractArea area in rgn.GetAreasOfSpot(Caster.GroundTarget))
            if (area is KeepArea)
                ...
                {
                    if (Caster is GamePlayer)
                        MessageToCaster("Cannot summon a turret this high near a keep!",
                            eChatType.CT_SpellResisted);
                    return false;
                }

        foreach (GameNPC npc in Caster.CurrentRegion.GetNPCsInRadius(Caster.GroundTarget.X, Caster.GroundTarget.Y,
                     Caster.GroundTarget.Z, (ushort) Properties.TURRET_AREA_CAP_RADIUS, false, true))
        ...
        if (nCount >= Properties.TURRET_AREA_CAP_COUNT)
        {
            if (Caster is GamePlayer)
                MessageToCaster(
                    LanguageMgr.GetTranslation((Caster as GamePlayer).Client,
                        "SummonAnimistFnF.CheckBeginCast.TurretAreaCap"), eChatType.CT_SpellResisted);
            return false;
        }
        ...
        if (Caster.PetCount >= Properties.TURRET_PLAYER_CAP_COUNT)
        {
            if (Caster is GamePlayer)
                MessageToCaster(
                    LanguageMgr.GetTranslation((Caster as GamePlayer).Client,
                        "SummonAnimistFnF.CheckBeginCast.TurretPlayerCap"), eChatType.CT_SpellResisted);
            return false;
        }
    ...
    protected override void OnNpcReleaseCommand(DOLEvent e, object sender, EventArgs arguments)
    {
        ...
        if (Caster.ControlledBrain == null) ((GamePlayer) Caster).Out.SendPetWindow(null, ePetWindowAction.Close, 0, 0);
