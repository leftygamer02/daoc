...
public abstract class AbstractBuffShear : SpellHandler
{
    ...
    public override void OnDirectEffect(GameLiving target, double effectiveness)
    {
        ...
        var mez = FindEffectOnTarget(target, "Mesmerize");
        if (mez != null)
        {
            mez.Cancel(false);
            return;
        ...
        //check for spell.
        foreach (var effect in target.EffectList.GetAllOfType<GameSpellEffect>())
            if (effect.Spell.SpellType.ToString() == ShearSpellType)
            {
                if ((effect.Owner != effect.SpellHandler.Caster || effect.Spell.IsShearable) &&
                    effect.Spell.Value <= Spell.Value)
                {
                    SendEffectAnimation(target, 0, false, 1);
                    effect.Cancel(false);
                    MessageToCaster("Your spell rips away some of your target's enhancing magic.",
                        eChatType.CT_Spell);
                    MessageToLiving(target, "Some of your enhancing magic has been ripped away by a spell!",
                        eChatType.CT_Spell);
                }
                else
                {
                    SendEffectAnimation(target, 0, false, 0);
                    MessageToCaster("The target's connection to their enhancement is too strong for you to remove.",
...
public class RandomBuffShear : SpellHandler
{
    ...
    private static Type[] buffs = new Type[]
    {
