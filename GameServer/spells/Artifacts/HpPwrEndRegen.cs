...
public class HpPwrEndRegenSpellHandler : SpellHandler
{
    ...
    public override void OnDirectEffect(GameLiving target, double effectiveness)
    {
        ...
        var healthtick = (int) (target.MaxHealth * 0.05);
        var manatick = (int) (target.MaxMana * 0.05);
        var endutick = (int) (target.MaxEndurance * 0.05);

        var modendu = target.MaxEndurance - target.Endurance;
        if (modendu > endutick)
            modendu = endutick;
        target.Endurance += modendu;
        var modheal = target.MaxHealth - target.Health;
        if (modheal > healthtick)
            modheal = healthtick;
        target.Health += modheal;
        var modmana = target.MaxMana - target.Mana;
        if (modmana > manatick)
            modmana = manatick;
        target.Mana += modmana;
    }
