...
public class StarsProc : SpellHandler
{
    ...
    protected class BoltOnTargetAction : RegionECSAction
    {
        ...
        public BoltOnTargetAction(GameLiving actionSource, GameLiving boltTarget, StarsProc spellHandler) : base(
            actionSource)
        {
            if (boltTarget == null)
        ...
        protected override int OnTick(ECSGameTimer timer)
        {
            ...
            if (target == null) return 0;
            if (!target.IsAlive || target.ObjectState != GameObject.eObjectState.Active) return 0;

            var ad = m_handler.CalculateDamageToTarget(target, 1);
            ...
            if (m_handler.Spell.SubSpellID != 0 && SkillBase.GetSpellByID(m_handler.Spell.SubSpellID) != null)
            {
                var spellhandler = ScriptMgr.CreateSpellHandler(caster,
                    SkillBase.GetSpellByID(m_handler.Spell.SubSpellID), SkillBase.GetSpellLine("Mob Spells"));
