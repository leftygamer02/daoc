...
public class Tartaros : LifedrainSpellHandler
{
    ...
    public override void StealLife(AttackData ad)
    {
        ...
        var heal = (ad.Damage + ad.CriticalDamage) * 35 / 100;
        var mana = (ad.Damage + ad.CriticalDamage) * 21 / 100;
        var endu = (ad.Damage + ad.CriticalDamage) * 14 / 100;
        ...
        if (heal > 0)
            MessageToCaster("You drain " + endu + " endurance point" + (endu == 1 ? "." : "s."),
                eChatType.CT_Spell);
        else
