...
public class SnakeCharmer : LifedrainSpellHandler
{
    ...
    public override void StealLife(AttackData ad)
    {
        ...
        var heal = (ad.Damage + ad.CriticalDamage) * 50 / 100;
        var mana = (ad.Damage + ad.CriticalDamage) * 30 / 100;
        var endu = (ad.Damage + ad.CriticalDamage) * 20 / 100;
        ...
        if (heal > 0)
            MessageToCaster("You steal " + endu + " endurance point" + (endu == 1 ? "." : "s."),
                eChatType.CT_Spell);
        else
