...
public class BeltOfSun : SummonItemSpellHandler
{
        ...
        : base(caster, spell, line)
    {
        ...
        {
            MessageToCaster("The powers of the Belt of Sun, can only be Summon under the Sun light!",
        ...
        if (player.CharacterClass.ID == (int) eCharacterClass.Berserker)
        {
            ...
            m_SunTwoHanded =
                GameServer.Database.FindObjectByKey<ItemTemplate>("Sun_TwoHanded") ?? TwoHandedM; // 2handed Sword
            items.Add(GameInventoryItem.Create(m_SunTwoHanded));
        ...
        if (player.CharacterClass.ID == (int) eCharacterClass.Champion)
        {
            ...
            m_SunStaff =
                GameServer.Database.FindObjectByKey<ItemTemplate>("Sun_Slash") ?? TwoHandedH; // LargeWeapon
            items.Add(GameInventoryItem.Create(m_SunTwoHanded));
            return;
        }
        ...
        {
            player.Out.SendMessage("" + player.CharacterClass.Name + "'s cant Summon Light!", eChatType.CT_System,
        ...
        lock (player.Inventory)
        {
            ...
            foreach (var invItem in items)
                if (player.CurrentRegion.IsNightTime)
                {
                    ...
                    player.Out.SendMessage("The Power of Belt of Sun, has left you!", eChatType.CT_System,
                        eChatLoc.CL_SystemWindow);
                }
        }
