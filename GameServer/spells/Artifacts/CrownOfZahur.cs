...
public class EfficientHealing : SpellHandler
{
    ...
    public override IList<string> DelveInfo
    {
        get
        {
            ...
            //Cast
            list.Add(LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE, "DelveInfo.CastingTime",
...
public class Powershield : SpellHandler
{
    public void AttackedEvent(DOLEvent e, object sender, EventArgs arguments)
    {
        ...
        var HalfHP = player.MaxHealth / 2;
        var ManaHeal = player.MaxMana * 0.25;
        ...
        if (player.Health <= HalfHP)
        {
            if (player.Mana < ManaHeal)
            {
                ...
                player.Out.SendMessage("You convert " + player.Mana + " power into hit points!", eChatType.CT_Spell,
            ...
            player.Mana -= (int) ManaHeal;
            player.Out.SendMessage("You convert " + ManaHeal + " power into hit points!", eChatType.CT_Spell,
                eChatLoc.CL_ChatWindow);
            return;
        }
    }
    ...
    {
        GameEventMgr.AddHandler(m_caster, GameLivingEvent.AttackedByEnemy, new DOLEventHandler(AttackedEvent));
    }

    public override int OnEffectExpires(GameSpellEffect effect, bool noMessages)
    {
        GameEventMgr.RemoveHandler(m_caster, GameLivingEvent.AttackedByEnemy, new DOLEventHandler(AttackedEvent));
        return 0;
    }
