...
public class DoTSpellHandler : SpellHandler
{
    ...
    {
        return Spell.SpellType == compare.SpellHandler.Spell.SpellType &&
               Spell.DamageType == compare.SpellHandler.Spell.DamageType &&
               SpellLine.IsBaseLine == compare.SpellHandler.SpellLine.IsBaseLine;
    }
    ...
    public override AttackData CalculateDamageToTarget(GameLiving target, double effectiveness)
    {
        ...
        if (SpellLine.KeyName == GlobalSpellsLines.Mundane_Poisons)
        {
            ...
            if (ra != null)
            {
                var additional = (int) ((float) ad.Damage * ((float) ra.Amount / 100));
    ...
    public override void CalculateDamageVariance(GameLiving target, out double min, out double max)
    {
        ...
        if (m_caster is GamePlayer)
        {
            if (m_spellLine.KeyName == GlobalSpellsLines.Mundane_Poisons)
            {
                ...
                if (target.Level > 0) min = 0.25 + (speclevel - 1) / (double) target.Level;
            }
    ...
    public override void SendDamageMessages(AttackData ad)
    {
        ...
        if (Spell.Name.StartsWith("Proc"))
            MessageToCaster(string.Format(LanguageMgr.GetTranslation(PlayerReceivingMessages.Client,
                "DoTSpellHandler.SendDamageMessages.YouHitFor",
                ad.Target.GetName(0, false), ad.Damage)), eChatType.CT_YouHit);
        else
            MessageToCaster(string.Format(LanguageMgr.GetTranslation(PlayerReceivingMessages.Client,
                "DoTSpellHandler.SendDamageMessages.YourHitsFor",
        ...
        if (CriticalDamage > 0)
            MessageToCaster(
                "You critically hit for an additional " + CriticalDamage + " damage!" + " (" +
                m_caster.DotCriticalChance + "%)", eChatType.CT_YouHit);
    ...
    public override void OnEffectPulse(GameSpellEffect effect)
    {
        ...
        if (effect.Owner.IsAlive)
        {
            ...
            Message.SystemToArea(effect.Owner, Util.MakeSentence(Spell.Message2, effect.Owner.GetName(0, false)),
                eChatType.CT_YouHit, effect.Owner);
    ...
    public override double CalculateDamageBase(GameLiving target)
    {
            ...
            m_spellLine.KeyName != GlobalSpellsLines.Item_Spells)
        {
            ...
            {
                var manaStatValue = m_caster.GetModified(eProperty.Intelligence);
                spellDamage *= (manaStatValue + 200) / 275.0;
                if (spellDamage < 0)
                    spellDamage = 0;
            }
    ...
    private int CalculateCriticalDamage(AttackData ad)
    {
        ...
        var criticalChance = Caster.DotCriticalChance;

        if (criticalChance < 0)
            return 0;

        var randNum = Util.CryptoNextInt(1, 100);
        var critCap = Math.Min(50, criticalChance);

        if (Caster is GamePlayer spellCaster && spellCaster.UseDetailedCombatLog && critCap > 0)
            spellCaster.Out.SendMessage($"dot crit chance: {critCap} random: {randNum}", eChatType.CT_DamageAdd,
                eChatLoc.CL_SystemWindow);

        if (critCap > randNum && ad.Damage >= 1)
        {
            var critmax = ad.Target is GamePlayer ? ad.Damage / 2 : ad.Damage;
