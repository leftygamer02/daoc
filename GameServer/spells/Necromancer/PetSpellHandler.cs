...
internal class PetSpellHandler : SpellHandler
{
    ...
    public override bool CastSpell()
    {
        ...
        if (GameServer.ServerRules.IsAllowedToCastSpell(Caster, m_spellTarget, Spell, SpellLine) &&
            CheckBeginCast(m_spellTarget))
        {
            if (Spell.CastTime > 0)
        ...
        if (Caster.ControlledBrain == null)
        {
            MessageToCaster(
                LanguageMgr.GetTranslation((Caster as GamePlayer).Client,
                    "PetSpellHandler.CheckBeginCast.NoControlledBrainForCast"), eChatType.CT_SpellResisted);
            return false;
        }
    ...
    public override void FinishSpellCast(GameLiving target)
    {
        ...
        var powerCost = PowerCost(player);

        if (powerCost > 0)
            player.ChangeMana(player, eManaChangeType.Spell, -powerCost);

        var petBrain = player.ControlledBrain as ControlledNpcBrain;
        ...
        if (Spell.RecastDelay > 0 && m_startReuseTimer)
            foreach (var spell in SkillBase.GetSpellList(SpellLine.KeyName))
