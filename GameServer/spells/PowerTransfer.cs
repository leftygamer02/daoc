...
internal class PowerTransfer : SpellHandler
{
    ...
    public override void OnDirectEffect(GameLiving target, double effectiveness)
    {
        ...
        var powerTransfer = (int) Math.Min(Spell.Value, owner.Mana);
        var powerDrained = -owner.ChangeMana(owner, eManaChangeType.Spell, -powerTransfer);

        if (powerDrained <= 0)
            return;

        var powerHealed = target.ChangeMana(owner, eManaChangeType.Spell, powerDrained);
