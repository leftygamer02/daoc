    ...
    public override void OnDirectEffect(GameLiving target, double effectiveness)
    {
        ...
        if (target is GamePlayer)
            MessageToLiving(target,
                LanguageMgr.GetTranslation((target as GamePlayer).Client, "Amnesia.MessageToTarget"),
                eChatType.CT_Spell);
        ...
        if (target is GameNPC)
        {
            ...
            if (aggroBrain != null)
                if (Util.Chance(Spell.AmnesiaChance) && npc.TargetObject != null &&
                    npc.TargetObject is GameLiving living)
                {
