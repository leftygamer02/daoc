...
public class ValkyrieOffensiveProcSpellHandler : SpellHandler
{
    ...
    public void EventHandler(DOLEvent e, object sender, EventArgs arguments)
    {
        ...
        if (Util.Chance(15))
        {
            var m_procSpell = SkillBase.GetSpellByID((int) Spell.Value);
            var handler = ScriptMgr.CreateSpellHandler((GameLiving) sender, m_procSpell,
    ...
    // constructor
    public ValkyrieOffensiveProcSpellHandler(GameLiving caster, Spell spell, SpellLine line) : base(caster, spell,
        line)
    {
    }
}