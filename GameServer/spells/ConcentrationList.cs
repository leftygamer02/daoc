...
public class ConcentrationList : IEnumerable<IConcentrationEffect>
{
    ...
    public void Add(IConcentrationEffect effect)
    {
        ...
        lock (m_lockObject) // Mannen 10:56 PM 10/30/2006 - Fixing every lock(this)
        {
            ...
            else
            {
                ...
                {
                    var lastEffect = (IConcentrationEffect) m_concSpells[m_concSpells.Count - 1];
    ...
    public void CancelAll(bool leaveself, bool updatePlayer)
    {
        if (m_concSpells != null)
        {
            ...
            BeginChanges();
            foreach (var fx in concEffect.Where(eff =>
                         !leaveself || (leaveself && eff.OwnerName != m_owner.Name)))
    ...
    public void CommitChanges()
    {
        ...
        if (player != null && m_changeCounter >= 0) player.Out.SendConcentrationList();
    }
    ...
    /// </summary>
    public short UsedConcentration => m_usedConcPoints;

    /// <summary>
    /// Gets the count of conc spells in the list
    /// </summary>
    public byte ConcSpellsCount => m_concSpellsCount;
