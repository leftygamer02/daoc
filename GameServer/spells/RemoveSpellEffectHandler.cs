...
public abstract class RemoveSpellEffectHandler : SpellHandler
{
    ...
    /// RR4: now its a list of effects to remove
    /// </summary>
    public virtual List<string> SpellTypesToRemove => m_spellTypesToRemove;
    ...
    public override void OnDirectEffect(GameLiving target, double effectiveness)
    {
        ...
        foreach (var toRemove in SpellTypesToRemove)
        {
            //GameSpellEffect effect = target.FindEffectOnTarget(toRemove);
            Enum.TryParse(toRemove, out eEffect effectType);
            var effect = EffectListService.GetEffectOnTarget(target, effectType);
