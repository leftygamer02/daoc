...
public abstract class BaseProcSpellHandler : SpellHandler
{
    ...
    {
        var oldProcSpell = SkillBase.GetSpellByID((int) oldeffect.Spell.Value);
        var newProcSpell = SkillBase.GetSpellByID((int) neweffect.Spell.Value);
        ...
        if (oldeffect is GameSpellAndImmunityEffect == false ||
            ((GameSpellAndImmunityEffect) oldeffect).ImmunityState == false)
    ...
    public override bool IsOverwritable(GameSpellEffect compare)
    {
        ...
        if (compare.Spell.SpellType != Spell.SpellType)
            return false;
        var oldProcSpell = SkillBase.GetSpellByID((int) Spell.Value);
        var newProcSpell = SkillBase.GetSpellByID((int) compare.Spell.Value);
        if (oldProcSpell == null || newProcSpell == null)
            return true;
...
//                list.Add("Function: " + (string)(Spell.SpellType == "" ? "(not implemented)" : Spell.SpellType));
            list.Add(LanguageMgr.GetTranslation((Caster as GamePlayer).Client,
                "ProcSpellHandler.DelveInfo.Function",
                (string) (Spell.SpellType.ToString() == "" ? "(not implemented)" : Spell.SpellType.ToString())));
...
//                else if (Spell.Duration > 60000) list.Add(string.Format("Duration: {0}:{1} min", Spell.Duration / 60000, (Spell.Duration % 60000 / 1000).ToString("00")));
            ...
            // Recursion check
            var nextDelveDepth = (byte) (DelveInfoDepth + 1);
            if (nextDelveDepth > MAX_DELVE_RECURSION)
            {
                list.Add("(recursion - see server logs)");
                log.ErrorFormat("Spell delve info recursion limit reached. Source spell ID: {0}, Sub-spell ID: {1}",
                    m_spell.ID, m_procSpell.ID);
            }
            else
            {
                ...
                list.Add(" "); //empty line
                var subSpellHandler = ScriptMgr.CreateSpellHandler(Caster, m_procSpell, m_procSpellLine);
                ...
                var subSpellDelve = subSpellHandler.DelveInfo;
...
public class OffensiveProcSpellHandler : BaseProcSpellHandler
{
    ...
    public void EventHandler(AttackData ad)
    {
        ...
        {
            var handler =
                ScriptMgr.CreateSpellHandler((GameLiving) ad.Attacker, m_procSpell, m_procSpellLine);
...
public class DefensiveProcSpellHandler : BaseProcSpellHandler
{
    ...
    public void EventHandler(AttackData ad)
    {
        ...
        {
            var handler =
                ScriptMgr.CreateSpellHandler((GameLiving) ad.Target, m_procSpell, m_procSpellLine);
            if (handler != null)
                switch (m_procSpell.Target.ToLower())
                {
                    case "enemy":
...
public class OffensiveProcPvESpellHandler : OffensiveProcSpellHandler
{
    ...
    protected override void EventHandler(DOLEvent e, object sender, EventArgs arguments)
    {
        ...
        var target = args.AttackData.Target as GameNPC;

        if (target != null && !(target.Brain is IControlledBrain &&
                                ((IControlledBrain) target.Brain).GetPlayerOwner() != null))
