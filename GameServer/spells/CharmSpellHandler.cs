...
public class CharmSpellHandler : SpellHandler
{
    ...
    public override bool CheckEndCast(GameLiving selectedTarget)
    {
        ...
        if (charmMob == null)
        {
            ...
            MessageToCaster(
                LanguageMgr.GetTranslation(casterPlayer.Client, "GamePlayer.Target.Spell.SelectATarget"),
        ...
        if (Caster is GamePlayer && casterPlayer.ControlledBrain != null &&
            casterPlayer.ControlledBrain != charmMob.Brain)
        {
            // Message: You already have a charmed creature, release it first!
            MessageToCaster(
                LanguageMgr.GetTranslation(casterPlayer.Client, "CharmSpell.EndCast.Fail.AlreadyOwnCharmed"),
                eChatType.CT_SpellResisted);
            return false;
        }
        ...
        if (m_controlledBrain == null && Caster.ControlledBrain == null)
        {
            ...
            if (charmMob.Brain != null && charmMob.Brain is IControlledBrain &&
                ((IControlledBrain) charmMob.Brain).Owner as GamePlayer != Caster)
            {
                // Message: {0} is currently being controlled.
                MessageToCaster(
                    LanguageMgr.GetTranslation(casterPlayer.Client, "CharmSpell.EndCast.Fail.CurrentlyControlled",
                        charmMob.GetName(0, true)), eChatType.CT_SpellResisted);
                return false;
            }
            ...
            if (!charmMob.IsAlive)
            {
                // Message: {0} is dead!
                MessageToCaster(
                    LanguageMgr.GetTranslation(casterPlayer.Client, "GamePlayer.Target.Fail.IsDead",
                        charmMob.GetName(0, true)), eChatType.CT_SpellResisted);
                return false;
            }

            // Make sure the pet is in the same zone
            if (charmMob.CurrentRegion != Caster.CurrentRegion) return false;
            ...
            if (charmMob.Realm != 0)
            {
                ...
                MessageToCaster(
                    LanguageMgr.GetTranslation(casterPlayer.Client, "CharmSpell.EndCast.Fail.CantBeCharmed",
                        charmMob.GetName(0, true)), eChatType.CT_SpellResisted);
                return false;
            }
            ...
            if (charmMob.BodyType is > 0 and < 12)
            {
                ...
                if (charmMob is GameStableMaster) // Horse route NPCs
                    isCharmable = false;
                if (charmMob is AlchemistsMaster or FletchingMaster or ArmorCraftingMaster or WeaponCraftingMaster
                    or SpellCraftingMaster or TailoringMaster or BasicCraftingMaster
                    or SiegecraftingMaster) // Any Craft Master NPCs
                    isCharmable = false;
            ...
            if (m_spell.AmnesiaChance is > (ushort) eCharmType.All and <= (ushort) eCharmType.Reptile)
            {
                ...
                if (!isCharmable)
                {
                    ...
                    MessageToCaster(
                        LanguageMgr.GetTranslation(casterPlayer.Client, "CharmSpell.EndCast.Fail.WrongType"),
                        eChatType.CT_SpellResisted);
                    return false;
                }
            }
            ...
            if (casterPlayer.CharacterClass.ID is (int) eCharacterClass.Minstrel or (int) eCharacterClass.Mentalist)
            {
                ...
                if (charmMob.Level > Spell.Value)
                {
                    ...
                    MessageToCaster(
                        LanguageMgr.GetTranslation(casterPlayer.Client, "CharmSpell.EndCast.Fail.TooStrong",
                            charmMob.GetName(0, true)), eChatType.CT_SpellResisted);
                    return false;
                }
            ...
            if (casterPlayer.CharacterClass.ID is (int) eCharacterClass.Hunter or (int) eCharacterClass.Sorcerer)
            {
                ...
                if (charmMob.InCombat)
                {
                    ...
                    MessageToCaster(
                        LanguageMgr.GetTranslation(casterPlayer.Client, "SpellEffect.Charm.Err.InCombat",
                            selectedTarget.GetName(0, false), selectedTarget.GetPronoun(1, false)),
                        eChatType.CT_SpellResisted);
                    return false;
                }
            }
    ...
    public override void ApplyEffectOnTarget(GameLiving target, double effectiveness)
    {
        ...
        if (Spell.Pulse != 0 && Caster is GamePlayer && ((GamePlayer) Caster).ControlledBrain != null &&
            ((GamePlayer) Caster).ControlledBrain.Body !=
            (GameNPC) target)
        ...
        if (Caster is GamePlayer charmCaster)
        {
            ...
            if (Spell.Pulse != 0) // not permanent
            {
                ...
                var sweetSpot = Caster.Level * 0.66 + Caster.GetModifiedSpecLevel(m_spellLine.Spec) * 0.33 -
                                target.Level;
            ...
            var spellResistChance = resistChance;
            ...
            {
                var song = EffectListService.GetPulseEffectOnTarget(Caster as GamePlayer);
                if (song != null && song.SpellHandler.Spell.InstrumentRequirement == 0 &&
                    song.SpellHandler.Spell.CastTime == 0)
            ...
            if (target.IsWithinRadius(charmCaster, 2000))
            {
                if (charmCaster.Client.Account.PrivLevel > 1)
                    MessageToCaster("Resist Chance=" + resistString + "; Roll=" + rollString,
                        eChatType.CT_SpellResisted);
                ...
                if (resistResult <= spellResistChance)
                {
                    ...
                    MessageToCaster(
                        LanguageMgr.GetTranslation(charmCaster.Client, "GamePlayer.StartCharm.Fail.Resist",
                            target.GetName(0, true), resistString), eChatType.CT_SpellResisted);
            ...
            if (!target.IsWithinRadius(Caster, 2000))
            {
                ...
                MessageToCaster(
                    LanguageMgr.GetTranslation(charmCaster.Client, "GamePlayer.GamePet.Movement.TooFarAway"),
                    eChatType.CT_SpellResisted);
                return;
            }
        }
    ...
    public void ReleaseEventHandler(DOLEvent e, object sender, EventArgs arguments)
    {
        ...
        var charm = new List<ECSGameEffect>();
    ...
    public override bool IsNewEffectBetter(GameSpellEffect oldeffect, GameSpellEffect neweffect)
    {
        if (oldeffect.Spell.SpellType != neweffect.Spell.SpellType)
        {
            if (log.IsWarnEnabled)
                log.Warn("Spell effect compare with different types " + oldeffect.Spell.SpellType + " <=> " +
                         neweffect.Spell.SpellType + "\n" + Environment.StackTrace);
    ...
    public override IList<string> DelveInfo
    {
        get
        {
            ...
            list.Add(LanguageMgr.GetTranslation(((GamePlayer) Caster).Client,
                "CharmSpellHandler.DelveInfo.Function",
                Spell.SpellType.ToString() == "" ? "(not implemented)" : Spell.SpellType.ToString()));
            list.Add(" "); //empty line
            ...
            list.Add(LanguageMgr.GetTranslation(((GamePlayer) Caster).Client,
                "CharmSpell.DelveInfo.Desc.NonNamedEpic"));
            list.Add(" "); //empty line
            if (Spell.InstrumentRequirement != 0)
                list.Add(LanguageMgr.GetTranslation(((GamePlayer) Caster).Client, "DelveInfo.InstrumentRequire",
            ...
            if (Spell.Duration >= ushort.MaxValue * 1000)
                list.Add(LanguageMgr.GetTranslation(((GamePlayer) Caster).Client, "DelveInfo.Duration") +
                         " Permanent.");
            else if (Spell.Duration > 60000)
                list.Add(string.Format(
                    LanguageMgr.GetTranslation(((GamePlayer) Caster).Client, "DelveInfo.Duration") +
            ...
            if (Spell.Frequency != 0)
                list.Add(LanguageMgr.GetTranslation(((GamePlayer) Caster).Client, "DelveInfo.Frequency",
                    (Spell.Frequency * 0.001).ToString("0.0")));
            if (Spell.Power != 0)
                list.Add(LanguageMgr.GetTranslation(((GamePlayer) Caster).Client, "DelveInfo.PowerCost",
                    Spell.Power.ToString("0;0'%'")));
            list.Add(LanguageMgr.GetTranslation(((GamePlayer) Caster).Client, "DelveInfo.CastingTime",
                (Spell.CastTime * 0.001).ToString("0.0## sec;-0.0## sec;'instant'")));
            if (Spell.RecastDelay > 60000)
                list.Add(LanguageMgr.GetTranslation(((GamePlayer) Caster).Client, "DelveInfo.RecastTime") +
                         Spell.RecastDelay / 60000 + ":" + (Spell.RecastDelay % 60000 / 1000).ToString("00") +
                         " min");
            else if (Spell.RecastDelay > 0)
                list.Add(LanguageMgr.GetTranslation(((GamePlayer) Caster).Client, "DelveInfo.RecastTime") +
                         (Spell.RecastDelay / 1000).ToString() + " sec");
            if (Spell.Concentration != 0)
                list.Add(LanguageMgr.GetTranslation(((GamePlayer) Caster).Client, "DelveInfo.ConcentrationCost",
                    Spell.Concentration));
            if (Spell.Radius != 0)
                list.Add(LanguageMgr.GetTranslation(((GamePlayer) Caster).Client, "DelveInfo.Radius",
                    Spell.Radius));
            if (Spell.DamageType != eDamageType.Natural)
                list.Add(LanguageMgr.GetTranslation(((GamePlayer) Caster).Client, "DelveInfo.Damage",
