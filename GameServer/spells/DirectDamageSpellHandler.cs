...
public class DirectDamageSpellHandler : SpellHandler
{
    ...
    {
        if (!m_castFailed) m_caster.Mana -= PowerCost(target);
    ...
    public override void OnDirectEffect(GameLiving target, double effectiveness)
    {
        ...
        if (Spell.Target.ToLower() == "cone" || (Spell.Target == "Enemy" && Spell.IsPBAoE))
        {
            ...
            else
            {
                ...
                {
                    var brain = (Caster as GameNPC).Brain as IControlledBrain;
    ...
    protected virtual void DealDamageCheckLOS(GamePlayer player, ushort response, ushort targetOID)
    {
        ...
        if ((response & 0x100) == 0x100)
        {
            try
            {
                ...
                {
                    var effectiveness =
                        player.TempProperties.getProperty<double>(LOSEFFECTIVENESS + target.ObjectID, 1.0);
                    DealDamage(target, effectiveness);
