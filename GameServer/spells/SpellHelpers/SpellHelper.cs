...
public static class SpellHelper
{
    ...
    {
        return target.EffectList.OfType<GameSpellEffect>().Where(fx =>
            !(fx is GameSpellAndImmunityEffect && ((GameSpellAndImmunityEffect) fx).ImmunityState)
    ...
    {
        return target.effectListComponent.ConcentrationEffects.OfType<PulsingSpellEffect>().Where(pfx =>
            pfx.SpellHandler != null && pfx.SpellHandler.Spell != null
    ...
    private static IEnumerable<PulsingSpellEffect> PulsingSpellsOnTarget(this GameLiving target,
        ISpellHandler handler)
    {
        return target.effectListComponent.ConcentrationEffects.OfType<PulsingSpellEffect>()
            .Where(pfx => pfx.SpellHandler == handler);
    }
