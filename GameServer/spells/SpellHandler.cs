...
public class SpellHandler : ISpellHandler
{
    ...
    protected bool CheckInstrument()
    {
        ...
        if (instrument == null ||
            instrument.Object_Type !=
            (int) eObjectType
                .Instrument) // || (instrument.DPS_AF != 4 && instrument.DPS_AF != m_spell.InstrumentRequirement))
            return false;
    ...
    {
        var effects =
            living.effectListComponent
                .GetAllPulseEffects(); //.ConcentrationEffects.Where(e => e is ECSPulseEffect).ToArray();
        for (var i = 0; i < effects.Count(); i++) EffectService.RequestImmediateCancelConcEffect(effects[i]);
    }
    /// <summary>
...
// 		}
    ...
    {
        var freq = Spell != null ? Spell.Frequency : 0;
        // return new GameSpellEffect(this, CalculateEffectDuration(target, effectiveness), freq, effectiveness);

        new ECSPulseEffect(target, this, CalculateEffectDuration(target, effectiveness), freq, effectiveness,
            Spell.Icon);
    }
    ...
    public virtual bool CastSpell(GameLiving targetObject)
    {
        ...
        {
            var effect =
                (EngageECSGameEffect) EffectListService.GetEffectOnTarget(Caster, eEffect.Engage);
        ...
        else if (GameServer.ServerRules.IsAllowedToCastSpell(Caster, m_spellTarget, Spell, m_spellLine))
        {
            if (CheckBeginCast(m_spellTarget))
            {
                ...
                if (!Spell.IsConcentration && Caster.TargetObject == m_spellTarget &&
                    Caster.TargetObject as GamePlayer != null
                    && (Caster.TargetObject as GamePlayer).IsShade &&
                    Spell.SpellType != (byte) eSpellType.UniPortal)
                ...
                else
                {
                    if (Caster.ControlledBrain == null || Caster.ControlledBrain.Body == null ||
                        !(Caster.ControlledBrain.Body is NecromancerPet))
                        SendCastAnimation(0);
    ...
    public virtual void StartCastTimer(GameLiving target)
    {
        ...
        var time = CalculateCastingTime();

        var step1 = time / 3;
        if (step1 > Properties.SPELL_INTERRUPT_MAXSTAGELENGTH)
            step1 = Properties.SPELL_INTERRUPT_MAXSTAGELENGTH;
        if (step1 < 1)
            step1 = 1;

        var step3 = time / 3;
        if (step3 > Properties.SPELL_INTERRUPT_MAXSTAGELENGTH)
            step3 = Properties.SPELL_INTERRUPT_MAXSTAGELENGTH;
        if (step3 < 1)
            step3 = 1;

        var step2 = time - step1 - step3;
        if (step2 < 1)
            step2 = 1;

        if (Caster is GamePlayer && Properties.ENABLE_DEBUG)
            (Caster as GamePlayer).Out.SendMessage(
                "[DEBUG] spell time = " + time + ", step1 = " + step1 + ", step2 = " + step2 + ", step3 = " + step3,
        ...
        if (m_caster.IsMoving || m_caster.IsStrafing) CasterMoves();
    }
    ...
    {
        if (Spell.SpellType != (byte) eSpellType.PveResurrectionIllness &&
            Spell.SpellType != (byte) eSpellType.RvrResurrectionIllness)
        {
            if (Spell.InstrumentRequirement == 0)
            {
                if (Caster is GamePlayer playerCaster)
                    ...
                    MessageToCaster(
                        LanguageMgr.GetTranslation(playerCaster.Client,
                            "SpellHandler.CastSpell.Msg.YouBeginCasting", Spell.Name), eChatType.CT_Spell);
                if (Caster is NecromancerPet {Owner: GamePlayer casterOwner})
                    // Message: {0} begins casting a {1} spell!
                    casterOwner.Out.SendMessage(
                        LanguageMgr.GetTranslation(casterOwner.Client.Account.Language,
                            "SpellHandler.CastSpell.Msg.PetBeginsCasting", Caster.GetName(0, true), Spell.Name),
            ...
            {
                MessageToCaster(
                    LanguageMgr.GetTranslation(songCaster.Client, "SpellHandler.CastSong.Msg.YouBeginPlaying",
                        Spell.Name), eChatType.CT_Spell);
            }
        }
    }
    ...
    public virtual bool CasterIsAttacked(GameLiving attacker)
    {
        ...
        if (IsCasting && GameLoop.GameLoopTime < _castStartTick + _calculatedCastTime * 0.5)
        {
            ...
            {
                casterOwner.LastInterruptMessage =
                    $"Your {Caster.Name} was attacked by {attacker.Name} and their spell was interrupted!";
            ...
            {
                playerCaster.LastInterruptMessage =
                    $"{attacker.GetName(0, true)} attacks you and your spell is interrupted!";
    ...
    public virtual bool CheckBeginCast(GameLiving selectedTarget, bool quiet)
    {
        ...
        {
            if (m_caster.LastPulseCast.Equals(Spell) && m_spell.IsPulsing && m_spell.Pulse != 0 &&
                m_spell.Frequency > 0)
            {
                if (Spell.InstrumentRequirement == 0)
                    MessageToCaster("You cancel your effect.", eChatType.CT_Spell);
                ...
                var cancelEffect = Caster.effectListComponent.GetSpellEffects(eEffect.Pulse)
                    .Where(effect => effect.SpellHandler.Spell.Equals(Spell)).FirstOrDefault();
        ...
        {
            var nextSpellAvailTime =
                m_caster.TempProperties.getProperty<long>(GamePlayer.NEXT_SPELL_AVAIL_TIME_BECAUSE_USE_POTION);

            if (nextSpellAvailTime > m_caster.CurrentRegion.Time &&
                Spell.CastTime > 0) // instant spells ignore the potion cast delay
            {
                playerCaster.Out.SendMessage(
                    LanguageMgr.GetTranslation(playerCaster.Client, "GamePlayer.CastSpell.MustWaitBeforeCast",
                        (nextSpellAvailTime - m_caster.CurrentRegion.Time) / 1000), eChatType.CT_System,
        ...
        if (Spell.Range > 0)
        {
            var SelectiveBlindness = Caster.EffectList.GetOfType<SelectiveBlindnessEffect>();
            if (SelectiveBlindness != null)
            {
                var EffectOwner = SelectiveBlindness.EffectSource;
                if (EffectOwner == selectedTarget)
                {
                    if (m_caster is GamePlayer && !quiet)
                        ((GamePlayer) m_caster).Out.SendMessage(
                            string.Format("{0} is invisible to you!", selectedTarget.GetName(0, true)),
        ...
        if (selectedTarget != null && selectedTarget.HasAbility("DamageImmunity") &&
            Spell.SpellType == (byte) eSpellType.DirectDamage && Spell.Radius == 0)
        {
            if (!quiet)
        ...
        if (m_caster.attackComponent.AttackState && m_spell.CastTime != 0)
            if (m_caster.CanCastInCombat(Spell) == false)
            {
                if (m_caster is not GamePet)
                    m_caster.attackComponent
                        .StopAttack(); //dont stop melee for pet (probaby look at stopping attack just for game player)
                return false;
            }
        ...
        if (m_caster is GamePlayer)
        {
            if (!m_spell.Uninterruptible && m_spell.CastTime > 0
                                         && !m_caster.effectListComponent.ContainsEffectForEffectType(
                                             eEffect.QuickCast)
                                         && !m_caster.effectListComponent.ContainsEffectForEffectType(
                                             eEffect.MasteryOfConcentration))
                if (Caster.InterruptAction > 0 && Caster.InterruptTime > GameLoop.GameLoopTime)
                {
                    if (!quiet)
                        MessageToCaster(
                            "You must wait " +
                            ((Caster.InterruptTime - GameLoop.GameLoopTime) / 1000 + 1).ToString() +
                            " seconds to cast a spell!", eChatType.CT_SpellResisted);
                    return false;
                }
        }
        ...
        {
            var left = m_caster.GetSkillDisabledDuration(m_spell);
            if (left > 0)
            {
                if (m_caster is NecromancerPet &&
                    ((m_caster as NecromancerPet).Owner as GamePlayer).Client.Account.PrivLevel >
                ...
                else
                {
                    if (!quiet)
                        MessageToCaster(
                            "You must wait " + (left / 1000 + 1).ToString() + " seconds to use this spell!",
                            eChatType.CT_System);
                    return false;
                }
            }
        ...
        var targetType = m_spell.Target.ToLower();

        if (targetType == "pet")
            if (selectedTarget == null || !Caster.IsControlledNPC(selectedTarget as GameNPC) ||
                (selectedTarget != null && selectedTarget != Caster.ControlledBrain?.Body))
            {
                if (!quiet)
        ...
        if (targetType == "area")
        {
            ...
            {
                if (!quiet)
                    MessageToCaster("Your area target is out of range. Select a closer target.",
                        eChatType.CT_SpellResisted);
                return false;
            }
        ...
        if (Spell.Power != 0 && m_caster is GamePlayer &&
            (m_caster as GamePlayer).CharacterClass.ID != (int) eCharacterClass.Savage &&
            m_caster.Mana < PowerCost(selectedTarget) &&
            EffectListService.GetAbilityEffectOnTarget(Caster, eEffect.QuickCast) == null &&
            Spell.SpellType != (byte) eSpellType.Archery)
        {
            if (!quiet)
        ...
        if (m_caster is GamePlayer && m_spell.Concentration > 0)
        {
            if (m_caster.Concentration < m_spell.Concentration)
            {
                if (!quiet)
                    MessageToCaster(
                        "This spell requires " + m_spell.Concentration + " concentration points to cast!",
                        eChatType.CT_SpellResisted);
                return false;
            }
            ...
            if (m_caster is GamePlayer p)
                maxConc += p.effectListComponent.ConcentrationEffects.Count(concentrationEffect =>
                    concentrationEffect.SpellHandler?.Spell?.ID != null
            ...
            {
                if (!quiet)
                    MessageToCaster($"You can only cast up to {MAX_CONC_SPELLS} simultaneous concentration spells!",
                        eChatType.CT_SpellResisted);
                return false;
            }
        }
        ...
        {
            var engage =
                (EngageECSGameEffect) EffectListService.GetEffectOnTarget(m_caster, eEffect.Engage);
            if (engage != null)
                engage.Cancel(false);
        }
    ...
    private void CheckPlayerLosDuringCastCallback(GamePlayer player, ushort response, ushort sourceOID,
        ushort targetOID)
    {
    ...
    private void CheckPetLosDuringCastCallback(GameLiving living, ushort response, ushort sourceOID,
        ushort targetOID)
    {
    ...
    public virtual bool CheckEndCast(GameLiving target)
    {
        ...
        else if (m_spell.Target.ToLower() != "self" && m_spell.Target.ToLower() != "group" &&
                 m_spell.Target.ToLower() != "cone" && m_spell.Range > 0)
        {
            ...
            switch (m_spell.Target)
            {
                case "Enemy":
                    ...
                    if (!GameServer.ServerRules.IsAllowedToAttack(Caster, target, false)) return false;
        ...
        {
            MessageToCaster("You have exhausted all of your power and cannot cast spells!",
                eChatType.CT_SpellResisted);
            return false;
        }

        if (Spell.Power > 0 && m_caster.Mana < PowerCost(target) &&
            EffectListService.GetAbilityEffectOnTarget(Caster, eEffect.QuickCast) == null &&
            Spell.SpellType != (byte) eSpellType.Archery)
        {
            MessageToCaster("You don't have enough power to cast that!", eChatType.CT_SpellResisted);
        ...
        {
            MessageToCaster("This spell requires " + m_spell.Concentration + " concentration points to cast!",
                eChatType.CT_SpellResisted);
            return false;
        }

        if (m_caster is GamePlayer && m_spell.Concentration > 0 &&
            m_caster.effectListComponent.ConcentrationEffects.Count >= MAX_CONC_SPELLS)
        {
            MessageToCaster($"You can only cast up to {MAX_CONC_SPELLS} simultaneous concentration spells!",
                eChatType.CT_SpellResisted);
            return false;
    ...
    public virtual bool CheckDuringCast(GameLiving target, bool quiet)
    {
        ...
        if (m_caster.ObjectState != GameObject.eObjectState.Active) return false;
                 ...
                 m_spell.Target.ToLower() != "cone" && m_spell.Range > 0)
        {
            if (m_spell.Target.ToLower() != "pet")
            {
                ...
                if (Properties.CHECK_LOS_DURING_CAST && GameLoop.GameLoopTime >
                    _lastDuringCastLosCheckTime + Properties.CHECK_LOS_DURING_CAST_MINIMUM_INTERVAL)
                {
            ...
            switch (m_spell.Target.ToLower())
            {
                case "enemy":
                    ...
                    if (!GameServer.ServerRules.IsAllowedToAttack(Caster, target, quiet)) return false;
            ...
            if (Caster.InterruptTime > 0 && Caster.InterruptTime > m_started)
            {
                if (!quiet)
                {
                    if (Caster.LastInterruptMessage != "")
                        ...
                        MessageToCaster(
                            "You are interrupted and must wait " +
                            ((Caster.InterruptTime - m_started) / 1000 + 1).ToString() +
                            " seconds to cast a spell!", eChatType.CT_SpellResisted);
                }
                 ...
                 m_spell.Target.ToLower() != "cone" && m_spell.Range > 0)
        {
            ...
            switch (m_spell.Target)
            {
                case "Enemy":
                    ...
                    if (m_spell.SpellType != (byte) eSpellType.PetSpell && !m_caster.TargetInView &&
                        !Caster.IsWithinRadius(target, 50))
                    {
                        if (!quiet)
                            MessageToCaster("Your target is not in view. The spell fails.",
                                eChatType.CT_SpellResisted);
                        return false;
                    }
    ...
    public void Tick(long currentTick)
    {
        switch (CastState)
        {
            case eCastState.Precast:
                ...
                else if (Spell.Target == "Pet")
                {
                    ...
                    if (m_spellTarget == null || !Caster.IsControlledNPC(m_spellTarget as GameNPC) ||
                        (m_spellTarget != null && m_spellTarget != Caster.ControlledBrain?.Body))
                        if (Caster.ControlledBrain != null && Caster.ControlledBrain.Body != null)
                            m_spellTarget = Caster.ControlledBrain.Body;
                }
                else
        ...
        if (CastState == eCastState.Finished)
        {
            ...
            if (Spell.IsFocus)
            {
                ...
                else
                {
                    ...
                    var stone = Caster.Inventory.GetFirstItemByName("Personal Bind Recall Stone",
        ...
        if (CastState == eCastState.Cleanup) CleanupSpellCast();
    }
    ...
    public virtual int PowerCost(GameLiving target)
    {
        ...
        var ValhallasBlessing = m_caster.EffectList.GetOfType<ValhallasBlessingEffect>();
        if (ValhallasBlessing != null && Util.Chance(75))
            return 0;

        //patch 1.108 increases the chance to not use power to 50%.
        var FungalUnion = m_caster.EffectList.GetOfType<FungalUnionEffect>();
        {
        ...
        var syphon = Caster.GetModified(eProperty.ArcaneSyphon);
        if (syphon > 0)
        ...
        var power = basepower * 1.2; //<==NOW holding basepower*1.2 within 'power'

        var focusProp = SkillBase.SpecToFocus(SpellLine.Spec);
        if (focusProp != eProperty.Undefined)
        {
            var focusBonus = Caster.GetModified(focusProp) * 0.4;
            if (Spell.Level > 0)
            ...
            if (Caster is GamePlayer)
            {
                ...
                var specBonus = Math.Min(spec, 50) / (Spell.Level * 1.0);
                if (specBonus > 1)
                    specBonus = 1;
            ...
            power -= basepower *
                     focusBonus; //<== So i can finally use 'basepower' for both calculations: % and absolut
        }
    ...
    {
        var range = Math.Max(32, (int) (Spell.Range * Caster.GetModified(eProperty.SpellRange) * 0.01));
        return range;
    ...
    public virtual void InterruptCasting()
    {
        ...
        if (IsCasting)
            Parallel.ForEach(m_caster.GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE).Cast<GamePlayer>(),
                player => { player.Out.SendInterruptAnimation(m_caster); });
        ...
        if (m_castTimer != null)
        {
            ...
            if (m_caster is GamePlayer) ((GamePlayer) m_caster).ClearSpellQueue();
        }
    ...
    public virtual void AmnesiaInterruptCasting()
    {
        ...
        if (m_caster is GamePlayer p && p.castingComponent != null) p.castingComponent.spellHandler = null;
        //p.castingComponent.queuedSpellHandler = null;
        if (m_castTimer != null)
        {
            m_castTimer.Stop();
    ...
    protected class DelayedCastTimer : GameTimer
    {
        ...
        public DelayedCastTimer(GameLiving actionSource, SpellHandler handler, GameLiving target, int delay1,
            int delay2)
        ...
        protected override void OnTick()
        {
            try
            {
                ...
                else if (m_stage == 2)
                {
                    ...
                    if (m_handler.CheckEndCast(m_target)) m_handler.FinishSpellCast(m_target);
                }
                else
                {
                    m_stage = 4;
                ...
                if (m_caster is GamePlayer && Properties.ENABLE_DEBUG && m_stage < 3)
                    (m_caster as GamePlayer).Out.SendMessage("[DEBUG] step = " + (m_handler.Stage + 1),
                        eChatType.CT_System, eChatLoc.CL_SystemWindow);
    ...
    public virtual void SendEffectAnimation(GameObject target, ushort boltDuration, bool noSound, byte success)
    {
        ...
        //}
        Parallel.ForEach(target.GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE).Cast<GamePlayer>(),
            player =>
            {
                player.Out.SendSpellEffectAnimation(m_caster, target, m_spell.ClientEffect, boltDuration, noSound,
                    success);
            });
    }
    ...
    public virtual void SendEffectAnimation(GameObject target, ushort clientEffect, ushort boltDuration,
    ...
    {
        var playerCaster = Caster as GamePlayer;
        ...
        if (Spell.InstrumentRequirement == 0 && Spell.ClientEffect != 0)
                ...
                Spell.SpellType != (byte) eSpellType.RvrResurrectionIllness)
            {
                if (playerCaster != null)
                    ...
                    MessageToCaster(
                        LanguageMgr.GetTranslation(playerCaster.Client, "SpellHandler.CastSpell.Msg.YouCastSpell",
                            Spell.Name), eChatType.CT_Spell);
                if (Caster is NecromancerPet {Owner: GamePlayer casterOwner})
                    // Message: {0} cast a {1} spell!
                    casterOwner.Out.SendMessage(
                        LanguageMgr.GetTranslation(casterOwner.Client.Account.Language,
                            "SpellHandler.CastSpell.Msg.PetCastSpell", Caster.GetName(0, true), Spell.Name),
                        eChatType.CT_Spell, eChatLoc.CL_SystemWindow);
                foreach (GamePlayer player in m_caster.GetPlayersInRadius(WorldMgr.INFO_DISTANCE))
                    if (player != m_caster)
                        // Message: {0} casts a spell!
                        player.MessageFromArea(m_caster,
                            LanguageMgr.GetTranslation(player.Client, "SpellHandler.CastSpell.Msg.LivingCastsSpell",
                                Caster.GetName(0, true)), eChatType.CT_Spell, eChatLoc.CL_SystemWindow);
            }
        ...
        {
            var quickcast =
                (QuickCastECSGameEffect) EffectListService.GetAbilityEffectOnTarget(m_caster, eEffect.QuickCast);
            if (quickcast != null && Spell.CastTime > 0)
            {
                ...
                ((GamePlayer) m_caster).DisableSkill(SkillBase.GetAbility(Abilities.Quickcast),
                    QuickCastAbilityHandler.DISABLE_DURATION);
        ...
        if (m_spell.RecastDelay > 0 && m_startReuseTimer)
        {
            if (m_caster is GamePlayer)
            {
                ...
                var gp_caster = m_caster as GamePlayer;
                foreach (var skills in gp_caster.GetAllUsableSkills())
                    if (skills.Item1 is Spell &&
                        (((Spell) skills.Item1).ID == m_spell.ID || (((Spell) skills.Item1).SharedTimerGroup != 0 &&
                                                                     ((Spell) skills.Item1).SharedTimerGroup ==
                                                                     m_spell.SharedTimerGroup)))
                ...
                foreach (var sp in sl.Item2)
                    if (sp is Spell &&
                        (((Spell) sp).ID == m_spell.ID || (((Spell) sp).SharedTimerGroup != 0 &&
                                                           ((Spell) sp).SharedTimerGroup ==
                                                           m_spell.SharedTimerGroup)))
        ...
        if (!Spell.IsPulsingEffect && !Spell.IsPulsing &&
            Caster is GamePlayer {CharacterClass: not ClassSavage})
            m_caster.ChangeEndurance(m_caster, eEnduranceChangeType.Spell, -5);

        GameEventMgr.Notify(GameLivingEvent.CastFinished, m_caster,
            new CastingEventArgs(this, target, m_lastAttackData));
    }
    ...
    public virtual IList<GameLiving> SelectTargets(GameObject castTarget)
    {
        ...
        var target = castTarget as GameLiving;
        var modifiedTarget = Spell.Target.ToLower();
        ...
        if (modifiedTarget == "pet" && !HasPositiveEffect)
        {
            ...
            if (Spell.SpellType != (byte) eSpellType.TurretPBAoE) target = Caster.ControlledBrain.Body;
        }
        ...
        switch (modifiedTarget)
        {
            ...
            case "area":
                ...
                //selected!
                if (Spell.SpellType == (byte) eSpellType.SummonAnimistPet ||
                    Spell.SpellType == (byte) eSpellType.SummonAnimistFnF)
                {
                    list.Add(Caster);
                }
                else if (modifiedRadius > 0)
                {
                    ...
                    Parallel.ForEach(
                        WorldMgr.GetPlayersCloseToSpot(Caster.CurrentRegionID, Caster.GroundTarget.X,
                            Caster.GroundTarget.Y, Caster.GroundTarget.Z, modifiedRadius).Cast<GamePlayer>(),
                        player =>
                        {
                            if (GameServer.ServerRules.IsAllowedToAttack(Caster, player, true))
                            {
                                // Apply Mentalist RA5L
                                var SelectiveBlindness =
                                    Caster.EffectList.GetOfType<SelectiveBlindnessEffect>();
                                if (SelectiveBlindness != null)
                                {
                                    var EffectOwner = SelectiveBlindness.EffectSource;
                                    if (EffectOwner == player)
                                    {
                                        if (Caster is GamePlayer player1)
                                            player1.Out.SendMessage(
                                                string.Format("{0} is invisible to you!", player.GetName(0, true)),
                                                eChatType.CT_Missed, eChatLoc.CL_SystemWindow);
                                    }
                                    else
                                    {
                    ...
                    Parallel.ForEach(
                        WorldMgr.GetNPCsCloseToSpot(Caster.CurrentRegionID, Caster.GroundTarget.X,
                            Caster.GroundTarget.Y, Caster.GroundTarget.Z, modifiedRadius).Cast<GameNPC>(), npc =>
                        {
                            if (npc is GameStorm)
                                aoeMobs.Add(npc);
                    ...
                    if (Caster is GamePlayer player && player.CharacterClass.Name.ToLower() == "bonedancer")
                    {
                        foreach (var pet in player.GetNPCsInRadius((ushort) Spell.Range))
                            if (pet is CommanderPet commander && commander.Owner == player)
                                list.Add(commander);
                            else if (pet is BDSubPet {Brain: IControlledBrain brain} subpet &&
                                     brain.GetPlayerOwner() == player)
                                if (!Spell.IsHealing)
                                    list.Add(subpet);
                    }
                    else
            ...
            case "bdsubpet":
            {
                ...
                if (petBody != null)
                {
                    ...
                    if (list.Count < 1)
                        player.Out.SendMessage("You don't have any subpet to cast this spell on!",
                                ...
                                if (EffectOwner == target)
                                {
                                    if (Caster is GamePlayer player)
                                        player.Out.SendMessage(
                                            string.Format("{0} is invisible to you!", target.GetName(0, true)),
                                            eChatType.CT_Missed, eChatLoc.CL_SystemWindow);
                                }
                ...
                else
                {
                    if (target != null && GameServer.ServerRules.IsSameRealm(Caster, target, true))
                    {
                        if (target is GamePlayer player &&
                            player.CharacterClass.ID == (int) eCharacterClass.Necromancer && player.IsShade)
                        {
                            // Only buffs, Necromancer's power transfer, and teleport spells can be casted on the shade
                            if (Spell.IsBuff || Spell.SpellType == (byte) eSpellType.PowerTransferPet ||
                                Spell.SpellType == (byte) eSpellType.UniPortal)
                                list.Add(player);
                            else
            ...
            case "group":
            {
                ...
                if (group == null)
                {
                    ...
                    {
                        var casterbrain = (m_caster as GameNPC).Brain as IControlledBrain;

                        var player = casterbrain.GetPlayerOwner();
                ...
                if (group != null)
                    foreach (var living in group.GetMembersInTheGroup())
                        ...
                        if (m_caster.IsWithinRadius(living, spellRange))
                        {
                            ...
                            var npc = living.ControlledBrain;
                            if (npc != null)
                            {
                                //Add our first pet
                                var petBody2 = npc.Body;
    ...
    public virtual void CastSubSpells(GameLiving target)
    {
        ...
        foreach (var spellID in subSpellList.Union(m_spell.MultipleSubSpells))
        {
    ...
    public virtual List<GameLiving> GetGroupAndPets(Spell spell)
    {
        ...
        var groupMembers = Caster.Group?.GetMembersInTheGroup() ??
                           (Caster as NecromancerPet)?.Owner.Group?.GetMembersInTheGroup();

        if (groupMembers == null)
            groupMembers = new List<GameLiving>() {Caster};

        foreach (var living in groupMembers)
        {
            var controlledBrain = living.ControlledBrain;
            var subControlledBrains = controlledBrain?.Body.ControlledNpcList;

            if (subControlledBrains != null)
                foreach (var subControlledBrain in subControlledBrains.Where(x =>
                             x != null && Caster.IsWithinRadius(x.Body, spell.Range)))
    ...
    public virtual bool StartSpell(GameLiving target)
    {
        ...
        if (HasPositiveEffect && m_spellTarget is GamePlayer p && Caster is GamePlayer c &&
            m_spellTarget != Caster && p.NoHelp)
        {
            c.Out.SendMessage(
                m_spellTarget.Name + " has chosen to walk the path of solitude, and your spell fails.",
                eChatType.CT_SpellResisted, eChatLoc.CL_SystemWindow);
            return false;
        }
        ...
        var effectiveness = Caster.Effectiveness;
...
// 			}
        ...
        if (Caster is GamePlayer && (Caster as GamePlayer).CharacterClass.ID == (int) eCharacterClass.Warlock &&
            m_spell.IsSecondary)
        {
            var uninterruptibleSpell =
                Caster.TempProperties.getProperty<Spell>(UninterruptableSpellHandler.WARLOCK_UNINTERRUPTABLE_SPELL);

            if (uninterruptibleSpell != null && uninterruptibleSpell.Value > 0)
            {
                var nerf = uninterruptibleSpell.Value;
        ...
        Parallel.ForEach(targets, t =>
        {
            ...
            var spellResistChance = CalculateSpellResistChance(t);
            var randNum = 0;
            var UseRNGOverride = Properties.OVERRIDE_DECK_RNG;
            ...
            {
                var dist = t.GetDistanceTo(m_spellTarget);
                if (dist >= 0)
                    ApplyEffectOnTarget(t, effectiveness - CalculateAreaVariance(t, dist, Spell.Radius));
            }
        ...
        if (Spell.Target.ToLower() == "ground") ApplyEffectOnTarget(null, 1);
    ...
    public virtual void ApplyEffectOnTarget(GameLiving target, double effectiveness)
    {
        ...
        if (target is Keeps.GameKeepDoor || target is Keeps.GameKeepComponent)
        {
            ...
            {
                if (!isSilent)
                    MessageToCaster(string.Format("Your spell has no effect on the {0}!", target.Name),
                        eChatType.CT_SpellResisted);
    ...
    {
        if (CastingCompleteEvent != null) CastingCompleteEvent(this);
    }
    ...
    {
        var oldspell = oldeffect.Spell;
        var newspell = neweffect.Spell;
...
//			}
    ...
    public virtual bool IsCancellable(GameSpellEffect compare)
    {
        if (compare.SpellHandler != null)
            if ((compare.SpellHandler.AllowCoexisting || AllowCoexisting)
                && (!compare.SpellHandler.SpellLine.KeyName.Equals(SpellLine.KeyName,
    ...
    {
        if (!target.IsAlive || target.effectListComponent == null) return;

        // eChatType noOverwrite = (Spell.Pulse == 0) ? eChatType.CT_SpellResisted : eChatType.CT_SpellPulse;

        CreateECSEffect(new ECSGameEffectInitParams(target, CalculateEffectDuration(target, effectiveness),
            effectiveness, this));
    ...
    public virtual void OnEffectRemove(GameSpellEffect effect, bool overwrite)
    {
        if (!overwrite)
        {
            if (Spell.IsFocus) FocusSpellAction( /*null, Caster, null*/);

            // Re-Enable Cancellable Effects.
            var enableEffect = effect.Owner.EffectList.OfType<GameSpellEffect>()
                .Where(eff =>
                    eff != effect && eff.SpellHandler != null && eff.SpellHandler.IsOverwritable(effect) &&
                    eff.SpellHandler.IsCancellable(effect));
    ...
    public virtual int CalculateSpellResistChance(GameLiving target)
    {
        ...
        if (m_spellLine.KeyName == GlobalSpellsLines.Item_Effects && m_spellItem != null)
        {
            ...
            if (playerCaster != null)
            {
                var itemSpellLevel = m_spellItem.Template.LevelRequirement > 0
                    ? m_spellItem.Template.LevelRequirement
                    : Math.Min(playerCaster.MaxLevel, m_spellItem.Level);
    ...
    public virtual void SendSpellResistMessages(GameLiving target)
    {
        ...
        if (target is GameNPC)
        {
            ...
            if (brain != null)
            {
                ...
                if (owner != null)
                    this.MessageToLiving(owner, eChatType.CT_SpellResisted, "Your {0} resists the effect!",
                        target.Name);
            }
        }
        else
        ...
        this.MessageToCaster(eChatType.CT_SpellResisted,
            "{0} resists the effect!" + " (" + CalculateSpellResistChance(target).ToString("0.0") + "%)",
    ...
    public virtual void StartSpellResistInterruptTimer(GameLiving target)
    {
        ...
        if (!(Spell.SpellType.ToString().IndexOf("debuff", StringComparison.OrdinalIgnoreCase) >= 0 &&
              Spell.CastTime == 0))
    ...
    public void MessageToCaster(string message, eChatType type)
    {
        if (Caster is GamePlayer playerCaster)
        {
            playerCaster.MessageToSelf(message, type);
                                             ...
                                             && type is eChatType.CT_YouHit or eChatType.CT_SpellResisted
                                                 or eChatType.CT_Spell)
        {
            var playerOwner = npcCasterBrain.GetPlayerOwner();
    ...
    {
        if (message != null && message.Length > 0) living.MessageToSelf(message, type);
    }
    ...
    /// </summary>
    public bool IsCasting => CastState == eCastState.Casting; //return m_castTimer != null && m_castTimer.IsAlive; }
    ...
    public virtual IList<string> DelveInfo
    {
        get
        {
            ...
            if (Caster is GamePlayer || (Caster is GameNPC && (Caster as GameNPC).Brain is IControlledBrain &&
                ...
                p = Caster is GamePlayer
                    ? Caster as GamePlayer
                    : ((Caster as GameNPC).Brain as IControlledBrain).GetPlayerOwner();

            list.Add(Spell.Description);
            list.Add(" "); //empty line
            if (Spell.InstrumentRequirement != 0)
                list.Add(p != null
                    ? LanguageMgr.GetTranslation(p.Client, "DelveInfo.InstrumentRequire",
                        GlobalConstants.InstrumentTypeToName(Spell.InstrumentRequirement))
                    : LanguageMgr.GetTranslation(Properties.SERV_LANGUAGE,
                        "DelveInfo.InstrumentRequire",
                        GlobalConstants.InstrumentTypeToName(Spell.InstrumentRequirement)));
            if (Spell.Damage != 0)
                list.Add(p != null
                    ? LanguageMgr.GetTranslation(p.Client, "DelveInfo.Damage",
                        Spell.Damage.ToString("0.###;0.###'%'"))
                    : LanguageMgr.GetTranslation(Properties.SERV_LANGUAGE, "DelveInfo.Damage",
                        Spell.Damage.ToString("0.###;0.###'%'")));
            if (Spell.LifeDrainReturn != 0)
                list.Add(p != null
                    ? LanguageMgr.GetTranslation(p.Client, "DelveInfo.HealthReturned", Spell.LifeDrainReturn)
                    : LanguageMgr.GetTranslation(Properties.SERV_LANGUAGE,
                        "DelveInfo.HealthReturned", Spell.LifeDrainReturn));
            else if (Spell.Value != 0)
                list.Add(p != null
                    ? LanguageMgr.GetTranslation(p.Client, "DelveInfo.Value",
                        Spell.Value.ToString("0.###;0.###'%'"))
                    : LanguageMgr.GetTranslation(Properties.SERV_LANGUAGE, "DelveInfo.Value",
                        Spell.Value.ToString("0.###;0.###'%'")));
            list.Add(p != null
                ? LanguageMgr.GetTranslation(p.Client, "DelveInfo.Target", Spell.Target)
                : LanguageMgr.GetTranslation(Properties.SERV_LANGUAGE, "DelveInfo.Target",
                    Spell.Target));
            if (Spell.Range != 0)
                list.Add(p != null
                    ? LanguageMgr.GetTranslation(p.Client, "DelveInfo.Range", Spell.Range)
                    : LanguageMgr.GetTranslation(Properties.SERV_LANGUAGE, "DelveInfo.Range",
                        Spell.Range));
            if (Spell.Duration >= ushort.MaxValue * 1000)
                list.Add(p != null
                    ? LanguageMgr.GetTranslation(p.Client, "DelveInfo.Duration") + " Permanent."
                    : LanguageMgr.GetTranslation(Properties.SERV_LANGUAGE, "DelveInfo.Duration") +
                      " Permanent.");
            else if (Spell.Duration > 60000)
                list.Add(p != null
                    ? LanguageMgr.GetTranslation(p.Client, "DelveInfo.Duration") + " " + Spell.Duration / 60000 +
                      ":" + (Spell.Duration % 60000 / 1000).ToString("00") + " min"
                    : LanguageMgr.GetTranslation(Properties.SERV_LANGUAGE, "DelveInfo.Duration") +
                      " " + Spell.Duration / 60000 + ":" + (Spell.Duration % 60000 / 1000).ToString("00") + " min");
            else if (Spell.Duration != 0)
                list.Add(p != null
                    ? LanguageMgr.GetTranslation(p.Client, "DelveInfo.Duration") + " " +
                      (Spell.Duration / 1000).ToString("0' sec';'Permanent.';'Permanent.'")
                    : LanguageMgr.GetTranslation(Properties.SERV_LANGUAGE, "DelveInfo.Duration") +
                      " " + (Spell.Duration / 1000).ToString("0' sec';'Permanent.';'Permanent.'"));
            if (Spell.Frequency != 0)
                list.Add(p != null
                    ? LanguageMgr.GetTranslation(p.Client, "DelveInfo.Frequency",
                        (Spell.Frequency * 0.001).ToString("0.0"))
                    : LanguageMgr.GetTranslation(Properties.SERV_LANGUAGE, "DelveInfo.Frequency",
                        (Spell.Frequency * 0.001).ToString("0.0")));
            if (Spell.Power != 0)
                list.Add(p != null
                    ? LanguageMgr.GetTranslation(p.Client, "DelveInfo.PowerCost", Spell.Power.ToString("0;0'%'"))
                    : LanguageMgr.GetTranslation(Properties.SERV_LANGUAGE, "DelveInfo.PowerCost",
                        Spell.Power.ToString("0;0'%'")));
            list.Add(p != null
                ? LanguageMgr.GetTranslation(p.Client, "DelveInfo.CastingTime",
                    (Spell.CastTime * 0.001).ToString("0.0## sec;-0.0## sec;'instant'"))
                : LanguageMgr.GetTranslation(Properties.SERV_LANGUAGE, "DelveInfo.CastingTime",
                    (Spell.CastTime * 0.001).ToString("0.0## sec;-0.0## sec;'instant'")));
            if (Spell.RecastDelay > 60000)
                list.Add(p != null
                    ? LanguageMgr.GetTranslation(p.Client, "DelveInfo.RecastTime") + " " +
                      (Spell.RecastDelay / 60000).ToString() + ":" +
                      (Spell.RecastDelay % 60000 / 1000).ToString("00") + " min"
                    : LanguageMgr.GetTranslation(Properties.SERV_LANGUAGE,
                          "DelveInfo.RecastTime") + " " + (Spell.RecastDelay / 60000).ToString() + ":" +
                      (Spell.RecastDelay % 60000 / 1000).ToString("00") + " min");
                ...
                list.Add(p != null
                    ...
                    : LanguageMgr.GetTranslation(Properties.SERV_LANGUAGE,
                        "DelveInfo.RecastTime") + " " + (Spell.RecastDelay / 1000).ToString() + " sec");
            if (Spell.Concentration != 0)
                list.Add(p != null
                    ? LanguageMgr.GetTranslation(p.Client, "DelveInfo.ConcentrationCost", Spell.Concentration)
                    : LanguageMgr.GetTranslation(Properties.SERV_LANGUAGE,
                        "DelveInfo.ConcentrationCost", Spell.Concentration));
            if (Spell.Radius != 0)
                list.Add(p != null
                    ? LanguageMgr.GetTranslation(p.Client, "DelveInfo.Radius", Spell.Radius)
                    : LanguageMgr.GetTranslation(Properties.SERV_LANGUAGE, "DelveInfo.Radius",
                        Spell.Radius));
            ...
            if (Spell.IsFocus)
                list.Add(p != null
                    ? LanguageMgr.GetTranslation(p.Client, "DelveInfo.Focus")
                    : LanguageMgr.GetTranslation(Properties.SERV_LANGUAGE, "DelveInfo.Focus"));
    ...
    public static GameSpellEffect FindEffectOnTarget(GameLiving target, string spellType, string spellName)
    {
        ...
        {
            foreach (var fx in target.EffectList)
            {
                ...
                if (effect.SpellHandler.Spell != null &&
                    effect.SpellHandler.Spell.SpellType.ToString() == spellType &&
                    effect.SpellHandler.Spell.Name == spellName)
                    return effect;
            }
        }
    ...
    public static PulsingSpellEffect FindPulsingSpellOnTarget(GameLiving living, ISpellHandler handler)
    {
        lock (living.effectListComponent.ConcentrationEffectsLock)
        {
            ...
            {
                var pulsingSpell = concEffect as PulsingSpellEffect;
    ...
    public virtual void CalculateDamageVariance(GameLiving target, out double min, out double max)
    {
        ...
        {
            var brain = (m_caster as GameNPC).Brain as IControlledBrain;
        ...
        {
            //Get the root owner
            var owner = ((IControlledBrain) ((GameNPC) m_caster).Brain).GetLivingOwner();
            if (owner != null)
            {
    ...
    {
        var cappedDamage = damage;

        if (player.Level < 13) cappedDamage = 4.1 * player.Level;

        if (player.Level < 50) cappedDamage = 3.8 * player.Level;
    ...
    {
        if (npc.Level < 50) return Math.Min(damage, 4.7 * npc.Level);
...
*/
                    ...
                    if (pet.Owner is GamePlayer own)
                    {
                        ...
                        var manaStatValue = own.GetModified((eProperty) own.CharacterClass.ManaStat);
                ...
                var modSkill = pet.Owner.GetModifiedSpecLevel(m_spellLine.Spec) -
            ...
            {
                var weaponskillScalar = (3 + .02 * player.GetWeaponStat(player.ActiveWeapon)) /
            ...
            {
                //Delve * (acu/200+1) * (plusskillsfromitems/200+1) * (Relicbonus+1) * (mom+1) * (1 - enemyresist) 
                var manaStatValue = player.GetModified((eProperty) player.CharacterClass.ManaStat);
                //spellDamage *= ((manaStatValue - 50) / 275.0) + 1;
                spellDamage *= (manaStatValue - player.Level) * 0.005 + 1;
                var modSkill = player.GetModifiedSpecLevel(m_spellLine.Spec) -
        ...
        else if (Caster is GameNPC)
        {
            ...
            var manaStatValue = npc.GetModified(eProperty.Intelligence);
    ...
    public virtual int CalculateToHitChance(GameLiving target)
    {
        ...
        var spellbonus = caster.GetModified(eProperty.SpellLevel);
        ...
        if (playerCaster != null && (m_spellLine.KeyName == GlobalSpellsLines.Combat_Styles_Effect ||
                                     m_spellLine.KeyName.StartsWith(GlobalSpellsLines.Champion_Lines_StartWith)))
        {
            var lastAD = playerCaster.TempProperties.getProperty<AttackData>("LastAttackData", null);
            spellLevel = lastAD != null && lastAD.Style != null
                ? lastAD.Style.Level
                : Math.Min(playerCaster.MaxLevel, target.Level);
        }
        //Console.WriteLine($"Spell level {spellLevel}");

        var bonustohit = m_caster.GetModified(eProperty.ToHitBonus);
        ...
        */
        ...
        var hitchance = 88 + (spellLevel - target.Level) / 2 + bonustohit;

        if (!(caster is GamePlayer && target is GamePlayer))
        {
            var mobScalar = m_caster.GetConLevel(target) > 3 ? 3 : m_caster.GetConLevel(target);
        ...
        if (Caster is GameNPC) hitchance = (int) (87.5 - (target.Level - Caster.Level));
        ...
        if (Caster.effectListComponent.ContainsEffectForEffectType(eEffect.MajesticWill))
        {
            ...
            if (effect != null) hitchance += (int) effect.Effectiveness * 5;
        }
    ...
    public virtual int AdjustDamageForHitChance(int damage, int hitChance)
    {
        ...
        if (hitChance < 55)
            adjustedDamage += (int) (adjustedDamage * (hitChance - 55) *
                                     Properties.SPELL_HITCHANCE_DAMAGE_REDUCTION_MULTIPLIER *
                                     0.01);
    ...
    public virtual AttackData CalculateDamageToTarget(GameLiving target, double effectiveness)
    {
        ...
        var spellDamage = CalculateDamageBase(target);
        ...
        var finalDamage = Util.Random((int) (minVariance * spellDamage), (int) (maxVariance * spellDamage));
        ...
        var hitChance = CalculateToHitChance(ad.Target);
        ...
        if (m_caster is GamePlayer || (m_caster is GameNPC && (m_caster as GameNPC).Brain is IControlledBrain &&
                                       m_caster.Realm != 0))
        {
            if (target is GamePlayer)
                finalDamage = (int) ((double) finalDamage * Properties.PVP_SPELL_DAMAGE);
            else if (target is GameNPC)
                finalDamage = (int) ((double) finalDamage * Properties.PVE_SPELL_DAMAGE);
        }
        ...
        var damageType = DetermineSpellDamageType();

        #region Resists

        var property = target.GetResistTypeForDamage(damageType);
        ...
        var primaryResistModifier = ad.Target.GetResist(damageType);
         ...
         */
        var resiPierce = Caster.GetModified(eProperty.ResistPierce);
        var ply = Caster as GamePlayer;
        ...
        var secondaryResistModifier = target.SpecBuffBonusCategory[(int) property];
        ...
        //secondary resists
        resistModifier +=
            (int) ((finalDamage + (double) resistModifier) * (double) secondaryResistModifier * -0.01);
        //apply resists
        ...
        if (finalDamage > DamageCap(effectiveness)) finalDamage = (int) DamageCap(effectiveness);
        ...
        {
            criticalchance =
                0; //atlas - DoTs can only crit with Wild Arcana. This is handled by the DoTSpellHandler directly
            cdamage = 0;
        }
        else
        {
        ...
        var randNum = Util.CryptoNextInt(1, 100); //grab our random number
        var critCap = Math.Min(50, criticalchance); //crit chance can be at most  50%

        if (Caster is GamePlayer spellCaster && spellCaster.UseDetailedCombatLog && critCap > 0)
            spellCaster.Out.SendMessage($"spell crit chance: {critCap} random: {randNum}", eChatType.CT_DamageAdd,
                eChatLoc.CL_SystemWindow);

        if (critCap > randNum && finalDamage >= 1)
        {
            var critmax = ad.Target is GamePlayer ? finalDamage / 2 : finalDamage;
        ...
        {
            var manaconversion = (int) Math.Round(((double) ad.Damage + (double) ad.CriticalDamage) *
                (double) ad.Target.GetModified(eProperty.Conversion) / 200);
            //int enduconversion=(int)Math.Round((double)manaconversion*(double)ad.Target.MaxEndurance/(double)ad.Target.MaxMana);
            var enduconversion = (int) Math.Round(((double) ad.Damage + (double) ad.CriticalDamage) *
                (double) ad.Target.GetModified(eProperty.Conversion) / 200);
            ...
            if (manaconversion >= 1)
                (ad.Target as GamePlayer).Out.SendMessage("You gain " + manaconversion + " power points",
                    eChatType.CT_Spell, eChatLoc.CL_SystemWindow);
            if (enduconversion >= 1)
                (ad.Target as GamePlayer).Out.SendMessage("You gain " + enduconversion + " endurance points",
                    eChatType.CT_Spell, eChatLoc.CL_SystemWindow);
    ...
    public virtual void SendDamageMessages(AttackData ad)
    {
        ...
        if (Caster is GamePlayer || Caster is NecromancerPet)
            MessageToCaster(
                string.Format("You hit {0} for {1}{2} damage!", ad.Target.GetName(0, false), ad.Damage, modmessage),
                eChatType.CT_YouHit);
        else if (Caster is GameNPC)
        ...
        if (ad.CriticalDamage > 0)
            MessageToCaster(
                "You critically hit for an additional " + ad.CriticalDamage + " damage!" + " (" +
                m_caster.SpellCriticalChance + "%)", eChatType.CT_YouHit);
    }
    ...
    {
        ad.AttackResult = eAttackResult.HitUnstyled;
        if (showEffectAnimation) SendEffectAnimation(ad.Target, 0, false, 1);
        ...
        if (ad.Damage > 0)
            Parallel.ForEach(ad.Target.GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE).Cast<GamePlayer>(),
                player =>
                {
                    player.Out.SendCombatAnimation(ad.Attacker, ad.Target, 0, 0, 0, 0, (byte) attackResult,
                        ad.Target.HealthPercent);
                });
    ...
    public virtual void TooltipDelve(ref MiniDelveWriter dw)
    {
        ...
        if (Spell.HasSubSpell)
            if (Spell.SpellType == (byte) eSpellType.Bomber ||
                Spell.SpellType == (byte) eSpellType.SummonAnimistFnF)
    ...
    private void WriteSpecial(ref MiniDelveWriter dw)
    {
        switch ((eSpellType) Spell.SpellType)
        {
            ...
            case eSpellType.Charm:
                ...
                switch ((CharmSpellHandler.eCharmType) Spell.AmnesiaChance)
                {
                    case CharmSpellHandler.eCharmType.All:
                        ...
                        dw.AddKeyValuePair("delve_string",
                            LanguageMgr.GetTranslation(((GamePlayer) Caster).Client,
                                "CharmSpell.DelveInfo.Desc.AllMonsterTypes"));
                        break;
                    case CharmSpellHandler.eCharmType.Animal:
                        ...
                        dw.AddKeyValuePair("delve_string",
                            LanguageMgr.GetTranslation(((GamePlayer) Caster).Client,
                                "CharmSpell.DelveInfo.Desc.Animal"));
                        break;
                    case CharmSpellHandler.eCharmType.Humanoid:
                        ...
                        dw.AddKeyValuePair("delve_string",
                            LanguageMgr.GetTranslation(((GamePlayer) Caster).Client,
                                "CharmSpell.DelveInfo.Desc.Humanoid"));
                        break;
                    case CharmSpellHandler.eCharmType.Insect:
                        ...
                        dw.AddKeyValuePair("delve_string",
                            LanguageMgr.GetTranslation(((GamePlayer) Caster).Client,
                                "CharmSpell.DelveInfo.Desc.Insect"));
                        break;
                    case CharmSpellHandler.eCharmType.HumanoidAnimal:
                        ...
                        dw.AddKeyValuePair("delve_string",
                            LanguageMgr.GetTranslation(((GamePlayer) Caster).Client,
                                "CharmSpell.DelveInfo.Desc.HumanoidAnimal"));
                        break;
                    case CharmSpellHandler.eCharmType.HumanoidAnimalInsect:
                        ...
                        dw.AddKeyValuePair("delve_string",
                            LanguageMgr.GetTranslation(((GamePlayer) Caster).Client,
                                "CharmSpell.DelveInfo.Desc.HumanoidAnimalInsect"));
                        break;
                    case CharmSpellHandler.eCharmType.HumanoidAnimalInsectMagical:
                        ...
                        dw.AddKeyValuePair("delve_string",
                            LanguageMgr.GetTranslation(((GamePlayer) Caster).Client,
                                "CharmSpell.DelveInfo.Desc.HumanoidAnimalInsectMagical"));
                        break;
                    case CharmSpellHandler.eCharmType.HumanoidAnimalInsectMagicalUndead:
                        ...
                        dw.AddKeyValuePair("delve_string",
                            LanguageMgr.GetTranslation(((GamePlayer) Caster).Client,
                                "CharmSpell.DelveInfo.Desc.HumanoidAnimalInsectMagicalUndead"));
                        break;
                }
            ...
            case eSpellType.Disease:
                dw.AddKeyValuePair("delve_string",
                    "Inflicts a wasting disease on the target that slows target by 15 %, reduces strength by 7.5 % and inhibits healing by 50 %");
                break;
            //case eSpellType.DefensiveProc:
            //case eSpellType.OffensiveProc:
            ...
            case eSpellType.FatigueConsumptionBuff:
                dw.AddKeyValuePair("delve_string",
                    $"The target's actions require {(int) Spell.Value}% less endurance.");
                break;
            case eSpellType.FatigueConsumptionDebuff:
                dw.AddKeyValuePair("delve_string",
                    $"The target's actions require {(int) Spell.Value}% more endurance.");
                break;
            case eSpellType.MeleeDamageBuff:
            ...
            case eSpellType.TurretPBAoE:
                dw.AddKeyValuePair("delve_string",
                    $"Target takes {(int) Spell.Damage} damage. Spell affects everyone in the immediate radius of the caster's pet, and does less damage the further away they are from the caster's pet.");
                break;
            ...
            case eSpellType.MultiTarget:
                dw.AddKeyValuePair("delve_string",
                    $"Hits {(int) Spell.Value} additonal target(s) within melee range.");
                break;
            case eSpellType.PiercingMagic:
                dw.AddKeyValuePair("delve_string",
                    $"Effectiveness of the target's spells is increased by {(int) Spell.Value}%. Against higher level opponents than the target, this should reduce the chance of a full resist.");
                break;
            case eSpellType.StyleTaunt:
                if (Spell.Value < 0)
                    dw.AddKeyValuePair("delve_string",
                        $"Decreases your threat to monster targets by {-(int) Spell.Value} damage.");
                break;
            case eSpellType.NaturesShield:
                dw.AddKeyValuePair("delve_string",
                    $"Gives the user a {(int) Spell.Value}% base chance to block ranged melee attacks while this style is prepared.");
                break;
            case eSpellType.SlashResistDebuff:
                dw.AddKeyValuePair("delve_string",
                    $"Decreases target's resistance to Slash by {(int) Spell.Value}% for {(int) Spell.Duration / 1000} seconds.");
                break;
        }
    }
    ...
    protected virtual int GetDurationType()
    {
        ...
        if (Spell.Duration > 0) return 2;

        if (Spell.IsConcentration) return 4;
