...
public class DirectDamageDebuffSpellHandler : AbstractResistDebuff
{
    ...
    public override void OnDirectEffect(GameLiving target, double effectiveness)
    {
        ...
        if (Spell.Target.ToLower() == "cone" || (Spell.Target == "Enemy" && Spell.IsPBAoE))
        {
            ...
            else
            {
                ...
                {
                    var brain = (Caster as GameNPC).Brain as AI.Brain.IControlledBrain;
                    //Ryan: edit for BD
    ...
    private void DealDamageCheckLOS(GamePlayer player, ushort response, ushort targetOID)
    {
        ...
        if ((response & 0x100) == 0x100)
            try
            {
                var target = Caster.CurrentRegion.GetObject(targetOID) as GameLiving;
                if (target != null)
                {
                    var effectiveness =
                        (double) player.TempProperties.getProperty<object>(LOSEFFECTIVENESS, null);
                    DealDamage(target, effectiveness);
    ...
    public override void ApplyEffectOnTarget(GameLiving target, double effectiveness)
    {
        ...
        {
            var tmpeffective = effectiveness;
            /*
        ...
        get
        {
            ...
            */
            ...
            list.Add(LanguageMgr.GetTranslation((Caster as GamePlayer).Client,
                "DirectDamageDebuffSpellHandler.DelveInfo.Function"));
            list.Add(" "); //empty line
            ...
            if (Spell.Value != 0)
                list.Add(string.Format(LanguageMgr.GetTranslation((Caster as GamePlayer).Client,
                    "DirectDamageDebuffSpellHandler.DelveInfo.Decrease", DebuffTypeName, Spell.Value)));
    ...
    // constructor
    public DirectDamageDebuffSpellHandler(GameLiving caster, Spell spell, SpellLine line) : base(caster, spell,
        line)
    {
    }
}