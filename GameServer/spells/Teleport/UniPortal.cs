    ...
    public override void ApplyEffectOnTarget(GameLiving target, double effectiveness)
    {
        ...
        if (player.InCombat || GameRelic.IsPlayerCarryingRelic(player))
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language, "GamePlayer.UseSlot.CantUseInCombat"),
        ...
        player.LeaveHouse();
        player.MoveTo((ushort) m_destination.RegionID, m_destination.X, m_destination.Y, m_destination.Z,
...
public class UniPortalKeep : SpellHandler
{
    ...
    public override void ApplyEffectOnTarget(GameLiving target, double effectiveness)
    {
        ...
        */
        ...
        var effect = new UniPortalEffect(this, 1500);
        effect.Start(player);

        player.LeaveHouse();
        player.MoveTo((ushort) m_destination.Region, m_destination.X, m_destination.Y, m_destination.Z,
            (ushort) m_destination.Heading);
    }
}