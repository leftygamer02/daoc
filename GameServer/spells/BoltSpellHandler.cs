...
public class BoltSpellHandler : SpellHandler
{
    ...
    public override void FinishSpellCast(GameLiving target)
    {
        ...
        if ((target is GameKeepDoor || target is GameKeepComponent) &&
            Spell.SpellType != (byte) eSpellType.SiegeArrow &&
            Spell.SpellType != (byte) eSpellType.SiegeDirectDamage)
        {
    ...
    private void DealDamage(GameLiving target)
    {
        ...
        var bolt = new BoltOnTargetAction(Caster, target, this);
    ...
    protected class BoltOnTargetAction : RegionECSAction
    {
        ...
        protected override int OnTick(ECSGameTimer timer)
        {
            ...
            var missrate = m_handler.CalculateSpellResistChance(target);
            var combatMiss = false;
            ...
            if (target is GameNPC || caster is GameNPC)
                missrate += (int) (ServerProperties.Properties.PVE_SPELL_CONHITPERCENT *
                                   caster.GetConLevel(target));
            ...
            var ad = m_handler.CalculateDamageToTarget(target, 0.65);
            ...
            if (missrate > rand)
            {
                ...
                if (combatMiss)
                    m_handler.MessageToCaster($"{target.Name} is in combat and your bolt misses!",
                        eChatType.CT_YouHit);
                else
            ...
            if (target is GamePlayer)
            {
                ...
                if (lefthand != null && (player.ActiveWeapon == null ||
                    ...
                    if (target.IsObjectInFront(caster, 180) && lefthand.Object_Type == (int) eObjectType.Shield)
                    {
                        ...
                        var blockchance = (player.Dexterity * 2 - 100) / 40.0 + shield + 5;
                        ...
                        {
                            m_handler.MessageToLiving(player,
                                "You partially block " + caster.GetName(0, false) + "'s spell!",
                                eChatType.CT_Missed);
                ...
                if (target.IsObjectInFront(caster, 120) && (target.styleComponent.NextCombatStyle?.ID == 394 ||
                                                            target.styleComponent.NextCombatBackupStyle?.ID == 394))
                {
                    m_handler.MessageToLiving(player, "You block " + caster.GetName(0, false) + "'s spell!",
                        eChatType.CT_Missed);
                    m_handler.MessageToCaster(player.GetName(0, true) + " blocks!", eChatType.CT_YouHit);
                    blocked = true;
                    ad.Damage = 0;
            ...
            var effectiveness = 1.0 + caster.GetModified(eProperty.SpellDamage) * 0.01;
            ...
            if (blocked == false)
            {
                ...
                var ws = caster.Level * 2.85 * (1.0 + (caster.GetModified(eProperty.Dexterity) - 50) / 200.0);
                var playerBaseAF = ad.Target is GamePlayer ? ad.Target.Level * 45 / 50d : 45;
                ...
                var physMod = (int) (damage * ad.Target.GetResist(ad.DamageType) / -100.0);
                ad.Modifier += physMod;
                damage += physMod;
            ...
            if (ad.Damage > 0)
            {
                ...
                // "{0} is hit by a bolt of runic energy!"
                Message.SystemToArea(target, Util.MakeSentence(m_handler.Spell.Message2, target.GetName(0, true)),
                    eChatType.CT_System, target, caster);
