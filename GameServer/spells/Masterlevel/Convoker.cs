...
namespace DOL.GS.Spells
{
    ...
    public class SummonWoodSpellHandler : SummonItemSpellHandler
    {
        ...
        {
            var template = GameServer.Database.FindObjectByKey<ItemTemplate>("mysticwood_wooden_boards");
            if (template != null)
            {
    ...
    public class SummonWarcrystalSpellHandler : SummonItemSpellHandler
    {
            ...
            : base(caster, spell, line)
        {
            ...
            var template = GameServer.Database.FindObjectByKey<ItemTemplate>(ammo);
            if (template != null)
            {
                items.Add(GameInventoryItem.Create(template));
                foreach (var item in items)
    ...
    public class BattlewarderSpellHandler : SpellHandler
    {
        ...
        public override void OnEffectStart(GameSpellEffect effect)
        {
            ...
            if (effect.Owner is GamePlayer)
            {
                var casterPlayer = effect.Owner as GamePlayer;
                if (casterPlayer.GroundTarget != null && casterPlayer.GroundTargetInView)
                {
                    ...
                    GameEventMgr.AddHandler(casterPlayer, GameLivingEvent.CastStarting,
                        new DOLEventHandler(PlayerMoves));
                    GameEventMgr.AddHandler(casterPlayer, GameLivingEvent.AttackFinished,
                        new DOLEventHandler(PlayerMoves));
        ...
        public override int OnEffectExpires(GameSpellEffect effect, bool noMessages)
        {
            ...
            if (effect.Owner is GamePlayer)
            {
                ...
                GameEventMgr.RemoveHandler(casterPlayer, GameLivingEvent.CastStarting,
                    new DOLEventHandler(PlayerMoves));
                GameEventMgr.RemoveHandler(casterPlayer, GameLivingEvent.AttackFinished,
                    new DOLEventHandler(PlayerMoves));
            }
        ...
        {
            var player = sender as GameLiving;
    ...
    public class BrittleGuardSpellHandler : MasterlevelHandling
    {
        ...
        private GameNPC summoned = null;
        private GameSpellEffect beffect = null;
        ...
        public override void ApplyEffectOnTarget(GameLiving target, double effectiveness)
        {
            ...
            beffect = CreateSpellEffect(target, effectiveness);
            {
                ...
                var controlledBrain = new BrittleBrain(player);
        ...
        //public class Convoker9Handler : MasterlevelBuffHandling
    {
        ...
        public override void ApplyEffectOnTarget(GameLiving target, double effectiveness)
        {
            foreach (var jg in target.EffectList.GetAllOfType<JuggernautEffect>())
                if (jg != null)
                {
    ...
    public class Convoker10SpellHandler : MasterlevelHandling
    {
        ...
        private int x, y, z;
        private GameNPC summoned = null;
        private ECSGameTimer m_growTimer;
        ...
        public override void ApplyEffectOnTarget(GameLiving target, double effectiveness)
        {
            ...
            var controlledBrain = new TitanBrain(player);
            controlledBrain.IsMainPet = false;
            ...
            m_growTimer = new ECSGameTimer((GameObject) m_caster, new ECSGameTimer.ECSTimerCallback(TitanGrows),
                C_GROWTIMER);
        }
    ...
    public class TitanBrain : ControlledNpcBrain
    {
        ...
        public override void Think()
        {
            ...
            if (enemies.Count > 0 && Target == null)
            {
                ...
                var targetnum = Util.Random(0, enemies.Count - 1);
