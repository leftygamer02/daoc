...
public class FZSpellHandler : MasterlevelHandling
{
    ...
    public override void OnDirectEffect(GameLiving target, double effectiveness)
    {
        ...
        if (player != null && player.IsAlive) Zephyr(player);
    }
    ...
    private void OnAttack(DOLEvent e, object sender, EventArgs arguments)
    {
        ...
        var spellAbsorbed = (int) (0.01 * absorbPercent * Spell.Damage);
    ...
    private void ArriveAtTarget(DOLEvent e, object obj, EventArgs args)
    {
        ...
        var target = npc.TempProperties.getProperty<object>("target", null) as GamePlayer;
        ...
        player.Out.SendMessage("You are picked up by a forceful zephyr!", eChatType.CT_System,
        ...
        if (Caster is GamePlayer)
        {
            ...
            m_loc = GetTargetLoc();
            (Caster as GamePlayer).Out.SendCheckLOS(Caster as GamePlayer, m_npc,
                new CheckLOSResponse(ZephyrCheckLOS));
        }
    }
...
public class PhaseshiftHandler : MasterlevelHandling
{
    ...
    private void OnAttack(DOLEvent e, object sender, EventArgs arguments)
    {
        ...
        if (ad.Attacker is GamePlayer)
        {
            ad.Damage = 0;
            ad.CriticalDamage = 0;
            var player = ad.Attacker as GamePlayer;
...
public class Groupport : MasterlevelHandling
{
    ...
    public override void OnDirectEffect(GameLiving target, double effectiveness)
    {
        ...
        if (player != null && player.Group != null)
        {
            ...
            {
                player.Out.SendMessage("You can't teleport a group that is in combat!", eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);
                return;
            }
            else
            {
                foreach (var pl in player.Group.GetPlayersInTheGroup())
                    if (pl != null)
                    {
                        ...
                        pl.MoveTo((ushort) player.BindRegion, player.BindXpos, player.BindYpos, player.BindZpos,
