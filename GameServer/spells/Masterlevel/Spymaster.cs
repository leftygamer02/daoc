...
namespace DOL.GS.Spells
{
    ...
    public class SabotageSpellHandler : SpellHandler
    {
        public override void OnDirectEffect(GameLiving target, double effectiveness)
        {
            ...
            {
                var targetFont = target as GameFont;
                targetFont.Delete();
    ...
    public class LoockoutSpellHandler : SpellHandler
    {
        ...
        public override void OnEffectStart(GameSpellEffect effect)
        {
            ...
            if (!m_target.IsAlive || m_target.ObjectState != GameObject.eObjectState.Active ||
        ...
        private void PlayerAction(DOLEvent e, object sender, EventArgs args)
        {
            ...
            var effect = FindEffectOnTarget(m_target, "Loockout");
            if (effect != null) effect.Cancel(false);
            var effect2 = FindStaticEffectOnTarget(Caster, typeof(LoockoutOwner));
    ...
    public class EssenceFlareSpellHandler : SummonItemSpellHandler
    {
        public EssenceFlareSpellHandler(GameLiving caster, Spell spell, SpellLine line)
            : base(caster, spell, line)
        {
            var template = GameServer.Database.FindObjectByKey<ItemTemplate>("Meschgift");
            if (template != null)
            {
    ...
    public class GroupstealthHandler : MasterlevelHandling
    {
        ...
        public override void OnEffectStart(GameSpellEffect effect)
        {
            ...
            {
                var playerTarget = effect.Owner as GamePlayer;
                ...
                if (effect.Owner != Caster)
                {
                    ...
                    GameEventMgr.AddHandler(playerTarget, GameLivingEvent.AttackFinished,
                        new DOLEventHandler(PlayerAction));
                    GameEventMgr.AddHandler(playerTarget, GameLivingEvent.CastStarting,
                        new DOLEventHandler(PlayerAction));
        ...
        public override int OnEffectExpires(GameSpellEffect effect, bool noMessages)
        {
            if (effect.Owner != Caster && effect.Owner is GamePlayer)
            {
                //effect.Owner.BuffBonusCategory1[(int)eProperty.Skill_Stealth] -= 80;
                var playerTarget = effect.Owner as GamePlayer;
                GameEventMgr.RemoveHandler(playerTarget, GameLivingEvent.AttackFinished,
                    new DOLEventHandler(PlayerAction));
                GameEventMgr.RemoveHandler(playerTarget, GameLivingEvent.CastStarting,
                    new DOLEventHandler(PlayerAction));
        ...
        private void PlayerAction(DOLEvent e, object sender, EventArgs args)
        {
            ...
            {
                MessageToLiving((GameLiving) player, "You are attacking. Your camouflage fades!",
                    eChatType.CT_SpellResisted);
                OnEffectExpires(m_effect, true);
                return;
            }
            ...
            if (args is CastingEventArgs)
            {
                if ((args as CastingEventArgs).SpellHandler.Caster != Caster)
                    return;
                MessageToLiving((GameLiving) player, "You are casting a spell. Your camouflage fades!",
                    eChatType.CT_SpellResisted);
                OnEffectExpires(m_effect, true);
                return;
            }
            ...
            {
                MessageToLiving((GameLiving) player, "You are moving. Your camouflage fades!",
                    eChatType.CT_SpellResisted);
                OnEffectExpires(m_effect, true);
                return;
            }
