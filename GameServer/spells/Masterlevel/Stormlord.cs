...
public class StormMissHit : MasterlevelBuffHandling
{
    ...
    public override void ApplyEffectOnTarget(GameLiving target, double effectiveness)
    {
        ...
        if (target is GamePlayer)
            ((GamePlayer) target).Out.SendMessage("You're harder to hit!", eChatType.CT_YouWereHit,
...
public class VacuumVortexSpellHandler : SpellHandler
{
    ...
    public override void OnDirectEffect(GameLiving target, double effectiveness)
    {
        ...
        foreach (GameStorm targetStorm in targets)
            ...
            {
                var targetNPC = targetStorm as GameNPC;
                var range = Util.Random(0, 750);
                var angle = Util.RandomDouble() * 2 * Math.PI;
                targetNPC.WalkTo(targetNPC.X + (int) (range * Math.Cos(angle)),
                    targetNPC.Y + (int) (range * Math.Sin(angle)), targetNPC.Z, targetNPC.MaxSpeed);
            }
    }
...
public class StormDexQuickDebuff : DualStatDebuff
{
    ...
    public override void ApplyEffectOnTarget(GameLiving target, double effectiveness)
    {
        ...
        if (target is GamePlayer)
            ((GamePlayer) target).Out.SendMessage("Your dexterity and quickness decreased!",
                eChatType.CT_YouWereHit, eChatLoc.CL_SystemWindow);
    }
...
public class FocusingWindsSpellHandler : SpellHandler
{
    ...
    public override void OnEffectStart(GameSpellEffect effect)
    {
        ...
        {
            var targetStorm = effect.Owner as GameStorm;
...
public class StormStrConstDebuff : DualStatDebuff
{
    ...
    public override void ApplyEffectOnTarget(GameLiving target, double effectiveness)
    {
        ...
        if (target is GamePlayer)
            ((GamePlayer) target).Out.SendMessage("Your strenght and constitution decreased!",
                eChatType.CT_YouWereHit, eChatLoc.CL_SystemWindow);
    }
...
public class StormAcuityDebuff : SingleStatDebuff
{
    ...
    public override void ApplyEffectOnTarget(GameLiving target, double effectiveness)
    {
        ...
        if (target is GamePlayer)
            ((GamePlayer) target).Out.SendMessage("Your acuity decreased!", eChatType.CT_YouWereHit,
                eChatLoc.CL_SystemWindow);
    }
...
public class StormEnergyTempest : SpellHandler
{
    ...
    {
        if (Spell.Damage < 0) return m_spellTarget.MaxHealth * -Spell.Damage * .01 * 3.0 * effectiveness;
