...
public class SummonMerchantSpellHandler : SpellHandler
{
    ...
    public override void ApplyEffectOnTarget(GameLiving target, double effectiveness)
    {
        ...
        else
        {
            ...
            {
                Npc = new GameAtlasMerchant();
                Npc = (GameAtlasMerchant) Assembly.GetAssembly(typeof(GameServer))
                    .CreateInstance(template.ClassType, false);
            }
            catch (Exception e)
            {
            }
            ...
            if (Npc == null)
                try
                {
                    Npc = (GameAtlasMerchant) Assembly.GetExecutingAssembly()
                        .CreateInstance(template.ClassType, false);
                }
                catch (Exception e)
                {
        ...
        var point = m_caster.GetPointFromHeading(m_caster.Heading, 64);
        Npc.X = point.X;
        Npc.Y = point.Y;
        Npc.Z = m_caster.Z;
