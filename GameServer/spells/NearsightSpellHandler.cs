...
public class NearsightSpellHandler : ImmunityEffectSpellHandler
{
    ...
    {
        //Bonedancer rr5
        if (target.EffectList.GetOfType<AllureofDeathEffect>() != null) return AllureofDeathEffect.nschance;
    ...
    public override IList<string> DelveInfo
    {
        get
        {
            ...
            list.Add(LanguageMgr.GetTranslation((Caster as GamePlayer).Client,
                "NearsightSpellHandler.DelveInfo.Function",
            ...
            if (Spell.Value != 0)
                list.Add(string.Format(LanguageMgr.GetTranslation((Caster as GamePlayer).Client, "DelveInfo.Value",
                    (int) Spell.Value)) + "%");
    ...
    // constructor
    public NearsightSpellHandler(GameLiving caster, Spell spell, SpellLine spellLine) : base(caster, spell,
        spellLine)
    {
    }
}
...
public class NearsightReductionSpellHandler : SpellHandler
{
    ...
    // constructor
    public NearsightReductionSpellHandler(GameLiving caster, Spell spell, SpellLine spellLine) : base(caster, spell,
        spellLine)
    {
    }
}