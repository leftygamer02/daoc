...
public class ArcheryDoTSpellHandler : DoTSpellHandler
{
    ...
    {
        var speclevel = 1;
        if (m_caster is GamePlayer) speclevel = ((GamePlayer) m_caster).GetModifiedSpecLevel(Specs.Archery);

        min = 1.25;
        max = 1.25;

        if (target.Level > 0) min = 0.75 + (speclevel - 1) / (double) target.Level * 0.5;
