...
public class ArrowSpellHandler : SpellHandler
{
    ...
    public override bool StartSpell(GameLiving target)
    {
        ...
        foreach (var targ in SelectTargets(target))
        {
            DealDamage(targ);
            targetCount++;
    ...
    private void DealDamageCheckLOS(GamePlayer player, ushort response, ushort targetOID)
    {
        ...
        {
            var target = (GameLiving) Caster.CurrentRegion.GetObject(targetOID);
            if (target != null)
                DealDamage(target);
        }
    }
    ...
    {
        var ticksToTarget = m_caster.GetDistanceTo(target) * 100 / 85; // 85 units per 1/10s
        var delay = 1 + ticksToTarget / 100;
        foreach (GamePlayer player in target.GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE))
            player.Out.SendSpellEffectAnimation(m_caster, target, m_spell.ClientEffect, (ushort) delay, false, 1);

        var arrow = new ArrowOnTargetAction(Caster, target, this);
    ...
    protected class ArrowOnTargetAction : RegionECSAction
    {
        ...
        {
            var target = m_arrowTarget;
            var caster = (GameLiving) m_actionSource;
            if (target == null || !target.IsAlive || target.ObjectState != GameObject.eObjectState.Active ||
                target.CurrentRegionID != caster.CurrentRegionID) return 0;

            var missrate = 100 - m_handler.CalculateToHitChance(target);
            ...
            var ad = m_handler.CalculateDamageToTarget(target,
                0.5 - caster.GetModified(eProperty.SpellDamage) * 0.01);

            // check for bladeturn miss
            if (ad.AttackResult == eAttackResult.Missed) return 0;
            ...
            var arrowBlock = false;

            if (target is GamePlayer && !target.IsStunned && !target.IsMezzed && !target.IsSitting &&
                m_handler.Spell.LifeDrainReturn != (int) Archery.eShotType.Critical)
            {
                ...
                if (lefthand != null && (player.ActiveWeapon == null ||
                    ...
                    if (target.IsObjectInFront(caster, 180) && lefthand.Object_Type == (int) eObjectType.Shield)
                    {
                        ...
                        var shield = 0.5 * player.GetModifiedSpecLevel(Specs.Shields);
                        var blockchance = (player.Dexterity * 2 - 100) / 40.0 + shield + 0 * 3 + 5;
                        blockchance += 30;
                        ...
                        {
                            var engage =
                                (EngageECSGameEffect) EffectListService.GetEffectOnTarget(target, eEffect.Engage);
                            if (engage != null && target.attackComponent.AttackState &&
                                engage.EngageTarget == caster)
                            {
                                ...
                                if (engage.EngageTarget.LastAttackedByEnemyTick > GameLoop.GameLoopTime -
                                    EngageAbilityHandler.ENGAGE_ATTACK_DELAY_TICK)
                                {
                                    if (engage.Owner is GamePlayer)
                                        (engage.Owner as GamePlayer).Out.SendMessage(
                                            engage.EngageTarget.GetName(0, true) +
                                            " has been attacked recently and you are unable to engage.",
                                ...
                                else
                                {
                                    ...
                                    if (engage.Owner is GamePlayer)
                                        (engage.Owner as GamePlayer).Out.SendMessage(
                                            "You concentrate on blocking the blow!", eChatType.CT_Skill,
                        ...
                        {
                            arrowBlock = true;
                            m_handler.MessageToLiving(player, "You block " + caster.GetName(0, false) + "'s arrow!",
                                eChatType.CT_System);
                            if (m_handler.Spell.Target.ToLower() != "area")
                            {
                                m_handler.MessageToCaster(player.GetName(0, true) + " blocks your arrow!",
                                    eChatType.CT_System);
            ...
            if (arrowBlock == false)
            {
                ...
                var damage = m_handler.Spell.Damage / 2; // another half is physical damage
                ...
                var ws = caster.Level * 8 * (1.0 + (caster.GetModified(eProperty.Dexterity) - 50) / 200.0);

                damage *= (ws + 90.68) / (target.GetArmorAF(ad.ArmorHitLocation) + 20 * 4.67);
                damage *= 1.0 - Math.Min(0.85, ad.Target.GetArmorAbsorb(ad.ArmorHitLocation));
                ad.Modifier = (int) (damage *
                    (ad.Target.GetResist(ad.DamageType) + SkillBase.GetArmorResist(armor, ad.DamageType)) / -100.0);
                damage += ad.Modifier;
                ...
                if (caster.ActiveWeapon != null)
                {
                    ...
                    // Condition
                    ad.Damage = (int) ((double) ad.Damage * Math.Min(1.0,
                        (double) caster.ActiveWeapon.Condition / (double) caster.ActiveWeapon.MaxCondition));
                    ...
                    var spellRequiredDPS = 12 + 3 * m_handler.Spell.Level;

                    if (caster.ActiveWeapon.DPS_AF < spellRequiredDPS)
                    {
                        var percentReduction = (double) caster.ActiveWeapon.DPS_AF / (double) spellRequiredDPS;
                ...
                if (ad.CriticalDamage > 0)
                {
                    ...
                    {
                        var critMax = target is GamePlayer ? ad.Damage / 2 : ad.Damage;
            ...
            {
                var subspell = SkillBase.GetSpellByID(m_handler.Spell.SubSpellID);
                if (subspell != null)
                {
                    subspell.Level = m_handler.Spell.Level;
                    var spellhandler = ScriptMgr.CreateSpellHandler(m_handler.Caster, subspell,
                        SkillBase.GetSpellLine(GlobalSpellsLines.Combat_Styles_Effect));
                    if (spellhandler != null) spellhandler.StartSpell(target);
                }
            }

            if (arrowBlock == false && m_handler.Caster.ActiveWeapon != null &&
                GlobalConstants.IsBowWeapon((eObjectType) m_handler.Caster.ActiveWeapon.Object_Type))
