...
public class Archery : ArrowSpellHandler
{
    ...
    public override bool CheckBeginCast(GameLiving selectedTarget)
    {
        ...
        // Is PS ?
        var Phaseshift = FindEffectOnTarget(Caster, "Phaseshift");
        if (Phaseshift != null &&
            (Spell.InstrumentRequirement == 0 || Spell.SpellType == (byte) eSpellType.Mesmerize))
        {
        ...
        // Is Shield Disarm ?
        var shieldDisarm = Caster.EffectList.GetOfType<ShieldTripDisarmEffect>();
        ...
        if (m_spell.RecastDelay > 0)
        {
            ...
            if (left > 0)
            {
                MessageToCaster("You must wait " + (left / 1000 + 1).ToString() + " seconds to use this spell!",
                    eChatType.CT_System);
                return false;
            }
        }
        ...
        if (Caster != null && Caster is GamePlayer && Caster.ActiveWeapon != null &&
            GlobalConstants.IsBowWeapon((eObjectType) Caster.ActiveWeapon.Object_Type))
        {
            if (Spell.LifeDrainReturn == (int) eShotType.Critical && !Caster.IsStealthed)
            {
                MessageToCaster("You must be stealthed and wielding a bow to use this ability!",
                    eChatType.CT_SpellResisted);
                return false;
            }
    ...
    {
        var bonustohit = Caster.GetModified(eProperty.ToHitBonus);

        // miss rate is 0 on same level opponent
        var hitchance = 100 + bonustohit;
    ...
    public override int AdjustDamageForHitChance(int damage, int hitChance)
    {
        ...
        if (hitChance < 85) adjustedDamage += (int) (adjustedDamage * (hitChance - 85) * 0.038);
    ...
    {
        var ad = base.CalculateDamageToTarget(target, effectiveness);
        GamePlayer player;
            ...
            switch (Spell.LifeDrainReturn)
            {
                case (int) eShotType.Critical:
                {
                    if (target is GamePlayer)
                    {
                        ...
                        player.Out.SendMessage("A shot penetrated your magic barrier!", eChatType.CT_SpellResisted,
                ...
                default:
                {
                    if (Caster is GamePlayer)
                    {
                        ...
                        player.Out.SendMessage("Your strike was absorbed by a magical barrier!",
                    ...
                    {
                        player = target as GamePlayer;
                        player.Out.SendMessage("The blow was absorbed by a magical barrier!",
                            eChatType.CT_SpellResisted, eChatLoc.CL_SystemWindow);
        ...
        if (ad.AttackResult != eAttackResult.Missed)
        {
            ...
            if (Spell.Target.ToLower() == "area") ad.Damage = (int) (ad.Damage * 0.815);
        }
    ...
    {
        var ef = FindEffectOnTarget(Caster, "ArrowDamageTypes");
        if (ef != null)
    ...
    public override void FinishSpellCast(GameLiving target)
    {
        ...
        if (Caster is GamePlayer && Caster.IsStealthed) (Caster as GamePlayer).Stealth(false);
        ...
        if (Spell.Target.ToLower() == "area")
        {
            ...
            foreach (GameLiving npc in WorldMgr.GetNPCsCloseToSpot(Caster.CurrentRegionID, Caster.GroundTarget.X,
                         Caster.GroundTarget.Y, Caster.GroundTarget.Z, (ushort) Spell.Radius))
    ...
    public override int CalculateCastingTime()
    {
        ...
        var ticks = m_spell.CastTime;

        var percent = 1.0;
        var dex = Caster.GetModified(eProperty.Dexterity);
        ...
        var player = m_caster as GamePlayer;

        if (player != null) percent *= 1.0 - m_caster.GetModified(eProperty.CastingSpeed) * 0.01;
    ...
    public override bool CasterIsAttacked(GameLiving attacker)
    {
        ...
        {
            var mod = Caster.GetConLevel(attacker);
            double chance = 65;
            chance += mod * 10;
            ...
            {
                Caster.TempProperties.setProperty(INTERRUPT_TIMEOUT_PROPERTY,
                    GameLoop.GameLoopTime + Caster.SpellInterruptDuration);
                MessageToLiving(Caster, attacker.GetName(0, true) + " attacks you and your shot is interrupted!",
                    eChatType.CT_SpellResisted);
                InterruptCasting();
                return true;
            }
    ...
    public override IList<string> DelveInfo
    {
        get
        {
            ...
            if (Spell.Duration >= ushort.MaxValue * 1000)
                ...
                list.Add(string.Format("Duration: {0}:{1} min", Spell.Duration / 60000,
                    (Spell.Duration % 60000 / 1000).ToString("00")));
            ...
            if (Spell.RecastDelay > 60000)
                list.Add("Recast time: " + (Spell.RecastDelay / 60000).ToString() + ":" +
