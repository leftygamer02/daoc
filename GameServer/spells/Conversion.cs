...
public class ConversionSpellHandler : SpellHandler
{
    ...
    public override void OnEffectStart(GameSpellEffect effect)
    {
        ...
        var toLiving = Spell.Pulse == 0 ? eChatType.CT_Spell : eChatType.CT_SpellPulse;
        var toOther = Spell.Pulse == 0 ? eChatType.CT_System : eChatType.CT_Spell;
    ...
    private void OnAttack(DOLEvent e, object sender, EventArgs arguments)
    {
        ...
        var reduceddmg = living.TempProperties.getProperty<int>(ConvertDamage);
        var absorbPercent = Spell.Damage;
        var damageConverted = (int) (0.01 * absorbPercent * (ad.Damage + ad.CriticalDamage));
        ...
        if (ad.Damage > 0)
            MessageToLiving(ad.Target,
                string.Format("You convert {0} damage into " + damageConverted + " Health.", damageConverted),
                eChatType.CT_Spell);
        MessageToLiving(ad.Attacker,
            string.Format("A magical spell absorbs {0} damage of your attack!", damageConverted),
            eChatType.CT_Spell);
    ...
    private void OnAttack(DOLEvent e, object sender, EventArgs arguments)
    {
        ...
        if (ad.Damage > 0)
            switch (attackedByEnemy.AttackData.AttackType)
            {
                ...
                {
                    var reduceddmg = living.TempProperties.getProperty<int>(ConvertDamage, 0);
                    var absorbPercent = Spell.Damage;
                    var damageConverted = (int) (0.01 * absorbPercent * (ad.Damage + ad.CriticalDamage));
                    if (damageConverted > reduceddmg)
                    {
                    ...
                    MessageToLiving(ad.Target,
                        string.Format("You convert {0} damage into " + damageConverted + " Health.",
                            damageConverted), eChatType.CT_Spell);
                    MessageToLiving(ad.Attacker,
                        string.Format("A magical spell absorbs {0} damage of your attack!", damageConverted),
                        eChatType.CT_Spell);
                    if (Caster.Health != Caster.MaxHealth)
                    {
                        MessageToCaster("You convert " + damageConverted + " damage into health.",
                            eChatType.CT_Spell);
                        Caster.Health = Caster.Health + damageConverted;
                    }
                    else
                    ...
                    if (Caster.Endurance != Caster.MaxEndurance)
                    {
                        MessageToCaster("You convert " + damageConverted + " damage into endurance",
                            eChatType.CT_Spell);
                        Caster.Endurance = Caster.Endurance + damageConverted;
                    }
                    else
                    ...
                    if (Caster.Mana != Caster.MaxMana)
                    {
                        MessageToCaster("You convert " + damageConverted + " damage into power.",
                            eChatType.CT_Spell);
                        Caster.Mana = Caster.Mana + damageConverted;
                    }
                    else
