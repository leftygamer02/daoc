...
public class ChamberSpellHandler : SpellHandler
{
    ...
    {
        var caster = (GamePlayer) m_caster;
        m_spellTarget = caster.TargetObject as GameLiving;
        var effect = FindEffectOnTarget(caster, "Chamber", m_spell.Name);
        if (effect != null && m_spell.Name == effect.Spell.Name)
        {
            ...
            var PhaseShift = FindEffectOnTarget(m_spellTarget, "Phaseshift");
            ...
            var duration = caster.GetSkillDisabledDuration(m_spell);
            if (duration > 0)
            {
                MessageToCaster("You must wait " + (duration / 1000 + 1) + " seconds to use this spell!",
                    eChatType.CT_System);
                return false;
            }
            ...
            if (PhaseShift != null)
            {
                MessageToCaster(m_spellTarget.Name + " is Phaseshifted and can't be attacked!",
                    eChatType.CT_System);
                return false;
            }
                ...
                if (EffectOwner == m_spellTarget)
                {
                    if (m_caster is GamePlayer)
                        ((GamePlayer) m_caster).Out.SendMessage(
                            string.Format("{0} is invisible to you!", m_spellTarget.GetName(0, true)),
            ...
            if (GameServer.ServerRules.IsAllowedToAttack(Caster, m_spellTarget, true) &&
                chamber.PrimarySpell.Target.ToLower() == "realm")
            {
            ...
            {
                spellhandler2 =
                    ScriptMgr.CreateSpellHandler(caster, chamber.SecondarySpell, chamber.SecondarySpellLine);
            ...
            if (m_caster is GamePlayer)
            {
                var player_Caster = Caster as GamePlayer;
                foreach (var spellline in player_Caster.GetSpellLines())
                foreach (var sp in SkillBase.GetSpellList(spellline.KeyName))
        ...
        else
        {
            ...
            if (Caster is GamePlayer && duration == 0)
                ((GamePlayer) Caster).Out.SendMessage("Select the first spell for your " + Spell.Name + ".",
    ...
    public override void FinishSpellCast(GameLiving target)
    {
        ...
        if (Spell.InstrumentRequirement == 0)
        {
            ...
            else
            {
                ...
                var neweffect = CreateSpellEffect(target, 1);
            ...
            foreach (GamePlayer player in m_caster.GetPlayersInRadius(WorldMgr.INFO_DISTANCE))
                if (player != m_caster)
                    player.MessageFromArea(m_caster, m_caster.GetName(0, true) + " casts a spell!",
        ...
        if (m_caster is GamePlayer)
        {
            ...
            if (quickcast != null && Spell.CastTime > 0)
            {
                ...
                m_caster.DisableSkill(SkillBase.GetAbility(Abilities.Quickcast),
