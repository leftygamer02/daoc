...
public class LifeTransferSpellHandler : SpellHandler
{
    ...
    public virtual bool HealTarget(GameLiving target, int amount)
    {
        ...
        var heal = target.ChangeHealth(Caster, eHealthChangeType.Spell, amount);
        ...
        {
            var joueur_a_considerer = m_caster is NecromancerPet
                ? ((m_caster as NecromancerPet).Brain as IControlledBrain).GetPlayerOwner()
                : m_caster as GamePlayer;

            var POURCENTAGE_SOIN_RP =
                ServerProperties.Properties.HEAL_PVP_DAMAGE_VALUE_RP; // ...% de bonus RP pour les soins effectués
            var Bonus_RP_Soin = Convert.ToInt64((double) healedrp * POURCENTAGE_SOIN_RP / 100);

            if (Bonus_RP_Soin >= 1)
            {
                var stats = joueur_a_considerer.Statistics as PlayerStatistics;
                ...
                joueur_a_considerer.Out.SendMessage(
                    "You gain " + Bonus_RP_Soin.ToString() + " realmpoints for healing a member of your Realm",
        ...
        MessageToCaster("You heal " + target.GetName(0, false) + " for " + heal + " hit points!",
            eChatType.CT_Spell);
        MessageToLiving(target, "You are healed by " + m_caster.GetName(0, false) + " for " + heal + " hit points.",
            eChatType.CT_Spell);
        if (heal < amount)
