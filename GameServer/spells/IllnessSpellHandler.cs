...
public class PveResurrectionIllness : AbstractIllnessSpellHandler
{
    ...
    {
        var targetPlayer = m_spellTarget as GamePlayer;
        if (targetPlayer != null)
        {
            // Higher level rez spells reduce duration of rez sick.
            if (targetPlayer.TempProperties.getAllProperties()
                .Contains(GamePlayer.RESURRECT_REZ_SICK_EFFECTIVENESS))
            {
                var rezSickEffectiveness =
                    targetPlayer.TempProperties.getProperty<double>(GamePlayer.RESURRECT_REZ_SICK_EFFECTIVENESS);
            ...
            if (targetPlayer.GetModified(eProperty.ResIllnessReduction) > 0)
                initParams.Duration = initParams.Duration *
                    (100 - targetPlayer.GetModified(eProperty.ResIllnessReduction)) / 100;
        }
    ...
    public PveResurrectionIllness(GameLiving caster, Spell spell, SpellLine spellLine) : base(caster, spell,
        spellLine)
    {
    }
}
...
public class AbstractIllnessSpellHandler : SpellHandler
{
    ...
    {
        var modifier = 1.0;
        var ab = target.GetAbility<RealmAbilities.VeilRecoveryAbility>();
        if (ab != null)
    ...
    public AbstractIllnessSpellHandler(GameLiving caster, Spell spell, SpellLine spellLine) : base(caster, spell,
        spellLine)
    {
    }
}