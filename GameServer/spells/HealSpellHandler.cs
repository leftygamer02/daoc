...
public class HealSpellHandler : SpellHandler
{
    ...
    public override bool StartSpell(GameLiving target)
    {
        ...
        foreach (var healTarget in targets)
        {
            var heal = Util.Random(minHeal, maxHeal);

            if (SpellLine.KeyName == GlobalSpellsLines.Item_Effects) heal = maxHeal;
    ...
    {
        if (target == null || target.ObjectState != GameObject.eObjectState.Active) return false;
...
//             }
        ...
        var criticalchance = Caster.GetModified(eProperty.CriticalHealHitChance);
        double effectiveness = 0;
        if (Caster is GamePlayer)
            effectiveness = (Caster as GamePlayer).Effectiveness +
                            (double) Caster.GetModified(eProperty.HealingEffectiveness) * 0.01;
        if (Caster is GameNPC)
            effectiveness = 1.0;

        //USE DOUBLE !
        var cache = amount * effectiveness;
        ...
        {
            spellCaster.Out.SendMessage($"heal crit chance: {criticalchance} random: {randNum}",
                eChatType.CT_DamageAdd, eChatLoc.CL_SystemWindow);
            spellCaster.Out.SendMessage(
                $"heal effectiveness: {Caster.GetModified(eProperty.HealingEffectiveness)}%",
                eChatType.CT_DamageAdd, eChatLoc.CL_SystemWindow);
        }

        if (criticalchance > randNum)
        {
            var minValue = amount / 10;
            var maxValue = amount / 2 + 1;
            criticalvalue = Util.RandomDouble() * (maxValue - minValue) + minValue;
            criticalvalue =
                Math.Round(criticalvalue,
                    MidpointRounding
                        .ToEven); /// [Atlas - Takii] Round crit value for simplicity and to remove decimals from combat log.
        }
        ...
        var heal = target.ChangeHealth(Caster, eHealthChangeType.Spell, (int) amount);
        ...
        {
            MessageToCaster("You heal " + target.GetName(0, false) + " for " + heal + " hit points!",
                eChatType.CT_Spell);
            MessageToLiving(target,
                "You are healed by " + m_caster.GetName(0, false) + " for " + heal + " hit points.",
                eChatType.CT_Spell);
            if (heal < amount)
            {
                #region PVP DAMAGE
            ...
            if (heal > 0 && criticalvalue > 0)
                MessageToCaster("Your heal criticals for an extra " + criticalvalue + " hit points!",
                    eChatType.CT_Spell);
        }
        ...
        foreach (var gain in attackers.ToList())
            if (gain is GameNPC npc)
            {
                ...
                if (npc.Brain is StandardMobBrain mobBrain) mobBrain.AddToAggroList(Caster, ad.Damage);
    ...
    public virtual bool ProcHeal(GameLiving target, double amount)
    {
        ...
        var heal = target.ChangeHealth(Caster, eHealthChangeType.Spell, (int) Math.Round(amount));
    ...
    {
        var spellValue = m_spell.Value;
        var casterPlayer = m_caster as GamePlayer;
        ...
        var upperLimit = (int) (spellValue * 1.25);
        ...
        var lowerLimit = (int) (spellValue * eff);
