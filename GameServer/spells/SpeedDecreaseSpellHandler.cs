...
public class SpeedDecreaseSpellHandler : UnbreakableSpeedDecreaseSpellHandler
{
    ...
    {
        if (crit)
            initParams.Effectiveness *=
                2; //critical hit effectiveness needs to be set after duration is calculated to prevent double duration
    ...
    public override void ApplyEffectOnTarget(GameLiving target, double effectiveness)
    {
        ...
        if (Spell.Value == 99 && (target.effectListComponent.Effects.ContainsKey(eEffect.SnareImmunity) ||
                                  target.effectListComponent.Effects.ContainsKey(eEffect.SpeedOfSound)))
        ...
        if (criticalChance > 0)
        {
            ...
            if (playerCaster?.UseDetailedCombatLog == true && critCap > 0)
                playerCaster.Out.SendMessage($"Debuff crit chance: {critCap} random: {randNum}",
                    eChatType.CT_DamageAdd, eChatLoc.CL_SystemWindow);

            if (critCap > randNum)
            {
                crit = true;
                playerCaster?.Out.SendMessage($"Your snare is doubly effective!", eChatType.CT_YouHit,
    ...
    public override void OnEffectStart(GameSpellEffect effect)
    {
        ...
        if (effect.Owner.EffectList.GetOfType<ChargeEffect>() != null ||
            effect.Owner.effectListComponent.Effects.ContainsKey(eEffect.SpeedOfSound) ||
            effect.Owner.TempProperties.getProperty("Charging", false))
        {
            MessageToCaster(effect.Owner.Name + " is moving too fast for this spell to have any effect!",
        ...
        var mezz = EffectListService.GetEffectOnTarget(effect.Owner, eEffect.Mez);
        if (mezz != null)
