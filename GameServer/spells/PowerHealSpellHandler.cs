...
public class PowerHealSpellHandler : SpellHandler
{
    ...
    public virtual bool HealTarget(GameLiving target, int amount)
    {
        ...
        var heal = target.ChangeMana(Caster, eManaChangeType.Spell, amount);
        ...
        {
            MessageToCaster("You restore " + target.GetName(0, false) + " for " + heal + " power points!",
                eChatType.CT_Spell);
            MessageToLiving(target,
                "Your power was restored by " + m_caster.GetName(0, false) + " for " + heal + " points.",
                eChatType.CT_Spell);
            if (heal < amount)
