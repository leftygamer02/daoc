...
public class LanguageMgr
{
    ...
    {
        soleInstance = testDouble;
    }

    protected virtual bool TryGetTranslationImpl(out string translation, ref string language, string translationId,
        ref object[] args)
    {
        translation = "";
        ...
        if (Util.IsEmpty(language) || !m_translations.ContainsKey(language)) language = DefaultLanguage;

        var result = GetLanguageDataObject(language, translationId,
            LanguageDataObject.eTranslationIdentifier.eSystem);
        if (result == null)
        {
        ...
        {
            log.ErrorFormat(
                "[Language-Manager] Parameter number incorrect: {0} for language {1}, Arg count = {2}, sentence = '{3}', args[0] = '{4}'",
    ...
    /// </summary>
    private static
        IDictionary<string, IDictionary<LanguageDataObject.eTranslationIdentifier, IList<LanguageDataObject>>>
        m_translations;
    ...
    private static string LangPath
    {
        get
        {
            if (soleInstance.LangPathImpl == "")
                soleInstance.LangPathImpl =
                    Path.Combine(GameServer.Instance.Configuration.RootDirectory, "languages");
    ...
    /// </summary>
    public static string DefaultLanguage => GS.ServerProperties.Properties.SERV_LANGUAGE; // EN by default.
    ...
    public static IEnumerable<string> Languages
    {
        get
        {
            foreach (var language in m_translations.Keys)
    ...
    public static bool Init()
    {
        m_translations =
            new Dictionary<string,
                IDictionary<LanguageDataObject.eTranslationIdentifier, IList<LanguageDataObject>>>();
    ...
    private static bool LoadTranslations()
    {
        ...
        var fileSentences = new ArrayList();
        var defaultLanguageDirectoryFound = false;
        var defaultLanguageFilesFound = false;
        foreach (var langDir in Directory.GetDirectories(LangPath, "*", SearchOption.TopDirectoryOnly))
        {
            var language = langDir.Substring(langDir.LastIndexOf(Path.DirectorySeparatorChar) + 1).ToUpper();
            if (language != DefaultLanguage)
            {
                if (language !=
                    "CU") // Ignore the custom language folder. This check should be removed in the future! (code written: may 2012)
                    fileSentences.AddRange(ReadLanguageDirectory(Path.Combine(LangPath, language), language));
            }
            else
            {
                defaultLanguageDirectoryFound = true;
                var sentences = ReadLanguageDirectory(Path.Combine(LangPath, language), language);
        ...
        {
            log.Error("Could not find default '" + DefaultLanguage +
                      "' language directory, server can't start without it!");
            return false;
        }

        if (!defaultLanguageFilesFound)
        {
            log.Error("Default '" + DefaultLanguage +
                      "' language files missing, server can't start without those files!");
            return false;
        }

        if (GS.ServerProperties.Properties.USE_DBLANGUAGE)
        {
            var newEntries = 0;
            var updatedEntries = 0;

            var dbos = GameServer.Database.SelectAllObjects<DBLanguageSystem>();
                ...
                foreach (string[] sentence in fileSentences)
                {
                    ...
                    foreach (var dbo in dbos)
                    {
                        ...
                        if (dbo.Text != sentence[TEXT])
                        {
                            ...
                            GameServer.Database
                                .SaveObject(
                                    dbo); // Please be sure to use the UTF-8 format for your language files, otherwise
                            // some database rows will be updated on each server start, because one char
                            // differs from the one within the database.
                            updatedEntries++;

                            if (log.IsWarnEnabled)
                                log.Warn("[Language-Manager] Language <" + sentence[LANGUAGE] +
                                         "> TranslationId <" + dbo.TranslationId + "> updated in database!");
                        }
                    ...
                    if (!found)
                    {
                        ...
                        if (log.IsWarnEnabled)
                            log.Warn("[Language-Manager] Language <" + dbo.Language + "> TranslationId <" +
                                     dbo.TranslationId + "> added into the database.");
                    }
                }
        ...
        Util.AddRange(lngObjs,
            (IList<LanguageDataObject>) GameServer.Database.SelectAllObjects<DBLanguageGameObject>());
    ...
    {
        var count = 0;
        var langPath = Path.Combine(LangPath, language);
    ...
    {
        var sentences = new ArrayList();
        foreach (var languageFile in Directory.GetFiles(path, "*", SearchOption.AllDirectories))
        {
            if (!languageFile.EndsWith(".txt"))
                continue;

            var lines = File.ReadAllLines(languageFile, Encoding.GetEncoding("utf-8"));
    ...
    public static LanguageDataObject GetLanguageDataObject(string language, string translationId,
        LanguageDataObject.eTranslationIdentifier translationIdentifier)
    {
        ...
        foreach (var colObj in m_translations[language][translationIdentifier])
        {
    ...
    public static bool TryGetTranslation(out LanguageDataObject translation, GameClient client,
        ITranslatableObject obj)
    {
        if (client == null)
        {
            translation = null;
        ...
        return TryGetTranslation(out translation, client.Account == null ? string.Empty : client.Account.Language,
            obj);
    }
        ...
        params object[] args)
    {
        ...
        var result = TryGetTranslation(out translation,
            client.Account == null ? DefaultLanguage : client.Account.Language, translationId, args);

        if (client.Account != null)
            if (client.Account.PrivLevel > 1 && client.Player != null && result)
                if (client.ClientState == GameClient.eClientState.Playing)
                {
                    var debug = client.Player.TempProperties.getProperty("LANGUAGEMGR-DEBUG", false);
                    if (debug)
    ...
    public static string GetTranslationErrorText(string lang, string TranslationID)
    {
        try
        {
            if (TranslationID.Contains(".") && TranslationID.TrimEnd().EndsWith(".") == false &&
                TranslationID.StartsWith("'") == false)
        ...
        params object[] args)
    {
        ...
        string retval;
        if (TryGetTranslation(out retval, player.Client.Account.Language, translationId, args)) return retval;
    ...
    public static bool RegisterLanguageDataObject(LanguageDataObject obj)
    {
        if (obj != null)
            lock (m_translations)
            {
                ...
                {
                    IDictionary<LanguageDataObject.eTranslationIdentifier, IList<LanguageDataObject>> col =
                        new Dictionary<LanguageDataObject.eTranslationIdentifier, IList<LanguageDataObject>>();
