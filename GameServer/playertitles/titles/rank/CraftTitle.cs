...
public class CraftTitle : SimplePlayerTitle
{
    ...
    public override string GetValue(GamePlayer source, GamePlayer player)
    {
        if (player.CraftingPrimarySkill == eCraftingSkill.NoCrafting ||
            !player.CraftingSkills.ContainsKey(player.CraftingPrimarySkill))
            return string.Format(LanguageMgr.TryTranslateOrDefault(source, "!BasicCrafting!",
                "Crafting.Name.BasicCrafting"));
    ...
    {
        if (player.CraftingPrimarySkill != eCraftingSkill.NoCrafting) return true;
