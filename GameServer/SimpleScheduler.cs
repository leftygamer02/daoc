...
public class SimpleScheduler
{
    ...
    /// </summary>
    public static long Ticks => Stopwatch.GetTimestamp() / (Stopwatch.Frequency / 1000);
    ...
    private void ConsumePendingScheduledTasks()
    {
        ...
        foreach (var tasks in ScheduledTimers.Where(ent => ent.Key <= Ticks))
        {
            StartedTicksIndex.Add(tasks.Key);
            foreach (var taskEntry in tasks.Value)
            {
    ...
    private void LaunchScheduledTask(ScheduledTask task)
    {
        ...
        if (log.IsWarnEnabled)
        {
            ...
            if (runningTime > 500)
                log.WarnFormat("Simple Scheduler Task (interval:{1}) execution took {0}ms! ({2}.{3})", runningTime,
                    delay, task.Method.GetMethodInfo().ReflectedType, task.Method.GetMethodInfo().Name);
        }
    ...
    private void SignalNextDueTick(long value)
    {
        ...
        } while (System.Threading.Interlocked.CompareExchange(ref NextDueTick, value, initialValue) !=
                 initialValue);
