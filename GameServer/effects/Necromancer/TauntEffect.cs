...
internal class TauntEffect : StaticEffect, IGameEffect
{
    ...
    public override void Start(GameLiving target)
    {
        ...
        foreach (GamePlayer player in target.GetPlayersInRadius(WorldMgr.INFO_DISTANCE))
        {
            ...
            var chatType = player != null && player == petOwner
                ? eChatType.CT_Spell
                : eChatType.CT_System;

            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language, "Effects.Necro.TauntEffect.SeemsChange",
