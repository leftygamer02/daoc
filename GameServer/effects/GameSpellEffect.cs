...
public class GameSpellEffect : IGameEffect, IConcentrationEffect
{
    ...
    /// </summary>
    private readonly object m_LockObject = new(); // dummy object for thread sync - Mannen
    ...
    /// </summary>
    public bool ImmunityState => IsExpired && m_timer != null && m_timer.IsAlive;
    ...
    public GameSpellEffect(ISpellHandler handler, int duration, int pulseFreq) : this(handler, duration, pulseFreq,
        1)
    {
    }
            ...
            lock (m_LockObject)
            {
                ...
                catch (Exception ex)
                {
                    if (log.IsWarnEnabled)
                        log.WarnFormat(
                            "Effect ({0}) Could not Begin Change in living - {1} - Spell Effect List, {2}", this,
                            Owner, ex);
                    commitChange = false;
                }
                ...
                if (!Owner.EffectList.Add(this))
                {
                    if (log.IsWarnEnabled)
                        log.WarnFormat(
                            "{0}: effect was not added to the effects list, not starting it either. (effect class:{1} spell type:{2} spell name:'{3}')",
                            Owner.Name, GetType().FullName, Spell.SpellType, Name);
                    return;
                }

                // Add concentration Effect To Caster List.
                if (Concentration > 0 && SpellHandler != null && SpellHandler.Caster != null &&
                    SpellHandler.Caster.effectListComponent.ConcentrationEffects != null)
            ...
            if (playerCanceled && ((SpellHandler != null && !SpellHandler.HasPositiveEffect) || ImmunityState))
            {
                ...
                if (player != null)
                    player.Out.SendMessage(
                        LanguageMgr.GetTranslation((Owner as GamePlayer).Client,
                            "Effects.GameSpellEffect.CantRemoveEffect"), eChatType.CT_System,
    ...
    protected virtual void ReplaceEffect(GameSpellEffect effect, bool enable)
    {
        ...
        {
            if (log.IsWarnEnabled)
                log.WarnFormat("{0} is trying to overwrite {1},  which has concentration {2}", effect.Name, Name,
                    Concentration);
            return;
        }
    ...
    {
        if (RestoredEffect && RemainingTime > 5000)
        {
        ...
        if (m_handler != null)
        {
            var eff = m_handler.GetSavedEffect(this);
            return eff;
        }
    ...
    public override string ToString()
    {
        return string.Format(
            ...
            Duration, Owner == null ? "(null)" : Owner.Name, PulseFreq, RemainingTime, Effectiveness, m_expired,
            SpellHandler == null ? "(null)" : SpellHandler.ToString());
    }
    ...
    protected sealed class PulsingEffectTimer : GameTimer
    {
        ...
        /// </summary>
        public int TimeSinceStart => IsAlive ? m_timeSinceStart - TimeUntilElapsed : 0;

        /// <summary>
        /// Get tick when effect Last Started.
        /// </summary>
        public long LastStartedTick { get; private set; }
