...
public class StagEffect : TimedEffect, IGameEffect
{
    ...
    */
    ...
    public override void Start(GameLiving living)
    {
        ...
        var m_amountPercent = (m_level + 0.5 + Util.RandomDouble()) / 10; //+-5% random
        if (living is GamePlayer)
            m_amount = (int) ((living as GamePlayer).CalculateMaxHealth(living.Level,
                living.GetModified(eProperty.Constitution)) * m_amountPercent);
        ...
        if (living is GamePlayer)
        {
            ...
            (living as GamePlayer).Out.SendMessage(
                LanguageMgr.GetTranslation((living as GamePlayer).Client, "Effects.StagEffect.HuntsSpiritChannel"),
        ...
        if (m_owner is GamePlayer)
        {
            ...
            (m_owner as GamePlayer).Out.SendMessage(
                LanguageMgr.GetTranslation((m_owner as GamePlayer).Client,
                    "Effects.StagEffect.YourHuntsSpiritEnds"), eChatType.CT_YouHit, eChatLoc.CL_SystemWindow);
        }
    }
    ...
    /// Name of the effect
    /// </summary>
    public override string Name => LanguageMgr.GetTranslation(((GamePlayer) Owner).Client, "Effects.StagEffect.Name");

    /// <summary>
    /// Icon to show on players, can be id
    /// </summary>
    public override ushort Icon => 480;
