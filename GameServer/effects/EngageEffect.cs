...
public class EngageEffect : StaticEffect, IGameEffect
{
    ...
    /// </summary>
    private GameLiving m_engageTarget;
    ...
    public override void Start(GameLiving engageSource)
    {
        ...
        if (m_owner is GamePlayer)
            (m_owner as GamePlayer).Out.SendMessage(
                LanguageMgr.GetTranslation((m_owner as GamePlayer).Client, "Effects.EngageEffect.ConcOnBlockingX",
                    m_engageTarget.GetName(0, false)), eChatType.CT_System, eChatLoc.CL_SystemWindow);
        /*
            ...
            if (playerCancel)
                (m_owner as GamePlayer).Out.SendMessage(
                    LanguageMgr.GetTranslation((m_owner as GamePlayer).Client,
                        "Effects.EngageEffect.YouNoConcOnBlock"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
            else
                (m_owner as GamePlayer).Out.SendMessage(
                    LanguageMgr.GetTranslation((m_owner as GamePlayer).Client,
                        "Effects.EngageEffect.YouNoAttemptToEngageT"), eChatType.CT_System,
    ...
    public override string Name
    {
        get
        {
            if (m_engageTarget != null)
                return LanguageMgr.GetTranslation(((GamePlayer) Owner).Client, "Effects.EngageEffect.EngageName",
                    m_engageTarget.GetName(0, false));
    ...
    public override IList<string> DelveInfo
    {
        get
        {
            ...
            delveInfoList.Add(LanguageMgr.GetTranslation(((GamePlayer) Owner).Client,
                "Effects.EngageEffect.InfoEffect"));
            delveInfoList.Add(" ");
            delveInfoList.Add(LanguageMgr.GetTranslation(((GamePlayer) Owner).Client,
                "Effects.EngageEffect.XEngagesY", m_owner.GetName(0, true), m_engageTarget.GetName(0, false)));
