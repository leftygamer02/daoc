...
public class AtlasOF_SiegeBolt : TimedRealmAbility, ISpellCastingAbilityHandler
{
    ...
    private const int
        m_dmgValue =
            65000; //from thread: you can ONE SHOT miles gates, keep doors at level 1 ( just take keep by another realm for exemple ) to level 2 doors, OS ram and other siege equipement ...
    ...
    public override void Execute(GameLiving living)
    {
        ...
        if (m_caster == null || m_caster.castingComponent == null)
            return;
        var m_target = m_caster.TargetObject as GameObject;
        if (m_target == null)
            return;
        if (m_target is not GameSiegeWeapon && m_target is not GameKeepDoor)
        {
            m_caster.Out.SendMessage("You can only cast this spell on keep doors and siege weapons.",
