...
namespace DOL.GS.RealmAbilities
{
	...
	public class AtlasOF_Volley : TimedRealmAbility
	{
		...
		public override void Execute(GameLiving living)
		{
			...
			{
				player.Out.SendMessage("You need a ground target to use this ability.", eChatType.CT_YouHit, eChatLoc.CL_SystemWindow);
				return;
			}
			if (player.GetDistanceTo(player.GroundTarget) > player.attackComponent.AttackRange)
			{
				player.Out.SendMessage("Your ground target is out of range.", eChatType.CT_YouHit, eChatLoc.CL_SystemWindow);
				return;
			}	
			if (player.GetDistanceTo(player.GroundTarget) < player.attackComponent.AttackRange * .66)
			{
				player.Out.SendMessage("Your ground target must be at least 2/3rds of your attack range.", eChatType.CT_YouHit, eChatLoc.CL_SystemWindow);
				return;
			}	
			...
			{
				if (player.rangeAttackComponent.RangedAttackType == eRangedAttackType.Volley)
				{
			...
			player.rangeAttackComponent.RangedAttackType = eRangedAttackType.Volley;
