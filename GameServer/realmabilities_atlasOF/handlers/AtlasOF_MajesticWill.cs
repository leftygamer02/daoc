...
public class AtlasOF_MajesticWill : TimedRealmAbility
{
    ...
    private int m_duration = 60000; // 30s
    ...
    public override void Execute(GameLiving living)
    {
        ...
        new AtlasOF_MajesticWillECSEffect(new ECSGameEffectInitParams(player, m_duration, Level,
            CreateSpell(living)));
    }

    private SpellHandler CreateSpell(GameLiving owner)
    {
        var tmpSpell = new DBSpell();
        ...
        tmpSpell.Description =
            "Your targets chance of resisting your spells is reduced by 5% per level per level of this ability for 60 seconds.";
        var spellLine = new SpellLine("RAs", "RealmAbilities", "RealmAbilities", true);
        var
            tmpHandler =
                new SpellHandler(owner, new Spell(tmpSpell, 0),
                    spellLine); // make spell level 0 so it bypasses the spec level adjustment code
        return tmpHandler;
    }
