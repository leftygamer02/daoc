...
public class AtlasOF_ForestheartAmbusher : TimedRealmAbility
{
    ...
    public override void Execute(GameLiving living)
    {
        ...
        var rgn = WorldMgr.GetRegion(_caster.CurrentRegion.ID);

        if (rgn?.GetZone(_caster.GroundTarget.X, _caster.GroundTarget.Y) == null)
        {
            _caster.MessageToSelf(
                LanguageMgr.GetTranslation(_caster.Client, "SummonAnimistFnF.CheckBeginCast.NoGroundTarget"),
        ...
        {
            _caster.MessageToSelf(
                LanguageMgr.GetTranslation(_caster.Client, "SummonAnimistPet.CheckBeginCast.GroundTargetNotInView"),
                eChatType.CT_SpellResisted);
            return;
        }
        ...
        {
            _caster.MessageToSelf(
                LanguageMgr.GetTranslation(_caster.Client,
                    "SummonAnimistPet.CheckBeginCast.GroundTargetNotInSpellRange"), eChatType.CT_SpellResisted);
            return;
        }
    ...
    public override IList<string> DelveInfo
    {
        get
        {
            ...
            delveInfoList.Add(
                "Summons ground-targetted 100% pet for 3 minutes. Automatically acquires targets that enter its aggression radius.");
