...
public class AtlasOF_FaithHealing : TimedRealmAbility
{
    ...
    public override void Execute(GameLiving living)
    {
        ...
        {
            var livingCurrentSpot = new Point3D(living.X, living.Y, living.Z);
            foreach (var player in living.Group.GetPlayersInTheGroup())
                if (player.GetDistanceTo(livingCurrentSpot) <= Range)
                {
                    ...
                    var healed = player.ChangeHealth(living, eHealthChangeType.Spell, healAmount);
                    ...
                    {
                        if (player == pl)
                            pl.Out.SendMessage("You heal yourself for " + healed + " hit points",
                                eChatType.CT_Spell, eChatLoc.CL_SystemWindow);
                        else
                            pl.Out.SendMessage("You heal " + player.Name + " for " + healed + " hit points",
                                eChatType.CT_Spell, eChatLoc.CL_SystemWindow);
                    }
                }
        }
        ...
        {
            var healAmount = living.MaxHealth;
            var healed = living.ChangeHealth(living, eHealthChangeType.Spell, healAmount);
            if (healed > 0) didHeal = true;
            SendSpellEffectsToLiving(living, didHeal);
            if (didHeal && living is GamePlayer pla)
                pla.Out.SendMessage("You heal yourself for " + healed + " hit points", eChatType.CT_Spell,
