...
public class AtlasOF_MasteryOfPain : MasteryOfPain
{
    ...
    {
        return AtlasRAHelpers.GetAugDexLevel(player) >= 2;
    }

    // MoP is 5% per level unlike most other Mastery RAs.
    public override int GetAmountForLevel(int level)
    {
        return AtlasRAHelpers.GetPropertyEnhancer5AmountForLevel(level);
    }
...
public class AtlasOF_MasteryOfArms : RAPropertyEnhancer
{
    ...
    public override bool CheckRequirement(GamePlayer player)
    {
        ...
        if (player.CharacterClass.ID == (byte) eCharacterClass.Friar) return AtlasRAHelpers.GetAugDexLevel(player) >= 3;
...
public class AtlasOF_Dodger : RAPropertyEnhancer
{
    ...
    public override bool CheckRequirement(GamePlayer player)
    {
        return AtlasRAHelpers.GetAugQuiLevel(player) >= 2;
    }
...
public class AtlasOF_MasteryOfStealth : MasteryOfStealthAbility
{
    ...
    public override int CostForUpgrade(int level)
    {
        return AtlasRAHelpers.GetCommonUpgradeCostFor3LevelsRA(level);
    }
}
