...
public class AtlasOF_Grapple : TimedRealmAbility, ISpellCastingAbilityHandler
{
    ...
    public override void Execute(GameLiving living)
    {
        ...
        foreach (GameNPC npc in caster.GetNPCsInRadius(m_range)) CastSpellOn(npc, caster);
    ...
    public void CastSpellOn(GameLiving target, GameLiving caster)
    {
        ...
        {
            var dd = ScriptMgr.CreateSpellHandler(caster, m_spell, m_spellline);
            if (caster is GamePlayer p)
                p.Out.SendMessage($"You grapple {target.Name} and they are slowed!", eChatType.CT_Spell,
