...
public class AtlasOF_BatteryOfLifeECSEffect : ECSGameAbilityEffect
{
    ...
    private bool HealthPoolRemains => m_HealthPool > 0;
    ...
    public override void OnEffectPulse()
    {
        ...
        {
            var livingToHeal = new Dictionary<GameLiving, int>();
            ...
            var sortedLiving =
                (from entry in livingToHeal orderby entry.Value ascending select entry).ToDictionary(
                    pair => pair.Key, pair => pair.Value);

            foreach (var healed in sortedLiving)
            {
                var currentLiving = healed.Key;
                var difference = currentLiving.MaxHealth - currentLiving.Health;

                if (!HealthPoolRemains) EffectService.RequestImmediateCancelEffect(this);
                if (m_HealthPool > difference)
                {
                    if (currentLiving is GamePlayer ptarg)
                        ptarg.Out.SendMessage(
                            $"{OwnerName}'s Battery of Life heals you for {difference} health points!",
                            eChatType.CT_Spell, eChatLoc.CL_SystemWindow);
                    if (Owner is GamePlayer pcaster)
                        pcaster.Out.SendMessage(
                            $"Your Battery of Life heals {currentLiving.Name} for {difference} health points!",
                            eChatType.CT_Spell, eChatLoc.CL_SystemWindow);
