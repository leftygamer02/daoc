...
namespace DOL.GS.Effects
{
    public class AtlasOF_VolleyECSEffect : ECSGameAbilityEffect
    {
        ...
        public override ushort Icon => 4281; //3083,7080,3079(icons)
        ...
        public override void OnStartEffect()
        {
            ...
            var attackWeapon = m_player.ActiveWeapon; //define user weapon
            byte HoldAttack = 255; //0x1E;//30 seconds
            var speed = attackWeapon.SPD_ABS * 100; //weapon speed used to timer
            var
                readyTimer =
                    new ECSGameTimer(m_player, new ECSGameTimer.ECSTimerCallback(ReadyToFire),
                        speed); //timer to prepare bow
            var tiredTimer =
                new ECSGameTimer(m_player, new ECSGameTimer.ECSTimerCallback(TooTired1stShoot),
                    32500); //timer too tired
            ...
            m_player.Out.SendMessage("You prepare to unleash a volley of arrows!", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            m_player.Out.SendMessage(string.Format("You prepare to fire. ({0}s to fire)", (double) speed / 1000),
                eChatType.CT_Important, eChatLoc.CL_SystemWindow);
            var model = m_player.ActiveWeapon == null ? 0 : m_player.ActiveWeapon.Model;

            if (!BowPreparation)
            {
                foreach (GamePlayer player in m_player.GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE))
                    player.Out.SendCombatAnimation(m_player, null, (ushort) model, 0x00, player.Out.BowPrepare,
                        HoldAttack, 0x00, m_player.HealthPercent);
        ...
        {
            var volley = EffectListService.GetEffectOnTarget(m_player, eEffect.Volley);
            var attackWeapon = m_player.ActiveWeapon;
            ...
            {
                var shot1 =
                    new ECSGameTimer(m_player, new ECSGameTimer.ECSTimerCallback(TooTired1stShoot), 32500);
            ...
            {
                var shot2 =
                    new ECSGameTimer(m_player, new ECSGameTimer.ECSTimerCallback(TooTired2ndShoot), 32500);
            ...
            {
                var shot3 =
                    new ECSGameTimer(m_player, new ECSGameTimer.ECSTimerCallback(TooTired3thShoot), 32500);
            ...
            {
                var shot4 =
                    new ECSGameTimer(m_player, new ECSGameTimer.ECSTimerCallback(TooTired4thShoot), 32500);
            ...
            {
                var shot5 =
                    new ECSGameTimer(m_player, new ECSGameTimer.ECSTimerCallback(TooTired5thShoot), 32500);
            ...
            foreach (GamePlayer playerInRadius in m_player.GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE))
                playerInRadius.Out.SendCombatAnimation(m_player, null, (ushort) model, 0x00,
                    playerInRadius.Out.BowPrepare, HoldAttack, 0x00, 0x00); //bow animation
        }
        ...
        private protected int TooTired1stShoot(ECSGameTimer timer) //1st shot
        {
            ...
            {
                m_player.Out.SendMessage("You are too tired to hold your volley any longer!", eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);
                //Cancel(false);
                AbortShot = true;
        ...
        protected IList SelectTargets()
        {
            ...
            foreach (GamePlayer VolleePlayerTarget in WorldMgr.GetPlayersCloseToSpot(m_player.CurrentRegionID,
                         m_player.GroundTarget.X, m_player.GroundTarget.Y, m_player.GroundTarget.Z, radiusToCheck))
                if (VolleePlayerTarget != null)
                    if (VolleePlayerTarget.IsAlive && VolleePlayerTarget.Client.Account.PrivLevel == 1 &&
                        !list.Contains(VolleePlayerTarget) && (VolleePlayerTarget.Realm != m_player.Realm ||
                                                               m_player.DuelTarget == VolleePlayerTarget))
                        if (Util.Chance(50))
                            list.Add(VolleePlayerTarget); //add player to list of potentional targets

            foreach (GameNPC VolleeNPCTarget in WorldMgr.GetNPCsCloseToSpot(m_player.CurrentRegionID,
                         m_player.GroundTarget.X, m_player.GroundTarget.Y, m_player.GroundTarget.Z, radiusToCheck))
            {
        ...
        public void decNbShoot()
        {
            ...
            if (nbShoot == 0)
            {
                ...
                Cancel(false);
                var volley = m_player.GetAbility<AtlasOF_Volley>();
        ...
        public void LaunchVolley(GamePlayer player)
        {
            ...
            {
                player.Out.SendMessage("You need arrows to use Volley!", eChatType.CT_SpellResisted,
                    eChatLoc.CL_SystemWindow);
                return;
            }
            ...
            if (player.ActiveWeaponSlot != eActiveWeaponSlot.Distance)
            {
                player.Out.SendMessage(
                    LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "Skill.Ability.CannotUse.CriticalShot.NoRangedWeapons"), eChatType.CT_Important,
                    eChatLoc.CL_SystemWindow);
                return;
            }
            ...
            {
                player.Out.SendMessage("You must have a ground target to use Volley!", eChatType.CT_SpellResisted,
                    eChatLoc.CL_SystemWindow);
                return;
            }
            ...
            foreach (GamePlayer playerInRadius in m_player.GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE))
            {
                ...
                playerInRadius.Out.SendCombatAnimation(m_player, null, (ushort) model, 0x00,
                    playerInRadius.Out.BowShoot, 0, 0x00, 0x00);
            }

            IsReadyToFire = false;
            var ticksToTarget = m_player.GetDistanceTo(m_player.GroundTarget) * 1000 /
                                RangeAttackComponent.PROJECTILE_FLIGHT_SPEED;
            ...
            var arrowRecoveryChance = player.GetModified(eProperty.ArrowRecovery);
            ...
            if (m_player.TempProperties.getProperty<ECSGameTimer>("volley_readyTimerAgain") == null)
            {
                ...
                if (nbShoot == 4)
                {
                    var shot2Continue = new ECSGameTimer(player,
                        new ECSGameTimer.ECSTimerCallback(TooTired2ndShoot), 32500);
                    m_player.TempProperties.setProperty("volley_shot2Continue", shot2Continue);
                }
                else if (nbShoot == 3)
                {
                    var shot3Continue = new ECSGameTimer(player,
                        new ECSGameTimer.ECSTimerCallback(TooTired3thShoot), 32500);
                    m_player.TempProperties.setProperty("volley_shot3Continue", shot3Continue);
                }
                else if (nbShoot == 2)
                {
                    var shot4Continue = new ECSGameTimer(player,
                        new ECSGameTimer.ECSTimerCallback(TooTired4thShoot), 32500);
                    m_player.TempProperties.setProperty("volley_shot4Continue", shot4Continue);
                }
                else if (nbShoot == 1)
                {
                    var shot5Continue = new ECSGameTimer(player,
                        new ECSGameTimer.ECSTimerCallback(TooTired5thShoot), 32500);
            ...
            if (nbShoot >= 1)
            {
                player.Out.SendMessage("You have " + nbShoot + " arrows to be drawn!", eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);

                foreach (GamePlayer playerInRadius in player.GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE))
                    playerInRadius.Out.SendCombatAnimation(player, null, (ushort) model, 0x00,
                        playerInRadius.Out.BowPrepare, 0x1E, 0x00, 0x00);
            }
        }
        ...
        {
            var attackWeapon = archer.ActiveWeapon;
            var damagetype = archer.attackComponent.AttackDamageType(attackWeapon);
            var modifier = archer.RealmLevel;
            double effectivenes = 1;
            var weaponspeed = attackWeapon.SPD_ABS * 0.1;
            var ClampedDamage = 1.2 + archer.Level * 0.3;
            var WeaponBonus2H = 1.1 + 0.005 * archer.WeaponSpecLevel(attackWeapon);
            var SlowWeaponBonus = 1 + (weaponspeed - 2) * 0.03;
            var meleeRelicBonus = 1.0 + RelicMgr.GetRelicBonusModifier(archer.Realm, eRelicType.Strength);
            var WeaponDPS = attackWeapon.DPS_AF * 0.1;
            double TargetAF = 1;
            ...
            double TargetABS;
            var AcherWeaponSkill = archer.GetWeaponSkill(attackWeapon);
            var baseDamage = WeaponDPS * (AcherWeaponSkill / TargetAF) * meleeRelicBonus * SlowWeaponBonus *
                             WeaponBonus2H * weaponspeed; //calculate dmg
                            ...
                            case eDamageType.Crush:
                            {
                                switch (armor.Object_Type)
                                {
                                    ...
                                    case (int) eObjectType.Leather:
                                        baseDamage *= vulnerable;
                                        break;
                                    case (int) eObjectType.Studded:
                                        baseDamage *= resistant;
                                        break;
            ...
            if (target is GamePlayer)
                baseDamage += (int) (baseDamage *
                                     (target.GetResist(damagetype) + SkillBase.GetArmorResist(armor, damagetype)) *
                                     -0.007); //calculate dmg based on armor resists
            else
            ...
            foreach (GamePlayer player in ad.Attacker.GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE))
                player.Out.SendCombatAnimation(null, target, 0, 0, 0, 0, 0x0A,
                    target.HealthPercent); //being attacked animation

            if (m_player != null)
            {
                m_player.Out.SendMessage(
                    LanguageMgr.GetTranslation(m_player.Client.Account.Language,
                        "Effects.VolleyEffect.MBHitsExtraDamage", target.GetName(0, false), ad.Damage),
                    eChatType.CT_YouHit, eChatLoc.CL_SystemWindow);
                if (ad.CriticalDamage > 0)
                    m_player.Out.SendMessage(
                        LanguageMgr.GetTranslation(m_player.Client.Account.Language,
                            "Effects.VolleyEffect.MBHitsExtraDamageCrit", target.GetName(0, false), ad.CriticalDamage),
                        eChatType.CT_YouHit, eChatLoc.CL_SystemWindow);
                var playerTarget = target as GamePlayer;
                if (playerTarget != null)
                {
                    playerTarget.Out.SendMessage(
                        LanguageMgr.GetTranslation(playerTarget.Client.Account.Language,
                            "Effects.VolleyEffect.XMBExtraDamageToYou", archer.GetName(0, false),
                            Convert.ToString(ad.ArmorHitLocation).ToLower(), ad.Damage), eChatType.CT_Damaged,
                        eChatLoc.CL_SystemWindow);
                    if (ad.CriticalDamage > 0)
                        playerTarget.Out.SendMessage(
                            LanguageMgr.GetTranslation(playerTarget.Client.Account.Language,
                                "Effects.VolleyEffect.XMBExtraDamageToYouCrit", archer.GetName(0, false),
            ...
            if (ad.Damage > 0)
            {
                ...
                if (ad.Target.effectListComponent
                    .ContainsEffectForEffectType(eEffect.MovementSpeedBuff)) //cancel speed on target
                {
                ...
                if (ad.Attacker.effectListComponent
                    .ContainsEffectForEffectType(eEffect.MovementSpeedBuff)) //cancel speed on archer
                {
        ...
        {
            var useArgs = args as UseSlotEventArgs;
            var player = sender as GamePlayer;
            if (player == null) return;
            var slot = useArgs.Slot;
            var type = useArgs.Type;
            var attackrangeMin = 2000 * 0.66; //minimum attack range
            ...
            {
                player.Out.SendMessage("You ground target is too close to use Volley!", eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);
                return;
            }
            ...
            {
                player.Out.SendMessage("You ground target is too far away to use Volley!", eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);
                return;
            }
        ...
        private int MakeAnimation(ECSGameTimer timer)
        {
            ...
            if (targets.Count > 0)
            {
                ...
                var target = (GameLiving) targets[Util.Random(0, targets.Count - 1)];
            ...
            {
                m_player.Out.SendMessage("Your shot sails clear of all targets!", eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);
            }
        ...
        public void OnPlayerMoved()
        {
            ...
            var volley = m_player.GetAbility<AtlasOF_Volley>();
            m_player.DisableSkill(volley, AtlasOF_Volley.DISABLE_DURATION);
            m_player.Out.SendMessage("You move and interrupt your volley!", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
        ...
        public void OnPlayerSwitchedWeapon()
        {
            ...
            var volley = m_player.GetAbility<AtlasOF_Volley>();
            m_player.DisableSkill(volley, AtlasOF_Volley.DISABLE_DURATION);
            m_player.Out.SendMessage("You put away your bow and interrupt your volley!", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
        ...
        public void OnAttacked()
        {
            ...
            var volley = m_player.GetAbility<AtlasOF_Volley>();
            m_player.DisableSkill(volley, AtlasOF_Volley.DISABLE_DURATION);
            m_player.Out.SendMessage("You have been attacked and your volley is interrupted!", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
...
namespace DOL.GS
{
    public class VolleyMob : GameNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var volley = new StandardMobBrain();
            SetOwnBrain(volley);
            var success = base.AddToWorld();
            if (success) new ECSGameTimer(this, new ECSGameTimer.ECSTimerCallback(Show_Effect), 100);
