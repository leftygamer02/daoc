...
public class AmelioratingMelodiesECSEffect : ECSGameAbilityEffect
{
    ...
    public override void OnEffectPulse()
    {
        ...
        foreach (var p in playersToHeal)
            if (p.Health < p.MaxHealth && OwnerPlayer.IsWithinRadius(p, m_range) && p.IsAlive)
            {
                ...
                OwnerPlayer.Out.SendMessage(
                    "Your Ameliorating Melodies heal " + p.Name + " for " + heal.ToString() + " hit points.",
                    eChatType.CT_Spell, eChatLoc.CL_SystemWindow);
                p.Out.SendMessage(
                    OwnerPlayer.Name + "'s Ameliorating Melodies heals you for " + heal.ToString() + " hit points.",
                    eChatType.CT_Spell, eChatLoc.CL_SystemWindow);
            }
