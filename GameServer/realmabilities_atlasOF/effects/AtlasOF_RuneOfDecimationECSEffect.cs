...
public class AtlasOF_RuneOfDecimationECSEffect : ECSGameSpellEffect
{
    ...
    public override void OnEffectPulse()
    {
        ...
        var DetonateTargets = new List<GameLiving>();
        ...
        if (shouldDetonate)
        {
            foreach (GamePlayer i_player in Owner.GetPlayersInRadius(WorldMgr.INFO_DISTANCE))
                i_player.Out.SendMessage($"{SpellHandler.Caster.Name}'s Rune of Decimation trap detonates!",
            ...
            foreach (var target in DetonateTargets)
            {
                ...
                {
                    pl.Out.SendMessage($"You take {ad.Damage}({ad.Modifier}) damage from a Rune of Decimation!",
                ...
                if (SpellHandler.Caster is GamePlayer c)
                    c.Out.SendMessage(
                        $"Your Rune of Decimation deals {ad.Damage}({ad.Modifier}) damage to {target?.Name}!",
                        eChatType.CT_Damaged, eChatLoc.CL_SystemWindow);
