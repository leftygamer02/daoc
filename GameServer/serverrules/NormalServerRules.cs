...
public class NormalServerRules : AbstractServerRules
{
    ...
    public override bool IsAllowedToAttack(GameLiving attacker, GameLiving defender, bool quiet)
    {
        ...
        if (attacker.Realm == defender.Realm &&
            !(attacker is GamePlayer && ((GamePlayer) attacker).DuelTarget == defender))
        {
            // allow confused mobs to attack same realm
            if (attacker is GameNPC && (attacker as GameNPC).IsConfused)
                return true;

            if (attacker.Realm == 0) return FactionMgr.CanLivingAttack(attacker, defender);
    ...
    public override bool IsAllowedCharsInAllRealms(GameClient client)
    {
        if (client.Account.PrivLevel > 1)
            return true;
        if (Properties.ALLOW_ALL_REALMS)
            return true;
        return false;
    }
    ...
    public override bool IsAllowedToJoinGuild(GamePlayer source, Guild guild)
    {
        ...
        if (Properties.ALLOW_CROSS_REALM_GUILDS == false && guild.Realm != eRealm.None &&
            source.Realm != guild.Realm)
            return false;
    ...
    public override bool IsAllowedToTrade(GameLiving source, GameLiving target, bool quiet)
    {
        ...
        if (source is GamePlayer && target is GamePlayer)
            if ((source as GamePlayer).Client.Account.PrivLevel > 1 ||
                (target as GamePlayer).Client.Account.PrivLevel > 1)
                return true;
    ...
    public override bool IsAllowedToCraft(GamePlayer player, ItemTemplate item)
    {
        return player.Realm == (eRealm) item.Realm ||
               (item.Realm == 0 && Properties.ALLOW_CRAFT_NOREALM_ITEMS);
    }
    ...
    protected override eObjectType[] GetCompatibleObjectTypes(eObjectType objectType)
    {
        if (m_compatibleObjectTypes == null)
        {
            ...
            //alb
            m_compatibleObjectTypes[(int) eObjectType.CrushingWeapon] =
                new eObjectType[] {eObjectType.CrushingWeapon};
            m_compatibleObjectTypes[(int) eObjectType.SlashingWeapon] =
            ...
            m_compatibleObjectTypes[(int) eObjectType.TwoHandedWeapon] =
                new eObjectType[] {eObjectType.TwoHandedWeapon};
            m_compatibleObjectTypes[(int) eObjectType.PolearmWeapon] =
                new eObjectType[] {eObjectType.PolearmWeapon};
            ...
            m_compatibleObjectTypes[(int) eObjectType.Axe] =
                new eObjectType[] {eObjectType.Axe, eObjectType.LeftAxe};
    ...
    public override void ResetKeep(GuardLord lord, GameObject killer)
    {
        ...
        if (ConquestService.ConquestManager.ActiveObjective != null &&
            ConquestService.ConquestManager.ActiveObjective.Keep == lord.Component.Keep)
            ConquestService.ConquestManager.ConquestCapture(ConquestService.ConquestManager.ActiveObjective.Keep);

        if (ConquestService.ConquestManager.GetSecondaryObjectives()
                .FirstOrDefault(conq => conq.Keep == lord.Component.Keep) != null)
