    ...
    public bool IsAllowedToJump(ZonePoint targetPoint, GamePlayer player)
    {
        ...
        if (player.Group != null)
        {
            ...
            foreach (var region in WorldMgr.GetAllRegions())
                if (region is AdventureWingInstance && ((AdventureWingInstance) region).Group != null &&
                    ((AdventureWingInstance) region).Group == player.Group)
                {
                    // Our group has an instance !
                    previousInstance = (AdventureWingInstance) region;
                    break;
                }
                else if (region is AdventureWingInstance && ((AdventureWingInstance) region).Player != null &&
                         ((AdventureWingInstance) region).Player == player.Group.Leader)
                {
        ...
        else
        {
            ...
            foreach (var region in WorldMgr.GetAllRegions())
                if (region is AdventureWingInstance && ((AdventureWingInstance) region).Player != null &&
                    ((AdventureWingInstance) region).Player == player)
                {
                    // I have an Instance !
                    previousInstance = (AdventureWingInstance) region;
                    previousInstance.Group = player.Group;
            ...
            if (previousInstance.Skin != targetPoint.TargetRegion)
            {
                ...
                if (previousInstance.NumPlayers > 0)
                {
                    ...
                    player.Out.SendMessage(
                        "You have another instance (" + previousInstance.Description +
                        ") running with people in it !", eChatType.CT_System, eChatLoc.CL_SystemWindow);
                    return false;
                }
                else
                {
                    log.Warn("Player : " + player.Name + " requested new Instance, destroying instance " +
                             previousInstance.Description + ", ID: " + previousInstance.ID + ", type=" +
                             previousInstance.GetType().ToString() + ".");
        ...
        if (previousInstance == null)
        {
            ...
            previousInstance =
                (AdventureWingInstance) WorldMgr.CreateInstance(targetPoint.TargetRegion,
                    typeof(AdventureWingInstance));
            if (targetPoint.SourceRegion != 0 && targetPoint.SourceRegion == player.CurrentRegionID)
                //source loc seems legit...
                previousInstance.SourceEntrance = new GameLocation("source", targetPoint.SourceRegion,
                    targetPoint.SourceX, targetPoint.SourceY, targetPoint.SourceZ);
            ...
            previousInstance.LoadFromDatabase(previousInstance.RegionData.Mobs, ref mobs, ref merchants, ref items,
                ref bindpoints);
            ...
            foreach (var region in WorldMgr.GetAllRegions())
                if (region is AdventureWingInstance && (AdventureWingInstance) region != previousInstance)
                {
                    ...
                    if (to_clean.NumPlayers == 0)
                    {
                        ...
                        else if (to_clean.Player != null && (to_clean.Player == player ||
                                                             (player.Group != null &&
                                                              player.Group.Leader == to_clean.Player)))
            ...
            //enumerate to_delete
            foreach (var region in to_delete)
            {
                log.Warn("Player : " + player.Name + " has provoked an instance cleanup - " + region.Description +
                         ", ID: " + region.ID + ", type=" + region.GetType().ToString() + ".");
        ...
        //get loc of instance
        if (previousInstance != null)
            loc = new GameLocation(previousInstance.Description + " (instance)", previousInstance.ID,
                targetPoint.TargetX, targetPoint.TargetY, targetPoint.TargetZ, targetPoint.TargetHeading);
        ...
        player.Out.SendMessage("Something went Wrong when creating Instance !", eChatType.CT_System,
