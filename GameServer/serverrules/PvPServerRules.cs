...
public class PvPServerRules : AbstractServerRules
{
    ...
    public override void ImmunityExpiredCallback(GamePlayer player)
    {
        ...
        if (player.Level < m_safetyLevel && player.SafetyFlag)
            player.Out.SendMessage(
                "Your temporary invulnerability timer has expired, but your /safety flag is still on.",
    ...
    public override void OnReleased(DOLEvent e, object sender, EventArgs args)
    {
        ...
        if (player.TempProperties.getProperty<object>(KILLED_BY_PLAYER_PROP, null) != null)
        {
            ...
            StartImmunityTimer(player,
                ServerProperties.Properties.TIMER_KILLED_BY_PLAYER * 1000); //When Killed by a Player
        }
        else
        {
            StartImmunityTimer(player,
                ServerProperties.Properties.TIMER_KILLED_BY_MOB * 1000); //When Killed by a Mob
        }
    }
    ...
    public override bool IsAllowedToAttack(GameLiving attacker, GameLiving defender, bool quiet)
    {
        ...
        if (playerAttacker != null && playerDefender != null)
        {
            ...
            if (playerAttacker.DuelTarget != defender)
            {
                ...
                var mybattlegroup =
                    (BattleGroup) playerAttacker.TempProperties.getProperty<object>(
                ...
                if (m_safeRegions != null)
                    foreach (var reg in m_safeRegions)
                        if (playerAttacker.CurrentRegionID == reg)
                        {
                            if (quiet == false)
                                MessageToLiving(playerAttacker,
                                    "You're currently in a safe zone, you can't attack other players here.");
                            return false;
                        }
                ...
                {
                    var unsafeRegion = false;
                    foreach (var regionID in m_unsafeRegions)
                    ...
                    if (unsafeRegion == false)
                    {
                        ...
                        if (quiet == false)
                            MessageToLiving(attacker,
                                playerDefender.Name + " has " + playerDefender.GetPronoun(1, false) +
                                " safety flag on and is in a safe area, you can't attack " +
                                playerDefender.GetPronoun(2, false) + " here.");
                        return false;
                    }
                }
            }
        ...
        if (attacker.Realm == 0 && defender.Realm == 0) return FactionMgr.CanLivingAttack(attacker, defender);
        ...
        if (defender is GameNPC && defender.Realm != 0 && attacker.Realm != 0 &&
            defender is GameKeepGuard == false && defender is GameFont == false)
        {
        ...
        if (attacker is GameNPC && attacker.Realm != 0 && defender.Realm != 0 &&
            attacker is GameKeepGuard == false) return false;
        ...
        if (defender is GameKeepGuard && attacker is GamePlayer
                                      && GameServer.KeepManager.IsEnemy(defender as GameKeepGuard,
                                          attacker as GamePlayer) == false)
        {
            if (quiet == false) MessageToLiving(attacker, "You can't attack a friendly NPC!");
            return false;
        }
        ...
        if (attacker is GameKeepGuard && defender is GamePlayer
                                      && GameServer.KeepManager.IsEnemy(attacker as GameKeepGuard,
                                          defender as GamePlayer) == false)
            return false;
    ...
    public override bool IsAllowedToCastSpell(GameLiving caster, GameLiving target, Spell spell,
        SpellLine spellLine)
    {
        ...
        if (casterPlayer != null)
            if (casterPlayer.IsInvulnerableToAttack)
            {
                ...
                if (caster != target || spell.Target == "Area" || spell.Target == "Enemy" ||
                    (spell.Target == "Group" && spell.SpellType != (byte) eSpellType.SpeedEnhancement))
                {
                    MessageToLiving(caster,
                        "You can only cast spells on yourself until your PvP invulnerability timer wears off!",
    ...
    public override bool IsSameRealm(GameLiving source, GameLiving target, bool quiet)
    {
        ...
        {
            var controlled = ((GameNPC) source).Brain as IControlledBrain;
            if (controlled != null)
            {
        ...
        {
            var controlled = ((GameNPC) target).Brain as IControlledBrain;
            if (controlled != null)
    ...
    public override IList<string> FormatPlayerStatistics(GamePlayer player)
    {
        ...
        if (player.KillsAlbionPlayers + player.KillsMidgardPlayers + player.KillsHiberniaPlayers > 0)
        {
            ...
            if (player.KillsMidgardPlayers > 0)
                stat.Add(LanguageMgr.GetTranslation(player.Client.Account.Language,
                    "PlayerStatistic.Kill.MidgardPlayer") + ": " + player.KillsMidgardPlayers.ToString("N0"));
            if (player.KillsHiberniaPlayers > 0)
                stat.Add(LanguageMgr.GetTranslation(player.Client.Account.Language,
                    "PlayerStatistic.Kill.HiberniaPlayer") + ": " + player.KillsHiberniaPlayers.ToString("N0"));
            total = player.KillsMidgardPlayers + player.KillsAlbionPlayers + player.KillsHiberniaPlayers;
            stat.Add(
                LanguageMgr.GetTranslation(player.Client.Account.Language, "PlayerStatistic.Kill.TotalPlayers") +
                ": " + total.ToString("N0"));
        }
        ...
        if (player.KillsAlbionSolo + player.KillsMidgardSolo + player.KillsHiberniaSolo > 0)
        {
            ...
            if (player.KillsHiberniaSolo > 0)
                stat.Add(LanguageMgr.GetTranslation(player.Client.Account.Language,
                    "PlayerStatistic.Solo.HiberniaPlayer") + ": " + player.KillsHiberniaSolo.ToString("N0"));
            total = player.KillsMidgardSolo + player.KillsAlbionSolo + player.KillsHiberniaSolo;
            stat.Add(
                LanguageMgr.GetTranslation(player.Client.Account.Language, "PlayerStatistic.Solo.TotalPlayers") +
                ": " + total.ToString("N0"));
        }
        ...
        if (player.CapturedKeeps + player.CapturedTowers > 0)
        {
            ...
            if (player.CapturedTowers > 0)
                stat.Add(LanguageMgr.GetTranslation(player.Client.Account.Language,
                    "PlayerStatistic.Capture.Towers") + ": " + player.CapturedTowers.ToString("N0"));
    ...
    public override void ResetKeep(GuardLord lord, GameObject killer)
    {
        ...
        {
            var group = (killer as GamePlayer).Group;
            if (group != null)
        ...
        {
            var player = ((killer as GameNPC).Brain as IControlledBrain).GetPlayerOwner();
            Group group = null;
            if (player != null)
                group = player.Group;
            if (group != null)
