...
public class MinotaurRelic : GameStaticItem
{
    ...
    private DBMinotaurRelic m_dbRelic;
    ...
    public override bool Interact(GamePlayer player)
    {
        ...
        foreach (GameNPC npc in GetNPCsInRadius(100))
            if (npc.Model == 1583)
            {
                player.Out.SendMessage("You cannot pickup " + GetName(0, false) + ". It is locked!",
        ...
        if (Owner != null)
        {
            player.Out.SendMessage("This Relic is owned by someone else!", eChatType.CT_System,
        ...
        if (player.Group != null)
            foreach (var pl in player.Group.GetPlayersInTheGroup())
                if (pl.MinotaurRelic != null)
                {
                    player.Out.SendMessage("Someone in your group already have a Relic!", eChatType.CT_System,
                        eChatLoc.CL_SystemWindow);
                    return false;
                }
    ...
    protected virtual void PlayerTakesRelic(GamePlayer player)
    {
        ...
        m_spellHandler = ScriptMgr.CreateSpellHandler(m_owner, RelicSpell,
    ...
    protected void ApplyRelicEffect()
    {
        ...
        if (m_owner != null)
            switch (RelicTarget.ToLower())
            {
                ...
                case "group":
                    ...
                    {
                        foreach (var plr in m_owner.Group.GetPlayersInTheGroup())
                            if (plr != null && !newPlayerlist.Contains(plr) &&
                                m_owner.IsWithinRadius(plr, WorldMgr.VISIBILITY_DISTANCE))
                ...
                case "realm":
                    foreach (GamePlayer plr in m_owner.GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE))
                        if (plr != null && GameServer.ServerRules.IsAllowedToAttack(m_owner, plr, true) == false &&
                            !newPlayerlist.Contains(plr))
                            newPlayerlist.Add(plr);
        ...
        lock (Playerlist)
        {
            foreach (var plr in Playerlist)
            {
                ...
                if (!newPlayerlist.Contains(plr))
                    try
                    {
                        lock (plr.EffectList)
                        {
                            var check = SpellHandler.FindEffectOnTarget(plr,
                                m_gameSpellEffect.Spell.SpellType.ToString());
                            if (check != null)
                                check.Cancel(false);
                        }
                    }
            ...
            foreach (var plr in newPlayerlist)
            {
                if (plr == null) continue;
                try
                {
                    ...
                    {
                        var check =
                            SpellHandler.FindEffectOnTarget(plr, m_gameSpellEffect.Spell.SpellType.ToString());
                        if (check == null)
                        {
                            var handler = ScriptMgr.CreateSpellHandler(plr, RelicSpell,
                                SkillBase.GetSpellLine(GlobalSpellsLines.Reserved_Spells));
    ...
    protected virtual void Update(GameLiving living)
    {
        ...
        foreach (var clt in WorldMgr.GetClientsOfRegion(CurrentRegionID))
        {
    ...
    protected virtual void SetHandlers(GamePlayer player, bool start)
    {
        if (start)
        {
            ...
            GameEventMgr.AddHandler(player, GameLivingEvent.GainedRealmPoints, new DOLEventHandler(RealmPointGain));
            GameEventMgr.AddHandler(player, GamePlayerEvent.RegionChanged, new DOLEventHandler(PlayerAbsence));
            GameEventMgr.AddHandler(player, GameLivingEvent.RegionChanging, new DOLEventHandler(PlayerAbsence));
        ...
        else
        {
            ...
            GameEventMgr.RemoveHandler(player, GameLivingEvent.GainedRealmPoints,
                new DOLEventHandler(RealmPointGain));
            GameEventMgr.RemoveHandler(player, GamePlayerEvent.RegionChanged, new DOLEventHandler(PlayerAbsence));
            GameEventMgr.RemoveHandler(player, GameLivingEvent.RegionChanging, new DOLEventHandler(PlayerAbsence));
    ...
    protected void RealmPointGain(DOLEvent e, object sender, EventArgs args)
    {
        if (sender is GamePlayer && args is GainedRealmPointsEventArgs)
        {
            ...
            var arg = args as GainedRealmPointsEventArgs;
    ...
    protected void PlayerAbsence(DOLEvent e, object sender, EventArgs args)
    {
        ...
        if (e == GamePlayerEvent.AcceptGroup)
        {
            ...
            foreach (var pl in player.Group.GetPlayersInTheGroup())
                if (pl.MinotaurRelic != null)
                {
                    player.Out.SendMessage("Someone in your group already has a Relic!", eChatType.CT_System,
                        eChatLoc.CL_SystemWindow);
    ...
    public override bool AddToWorld()
    {
        if (SpawnLocked)
            if (X == SpawnX && Y == SpawnY)
                ...
                {
                    var protector =
                        (GameObject) GetType().Assembly.CreateInstance(ProtectorClassType, false);
                    ...
                    {
                        log.Debug(string.Format("[Minotaur Relic] ClassType: {0} was not found, Relic not loaded!",
                            ProtectorClassType));
                        return false;
                    }
                }
