...
public sealed class MinotaurRelicManager
{
    ...
    /// </summary>
    public static readonly Dictionary<string, MinotaurRelic> m_minotaurrelics = new();
    ...
    {
        foreach (var relic in m_minotaurrelics.Values)
        {
        ...
        try
        {
            ...
            foreach (var dbrelic in relics)
            {
                ...
                var relic = new MinotaurRelic(dbrelic);
    ...
    private static Timer m_mapUpdateTimer;

    public static void InitMapUpdate()
    {
        m_mapUpdateTimer =
            new Timer(new TimerCallback(MapUpdate), null, 0,
                30 * 1000); //30sec Lifeflight change this to 15 seconds
    }
    ...
    {
        var relics = new Dictionary<ushort, IList<MinotaurRelic>>();
        ...
        foreach (var clt in WorldMgr.GetAllPlayingClients())
        {
            if (clt == null || clt.Player == null)
                continue;

            if (relics.ContainsKey(clt.Player.CurrentRegionID))
                foreach (var relic in relics[clt.Player.CurrentRegionID])
                    clt.Player.Out.SendMinotaurRelicMapUpdate((byte) relic.RelicID, relic.CurrentRegionID, relic.X,
                        relic.Y, relic.Z);
        }
    }
