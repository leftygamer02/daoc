...
{
    private static GameEventMgr soleInstance = new();
    ...
    public static int NumObjectHandlers
    {
        ...
        {
            var numHandlers = 0;
            var cloneGameObjectEventCollections =
                new Dictionary<object, DOLEventHandlerCollection>(m_gameObjectEventCollections);

            foreach (var handler in cloneGameObjectEventCollections.Values) numHandlers += handler.Count;
    ...
    public static void RegisterGlobalEvents(Assembly asm, Type attribute, DOLEvent e)
    {
        ...
        foreach (var type in asm.GetTypes())
        {
            ...
            {
                var myAttribs = mInfo.GetCustomAttributes(attribute, false);
                if (myAttribs.Length != 0)
                    try
                    {
                        GlobalHandlerCollection.AddHandler(e,
                            (DOLEventHandler) Delegate.CreateDelegate(typeof(DOLEventHandler), mInfo));
                    }
                    catch (Exception ex)
                    {
                        if (log.IsErrorEnabled)
                            log.Error("Error registering global event. Method: " + type.FullName + "." + mInfo.Name,
                                ex);
                    }
            }
        }
            ...
            try
            {
                ...
                if (!m_gameObjectEventCollections.TryGetValue(obj, out col))
                {
                    ...
                    if (Lock.TryEnterWriteLock(LOCK_TIMEOUT))
                        ...
                        log.ErrorFormat("Timeout exceeded on attempt to AddHandler for object: {0}, event: {1}",
                            obj.ToString(), e.Name);
                }
    ...
    public static void RemoveHandler(object obj, DOLEvent e, DOLEventHandler del)
    {
        ...
        if (Lock.TryEnterReadLock(LOCK_TIMEOUT))
            ...
            log.ErrorFormat("Timeout exceeded on attempt to RemoveHandler for object: {0}, event: {1}",
                obj.ToString(), e.Name);

        if (col != null) col.RemoveHandler(e, del);
    }

    public static void RemoveAllHandlersForObject(object obj)
    {
        if (obj == null) throw new ArgumentNullException("obj", "No object given!");
        ...
        if (Lock.TryEnterReadLock(LOCK_TIMEOUT))
            ...
            log.ErrorFormat("Timeout exceeded (Read) on attempt to RemoveAllHandlersForObject: {0}",
                obj.ToString());
        ...
        if (col != null)
        {
            ...
            if (Lock.TryEnterWriteLock(LOCK_TIMEOUT))
                ...
                log.ErrorFormat("Timeout exceeded (Write) on attempt to RemoveAllHandlersForObject: {0}",
                    obj.ToString());
        }
    }
    ...
    public static void Notify(DOLEvent e, object sender, EventArgs eArgs)
    {
        ...
        if (sender != null)
        {
            ...
            {
                log.ErrorFormat("Timeout exceeded on attempt to Notify event: {0} for object: {1}", e.Name,
                    sender.ToString());
            }
        }
