...
public class PlayerTradeWindow : ITradeWindow
{
    ...
    /// </summary>
    public ArrayList PartnerTradeItems => m_partnerWindow.TradeItems;
    ...
    /// </summary>
    public long PartnerTradeMoney => m_partnerWindow.TradeMoney;

    /// <summary>
    /// Gets the owner of this window and items in it
    /// </summary>
    public GamePlayer Owner => m_owner;

    /// <summary>
    /// Gets the owner of this window and items in it
    /// </summary>
    public GamePlayer Partner => m_partnerWindow.Owner;
    ...
    /// </summary>
    public object Sync => m_sync;

    /// <summary>
    /// Gets the item count in trade window
    /// </summary>
    public int ItemsCount => m_tradeItems.Count;

    /// <summary>
    /// Gets the item count in partner trade window
    /// </summary>
    public int PartnerItemsCount => m_partnerWindow.TradeItems.Count;
    ...
    public bool Repairing
    {
        ...
        set
        {
            ...
            if (!m_recipiant)
            {
                m_owner.Out.SendMessage("Only a recipient of a trade can initiate a repair.",
            ...
            {
                m_owner.Out.SendMessage("You can only repair one item at a time!", eChatType.CT_System,
            ...
            if (ItemsCount > 0)
            {
                m_owner.Out.SendMessage("Your trade windows side must be empty to repair!", eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);
                m_partnerWindow.m_repair = false;
                m_repair = false;
                return;
            ...
            var itemToRepair = (InventoryItem) m_partnerWindow.TradeItems[0];
    ...
    public bool Combine
    {
        ...
        set
        {
            ...
            if (!m_recipiant)
            {
                m_owner.Out.SendMessage("Only a recipient of a trade can initiate a combine.",
                    eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                m_partnerWindow.m_combine = false;
                m_combine = false;
                return;
            ...
            if (m_partnerWindow.ItemsCount != 1)
            {
                m_owner.Out.SendMessage("You can only combine your items into one item!", eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);
                m_partnerWindow.m_combine = false;
                m_combine = false;
                return;
            ...
            var itemToCombine = (InventoryItem) m_partnerWindow.TradeItems[0];
    ...
    public bool AcceptTrade()
    {
        ...
        {
            m_tradeAccept = true;
            var partner = m_partnerWindow.Owner;

            partner.Out.SendMessage(m_owner.Name + " has accepted the trade.", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);

            // Check if the tradepartner has also agreed to the trade
            if (!m_partnerWindow.m_tradeAccept) return false;

            var logTrade = ServerProperties.Properties.LOG_TRADES;
            if (m_owner.Client.Account.PrivLevel > 1 || partner.Client.Account.PrivLevel > 1)
                logTrade = true;

            //Test if we and our partner have enough money
            var enoughMoney = m_owner.RemoveMoney(TradeMoney);
            var partnerEnoughMoney = partner.RemoveMoney(m_partnerWindow.TradeMoney);
            ...
            if (!enoughMoney || !partnerEnoughMoney)
            {
                if (!enoughMoney)
                {
                    ...
                    if (partnerEnoughMoney)
                    {
                        ...
                        InventoryLogging.LogInventoryAction(partner, m_owner, eInventoryActionType.Trade,
                            m_partnerWindow.TradeMoney);
                    }

                    m_owner.Out.SendMessage("You don't have enough money.", eChatType.CT_Merchant,
                        eChatLoc.CL_SystemWindow);
                    partner.Out.SendMessage(m_owner.Name + " doesn't have enough money.", eChatType.CT_Merchant,
                        eChatLoc.CL_SystemWindow);
                }
                ...
                if (!partnerEnoughMoney)
                {
                    ...
                    if (enoughMoney)
                    {
                        ...
                        InventoryLogging.LogInventoryAction(m_owner, partner, eInventoryActionType.Trade,
                            TradeMoney);
                    }

                    partner.Out.SendMessage("You don't have enough money.", eChatType.CT_Merchant,
                        eChatLoc.CL_SystemWindow);
                    m_owner.Out.SendMessage(partner.Name + " doesn't have enough money.", eChatType.CT_Merchant,
                        eChatLoc.CL_SystemWindow);
                }
            ...
            if (m_combine == true)
            {
                var crafter = m_recipiant == true ? m_owner : partner;
                // --------------------------------------------------------------
                // Luhz Crafting Update:
                // Players may now have any, and all, "primary" crafting skills.
            ...
            else if (m_repair == true)
            {
                var crafter = m_recipiant == true ? m_owner : partner;
                var itemToRepair =
                    (InventoryItem) (m_recipiant == true ? m_partnerWindow.TradeItems[0] : TradeItems[0]);
                if (itemToRepair != null) crafter.RepairItem(itemToRepair);
            }
            else
            {
                //Calculate the count of items 
                var myTradeItemsCount = m_tradeItems.Count;
                var partnerTradeItemsCount = m_partnerWindow.TradeItems.Count;

                //Test if we and our partner have enough space in inventory
                var mySpaceNeeded = Math.Max(0, partnerTradeItemsCount - myTradeItemsCount);
                var partnerSpaceNeeded = Math.Max(0, myTradeItemsCount - partnerTradeItemsCount);
                var enoughSpace = m_owner.Inventory.IsSlotsFree(mySpaceNeeded, eInventorySlot.FirstBackpack,
                    eInventorySlot.LastBackpack);
                var partnerEnoughSpace = partner.Inventory.IsSlotsFree(partnerSpaceNeeded,
                ...
                if (!enoughSpace || !partnerEnoughSpace)
                {
                    if (!enoughSpace)
                    {
                        m_owner.Out.SendMessage("You don't have enough space in your inventory.",
                            eChatType.CT_Merchant, eChatLoc.CL_SystemWindow);
                        partner.Out.SendMessage(m_owner.Name + " doesn't have enough space in his inventory.",
                            eChatType.CT_Merchant, eChatLoc.CL_SystemWindow);
                    }

                    if (!partnerEnoughSpace)
                    {
                        partner.Out.SendMessage("You don't have enough space in your inventory.",
                            eChatType.CT_Merchant, eChatLoc.CL_SystemWindow);
                        m_owner.Out.SendMessage(partner.Name + " doesn't have enough space in his inventory.",
                            eChatType.CT_Merchant, eChatLoc.CL_SystemWindow);
                    }
                ...
                var ownerTradeItems = (ArrayList) TradeItems.Clone();
                var partnerTradeItems = (ArrayList) m_partnerWindow.TradeItems.Clone();
                ...
                foreach (InventoryItem item in ownerTradeItems)
                    lock (m_owner.Inventory)
                    {
                        ...
                        {
                            if (logTrade)
                                GameServer.Instance.LogGMAction("   NOTItem: " + m_owner.Name + "(" +
                                                                m_owner.Client.Account.Name + ") -> " +
                                                                partner.Name + "(" + partner.Client.Account.Name +
                                                                ") : " + item.Name + "(" + item.Id_nb + ")");
                ...
                foreach (InventoryItem item in ownerTradeItems)
                {
                    if (m_owner.Guild != partner.Guild) item.Emblem = 0;
                    ...
                    if (item.PoisonSpellID > 0 && !(partner.CharacterClass.ID == 9 ||
                                                    partner.CharacterClass.ID == 23 ||
                                                    partner.CharacterClass.ID == 49))
                    {
                    ...
                    if (!tradeSuccess)
                    {
                        log.Error("Trade item was not added to Partner first free slot.  Owner = " + m_owner.Name +
                                  ", Partner = " + partner.Name + "; Item = " + item.Id_nb);
                    }
                    else
                    {
                        InventoryLogging.LogInventoryAction(m_owner, partner, eInventoryActionType.Trade,
                        ...
                        if (logTrade)
                            GameServer.Instance.LogGMAction("   Item: " + m_owner.Name + "(" +
                                                            m_owner.Client.Account.Name + ") -> " + partner.Name +
                                                            "(" + partner.Client.Account.Name + ") : " + item.Name +
                                                            "(" + item.Id_nb + ")");
                    }
                }
                ...
                foreach (InventoryItem item in partnerTradeItems)
                {
                    ...
                    // If PLayer is not Infiltrator (9), Shadowblade (23), Nightshade (49), remove Envenom bonus before add the item in the inventory
                    if (item.PoisonSpellID > 0 && !(m_owner.CharacterClass.ID == 9 ||
                                                    m_owner.CharacterClass.ID == 23 ||
                                                    m_owner.CharacterClass.ID == 49))
                    {
                        itemtoadd = GameInventoryItem.Create(itemtoadd);
                        itemtoadd.PoisonCharges = 0;
                        itemtoadd.PoisonMaxCharges = 0;
                    ...
                    if (!tradeSuccess)
                    {
                        log.Error("Trade item was not added to Owner first free slot.  Owner = " + m_owner.Name +
                                  ", Partner = " + partner.Name + "; Item = " + item.Id_nb);
                    }
                    else
                    {
                        InventoryLogging.LogInventoryAction(partner, m_owner, eInventoryActionType.Trade,
                            item.Template, item.Count);
                        if (logTrade)
                            GameServer.Instance.LogGMAction("   Item: " + partner.Name + "(" +
                                                            partner.Client.Account.Name + ") -> " + m_owner.Name +
                                                            "(" + m_owner.Client.Account.Name + ") : " + item.Name +
                                                            "(" + item.Id_nb + ")");
                    }
                }
                ...
                m_owner.Out.SendMessage(
                    "Trade Completed. " + myTradeItemsCount + " items for " + partnerTradeItemsCount + " items.",
                    eChatType.CT_System, eChatLoc.CL_SystemWindow);
                partner.Out.SendMessage(
                    "Trade Completed. " + partnerTradeItemsCount + " items for " + myTradeItemsCount + " items.",
                    eChatType.CT_System, eChatLoc.CL_SystemWindow);
            ...
            if (logTrade)
            {
                if (m_partnerWindow.TradeMoney > 0)
                    GameServer.Instance.LogGMAction("  Money: " + partner.Name + "(" + partner.Client.Account.Name +
                                                    ") -> " + m_owner.Name + "(" + m_owner.Client.Account.Name +
                                                    ") : " + m_partnerWindow.TradeMoney + "coppers");
                if (TradeMoney > 0)
                    GameServer.Instance.LogGMAction("  Money: " + m_owner.Name + "(" + m_owner.Client.Account.Name +
                                                    ") -> " + partner.Name + "(" + partner.Client.Account.Name +
                                                    ") : " + TradeMoney + "coppers");
            }
