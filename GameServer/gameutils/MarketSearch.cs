...
public class MarketSearch
{
        ...
        */
        ...
        public override string ToString()
        {
            ...
            return string.Format(
                "name:'{0}' | slot:{1,2} | bonus1:{2,2} | bonus1value:{3,2} | bonus2:{4,2} | bonus2value:{5,2} | bonus3:{6,2} | bonus3value: {7,2} | proc:{8} | armorType:{9,2} | damageType:{10} | levelMin:{11,2} | levelMax:{12,2} | minQual:{13,3} | priceMin:{14,2} | priceMax:{15,2} | playerCrafted:{16} | visual:{17} | clientVersion:{18}",
                name, slot, bonus1, bonus1Value, bonus2, bonus2Value, bonus3, bonus3Value, proc, armorType,
                damageType, levelMin, levelMax, minQual, priceMin, priceMax, playerCrafted, visual, clientVersion);
        }
    }
    ...
    public virtual List<InventoryItem> FindItemsInList(IList<InventoryItem> inventoryItems, SearchData search)
    {
        ...
        if (ServerProperties.Properties.MARKET_ENABLE_LOG && search.page == 0)
            log.DebugFormat("MarketSearch: [{0}:{1}] SEARCHING MARKET: {2}", m_searchPlayer.Name,
                m_searchPlayer.Client.Account.Name, search);
    ...
    protected virtual bool CheckSlot(InventoryItem item, int slot)
    {
        if (ServerProperties.Properties.MARKET_ENABLE_LOG)
        {
            ...
            log.DebugFormat("Market Explorer Search current item '{0}' as itemType '{1}' comparing with '{2}'.",
                item.Name, item.Item_Type, (int) eInventorySlot.ArmsArmor);
            log.DebugFormat("Market Explorer Search for items on slot '{0}' and for item.Type '{1}'.", slot,
                item.Item_Type);
        }
        ...
        if (slot != -1)
            switch (slot)
            {
                ...
                case 4:
                    return item.Object_Type == (int) eObjectType.Magical &&
                           item.Item_Type == (int) eInventorySlot.Jewellery;
                ...
                case 9:
                    return item.Object_Type == (int) eObjectType.Magical &&
                           item.Item_Type == (int) eInventorySlot.Neck;
                case 12:
                    return item.Object_Type == (int) eObjectType.Magical &&
                           item.Item_Type == (int) eInventorySlot.Waist;
                case 13:
                    return item.Object_Type == (int) eObjectType.Magical &&
                           (item.Item_Type == (int) eInventorySlot.RightBracer ||
                            item.Item_Type == (int) eInventorySlot.LeftBracer);
                case 15:
                    return item.Object_Type == (int) eObjectType.Magical &&
                           (item.Item_Type == (int) eInventorySlot.RightRing ||
                            item.Item_Type == (int) eInventorySlot.LeftRing);
                case 100:
                    return item.Item_Type == (int) eInventorySlot.RightHandWeapon ||
                ...
                case 104:
                    return item.Object_Type == (int) eObjectType.Instrument &&
    ...
    protected virtual bool CheckForArmorType(InventoryItem item, int type)
    {
        switch (type)
        {
            ...
            default:
                ...
                ChatUtil.SendErrorMessage(m_searchPlayer,
                    "Unhandled armor type (" + type + ") specified in search!");
                //m_searchHasError = true;
                return false;
        }
    }
    ...
    protected virtual bool CheckForSkill(InventoryItem item, int skill)
    {
        if (skill > 0)
            switch (skill)
            {
                ...
                case 134:
                    return CheckForProperty(item, (int) eProperty.Skill_SpectralGuard) ||
                ...
                default:
                    ...
                    ChatUtil.SendErrorMessage(m_searchPlayer,
                        "Unhandled skill (" + skill + ") specified in search!");
                    //m_searchHasError = true;
                    break;
            }
    ...
    protected virtual bool CheckForBonus(InventoryItem item, int bonus)
    {
        if (bonus > 0)
            switch (bonus)
            {
                ...
                case 9:
                {
                    for (var i = (int) eProperty.ToABonus_First; i <= (int) eProperty.ToABonus_Last; i++)
                ...
                case 20:
                {
                    ...
                    for (var i = (int) eProperty.MaxSpeed; i <= (int) eProperty.Acuity; i++)
                        if (CheckForProperty(item, i))
                            return true;

                    for (var i = (int) eProperty.EvadeChance; i <= (int) eProperty.ToHitBonus; i++)
                        if (CheckForProperty(item, i))
                            return true;

                    for (var i = (int) eProperty.DPS; i <= (int) eProperty.MaxProperty; i++)
                        if (CheckForProperty(item, i))
                            return true;
                ...
                default:
                    ...
                    ChatUtil.SendErrorMessage(m_searchPlayer,
                        "Unhandled bonus type (" + bonus + ") specified in search!");
                    //m_searchHasError = true;
                    break;
            }
    ...
    protected virtual bool CheckForDamageType(InventoryItem item, int damageType)
    {
        if (GlobalConstants.IsWeapon(item.Object_Type))
        {
            ...
            else
            {
                ...
                ChatUtil.SendErrorMessage(m_searchPlayer,
                    "Unhandled damage type (" + damageType + ") specified in search!");
                //m_searchHasError = true;
                return false;
            }
        }
