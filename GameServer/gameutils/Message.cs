...
public static class Message
{
    ...
    public static void ChatToArea(GameObject centerObject, string message, eChatType chatType,
    ...
    public static void ChatToArea(GameObject centerObject, string message, eChatType chatType, ushort distance,
        params GameObject[] excludes)
    {
    ...
    public static void SystemToArea(GameObject centerObject, string message, eChatType chatType,
        params GameObject[] excludes)
    {
    ...
    /// <remarks>If the centerObject is a player, he won't receive the message</remarks>
    public static void SystemToOthers2(GameObject centerObject, eChatType chatType, string LanguageMessageID,
        params object[] args)
    {
        ...
        foreach (GamePlayer player in centerObject.GetPlayersInRadius(WorldMgr.INFO_DISTANCE))
            if (!(centerObject is GamePlayer && centerObject == player))
                player.MessageFromArea(centerObject,
                    LanguageMgr.GetTranslation(player.Client.Account.Language, LanguageMessageID, args), chatType,
    ...
    /// <param name="excludes">An optional list of excluded players</param>
    /// <remarks>If the center object is a player, he will get the message too</remarks>
    public static void SystemToArea(GameObject centerObject, string message, eChatType chatType, ushort distance,
        params GameObject[] excludes)
    {
    ...
    public static void MessageToArea(GameObject centerObject, string message, eChatType chatType, eChatLoc chatLoc,
        ushort distance, params GameObject[] excludes)
    {
        ...
        if (centerObject != null)
        {
            ...
            foreach (GamePlayer player in
                     newList) // Send message to each GamePlayer within the specified distance of the centerObject
            {
                var excluded = false;

                if (excludes !=
                    null) // If entities are specified for exclusion (i.e., != null), then perform following actions
                    foreach (var obj in
                             excludes) // For each param in excludes, set exclude to true for GamePlayers and don't send message
                        if (obj == player)
                        {
                            excluded = true;
                            break;
                ...
                if (excluded == false)
                    player?.MessageFromArea(centerObject, message, chatType,
                        chatLoc); // If no excludes are specified (i.e., 'null'), send message to everyone in radius
            }
        }
    }
}