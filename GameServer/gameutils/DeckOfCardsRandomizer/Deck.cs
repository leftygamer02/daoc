...
public class PlayerDeck
{
    ...
    private void InitializeDeck()
    {
        lock (_cardLock)
        {
            ...
            for (var i = 0; i < NUM_NORMAL_DECKS; i++)
            for (var j = 0; j < 100; j++)
                _cards.Push(j + 1); //offset by 1 to only generate 'cards' with values 1-100

            for (var i = 0; i < NUM_BONUS_DECKS; i++)
            for (var j = 100 - BONUS_DECK_SIZE; j < 100; j++)
    ...
    private void Shuffle()
    {
        ...
        var cardsLength = _cards.Count - 1;
        while (cardsLength > 1)
        {
            cardsLength--;
            var k = Util.CryptoNextInt(cardsLength + 1);
        ...
        */
        ...
        {
            _cards.Clear();
            foreach (var i in preshuffle) _cards.Push(i);
        }
    }
    ...
    public double GetPseudoDouble()
    {
        ...
        var second = Util.CryptoNextInt(99); //just use a simple random for the .XX values
        ...
        var pseudoDouble = append / (double) 9999;
        return pseudoDouble;
    }

    public string SaveDeckToJSON()
    {
        var json = JsonConvert.SerializeObject(_cards.Reverse());
        return json;
    }
