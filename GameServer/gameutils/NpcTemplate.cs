...
public class NpcTemplate : INpcTemplate
{
    ...
    public NpcTemplate(DBNpcTemplate data)
    {
        ...
        {
            var spells = data.Spells.Split(';');
            for (var k = 0; k < spells.Length; k++)
            {
                var id = int.Parse(spells[k]);
                var sp = SkillBase.GetSpellByID(id);
                if (sp != null)
                    m_spells.Add(sp);
        ...
        if (data.Styles != null && data.Styles.Length > 0)
        {
            ...
            for (var i = 0; i < styles.Length; i++)
            {
                ...
                var styleAndClass = styles[i].Split('|');
                if (styleAndClass.Length != 2) continue;
                var stylePart = styleAndClass[0].Trim();
                var classPart = styleAndClass[1].Trim();
                if (stylePart.Length == 0 || classPart.Length == 0) continue;
                var styleID = int.Parse(stylePart);
                var classID = int.Parse(classPart);
                var style = SkillBase.GetStyleByID(styleID, classID);
                m_styles.Add(style);
            }
        }
        ...
        if (data.Abilities != null && data.Abilities.Length > 0)
            foreach (var splitab in Util.SplitCSV(data.Abilities))
            {
                var ab = splitab.Split('|');
                if (splitab.Trim().Length == 0) continue;
                var id = 1;
                if (ab.Length > 1)
                    id = int.Parse(ab[1]);
                var abil = SkillBase.GetAbility(ab[0], id);
                if (abil != null)
    ...
    public NpcTemplate(GameNPC mob)
    {
        ...
        if (mob.Abilities != null && mob.Abilities.Count > 0)
            try
            {
                ...
                foreach (var mobAbility in mob.Abilities.Values) m_abilities.Add(mobAbility);
            }
            catch (Exception ex)
            {
        ...
        if (mob.Spells != null && mob.Spells.Count > 0)
            try
            {
                ...
                foreach (Spell mobSpell in mob.Spells) m_spells.Add(mobSpell);
            }
            catch (Exception ex)
            {
                log.Error("Trapped Error: ", ex);
        ...
        if (mob.Styles != null && mob.Styles.Count > 0)
            try
            {
                ...
                foreach (Style mobStyle in mob.Styles) m_styles.Add(mobStyle);
            }
            catch (Exception ex)
            {
                log.Error("Trapped Error: ", ex);
        ...
        var brain = mob.Brain as AI.Brain.StandardMobBrain;
        if (brain != null)
        {
        ...
        {
            var merchant = mob as GameMerchant;

            if (merchant != null) merchant.TradeItems = new MerchantTradeItems(ItemsListTemplateID);
        }
    }
    ...
    {
        get => m_visibleActiveWeaponSlot;
        set => m_visibleActiveWeaponSlot = value;
    }

    public virtual void SaveIntoDatabase()
    {
        var tmp = GameServer.Database.FindObjectByKey<DBNpcTemplate>(TemplateId);
        var add = false;
