...
public class LootGeneratorAurulite : LootGeneratorBase
{
    ...
    public override LootList GenerateLoot(GameNPC mob, GameObject killer)
    {
        ...
        var aurulite = GameServer.Database.FindObjectByKey<ItemTemplate>(m_aurulite.Id_nb);
        ...
        try
        {
            ...
            if (!mob.Name.ToLower().Equals(mob.Name))
                ...
                maxcount = (int) Math.Round(maxcount *
                                            ServerProperties.Properties.LOOTGENERATOR_AURULITE_NAMED_COUNT);

            // add to loot
            if (maxcount > 0 && Util.Chance(ServerProperties.Properties.LOOTGENERATOR_AURULITE_BASE_CHANCE +
                                            Math.Max(10, killedcon)))
                // Add to fixed to prevent overrides with loottemplate
                loot.AddFixed(aurulite,
                    (int) Math.Ceiling(maxcount * ServerProperties.Properties.LOOTGENERATOR_AURULITE_AMOUNT_RATIO));
        }
        catch
        {
