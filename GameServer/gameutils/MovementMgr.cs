...
public class MovementMgr
{
    ...
    private static Dictionary<string, DBPath> m_pathCache = new();

    private static Dictionary<string, SortedList<int, DBPathPoint>> m_pathpointCache = new();
    ...
    {
        var allPaths = GameServer.Database.SelectAllObjects<DBPath>();
        foreach (var path in allPaths) m_pathCache.Add(path.PathID, path);

        var duplicateCount = 0;

        var allPathPoints = GameServer.Database.SelectAllObjects<DBPathPoint>();
        foreach (var pathPoint in allPathPoints)
            ...
            {
                var pList = new SortedList<int, DBPathPoint>();
    ...
    public static void UpdatePathInCache(string pathID)
    {
        ...
        foreach (var pathPoint in pathPoints) m_pathpointCache[pathPoint.PathID].Add(pathPoint.Step, pathPoint);
    }
    ...
    public static PathPoint LoadPath(string pathID)
    {
        lock (LockObject)
        {
            ...
            if (m_pathCache.ContainsKey(pathID)) dbpath = m_pathCache[pathID];
    ...
    public static void SavePath(string pathID, PathPoint path)
    {
        ...
        var dbpath = DOLDB<DBPath>.SelectObject(DB.Column("PathID").IsEqualTo(pathID));
        if (dbpath != null) GameServer.Database.DeleteObject(dbpath);
        ...
        var root = FindFirstPathPoint(path);
