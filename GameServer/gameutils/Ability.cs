...
public class Ability : NamedSkill
{
    ...
    public Ability(DBAbility dba, int level, string spec, int speclevel)
        : this(dba.KeyName, dba.Name, dba.Description, dba.AbilityID, (ushort) dba.IconID, level, spec, speclevel,
            dba.InternalID)
    {
    }
    ...
    public Ability(string keyname, string displayname, string description, int id, ushort icon, int level,
        string spec, int speclevel, int internalID)
    ...
    private void UpdateCurrentName()
    {
        try
        {
            var name = m_serializedNames;
            var nameByLevel = new Dictionary<int, string>();
            foreach (var levelNamePair in Util.SplitCSV(name.Trim()))
            {
                if (levelNamePair.Trim().Length <= 0)
                    continue;

                var levelAndName = levelNamePair.Trim().Split('|');
            ...
            else
            {
                ...
                if (entry.Value == null && log.IsWarnEnabled)
                    log.WarnFormat(
                        "Parsing ability display name: keyname='{0}' m_serializedNames='{1}', No Value for Level {2}",
                        KeyName, m_serializedNames, level);
            }

            var roman = getRomanLevel();
    ...
    public virtual IList<string> DelveInfo
    {
        get
        {
            ...
            foreach (var part in m_description.Split(new char[] {'|'})) list.Add(part);
