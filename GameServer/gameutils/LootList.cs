...
public class LootList
{
    ...
    {
        var entry = new LootEntry(chance, loot, count);
    ...
    {
        if (list.m_randomItemDrops != null) m_randomItemDrops.AddRange(list.m_randomItemDrops);

        if (list.m_fixedItemDrops != null) m_fixedItemDrops.AddRange(list.m_fixedItemDrops);
    ...
    {
        var loot = new List<ItemTemplate>(m_fixedItemDrops.Count + DropCount);
        ...
        if (DropCount > 0)
        {
            ...
            foreach (var lootEntry in m_randomItemDrops)
            ...
            if (lootCandidates.Count > 0)
                for (var i = 0; i < DropCount && lootCandidates.Count != 0; i++)
                {
                    var tmpidx = Util.Random(lootCandidates.Count - 1);
