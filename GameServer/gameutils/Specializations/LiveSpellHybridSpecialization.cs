...
public class LiveSpellHybridSpecialization : Specialization
{
    ...
    protected override IDictionary<SpellLine, List<Skill>> GetLinesSpellsForLiving(GameLiving living, int level)
    {
        ...
        var buffer = new Dictionary<SpellLine, List<Skill>>();

        var lines = GetSpellLinesForLiving(living, level);
        ...
        foreach (var ls in lines)
        {
            ...
            var lib = SkillBase.GetSpellList(ls.KeyName).Where(item => item.Level <= ls.Level);
            ...
            var firstBetterSpellNoGroup = lib.Where(item => item.Group == 0)
                .GroupBy(item => new
                    {item.SpellType, item.Target, item.IsAoE, item.IsInstantCast, item.HasSubSpell})
            ...
            var firstBetterSpellFinal = firstBetterSpellGroup.SelectMany(el => el)
                .Union(firstBetterSpellNoGroup.SelectMany(el => el))
            ...
            var baseOrderGroup = lib.Where(item => item.Group != 0)
                ...
                .Select(ins => ins.OrderBy(it => it.Level).Take(take));
            // Join and sort...
            IEnumerable<Spell> baseOrderFinal = baseOrderNoGroup.SelectMany(el => el)
                .Union(baseOrderGroup.SelectMany(el => el))
                .Where(item => item != null).OrderBy(item => item.Level);

            foreach (var sp in baseOrderFinal)
            {
                // replace each spell of order with their best equivalent
                var index = firstBetterSpellFinal.FindIndex(od => (od.Group == 0 && sp.Group == 0
    ...
    protected virtual bool AllowMultipleSpellVersions(SpellLine line, GamePlayer player)
    {
        ...
        switch (line.Spec)
        {
            ...
            case Specs.Enhancement:
                if ((line.IsBaseLine || player.CharacterClass.ID == (int) eCharacterClass.Cleric) &&
