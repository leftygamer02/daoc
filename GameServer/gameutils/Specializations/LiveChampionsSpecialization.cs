...
public class LiveChampionsSpecialization : CareerSpecialization
{
    ...
    public virtual byte MaxChampionSpecLevel => 5;
    ...
    {
        if (living is GamePlayer) return base.GetAbilitiesForLiving(living, ((GamePlayer) living).ChampionLevel);
    ...
    protected override IDictionary<SpellLine, List<Skill>> GetLinesSpellsForLiving(GameLiving living, int level)
    {
        if (living is GamePlayer)
        {
            ...
            {
                var skills = spec.GetMiniLineSkillsForLiving(living, spec.GetSpecLevelForLiving(living))
                    .Where(e => e.Item1 is Spell);
    ...
    protected override List<Styles.Style> GetStylesForLiving(GameLiving living, int level)
    {
        if (living is GamePlayer)
        {
            ...
            var result = new List<Styles.Style>();

            foreach (var spec in specs)
            {
                var skills = spec.GetMiniLineSkillsForLiving(living, spec.GetSpecLevelForLiving(living))
                    .Where(e => e.Item1 is Styles.Style);

                foreach (var elem in skills)
                {
                    elem.Item1.Level = player.MaxLevel;
                    result.Add((Styles.Style) elem.Item1);
                }
            }
    ...
    /// </summary>
    /// <param name="living"></param>
    public virtual List<Tuple<MiniLineSpecialization, List<Tuple<Skill, byte>>>> GetTrainerTreeDisplay(
        GameLiving living, Type type)
    {
        if (typeof(MiniLineSpecialization).IsAssignableFrom(type))
        {
            ...
            var allLines =
                new List<Tuple<MiniLineSpecialization, List<Skill>>>();

            foreach (var spec in minispecs)
            {
                var minispec = (MiniLineSpecialization) spec;
                minispec.Level = living.GetBaseSpecLevel(minispec.KeyName);
                allLines.Add(new Tuple<MiniLineSpecialization, List<Skill>>(minispec,
            ...
            for (var i = 0; i < MaxChampionSpecLevel; i++)
            {
                var grouped = allLines.Where(l => l.Item2.Count > i)
                    .GroupBy(l => new {l.Item2[i].Name, l.Item2[i].ID})
                    .Where(grp => grp.Count() > 1 && grp.Count() < 4)
                    .Select(grp => grp.OrderBy(msp => msp.Item1.KeyName));
                ...
                foreach (var groups in grouped)
                    ...
                    else if (groups.Count() == 2)
                    {
                        ...
                        // Remaining
                        var ins = final.Count - 1;
                        final.Insert(ins, new Tuple<MiniLineSpecialization, List<Tuple<Skill, byte>>>(
                            groups.First().Item1,
                            groups.First().Item2.Skip(i).Select(e => new Tuple<Skill, byte>(e, 0)).ToList()));
            ...
            var pivot = final.Count > 0 ? final[0] : null;
            ...
            foreach (var elem in allLines)
            {
                ...
                {
                    // insert
                    var ins = final.FindIndex(el => el == pivot);
                    if (ins > -1)
    ...
    public virtual Tuple<byte, MiniLineSpecialization> GetSkillStatus(
        List<Tuple<MiniLineSpecialization, List<Tuple<Skill, byte>>>> tree, int skillIndex, int itemIndex)
    {
        try
        {
            // Test if Available For Training
            var findlink = tree[skillIndex].Item2.FindIndex(it => it.Item2 > 1);
            ...
            if (findlink > -1 && findlink <= itemIndex)
            {
                ...
                if (tree[skillIndex - 1].Item1.Level >= itemIndex)
                    return new Tuple<byte, MiniLineSpecialization>(
                        (byte) (tree[skillIndex - 1].Item1.Level > itemIndex ? 1 : 2),
                        tree[skillIndex - 1].Item1);
                // next line
                ...
                // If 3-attachments mainline too !
                    ...
                    return new Tuple<byte, MiniLineSpecialization>(
                        (byte) (tree[skillIndex].Item1.Level > itemIndex ? 1 : 2), tree[skillIndex].Item1);
            }
