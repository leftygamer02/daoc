...
public class Specialization : NamedSkill
{
    ...
    /// type of skill
    /// </summary>
    public override eSkillPage SkillType => eSkillPage.Specialization;

    /// <summary>
    /// Is this Specialization Trainable ?
    /// </summary>
    public virtual bool Trainable => true;
    ...
    /// </summary>
    public virtual bool HybridSpellList => false;
    ...
    protected virtual List<SpellLine> GetSpellLinesForLiving(GameLiving living, int level)
    {
        ...
        var spsl = SkillBase.GetSpecsSpellLines(KeyName);
        ...
        if (living is GamePlayer)
        {
            ...
            var tmp = spsl
                .Where(item => item.Item1.IsBaseLine || player.CharacterClass.HasAdvancedFromBaseClass())
            ...
            if (baseline.Any())
                ...
                foreach (var ls in tmp.Where(item => item.Item1.IsBaseLine && item.Item2 == 0))
                {
                    ls.Item1.Level = player.Level;
                    list.Add(ls.Item1);
                }
        ...
        else
        {
            ...
            foreach (var ls in spsl.OrderBy(item => item.Item1.IsBaseLine ? 0 : 1)
    ...
    protected virtual IDictionary<SpellLine, List<Skill>> GetLinesSpellsForLiving(GameLiving living, int level)
    {
        ...
        foreach (var sl in GetSpellLinesForLiving(living, level))
    ...
    {
        return SkillBase
            .GetSpecAbilityList(KeyName, living is GamePlayer ? ((GamePlayer) living).CharacterClass.ID : 0)
    ...
    protected virtual List<Ability> GetAbilitiesForLiving(GameLiving living, int level)
    {
        ...
        var abs = SkillBase.GetSpecAbilityList(KeyName,
            living is GamePlayer ? ((GamePlayer) living).CharacterClass.ID : 0);

        // Get order of first appearing skills
        var order = abs.GroupBy(item => item.KeyName)
        ...
        // Get best of skills
        var best = abs.Where(item => item.SpecLevelRequirement <= level)
        ...
        foreach (var ab in order)
            for (var r = 0; r < best.Count; r++)
    ...
    protected virtual List<Style> GetStylesForLiving(GameLiving living, int level)
    {
        ...
        var classid = 0;
        if (living is GamePlayer) classid = ((GamePlayer) living).CharacterClass.ID;
