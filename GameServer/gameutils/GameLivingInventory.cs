...
public abstract class GameLivingInventory : IGameInventory
{
    ...
    /// <returns></returns>
    protected virtual eInventorySlot FindSlot(eInventorySlot first, eInventorySlot last, bool searchFirst,
        bool searchNull)
    {
        lock (m_items) // Mannen 10:56 PM 10/30/2006 - Fixing every lock(this)
        {
    ...
    public virtual bool AddItem(eInventorySlot slot, InventoryItem item)
    {
        ...
        lock (m_items) // Mannen 10:56 PM 10/30/2006 - Fixing every lock(this)
        {
            ...
            if (item.OwnerID != null) item.OwnerID = null; // owner ID for NPC
    ...
    public virtual void ClearInventory()
    {
        ...
        foreach (var entry in m_items) tempList.Add(entry.Value);

        foreach (var item in tempList) RemoveItem(item);
    }
    ...
    public virtual ICollection<InventoryItem> VisibleItems
    {
        get
        {
            ...
            lock (m_items) // Mannen 10:56 PM 10/30/2006 - Fixing every lock(this)
            {
                foreach (var slot in VISIBLE_SLOTS)
                {
                    InventoryItem item;

                    if (m_items.TryGetValue(slot, out item)) items.Add(item);
                }
            }
    ...
    /// </summary>
    public virtual ICollection<InventoryItem> AllItems => m_items.Values;
    ...
    public virtual bool AddTemplate(InventoryItem sourceItem, int count, eInventorySlot minSlot,
        eInventorySlot maxSlot)
    {
        ...
        lock (m_items) // Mannen 10:56 PM 10/30/2006 - Fixing every lock(this)
        {
            ...
            do
            {
                ...
                var curSlot = GetValidInventorySlot(i);
                ...
                var countFree = curItem.MaxCount - curItem.Count;
                ...
                if (count < 0) throw new Exception("Count is less than zero while filling gaps, should never happen!");
            } while (++i <= maxSlot);
            ...
            try
            {
                ...
                {
                    InventoryItem item;
                    var itemSlot = slot.Key;
                    var itemCount = slot.Value;

                    if (itemCount > 0) // existing item should be changed
                    {
                        if (m_items.TryGetValue(itemSlot, out item)) AddCountToStack(item, itemCount);
                    }
        ...
        get
        {
            ...
            foreach (var item in items)
            {
                if (!EQUIP_SLOTS.Contains((eInventorySlot) item.SlotPosition))
                    continue;
                if ((eInventorySlot) item.SlotPosition is eInventorySlot.FirstQuiver or eInventorySlot.SecondQuiver
                    or eInventorySlot.ThirdQuiver or eInventorySlot.FourthQuiver)
                    continue;
                weight += item.Weight;
            }
    ...
    {
        var changes = Interlocked.Decrement(ref m_changesCounter);
        if (changes < 0)
        {
            if (Log.IsErrorEnabled)
    ...
    public object InventorySlotLock = new();
