...
public class GZipCompress
{
    ...
    public static bool compressFile(string source, string dest)
    {
        try
        {
            ...
            var buf = new byte[fstream.Length];
            ...
            var zipStream = new GZipStream(fstream, CompressionMode.Compress, false);
...
public class MailMgr
{
    ...
    private static Queue m_mailQueue = new();
    ...
    public static bool Init()
    {
        ...
        var file = new FileInfo("./config/MailConfig.xml");
        ...
        if (GS.ServerProperties.Properties.LOG_EMAIL_ADDRESSES != "")
            SendLogs(GS.ServerProperties.Properties.LOG_EMAIL_ADDRESSES);
    ...
    public static bool SendMail(string to, string subject, string message, string fromName, string fromAddress)
    {
        ...
        try
        {
            ...
            foreach (var str in Util.SplitCSV(to))
                mail.To.Add(str);
    ...
    {
        var info = new FileInfo(GameServer.Instance.Configuration.LogConfigFile);
        var dirList = new Queue();

        try
        {
            if (info.Exists)
            {
                var archiveList = new Queue(); // List made of all archived logs urls.

                // We parse the xml file to find urls of logs.
                var configFileReader = new XmlTextReader(info.OpenRead());
                ...
                while (configFileReader.Read())
                    if (configFileReader.LocalName == "file")
                    {
                        ...
                        foreach (var file in Directory.GetFiles(info.DirectoryName, info.Name + "*"))
    ...
    public static bool SendLogs(string to)
    {
        ...
        try
        {
            ...
            var archivedLogsUrls = new Queue(GetArchivedLogsUrls());
            ...
            if (archivedLogsUrls.Count > 0)
            {
                ...
                foreach (string file in archivedLogsUrls)
                {
                    ...
                    var info = new FileInfo(file + ".gz");
                    var a = new Attachment(file + ".gz");
                    a.Name = info.Name;
    ...
    private static void RunTask(object state)
    {
        ...
        lock (m_mailQueue.SyncRoot)
        {
            while (m_mailQueue.Count > 0)
            {
                try
                {
                    var messageCount = m_mailQueue.Count;
