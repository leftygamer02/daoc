...
public sealed class GuildMgr
{
    ...
    /// </summary>
    private static readonly HybridDictionary m_guilds = new();

    /// <summary>
    /// ArrayList of all GuildIDs to GuildNames
    /// </summary>
    private static readonly HybridDictionary m_guildids = new();
    ...
    private static readonly Dictionary<string, Dictionary<string, GuildMemberDisplay>> m_guildXAllMembers = new();
    ...
    public static void AddPlayerToAllGuildPlayersList(GamePlayer player)
    {
        if (m_guildXAllMembers.ContainsKey(player.GuildID))
            if (!m_guildXAllMembers[player.GuildID].ContainsKey(player.InternalID))
            {
                ...
                var member = new GuildMemberDisplay(player.InternalID,
                    player.Name,
    ...
    public static Guild CreateGuild(eRealm realm, string guildName, GamePlayer creator = null)
    {
        ...
        {
            if (creator != null)
                creator.Out.SendMessage(guildName + " already exists!", eChatType.CT_System,
        ...
        try
        {
            ...
            dbGuild.GuildID = Guid.NewGuid().ToString();
            dbGuild.Realm = (byte) realm;
            var newguild = new Guild(dbGuild);
            if (newguild.AddToDatabase() == false)
            {
                if (creator != null)
                    creator.Out.SendMessage("Database error, unable to add a new guild!", eChatType.CT_System,
                        eChatLoc.CL_SystemWindow);
            ...
            if (log.IsDebugEnabled)
                log.Debug("Create guild; guild name=\"" + guildName + "\" Realm=" +
                          GlobalConstants.RealmToName(newguild.Realm));
    ...
    public static bool DeleteGuild(string guildName)
    {
        try
        {
            var removeGuild = GetGuildByName(guildName);
            ...
            foreach (var cha in DOLDB<DOLCharacters>.SelectObjects(
                         DB.Column("GuildID").IsEqualTo(guild.GuildID)))
                cha.GuildID = "";
            ...
            {
                foreach (var ply in removeGuild.GetListOfOnlineMembers())
                {
                    ply.Guild = null;
                    ply.GuildID = "";
                    ply.GuildName = "";
    ...
    {
        var g = GetGuildByName(guildName);
        if (g == null)
            return "";
        return g.GuildID;
    }
    ...
    public static bool LoadAllGuilds()
    {
        ...
        foreach (var obj in guildObjs)
        {
                ...
                obj.Ranks[9] == null)
            {
                ...
                myguild = new Guild(DOLDB<DBGuild>.SelectObjects(DB.Column("GuildID").IsEqualTo(obj.GuildID))
                    .FirstOrDefault());
            }
            ...
            var guildCharacters =
                DOLDB<DOLCharacters>.SelectObjects(DB.Column("GuildID").IsEqualTo(myguild.GuildID));
        ...
        foreach (var dball in allianceObjs)
        {
            ...
            if (dball != null && dball.DBguilds != null)
                foreach (var mydbgui in dball.DBguilds)
                {
    ...
    public static void SaveAllGuilds()
    {
        ...
        try
        {
            lock (m_guilds.SyncRoot)
            {
                foreach (Guild g in m_guilds.Values) g.SaveIntoDatabase();
            }
        }
        catch (Exception e)
        {
    ...
    public static void ChangeEmblem(GamePlayer player, int oldemblem, int newemblem)
    {
        ...
        if (oldemblem != 0)
        {
            ...
            InventoryLogging.LogInventoryAction(player, "(GUILD;" + player.GuildName + ")",
                eInventoryActionType.Other, COST_RE_EMBLEM);
            var objs = DOLDB<InventoryItem>.SelectObjects(DB.Column("Emblem").IsEqualTo(oldemblem));

            foreach (var item in objs) item.Emblem = newemblem;
            ...
            {
                var guildHouse = Housing.HouseMgr.GetHouse(player.Guild.GuildHouseNumber);
    ...
    public static List<Guild> GetAllGuilds()
    {
        ...
        lock (m_guilds.SyncRoot)
        {
            foreach (Guild guild in m_guilds.Values) guilds.Add(guild);
        }
    ...
    public class GuildMemberDisplay
    {
        ...
        private string m_characterClassID;

        public string ClassID
        {
            get => m_characterClassID;
        ...
        public GuildMemberDisplay(string internalID, string name, string level, string classID, string rank,
            string group, string zoneOrOnline, string note)
        {
        ...
        public string ToString(int position, int guildPop)
        {
            return string.Format("E,{0},{1},{2},{3},{4},{5},{6},\"{7}\",\"{8}\"",
                position, guildPop, m_name, m_level, m_characterClassID, m_rank, m_groupSize, m_zoneOnline,
                m_guildNote);
        }
