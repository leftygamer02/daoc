...
public class LosCheckMgr
{
    ...
    /// </summary>
    private readonly Dictionary<GamePlayer, long> m_clientChecks = new();

    /// <summary>
    /// Dictionary to keep players and last CheckLos Ticks.
    /// </summary>
    private Dictionary<GamePlayer, long> ClientChecks => m_clientChecks;

    /// <summary>
    /// Dictionary to keep clients LoS stats Tuple(Number of checks, average reply time, last reply time)
    /// </summary>
    private readonly Dictionary<GamePlayer, Tuple<int, int, int>> m_clientStats = new();

    /// <summary>
    /// Dictionary to keep clients LoS stats Tuple(Number of checks, average reply time, last reply time)
    /// </summary>
    private Dictionary<GamePlayer, Tuple<int, int, int>> ClientStats => m_clientStats;

    /// <summary>
    /// Dictionary to keep Target to Target LoSChecks and cache them with updated time.
    /// </summary>
    private readonly Dictionary<Tuple<GameObject, GameObject>, Tuple<long, bool>> m_responsesCache = new();

    /// <summary>
    /// Dictionary to keep Target to Target LoSChecks and cache them with updated time.
    /// </summary>
    private Dictionary<Tuple<GameObject, GameObject>, Tuple<long, bool>> ResponsesCache => m_responsesCache;

    /// <summary>
    /// Dictionary to keep Pending LoSChecks
    /// </summary>
    private readonly Dictionary<Tuple<GamePlayer, ushort, ushort>, long> m_pendingChecks = new();

    /// <summary>
    /// Dictionary to keep Pending LoSChecks
    /// </summary>
    private Dictionary<Tuple<GamePlayer, ushort, ushort>, long> PendingChecks => m_pendingChecks;

    /// <summary>
    /// Dictionary holding a list of object needing Notify
    /// </summary>
    private Dictionary<Tuple<GameObject, GameObject>, List<IDOLEventHandler>> m_registeredLosEvents = new();

    /// <summary>
    /// Dictionary holding a list of object needing Notify
    /// </summary>
    private Dictionary<Tuple<GameObject, GameObject>, List<IDOLEventHandler>> RegisteredLosEvents =>
        m_registeredLosEvents;
    ...
    /// </summary>
    public static int LOSMGR_QUERY_TIMEOUT => ServerProperties.Properties.LOSMGR_QUERY_TIMEOUT;

    /// <summary>
    /// Default PvP LoS Cache Timeout in ms
    /// </summary>		
    public static int LOSMGR_PLAYER_VS_PLAYER_CACHE_TIMEOUT =>
        ServerProperties.Properties.LOSMGR_PLAYER_VS_PLAYER_CACHE_TIMEOUT;

    /// <summary>
    /// Default PvE LoS Cache Timeout in ms
    /// </summary>		
    public static int LOSMGR_PLAYER_VS_ENVIRONMENT_CACHE_TIMEOUT =>
        ServerProperties.Properties.LOSMGR_PLAYER_VS_ENVIRONMENT_CACHE_TIMEOUT;

    /// <summary>
    /// Default EvE LoS Cache Timeout in ms
    /// </summary>		
    public static int LOSMGR_ENVIRONMENT_VS_ENVIRONMENT_CACHE_TIMEOUT =>
        ServerProperties.Properties.LOSMGR_ENVIRONMENT_VS_ENVIRONMENT_CACHE_TIMEOUT;

    /// <summary>
    /// Player Los Frequency in ms 
    /// </summary>				
    public static int LOSMGR_PLAYER_CHECK_FREQUENCY => ServerProperties.Properties.LOSMGR_PLAYER_CHECK_FREQUENCY;

    /// <summary>
    /// PvP Los range Threshold (should be short)
    /// </summary>
    public static int LOSMGR_PLAYER_VS_PLAYER_RANGE_THRESHOLD =>
        ServerProperties.Properties.LOSMGR_PLAYER_VS_PLAYER_RANGE_THRESHOLD;

    /// <summary>
    /// PvE Los range Threshold
    /// </summary>
    public static int LOSMGR_PLAYER_VS_ENVIRONMENT_RANGE_THRESHOLD =>
        ServerProperties.Properties.LOSMGR_PLAYER_VS_ENVIRONMENT_RANGE_THRESHOLD;

    /// <summary>
    /// EvE Los range Threshold should be largest
    /// </summary>
    public static int LOSMGR_ENVIRONMENT_VS_ENVIRONMENT_RANGE_THRESHOLD =>
        ServerProperties.Properties.LOSMGR_ENVIRONMENT_VS_ENVIRONMENT_RANGE_THRESHOLD;

    /// <summary>
    /// Contamination Radius used for EvE and max range checks.
    /// </summary>
    public static int LOSMGR_MAX_CONTAMINATION_RADIUS => ServerProperties.Properties.LOSMGR_MAX_CONTAMINATION_RADIUS;

    /// <summary>
    /// Contamination radius in which NPC receive LoS Checks (should be under MAX)
    /// </summary>
    public static int LOSMGR_NPC_CONTAMINATION_RADIUS => ServerProperties.Properties.LOSMGR_NPC_CONTAMINATION_RADIUS;

    /// <summary>
    /// Contamination radius in which Player Pets receive LoS Checks
    /// </summary>
    public static int LOSMGR_PET_CONTAMINATION_RADIUS => ServerProperties.Properties.LOSMGR_PET_CONTAMINATION_RADIUS;

    /// <summary>
    /// Contamination radius in which Players receive LoS Checks
    /// </summary>
    public static int LOSMGR_PLAYER_CONTAMINATION_RADIUS =>
        ServerProperties.Properties.LOSMGR_PLAYER_CONTAMINATION_RADIUS;
    ...
    public static int LOSMGR_GUARD_CONTAMINATION_RADIUS =>
        ServerProperties.Properties.LOSMGR_GUARD_CONTAMINATION_RADIUS;

    /// <summary>
    /// Contamination radius Z-factor
    /// </summary>
    public static double LOSMGR_CONTAMINATION_ZFACTOR => ServerProperties.Properties.LOSMGR_CONTAMINATION_ZFACTOR;

    /// <summary>
    /// Max Cleanup Entries, to prevent locking the Dictionaries for too long
    /// </summary>				
    public static int MAX_CLEANUP_ENTRIES => ServerProperties.Properties.LOSMGR_CLEANUP_ENTRIES;

    /// <summary>
    /// Cleanup Frequency in ms, too long cleanup can be intensive and won't clean all up, too fast and it could clean useful data
    /// </summary>				
    public static int LOSMGR_CLEANUP_FREQUENCY => ServerProperties.Properties.LOSMGR_CLEANUP_FREQUENCY;

    /// <summary>
    /// Get the Debug level of this Los Check Manager
    /// </summary>
    public static int LOSMGR_DEBUG_LEVEL => ServerProperties.Properties.LOSMGR_DEBUG_LEVEL;
    ...
    public LosCheckMgr()
    {
        var rndRange = LOSMGR_CLEANUP_FREQUENCY >> 3;
        m_timerCleanup = new System.Timers.Timer(Util.Random(LOSMGR_CLEANUP_FREQUENCY - rndRange,
            LOSMGR_CLEANUP_FREQUENCY + rndRange));
    ...
    public void LosCheck(GamePlayer player, GameObject source, GameObject target, LosMgrResponse notifier,
        bool cached = true, int timeout = 0)
    {
    ...
    /// <param name="cached">Use a cached result</param>
    /// <param name="timeout">Cache Timeout, 0 = default</param>
    public void LosCheck(GamePlayer player, GameObject source, GameObject target, IDOLEventHandler notifier,
        bool cached = true, int timeout = 0)
    {
        ...
        if (LOSMGR_DEBUG_LEVEL >= LOSMGR_DEBUG_WARN)
            log.Warn("LOSMGR_W : Starting Los Check with - Player : " + player.Name + " Source : " + source.Name +
                     " Target : " + target.Name + ".");
        ...
        if (source.IsWithinRadius(target, GetDefaultThreshold(source, target), false))
        {
            ...
            if (LOSMGR_DEBUG_LEVEL >= LOSMGR_DEBUG_DEBUG)
                log.Warn("LOSMGR_D : Threshold hitted (" + GetDefaultThreshold(source, target) +
                         ") with - Player : " + player.Name + " Source : " + source.Name + " Target : " +
                         target.Name + ".");

            notifier.Notify(GameObjectEvent.FinishedLosCheck, player,
                new LosCheckData(source, target, GameTimer.GetTickCount(), true));
            return;
        }
        ...
        if (cached)
            try
            {
                var los = GetLosCheckFromCache(source, target, timeout);
                notifier.Notify(GameObjectEvent.FinishedLosCheck, player,
                    new LosCheckData(source, target, GameTimer.GetTickCount(), los));
                return;
            }
        ...
        var cacheKey = new Tuple<GameObject, GameObject>(source, target);

        // We need to lock the Pending list during the checks
        lock (((ICollection) PendingChecks).SyncRoot)
        {
            var pendings = (from pending in PendingChecks
                where (pending.Key.Item2 == source.ObjectID && pending.Key.Item3 == target.ObjectID) ||
                      (pending.Key.Item2 == target.ObjectID && pending.Key.Item3 == source.ObjectID)
                select pending.Key).Take(1);
            ...
            foreach (var pend in pendings)
            {
                ...
                if (LOSMGR_DEBUG_LEVEL >= LOSMGR_DEBUG_INFO)
                    log.Warn("LOSMGR_D : Registered to an other LoS with - Player : " + player.Name + " Source : " +
        ...
        {
            // Get best checker !
            var checker = GetBestLosChecker(source, target);

            if (checker != null)
            {
                // FIXME Debug
                if (LOSMGR_DEBUG_LEVEL >= LOSMGR_DEBUG_WARN)
                    log.Warn("LOSMGR_D : Deferred LoSCheck to " + checker.Name + " From - Player : " + player.Name +
    ...
    /// <param name="cached">Use a cached result</param>
    /// <param name="timeout">Cache Timeout, 0 = default</param>
    public void LosCheckVincinity(GameObject source, GameObject target, LosMgrResponse notifier, bool cached = true,
        int timeout = 0)
    {
    ...
    /// <param name="cached">Use a cached result</param>
    /// <param name="timeout">Cache Timeout, 0 = default</param>
    public void LosCheckVincinity(GameObject source, GameObject target, IDOLEventHandler notifier,
        bool cached = true, int timeout = 0)
    {
        // FIXME debug
        if (LOSMGR_DEBUG_LEVEL >= LOSMGR_DEBUG_WARN)
            log.Warn("LOSMGR_W : Starting Vincinity Los Check Between - Source : " + source.Name + " Target : " +
                     target.Name + ".");
        ...
        if (source.IsWithinRadius(target, GetDefaultThreshold(source, target), false))
            ...
            foreach (GamePlayer player in source.GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE))
                if (player.ObjectState == GameObject.eObjectState.Active)
                {
                    // FIXME debug
                    if (LOSMGR_DEBUG_LEVEL >= LOSMGR_DEBUG_DEBUG)
                        log.Warn("LOSMGR_D : Vincinity Hitted Treshold (" + GetDefaultThreshold(source, target) +
                                 ") - Source : " + source.Name + " Target : " + target.Name + ".");
        ...
        lock (((ICollection) PendingChecks).SyncRoot)
        {
            ...
            foreach (var pend in pendings)
            {
                // FIXME debug
                if (LOSMGR_DEBUG_LEVEL >= LOSMGR_DEBUG_INFO)
                    log.Warn("LOSMGR_I : Vincinity Registered to an other LoS - Source : " + source.Name +
                             " Target : " + target.Name + ".");
        ...
        {
            var owner = ((IControlledBrain) ((GameNPC) source).Brain).GetPlayerOwner();
        ...
        {
            var tgtowner = ((IControlledBrain) ((GameNPC) target).Brain).GetPlayerOwner();
        ...
        if (checker != null)
        {
            // FIXME debug
            if (LOSMGR_DEBUG_LEVEL >= LOSMGR_DEBUG_INFO)
                log.Warn("LOSMGR_I : Vincinity found best checker " + checker.Name + " - Source : " + source.Name +
                         " Target : " + target.Name + ".");
    ...
    private void EventNotifierLosCheck(GamePlayer player, GameObject source, GameObject target)
    {
        // FIXME debug
        if (LOSMGR_DEBUG_LEVEL >= LOSMGR_DEBUG_WARN)
            log.Warn("LOSMGR_W : LoS Sent - Player : " + player.Name + ", Source : " + source.Name + ", Target : " +
                     target.Name + ".");

        // pending key to store this lookup
        var checkerKey =
            new Tuple<GamePlayer, ushort, ushort>(player, (ushort) source.ObjectID, (ushort) target.ObjectID);

        var time = GameTimer.GetTickCount();
    ...
    private void LosResponseHandler(GamePlayer player, ushort response, ushort sourceOID, ushort targetOID)
    {
        ...
        // get time
        var sent = GameTimer.GetTickCount();
        var time = sent;

        // Check result
        var losOK = (response & 0x100) == 0x100;

        // key for pending
        var checkerKey =
            new Tuple<GamePlayer, ushort, ushort>(player, sourceOID, targetOID);

        // Object from OID
        var source = player.CurrentRegion.GetObject(sourceOID);
        var target = player.CurrentRegion.GetObject(targetOID);

        // FIXME remove debug
        if (LOSMGR_DEBUG_LEVEL >= LOSMGR_DEBUG_WARN)
            log.Warn("LOSMGR_W : Los Check Finished - Player : " + player.Name + " Source : " +
                     (source != null ? source.Name : "null") + "(" + sourceOID + ") Target : " +
                     (target != null ? target.Name : "null") + "(" + targetOID + ") LoS : " +
                     (losOK ? "OK" : "KO"));
        ...
        lock (((ICollection) PendingChecks).SyncRoot)
        {
            ...
            if (PendingChecks.ContainsKey(checkerKey))
            {
                ...
                if (ClientStats.ContainsKey(player))
                {
                    ...
                    total += time - sent;
                    var count = ClientStats[player].Item1 + 1;
                    ClientStats[player] = new Tuple<int, int, int>(count, (int) total / count, (int) (time - sent));
                    // FIXME debug
                    if (LOSMGR_DEBUG_LEVEL >= LOSMGR_DEBUG_DEBUG)
                        log.Warn("LOSMGR_D : Clients Stats Update : " + player.Name + ", count : " + count +
                                 ", average : " + (int) total / count + ", instant : " + (int) (time - sent));
                }
                else
                {
            ...
            else
            {
                ...
                // FIXME Display debug
                if (LOSMGR_DEBUG_LEVEL >= LOSMGR_DEBUG_INFO)
                    log.Warn("LOSMGR_I : Los Check Without pending objects - Player : " + player.Name +
                             " Source : " + (source != null ? source.Name : "null") + "(" + sourceOID +
                             ") Target : " + (target != null ? target.Name : "null") + "(" + targetOID +
                             ") LoS : " + (losOK ? "OK" : "KO") + " !");
            }
        }
        ...
        {
            var timeout = GetDefaultTimeouts(source, target);
        ...
        // key for registered
        var cachekey = new Tuple<GameObject, GameObject>(source, target);
        var rcachekey = new Tuple<GameObject, GameObject>(target, source);

        // We should Notify objects asking for this Check or the reverse one.
        lock (((ICollection) RegisteredLosEvents).SyncRoot)
        {
            var losData = new LosCheckData(source, target, sent, losOK);
    ...
    private static void NotifyObjects(IList<IDOLEventHandler> notifiers, GamePlayer player, LosCheckData data)
    {
        ...
        {
            foreach (var notifier in notifiers) notifier.Notify(GameObjectEvent.FinishedLosCheck, player, data);
    ...
    private void UpdateVincinityLosCache(GameObject source, GameObject target, bool losOK, long time)
    {
        ...
        foreach (GameNPC contamined in source.GetNPCsInRadius((ushort) LOSMGR_MAX_CONTAMINATION_RADIUS))
        {
            ...
            if ((isObjectFromPlayer(target) || isObjectFromPlayer(source)) && isObjectFromPlayer(contamined))
            {
                ...
                if (LOSMGR_PET_CONTAMINATION_RADIUS > 0 &&
                    source.GetDistanceTo(contamined, LOSMGR_CONTAMINATION_ZFACTOR) <=
                    LOSMGR_PET_CONTAMINATION_RADIUS)
                {
                    // FIXME debug
                    if (LOSMGR_DEBUG_LEVEL >= LOSMGR_DEBUG_DEBUG)
                        log.Warn("LOSMGR_D : Contamination Los Check (Pet PvP) of " + source.Name + " and " +
                                 target.Name + " Contaminated " + contamined.Name + ", range : " +
                                 source.GetDistanceTo(contamined, LOSMGR_CONTAMINATION_ZFACTOR));
            ...
            if (contamined is GameKeepGuard)
            {
                ...
                if (LOSMGR_GUARD_CONTAMINATION_RADIUS > 0 &&
                    source.GetDistanceTo(contamined, LOSMGR_CONTAMINATION_ZFACTOR) <=
                    LOSMGR_GUARD_CONTAMINATION_RADIUS)
                {
                    // FIXME debug
                    if (LOSMGR_DEBUG_LEVEL >= LOSMGR_DEBUG_DEBUG)
                        log.Warn("LOSMGR_D : Contamination Los Check (Grd PvE) of " + source.Name + " and " +
                                 target.Name + " Contaminated " + contamined.Name + ", range : " +
                                 source.GetDistanceTo(contamined, LOSMGR_CONTAMINATION_ZFACTOR));
            ...
            {
                // Update using PvE cache Timeout
                if (LOSMGR_NPC_CONTAMINATION_RADIUS > 0 &&
                    source.GetDistanceTo(contamined, LOSMGR_CONTAMINATION_ZFACTOR) <=
                    LOSMGR_NPC_CONTAMINATION_RADIUS)
                {
                    // FIXME debug
                    if (LOSMGR_DEBUG_LEVEL >= LOSMGR_DEBUG_DEBUG)
                        log.Warn("LOSMGR_D : Contamination Los Check (Mob PvE) of " + source.Name + " and " +
                                 target.Name + " Contaminated " + contamined.Name + ", range : " +
                                 source.GetDistanceTo(contamined, LOSMGR_CONTAMINATION_ZFACTOR));
            ...
            if (source.GetDistanceTo(contamined, LOSMGR_CONTAMINATION_ZFACTOR) <= LOSMGR_MAX_CONTAMINATION_RADIUS)
            {
                // FIXME debug
                if (LOSMGR_DEBUG_LEVEL >= LOSMGR_DEBUG_DEBUG)
                    log.Warn("LOSMGR_D : Contamination Los Check (Mob EvE) of " + source.Name + " and " +
                             target.Name + " Contaminated " + contamined.Name + ", range : " +
                             source.GetDistanceTo(contamined, LOSMGR_CONTAMINATION_ZFACTOR));
        ...
        foreach (GamePlayer contamined in source.GetPlayersInRadius((ushort) LOSMGR_NPC_CONTAMINATION_RADIUS))
        {
            ...
            {
                if (LOSMGR_PLAYER_CONTAMINATION_RADIUS > 0 &&
                    source.GetDistanceTo(contamined, LOSMGR_CONTAMINATION_ZFACTOR) <=
                    LOSMGR_PLAYER_CONTAMINATION_RADIUS)
                {
                    // FIXME debug
                    if (LOSMGR_DEBUG_LEVEL >= LOSMGR_DEBUG_DEBUG)
                        log.Warn("LOSMGR_D : Contamination Los Check (Pl PvP) of " + source.Name + " and " +
                                 target.Name + " Contaminated " + contamined.Name + ", range : " +
                                 source.GetDistanceTo(contamined, LOSMGR_CONTAMINATION_ZFACTOR));
            ...
            if (isObjectFromPlayer(target) || isObjectFromPlayer(source))
            {
                    ...
                    LOSMGR_PET_CONTAMINATION_RADIUS)
                {
                    // FIXME debug
                    if (LOSMGR_DEBUG_LEVEL >= LOSMGR_DEBUG_DEBUG)
                        log.Warn("LOSMGR_D : Contamination Los Check (Pl PvPets) of " + source.Name + " and " +
                                 target.Name + " Contaminated " + contamined.Name + ", range : " +
                                 source.GetDistanceTo(contamined, LOSMGR_CONTAMINATION_ZFACTOR));
            ...
            if (source.GetDistanceTo(contamined, LOSMGR_CONTAMINATION_ZFACTOR) <= LOSMGR_NPC_CONTAMINATION_RADIUS)
            {
                // FIXME debug
                if (LOSMGR_DEBUG_LEVEL >= LOSMGR_DEBUG_DEBUG)
                    log.Warn("LOSMGR_D : Contamination Los Check (Pl PvE) of " + source.Name + " and " +
                             target.Name + " Contaminated " + contamined.Name + ", range : " +
                             source.GetDistanceTo(contamined, LOSMGR_CONTAMINATION_ZFACTOR));
    ...
    private void UpdateLosCacheItem(GameObject source, GameObject target, bool losOK, int timeout, long time)
    {
        ...
        var cacheValue = new Tuple<long, bool>(time, losOK);
    ...
    {
        return (obj is GameNPC && ((GameNPC) obj).Brain is IControlledBrain &&
                ServerProperties.Properties.ALWAYS_CHECK_PET_LOS &&
                ((IControlledBrain) ((GameNPC) obj).Brain).GetPlayerOwner() != null)
               || obj is GamePlayer;
    }
    ...
    public bool IsAvailableForLosCheck(GameObject checker, GameObject source, GameObject target)
    {
        if (checker is GamePlayer && target != null && source != null)
        {
            ...
            {
                if (ClientChecks.ContainsKey((GamePlayer) checker) &&
                    GetElapsedTicks(ClientChecks[(GamePlayer) checker]) < LOSMGR_PLAYER_CHECK_FREQUENCY)
                {
                    // FIXME Debug
                    if (LOSMGR_DEBUG_LEVEL >= LOSMGR_DEBUG_WARN)
    ...
    private GamePlayer GetBestLosChecker(GameObject source, GameObject target)
    {
        ...
        var playersQuery =
            from GamePlayer player in source.GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE) select player;

        var players = new List<GamePlayer>();

        foreach (var test in playersQuery)
        ...
        lock (((ICollection) ClientChecks).SyncRoot)
        {
            var checkers = (from checker in players
                where !ClientChecks.ContainsKey(checker) || !ClientStats.ContainsKey(checker)
                select checker).Take(1);

            // Pick one that isn't in cache
            foreach (var choosed in checkers)
            {
                // FIXME Debug
                if (LOSMGR_DEBUG_LEVEL >= LOSMGR_DEBUG_DEBUG)
        ...
        lock (((ICollection) ClientStats).SyncRoot)
        {
            ...
            var bestavails = (from best in ClientStats.Keys
                where players.Contains(best) &&
                      ClientChecks.ContainsKey(best) &&
                      currentTime - ClientChecks[best] > LOSMGR_PLAYER_CHECK_FREQUENCY
                orderby ClientStats[best].Item1, ClientStats[best].Item3 descending
                select best).Take(1);

            // Pick the best
            foreach (var choosed in bestavails)
            {
                // FIXME Debug
                if (LOSMGR_DEBUG_LEVEL >= LOSMGR_DEBUG_DEBUG)
                    log.Warn("LOSMGR_D : Vincinity Player choosen because available and best checker - " +
                             choosed.Name);
            ...
            var betters = (from best in ClientStats.Keys
                where players.Contains(best)
                orderby ClientStats[best].Item3 descending, ClientStats[best].Item2 descending
                select best).Take(1);

            // Pick the best
            foreach (var choosed in betters)
            {
                // FIXME Debug
                if (LOSMGR_DEBUG_LEVEL >= LOSMGR_DEBUG_DEBUG)
                    log.Warn("LOSMGR_D : Vincinity Player choosen because best checker even if not available - " +
                             choosed.Name);
        ...
        foreach (var choosed in players)
        {
            // FIXME Debug
            if (LOSMGR_DEBUG_LEVEL >= LOSMGR_DEBUG_DEBUG)
    ...
    {
        var elapsed = (long) GameTimer.GetTickCount() - time;
    ...
    {
        if (timeout == 0) timeout = GetDefaultTimeouts(source, target);

        var key = new Tuple<GameObject, GameObject>(source, target);
        ...
        lock (((ICollection) ResponsesCache).SyncRoot)
        {
                 ...
                 (!ResponsesCache[key].Item2 && GetElapsedTicks(ResponsesCache[key].Item1) < timeout >> 1)))
            {
                // FIXME Display debug
                if (LOSMGR_DEBUG_LEVEL >= LOSMGR_DEBUG_INFO)
                    log.Warn("LOSMGR_I : Cached LoS returned, source : " + (source != null ? source.Name : "null") +
                             ", target : " + (target != null ? target.Name : "null") + ", Los : " +
                             (ResponsesCache[key].Item2 ? "OK" : "KO") + ", timeout : " + timeout);
        ...
        // FIXME Display debug
        if (LOSMGR_DEBUG_LEVEL >= LOSMGR_DEBUG_DEBUG)
            log.Warn("LOSMGR_D : Cached LoS Missed, timeout : " + timeout + ", source : " +
                     (source != null ? source.Name : "null") + ", target : " +
    ...
    {
        var obsoleteTime = GameTimer.GetTickCount() - LOSMGR_CLEANUP_FREQUENCY;

        lock (((ICollection) ResponsesCache).SyncRoot)
        {
            foreach (var toClean in (from responses in ResponsesCache
                         where responses.Value.Item1 < obsoleteTime
                         select responses.Key).Take(MAX_CLEANUP_ENTRIES))
    ...
    private void CleanUpPending()
    {
        ...
        lock (((ICollection) PendingChecks).SyncRoot)
        {
            foreach (var toClean in (from pendings in PendingChecks
                         where pendings.Value < obsoleteTime
                         select pendings.Key).Take(MAX_CLEANUP_ENTRIES))
    ...
    private void CleanUpClients()
    {
        ...
        lock (((ICollection) ClientChecks).SyncRoot)
        {
            foreach (var toClean in (from clients in ClientChecks
                         where clients.Value < obsoleteTime
                         select clients.Key).Take(MAX_CLEANUP_ENTRIES))
            {
    ...
    private void CleanUpNotifiers()
    {
        lock (((ICollection) RegisteredLosEvents).SyncRoot)
        {
            foreach (var toClean in (from notifiers in RegisteredLosEvents
                         where (notifiers.Value != null && notifiers.Value.Count < 1) ||
                               notifiers.Key.Item1 == null || notifiers.Key.Item1.ObjectState !=
                               GameObject.eObjectState.Active
                         select notifiers.Key).Take(MAX_CLEANUP_ENTRIES))
    ...
    private static void PendingLosCheck(object sender, System.Timers.ElapsedEventArgs args)
    {
        ...
        var chk = sender as LosCheckMgr;

        var currenTime = GameTimer.GetTickCount();
        ...
        {
            pendingTimeout = from pending in chk.PendingChecks
                where currenTime - pending.Value >= LOSMGR_QUERY_TIMEOUT
                select pending.Key;
        }

        // Launch them to vincinity
        foreach (var toRetry in pendingTimeout)
        {
            var source = toRetry.Item1.CurrentRegion.GetObject(toRetry.Item2);
            var target = toRetry.Item1.CurrentRegion.GetObject(toRetry.Item3);

            if (source != null && target != null)
            {
                var checker = chk.GetBestLosChecker(source, target);

                if (checker != null)
                {
                    if (LOSMGR_DEBUG_LEVEL >= LOSMGR_DEBUG_INFO)
                        log.Warn("LOSMGR_I : Packet Resent, Player : " + checker.Name + ", Source : " +
                                 source.Name + ", Target : " + target.Name);
...
public class LosCheckData : EventArgs
{
    ...
    public GameObject SourceObject => m_sourceObject;
...
public class LosMgrResponseHandler : IDOLEventHandler
{
    ...
    {
        m_delegate((GamePlayer) sender, ((LosCheckData) args).SourceObject, ((LosCheckData) args).TargetObject,
            ((LosCheckData) args).LosOK, args, TempProperties);
    }
}

public delegate void LosMgrResponse(GamePlayer checker, GameObject source, GameObject target, bool losOK,
    EventArgs args, PropertyCollection tempProperties);
