...
public class Alliance
{
    ...
    public void AddGuild(Guild myguild)
    {
        lock (Guilds.SyncRoot)
        {
            ...
            SaveIntoDatabase();
            SendMessageToAllianceMembers(myguild.Name + " has joined the alliance of " + m_dballiance.AllianceName,
    ...
    public void RemoveGuild(Guild myguild)
    {
        lock (Guilds.SyncRoot)
        {
            ...
            {
                SendMessageToAllianceMembers(
                    myguild.Name + " has disbanded the alliance of " + m_dballiance.AllianceName,
                    PacketHandler.eChatType.CT_System, PacketHandler.eChatLoc.CL_SystemWindow);
                var mgl = new ArrayList(Guilds);
            ...
            myguild.SendMessageToGuildMembers(
                myguild.Name + " has left the alliance of " + m_dballiance.AllianceName,
                PacketHandler.eChatType.CT_System, PacketHandler.eChatLoc.CL_SystemWindow);
            SendMessageToAllianceMembers(myguild.Name + " has left the alliance of " + m_dballiance.AllianceName,
                PacketHandler.eChatType.CT_System, PacketHandler.eChatLoc.CL_SystemWindow);
        }
    }
    ...
    public void PromoteGuild(Guild myguild)
    {
        lock (Guilds.SyncRoot)
        {
            ...
            SendMessageToAllianceMembers(myguild.Name + " is the new leader of the alliance",
                PacketHandler.eChatType.CT_Alliance, PacketHandler.eChatLoc.CL_SystemWindow);
        }
    }
    ...
    public void SendMessageToAllianceMembers(string msg, PacketHandler.eChatType type, PacketHandler.eChatLoc loc)
    {
        lock (Guilds.SyncRoot)
        {
            foreach (Guild guild in Guilds) guild.SendMessageToGuildMembers(msg, type, loc);
        }
    }
    ...
    public void SaveIntoDatabase()
    {
        ...
        lock (Guilds.SyncRoot)
        {
            foreach (Guild guild in Guilds) guild.SaveIntoDatabase();
        }
    }
}