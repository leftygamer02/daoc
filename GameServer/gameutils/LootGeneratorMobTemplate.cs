...
public class LootGeneratorMobTemplate : LootGeneratorBase
{
            ...
            lock (m_lootTemplates)
            {
                ...
                if (dbLootTemplates != null)
                {
                    ...
                    foreach (var dbTemplate in dbLootTemplates)
                    {
                        ...
                        var drop =
                            GameServer.Database.FindObjectByKey<ItemTemplate>(dbTemplate.ItemTemplateID);

                        if (drop == null)
                        {
                            if (log.IsErrorEnabled)
                                log.Error("ItemTemplate: " + dbTemplate.ItemTemplateID +
                                          " is not found, it is referenced from DropTemplateXItemTemplate: " +
    ...
    public override void Refresh(GameNPC mob)
    {
        ...
        var mxlts =
            DOLDB<MobDropTemplate>.SelectObjects(DB.Column("MobName").IsEqualTo(mob.Name));
    ...
    {
        var lootTemplates =
            DOLDB<DropTemplateXItemTemplate>.SelectObjects(DB.Column("TemplateName").IsEqualTo(templateName));
    ...
    public override LootList GenerateLoot(GameNPC mob, GameObject killer)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                var lootTemplatesToDrop = new List<DropTemplateXItemTemplate>();
    ...
    private static LootList GenerateLootFromMobXLootTemplates(MobDropTemplate mobXLootTemplates,
        List<DropTemplateXItemTemplate> lootTemplates, LootList lootList, GamePlayer player)
    {
