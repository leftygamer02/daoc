...
public class RealmTimer
{
    public static bool CanPvP(GamePlayer player)
    {
        ...
        var playerAccount = player.Client.Account;
        ...
        var timeLeftOnTimer = TimeLeftOnTimer(player);
    ...
    public static void CheckRealmTimer(GamePlayer player)
    {
        ...
        var playerInRVRZone = false;
        if (player.CurrentZone.IsRvR)
        {
            var area =
                player.CurrentZone?.GetAreasOfSpot(player.X, player.Y, player.Z).FirstOrDefault() as AbstractArea;
            ...
            else if (player.CurrentZone.ID == 12)
            {
                ...
                else if (player.Y > player.CurrentZone.YOffset + 58000 &&
                         player.CurrentZone.XOffset + 12000 < player.X &&
                         player.X < player.CurrentZone.XOffset + 30000)
                    return;
                else
                    playerInRVRZone = true;
            }
    ...
    public static void SaveRealmTimer(GamePlayer player)
    {
        ...
        var LastCombatTickPvPDateTime =
            DateTime.Now.AddMilliseconds(-(GameLoop.GameLoopTime - player.LastCombatTickPvP));

        //Don't update realmtimer it is still in effect and players realm is not the realm_timer_realm
        var current_realm_timer_realm = (eRealm) CurrentRealm(player);
    ...
    public static int CurrentRealm(GamePlayer player)
    {
        ...
        if ((DateTime.Now - playerAccount.Realm_Timer_Last_Combat).TotalMinutes <
            ServerProperties.Properties.PVP_REALM_TIMER_MINUTES)
    ...
    public static double TimeLeftOnTimer(GamePlayer player)
    {
        ...
        var timeSinceLastCombat = (DateTime.Now - playerAccount.Realm_Timer_Last_Combat).TotalMinutes;
        ...
        if (timeSinceLastCombat < ServerProperties.Properties.PVP_REALM_TIMER_MINUTES &&
            (eRealm) playerAccount.Realm_Timer_Realm != player.Realm)
