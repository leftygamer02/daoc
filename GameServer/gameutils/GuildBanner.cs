...
public class GuildBanner
{
    ...
    public GuildBannerItem BannerItem => m_item;
    ...
    public void Start()
    {
        if (m_player.Group != null)
        {
            if (m_player != null)
            {
                var groupHasBanner = false;

                foreach (var groupPlayer in m_player.Group.GetPlayersInTheGroup())
                ...
                if (groupHasBanner == false)
                {
                    if (m_item == null)
                    {
                        var item = new GuildBannerItem(GuildBannerTemplate);
                ...
                {
                    m_player.Out.SendMessage("Someone in your group already has a guild banner active!",
        ...
        {
            m_player.Out.SendMessage("You have left the group and your guild banner disappears!", eChatType.CT_Loot,
    ...
    private int TimerTick(ECSGameTimer timer)
    {
        foreach (GamePlayer player in m_player.GetPlayersInRadius(1500))
            if (player.Group != null && m_player.Group != null && m_player.Group.IsInTheGroup(player))
                ...
                {
                    var effect = GuildBannerEffect.CreateEffectOfClass(m_player, player);

                    if (effect != null)
                    {
                        var oldEffect =
                            player.EffectList.GetOfType(effect.GetType()) as GuildBannerEffect;
                        if (oldEffect == null)
                        {
                            effect.Start(player);
                        }
    ...
    protected virtual void AddHandlers()
    {
        ...
        GameEventMgr.AddHandler(m_player, GamePlayerEvent.StealthStateChanged,
            new DOLEventHandler(PlayerLoseBanner));
        GameEventMgr.AddHandler(m_player, GamePlayerEvent.Linkdeath, new DOLEventHandler(PlayerLoseBanner));
        GameEventMgr.AddHandler(m_player, GameLivingEvent.RegionChanging, new DOLEventHandler(PlayerLoseBanner));
        GameEventMgr.AddHandler(m_player, GameLivingEvent.Dying, new DOLEventHandler(PlayerDied));
    }
    ...
    protected virtual void RemoveHandlers()
    {
        ...
        GameEventMgr.RemoveHandler(m_player, GamePlayerEvent.StealthStateChanged,
            new DOLEventHandler(PlayerLoseBanner));
        GameEventMgr.RemoveHandler(m_player, GamePlayerEvent.Linkdeath, new DOLEventHandler(PlayerLoseBanner));
        GameEventMgr.RemoveHandler(m_player, GameLivingEvent.RegionChanging, new DOLEventHandler(PlayerLoseBanner));
        GameEventMgr.RemoveHandler(m_player, GameLivingEvent.Dying, new DOLEventHandler(PlayerDied));
    }
    ...
    protected void PlayerLoseBanner(DOLEvent e, object sender, EventArgs args)
    {
        Stop();
        m_player.GuildBanner = null;
        m_player.Guild.SendMessageToGuildMembers(string.Format("{0} has put away the guild banner!", m_player.Name),
    ...
    {
        var arg = args as DyingEventArgs;
        if (arg == null) return;
        var killer = arg.Killer as GameObject;
        GamePlayer playerKiller = null;

        if (killer is GamePlayer)
            playerKiller = killer as GamePlayer;
        else if (killer is GameNPC && (killer as GameNPC).Brain != null &&
                 (killer as GameNPC).Brain is AI.Brain.IControlledBrain)
            playerKiller = ((killer as GameNPC).Brain as AI.Brain.IControlledBrain).Owner as GamePlayer;

        Stop();
        m_player.Guild.SendMessageToGuildMembers(m_player.Name + " has dropped the guild banner!",
            eChatType.CT_Guild, eChatLoc.CL_SystemWindow);
        ...
        if (playerKiller != null)
        {
            ...
            if (playerKiller.Group != null)
                foreach (var player in playerKiller.Group.GetPlayersInTheGroup())
        ...
        if (m_player.Group != null)
            foreach (var player in m_player.Group.GetPlayersInTheGroup())
                gameItem.AddOwner(player);
    ...
    public ItemTemplate GuildBannerTemplate
    {
        get
        {
            ...
            {
                var guildIDNB = "GuildBanner_" + m_player.Guild.GuildID;
