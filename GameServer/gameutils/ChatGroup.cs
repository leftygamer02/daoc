...
public class ChatGroup
{
    ...
    /// </summary>
    protected HybridDictionary m_chatgroupMembers = new();
    ...
    public virtual bool AddPlayer(GamePlayer player, bool leader)
    {
        ...
        lock (m_chatgroupMembers)
        {
            ...
            foreach (GamePlayer member in Members.Keys)
                member.Out.SendMessage(player.Name + " has joined the chat group.", eChatType.CT_System,
    ...
    public virtual bool RemovePlayer(GamePlayer player)
    {
        ...
        lock (m_chatgroupMembers)
        {
            ...
            foreach (GamePlayer member in Members.Keys)
                member.Out.SendMessage(player.Name + " has left the chat group.", eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);

            if (m_chatgroupMembers.Count == 1)
            {
                var lastPlayers = new ArrayList(m_chatgroupMembers.Count);
                lastPlayers.AddRange(m_chatgroupMembers.Keys);
                foreach (GamePlayer plr in lastPlayers) RemovePlayer(plr);
            }
        }
