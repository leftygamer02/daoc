...
public class MerchantTradeItems
{
    ...
    /// Item list id
    /// </summary>
    public string ItemsListID => m_itemsListID;

    /// <summary>
    /// Holds item template instances defined with script
    /// </summary>
    protected HybridDictionary m_usedItemsTemplates = new();
    ...
    public virtual bool AddTradeItem(int page, eMerchantWindowSlot slot, ItemTemplate item)
    {
        lock (m_usedItemsTemplates.SyncRoot)
        {
            ...
            var pageSlot = GetValidSlot(page, slot);

            if (pageSlot == eMerchantWindowSlot.Invalid)
            {
                log.ErrorFormat("Invalid slot {0} specified for page {1} of TradeItemList {2}", slot, page,
                    ItemsListID);
                return false;
            }
    ...
    public virtual IDictionary GetItemsInPage(int page)
    {
        try
        {
            var itemsInPage = new HybridDictionary(MAX_ITEM_IN_TRADEWINDOWS);
            if (m_itemsListID != null && m_itemsListID.Length > 0)
            {
                ...
                {
                    var item =
                        GameServer.Database.FindObjectByKey<ItemTemplate>(merchantitem.ItemTemplateID);
                    if (item != null)
                    {
                        var slotItem = (ItemTemplate) itemsInPage[merchantitem.SlotPosition];
                        if (slotItem == null)
                            itemsInPage.Add(merchantitem.SlotPosition, item);
                        else
                            log.ErrorFormat("two merchant items on same page/slot: listID={0} page={1} slot={2}",
                                m_itemsListID, page, merchantitem.SlotPosition);
                    }
                    else
                    {
            ...
            lock (m_usedItemsTemplates.SyncRoot)
            {
                foreach (DictionaryEntry de in m_usedItemsTemplates)
                    if ((int) de.Key >= MAX_ITEM_IN_TRADEWINDOWS * page && (int) de.Key <
                        MAX_ITEM_IN_TRADEWINDOWS * page + MAX_ITEM_IN_TRADEWINDOWS)
    ...
    public virtual ItemTemplate GetItem(int page, eMerchantWindowSlot slot)
    {
        try
        {
            ...
            if (m_itemsListID != null && m_itemsListID.Length > 0)
            {
                var itemToFind = DOLDB<MerchantItem>.SelectObject(DB.Column("ItemListID").IsEqualTo(m_itemsListID)
                    .And(DB.Column("PageNumber").IsEqualTo(page))
                    .And(DB.Column("SlotPosition").IsEqualTo((int) slot)));
                if (itemToFind != null)
        ...
        catch (Exception e)
        {
            if (log.IsErrorEnabled)
                log.Error(
                    "Loading merchant items list (" + m_itemsListID + ") page (" + page + ") slot (" + slot + "): ",
                    e);
            return null;
        }
    }
        ...
        try
        {
            ...
            if (m_itemsListID != null && m_itemsListID.Length > 0)
            {
                ...
                foreach (var merchantitem in itemList)
                {
                    ...
                    if (item != null)
                    {
                        var slotItem = (ItemTemplate) allItems[merchantitem.SlotPosition];
                        if (slotItem == null)
                            allItems.Add(merchantitem.SlotPosition, item);
                        else
                            log.ErrorFormat("two merchant items on same page/slot: listID={0} page={1} slot={2}",
                                m_itemsListID, merchantitem.PageNumber, merchantitem.SlotPosition);
                    }
                }
            }
            ...
            lock (m_usedItemsTemplates.SyncRoot)
            {
                foreach (DictionaryEntry de in m_usedItemsTemplates) allItems[(int) de.Key] = (ItemTemplate) de.Value;
            }
    ...
    public virtual eMerchantWindowSlot GetValidSlot(int page, eMerchantWindowSlot slot)
    {
        ...
        {
            var itemsInPage = GetItemsInPage(page);
            for (var i = (int) eMerchantWindowSlot.FirstInPage; i < (int) eMerchantWindowSlot.LastInPage; i++)
