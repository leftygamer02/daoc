...
public class GamePlayerInventory : GameLivingInventory
{
    ...
    public override bool LoadFromDatabase(string inventoryID)
    {
        lock (m_items)
        {
            try
            {
                ...
                var filterBySlot = DB.Column("SlotPosition").IsLessOrEqualTo((int) eInventorySlot.LastVault)
                    .Or(DB.Column("SlotPosition").IsGreaterOrEqualTo(500)
                        .And(DB.Column("SlotPosition").IsLessThan(600)));
                var items = DOLDB<InventoryItem>.SelectObjects(DB.Column("OwnerID").IsEqualTo(inventoryID)
                    .And(filterBySlot));
                ...
                foreach (var item in items)
                    try
                    {
                        ...
                        if (item.CanUseEvery > 0) item.SetCooldown();

                        if (GetValidInventorySlot((eInventorySlot) item.SlotPosition) == eInventorySlot.Invalid)
                        {
                            if (Log.IsErrorEnabled)
                                Log.Error(
                                    "Tried to load an item in invalid slot, ignored. Item id=" + item.ObjectId);
                        ...
                        {
                            Log.ErrorFormat("Item '{0}', ClassType '{1}' failed valid test for player '{2}'!",
                                item.Name, item.ClassType, m_player.Name);
                            var invalidItem = new GameInventoryItem();
                        ...
                        if (Log.IsWarnEnabled)
                            ...
                            if (GlobalConstants.IsWeapon(item.Object_Type)
                                ...
                                Log.Warn(m_player.Name + ": weapon with damage type 0 is loaded \"" + item.Name +
                                         "\" (" + item.ObjectId + ")");
                    }
                    catch (Exception ex)
                    {
            ...
            try
            {
                foreach (var item in m_items)
                    try
                    {
                        ...
                        var canPersist = true;
                        var gameItem = currentItem as GameInventoryItem;
                        if (gameItem != null) canPersist = gameItem.CanPersist;

                        if (canPersist == false)
                            continue;

                        if (GetValidInventorySlot((eInventorySlot) currentItem.SlotPosition) ==
                            eInventorySlot.Invalid)
                        {
                            if (Log.IsErrorEnabled)
                                Log.Error("item's slot position is invalid. item slot=" + currentItem.SlotPosition +
                                          " id=" +
                        ...
                        {
                            var itemOwner = currentItem.OwnerID ?? "(null)";

                            if (Log.IsErrorEnabled)
                                Log.Error("item owner id (" + itemOwner + ") not equals player ID (" +
                                          m_player.InternalID + "); item ID=" +
                                          currentItem.ObjectId);
                        ...
                        if (currentItem.Dirty)
                        {
                            ...
                            {
                                if (Log.IsErrorEnabled)
                                    Log.Error("Item slot and real slot position are different. Item slot=" +
                                              currentItem.SlotPosition +
                            ...
                            var checkItem =
                                GameServer.Database.FindObjectByKey<InventoryItem>(currentItem.ObjectId);

                            if (checkItem == null || checkItem.OwnerID != m_player.InternalID)
                            {
                                if (checkItem != null)
                                    Log.ErrorFormat(
                                        "Item '{0}' : '{1}' does not have same owner id on save inventory.  Game Owner = '{2}' : '{3}', DB Owner = '{4}'",
                                        currentItem.Name, currentItem.ObjectId, m_player.Name, m_player.InternalID,
                                        checkItem.OwnerID);
                                else
                                    Log.ErrorFormat("Item '{0}' : '{1}' not found in DBInventory for player '{2}'",
                                        currentItem.Name, currentItem.Id_nb, m_player.Name);
    ...
    {
        var savePosition = item.SlotPosition;
        var saveOwnerID = item.OwnerID;
        ...
        var canPersist = true;
        var gameItem = item as GameInventoryItem;
        if (gameItem != null) canPersist = gameItem.CanPersist;
        ...
        if (canPersist)
        {
            if (addObject)
            {
                ...
                {
                    m_player.Out.SendMessage("Error adding item to the database, item may be lost!",
                        eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                    Log.ErrorFormat("Error adding item {0}:{1} for player {2} into the database during AddItem!",
                        item.Id_nb, item.Name, m_player.Name);
                    m_items.Remove(slot);
            ...
            else
            {
                ...
                {
                    m_player.Out.SendMessage("Error saving item to the database, this item may be lost!",
                        eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                    Log.ErrorFormat("Error saving item {0}:{1} for player {2} into the database during AddItem!",
                        item.Id_nb, item.Name, m_player.Name);
                    m_items.Remove(slot);
                    item.SlotPosition = savePosition;
                    item.OwnerID = saveOwnerID;
        ...
        if (IsEquippedSlot((eInventorySlot) item.SlotPosition))
            m_player.Notify(PlayerInventoryEvent.ItemEquipped, this,
                new ItemEquippedArgs(item, eInventorySlot.Invalid));

        if (item is IGameInventoryItem) (item as IGameInventoryItem).OnReceive(m_player);
    ...
    protected bool RemoveItem(InventoryItem item, bool deleteObject)
    {
        ...
        {
            if (Log.IsErrorEnabled)
                Log.Error(m_player.Name + ": PlayerInventory -> tried to remove item with wrong owner (" +
                          (item.OwnerID ?? "null") +
        ...
        if (canPersist)
        {
            if (deleteObject)
            {
                ...
                {
                    m_player.Out.SendMessage("Error deleting item from the database, operation aborted!",
                        eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                    Log.ErrorFormat(
                        "Error deleting item {0}:{1} for player {2} from the database during RemoveItem!",
                        item.Id_nb, item.Name, m_player.Name);
            ...
            else
            {
                if (GameServer.Database.SaveObject(item) == false)
                {
                    m_player.Out.SendMessage("Error saving item to the database, operation aborted!",
                        eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                    Log.ErrorFormat("Error saving item {0}:{1} for player {2} to the database during RemoveItem!",
                        item.Id_nb, item.Name, m_player.Name);
                    m_items.Add(oldSlot, item);
                    item.SlotPosition = savePosition;
                    item.OwnerID = saveOwnerID;
        ...
        var window = m_player.TradeWindow;
        if (window != null)
        ...
        if (item is IGameInventoryItem) (item as IGameInventoryItem).OnLose(m_player);
    ...
    public override bool MoveItem(eInventorySlot fromSlot, eInventorySlot toSlot, int itemCount)
    {
        if (!m_player.IsAlive)
        {
            m_player.Out.SendMessage("You can't change your inventory when dead!", eChatType.CT_System,
        ...
        lock (m_items) // Mannen 10:56 PM 10/30/2006 - Fixing every lock(this)
        {
            ...
            {
                ChatUtil.SendDebugMessage(m_player,
                    string.Format("Invalid slot from: {0}, to: {1}!", fromSlot, toSlot));
                m_player.Out.SendInventorySlotsUpdate(null);
                return false;
            }
                ...
                (fromSlot >= eInventorySlot.FirstBagHorse && fromSlot <= eInventorySlot.LastBagHorse))
            {
                if (fromSlot == eInventorySlot.Horse)
                {
                    ...
                    m_player.Out.SendMessage("You can't move your active horse into a saddlebag!",
                        eChatType.CT_System, eChatLoc.CL_SystemWindow);
                    valid = false;
                }

                if (valid && m_player.Client.Account.PrivLevel == 1)
                    if (m_player.CanUseHorseInventorySlot((int) fromSlot) == false ||
                        m_player.CanUseHorseInventorySlot((int) toSlot) == false)
                        valid = false;
            }
            /***********************************************/
            ...
            if (valid && fromItem.AllowedClasses != "0")
            {
                ...
                {
                    valid = false;
                    foreach (var allowed in Util.SplitCSV(fromItem.AllowedClasses, true))
                    ...
                    if (!valid)
                        m_player.Out.SendMessage("Your class cannot use this item!", eChatType.CT_System,
                            eChatLoc.CL_SystemWindow);
                }
            }
            ...
            if (valid && toItem != null && toItem.AllowedClasses != "0")
            {
                ...
                {
                    valid = false;
                    foreach (var allowed in Util.SplitCSV(toItem.AllowedClasses, true))
                        if (m_player.CharacterClass.ID.ToString() == allowed ||
                            m_player.Client.Account.PrivLevel > 1)
                        {
                            valid = true;
            ...
            if (valid)
                switch (toSlot)
                {
                    ...
                    case eInventorySlot.Mythical:
                        ...
                        {
                            valid = false;
                            m_player.Out.SendMessage(fromItem.GetName(0, true) + " can't go there!",
                                eChatType.CT_System,
                                eChatLoc.CL_SystemWindow);
                        }
                        ...
                        if (valid && fromItem.Type_Damage > m_player.ChampionLevel)
                        {
                            ...
                            m_player.Out.SendMessage(
                                "You can't use " + fromItem.GetName(0, true) +
                                " , you should increase your champion level.",
                                eChatType.CT_System, eChatLoc.CL_SystemWindow);
                        }
                    ...
                    //weapon slots
                    case eInventorySlot.RightHandWeapon:
                        if (fromItem.Object_Type ==
                            (int) eObjectType.Shield //shield can't be used in right hand slot
                            ||
                        ...
                        {
                            valid = false;
                            m_player.Out.SendMessage("You have no skill in using this weapon type!",
                                eChatType.CT_System,
                                eChatLoc.CL_SystemWindow);
                        }
                    ...
                    case eInventorySlot.TwoHandWeapon:
                        if (fromItem.Object_Type == (int) eObjectType.Shield //shield can't be used in 2h slot
                            || (fromItem.Item_Type !=
                                (int) eInventorySlot.RightHandWeapon //right hand weapons can be used in 2h slot
                                && fromItem.Item_Type !=
                                (int) eInventorySlot.LeftHandWeapon //left hand weapons can be used in 2h slot
                                && fromItem.Item_Type !=
                                (int) eInventorySlot.TwoHandWeapon //2h weapons can be used in 2h slot
                                && fromItem.Object_Type !=
                                (int) eObjectType.Instrument)) //instruments can be used in 2h slot
                        {
                            valid = false;
                            m_player.Out.SendMessage(fromItem.GetName(0, true) + " can't go there!",
                                eChatType.CT_System,
                    ...
                    case eInventorySlot.LeftHandWeapon:
                        if (fromItem.Item_Type != (int) toSlot ||
                            (fromItem.Object_Type != (int) eObjectType.Shield &&
                             !m_player.attackComponent.CanUseLefthandedWeapon))
                    ...
                    case eInventorySlot.DistanceWeapon:
                        ...
                        if (fromItem.Item_Type != (int) toSlot &&
                            fromItem.Object_Type != (int) eObjectType.Instrument)
                    ...
                    case eInventorySlot.ArmsArmor:
                        ...
                        {
                            valid = false;
                            m_player.Out.SendMessage("You have no skill in wearing this armor type!",
                                eChatType.CT_System,
                                eChatLoc.CL_SystemWindow);
                        }
                    ...
                    case eInventorySlot.FourthQuiver:
                        if (fromItem.Object_Type != (int) eObjectType.Arrow &&
                            fromItem.Object_Type != (int) eObjectType.Bolt)
                        {
                            valid = false;
                            m_player.Out.SendMessage("You can't put your " + fromItem.Name + " in your quiver!",
                                eChatType.CT_System,
                                eChatLoc.CL_SystemWindow);
                        }
            ...
            {
                if (m_player.Client.Account.PrivLevel == 1) valid = false;

                m_player.Out.SendMessage("You cannot put an item from this realm!", eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);
            }
            ...
            if (valid && toItem != null)
                switch (fromSlot)
                {
                    ...
                    case eInventorySlot.Mythical:
                        ...
                        {
                            valid = false;
                            m_player.Out.SendMessage(toItem.GetName(0, true) + " can't go there!",
                                eChatType.CT_System,
                                eChatLoc.CL_SystemWindow);
                        }
                        ...
                        if (valid && toItem.Type_Damage > m_player.ChampionLevel)
                        {
                            valid = false;
                            m_player.Out.SendMessage(
                                "You can't use " + toItem.GetName(0, true) +
                                " , you should increase your champion level.", eChatType.CT_System,
                                eChatLoc.CL_SystemWindow);
                        }
                    ...
                    case eInventorySlot.RightHandWeapon:
                        if (toItem.Object_Type == (int) eObjectType.Shield //shield can't be used in right hand slot
                            ...
                            (toItem.Item_Type !=
                             (int) eInventorySlot
                                 .RightHandWeapon //right hand weapons can be used in right hand slot
                    ...
                    case eInventorySlot.LeftHandWeapon:
                        if (toItem.Item_Type != (int) fromSlot ||
                            (toItem.Object_Type != (int) eObjectType.Shield &&
                             !m_player.attackComponent.CanUseLefthandedWeapon))
                            //shield can be used only in left hand slot
                        {
                            valid = false;
                    ...
                    case eInventorySlot.DistanceWeapon:
                        if (toItem.Item_Type != (int) fromSlot &&
                            toItem.Object_Type != (int) eObjectType.Instrument)
                        {
                            valid = false;
                            m_player.Out.SendMessage(toItem.GetName(0, true) + " can't go there!",
                                eChatType.CT_System,
                    ...
                    case eInventorySlot.ThirdQuiver:
                    case eInventorySlot.FourthQuiver:
                        if (toItem.Object_Type != (int) eObjectType.Arrow &&
                            toItem.Object_Type != (int) eObjectType.Bolt)
                        {
                            valid = false;
                            m_player.Out.SendMessage("You can't put your " + toItem.Name + " in your quiver!",
                                eChatType.CT_System,
                                eChatLoc.CL_SystemWindow);
                        }
            ...
            if (valid) base.MoveItem(fromSlot, toSlot, itemCount);
        }
    ...
    protected override bool StackItems(eInventorySlot fromSlot, eInventorySlot toSlot, int itemCount)
    {
        ...
        if (itemCount == 0) itemCount = fromItem.Count > 0 ? fromItem.Count : 1;
    ...
    protected override bool ExchangeItems(eInventorySlot fromSlot, eInventorySlot toSlot)
    {
        ...
        var fromSlotEquipped = IsEquippedSlot(fromSlot);
        var toSlotEquipped = IsEquippedSlot(toSlot);
        ...
        if (fromSlotEquipped != toSlotEquipped)
        {
            if (toItem != null)
            {
                if (toSlotEquipped) // item was equipped
                    m_player.Notify(PlayerInventoryEvent.ItemUnequipped, this,
                        new ItemUnequippedArgs(toItem, toSlot));
                else
            ...
            if (fromItem != null)
            {
                if (fromSlotEquipped) // item was equipped
                    m_player.Notify(PlayerInventoryEvent.ItemUnequipped, this,
                        new ItemUnequippedArgs(fromItem, fromSlot));
                else
                    m_player.Notify(PlayerInventoryEvent.ItemEquipped, this,
                        new ItemEquippedArgs(fromItem, fromSlot));
            }
        }
    ...
    public virtual bool IsEquippedSlot(eInventorySlot slot)
    {
        ...
        {
            foreach (var staticSlot in EQUIP_SLOTS)
                if (slot == staticSlot)
                    return true;
    ...
    public override int InventoryWeight
    {
        get
        {
            ...
            foreach (var item in items)
            {
                if ((eInventorySlot) item.SlotPosition < eInventorySlot.FirstBackpack ||
                    (eInventorySlot) item.SlotPosition > eInventorySlot.LastBackpack)
                    continue;
    ...
    protected virtual bool DyeItem(InventoryItem dye, InventoryItem objectToDye)
    {
        ...
        var itemObjType = objectToDye.Object_Type;
        var itemItemType = objectToDye.Item_Type;
        ...
        switch (dye.Model)
        {
            ...
            case 495: //Dye pack
                ...
                break;
            case 538: //Dye pot
                if (itemObjType >= 1 && itemObjType <= 26) canApply = true;
    ...
    protected override void UpdateChangedSlots()
    {
        lock (m_changedSlots)
        {
            ...
            foreach (var inv in invSlots) slotsToUpdate.Add((int) inv);
        ...
        var encumberanceUpdated = false;

        lock (InventorySlotLock)
        {
            foreach (var updatedSlot in m_changedSlots)
            {
                // update appearance if one of changed slots is visible
                if (!appearanceUpdated)
                    foreach (var visibleSlot in VISIBLE_SLOTS)
                    {
