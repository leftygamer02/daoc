...
{
    private static ItemTemplate m_atlanteanglass =
        GameServer.Database.FindObjectByKey<ItemTemplate>("atlanteanglass");
    ...
    public override LootList GenerateLoot(GameNPC mob, GameObject killer)
    {
        ...
        try
        {
            ...
            var atlanteanGlass = GameServer.Database.FindObjectByKey<ItemTemplate>(m_atlanteanglass.Id_nb);
            // ItemTemplate atlanteanGlass = new ItemTemplate(m_atlanteanglass);  Creating a new ItemTemplate throws an exception later

            var killedcon = (int) player.GetConLevel(mob) + 3;
            ...
            if (!mob.Name.ToLower().Equals(mob.Name))
                //Named mob, more cash !
                maxcount = (int) Math.Round(maxcount *
                                            ServerProperties.Properties.LOOTGENERATOR_ATLANTEANGLASS_NAMED_COUNT);

            if (maxcount > 0 && Util.Chance(ServerProperties.Properties.LOOTGENERATOR_ATLANTEANGLASS_BASE_CHANCE +
                                            Math.Max(10, killedcon)))
