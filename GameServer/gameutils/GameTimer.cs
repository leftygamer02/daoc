...
public abstract class GameTimer
{
        ...
        set
        {
            if (value < 0 || value > MaxInterval)
                throw new ArgumentOutOfRangeException("value", value,
                    "Interval value must be in 1 .. " + MaxInterval.ToString() + " range.");
            m_interval = value;
        }
    }
    ...
    /// </summary>
    public long MaxInterval => m_time.MaxInterval;

    /// <summary>
    /// Checks whether this timer is disabled
    /// </summary>
    public bool IsAlive => (m_tick & TIMER_DISABLED) == 0;
    ...
    {
        return string.Format("{0} tick:0x{1:X8} interval:{2} manager:'{3}'", GetType().FullName, m_tick, m_interval,
            m_time.Name);
    }
...
//		}
    ...
    public class TimeManager
    {
        ...
        protected struct CacheBucket
        {
            ...
            /// The empty bucket
            /// </summary>
            public static readonly CacheBucket EmptyBucket = new();
        }
...
#if MonitorCallbacks
				FileStream stream =
 new FileStream("logs\\delays-" + m_name + ".log", FileMode.Append, FileAccess.Write, FileShare.Read);
        ...
        /// Gets the maximal allowed interval
        /// </summary>
        public int MaxInterval => (1 << (LONGTERM_BITS + TABLE_BITS + BUCKET_BITS)) - (1 << (TABLE_BITS + BUCKET_BITS));
        ...
        /// </summary>
        public int ActiveTimers => m_activeTimers;

        /// <summary>
        /// Gets the invoked timers count
        /// </summary>
        public long InvokedCount => m_invokedCount;
        ...
        public int CountTimers()
        {
            lock (m_buckets)
            {
                var res = 0;
                GameTimer t;
                for (var i = 0; i < m_cachedBucket.Length; i++)
                {
            ...
            lock (m_lockObject)
            {
                ...
                if (!m_timeThread.Join(10000))
                    try
                    {
                        ...
                        {
                            var state = m_timeThread.ThreadState;
                            log.ErrorFormat(
                                "failed to stop the time thread \"{0}\" in 10 seconds (thread state={1}); thread stacktrace:\n",
                                m_name, state);
                    ...
                    finally
                    {
                        ...
                        try
                        {
                            ...
                            {
                                log.ErrorFormat(
                                    "Timer Thread ({0}) can't stop after abort... maybe remaining threads going... trying again !");
        ...
        protected internal virtual void InsertTimer(GameTimer t, int offsetTick)
        {
            if (offsetTick > MaxInterval || offsetTick < 1)
                throw new ArgumentOutOfRangeException("offsetTick", offsetTick.ToString(),
                    "Offset must be in range from 1 to " + MaxInterval);

            var timer = t;

            lock (m_buckets)
            {
                var timerTick = timer.m_tick;
                ...
                if ((timerTick & TIMER_DISABLED) == 0) RemoveTimerUnsafe(timer);
                ...
                if (offsetTick <= CACHE_MASK + 1)
                {
                    ...
                    var bucket = m_cachedBucket[targetTick];
                    var prev = bucket.LastTimer;
                    if (prev != null)
                    {
                ...
                else
                {
                    ...
                    var next = m_buckets[targetTick];
                    m_buckets[targetTick] = timer;
                    if (next != null) timer.m_nextTimer = next;
                }
            }
        }
        ...
        protected internal virtual void RemoveTimer(GameTimer timer)
        {
            lock (m_buckets)
            {
        ...
        private void RemoveTimerUnsafe(GameTimer timer)
        {
            ...
            var cachedIndex = tick & CACHE_MASK;
            var bucket = m_cachedBucket[cachedIndex];
            ...
            var timerChain = bucket.FirstTimer;
            GameTimer prev;
        ...
        private void TimeThread()
        {
            ...
            while (m_running)
                try
                {
                    ...
                    lock (m_buckets)
                    {
                        if (GameEvents.RegionTimersResynch.watch != null)
                            m_time = GameEvents.RegionTimersResynch.watch.ElapsedMilliseconds;
                        else m_time++;

                        var newTick = m_tick = (m_tick + 1) & TICK_MASK;
                        if ((newTick & BUCKET_MASK) == 0)
                        {
                            // cache next bucket
                            var index = newTick >> BUCKET_BITS;
                        ...
                        var cacheIndex = m_tick & CACHE_MASK;
                        chain = m_cachedBucket[cacheIndex].FirstTimer;
                        if (chain != null) m_cachedBucket[cacheIndex] = CacheBucket.EmptyBucket;
                    }

                    var current = chain;
                    var curTick = m_tick;
                    var currentBucketMax = (curTick & ~BUCKET_MASK) + BUCKET_MASK;
                    while (current != null)
                    {
                        if (current.m_tick == curTick)
                        {
                            try
                            {
                                var callbackStart = GetTickCount();
                                ...
                                if (GetTickCount() - callbackStart > 250)
                                    if (log.IsWarnEnabled)
                                    {
                                        ...
                                        {
                                            curStr = "error in timer.ToString(): " + current.GetType().FullName +
                                                     "; " + ee.ToString();
                                        }

                                        var warning = "callback took " + (GetTickCount() - callbackStart) +
                                                      "ms! " + curStr;
                                        log.Warn(warning);
                                    }
                        ...
                        lock (m_buckets)
                        {
                            ...
                            else
                            {
                                ...
                                if (tick - curTick <= CACHE_MASK + 1)
                                {
                                    ...
                                    var bucket = m_cachedBucket[tick];
                                    var prev = bucket.LastTimer;
                                    current.m_nextTimer = null;
                                    if (prev != null)
                                    {
                                ...
                                else
                                {
                                    if ((tick & TICK_MASK) > currentBucketMax)
                                        current.m_tick =
                                            tick += TICK_MASK +
                                                    1; // extra pass if the timer is ahead of current tick
                                    tick = (tick >> BUCKET_BITS) & TABLE_MASK;
