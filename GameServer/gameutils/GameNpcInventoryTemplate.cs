...
public class GameNpcInventoryTemplate : GameLivingInventory
{
    ...
    /// </summary>
    protected static readonly Hashtable m_usedInventoryItems = new(1024);

    /// <summary>
    /// Holds already used inventory template instances
    /// </summary>
    protected static readonly Hashtable m_usedInventoryTemplates = new(256);
    ...
    {
        var temp = new GameNpcInventoryTemplate().CloseTemplate();
        ...
        int emblem = 0)
    {
        lock (m_items)
        {
            lock (m_usedInventoryItems.SyncRoot)
            {
                ...
                var itemID = string.Format("{0}:{1},{2},{3},{4}", slot, model, color, effect, extension);
    ...
    public GameNpcInventoryTemplate CloseTemplate()
    {
        lock (m_items)
        {
            lock (m_usedInventoryTemplates.SyncRoot)
            {
                ...
                {
                    m_isClosed = true;
                    var templateID = new StringBuilder(m_items.Count * 16);
                    ...
                    var finalTemplate =
                        m_usedInventoryTemplates[templateID.ToString()] as GameNpcInventoryTemplate;
        ...
        lock (m_items)
        {
            ...
            foreach (var npcItem in npcEquip)
                if (!AddNPCEquipment((eInventorySlot) npcItem.Slot, npcItem.Model, npcItem.Color, npcItem.Effect,
                        npcItem.Extension, npcItem.Emblem))
                    if (log.IsWarnEnabled)
                        log.Warn("Error adding NPC equipment for templateID " + templateID + ", ModelID=" +
                                 npcItem.Model + ", slot=" + npcItem.Slot);
        }
    ...
    public static bool Init()
    {
        try
        {
            ...
            foreach (var equip in GameServer.Database.SelectAllObjects<NPCEquipment>())
            {
                ...
                foreach (var item in m_items.Values)
                {
                    ...
                    foreach (var npcItem in npcEquipment)
                    {
                        ...
                        if (item.Model != npcItem.Model || item.Color != npcItem.Color ||
                            item.Effect != npcItem.Effect || item.Emblem != npcItem.Emblem)
                        {
    ...
    /// <returns>false</returns>
    public override bool AddTemplate(InventoryItem template, int count, eInventorySlot minSlot,
    ...
    /// <param name="maxSlot">The last slot</param>
    /// <returns>false</returns>
    public override bool RemoveTemplate(string templateID, int count, eInventorySlot minSlot,
        eInventorySlot maxSlot)
    {
        return false;
    }
