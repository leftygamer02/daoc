...
public class Util
{
    private static Util soleInstance = new();
    private int lastRandomInt = 0;
    private double lastRandomDouble = 0.0;

    public static void LoadTestDouble(Util testDouble)
    {
    ...
    {
        var rand = RandomGen.Next(min, max + 1);
        return rand;
        //if (lastRandomInt == 0)
        //{				
        //	lastRandomInt = rand;
    ...
    /// </summary>
    [ThreadStatic] private static Random m_random = null;

    [ThreadStatic] private static RNGCryptoServiceProvider m_cryptoRandom = null;
    ...
    public static RNGCryptoServiceProvider CryptoRandom
    {
        get
        {
            if (m_cryptoRandom == null) m_cryptoRandom = new RNGCryptoServiceProvider();
    ...
    public static int CryptoNextInt(int minValue, int maxValue)
    {
        ...
        while (true)
        {
            ...
            if (rand < max - remainder || counter > 10) return (int) (minValue + rand % diff);
        }
    }
    ...
    public static List<string> SplitCSV(string str, bool rangeCheck = false)
    {
        ...
        var advancedResultat = new List<string>();
        foreach (var currentResultat in resultat)
            if (currentResultat.Contains('-'))
            {
                ...
                if (int.TryParse(currentResultat.Split(secondarySeparator)[0], out from) &&
                    int.TryParse(currentResultat.Split(secondarySeparator)[1], out to))
                {
                    if (from > to)
                    {
                        var tmp = to;
    ...
    public static string MakeSentence(string message, params string[] args)
    {
        ...
        var res = string.Format(message, args);
        if (res.Length > 0 && char.IsLower(res[0])) res = char.ToUpper(res[0]) + res.Substring(1);
    ...
    public static IList<string> ContainsKey(string str, string startKey, params string[] args)
    {
        ...
        {
            var results = new List<string>(args.Length + 1);
            results.Add(startKey);

            if (args != null)
            {
                // reduce string
                var rem = str.Trim().Substring(startKey.Length).Trim().ToLower();

                // search for keyword
                foreach (var keyW in args)
                {
                    var keyWord = keyW;

                    var index = rem.IndexOf(keyWord.ToLower());
...
#pragma warning restore 0618
    ...
    public static string GetFormattedStackTraceFrom(Thread targetThread)
    {
        ...
        try
        {
            ...
            foreach (var frame in clrThread.EnumerateStackTrace())
            {
                ...
                if (method != null) sb.AppendLine($"   at {method.Signature}");
            }
        }
        catch (Exception e)
        {
    ...
    {
        foreach (var item in addList) list.Add(item);
    }
