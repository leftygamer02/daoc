...
{
    private int SMALLCHEST_CHANCE = ServerProperties.Properties.BASE_SMALLCHEST_CHANCE;
    private int LARGECHEST_CHANCE = ServerProperties.Properties.BASE_LARGECHEST_CHANCE;

    public override LootList GenerateLoot(GameNPC mob, GameObject killer)
    {
        var loot = base.GenerateLoot(mob, killer);
        var small = SMALLCHEST_CHANCE;
        var large = LARGECHEST_CHANCE;
        if (Util.Chance(small))
        {
            var lvl = mob.Level + 1;
            if (lvl < 1) lvl = 1;
            var minLoot = ServerProperties.Properties.SMALLCHEST_MULTIPLIER * lvl * lvl;
        ...
        if (Util.Chance(large))
        {
            var lvl = mob.Level + 1;
            if (lvl < 1) lvl = 1;
            var minLoot = ServerProperties.Properties.LARGECHEST_MULTIPLIER * lvl * lvl;
            long moneyCount = minLoot + Util.Random(minLoot >> 1);
            moneyCount = (long) ((double) moneyCount * ServerProperties.Properties.MONEY_DROP);
            var money = new ItemTemplate();
            money.Model = 488;
