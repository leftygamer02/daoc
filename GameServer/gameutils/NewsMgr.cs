...
public class NewsMgr
{
        ...
        for (var type = 0; type <= 2; type++)
        {
            ...
            if (type > 0)
                newsList = DOLDB<DBNews>.SelectObjects(DB.Column("Type").IsEqualTo(type)
                    .And(DB.Column("Realm").IsEqualTo(0).Or(DB.Column("Realm").IsEqualTo(realm))));
            else
                newsList = DOLDB<DBNews>.SelectObjects(DB.Column("Type").IsEqualTo(type));

            newsList = newsList.OrderByDescending(it => it.CreationDate).Take(5).ToArray();
            var n = newsList.Count;
            ...
            while (n > 0)
            {
                ...
                client.Out.SendMessage(
                    string.Format("N,{0},{1},{2},\"{3}\"", news.Type, index++, RetElapsedTime(news.CreationDate),
                        news.Text), eChatType.CT_SocialInterface, eChatLoc.CL_SystemWindow);
            }
        }
    }
    ...
    {
        var playerEnterGame = DateTime.Now.Subtract(dt);
        string newsTime;
