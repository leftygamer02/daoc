...
public class GameDuel
{
    ...
    public virtual void Stop()
    {
        ...
        Starter.Out.SendMessage(LanguageMgr.GetTranslation(Starter.Client, "GamePlayer.DuelStop.DuelEnds"),
    ...
    protected virtual void StopNegativeEffects(GamePlayer target, GameLiving Caster)
    {
        ...
        foreach (var spellEffect in spelle.Where(spellEffect =>
                     spellEffect != null && !spellEffect.HasPositiveEffect && spellEffect.Caster == Caster &&
                     spellEffect.Owner == target))
            EffectService.RequestImmediateCancelEffect(
                EffectListService.GetSpellEffectOnTarget(target, spellEffect.EffectType));

        foreach (var effect in effects.Where(effect =>
                     effect != null && !effect.HasPositiveEffect && effect.Owner == target))
            EffectService.RequestImmediateCancelEffect(
                EffectListService.GetEffectOnTarget(target, effect.EffectType));
    }
    ...
    protected virtual void StopImmunityEffects(GamePlayer target)
    {
        ...
        var spelle = target.effectListComponent.GetSpellEffects();
        foreach (var spellEffect in spelle.Where(spellEffect =>
                     spellEffect != null && spellEffect is ECSImmunityEffect && spellEffect.Caster != target))
            EffectService.RequestImmediateCancelEffect(
                EffectListService.GetImmunityEffectOnTarget(target, spellEffect.EffectType));

        foreach (var effect in effects.Where(effect =>
                     effect != null && effect is ECSImmunityEffect && effect.Owner != Caster))
            EffectService.RequestImmediateCancelEffect(
                EffectListService.GetImmunityEffectOnTarget(target, effect.EffectType));
    }
}