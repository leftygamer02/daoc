...
public class SinglePermission
{
    ...
    {
        var obj = DOLDB<DBSinglePermission>.SelectObject(DB.Column("Command").IsEqualTo(command)
            .And(DB.Column("PlayerID").IsEqualTo(player.ObjectId)
                .Or(DB.Column("PlayerID").IsEqualTo(player.AccountName))));
        if (obj == null)
            return false;
        return true;
    }
    ...
    {
        var perm = new DBSinglePermission();
    ...
    {
        var obj = DOLDB<DBSinglePermission>.SelectObject(DB.Column("Command").IsEqualTo(command)
            .And(DB.Column("PlayerID").IsEqualTo(player.AccountName)));
        if (obj == null) return false;
