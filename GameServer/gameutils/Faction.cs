...
public class Faction
{
    ...
    public void SaveAggroToFaction()
    {
        ...
        {
            foreach (string charID in m_updatePlayer) SaveAggroToFaction(charID);
    ...
    public void SaveAggroToFaction(string charID)
    {
        ...
        var dbfactionAggroLevel = DOLDB<DBFactionAggroLevel>.SelectObject(DB.Column("CharacterID").IsEqualTo(charID)
            .And(DB.Column("FactionID").IsEqualTo(ID)));
    ...
    /// friend factions
    /// </summary>
    public ArrayList FriendFactions => m_friendFactions;
    ...
    /// enemy factions
    /// </summary>
    public ArrayList EnemyFactions => m_enemyFactions;
    ...
    /// </summary>
    public int BaseAggroLevel => m_baseAggroLevel;
    ...
    /// </summary>
    public Hashtable PlayerxFaction => m_playerxFaction;
    ...
    {
        foreach (Faction faction in m_friendFactions) faction.ChangeAggroLevel(killer, INCREASE_AGGRO_AMOUNT);

        foreach (Faction faction in m_enemyFactions) faction.ChangeAggroLevel(killer, DECREASE_AGGRO_AMOUNT);
    }
    ...
    {
        // remember the player
        if (!m_updatePlayer.Contains(player.ObjectId)) m_updatePlayer.Add(player.ObjectId);
        ...
        var newAggro = oldAggro + amount;
        // clamp it between MIN and MAX
        if (newAggro < MIN_AGGRO_VALUE)
            newAggro = MIN_AGGRO_VALUE;
        else if (newAggro > MAX_AGGRO_VALUE) newAggro = MAX_AGGRO_VALUE;
        ...
        // tell the player
        var msg = "Your relationship with " + Name + " has ";
        if (amount > 0)
            msg += "decreased.";
        else
            msg += "increased.";
