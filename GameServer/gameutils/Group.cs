...
public class Group
{
    ...
    /// </summary>
    public byte MemberCount => (byte) m_groupMembers.Count;
    ...
    public Quests.AbstractMission Mission
    {
        ...
        {
            m_mission = value;
            foreach (var player in m_groupMembers.OfType<GamePlayer>())
            {
    ...
    public virtual bool AddMember(GameLiving living)
    {
        ...
        SendMessageToGroupMembers(string.Format("{0} has joined the group.", living.Name), eChatType.CT_System,
        ...
        if (hasGrouped == null)
        {
            var groupedChar = new DOLCharactersXCustomParam();
    ...
    public virtual bool RemoveMember(GameLiving living)
    {
        ...
        SendMessageToGroupMembers(string.Format("{0} has left the group.", living.Name), eChatType.CT_System,
            eChatLoc.CL_SystemWindow);
        ...
        if (MemberCount > 1 && LivingLeader == living)
        {
            ...
            if (newLeader != null)
            {
                ...
                SendMessageToGroupMembers(string.Format("{0} is the new group leader.", Leader.Name),
    ...
    {
        var allOk = m_groupMembers.FreezeWhile<bool>(l =>
        {
            if (!l.Contains(living))
                return false;

            var ind = living.GroupIndex;
            var oldLeader = l[0];
            l[ind] = oldLeader;
            l[0] = living;
            LivingLeader = living;
    ...
    public bool SwitchPlayers(GameLiving source, GameLiving target)
    {
        var allOk = m_groupMembers.FreezeWhile<bool>(l =>
        {
            ...
            var sourceInd = source.GroupIndex;
            var targetInd = target.GroupIndex;
        ...
        if (allOk)
        {
            // all went ok
            UpdateGroupWindow();
            SendMessageToGroupMembers(string.Format("Switched group member {0} with {1}", source.Name, target.Name),
                eChatType.CT_System, eChatLoc.CL_SystemWindow);
        }
    ...
    {
        var player = m_groupMembers.FreezeWhile<GamePlayer>(l =>
        {
            var player = l[index] as GamePlayer;
    ...
    public void UpdateAllToMember(GamePlayer player, bool updateIcons, bool updateOtherRegions)
    {
        ...
        foreach (var living in m_groupMembers)
    ...
    public string GroupMemberString(GamePlayer player)
    {
        lock (m_groupMembers)
        {
            var text = new StringBuilder(64); //create the string builder
            text.Length = 0;
            var mybattlegroup =
            ...
            foreach (GamePlayer plr in m_groupMembers)
            {
                ...
                {
                    if ((bool) mybattlegroup.Members[plr] == true) text.Append("<Leader> ");
    ...
    public string GroupMemberClassString(GamePlayer player)
    {
        lock (m_groupMembers)
        {
            ...
            var mybattlegroup =
                (BattleGroup) player.TempProperties.getProperty<object>(BattleGroup.BATTLEGROUP_PROPERTY, null);
            foreach (GamePlayer plr in m_groupMembers) text.Append($"{plr.Name} ({plr.CharacterClass.Name}) ");
