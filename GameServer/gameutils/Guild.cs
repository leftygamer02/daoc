...
public class Guild
{
    ...
    /// </summary>
    protected readonly Dictionary<string, GamePlayer> m_onlineGuildPlayers = new();

    /// <summary>
    /// Use this object to lock the guild member list
    /// </summary>
    public object m_memberListLock = new();
    ...
    /// </summary>
    protected List<AbstractGameKeep> m_claimedKeeps = new();

    public eRealm Realm
    {
        get => (eRealm) m_DBguild.Realm;
        set => m_DBguild.Realm = (byte) value;
    }

    public string Webpage
    {
        get => m_DBguild.Webpage;
        set => m_DBguild.Webpage = value;
    }

    public DBRank[] Ranks
    {
        get => m_DBguild.Ranks;
        set => m_DBguild.Ranks = value;
    }
    ...
    public bool GuildOwnsHouse
    {
        ...
        set => m_DBguild.HaveGuildHouse = value;
    }
    ...
    public void SetGuildBank(GamePlayer donating, double amount)
    {
        ...
        if (amount < 0)
        {
            donating.Out.SendMessage(
                LanguageMgr.GetTranslation(donating.Client, "Scripts.Player.Guild.DepositInvalid"),
        ...
        {
            donating.Out.SendMessage(
                LanguageMgr.GetTranslation(donating.Client, "Scripts.Player.Guild.DepositFull"), eChatType.CT_Guild,
        ...
        {
            donating.Out.SendMessage("You don't have this amount of money !", eChatType.CT_Important,
                eChatLoc.CL_SystemWindow);
            return;
        }

        donating.Out.SendMessage(
            LanguageMgr.GetTranslation(donating.Client, "Scripts.Player.Guild.DepositAmount",
                Money.GetString(long.Parse(amount.ToString()))), eChatType.CT_Loot, eChatLoc.CL_SystemWindow);

        donating.Guild.UpdateGuildWindow();
        m_DBguild.Bank += amount;

        InventoryLogging.LogInventoryAction(donating, "(GUILD;" + Name + ")", eInventoryActionType.Other,
            long.Parse(amount.ToString()));
    ...
    public void WithdrawGuildBank(GamePlayer withdraw, double amount)
    {
        if (amount < 0)
        {
            withdraw.Out.SendMessage(
                LanguageMgr.GetTranslation(withdraw.Client, "Scripts.Player.Guild.WithdrawInvalid"),
                eChatType.CT_Guild, eChatLoc.CL_SystemWindow);
            return;
        }
        ...
        {
            withdraw.Out.SendMessage(
                LanguageMgr.GetTranslation(withdraw.Client, "Scripts.Player.Guild.WithdrawTooMuch"),
                eChatType.CT_Guild, eChatLoc.CL_SystemWindow);
            return;
        }

        withdraw.Out.SendMessage(
            LanguageMgr.GetTranslation(withdraw.Client, "Scripts.Player.Guild.Withdrawamount",
                Money.GetString(long.Parse(amount.ToString()))), eChatType.CT_Guild, eChatLoc.CL_SystemWindow);
    ...
    public DateTime GuildBannerLostTime
    {
        get
        {
            if (m_DBguild.GuildBannerLostTime == null) return new DateTime(2010, 1, 1);
    ...
    /// </summary>
    public int MemberOnlineCount => m_onlineGuildPlayers.Count;
    ...
    public bool RemoveOnlineMember(GamePlayer player)
    {
        lock (m_memberListLock)
        {
            if (m_onlineGuildPlayers.ContainsKey(player.InternalID))
            {
                ...
                var memberList =
                    GuildMgr.GetAllGuildMembers(player.GuildID);
    ...
    public bool AddPlayer(GamePlayer addPlayer, DBRank rank)
    {
        ...
        if (addPlayer.Realm != Realm) return false;
        ...
        try
        {
            ...
            addPlayer.Out.SendMessage("You have agreed to join " + Name + "!", eChatType.CT_Group,
                eChatLoc.CL_SystemWindow);
            addPlayer.Out.SendMessage("Your current rank is " + addPlayer.GuildRank.Title + "!", eChatType.CT_Group,
                eChatLoc.CL_SystemWindow);
            SendMessageToGuildMembers(addPlayer.Name + " has joined the guild!", eChatType.CT_Group,
                eChatLoc.CL_SystemWindow);
        }
        catch (Exception e)
        {
    ...
    public bool RemovePlayer(string removername, GamePlayer member)
    {
        try
        {
            ...
            if (removername == member.Name)
                member.Out.SendMessage("You leave the guild.", eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);
            else
                member.Out.SendMessage(removername + " removed you from " + Name,
                    eChatType.CT_System, eChatLoc.CL_SystemWindow);
        }
        catch (Exception e)
        {
    ...
    public bool HasRank(GamePlayer member, eRank rankNeeded)
    {
        try
        {
            ...
            {
                log.Debug("Player " + member.Name + " (" + member.InternalID + ") is not a member of guild " +
                          Name);
                return false;
            }
    ...
    {
        var guildPlayers = new List<GamePlayer>();
        lock (m_memberListLock)
        {
            guildPlayers = m_onlineGuildPlayers.Values.ToList();
        }

        foreach (var pl in guildPlayers)
        {
            if (!HasRank(pl, eRank.GcHear)) continue;
    ...
    {
        if (amount > m_DBguild.BountyPoints) return false;
    ...
    public long GuildLevel =>
        ...
        (long) (Math.Sqrt(m_DBguild.RealmPoints / 10000) + 1);
    ...
    {
        get => (eBonusType) m_DBguild.BonusType;
        set
        {
    ...
    public DateTime BonusStartTime
    {
        get
        {
            if (m_DBguild.BonusStartTime == null) return new DateTime(2010, 1, 1);
    ...
    public string GuildBannerStatus(GamePlayer player)
    {
        ...
        if (player.Guild != null)
            ...
            {
                foreach (var plr in player.Guild.GetListOfOnlineMembers())
                ...
                if (bannerStatus == "None") bannerStatus = "Not Summoned";
    ...
    public void UpdateGuildWindow()
    {
        ...
        Parallel.ForEach(guildPlayers, player => { player.Guild.UpdateMember(player); });
