...
public class BattleGroup
{
    ...
    private bool battlegroupLootType = false;
    private GamePlayer battlegroupTreasurer = null;
    private int battlegroupLootTypeThreshold = 0;
    ...
    public void StartRecordingRolls(int maxRoll = 1000)
    {
        ...
        foreach (GamePlayer ply in Members.Keys)
            ply.Out.SendMessage(
                $"{Leader.Name} has initiated the recording. Use /random {maxRoll} now to roll for this item.",
                eChatType.CT_BattleGroupLeader, eChatLoc.CL_ChatWindow);
    }
    ...
    public void StopRecordingRolls()
    {
        ...
        foreach (GamePlayer ply in Members.Keys)
            ply.Out.SendMessage($"{Leader.Name} stopped the recording. Use /bg showrolls to display the results.",
                eChatType.CT_BattleGroupLeader, eChatLoc.CL_ChatWindow);
    }
    ...
    public void ShowRollsWindow(GamePlayer player)
    {
        if (recordingRolls)
        {
            player.Client.Out.SendMessage("Rolls are being recorded. Please wait.", eChatType.CT_Important,
                eChatLoc.CL_SystemWindow);
            return;
        }
        ...
        {
            player.Client.Out.SendMessage("No rolls have been recorded yet.", eChatType.CT_Important,
                eChatLoc.CL_SystemWindow);
            return;
        }
    ...
    {
        if (battlegroupTreasurer != null && living != null) return battlegroupTreasurer == living;
    ...
    {
        if (m_battlegroupLeader != null && living != null) return m_battlegroupLeader == living;
    ...
    public virtual void SendMessageToBattleGroupMembers(string msg, eChatType type, eChatLoc loc)
    {
        lock (m_battlegroupMembers) // Mannen 10:56 PM 10/30/2006 - Fixing every lock(this)
        {
            foreach (GamePlayer player in m_battlegroupMembers.Keys) player.Out.SendMessage(msg, type, loc);
        }
    }
    ...
    public virtual bool RemoveBattlePlayer(GamePlayer player)
    {
        ...
        lock (m_battlegroupMembers)
        {
            ...
            {
                var lastPlayers = new ArrayList(m_battlegroupMembers.Count);
                lastPlayers.AddRange(m_battlegroupMembers.Keys);
                foreach (GamePlayer plr in lastPlayers) RemoveBattlePlayer(plr);
            }
            else if (leader && m_battlegroupMembers.Count >= 2)
            {
                ...
                lock (bgPlayers)
                {
                    ...
                    foreach (GamePlayer member in Members.Keys)
                        member.Out.SendMessage(randomPlayer.Name + " is the new leader of the battle group.",
                            eChatType.CT_BattleGroupLeader, eChatLoc.CL_SystemWindow);
                }
            }
        }
