...
public class Money
{
    ...
    {
        return ((((long) mithril * 1000L + (long) platinum) * 1000L + (long) gold) * 100L + (long) silver) * 100L +
               (long) copper;
    }
    ...
    public static string GetString(long money)
    {
        ...
        var copper = GetCopper(money);
        var silver = GetSilver(money);
        var gold = GetGold(money);
        var platin = GetPlatinum(money);
        var mithril = GetMithril(money);
    ...
    {
        var levelmod = level * level * level;

        var dCopper = levelmod / 0.6; // level 50, 100 quality; worth aprox 20 gold, sells for 10 gold
        var dQuality = quality / 100.0;
