...
public class StatPrint
{
    ...
    public static string GetProcessCounterName()
    {
        ...
        var id = process.Id;
        var perfCounterCat = new PerformanceCounterCategory("Process");
        foreach (DictionaryEntry entry in perfCounterCat.ReadCategory()["id process"])
        {
            var processCounterName = (string) entry.Key;
    ...
    public static void PrintStats(object state)
    {
        try
        {
            ...
            var time = newTick - m_lastMeasureTick;
            ...
            var inRate = (Statistics.BytesIn - m_lastBytesIn) / time;
            var outRate = (Statistics.BytesOut - m_lastBytesOut) / time;
            var inPckRate = (Statistics.PacketsIn - m_lastPacketsIn) / time;
            var outPckRate = (Statistics.PacketsOut - m_lastPacketsOut) / time;
            ...
            var globalHandlers = GameEventMgr.NumGlobalHandlers;
            var objectHandlers = GameEventMgr.NumObjectHandlers;

            if (log.IsInfoEnabled)
            {
                var stats = new StringBuilder(256)
                    .Append("-stats- Mem=").Append(GC.GetTotalMemory(false) / 1024 / 1024).Append("MB")
                    .Append("  Clients=").Append(GameServer.Instance.ClientCount)
                    .Append("  Down=").Append(inRate / 1024).Append("kb/s (")
                    .Append(Statistics.BytesIn / 1024 / 1024).Append("MB)")
                    .Append("  Up=").Append(outRate / 1024).Append("kb/s (")
                    .Append(Statistics.BytesOut / 1024 / 1024).Append("MB)")
                    .Append("  In=").Append(inPckRate).Append("pck/s (").Append(Statistics.PacketsIn / 1000)
                    .Append("K)")
                    .Append("  Out=").Append(outPckRate).Append("pck/s (").Append(Statistics.PacketsOut / 1000)
                    .Append("K)")
                ...
                {
                    foreach (var mgr in WorldMgr.GetRegionTimeManagers())
                    {
                        var ts = (TimerStats) m_timerStatsByMgr[mgr];
                        if (ts == null)
                        {
                            ts = new TimerStats();
                        ...
                        var curInvoked = mgr.InvokedCount;
                        var invoked = curInvoked - ts.InvokedCount;
            ...
            if (log.IsFatalEnabled)
                lock (m_timerStatsByMgr.SyncRoot)
                {
                    foreach (var mgr in WorldMgr.GetRegionTimeManagers())
                    {
                        ...
                        var curTick = mgr.CurrentTime;
                        if (ts.Time == curTick)
                        {
                            log.FatalFormat("{0} stopped ticking; timer stacktrace:\n{1}\n", mgr.Name,
                                mgr.GetFormattedStackTrace());
        ...
        finally
        {
            lock (typeof(StatPrint))
            {
                if (m_timer != null) m_timer.Change(ServerProperties.Properties.STATPRINT_FREQUENCY, 0);
            }
        }
    }
        ...
        string instanceName)
    {
        ...
        catch (Exception ex)
        {
            ...
            if (log.IsWarnEnabled)
                log.Warn(ex.GetType().Name + " '" + categoryName + "/" + counterName +
                         "' counter won't be available: " + ex.Message);
        }
