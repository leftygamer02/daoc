...
public class ServerListUpdate
{
    ...
    public static bool Init()
    {
        ...
        if (args != null)
        {
            ...
            {
                log.Error(
                    "DAoCPortal Error: Validating Arguments Failed! The server will not be added/updated on the list!");
    ...
    private static void StartList()
    {
        ...
        var ClientCount = WorldMgr.GetAllPlayingClientsCount().ToString();

        //Set up our URI to be passed to the WebClient.
        var Updater = UrlEncode(UpdateURL + "&username=" + Properties.SERVER_LIST_UPDATE_USER +
                                "&password=" + Properties.SERVER_LIST_UPDATE_PASS + "&totalclients=" +
                                ClientCount + "&version=" + ScriptVersion);
