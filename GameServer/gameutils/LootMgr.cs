...
public sealed class LootMgr
{
    ...
    /// </summary>
    private static readonly HybridDictionary m_ClassGenerators = new();

    /// <summary>
    /// List of global Lootgenerators 
    /// </summary>
    private static readonly IList m_globalGenerators = new ArrayList();

    /// <summary>
    /// List of Lootgenerators related by mobname
    /// </summary>
    private static readonly HybridDictionary m_mobNameGenerators = new();

    /// <summary>
    /// List of Lootgenerators related by mobguild
    /// </summary>
    private static readonly HybridDictionary m_mobGuildGenerators = new();

    /// <summary>
    /// List of Lootgenerators related by region ID
    /// </summary>
    private static readonly HybridDictionary m_mobRegionGenerators = new();

    // /// <summary>
    // /// List of Lootgenerators related by mobfaction
    // /// </summary>
    private static readonly HybridDictionary m_mobFactionGenerators = new();
    ...
    public static bool Init()
    {
        ...
        if (m_lootGenerators != null) // did we find any loot generators
            foreach (var dbGenerator in m_lootGenerators)
            {
                var generator = GetGeneratorInCache(dbGenerator);
                if (generator == null)
                {
                    ...
                    if (generatorType == null)
                        generatorType = Assembly.GetAssembly(typeof(GameServer))
                            .GetType(dbGenerator.LootGeneratorClass);
    ...
    private static ILootGenerator GetGeneratorInCache(LootGenerator dbGenerator)
    {
        if (m_ClassGenerators[dbGenerator.LootGeneratorClass + dbGenerator.ExclusivePriority] != null)
            return (ILootGenerator) m_ClassGenerators[
                dbGenerator.LootGeneratorClass + dbGenerator.ExclusivePriority];

        return null;
    }

    public static void UnRegisterLootGenerator(ILootGenerator generator, string mobname, string mobguild,
        string mobfaction)
    {
    ...
    public static void UnRegisterLootGenerator(ILootGenerator generator, string mobname, string mobguild,
        string mobfaction, int mobregion)
    {
        if (generator == null)
            return;
        ...
        if (!Util.IsEmpty(mobname))
            ...
            {
                // Parse CSV
                var mobNames = Util.SplitCSV(mobname);

                foreach (var mob in mobNames)
            ...
            {
                if (log.IsDebugEnabled)
                    log.Debug("Could not Parse mobNames for Removing LootGenerator : " +
        ...
        if (!Util.IsEmpty(mobguild))
            ...
            {
                // Parse CSV
                var mobGuilds = Util.SplitCSV(mobguild);

                foreach (var guild in mobGuilds)
            ...
            {
                if (log.IsDebugEnabled)
                    log.Debug("Could not Parse mobGuilds for Removing LootGenerator : " +
                              generator.GetType().FullName);
            }
        ...
        if (!Util.IsEmpty(mobfaction))
            ...
            {
                // Parse CSV
                var mobFactions = Util.SplitCSV(mobfaction);

                foreach (var sfaction in mobFactions)
                    try
                    {
                        var ifaction = int.Parse(sfaction);
            ...
            {
                if (log.IsDebugEnabled)
                    log.Debug("Could not Parse mobFactions for Removing LootGenerator : " +
                              generator.GetType().FullName);
            }
        ...
        if (mobregion > 0)
        {
            var regionList = (IList) m_mobRegionGenerators[mobregion];
            if (regionList != null) regionList.Remove(generator);
        }
    ...
    public static void RegisterLootGenerator(ILootGenerator generator, string mobname, string mobguild,
        string mobfaction, int mobregion)
    {
        if (generator == null)
            return;
        ...
        if (!Util.IsEmpty(mobname))
            ...
            try
            {
                ...
                foreach (var mob in mobNames)
                {
                    if ((IList) m_mobNameGenerators[mob] == null) m_mobNameGenerators[mob] = new ArrayList();
            ...
            {
                if (log.IsDebugEnabled)
                    log.Debug("Could not Parse mobNames for Registering LootGenerator : " +
                              generator.GetType().FullName);
            }
        ...
        if (!Util.IsEmpty(mobguild))
            ...
            try
            {
                ...
                foreach (var guild in mobGuilds)
                {
                    if ((IList) m_mobGuildGenerators[guild] == null) m_mobGuildGenerators[guild] = new ArrayList();
            ...
            {
                if (log.IsDebugEnabled)
                    log.Debug("Could not Parse mobGuilds for Registering LootGenerator : " +
                              generator.GetType().FullName);
            }
        ...
        if (!Util.IsEmpty(mobfaction))
            ...
            {
                if (log.IsDebugEnabled)
                    log.Debug("Could not Parse mobFactions for Registering LootGenerator : " +
                              generator.GetType().FullName);
            }
    ...
    public static ItemTemplate[] GetLoot(GameNPC mob, GameObject killer)
    {
        ...
        var generators = GetLootGenerators(mob);
    ...
    public static IList GetLootGenerators(GameNPC mob)
    {
        ...
        var nameGenerators = (IList) m_mobNameGenerators[mob.Name];
        var guildGenerators = (IList) m_mobGuildGenerators[mob.GuildName];
        var regionGenerators = (IList) m_mobRegionGenerators[(int) mob.CurrentRegionID];
        IList factionGenerators = null;
        if (mob.Faction != null)
            factionGenerators = (IList) m_mobFactionGenerators[mob.Faction.ID];

        var allGenerators = new ArrayList();
        ...
        foreach (ILootGenerator generator in allGenerators)
        {
            if (generator.ExclusivePriority > 0)
                if (exclusiveGenerator == null ||
                    exclusiveGenerator.ExclusivePriority < generator.ExclusivePriority)
