...
public sealed class BoatMgr
{
    ...
    /// </summary>
    private static readonly HybridDictionary m_boats = new();

    /// <summary>
    /// ArrayList of all boatid's to BoatNames
    /// </summary>
    private static readonly HybridDictionary m_boatids = new();
    ...
    public static GameBoat CreateBoat(GamePlayer creator, GameBoat boat)
    {
        ...
        try
        {
            ...
            {
                if (creator != null)
                    creator.Out.SendMessage(boat.Name + " already exists!", eChatType.CT_System,
    ...
    public static bool DeleteBoat(string boatName)
    {
        try
        {
            var removeBoat = GetBoatByName(boatName);
            // Does boat exist, if not return null
            if (removeBoat == null) return false;
    ...
    public static void SaveAllBoats()
    {
        ...
        try
        {
            lock (m_boats.SyncRoot)
            {
                foreach (GameBoat b in m_boats.Values) b.SaveIntoDatabase();
            }
        }
        catch (Exception e)
        {
    ...
    {
        var boats = new ArrayList();
        lock (m_boats.SyncRoot)
        {
            foreach (GameBoat boat in m_boats.Values) boats.Add(boat);
        }
