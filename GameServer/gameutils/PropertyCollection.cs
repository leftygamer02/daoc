...
public class PropertyCollection
{
    ...
    /// </summary>
    private readonly ReaderWriterDictionary<object, object> _props = new();
    ...
    {
        return getProperty<T>(key, default);
    }
    ...
    public T getProperty<T>(object key, T def, bool loggued)
    {
        ...
        var exists = _props.TryGetValue(key, out val);

        if (loggued)
            if (!exists)
            {
                if (Log.IsWarnEnabled)
                    Log.Warn("Property '" + key + "' is required but not found, default value '" + def +
                             "' is used.");
