...
public class AttackData
{
    ...
    /// </summary>
    public List<ISpellHandler> StyleEffects => m_styleEffects;
    ...
    public bool IsMeleeAttack =>
        m_attackType == eAttackType.MeleeOneHand
    ...
    public bool IsRandomFumble
    {
        get
        {
            var randNum = Util.CryptoNextDouble();
            var fumbleChance = Attacker.ChanceToFumble;

            var overrideRNG = ServerProperties.Properties.OVERRIDE_DECK_RNG;
            ...
            if (Attacker is GamePlayer p)
            {
                ...
                if (p.UseDetailedCombatLog)
                    p.Out.SendMessage(
                        $"Your chance to fumble: {(100 * fumbleChance).ToString("0.##")}% rand: {(100 * randNum).ToString("0.##")}",
