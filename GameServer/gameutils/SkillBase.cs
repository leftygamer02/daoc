...
public class SkillBase
{
    ...
    protected static readonly Dictionary<int, IDictionary<int, Tuple<Spell, int>>> m_stylesProcs = new();
                ...
                foreach (var spell in spelldb)
                    ...
                    catch (Exception e)
                    {
                        if (log.IsErrorEnabled)
                            log.ErrorFormat("{0} with spellid = {1} spell.TS= {2}", e.Message, spell.SpellID,
                                spell.ToString());
                    }
    ...
    public static void LoadSpellLines()
    {
        ...
        try
        {
            ...
            if (dbo != null)
            {
                ...
                foreach (var line in dbo)
                    try
                    {
                        m_spellLineIndex.Add(line.KeyName,
                            new SpellLine(line.KeyName, line.Name, line.Spec, line.IsBaseLine));
                    }
                    catch (Exception e)
                    {
                        if (log.IsErrorEnabled)
                            log.ErrorFormat("{0} with Spell Line = {1} line.TS= {2}", e.Message, line.KeyName,
                                line.ToString());
                    }
            ...
            var dbox = GameServer.Database.SelectAllObjects<DBLineXSpell>();
            ...
            if (dbox != null)
            {
                ...
                foreach (var lxs in dbox)
                    ...
                    {
                        if (log.IsErrorEnabled)
                            log.ErrorFormat("LineXSpell Spell Adding Error : {0}, Line {1}, Spell {2}, Level {3}",
                                e.Message, lxs.LineName, lxs.SpellID, lxs.Level);
                    }
            ...
            // sort spells
            foreach (var sps in m_lineSpells.Keys.ToList())
    ...
    public static int ReloadSpellLines()
    {
        ...
        try
        {
            foreach (var lineName in m_spellLineIndex.Keys)
            {
                ...
                if (spells != null)
                {
                    ...
                    foreach (var lxs in spells)
                        try
                        {
                            ...
                            for (var r = 0; r < m_lineSpells[lineName].Count; r++)
                                if (m_lineSpells[lineName][r] != null &&
                                    m_lineSpells[lineName][r].ID == lxs.SpellID)
                                {
                        ...
                        {
                            if (log.IsErrorEnabled)
                                log.ErrorFormat("LineXSpell Adding Error : {0}, Line {1}, Spell {2}, Level {3}",
                                    e.Message, lxs.LineName, lxs.SpellID, lxs.Level);
                        }

                    // Line can need a sort...
                    m_lineSpells[lineName] =
                        m_lineSpells[lineName].OrderBy(e => e.Level).ThenBy(e => e.ID).ToList();
                }
            }
        }
        finally
    ...
    private static void LoadAbilities()
    {
        ...
        try
        {
            ...
            if (abilities != null)
            {
                ...
                foreach (var dba in abilities)
                    try
                    {
                        // test only...
                        var ability = GetNewAbilityInstance(dba);

                        m_abilityIndex.Add(ability.KeyName, dba);

                        if (log.IsDebugEnabled)
                            log.DebugFormat("Ability {0} successfuly instanciated from {1} (expeted {2})",
                                dba.KeyName, dba.Implementation, ability.GetType());
                    }
                    catch (Exception e)
                    {
                        if (log.IsWarnEnabled)
                            log.WarnFormat("Error while Loading Ability {0} with Class {1} : {2}", dba.KeyName,
                                dba.Implementation, e);
                    }

                abilities = null;
            }

            if (log.IsInfoEnabled) log.InfoFormat("Total abilities loaded: {0}", m_abilityIndex.Count);
        }
        finally
        {
            m_syncLockUpdates.ExitWriteLock();
        ...
        try
        {
            ...
            if (classxra != null)
            {
                foreach (var cxra in classxra)
                {
                    ...
                    try
                    {
                        var dba = m_abilityIndex[cxra.AbilityKey];
                    ...
                    {
                        if (log.IsWarnEnabled)
                            log.WarnFormat("Error while Adding RealmAbility {0} to Class {1} : {2}",
                                cxra.AbilityKey, cxra.CharClass, e);
                    }
                }
    ...
    public static int LoadSpecializations()
    {
        ...
        try
        {
            ...
            if (specs != null)
            {
                ...
                {
                    var str = new StringBuilder("Specialization ");
                    str.AppendFormat("{0} - ", spec.KeyName);

                    Specialization gameSpec = null;
                    if (Util.IsEmpty(spec.Implementation, true) == false)
                        gameSpec = GetNewSpecializationInstance(spec.KeyName, spec.Implementation, spec.Name,
                            spec.Icon, spec.SpecializationID);
                    else
                        gameSpec = new Specialization(spec.KeyName, spec.Name, spec.Icon, spec.SpecializationID);

                    if (log.IsDebugEnabled)
                        log.DebugFormat("Specialization {0} successfuly instanciated from {1} (expected {2})",
                            spec.KeyName, gameSpec.GetType().FullName, spec.Implementation);

                    var entry =
                        new Tuple<Type, string, ushort, int>(gameSpec.GetType(), spec.Name, spec.Icon,
                    ...
                    if (spec.AbilityConstraints != null)
                    {
                        ...
                        foreach (var specx in spec.AbilityConstraints)
                            try
                            {
                                m_specsAbilities[spec.KeyName]
                                    .Add(new Tuple<string, byte, int, int>(m_abilityIndex[specx.AbilityKey].KeyName,
                                        (byte) specx.SpecLevel, specx.AbilityLevel, specx.ClassId));
                                count++;
                            }
                            catch (Exception e)
                            {
                                if (log.IsWarnEnabled)
                                    log.WarnFormat(
                                        "Specialization : {0} while adding Spec X Ability {1}, from Spec {2}({3}), Level {4}",
                                        e.Message, specx.AbilityKey, specx.Spec, specx.SpecLevel,
                                        specx.AbilityLevel);
                            }
                    ...
                    if (spec.SpellLines != null)
                        foreach (var line in spec.SpellLines)
                        {
                            ...
                            try
                            {
                                m_specsSpellLines[spec.KeyName]
                                    .Add(new Tuple<SpellLine, int>(m_spellLineIndex[line.KeyName],
                                        line.ClassIDHint));
                                count++;
                            }
                            catch (Exception e)
                            {
                                if (log.IsWarnEnabled)
                                    log.WarnFormat(
                                        "Specialization : {0} while adding Spec X SpellLine {1} from Spec {2}, ClassID {3}",
                                        e.Message, line.KeyName, line.Spec, line.ClassIDHint);
                            }
                        }
                    ...
                    count = 0;
                    if (spec.Styles != null)
                        foreach (var specStyle in spec.Styles)
                        {
                            // Update Style Career
                            ...
                            var newStyle = new Style(specStyle);

                            m_specsStyles[spec.KeyName][specStyle.ClassId]
                                .Add(new Tuple<Style, byte>(newStyle, (byte) specStyle.SpecLevelRequirement));

                            // Update Style Index.
                            var styleKey =
                                new KeyValuePair<int, int>(newStyle.ID, specStyle.ClassId);
                            ...
                            else
                            {
                                if (log.IsWarnEnabled)
                                    log.WarnFormat(
                                        "Specialization {0} - Duplicate Style Key, StyleID {1} : ClassID {2}, Ignored...",
                                        spec.KeyName, newStyle.ID, specStyle.ClassId);
                            }

                            // load Procs
                            if (specStyle.AttachedProcs != null)
                                foreach (var styleProc in specStyle.AttachedProcs)
                                    if (m_spellIndex.ContainsKey(styleProc.SpellID))
                                    {
                                        if (!m_stylesProcs.ContainsKey(specStyle.ID))
                                            m_stylesProcs.Add(specStyle.ID,
                                                new Dictionary<int, Tuple<Spell, int>>());

                                        if (!m_stylesProcs[specStyle.ID].ContainsKey(styleProc.ClassID))
                                            m_stylesProcs[specStyle.ID].Add(styleProc.ClassID,
                                                new Tuple<Spell, int>(m_spellIndex[styleProc.SpellID],
                                                    styleProc.Chance));
                                    }
                        }

                    // We've added all the styles to their respective lists.  Now lets go through and sort them by their level
                    foreach (var keyname in m_specsStyles.Keys)
                    foreach (var classid in m_specsStyles[keyname].Keys)
    ...
    public static void LoadClassSpecializations()
    {
        ...
        try
        {
            ...
            //Retrieve from DB
            var dbo = GameServer.Database.SelectAllObjects<ClassXSpecialization>();
            var summary = new Dictionary<int, StringBuilder>();
            ...
            if (dbo != null)
            {
                ...
                foreach (var career in dbo)
                {
                    ...
                    if (!m_specsByClass[career.ClassID].ContainsKey(career.SpecKeyName))
                    {
                        ...
                        summary[career.ClassID]
                            .AppendFormat("{0}({1}), ", career.SpecKeyName, career.LevelAcquired);
                    }
                    else
                    {
                        if (log.IsWarnEnabled)
                            log.WarnFormat("Duplicate Sepcialization Key {0} for Class Career : {1}",
                                career.SpecKeyName, career.ClassID);
                    }
                }
            }
        ...
        try
        {
            ...
            foreach (var entry in ht)
            {
                ...
                else
                {
                    ...
                    {
                        // test only...
                        var handler = GetNewAbilityActionHandler(entry.Value);
                    ...
                    catch (Exception ex)
                    {
                        if (log.IsErrorEnabled)
                            log.ErrorFormat(
                                "Error While instantiacting IAbilityHandler {0} using {1} in GameServer : {2}",
                                entry.Key, entry.Value, ex);
                    }
                }
            }
            ...
            foreach (var asm in ScriptMgr.Scripts)
            {
                ...
                foreach (var entry in ht)
                {
                    ...
                    catch (Exception ex)
                    {
                        if (log.IsErrorEnabled)
                            log.ErrorFormat(
                                "Error While instantiacting IAbilityHandler {0} using {1} in GameServerScripts : {2}",
                                entry.Key, entry.Value, ex);
                    }
    ...
    private static void LoadSkillHandlers()
    {
        ...
        try
        {
            ...
            var ht =
                ScriptMgr.FindAllSpecActionHandler(Assembly.GetExecutingAssembly());
            ...
            foreach (var entry in ht)
            {
                ...
                else
                {
                    ...
                    {
                        // test only...
                        var handler = GetNewSpecActionHandler(entry.Value);
                        var test = handler.ToString();
                    ...
                    catch (Exception ex)
                    {
                        if (log.IsWarnEnabled)
                            log.WarnFormat(
                                "Error While instantiacting ISpecActionHandler {0} using {1} in GameServer : {2}",
                                entry.Key, entry.Value, ex);
                    }
                }
            }
            ...
            foreach (var asm in ScriptMgr.Scripts)
            {
                ...
                foreach (var entry in ht)
                {
                    ...
                    catch (Exception ex)
                    {
                        if (log.IsWarnEnabled)
                            log.WarnFormat(
                                "Error While instantiacting ISpecActionHandler {0} using {1} in GameServerScripts : {2}",
                                entry.Key, entry.Value, ex);
                    }
    ...
    private static void InitPropertyTypes()
    {
        ...
        m_propertyTypes[(int) eProperty.Skill_Critical_Strike] =
            ePropertyType.Skill | ePropertyType.SkillMeleeWeapon;
        m_propertyTypes[(int) eProperty.Skill_Crushing] = ePropertyType.Skill | ePropertyType.SkillMeleeWeapon;
        m_propertyTypes[(int) eProperty.Skill_Flexible_Weapon] =
            ePropertyType.Skill | ePropertyType.SkillMeleeWeapon;
        ...
        m_propertyTypes[(int) eProperty.Skill_Thrown_Weapons] =
            ePropertyType.Skill | ePropertyType.SkillMeleeWeapon;
    ...
    private static void RegisterPropertyNames()
    {
        ...
        m_propertyNames.Add(eProperty.OffhandDamageAndChance, LanguageMgr.GetTranslation(
            ServerProperties.Properties.DB_LANGUAGE,
        ...
        m_propertyNames.Add(eProperty.CriticalArcheryHitChance,
            LanguageMgr.GetTranslation(ServerProperties.Properties.DB_LANGUAGE,
                "SkillBase.RegisterPropertyNames.CriticalArcheryHit"));
        m_propertyNames.Add(eProperty.CriticalMeleeHitChance,
            LanguageMgr.GetTranslation(ServerProperties.Properties.DB_LANGUAGE,
                "SkillBase.RegisterPropertyNames.CriticalMeleeHit"));
        m_propertyNames.Add(eProperty.CriticalSpellHitChance,
            LanguageMgr.GetTranslation(ServerProperties.Properties.DB_LANGUAGE,
                "SkillBase.RegisterPropertyNames.CriticalSpellHit"));
        m_propertyNames.Add(eProperty.CriticalHealHitChance,
            LanguageMgr.GetTranslation(ServerProperties.Properties.DB_LANGUAGE,
                "SkillBase.RegisterPropertyNames.CriticalHealHit"));
    ...
    private static readonly int[] m_armorResists =
        new int[1 << (REALM_BITCOUNT + DAMAGETYPE_BITCOUNT + ARMORTYPE_BITCOUNT)];
    ...
    public static int GetArmorResist(InventoryItem armor, eDamageType damageType)
    {
        ...
        var realm = armor.Template.Realm - (int) eRealm._First;
        var armorType = armor.Template.Object_Type - (int) eObjectType._FirstArmor;
    ...
    private static void WriteMeleeResists(eRealm realm, eObjectType armorType, int slash, int crush, int thrust)
    {
        if (realm < eRealm._First || realm > eRealm._LastPlayerRealm)
            throw new ArgumentOutOfRangeException("realm", realm,
                "Realm should be between _First and _LastPlayerRealm.");
        if (armorType < eObjectType._FirstArmor || armorType > eObjectType._LastArmor)
            throw new ArgumentOutOfRangeException("armorType", armorType,
                "Armor type should be between _FirstArmor and _LastArmor");

        var off = (realm - eRealm._First) << (DAMAGETYPE_BITCOUNT + ARMORTYPE_BITCOUNT);
    ...
    private static void WriteMagicResists(eRealm realm, eObjectType armorType, int heat, int cold, int matter,
        int energy)
    {
        if (realm < eRealm._First || realm > eRealm._LastPlayerRealm)
            throw new ArgumentOutOfRangeException("realm", realm,
    ...
    public static IAbilityActionHandler GetAbilityActionHandler(string keyName)
    {
        ...
        if (exists) return GetNewAbilityActionHandler(handlerType);
    ...
    public static ISpecActionHandler GetSpecActionHandler(string keyName)
    {
        ...
        if (exists)
            try
            {
                var newHndl = GetNewSpecActionHandler(handlerType);
                return newHndl;
            }
            catch (Exception e)
            {
                if (log.IsErrorEnabled)
                    log.ErrorFormat("Error while instanciating ISpecActionHandler {0} From Handler {2}: {1}",
                        keyName, e, handlerType);
            }
    ...
    public static void AddScriptedStyle(Specialization spec, DBStyle style)
    {
        ...
        try
        {
            ...
            m_specsStyles[spec.KeyName][style.ClassId]
                .Add(new Tuple<Style, byte>(st, (byte) style.SpecLevelRequirement));

            var styleKey = new KeyValuePair<int, int>(st.ID, style.ClassId);
            if (!m_styleIndex.ContainsKey(styleKey))
    ...
    public static void RegisterSpec(Specialization spec)
    {
        ...
        {
            var entry =
                new Tuple<Type, string, ushort, int>(spec.GetType(), spec.Name, spec.Icon, spec.ID);
    ...
    {
        var ras = new List<DBAbility>();
        m_syncLockUpdates.EnterReadLock();
        try
        {
            if (m_classRealmAbilities.ContainsKey(classID))
                foreach (var str in m_classRealmAbilities[classID])
                    try
                    {
        ...
        return ras.Select(e => GetNewAbilityInstance(e)).Where(ab => ab is RealmAbility).Cast<RealmAbility>()
            .ToList();
    }
    ...
    public static Ability GetAbility(string keyname, int level)
    {
        ...
        try
        {
            if (m_abilityIndex.ContainsKey(keyname)) dbab = m_abilityIndex[keyname];
        }
        finally
        {
            m_syncLockUpdates.ExitReadLock();
        ...
        if (dbab != null)
        {
            var dba = GetNewAbilityInstance(dbab);
            dba.Level = level;
            return dba;
        }
    ...
    public static IList<Tuple<SpellLine, int>> GetSpecsSpellLines(string specName)
    {
        ...
        try
        {
            if (m_specsSpellLines.ContainsKey(specName))
                foreach (var entry in m_specsSpellLines[specName])
    ...
    public static SpellLine GetSpellLine(string keyname, bool create)
    {
        ...
        if (create)
        {
            if (log.IsWarnEnabled) log.WarnFormat("Spell-Line {0} unknown, creating temporary line.", keyname);
    ...
    public static void AddScriptedSpell(string spellLineID, Spell spell)
    {
        ...
        try
        {
            ...
            var added = false;
            for (var r = 0; r < m_lineSpells[spellLineID].Count; r++)
                try
                {
                    if ((m_lineSpells[spellLineID][r] != null &&
                         spell.ID > 0 && m_lineSpells[spellLineID][r].ID == spell.ID &&
                         m_lineSpells[spellLineID][r].Name.ToLower().Equals(spell.Name.ToLower()) &&
                         m_lineSpells[spellLineID][r].SpellType.ToString().ToLower()
                             .Equals(spell.SpellType.ToString().ToLower()))
                        || (m_lineSpells[spellLineID][r].Name.ToLower().Equals(spell.Name.ToLower()) &&
                            m_lineSpells[spellLineID][r].SpellType.ToString().ToLower()
                                .Equals(spell.SpellType.ToString().ToLower())))
                    {
    ...
    public static void AddSpellToSpellLine(string spellLineID, int spellID, int level = 1)
    {
        ...
        try
        {
            ...
            try
            {
                var spl = (Spell) m_spellIndex[spellID].Clone();
                spl.Level = level;
                ...
                m_lineSpells[spellLineID] =
                    m_lineSpells[spellLineID].OrderBy(e => e.Level).ThenBy(e => e.ID).ToList();
            }
            catch (Exception e)
            {
                if (log.IsErrorEnabled)
    ...
    public static Specialization GetSpecialization(string keyname, bool create)
    {
        ...
        if (create)
        {
            if (log.IsWarnEnabled) log.WarnFormat("Specialization {0} unknown", keyname);
    ...
    {
        var dictRes = new Dictionary<Specialization, int>();
        m_syncLockUpdates.EnterReadLock();
        IDictionary<string, int> entries = new Dictionary<string, int>();
        try
        {
            if (m_specsByClass.ContainsKey(classID)) entries = new Dictionary<string, int>(m_specsByClass[classID]);
        }
        finally
        {
            m_syncLockUpdates.ExitReadLock();
        ...
        foreach (var constraint in entries)
            try
            {
                var spec = GetSpecialization(constraint.Key, false);
        ...
        try
        {
            if (m_specsByClass.ContainsKey(0)) entries = new Dictionary<string, int>(m_specsByClass[0]);
        }
        finally
        {
            m_syncLockUpdates.ExitReadLock();
    ...
    {
        m_syncLockUpdates.EnterReadLock();
        var entries = new List<Tuple<Style, byte>>();
        try
        {
    ...
    {
        m_syncLockUpdates.EnterReadLock();
        var entries = new List<Tuple<string, byte, int, int>>();
        try
        {
        ...
        var abRes = new List<Ability>();
        foreach (var constraint in entries)
        {
            if (constraint.Item4 != 0 && constraint.Item4 != classID)
                continue;

            var ab = GetNewAbilityInstance(constraint.Item1, constraint.Item3);
            ab.Spec = specID;
    ...
    {
        var styleKey = new KeyValuePair<int, int>(styleID, classId);
        Style style;
        m_syncLockUpdates.EnterReadLock();
        try
        {
    ...
    {
        var procres = new List<Tuple<Spell, int, int>>();
        m_syncLockUpdates.EnterReadLock();
        var entries = new Dictionary<int, Tuple<Spell, int>>();
        try
        {
    ...
    public static Spell FindSpell(int spellID, SpellLine line)
    {
        ...
        if (line != null)
        {
            var spells = GetSpellList(line.KeyName);
            foreach (var lineSpell in spells)
        ...
        if (spell == null) spell = GetSpellByID(spellID);
    ...
    {
        string name;
        if (!m_propertyNames.TryGetValue(prop, out name)) name = "Property" + (int) prop;
    ...
    private static Ability GetNewAbilityInstance(string keyname, int level)
    {
        ...
        try
        {
            if (m_abilityIndex.ContainsKey(keyname)) dba = m_abilityIndex[keyname];
        }
        finally
        {
            m_syncLockUpdates.ExitReadLock();
    ...
    private static Ability GetNewAbilityInstance(DBAbility dba)
    {
        ...
        if (Util.IsEmpty(dba.Implementation, true) == false)
        {
            ...
            foreach (var asm in AppDomain.CurrentDomain.GetAssemblies())
                try
                {
                    ab = (Ability) asm.CreateInstance(
                        dba.Implementation, // string including namespace of the type
                        true,
            ...
            if (ab == null)
            {
                ...
                if (log.IsWarnEnabled)
                    log.WarnFormat("Could not Instanciate Ability {0} from {1} reverting to default Ability...",
                        dba.KeyName, dba.Implementation);
            }
        }
        else
        {
    ...
    private static Specialization GetNewSpecializationInstance(string keyname,
        Tuple<Type, string, ushort, int> entry)
    {
        Specialization gameSpec = null;
        ...
        try
        {
            gameSpec = (Specialization) entry.Item1.Assembly.CreateInstance(
                entry.Item1.FullName, // string including namespace of the type
                true,
                BindingFlags.Default,
                null,
                new object[] {keyname, entry.Item2, entry.Item3, entry.Item4},
                null,
                null);

            // instanciation worked
            if (gameSpec != null) return gameSpec;
        }
        catch
        {
        }
    ...
    private static Specialization GetNewSpecializationInstance(string keyname, string type, string name,
        ushort icon, int id)
    {
        Specialization gameSpec = null;
        ...
        foreach (var asm in AppDomain.CurrentDomain.GetAssemblies())
            try
            {
                gameSpec = (Specialization) asm.CreateInstance(
                    type, // string including namespace of the type
                    true,
                    BindingFlags.Default,
                    null,
                    new object[] {keyname, name, icon, id},
                    null,
                    null);

                // instanciation worked
                if (gameSpec != null) break;
            }
            catch
            {
            }
        ...
        if (gameSpec == null)
        {
            ...
            if (log.IsErrorEnabled)
                log.ErrorFormat(
                    "Could not Instanciate Specialization {0} from {1} reverting to default Specialization...",
                    keyname, type);
        }
