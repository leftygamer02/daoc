...
public static class AppealMgr
{
    ...
    public static List<GamePlayer> StaffList = new();
    ...
    public static void NotifyStaff()
    {
        ...
        foreach (var a in Appeals)
        {
            ...
            switch (a.Severity)
            {
                case (int) eSeverity.Low:
                    low++;
                    break;
                case (int) eSeverity.Medium:
                    med++;
                    break;
                case (int) eSeverity.High:
                    high++;
                    break;
                case (int) eSeverity.Critical:
                    crit++;
                    break;
            }
        }
        ...
        if (Appeals.Count >= 2)
        {
            MessageToAllStaff("There are " + Appeals.Count + " Appeals in the queue.", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            MessageToAllStaff("There are " + Appeals.Count + " Appeals in the queue.", eChatType.CT_Say,
                eChatLoc.CL_ChatWindow);
        }

        if (Appeals.Count == 1)
        {
            MessageToAllStaff("There is " + Appeals.Count + " appeal in the queue.", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            MessageToAllStaff("There is " + Appeals.Count + " appeal in the queue.", eChatType.CT_Say,
                eChatLoc.CL_ChatWindow);
        }

        MessageToAllStaff(
            "Crit:" + crit + ", High:" + high + ", Med:" + med + ", Low:" + low + ".  [use /gmappeal]",
        ...
        if (crit >= 1)
        {
            MessageToAllStaff("Critical Appeals may need urgent attention!", eChatType.CT_YouDied,
                eChatLoc.CL_SystemWindow);
    ...
    public static void MessageToAllStaff(string msg)
    {
        ...
        foreach (var staffplayer in StaffList)
    ...
    {
        var appeal = DOLDB<DBAppeal>.SelectObject(DB.Column("Name").IsEqualTo(name));
        return appeal;
    }

    public static DBAppeal GetAppealByAccountName(string name)
    {
        var appeal = DOLDB<DBAppeal>.SelectObject(DB.Column("Account").IsEqualTo(name));
        return appeal;
    }
    ...
    public static IList<DBAppeal> GetAllAppeals()
    {
        ...
        foreach (var c in clientlist)
            try
            {
                var ap = GetAppealByPlayerName(c.Player.Name);
                if (ap != null) rlist.Add(ap);
            }
            catch
            {
    ...
    public static void CreateAppeal(GamePlayer Player, int Severity, string Status, string Text)
    {
        if (Player.IsMuted)
        {
            Player.Out.SendMessage(
                "[Appeals]: " + LanguageMgr.GetTranslation(Player.Client.Account.Language,
                    "Scripts.Players.Appeal.YouAreMuted"), eChatType.CT_Important, eChatLoc.CL_ChatWindow);
            return;
        }

        var HasPendingAppeal = Player.TempProperties.getProperty<bool>("HasPendingAppeal");
        if (HasPendingAppeal)
        {
            Player.Out.SendMessage(
                "[Appeals]: " + LanguageMgr.GetTranslation(Player.Client.Account.Language,
                    "Scripts.Players.Appeal.AlreadyActiveAppeal"), eChatType.CT_Important, eChatLoc.CL_ChatWindow);
            return;
        }

        var eText = GameServer.Database.Escape(Text); //prevent SQL injection
        var TimeStamp = DateTime.Now.ToLongTimeString() + " " + DateTime.Now.ToLongDateString();
        var appeal = new DBAppeal(Player.Name, Player.Client.Account.Name, Severity, Status, TimeStamp, eText);
        GameServer.Database.AddObject(appeal);
        Player.TempProperties.setProperty("HasPendingAppeal", true);
        Player.Out.SendMessage(
            "[Appeals]: " + LanguageMgr.GetTranslation(Player.Client.Account.Language,
                "Scripts.Players.Appeal.AppealSubmitted"), eChatType.CT_Important, eChatLoc.CL_ChatWindow);
        Player.Out.SendMessage(
            "[Appeals]: " + LanguageMgr.GetTranslation(Player.Client.Account.Language,
                "Scripts.Players.Appeal.IfYouLogOut"), eChatType.CT_Important, eChatLoc.CL_ChatWindow);
    ...
    public static void ChangeStatus(string staffname, GamePlayer target, DBAppeal appeal, string status)
    {
        ...
        MessageToAllStaff("Staffmember " + staffname + " has changed the status of " + target.Name +
                          "'s appeal to " + status + ".");
        target.Out.SendMessage(
            "[Appeals]: " + LanguageMgr.GetTranslation(target.Client, "Scripts.Players.Appeal.StaffChangedStatus",
                staffname, status), eChatType.CT_Important, eChatLoc.CL_ChatWindow);
        return;
    }
    ...
    {
        MessageToAllStaff("[Appeals]: " + "Staffmember " + staffname + " has just closed " + Player.Name +
                          "'s appeal.");
        Player.Out.SendMessage(
            "[Appeals]: " + LanguageMgr.GetTranslation(Player.Client.Account.Language,
                "Scripts.Players.Appeal.StaffClosedYourAppeal", staffname), eChatType.CT_Important,
            eChatLoc.CL_ChatWindow);
    ...
    {
        MessageToAllStaff("[Appeals]: " + "Staffmember " + staffname + " has just closed " + appeal.Name +
                          "'s (offline) appeal.");
        GameServer.Database.DeleteObject(appeal);
        return;
    }
    ...
    public static void CancelAppeal(GamePlayer Player, DBAppeal appeal)
    {
        ...
        Player.Out.SendMessage(
            "[Appeals]: " + LanguageMgr.GetTranslation(Player.Client.Account.Language,
                "Scripts.Players.Appeal.CanceledYourAppeal"), eChatType.CT_Important, eChatLoc.CL_ChatWindow);
        Player.Out.SendPlaySound(eSoundType.Craft, 0x02);
        GameServer.Database.DeleteObject(appeal);
        Player.TempProperties.setProperty("HasPendingAppeal", false);
        return;
    ...
    public static void PlayerEnter(DOLEvent e, object sender, EventArgs args)
    {
        ...
        if (player.Client.Account.PrivLevel > (uint) ePrivLevel.Player)
        {
            ...
            if (Appeals.Count > 0)
                player.Out.SendMessage(
                    "[Appeals]: " + "There are " + Appeals.Count +
                    " appeals in the queue!  Use /gmappeal to work the appeals queue.", eChatType.CT_Important,
                    eChatLoc.CL_ChatWindow);
        }

        //Check if there is an existing appeal belonging to this player.
        var appeal = GetAppealByAccountName(player.Client.Account.Name);

        if (appeal == null)
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language, "Scripts.Players.Appeal.LoginMessage"),
                eChatType.CT_System, eChatLoc.CL_SystemWindow);
            return;
        }
        ...
        player.Out.SendMessage(
            "[Appeals]: " + LanguageMgr.GetTranslation(player.Client.Account.Language,
                "Scripts.Players.Appeal.YouHavePendingAppeal"), eChatType.CT_Important, eChatLoc.CL_ChatWindow);
        player.TempProperties.setProperty("HasPendingAppeal", true);
        NotifyStaff();
    }
    ...
    public static void PlayerQuit(DOLEvent e, object sender, EventArgs args)
    {
        ...
        if (player == null)
            return;
        if (player.Client.Account.PrivLevel > (uint) ePrivLevel.Player) StaffList.Remove(player);
    }
