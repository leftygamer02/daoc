...
public static class InventoryLogging
{
    ...
    public static Func<GameObject, string> GetGameObjectString = obj =>
        obj == null
            ? "(null)"
            : "(" + obj.Name + ";" + obj.GetType() + ";" + obj.X + ";" + obj.Y + ";" + obj.Z + ";" +
              obj.CurrentRegionID + ")";
    ...
    public static void LogInventoryAction(GameObject source, GameObject destination, eInventoryActionType type,
        ItemTemplate item, int count = 1)
    {
    ...
    /// <param name="type">Type of action (trade, loot, quest, ...)</param>
    /// <param name="item">The item or money account traded</param>
    public static void LogInventoryAction(string source, GameObject destination, eInventoryActionType type,
        ItemTemplate item, int count = 1)
    {
    ...
    /// <param name="type">Type of action (trade, loot, quest, ...)</param>
    /// <param name="item">The item or money account traded</param>
    public static void LogInventoryAction(GameObject source, string destination, eInventoryActionType type,
        ItemTemplate item, int count = 1)
    {
    ...
    /// <param name="type">Type of action (trade, loot, quest, ...)</param>
    /// <param name="item">The item or money account traded</param>
    public static void LogInventoryAction(string source, string destination, eInventoryActionType type,
        ItemTemplate item, int count = 1)
    {
        ...
        try
        {
            GameServer.Instance.LogInventoryAction(string.Format(format, source ?? "(null)",
                destination ?? "(null)", GetItemString(item, count)));
        }
        catch (Exception e)
        {
    ...
    public static void LogInventoryAction(string source, string destination, eInventoryActionType type, long money)
    {
        ...
        {
            GameServer.Instance.LogInventoryAction(string.Format(format, source ?? "(null)",
                destination ?? "(null)", GetMoneyString(money)));
        }
        catch (Exception e)
        {
            if (log.IsErrorEnabled)
