...
public class CardMgr
{
    ...
    private class Card
    {
        ...
        public GameClient Dealer => m_dealer;
    ...
    private class DealerDeck
    {
        ...
        public bool Init(GameClient Dealer, uint numDecks)
        {
            ...
            try
            {
                cards = new uint[52 * numDecks];
                m_Cards = new Queue((int) (52 * numDecks));
            }
            catch (Exception)
            {
                return false;
    ...
    private class PlayerHand
    {
        ...
        {
            foreach (Card c in m_hand) c.Up = true;
        }
        ...
        public void Held(GameClient source)
        {
            if (m_hand.Count == 0)
            {
                source.Out.SendMessage(
                    (source == m_owner ? "You have " : m_owner.Player.Name + " has ") + "no cards.",
            ...
            foreach (Card c in m_hand)
                if (source == m_owner || c.Up)
                    cards += c.Id + " - " + c.Name + "\n";
            source.Out.SendMessage(
                (source == m_owner ? "You are holding " : m_owner.Player.Name + " is holding ") + m_hand.Count +
                (m_hand.Count > 1 ? " cards." : " card."), eChatType.CT_System, eChatLoc.CL_SystemWindow);
        ...
        public Card Discard(uint selection)
        {
            foreach (Card c in m_hand)
                if (c.Id == selection)
                {
                    ...
                    {
                        foreach (var Groupee in m_owner.Player.Group.GetPlayersInTheGroup())
                            if (Groupee == m_owner.Player)
                                m_owner.Out.SendMessage("You discard the " + c.Name + " from your hand.",
                                    eChatType.CT_Emote, eChatLoc.CL_SystemWindow);
                            else
                                Groupee.Client.Out.SendMessage(
                                    m_owner.Player.Name + " discards " + (c.Up ? "the " + c.Name : "a card") +
                                    " from their hand.", eChatType.CT_Emote, eChatLoc.CL_SystemWindow);
                    }
                    else
                    {
                        m_owner.Out.SendMessage("You cannot play cards without a group!", eChatType.CT_System,
            ...
            m_owner.Out.SendMessage("No card with ID " + selection + " exists in your hand!", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            return null;
        }
        ...
        {
            var cards = (ArrayList) m_hand.Clone();
            m_hand.Clear();
            if (m_owner.Player.Group == null)
                m_owner.Out.SendMessage("You discard all your cards.", eChatType.CT_Emote,
                    eChatLoc.CL_SystemWindow);
            else
                foreach (var Groupee in m_owner.Player.Group.GetPlayersInTheGroup())
                    Groupee.Client.Out.SendMessage(
                        Groupee.Client == m_owner
                            ? "You discard all your cards."
                            : m_owner.Player + " discards all their cards.", eChatType.CT_Emote,
                        eChatLoc.CL_SystemWindow);
            return cards;
        }
    };

    private static Hashtable m_dealerDecks = new();
    private static Hashtable m_playerHands = new();
    ...
    private static GameClient GroupDealer(GameClient player)
    {
        ...
        foreach (var Groupee in player.Player.Group.GetPlayersInTheGroup())
    ...
    {
        if (IsDealer(player)) m_dealerDecks.Remove(player.Player.ObjectId);
    }
    ...
    public static void Shuffle(GameClient player, uint numDecks)
    {
        ...
        {
            player.Out.SendMessage("You must have a group to play cards!", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            return;
        }

        var Dealer = GroupDealer(player);
        DealerDeck newDecks;

        /* First clear out any previous decks for the group */
        if (Dealer != null) QuitDealing(Dealer);
            ...
            foreach (var Groupee in player.Player.Group.GetPlayersInTheGroup())
            {
                ...
                if (Groupee == player.Player)
                    player.Out.SendMessage(
                        "You shuffle " + numDecks + (numDecks > 1 ? " decks " : " deck ") + "of cards.",
                        eChatType.CT_Emote, eChatLoc.CL_SystemWindow);
                else
                    Groupee.Client.Out.SendMessage(
                        player.Player.Name + " shuffles " + numDecks + (numDecks > 1 ? " decks " : " deck ") +
                        "of cards.", eChatType.CT_Emote, eChatLoc.CL_SystemWindow);
            }
        }
        catch (Exception)
        {
    ...
    public static void Deal(GameClient dealer, GameClient player, bool up)
    {
        ...
        {
            dealer.Out.SendMessage(player.Player.Name + " must be in your group to play cards!",
                eChatType.CT_System, eChatLoc.CL_SystemWindow);
            return;
        }
        ...
        if (!IsDealer(dealer))
        {
            dealer.Out.SendMessage("You must use /shuffle to prepare cards before dealing!", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            return;
        }
        ...
        if (c == null)
        {
            dealer.Out.SendMessage("There are no cards left in the deck. Use /shuffle to prepare a new deck.",
                eChatType.CT_System, eChatLoc.CL_SystemWindow);
            return;
        }

        hand.AddCard(c, up);
        foreach (var Groupee in dealer.Player.Group.GetPlayersInTheGroup())
            if (Groupee == dealer.Player)
                dealer.Out.SendMessage(
                    "You deal " + (player == dealer ? "yourself" : player.Player.Name) +
                    (up ? " the " + c.Name : " a card face down") + ".", eChatType.CT_Emote,
                    eChatLoc.CL_SystemWindow);
            else if (Groupee == player.Player)
                player.Out.SendMessage(dealer.Player.Name + " deals you the " + c.Name + ".", eChatType.CT_Emote,
                    eChatLoc.CL_SystemWindow);
            else
                Groupee.Client.Out.SendMessage(
                    dealer.Player.Name + " deals " + (player == dealer ? "themself" : player.Player.Name) +
                    (up ? " the " + c.Name : " a card face down") + ".", eChatType.CT_Emote,
                    eChatLoc.CL_SystemWindow);
    ...
    public static void Held(GameClient source, GameClient target)
    {
        if (!IsPlayer(target))
        {
            source.Player.Out.SendMessage(
                (source == target ? "You have" : target.Player.Name + " has") + " no cards.", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            return;
        }
    ...
    public static void Show(GameClient player)
    {
        ...
        if (!IsPlayer(player))
        {
            player.Out.SendMessage("You have no cards.", eChatType.CT_System, eChatLoc.CL_SystemWindow);
            return;
        }
        ...
        foreach (var Groupee in player.Player.Group.GetPlayersInTheGroup())
            if (Groupee == player.Player)
                ...
                Groupee.Client.Out.SendMessage(player.Player.Name + " shows their hand.", eChatType.CT_Emote,
                    eChatLoc.CL_SystemWindow);
    }
    ...
    public static void Discard(GameClient player, uint selection)
    {
        ...
        if (!IsPlayer(player))
        {
            player.Out.SendMessage("You have no cards to discard.", eChatType.CT_System, eChatLoc.CL_SystemWindow);
            return;
        }
