...
public abstract class AbstractCommandHandler
{
    ...
    {
        var spamKey = commandName + "NOSPAM";
        var tick = player.TempProperties.getProperty<long>(spamKey, 0);

        if (tick > 0 && player.CurrentRegion.Time - tick <= 0) player.TempProperties.removeProperty(spamKey);

        var changeTime = player.CurrentRegion.Time - tick;

        if (tick > 0 && player.CurrentRegion.Time - tick < delay) return true;
    ...
    public virtual void DisplaySyntax(GameClient client, string subcommand)
    {
        ...
        foreach (var sentence in attrib[0].Usage)
        {
            var words = sentence.Split(new[] {' '}, 3);

            if (words.Length >= 2 && words[1].Equals(subcommand)) ChatUtil.SendSystemMessage(client, sentence, null);
        }
    ...
    public virtual void DisplaySyntax(GameClient client, string subcommand1, string subcommand2)
    {
        ...
        foreach (var sentence in attrib[0].Usage)
        {
            var words = sentence.Split(new[] {' '}, 4);
