...
namespace DOL.GS.Commands
{
	...
	public abstract class AbstractCommandHandler
	{
		...
		public virtual void DisplayMessage(GameClient client, string message, params object[] objs)
		{
			...
			ChatUtil.SendTypeMessage("system", client, string.Format(message, objs));
		}
		...
		public virtual void DisplaySyntax(GameClient client)
		{
			...
			if (client.Account.PrivLevel > 1)
			{
				...
				if (attrib[0].Header != "")
					ChatUtil.SendTypeMessage("cmdHeader", client, attrib[0].Header, command, plvlReq);
				else
					// Message: <----- '/{0}' Command ----->
					ChatUtil.SendTypeMessage("cmdHeader", client, "AllCommands.Header.Basic.Commands", command);
			}
			
			// Include main command type description below the header
			// Example: Creates new, manages existing, and controls character assignment for Atlas DAoC accounts.
			ChatUtil.SendTypeMessage("cmdDesc", client, attrib[0].Description, null);
			// Line break
			ChatUtil.SendTypeMessage("command", client, " ");
			// Message: Use the following syntax for this command:
			ChatUtil.SendTypeMessage("command", client, "AllCommands.Text.General.UseSyntax", null);
			...
			foreach (var sentence in attrib[0].Usage)
			{
				...
				if (sentence.Contains(".Syntax."))
				{
					...
					ChatUtil.SendTypeMessage("cmdSyntax", client, sentence, null);
				}
				...
				else
				{
					...
					ChatUtil.SendTypeMessage("cmdUsage", client, sentence, null);
				}
			}
		}
		...
		/// <param name="crud">Specifies whether this command may be accomplished using the Atlas Admin tool (e.g., true)</param>
		...
		public void DisplayHeadSyntax(GameClient client, string command, string subcommand, string subcommand2, int plvl, bool crud, string syntaxID, string usageID)
		{
			if (client == null || !client.IsPlaying)
				return;
			...
			if (subcommand != "")
			{
				if (subcommand2 != "")
					...
					ChatUtil.SendTypeMessage("cmdHeader", client, "AllCommands.Header.General.2Subcommand", command, subVar, subVar2, plvlReq);
				else
					// Message: <----- '/{0}{1}' Subcommand {2}----->
					ChatUtil.SendTypeMessage("cmdHeader", client, "AllCommands.Header.General.Subcommand", command, subVar, plvlReq);
			}
			if (subcommand == "")
				// Message: <----- '/{0}' Command {2}----->
				ChatUtil.SendTypeMessage("cmdHeader", client, "AllCommands.Header.General.Commands", command, plvlReq);
			if (crud == false || client.Account.PrivLevel == 1)
				// Message: Use the following syntax for this command:
				ChatUtil.SendTypeMessage("command", client, "AllCommands.Text.General.UseSyntax", null);
			if (crud && client.Account.PrivLevel > 1)
				...
				ChatUtil.SendTypeMessage("command", client, "AllCommands.Text.General.SyntaxCRUD", null);
			// Example: /account command
			ChatUtil.SendTypeMessage("cmdSyntax", client, syntaxID, null);
			// Example: Provides additional information regarding the '/account' command type.
			ChatUtil.SendTypeMessage("cmdUsage", client, usageID, null);
		}
		...
		public void DisplayHeaderSyntax(GameClient client, string headerID)
		{
			...
			// Example: /account command
			ChatUtil.SendTypeMessage("cmdHeader", client, headerID, null);
		}
