...
public class DynCodeCommandHandler : AbstractCommandHandler, ICommandHandler
{
    ...
    public static void ExecuteCode(GameClient client, string methodBody)
    {
        ...
        if (errorMessages.Any())
        {
            if (client.Player != null)
            {
                client.Out.SendMessage(
                    LanguageMgr.GetTranslation(client.Account.Language, "AdminCommands.Code.ErrorCompiling"),
        ...
        try
        {
            methodinf.Invoke(null,
                new object[] {client.Player == null ? null : client.Player.TargetObject, client.Player});

            if (client.Player != null)
                client.Out.SendMessage(
                    LanguageMgr.GetTranslation(client.Account.Language, "AdminCommands.Code.CodeExecuted"),
    ...
    private static string GetCode(string methodBody)
    {
        ...
        text.Append(
            "	if (Client==null || Client.Player==null) Log.Debug(str);\n	else Client.Out.SendMessage(str, eChatType.CT_System, eChatLoc.CL_SystemWindow);\n}\n");
        text.Append(
            "public static void DynMethod(GameObject target, GamePlayer player) {\nif (player!=null) Client = player.Client;\n");
    ...
    public void OnCommand(GameClient client, string[] args)
    {
        ...
        var code = string.Join(" ", args, 1, args.Length - 1);
