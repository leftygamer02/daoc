			...
			switch (args[1].ToLower())
			{
                ...
                case "create":
                    {
                        ...
                        if (Regex.IsMatch(accountname, @"^[a-z0-9]*$"))
                        {
	                        ...
	                        if (accountname.Length < 4 || password.Length < 4)
	                        {
		                        ...
		                        ChatUtil.SendTypeMessage("error", client, "AdminCommands.Account.Err.InvalidNamePass", null);
	                            return;
	                        }
	                        ...
	                        if (account != null)
	                        {
		                        ...
		                        ChatUtil.SendTypeMessage("error", client, "AdminCommands.Account.Err.AlreadyRegistered", accountname);
	                            return;
	                        }
	                        ...
	                        ChatUtil.SendTypeMessage("success", client, "AdminCommands.Account.Msg.AccountCreated", account.Name);
                        }
                        else
                        {
	                        ...
	                        ChatUtil.SendTypeMessage("error", client, "AdminCommands.Account.Err.SpecialChars", null);
                        }
                    }
                    break;
                #endregion Create
				...
				case "changepassword":
					{
						...
						if (acc == null)
						{
							...
							ChatUtil.SendTypeMessage("error", client, "AdminCommands.Account.Err.AccountNotFound", accountname);
							return;
						}
						...
						if (newpass.Length < 4)
						{
							...
							ChatUtil.SendTypeMessage("error", client, "AdminCommands.Account.Err.PasswordChars", accountname);
							return;
						}
						...
						ChatUtil.SendTypeMessage("success", client, "AdminCommands.Account.Msg.PasswordChanged", accountname);
				...
				case "delete":
					{
						...
						if (acc == null)
						{
                            ...
                            ChatUtil.SendTypeMessage("error", client, "AllCommands.Command.Err.NoAccountExists", accountname);
							return;
						}
						...
						ChatUtil.SendTypeMessage("success", client, "AdminCommands.Account.Msg.AccountDeleted", acc.Name);
						return;
					}
				#endregion Delete
                ...
                case "deletecharacter":
                    {
                        ...
                        if (cha == null)
                        {
                            ...
                            ChatUtil.SendTypeMessage("error", client, "AllCommands.Command.Err.NoExistPlayer", charname);
                            return;
                        }
						...
						ChatUtil.SendTypeMessage("success", client, "AdminCommands.Account.Msg.CharacterDeleted", cha.Name);
                        return;
                    }
                ...
                case "movecharacter":
                    {
                        ...
                        {
	                        // Message: No character exists with the name '{0}'.
                            ChatUtil.SendTypeMessage("error", client, "AllCommands.Command.Err.NoExistPlayer", null);
                            return;
                        }
                        ...
                        switch ((eRealm)cha.Realm)
                        {
                            ...
                            default:
                            {
	                            ...
	                            ChatUtil.SendTypeMessage("error", client, "AdminCommands.Account.Err.CharNotFromValidRealm", cha.Name, cha.Realm);
                                return;
	                        }
                        }
                        ...
                        if (freeslot == 0)
                        {
	                        ...
	                        ChatUtil.SendTypeMessage("error", client, "AdminCommands.Account.Err.NoFreeSlots", accountname);
                            return;
                        }
                        ...
                        ChatUtil.SendTypeMessage("success", client, "AdminCommands.Account.Msg.CharacterMovedToAccount", cha.Name, acc.Name);
                        return;
                    }
				...
				case "status":
					{
						...
						{
							// Message: No account exists with the name '{0}'.
							ChatUtil.SendTypeMessage("error", client, "AllCommands.Command.Err.NoExistPlayer", accountname);
							return;
						}

						int status;
                        // Exception: The value you entered was not expected. Please provide a number between '0' and '255'.
                        try { status=Convert.ToInt32(args[3]); } catch(Exception) { ChatUtil.SendTypeMessage("error", client, "AdminCommands.Account.Err.StatusValueReq", null); return; }
						...
						if(status >= 0 && status < 256 )
						{
							...
							ChatUtil.SendTypeMessage("success", client, "AdminCommands.Account.Msg.SetStatus", acc.Name, acc.Status, status);
						...
						else
						{
							...
							ChatUtil.SendTypeMessage("error", client, "AdminCommands.Account.Err.StatusValueReq", null);
						}
						return;
					}
				#endregion Status
				...
				case "unban":
					{
						...
						if (banacc.Count == 0)
						{
							...
							ChatUtil.SendTypeMessage("error", client, "AdminCommands.Account.Err.AccountBanNotFound", accountname);
							return;
						}
						...
						ChatUtil.SendTypeMessage("success", client, "AdminCommands.Account.Msg.AccountUnbanned", accountname);
						return;
					}
				#endregion Unban
                ...
                case "accountname":
                    {
                        ...
                        ChatUtil.SendTypeMessage("success", client, "AdminCommands.Account.Msg.AccNameForChar", Char.Name, accname);
                        return;
                    }
				...
				case "command":
				{
					ChatUtil.SendWindowMessage(client, "Using the '/account' Command", 
						...
						"Dialog.Header.Content.WebAdmin", 
						" ", 
						...
						"AdminCommands.Account.Comm.Desc2", 
						" ", 
						// Message: https://admin.atlasfreeshard.com
						"Hyperlinks.Atlas.WebAdminTool", 
						" ", 
						" ", 
