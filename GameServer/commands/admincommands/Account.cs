...
public class AccountCommand : AbstractCommandHandler, ICommandHandler
{
        ...
        switch (args[1].ToLower())
        {
            ...
            case "create":
            {
                ...
                if (Regex.IsMatch(accountname, @"^[a-z0-9]*$"))
                {
                    ...
                    if (account != null)
                    {
                        ...
                        ChatUtil.SendErrorMessage(client, "AdminCommands.Account.Err.AlreadyRegistered",
                            accountname);
                        return;
                    }
            ...
            case "movecharacter":
            {
                ...
                switch ((eRealm) cha.Realm)
                {
                    ...
                    default:
                    {
                        ...
                        ChatUtil.SendErrorMessage(client, "AdminCommands.Account.Err.CharNotFromValidRealm",
                            cha.Name, cha.Realm);
                        return;
                    }
                }
                ...
                var playingclient = WorldMgr.GetClientByPlayerName(cha.Name, true, false);
                ...
                ChatUtil.SendErrorMessage(client, "AdminCommands.Account.Msg.CharacterMovedToAccount", cha.Name,
                    acc.Name);
                return;
            }
            ...
            case "status":
            {
                ...
                if (status >= 0 && status < 256)
                {
                    ...
                    ChatUtil.SendErrorMessage(client, "AdminCommands.Account.Msg.SetStatus", acc.Name, acc.Status,
                        status);
            ...
            case "unban":
            {
                ...
                var banacc = DOLDB<DBBannedAccount>.SelectObjects(DB.Column("Type").IsEqualTo("A")
                    .Or(DB.Column("Type").IsEqualTo("B")).And(DB.Column("Account").IsEqualTo(accountname)));
            ...
            case "accountname":
            {
                ...
                var accname = GetAccountName(Char.Name);
    ...
    {
        var client = WorldMgr.GetClientByAccountName(name, true);
        if (client != null)
    ...
    {
        var client = WorldMgr.GetClientByPlayerName(charname, true, false);
        if (client != null)
    ...
    {
        var playingclient = WorldMgr.GetClientByAccountName(acc.Name, true);
        if (playingclient != null)
        {
            playingclient.Out.SendPlayerQuit(true);
            playingclient.Disconnect();
