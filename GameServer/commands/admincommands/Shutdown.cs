...
public class ShutdownCommandHandler : AbstractCommandHandler, ICommandHandler
{
    ...
    public static void AutomaticShutdown(object param)
    {
        ...
        var uptime = TimeSpan.FromMilliseconds(GameServer.Instance.TickCount);

        if (uptime.TotalHours >= Properties.HOURS_UPTIME_BETWEEN_SHUTDOWN &&
            DateTime.Now.Hour == AUTOMATEDSHUTDOWN_HOURTOSHUTDOWN)
        {
            ...
            foreach (var m_client in WorldMgr.GetAllPlayingClients())
                m_client.Out.SendDialogBox(eDialogCode.SimpleWarning, 0, 0, 0, 0, eDialogType.Ok, true,
                    "Automated server restart / backup triggered. Restart in " + m_counter / 60 +
                    " mins! (Restart at " + date.ToString("HH:mm \"GMT\" zzz") + ")");
            ...
            var msg = "Automated server restart in " + m_counter / 60 + " mins! (Restart at " +
                      date.ToString("HH:mm \"GMT\" zzz") + ")";
            log.Warn(msg);
        ...
        {
            log.Info("Uptime = " + uptime.TotalHours.ToString("N1") + ", restart uptime = " +
                     Properties.HOURS_UPTIME_BETWEEN_SHUTDOWN.ToString() +
                     " | Current hour = " + DateTime.Now.Hour.ToString() + ", restart hour = " +
                     AUTOMATEDSHUTDOWN_HOURTOSHUTDOWN.ToString());
        }
    }
    ...
    public static void CountDown(int seconds)
    {
        ...
        else
        {
            ...
            if (secs == 119 && GameServer.Instance.ServerStatus != eGameServerStatus.GSS_Closed &&
                Properties.DISCORD_ACTIVE &&
                !string.IsNullOrEmpty(Properties.DISCORD_WEBHOOK_ID)) // 2 mins remaining
            {
                ...
                var message = new DiscordMessage(
                    ...
                    new[]
                    {
                        new DiscordMessageEmbed(
                            color: 15158332,
                            description:
                            "The server will reboot in **2 minutes** and is temporarily not accepting new incoming connections!\n Stay tuned for the patch notes.",
                            thumbnail: new DiscordMessageEmbedThumbnail(
                                "https://cdn.discordapp.com/emojis/893545614942564412.webp")
                        )
                    }
                );
    ...
    public void OnCommand(GameClient client, string[] args)
    {
        ...
        switch (args[1])
        {
            ...
            case "stop":
            {
                ...
                if (m_counter != 0)
                {
                    ...
                    foreach (var playingClient in WorldMgr.GetAllPlayingClients())
                    {
                        // Message: "{0} stopped the server shutdown!"
                        ChatUtil.SendDebugMessage(playingClient, "AdminCommands.Shutdown.Msg.StaffCancel",
                            user.Name);
            ...
            case "on":
            {
                ...
                if (args.Length == 3)
                {
                    ...
                    var shutdownsplit = args[2].Split(':');

                    if (args[2].Contains(':') == false || shutdownsplit[0].Length is < 1 or > 2 ||
                        shutdownsplit[1].Length is < 2 or > 2)
                    {
                    ...
                    var hour = Convert.ToInt32(shutdownsplit[0]);
                    var min = Convert.ToInt32(shutdownsplit[1]);
            ...
            default:
            {
                ...
                if (args.Length == 2)
                {
                    ...
                    try
                    {
                        m_counter = Convert.ToInt32(args[1]);
                    }
        ...
        var msg = "AdminCommands.Shutdown.Msg.CountdownMins";
        var popup = m_counter / 60 < 60;
        var counter = m_counter / 60;
        ...
        foreach (var m_client in WorldMgr.GetAllPlayingClients())
        {
            if (popup)
            {
                ...
                shutdown.Add(" ");
                // Message: "----- ATTENTION -----"
                shutdown.Add(LanguageMgr.GetTranslation(client.Account.Language,
                ...
                shutdown.Add(LanguageMgr.GetTranslation(client.Account.Language,
                    "AdminCommands.Shutdown.Msg.ScheduledShutdown", date.ToString("HH:mm \"GMT\" zzz")));
                shutdown.Add(" ");
            ...
            ChatUtil.SendServerMessage(m_client, "AdminCommands.Shutdown.Msg.AttentionShutdown", m_counter / 60,
                date.ToString("HH:mm \"GMT\" zzz"));
        }
            ...
            var message = new DiscordMessage(
                ...
                new[]
                {
                    new DiscordMessageEmbed(
                        ...
                        thumbnail: new DiscordMessageEmbedThumbnail(
                            "https://cdn.discordapp.com/attachments/879754382231613451/959414859932532756/unknown.png")
                    )
                }
            );
