				...
				case "single":
				{
					switch (args.Length)
					{
						...
						case 3:
						{
							...
							ChatUtil.SendTypeMessage("error", client, "AllCommands.Command.Err.SpecifyName", null);
							return;
						}
						...
						case >= 4:
						{
							...
							if (targetClient == null)
							{
								...
								ChatUtil.SendTypeMessage("error", client, "AllCommands.Command.Err.NoOnlineChar", args[3]);
								return;
							}
							...
							ChatUtil.SendTypeMessage("success", client, "AdminCommands.Plvl.Msg.AddSinglePerm", target.Name, args[2]);

							if (target != client.Player)
							{
								// Message: {0} has given you access to the '/{1}' command! Type '/{1}' to view the command list.
								ChatUtil.SendTypeMessage("staff", target.Client, "AdminCommands.Plvl.Msg.GaveSinglePerm", client.Player.Name, args[2]);
							}
							return;
						}
					}
				...
				case "singleaccount":
				{
					switch (args.Length)
					{
						...
						case >= 4:
						{
							...
							{
								// Message: No character is online with the name '{0}'.
								ChatUtil.SendTypeMessage("error", client, "AllCommands.Command.Err.NoOnlineChar", null);
								return;
							}
							...
							ChatUtil.SendTypeMessage("success", client, "AdminCommands.Plvl.Msg.AddAcct", target.Name, args[2]);

							// Sends message to target
							if (target != client.Player)
							{
								// Message: {0} has given your account access to the '/{1}' command! Type '/{1}' to view the command list.
								ChatUtil.SendTypeMessage("staff", target.Client, "AdminCommands.Plvl.Msg.GaveAcct", client.Player.Name, args[2]);
							}
							return;
						}
					}
					...
					switch (args.Length)
					{
						...
						case >= 4:
						{
							...
							else
							{
								...
								ChatUtil.SendTypeMessage("error", client, "AdminCommands.Plvl.Err.NoPermFound",
									target.Name, args[2]);
								return;
							}

							// Message: You have revoked {0}'s access to the '/{1}' command!
							ChatUtil.SendTypeMessage("success", client, "AdminCommands.Plvl.Msg.RevokeSinglePerm", target.Name, args[2]);
							...
							if (target != client.Player)
							{
								...
								ChatUtil.SendTypeMessage("staff", target, "AdminCommands.Plvl.Msg.DelSinglePerm", client.Player.Name, args[2]);
								return;
							}
						}
						return;
	                ...
	                switch (args.Length)
					{
						...
						case >= 4:
						{
							...
							else
							{
								...
								ChatUtil.SendTypeMessage("error", client, "AdminCommands.Plvl.Err.NoAcctPermFound", target.Name, args[2]);
								return;
							}

							// Message: You have revoked {0}'s account access to the '/{1}' command!
							ChatUtil.SendTypeMessage("success", client, "AdminCommands.Plvl.Msg.RevokeAcctPerm", target.Name, args[2]);
							...
							if (target != client.Player)
							{
								...
								ChatUtil.SendTypeMessage("staff", target.Client, "AdminCommands.Plvl.Msg.DelAcctPerm", client.Player.Name, args[2]);
								return;
							}
						}
						return;
				...
				default:
					{
						...
						if (args.Length > 2)
						{
							...
							{
								// Message: No character is online with the name '{0}'.
								ChatUtil.SendTypeMessage("error", client, "AllCommands.Command.Err.NoOnlineChar", args[2]);
								return;
							}
						...
						if (args[1] == "1" || args[1] == "2" && client.Player == target && target == null)
						{
							...
							if (SinglePermission.HasPermission(client.Player, "plvl") == false)
							{
								...
								ChatUtil.SendTypeMessage("dialogWarn", client, "AdminCommands.Plvl.Err.NoPlvlPerm", null);
								return;
							}
						}
						...
						if (target != null)
						{
							...
							ChatUtil.SendTypeMessage("success", client, "AdminCommands.Plvl.Msg.PlvlSet", target.Name, plvl.ToString());

							if (target != client.Player)
								// Message: {0} has changed your account's privilege level to {1}!
								ChatUtil.SendTypeMessage("staff", target.Client, "AdminCommands.Plvl.Msg.YourPlvlSet", client.Player.Name, plvl.ToString());
						}
						break;
					}
				#endregion Plvl
