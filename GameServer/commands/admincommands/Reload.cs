...
public class ReloadCommandHandler : ICommandHandler
{
    ...
    private static void SendSystemMessageMob(GameClient client)
    {
        if (client.Player != null)
        {
            client.Out.SendMessage(" /reload mob ' reload all mob in region.", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            client.Out.SendMessage(
                " /reload mob ' realm <0/1/2/3>' reload all mob with specifique realm in region.",
                eChatType.CT_System, eChatLoc.CL_SystemWindow);
            client.Out.SendMessage(
                " /reload mob ' name <name_you_want>' reload all mob with specifique name in region.",
                eChatType.CT_System, eChatLoc.CL_SystemWindow);
            client.Out.SendMessage(
                " /reload mob ' model <model_ID>' reload all mob with specifique model in region.",
                eChatType.CT_System, eChatLoc.CL_SystemWindow);
        }
    }
    ...
    private static void SendSystemMessageObject(GameClient client)
    {
        if (client.Player != null)
        {
            client.Out.SendMessage(" /reload object ' reload all static object in region.", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            client.Out.SendMessage(
                " /reload object ' realm <0/1/2/3>' reload all static object with specifique realm in region.",
                eChatType.CT_System, eChatLoc.CL_SystemWindow);
            client.Out.SendMessage(
                " /reload object ' name <name_you_want>' reload all static object with specifique name in region.",
                eChatType.CT_System, eChatLoc.CL_SystemWindow);
            client.Out.SendMessage(
                " /reload object ' model <model_ID>' reload all static object with specifique model in region.",
                eChatType.CT_System, eChatLoc.CL_SystemWindow);
        }
    }
    ...
    private static void SendSystemMessageRealm(GameClient client)
    {
        ...
        {
            client.Out.SendMessage(
                "\n /reload <object/mob> realm <0/1/2/3>' reload all element with specifique realm in region.",
                eChatType.CT_System, eChatLoc.CL_SystemWindow);
            client.Out.SendMessage(" can use 0/1/2/3 or n/a/m/h or no/alb/mid/hib....", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
        }
    }
    ...
    private static void SendSystemMessageName(GameClient client)
    {
        if (client.Player != null)
            client.Out.SendMessage(
                "\n /reload <object/mob>  name <name_you_want>' reload all element with specified name in region.",
                eChatType.CT_System, eChatLoc.CL_SystemWindow);
    }
    ...
    private static void SendSystemMessageModel(GameClient client)
    {
        if (client.Player != null)
            client.Out.SendMessage(
                "\n /reload <object/mob>  model <model_ID>' reload all element with specified model_ID in region.",
                eChatType.CT_System, eChatLoc.CL_SystemWindow);
    }
    ...
    public void OnCommand(GameClient client, string[] args)
    {
        ...
        if (argLength < 1)
        {
            if (client.Player != null)
            {
                ...
                client.Out.SendMessage(" /reload specs - reload all specializations.", eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);
                client.Out.SendMessage(
                    " /reload spells - reload a spells and spelllines, checking db for changed and new spells.",
                    eChatType.CT_System, eChatLoc.CL_SystemWindow);
                client.Out.SendMessage(" /reload teleports - reload all teleport locations", eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);
                client.Out.SendMessage(" /reload npctemplates - reload all NPCTemplates", eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);
            }
        ...
        else if (argLength > 1)
        {
            if (args[2] == "realm" || args[2] == "Realm")
            {
                ...
                else if (args[3] == "1" || args[3] == "a" || args[3] == "alb" || args[3] == "Alb" ||
                         args[3] == "albion" || args[3] == "Albion")
                {
                    arg = "Albion";
                }
                else if (args[3] == "2" || args[3] == "m" || args[3] == "mid" || args[3] == "Mid" ||
                         args[3] == "midgard" || args[3] == "Midgard")
                {
                    arg = "Midgard";
                }
                else if (args[3] == "3" || args[3] == "h" || args[3] == "hib" || args[3] == "Hib" ||
                         args[3] == "hibernia" || args[3] == "Hibernia")
                {
                    arg = "Hibernia";
                }
                else
        ...
        if (args[1] == "mob" || args[1] == "Mob")
        {
            ...
            if (argLength > 1) ReloadMobs(client.Player, region, args[2], arg);
        }
        ...
        if (args[1] == "object" || args[1] == "Object")
        {
            ...
            if (argLength > 1) ReloadStaticItem(region, args[2], arg);
        }

        if (args[1].ToLower() == "spells")
        {
            SkillBase.ReloadDBSpells();
            var loaded = SkillBase.ReloadSpellLines();
            if (client != null)
                ChatUtil.SendSystemMessage(client,
                    string.Format("Reloaded db spells and {0} spells for all lines !", loaded));
        ...
        {
            var count = SkillBase.LoadSpecializations();
            if (client != null)
                client.Out.SendMessage(string.Format("{0} specializations loaded.", count), eChatType.CT_Important,
                    eChatLoc.CL_SystemWindow);
        ...
        if (args[1].ToLower() == "teleports")
        {
            ...
            if (client != null)
                client.Out.SendMessage("Teleport locations reloaded.", eChatType.CT_Important,
                    eChatLoc.CL_SystemWindow);
    ...
    private void ReloadMobs(GamePlayer player, ushort region, string arg1, string arg2)
    {
        ...
        foreach (var mob in WorldMgr.GetNPCsFromRegion(region))
            if (!mob.LoadedFromScript)
            {
                if (arg1 == "all")
                {
                    ...
                    var mobs = GameServer.Database.FindObjectByKey<Mob>(mob.InternalID);
                    if (mobs != null)
                    {
                ...
                if (arg1 == "model")
                    if (mob.Model == Convert.ToUInt16(arg2))
                    {
                        ...
                        var mobs = GameServer.Database.FindObjectByKey<WorldObject>(mob.InternalID);
                        if (mobs != null)
                        {
                            mob.LoadFromDatabase(mobs);
                            mob.AddToWorld();
    ...
    private void ReloadStaticItem(ushort region, string arg1, string arg2)
    {
        ...
        foreach (var staticItem in WorldMgr.GetStaticItemFromRegion(region))
            if (!staticItem.LoadedFromScript)
            {
                if (arg1 == "all")
                {
                    ...
                    var obj = GameServer.Database.FindObjectByKey<WorldObject>(staticItem.InternalID);
                    if (obj != null)
                    {
