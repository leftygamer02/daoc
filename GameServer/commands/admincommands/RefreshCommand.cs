...
{
    private static readonly Dictionary<string, MethodInfo> m_refreshCommandCache = new();
    ...
    public void OnCommand(GameClient client, string[] args)
    {
        ...
        // Join args
        var arg = string.Join(" ", args.Skip(1)).Trim();
        ...
        else
        {
            ...
            else
            {
                ...
                try
                {
                    var value = method.Value.Invoke(null, new object[] { });
                    if (value != null)
                ...
                catch (Exception e)
                {
                    DisplayMessage(client,
                        string.Format("-!- Error while refreshing Module's static cache for: {0} - {1}", method.Key,
                            e));
                }

                DisplayMessage(client,
                    string.Format("-.- Refresh Module's static cache Finished for: {0}", method.Key));
            }
        }
    }
    ...
    public static void InitRefreshCmdCache()
    {
        ...
        foreach (var method in type.GetMethods())
        {
            ...
            var attribs = method.GetCustomAttributes(typeof(RefreshCommandAttribute), false);
            if (attribs.Length == 0)
                continue;
