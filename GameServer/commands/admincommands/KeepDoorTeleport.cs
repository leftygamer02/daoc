...
public class KeepDoorTeleportCommandHandler : AbstractCommandHandler, ICommandHandler
{
    ...
    public void OnCommand(GameClient client, string[] args)
    {
        ...
        switch (args[1].ToLower())
        {
            case "add":
            {
                ...
                if (npcString == "")
                {
                    client.Out.SendMessage("You must specify a teleport string to whisper the npc.",
                ...
                {
                    client.Out.SendMessage("Valid strings are \"enter\" and \"exit\"", eChatType.CT_System,
                ...
                if (args[3] == "")
                {
                    client.Out.SendMessage("You must specify the teleport type", eChatType.CT_System,
                        eChatLoc.CL_SystemWindow);
                    return;
                }
                ...
                {
                    client.Out.SendMessage("Valid types are \"in\" and \"out\"", eChatType.CT_System,
                        eChatLoc.CL_SystemWindow);
                    return;
                }
                ...
                var keep = GameServer.KeepManager.GetKeepCloseToSpot(client.Player.CurrentRegionID, client.Player,
                    WorldMgr.VISIBILITY_DISTANCE);
                if (keep == null)
                {
                    client.Out.SendMessage("You need to be inside a keep area.", eChatType.CT_System,
                        eChatLoc.CL_SystemWindow);
                    return;
                }
    ...
    private void AddTeleport(GameClient client, string Text, AbstractGameKeep keep, string teleportType)
    {
        ...
        var verification = GameServer.Database.SelectObject<DBKeepDoorTeleport>(DB.Column("KeepID")
            .IsEqualTo(keep.KeepID)
            .And(DB.Column("Text").IsEqualTo(Text).And(DB.Column("Type").IsEqualTo(teleportType))));
        if (verification != null)
        {
            client.Out.SendMessage(string.Format("Teleport ID with same parameter already exists!"),
                eChatType.CT_System, eChatLoc.CL_SystemWindow);
            return;
        }

        var teleport = new DBKeepDoorTeleport();
        teleport.Text = Text;
