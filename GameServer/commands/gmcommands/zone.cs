...
public class ZoneCommandHandler : AbstractCommandHandler, ICommandHandler
{
    public void OnCommand(GameClient client, string[] args)
    {
        try
        {
            ...
            if (args[1].ToLower() == "info")
            {
                ...
                info.Add(" ");
                info.Add(string.Format(" Objects in zone: {0}, Total allowed for region: {1}",
                    client.Player.CurrentZone.TotalNumberOfObjects,
                    ServerProperties.Properties.REGION_MAX_OBJECTS));
                info.Add(" ");
                ...
                var dbZone = DOLDB<Zones>.SelectObject(DB.Column("ZoneID").IsEqualTo(zone.ID)
                    .And(DB.Column("RegionID").IsEqualTo(zone.ZoneRegion.ID)));

                if (dbZone != null)
                {
                    var dflag = "Use Region";
            ...
            {
                zone = WorldMgr.GetZone(client.Player.CurrentZone.ID);
                var divingFlag = Convert.ToByte(args[2]);
                if (divingFlag > 2)
                {
                ...
                DisplayMessage(client,
                    string.Format("Diving Flag for {0}:{1} changed to {2} ({3}).", zone.ID, zone.Description,
                        divingFlag, dflag));
                return;
            }
            ...
            {
                zone = WorldMgr.GetZone(client.Player.CurrentZone.ID);
                var waterlevel = Convert.ToInt32(args[2]);
                ...
                client.Player.MoveTo(client.Player.CurrentRegionID, client.Player.X, client.Player.Y,
                    client.Player.Z + 1, client.Player.Heading);

                DisplayMessage(client,
                    string.Format("Waterlevel for {0}:{1} changed to {2}.", zone.ID, zone.Description, waterlevel));
                return;
            }
            ...
            foreach (var c in string.Join(" ", args, 2, 4))
                if (char.IsLetter(c))
                {
                    DisplaySyntax(client);
                    return;
            ...
            if (args[6].ToLower().StartsWith("t"))
            {
                ...
                client.Player.Out.SendCustomDialog(
                    string.Format("Are you sure you wan't to over write {0} in the database?", zone.Description),
                    new CustomDialogResponse(AreYouSure));
            }
            else
            {
                client.Player.Out.SendCustomDialog(
                    string.Format(
                        "The zone settings for {0} will be reverted back to database settings on server restart.",
    ...
    public static void AreYouSure(GamePlayer player, byte response)
    {
        ...
        var zone = player.TempProperties.getProperty<Zone>("ZONE_BONUS_SAVE");

        if (response != 0x01)
        {
            player.Out.SendCustomDialog(
                string.Format("{0}'s bonuses will not be saved to the database!", zone.Description), null);
        ...
        player.Out.SendCustomDialog(
            string.Format(
                "{0}'s new zone bonuses have been updated to the database and changes have already taken effect!",
                zone.Description), null);
