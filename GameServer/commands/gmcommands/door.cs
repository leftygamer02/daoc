...
public class NewDoorCommandHandler : AbstractCommandHandler, ICommandHandler
{
    ...
    public void OnCommand(GameClient client, string[] args)
    {
        ...
        {
            client.Out.SendMessage("You can't add doors inside an instance.", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            return;
        }
    ...
    private void add(GameClient client, GameDoor targetDoor)
    {
        ...
        if (DOOR != null)
        {
            client.Out.SendMessage("The door is already in the database", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            return;
        }
    ...
    private void delete(GameClient client, GameDoor targetDoor)
    {
        ...
        if (DOOR == null)
        {
            client.Out.SendMessage("This door doesn't exist in the database", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            return;
        }
    }
    ...
    private void sound(GameClient client, GameDoor targetDoor, string[] args)
    {
        ...
        try
        {
            if (args.Length > 2)
            {
                ...
                targetDoor.SaveIntoDatabase();
                client.Out.SendMessage("You set the door sound to " + doorSound, eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);
            }
            else
            {
    ...
    private void info(GameClient client, GameDoor targetDoor)
    {
        ...
        var doorType = DoorRequestHandler.m_handlerDoorID / 100000000;
    ...
    private void locked(GameClient client, GameDoor targetDoor)
    {
        targetDoor.Locked = 1;
        targetDoor.SaveIntoDatabase();
        client.Out.SendMessage("Door " + targetDoor.Name + " is locked", eChatType.CT_System,
            eChatLoc.CL_SystemWindow);
    }
    ...
    private void unlocked(GameClient client, GameDoor targetDoor)
    {
        targetDoor.Locked = 0;
        targetDoor.SaveIntoDatabase();
        client.Out.SendMessage("Door " + targetDoor.Name + " is unlocked", eChatType.CT_System,
            eChatLoc.CL_SystemWindow);
    }
