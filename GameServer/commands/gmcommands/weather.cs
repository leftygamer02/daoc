        ...
        if (args.Length >= 2)
        {
            ...
            switch (action)
            {
                case "info":
                    break;
                case "restart":
                    if (GameServer.Instance.WorldManager.WeatherManager.RestartWeather(
                ...
                case "start":
                    if (args.Length > 2)
                    {
                        try
                        {
                            var position = Convert.ToUInt32(args[2]);
                            var width = Convert.ToUInt32(args[3]);
                            var speed = Convert.ToUInt16(args[4]);
                            var diffusion = Convert.ToUInt16(args[5]);
                            var intensity = Convert.ToUInt16(args[6]);
                            if (!GameServer.Instance.WorldManager.WeatherManager.StartWeather(
                                    client.Player.CurrentRegionID, position, width, speed, diffusion, intensity))
                            {
                                DisplayMessage(client,
                                    "Weather (start): Weather could not be started in this Region!");
                                break;
                            }
                        }
                        catch
                    ...
                    {
                        if (!GameServer.Instance.WorldManager.WeatherManager.StartWeather(client.Player
                                .CurrentRegionID))
                        {
    ...
    public void PrintInfo(GameClient client)
    {
        ...
        else
        {
            if (weather.StartTime == 0)
                ...
                DisplayMessage(client, "Weather (info): Current Position - {0} - {1}",
                    weather.CurrentPosition(Scheduler.SimpleScheduler.Ticks), weather);
        }
    }
}