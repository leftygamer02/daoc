        ...
        try
        {
            switch (args[1].ToLower())
            {
                ...
                case "blank":
                {
                    ...
                    var item = new GameInventoryItem(newTemplate);
                    if (client.Player.Inventory.AddItem(eInventorySlot.FirstEmptyBackpack, item))
                    {
                        client.Out.SendMessage(
                            LanguageMgr.GetTranslation(client.Account.Language,
                                "GMCommands.Item.Blank.ItemCreated"), eChatType.CT_System,
                            eChatLoc.CL_SystemWindow);
                        InventoryLogging.LogInventoryAction(client.Player, client.Player,
                    ...
                    else
                    {
                        client.Out.SendMessage(
                            LanguageMgr.GetTranslation(client.Account.Language,
                                "GMCommands.Item.Blank.CreationError"), eChatType.CT_System,
                            eChatLoc.CL_SystemWindow);
                    }
                ...
                case "classes":
                {
                    var slot = (int) eInventorySlot.LastBackpack;

                    if (args.Length >= 4) slot = Convert.ToInt32(args[3]);

                    var item = client.Player.Inventory.GetItem((eInventorySlot) slot);

                    if (item == null)
                    {
                        client.Out.SendMessage(
                            LanguageMgr.GetTranslation(client.Account.Language,
                                "GMCommands.Item.Count.NoItemInSlot", slot), eChatType.CT_System,
                            eChatLoc.CL_SystemWindow);
                        return;
                    }
                ...
                case "create":
                {
                    var template = GameServer.Database.FindObjectByKey<ItemTemplate>(args[2]);
                    if (template == null)
                    {
                        client.Out.SendMessage(
                            LanguageMgr.GetTranslation(client.Account.Language, "GMCommands.Item.Create.NotFound",
                                args[2]), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                        return;
                    }
                    else
                    {
                        ...
                        if (client.Player.Inventory.AddItem(eInventorySlot.FirstEmptyBackpack, item))
                        {
                            client.Out.SendMessage(
                                LanguageMgr.GetTranslation(client.Account.Language,
                                    "GMCommands.Item.Create.Created", item.Level, item.GetName(0, false), count),
                ...
                case "count":
                {
                    ...
                    if (!item.IsStackable)
                    {
                        client.Out.SendMessage(
                            LanguageMgr.GetTranslation(client.Account.Language,
                                "GMCommands.Item.Count.NotStackable", item.GetName(0, true)), eChatType.CT_System,
                            eChatLoc.CL_SystemWindow);
                        return;
                    }
                ...
                case "info":
                {
                    var obj = GameServer.Database.FindObjectByKey<ItemTemplate>(args[2]);
                    if (obj == null)
                    {
                        client.Out.SendMessage(
                            LanguageMgr.GetTranslation(client.Account.Language,
                                "GMCommands.Item.Info.ItemTemplateUnknown", args[2]), eChatType.CT_System,
                            eChatLoc.CL_SystemWindow);
                        return;
                    }

                    var invItem = GameInventoryItem.Create(obj);
                    var objectInfo = new List<string>();
                    invItem.WriteTechnicalInfo(objectInfo, client);
                    client.Out.SendCustomTextWindow(
                        LanguageMgr.GetTranslation(client.Account.Language, "GMCommands.Item.Info.Informations",
                            obj.Id_nb), objectInfo);
                    break;
                }
                ...
                case "extension":
                {
                    ...
                    if (item.Template is ItemUnique ||
                        (item.Template is ItemTemplate && (item.Template as ItemTemplate).AllowUpdate))
                ...
                case "price":
                {
                    ...
                    item.Price = Money.GetMoney(0, (int) (Convert.ToInt16(args[2]) % 1000),
                        (int) (Convert.ToInt16(args[3]) % 1000), (int) (Convert.ToByte(args[4]) % 100),
                        (int) (Convert.ToByte(args[5]) % 100));
                    client.Out.SendInventoryItemsUpdate(new InventoryItem[] {item});
                    break;
                }
                ...
                case "condition":
                {
                    ...
                    var con = Convert.ToInt32(args[2]);
                    var maxcon = Convert.ToInt32(args[3]);
                    item.Condition = con;
                ...
                case "durability":
                {
                    ...
                    UpdateAllowed(item, client);
                    var Dur = Convert.ToInt32(args[2]);
                    var MaxDur = Convert.ToInt32(args[3]);
                    item.Durability = Dur;
                    if (item.Template is ItemUnique ||
                        (item.Template is ItemTemplate && (item.Template as ItemTemplate).AllowUpdate))
                ...
                case "quality":
                {
                    ...
                    var Qua = Convert.ToInt32(args[2]);
                    item.Quality = Qua;
                    client.Out.SendInventoryItemsUpdate(new InventoryItem[] {item});
                    break;
                }
                ...
                case "bonus":
                {
                    ...
                    var Bonus = Convert.ToInt32(args[2]);
                    item.Bonus = Bonus;
                    client.Out.SendInventoryItemsUpdate(new InventoryItem[] {item});
                    break;
                }
                ...
                case "mbonus":
                {
                    ...
                    catch
                    {
                        client.Out.SendMessage(
                            LanguageMgr.GetTranslation(client.Account.Language,
                                "GMCommands.Item.mBonus.NonSetBonusNumber"), eChatType.CT_System,
                            eChatLoc.CL_SystemWindow);
                    }
                    ...
                    try
                    {
                        ...
                        if (bonusType < 0 || bonusType >= (int) eProperty.MaxProperty)
                        {
                            client.Out.SendMessage(
                                LanguageMgr.GetTranslation(client.Account.Language,
                                    "GMCommands.Item.mBonus.TypeShouldBeInRange",
                                    (int) (eProperty.MaxProperty - 1)), eChatType.CT_System,
                                eChatLoc.CL_SystemWindow);
                            break;
                        }
                    }
                    catch
                    {
                        client.Out.SendMessage(
                            LanguageMgr.GetTranslation(client.Account.Language,
                                "GMCommands.Item.mBonus.NonSetBonusType"), eChatType.CT_System,
                            eChatLoc.CL_SystemWindow);
                    }
                    ...
                    try
                    {
                        ...
                        switch (num)
                        {
                            ...
                            default:
                                client.Out.SendMessage(
                                    LanguageMgr.GetTranslation(client.Account.Language,
                                        "GMCommands.Item.mBonus.UnknownBonusNumber", num), eChatType.CT_System,
                                    eChatLoc.CL_SystemWindow);
                                return;
                        }
                    ...
                    catch
                    {
                        client.Out.SendMessage(
                            LanguageMgr.GetTranslation(client.Account.Language,
                                "GMCommands.Item.mBonus.NotSetBonusValue"), eChatType.CT_System,
                            eChatLoc.CL_SystemWindow);
                    }
                ...
                case "spell":
                {
                    ...
                    var Charges = Convert.ToInt32(args[2]);
                    var MaxCharges = Convert.ToInt32(args[3]);
                ...
                case "spell1":
                {
                    ...
                    var Charges = Convert.ToInt32(args[2]);
                    var MaxCharges = Convert.ToInt32(args[3]);
                    var SpellID1 = Convert.ToInt32(args[4]);
                ...
                case "levelrequired":
                {
                    ...
                    var setting = Convert.ToInt32(args[2]);
                ...
                case "flags":
                {
                    var flags = Convert.ToInt32(args[2]);
                    var slot = (int) eInventorySlot.LastBackpack;
                ...
                case "salvageid":
                {
                    var salvageID = Convert.ToInt32(args[2]);
                    var slot = (int) eInventorySlot.LastBackpack;
                ...
                case "salvageinfo":
                {
                    ...
                    if (args.Length == 3) slot = Convert.ToInt32(args[2]);
                    ...
                    var salvageLevel = CraftingMgr.GetItemCraftLevel(item) / 100;
                    ...
                    if (ServerProperties.Properties.USE_SALVAGE_PER_REALM)
                        whereClause = whereClause.And(DB.Column("Realm").IsEqualTo((int) eRealm.None)
                    ...
                    if (salvageYield != null) yield = salvageYield.Clone() as SalvageYield;
                    ...
                    var material = GameServer.Database.FindObjectByKey<ItemTemplate>(yield.MaterialId_nb);
                    var materialName = yield.MaterialId_nb;
                    ...
                    if (calculated == false)
                    {
                        if (yield != null)
                        {
                            ...
                            list.Add("          Realm: " + (yield.Realm == 0
                                ? "Any"
                                : GlobalConstants.RealmToName((eRealm) yield.Realm)));
                    ...
                    else
                    {
                        ...
                        list.Add("       Material: " + materialName);
                        list.Add("          Count: " +
                                 Salvage.GetMaterialYield(client.Player, item, yield, material));
                        list.Add("          Realm: " +
                                 (yield.Realm == 0 ? "Any" : GlobalConstants.RealmToName((eRealm) yield.Realm)));
                        list.Add("      PackageID: " + yield.PackageID);
                    }
                ...
                case "saveunique":
                {
                    ...
                    if (args.Length >= 4)
                    {
                        ...
                        if (slot > (int) eInventorySlot.LastBackpack) slot = (int) eInventorySlot.LastBackpack;
                    ...
                    if (item.Template is ItemUnique)
                    {
                        var itemUnique = item.Template as ItemUnique;
                        Log.Debug("update ItemUnique " + item.Template.Id_nb);
                        GameServer.Database.SaveObject(itemUnique);
                        client.Out.SendMessage(string.Format("ItemUnique {0} updated!", itemUnique.Id_nb),
                            eChatType.CT_System, eChatLoc.CL_SystemWindow);
                    }
                    else
                    {
                        DisplayMessage(client,
                            "This is not an ItemUnique.  To create a new ItemUnique use addunique");
                        return;
                    }
                }
                    break;
                ...
                case "addunique":
                {
                    ...
                    else if (idnb == string.Empty)
                    {
                        ...
                        {
                            DisplayMessage(client,
                                "You need to provide a new id_nb to save this ItemUnique as an ItemTemplate.  Use saveunique to save this ItemUnique.");
                            return;
                        }
                    ...
                    if (temp == null)
                    {
                        if (args[1].ToLower() == "save")
                            try
                            {
                                ...
                                var itemTemplate = new ItemTemplate(item.Template)
                                {
                                    Id_nb = idnb
                                };
                                ...
                                var newItem = GameInventoryItem.Create(itemTemplate);
                                if (client.Player.Inventory.AddItem((eInventorySlot) slot, newItem))
                                    InventoryLogging.LogInventoryAction(client.Player, client.Player,
                                        eInventoryActionType.Other, newItem.Template, newItem.Count);
                            }
                            catch (Exception ex)
                            {
                            ...
                            try
                            {
                                ...
                                DisplayMessage(client,
                                    "Added New ItemUnique: " + unique.Id_nb + " (" + unique.ObjectId + ")");
                                var newItem = GameInventoryItem.Create(unique);
                                if (client.Player.Inventory.AddItem((eInventorySlot) slot, newItem))
                                    InventoryLogging.LogInventoryAction(client.Player, client.Player,
                                        eInventoryActionType.Other, newItem.Template, newItem.Count);
                            }
                ...
                case "findid":
                {
                    ...
                    if (name != "")
                    {
                        ...
                        DisplayMessage(client,
                            LanguageMgr.GetTranslation(client.Account.Language,
                                "GMCommands.Item.FindID.MatchingIDsForX", name, items.Count), new object[] { });
                ...
                case "findname":
                {
                    ...
                    if (name != "")
                    {
                        ...
                        DisplayMessage(client,
                            LanguageMgr.GetTranslation(client.Account.Language,
                                "GMCommands.Item.FindName.MatchingNamesForX", name, items.Count), new object[] { });
                        foreach (var item in items)
                ...
                case "loadpackage":
                {
                    if (args[2] != "")
                    {
                        ...
                        var packageItems =
                            DOLDB<ItemTemplate>.SelectObjects(DB.Column("PackageID").IsEqualTo(args[2]));
    ...
    private void UpdateAllowed(InventoryItem item, GameClient client)
    {
        ...
        else
        {
            ...
            client.Out.SendMessage(
                ...
                item.Template.Id_nb, eChatType.CT_Staff, eChatLoc.CL_SystemWindow);
            DisplayMessage(client,
                "** When this item is saved all changes will also be made to the source ItemTemplate: " +
                item.Template.Id_nb);
        }
