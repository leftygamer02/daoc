...
public class GMAppealCommandHandler : AbstractCommandHandler, ICommandHandler
{
    public void OnCommand(GameClient client, string[] args)
    {
        ...
        {
            AppealMgr.MessageToClient(client,
                LanguageMgr.GetTranslation(client.Account.Language, "Scripts.Players.Appeal.SystemDisabled"));
            return;
        }
        ...
        switch (args[1])
        {
            ...
            case "assist":
            {
                ...
                var targetClient =
                    WorldMgr.GuessClientByPlayerNameAndRealm(targetName, 0, false, out result);
                switch (result)
                {
                    case 2: // name not unique
                        AppealMgr.MessageToClient(client,
                            LanguageMgr.GetTranslation(client.Account.Language,
                                "Scripts.Players.Appeal.NameNotUnique"));
                        return;
                    case 3: // exact match
                    case 4: // guessed name
                        break;
                ...
                if (targetClient == null)
                {
                    // nothing found
                    AppealMgr.MessageToClient(client,
                        LanguageMgr.GetTranslation(client.Account.Language, "Scripts.Players.Appeal.PlayerNotFound",
                            targetName));
                    return;
                }
                ...
                if (appeal != null)
                {
                    if (appeal.Status != "Being Helped")
                    {
                        ...
                        var message = LanguageMgr.GetTranslation(client.Account.Language,
                            "Scripts.Players.Appeal.RandMessage" + Util.Random(4), targetClient.Player.Name);
                    ...
                    else
                    {
                        AppealMgr.MessageToClient(client,
                            LanguageMgr.GetTranslation(client.Account.Language,
                                "Scripts.Players.Appeal.BeingHelped"));
                        break;
                    }
                }

                AppealMgr.MessageToClient(client,
                    LanguageMgr.GetTranslation(client.Account.Language, "Scripts.Players.Appeal.DoesntHaveAppeal"));
                break;
            }
            ...
            case "view":
            {
                ...
                if (appeal != null)
                {
                    ...
                    msg.Add("[Appeal]: " + appeal.Name + ", [Status]: " + appeal.Status + ", [Priority]: " +
                            appeal.SeverityToName + " [Issue]: " + appeal.Text + ", [Time]: " + appeal.Timestamp +
                            ".\n");
            ...
            case "release":
            {
                ...
                if (appeal != null)
                {
                    ...
                    else
                    {
                        AppealMgr.MessageToClient(client,
                            LanguageMgr.GetTranslation(client.Account.Language,
                                "Scripts.Players.Appeal.NotBeingHelped"));
                        return;
                    }
                }
            ...
            case "listall":
            {
                ...
                if (appeallist.Count < 1 || appeallist == null)
                {
                    AppealMgr.MessageToClient(client,
                        LanguageMgr.GetTranslation(client.Account.Language,
                            "Scripts.Players.Appeal.NoAppealsinQueue"));
                    return;
                }
                ...
                var total = appeallist.Count;
                msg.Add(LanguageMgr.GetTranslation(client.Account.Language,
                    "Scripts.Players.Appeal.CurrentStaffAvailable", AppealMgr.StaffList.Count, total) + "\n");
                ...
                if (crit > 0)
                {
                    ...
                    foreach (var a in appeallist)
                        if (a.Severity == (int) AppealMgr.eSeverity.Critical)
                            msg.Add("[Name]: " + a.Name + ", [Status]: " + a.Status + ", [Priority]: " +
                                    a.SeverityToName + " [Issue]: " + a.Text + ", [Time]: " + a.Timestamp + ".\n");
                }
            ...
            case "close":
            {
                ...
                if (appeal == null)
                {
                    AppealMgr.MessageToClient(client,
                        LanguageMgr.GetTranslation(client.Account.Language,
                            "Scripts.Players.Appeal.DoesntHaveAppeal"));
                    return;
                }
            ...
            case "closeoffline":
            {
                ...
                if (appeal == null)
                {
                    AppealMgr.MessageToClient(client,
                        LanguageMgr.GetTranslation(client.Account.Language,
                            "Scripts.Players.Appeal.CantFindAppeal"));
                    return;
                }
                ...
                var targetNameTwo = args[2];
                var resultTwo = 0;
                var targetClient =
                    WorldMgr.GuessClientByPlayerNameAndRealm(targetNameTwo, 0, false, out resultTwo);
                switch (resultTwo)
                {
                    case 2: // name not unique
                        AppealMgr.MessageToClient(client,
                ...
                else
                {
                    ...
                    AppealMgr.MessageToClient(targetClient,
                        LanguageMgr.GetTranslation(client.Account.Language,
                            "Scripts.Players.Appeal.StaffClosedYourAppeal", client.Player.Name));
            ...
            case "jumpto":
            {
                try
                {
                    var p = client.Player.TempProperties.getProperty<GamePlayer>("AppealAssist");
                    if (p.ObjectState == GameObject.eObjectState.Active)
                    {
                        var oldlocation = new GameLocation("old", client.Player.CurrentRegionID,
                ...
                catch
                {
                    AppealMgr.MessageToClient(client,
                        LanguageMgr.GetTranslation(client.Account.Language,
                            "Scripts.Players.Appeal.MustBeAssisting"));
                    break;
                }
            }
            case "jumpback":
            {
                var jumpback = client.Player.TempProperties.getProperty<GameLocation>("AppealJumpOld");
                ...
                else
                {
                    AppealMgr.MessageToClient(client,
                        LanguageMgr.GetTranslation(client.Account.Language,
                            "Scripts.Players.Appeal.NoLocationToJump"));
                }
            ...
            case "mute":
            {
                var mute = client.Player.TempProperties.getProperty<bool>("AppealMute");
                if (mute == false)
                {
                    client.Player.TempProperties.setProperty("AppealMute", true);
                    AppealMgr.MessageToClient(client,
                        LanguageMgr.GetTranslation(client.Account.Language,
                            "Scripts.Players.Appeal.NoLongerReceiveMsg"));
                    AppealMgr.MessageToClient(client,
                        LanguageMgr.GetTranslation(client.Account.Language,
                            "Scripts.Players.Appeal.UseCmdTurnBackOn"));
                ...
                else
                {
                    ...
                    AppealMgr.MessageToClient(client,
                        LanguageMgr.GetTranslation(client.Account.Language,
                            "Scripts.Players.Appeal.NowReceiveMsg"));
                    AppealMgr.MessageToClient(client,
                        LanguageMgr.GetTranslation(client.Account.Language,
                            "Scripts.Players.Appeal.UseCmdTurnBackOff"));
