...
public class BlinkCommandHandler : ICommandHandler
{
    public void OnCommand(GameClient client, string[] args)
    {
        ...
        if (args.Length > 1)
        {
            ...
            if (byte.TryParse(args[1].ToLower(), out value))
                ...
                if (Enum.IsDefined(typeof(ePanel), value))
                {
                    ...
                    client.Out.SendMessage("Start blinking UI part: " + Enum.GetName(typeof(ePanel), value),
                    ...
                    if (player.TargetObject != null && player.TargetObject is GamePlayer &&
                        (player.TargetObject as GamePlayer).Client.IsPlaying)
        ...
        if (sendBlinkPanel == false) Usage(client);
    }
    ...
    private void Usage(GameClient client)
    {
        ...
        foreach (var panelID in Enum.GetNames(typeof(ePanel)))
        {
