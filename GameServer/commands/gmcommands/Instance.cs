...
public class InstanceCommandHandler : AbstractCommandHandler, ICommandHandler
{
    ...
    public void OnCommand(GameClient client, string[] args)
    {
        ...
        switch (args[1].ToLower())
        {
            ...
            case "key":
                var newKey = string.Join(" ", args, 2, args.Length - 2);
            ...
            case "entry":
            {
                try
                {
                    ...
                    var element = new DBInstanceXElement();
                    ...
                    SendMessage(client,
                        "Created an element here! Use your memory for now, I sure as hell dont have anything else to show you where it is ^^");

                    //Only create ones that have namespaces (signified by '.')
                    if (theType.Contains("."))
                    {
                        SendMessage(client,
                            "theType suspected to be a ClassType - attempting to invoke a marker of this class.");
                        GameObject obj = null;
                        ...
                        if (args.Length == 4)
                        {
                            ...
                            if ((GameNPC) obj != null && templateID != 0)
                            {
                                ...
                                if (npcTemplate != null) ((GameNPC) obj).LoadTemplate(npcTemplate);
                            }
                        }
                        ...
                        if (!obj.AddToWorld())
                            client.Out.SendMessage("Error: Object not added to world correctly!",
                ...
                catch (Exception ex)
                {
                    client.Out.SendMessage(
                        "An Exception has occurred when trying to add object, review server error logs! Exception: " +
                        ex.Message, eChatType.CT_Important, eChatLoc.CL_SystemWindow);
            ...
            case "remove":
            {
                var obj = client.Player.TargetObject;
                if (obj == null)
                    return;
                var ObjectId = obj.Name + obj.GuildName;
                DataObject o = GameServer.Database.FindObjectByKey<DBInstanceXElement>(ObjectId);

                if (o == null)
                {
                    client.Out.SendMessage("Could not find the entry in the database! <key=" + ObjectId + ">",
            ...
            case "create":
            {
                ...
                try
                {
                    if (args.Length < 3)
                        throw new Exception(
                            "You need to provide a skin id.  A skin is the ID of the region you want this instance to look like.");

                    var newInstance = player.TempProperties.getProperty<object>(key, null) as Instance;

                    if (newInstance != null)
                        throw new Exception("You already have an instance '" + key +
                                            "' created, please close it before creating another.");

                    var skinID = Convert.ToUInt16(args[2]);
                    ...
                    {
                        SendMessage(client,
                            "Instance created, now loading elements for instance '" + key + "' from the DB.");
            ...
            case "test":
            {
                ...
                else
                {
                    ...
                    var saveLocation = new GameLocation(player.Name + "_exit", player.CurrentRegionID,
                    ...
                    {
                        SendMessage(client,
                            "MoveTo to entrance failed, now trying to move to current location inside the instance.");

                        if (!player.MoveTo(newInstance.ID, player.X, player.Y, player.Z, player.Heading))
                        {
                            SendMessage(client,
                                "That failed as well.  Either add an entrance to this instance or move in the world to a corresponding instance location.");
                            success = false;
                        }
                    }
                    ...
                    {
                        SendMessage(client,
                            "Welcome to Instance ID " + newInstance.ID + ", Skin: " + newInstance.Skin + ", with " +
                            newInstance.Zones.Count + " zones and " + newInstance.Objects.Length +
            ...
            case "exit":
            {
                ...
                var saveLocation =
                    player.TempProperties.getProperty<object>(player.Name + "_exit", null) as GameLocation;

                if (saveLocation == null)
                {
                    var sourceRegion = (player.CurrentRegion as BaseInstance).Skin;
                ...
                {
                    player.MoveTo(saveLocation.RegionID, saveLocation.X, saveLocation.Y, saveLocation.Z,
                        saveLocation.Heading);
                }
            }
                break;
