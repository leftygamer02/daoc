...
public class RestartCommandHandler : AbstractCommandHandler, ICommandHandler
{
    ...
    {
        client.Out.SendCustomDialog(string.Format("Do you wish to reboot the server instantly!!"),
            new CustomDialogResponse(RebootResponse));
    }
    ...
    public static void ShutDownServer()
    {
        if (GameServer.Instance.IsRunning)
        {
            ...
            Thread.Sleep(2000);
            Process.Start(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "DOLServer.exe"));
            Environment.Exit(0);
        }
    }
}