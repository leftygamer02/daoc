...
public class PathCommandHandler : AbstractCommandHandler, ICommandHandler
{
    ...
    private void PathCreate(GameClient client)
    {
        ...
        var startpoint =
            new PathPoint(client.Player.X, client.Player.Y, client.Player.Z, 5000, ePathType.Once);
        ...
        client.Player.Out.SendMessage("Path creation started! You can add new pathpoints via /path add now!",
    ...
    {
        var path = (PathPoint) client.Player.TempProperties.getProperty<object>(TEMP_PATH_LAST, null);
        if (path == null)
        {
        ...
        var newpp = new PathPoint(client.Player.X, client.Player.Y, client.Player.Z, speedlimit, path.Type);
    ...
    private void PathSpeed(GameClient client, string[] args)
    {
        ...
        var pathpoint = (PathPoint) client.Player.TempProperties.getProperty<object>(TEMP_PATH_FIRST, null);
    ...
    private void PathTravel(GameClient client)
    {
        ...
        var speed = Math.Min(((GameNPC) client.Player.TargetObject).MaxSpeedBase, (short) path.MaxSpeed);

        // clear any current path
        ((GameNPC) client.Player.TargetObject).CurrentWayPoint = null;

        // set the new path
        ((GameNPC) client.Player.TargetObject).CurrentWayPoint =
            (PathPoint) client.Player.TempProperties.getProperty<object>(TEMP_PATH_FIRST, null);
    ...
    private void PathLoad(GameClient client, string[] args)
    {
        ...
        var path = MovementMgr.LoadPath(pathname);
        if (path != null)
        {
            RemoveAllTempPathObjects(client);
    ...
    private void PathAssignTaxiRoute(GameClient client, string[] args)
    {
        ...
        var target = string.Join(" ", args, 2, args.Length - 2);
        ;
        var ticketFound = false;
        var ticket = "Ticket to " + target;
        // Most //
