...
public class KeepComponentCommandHandler : AbstractCommandHandler, ICommandHandler
{
    ...
    public void OnCommand(GameClient client, string[] args)
    {
        ...
        var myKeep = GameServer.KeepManager.GetKeepCloseToSpot(client.Player.CurrentRegionID,
            client.Player, WorldMgr.OBJ_UPDATE_DISTANCE);

        if (myKeep == null) DisplayMessage(client, "You are not near a keep.");
        ...
        switch (args[1])
        {
            ...
            case "create":
            {
                ...
                {
                    var i = 0;
                    foreach (var str in Enum.GetNames(typeof(GameKeepComponent.eComponentSkin)))
                    {
                ...
                var angle = myKeep.Heading * (Math.PI * 2 / 360); // angle*2pi/360;
                ...
                */
                ...
                client.Out.SendMessage(
                    LanguageMgr.GetTranslation(client.Account.Language,
                        "GMCommands.KeepComponents.Create.KCCreated"), eChatType.CT_System,
            ...
            case "move":
            {
                ...
                client.Out.SendMessage(
                    "Component moved.  Use /keepcomponent save to save, or reload to reload the original position.",
            ...
            case "rotate":
            {
                try
                {
                    var amount = Convert.ToUInt16(args[2]);
                    ...
                    client.Out.SendKeepComponentDetailUpdate(component);
                    client.Out.SendMessage(
                        "Component rotated.  Use /keepcomponent save to save, or reload to reload the original position.",
                        eChatType.CT_System, eChatLoc.CL_SystemWindow);
                }
                catch
                {
            ...
            case "skin":
            {
                ...
                component.Skin = skin;
                foreach (var cli in WorldMgr.GetClientsOfRegion(client.Player.CurrentRegionID))
                {
                ...
                client.Out.SendMessage(
                    "Component skin updated.  Use /keepcomponent save to save, or reload to reload the original skin.",
                    eChatType.CT_System, eChatLoc.CL_SystemWindow);
            }
                break;
            ...
            case "delete":
            {
                ...
                client.Out.SendMessage(
                    LanguageMgr.GetTranslation(client.Account.Language,
                        "GMCommands.KeepComponents.Delete.YDeleteKC"), eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);
            }
                break;
            ...
            case "reload":
            {
                ...
                var dbcomponent = DOLDB<DBKeepComponent>.SelectObject(DB.Column("KeepID")
                    .IsEqualTo(component.Keep.KeepID).And(DB.Column("ID").IsEqualTo(component.ID)));
    ...
    public int CalcCX(GamePlayer player, AbstractGameKeep myKeep, double angle)
    {
        if (Math.Abs(Math.Sin(angle)) < 0.0001) //for approximations, == 0 wont work.
            ...
            return (int) ((148 * Math.Sin(angle) * myKeep.X - 148 * Math.Sin(angle) * player.X + player.Y -
                           myKeep.Y)
                          / (148 * Math.Sin(angle) - 148 * 148 * 2 * Math.Sin(angle) * Math.Cos(angle)));
    }
    ...
    public int CalcCY(GamePlayer player, AbstractGameKeep myKeep, double angle)
    {
        ...
        {
            var cx = (int) ((148 * Math.Sin(angle) * myKeep.X - 148 * Math.Sin(angle) * player.X + player.Y -
                             myKeep.Y)
                            / (148 * Math.Sin(angle) - 148 * 148 * 2 * Math.Sin(angle) * Math.Cos(angle)));
