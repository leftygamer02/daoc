...
public class ObjectCommandHandler : AbstractCommandHandler, ICommandHandler
{
    public void OnCommand(GameClient client, string[] args)
    {
        ...
        var targetObject = client.Player.TargetObject as GameStaticItem;

        if (targetObject == null && args[1] != "create" && args[1] != "fastcreate" && args[1] != "target" &&
            args[1] != "quests")
        {
            client.Out.SendMessage("Type /object for command overview", eChatType.CT_System,
        ...
        switch (args[1])
        {
            case "info":
            {
                ...
                var invItem = targetObject as WorldInventoryItem;
            ...
            case "create":
            {
                var theType = "DOL.GS.GameStaticItem";
                if (args.Length > 2)
                    theType = args[2];

                var obj = CreateItem(client, theType);
            ...
            case "fastcreate":
            {
                ...
                var obj = CreateItem(client, null);
            ...
            case "copy":
            {
                var item = CreateItemInstance(client, targetObject.GetType().FullName);
                if (item == null)
                {
                    ChatUtil.SendSystemMessage(client,
                        "There was an error creating an instance of " + targetObject.GetType().FullName + "!");
                    return;
                }
            ...
            case "respawn":
            {
                ...
                if (int.TryParse(args[2], out respawn))
                {
                    ...
                    DisplayMessage(client,
                        "Object RespawnInterval set to " + targetObject.RespawnInterval + " seconds.");
                }
            ...
            case "quests":
            {
                try
                {
                    ...
                    if (client.Player.TargetObject is GameNPC)
                        foreach (GamePlayer player in client.Player.TargetObject.GetPlayersInRadius(
                                     WorldMgr.VISIBILITY_DISTANCE))
                            player.Out.SendNPCsQuestEffect(client.Player.TargetObject as GameNPC,
                                (client.Player.TargetObject as GameNPC).GetQuestIndicator(player));

                    client.Out.SendMessage(
                        targetObject.DataQuestList.Count + " Data Quests loaded for this object.",
    ...
    private GameStaticItem CreateItem(GameClient client, string itemClassName)
    {
        ...
        if (obj == null)
        {
            client.Out.SendMessage("There was an error creating an instance of " + itemClassName + "!",
                eChatType.CT_System, eChatLoc.CL_SystemWindow);
            return null;
        }
