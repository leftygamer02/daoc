...
public class JumpCommandHandler : AbstractCommandHandler, ICommandHandler
{
    ...
    public void OnCommand(GameClient client, string[] args)
    {
        try
        {
            ...
            {
                client.Player.MoveTo(client.Player.CurrentRegionID, client.Player.GroundTarget.X,
                    client.Player.GroundTarget.Y, client.Player.GroundTarget.Z, client.Player.Heading);
                return;
            }
            ...
            else if (args.Length >= 3 && args[1].ToLower() == "to" &&
                     (args[2].ToLower() == "house" || args[2].ToLower() == "myhouse"))
            {
                House house = null;
                if (args[2] == "myhouse")
                ...
                if (house != null)
                    ...
                    client.Out.SendMessage("This house number is not owned by anyone!", eChatType.CT_System,
            ...
            {
                client.Player.MoveTo(Convert.ToUInt16(args[3]), client.Player.X, client.Player.Y, client.Player.Z,
            ...
            if (args.Length == 3 && args[1] == "to")
            {
                ...
                if (clientc == null)
                {
                    ...
                    if (npcs.Length > 0)
                    {
                        ...
                        client.Out.SendMessage(
                            LanguageMgr.GetTranslation(client.Account.Language, "GMCommands.Jump.JumpToX",
                                npcs[0].CurrentRegion.Description), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                        if (jumpTarget.InHouse && jumpTarget.CurrentHouse != null)
                            jumpTarget.CurrentHouse.Enter(client.Player);
                        else
                            client.Player.MoveTo(jumpTarget.CurrentRegionID, jumpTarget.X, jumpTarget.Y,
                                jumpTarget.Z, jumpTarget.Heading);

                        return;
                    }

                    client.Out.SendMessage(
                        LanguageMgr.GetTranslation(client.Account.Language, "GMCommands.Jump.CannotBeFound",
                ...
                if (CheckExpansion(client, clientc, clientc.Player.CurrentRegionID))
                {
                    client.Out.SendMessage(
                        LanguageMgr.GetTranslation(client.Account.Language, "GMCommands.Jump.JumpToX",
                            clientc.Player.CurrentRegion.Description), eChatType.CT_System,
                        eChatLoc.CL_SystemWindow);
                    if (clientc.Player.CurrentHouse != null && clientc.Player.InHouse)
                        clientc.Player.CurrentHouse.Enter(client.Player);
                    else
                        client.Player.MoveTo(clientc.Player.CurrentRegionID, clientc.Player.X, clientc.Player.Y,
                            clientc.Player.Z, client.Player.Heading);
                    return;
                }

                client.Out.SendMessage("You don't have an expansion needed to jump to this location.",
            ...
            else if (args.Length == 4 && args[1] == "to")
            {
                ...
                if (clientc == null)
                {
                    ...
                    var npcs = WorldMgr.GetNPCsByName(args[2], (eRealm) realm);
                    ...
                    if (npcs.Length > 0)
                    {
                        ...
                        client.Out.SendMessage(
                            LanguageMgr.GetTranslation(client.Account.Language, "GMCommands.Jump.JumpToX",
                                npcs[0].CurrentRegion.Description), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                        client.Player.MoveTo(jumpTarget.CurrentRegionID, jumpTarget.X, jumpTarget.Y, jumpTarget.Z,
                            jumpTarget.Heading);
                        return;
                    }
                    ...
                    client.Out.SendMessage(
                        LanguageMgr.GetTranslation(client.Account.Language, "GMCommands.Jump.CannotBeFoundInRealm",
                            args[2], realm), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                    return;
                }
                    ...
                    if (clientc.Player.InHouse)
                    {
                        client.Out.SendMessage(
                            LanguageMgr.GetTranslation(client.Account.Language,
                                "GMCommands.Jump.CannotJumpToInsideHouse"), eChatType.CT_System,
                            eChatLoc.CL_SystemWindow);
                        return;
                    }
            ...
            else if (args.Length == 3 && args[1].ToLower() == "above")
            {
                ...
                if (CheckExpansion(client, clientc, clientc.Player.CurrentRegionID))
                {
                    ...
                    if (clientc.Player.CurrentRegion.IsDungeon)
                        client.Out.SendMessage(
                            "Player is currently in a dungeon and it's not safe to port above them.",
                            eChatType.CT_Staff, eChatLoc.CL_SystemWindow);
                    else
                        client.Player.MoveTo(clientc.Player.CurrentRegionID, clientc.Player.X, clientc.Player.Y,
                            clientc.Player.Z + 10000, client.Player.Heading);
                    return;
                }
            ...
            else if (args.Length == 4 && args[2] == "to" && args[3] == "jail")
            {
                ...
                {
                    client.Out.SendMessage(
                        LanguageMgr.GetTranslation(client.Account.Language, "GMCommands.Jump.PlayerIsNotInGame",
            ...
            {
                client.Player.MoveTo(client.Player.CurrentRegionID, Convert.ToInt32(args[2]),
                    Convert.ToInt32(args[3]), Convert.ToInt32(args[4]), client.Player.Heading);
                return;
            }
            ...
            else if (args.Length == 6 && args[1] == "to")
            {
                ...
                {
                    client.Player.MoveTo(Convert.ToUInt16(args[5]), Convert.ToInt32(args[2]),
                        Convert.ToInt32(args[3]), Convert.ToInt32(args[4]), client.Player.Heading);
                    return;
                }
            ...
            else if (args.Length == 6 && args[2] == "to")
            {
                ...
                clientc.Player.MoveTo(clientc.Player.CurrentRegionID, Convert.ToInt32(args[3]),
                    Convert.ToInt32(args[4]), Convert.ToInt32(args[5]), clientc.Player.Heading);
                return;
            }
            ...
            else if (args.Length == 7 && args[2] == "to")
            {
                ...
                {
                    clientc.Player.MoveTo(Convert.ToUInt16(args[6]), Convert.ToInt32(args[3]),
                        Convert.ToInt32(args[4]), Convert.ToInt32(args[5]), clientc.Player.Heading);
                    return;
                }
            ...
            else if (args.Length == 4 && args[2] == "to")
            {
                ...
                if (clientto == null)
                {
                    client.Out.SendMessage(
                        LanguageMgr.GetTranslation(client.Account.Language, "GMCommands.Jump.PlayerIsNotInGame",
                            args[3]), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                    return;
                }
                else
                {
                    if (CheckExpansion(clientto, clientc, clientto.Player.CurrentRegionID))
                    {
                        if (clientto.Player.CurrentHouse != null && clientto.Player.InHouse)
                            ...
                            clientc.Player.MoveTo(clientto.Player.CurrentRegionID, clientto.Player.X,
                                clientto.Player.Y, clientto.Player.Z, client.Player.Heading);
                        return;
                    }
            ...
            else if (args.Length > 1 && args[1] == "push")
            {
                ...
                locations =
                    client.Player.TempProperties.getProperty<object>(TEMP_KEY_JUMP, null) as Stack<GameLocation>;
                ...
                locations.Push(new GameLocation("temploc", client.Player.CurrentRegionID, client.Player.X,
                    client.Player.Y, client.Player.Z, client.Player.Heading));

                var message = LanguageMgr.GetTranslation(client.Account.Language, "GMCommands.Jump.Pushed");

                if (locations.Count > 1)
                    message += " " + LanguageMgr.GetTranslation(client.Account.Language,
                        "GMCommands.Jump.PushedTotal", locations.Count);

                message += " - " +
                           LanguageMgr.GetTranslation(client.Account.Language, "GMCommands.Jump.PopInstructions");
            ...
            else if (args.Length > 1 && args[1] == "pop")
            {
                ...
                {
                    client.Out.SendMessage(
                        LanguageMgr.GetTranslation(client.Account.Language, "GMCommands.Jump.NothingPushed"),
                        eChatType.CT_System, eChatLoc.CL_SystemWindow);
                    return;
                }

                var jumploc = locations.Pop();
    ...
    {
        var reg = WorldMgr.GetRegion(RegionID);
        if (reg != null && reg.Expansion > (int) clientJumpee.ClientType)
        {
            clientJumper.Out.SendMessage(
                LanguageMgr.GetTranslation(clientJumper, "GMCommands.Jump.CheckExpansion.CannotJump",
                    clientJumpee.Player.Name, reg.Description), eChatType.CT_System, eChatLoc.CL_SystemWindow);
            if (clientJumper != clientJumpee)
                clientJumpee.Out.SendMessage(
                    LanguageMgr.GetTranslation(clientJumpee, "GMCommands.Jump.CheckExpansion.ClientNoSup",
                        clientJumper.Player.Name, reg.Description), eChatType.CT_System, eChatLoc.CL_SystemWindow);
            return false;
        }
