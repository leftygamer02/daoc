...
public class TitleGmCommandHandler : AbstractCommandHandler, ICommandHandler
{
    public void OnCommand(GameClient client, string[] args)
    {
        ...
        if (target == null)
        {
            client.Out.SendMessage("You must target a player to change his titles!", eChatType.CT_System,
        ...
        if (args.Length > 2)
        {
            var title = PlayerTitleMgr.GetTitleByTypeName(args[2]);
            if (title == null)
            {
                client.Out.SendMessage("Title '" + args[2] + "' not found.", eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);
                return;
            }
        ...
        else if (args.Length > 1)
        {
            switch (args[1].ToLower())
            {
                case "list":
                {
                    ...
                    foreach (var title in target.Titles)
                    {
