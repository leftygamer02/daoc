...
public class PlayerCommandHandler : AbstractCommandHandler, ICommandHandler
{
        ...
        switch (args[1])
        {
            ...
            case "name":
            {
                ...
                player.Out.SendMessage(
                    client.Player.Name + "(PrivLevel: " + client.Account.PrivLevel + ") has changed your name to " +
                    player.Name +
                    "!", eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                client.Out.SendMessage("You successfully changed this players name to " + player.Name + "!",
                    eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                client.Out.SendMessage("Tell the player to Log out and back in to complete the change.",
            ...
            case "lastname":
            {
                ...
                switch (args[2])
                {
                    ...
                    case "reset":
                    {
                        ...
                        client.Out.SendMessage("You cleared " + player.Name + "'s lastname successfully!",
                            eChatType.CT_Important,
                            eChatLoc.CL_SystemWindow);
                        player.Out.SendMessage(
                            client.Player.Name + "(PrivLevel: " + client.Account.PrivLevel +
                            ") has cleared your lastname!",
                            eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                        player.SaveIntoDatabase();
                        break;
                    }
                    ...
                    if (pToLevel.Level < 40 || pToLevel.IsLevelSecondStage)
                    {
                        ...
                        if (pToLevel != client.Player)
                            pToLevel.Out.SendMessage(
                                client.Player.Name + "(PrivLevel: " + client.Account.PrivLevel +
                                ") has given you a free level!",
                                eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                    }
                    else
                    {
                        pToLevel.GainExperience(eXPSource.Other,
                            pToLevel.ExperienceForCurrentLevelSecondStage - pToLevel.Experience);
                        ...
                        if (pToLevel != client.Player)
                            pToLevel.Out.SendMessage(
                                client.Player.Name + "(PrivLevel: " + client.Account.PrivLevel +
                                ") has given you a free half level!",
                                eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                    }
                }
            ...
            case "level":
            {
                try
                {
                    ...
                    if (args[1] == "level") newLevel = Convert.ToByte(args[2]);

                    if (newLevel <= 0 || newLevel > 255)
                    {
                        client.Out.SendMessage(player.Name + "'s level can only be set to a number 1 to 255!",
                            eChatType.CT_Important,
                            eChatLoc.CL_SystemWindow);
                        return;
                    }

                    if (newLevel < player.Level || args[1] == "reset") player.Reset();
                    ...
                    if (newLevel > curLevel)
                    {
                        ...
                        if (newLevel > 40)
                        {
                            if (curLevel < 40)
                                curLevel = 40;
                            for (var i = curLevel; i < newLevel; i++)
                    ...
                    if (args[1] == "reset")
                    {
                        client.Out.SendMessage("You have reset " + player.Name + "!", eChatType.CT_Important,
                            eChatLoc.CL_SystemWindow);
                        player.Out.SendMessage(
                            client.Player.Name + "(PrivLevel: " + client.Account.PrivLevel +
                            ") has respecced your skills and reset your spec points!",
                            eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                    }
                    else
                    {
                        ...
                        player.Out.SendMessage(
                            client.Player.Name + "(PrivLevel: " + client.Account.PrivLevel +
                            ") has changed your level to " + newLevel + "!",
                            eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                    }
            ...
            case "startchampion":
                try
                {
                    ...
                    if (player.Champion == false)
                    {
                        ...
                        player.SaveIntoDatabase();
                        client.Out.SendMessage(player.Name + " is now on the path of the Champion!",
                            eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                        player.Out.SendMessage(
                            client.Player.Name + "(PrivLevel: " + client.Account.PrivLevel +
                            ") has started you on the path of the Champion!", eChatType.CT_Important,
                            eChatLoc.CL_SystemWindow);
                    }
                    else
                    {
                        client.Out.SendMessage(player.Name + " is already on the path of the Champion!",
                            eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                    }
                }
                catch
            ...
            case "clearchampion":
                ...
                try
                {
                    ...
                    client.Out.SendMessage("You have cleared " + player.Name + "'s Champion levels!",
                        eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                    player.Out.SendMessage(
                        client.Player.Name + "(PrivLevel: " + client.Account.PrivLevel +
                        ") has cleared your Champion levels!", eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                }
            ...
            case "respecchampion":
                ...
                try
                {
                    ...
                    client.Out.SendMessage("You have respecced " + player.Name + "'s Champion levels!",
                        eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                    player.Out.SendMessage(
                        client.Player.Name + "(PrivLevel: " + client.Account.PrivLevel +
                        ") has respecced your Champion levels!", eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                }
            ...
            case "startml":
                ...
                try
                {
                    ...
                    if (player.MLGranted == false)
                    {
                        ...
                        player.SaveIntoDatabase();
                        client.Out.SendMessage(player.Name + " is now ready to start Master Level training!",
                            eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                        player.Out.SendMessage(
                            client.Player.Name + "(PrivLevel: " + client.Account.PrivLevel +
                            ") has started your Master Level training!", eChatType.CT_Important,
                            eChatLoc.CL_SystemWindow);
                    }
                    else
                    {
                        client.Out.SendMessage(player.Name + " has already started Master Level training!",
                            eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                    }
                }
                catch (Exception)
            ...
            case "setml":
                ...
                try
                {
                    ...
                    var level = Convert.ToByte(args[2]);
                    ...
                    client.Out.SendMessage(player.Name + " Master Level is set to " + level + "!",
                        eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                    player.Out.SendMessage(
                        client.Player.Name + "(PrivLevel: " + client.Account.PrivLevel +
                        ") has set your Master Level to " + level + "!", eChatType.CT_Important,
                        eChatLoc.CL_SystemWindow);
                }
                catch (Exception)
                {
            ...
            case "setmlline":
                ...
                try
                {
                    ...
                    var line = Convert.ToByte(args[2]);
                    ...
                    player.Out.SendUpdatePlayer();
                    player.Out.SendMasterLevelWindow((byte) player.MLLevel);
                    client.Out.SendMessage(player.Name + " Master Line is set to " + line + "!",
                        eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                    player.Out.SendMessage(
                        client.Player.Name + "(PrivLevel: " + client.Account.PrivLevel +
                        ") has set your Master Line to " + line + "!", eChatType.CT_Important,
                        eChatLoc.CL_SystemWindow);
                }
                catch (Exception)
                {
            ...
            case "setmlstep":
                ...
                try
                {
                    ...
                    {
                        client.Out.SendMessage(player.Name + " has already finished all Master Levels!",
                            eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                        return;
                    }
                    ...
                    {
                        client.Out.SendMessage("Valid levels are 0 - " + GamePlayer.ML_MAX_LEVEL + "!",
                            eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                        return;
                    }

                    // Possible steps per level varies, max appears to be 11
                    var step = Convert.ToByte(args[3]);

                    var setFinished = true;
                    if (args.Length > 4) setFinished = Convert.ToBoolean(args[4]);

                    if (setFinished && player.HasFinishedMLStep(player.MLLevel + 1, step))
                    {
                        client.Out.SendMessage(
                            player.Name + " has already finished step " + step + " for Master Level " +
                            (player.MLLevel + 1) + "!", eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                    }
                    else if (setFinished == false && player.HasFinishedMLStep(player.MLLevel + 1, step) == false)
                    {
                        client.Out.SendMessage(
                            player.Name + " has not yet finished step " + step + " for Master Level " +
                            (player.MLLevel + 1) + "!", eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                    }
                    else
                    {
                        ...
                        {
                            client.Out.SendMessage(
                                player.Name + " has now finished step " + step + " for Master Level " +
                                (player.MLLevel + 1) + "!", eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                            player.Out.SendMessage(
                                client.Player.Name + "(PrivLevel: " + client.Account.PrivLevel + ") has set step " +
                                step + " completed for Master Level " + (player.MLLevel + 1) + "!",
                                eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                        }
                        else
                        {
                            client.Out.SendMessage(
                                player.Name + " has no longer finished step " + step + " for Master Level " +
                                (player.MLLevel + 1) + "!", eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                            player.Out.SendMessage(
                                client.Player.Name + "(PrivLevel: " + client.Account.PrivLevel + ") has set step " +
                                step + " as unfinished for Master Level " + (player.MLLevel + 1) + "!",
                                eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                        }
            ...
            case "realm":
            {
                try
                {
                    var newRealm = Convert.ToByte(args[2]);
                    var player = client.Player.TargetObject as GamePlayer;
                    ...
                    client.Out.SendMessage(
                        "You successfully changed " + player.Name + "'s realm to " +
                        GlobalConstants.RealmToName((eRealm) newRealm) +
                        "!", eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                    player.Out.SendMessage(
                        client.Player.Name + " has changed your realm to " +
                        GlobalConstants.RealmToName((eRealm) newRealm) + "!",
                        eChatType.CT_Important, eChatLoc.CL_SystemWindow);
            ...
            case "model":
            {
                ...
                try
                {
                    ...
                    switch (args[2])
                    {
                        case "reset":
                        {
                            player.Model = (ushort) player.Client.Account.Characters[player.Client.ActiveCharIndex]
                                .CreationModel;
                            client.Out.SendMessage(
                            ...
                            player.Out.SendMessage(
                                client.Player.Name + "(PrivLevel: " + client.Account.PrivLevel +
                                ") has changed your model back to its original creation model!",
                                eChatType.CT_Important,
                                eChatLoc.CL_SystemWindow);
                            player.Out.SendUpdatePlayer();
                            player.SaveIntoDatabase();
                        ...
                        default:
                        {
                            ...
                            player.Out.SendMessage(
                                client.Player.Name + "(PrivLevel: " + client.Account.PrivLevel +
                                ") has changed your form! (ID:#" + modelID +
                                ")", eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                            player.Out.SendUpdatePlayer();
                            player.SaveIntoDatabase();
                        }
            ...
            case "money":
            {
                ...
                try
                {
                    ...
                    switch (args[2])
                    {
                        case "copp":
                        {
                            var amount = long.Parse(args[3]);
                            player.AddMoney(amount);
                            InventoryLogging.LogInventoryAction(client.Player, player, eInventoryActionType.Other,
                                amount);
                            client.Out.SendMessage("You gave " + player.Name + " copper successfully!",
                                eChatType.CT_Important,
                                eChatLoc.CL_SystemWindow);
                            player.Out.SendMessage(
                                client.Player.Name + "(PrivLevel: " + client.Account.PrivLevel +
                                ") has given you some copper!",
                                eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                            return;
                        }
                        ...
                        case "silv":
                        {
                            var amount = long.Parse(args[3]) * 100;
                            player.AddMoney(amount);
                            InventoryLogging.LogInventoryAction(client.Player, player, eInventoryActionType.Other,
                                amount);
                            client.Out.SendMessage("You gave " + player.Name + " silver successfully!",
                                eChatType.CT_Important,
                                eChatLoc.CL_SystemWindow);
                            player.Out.SendMessage(
                                client.Player.Name + "(PrivLevel: " + client.Account.PrivLevel +
                                ") has given you some silver!",
                                eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                            return;
                        }
                        ...
                        case "gold":
                        {
                            var amount = long.Parse(args[3]) * 100 * 100;
                            player.AddMoney(amount);
                            InventoryLogging.LogInventoryAction(client.Player, player, eInventoryActionType.Other,
                                amount);
                            client.Out.SendMessage("You gave " + player.Name + " gold successfully!",
                                eChatType.CT_Important,
                                eChatLoc.CL_SystemWindow);
                            player.Out.SendMessage(
                                client.Player.Name + "(PrivLevel: " + client.Account.PrivLevel +
                                ") has given you some gold!",
                                eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                            return;
                        }
                        ...
                        case "plat":
                        {
                            var amount = long.Parse(args[3]) * 100 * 100 * 1000;
                            player.AddMoney(amount);
                            InventoryLogging.LogInventoryAction(client.Player, player, eInventoryActionType.Other,
                                amount);
                            client.Out.SendMessage("You gave " + player.Name + " platinum successfully!",
                                eChatType.CT_Important,
                                eChatLoc.CL_SystemWindow);
                            player.Out.SendMessage(
                                client.Player.Name + "(PrivLevel: " + client.Account.PrivLevel +
                                ") has given you some platinum!",
                                eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                            return;
                        }
                        ...
                        case "mith":
                        {
                            var amount = long.Parse(args[3]) * 100 * 100 * 1000 * 1000;
                            player.AddMoney(amount);
                            InventoryLogging.LogInventoryAction(client.Player, player, eInventoryActionType.Other,
                                amount);
                            client.Out.SendMessage("You gave " + player.Name + " mithril successfully!",
                                eChatType.CT_Important,
                                eChatLoc.CL_SystemWindow);
                            player.Out.SendMessage(
                                client.Player.Name + "(PrivLevel: " + client.Account.PrivLevel +
                                ") has given you some mithril!",
                                eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                            return;
                        }
                    }
            ...
            case "rps":
            {
                ...
                try
                {
                    ...
                    client.Out.SendMessage("You gave " + player.Name + " " + amount + " realmpoints succesfully!",
                        eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                    player.Out.SendMessage(
                        client.Player.Name + "(PrivLevel: " + client.Account.PrivLevel + ") has given you " +
                        amount + " realmpoints!",
                        eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                    player.SaveIntoDatabase();
                    player.Out.SendUpdatePlayer();
            ...
            case "xpa":
            {
                ...
                try
                {
                    ...
                    var xpSource = eXPSource.Other;
                    if (args[1].ToLower() == "xpa") xpSource = eXPSource.NPC;
            ...
            case "clxp":
            {
                ...
                try
                {
                    ...
                    client.Out.SendMessage(
                        "You gave " + player.Name + " " + amount + " Champion experience succesfully!",
                        eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                    player.Out.SendMessage(
                        client.Player.Name + "(PrivLevel: " + client.Account.PrivLevel + ") has given you " +
                        amount + " Champion experience!", eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                    ...
                    if (gainedLevel)
                        player.Out.SendMessage("You reached champion level " + player.ChampionLevel + "!",
                            eChatType.CT_Important, eChatLoc.CL_SystemWindow);
            ...
            case "mlxp":
            {
                ...
                try
                {
                    ...
                    client.Out.SendMessage("You gave " + player.Name + " " + amount + " ML experience succesfully!",
                        eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                    player.Out.SendMessage(
                        client.Player.Name + "(PrivLevel: " + client.Account.PrivLevel + ") has given you " +
                        amount + " ML experience!", eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                    ...
                    if (player.MLExperience < 0) player.MLExperience = 0;
            ...
            case "stat":
            {
                ...
                try
                {
                    var value = Convert.ToInt16(args[3]);
            ...
            case "friend":
            {
                ...
                if (player == null)
                {
                    client.Out.SendMessage("You need a valid target!", eChatType.CT_System,
                        eChatLoc.CL_SystemWindow);
                    return;
                }
                ...
                if (args[2] == "list")
                {
                    var list = player.SerializedFriendsList;
                    client.Out.SendCustomTextWindow(player.Name + "'s Friend List", list);
                    return;
                }

                var name = string.Join(" ", args, 2, args.Length - 2);

                var result = 0;
                var fclient = WorldMgr.GuessClientByPlayerNameAndRealm(name, 0, true, out result);
                if (fclient != null &&
                    !GameServer.ServerRules.IsSameRealm(fclient.Player, player.Client.Player, true))
                    fclient = null;
                ...
                if (fclient == null)
                {
                    ...
                    {
                        player.Out.SendMessage(
                            client.Player.Name + "(PrivLevel: " + client.Account.PrivLevel + ") has removed " +
                            player.Name +
                ...
                switch (result)
                {
                    case 2: // name not unique
                        client.Out.SendMessage("Character name is not unique.", eChatType.CT_System,
                            eChatLoc.CL_SystemWindow);
                        return;
                    case 3: // exact match
                    case 4: // guessed name
                        ...
                        {
                            client.Out.SendMessage("You can't add that player to his or her own friend list!",
                                eChatType.CT_Important,
                                eChatLoc.CL_SystemWindow);
                            return;
                        }
                        ...
                        {
                            player.Out.SendMessage(
                                client.Player.Name + "(PrivLevel: " + client.Account.PrivLevel + ") has added " +
                                name +
                ...
                switch (args[2])
                {
                    case "line":
                    {
                        ...
                        player.Client.Out.SendMessage(
                            client.Player.Name + "(PrivLevel: " + client.Account.PrivLevel + ") has awarded you " +
                            amount +
            ...
            case "save":
            {
                ...
                if (args.Length == 2 && player != null)
                {
                    player.Out.SendMessage(
                        client.Player.Name + "(PrivLevel: " + client.Account.PrivLevel +
                        ") has saved your character.",
                        eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                    client.Out.SendMessage(player.Name + " saved successfully!", eChatType.CT_Important,
                        eChatLoc.CL_SystemWindow);
                    player.SaveIntoDatabase();
                }
                ...
                if (args.Length == 3)
                    switch (args[2])
                    {
                        case "all":
                        {
                            ...
                            client.Out.SendMessage("Saved all characters!", eChatType.CT_Important,
                                eChatLoc.CL_SystemWindow);
                        }
                            break;
            ...
            case "kick":
            {
                ...
                if (args.Length == 3)
                    switch (args[2])
                    {
                        case "all":
                        {
                            foreach (var allplayer in WorldMgr.GetAllPlayingClients())
            ...
            case "rez":
            {
                ...
                if (args.Length == 2 && player != null)
                {
                    if (!player.IsAlive)
                    {
                        ...
                        player.MoveTo(client.Player.CurrentRegionID, client.Player.X, client.Player.Y,
                            client.Player.Z,
                            client.Player.Heading);

                        client.Out.SendMessage("You resurrected " + player.Name + " successfully!",
                            eChatType.CT_Important,
                            eChatLoc.CL_SystemWindow);
                    ...
                    {
                        client.Out.SendMessage("Player is not dead!", eChatType.CT_Important,
                            eChatLoc.CL_SystemWindow);
                        return;
                    }
                }
                ...
                if (args.Length >= 3)
                    switch (args[2])
                    {
                        case "albs":
                        {
                            foreach (var aplayer in WorldMgr.GetClientsOfRealm(eRealm.Albion))
                                if (!aplayer.Player.IsAlive)
                                {
                                    ...
                                    aplayer.Player.MoveTo(client.Player.CurrentRegionID, client.Player.X,
                                        client.Player.Y, client.Player.Z,
                                        client.Player.Heading);
                        ...
                        case "hibs":
                        {
                            foreach (var hplayer in WorldMgr.GetClientsOfRealm(eRealm.Hibernia))
                                if (!hplayer.Player.IsAlive)
                                {
                                    ...
                                    hplayer.Player.MoveTo(client.Player.CurrentRegionID, client.Player.X,
                                        client.Player.Y, client.Player.Z,
                                        client.Player.Heading);
                        ...
                        case "mids":
                        {
                            foreach (var mplayer in WorldMgr.GetClientsOfRealm(eRealm.Midgard))
                                if (!mplayer.Player.IsAlive)
                                {
                                    ...
                                    mplayer.Player.MoveTo(client.Player.CurrentRegionID, client.Player.X,
                                        client.Player.Y, client.Player.Z,
                                        client.Player.Heading);
                        ...
                        case "self":
                        {
                            ...
                            if (!selfplayer.IsAlive)
                            {
                                ...
                                selfplayer.MoveTo(client.Player.CurrentRegionID, client.Player.X, client.Player.Y,
                                    client.Player.Z,
                                    client.Player.Heading);

                                selfplayer.Out.SendMessage("You revive yourself.", eChatType.CT_Important,
                                    eChatLoc.CL_SystemWindow);
                            ...
                            {
                                client.Out.SendMessage("You are not dead!", eChatType.CT_Important,
                                    eChatLoc.CL_SystemWindow);
                                return;
                            }
                        }
                        ...
                        case "all":
                        {
                            foreach (var allplayer in WorldMgr.GetAllPlayingClients())
                                if (!allplayer.Player.IsAlive)
                                {
                                    ...
                                    allplayer.Player.MoveTo(client.Player.CurrentRegionID, client.Player.X,
                                        client.Player.Y, client.Player.Z,
                                        client.Player.Heading);
            ...
            case "kill":
            {
                ...
                if (args.Length == 2 && player != null)
                {
                    if (player.Client.Account.PrivLevel > 1)
                    {
                        client.Out.SendMessage("This command can not be used on Gamemasters!",
                            eChatType.CT_Important,
                            eChatLoc.CL_SystemWindow);
                        return;
                    }
                    ...
                    if (player.IsAlive)
                    {
                        ...
                        client.Out.SendMessage("You killed " + player.Name + " successfully!",
                            eChatType.CT_Important,
                            eChatLoc.CL_SystemWindow);
                        player.Out.SendMessage(client.Player.Name + " has killed you!", eChatType.CT_Important,
                            eChatLoc.CL_SystemWindow);
                    }
                    else
                    {
                        client.Out.SendMessage("Player is not alive!", eChatType.CT_Important,
                            eChatLoc.CL_SystemWindow);
                        return;
                    }
                }
                ...
                switch (args[2])
                {
                    ...
                    case "self":
                    {
                        ...
                        else
                        {
                            ...
                            client.Out.SendMessage("Good bye cruel world!", eChatType.CT_Important,
                                eChatLoc.CL_SystemWindow);
                        }
                    }
                        break;
                    ...
                    default:
                    {
                        client.Out.SendMessage("'" + args[2] + "' is not a valid arguement.",
                            eChatType.CT_Important,
                            eChatLoc.CL_SystemWindow);
                    }
                        break;
            ...
            case "jump":
            {
                ...
                switch (args[2])
                {
                    case "guild":
                    {
                        ...
                        foreach (var pname in WorldMgr.GetAllPlayingClients())
                        {
                            var guild = string.Join(" ", args, 3, args.Length - 3);
                            ...
                            {
                                count++;
                                pname.Player.MoveTo(client.Player.CurrentRegionID, client.Player.X, client.Player.Y,
                                    client.Player.Z,
                                    client.Player.Heading);
                            }
                        }

                        client.Out.SendMessage(count + " players jumped!", eChatType.CT_Important,
                            eChatLoc.CL_SystemWindow);
                    }
                        break;
                    ...
                    case "group":
                    {
                        ...
                        foreach (var pname in WorldMgr.GetAllPlayingClients())
                        {
                            ...
                            if (name == pname.Player.Name)
                                foreach (var groupedplayers in pname.Player.Group.GetMembersInTheGroup())
                                {
                                    groupedplayers.MoveTo(client.Player.CurrentRegionID, client.Player.X,
                                        client.Player.Y, client.Player.Z,
                                        client.Player.Heading);
                                    count++;
                                }
                    ...
                    case "cg":
                    {
                        ...
                        foreach (var pname in WorldMgr.GetAllPlayingClients())
                        {
                            ...
                            var cg = (ChatGroup) pname.Player.TempProperties.getProperty<object>(
                                ChatGroup.CHATGROUP_PROPERTY, null);

                            if (name == pname.Player.Name)
                                foreach (GamePlayer cgplayers in cg.Members.Keys)
                                {
                                    cgplayers.MoveTo(client.Player.CurrentRegionID, client.Player.X,
                                        client.Player.Y, client.Player.Z,
                                        client.Player.Heading);
                                    count++;
                                }
                    ...
                    case "bg":
                    {
                        ...
                        foreach (var pname in WorldMgr.GetAllPlayingClients())
                        {
                            ...
                            var cg = (BattleGroup) pname.Player.TempProperties.getProperty<object>(
                                BattleGroup.BATTLEGROUP_PROPERTY, null);
            ...
            case "update":
            {
                ...
                client.Out.SendMessage(player.Name + " updated successfully!", eChatType.CT_Important,
                    eChatLoc.CL_SystemWindow);
            }
                break;
                ...
                if (byte.TryParse(args[2], out activeBags))
                {
                    if (activeBags <= 0x0F)
                    {
                        ...
                        client.Player.Out.SendMessage(
                            string.Format("{0}'s active saddlebags set to 0x{1:X2}!", player.Name,
                                player.ActiveSaddleBags), eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                        player.Out.SendMessage(
                            string.Format("Your active saddlebags have been set to 0x{0:X2} by {1}!",
                                player.ActiveSaddleBags, client.Player.Name), eChatType.CT_Important,
                            eChatLoc.CL_SystemWindow);
            ...
            case "showgroup":
            {
                ...
                {
                    client.Out.SendMessage("Player does not have a group!", eChatType.CT_System,
                        eChatLoc.CL_SystemWindow);
                    return;
                }
            ...
            case "showeffects":
            {
                ...
                var positiveEffects = new ArrayList();
                var negativeEffects = new ArrayList();
                ...
                {
                    foreach (var e in player.effectListComponent.GetSpellEffects())
                    {
                    ...
                    if (positiveEffects.Count > 0)
                        ...
                        foreach (ECSGameSpellEffect e in positiveEffects)
                        {
                            ...
                            effects.Add(" -- " + e.SpellHandler.Spell.Name + " (" + e.EffectType + ", level " +
                                        e.SpellHandler.Spell.Level + "): " + caster + " (Caster), " +
                                        e.GetRemainingTimeForClient() / 1000 + " seconds remaining");
                        }
            ...
            case "allchars":
            {
                ...
                var targetClient = targetPlayer == null ? null : targetPlayer.Client;

                if (args.Length > 2) targetClient = WorldMgr.GetClientByPlayerName(args[2], true, false);
                ...
                {
                    var characterNames = string.Empty;

                    foreach (var acctChar in targetClient.Account.Characters)
                        if (acctChar != null)
            ...
            case "areas":
            {
                ...
                {
                    var areaInfo = area.GetType().Name + ", ID:" + area.ID;
                    if (area is QuestSearchArea)
                    {
                        var questArea = area as QuestSearchArea;
                        ...
                        if (questArea.DataQuest != null)
                        {
                            ...
                            if (questArea.Step > 0)
                                ...
                                areaInfo += ", Eligible = " +
                                            questArea.DataQuest.CheckQuestQualification(targetPlayer);
                        }
                    }
    ...
    private void Show_Inventory(GamePlayer player, GameClient client, string limitType)
    {
        ...
        if (limitType == "vault")
        {
            ...
            text.Add("  ----- Vault:");
            foreach (var item in player.Inventory.AllItems)
                if (item.SlotPosition >= (int) eInventorySlot.FirstVault &&
                    item.SlotPosition <= (int) eInventorySlot.LastVault)
                    text.Add(item.Count.ToString("000") + " " + item.Name + " (" + item.Id_nb + ")");
        }
        ...
        if (!limitShown) text.Add("Unkown command.  Use wear | bag | vault | house | cons");
    ...
    private void Show_Info(GamePlayer player, GameClient client)
    {
        ...
        text.Add("PLAYER INFORMATION (Client # " + player.Client.SessionID + ", " + player.GetType().FullName +
                 ")");
        text.Add("  - Name Lastname : " + player.Name + " " + player.LastName);
        text.Add("  - Realm Level Gender Class : " + GlobalConstants.RealmToName(player.Realm) + " " +
                 player.Level + " " + player.Gender + " " + player.CharacterClass.Name + " (" +
                 player.CharacterClass.ID + ")");
        text.Add("  - Guild : " + player.GuildName + " " +
                 (player.GuildRank != null ? "Rank: " + player.GuildRank.RankLevel.ToString() : ""));
        text.Add("  - XPs/RPs/BPs : " + player.Experience + " xp, " + player.RealmPoints + " rp, " +
                 player.BountyPoints + " bp");
        ...
        if (player.MLGranted)
            text.Add("  - Master Levels :  ML " + player.MLLevel + ", " + player.MLExperience + " mlxp , MLLine " +
                     player.MLLine);
        else
        ...
        text.Add("  - Account Name & IP : " + player.Client.Account.Name + " from " +
                 player.Client.Account.LastLoginIP);
        ...
        for (var stat = eProperty.Stat_First; stat <= eProperty.Stat_Last; stat++, cnt++)
        {
        ...
        sTitle = "";
        cnt = 0;
        for (var res = eProperty.Resist_First; res <= eProperty.Resist_Last; res++, cnt++)
        {
        ...
        sCurrent = "";
        foreach (var spec in player.GetSpecList()) sCurrent += spec.Name + " = " + spec.Level + " ; ";
