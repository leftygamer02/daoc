    ...
    public void OnCommand(GameClient client, string[] args)
    {
        ...
        catch
        {
            DisplayMessage(client,
                LanguageMgr.GetTranslation(client.Account.Language, "GMCommands.Cast.InvalidId"));
            return;
        }
        ...
        {
            DisplayMessage(client,
                LanguageMgr.GetTranslation(client.Account.Language, "GMCommands.Cast.IdNegative"));
            return;
        }
        ...
        switch (type)
        {
            ...
            {
                DisplayMessage(client,
                    LanguageMgr.GetTranslation(client.Account.Language, "GMCommands.Cast.EffectExecuted",
                        id.ToString()));

                var effect = new DummyEffect((ushort) id);
            ...
            {
                DisplayMessage(client,
                    LanguageMgr.GetTranslation(client.Account.Language, "GMCommands.Cast.CastExecuted",
                        id.ToString()));
                foreach (GamePlayer player in client.Player.GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE))
            ...
            case "spell":
            {
                ...
                var line = new SpellLine("GMCast", "GM Cast", "unknown", false);
                if (spell != null)
                {
                    if (target is GamePlayer && target != client.Player && spell.Target.ToLower() != "self")
                    {
                        DisplayMessage(client,
                            LanguageMgr.GetTranslation(client.Account.Language,
                                "GMCommands.Cast.Spell.CastOnLiving", spell.Name, target.Name));
                        DisplayMessage(((GamePlayer) target).Client,
                            LanguageMgr.GetTranslation(((GamePlayer) target).Client,
                                "GMCommands.Cast.Spell.GMCastOnYou",
                                client.Account.PrivLevel == 2 ? "GM" : "Admin", client.Player.Name));
                    }
                    else if (target == client.Player || spell.Target.ToLower() == "self")
                    {
                        DisplayMessage(client,
                            LanguageMgr.GetTranslation(client.Account.Language, "GMCommands.Cast.Spell.CastOnSelf",
                                spell.Name));
                    }

                    var spellHandler = ScriptMgr.CreateSpellHandler(client.Player, spell, line);
                ...
                {
                    DisplayMessage(client,
                        LanguageMgr.GetTranslation(client.Account.Language, "GMCommands.Cast.Spell.Inexistent",
                            id.ToString()));
                }
            ...
            case "style":
            {
                ...
                client.Player.Out.SendCombatAnimation(client.Player, null,
                    (ushort) client.Player.ActiveWeapon.Model, 0, id, 0, (byte) 11, 100);
                break;
            }
            ...
            case "sound":
                DisplayMessage(client,
                    LanguageMgr.GetTranslation(client.Account.Language, "GMCommands.Cast.SoundPlayed",
                        id.ToString()));
