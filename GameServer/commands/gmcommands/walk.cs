...
public class WalkCommandHandler : AbstractCommandHandler, ICommandHandler
{
    public void OnCommand(GameClient client, string[] args)
    {
        ...
        if (args.Length == 1 && args[0] == "&stop")
        {
            if (targetNPC == null)
            {
                client.Out.SendMessage("Type /stop to stop your target npc from moving", eChatType.CT_System,
        ...
        if (args.Length < 4)
        {
            ...
            client.Out.SendMessage("'/walk <xoff> <yoff> <zoff> <speed>'", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            return;
        }
        ...
        if (targetNPC == null)
        {
            client.Out.SendMessage("Type /walk for command overview", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            return;
        }
