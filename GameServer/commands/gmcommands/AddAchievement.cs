...
public class AddAchievementCommandHandler : AbstractCommandHandler, ICommandHandler
{
    ...
    public void OnCommand(GameClient client, string[] args)
    {
        ...
        if (args[1] == "add")
        {
            ...
            {
                client.Player.Out.SendMessage("You need a target to add an achievement.", eChatType.CT_System,
            ...
            {
                client.Player.Out.SendMessage("You need a player target to add an achievement.",
        ...
        {
            var bg = (BattleGroup) client.Player.TempProperties.getProperty<object>(
                BattleGroup.BATTLEGROUP_PROPERTY, null);

            if (bg == null)
            {
                client.Player.Out.SendMessage("You need to join the target battlegroup to add an achievement.",
                    eChatType.CT_System, eChatLoc.CL_SystemWindow);
                return;
            }
        ...
        else if (args[1] == "addgroup")
        {
            ...
            {
                client.Player.Out.SendMessage("You need to join the target group to add an achievement.",
                    eChatType.CT_System, eChatLoc.CL_SystemWindow);
                return;
            }
            ...
            foreach (var gP in group.GetPlayersInTheGroup())
            {
        ...
        client.Out.SendCustomDialog($"Confirm adding the achievement \"{achievementName}\" to {count} player(s)?",
            AddAchievementResponseHandler);
    }
    ...
    protected virtual void AddAchievementResponseHandler(GamePlayer player, byte response)
    {
        if (response == 1)
        {
            ...
            {
                player.Out.SendMessage("Error loading the temp properties", eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);
                return;
            }
            ...
            foreach (var p in targetPlayers)
            {
                ...
                p.Achieve(credit);
                log.Warn(
                    $"ACHIEVEMENT: Manually added credit for {achievementName} to player {p.Name} ({player.Name})");
            }

            player.Out.SendMessage($"Achievement {achievementName} added to the selected player(s)",
