...
public class GMInfoCommandHandler : AbstractCommandHandler, ICommandHandler
{
    ...
    {
        var hour = WorldMgr.GetCurrentGameTime() / 1000 / 60 / 60;
        var minute = WorldMgr.GetCurrentGameTime() / 1000 / 60 % 60;
        var seconde = WorldMgr.GetCurrentGameTime() / 1000 % 60;
        var ip = Dns.GetHostByName(Dns.GetHostName());
        var myNIC = ip.AddressList[0].ToString();

        var myInternetIP = ip.AddressList[0].ToString();

        var name = "(NoName)";
        var info = new List<string>();
        info.Add("        Current Region : " + client.Player.CurrentRegionID);
        info.Add(" ");
        var regionType = client.Player.CurrentRegion.GetType();
        ...
        if (client.Player.TargetObject != null)
        {
            ...
            if (client.Player.TargetObject is GameNPC)
            {
                ...
                info.Add(string.Format(" + Flags: {0} (0x{1})", ((GameNPC.eFlags) target.Flags).ToString("G"),
                    target.Flags.ToString("X")));
                info.Add(" ");
                ...
                var aggroBrain = target.Brain as IOldAggressiveBrain;
                if (aggroBrain != null)
                {
                ...
                if (target.NPCTemplate != null)
                    info.Add(" + NPCTemplate: " + "[" + target.NPCTemplate.TemplateId + "] " +
                             target.NPCTemplate.Name);

                info.Add(" + Roaming Range: " + target.RoamingRange);

                var respawn = TimeSpan.FromMilliseconds(target.RespawnInterval);
                ...
                else
                {
                    ...
                    info.Add(" + Respawn: " + days + hours + respawn.Minutes + " minutes " + respawn.Seconds +
                             " seconds");
                    info.Add(" + SpawnPoint:  " + target.SpawnPoint.X + ", " + target.SpawnPoint.Y + ", " +
                             target.SpawnPoint.Z);
                }
                ...
                */
                ...
                if (target.InCombat || target.attackComponent.AttackState)
                {
                    ...
                    if (target.attackComponent.attackAction != null)
                    {
                        ...
                        info.Add(
                            "AttackAction TimeUntilStart " + target.attackComponent.attackAction.TimeUntilStart);
                    }
                }
                ...
                {
                    var aggroList = (target.Brain as StandardMobBrain).AggroTable;
                    ...
                    if (aggroList.Count > 0)
                    {
                        ...
                        foreach (var living in aggroList.Keys)
                ...
                {
                    var drop = GameServer.Database.FindObjectByKey<ItemTemplate>(loot.ItemTemplateID);
            ...
            if (client.Player.TargetObject is GamePlayer)
            {
                ...
                sCurrent = "";
                foreach (var spec in target.GetSpecList()) sCurrent += "  - " + spec.Name + " = " + spec.Level + " \n";
                ...
                info.Add("  ----- Wearing:");
                foreach (var item in target.Inventory.EquippedItems)
            ...
            if (client.Player.TargetObject is GameStaticItem)
            {
                ...
                var invItem = target as WorldInventoryItem;
                if (invItem != null) info.Add(" Count: " + invItem.Item.Count);
            ...
            if (client.Player.TargetObject is GameKeepComponent)
            {
                ...
                {
                    info.Add(" + Keep Manager : " + GameServer.KeepManager.GetType().FullName);
                    var bg = GameServer.KeepManager.GetBattleground(client.Player.CurrentRegionID);
                    info.Add(" + Battleground (" + bg.MinLevel + " to " + bg.MaxLevel + ", max RL: " +
                             bg.MaxRealmLevel + ")");
                }
                else
                {
            ...
            if (client.Player.TargetObject is GameSiegeRam)
            {
                ...
                foreach (var rider in target.Riders)
                    if (rider != null)
        ...
        if (client.Player.TargetObject == null)
        {
            ...
            if (client.Player.InHouse)
            {
                ...
                var house = client.Player.CurrentHouse as House;

                name = house.Name;

                var level = house.Model - (house.Model - 1) / 4 * 4;
                var due = house.LastPaid.AddDays(ServerProperties.Properties.RENT_DUE_DAYS).AddHours(1) -
                          DateTime.Now;

                info.Add("  ------- HOUSE ------\n");
                info.Add(LanguageMgr.GetTranslation(client.Account.Language, "House.SendHouseInfo.Owner", name));
                info.Add(LanguageMgr.GetTranslation(client.Account.Language, "House.SendHouseInfo.Lotnum",
                    house.HouseNumber));
                ...
                info.Add(LanguageMgr.GetTranslation(client.Account.Language, "House.SendHouseInfo.PorchEnabled",
                    house.Porch ? " Present" : " Not Present"));
                info.Add(LanguageMgr.GetTranslation(client.Account.Language, "House.SendHouseInfo.PorchRoofColor",
                    Color(house.PorchRoofColor)));
                info.Add(" ");
                info.Add(LanguageMgr.GetTranslation(client.Account.Language,
                    "House.SendHouseInfo.ExteriorMaterials"));
                info.Add(LanguageMgr.GetTranslation(client.Account.Language, "House.SendHouseInfo.RoofMaterial",
                    MaterialWall(house.RoofMaterial)));
                info.Add(LanguageMgr.GetTranslation(client.Account.Language, "House.SendHouseInfo.WallMaterial",
                    MaterialWall(house.WallMaterial)));

                info.Add(LanguageMgr.GetTranslation(client.Account.Language, "House.SendHouseInfo.DoorMaterial",
                    MaterialDoor(house.DoorMaterial)));

                info.Add(LanguageMgr.GetTranslation(client.Account.Language, "House.SendHouseInfo.TrussMaterial",
                    MaterialTruss(house.TrussMaterial)));
                info.Add(LanguageMgr.GetTranslation(client.Account.Language, "House.SendHouseInfo.PorchMaterial",
                    MaterialTruss(house.PorchMaterial)));
                info.Add(LanguageMgr.GetTranslation(client.Account.Language, "House.SendHouseInfo.WindowMaterial",
                    MaterialTruss(house.WindowMaterial)));

                info.Add(" ");
                info.Add(
                    LanguageMgr.GetTranslation(client.Account.Language, "House.SendHouseInfo.ExteriorUpgrades"));
                info.Add(LanguageMgr.GetTranslation(client.Account.Language,
                    "House.SendHouseInfo.OutdoorGuildBanner",
                    house.OutdoorGuildBanner ? " Present" : " Not Present"));
                info.Add(LanguageMgr.GetTranslation(client.Account.Language,
                    "House.SendHouseInfo.OutdoorGuildShield",
                    house.OutdoorGuildShield ? " Present" : " Not Present"));
                info.Add(" ");
                info.Add(
                    LanguageMgr.GetTranslation(client.Account.Language, "House.SendHouseInfo.InteriorUpgrades"));
                info.Add(LanguageMgr.GetTranslation(client.Account.Language,
                    "House.SendHouseInfo.IndoorGuildBanner",
                    house.IndoorGuildBanner ? " Present" : " Not Present"));
                info.Add(LanguageMgr.GetTranslation(client.Account.Language,
                    "House.SendHouseInfo.IndoorGuildShield",
                    house.IndoorGuildShield ? " Present" : " Not Present"));
                info.Add(" ");
                info.Add(LanguageMgr.GetTranslation(client.Account.Language,
                    "House.SendHouseInfo.InteriorCarpets"));
                if (house.Rug1Color != 0)
                    info.Add(LanguageMgr.GetTranslation(client.Account.Language, "House.SendHouseInfo.Rug1Color",
                        Color(house.Rug1Color)));
                if (house.Rug2Color != 0)
                    info.Add(LanguageMgr.GetTranslation(client.Account.Language, "House.SendHouseInfo.Rug2Color",
                        Color(house.Rug2Color)));
                if (house.Rug3Color != 0)
                    info.Add(LanguageMgr.GetTranslation(client.Account.Language, "House.SendHouseInfo.Rug3Color",
                        Color(house.Rug3Color)));
                if (house.Rug4Color != 0)
                    info.Add(LanguageMgr.GetTranslation(client.Account.Language, "House.SendHouseInfo.Rug4Color",
                        Color(house.Rug4Color)));
                info.Add(" ");
                info.Add(LanguageMgr.GetTranslation(client.Account.Language, "House.SendHouseInfo.Lockbox",
                    Money.GetString(house.KeptMoney)));
                info.Add(LanguageMgr.GetTranslation(client.Account.Language, "House.SendHouseInfo.RentalPrice",
                    Money.GetString(HouseMgr.GetRentByModel(house.Model))));
                info.Add(LanguageMgr.GetTranslation(client.Account.Language, "House.SendHouseInfo.MaxLockbox",
                    Money.GetString(HouseMgr.GetRentByModel(house.Model) *
                                    ServerProperties.Properties.RENT_LOCKBOX_PAYMENTS)));
                info.Add(LanguageMgr.GetTranslation(client.Account.Language, "House.SendHouseInfo.RentDueIn",
                    due.Days, due.Hours));

                #endregion House

                client.Out.SendCustomTextWindow(
                    LanguageMgr.GetTranslation(client.Account.Language, "House.SendHouseInfo.HouseOwner", name),
                    info);
            }
            else // No target and not in a house
            {
                ...
                info.Add(" ");
                info.Add(" Alb players: " +
                         WorldMgr.GetClientsOfRegionCount(client.Player.CurrentRegion.ID, eRealm.Albion));
                info.Add(" Hib players: " +
                         WorldMgr.GetClientsOfRegionCount(client.Player.CurrentRegion.ID, eRealm.Hibernia));
                info.Add(" Mid players: " +
    ...
    {
        var armorSlots = new List<eArmorSlot>();
        ...
        foreach (var slot in armorSlots) totalArmor += living.GetArmorAF(slot);
