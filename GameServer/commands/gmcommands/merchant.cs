    ...
    public void OnCommand(GameClient client, string[] args)
    {
        ...
        if (args[1].ToLower() != "create" && targetMerchant == null)
        {
            DisplayMessage(client,
                LanguageMgr.GetTranslation(client.Account.Language, "GMCommands.Merchant.CommandOverview"));
            return;
        }
        ...
        switch (args[1].ToLower())
        {
            ...
            case "create":
            {
                var theType = "DOL.GS.GameMerchant";
                if (args.Length > 2)
                    theType = args[2];
                ...
                if (merchant == null)
                {
                    DisplayMessage(client,
                        LanguageMgr.GetTranslation(client.Account.Language,
                            "GMCommands.Merchant.ErrorCreateInstance", theType));
                    return;
                }
                ...
                merchant.Name = LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE,
                    "GMCommands.Merchant.NewName");
                merchant.Model = 9;
                //Fill the living variables
                merchant.CurrentSpeed = 0;
                merchant.MaxSpeedBase = 200;
                merchant.GuildName = LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE,
                    "GMCommands.Merchant.NewGuildName");
                merchant.Size = 50;
                merchant.AddToWorld();
                merchant.SaveIntoDatabase();
                DisplayMessage(client,
                    LanguageMgr.GetTranslation(client.Account.Language, "GMCommands.Merchant.Create.Created",
                        merchant.ObjectID));
                break;
            }
            ...
            case "info":
            {
                if (args.Length == 2)
                {
                    if (targetMerchant.TradeItems == null)
                        DisplayMessage(client,
                            LanguageMgr.GetTranslation(client.Account.Language,
                                "GMCommands.Merchant.Info.ArtListIsEmpty"));
                    else
                        DisplayMessage(client,
                            LanguageMgr.GetTranslation(client.Account.Language, "GMCommands.Merchant.Info.ArtList",
                                targetMerchant.TradeItems.ItemsListID));
                }
            ...
            case "save":
            {
                ...
                DisplayMessage(client,
                    LanguageMgr.GetTranslation(client.Account.Language, "GMCommands.Merchant.Save.SavedInDB"));
                break;
            }
            ...
            case "savelist":
            {
                var currentID = targetMerchant.TradeItems.ItemsListID;

                var itemList = DOLDB<MerchantItem>.SelectObjects(DB.Column("ItemListID").IsEqualTo(currentID));
                foreach (var merchantItem in itemList)
                {
                ...
                DisplayMessage(client,
                    "New MerchantItems list saved as '" + GameServer.Database.Escape(args[2]) + "'");
            ...
            case "remove":
            {
                ...
                DisplayMessage(client,
                    LanguageMgr.GetTranslation(client.Account.Language,
                        "GMCommands.Merchant.Remove.RemovedFromDB"));
                break;
            }
                ...
                switch (args[2].ToLower())
                {
                    ...
                    case "add":
                    {
                        if (args.Length == 4)
                            try
                            {
                                var templateID = args[3];
                                targetMerchant.TradeItems = new MerchantTradeItems(templateID);
                                targetMerchant.SaveIntoDatabase();
                                DisplayMessage(client,
                                    LanguageMgr.GetTranslation(client.Account.Language,
                                        "GMCommands.Merchant.Sell.Add.Loaded"));
                            }
                            catch (Exception)
                            {
                                DisplayMessage(client,
                                    LanguageMgr.GetTranslation(client.Account.Language,
                                        "GMCommands.Merchant.CommandOverview"));
                                return;
                            }
                    ...
                    case "remove":
                    {
                        if (args.Length == 3)
                        {
                            ...
                            DisplayMessage(client,
                                LanguageMgr.GetTranslation(client.Account.Language,
                                    "GMCommands.Merchant.Sell.Remove.Removed"));
                        }
                ...
                switch (args[2].ToLower())
                {
                    ...
                    case "add":
                    {
                        if (args.Length <= 6)
                            ...
                            {
                                var templateID = args[3];
                                var page = Convert.ToInt32(args[4]);
                                var slot = eMerchantWindowSlot.FirstEmptyInPage;

                                if (targetMerchant.TradeItems == null)
                                {
                                    DisplayMessage(client,
                                        LanguageMgr.GetTranslation(client.Account.Language,
                                            "GMCommands.Merchant.Articles.ListNoFound"));
                                    return;
                                }

                                var template =
                                    GameServer.Database.FindObjectByKey<ItemTemplate>(templateID);
                                if (template == null)
                                {
                                    DisplayMessage(client,
                                        LanguageMgr.GetTranslation(client.Account.Language,
                                            "GMCommands.Merchant.Articles.Add.ItemTemplateNoFound", templateID));
                                    return;
                                }

                                if (args.Length == 6) slot = (eMerchantWindowSlot) Convert.ToInt32(args[5]);
                                ...
                                if (slot == eMerchantWindowSlot.Invalid)
                                {
                                    DisplayMessage(client,
                                        LanguageMgr.GetTranslation(client.Account.Language,
                                            "GMCommands.Merchant.Articles.Add.PageAndSlotInvalid", page,
                                            MerchantTradeItems.MAX_PAGES_IN_TRADEWINDOWS - 1, slot,
                                            MerchantTradeItems.MAX_ITEM_IN_TRADEWINDOWS - 1));
                                    return;
                                }

                                var item = DOLDB<MerchantItem>.SelectObject(DB.Column("ItemListID")
                                    .IsEqualTo(targetMerchant.TradeItems.ItemsListID)
                                    .And(DB.Column("PageNumber").IsEqualTo(page))
                                    .And(DB.Column("SlotPosition").IsEqualTo(slot)));
                                if (item == null)
                                {
                                ...
                                DisplayMessage(client,
                                    LanguageMgr.GetTranslation(client.Account.Language,
                                        "GMCommands.Merchant.Articles.Add.ItemAdded"));
                            }
                            catch (Exception)
                            {
                                DisplayMessage(client,
                    ...
                    case "remove":
                    {
                        if (args.Length == 5)
                            try
                            {
                                ...
                                var slot = Convert.ToInt32(args[4]);

                                if (page < 0 || page >= MerchantTradeItems.MAX_PAGES_IN_TRADEWINDOWS)
                                {
                                    DisplayMessage(client,
                                        LanguageMgr.GetTranslation(client.Account.Language,
                                            "GMCommands.Merchant.Articles.Remove.PageInvalid", page,
                                            MerchantTradeItems.MAX_PAGES_IN_TRADEWINDOWS - 1));
                                    return;
                                }
                                ...
                                if (slot < 0 || slot >= MerchantTradeItems.MAX_ITEM_IN_TRADEWINDOWS)
                                {
                                    DisplayMessage(client,
                                        LanguageMgr.GetTranslation(client.Account.Language,
                                            "GMCommands.Merchant.Articles.Remove.SlotInvalid", slot,
                                            MerchantTradeItems.MAX_ITEM_IN_TRADEWINDOWS - 1));
                                    return;
                                }
                                ...
                                if (item == null)
                                {
                                    DisplayMessage(client,
                                        LanguageMgr.GetTranslation(client.Account.Language,
                                            "GMCommands.Merchant.Articles.Remove.SlotInPageIsAEmpty", slot, page));
                                    return;
                                }
                                ...
                                DisplayMessage(client,
                                    LanguageMgr.GetTranslation(client.Account.Language,
                                        "GMCommands.Merchant.Articles.Remove.SlotInPageCleaned", slot, page));
                            }
                            catch (Exception)
                            {
                                DisplayMessage(client,
                    ...
                    case "delete":
                    {
                        if (args.Length == 3)
                            try
                            {
                                ...
                                DisplayMessage(client,
                                    LanguageMgr.GetTranslation(client.Account.Language,
                                        "GMCommands.Merchant.Articles.Delete.DeletingListTemp"));

                                var merchantitems = DOLDB<MerchantItem>.SelectObjects(DB.Column("ItemListID")
                                    .IsEqualTo(targetMerchant.TradeItems.ItemsListID));
                                if (merchantitems.Count > 0) GameServer.Database.DeleteObject(merchantitems);

                                DisplayMessage(client,
                                    LanguageMgr.GetTranslation(client.Account.Language,
                                        "GMCommands.Merchant.Articles.Delete.ListDeleted"));
                            }
                            catch (Exception)
                            {
                                DisplayMessage(client,
            ...
            case "type":
            {
                ...
                targetMerchant.DeleteFromDatabase();
                DisplayMessage(client,
                    LanguageMgr.GetTranslation(client.Account.Language, "GMCommands.Merchant.Type.Changed", param));
                break;
            }
