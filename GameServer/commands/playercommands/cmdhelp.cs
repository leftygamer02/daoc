...
public class CmdHelpCommandHandler : AbstractCommandHandler, ICommandHandler
{
    public void OnCommand(GameClient client, string[] args)
    {
        ...
        var privilegeLevel = (ePrivLevel) client.Account.PrivLevel;
        var isCommand = true;
        ...
        if (isCommand)
        {
            var commandList = GetCommandList(privilegeLevel);
            DisplayMessage(client,
                LanguageMgr.GetTranslation(client.Account.Language, "Scripts.Players.Cmdhelp.PlvlCommands",
                    privilegeLevel.ToString()));
        ...
        else
        {
            ...
            var gameCommand = ScriptMgr.GetCommand(command);

            if (gameCommand == null)
            {
                DisplayMessage(client,
                    LanguageMgr.GetTranslation(client.Account.Language, "Scripts.Players.Cmdhelp.NoCommand",
                        command));
            }
            else
            {
                DisplayMessage(client,
                    LanguageMgr.GetTranslation(client.Account.Language, "Scripts.Players.Cmdhelp.Usage", command));
    ...
    private static object m_syncObject = new();
    ...
    private string[] GetCommandList(ePrivLevel privilegeLevel)
    {
        lock (m_syncObject)
        {
            ...
            {
                var commandList = ScriptMgr.GetCommandList(privilegeLevel, true);
