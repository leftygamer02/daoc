    ...
    public void OnCommand(GameClient client, string[] args)
    {
        ...
        else
        {
            ...
            if (task != null && task.TaskActive)
            {
                ...
                messages.Add("You have done " + task.TasksDone + " tasks out of " +
                             AbstractTask.MaxTasksDone(player.Level) + " until now.");
            ...
            {
                player.Out.SendMessage("You can do no more tasks at your current level", eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);
            }
            else
            {
                player.Out.SendMessage("You have currently no pending task", eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);
            }
        }
    }
