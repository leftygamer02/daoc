...
public class LFGCommandHandler : AbstractCommandHandler, ICommandHandler
{
    ...
    public void OnCommand(GameClient client, string[] args)
    {
        ...
        if (GameLoop.GameLoopTime - lastLFGTick < slowModeLength && client.Account.PrivLevel == 1) // 60 secs
        {
            ...
            ChatUtil.SendSystemMessage(client, "PLCommands.LFG.List.Wait",
                Properties.LFG_SLOWMODE_LENGTH - (GameLoop.GameLoopTime - lastLFGTick) / 1000);
            return;
        }
    ...
    private void Broadcast(GamePlayer player, string message)
    {
        foreach (var c in WorldMgr.GetAllPlayingClients())
            if (player.Realm == c.Player.Realm || c.Account.PrivLevel > 1)
            {
                ...
                c.Out.SendMessage(
                    LanguageMgr.GetTranslation(player.Client.Account.Language, "Scripts.Players.LFG.Message",
                        player.Name + " (" + player.Level + ", " + player.CharacterClass.Name + ")", message),
