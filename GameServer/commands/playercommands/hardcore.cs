...
namespace DOL.GS.GameEvents
{
    public class HardCoreLogin
    {
        ...
        private static void HCPlayerEntered(DOLEvent e, object sender, EventArgs arguments)
        {
            ...
            {
                var cha = DOLDB<DOLCharacters>.SelectObject(DB.Column("Name").IsEqualTo(player.Name));
                if (cha != null)
                {
                    Log.Warn("[HARDCORE] player " + player.Name + " has " + player.DeathCount +
                             " deaths and has been removed from the database.");
...
namespace DOL.GS.Commands
{
    ...
    public class HardcoreCommandHandler : AbstractCommandHandler, ICommandHandler
    {
        public void OnCommand(GameClient client, string[] args)
        {
            ...
            {
                client.Out.SendMessage("Your Hardcore flag is ON! Death will result in the character deletion.",
            ...
            if (args[1].ToLower().Equals("on"))
            {
                ...
                {
                    client.Out.SendMessage("You must be level 1 to activate Hardcore.", eChatType.CT_Important,
                        eChatLoc.CL_SystemWindow);
                    return;
                }

                client.Out.SendCustomDialog(
                    "Do you really want to activate the Hardcore flag? Death will be permanent.",
                    new CustomDialogResponse(HardcoreResponseHandler));
            }
        }
        ...
        protected virtual void HardcoreResponseHandler(GamePlayer player, byte response)
        {
            if (response == 1)
            {
                if (player.Level > 1)
                {
                    player.Out.SendMessage("You must be level 1 to activate Hardcore.", eChatType.CT_Important,
                        eChatLoc.CL_SystemWindow);
                    return;
                }
                ...
                player.HCFlag = true;
                player.Out.SendMessage("Your HARDCORE flag is ON. Your character will be deleted at death.",
                    eChatType.CT_Important, eChatLoc.CL_SystemWindow);
    ...
    public class HardcoreLadderCommandHandler : AbstractCommandHandler, ICommandHandler
    {
        ...
        public void OnCommand(GameClient client, string[] args)
        {
            ...
            var textList = GetHardcoreLadder();
...

            {
                ...
                IList<DOLCharacters> characters = GameServer.Database
                    .SelectObjects<DOLCharacters>(DB.Column("HCFlag").IsEqualTo(1)).OrderByDescending(x => x.Level)
                    .Take(50).ToList();
                ...
                foreach (var c in characters)
                {
                    ...
                    var className = ((eCharacterClass) c.Class).ToString();
                    var isSolo = false;
                    ...
                    var hasGrouped = DOLDB<DOLCharactersXCustomParam>.SelectObject(DB.Column("DOLCharactersObjectId")
                        .IsEqualTo(c.ObjectId).And(DB.Column("KeyName").IsEqualTo(customKey)));

                    if (hasGrouped == null || c.NoHelp) isSolo = true;

                    hcCharacters.Add(new HCCharacter()
                    {
                        CharacterName = c.Name, CharacterLevel = c.Level, CharacterClass = className, isSolo = isSolo
                    });
                }

                var position = 0;

                foreach (var hcCharacter in hcCharacters)
                {
                    position++;
...
namespace DOL.GS.PlayerTitles
{
    public class HardCoreTitle : SimplePlayerTitle
    {
        ...
        {
            player.Out.SendMessage("You have gained the Hardcore title!", eChatType.CT_Important,
                eChatLoc.CL_SystemWindow);
        }
    ...
    public class HardCoreSoloTitle : SimplePlayerTitle
    {
        ...
        public override void OnTitleGained(GamePlayer player)
        {
            player.Out.SendMessage("You have gained the Hardcore Solo Beetle title!", eChatType.CT_Important,
                eChatLoc.CL_SystemWindow);
        }
