...
public class ClassRogsCommandHandler : AbstractCommandHandler, ICommandHandler
{
    public void OnCommand(GameClient client, string[] args)
    {
        var realmLoyalty = DOLDB<AccountXRealmLoyalty>.SelectObject(DB.Column("AccountID")
            .IsEqualTo(client.Account.ObjectId).And(DB.Column("Realm").IsEqualTo(client.Player.Realm)));
        var ROGCap = 0;
        var tmpLoyal = realmLoyalty.LoyalDays > 30 ? 30 : realmLoyalty.LoyalDays;
        var cachedInput = 0;
        ...
        if (client.Player.Level == 50)
        {
            DisplayMessage(client,
                "You will now receive out of class ROGs " + client.Player.OutOfClassROGPercent + "% of the time.");
        }
        else
        {
