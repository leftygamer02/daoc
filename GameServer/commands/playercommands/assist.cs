...
//				return;
        ...
        if (args.Length > 1)
        {
                ...
                switch (GameServer.Instance.Configuration.ServerType)
                {
                    ...
                    case eGameServerType.GST_PvP:
                    {
                        ...
                        var clientBattleGroup =
                            client.Player.TempProperties.getProperty<BattleGroup>(BattleGroup.BATTLEGROUP_PROPERTY,
                                null);
                        ...
                        var clientChatGroup =
                            client.Player.TempProperties.getProperty<ChatGroup>(ChatGroup.CHATGROUP_PROPERTY, null);
            ...
            client.Out.SendMessage(
                LanguageMgr.GetTranslation(client.Account.Language, "Scripts.Players.Assist.MemberNotFound"),
        ...
        if (client.Player.TargetObject != null)
        {
            ...
            if (client.Player.TargetObject is GameNPC || client.Player.TargetObject is GamePlayer)
            {
                ...
                switch (GameServer.Instance.Configuration.ServerType)
                {
                    ...
                    case eGameServerType.GST_Normal:
                    {
                        var targetLiving = (GameLiving) client.Player.TargetObject;
                    ...
                    case eGameServerType.GST_PvE:
                    {
                        if (client.Player.TargetObject is GamePlayer)
                        {
                            ...
                            YouAssist(client, client.Player.TargetObject.Name,
                                (client.Player.TargetObject as GameLiving).TargetObject);
                            return;
                        }
                        ...
                        if (client.Player.TargetObject is GameNPC)
                        {
                            ...
                            else
                            {
                                ...
                                YouAssist(client, client.Player.TargetObject.GetName(0, true),
                                    (client.Player.TargetObject as GameLiving).TargetObject);
                                return;
                            }
                        }
                    ...
                    case eGameServerType.GST_PvP:
                    {
                        ...
                        if (client.Player.TargetObject is GamePlayer)
                        {
                            ...
                            //Lets check if the client and it's targeted player are in the same alliance.
                            if (client.Player.Guild != null && client.Player.Guild.alliance != null &&
                                client.Player.Guild.alliance.Contains(targetPlayer.Guild))
                            {
                                //We cannot assist our target when it has no target
                            ...
                            //They are no alliance members, maybe guild members?
                            if (client.Player.Guild != null &&
                                client.Player.Guild.GetOnlineMemberByID(targetPlayer.InternalID) != null)
                            {
                                //We cannot assist our target when it has no target
                                if (!HasTarget(client, targetPlayer))
                                    return;
                            ...
                            //Ok, they are not in the same alliance, guild or group - maybe in the same battle group?
                            var clientBattleGroup =
                                (BattleGroup) client.Player.TempProperties.getProperty<object>(
                            ...
                            //Ok, they are not in the same alliance, guild, group or battle group - maybe in the same chat group?
                            var clientChatGroup =
                                client.Player.TempProperties.getProperty<ChatGroup>(ChatGroup.CHATGROUP_PROPERTY,
                                    null);
                            if (clientChatGroup != null)
                        ...
                        if (client.Player.TargetObject is GameNPC)
                        {
                            ...
                            {
                                var targetPet = client.Player.TargetObject as GamePet;

                                if (targetPet.Owner is GamePlayer)
                                {
                                    var targetPlayer = targetPet.Owner as GamePlayer;
                                    ...
                                    if (client.Player.Group != null &&
                                        client.Player.Group.IsInTheGroup(targetPlayer))
                                    {
                                        //We cannot assist our target when it has no target
                                        if (!HasTarget(client, targetPet))
                                            return;
                                    ...
                                    //Ok, they are not in the same alliance, guild or group - maybe in the same battle group?
                                    var clientBattleGroup =
                                        client.Player.TempProperties.getProperty<BattleGroup>(
                                            BattleGroup.BATTLEGROUP_PROPERTY, null);
                                    if (clientBattleGroup != null)
                                        if (clientBattleGroup.Members.Contains(targetPlayer))
                                        {
                                    ...
                                    //Ok, they are not in the same alliance, guild, group or battle group - maybe in the same chat group?
                                    var clientChatGroup =
                                        client.Player.TempProperties.getProperty<ChatGroup>(
                            ...
                            if (client.Player.TargetObject is GameKeepGuard)
                            {
                                ...
                                var targetGuard = client.Player.TargetObject as GameKeepGuard;
                                var targetedGuardGuild = GuildMgr.GetGuildByName(targetGuard.GuildName);
                            ...
                            YouAssist(client, (client.Player.TargetObject as GameNPC).GetName(0, false),
                                (client.Player.TargetObject as GameNPC).TargetObject);
                            return;
                        }
                    }
                        break;
        ...
        client.Out.SendMessage(
            LanguageMgr.GetTranslation(client.Account.Language, "Scripts.Players.Assist.SelectMember"),
            eChatType.CT_System, eChatLoc.CL_SystemWindow);
        return;
    }
    ...
    private bool HasTarget(GameClient client, GameLiving livingToCheck)
    {
        ...
        //We cannot assist our target when it has no target.
        client.Out.SendMessage(
            LanguageMgr.GetTranslation(client.Account.Language, "Scripts.Players.Assist.DoesntHaveTarget",
                livingToCheck.GetName(0, true)), eChatType.CT_System, eChatLoc.CL_SystemWindow);
        return false;
    }
    ...
    private void NoValidTarget(GameClient client, GameLiving livingToAssist)
    {
        ...
        client.Out.SendMessage(
            LanguageMgr.GetTranslation(client.Account.Language, "Scripts.Players.Assist.NotValid",
                livingToAssist.GetName(0, true)), eChatType.CT_System, eChatLoc.CL_SystemWindow);
        return;
    }
    ...
    private bool SameRealm(GameClient client, GameLiving livingToCheck, bool usePvEPvPRule)
    {
        ...
        client.Out.SendMessage(
            LanguageMgr.GetTranslation(client.Account.Language, "Scripts.Players.Assist.NoRealmMember",
                livingToCheck.GetName(0, true)), eChatType.CT_System, eChatLoc.CL_SystemWindow);
        return false;
    }
    ...
    {
        client.Out.SendMessage(
            LanguageMgr.GetTranslation(client.Account.Language, "Scripts.Players.Assist.YouAssist", targetName),
            eChatType.CT_System, eChatLoc.CL_SystemWindow);
