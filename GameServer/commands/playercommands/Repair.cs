...
public class RepairCommandHandler : AbstractCommandHandler, ICommandHandler
{
    ...
    {
        "rowan", "elm", "oak", "oaken", "ironwood", "heartwood", "runewood", "stonewood", "ebonwood", "dyrwood",
        "duskwood"
    };
    ...
    public void OnCommand(GameClient client, string[] args)
    {
        ...
        var item = client.Player.TargetObject as WorldInventoryItem;
        if (item != null)
        {
        ...
        var door = client.Player.TargetObject as GameKeepDoor;
        if (door != null)
        {
        ...
        var weapon = client.Player.TargetObject as GameSiegeWeapon;
        if (weapon != null)
        {
    ...
    public bool PreFireChecks(GamePlayer player, GameLiving obj)
    {
        ...
        {
            var component = obj as GameKeepComponent;
        ...
        {
            var siegeweapon = obj as GameSiegeWeapon;
        ...
        var repairamount = GetNeededWoodForOneTick(obj.Level);
        var playerswood = CalculatePlayersWood(player);
        var woodDifference = repairamount - playerswood;

        if (woodDifference > 0)
        {
            DisplayMessage(player,
                "You need another " + woodDifference + " unit" + (woodDifference > 1 ? "s" : "") + " of wood!");
            return false;
        }
    ...
    {
        var player = (GamePlayer) timer.Properties.getProperty<object>("repair_player", null);
        var obj = (GameLiving) timer.Properties.getProperty<object>("repair_target", null);
        ...
        if (Util.ChanceDouble(CalculateRepairChance(player, obj)))
        {
            ...
            {
                var weapon = obj as GameSiegeWeapon;
    ...
    private static double CalculateRepairChance(GamePlayer player, GameObject obj)
    {
        ...
        var skillneeded = (obj.Level + 1) * 50;
        var chance = skill / skillneeded;
        return chance;
    }
    ...
    private static int CalculatePlayersWood(GamePlayer player)
    {
        ...
        foreach (var name in woodNames)
        {
            if (item.Name.Replace(" wooden boards", "").ToLower() != name)
                continue;

            var woodvalue = GetWoodValue(item.Name.ToLower());
