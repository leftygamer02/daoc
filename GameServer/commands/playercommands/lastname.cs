...
public class LastnameCommandHandler : AbstractCommandHandler, ICommandHandler
{
    ...
    public void OnCommand(GameClient client, string[] args)
    {
        ...
        if (client.Player.Level < LASTNAME_MIN_LEVEL && CraftSkill < LASTNAME_MIN_CRAFTSKILL)
        {
            client.Out.SendMessage(
                "You must be " + LASTNAME_MIN_LEVEL + "th level or " + LASTNAME_MIN_CRAFTSKILL +
                " in your primary trade skill to register a last name!", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            return;
        }

        /* When you don't have a lastname, change is for free, otherwise you need money */
        if (client.Player.LastName != "" &&
            client.Player.GetCurrentMoney() < Money.GetMoney(0, 0, LASTNAME_FEE, 0, 0))
        {
            client.Out.SendMessage(
                "Changing your last name costs " + Money.GetString(Money.GetMoney(0, 0, LASTNAME_FEE, 0, 0)) + "!",
        ...
        {
            client.Out.SendMessage("You must select a name registrar to set your last name with!",
                eChatType.CT_System, eChatLoc.CL_SystemWindow);
            return;
        }
        ...
        {
            client.Out.SendMessage("You are too far away to interact with " + client.Player.TargetObject.Name + ".",
                eChatType.CT_System, eChatLoc.CL_SystemWindow);
            return;
        }
        ...
        if (args.Length < 2)
        {
            ...
            client.Out.SendCustomDialog("Would you like to clear your last name?",
                new CustomDialogResponse(LastNameDialogResponse));
            return;
        }

        /* Get the name */
        var NewLastname = args[1];
        /* Check to ensure that lastnames do not exeed maximum length */
        if (NewLastname.Length > LASTNAME_MAXLENGTH)
        {
            client.Out.SendMessage("Last names can be no longer than " + LASTNAME_MAXLENGTH + " characters!",
                eChatType.CT_System, eChatLoc.CL_SystemWindow);
            return;
        }
        ...
        {
            client.Out.SendMessage("Your lastname must start with a valid, uppercase character!",
                eChatType.CT_System, eChatLoc.CL_SystemWindow);
            return;
        }
        ...
        {
            client.Out.SendMessage("Your lastname must consist of valid characters!", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            return;
        }
        ...
        {
            client.Out.SendMessage(NewLastname + " is not a legal last name! Choose another.", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            return;
        }

        client.Player.TempProperties.setProperty(LASTNAME_WEAK, NewLastname);
        client.Out.SendCustomDialog("Would you like to set your last name to \x000a" + NewLastname + "?",
            new CustomDialogResponse(LastNameDialogResponse));
    ...
    protected void LastNameDialogResponse(GamePlayer player, byte response)
    {
        ...
        {
            player.Out.SendMessage("You must select a name registrar to set your last name with!",
                eChatType.CT_System, eChatLoc.CL_SystemWindow);
            return;
        }
        ...
        {
            player.Out.SendMessage("You are too far away to interact with " + player.TargetObject.Name + ".",
                eChatType.CT_System, eChatLoc.CL_SystemWindow);
            return;
        }
        ...
        {
            player.Out.SendMessage(
                "You decline to " + (NewLastName != "" ? "take " + NewLastName + " as" : "clear") +
                " your last name.", eChatType.CT_System, eChatLoc.CL_SystemWindow);
            return;
        }
        ...
        if (player.LastName != "" && player.LastName != NewLastName)
        {
            ...
            InventoryLogging.LogInventoryAction(player, player.TargetObject, eInventoryActionType.Merchant,
                LASTNAME_FEE * 10000);
        }
        ...
        player.Out.SendMessage(
            "Your last name has been " + (NewLastName != "" ? "set to " + NewLastName : "cleared") + "!",
            eChatType.CT_System, eChatLoc.CL_SystemWindow);
    }
}