...
public class SayCommandHandler : AbstractCommandHandler, ICommandHandler
{
    public void OnCommand(GameClient client, string[] args)
    {
        ...
        var SayTick = client.Player.TempProperties.getProperty<long>(SAY_TICK);
        if (SayTick > 0 && SayTick - client.Player.CurrentRegion.Time <= 0)
            client.Player.TempProperties.removeProperty(SAY_TICK);

        var changeTime = client.Player.CurrentRegion.Time - SayTick;
        ...
        {
            client.Player.Out.SendMessage("You have been muted. You cannot talk.", eChatType.CT_Staff,
                eChatLoc.CL_SystemWindow);
            return;
        }
