...
public class WebDisplayCommandHandler : AbstractCommandHandler, ICommandHandler
{
    ...
    public void OnCommand(GameClient client, string[] args)
    {
        ...
        if (args[1].ToLower() == "position")
            WdChange(GlobalConstants.eWebDisplay.position, client.Player,
                args.Length == 3 ? args[2].ToLower() : null);
        if (args[1].ToLower() == "equipment")
            WdChange(GlobalConstants.eWebDisplay.equipment, client.Player,
                args.Length == 3 ? args[2].ToLower() : null);
        if (args[1].ToLower() == "template")
            WdChange(GlobalConstants.eWebDisplay.template, client.Player,
                args.Length == 3 ? args[2].ToLower() : null);
    ...
    {
        var webDisplay = client.Player.NotDisplayedInHerald;
        byte webDisplayFlag;

        var state = "/webdisplay <position|template|equipment|craft> [on|off]\n";
