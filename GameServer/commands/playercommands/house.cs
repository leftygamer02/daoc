...
public class HouseCommandHandler : AbstractCommandHandler, ICommandHandler
{
        ...
        try
        {
            ...
            if (house != null)
            {
                ...
                if (house.RegionID == client.Player.CurrentRegionID && client.Player.InHouse == false)
                {
                    ...
                    if (house.IsOccupied) client.Out.SendHouseOccupied(house, true);
                }
            ...
            if (client.Player.Guild != null && client.Player.Guild.GuildOwnsHouse &&
                client.Player.Guild.GuildHouseNumber > 0)
            {
                var guildHouse = HouseMgr.GetHouse(client.Player.Guild.GuildHouseNumber);
    ...
    public void HouseAdmin(GamePlayer player, string[] args)
    {
        ...
        var houses =
            (ArrayList) HouseMgr.GetHousesCloseToSpot(player.CurrentRegionID, player.X, player.Y, 700);
        if (houses.Count != 1)
        {
        ...
        {
            var newModel = Convert.ToInt32(args[2]);
            ...
            if (houses.Count == 1 && newModel != (houses[0] as House).Model)
            {
                ...
                GameServer.Instance.LogGMAction(player.Name + " changed house #" +
                                                (houses[0] as House).HouseNumber + " model to " + newModel);
            }
        ...
        if (args[1].ToLower() == "remove")
        {
            ...
            if (houses.Count == 1)
            {
                ...
                GameServer.Instance.LogGMAction(player.Name + " removed house #" +
                                                (houses[0] as House).HouseNumber);
            }
