...
public class WhereCommandHandler : AbstractCommandHandler, ICommandHandler
{
    public void OnCommand(GameClient client, string[] args)
    {
        ...
        if (targetnpc != null && CheckTargetIsGuard(targetnpc))
        {
            ...
            var npcs = WorldMgr.GetNPCsByNameFromRegion(name, client.Player.CurrentRegionID,
                (eRealm) client.Player.Realm);
            ...
            var npc = npcs[0];
            var heading = targetnpc.GetHeading(npc);
            var directionstring = GetDirectionFromHeading(heading);
    ...
    public bool CheckTargetIsGuard(GameLiving target)
    {
        ...
        if (name.StartsWith("Sir ") && (target.GuildName == null || target.GuildName == "")) return true;

        if (name.StartsWith("Captain ") && (target.GuildName == null || target.GuildName == "")) return true;

        if (name.StartsWith("Jarl ")) return true;

        if (name.StartsWith("Lady ") && (target.GuildName == null || target.GuildName == "")) return true;
