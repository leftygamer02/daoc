...
public class SellCommandHandler : AbstractCommandHandler, ICommandHandler
{
    public void OnCommand(GameClient client, string[] args)
    {
        ...
        if (args.Length >= 2)
        {
            ...
            else if (int.TryParse(args[1], out var bag))
            {
                switch (bag)
                {
                    case 1:
        ...
        if (client.Player is GamePlayer player && player.Inventory != null && args.Length >= 2)
        {
            if (player.TargetObject is GameMerchant merchant)
            {
                ...
                for (var i = firstItem; i <= lastItem; i++)
                {
                    ...
                    if (item != null)
                    {
                        if (item is {PackageID: "AtlasXPItem"} or {PackageID: "atlas_orbs_item"} or
                            {PackageID: "atlas_potion"}) continue;
            ...
            {
                client.Out.SendMessage("You must target a merchant.", eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);
            }
        }
        else
        {
            client.Out.SendMessage("Use: /sell <bag>, /sell <bag1-bag2>, /sell all", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
        }
    }
}