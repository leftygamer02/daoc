...
public class AdviceCommandHandler : AbstractCommandHandler, ICommandHandler
{
    ...
    public void OnCommand(GameClient client, string[] args)
    {
        ...
        if (GameLoop.GameLoopTime - lastAdviceTick < slowModeLength && client.Account.PrivLevel == 1) // 60 secs
        {
            ...
            ChatUtil.SendSystemMessage(client, "PLCommands.Advice.List.Wait",
                Properties.ADVICE_SLOWMODE_LENGTH - (GameLoop.GameLoopTime - lastAdviceTick) / 1000);
            return;
        }
        ...
        {
            var total = 0;
            var showPlayed = TimeSpan.FromSeconds(client.Player.PlayedTime);
            ...
            foreach (var playerClient in WorldMgr.GetAllClients())
            {
                if (playerClient.Player == null) continue;
                if (playerClient.Player.Advisor &&
                    ((playerClient.Player.Realm == client.Player.Realm &&
                      playerClient.Player.IsAnonymous == false) ||
                     client.Account.PrivLevel > 1))
                {
                    total++;
                    if (playerClient.Player.ClassNameFlag == false && playerClient.Player.CraftTitle
                            .GetValue(playerClient.Player, client.Player).StartsWith("Legendary"))
                        ...
                        ChatUtil.SendSystemMessage(client, "PLCommands.Advice.List.Result", total,
                            playerClient.Player.Name, playerClient.Player.Level,
                            playerClient.Player.CraftTitle.GetValue(playerClient.Player, client.Player),
                            showPlayed.Days, showPlayed.Hours, showPlayed.Minutes);
                    else
                        // Message: "{0}) {1}, Level {2} {3} ({4} days, {5} hours, {6} minutes played)"
                        // Example: 1) Kelt, Level 50 Bard (14 days, 3 hours, 31 minutes played)
                        ChatUtil.SendSystemMessage(client, "PLCommands.Advice.List.Result", total,
                            playerClient.Player.Name, playerClient.Player.Level,
                            playerClient.Player.CharacterClass.Name, showPlayed.Days, showPlayed.Hours,
                            showPlayed.Minutes);
                }
            }
        ...
        foreach (var playerClient in WorldMgr.GetAllClients())
        {
            ...
            if (playerClient.Player.Realm == client.Player.Realm ||
                ...
                ChatUtil.SendAdviceMessage(playerClient, "Social.SendAdvice.Msg.Channel",
                    getRealmString(client.Player.Realm), client.Player.Name, msg);
        }
