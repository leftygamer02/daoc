...
public class RollCommandHandler : AbstractCommandHandler, ICommandHandler
{
    ...
    private const string
        MESSAGE_RESULT_OTHER = "{0} rolls {1} dice and comes up with: {2}"; // client.Player.Name, dice, thrown
    ...
    public void OnCommand(GameClient client, string[] args)
    {
        ...
        // throw result
        var thrown = Util.Random(dice, dice * ONE_DIE_MAX_VALUE);

        // building roll result msg
        var selfMessage = string.Format(MESSAGE_RESULT_SELF, dice, thrown);
        var otherMessage = string.Format(MESSAGE_RESULT_OTHER, client.Player.Name, dice, thrown);
