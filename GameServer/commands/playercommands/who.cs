...
public class WhoCommandHandler : AbstractCommandHandler, ICommandHandler
{
    ...
    public void OnCommand(GameClient client, string[] args)
    {
        ...
        var resultMessages = new ArrayList();

        // get list of clients depending on server type
        foreach (var serverClient in WorldMgr.GetAllPlayingClients())
        {
            var addPlayer = serverClient.Player;
            if (addPlayer == null) continue;
            if (serverClient.Account.PrivLevel > (uint) ePrivLevel.Player &&
                serverClient.Player.IsAnonymous == false)
            {
        ...
        foreach (GameClient clients in clientsList)
            if (ApplyFilter(filters, clients.Player))
            {
                ...
                if (resultMessages.Count < MAX_LIST_SIZE && resultCount >= listStart)
                    resultMessages.Add(resultCount + ") " +
                                       FormatLine(clients.Player, client.Account.PrivLevel, client));
            }

        foreach (string str in resultMessages) DisplayMessage(client, str);
    ...
    private string FormatLine(GamePlayer player, uint PrivLevel, GameClient source)
    {
        ...
        var result = new StringBuilder(player.Name, 100);
        if (player.GuildName != "")
        {
            result.Append(" <");
        ...
        if (player.CurrentZone != null && GameServer.Instance.Configuration.ServerType != eGameServerType.GST_PvP)
        {
            ...
            if ((source.Account.PrivLevel == (uint) ePrivLevel.Player &&
                 player.Client.Account.PrivLevel == (uint) ePrivLevel.Player) ||
                source.Account.PrivLevel == (uint) ePrivLevel.Admin)
            {
                result.Append(" in ");
                ...
                if (source.Account.PrivLevel == (uint) ePrivLevel.Player && player.CurrentZone.IsRvR &&
                    player.CurrentRegion.ID is 1 or 100 or 200)
        ...
        var mychatgroup =
            (ChatGroup) player.TempProperties.getProperty<object>(ChatGroup.CHATGROUP_PROPERTY, null);
        if (mychatgroup != null && (mychatgroup.Members.Contains(player) ||
                                    (mychatgroup.IsPublic && (bool) mychatgroup.Members[player] == true)))
            result.Append(" [CG]");

        var mybattlegroup =
            (BattleGroup) player.TempProperties.getProperty<object>(BattleGroup.BATTLEGROUP_PROPERTY, null);
        if (mybattlegroup != null && (mybattlegroup.Members.Contains(player) ||
                                      (mybattlegroup.IsPublic && (bool) mybattlegroup.Members[player] == true)))
            result.Append(" [BG]");

        if (player.IsAnonymous) result.Append(" <ANON>");

        if (player.TempProperties.getProperty<string>(GamePlayer.AFK_MESSAGE) != null) result.Append(" <AFK>");

        if (player.Advisor) result.Append(" <ADV>");

        if (player.HCFlag) result.Append(" <HC>");

        if (player.NoHelp) result.Append(" <SOLO>");

        if (player.Client.Account.PrivLevel == (uint) ePrivLevel.GM) result.Append(" <GM>");

        if (player.Client.Account.PrivLevel == (uint) ePrivLevel.Admin) result.Append(" <Admin>");
    ...
    private void AddFilters(ArrayList filters, string[] args, int skip)
    {
        for (var i = skip; i < args.Length; i++)
            if (GameServer.Instance.Configuration.ServerType == eGameServerType.GST_PvP)
                ...
                try
                {
                    var currentNum = (int) Convert.ToUInt32(args[i]);
                    var nextNum = -1;
                    try
                    {
    ...
    private class SoloFilter : IWhoFilter
    {
        public bool ApplyFilter(GamePlayer player)
        {
            if (player.Group == null) return true;
