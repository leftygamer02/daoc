...
public class ViewReportsCommandHandler : AbstractCommandHandler, ICommandHandler
{
            ...
            switch (args[1])
            {
                case "close":
                {
                    if (client.Account.PrivLevel < 2)
                    {
                        client.Player.Out.SendMessage(
                            LanguageMgr.GetTranslation(client.Account.Language, "Scripts.Player.ViewReport.NoPriv"),
                    ...
                    if (args[2] == "")
                    {
                        client.Player.Out.SendMessage(
                            LanguageMgr.GetTranslation(client.Account.Language,
                                "Scripts.Player.ViewReport.Help.Close"), eChatType.CT_System,
                            eChatLoc.CL_SystemWindow);
                        break;
                    }

                    var repor = int.Parse(args[2]);
                    var report = GameServer.Database.FindObjectByKey<BugReport>(repor);
                    if (report == null)
                    {
                        client.Player.Out.SendMessage(
                            LanguageMgr.GetTranslation(client.Account.Language,
                                "Scripts.Player.ViewReport.InvalidReport"), eChatType.CT_System,
                            eChatLoc.CL_SystemWindow);
                        break;
                    }
                ...
                case "delete":
                {
                    ...
                    if (args[2] == "")
                    {
                        client.Player.Out.SendMessage(
                            LanguageMgr.GetTranslation(client.Account.Language,
                                "Scripts.Player.ViewReport.Help.Delete"), eChatType.CT_System,
                            eChatLoc.CL_SystemWindow);
                        break;
                    }
                    ...
                    foreach (var curReport in bugReports)
                    {
                    ...
                    client.Player.Out.SendMessage(
                        LanguageMgr.GetTranslation(client.Account.Language,
                            "Scripts.Player.ViewReport.ReportDeleted", report.ID), eChatType.CT_System,
                        eChatLoc.CL_SystemWindow);
                    break;
                }
                default:
                {
                    client.Player.Out.SendMessage(
                        LanguageMgr.GetTranslation(client.Account.Language,
                            "Scripts.Player.ViewReport.UnknownCommand"), eChatType.CT_System,
                        eChatLoc.CL_SystemWindow);
                    DisplayHelp(client);
                }
                    break;
    ...
    {
        client.Player.Out.SendMessage(
            LanguageMgr.GetTranslation(client.Account.Language, "Scripts.Player.ViewReport.Usage"),
            eChatType.CT_System, eChatLoc.CL_SystemWindow);
        client.Player.Out.SendMessage(
            LanguageMgr.GetTranslation(client.Account.Language, "Scripts.Player.ViewReport.Help.Close"),
            eChatType.CT_System, eChatLoc.CL_SystemWindow);
        client.Player.Out.SendMessage(
            LanguageMgr.GetTranslation(client.Account.Language, "Scripts.Player.ViewReport.Help.Delete"),
            eChatType.CT_System, eChatLoc.CL_SystemWindow);
    }
}