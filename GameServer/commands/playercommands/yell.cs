...
public class YellCommandHandler : AbstractCommandHandler, ICommandHandler
{
    public void OnCommand(GameClient client, string[] args)
    {
        ...
        var YELLTick = client.Player.TempProperties.getProperty<long>(YELL_TICK);
        if (YELLTick > 0 && YELLTick - client.Player.CurrentRegion.Time <= 0)
            client.Player.TempProperties.removeProperty(YELL_TICK);

        var changeTime = client.Player.CurrentRegion.Time - YELLTick;
        ...
        {
            client.Player.Out.SendMessage("You have been muted. You cannot yell.", eChatType.CT_Staff,
        ...
        if (args.Length < 2)
        {
            foreach (GamePlayer player in client.Player.GetPlayersInRadius(WorldMgr.YELL_DISTANCE))
                ...
                {
                    var headingtemp = player.GetHeading(client.Player);
                    var headingtotarget = (ushort) (headingtemp - player.Heading);
                    var direction = "";
                    ...
                    player.Out.SendMessage(client.Player.Name + " yells for help from the " + direction + "!",
