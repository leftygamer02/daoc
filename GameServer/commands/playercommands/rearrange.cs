...
public class RearrangeCommandHandler : AbstractCommandHandler, ICommandHandler
{
    ...
    {
        client.Out.SendMessage("The given source slot (" + slot + ") is empty.", eChatType.CT_System,
    ...
    private void InvalidSlot(GameClient client, int[] slots)
    {
        ...
        client.Out.SendMessage("Invalid character slot" + (slots.Length > 1 ? "s" : "") + ": " + str,
            eChatType.CT_System, eChatLoc.CL_SystemWindow);
    }

    private void NotSameRealm(GameClient client, int sourceSlot, int targetSlot)
    {
        client.Out.SendMessage("You cannot set a slot to a different realm! (source realm = " +
                               GetRealmBySlotIndex(sourceSlot) +
                               ", target realm = " + GetRealmBySlotIndex(targetSlot) + ")", eChatType.CT_System,
            eChatLoc.CL_SystemWindow);
    }

    private void SlotChanged(GameClient client, string name, int oldSlot, int newSlot)
    {
        client.Out.SendMessage("The character slot for " + name + " has been successfully changed. (old slot = " +
                               oldSlot +
    ...
    private void SendCharacterListWindow(GameClient client)
    {
        ...
        foreach (var firstSlot in m_firstCharacterSlotByRealm)
        {
            switch (firstSlot)
            {
                case 100:
            ...
            for (var i = firstSlot; i <= firstSlot + 9; i++)
            {
    ...
    private void SetSlot(GameClient client, int sourceSlot, int targetSlot)
    {
        ...
        var sourceBackup = new DOLCharactersBackup(source);
