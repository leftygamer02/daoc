...
public class TradeChannelCommandHandler : AbstractCommandHandler, ICommandHandler
{
    ...
    public void OnCommand(GameClient client, string[] args)
    {
        ...
        if (GameLoop.GameLoopTime - lastTradeTick < slowModeLength && client.Account.PrivLevel == 1) // 60 secs
        {
            ...
            ChatUtil.SendSystemMessage(client, "PLCommands.Trade.List.Wait",
                Properties.TRADE_SLOWMODE_LENGTH - (GameLoop.GameLoopTime - lastTradeTick) / 1000);
            return;
        }
    ...
    {
        foreach (var c in WorldMgr.GetClientsOfRealm(player.Realm))
