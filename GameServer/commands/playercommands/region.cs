...
public class RegionCommandHandler : AbstractCommandHandler, ICommandHandler
{
    ...
    {
        foreach (var c in WorldMgr.GetClientsOfRegion(player.CurrentRegionID))
            if (GameServer.ServerRules.IsAllowedToUnderstand(c.Player, player))
                c.Out.SendMessage(
                    LanguageMgr.GetTranslation(player.Client.Account.Language, "Scripts.Players.Region.Message",
