    ...
    public class NoHelpCommandHandler : AbstractCommandHandler, ICommandHandler
    {
        public void OnCommand(GameClient client, string[] args)
        {
            ...
            if (!client.Player.NoHelp)
            {
                ...
                var d1 = new DateTime(2022, 1, 4);
                var d2 = new DateTime(2022, 1, 7, 21, 0, 0);

                if (client.Player.Level == 1 || (hasGrouped == null && client.Player.CreationDate >= d1 &&
                                                 client.Player.CreationDate <= d2))
            ...
            public void OnCommand(GameClient client, string[] args)
            {
...

                {
                    ...
                    IList<DOLCharacters> characters = GameServer.Database
                        .SelectObjects<DOLCharacters>(DB.Column("NoHelp").IsEqualTo(1))
                    ...
                    foreach (var c in characters)
                    {
                        ...
                        {
                            CharacterName = c.Name, CharacterLevel = c.Level, CharacterClass = className,
                            isHC = c.HCFlag
                        });
                    }
                    ...
                    foreach (var soloCharacter in soloCharacters)
                    {
                        position++;
