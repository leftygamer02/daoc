    ...
    public void OnCommand(GameClient client, string[] args)
    {
        ...
        if (args[1].ToLower() == "specs")
        {
            ...
            client.Out.SendMessage(
                "API specialization details: " + (client.Player.HideSpecializationAPI ? "hidden" : "shown"),
                eChatType.CT_System, eChatLoc.CL_SystemWindow);
            GameServer.Database.SaveObject(client.Player
                .DBCharacter); // using this instead of SaveIntoDatabase() because we don't want to display it to the player to avoid save abuse
        }
    }
}