...
public class AppealCommandHandler : AbstractCommandHandler, ICommandHandler
{
    public void OnCommand(GameClient client, string[] args)
    {
        ...
        if (client.Player.IsMuted) return;
        ...
        if (args.Length == 1)
        {
            ...
            if (client.Account.PrivLevel > (uint) ePrivLevel.Player)
                AppealMgr.MessageToClient(client,
                    LanguageMgr.GetTranslation(client.Account.Language, "Scripts.Players.Appeal.UseGMappeal"));
        }
        ...
        {
            var cch = new CheckAppealCommandHandler();
        ...
        if (args.Length > 1)
        {
            ...
            if (HasPendingAppeal)
            {
                AppealMgr.MessageToClient(client,
                    LanguageMgr.GetTranslation(client.Account.Language,
                        "Scripts.Players.Appeal.AlreadyActiveAppeal", client.Player.Name));
                return;
            }
            ...
            {
                AppealMgr.MessageToClient(client,
                    LanguageMgr.GetTranslation(client.Account.Language, "Scripts.Players.Appeal.NeedMoreDetail"));
                return;
            }
            ...
            var p = client.Player as GamePlayer;
...
public class ReportBugCommandHandler : AbstractCommandHandler, ICommandHandler
{
    public void OnCommand(GameClient client, string[] args)
    {
        ...
        var report = new ReportCommandHandler();
...
public class ReportHarassCommandHandler : AbstractCommandHandler, ICommandHandler
{
    public void OnCommand(GameClient client, string[] args)
    {
        ...
        {
            AppealMgr.MessageToClient(client,
                LanguageMgr.GetTranslation(client.Account.Language, "Scripts.Players.Appeal.AlreadyActiveAppeal",
                    client.Player.Name));
            return;
        }
