...
public class EmoteCommandHandler : AbstractCommandHandler, ICommandHandler
{
    ...
    public void OnCommand(GameClient client, string[] args)
    {
        ...
        {
            client.Player.Out.SendMessage("You have been muted and cannot emote!", eChatType.CT_Staff,
        ...
        var Tick = client.Player.TempProperties.getProperty<long>(EMOTE_TICK);
        if (Tick > 0 && client.Player.CurrentRegion.Time - Tick <= 0) //
            client.Player.TempProperties.removeProperty(EMOTE_TICK);

        var changeTime = client.Player.CurrentRegion.Time - Tick;
        if (changeTime < ServerProperties.Properties.EMOTE_DELAY && Tick > 0)
        {
            var message = LanguageMgr.GetTranslation(client.Account.Language,
                "Scripts.Players.Emotes.Message" + Util.Random(1, 4).ToString());
    ...
    private void SendEmote(GamePlayer sourcePlayer, GameObject targetObject, eEmote emoteID, string[] emoteMessages)
    {
        ...
        else
        {
            ...
            messageToOthers = string.Format(emoteMessages[EMOTE_TO_OTHERS], sourcePlayer.Name,
                targetObject.GetName(0, false));
        ...
        string messageToTarget, string messageToOthers)
    {
        ...
        foreach (GamePlayer player in sourcePlayer.GetPlayersInRadius(EMOTE_RANGE_TO_OTHERS))
            if (player != sourcePlayer && player != targetPlayer &&
                !player.IsIgnoring(sourcePlayer)) // client and target gets unique messages
    ...
    private const byte
        EMOTE_TO_OTHERS = 3; // target selected; send to others; arg0 is client.name; arg1 is target.name
