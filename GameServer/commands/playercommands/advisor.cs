...
public class AdvisorCommandHandler : AbstractCommandHandler, ICommandHandler
{
    public void OnCommand(GameClient client, string[] args)
    {
        ...
        var craftingSkill =
            client.Player.GetCraftingSkillValue(client.Player.CraftingPrimarySkill) >=
            1000; // Identify primary crafting skill
        ...
        switch (args.Length)
        {
            ...
            case 1:
            {
                ...
                if ((client.Player.Level == 50 && played >= 15) ||
                    (craftingSkill && totalPlayed >= 15 && !client.Player.ClassNameFlag) ||
            ...
            case >= 3:
            {
                ...
                var name = !string.IsNullOrWhiteSpace(advisorName) && char.IsLower(advisorName, 0)
                    ? advisorName.Replace(advisorName[0], char.ToUpper(advisorName[0]))
                    : advisorName; // If first character in args[1] is lowercase, replace with uppercase character
                ...
                var advisorClient =
                    WorldMgr.GuessClientByPlayerNameAndRealm(name, 0, false, out result); // Figure out advisor name

                // Players cannot communicate with members of another realm
                if (advisorClient != null &&
                    !GameServer.ServerRules.IsAllowedToUnderstand(client.Player, advisorClient.Player))
                    advisorClient = null;
                    ...
                    advisorClient.Account.PrivLevel > (uint) ePrivLevel.Player)
                {
                    if (client.Account.PrivLevel == (uint) ePrivLevel.Player)
                    {
                        ...
                        ChatUtil.SendSendMessage(advisorClient.Player, "Social.ReceiveMessage.Advisor.StaffAnon",
