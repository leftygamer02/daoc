...
public class CloakCommandHandler : AbstractCommandHandler, ICommandHandler
{
    ...
    public void OnCommand(GameClient client, string[] args)
    {
        ...
        if (onOff == "on")
        {
            ...
            {
                client.Out.SendMessage("Your cloak is already visible.", eChatType.CT_YouWereHit,
        ...
        if (onOff == "off")
        {
            if (client.Player.IsCloakInvisible)
            {
                client.Out.SendMessage("Your cloak is already invisible.", eChatType.CT_YouWereHit,
                    eChatLoc.CL_SystemWindow);
                return;
            }
            else
