...
public class RealmCommandHandler : AbstractCommandHandler, ICommandHandler
{
    ...
    public void OnCommand(GameClient client, string[] args)
    {
        ...
        var midKeeps = "";
        var hibKeeps = "";
        var keepList = GameServer.KeepManager.GetFrontierKeeps();
        var albKeepList = GameServer.KeepManager.GetKeepsOfRegion(1);
        var midKeepList = GameServer.KeepManager.GetKeepsOfRegion(100);
        var hibKeepList = GameServer.KeepManager.GetKeepsOfRegion(200);

        foreach (var keep in albKeepList)
        {
            if (keep.Name.ToLower().Contains("myrddin") || keep.Name.ToLower().Contains("excalibur"))
                continue;

            if (keep is GameKeep) albKeeps += KeepStringBuilder(keep);
        }

        foreach (var keep in midKeepList)
        {
            if (keep.Name.ToLower().Contains("grallarhorn") || keep.Name.ToLower().Contains("mjollner"))
                continue;
            if (keep is GameKeep) midKeeps += KeepStringBuilder(keep);
        }

        foreach (var keep in hibKeepList)
        {
            if (keep.Name.ToLower().Contains("dagda") || keep.Name.ToLower().Contains("lamfhota"))
                continue;

            if (keep is GameKeep) hibKeeps += KeepStringBuilder(keep);
        }
        ...
        {
            realmInfo.Add(
                LanguageMgr.GetTranslation(client.Account.Language, "Scripts.Players.Realm.DarknessFalls") +
                ": All Realms");
        }
        else
        {
            realmInfo.Add(
                LanguageMgr.GetTranslation(client.Account.Language, "Scripts.Players.Realm.DarknessFalls") + ": " +
                GlobalConstants.RealmToName(DFEnterJumpPoint.DarknessFallOwner));
            if (DFEnterJumpPoint.LastRealmSwapTick + DFEnterJumpPoint.GracePeriod >= GameLoop.GameLoopTime)
            {
                ...
                if (DFEnterJumpPoint.PreviousOwner == eRealm.Midgard)
                    realmName = "Midgard";
                if (realmName != "")
                    realmInfo.Add(realmName + " can enter Darkness Falls for another " +
                                  TimeSpan.FromMilliseconds(pve).Minutes + "m " +
                                  TimeSpan.FromMilliseconds(pve).Seconds + "s");
            }
        }
        ...
        client.Out.SendCustomTextWindow(
            LanguageMgr.GetTranslation(client.Account.Language, "Scripts.Players.Realm.Title"), realmInfo);
    }
