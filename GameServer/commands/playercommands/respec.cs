...
{
    private const string RA_RESPEC = "realm_respec";
    private const string ALL_RESPEC = "all_respec";
    private const string LINE_RESPEC = "line_respec";
    private const string DOL_RESPEC = "dol_respec";
    private const string BUY_RESPEC = "buy_respec";
    private const string CHAMP_RESPEC = "champion_respec";
        ...
        if (args.Length < 2)
        {
            ...
            if (client.Player.RespecAmountAllSkill > 0)
            {
                DisplayMessage(client,
                    "You have " + client.Player.RespecAmountAllSkill + " full skill respecs available.");
            ...
            {
                DisplayMessage(client,
                    "You have " + client.Player.RespecAmountSingleSkill + " single-line respecs available.");
            ...
            {
                DisplayMessage(client,
                    "You have " + client.Player.RespecAmountRealmSkill + " realm skill respecs available.");
            ...
            {
                DisplayMessage(client,
                    "You have " + client.Player.RespecAmountDOL + " Atlas ( full skill ) respecs available.");
                DisplayMessage(client, "Target any trainer and use /respec all");
            }
        ...
        switch (args[1].ToLower())
        {
            ...
            case "all":
            {
                if ( /*client.Player.Level >= 50 || */
                    TimeSpan.FromSeconds(client.Player.PlayedTimeSinceLevel).Hours > 24)
                ...
                client.Out.SendCustomDialog(
                    "CAUTION: All respec changes are final with no second chance. Proceed carefully!",
            ...
            default:
            {
                ...
                var lineName = string.Join(" ", args, 1, args.Length - 1);
                var specLine = client.Player.GetSpecializationByName(lineName, false);
                if (specLine == null)
                {
    ...
    protected void RespecDialogResponse(GamePlayer player, byte response)
    {
        ...
        var realmSpecPoints = player.RealmSpecialtyPoints;
        ...
        {
            var specLine = (Specialization) player.TempProperties.getProperty<object>(LINE_RESPEC, null);
        ...
        if (player.TempProperties.getProperty(BUY_RESPEC, false))
        {
            ...
            {
                InventoryLogging.LogInventoryAction(player, "(respec)", eInventoryActionType.Merchant,
                    player.RespecCost * 10000);
        ...
        if (player.SkillSpecialtyPoints > specPoints)
        {
            ...
            DisplayMessage(player,
                "You regain " + (player.SkillSpecialtyPoints - specPoints) + " specialization points!");
        }

        if (player.RealmSpecialtyPoints > realmSpecPoints)
            DisplayMessage(player,
                "You regain " + (player.RealmSpecialtyPoints - realmSpecPoints) + " realm specialization points!");
        ...
        {
            foreach (var e in player.effectListComponent.GetAllEffects())
