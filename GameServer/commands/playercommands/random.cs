...
public class RandomCommandHandler : AbstractCommandHandler, ICommandHandler
{
    ...
    private const string
        MESSAGE_RESULT_SELF = "You pick a random number between 1 and {0}: {1}"; // thrownMax, thrown

    private const string
        MESSAGE_RESULT_OTHER =
            "{0} picks a random number between 1 and {1}: {2}"; // client.Player.Name, thrownMax, thrown
    ...
    public void OnCommand(GameClient client, string[] args)
    {
        ...
        // throw result
        var thrown = Util.Random(1, thrownMax);

        var mybattlegroup =
            client.Player.TempProperties.getProperty<BattleGroup>(BattleGroup.BATTLEGROUP_PROPERTY, null);
        if (mybattlegroup != null && mybattlegroup.IsRecordingRolls() &&
            thrownMax <= mybattlegroup.GetRecordingThreshold())
            mybattlegroup.AddRoll(client.Player, thrown);

        // building result messages
        var selfMessage = string.Format(MESSAGE_RESULT_SELF, thrownMax, thrown);
        var otherMessage = string.Format(MESSAGE_RESULT_OTHER, client.Player.Name, thrownMax, thrown);
