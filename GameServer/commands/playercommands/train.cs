...
public class TrainCommandHandler : AbstractCommandHandler, ICommandHandler
{
    ...
    private const string NotEnoughPointsLeft =
        "You don't have that many specialization points left for this level.";
    ...
    {
        if (!automated && IsSpammingCommand(client.Player, "train")) return;
        ...
        if (!ServerProperties.Properties.ALLOW_TRAIN_ANYWHERE &&
            ...
            (trainer == null || trainer.CanTrain(client.Player) == false))
        {
            client.Out.SendMessage("You have to be at your trainer to use this command.", eChatType.CT_System,
        ...
        var line = string.Join(" ", args, 1, args.Length - 2);
        ...
        if (spec == null)
        {
            client.Out.SendMessage("The provided skill could not be found.", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
        ...
        var currentSpecLevel = spec.Level;
        ...
        var specLevel = 0;
        var changed = false;
        var canAutotrainSpec = client.Player.GetAutoTrainPoints(spec, 4) != 0;
        var autotrainPoints = client.Player.GetAutoTrainPoints(spec, 3);
        ...
        for (var i = 0; i < level; i++)
        {
            ...
            if (client.Player.SkillSpecialtyPoints + autotrainPoints - skillSpecialtyPoints >=
                spec.Level + specLevel + 1)
            {
                changed = true;
