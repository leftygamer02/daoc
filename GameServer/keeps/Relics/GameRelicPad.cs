...
public class GameRelicPad : GameStaticItem
{
    ...
    private PadArea m_area = null;
    private GameRelic m_mountedRelic = null;
    ...
    public static void BroadcastDiscordRelic(string message, eRealm realm, string keepName)
    {
        ...
        switch (realm)
        {
            case eRealm._FirstPlayerRealm:
                color = 16711680;
                avatarUrl =
                    "https://cdn.discordapp.com/attachments/879754382231613451/977721734948081684/relic_alb.png";
                break;
            case eRealm._LastPlayerRealm:
                color = 32768;
                avatarUrl =
                    "https://cdn.discordapp.com/attachments/879754382231613451/977721735153606686/relic_hib.png";
                break;
            default:
                color = 255;
                avatarUrl =
    ...
    public void MountRelic(GameRelic relic, bool returning)
    {
        ...
        {
            /* Sending broadcast */
            var message = LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE,
                "GameRelicPad.MountRelic.Stored", relic.CurrentCarrier.Name,
                GlobalConstants.RealmToName((eRealm) relic.CurrentCarrier.Realm), relic.Name, Name);
            foreach (var cl in WorldMgr.GetAllPlayingClients())
            {
                if (cl.Player.ObjectState != eObjectState.Active) continue;
                cl.Out.SendMessage(
                    LanguageMgr.GetTranslation(cl.Account.Language, "GameRelicPad.MountRelic.Captured",
                        GlobalConstants.RealmToName((eRealm) relic.CurrentCarrier.Realm), relic.Name),
            ...
            var relicBG =
                (BattleGroup) relic.CurrentCarrier?.TempProperties.getProperty<object>(
                    BattleGroup.BATTLEGROUP_PROPERTY, null);
            var targets = new List<GamePlayer>();
            ...
            if (relicBG != null)
                ...
                foreach (var p in relic.CurrentCarrier.Group.GetPlayersInTheGroup())
                    targets.Add(p);
            else
        ...
        else
        {
            ...
            var message = string.Format("The {0} has been returned to {1}.", relic.Name, Name);
            foreach (var cl in WorldMgr.GetAllPlayingClients())
            {
                if (cl.Player.ObjectState != eObjectState.Active) continue;
                cl.Out.SendMessage(message + "\n" + message + "\n" + message, eChatType.CT_Important,
    ...
    public void RemoveRelic(GameRelic relic)
    {
        ...
        {
            var message = LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE,
                "GameRelicPad.RemoveRelic.Removed", relic.CurrentCarrier.Name,
                GlobalConstants.RealmToName((eRealm) relic.CurrentCarrier.Realm), relic.Name, Name);
            foreach (var cl in WorldMgr.GetAllPlayingClients())
            {
                if (cl.Player.ObjectState != eObjectState.Active) continue;
    ...
    public GameRelic MountedRelic => m_mountedRelic;
    ...
    public class PadArea : Area.Circle
    {
        ...
        {
            var relicOnPlayer =
                player.TempProperties.getProperty<object>(GameRelic.PLAYER_CARRY_RELIC_WEAK, null) as GameRelic;
                ...
                player.Client.Out.SendMessage(
                    string.Format(
                        LanguageMgr.GetTranslation(player.Client.Account.Language,
                            "GameRelicPad.OnPlayerEnter.EmptyRelicPad"), relicOnPlayer.RelicType),
                    eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                log.DebugFormat("Player {0} needs to find an empty {1} relic pad in order to place {2}.",
                    player.Name, relicOnPlayer.RelicType, relicOnPlayer.Name);
                return;
            }
