...
public class GameRelic : GameStaticItem
{
    ...
    private GamePlayer m_currentCarrier = null;
    private GameRelicPad m_currentRelicPad = null;
    private GameRelicPad m_returnRelicPad = null;
    private DateTime m_lastCapturedDate = DateTime.Now;
    private ECSGameTimer m_currentCarrierTimer;
    private DBRelic m_dbRelic;
    private eRelicType m_relicType;
    private ECSGameTimer m_returnRelicTimer;
    private long m_timeRelicOnGround = 0;

    protected int ReturnRelicInterval => Properties.RELIC_RETURN_TIME * 1000;
    ...
    /// </summary>
    public eRelicType RelicType => m_relicType;
    ...
    /// </summary>
    public eRealm OriginalRealm => m_originalRealm;
    ...
    {
        get => m_lastRealm;
        set => m_lastRealm = value;
    }

    public DateTime LastCaptureDate
    {
        get => m_lastCapturedDate;
    ...
    public override bool Interact(GamePlayer player)
    {
        ...
        if (!player.IsAlive)
        {
            player.Out.SendMessage("You cannot pickup " + GetName(0, false) + ". You are dead!",
        ...
        {
            player.Out.SendMessage("You cannot pickup " + GetName(0, false) + ". It is owned by your realm.",
                eChatType.CT_System, eChatLoc.CL_SystemWindow);
            return false;
        }
        ...
        if (IsMounted && !RelicMgr.CanPickupRelicFromShrine(player, this))
        {
            player.Out.SendMessage(
                "You cannot pickup " + GetName(0, false) + ". You need to capture your realms " +
                Enum.GetName(typeof(eRelicType), RelicType) + " relic first.", eChatType.CT_System,
    ...
    protected virtual void PlayerTakesRelic(GamePlayer player)
    {
        ...
        {
            player.Out.SendMessage("You are already carrying a relic.", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            return;
        }
        ...
        {
            player.Out.SendMessage("You cannot carry a relic while stealthed.", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            return;
        }
        ...
        if (IsMounted)
        {
            var keep = GameServer.KeepManager.GetKeepCloseToSpot(m_currentRelicPad.CurrentRegionID,
                m_currentRelicPad, WorldMgr.VISIBILITY_DISTANCE);
            ...
            {
                player.Out.SendMessage(
                    $"You must have {Properties.RELIC_PLAYERS_REQUIRED_ON_PAD} players nearby the pad before taking a relic.",
                    eChatType.CT_System, eChatLoc.CL_SystemWindow);
                return;
            }
        }
        ...
        if (player.Inventory.AddItem(eInventorySlot.FirstEmptyBackpack, m_item))
        {
            ...
            InventoryLogging.LogInventoryAction(this, player, eInventoryActionType.Other, m_item.Template,
                m_item.Count);
            ...
            if (IsMounted)
            {
                ...
                LastRealm = m_currentRelicPad
                    .Realm; // save who owned this in case of server restart while relic is off pad
        ...
        {
            player.Out.SendMessage("You dont have enough space in your backpack to carry this.",
                eChatType.CT_System, eChatLoc.CL_SystemWindow);
        }
    }
    ...
    protected virtual void PlayerLoosesRelic(bool removeFromInventory)
    {
        ...
        if (removeFromInventory)
            ...
            {
                var success = player.Inventory.RemoveItem(m_item);
                InventoryLogging.LogInventoryAction(player, this, eInventoryActionType.Other, m_item.Template,
                    m_item.Count);
                log.Debug("Remove " + m_item.Name + " from " + player.Name + "'s Inventory " +
        ...
        if (IsMounted == false)
        {
            ...
            m_returnRelicTimer = new ECSGameTimer(this, new ECSGameTimer.ECSTimerCallback(ReturnRelicTick),
                RelicEffectInterval);
            log.DebugFormat("{0} dropped, return timer for relic set to {1} seconds.", Name,
                ReturnRelicInterval / 1000);
    ...
    protected virtual int ReturnRelicTick(ECSGameTimer timer)
    {
        if (GameLoop.GameLoopTime - m_timeRelicOnGround < ReturnRelicInterval)
        {
            ...
            var effectID = (ushort) Util.Random(5811, 5815);
    ...
    private int CarrierTimerTick(ECSGameTimer timer)
    {
        ...
        if (CurrentCarrier != null && CurrentCarrier.Inventory.GetFirstItemByID(m_item.Id_nb,
                eInventorySlot.FirstBackpack, eInventorySlot.LastBackpack) == null)
        {
    ...
    protected virtual void SetHandlers(GamePlayer player, bool activate)
    {
        if (activate)
        {
            ...
            GameEventMgr.AddHandler(player, GameLivingEvent.Dying, new DOLEventHandler(PlayerAbsence));
            GameEventMgr.AddHandler(player, GamePlayerEvent.StealthStateChanged,
                new DOLEventHandler(PlayerAbsence));
        ...
        else
        {
            ...
            GameEventMgr.RemoveHandler(player, GameLivingEvent.Dying, new DOLEventHandler(PlayerAbsence));
            GameEventMgr.RemoveHandler(player, GamePlayerEvent.StealthStateChanged,
                new DOLEventHandler(PlayerAbsence));
            GameEventMgr.RemoveHandler(player, GamePlayerEvent.Linkdeath, new DOLEventHandler(PlayerAbsence));
            GameEventMgr.RemoveHandler(player, PlayerInventoryEvent.ItemDropped,
                new DOLEventHandler(PlayerAbsence));
        }
    }
    ...
    protected void PlayerAbsence(DOLEvent e, object sender, EventArgs args)
    {
        ...
        {
            var idArgs = args as ItemDroppedEventArgs;
    ...
    {
        var messages = base.GetExamineMessages(player);
        messages.Add(IsMounted
            ? "It is owned by " +
              (player.Realm == Realm ? "your realm" : GlobalConstants.RealmToName((eRealm) Realm)) + "."
            : "It is without owner, take it!");
        return messages;
    }
    ...
    public override void LoadFromDatabase(DataObject obj)
    {
        ...
        //get constant values
        var template = GetRelicTemplate(m_originalRealm, m_relicType);
