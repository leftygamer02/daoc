...
public class GameKeepComponent : GameLiving, IComparable, IGameKeepComponent
{
    ...
    public bool Climbing
    {
        get
        {
            ...
            {
                if (Skin == (int) eComponentSkin.Wall || Skin == (int) eComponentSkin.NewSkinClimbingWall ||
                    Skin == (int) eComponentSkin.Tower ||
                    (Skin == (int) eComponentSkin.NewSkinTower && !Keep.IsPortalKeep)) return true;
            }
            else
            {
                if (Skin == (int) eComponentSkin.Wall ||
                    (Skin == (int) eComponentSkin.NewSkinClimbingWall && !Keep.IsPortalKeep)) return true;
            }
    ...
    public override IList GetExamineMessages(GamePlayer player)
    {
        ...
        if (player.Client.Account.PrivLevel > 1) list.Add(Name + " with a Z of " + Z.ToString());
    ...
    {
        if (weapon.Flags ==
            10) //Bruiser or any other item needs Itemtemplate "Flags" set to 10 to proc on keep components
            return true;
        else return false;
    }
    ...
    {
        var myregion = WorldMgr.GetRegion((ushort) keep.Region);
        if (myregion == null)
            return;
        Keep = keep;
        ...
        var angle = keep.Heading * (Math.PI * 2 / 360); // angle*2pi/360;
        X = (int) (keep.X + ((sbyte) component.X * 148 * Math.Cos(angle) +
                             (sbyte) component.Y * 148 * Math.Sin(angle)));
        Y = (int) (keep.Y - ((sbyte) component.Y * 148 * Math.Cos(angle) -
                             (sbyte) component.X * 148 * Math.Sin(angle)));
        Z = keep.Z;
    ...
    public virtual void LoadPositions()
    {
        ...
        if (CurrentRegion is BaseInstance) region = (CurrentRegion as BaseInstance).Skin;

        var bg = GameServer.KeepManager.GetBattleground(region);

        Positions.Clear();

        var whereClause = DB.Column("ComponentSkin").IsEqualTo(Skin);
        if (Skin != (int) eComponentSkin.Keep && Skin != (int) eComponentSkin.Tower &&
            Skin != (int) eComponentSkin.Gate)
        ...
        {
            var list = Positions[position.TemplateID] as DBKeepPosition[];
            if (list == null)
            {
    ...
    public virtual void FillPositions()
    {
        foreach (DBKeepPosition[] positionGroup in Positions.Values)
            for (var i = Height; i >= 0; i--)
                ...
                {
                    var create = false;
                    var sKey = position.TemplateID + ID;
                    ...
                    switch (position.ClassType)
                    {
                        ...
                        case "DOL.GS.Keeps.Patrol":
                            if ((position.KeepType == (int) AbstractGameKeep.eKeepType.Any ||
                                 position.KeepType == (int) Keep.KeepType)
                        ...
                        case "DOL.GS.Keeps.FrontierHastener":
                            if (Keep.HasHastener && log.IsWarnEnabled)
                                log.Warn(
                                    $"FillPositions(): KeepComponent_ID {InternalID}, KeepPosition_ID {position.ObjectId}: There is already a {position.ClassType} on Keep {Keep.KeepID}");
                    ...
                    if (create)
                    {
                        ...
                        try
                        {
                            ...
                            var obj = (IKeepItem) asm.CreateInstance(position.ClassType, true);
                            if (obj != null)
                    ...
                    else
                    {
                        ...
                        * Otherwise, we're assuming the former position/component combination wasn't valid, and that's an error that should be reported in any case.						
                        ...
                        if (log.IsWarnEnabled)
                            log.Warn(
                                $"FillPositions(): Keep {Keep.KeepID} already has a {position.ClassType} assigned to Position {position.ObjectId} on Component {InternalID}");
                    }
        ...
        foreach (var guard in Keep.Guards.Values)
        {
            ...
            {
                if (guard.Position.Height <= guard.Component.Height &&
                    guard.ObjectState != eObjectState.Active && !guard.IsRespawning)
                    guard.AddToWorld();
            }
        }
        ...
        foreach (var banner in Keep.Banners.Values)
        {
            ...
            {
                if (banner.Position.Height <= banner.Component.Height &&
                    banner.ObjectState != eObjectState.Active)
                    banner.AddToWorld();
            }
        }
    }
    ...
    public override void SaveIntoDatabase()
    {
        ...
        if (New)
        {
            ...
            log.DebugFormat("Added new component {0} for keep ID {1}, skin {2}, health {3}", ID, Keep.KeepID, Skin,
                Health);
        }
        else
        {
        ...
        if (damageAmount > 0)
        {
            ...
            if (m_oldHealthPercent != HealthPercent)
            {
                ...
                foreach (var client in WorldMgr.GetClientsOfRegion(CurrentRegionID))
                {
                    ...
                    client.Out.SendKeepComponentDetailUpdate(
                        this); // I knwo this works, not sure if ObjectUpdate is needed - Tolakram
                }
            }
        }
    }
    ...
    public override void ModifyAttack(AttackData attackData)
    {
        ...
        var toughness = Properties.SET_STRUCTURES_TOUGHNESS;
        var baseDamage = attackData.Damage;
        var styleDamage = attackData.StyleDamage;
        var criticalDamage = 0;

        var source = attackData.Attacker;
        ...
        else if (source is GameNPC)
        {
            ...
            else
            {
                ...
                {
                    var player =
                        ((AI.Brain.IControlledBrain) ((GameNPC) source).Brain).Owner as GamePlayer;
                    if (player != null)
                    {
                        // special considerations for pet spam classes
                        if (player.CharacterClass.ID == (int) eCharacterClass.Theurgist ||
                            player.CharacterClass.ID == (int) eCharacterClass.Animist)
                        {
    ...
    {
        if (Keep == null) return "Keep is null!";
