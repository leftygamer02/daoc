...
public class GameKeepHookPoint : Point3D
{
    ...
    public bool IsFree => m_object == null;
    ...
    private void ObjectDie(DOLEvent e, object sender, EventArgs arguments)
    {
        ...
        var item = DOLDB<DBKeepHookPointItem>.SelectObject(DB.Column("KeepID").IsEqualTo(Component.Keep.KeepID)
            .And(DB.Column("ComponentID").IsEqualTo(Component.ID)).And(DB.Column("HookPointID").IsEqualTo(ID)));
        if (item != null)
