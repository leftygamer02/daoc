...
public class DefaultKeepManager : IKeepManager
{
    ...
    /// list of all keeps
    /// </summary>
    protected Hashtable m_keepList = new();

    public virtual Hashtable Keeps => m_keepList;

    protected List<Battleground> m_battlegrounds = new();

    public const int DEFAULT_FRONTIERS_REGION = 163; // New Frontiers

    public List<uint> m_frontierRegionsList = new();

    public virtual List<uint> FrontierRegionsList => m_frontierRegionsList;
    ...
    public virtual bool Load()
    {
        ...
        foreach (var r in WorldMgr.Regions.Values)
            if (r.IsFrontier)
        ...
        lock (m_keepList.SyncRoot)
        {
            ...
            foreach (var datakeep in keeps)
            {
                var keepRegion = WorldMgr.GetRegion(datakeep.Region);
                if (keepRegion == null)
                    continue;
            ...
            if (ServerProperties.Properties.USE_NEW_KEEPS == 2)
                log.ErrorFormat(
                    "ServerProperty USE_NEW_KEEPS is actually set to 2 but it is no longer used. Loading as if he were 0 but please set to 0 or 1 !");
            ...
            if (keepcomponents != null)
                keepcomponents
                    ...
                    .ForAll(components =>
                    {
                        ...
                        {
                            var keep = GetKeepByID(component.KeepID);
                            if (keep == null)
                                //missingKeeps = true;
                                continue;

                            var gamecomponent = keep.CurrentRegion.CreateGameKeepComponent();
    ...
    protected virtual void LoadHookPoints()
    {
        ...
        var hookPointList = new Dictionary<string, List<DBKeepHookPoint>>();

        var dbkeepHookPoints = GameServer.Database.SelectAllObjects<DBKeepHookPoint>();
        foreach (var dbhookPoint in dbkeepHookPoints)
        {
            List<DBKeepHookPoint> currentArray;
            var key = dbhookPoint.KeepComponentSkinID + "H:" + dbhookPoint.Height;
        ...
        {
            var key = component.Skin + "H:" + component.Height;
            if (hookPointList.ContainsKey(key))
            {
                var HPlist = hookPointList[key];
                if (HPlist != null && HPlist.Count != 0)
                {
                    foreach (var dbhookPoint in hookPointList[key])
                    {
                        var myhookPoint = new GameKeepHookPoint(dbhookPoint, component);
        ...
        var items = GameServer.Database.SelectAllObjects<DBKeepHookPointItem>();
        foreach (AbstractGameKeep keep in m_keepList.Values)
        foreach (var component in keep.KeepComponents)
        foreach (var hp in component.HookPoints.Values)
        {
            var item = items.FirstOrDefault(
                it => it.KeepID == component.Keep.KeepID && it.ComponentID == component.ID &&
                      it.HookPointID == hp.ID);
            if (item != null)
    ...
    {
        var myKeeps = new List<IGameKeep>();
        var keepsByID = new SortedList();
    ...
    public virtual AbstractGameKeep GetBGPK(GamePlayer player)
    {
        ...
        foreach (AbstractGameKeep keep in m_keepList.Values)
        {
            ...
            var bg = GetBattleground(keep.Region);
            if (bg == null) continue;
        ...
        foreach (var keep in GetKeepsOfRegion((ushort) tempKeep.Region))
    ...
    {
        var keepList = new List<AbstractGameKeep>();

        foreach (ushort regionID in m_frontierRegionsList) keepList.AddRange(GetKeepsOfRegion(regionID));
    ...
    {
        var regionKeeps = new List<AbstractGameKeep>();
        foreach (AbstractGameKeep keep in m_keepList.Values)
        {
    ...
    /// <param name="radius"></param>
    /// <returns></returns>
    public virtual ICollection<AbstractGameKeep> GetKeepsCloseToSpot(ushort regionid, int x, int y, int z,
        int radius)
    {
        var closeKeeps = new List<AbstractGameKeep>();
        ...
        lock (m_keepList.SyncRoot)
        {
            foreach (AbstractGameKeep keep in m_keepList.Values)
            {
                ...
                if (range < radiussqrt) closeKeeps.Add(keep);
            }
        }
    ...
    public virtual AbstractGameKeep GetKeepCloseToSpot(ushort regionid, int x, int y, int z, int radius)
    {
        ...
        {
            long radiussqrt = radius * radius;
            var lastKeepDistance = radiussqrt;
    ...
    {
        var realmXTower = new Dictionary<eRealm, int>(3);
    ...
    {
        var realmXTower = new Dictionary<eRealm, int>(4);
        realmXTower.Add(eRealm.Albion, 0);
        realmXTower.Add(eRealm.Hibernia, 0);
        realmXTower.Add(eRealm.Midgard, 0);
        ...
        lock (m_keepList.SyncRoot)
        {
            foreach (AbstractGameKeep keep in m_keepList.Values)
                if (m_frontierRegionsList.Contains(keep.Region) && keep is GameKeepTower &&
                    zones.Contains(keep.CurrentZone.ID))
                    realmXTower[keep.Realm] += 1;
        }
        ...
        lock (m_keepList.SyncRoot)
        {
            foreach (AbstractGameKeep keep in m_keepList.Values)
            {
                ...
                if (keep.Name.ToLower().Contains("dagda") || keep.Name.ToLower().Contains("lamfotha") ||
                    keep.Name.ToLower().Contains("grallarhorn") || keep.Name.ToLower().Contains("mjollner") ||
                    keep.Name.ToLower().Contains("myrddin") || keep.Name.ToLower().Contains("excalibur") ||
                    keep.Name.ToLower().Contains("portal"))
    ...
    {
        var myKeeps = new List<AbstractGameKeep>();
        foreach (AbstractGameKeep keep in m_keepList.Values) myKeeps.Add(keep);
    ...
    public virtual bool IsEnemy(AbstractGameKeep keep, GamePlayer target, bool checkGroup)
    {
        ...
        if (GameServer.Instance.Configuration.ServerType == eGameServerType.GST_PvP)
        {
            ...
            if (checkGroup && target.Group != null)
                foreach (var player in target.Group.GetPlayersInTheGroup())
    ...
    {
        var keep = 7 - GetKeepCountByRealm(realm);
    ...
    {
        var tower = 28 - GetTowerCountByRealm(realm);
    ...
    public virtual void UpdateBaseLevels()
    {
        lock (m_keepList.SyncRoot)
        {
            foreach (AbstractGameKeep keep in m_keepList.Values)
            {
                ...
                var newLevel = keep.BaseLevel;

                if (ServerProperties.Properties.BALANCE_TOWERS_SEPARATE)
                {
                    if (keep is GameKeepTower)
                        newLevel = (byte) (keep.DBKeep.BaseLevel +
                                           GameServer.KeepManager.GetRealmTowerBonusLevel((eRealm) keep.Realm));
                    else
                        newLevel = (byte) (keep.DBKeep.BaseLevel +
                                           GameServer.KeepManager.GetRealmKeepBonusLevel((eRealm) keep.Realm));
                }
                else
                {
                    newLevel = (byte) (keep.DBKeep.BaseLevel +
                ...
                if (keep.BaseLevel != newLevel)
                {
                    ...
                    foreach (var guard in keep.Guards.Values) guard.SetLevel();
                }
            }
        }
    }
    ...
    {
        foreach (var bg in m_battlegrounds)
    ...
    public virtual void ExitBattleground(GamePlayer player)
    {
        ...
        switch (player.Realm)
        {
            case eRealm.Albion:
                ...
                break;
            case eRealm.Midgard:
                location = "Svasudheim Faste";
                break;
            case eRealm.Hibernia:
                location = "Druim Ligen";
                break;
        }

        if (location != "")
        {
            var t = DOLDB<Teleport>.SelectObject(DB.Column("TeleportID").IsEqualTo(location));
            if (t != null)
