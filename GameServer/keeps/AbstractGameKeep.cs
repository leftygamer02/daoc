...
public abstract class AbstractGameKeep : IGameKeep
{
    ...
    /// Height of the keep
    /// </summary>
    public virtual int Height => GameServer.KeepManager.GetHeightFromLevel(Level);
    ...
    public virtual bool IsPortalKeep => (this is GameKeepTower && KeepComponents.Count > 1) || BaseLevel >= 100;
    ...
    public List<GameKeepComponent> KeepComponents { get; set; } = new();
    ...
    public List<IGameKeepComponent> SentKeepComponents
    {
        get
        {
            var ret = new List<IGameKeepComponent>();
            foreach (var comp in KeepComponents)
                ret.Add(comp);
        ...
        set
        {
            var newList = new List<GameKeepComponent>();
    ...
    public Dictionary<string, GameKeepDoor> Doors { get; set; } = new();

    public DBKeep DBKeep { get; set; }

    public Dictionary<string, GameKeepGuard> Guards { get; } = new();

    public Dictionary<string, GameKeepBanner> Banners { get; set; } = new();

    public Dictionary<string, Patrol> Patrols { get; set; } = new();
    ...
    public Zone CurrentZone
    {
        get
        {
            if (CurrentRegion != null) return CurrentRegion.GetZone(X, Y);
    ...
    /// the keep is more difficult the guild which have claimed gain more bonus
    /// </summary>
    public int DifficultyLevel => m_difficultyLevel[(int) Realm - 1];
    ...
    public virtual int LordRespawnTime
    {
        get
        {
                 ...
                 GameServer.Instance.Configuration.ServerType == eGameServerType.GST_PvP))
            {
                ...
                var iVariance = 1000 * Math.Abs(ServerProperties.Properties.GUARD_RESPAWN_VARIANCE);
                var iRespawn = 60 * (Math.Abs(ServerProperties.Properties.GUARD_RESPAWN) * 1000 +
                                     Util.Random(-iVariance, iVariance));

                return
                    iRespawn > 1000
                        ? iRespawn
                        : 1000; // Make sure we don't end up with an impossibly low respawn interval.
            }
    ...
    public virtual string Name
    {
        get
        {
            if (DBKeep != null)
            {
                var name = DBKeep.Name;

                if (ServerProperties.Properties.ENABLE_DEBUG) name += string.Format(" KID: {0}", KeepID);
    ...
    /// </summary>
    public eRealm OriginalRealm => (eRealm) DBKeep.OriginalRealm;
    ...
    {
        get => (eKeepType) DBKeep.KeepType;
        set => DBKeep.KeepType = (int) value;
    }
    ...
    public virtual void Load(DBKeep keep)
    {
        ...
        foreach (AbstractArea a in CurrentRegion.GetAreasOfSpot(keep.X, keep.Y, keep.Z))
            ...
            {
                log.Debug("Found a DBArea entry for " + keep.Name +
                          ", loading that instead of creating a new one.");
                area = a as KeepArea;
                area.Keep = this;
                break;
            }
    ...
    {
        var
            guards = new Dictionary<string, GameKeepGuard>(Guards); // Use a shallow copy
        ...
        var
            banners = new Dictionary<string, GameKeepBanner>(Banners); // Use a shallow copy
        ...
        var doors = new Dictionary<string, GameKeepDoor>(Doors); // Use a shallow copy
        foreach (var door in doors.Values)
        {
            door.Delete();
            var d = new GameDoor();
        ...
        if (area != null) CurrentRegion.RemoveArea(area);
    ...
    public virtual void LoadFromDatabase(DataObject keep)
    {
        ...
        {
            var myguild = GuildMgr.GetGuildByName(DBKeep.ClaimedGuildName);
            if (myguild != null)
            {
                Guild = myguild;
        ...
        else if (Level <= ServerProperties.Properties.MAX_KEEP_LEVEL &&
                 Level > ServerProperties.Properties.STARTING_KEEP_LEVEL && Guild == null)
    ...
    public virtual bool CheckForClaim(GamePlayer player)
    {
        if (InCombat)
        {
            player.Out.SendMessage(Name + " is under attack and can't be claimed.", eChatType.CT_System,
        ...
        {
            player.Out.SendMessage("The keep is not owned by your realm.", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            return false;
        }

        // Disabled check on DBKeep.BaseLevel to allow claiming of BG keeps
        if (DBKeep.BaseLevel != 50 && !ServerProperties.Properties.ALLOW_BG_CLAIM)
        {
            player.Out.SendMessage("This keep is not able to be claimed.", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            return false;
        }
        ...
        {
            player.Out.SendMessage("You must be in a guild to claim a keep.", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            return false;
        }
        ...
        {
            player.Out.SendMessage("You do not have permission to claim for your guild.", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            return false;
        }
        ...
        switch (ServerProperties.Properties.GUILDS_CLAIM_LIMIT)
        {
            ...
            case 1:
            {
                ...
                {
                    player.Out.SendMessage("Your guild already owns a keep.", eChatType.CT_System,
                        eChatLoc.CL_SystemWindow);
                    return false;
                }
            ...
            default:
            {
                if (player.Guild.ClaimedKeeps.Count >= ServerProperties.Properties.GUILDS_CLAIM_LIMIT)
                {
                    player.Out.SendMessage(
                        "Your guild already owns the limit of keeps (" +
                        ServerProperties.Properties.GUILDS_CLAIM_LIMIT + ")", eChatType.CT_System,
                        eChatLoc.CL_SystemWindow);
                    return false;
                }
        ...
        if (player.Group != null)
        {
            ...
            foreach (var p in player.Group.GetPlayersInTheGroup())
                ...
                if (p.CurrentAreas.Contains(Area)) //Check if player is in keep area
                    count++;

            var needed = ServerProperties.Properties.CLAIM_NUM;
    ...
    public virtual void Claim(GamePlayer player)
    {
        ...
        if (ServerProperties.Properties.GUILDS_CLAIM_LIMIT > 1)
            player.Guild.SendMessageToGuildMembers(
                "Your guild has currently claimed " + player.Guild.ClaimedKeeps.Count + " keeps of a maximum of " +
                ServerProperties.Properties.GUILDS_CLAIM_LIMIT, eChatType.CT_Guild, eChatLoc.CL_ChatWindow);

        ChangeLevel((byte) ServerProperties.Properties.STARTING_KEEP_CLAIM_LEVEL);

        PlayerMgr.BroadcastClaim(this);

        foreach (var guard in Guards.Values) guard.ChangeGuild();

        foreach (var banner in Banners.Values) banner.ChangeGuild();
    ...
    public virtual bool CheckForRelease(GamePlayer player)
    {
        ...
        {
            player.Out.SendMessage(Name + " is under attack and can't be released.", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
    ...
    public virtual void ChangeLevel(byte targetLevel)
    {
        ...
        {
            comp.UpdateLevel();
            foreach (var cln in WorldMgr.GetClientsOfRegion(CurrentRegion.ID))
                cln.Out.SendKeepComponentDetailUpdate(comp);

            comp.FillPositions();
        }

        foreach (var guard in Guards.Values) SetGuardLevel(guard);

        foreach (var p in Patrols.Values) p.ChangePatrolLevel();

        foreach (var door in Doors.Values) door.UpdateLevel();
    ...
    public virtual void SetGuardLevel(GameKeepGuard guard)
    {
        ...
        {
            var bonusLevel = 0;
            var multiplier = ServerProperties.Properties.KEEP_GUARD_LEVEL_MULTIPLIER;
    ...
    public TimeSpan ChangeLevelTimeRemaining
    {
        get
        {
            var totalsec = m_changeLevelTimer.TimeUntilElapsed / 1000;
            var min = totalsec / 60 % 60;
            var hours = totalsec / 3600 % 24;
    ...
    public virtual void Reset(eRealm realm)
    {
        ...
        //change realm
        foreach (var client in WorldMgr.GetClientsOfRegion(CurrentRegion.ID))
            client.Out.SendKeepComponentUpdate(this, false);

        //we reset all doors
        foreach (var door in Doors.Values) door.Reset(realm);
        ...
        foreach (var guard in Guards.Values)
            ...
            else if (guard is GuardLord == false) guard.Die(guard);

        //we reset the banners
        foreach (var banner in Banners.Values) banner.ChangeRealm();
    ...
    protected void ResetPlayersOfKeep()
    {
        ...
        foreach (var c in KeepComponents)
            if (c.Skin == id)
            {
                component = c;
                break;
        ...
        var hookpoint = component.HookPoints[97] as GameKeepHookPoint;

        if (hookpoint == null)
            return;

        //predict Z
        var hp = DOLDB<DBKeepHookPoint>.SelectObject(DB.Column("HookPointID").IsEqualTo(97)
            .And(DB.Column("Height").IsEqualTo(Height)));
        if (hp == null)
            return;
        var z = component.Z + hp.Z;

        foreach (GamePlayer player in component.GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE))
        {
            var d = hookpoint.GetDistance(player as IPoint2D);
            if (d > distance)
                continue;
    ...
    {
        var regionPlayerEventArgs = args as RegionPlayerEventArgs;
        var player = regionPlayerEventArgs.Player;
        player.Out.SendKeepInfo(this);
        foreach (var keepComponent in KeepComponents) player.Out.SendKeepComponentInfo(keepComponent);
    }
    ...
    protected void SendRemoveKeep()
    {
        foreach (var client in WorldMgr.GetClientsOfRegion(CurrentRegion.ID))
        {
            if (client.Player == null || client.ClientState != GameClient.eClientState.Playing ||
            ...
            // Remove Keep
            foreach (var comp in KeepComponents) player.Out.SendKeepComponentRemove(comp);
    ...
    protected void SendKeepInfo()
    {
        foreach (var client in WorldMgr.GetClientsOfRegion(CurrentRegion.ID))
        {
            ...
            // Add Keep
            player.Out.SendKeepInfo(this);
            foreach (var comp in KeepComponents) player.Out.SendKeepComponentInfo(comp);
        }
    }
}