...
public class PlayerMgr
{
    ...
    public static void BroadcastCapture(AbstractGameKeep keep)
    {
        ...
        if (keep.Realm != eRealm.None)
            message = string.Format(LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE,
                "PlayerManager.BroadcastCapture.Captured", GlobalConstants.RealmToName((eRealm) keep.Realm),
                keep.Name));
        else
            message = string.Format(LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE,
                "PlayerManager.BroadcastCapture.CapturedR0", GlobalConstants.RealmToName((eRealm) keep.Realm),
                keep.Name));
    ...
    {
        var message = string.Format(LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE,
            "PlayerManager.BroadcastRaize.Razed", keep.Name, GlobalConstants.RealmToName(realm)));
    ...
    {
        var claimMessage = string.Format(LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE,
    ...
    {
        var lostClaimMessage = string.Format(LanguageMgr.GetTranslation(
    ...
    public static void BroadcastDiscordRvR(string message, eRealm realm, string keepName)
    {
        ...
        switch (realm)
        {
            case eRealm._FirstPlayerRealm:
                color = 16711680;
                avatarUrl =
                    "https://cdn.discordapp.com/attachments/919610633656369214/928728399822860369/keep_alb.png";
                break;
            case eRealm._LastPlayerRealm:
                color = 32768;
                avatarUrl =
                    "https://cdn.discordapp.com/attachments/919610633656369214/928728400116478073/keep_hib.png";
                break;
            default:
                color = 255;
                avatarUrl =
                    "https://cdn.discordapp.com/attachments/919610633656369214/928728400523296768/keep_mid.png";
                break;
        }
    ...
    public static bool IsAllowedToInteract(GamePlayer player, AbstractGameKeep keep, eInteractType type)
    {
        ...
        switch (type)
        {
            case eInteractType.Claim:
            {
                if (keep.Guild != null)
                    return false;
                foreach (var k in GameServer.KeepManager.GetAllKeeps())
