...
public sealed class RelicMgr
{
    ...
    /// </summary>
    private static readonly Hashtable m_relics = new();


    /// <summary>
    /// list of all relicPads
    /// </summary>
    private static readonly ArrayList m_relicPads = new();
    ...
    public static bool Init()
    {
        lock (m_relics.SyncRoot)
        {
            ...
            foreach (GameRelicPad pad in m_relicPads) pad.RemoveRelic();

            // if relics are on the ground during init we will return them to their owners
            var lostRelics = new List<GameRelic>();
            ...
            foreach (var datarelic in relics)
            {
                ...
                var relic = new GameRelic(datarelic);
                m_relics.Add(datarelic.RelicID, relic);

                relic.AddToWorld();
                var pad = GetPadAtRelicLocation(relic);
                if (pad != null)
                {
            ...
            foreach (var lostRelic in lostRelics)
            {
                var returnRealm = (eRealm) lostRelic.LastRealm;

                if (returnRealm == eRealm.None) returnRealm = lostRelic.OriginalRealm;
    ...
    {
        var daysPassed = DateTime.Now.Subtract(relic.LastCaptureDate);
    ...
    {
        var myRelics = new ArrayList();
        foreach (GameRelic relic in m_relics.Values) myRelics.Add(relic);
    ...
    {
        var realmTypeRelics = new ArrayList();
    ...
    public static bool CanPickupRelicFromShrine(GamePlayer player, GameRelic relic)
    {
        ...
        if (player.Realm == relic.OriginalRealm)
            return true;
        var list = getRelics(player.Realm, relic.RelicType);
