...
public class PositionMgr
{
    ...
    public static DBKeepPosition GetUsablePosition(GameKeepGuard guard)
    {
        ...
        return DOLDB<DBKeepPosition>
            .SelectObjects(filterClassType.And(filterTemplateID).And(filterComponentSkin).And(filterHeight))
    ...
    {
        var angle = component.Keep.Heading * (Math.PI * 2 / 360); // angle*2pi/360;
    ...
    {
        var angle = component.Keep.Heading * (Math.PI * 2 / 360); // angle*2pi/360;
        var gx = inX - component.X;
        var gy = inY - component.Y;
        var C = Math.Cos(angle);
        var S = Math.Sin(angle);
        switch (component.ComponentHeading)
        {
    ...
    /// <returns></returns>
    public static DBKeepPosition CreatePosition(Type type, int height, GamePlayer player, string guardID,
        GameKeepComponent component)
    {
        var pos = CreatePosition(guardID, component, player);
        pos.Height = height;
    ...
    /// <returns>The position object</returns>
    public static DBKeepPosition CreatePatrolPosition(string guardID, GameKeepComponent component,
        GamePlayer player, AbstractGameKeep.eKeepType keepType)
    {
        var pos = CreatePosition(guardID, component, player);
        pos.Height = 0;
    ...
    public static void AddPosition(DBKeepPosition position)
    {
        ...
        {
            var list = component.Positions[position.TemplateID] as DBKeepPosition[];
            if (list == null)
            {
    ...
    public static PathPoint LoadPatrolPath(string pathID, GameKeepComponent component)
    {
        ...
        var pathType = ePathType.Once;

        if (dbpath != null) pathType = (ePathType) dbpath.PathType;

        if (pathpoints == null) pathpoints = DOLDB<DBPathPoint>.SelectObjects(DB.Column("PathID").IsEqualTo(pathID));

        foreach (var point in pathpoints) sorted.Add(point.Step, point);

        PathPoint prev = null;
        PathPoint first = null;
        for (var i = 0; i < sorted.Count; i++)
        {
            var pp = (DBPathPoint) sorted.GetByIndex(i);
            var p = new PathPoint(pp.X, pp.Y, pp.Z, pp.MaxSpeed, pathType);
    ...
    public static void SavePatrolPath(string pathID, PathPoint path, GameKeepComponent component)
    {
        ...
        var root = MovementMgr.FindFirstPathPoint(path);

        //Set the current pathpoint to the rootpoint!
        path = root;
        var dbp = new DBPath(pathID, ePathType.Loop);
        GameServer.Database.AddObject(dbp);

        var i = 1;
        do
        {
            var dbpp = new DBPathPoint(path.X, path.Y, path.Z, path.MaxSpeed);
            int x, y;
    ...
    {
        var ownerKeepId = doorID / 100000 % 1000;
        var towerNum = doorID / 10000 % 10;
        var keepID = ownerKeepId + towerNum * 256;
        var componentID = doorID / 100 % 100;
        var doorIndex = doorID % 10;

        var keep = GameServer.KeepManager.GetKeepByID(keepID);
        if (keep == null)
        {
            player.Out.SendMessage("Cannot create door as keep is null!", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            return;
        }

        GameKeepComponent component = null;
        foreach (var c in keep.KeepComponents)
            if (c.ID == componentID)
            {
                component = c;
                break;
        ...
        if (component == null)
        {
            player.Out.SendMessage("Cannot create door as component is null!", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            return;
        }
        ...
        player.Out.SendMessage(
            "Added door as a position to keep.  A server restart will be required to load this position.",
