...
public class GuardTemplateMgr
{
    ...
    private static void SetGuardAggression(GameKeepGuard guard)
    {
        ...
        else if (guard is GuardStaticArcher) (guard.Brain as KeepGuardBrain).SetAggression(99, 2100);
    }
    ...
    public static void SetGuardLevel(GameKeepGuard guard)
    {
        ...
        {
            if (guard.Component != null && guard.Component.Keep != null) guard.Component.Keep.SetGuardLevel(guard);
        }
    }
    ...
    private static void SetGuardGender(GameKeepGuard guard)
    {
        ...
        if (guard is GuardFighterRK) guard.Gender = eGender.Male;
    ...
    {
        if (!ServerProperties.Properties.AUTOMODEL_GUARDS_LOADED_FROM_DB && !guard.LoadedFromScript) return;
            ...
            case eRealm.Albion:
            {
                if (guard is GuardArcher)
                {
                    if (guard.Gender == eGender.Male)
                        ...
                        {
                            case 0:
                                guard.Model = SaracenMale;
                                break; //Saracen Male
                            case 1:
                                guard.Model = HighlanderMale;
                                break; //Highlander Male
                            case 2:
                                guard.Model = BritonMale;
                                break; //Briton Male
                            case 3:
                                guard.Model = IcconuMale;
                                break; //Icconu Male
                        }
                    else
                        switch (Util.Random(0, 3))
                        {
                            case 0:
                                guard.Model = SaracenFemale;
                                break; //Saracen Female
                            case 1:
                                guard.Model = HighlanderFemale;
                                break; //Highlander Female
                            case 2:
                                guard.Model = BritonFemale;
                                break; //Briton Female
                            case 3:
                                guard.Model = IcconuFemale;
                                break; //Icconu Female
                        }
                }
                else if (guard is GuardCaster)
                {
                    if (guard.Gender == eGender.Male)
                        ...
                        {
                            case 0:
                                guard.Model = AvalonianMale;
                                break; //Avalonian Male
                            case 1:
                                guard.Model = BritonMale;
                                break; //Briton Male
                        ...
                        {
                            case 0:
                                guard.Model = AvalonianFemale;
                                break; //Avalonian Female
                            case 1:
                                guard.Model = BritonFemale;
                                break; //Briton Female
                    ...
                    if (guard.Gender == eGender.Male)
                        switch (Util.Random(0, 3))
                        {
                            case 0:
                                ...
                                break; //Norse Male
                            case 1:
                                guard.Model = KoboldMale;
                                break; //Kobold Male
                            case 2:
                                guard.Model = DwarfMale;
                                break; //Dwarf Male
                            case 3:
                                guard.Model = ValkynMale;
                                break; //Valkyn Male
                        ...
                        {
                            case 0:
                                guard.Model = NorseFemale;
                                break; //Norse Female
                            case 1:
                                guard.Model = KoboldFemale;
                                break; //Kobold Female
                            case 2:
                                guard.Model = DwarfFemale;
                                break; //Dwarf Female
                            case 3:
                                guard.Model = ValkynFemale;
                                break; //Valkyn Female
            ...
            case eRealm.Hibernia:
            {
                if (guard is GuardArcher)
                {
                    if (guard.Gender == eGender.Male)
                        ...
                        {
                            case 0:
                                guard.Model = LurikeenMale;
                                break; //Lurikeen Male
                            case 1:
                                guard.Model = ElfMale;
                                break; //Elf Male
                            case 2:
                                guard.Model = CeltMale;
                                break; //Celt Male
                        ...
                        {
                            case 0:
                                guard.Model = LurikeenFemale;
                                break; //Lurikeen Female
                            case 1:
                                guard.Model = ElfFemale;
                                break; //Elf Female
                            case 2:
                                guard.Model = CeltFemale;
                                break; //Celt Female
                ...
                else if (guard is GuardFighter)
                {
                    if (guard.Gender == eGender.Male)
                        ...
                        {
                            case 0:
                                guard.Model = FirbolgFemale;
                                break; //Firbolg Female
                            case 1:
                                guard.Model = LurikeenFemale;
                                break; //Lurikeen Female
                            case 2:
                    ...
                    if (guard.Gender == eGender.Male)
                        ...
                        switch (Util.Random(0, 2))
                        {
                            ...
                            case 1:
                                ...
                                break; //Firbolg Female
                            case 2:
                                guard.Model = SylvianFemale;
                                break; //Sylvian Female
                        }
                }
                else if (guard is GuardLord || guard is MissionMaster)
                {
    ...
    private static void SetGuardName(GameKeepGuard guard)
    {
        ...
        {
            guard.Name =
                LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE, "SetGuardName.Hastener");
        ...
        {
            guard.Name = LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE,
                "SetGuardName.Gatekeeper");
        ...
        if (guard is GuardLord)
        {
            ...
            {
                guard.Name = LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE,
                    "SetGuardName.Commander", guard.CurrentZone.Description);
        ...
        switch (guard.ModelRealm)
        {
            ...
            case eRealm.Albion:
            {
                ...
                else if (guard is GuardFighter)
                {
                    ...
                    else
                    {
                        ...
                        {
                            guard.Name = LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE,
                                "SetGuardName.Armsman");
                            guard.TranslationId = "SetGuardName.Armsman";
                        }
                        else
                        {
                            guard.Name = LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE,
                                "SetGuardName.Armswoman");
                ...
                else if (guard is GuardLord && guard.Component != null && guard.Component.Keep != null)
                {
                    ...
                    {
                        guard.Name = LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE,
                            "SetGuardName.Lord", GetKeepShortName(guard.Component.Keep.Name));
                        guard.TranslationId = "SetGuardName.Lord";
                    }
                    else
                    {
                        guard.Name = LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE,
                            "SetGuardName.Lady", GetKeepShortName(guard.Component.Keep.Name));
                ...
                {
                    guard.Name = LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE,
                        "SetGuardName.Infiltrator");
                    guard.TranslationId = "SetGuardName.Infiltrator";
                }
                else if (guard is MissionMaster)
                {
                    guard.Name = LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE,
                        "SetGuardName.CaptainCommander");
            ...
            case eRealm.Midgard:
            {
                ...
                else if (guard is GuardCaster)
                {
                    ...
                    {
                        guard.Name = LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE,
                            "SetGuardName.MasterRunes");
                        guard.TranslationId = "SetGuardName.MasterRunes";
                    }
                    else
                    {
                        guard.Name = LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE,
                            "SetGuardName.Runemaster");
                ...
                else if (guard is GuardFighter)
                {
                    ...
                    {
                        guard.Name = LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE,
                            "SetGuardName.NordicJarl");
                        guard.TranslationId = "SetGuardName.NordicJarl";
                    }
                    else
                    {
                        guard.Name = LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE,
                            "SetGuardName.Huscarl");
                ...
                {
                    guard.Name = LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE,
                        "SetGuardName.Jarl", GetKeepShortName(guard.Component.Keep.Name));
                    guard.TranslationId = "SetGuardName.Jarl";
                }
                else if (guard is GuardStealther)
                {
                    guard.Name = LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE,
                        "SetGuardName.Shadowblade");
                    guard.TranslationId = "SetGuardName.Shadowblade";
                }
                else if (guard is MissionMaster)
                {
                    guard.Name = LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE,
                        "SetGuardName.HersirCommander");
            ...
            case eRealm.Hibernia:
            {
                ...
                else if (guard is GuardCaster)
                {
                    ...
                    {
                        guard.Name = LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE,
                            "SetGuardName.MasterEldritch");
                        guard.TranslationId = "SetGuardName.MasterEldritch";
                    }
                    else
                    {
                        guard.Name = LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE,
                            "SetGuardName.Eldritch");
                ...
                else if (guard is GuardFighter)
                {
                    ...
                    {
                        guard.Name = LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE,
                            "SetGuardName.Champion");
                        guard.TranslationId = "SetGuardName.Champion";
                    }
                    else
                    {
                        guard.Name = LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE,
                            "SetGuardName.Guardian");
                ...
                else if (guard is GuardLord)
                {
                    ...
                    {
                        guard.Name = LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE,
                            "SetGuardName.Chieftain", GetKeepShortName(guard.Component.Keep.Name));
                        guard.TranslationId = "SetGuardName.Chieftain";
                    }
                    else
                    {
                        guard.Name = LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE,
                            "SetGuardName.Chieftess", GetKeepShortName(guard.Component.Keep.Name));
                ...
                {
                    guard.Name = LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE,
                        "SetGuardName.Nightshade");
                    guard.TranslationId = "SetGuardName.Nightshade";
                }
                else if (guard is MissionMaster)
                {
                    guard.Name = LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE,
                        "SetGuardName.ChampionCommander");
        ...
        if (guard.Realm == eRealm.None)
            guard.Name = LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE,
                "SetGuardName.Renegade", guard.Name);
    }
    ...
    public static void SetGuardBrain(GameKeepGuard guard)
    {
        ...
        if (guard is MissionMaster) (guard.Brain as KeepGuardBrain).SetAggression(90, 400);
    }
    ...
    {
        if (guard.IsPortalKeepGuard || guard.Level == 255) guard.MaxSpeedBase = 575;

        if ((guard is GuardLord && guard.Component != null) || guard is GuardStaticArcher ||
        ...
        if (guard is GuardFighterRK) guard.MaxSpeedBase = 375;
    }
    ...
    private static void SetGuardStats(GameKeepGuard guard)
    {
        if (guard is GuardLord)
        {
            ...
            guard.Constitution = (short) ServerProperties.Properties.GAMENPC_BASE_CON;
            guard.Quickness = 60;
        }
        else if (guard is GuardFighterRK)
        {
        ...
        else if (guard is GuardCaster)
        {
            ...
            guard.Constitution = (short) (ServerProperties.Properties.GAMENPC_BASE_CON - 5);
            guard.Quickness = 40;
        }
        else if (guard.IsPortalKeepGuard || guard.Level == 255)
        {
