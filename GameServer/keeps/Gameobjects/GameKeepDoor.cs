...
public class GameKeepDoor : GameLiving, IDoor, IKeepItem
{
    ...
    public int OwnerKeepID => m_doorID / 100000 % 1000;

    public int TowerNum => m_doorID / 10000 % 10;

    public int KeepID => OwnerKeepID + TowerNum * 256;

    public int ComponentID => m_doorID / 100 % 100;

    public int DoorIndex => m_doorID % 10;
    ...
    public override eRealm Realm
    {
        get
        {
            if (Component == null || Component.Keep == null) return eRealm.None;
    ...
    public override byte Level
    {
        get
        {
            if (Component == null || Component.Keep == null) return 0;
    ...
    public override int Health
    {
        ...
        set
        {
            ...
            if (HealthPercent > DOOR_CLOSE_THRESHOLD && m_state == eDoorState.Open) CloseDoor();
        }
    }
    ...
    public override string Name
    {
        get
        {
            ...
            if (Properties.ENABLE_DEBUG) name += " ( C:" + ComponentID + " T:" + TemplateID + ")";
        ...
        if (damageAmount > 0 && IsAlive)
        {
            ...
            if (m_oldHealthPercent != HealthPercent)
            {
                ...
                Parallel.ForEach(GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE).OfType<GamePlayer>(),
                    player => { player?.Client.Out.SendObjectUpdate(this); });
            }
        }
    ...
    public override void ModifyAttack(AttackData attackData)
    {
        ...
        var toughness = Properties.SET_KEEP_DOOR_TOUGHNESS;
        ...
        if (Component.Keep is GameKeepTower) toughness = Properties.SET_TOWER_DOOR_TOUGHNESS;
        ...
        if (!GameServer.KeepManager.IsEnemy(this, player) || player.Client.Account.PrivLevel != 1)
        {
            ...
            else
            {
                ...
                if (IsObjectInFront(player, 180))
                {
                    ...
                    var keepdistance = int.MaxValue;
                    var gatedistance = int.MaxValue;
                    foreach (var c in Component.Keep.KeepComponents)
                    {
    ...
    public void DeleteObject()
    {
        ...
        if (Component != null)
        {
            if (Component.Keep != null) Component.Keep.Doors.Remove(ObjectID.ToString());
    ...
    public override void SaveIntoDatabase()
    {
        ...
        var dbDoor = GameServer.Database.FindObjectByKey<DBDoor>(InternalID);
    ...
    {
        var dbDoor = obj as DBDoor;
        if (dbDoor == null)
            return;

        base.LoadFromDatabase(obj);

        var curZone = WorldMgr.GetZone((ushort) (dbDoor.InternalID / 1000000));
        if (curZone == null)
            return;
        ...
        foreach (AbstractArea area in CurrentAreas)
            ...
            {
                var sKey = dbDoor.InternalID.ToString();
        ...
        m_state = m_isPostern || HealthPercent > DOOR_CLOSE_THRESHOLD
            ? eDoorState.Closed
            : (eDoorState) Enum.ToObject(typeof(eDoorState), dbDoor.State);
    ...
    public virtual void LoadFromPosition(DBKeepPosition pos, GameKeepComponent component)
    {
        ...
        {
            log.Error("Failed to load keep door from keepposition_id =" + pos.ObjectId + ". Component SkinID=" +
                      component.Skin + ". KeepID=" + component.Keep.KeepID);
        }
    }
    ...
    public int GenerateDoorID()
    {
        ...
        {
            var tower = m_component.Keep as GameKeepTower;
        ...
        var componentID = m_component.ID;

        //index not sure yet
        var doorIndex = Position.TemplateType;
        var id = 0;
        //add door type
    ...
    {
        Parallel.ForEach(GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE).OfType<GamePlayer>(),
            player => { player.SendDoorUpdate(this); });
