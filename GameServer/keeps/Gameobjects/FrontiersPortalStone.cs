...
public class FrontiersPortalStone : GameStaticItem, IKeepItem
{
    ...
    public void LoadFromPosition(DBKeepPosition pos, GameKeepComponent component)
    {
        ...
        m_component.Keep.TeleportStone = this;
        AddToWorld();
    }
    ...
    public override string Name => "Frontiers Portal Stone";

    public override ushort Model => 2603;
    ...
    public override bool Interact(GamePlayer player)
    {
        ...
        if (GameServer.KeepManager.FrontierRegionsList.Contains(player.CurrentRegionID))
        {
            if (player.Client.Account.PrivLevel == (int) ePrivLevel.Player)
            {
                ...
                if (Component != null && Component.Keep is GameKeep)
                    if ((Component.Keep as GameKeep).OwnsAllTowers == false ||
                        (Component.Keep as GameKeep).InCombat)
                        return false;
            ...
            var code = eDialogCode.WarmapWindowAlbion;
            switch (player.Realm)
            {
                case eRealm.Albion:
                    code = eDialogCode.WarmapWindowAlbion;
                    break;
                case eRealm.Midgard:
                    code = eDialogCode.WarmapWindowMidgard;
                    break;
                case eRealm.Hibernia:
                    code = eDialogCode.WarmapWindowHibernia;
                    break;
            }
        ...
        if (Component == null &&
            GameServer.KeepManager.FrontierRegionsList.Contains(player.CurrentRegionID) == false)
