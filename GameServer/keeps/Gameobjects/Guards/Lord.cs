...
public class GuardLord : GameKeepGuard
{
    ...
    public override int RealmPointsValue
    {
        get
        {
            ...
            var duration = (GameLoop.GameLoopTime - m_lastSpawnTime) / 1000L;

            if (duration < Properties.LORD_RP_WORTH_SECONDS) return 0;
    ...
    public override int BountyPointsValue
    {
        get
        {
            ...
            if (Component != null && Component.Keep != null) return Component.Keep.BountyPointsValue();
    ...
    public override long ExperienceValue
    {
        get
        {
            ...
            if (Component != null && Component.Keep != null) return Component.Keep.ExperiencePointsValue();
    ...
    public override double ExceedXPCapAmount
    {
        get
        {
            if (Component != null && Component.Keep != null) return Component.Keep.ExceedXPCapAmount();
    ...
    public override long MoneyValue
    {
        get
        {
            ...
            if (Component != null && Component.Keep != null) return Component.Keep.MoneyValue();
    ...
    public override void Die(GameObject killer)
    {
        if (isDeadOrDying == false)
        {
            ...
            if (Properties.LOG_KEEP_CAPTURES)
                try
                {
                    if (Component != null)
                    {
                        var keeplog = new Database.KeepCaptureLog();
                        ...
                        if (Component.Keep.StartCombatTick > 0)
                            keeplog.CombatTime =
                                (int) ((Component.Keep.CurrentRegion.Time - Component.Keep.StartCombatTick) / 1000 /
                                       60);
                        ...
                        lock (XPGainers.SyncRoot)
                        {
                            foreach (System.Collections.DictionaryEntry de in XPGainers)
                            {
                                ...
                                if (living != null) listRPGainers += living.Name + ";";
                            }
                        }
        ...
        if (Component != null) GameServer.ServerRules.ResetKeep(this, killer);
    }
    ...
    public override bool Interact(GamePlayer player)
    {
        ...
        {
            player.Out.SendMessage("You can't talk to the lord while under siege.", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            log.DebugFormat("KEEPWARNING: {0} attempted to interact with {1} of {2} while keep or lord in combat.",
                player.Name, Name, Component.Keep.Name);
            return false;
        }

        if (GameServer.ServerRules.IsAllowedToClaim(player, CurrentRegion))
            player.Out.SendMessage("Would you like to [Claim Keep] now? Or maybe [Release Keep]?",
    ...
    public override void TakeDamage(GameObject source, eDamageType damageType, int damageAmount, int criticalAmount)
    {
        ...
        else if (source is GameNPC && (source as GameNPC).Brain != null &&
                 (source as GameNPC).Brain is IControlledBrain &&
                 ((source as GameNPC).Brain as IControlledBrain).Owner is GamePlayer)
            attacker = ((source as GameNPC).Brain as IControlledBrain).Owner as GamePlayer;

        if ((attacker != null && IsWithinRadius(attacker, distance) == false) ||
            IsWithinRadius(source, distance) == false)
        {
            if (attacker != null)
                attacker.Out.SendMessage(Name + " is immune to damage from this range",
                    eChatType.CT_SpellResisted, eChatLoc.CL_SystemWindow);
            return;
        }

        if (attacker != null && Component != null && Component.Keep != null && IsAlive &&
            !GameServer.ServerRules.IsSameRealm(this, attacker, true))
    ...
    public override bool WhisperReceive(GameLiving source, string str)
    {
        ...
        if (!GameServer.ServerRules.IsSameRealm(this, player, true) ||
            !GameServer.ServerRules.IsAllowedToClaim(player, CurrentRegion))
            return false;
        ...
        switch (str)
        {
            case "Claim Keep":
            {
                ...
                {
                    player.Out.SendDialogBox(eDialogCode.KeepClaim, (ushort) player.ObjectID, 0, 0, 0,
                        eDialogType.YesNo, false, "Do you wish to claim\n" + Component.Keep.Name + "?");
                    return true;
                }
            ...
            case "Release Keep":
            {
                if (PlayerMgr.IsAllowedToInteract(player, Component.Keep, eInteractType.Release)) flag += 4;
    ...
    {
        return new LordBrain();
    }

    protected override void SetStats()
    {
        Strength =
            (short) (Properties.LORD_AUTOSET_STR_BASE + 10 * Level * Properties.LORD_AUTOSET_STR_MULTIPLIER);
        Dexterity = (short) (Properties.LORD_AUTOSET_DEX_BASE + Level * Properties.LORD_AUTOSET_DEX_MULTIPLIER);
        Constitution =
            (short) (Properties.LORD_AUTOSET_CON_BASE + Level * Properties.LORD_AUTOSET_CON_MULTIPLIER);
        Quickness = (short) (Properties.LORD_AUTOSET_QUI_BASE + Level * Properties.LORD_AUTOSET_QUI_MULTIPLIER);
        Intelligence =
            (short) (Properties.LORD_AUTOSET_INT_BASE + Level * Properties.LORD_AUTOSET_INT_MULTIPLIER);
    }
    ...
    {
        base.SetSpeed();
        if (Component != null) MaxSpeedBase = 0;
    }
    ...
    protected override void SetName()
    {
        if (Component == null)
        {
            Name = LanguageMgr.GetTranslation(Properties.SERV_LANGUAGE, "SetGuardName.Commander",
        ...
        switch (ModelRealm)
        {
            ...
            case eRealm.Albion:
                if (Gender == eGender.Male)
                    Name = LanguageMgr.GetTranslation(Properties.SERV_LANGUAGE, "SetGuardName.Lord",
                        GetKeepShortName(Component.Keep.Name));
                else
                    Name = LanguageMgr.GetTranslation(Properties.SERV_LANGUAGE, "SetGuardName.Lady",
                        GetKeepShortName(Component.Keep.Name));
                break;
            case eRealm.Midgard:
                Name = LanguageMgr.GetTranslation(Properties.SERV_LANGUAGE, "SetGuardName.Jarl",
                    GetKeepShortName(Component.Keep.Name));
                break;
            case eRealm.Hibernia:
                if (Gender == eGender.Male)
                    Name = LanguageMgr.GetTranslation(Properties.SERV_LANGUAGE, "SetGuardName.Chieftain",
                        GetKeepShortName(Component.Keep.Name));
                else
                    Name = LanguageMgr.GetTranslation(Properties.SERV_LANGUAGE, "SetGuardName.Chieftess",
                        GetKeepShortName(Component.Keep.Name));
                break;
        }
