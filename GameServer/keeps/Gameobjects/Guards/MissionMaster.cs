...
public class MissionMaster : GameKeepGuard
{
    public override bool Interact(GamePlayer player)
    {
        ...
        if (Component == null)
            SayTo(player,
                ...
                ". We have put out the call far and wide for heroes such as yourself to aid us in our ongoing struggle. It warms my heart good to to see a great " +
                player.CharacterClass.Name +
                " such as yourself willing to lay their life on the line in defence of the [realm].");
        else
            SayTo(player,
                "Hail and well met, " + player.Name +
                "! As the leader of our forces, I am calling upon our finest warriors to aid in the vanquishing of our enemies. Do you wish to do your duty in defence of our [realm]?");
        return true;
    }
    ...
    public override bool WhisperReceive(GameLiving source, string str)
    {
        ...
        if (!GameServer.ServerRules.IsSameRealm(this, player, true)) return false;
        ...
        if (str.ToLower().StartsWith("tower capture"))
        {
            ...
            else
            {
                ...
                player.Group.Mission = new CaptureMission(CaptureMission.eCaptureType.Tower, player.Group,
                    str.ToLower().Replace("tower capture", "").Trim());
            }
        }
        else if (str.ToLower().StartsWith("keep capture"))
        {
            ...
            else
            {
                ...
                player.Group.Mission = new CaptureMission(CaptureMission.eCaptureType.Keep, player.Group,
                    str.ToLower().Replace("keep capture", "").Trim());
            }
        }
        else
        {
            switch (str.ToLower())
            {
                ...
                case "enemy guards":
                {
                    ...
                    player.Group.Mission =
                        new KillMission(typeof(GameKeepGuard), 25, "enemy realm guards", player.Group);
                    break;
                }
                case "realm enemies":
                {
