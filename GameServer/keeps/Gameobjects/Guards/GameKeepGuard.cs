...
public class GameKeepGuard : GameNPC, IKeepItem
{
    ...
    public override void ProcessDeath(GameObject killer)
    {
        if (killer is GamePlayer p && ConquestService.ConquestManager.IsPlayerNearConquestObjective(p))
            ConquestService.ConquestManager.AddContributors(XPGainers.Keys.OfType<GamePlayer>().ToList());
    ...
    public bool IsTowerGuard
    {
        get
        {
            if (Component != null && Component.Keep != null) return Component.Keep is GameKeepTower;
    ...
    /// </summary>
    public override int Mana => 50000;

    public override int MaxHealth =>
        // (base.Level * 4)
        GetModified(eProperty.MaxHealth) + base.Level * 2;
    ...
    public void GuardStartSpellHealCheckLOS(GamePlayer player, ushort response, ushort targetOID)
    {
        ...
        {
            var healSpell = GetGuardHealSmallSpell(Realm);
    ...
    public void GuardStartSpellNukeCheckLOS(GamePlayer player, ushort response, ushort targetOID)
    {
        if ((response & 0x100) == 0x100)
            switch (Realm)
            {
                case eRealm.None:
                case eRealm.Albion:
                    LaunchSpell(47, "Pyromancy");
                    break;
                case eRealm.Midgard:
                    LaunchSpell(48, "Runecarving");
                    break;
                case eRealm.Hibernia:
                    LaunchSpell(47, "Way of the Eclipse");
                    break;
            }
    }
    ...
    private void LaunchSpell(int spellLevel, string spellLineName)
    {
        ...
        var castLine = SkillBase.GetSpellLine(spellLineName);
        var spells = SkillBase.GetSpellList(castLine.KeyName);
    ...
    public static void GuardSpam(GameKeepGuard guard)
    {
        ...
        var inArea = guard.GetEnemyCountInArea();
        var message = LanguageMgr.GetTranslation(Properties.SERV_LANGUAGE, "GameKeepGuard.GuardSpam.Killed",
            guard.Name, guard.Component.Keep.Name, inArea);
    ...
    /// </summary>
    public bool BeenAttackedRecently => GameLoop.GameLoopTime - LastAttackedByEnemyTick < 10 * 1000;
    ...
    public override bool AddToWorld()
    {
        ...
        if (PatrolGroup != null && !m_changingPositions)
        {
            ...
            foreach (var guard in PatrolGroup.PatrolGuards)
                if (guard.IsAlive && guard.CurrentWayPoint != null)
                {
                    CurrentWayPoint = guard.CurrentWayPoint;
                    m_changingPositions = true;
                    MoveTo(guard.CurrentRegionID, guard.X - Util.Random(200, 350), guard.Y - Util.Random(200, 350),
                        guard.Z, guard.Heading);
    ...
    {
        var temp = base.RespawnTimerCallback(respawnTimer);
        RefreshTemplate();
        return temp;
    }
    ...
    public override IList GetExamineMessages(GamePlayer player)
    {
        ...
        list.Add(LanguageMgr.GetTranslation(player.Client.Account.Language,
            "GameKeepGuard.GetExamineMessages.YouTarget", GetName(0, false)));

        if (Realm != eRealm.None)
        {
            list.Add(LanguageMgr.GetTranslation(player.Client.Account.Language,
                "GameKeepGuard.GetExamineMessages.YouExamine", GetName(0, false), GetPronoun(0, true),
                GetAggroLevelString(player, false)));
            if (Component != null)
            {
                var text = "";
                if (Component.Keep.Level > 1 && Component.Keep.Level < 250 &&
                    GameServer.ServerRules.IsSameRealm(player, this, true))
                    text = LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "GameKeepGuard.GetExamineMessages.Upgraded", GetPronoun(0, true), Component.Keep.Level);
                if (Properties.USE_KEEP_BALANCING && Component.Keep.Region == 163 &&
                    !(Component.Keep is GameKeepTower))
                    text += LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "GameKeepGuard.GetExamineMessages.Balancing", GetPronoun(0, true),
                        (Component.Keep.BaseLevel - 50).ToString());
                if (text != "")
                    list.Add(text);
            }
        }
        ...
        switch (form)
        {
            default:
            {
                ...
                if (Gender == eGender.Male)
                    s = LanguageMgr.GetTranslation(Properties.SERV_LANGUAGE,
                        "GameKeepGuard.GetPronoun.He");
                else
                    s = LanguageMgr.GetTranslation(Properties.SERV_LANGUAGE,
                        "GameKeepGuard.GetPronoun.She");
            ...
            case 1:
            {
                ...
                if (Gender == eGender.Male)
                    s = LanguageMgr.GetTranslation(Properties.SERV_LANGUAGE,
                        "GameKeepGuard.GetPronoun.His");
                else
                    s = LanguageMgr.GetTranslation(Properties.SERV_LANGUAGE,
                        "GameKeepGuard.GetPronoun.Hers");
                if (!firstLetterUppercase)
                    s = s.ToLower();
                break;
            }
            case 2:
            {
                ...
                if (Gender == eGender.Male)
                    s = LanguageMgr.GetTranslation(Properties.SERV_LANGUAGE,
                        "GameKeepGuard.GetPronoun.Him");
                else
                    s = LanguageMgr.GetTranslation(Properties.SERV_LANGUAGE,
                        "GameKeepGuard.GetPronoun.Her");
                if (!firstLetterUppercase)
                    s = s.ToLower();
                break;
            }
    ...
    public void DeleteObject()
    {
        if (Component != null)
        {
            ...
            {
                var skey = m_dataObjectID;
                if (Component.Keep.Guards.ContainsKey(skey))
                    Component.Keep.Guards.Remove(skey);
                else if (log.IsWarnEnabled)
                    log.Warn(
                        $"Can't find {Position.ClassType} with dataObjectId {m_dataObjectID} in Component InternalID {Component.InternalID} Guard list.");
            }
            else if (log.IsWarnEnabled)
            {
    ...
    public override void Delete()
    {
        if (HookPoint != null && Component != null)
            Component.Keep.Guards
                .Remove(m_templateID); //Remove(this.ObjectID); LoadFromPosition() uses position.TemplateID as the insertion key
    ...
    public override void DeleteFromDatabase()
    {
        foreach (AbstractArea area in CurrentAreas)
            if (area is KeepArea && Component != null)
                Component.Keep.Guards
                    .Remove(m_dataObjectID); //Remove(this.InternalID); LoadFromDatabase() adds using m_dataObjectID
    ...
    public void ChangeGuild()
    {
        ...
        {
            if (IsAlive) BroadcastLivingEquipmentUpdate();
    ...
    protected virtual KeepGuardBrain GetBrain()
    {
        return new();
    }
    ...
    {
        if (IsPortalKeepGuard) MaxSpeedBase = 575;
    ...
    {
        for (var i = (int) eProperty.Resist_First; i <= (int) eProperty.Resist_Last; i++)
    ...
    {
        Strength = (short) (Properties.GUARD_AUTOSET_STR_BASE +
                            10 * Level * Properties.GUARD_AUTOSET_STR_MULTIPLIER);
        Dexterity = (short) (Properties.GUARD_AUTOSET_DEX_BASE + Level * Properties.GUARD_AUTOSET_DEX_MULTIPLIER);
        Constitution =
            (short) (Properties.GUARD_AUTOSET_CON_BASE + Level * Properties.GUARD_AUTOSET_CON_MULTIPLIER);
        Quickness = (short) (Properties.GUARD_AUTOSET_QUI_BASE + Level * Properties.GUARD_AUTOSET_QUI_MULTIPLIER);
        Intelligence =
            (short) (Properties.GUARD_AUTOSET_INT_BASE + Level * Properties.GUARD_AUTOSET_INT_MULTIPLIER);
    }
    ...
    {
        var iVariance = 1000 * Math.Abs(Properties.GUARD_RESPAWN_VARIANCE);
        var iRespawn = 60 * (Math.Abs(Properties.GUARD_RESPAWN) * 1000 +
    ...
    {
        if (!Properties.AUTOMODEL_GUARDS_LOADED_FROM_DB && !LoadedFromScript) return;
    ...
    private static SpellLine GuardSpellLine { get; } = new("GuardSpellLine", "Guard Spells", "unknown", false);
}
