...
public class Patrol
{
    ...
    /// </summary>
    private AbstractGameKeep.eKeepType m_keepType = AbstractGameKeep.eKeepType.Any;
    ...
    /// </summary>
    public string PatrolID = Database.UniqueID.IDGenerator.GenerateID();

    /// <summary>
    /// The Guard Types that make up the Patrol
    /// </summary>
    public static Type[] GuardTypes = new Type[]
    {
        typeof(GuardFighter), typeof(GuardArcher), typeof(GuardHealer), typeof(GuardFighter), typeof(GuardArcher),
        typeof(GuardFighter)
    };
    ...
    /// </summary>
    public List<GameKeepGuard> PatrolGuards = new();
    ...
    public void InitialiseGuards()
    {
        ...
        if (PatrolGuards.Count < guardsOnPatrol)
            for (var i = 0; i < guardsOnPatrol; i++)
                CreatePatrolGuard(i);

        // tolakram - this might be redundant
        foreach (var guard in PatrolGuards) PositionMgr.LoadGuardPosition(SpawnPosition, guard);
    ...
    {
        var asm = Assembly.GetAssembly(typeof(GameServer));

        if (type < 0) type = 0;
        if (type > GuardTypes.Length - 1) type = GuardTypes.Length - 1;

        var guard = (GameKeepGuard) asm.CreateInstance(GuardTypes[type].FullName, true);
        ...
        if (ServerProperties.Properties.ENABLE_DEBUG) guard.Name += " PatrolID " + PatrolID;
    }

    public void DeletePatrol()
    {
        if (Component != null && Component.Keep != null)
            Component.Keep.Patrols
                .Remove(PatrolID); // .Remove(this) - InitialiseGuards() adds patrols using PatrolID as the key

        foreach (var guard in PatrolGuards)
            guard.DeleteObject();
    ...
    public void ChangePatrolLevel()
    {
        ...
        while (guardsToPatrol > PatrolGuards.Count) CreatePatrolGuard(PatrolGuards.Count);
        ...
        for (var i = 0; i < PatrolGuards.Count; i++)
        {
    ...
    public void GetMovementOffset(GameKeepGuard guard, out int x, out int y)
    {
        ...
        x = 0;
        y = 0;
        var index = PatrolGuards.IndexOf(guard);
        switch (index)
        {
            case 0:
                x = -modifier;
