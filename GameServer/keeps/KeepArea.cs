...
public class KeepArea : Area.Circle
{
    ...
    public KeepArea(AbstractGameKeep keep)
        : base(keep.Name, keep.X, keep.Y, 0,
            keep.IsPortalKeep ? PK_RADIUS : keep is GameKeepTower ? TOWER_RADIUS : KEEP_RADIUS
        )
    {
        Keep = keep;
    }
    ...
    {
        //[Ganrod] Nidel: NPE
        if (player == null || Keep == null) return;

        base.OnPlayerEnter(player);
        if (Keep.Guild != null)
            player.Out.SendMessage("Controlled by " + Keep.Guild.Name + ".", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
    }

    public void ChangeRadius(int newRadius)
    {
        GameServer.KeepManager.Log.Debug("ChangeRadius called for " + Keep.Name + " currently is " + m_Radius +
                                         " changing to " + newRadius);
    ...
    public override void LoadFromDatabase(DBArea area)
    {
        ...
        GameServer.KeepManager.Log.Debug("X: " + area.X + "(" + m_X + ") Y: " + area.Y + "(" + m_Y + ") Region:" +
                                         area.Region + " Radius: " + m_Radius);
    }
}