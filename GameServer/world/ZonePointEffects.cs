...
public class ZonePointEffect
{
    ...
    public static void OnScriptsCompiled(DOLEvent e, object sender, EventArgs args)
    {
        ...
        try
        {
            ...
            zp = new NpcTemplate(DOLDB<DBNpcTemplate>.SelectObjects(DB.Column("TemplateId").IsEqualTo(model))
                .FirstOrDefault());
        ...
        var zonePoints = GameServer.Database.SelectAllObjects<ZonePoint>();
        foreach (var z in zonePoints)
        {
            if (z.SourceRegion == 0) continue;

            // find target region for the current zonepoint
            var r = WorldMgr.GetRegion(z.TargetRegion);
            if (r == null)
            {
                log.Warn("Zonepoint Id (" + z.Id + ") references an inexistent target region " + z.TargetRegion +
                         " - skipping, ZP not created");
                continue;
            }

            var npc = new GameNPC(zp);
