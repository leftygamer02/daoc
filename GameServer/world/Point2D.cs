...
public class Point2D : IPoint2D
{
    ...
    public ushort GetHeading(IPoint2D point)
    {
        ...
        var heading = Math.Atan2(-dx, dy) * RADIAN_TO_HEADING;
    ...
    {
        var angle = heading * HEADING_TO_RADIAN;
        var targetX = X - Math.Sin(angle) * distance;
        var targetY = Y + Math.Cos(angle) * distance;
    ...
    {
        var dx = (double) X - point.X;
        var dy = (double) Y - point.Y;
    ...
    {
        if (radius > ushort.MaxValue) return GetDistance(point) <= radius;

        var rsquared = (uint) radius * (uint) radius;
