...
public class Zone : ITranslatableObject
{
    ...
    private const ushort
        SUBZONE_NBR_ON_ZONE_SIDE = 32; // MUST BE A POWER OF 2 (current implementation limit is 128 inclusive)
    ...
    private static readonly ushort
        SUBZONE_SHIFT = (ushort) Math.Round(Math.Log(SUBZONE_SIZE) / Math.Log(2)); // to get log in base 2

    private static readonly ushort SUBZONE_ARRAY_Y_SHIFT =
        (ushort) Math.Round(Math.Log(SUBZONE_NBR_ON_ZONE_SIDE) / Math.Log(2));
    ...
    public void Delete()
    {
        for (var i = 0; i < SUBZONE_NBR; i++)
            if (m_subZoneElements[i] != null)
            {
                for (var k = 0; k < m_subZoneElements[i].Length; k++)
    ...
    private void InitializeZone()
    {
        ...
        for (var i = 0; i < SUBZONE_NBR; i++)
        {
            ...
            for (var k = 0; k < m_subZoneElements[i].Length; k++) m_subZoneElements[i][k] = new SubNodeElement();
        }
    ...
    public virtual LanguageDataObject.eTranslationIdentifier TranslationIdentifier =>
        LanguageDataObject.eTranslationIdentifier.eZone;
    ...
    /// </summary>
    public int XOffset => m_XOffset;

    /// <summary>
    /// Returns the YOffset of this Zone
    /// </summary>
    public int YOffset => m_YOffset;
    ...
    {
        var xDiff = p_X - m_XOffset;
        var yDiff = p_Y - m_YOffset;
    ...
    public void ObjectEnterZone(GameObject p_Obj)
    {
        ...
        {
            var element = new SubNodeElement();
            element.data = p_Obj;
            ...
            if (log.IsDebugEnabled)
                log.Debug("Object " + p_Obj.ObjectID + " (" + (eGameObjectType) type + ") entering subzone " +
                          subZoneIndex + " in zone " + ID + " in region " + m_Region.ID);
    ...
    private void ObjectEnterZone(eGameObjectType objectType, SubNodeElement element)
    {
        ...
        if (log.IsDebugEnabled)
            log.Debug("Object " + element.data.ObjectID + "(" + objectType + ") entering (inner) subzone " +
                      subZoneIndex + " in zone " + ID + " in region " + m_Region.ID);

        if (subZoneIndex >= 0 && subZoneIndex < SUBZONE_NBR)
        {
            var type = (int) objectType;
    ...
    internal ArrayList GetObjectsInRadius(eGameObjectType type, int x, int y, int z, ushort radius,
        ArrayList partialList, bool ignoreZ)
    {
        if (!m_initialized) InitializeZone();
        // initialise parameters
        var sqRadius = (uint) radius * (uint) radius;
        int referenceSubzoneIndex = GetSubZoneIndex(x, y);
        var typeIndex = (int) type;

        var xInZone = x - m_XOffset; // x in zone coordinates
        var yInZone = y - m_YOffset; // y in zone coordinates

        var cellNbr = (radius >> SUBZONE_SHIFT) + 1; // radius in terms of subzone number
        var xInCell = xInZone >> SUBZONE_SHIFT; // xInZone in terms of subzone coord
        var yInCell = yInZone >> SUBZONE_SHIFT; // yInZone in terms of subzone coord

        var minColumn = xInCell - cellNbr;
        if (minColumn < 0) minColumn = 0;

        var maxColumn = xInCell + cellNbr;
        if (maxColumn > SUBZONE_NBR_ON_ZONE_SIDE - 1) maxColumn = SUBZONE_NBR_ON_ZONE_SIDE - 1;

        var minLine = yInCell - cellNbr;
        if (minLine < 0) minLine = 0;

        var maxLine = yInCell + cellNbr;
        if (maxLine > SUBZONE_NBR_ON_ZONE_SIDE - 1) maxLine = SUBZONE_NBR_ON_ZONE_SIDE - 1;

        var inZoneElements = new DOL.GS.Collections.Hashtable();
        var outOfZoneElements = new DOL.GS.Collections.Hashtable();

        for (var currentLine = minLine; currentLine <= maxLine; ++currentLine)
        {
            var currentSubZoneIndex = 0;
            SubNodeElement startElement = null;

            for (var currentColumn = minColumn; currentColumn <= maxColumn; ++currentColumn)
            {
                ...
                if (startElement != startElement.next)
                {
                    ...
                    if (currentSubZoneIndex == referenceSubzoneIndex)
                    {
                        lock (startElement)
                        {
                            ...
                            UnsafeAddToListWithDistanceCheck(startElement, x, y, z, sqRadius, typeIndex,
                                currentSubZoneIndex, partialList, inZoneElements, outOfZoneElements, ignoreZ);
                            UnsafeUpdateSubZoneTimestamp(currentSubZoneIndex, typeIndex);
                        }
                    }
                    else
                    {
                        var xLeft = currentColumn << SUBZONE_SHIFT;
                        var xRight = xLeft + SUBZONE_SIZE;
                        var yTop = currentLine << SUBZONE_SHIFT;
                        var yBottom = yTop + SUBZONE_SIZE;
                        ...
                        if (CheckMinDistance(xInZone, yInZone, xLeft, xRight, yTop, yBottom, sqRadius))
                        {
                            ...
                            if (CheckMaxDistance(xInZone, yInZone, xLeft, xRight, yTop, yBottom, sqRadius))
                                ...
                                lock (startElement)
                                {
                                    UnsafeAddToListWithoutDistanceCheck(startElement, typeIndex,
                                        currentSubZoneIndex, partialList, inZoneElements, outOfZoneElements);
                                    UnsafeUpdateSubZoneTimestamp(currentSubZoneIndex, typeIndex);
                                }
                            else
        ...
        if (inZoneElements.Count > 0)
        {
            ...
            if (log.IsDebugEnabled) log.Debug("Zone" + ID + " " + inZoneElements.Count + " objects moved inside zone");
        }
    ...
    private void UnsafeAddToListWithoutDistanceCheck(SubNodeElement startElement, int typeIndex, int subZoneIndex,
        ArrayList partialList, DOL.GS.Collections.Hashtable inZoneElements,
        DOL.GS.Collections.Hashtable outOfZoneElements)
    {
        var currentElement = startElement.next;
        ...
        do
        {
            ...
            if (ShouldElementMove(currentElement, typeIndex, subZoneIndex, inZoneElements, outOfZoneElements))
            {
                ...
                if (log.IsDebugEnabled)
                    log.Debug("Zone" + ID + ": " +
                              (currentObject != null ? "object " + currentObject.ObjectID : "ghost object") +
                              " removed for subzone");
            }
            else
            {
                ...
                // => add it
                if (!partialList.Contains(currentObject)) partialList.Add(currentObject);
        ...
        bool ignoreZ)
    {
        ...
        do
        {
            ...
            {
                if (CheckSquareDistance(x, y, z, currentObject.X, currentObject.Y, currentObject.Z, sqRadius,
                        ignoreZ) && !partialList.Contains(currentObject))
                    // the current object exists, is Active and still in the current subzone
    ...
    internal void Relocate(object state)
    {
        ...
        if (m_objectCount > 0)
        {
            ...
            for (var subZoneIndex = 0; subZoneIndex < m_subZoneElements.Length; subZoneIndex++)
            for (var typeIndex = 0; typeIndex < m_subZoneElements[subZoneIndex].Length; typeIndex++)
                if (GameTimer.GetTickCount() > m_subZoneTimestamps[(subZoneIndex << 2) | typeIndex])
                {
                    ...
                    lock (startElement)
                    {
                        if (startElement != startElement.next)
                        {
                            ...
                            do
                            {
                                if (ShouldElementMove(currentElement, typeIndex, subZoneIndex, inZoneElements,
                                        outOfZoneElements))
                                {
                                    elementToRemove = currentElement;
                                    currentElement = currentElement.next;
                                    ...
                                    if (log.IsDebugEnabled)
                                        log.Debug("Zone" + ID + " object " + elementToRemove.data.ObjectID +
                                                  " removed for subzone");
                                }
                                else
                                {
    ...
    {
        var nextUpdateTimestamp = (int) GameTimer.GetTickCount() + MAX_REFRESH_INTERVAL;
    ...
    private bool ShouldElementMove(SubNodeElement currentElement, int typeIndex, int subZoneIndex,
        DOL.GS.Collections.Hashtable inZoneElements, DOL.GS.Collections.Hashtable outOfZoneElements)
    {
        if (!m_initialized) InitializeZone();
        var removeElement = true;
        var currentObject = currentElement.data;
            ...
            && currentObject.CurrentRegion == ZoneRegion)
        {
            ...
            if (currentElementSubzoneIndex == -1)
            {
                ...
                var movedElements = (ArrayList) outOfZoneElements[typeIndex];
            ...
            else
            {
                ...
                if (removeElement = currentElementSubzoneIndex != subZoneIndex)
                {
                    ...
                    var newSubZoneStartElement =
                        m_subZoneElements[currentElementSubzoneIndex][typeIndex];
                    var movedElements = (ArrayList) inZoneElements[newSubZoneStartElement];
    ...
    private void PlaceElementsInZone(DOL.GS.Collections.Hashtable elements)
    {
        ...
        var entryEnumerator = elements.GetEntryEnumerator();
    ...
    private void PlaceElementsInOtherZones(DOL.GS.Collections.Hashtable elements)
    {
        ...
        while (entryEnumerator.MoveNext())
        {
            ...
            for (var i = 0; i < currentList.Count; i++)
            {
                ...
                if (currentZone != null) currentZone.ObjectEnterZone((eGameObjectType) currentType, currentElement);
            }
        }
    }
        ...
        bool ignoreZ)
    {
        ...
        if (dist > sqDistance) return false;
    ...
    private bool CheckMaxDistance(int x, int y, int xLeft, int xRight, int yTop, int yBottom, uint squareRadius)
    {
        ...
        var xdiff = Math.Max(FastMath.Abs(x - xLeft), FastMath.Abs(x - xRight));
        var ydiff = Math.Max(FastMath.Abs(y - yTop), FastMath.Abs(y - yBottom));
        distance = (long) xdiff * xdiff + (long) ydiff * ydiff;
    ...
    {
        var npcs = GetNPCsOfZone(new eRealm[] {realm}, 0, 0, compareLevel, conLevel, true);
        var randomNPC = npcs.Count == 0 ? null : npcs[0];
        return randomNPC;
    }
    ...
    {
        var npcs = GetNPCsOfZone(realms, minLevel, maxLevel, 0, 0, true);
        var randomNPC = npcs.Count == 0 ? null : npcs[0];
        return randomNPC;
    }
    ...
    /// <returns></returns>
    public List<GameNPC> GetNPCsOfZone(eRealm[] realms, int minLevel, int maxLevel, int compareLevel, int conLevel,
        bool firstOnly)
    {
        var list = new List<GameNPC>();

        try
        {
            if (!m_initialized) InitializeZone();
            // select random starting subzone and iterate over all objects in subzone than in all subzone...
            var currentSubZoneIndex = Util.Random(SUBZONE_NBR);
            var startSubZoneIndex = currentSubZoneIndex;
            GameNPC currentNPC = null;
            var stopSearching = false;
            do
            {
                var startElement = m_subZoneElements[currentSubZoneIndex][(int) eGameObjectType.NPC];
                lock (startElement)
                {
                    // if list is not empty
                    if (startElement != startElement.next)
                    {
                        var curElement = startElement.next;
                        do
                        {
                            ...
                            for (var i = 0; i < realms.Length; ++i)
                            {
                                ...
                                if (currentNPC.Realm == realm)
                                {
                                    ...
                                    {
                                        addToList = (int) GameObject.GetConLevel(compareLevel, currentNPC.Level) ==
                                                    conLevel;
                                    }
                                    else
                                    {
                ...
                if (++currentSubZoneIndex >= SUBZONE_NBR) currentSubZoneIndex = 0;
