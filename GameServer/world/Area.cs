...
public class Area
{
    public class Square : AbstractArea
    {
        ...
        {
            var m_xdiff = (long) x - X;
            if (m_xdiff < 0 || m_xdiff > Width)
                return false;

            var m_ydiff = (long) y - Y;
    ...
    public class Polygon : AbstractArea
    {
        ...
        public string StringPoints
        {
            ...
            set
            {
                ...
                var points = m_stringpoints.Split('|');
                foreach (var point in points)
                {
                    var pts = point.Split(';');
                    if (pts.Length != 2) continue;
                    var x = Convert.ToInt32(pts[0]);
                    var y = Convert.ToInt32(pts[1]);
        ...
        public override bool IsContaining(IPoint3D obj)
        {
            ...
            var oldpt = new Point2D(m_points[m_points.Count - 1].X, m_points[m_points.Count - 1].Y);

            foreach (var pt in m_points)
            {
                var newpt = new Point2D(pt.X, pt.Y);
    ...
    public class BindArea : Circle
    {
        ...
        public BindPoint BindPoint => m_dbBindPoint;
