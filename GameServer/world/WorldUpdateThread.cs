...
public static class WorldUpdateThread
{
    ...
    private static uint GetPlayertoPlayerUpdateInterval =>
        Math.Max(ServerProperties.Properties.WORLD_PLAYERTOPLAYER_UPDATE_INTERVAL,
            MIN_PLAYER_UPDATE_RATE);
    ...
    private static void UpdatePlayerNPCs(GamePlayer player, long nowTicks)
    {
        ...
        var npcs = player.GetNPCsInRadius(WorldMgr.VISIBILITY_DISTANCE).Cast<GameNPC>()
            .Where(n => n != null && n.IsVisibleTo(player)).ToArray();
        ...
        try
        {
            ...
            foreach (var objEntry in player.Client.GameObjectUpdateArray)
            {
                ...
                if (obj is GameNPC && ((GameNPC) obj).Brain is IControlledBrain &&
                    ((IControlledBrain) ((GameNPC) obj).Brain).GetPlayerOwner() == player)
                    continue;

                // We have a NPC in cache that is not in vincinity
                if (obj is GameNPC && !npcs.Contains((GameNPC) obj) &&
                    nowTicks - objEntry.Value >= GetPlayerNPCUpdateInterval)
                {
        ...
        try
        {
            ...
            foreach (var lnpc in npcs)
            {
                ...
                if (player.Client.GameObjectUpdateArray.TryGetValue(
                        new Tuple<ushort, ushort>(npc.CurrentRegionID, (ushort) npc.ObjectID), out lastUpdate))
                {
                    // This NPC Needs Update
                    if (nowTicks - lastUpdate >= GetPlayerNPCUpdateInterval) player.Client.Out.SendObjectUpdate(npc);
                }
                else
                {
    ...
    private static void UpdatePlayerItems(GamePlayer player, long nowTicks)
    {
        ...
        var objs = player.GetItemsInRadius(WorldMgr.OBJ_UPDATE_DISTANCE).Cast<GameStaticItem>()
            .Where(i => i != null && i.IsVisibleTo(player)).ToArray();
        ...
        {
            if (log.IsErrorEnabled)
                log.ErrorFormat("Error while Cleaning Static Item cache for Player : {0}, Exception : {1}",
                    player.Name, e);
        }
        ...
        try
        {
            ...
            foreach (var lobj in objs)
            {
                var staticObj = lobj;
                // Get last update time
                long lastUpdate;
                if (player.Client.GameObjectUpdateArray.TryGetValue(
                        new Tuple<ushort, ushort>(staticObj.CurrentRegionID, (ushort) staticObj.ObjectID),
                        out lastUpdate))
                {
        ...
        {
            if (log.IsErrorEnabled)
                log.ErrorFormat("Error while updating Static Item for Player : {0}, Exception : {1}", player.Name,
                    e);
        }
    }
    ...
    private static void UpdatePlayerDoors(GamePlayer player, long nowTicks)
    {
        ...
        var doors = player.GetDoorsInRadius(WorldMgr.OBJ_UPDATE_DISTANCE).Cast<IDoor>().OfType<GameObject>()
            .Where(o => o.IsVisibleTo(player)).ToArray();
        ...
        {
            if (log.IsErrorEnabled)
                log.ErrorFormat("Error while Cleaning Doors cache for Player : {0}, Exception : {1}", player.Name,
                    e);
        }
        ...
        try
        {
            ...
            foreach (IDoor ldoor in doors)
            {
                ...
                long lastUpdate;
                if (player.Client.GameObjectUpdateArray.TryGetValue(
                        new Tuple<ushort, ushort>(((GameObject) door).CurrentRegionID, (ushort) door.ObjectID),
                        out lastUpdate))
                {
                    // This Door Needs Update
                    if (nowTicks - lastUpdate >= GetPlayerItemUpdateInterval) player.SendDoorUpdate(door);
                }
                else
                {
    ...
    public static void UpdatePlayerHousing(GamePlayer player, long nowTicks)
    {
        ...
        var housesDict = HouseMgr.GetHouses(player.CurrentRegionID);
        // Build Vincinity List
        var houses = housesDict.Values
            .Where(h => h != null && player.IsWithinRadius(h, HousingConstants.HouseViewingDistance)).ToArray();
        ...
        try
        {
            ...
            foreach (var houseEntry in player.Client.HouseUpdateArray)
            {
                ...
                var house = HouseMgr.GetHouse(houseKey.Item1, houseKey.Item2);
        ...
        {
            if (log.IsErrorEnabled)
                log.ErrorFormat("Error while Cleaning House cache for Player : {0}, Exception : {1}", player.Name,
                    e);
        }
        ...
        try
        {
            ...
            foreach (var lhouse in houses)
            {
                ...
                if (player.Client.HouseUpdateArray.TryGetValue(
                        new Tuple<ushort, ushort>(house.RegionID, (ushort) house.HouseNumber), out lastUpdate))
                {
    ...
    private static bool StartPlayerUpdateTask(GameClient client,
        IDictionary<GameClient, Tuple<long, Task, Region>> clientsUpdateTasks, long begin)
    {
        ...
        if (!clientsUpdateTasks.TryGetValue(client, out clientEntry))
        {
            ...
            clientEntry = new Tuple<long, Task, Region>(begin,
                Task.Factory.StartNew(() => UpdatePlayerWorld(player)), player.CurrentRegion);
        ...
        else
        {
            ...
            var lastUpdate = clientEntry.Item1;
            var taskEntry = clientEntry.Item2;
            var lastRegion = clientEntry.Item3;
            ...
            if (!taskEntry.IsCompleted)
            {
                ...
                if (begin - lastUpdate > GetPlayerWorldUpdateInterval)
                    if (log.IsWarnEnabled && GameLoop.GameLoopTime -
                        player.TempProperties.getProperty<long>("LAST_WORLD_UPDATE_THREAD_WARNING", 0) >= 1000)
                    {
                        log.WarnFormat(
                            "Player Update Task ({0}) Taking more than world update refresh rate : {1} ms (real {2} ms) - Task Status : {3}!",
                            player.Name, GetPlayerWorldUpdateInterval, begin - lastUpdate, taskEntry.Status);
                        player.TempProperties.setProperty("LAST_WORLD_UPDATE_THREAD_WARNING",
            ...
            if (taskEntry.IsFaulted)
                if (log.IsErrorEnabled)
                    log.ErrorFormat("Error in World Update Thread, Player Task ({0})! Exception : {1}", player.Name,
                        taskEntry.Exception);
            ...
            if (PlayerNeedUpdate(lastUpdate))
            {
                ...
                var newClientEntry = new Tuple<long, Task, Region>(begin,
                    Task.Factory.StartNew(() => UpdatePlayerWorld(player)), lastRegion);
                // Register Tuple
    ...
    private static bool IsTaskCompleted(GameClient client,
        IDictionary<GameClient, Tuple<long, Task, Region>> clientsUpdateTasks)
    {
        Tuple<long, Task, Region> clientEntry;
    ...
    public static void WorldUpdateThreadStart()
    {
        ...
        while (running)
            try
            {
                ...
                var begin = GameLoop.GameLoopTime;

                // Get All Clients
                var clients = WorldMgr.GetAllClients();

                // Clean Tasks Dict on Client Exiting.
                foreach (var cli in clientsUpdateTasks.Keys.ToArray())
                {
                    if (cli == null)
                        continue;

                    var player = cli.Player;

                    var notActive = cli.ClientState != GameClient.eClientState.Playing || player == null ||
                                    player.ObjectState != GameObject.eObjectState.Active;
                    var notConnected = !clients.Contains(cli);
                ...
                for (var cl = 0; cl < clients.Count; cl++)
                {
                    ...
                    {
                        if (log.IsErrorEnabled)
                            log.Error("account has no active player but is playing, disconnecting! => " +
                                      client.Account.Name);
                ...
                var took = GameLoop.GameLoopTime - begin;
