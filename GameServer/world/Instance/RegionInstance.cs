...
public class RegionInstance : BaseInstance
{
    ...
    /// </summary>
    public override void LoadFromDatabase(Mob[] mobObjs, ref long mobCount, ref long merchantCount,
        ref long itemCount, ref long bindCount)
    {
        ...
        if (mobObjs.Length > 0)
            foreach (var mob in mobObjs)
            {
                ...
                // Default Classtype
                var classtype = ServerProperties.Properties.GAMENPC_DEFAULT_CLASSTYPE;
        ...
        if (staticObjs.Count > 0)
            ...
            {
                var itemclone = (WorldObject) item.Clone();
        ...
        foreach (var area in areaObjs)
        {
            ...
            // clone DB object.
            var newDBArea = (DBArea) area.Clone();
            newDBArea.AllowAdd = false;
            newDBArea.Region = ID;
            // Instantiate Area with cloned DB object and add to region
            try
            {
                var newArea = (AbstractArea) gasm.CreateInstance(newDBArea.ClassType, false);
        ...
        if (myMobCount + myItemCount + myMerchantCount > 0)
        {
            if (log.IsInfoEnabled)
                log.Info(string.Format(
                    "AdventureWingInstance: {0} ({1}) loaded {2} mobs, {3} merchants, {4} items, {5}/{6} areas from DB ({7})",
                    Description, ID, myMobCount, myMerchantCount, myItemCount, areaCnt, areaObjs.Count,
                    TimeManager.Name));
