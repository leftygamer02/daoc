...
public class Instance : BaseInstance
{
    ...
    public virtual void LoadFromDatabase(string instanceName)
    {
        ...
        foreach (var entry in objects)
        {
            ...
            switch (entry.ClassType)
            {
                case "entrance":
                {
                    ...
                    m_entranceLocation = new GameLocation(instanceName + "entranceRegion" + ID, ID, entry.X,
                        entry.Y, entry.Z, entry.Heading);
        ...
        log.Info("Successfully loaded a db entry to " + Description + " - Region ID " + ID + ". Loaded Entities: " +
                 count);
    }
