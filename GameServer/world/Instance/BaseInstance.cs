...
public class BaseInstance : Region
{
    ...
    public virtual void Start()
    {
        ...
        foreach (var z in m_zones) m_zoneSkinMap.Add(z.ZoneSkinID, z);
    }
    ...
    /// </summary>
    public override ushort ID => m_regionID;
    ...
    /// </summary>
    public override ushort Skin => m_skinID;
    ...
    public virtual void OnPlayerLeaveInstance(GamePlayer player)
    {
        ...
        {
            log.Warn("Instance now empty, destroying instance " + Description + ", ID: " + ID + ", type=" +
                     GetType().ToString() + ".");
            WorldMgr.RemoveInstance(this);
        }
    }
    ...
    protected class AutoCloseRegionTimer : GameTimer
    {
        ...
        protected override void OnTick()
        {
            ...
            {
                log.Warn(
                    "Players were still in the region on AutoRemoveregionTimer Tick! Please check the overridden voids OnPlayerEnter/Exit to ensure that a 'base.OnPlayerEnter/Exit' is included!");
            }
            else
            {
                //Collapse the zone!
                //Thats my favourite bit ;)
                log.Info(m_instance.Name + " (ID: " + m_instance.ID +
                         ") just reached the timeout for the removal timer. The region is empty, and will now be demolished and removed from the world. Entering OnCollapse!");
                Stop();
    ...
    /// </summary>
    protected Dictionary<int, Zone> m_zoneSkinMap = new();
    ...
    public override IList<IArea> GetAreasOfZone(Zone zone, IPoint3D p, bool checkZ)
    {
        ...
        if (checkZone == null) return areas;

        // Players will always request the skinned zone so map it to the actual instance zone
        if (m_zoneSkinMap.ContainsKey(zone.ID)) checkZone = m_zoneSkinMap[zone.ID];

        var zoneIndex = Zones.IndexOf(checkZone);
        ...
        if (zoneIndex >= 0)
            lock (m_lockAreas)
            {
                try
                {
                    ...
                    {
                        var area = (IArea) Areas[m_ZoneAreas[zoneIndex][i]];
                ...
                {
                    log.Error(
                        "GetAreaOfZone: Caught exception for Zone " + zone.Description + ", Area count " +
                        m_ZoneAreasCount[zoneIndex] + ".", e);
                }
            }
    ...
    public IEnumerable<GameNPC> GetMobsInsideInstance(bool alive)
    {
        ...
        {
            if (alive)
                return new List<GameNPC>(from regionObjects in Objects
                    where regionObjects is GameNPC &&
                          (((GameNPC) regionObjects).Flags & GameNPC.eFlags.PEACE) != GameNPC.eFlags.PEACE &&
                          ((GameNPC) regionObjects).IsAlive
                    select (GameNPC) regionObjects);
            else
                return new List<GameNPC>(from regionObjects in Objects
                    where regionObjects is GameNPC && (((GameNPC) regionObjects).Flags & GameNPC.eFlags.PEACE) !=
                        GameNPC.eFlags.PEACE
                    select (GameNPC) regionObjects);
        }
    }
