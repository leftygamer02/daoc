...
public class AdventureWingInstance : RegionInstance
{
    ...
    public void UpdateInstanceOwner()
    {
        ...
        var stillOwner = false;

        // group instance
        if (m_group != null)
            // check if group still inside
            foreach (var ininstance in PlayersInside)
        ...
        ref long itemCount, ref long bindCount)
    {
        ...
        foreach (var mob in GetMobsInsideInstance(true)) mob.RespawnInterval = -1;
    }
    ...
    public override void OnPlayerLeaveInstance(GamePlayer player)
    {
        ...
        else
        {
            ...
            foreach (var mob in GetMobsInsideInstance(true))
            {
                // there is still something => standard autoclosure + break;
                log.Warn("Instance now empty, will destroy instance " + Description + ", ID: " + ID + ", type=" +
                         GetType().ToString() + ". In " +
                         ServerProperties.Properties.ADVENTUREWING_TIME_TO_DESTROY + " min.");
