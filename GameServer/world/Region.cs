...
public class Region
{
    ...
    /// </summary>
    public readonly object ObjectsSyncLock = new();
    ...
    /// </summary>
    public int TotalNumberOfObjects => m_objectsInRegion;
    ...
    protected object m_lockAreas = new();
    ...
    protected Dictionary<ushort, IArea> Areas => m_Areas;
    ...
    public static Region Create(GameTimer.TimeManager time, RegionData data)
    {
        try
        {
            ...
            if (string.IsNullOrEmpty(data.ClassType) == false)
            {
                ...
                if (t == null) t = ScriptMgr.GetType(data.ClassType);

                if (t != null)
                {
                    var info = t.GetConstructor(new Type[]
                        {typeof(GameTimer.TimeManager), typeof(RegionData)});

                    var r = (Region) info.Invoke(new object[] {time, data});
        ...
        {
            log.ErrorFormat("Failed to start region {0} with requested classtype: {1}.  Exception: {2}!", data.Id,
                data.ClassType, ex.Message);
        }
    ...
    public Region(GameTimer.TimeManager time, RegionData data)
    {
        ...
        for (var i = 0; i < 64; i++) m_ZoneAreas[i] = new ushort[AbstractArea.MAX_AREAS_PER_ZONE];
    ...
    public virtual void OnCollapse()
    {
        ...
        foreach (var z in m_zones) z.Delete();
    ...
    /// </summary>
    public virtual string Name => m_regionData.Name;
    ...
    /// </summary>
    public virtual string Description => m_regionData.Description;
    ...
    /// </summary>
    public virtual int Expansion => m_regionData.Expansion + 1;

    /// <summary>
    /// Gets or Sets the water level in this region
    /// </summary>
    public virtual int WaterLevel => m_regionData.WaterLevel;

    /// <summary>
    /// Gets or Sets diving flag for region
    /// Note: This flag should normally be checked at the zone level
    /// </summary>
    public virtual bool IsRegionDivingEnabled => m_regionData.DivingEnabled;

    /// <summary>
    /// Does this region contain housing?
    /// </summary>
    public virtual bool HousingEnabled => m_regionData.HousingEnabled;
    ...
    public virtual bool IsPM
    {
        get
        {
            ...
            var hour = cTime / 1000 / 60 / 60;
            var pm = hour >= 12 && hour <= 23;
    ...
    public virtual void LoadFromDatabase(Mob[] mobObjs, ref long mobCount, ref long merchantCount,
        ref long itemCount, ref long bindCount)
    {
        if (!LoadObjects)
            return;

        var gasm = Assembly.GetAssembly(typeof(GameServer));
        ...
        var count = mobObjs.Length + staticObjs.Count;
        if (count > 0) PreAllocateRegionSpace(count + 100);
        var myItemCount = staticObjs.Count;
        var myMobCount = 0;
        var myMerchantCount = 0;
        var myBindCount = bindPoints.Count;
        ...
        if (mobObjs.Length > 0)
            foreach (var mob in mobObjs)
            {
                ...
                // Default Classtype
                var classtype = Properties.GAMENPC_DEFAULT_CLASSTYPE;

                // load template if any
                INpcTemplate template = null;
                if (mob.NPCTemplateID != -1) template = NpcTemplateMgr.GetTemplate(mob.NPCTemplateID);


                if (Properties.USE_NPCGUILDSCRIPTS && mob.Guild.Length > 0 && mob.Realm >= 0 &&
                    mob.Realm <= (int) eRealm._Last)
                {
                    var type = ScriptMgr.FindNPCGuildScriptClass(mob.Guild, (eRealm) mob.Realm);
                    if (type != null)
                        try
                        {
                ...
                if (myMob == null)
                {
                    if (template != null && template.ClassType != null && template.ClassType.Length > 0 &&
                        template.ClassType != Mob.DEFAULT_NPC_CLASSTYPE && template.ReplaceMobValues)
                        classtype = template.ClassType;
                    else if (mob.ClassType != null && mob.ClassType.Length > 0 &&
                             mob.ClassType != Mob.DEFAULT_NPC_CLASSTYPE)
                ...
                if (myMob != null)
                {
                    ...
                    catch (Exception e)
                    {
                        if (log.IsErrorEnabled)
                            log.Error(
                                "Failed: " + myMob.GetType().FullName + ":LoadFromDatabase(" +
                                mob.GetType().FullName + ");", e);
                        throw;
                    }
        ...
        if (staticObjs.Count > 0)
            foreach (var item in staticObjs)
            {
        ...
        foreach (var point in bindPoints) AddArea(new Area.BindArea("bind point", point));

        if (myMobCount + myItemCount + myMerchantCount + myBindCount > 0)
        {
            if (log.IsInfoEnabled)
                log.Info(string.Format(
                    "Region: {0} ({1}) loaded {2} mobs, {3} merchants, {4} items {5} bindpoints, from DB ({6})",
                    Description, ID, myMobCount, myMerchantCount, myItemCount, myBindCount, TimeManager.Name));

            log.Debug("Used Memory: " + GC.GetTotalMemory(false) / 1024 / 1024 + "MB");

            if (allErrors != string.Empty)
                log.Error("Error loading the following NPC ClassType(s), GameNPC used instead:" +
                          allErrors.TrimEnd(','));
    ...
    internal bool AddObject(GameObject obj)
    {
        ...
        lock (ObjectsSyncLock)
        {
            if (obj.ObjectID != -1)
            {
                ...
                log.Warn(obj.Name + " should be added to " + Description + " but had already an OID(" +
                         obj.ObjectID + ") => not added\n" + Environment.StackTrace);
                return false;
            }

            var objectsRef = m_objects;

            //*** optimized object management for memory saving primary but keeping it very fast - Blue ***

            // find first free slot for the object
            var objID = m_nextObjectSlot;
            if (objID >= m_objects.Length || m_objects[objID] != null)
            {
                ...
                if (m_objects.Length > m_objectsInRegion)
                {
                    ...
                    while (!found && i >= 0)
                    {
                        if (m_objectsAllocatedSlots[i] != 0xffffffff)
                        {
                            ...
                            var currentIndex = i * 32;
                            var upperBound = (i + 1) * 32;
                ...
                else
                {
                    ...
                    else if (objectsRef.Length >= Properties.REGION_MAX_OBJECTS)
                    {
                        // no available slot
                        if (log.IsErrorEnabled)
                            log.Error("Can't add new object - region '" + Description + "' is full. (object: " +
                                      obj.ToString() + ")");
                        return false;
                    }
                    else
                    {
                        ...
                        var size = (int) (m_objects.Length * 1.20);
                    ...
                    var diff = objectsRef.Length / 32 - m_objectsAllocatedSlots.Length;
                    if (diff >= 0)
                    {
                        var newBitArray =
                            new uint[Math.Max(m_objectsAllocatedSlots.Length + diff + 50,
                                100)]; // add at least 100 integers, makes it resize less often, serves 3200 new objects, only 400 bytes
            ...
            else
            {
                // no available slot
                if (log.IsErrorEnabled)
                    log.Error("Can't add new object - region '" + Description + "' (object: " + obj.ToString() +
                              "); OID is used by " + oidObj.ToString());
                return false;
            }
        }
    }
    ...
    internal void RemoveObject(GameObject obj)
    {
        lock (ObjectsSyncLock)
        {
            ...
            var inPlace = m_objects[obj.ObjectID - 1];
            if (inPlace == null)
            {
                log.Error("RemoveObject conflict! OID" + obj.ObjectID + " " + obj.Name + "(" + obj.CurrentRegionID +
                          ") but there was no object at that slot");
            ...
            {
                log.Error("RemoveObject conflict! OID" + obj.ObjectID + " " + obj.Name + "(" + obj.CurrentRegionID +
                          ") but there was another object already " + inPlace.Name + " region:" +
                          inPlace.CurrentRegionID + " state:" + inPlace.ObjectState);
                log.Error(new StackTrace().ToString());
                return;
            }
    ...
    public Zone GetZone(int x, int y)
    {
        ...
        foreach (var zone in m_zones)
            if (zone.XOffset <= varX && zone.YOffset <= varY && zone.XOffset + zone.Width > varX &&
                zone.YOffset + zone.Height > varY)
                return zone;
    ...
    public virtual IArea AddArea(IArea area)
    {
        lock (m_lockAreas)
        {
            ...
            foreach (var areaID in m_Areas.Keys)
    ...
    public virtual void RemoveArea(IArea area)
    {
        lock (m_lockAreas)
        {
            if (m_Areas.ContainsKey(area.ID) == false) return;

            m_Areas.Remove(area.ID);
            var ZoneCount = Zones.Count;

            for (var zonePos = 0; zonePos < ZoneCount; zonePos++)
            for (var areaPos = 0; areaPos < m_ZoneAreasCount[zonePos]; areaPos++)
                if (m_ZoneAreas[zonePos][areaPos] == area.ID)
                {
                    // move the remaining m_ZoneAreas array one to the left
                    for (var i = areaPos; i < m_ZoneAreasCount[zonePos] - 1; i++)
    ...
    public virtual IList<IArea> GetAreasOfZone(Zone zone, IPoint3D p, bool checkZ)
    {
        lock (m_lockAreas)
        {
            ...
            if (zoneIndex >= 0)
                try
                {
                    for (var i = 0; i < m_ZoneAreasCount[zoneIndex]; i++)
                    {
                        var area = (IArea) m_Areas[m_ZoneAreas[zoneIndex][i]];
                        if (area.IsContaining(p, checkZ)) areas.Add(area);
                    }
                }
                catch (Exception e)
                {
    ...
    protected IEnumerable GetInRadius(Zone.eGameObjectType type, int x, int y, int z, ushort radius,
        bool withDistance, bool ignoreZ)
    {
        // check if we are around borders of a zone
        var startingZone = GetZone(x, y);

        if (startingZone != null)
        {
            var res = startingZone.GetObjectsInRadius(type, x, y, z, radius, new ArrayList(), ignoreZ);

            var sqRadius = (uint) radius * radius;
        ...
        else
        {
            if (log.IsDebugEnabled)
                log.Error("GetInRadius starting zone is null for (" + type + ", " + x + ", " + y + ", " + z + ", " +
                          radius + ") in Region ID=" + ID);
    ...
    private static bool CheckShortestDistance(Zone zone, int x, int y, uint squareRadius)
    {
        ...
        var xLeft = zone.XOffset;
        var xRight = zone.XOffset + zone.Width;
        var yTop = zone.YOffset;
        var yBottom = zone.YOffset + zone.Height;
        long distance = 0;
    ...
    public class ObjectEnumerator : IEnumerator, IEnumerable
    {
        ...
        /// </summary>
        public virtual object Current => m_currentObj;
    ...
    public class PlayerDistanceEnumerator : DistanceEnumerator
    {
        ...
        public override object Current
        {
            get
            {
                var obj = (GamePlayer) m_currentObj;
    ...
    public class ItemDistanceEnumerator : DistanceEnumerator
    {
        ...
        public override object Current
        {
            get
            {
                var obj = (GameStaticItem) m_currentObj;
    ...
    {
        foreach (var zone in m_zones) zone.Relocate(null);
