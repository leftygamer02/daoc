...
namespace DOL.GS
{
  ...
  public sealed class PathCalculator
  {
    ...
    {
      if (owner.Position.Distance(target) < MIN_PATHING_DISTANCE)
    ...
    public async Task<Tuple<Vector3?, NoPathReason>> CalculateNextTargetAsync(Vector3 target)
    {
      ...
      var next = _pathNodes.Peek();
            //Temp commented out
      //if (Owner.DebugMode)
      //  Owner.DebugSend("Sending NPC {0} to {1} to reach {2} (remaining nodes={3}; dist={4})", Owner.Name, next, target, _pathNodes.Count, next.Position.Distance(Owner.Position));
