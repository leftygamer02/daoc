...
namespace DOL.GS
{
    ...
    public class ShardedRpcPathingMgr : IPathingMgr
    {
        ...
        private PathingService.PathingServiceClient InitClient(int shard)
        {
            ...
            var channel = new Channel("localhost", FIRST_PORT + shard, ChannelCredentials.Insecure);
            return new PathingService.PathingServiceClient(channel);
        }

        private PathingService.PathingServiceClient PickClient(Zone zone)
        {
            var shardsThatCanServe = NavmeshAvailableOnWhichShards[zone.ID];
            var index = LastUsedIndex[zone.ID]++;
        ...
        {
            if (NavmeshAvailableOnWhichShards[zone.ID].Count == 0)
            {
        ...
        {
            return zone != null && NavmeshAvailableOnWhichShards[zone.ID].Count > 0;
        }
