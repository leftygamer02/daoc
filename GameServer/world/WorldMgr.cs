...
public static class WorldMgr
{
    ...
    private static object m_syncTeleport = new();
    ...
    public static Dictionary<string, DateTime> RvRLinkDeadPlayers = new();
    ...
    {
        var realm = (eRealm) teleport.Realm;
        var teleportKey = string.Format("{0}:{1}", teleport.Type, teleport.TeleportID);
    ...
    public static bool EarlyInit(out RegionData[] regionsData)
    {
        ...
        if (ServerProperties.Properties.DEBUG_LOAD_REGIONS != string.Empty)
            foreach (var loadRegion in Util.SplitCSV(ServerProperties.Properties.DEBUG_LOAD_REGIONS, true))
        ...
        var hasFrontierRegion = false;

        var regions = new List<RegionData>(512);
        foreach (var dbRegion in GameServer.Database.SelectAllObjects<DBRegions>())
        {
        ...
        var cpuCount = GameServer.Instance.Configuration.CPUUse;
        if (cpuCount < 1)
            cpuCount = 1;

        var timers = new GameTimer.TimeManager[cpuCount];
        ...
        if (hasFrontierRegion == false)
        {
            ...
            if (m_regions.TryGetValue(Keeps.DefaultKeepManager.DEFAULT_FRONTIERS_REGION, out frontier))
                ...
                log.ErrorFormat("Can't find default Frontier region {0}!",
                    Keeps.DefaultKeepManager.DEFAULT_FRONTIERS_REGION);
        }

        foreach (var dbZone in GameServer.Database.SelectAllObjects<Zones>())
        {
    ...
    public static string LoadTeleports()
    {
        ...
        var numTeleports = new int[3];
        foreach (var teleport in objs)
        {
        ...
        return string.Format("Loaded {0} Albion, {1} Midgard and {2} Hibernia teleport locations", numTeleports[0],
            numTeleports[1], numTeleports[2]);
    }
    ...
    public static bool Init(RegionData[] regionsData)
    {
        try
        {
            ...
            long items = 0;
            long bindpoints = 0;
            regionsData.AsParallel().WithDegreeOfParallelism(GameServer.Instance.Configuration.CPUUse << 2).ForAll(
                data =>
                {
                    Region reg;
                ...
                Math.Max(0,
                    Math.Min(1000,
                        ServerProperties.Properties
                            .WORLD_DAY_INCREMENT)); // increments > 1000 do not render smoothly on clients
            m_dayStartTick =
                (int) GameTimer.GetTickCount() -
                (int) (DAY / Math.Max(1, m_dayIncrement) / 2); // set start time to 12pm
            m_dayResetTimer = new Timer(new TimerCallback(DayReset), null, DAY / Math.Max(1, m_dayIncrement) / 2,
                DAY / Math.Max(1, m_dayIncrement));
    ...
    {
        var timers = m_regionTimeManagers;
    ...
    private static void RelocateRegions()
    {
        ...
        while (m_relocationThread != null && m_relocationThread.IsAlive)
            try
            {
                ...
                foreach (var region in regionsClone)
    ...
    private static void DayReset(object sender)
    {
        ...
        foreach (var client in GetAllPlayingClients())
            if (client.Player != null && client.Player.CurrentRegion != null &&
                client.Player.CurrentRegion.UseTimeManager)
                client.Out.SendTime();
    }
    ...
    public static void StartDay(uint dayInc, double percent)
    {
        ...
        {
            m_dayStartTick =
                (int) GameTimer.GetTickCount() - (int) (dayStart / m_dayIncrement); // set start time to ...
    ...
    {
        get => m_lastWorldObjectUpdateTick;
    ...
    {
        var region = Region.Create(time, data);
    ...
    public static RegionEntry[] GetRegionList()
    {
        ...
        var port = string.Format("{0:D00000}", GameServer.Instance.Configuration.RegionPort);
    ...
    /// </summary>
    public static void RegisterZone(ZoneData zoneData, ushort zoneID, ushort regionID, string zoneName, int xpBonus,
        int rpBonus, int bpBonus, int coinBonus, byte realm)
    {
        ...
        if (zoneData.WaterLevel == 0) zoneData.WaterLevel = region.WaterLevel;

        var isDivingEnabled = region.IsRegionDivingEnabled;
    ...
    {
        var methodOfType = typeof(Enumerable).GetMethod("OfType");
        var genericOfType = methodOfType.MakeGenericMethod(new Type[] {type});
        // Use .NET 4 covariance
        var result = (IEnumerable<object>) genericOfType.Invoke(null, new object[] {input});

        var methodToArray = typeof(Enumerable).GetMethod("ToArray");
        var genericToArray = methodToArray.MakeGenericMethod(new Type[] {type});
    ...
    /// <param name="objectType">The type of the object you search</param>
    /// <returns>All objects with the specified parameters</returns>
    public static GameObject[] GetObjectsByNameFromRegion(string name, ushort regionID, eRealm realm,
        Type objectType)
    {
        Region reg;
        if (!m_regions.TryGetValue(regionID, out reg))
        ...
        return (GameObject[]) reg.Objects.Where(obj => obj != null && obj.Realm == realm && obj.Name == name)
            .OfTypeAndToArray(objectType);
    }
    ...
    {
        return (GameObject[]) m_regions.Values
            .Select(reg => GetObjectsByNameFromRegion(name, reg.ID, realm, objectType))
    ...
    {
        return m_regions.Values.Select(r =>
                r.Objects.OfType<GameNPC>().Where(npc => npc.Realm == realm && npc.GuildName == guild))
    ...
    {
        return m_regions.Values.Select(r =>
                r.Objects.OfType<GameNPC>().Where(npc => npc.Realm == realm && type.IsInstanceOfType(npc)))
            .SelectMany(objs => objs).ToList();
    }
    ...
    public static List<GameNPC> GetNPCsByType(Type type, eRealm realm, ushort region)
    {
        ...
        return reg.Objects.OfType<GameNPC>().Where(npc => npc.Realm == realm && type.IsInstanceOfType(npc))
            .ToList();
    }
    ...
    public static void RemoveClient(GameClient entry)
    {
        ...
        if (sessionid > 0) RemoveSessionID(sessionid);
    }
    ...
    public static GameClient GetClientByPlayerName(string playerName, bool exactMatch, bool activeRequired)
    {
        if (exactMatch)
        {
            ...
            return client.Player.Name.ToLower() == playerName.ToLower()
                ? client
                : null; //only return if it's an exact match
        }
        else
        {
            var x = 0;
    ...
    /// <param name="activeRequired"></param>
    /// <returns>The found GameClient or null</returns>
    public static List<GameClient> GetClientByPlayerNameAndRealm(string playerName, eRealm realm,
        bool activeRequired)
    {
        var potentialMatches = new List<GameClient>();
        lock (m_clients.SyncRoot)
        {
            foreach (var client in m_clients)
                if (client != null && client.Player != null &&
                    (realm == eRealm.None || client.Player.Realm == realm))
                {
                    if (activeRequired && (!client.IsPlaying ||
                                           client.Player.ObjectState != GameObject.eObjectState.Active))
                        continue;

                    if (0 == string.Compare(client.Player.Name, playerName,
                            StringComparison.OrdinalIgnoreCase)) // case insensitive comapre
                    {
    ...
    /// <param name="activeRequired"></param>
    /// <returns>The found GameClient or null</returns>
    public static GameClient GuessClientByPlayerNameAndRealm(string playerName, eRealm realm, bool activeRequired,
        out int result)
    {
        // first try exact match in case player with "abcde" name is
        // before "abc" in list and user typed "abc"
        var guessedClient =
            GetClientByPlayerNameAndRealm(playerName, realm, activeRequired).FirstOrDefault();
        ...
        // now trying to guess
        var compareName = playerName.ToLower();
    ...
    public static GameClient GetClientByPlayerNameFromRegion(string playerName, ushort regionID, bool exactMatch,
        bool activeRequired)
    {
        var client = GetClientByPlayerName(playerName, exactMatch, activeRequired);
    ...
    {
        var obj = GetObjectByIDFromRegion(regionID, oID);
    ...
    public static IDictionary<ushort, RegionData> RegionData => m_regionData;
    ...
    public static Dictionary<ushort, List<ZoneData>> ZonesData => m_zonesData;
    ...
    public static BaseInstance CreateInstance(ushort requestedID, ushort skinID, Type instanceType)
    {
        ...
        {
            log.Error(
                "Invalid type given for instance creation: " + instanceType + ". Returning null instance now.");
            return null;
        }
        ...
        var time = m_regionTimeManagers[0];
        ...
        var info = instanceType.GetConstructor(new Type[]
            {typeof(ushort), typeof(GameTimer.TimeManager), typeof(RegionData)});
        ...
        var RequestedAnID = requestedID == 0 ? false : true;
        ...
        if (!m_regions.FreezeWhile<bool>(regions =>
            {
                ...
                try
                {
                    instance = (BaseInstance) info.Invoke(new object[]
                        {ID, time, data}); //new Instance(ID, time, data);
        ...
        if (m_zonesData.ContainsKey(data.Id)) list = m_zonesData[data.Id];
        ...
        foreach (var dat in list)
            ...
            if (m_zones.FreezeWhile<bool>(zones =>
                ...
                RegisterZone(dat, (ushort) zoneID, ID, string.Format("{0} (Instance)", dat.Description), 0, 0, 0, 0,
                    0);
    ...
    public static void RemoveInstance(BaseInstance instance)
    {
        ...
        {
            foreach (var zn in instance.Zones)
