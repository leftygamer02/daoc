    ...
    public class Tabor : GameNPC
    {
        ...
        public override void Die(GameObject killer)
        {
            BroadcastMessage(string.Format(
                "As {0} falls to the ground, you feel a breeze in the air.\nA swirl of dirt covers the area.", Name));
            SpawnSwirlDirt();
            base.Die(killer);
        }
...
namespace DOL.GS
{
    public class TaborGhost : GameNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            RespawnInterval = -1;
            var sbrain = new TaborGhostBrain();
            SetOwnBrain(sbrain);
...
namespace DOL.GS
{
    public class SwirlDirt : GameNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new SwirlDirtBrain();
            SetOwnBrain(sbrain);
            LoadedFromScript = true;
            RespawnInterval = -1;
            var success = base.AddToWorld();
            if (success) new ECSGameTimer(this, new ECSGameTimer.ECSTimerCallback(Show_Effect), 1000);
