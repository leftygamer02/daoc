...
namespace DOL.GS
{
    public class Blackthorn : GameNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new BlackthornBrain();
            SetOwnBrain(sbrain);
...
namespace DOL.AI.Brain
{
    public class BlackthornBrain : StandardMobBrain
    {
        ...
        private protected List<GameNPC> npcs_to_call = new();
        private protected List<GameNPC> BafNpcs = new();
        ...
        private protected void PickRandomMob()
        {
            foreach (GameNPC npc in Body.GetNPCsInRadius(1000))
                if (npc != null && npc.IsAlive && npc.Name.ToLower() == "lunantishee" && !npcs_to_call.Contains(npc) &&
                    !npc.IsControlledNPC(npc))
            ...
            if (npcs_to_call.Count > 0)
            {
                if (!PickedNpc)
                {
                    var mob = npcs_to_call[Util.Random(0, npcs_to_call.Count - 1)]; //picking randomly mob from list
                    RandomNpc = mob;
                    PickedNpc = true;
                }
            ...
            if (Body.IsAlive && !HasAggro && !CanAddNpcs)
            {
                foreach (GameNPC npc in Body.GetNPCsInRadius(1000))
                    if (npc != null && npc.IsAlive) //found baf mobs
                        if (npc.PackageID == Body.PackageID && npc != Body && npc.Name.ToLower() == "lunantishee" &&
                            !BafNpcs.Contains(npc) && !npc.IsControlledNPC(npc))
                            BafNpcs.Add(npc);
