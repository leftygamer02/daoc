...
namespace DOL.GS
{
    public class QuillanMuire : GameNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new QuillanMuireBrain();
            SetOwnBrain(sbrain);
...
namespace DOL.GS
{
    public class MuireHerbalist : GameNPC
    {
        ...
        public override short Constitution
        {
            ...
            set => base.Constitution = 100;
        }

        public override short Dexterity
        {
            get => base.Dexterity;
            set => base.Dexterity = 180;
        }
        ...
        public override bool AddToWorld()
        {
            ...
            Faction = FactionMgr.GetFactionByID(782);
            Faction.AddFriendFaction(FactionMgr.GetFactionByID(782));
            var sbrain = new MuireHerbalistBrain();
            SetOwnBrain(sbrain);
            LoadedFromScript = false;
            SaveIntoDatabase();
            base.AddToWorld();
        ...
        private protected void HealAndBuff()
        {
                        ...
                        if (spell != null)
                        {
                            if (npc.HealthPercent < 50)
                            {
                                ...
                                if (!Body.IsCasting)
                                {
                                    ...
                                    Body.CastSpell(MuireHerbalistHeal,
            ...
            if (!Ishealing)
                foreach (GameNPC npc in Body.GetNPCsInRadius(500))
                    if (npc != null && npc.IsAlive && (npc.Name == "Muire Hero" || npc.Name == "Muire Champion" ||
                                                       npc.Name == "Quillan Muire"))
                    {
                        if (!Body.IsCasting &&
                            !npc.effectListComponent.ContainsEffectForEffectType(eEffect.StrengthBuff))
                        {
                            IsBuffing = true;
                            Body.TargetObject = npc;
                            Body.CastSpell(MuireHerbalist_Buff_STR,
                                SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells), false);
                        }
                        else
                        {
                            ...
                            IsBuffing = false;
                            if (!Body.IsCasting &&
                                !Body.effectListComponent.ContainsEffectForEffectType(eEffect.StrengthBuff))
                            {
                                IsBuffingSelf = true;
