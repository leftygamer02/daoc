...
namespace DOL.GS
{
    public class RockyGolem : GameNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new RockyGolemBrain();
            SetOwnBrain(sbrain);
            LoadedFromScript = false; //load from database
            SaveIntoDatabase();
            var success = base.AddToWorld();
            if (success) new ECSGameTimer(this, new ECSGameTimer.ECSTimerCallback(PrepareTeleport), 1000);
        ...
        private protected int PrepareTeleport(ECSGameTimer timer)
        {
            BroadcastMessage(string.Format(
                "The {0} says, \"Ahh, I feel the power of the stone... through the living rock I send you!\"", Name));
        ...
        private protected int TeleportPlayers(ECSGameTimer timer)
        {
            //region 224
            //1st teleports
            var port1a = new Point3D(34172, 30368, 14748); //heading 16
            var port1b = new Point3D(34173, 30381, 15310); //heading 1999
            var port1c = new Point3D(34171, 30381, 16366); //heading 2017
            //2nd teleports
            var port2a = new Point3D(30876, 32132, 14492); //heading 3058
            var port2b = new Point3D(30878, 32133, 16410); //heading 3042
            var port2c = new Point3D(30900, 32131, 15654); //heading 997
            //3th teleports
            var port3a = new Point3D(30112, 32645, 13980); //heading 3018
            var port3b = new Point3D(30131, 32643, 14940); //heading 1029
            var port3c = new Point3D(30131, 32643, 13021); //heading 865
