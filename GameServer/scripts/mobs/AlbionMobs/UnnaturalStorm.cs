...
namespace DOL.GS
{
    public class UnnaturalStorm : GameNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new UnnaturalStormBrain();
            SetOwnBrain(sbrain);
            ...
            foreach (var client in WorldMgr.GetClientsOfZone(CurrentZone.ID))
            {
                ...
                if (client.IsPlaying)
                    client.Out.SendMessage(
                        "An intense supernatural storm explodes in the sky over the northeastern expanse of Lyonesse!",
                        eChatType.CT_Broadcast, eChatLoc.CL_ChatWindow);
            }

            for (var i = 0; i < Util.Random(4, 5); i++)
            {
                var Add = new UnnaturalStormAdds();
...
namespace DOL.GS
{
    public class UnnaturalStormAdds : GameNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            MeleeDamageType = eDamageType.Crush;
            Race = 2003;
            Flags = (eFlags) 60; //notarget noname flying
            MaxSpeedBase = 0;
            RespawnInterval = -1;

            var sbrain = new UnnaturalStormAddsBrain();
            SetOwnBrain(sbrain);
            LoadedFromScript = true;
            var success = base.AddToWorld();
            if (success) new ECSGameTimer(this, new ECSGameTimer.ECSTimerCallback(Show_Effect), 500);
...
namespace DOL.GS
{
    public class UnnaturalStormController : GameNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new UnnaturalStormControllerBrain();
            SetOwnBrain(sbrain);
            base.AddToWorld();
            return true;
        }
