...
namespace DOL.GS
{
    public class AmminusPilus : GameNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new AmminusPilusBrain();
            SetOwnBrain(sbrain);
        ...
        protected int Show_Effect(ECSGameTimer timer)
        {
            if (IsAlive)
            {
                Parallel.ForEach(GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE).OfType<GamePlayer>(),
                    player => { player?.Out.SendSpellEffectAnimation(this, this, 5920, 0, false, 0x01); });
