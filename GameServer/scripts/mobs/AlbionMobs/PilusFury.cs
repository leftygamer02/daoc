...
namespace DOL.GS
{
    public class PilusFury : GameNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            Size = 50;
            Flags = (eFlags) 44; //notarget noname flying
            MaxSpeedBase = 0;
            RespawnInterval = -1;

            var sbrain = new PilusFuryBrain();
            SetOwnBrain(sbrain);
...
namespace DOL.AI.Brain
{
    public class PilusFuryBrain : APlayerVicinityBrain
    {
        ...
        private List<GameLiving> DD_Enemys = new();
        ...
        public override void Think()
        {
            if (Body.IsAlive)
            {
                foreach (GamePlayer player in Body.GetPlayersInRadius(10000)) HandlePlayerCheck(player);

                foreach (GameNPC npc in Body.GetNPCsInRadius(10000)) HandleNpcCheck(npc);
        ...
        private void HandlePlayerCheck(GamePlayer player)
        {
            ...
            {
                var nearbyPoint = _points.FirstOrDefault(point =>
                    ((point == _points[0] || point == _points[4]) && player.IsWithinRadius(point, 400))
                ...
                if (nearbyPoint != null)
                {
                    if (player.CharacterClass.ID == (int) eCharacterClass.Necromancer && player.ControlledBrain != null)
                    {
                        ...
                        {
                            var pet = (NecromancerPet) player.ControlledBrain.Body;
                            HandleNpcCheck(pet);
                        }
                    }
                    else
                ...
                else
                {
                    ...
                    if (DD_Enemys.Contains(player)) DD_Enemys.Remove(player);
                }
            }
        ...
        private void HandleNpcCheck(GameNPC npc)
        {
            ...
            {
                var playerOwner = pet.Owner as GamePlayer;
                var nearbyPoint = _points.FirstOrDefault(point =>
                    ((point == _points[0] || point == _points[4]) && pet.IsWithinRadius(point, 400))
                    || pet.IsWithinRadius(point, 200));

                if (nearbyPoint != null)
                {
                    if (!DD_Enemys.Contains(pet)) DD_Enemys.Add(pet);
                }
                else
                {
                    if (DD_Enemys.Contains(pet)) DD_Enemys.Remove(pet);
                }
        ...
        private int PrepareDD(ECSGameTimer timer)
        {
            if (DD_Enemys.Count > 0)
                foreach (var targets in DD_Enemys)
        ...
        private protected void DamageTarget(GameLiving target, GameNPC caster)
        {
            ...
            if (target is NecromancerPet pet)
                ...
                {
                    var PetOwner = pet.Owner as GamePlayer;
