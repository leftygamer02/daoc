...
namespace DOL.GS
{
    public class PygmyGoblinTangler : GameNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new PygmyGoblinTanglerBrain();
            if (NPCTemplate != null)
            {
            ...
            if (HasAggro && Body.TargetObject != null)
                if (!BringGoblins)
                {
                    foreach (GameNPC npc in Body.GetNPCsInRadius(500))
                    {
                        ...
                        if (npc != null && npc.IsAlive && npc.Name.ToLower() == "pygmy goblin" &&
                            npc.Brain is not ControlledNpcBrain && npc.Brain is StandardMobBrain brain)
