...
namespace DOL.GS
{
    public class HrimthursaIcetouch : GameEpicNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new HrimthursaIcetouchBrain();
            SetOwnBrain(sbrain);
    ...
    public class HrimthursaIcetouchBrain : StandardMobBrain
    {
        ...
        public override void Think()
        {
            if (Body.IsAlive)
            {
                ...
                var NpcToHeal = new List<GameNPC>();
                ...
                if (NpcToHeal.Count > 0)
                    ...
                    {
                        var mob =
                            NpcToHeal[Util.Random(0, NpcToHeal.Count - 1)]; //pick randomly mob that need to be healed
                        HealNpc = mob;
                        LockNpc = true;
                    }
                ...
                if (Body.InCombatInLast(20 * 1000) == false &&
                    Body.InCombatInLast(25 * 1000)) //reset checks if not in aggro after x sec
                {
                    CanHeal = false;
                    HealNpc = null;
                    LockNpc = false;
