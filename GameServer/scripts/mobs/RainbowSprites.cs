...
namespace DOL.GS
{
    public class RainbowSpriteTan : GameNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new RainbowSpriteTanBrain();
            if (NPCTemplate != null)
            {
                ...
                if (!CallforHelp)
                    if (Body.HealthPercent <= 20)
                    {
                        foreach (GameNPC npc in Body.GetNPCsInRadius(1000))
                        {
                            ...
                            if (npc != null && npc.IsAlive && npc.Name.ToLower() == "rainbow sprite" &&
                                npc.Brain is not ControlledNpcBrain && npc.Brain is RainbowSpriteTanBrain brain &&
                                npc != Body)
...
namespace DOL.GS
{
    public class RainbowSpriteWhite : GameNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new RainbowSpriteWhiteBrain();
            if (NPCTemplate != null)
            {
                sbrain.AggroLevel = NPCTemplate.AggroLevel;
                sbrain.AggroRange = NPCTemplate.AggroRange;
                ...
                if (!CallforHelp)
                    if (Body.HealthPercent <= 20)
                    {
                        foreach (GameNPC npc in Body.GetNPCsInRadius(1000))
                        {
                            var target = Body.TargetObject as GameLiving;
                            if (npc != null && npc.IsAlive && npc.Name.ToLower() == "rainbow sprite" &&
                                npc.Brain is not ControlledNpcBrain && npc.Brain is RainbowSpriteWhiteBrain brain &&
                                npc != Body)
                                if (target != null && target.IsAlive && brain != null && !brain.HasAggro)
                                    brain.AddToAggroList(target, 10);
                        }
...
namespace DOL.GS
{
    public class RainbowSpriteBlue : GameNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new RainbowSpriteBlueBrain();
            if (NPCTemplate != null)
            {
                sbrain.AggroLevel = NPCTemplate.AggroLevel;
                sbrain.AggroRange = NPCTemplate.AggroRange;
                ...
                if (!CallforHelp)
                    if (Body.HealthPercent <= 20)
                    {
                        foreach (GameNPC npc in Body.GetNPCsInRadius(1000))
                        {
                            var target = Body.TargetObject as GameLiving;
                            if (npc != null && npc.IsAlive && npc.Name.ToLower() == "rainbow sprite" &&
                                npc.Brain is not ControlledNpcBrain && npc.Brain is RainbowSpriteBlueBrain brain &&
                                npc != Body)
                                if (target != null && target.IsAlive && brain != null && !brain.HasAggro)
                                    brain.AddToAggroList(target, 10);
                        }
...
namespace DOL.GS
{
    public class RainbowSpriteGreen : GameNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new RainbowSpriteGreenBrain();
            if (NPCTemplate != null)
            {
                sbrain.AggroLevel = NPCTemplate.AggroLevel;
                sbrain.AggroRange = NPCTemplate.AggroRange;
                ...
                if (!CallforHelp)
                    if (Body.HealthPercent <= 20)
                    {
                        foreach (GameNPC npc in Body.GetNPCsInRadius(1000))
                        {
                            var target = Body.TargetObject as GameLiving;
                            if (npc != null && npc.IsAlive && npc.Name.ToLower() == "rainbow sprite" &&
                                npc.Brain is not ControlledNpcBrain && npc.Brain is RainbowSpriteGreenBrain brain &&
                                npc != Body)
                                if (target != null && target.IsAlive && brain != null && !brain.HasAggro)
                                    brain.AddToAggroList(target, 10);
                        }
