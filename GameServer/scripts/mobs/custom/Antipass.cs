    ...
    public class AntipassBrain : StandardMobBrain
    {
        ...
        public override void Think()
        {
            foreach (GamePlayer player in Body.GetPlayersInRadius((ushort) AggroRange))
                ...
                {
                    var angle = 0.00153248422;
                    player.MoveTo(player.CurrentRegionID,
                        (int) (Body.X - (AggroRange + 10) * Math.Sin(angle * Body.Heading)),
                        (int) (Body.Y + (AggroRange + 10) * Math.Cos(angle * Body.Heading)), Body.Z, player.Heading);
                }
        }
    }
}