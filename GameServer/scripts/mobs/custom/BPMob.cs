...
public class BPMob : GameNPC
{
...

    {
        ...
        var multiplier = Util.Random(2, 3);
        var bonus = Util.Random(1, 3);
        var chance = Util.Random(1, 25);
        ...
        var playersonline = 0;
        foreach (var playerclient in WorldMgr.GetAllPlayingClients())
...

        {
            if (player.Group != null)
            {
                ...
                foreach (GamePlayer player2 in player.Group.GetMembersInTheGroup())
                {
                    ...
                    {
                        player2.Out.SendMessage("You are RR7 or higher, you will not be rewarded here anymore!",
                    ...
                    if (playersonline >= 50)
                        ...
                        {
                            player2.Out.SendMessage(
                                "There are " + playersonline +
                                " players online and your in the farmzone, why don't you go play with them!",
                                eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                            player2.MoveTo(79, 32401, 12245, 17413, 1902);
                        }
                    ...
                    if (isjackpot)
                    {
                        player2.Out.SendMessage("JACKPOT!!!", eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
                        player2.Out.SendPlaySound(eSoundType.Craft, 0x04);
                        player2.Out.SendMessage("You just got " + multiplier + "x multiplier bonus points!  Woot!",
                            eChatType.CT_ScreenCenterSmaller, eChatLoc.CL_SystemWindow);
                    }

                    player2.Out.SendMessage("You Get " + rewardbp + " bounty points!", eChatType.CT_Important,
                        eChatLoc.CL_SystemWindow);
                    player2.Out.SendMessage("You Get " + rewardbp + " bounty points!",
                        eChatType.CT_ScreenCenterSmaller, eChatLoc.CL_SystemWindow);
                }
            }
            ...
            else
            {
                ...
                {
                    player.Out.SendMessage("You are RR7 or higher, you will not be rewarded here anymore!",
                        eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                ...
                else
                {
                    ...
                    if (isjackpot)
                    {
                        ...
                        player.Out.SendMessage("You just got " + multiplier + "x multiplier bonus points!  Woot!",
                            eChatType.CT_ScreenCenterSmaller, eChatLoc.CL_SystemWindow);
                    }

                    player.Out.SendMessage("You Get " + rewardbp + " bounty points!", eChatType.CT_Important,
                        eChatLoc.CL_SystemWindow);
                    player.Out.SendMessage("You Get " + rewardbp + " bounty points!",
                        eChatType.CT_ScreenCenterSmaller, eChatLoc.CL_SystemWindow);
                }
            }
