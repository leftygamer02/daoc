...
public class BPPortMob : GameNPC
{
    ...
    private GameLiving m_BPPortMobTarget;

    public GameLiving BPPortMobTarget
    {
        get => m_BPPortMobTarget;
        set => m_BPPortMobTarget = value;
    }
    ...
    public override void Die(GameObject killer)
    {
        ...
        foreach (GamePlayer player in GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE)) //3600 units
        {
            ...
            player.Out.SendMessage("An item has appeared in your inventory!", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            player.GainBountyPoints(Level * 5); // grants BP at Server BP rate * 5 * level of mob
