...
namespace DOL.GS
{
    public class ApocInitializator : GameNPC
    {
        ...
        {
            BroadcastMessage(
                string.Format("Fames says loudly, 'I sense presence of many, the presence of power and ambition...'"));
        ...
        public int Message_timer5(ECSGameTimer timer)
        {
            BroadcastMessage(string.Format(
                "Funus says with a gravely hiss, 'It is the fate of man to die, to expire like the flame of a candle.'"));
        ...
        {
            BroadcastMessage(string.Format("Fames asks, 'You, " + RandomTarget.Name +
                                           ", do you come to challenge fate? Come to me with your answer so that I may see the answer" +
        ...
        private List<GamePlayer> PlayersInRoom = new();
        ...
        public void PlayerEnter()
        {
            ...
            {
                var ptarget = (GamePlayer) PlayersInRoom[Util.Random(1, PlayersInRoom.Count) - 1];
        ...
        {
            if (IsAlive)
                if (!Fames.FamesIsUp && !Bellum.BellumUP && !Morbus.MorbusUP && !Funus.FunusUp && !Apocalypse.ApocUP &&
                    !start_respawn_check)
                {
                    RandomTarget = null; //reset picked player
                    PlayersInRoom.Clear();
                    var time = ServerProperties.Properties.SET_SI_EPIC_ENCOUNTER_RESPAWNINTERVAL *
                               60000; //1min is 60000miliseconds 
                    new ECSGameTimer(this, new ECSGameTimer.ECSTimerCallback(DoRespawnNow), time);
                    log.Debug("Starting respawn time for final caer sidi encounter, will respawn in " + time / 60000 +
                              " minutes!");
        ...
        public static void ScriptLoaded(DOLEvent e, object sender, EventArgs args)
        {
            ...
            {
                log.Warn(
                    "Apoc Initializator exist ingame, remove it and restart server if you want to add by script code.");
            }
        }
        ...
        public void DontAllowLeaveRoom()
        {
            ...
            if (CurrentRegionID == 60) //caer sidi
            {
                if (FamesBrain.StartedFames == true || BellumBrain.StartedBellum == true ||
                    MorbusBrain.StartedMorbus == true || FunusBrain.StartedFunus == true ||
                    ApocalypseBrain.StartedApoc == true)
                    foreach (GamePlayer player in GetPlayersInRadius(1500))
                        if (player != null)
                            if (player.IsAlive)
                                if (player.IsWithinRadius(point1, 150) &&
                                    player.Client.Account.PrivLevel == 1) //only players will be ported back
                                {
                                    player.MoveTo(60, 29469, 25244, 19490, 2014);
                                    player.Out.SendMessage("Magic energy moves you to the center of room!",
...
namespace DOL.GS
{
    public class Fames : GameEpicBoss
    {
        ...
        public override bool Interact(GamePlayer player)
        {
            ...
            var player2 = ApocInitializator.RandomTarget;
            if (CanInteract == false)
                if (player == player2)
                {
                    ...
                    player.Out.SendMessage("Fames says, Well? Do you challenge fate itself?\n" +
                                           ...
                                           "Say [yes] and prepare yourselves.", eChatType.CT_Say,
            ...
            if (CanInteract == false)
            {
                if (t == ApocInitializator.RandomTarget || ApocInitializator.OthersCanInteract == true)
                {
                    ...
                    switch (str.ToLower())
                    {
                        case "no":
                        {
                            t.Out.SendMessage("Then be gone and continue on with what you were meant to do.",
                        ...
                        case "yes":
                        {
                            foreach (GamePlayer player in GetPlayersInRadius(2500))
                                if (player != null)
                                {
                                    player.Out.SendMessage("Fames says, 'Done. You are brave " + t.CharacterClass.Name +
                                                           " ... or foolish. While it is most certain that your" +
                                                           " actions will have little chance to alter the course of fate, you and your companions are granted a few grains of time," +
                                                           " two minutes in your terms, to prepare.",
                                        eChatType.CT_Broadcast, eChatLoc.CL_ChatWindow);
                                    new ECSGameTimer(this, new ECSGameTimer.ECSTimerCallback(StartFamesTimer),
                                        120000); //2min
                                    CanInteract = true;
                                }
                        }
        ...
        public override void Die(GameObject killer) //on kill generate orbs
        {
            ...
            {
                BroadcastMessage(
                    string.Format("Bellum says, 'Prepare yourselves for war. One minute, you are granted.'"));
                new ECSGameTimer(this, new ECSGameTimer.ECSTimerCallback(SpawnHorsemanBellum),
                    60000); //60s before starting
                prepareBellum = true;
            }
    ...
    public class FamesBrain : StandardMobBrain
    {
        ...
        public override void Think()
        {
            ...
            if (Body.InCombat || HasAggro ||
                Body.attackComponent.AttackState == true) //bring mobs from rooms if mobs got set PackageID="FamesBaf"
            {
                StartedFames = true;
                if (spawn_fate == false)
                {
                ...
                if (BafMobs == false)
                    foreach (var npc in WorldMgr.GetNPCsFromRegion(Body.CurrentRegionID))
                        if (npc != null)
                            ...
                            {
                                AddAggroListTo(
                                    npc.Brain as StandardMobBrain); // add to aggro mobs with FamesBaf PackageID
                                BafMobs = true;
                            }
            }
...
namespace DOL.GS
{
    public class Bellum : GameEpicBoss
    {
        ...
        public override void Die(GameObject killer) //on kill generate orbs
        {
            ...
            {
                BroadcastMessage(string.Format(
                    "Morbus says, 'Sometimes it is the smallest things that are the most deadly. Be prepared in one minute..'"));
                new ECSGameTimer(this, new ECSGameTimer.ECSTimerCallback(SpawnHorsemanMorbus),
                    60000); //60s before starting
                prepareMorbus = true;
            }
            ...
            foreach (GameNPC npc in GetNPCsInRadius(4000))
                if (npc != null)
                    if (npc.IsAlive)
                        if (npc.Brain is WarIncarnateCrushBrain || npc.Brain is WarIncarnateSlashBrain ||
                            npc.Brain is WarIncarnateThrustBrain)
                            npc.Die(this);
...
namespace DOL.AI.Brain
{
    public class BellumBrain : StandardMobBrain
    {
        ...
        public void SpawnCrushWeapon()
        {
            ...
            Add.RespawnInterval = -1;
            Add.PackageID = "BellumBaf";
            var adds = new WarIncarnateCrushBrain();
        ...
        public void SpawnSlashWeapon()
        {
            ...
            Add.RespawnInterval = -1;
            Add.PackageID = "BellumBaf";
            var adds = new WarIncarnateSlashBrain();
            Add.SetOwnBrain(adds);
            Add.AddToWorld();
        }
        ...
        {
            var Add = new WarIncarnateThrust();
            Add.X = Body.X + 200;
            Add.Y = Body.Y;
            Add.Z = Body.Z;
            Add.CurrentRegionID = Body.CurrentRegionID;
            ...
            Add.RespawnInterval = -1;
            Add.PackageID = "BellumBaf";
            var adds = new WarIncarnateThrustBrain();
            Add.SetOwnBrain(adds);
            Add.AddToWorld();
        }
    }
    ...
    public class WarIncarnateCrushBrain : StandardMobBrain
    {
        ...
        public void SpawnCrushWeapons()
        {
            for (var i = 0; i < 4; i++)
            {
                ...
                Add.RespawnInterval = -1;
                Add.PackageID = "BellumBaf";
                var smb = new WarIncarnateCrushBrain();
                smb.AggroLevel = 100;
                smb.AggroRange = 1000;
                Add.AddBrain(smb);
                Add.AddToWorld();
                var brain = (WarIncarnateCrushBrain) Add.Brain;
    ...
    public class WarIncarnateSlashBrain : StandardMobBrain
    {
        ...
        public void SpawnSlashWeapons()
        {
            for (var i = 0; i < 4; i++)
            {
                ...
                Add.RespawnInterval = -1;
                Add.PackageID = "BellumBaf";
                var smb = new WarIncarnateSlashBrain();
                smb.AggroLevel = 100;
                smb.AggroRange = 1000;
                Add.AddBrain(smb);
                Add.AddToWorld();
                var brain = (WarIncarnateSlashBrain) Add.Brain;
                brain.AddToAggroList(ptarget, 1);
                Add.StartAttack(ptarget);
            }
        }
    ...
    public class WarIncarnateThrustBrain : StandardMobBrain
    {
        ...
        public void SpawnThrustWeapons()
        {
            for (var i = 0; i < 4; i++)
            {
                ...
                Add.RespawnInterval = -1;
                Add.PackageID = "BellumBaf";
                var smb = new WarIncarnateThrustBrain();
                smb.AggroLevel = 100;
                smb.AggroRange = 1000;
                Add.AddBrain(smb);
                Add.AddToWorld();
                var brain = (WarIncarnateThrustBrain) Add.Brain;
                brain.AddToAggroList(ptarget, 1);
                Add.StartAttack(ptarget);
            }
        }
...
namespace DOL.GS
{
    public class Morbus : GameEpicBoss
    {
        ...
        public override void Die(GameObject killer) //on kill generate orbs
        {
            ...
            {
                BroadcastMessage(string.Format(
                    "Funus says, 'Prepare to die. Sixty seconds you are given to arrange for the event.'\n" +
                ...
                new ECSGameTimer(this, new ECSGameTimer.ECSTimerCallback(SpawnHorsemanFunus),
                    60000); //60s before starting
                prepareFunus = true;
            }
        ...
        {
            var Add = new Funus(); //inside controller
            Add.X = 29467;
            Add.Y = 25235;
            Add.Z = 19490;
            Add.CurrentRegion = CurrentRegion;
...
namespace DOL.AI.Brain
{
    public class MorbusBrain : StandardMobBrain
    {
        ...
        public override void Think()
        {
            ...
            else if (Body.InCombatInLast(40 * 1000) == false && Body.InCombatInLast(45 * 1000))
            {
                Body.Health = Body.MaxHealth;
                Body.Model = 952;
                Body.Size = 140;
            ...
            if (Body.InCombat || HasAggro || Body.attackComponent.AttackState == true)
            {
                ...
                if (BafMobs3 == false)
                    foreach (var npc in WorldMgr.GetNPCsFromRegion(Body.CurrentRegionID))
                        if (npc != null)
                            ...
                            {
                                AddAggroListTo(
                                    npc.Brain as MorbusSwarmBrain); // add to aggro mobs with MorbusBaf PackageID
                                BafMobs3 = true;
                            }
                ...
                if (Morbus.Morbus_Swarm_count == 0)
                    ...
                    {
                        new ECSGameTimer(Body, new ECSGameTimer.ECSTimerCallback(SpawnSwarm),
                            Util.Random(25000, 40000)); //25s-40s
        ...
        {
            if (Body.IsAlive)
                for (var i = 0; i < Util.Random(6, 10); i++)
                {
                    var Add = new MorbusSwarm();
...
namespace DOL.GS
{
    public class Funus : GameEpicBoss
    {
        ...
        public override void TakeDamage(GameObject source, eDamageType damageType, int damageAmount, int criticalAmount)
        {
            if (source is GamePlayer)
            {
                var truc = source as GamePlayer;
                //cleri,merc,arms,infi,scout
                if (truc.CharacterClass.ID == 6 ||
                    (truc.CharacterClass.ID == 11 && truc.ActiveWeapon.Object_Type == 5) ||
                    (truc.CharacterClass.ID == 2 && truc.ActiveWeapon.Object_Type == 10)
                    || (truc.CharacterClass.ID == 9 && truc.ActiveWeapon.Object_Type == 10) ||
                    (truc.CharacterClass.ID == 3 && truc.ActiveWeapon.Object_Type == 9))
                {
                    base.TakeDamage(source, damageType, damageAmount, criticalAmount);
                }
                else
                {
                    truc.Out.SendMessage(Name + " absorbs all your damage to heal iself!", eChatType.CT_System,
                        eChatLoc.CL_ChatWindow);
                    BroadcastMessage(string.Format("Funus takes damage from " + source.Name +
                                                   " and restoring it's whole health."));
                    Health += MaxHealth;
                    base.TakeDamage(source, damageType, 0, 0);
                    return;
                }
            ...
            {
                var truc = source as GamePet;
                var pet_owner = truc.Owner as GamePlayer;
                pet_owner.Out.SendMessage(Name + " absorbs all your damage to heal iself!", eChatType.CT_System,
                    eChatLoc.CL_ChatWindow);
                BroadcastMessage(string.Format("Funus takes damage from " + pet_owner.Name +
                                               " and restoring it's whole health."));
                Health += MaxHealth;
                base.TakeDamage(source, damageType, 0, 0);
                return;
        ...
        public override void Die(GameObject killer) //on kill generate orbs
        {
            ...
            {
                BroadcastMessage(string.Format(
                    "A thunderous voice echoes off the walls, 'Well done. You have succeeded in besting my harbingers.'"));
    ...
    public class Apocalypse : GameEpicBoss
    {
        ...
        public override bool AddToWorld()
        {
            ...
            foreach (var client in WorldMgr.GetClientsOfRegion(CurrentRegionID))
            {
                ...
                if (client.IsPlaying)
                    client.Out.SendSoundEffect(2452, 0, 0, 0, 0,
                        0); //play sound effect for every player in boss currentregion
            }

            KilledEnemys = 0;
            var adds = new ApocalypseBrain();
            SetOwnBrain(adds);
            base.AddToWorld();
            return true;
        }
        ...
        {
            if (enemy is GamePlayer) ++KilledEnemys;
    ...
    public class ApocalypseBrain : StandardMobBrain
    {
        ...
        public override void Think()
        {
            ...
            if (Body.IsAlive) //bring mobs from rooms if mobs got set PackageID="ApocBaf"
            {
                ...
                {
                    var point1 = new Point3D();
                    point1.X = Body.SpawnPoint.X;
                    point1.Y = Body.SpawnPoint.Y + 100;
                    point1.Z = Body.SpawnPoint.Z + 750;
                    ClearAggroList();
                            ...
                            foreach (GamePlayer player in Body.GetPlayersInRadius(2500))
                                if (player != null)
                                    player.Out.SendMessage(
                                        "Apocalypse says, 'Is it power? Fame? Fortune? Perhaps it is all three.'",
                                        eChatType.CT_Broadcast, eChatLoc.CL_ChatWindow);
                        ...
                        foreach (var npc in WorldMgr.GetNPCsFromRegion(Body.CurrentRegionID))
                            if (npc != null)
                                if (npc.IsAlive && npc.PackageID == "ApocBaf" && npc.Brain is HarbringerOfFateBrain)
                                    AddAggroListTo(
                                        npc.Brain as HarbringerOfFateBrain); // add to aggro mobs with ApocBaf PackageID
                    ...
                    else
                    {
                        if (fly_phase1 == false)
                        {
                            ...
                            foreach (GamePlayer player in Body.GetPlayersInRadius(2500))
                                if (player != null)
                                    player.Out.SendMessage(
                                        "Apocalypse says, 'I wonder, also, about the motivation that drives one to such an audacious move.'",
                                        eChatType.CT_Broadcast, eChatLoc.CL_ChatWindow);
        ...
        public int FlyPhaseStart(ECSGameTimer timer)
        {
            if (Body.IsAlive)
            {
                ...
                new ECSGameTimer(Body, new ECSGameTimer.ECSTimerCallback(FlyPhaseDuration),
                    30000); //30s duration of phase
            }
        ...
        public int SpawnHarbringers(ECSGameTimer timer)
        {
            if (Apocalypse.KilledEnemys == 4) //he doint it only once, spawning 2 harbringers is killed 4 players
            {
                foreach (GamePlayer player in Body.GetPlayersInRadius(2500))
                    if (player != null)
                        player.Out.SendMessage(
                            "Apocalypse says, 'One has to wonder what kind of power lay behind that feat, for my harbingers of fate were no small adversaries.'",
                            eChatType.CT_Broadcast, eChatLoc.CL_ChatWindow);

                for (var i = 0; i < 2; i++)
                {
                    var Add = new HarbringerOfFate();
            ...
            if (Body.HealthPercent <= 50 &&
                pop_harbringers == false) /// spawning another 2 harbringers if boss is at 50%
            {
                foreach (GamePlayer player in Body.GetPlayersInRadius(2500))
                    if (player != null)
                        player.Out.SendMessage(
                            "Apocalypse says, 'In all of this, however, it would seem that you have overlooked " +
                            "the small matter of price for your actions. I am not the vengeful sort, so do not take this the wrong way," +
                            " but good harbingers are hard to come by. And, thanks to you, they will need to be replaced.'",
                            eChatType.CT_Broadcast, eChatLoc.CL_ChatWindow);
...
namespace DOL.GS
{
    public class HarbringerOfFate : GameEpicNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            AbilityBonus[(int) eProperty.Resist_Energy] = 26;
            AbilityBonus[(int) eProperty.Resist_Spirit] = 25;
            AbilityBonus[(int) eProperty.Resist_Slash] = 30;
            AbilityBonus[(int) eProperty.Resist_Crush] = 30;
            AbilityBonus[(int) eProperty.Resist_Thrust] = 30;
            ...
            var adds = new HarbringerOfFateBrain();
            SetOwnBrain(adds);
            base.AddToWorld();
            return true;
        }
...
namespace DOL.GS
{
    public class RainOfFire : GameEpicNPC
    {
        ...
        public override void EnemyKilled(GameLiving enemy)
        {
            if (enemy is GamePlayer) ++Apocalypse.KilledEnemys;

            base.EnemyKilled(enemy);
        }

        public override int MaxHealth => 10000 * Constitution / 100;
...
namespace DOL.AI.Brain
{
    public class RainOfFireBrain : StandardMobBrain
    {
        ...
        public override void Think()
        {
            if (Body.IsAlive) PickTarget();

            base.Think();
        }

        private List<GamePlayer> ApocPlayerList = new();
        ...
        public void PickTarget()
        {
            ...
            if (ApocPlayerList.Count > 0)
                if (cast_dd == false)
                {
                    var ptarget = (GamePlayer) ApocPlayerList[Util.Random(1, ApocPlayerList.Count) - 1];
                    DD_Target = ptarget;
        ...
        {
            var oldTarget = Body.TargetObject;
