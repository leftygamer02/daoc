...
namespace DOL.GS
{
    public class SyssroRuthless : GameEpicBoss
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new SyssroRuthlessBrain();
            SetOwnBrain(sbrain);
            ...
            if (HasAggro && Body.TargetObject != null)
            {
                if (IsTargetPicked == false)
                {
                    new ECSGameTimer(Body, new ECSGameTimer.ECSTimerCallback(ThrowPlayer),
                        Util.Random(10000, 15000)); //timer to port and pick player
                    IsTargetPicked = true;
                }
...
namespace DOL.GS
{
    public class PitMonster : GameNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new PitMonsterBrain();
            SetOwnBrain(sbrain);
    ...
    public class PitMonsterBrain : StandardMobBrain
    {
        ...
        public override void Think()
        {
            foreach (GamePlayer player in Body.GetPlayersInRadius((ushort) Body.AttackRange))
            {
                ...
                if (player == null || !player.IsAlive) ClearAggroList();
            }
