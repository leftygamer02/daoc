...
namespace DOL.GS
{
    public class DraargusMighty : GameEpicBoss
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new DraargusMightyBrain();
            SetOwnBrain(sbrain);
...
namespace DOL.GS
{
    public class DraugynSphere : GameEpicNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new DraugynSphereBrain();
            SetOwnBrain(sbrain);
        ...
        protected int Show_Effect(ECSGameTimer timer)
        {
            if (IsAlive)
            {
                Parallel.ForEach(GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE).OfType<GamePlayer>(),
                    player => { player?.Out.SendSpellEffectAnimation(this, this, 55, 0, false, 0x01); });
