...
namespace DOL.AI.Brain
{
    public class XanxicarBrain : StandardMobBrain
    {
        ...
        private System.Collections.Generic.List<GamePlayer> Port_Enemys = new();
        private System.Collections.Generic.List<GamePlayer> DD_Enemys = new();
        ...
        public int GlarePlayer(ECSGameTimer timer)
        {
            if (Body.IsAlive)
            {
                ...
                {
                    var Target = DD_Enemys[Util.Random(0, DD_Enemys.Count - 1)];
        ...
        public int BombAnnounce(ECSGameTimer timer)
        {
            BroadcastMessage(string.Format(
                "Xanxicar bellows in rage and prepares massive stomp at all of the creatures attacking him."));
        ...
        public override void Think()
        {
            ...
            if (Body.InCombat && Body.IsAlive && HasAggro)
            {
                ...
                {
                    new ECSGameTimer(Body, new ECSGameTimer.ECSTimerCallback(ThrowPlayer),
                        Util.Random(20000, 40000)); //timer to port and pick player
                    IsTargetPicked = true;
                }

                if (IsTargetPicked2 == false)
                {
                    new ECSGameTimer(Body, new ECSGameTimer.ECSTimerCallback(GlarePlayer),
                        Util.Random(35000, 45000)); //timer to glare at player
                ...
                if (Body.HealthPercent <= 50)
                {
                    ...
                    if (SpawnAddsOnce && CheckForSingleAdd == false &&
                        XanxicarianChampion.XanxicarianChampionCount == 0)
                    {
                        new ECSGameTimer(Body, new ECSGameTimer.ECSTimerCallback(SpawnMoreAdds),
                            Util.Random(15000, 25000)); //spawn 1 add every 25-35s
        ...
        public void SpawnAdds()
        {
            for (var i = 0; i < 5; i++)
            {
                var Add = new XanxicarianChampion();
...
namespace DOL.GS
{
    public class XanxicarianChampion : GameEpicNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new XanxicarianChampionBrain();
            SetOwnBrain(sbrain);
