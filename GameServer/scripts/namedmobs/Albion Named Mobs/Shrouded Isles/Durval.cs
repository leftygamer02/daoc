...
namespace DOL.AI.Brain
{
    public class DurvalBrain : StandardMobBrain
    {
        ...
        public override void Think()
        {
            ...
            if (HasAggro)
            {
                foreach (var npc in WorldMgr.GetNPCsFromRegion(3500))
                        ...
                        foreach (Spell spells in Body.Spells)
                            if (spells != null)
                            {
                                ...
                                if (Util.Chance(100))
                                {
                                    ...
                                    if (target != null)
                                    {
                                        if (!Body.IsCasting && Body.GetSkillDisabledDuration(Boss_Mezz) == 0 &&
                                            !target.effectListComponent.ContainsEffectForEffectType(eEffect.Mez) &&
                                            !target.effectListComponent
                                                .ContainsEffectForEffectType(eEffect.MezImmunity))
                                            Body.CastSpell(Boss_Mezz,
                                                SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells), false);
                                        else if (!Body.IsCasting && Body.GetSkillDisabledDuration(DurvalDisease) == 0 &&
                                                 !target.effectListComponent.ContainsEffectForEffectType(
                                                     eEffect.Disease))
                                            Body.CastSpell(DurvalDisease,
                                                SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells), false);
                                        else if (!Body.IsCasting && Body.GetSkillDisabledDuration(Bubble) == 0 &&
                                                 !target.effectListComponent.ContainsEffectForEffectType(
                                                     eEffect.Bladeturn))
                                            Body.CastSpell(Bubble, SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells),
                                                false);
                                        else
                                            Body.CastSpell(Durval_DD,
                                                SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells), false);
                                    }
                                }
                            }
