...
namespace DOL.GS
{
    public class RisnirCruss : GameEpicBoss
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new RisnirCrussBrain();
            SetOwnBrain(sbrain);
...
namespace DOL.AI.Brain
{
    public class RisnirCrussBrain : StandardMobBrain
    {
            ...
            if (!CheckProximityAggro())
            {
                ...
                if (!RemoveAdds)
                {
                    foreach (GameNPC npc in Body.GetNPCsInRadius(5000))
                        if (npc != null && npc.IsAlive && npc.RespawnInterval == -1 &&
                            npc.PackageID == "RisnirCrussAdd")
                            npc.Die(Body);
        ...
        protected int Show_Effect(ECSGameTimer timer)
        {
            ...
            {
                Parallel.ForEach(Body.GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE).OfType<GamePlayer>(),
                    player => { player?.Out.SendSpellEffectAnimation(Body, Body, 6085, 0, false, 0x01); });
