...
namespace DOL.GS
{
    public class Colialt : GameEpicBoss
    {
        ...
        {
            for (var i = 0; i < Util.Random(10, 15); i++)
            {
                var add = new ColialtAdds();
                ...
                if (Body.HealthPercent <= 30)
                {
                    ...
                    if (Body.TargetObject != null)
                        if (!Body.IsCasting && !Body.IsMoving && ColialtPhase)
                        {
                            ...
                            {
                                if (Body.IsMoving &&
                                    Body.TargetObject.IsWithinRadius(Body.TargetObject, ColialtLifeDrain.Range))
                                    Body.StopFollowing();
                                else
                                    Body.Follow(Body.TargetObject, ColialtLifeDrain.Range - 50, 5000);

                                Body.TurnTo(Body.TargetObject);
                                Body.CastSpell(ColialtLifeDrain, SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells),
                                    false);
                            }
                        }
                }
...
namespace DOL.GS
{
    public class ColialtAdds : GameNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new ColialtAddsBrain();
            SetOwnBrain(sbrain);
