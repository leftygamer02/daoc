        ...
        private int SpamMessage(ECSGameTimer timer)
        {
            if (HasAggro)
                BroadcastMessage(Body.Name +
                                 " says, \"The living can never conquer the eternal darkness of death incarnate!\"");
                ...
                if (!RemoveAdds)
                {
                    foreach (GameNPC npc in Body.GetNPCsInRadius(5000))
                        if (npc != null && npc.IsAlive && npc.Brain is VortanosAddBrain)
                        {
                            ...
                            {
                                BroadcastMessage(Body.Name +
                                                 " says, \"Sleep my unwilling prisoners, you are no longer needed here\"");
                                NotInCombat = true;
                            }
                        }
...
namespace DOL.GS
{
    public class VortanosAdd : GameNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new VortanosAddBrain();
            SetOwnBrain(sbrain);
            RoamingRange = 300;
