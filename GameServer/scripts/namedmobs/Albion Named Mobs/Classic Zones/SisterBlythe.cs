...
namespace DOL.GS
{
    public class SisterBlythe : GameEpicNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new SisterBlytheBrain();
            SetOwnBrain(sbrain);
            base.AddToWorld();
            return true;
        }
        ...
        {
            var spawn = new Point3D(322192, 671493, 2764);
            for (var i = 0; i < 4; i++)
            {
                var npc = new FallenExecutioner();
        ...
        public override void Think()
        {
            ...
            if (HasAggro && Body.TargetObject != null)
            {
                ...
                if (!Message1)
                {
                    ...
                    {
                        case 1:
                            BroadcastMessage("Sister Blythe shouts in a language you cannot understand!");
                            break;
                        case 2:
                            BroadcastMessage(string.Format(
                                "{0} says, \"Come my pets! Let us show these fools what comes of failure!\"",
                                Body.Name));
                            break;
                    }
                ...
                if (FallenExecutionerCount < 4)
                    if (!Message2)
                    {
                        new ECSGameTimer(Body, new ECSGameTimer.ECSTimerCallback(SisterSummonEffect),
...
namespace DOL.GS
{
    public class FallenExecutioner : GameNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new FallenExecutionerBrain();
            SetOwnBrain(sbrain);
