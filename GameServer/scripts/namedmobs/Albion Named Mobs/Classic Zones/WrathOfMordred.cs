...
namespace DOL.GS
{
    public class WrathOfMordred : GameEpicBoss
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new WrathOfMordredBrain();
            SetOwnBrain(sbrain);
        ...
        public override void OnAttackedByEnemy(AttackData ad)
        {
            if (ad != null && ad.AttackResult == eAttackResult.Parried)
            {
                styleComponent.NextCombatBackupStyle =
                    AfterParry; //boss parried so prepare after parry style backup style
    ...
    public class WrathOfMordredBrain : StandardMobBrain
    {
        ...
        private int WalkSide(ECSGameTimer timer)
        {
            if (Body.IsAlive && HasAggro && Body.TargetObject != null &&
                Body.IsWithinRadius(Body.TargetObject, Body.AttackRange))
