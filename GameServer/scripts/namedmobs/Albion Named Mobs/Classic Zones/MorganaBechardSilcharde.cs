...
namespace DOL.AI.Brain
{
    public class MorganaBrain : StandardMobBrain
    {
        ...
        public override void Think()
        {
            if (!PlayerAreaCheck)
                foreach (GamePlayer player in Body.GetPlayersInRadius(1000))
                    ...
                    {
                        var quest =
                            player.IsDoingQuest(typeof(GS.Quests.Albion.Academy_50)) as GS.Quests.Albion.Academy_50;
                        if (quest != null && quest.Step == 1)
                        {
                            SpawnDemons = true;
                            player.Out.SendMessage(
                                "Ha, is this all the forces of Albion have to offer? I expected a whole army leaded by my brother Arthur, but what do they send a little group of adventurers lead by a poor " +
                                player.CharacterClass.Name + "?", eChatType.CT_Say, eChatLoc.CL_ChatWindow);
                            PlayerAreaCheck = true;
                        }
                    }

            INpcTemplate npcTemplate = NpcTemplateMgr.GetTemplate(700000001);
            if (Morgana.BechardMinionCount > 0 && Morgana.BechardDemonicMinionsCount > 0 &&
                Morgana.SilchardeMinionCount > 0 && Morgana.SilchardeDemonicMinionsCount > 0)
                if (Morgana.BechardDemonicMinionsCount >= 10 && Morgana.SilchardeDemonicMinionsCount >= 10)
                    if (!Morganacast)
                    {
                        ...
                        {
                            BroadcastMessage(
                                "Morgana shouts, \"I cannot believe my creations have been undone so easily! Heed my words mortal! You may have won this battle but I shall return! On that day all who walk this realm will know what fear truly is!" +
        ...
        private int MorganaCast(ECSGameTimer timer)
        {
            ...
            var resetTimer = Properties.SET_EPIC_QUEST_ENCOUNTER_RESPAWNINTERVAL * 60000; //1h to reset encounter
            new ECSGameTimer(Body, new ECSGameTimer.ECSTimerCallback(RestartMorgana),
                resetTimer); //reset whole encounter here
            return 0;
        }
...
namespace DOL.GS
{
    public class Bechard : GameEpicNPC
    {
        ...
        public override int MaxHealth => 7000;
        ...
        {
            var spawn = new Point3D(306041, 670103, 3310);
            for (var i = 0; i < Morgana.BechardMinionCount + Util.Random(4, 6); i++)
            {
                var npc = new DemonicMinion();
...
namespace DOL.GS
{
    public class Silcharde : GameEpicNPC
    {
        ...
        {
            var spawn = new Point3D(306041, 670103, 3310);
            for (var i = 0; i < Morgana.SilchardeMinionCount + Util.Random(4, 6); i++)
            {
                var npc = new DemonicMinion();
                npc.X = spawn.X + Util.Random(-150, 150);
                npc.Y = spawn.Y + Util.Random(-150, 150);
...
namespace DOL.GS
{
    public class DemonicMinion : GameNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new DemonicMinionBrain();
            SetOwnBrain(sbrain);
            LoadedFromScript = true;
            RespawnInterval = -1;
            base.AddToWorld();
