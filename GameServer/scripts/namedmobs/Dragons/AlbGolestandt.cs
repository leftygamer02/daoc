...
namespace DOL.GS
{
    public class AlbGolestandt : GameEpicBoss
    {
        ...
        public static ushort LairRadius => 2000;
        ...
        public override void LoadFromDatabase(DataObject obj)
        {
            ...
            var dragonName = Name.Split(new char[] {' '});
        ...
        private static Point3D spawnPoint = new(391344, 755419, 395);

        public override ushort SpawnHeading
        {
            get => base.SpawnHeading;
            set => base.SpawnHeading = 2071;
        }

        public override Point3D SpawnPoint
        {
            get => spawnPoint;
            set => base.SpawnPoint = spawnPoint;
        }
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new AlbGolestandtBrain();
            SetOwnBrain(sbrain);
            sbrain.Start();
        ...
        public override void EnemyKilled(GameLiving enemy)
        {
            ...
            {
                var player = (GamePlayer) enemy;
                foreach (var client in WorldMgr.GetClientsOfZone(CurrentZone.ID))
                {
                    if (client == null) break;
                    if (client.Player == null) continue;
                    if (client.IsPlaying)
                        client.Out.SendMessage(
                            Name + " roars in triumph as another " + player.CharacterClass.Name +
                            " falls before his might.", eChatType.CT_Say, eChatLoc.CL_ChatWindow);
                }
            }
...
namespace DOL.AI.Brain
{
    public class AlbGolestandtBrain : StandardMobBrain
    {
        ...
        public static List<GameNPC> DragonAdds = new();
        private List<Point3D> _roamingPathPoints = new();
        ...
        public override void Think()
        {
            if (!CheckProximityAggro())
            {
                ...
                if (!checkForMessangers)
                {
                    ...
                    {
                        foreach (var messenger in DragonAdds)
                            if (messenger != null && messenger.IsAlive && messenger.Brain is GolestandtMessengerBrain)
                                messenger.RemoveFromWorld();

                        foreach (var granitegiant in DragonAdds)
                            if (granitegiant != null && granitegiant.IsAlive &&
                                granitegiant.Brain is GolestandtSpawnedAdBrain)
            ...
            if (Body.CurrentRegion.IsPM && Body.CurrentRegion.IsNightTime == false && !LockIsRestless &&
                !Body.InCombatInLast(30000) && _lastRoamIndex < _roamingPathPoints.Count) //Dragon will start roam
            {
                ...
                _lastRoamIndex = 0;
                LockEndRoute = false;
                foreach (var client in WorldMgr.GetClientsOfZone(Body.CurrentZone.ID)) //from current zone
                {
                    if (client == null) break;
                    if (client.Player == null) continue;
                    if (client.IsPlaying)
                    {
                        client.Out.SendSoundEffect(2467, 0, 0, 0, 0,
                            0); //play sound effect for every player in boss currentregion
                        client.Out.SendMessage(
                            "A voice explodes across the land. You hear a roar in the distance, 'I will grind your bones and shred your flesh.'"
            ...
            if (IsRestless) //special glare phase, during dragon roam it will cast glare like a mad
                ...
                {
                    new ECSGameTimer(Body, new ECSGameTimer.ECSTimerCallback(PrepareGlareRoam),
                        Util.Random(5000, 8000)); //Glare at target every 5-10s
                    CanGlare2 = true;
                }
            ...
            if (HasAggro && Body.TargetObject != null)
            {
                ...
                {
                    var throwPlayer = new ECSGameTimer(Body, new ECSGameTimer.ECSTimerCallback(ThrowPlayer),
                        Util.Random(60000, 80000)); //Teleport 2-5 Players every 60-80s
                ...
                {
                    var prepareGlare = new ECSGameTimer(Body, new ECSGameTimer.ECSTimerCallback(PrepareGlare),
                        Util.Random(40000, 60000)); //Glare at target every 40-60s
                ...
                {
                    var prepareStun = new ECSGameTimer(Body, new ECSGameTimer.ECSTimerCallback(PrepareStun),
                        Util.Random(120000, 180000)); //prepare Stun every 120s-180s
                ...
                {
                    var spawnMessengers = new ECSGameTimer(Body,
                        new ECSGameTimer.ECSTimerCallback(SpawnMssengers),
                        Util.Random(80000, 90000)); //spawn messengers at 50% hp every 80/90s
        ...
        private void DragonFlyingPath()
        {
            if (IsRestless && Body.IsAlive)
            {
                ...
                if (Body.IsWithinRadius(_roamingPathPoints[_lastRoamIndex], 100)) _lastRoamIndex++;
        ...
        private List<GamePlayer> randomlyPickedPlayers = new();
        ...
        private int ThrowPlayer(ECSGameTimer timer)
        {
            if (Body.IsAlive && HasAggro)
            {
                ...
                {
                    randomlyPickedPlayers =
                        GetRandomElements(Port_Enemys, Util.Random(2, 5)); //pick 2-5players from list to new list

                    if (randomlyPickedPlayers.Count > 0)
                    {
                        foreach (var player in randomlyPickedPlayers)
                            if (player != null && player.IsAlive && player.Client.Account.PrivLevel == 1 && HasAggro &&
                                player.IsWithinRadius(Body, 2000))
                            {
                                player.Out.SendMessage(
                                    Body.Name +
                                    " begins flapping his wings violently. You struggle to hold your footing on the ground!",
                                    eChatType.CT_Broadcast, eChatLoc.CL_ChatWindow);
                                switch (Util.Random(1, 5))
                                {
                                    case 1:
                                        player.MoveTo(Body.CurrentRegionID, 391348, 755751, 1815, 2069);
                                        break; //lair spawn point
                                    case 2:
                                        player.MoveTo(Body.CurrentRegionID, 398605, 754458, 1404, 1042);
                                        break;
                                    case 3:
                                        player.MoveTo(Body.CurrentRegionID, 392450, 743176, 1404, 4063);
                                        break;
                                    case 4:
                                        player.MoveTo(Body.CurrentRegionID, 383669, 758112, 1847, 3003);
                                        break;
                                    case 5:
                                        player.MoveTo(Body.CurrentRegionID, 401432, 755310, 1728, 1065);
                                        break;
                                }
                            }
        ...
        private List<string> glare_text = new()
        {
        ...
        private List<GamePlayer> Glare_Enemys = new();
        ...
        private int PrepareGlare(ECSGameTimer timer)
        {
            if (!IsRestless && HasAggro && Body.IsAlive)
            {
                ...
                if (Glare_Enemys.Count > 0)
                {
                    var Target = Glare_Enemys[Util.Random(0, Glare_Enemys.Count - 1)];
                    RandomTarget = Target;
                    if (RandomTarget != null && RandomTarget.IsAlive &&
                        RandomTarget.IsWithinRadius(Body, Dragon_DD.Range))
                    {
                        BroadcastMessage(string.Format("{0} stares at {1} and prepares a massive attack.", Body.Name,
                            RandomTarget.Name));
        ...
        {
            if (!IsRestless && HasAggro && Body.IsAlive && RandomTarget != null && RandomTarget.IsAlive &&
                RandomTarget.IsWithinRadius(Body, Dragon_DD.Range) && !Body.IsCasting)
            {
                Body.TargetObject = RandomTarget;
                Body.TurnTo(RandomTarget);
                Body.CastSpell(Dragon_DD, SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells));
                var glaretext = glare_text[Util.Random(0, glare_text.Count - 1)];
                RandomTarget.Out.SendMessage(string.Format(glaretext, Body.Name, RandomTarget.CharacterClass.Name),
        ...
        private List<string> glareroam_text = new()
        {
            "{0} shouts, 'Foolish {1}! Your flesh will make a splendid meal.'",
            "{0} shouts, 'Perhaps your dark ages would end if {1}s like you continue to be weeded out!'",
            "{0} shouts, 'Meddle not in the affairs of dragons, {1}! Yes, you are indeed crunchy.'"
        ...
        private List<GamePlayer> GlareRoam_Enemys = new();
        ...
        private int PrepareGlareRoam(ECSGameTimer timer)
        {
            if (IsRestless && Body.IsAlive)
            {
                ...
                {
                    var Target = GlareRoam_Enemys[Util.Random(0, GlareRoam_Enemys.Count - 1)];
                    RandomTarget2 = Target;
                    if (RandomTarget2 != null && RandomTarget2.IsAlive &&
                        RandomTarget2.IsWithinRadius(Body, Dragon_DD2.Range))
                    {
                        foreach (GamePlayer player in Body.GetPlayersInRadius(5000))
                            if (player != null)
                                player.Out.SendMessage(
                                    string.Format("{0} stares at {1} and prepares a massive attack.", Body.Name,
                                        RandomTarget2.Name), eChatType.CT_Broadcast, eChatLoc.CL_ChatWindow);
        ...
        {
            if (IsRestless && Body.IsAlive && RandomTarget2 != null && RandomTarget2.IsAlive &&
                RandomTarget2.IsWithinRadius(Body, Dragon_DD2.Range) && !Body.IsCasting)
            {
                Body.TargetObject = RandomTarget2;
                Body.TurnTo(RandomTarget2);
                Body.CastSpell(Dragon_DD2, SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells),
                    false); //special roaming glare
                var glaretextroam = glareroam_text[Util.Random(0, glareroam_text.Count - 1)];
                RandomTarget2.Out.SendMessage(
                    string.Format(glaretextroam, Body.Name, RandomTarget2.CharacterClass.Name), eChatType.CT_Say,
                    eChatLoc.CL_ChatWindow);
            }
        ...
        private List<string> breath_text = new()
        {
        ...
        {
            var message = breath_text[Util.Random(0, breath_text.Count - 1)];
        ...
        {
            for (var i = 0; i <= Util.Random(3, 5); i++)
            {
                var messenger = new GolestandtMessenger();
...
namespace DOL.GS
{
    public class GolestandtMessenger : GameNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            Faction = FactionMgr.GetFactionByID(31);
            Faction.AddFriendFaction(FactionMgr.GetFactionByID(31));
            var adds = new GolestandtMessengerBrain();
...
namespace DOL.AI.Brain
{
    public class GolestandtMessengerBrain : StandardMobBrain
    {
        ...
        {
            var point1 = new Point3D(391353, 752390, 200);
            var point2 = new Point3D(391185, 749212, 363);
            var point3 = new Point3D(393537, 748310, 563);
        ...
        {
            var point1 = new Point3D(394175, 755677, 200);
            var point2 = new Point3D(395993, 753955, 200);
            var point3 = new Point3D(397102, 752316, 448);
        ...
        {
            var point1 = new Point3D(390959, 758732, 431);
            var point2 = new Point3D(394489, 758450, 411);
            var point3 = new Point3D(397504, 756902, 324);
        ...
        {
            var point1 = new Point3D(388541, 754932, 204);
            var point2 = new Point3D(386798, 756434, 323);
            var point3 = new Point3D(385011, 757680, 523);
        ...
        {
            if (Body.IsAlive) Body.RemoveFromWorld();
        ...
        private protected void SpawnGraniteGiants()
        {
            for (var i = 0; i <= Util.Random(3, 5); i++)
            {
                var add = new GolestandtSpawnedAdd();
...
namespace DOL.GS
{
    public class GolestandtSpawnedAdd : GameNPC
    {
        ...
        private List<string> adds_names = new()
        {
        ...
        public override bool AddToWorld()
        {
            ...
            switch (Name)
            {
                case "granite giant stonelord":
                    Model = 2386;
                    Size = (byte) Util.Random(150, 170);
                    break;
                case "granite giant pounder":
                    Model = 2386;
                    Size = (byte) Util.Random(130, 150);
                    break;
                case "granite giant outlooker":
                    Model = 2386;
                    Size = (byte) Util.Random(130, 140);
                    break;
            }
            ...
            MaxSpeedBase = 225;
            var sbrain = new GolestandtSpawnedAdBrain();
...
namespace DOL.AI.Brain
{
    public class GolestandtSpawnedAdBrain : StandardMobBrain
    {
        ...
        private protected void Path1()
        {
            var point1 = new Point3D(393690, 747560, 585);
            var point2 = new Point3D(391348, 749033, 374);
            var point3 = new Point3D(391351, 755567, 412);
        ...
        private protected void Path2()
        {
            var point1 = new Point3D(397218, 752345, 444);
            var point2 = new Point3D(394848, 755457, 200);
            var point3 = new Point3D(391445, 755608, 410);
            if (!Body.IsWithinRadius(point1, 30) && path2point1 == false)
            {
                Body.WalkTo(point1, 200);
            }
        ...
        private protected void Path3()
        {
            var point1 = new Point3D(397504, 756902, 324);
            var point2 = new Point3D(390933, 758814, 446);
            var point3 = new Point3D(391331, 755730, 398);
            if (!Body.IsWithinRadius(point1, 30) && path3point1 == false)
            {
                Body.WalkTo(point1, 200);
            }
        ...
        private protected void Path4()
        {
            var point1 = new Point3D(384804, 757887, 537);
            var point2 = new Point3D(388423, 754910, 212);
            var point3 = new Point3D(391103, 755534, 380);
