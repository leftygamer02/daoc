...
namespace DOL.GS
{
    public class MidGjalpinulva : GameEpicBoss
    {
        ...
        private static Point3D spawnPoint = new(708888, 1021439, 3014);

        public override ushort SpawnHeading
        {
            get => base.SpawnHeading;
            set => base.SpawnHeading = 2531;
        }
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new MidGjalpinulvaBrain();
            SetOwnBrain(sbrain);
            sbrain.Start();
        ...
        public override void EnemyKilled(GameLiving enemy)
        {
            if (enemy != null && enemy is GamePlayer)
            {
                ...
                player.Out.SendMessage(Name + " says, 'Your soul now belongs to the " + Name, eChatType.CT_Say,
                    eChatLoc.CL_ChatWindow);
            }
...
namespace DOL.AI.Brain
{
    public class MidGjalpinulvaBrain : StandardMobBrain
    {
            ...
            if (!CheckProximityAggro())
            {
                ...
                if (!checkForMessangers)
                {
                    if (DragonAdds.Count > 0)
                    {
                        ...
                        foreach (var drakulv in DragonAdds)
                ...
                foreach (var client in WorldMgr.GetClientsOfZone(Body.CurrentZone.ID)) //from current zone
                {
                    ...
                    if (client.IsPlaying)
                    {
                        ...
                        client.Out.SendMessage(
                            "A booming voice echoes through the canyons, I grow restless.Who has dared to enter my domain ? " +
                            "I shall freeze their flesh and grind their bones to dust!", eChatType.CT_Broadcast,
                            eChatLoc.CL_ChatWindow);
                    }
                }
                        ...
                        foreach (var player in randomlyPickedPlayers)
                                ...
                                player.IsWithinRadius(Body, 2000))
                            {
                                player.Out.SendMessage(
                                    Body.Name +
                                    " begins flapping her wings violently. You struggle to hold your footing on the ground!",
                                    eChatType.CT_Broadcast, eChatLoc.CL_ChatWindow);
                                switch (Util.Random(1, 5))
                                {
                                    case 1:
                                        player.MoveTo(100, 708632, 1021688, 3721, 2499);
                                        break; //lair spawn point
                                    case 2:
                                        player.MoveTo(100, 713073, 1015679, 3833, 441);
                                        break;
                                    case 3:
                                        player.MoveTo(100, 713388, 1024499, 3833, 1372);
                                        break;
                                    case 4:
                                        player.MoveTo(100, 705812, 1024952, 3833, 2573);
                                        break;
                                    case 5:
                                        player.MoveTo(100, 706019, 1018867, 3833, 3521);
                                        break;
                                }
                            }
        ...
        private int SpawnMssengers(ECSGameTimer timer)
        {
            ...
            {
                var messenger = new GjalpinulvaMessenger();
...
namespace DOL.GS
{
    public class GjalpinulvaMessenger : GameNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            Faction = FactionMgr.GetFactionByID(781);
            Faction.AddFriendFaction(FactionMgr.GetFactionByID(781));
            var adds = new GjalpinulvaMessengerBrain();
...
namespace DOL.AI.Brain
{
    public class GjalpinulvaMessengerBrain : StandardMobBrain
    {
        ...
        {
            var point1 = new Point3D(710329, 1019375, 2824);
            var point2 = new Point3D(710514, 1016616, 2893);
            var point3 = new Point3D(710434, 1013684, 2783);
        ...
        {
            var point1 = new Point3D(706980, 1019434, 2824);
            var point2 = new Point3D(702629, 1021259, 2800);
            var point3 = new Point3D(699391, 1019292, 2681);
        ...
        {
            var point1 = new Point3D(710841, 1023038, 2824);
            var point2 = new Point3D(714212, 1025142, 2782);
        ...
        {
            var point1 = new Point3D(706824, 1023914, 2759);
            var point2 = new Point3D(708924, 1025927, 2817);
            var point3 = new Point3D(712828, 1026645, 2824);
        ...
        private protected void SpawnDrakulvs()
        {
            for (var i = 0; i <= Util.Random(3, 5); i++)
            {
                var add = new GjalpinulvaSpawnedAdd();
...
namespace DOL.GS
{
    public class GjalpinulvaSpawnedAdd : GameNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            switch (Name)
            {
                case "drakulv executioner":
                    Model = 625;
                    Size = (byte) Util.Random(130, 150);
                    break;
                case "drakulv disciple":
                    Model = 617;
                    Size = (byte) Util.Random(120, 140);
                    break;
                case "drakulv soultrapper":
                    Model = 624;
            ...
            MaxSpeedBase = 225;
            var sbrain = new GjalpinulvaSpawnedAdBrain();
...
namespace DOL.AI.Brain
{
    public class GjalpinulvaSpawnedAdBrain : StandardMobBrain
    {
        ...
        private protected void Path1()
        {
            var point1 = new Point3D(710646, 1016748, 2918);
            var point2 = new Point3D(710546, 1018812, 2824);
            var point3 = new Point3D(708814, 1021611, 3028);
        ...
        private protected void Path2()
        {
            var point1 = new Point3D(702705, 1020839, 2818);
            var point2 = new Point3D(707015, 1019589, 2824);
            var point3 = new Point3D(708814, 1021611, 3028);
            if (!Body.IsWithinRadius(point1, 30) && path2point1 == false)
            {
                Body.WalkTo(point1, 200);
        ...
        private protected void Path3()
        {
            var point1 = new Point3D(712485, 1024302, 2824);
            var point2 = new Point3D(708814, 1021611, 3028);
            if (!Body.IsWithinRadius(point1, 30) && path3point1 == false)
            {
                Body.WalkTo(point1, 200);
            }
        ...
        private protected void Path4()
        {
            var point1 = new Point3D(709203, 1025740, 2824);
            var point2 = new Point3D(706957, 1024039, 2745);
            var point3 = new Point3D(708814, 1021611, 3028);
