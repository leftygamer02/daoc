...
namespace DOL.GS.Scripts
{
    public class Legion : GameEpicBoss
    {
        ...
        public static void ScriptLoaded(DOLEvent e, object sender, EventArgs args)
        {
            ...
            var region = WorldMgr.GetRegion(249);
        ...
        public override void TakeDamage(GameObject source, eDamageType damageType, int damageAmount, int criticalAmount)
        {
            ...
            if (source is GamePlayer || source is GamePet)
            {
                if (!source.IsWithinRadius(this, distance)) //take no damage from source that is not in radius 1000
                {
                    ...
                    if (truc != null)
                        truc.Out.SendMessage(Name + " is not attackable from this range and is immune to your damage!",
                            eChatType.CT_System,
                            eChatLoc.CL_ChatWindow);
        ...
        {
            var numPlayers = AwardLegionKillPoint();
    ...
    public class LegionBrain : EpicBossBrain
    {
        ...
        public override void Think()
        {
            ...
            if (HasAggro && Body.TargetObject != null)
            {
                ...
                {
                    ReleaseAoeLifetap();
                    var castAoe = new ECSGameTimer(Body, new ECSGameTimer.ECSTimerCallback(ResetAoe), 10000);
                ...
                if (!CanThrow)
                {
                    var throwPlayer = new ECSGameTimer(Body, new ECSGameTimer.ECSTimerCallback(ThrowPlayer),
                        Util.Random(40000, 65000)); //throw players
        ...
        private void DestroyDamnBubble()
        {
            if (Body.TargetObject != null && HasAggro)
            {
                ...
                if (Util.Chance(100))
                    if (target.effectListComponent.ContainsEffectForEffectType(eEffect.Bladeturn) && target != null &&
                        target.IsAlive)
                    {
                        ...
                        if (effect != null)
                        {
                            ...
                            if (target is GamePlayer player)
                                if (player != null && player.IsAlive)
                                    player.Out.SendMessage("Legion consume your bladeturn effect!", eChatType.CT_Say,
                                        eChatLoc.CL_ChatWindow);
                        }
                    }
            }
        ...
        private void SpawnAdds()
        {
            if (Body.InCombat && Body.IsAlive && HasAggro)
                ...
                {
                    if (playerNearby != null && playerNearby.Client.Account.PrivLevel == 1) PlayerCountInLegionLair++;
        ...
        private static int
            topPlayersToIngore =
                5; //we determine here how many players from top aggro table will be ignored in teleporting
        ...
        private int ThrowPlayer(ECSGameTimer timer)
        {
            if (Body.IsAlive && HasAggro)
            {
                ...
                var tempAggroTable = aggroList
                    .OrderByDescending(x => x.Value).Skip(topPlayersToIngore).OrderBy(x => random.Next());
                foreach (var items in tempAggroTable)
                ...
                if (Port_Enemys.Count > 0)
                {
                    randomlyPickedPlayers =
                        GetRandomElements(Port_Enemys, Util.Random(8, 16)); //pick 5-8players from list to new list
                    ...
                    if (randomlyPickedPlayers.Count > 0)
                    {
                        foreach (var player in randomlyPickedPlayers)
                            if (player != null && player.IsAlive && player.Client.Account.PrivLevel == 1 && HasAggro &&
                                player.IsWithinRadius(Body, 2500))
                                player.MoveTo(249, 48200, 49566, 20833, 1028);
                        //player.BroadcastUpdate();
...
namespace DOL.AI.Brain
{
    public class LegionAddBrain : StandardMobBrain
    {
        ...
        public override void Think()
        {
            if (Body.InCombatInLast(60 * 1000) == false && Body.InCombatInLast(65 * 1000)) Body.RemoveFromWorld();
...
namespace DOL.GS
{
    public class Behemoth : GameEpicBoss
    {
        ...
        public override int MaxHealth => 600000;
