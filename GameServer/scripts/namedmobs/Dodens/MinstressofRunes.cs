...
namespace DOL.GS
{
    public class MistressRunes : GameEpicBoss
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new MistressRunesBrain();
            SetOwnBrain(sbrain);
...
namespace DOL.AI.Brain
{
    public class MistressRunesBrain : StandardMobBrain
    {
            ...
            if (HasAggro && Body.TargetObject != null)
            {
                ...
                if (!Body.IsCasting && CanCast)
                    if (RandomTarget != null && RandomTarget.IsAlive)
                    {
                        ...
                        if (Body.GetSkillDisabledDuration(NearsightMistress) == 0 &&
                            (!Body.IsCasting || Body.IsCasting))
        ...
        private int CastSpears(ECSGameTimer timer)
        {
            ...
            {
                var message = mistress_text[Util.Random(0, mistress_text.Count - 1)];
        ...
        private List<string> mistress_text = new()
        {
        ...
        private List<GamePlayer> Enemys_To_NS = new();
        ...
        private int PickRandomTarget(ECSGameTimer timer)
        {
            ...
            if (Enemys_To_NS.Count > 0)
            {
                ...
                {
                    var Target =
                        Enemys_To_NS[Util.Random(0, Enemys_To_NS.Count - 1)]; //pick random target from list
        ...
        {
            if (RandomTarget != null && Enemys_To_NS.Contains(RandomTarget) &&
                RandomTarget.effectListComponent.ContainsEffectForEffectType(eEffect.Nearsight))
