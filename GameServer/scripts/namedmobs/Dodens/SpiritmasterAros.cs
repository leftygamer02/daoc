...
namespace DOL.GS
{
    public class SpiritmasterAros : GameEpicBoss
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new SpiritmasterArosBrain();
            SetOwnBrain(sbrain);
        ...
        public override void Die(GameObject killer)
        {
            ...
            {
                case 1:
                    BroadcastMessage("'You will remember my name! " + Name + "!'");
                    break;
                case 2:
                    BroadcastMessage(Name + " trips and falls on the hard stone floor.");
                    break;
            }
        ...
        public override void DealDamage(AttackData ad)
        {
            if (ad != null && ad.AttackType == AttackData.eAttackType.Spell && ad.Damage > 0 &&
                ad.DamageType == eDamageType.Cold)
                Health += ad.Damage / 6;
            base.DealDamage(ad);
        }
    }
...
namespace DOL.AI.Brain
{
    public class SpiritmasterArosBrain : StandardMobBrain
    {
        ...
        private List<string> Aros_bomb_text = new()
        {
                ...
                if (!Body.IsCasting && !Body.IsMoving && target != null)
                    foreach (Spell spells in Body.Spells)
                        if (spells != null)
                        {
                            ...
                            {
                                var message = Aros_bomb_text[Util.Random(0, Aros_bomb_text.Count - 1)];
                            ...
                            if (Util.Chance(100))
                            {
                                ...
                                {
                                    Body.CastSpell(Aros_Bomb, SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells),
                                        false);
                                }
                                else if (Body.GetSkillDisabledDuration(Aros_Debuff) == 0)
                                {
                                    ...
                                    if (DebuffTarget != null && CanCastDebuff)
                                    {
                                        ...
                                        Body.CastSpell(Aros_Debuff,
                                            SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells), false);
                                    }
                                }
                                else if (!Body.IsCasting && Body.GetSkillDisabledDuration(Aros_Bomb) > 0 &&
                                         Body.GetSkillDisabledDuration(Aros_Debuff) > 0)
                                {
                                    Body.TurnTo(target);
                                    Body.CastSpell(Aros_DD, SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells),
                                        false);
                                }
                            }
                        }
        ...
        private void SummonPet()
        {
            ...
            if (HasAggro)
            {
                var pet = new SpiritmasterArosPet();
                pet.X = Body.X;
                pet.Y = Body.Y - 100;
                pet.Z = Body.Z;
        ...
        private List<GamePlayer> Enemys_To_Debuff = new();
            ...
            if (HasAggro)
            {
                foreach (GamePlayer player in Body.GetPlayersInRadius(2000))
                    if (player != null && player.IsAlive && player.Client.Account.PrivLevel == 1 &&
                        player.CharacterClass.ID != (int) eCharacterClass.Necromancer)
                ...
                if (Enemys_To_Debuff.Count > 0)
                    ...
                    {
                        var Target =
                            Enemys_To_Debuff[Util.Random(0, Enemys_To_Debuff.Count - 1)]; //pick random target from list
...
namespace DOL.GS
{
    public class SpiritmasterArosPet : GameNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            Flags ^= eFlags.GHOST;
            Realm = eRealm.None;
            var adds = new SpiritmasterArosPetBrain();
            adds.AggroRange = 600;
            adds.AggroLevel = 100;
            SetOwnBrain(adds);
            base.AddToWorld();
