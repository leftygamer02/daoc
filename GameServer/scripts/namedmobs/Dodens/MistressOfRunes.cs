...
namespace DOL.AI.Brain
{
    public class MistressOfRunesBrain : StandardMobBrain
    {
        ...
        public MistressOfRunesBrain() : base()
        {
            ...
            m_SpearAnnounce = new string[]
            {
                "{0} casts a magical flaming spear on {1}!",
        ...
        /// 100 = full strength
        /// </summary>
        public virtual int MistressDifficulty => GS.ServerProperties.Properties.SET_DIFFICULTY_ON_EPIC_ENCOUNTERS;
            ...
            if (Body.InCombat && Body.IsAlive && HasAggro)
                if (Body.TargetObject != null)
                    foreach (GamePlayer player in Body.GetPlayersInRadius(2000))
                    {
                        ...
                        //cast AoE Spears
                        new ECSGameTimer(Body, new ECSGameTimer.ECSTimerCallback(timer => CastSpear(timer, player)),
                            4000);
                    }
        ...
        {
            var mistress = Body as MistressOfRunes;
            if (mistress == null) return false;

            var inRangeLiving = new ArrayList();

            lock ((AggroTable as ICollection).SyncRoot)
            {
                var enumerator = AggroTable.GetEnumerator();
                while (enumerator.MoveNext())
                {
                    var living = enumerator.Current.Key;
                    if (living != null &&
                        living.IsAlive &&
        ...
        private int CastSpear(ECSGameTimer timer, GameLiving target)
        {
            ...
            var cast = Body.CastSpell(AoESpear, SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells));

            if (Body.GetSkillDisabledDuration(AoESpear) > 0) cast = false;

            if (castsSpear && cast && Body.IsCasting)
            {
                castsSpear = false;
                var messageNo = Util.Random(1, m_SpearAnnounce.Length) - 1;
        ...
        /// </summary>
        protected int NearsightChance => m_NearsightChance;
        ...
        {
            get => m_NearsightTarget;
            set
            {
                m_NearsightTarget = value;
                PrepareToNearsight();
            }
        }
        ...
        public bool CheckNearsight(GameLiving target)
        {
            ...
            var success = Util.Chance(NearsightChance);
            if (success)
        ...
        private int CastNearsight(ECSGameTimer timer)
        {
            ...
            Body.Z = Body.SpawnPoint
                .Z; // this is a fix to correct Z errors that sometimes happen during Mistress fights
            Body.TurnTo(NearsightTarget);
            var cast = Body.CastSpell(Nearsight, SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells));
            if (Body.GetSkillDisabledDuration(Nearsight) > 0) cast = false;
