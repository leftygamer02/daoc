...
namespace DOL.GS
{
    public class GlacierGiant : GameEpicBoss
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new GlacierGiantBrain();
            SetOwnBrain(sbrain);
        ...
        public static void ScriptLoaded(DOLEvent e, object sender, EventArgs args)
        {
            ...
            if (npcs.Length == 0)
            {
                ...
                var ubrain = new GlacierGiantBrain();
                ubrain.AggroLevel = 0;
...
namespace DOL.AI.Brain
{
    public class GlacierGiantBrain : EpicBossBrain
    {
        ...
        public override void Think()
        {
            ...
            if (Body.InCombatInLast(30 * 1000) == false && Body.InCombatInLast(35 * 1000) && !HasAggro)
                Body.Health = Body.MaxHealth;
            base.Think();
        }
        ...
        private List<GamePlayer> Enemys_To_Port = new();
        private List<GamePlayer> Teleported_Players = new();
        ...
        public void TeleportPlayer()
        {
            ...
            {
                var PortTarget = (GamePlayer) Enemys_To_Port[Util.Random(0, Enemys_To_Port.Count - 1)];
                RandomTarget = PortTarget;
                if (RandomTarget.IsAlive && RandomTarget != null && RandomTarget.IsWithinRadius(Body, 2000) &&
                    !Teleported_Players.Contains(RandomTarget))
                {
                    switch (Util.Random(1, 6))
                    {
                        case 1:
                            RandomTarget.MoveTo(Body.CurrentRegionID, 663537 + Util.Random(-2000, 2000),
                                626415 + Util.Random(-2000, 2000), 7790 + Util.Random(300, 600), Body.Heading);
                            break;
                        case 2:
                            RandomTarget.MoveTo(Body.CurrentRegionID, 647342 + Util.Random(-2000, 2000),
                                617589 + Util.Random(-2000, 2000), 8533 + Util.Random(300, 600), Body.Heading);
                            break;
                        case 3:
                            RandomTarget.MoveTo(Body.CurrentRegionID, 645157 + Util.Random(-2000, 2000),
                                630671 + Util.Random(-2000, 2000), 11530 + Util.Random(300, 600), Body.Heading);
                            break;
                        case 4:
                            RandomTarget.MoveTo(Body.CurrentRegionID, 654502 + Util.Random(-2000, 2000),
                                630523 + Util.Random(-2000, 2000), 8762 + Util.Random(300, 600), Body.Heading);
                            break;
                        case 5:
                            RandomTarget.MoveTo(Body.CurrentRegionID, 670626 + Util.Random(-2000, 2000),
                                630046 + Util.Random(-2000, 2000), 7515 + Util.Random(300, 600), Body.Heading);
                            break;
                        case 6:
                            RandomTarget.MoveTo(Body.CurrentRegionID, 642185 + Util.Random(-2000, 2000),
                                620183 + Util.Random(-2000, 2000), 10014 + Util.Random(300, 600), Body.Heading);
                            break;
                    }
                    ...
                    foreach (GamePlayer player in Body.GetPlayersInRadius(2000))
                        if (player != null)
                            player.Out.SendMessage("Glacier Giant kick away " + RandomTarget.Name + "!",
                    ...
                    if (RandomTarget != null && RandomTarget.IsAlive && !Teleported_Players.Contains(RandomTarget))
                    {
                        ...
                        if (Clear_List == false)
                        {
                            new ECSGameTimer(Body, new ECSGameTimer.ECSTimerCallback(ListCleanTimer),
                                45000); //clear list of teleported players, so it will not pick instantly already teleported target
                            Clear_List = true;
                        }
                    }
