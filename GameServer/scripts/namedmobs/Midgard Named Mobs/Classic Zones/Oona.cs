            ...
            if (source is GamePlayer || source is GamePet)
            {
                ...
                if (!source.IsWithinRadius(spawn, TetherRange)) //dont take any dmg 
                {
                        ...
                        || damageType == eDamageType.Slash)
                    {
                        ...
                        if (truc != null)
                            truc.Out.SendMessage(Name + " is immune to damage form this distance!", eChatType.CT_System,
            ...
            if (enemy is GamePlayer)
                if (player != null)
                {
                    ...
                    BroadcastMessage(string.Format(
                        "Perhaps your pathetic gods will grant you another life, {0}. In the meantime, Hibernia shall defeat Midgard, and your spirit shall help!",
                        player.Name));
                }
                        ...
                        if (spells != null)
                        {
                            ...
                            if (Util.Chance(100))
                            {
                                if (Body.GetSkillDisabledDuration(OonaBolt) == 0)
                                    Body.CastSpell(OonaBolt, SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells),
                                        false);
                                else
...
namespace DOL.GS
{
    public class OonaUndeadSoldier : GameNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new OonaUndeadSoldierBrain();
            SetOwnBrain(sbrain);
            base.AddToWorld();
            return true;
        }
...
namespace DOL.GS
{
    public class OonaUndeadAdd : GameNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new OonaUndeadAddBrain();
            SetOwnBrain(sbrain);
