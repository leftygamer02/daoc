                    ...
                    foreach (Spell spells in Body.Spells)
                        if (spells != null)
                        {
                            ...
                            if (Util.Chance(100))
                            {
                                ...
                                if (target != null)
                                {
                                    if (!Body.IsCasting && Body.GetSkillDisabledDuration(VasremSCDebuff) == 0 &&
                                        !target.effectListComponent.ContainsEffectForEffectType(eEffect.StrConDebuff))
                                        Body.CastSpell(VasremSCDebuff,
                                            SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells), false);
                                    else if (!Body.IsCasting && Body.GetSkillDisabledDuration(VasremDebuffDQ) == 0 &&
                                             !target.effectListComponent.ContainsEffectForEffectType(
                                                 eEffect.DexQuiDebuff))
                                        Body.CastSpell(VasremDebuffDQ,
                                            SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells), false);
                                    else
                                        Body.CastSpell(Vasrem_Lifetap,
                                            SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells), false);
                                }
                            }
                        }
