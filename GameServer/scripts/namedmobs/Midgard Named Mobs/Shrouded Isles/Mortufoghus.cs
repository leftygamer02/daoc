...
namespace DOL.GS
{
    public class Mortufoghus : GameEpicBoss
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new MortufoghusBrain();
            SetOwnBrain(sbrain);
            ...
            if (HasAggro && Body.TargetObject != null)
            {
                ...
                if (IsTargetPicked == false)
                {
                    new ECSGameTimer(Body, new ECSGameTimer.ECSTimerCallback(ThrowPlayer),
                        Util.Random(25000, 35000)); //timer to port and pick player
                    IsTargetPicked = true;
                }
            }
        ...
        public int ThrowPlayer(ECSGameTimer timer)
        {
            if (Body.IsAlive && HasAggro)
            {
                ...
                if (Port_Enemys.Count > 0)
                {
                    ...
                    {
                        RandomTarget.MoveTo(Body.CurrentRegionID, Body.X + Util.Random(-1500, 1500),
                            Body.Y + Util.Random(-1500, 1500), Body.Z, Body.Heading);
