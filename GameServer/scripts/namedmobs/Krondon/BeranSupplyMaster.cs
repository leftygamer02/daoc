...
namespace DOL.GS
{
    public class BeranSupplyMaster : GameEpicBoss
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new BeranSupplyMasterBrain();
            SetOwnBrain(sbrain);
...
namespace DOL.AI.Brain
{
    public class BeranSupplyMasterBrain : StandardMobBrain
    {
        ...
        public int CallForHelp(ECSGameTimer timer)
        {
            if (HasAggro)
            {
                for (var i = 0; i < Util.Random(2, 4); i++)
                {
                    var add = new GameNPC();
...
namespace DOL.GS
{
    public class BarrelExplosive : GameNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new BarrelExplosiveBrain();
            SetOwnBrain(sbrain);
            ...
            if (success)
            {
                ...
                new ECSGameTimer(this, new ECSGameTimer.ECSTimerCallback(Explode),
                    8000); //8 seconds until this will explode and deal heavy heat dmg
            }
        ...
        protected int Show_Effect(ECSGameTimer timer)
        {
            if (IsAlive)
            {
                Parallel.ForEach(GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE).OfType<GamePlayer>(),
                    player => { player?.Out.SendSpellEffectAnimation(this, this, 5976, 0, false, 0x01); });
