...
namespace DOL.GS
{
    public class MorgusUrgalorg : GameEpicBoss
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new MorgusUrgalorgBrain();
            SetOwnBrain(sbrain);
    ...
    public class MorgusUrgalorgBrain : StandardMobBrain
    {
        ...
        public void PickRandomTarget()
        {
            ...
            if (Enemys_To_DD.Count > 0)
                ...
                {
                    var Target =
                        Enemys_To_DD[Util.Random(0, Enemys_To_DD.Count - 1)]; //pick random target from list
                            ...
                            if (spells != null)
                            {
                                ...
                                if (Util.Chance(100))
                                {
                                    ...
                                    if (CanCast)
                                    {
                                        ...
                                        if (RandomTarget != null && RandomTarget.IsAlive && CanCast)
                                        {
                                            if (!Body.IsCasting && Body.GetSkillDisabledDuration(Morgus_Bolt) == 0)
                                            {
                                                ...
                                                Body.CastSpell(Morgus_Bolt,
                                                    SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells), false);
                                            }

                                            if (!Body.IsCasting && Body.GetSkillDisabledDuration(Morgus_Bolt) > 0 &&
                                                Body.GetSkillDisabledDuration(Morgus_Bolt2) == 0)
                                            {
                                                Body.TargetObject = RandomTarget;
                                                Body.TurnTo(RandomTarget);
                                                Body.CastSpell(Morgus_Bolt2,
                                                    SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells), false);
                                            }
                                        }
                                    ...
                                    else
                                    {
                                        ...
                                        Body.CastSpell(Morgus_DD, SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells),
                                            false);
                                    }
                                }
                            }
