...
namespace DOL.GS
{
    public class DebenSeGecynde : GameEpicBoss
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new DebenSeGecyndeBrain();
            SetOwnBrain(sbrain);
...
namespace DOL.AI.Brain
{
    public class DebenSeGecyndeBrain : StandardMobBrain
    {
        ...
        public int SpawnAdds(ECSGameTimer timer)
        {
            if (HasAggro)
            {
                for (var i = 0; i < Util.Random(1, 3); i++)
                {
                    var npc = new DebenFighter();
...
namespace DOL.GS
{
    public class DebenFighter : GameEpicNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new DebenFighterBrain();
            SetOwnBrain(sbrain);
        ...
        public override void Think()
        {
            if (HasAggro && Body.TargetObject != null)
                if (!Body.IsCasting && !Body.IsBeingInterrupted)
                {
                    ...
                    {
                        case 1:
                            Body.CastSpell(Mage_DD, SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells));
                            break;
                        case 2:
                            Body.CastSpell(Mage_DD2, SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells));
                            break;
                    }
                }
