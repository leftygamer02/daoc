...
namespace DOL.GS
{
    public class DrihtenElreden : GameEpicBoss
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new DrihtenElredenBrain();
            SetOwnBrain(sbrain);
...
namespace DOL.AI.Brain
{
    public class DrihtenElredenBrain : StandardMobBrain
    {
        ...
        private List<GameNPC> CallHelp = new();
        private List<GameNPC> PulledMobs = new();
        ...
        public void BringHelp()
        {
            if (HasAggro)
                foreach (GameNPC npc in Body.GetNPCsInRadius(2500))
                {
                    ...
                    if (npc.IsAlive && npc.PackageID == "DrihtenBaf" && !CallHelp.Contains(npc) &&
                        !PulledMobs.Contains(npc))
                        CallHelp.Add(npc);
                }

            if (CallHelp.Count > 0)
            {
                var friend = CallHelp[Util.Random(0, CallHelp.Count - 1)];
