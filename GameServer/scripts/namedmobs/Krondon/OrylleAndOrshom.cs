            ...
            if (HasAggro && Body.TargetObject != null)
            {
                if (IsTargetPicked == false && OrshomFire.FireCount > 0)
                {
                    new ECSGameTimer(Body, new ECSGameTimer.ECSTimerCallback(ThrowPlayer),
                        Util.Random(15000, 20000)); //timer to port and pick player
                    IsTargetPicked = true;
                }
...
namespace DOL.GS
{
    public class OrshomFire : GameEpicNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new OrshomFireBrain();
            SetOwnBrain(sbrain);
        ...
        protected int Show_Effect(ECSGameTimer timer)
        {
            if (IsAlive)
            {
                Parallel.ForEach(GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE).OfType<GamePlayer>(),
                    player => { player?.Out.SendSpellEffectAnimation(this, this, 7025, 0, false, 0x01); });
