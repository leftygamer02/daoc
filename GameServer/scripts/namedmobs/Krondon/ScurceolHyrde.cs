...
namespace DOL.GS
{
    public class ScurceolHyrde : GameEpicBoss
    {
        ...
        public override void TakeDamage(GameObject source, eDamageType damageType, int damageAmount, int criticalAmount)
        {
            ...
            {
                if (LyftMihtOne.Orb1Count > 0 || LyftMihtTwo.Orb2Count > 0 || LyftMihtThree.Orb3Count > 0 ||
                    LyftMihtFour.Orb4Count > 0)
                {
                        ...
                        || damageType == eDamageType.Slash)
                    {
                        ...
                        if (truc != null)
                            truc.Out.SendMessage(Name + " is overpowered and can't take any damage.",
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new ScurceolHyrdeBrain();
            SetOwnBrain(sbrain);
        ...
        public override void Die(GameObject killer)
        {
            foreach (GameNPC npc in GetNPCsInRadius(5000))
                if (npc != null)
                    if (npc.IsAlive && (npc.Brain is LyftMihtBrain1 || npc.Brain is LyftMihtBrain2 ||
                                        npc.Brain is LyftMihtBrain3 || npc.Brain is LyftMihtBrain4))
                    {
                        npc.RemoveFromWorld();
    ...
    public class LyftMihtBrain1 : StandardMobBrain
    {
        ...
        private protected int TeleportPlayer(ECSGameTimer timer)
        {
            if (RandomTarget.IsAlive && RandomTarget != null)
            {
                ...
                {
                    case 1:
                        RandomTarget.MoveTo(61, 50986, 20031, 16964, 3091);
                        break;
                    case 2:
                        RandomTarget.MoveTo(61, 51936, 21012, 16964, 2093);
                        break;
                    case 3:
                        RandomTarget.MoveTo(61, 52784, 20019, 16964, 982);
                        break;
                    case 4:
                        RandomTarget.MoveTo(61, 51940, 18968, 16964, 26);
                        break;
                }

                RandomTarget.TakeDamage(RandomTarget, eDamageType.Falling, RandomTarget.MaxHealth / 5, 0);
                RandomTarget.Out.SendMessage("You take falling damage!", eChatType.CT_Important,
        ...
        {
            if (ad != null && Util.Chance(25) && IsTargetPicked == false && ad.Attacker.IsAlive &&
                ad.Attacker != null && ad.Attacker is GamePlayer)
            {
