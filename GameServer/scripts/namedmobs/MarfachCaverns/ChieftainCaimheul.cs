...
namespace DOL.GS
{
    public class ChieftainCaimheul : GameEpicBoss
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new ChieftainCaimheulBrain();
            SetOwnBrain(sbrain);
        ...
        public static void ScriptLoaded(DOLEvent e, object sender, EventArgs args)
        {
            ...
            if (npcs.Length == 0)
            {
                ...
                HOC.Z = 15061;
                HOC.Heading = 694;
                var ubrain = new ChieftainCaimheulBrain();
            ...
            {
                log.Warn(
                    "Chieftain Caimheul exist ingame, remove it and restart server if you want to add by script code.");
            }
        }
    }
}
                ...
                if (Body.TargetObject != null)
                {
                    ...
                    if (Phase2 == false)
                    {
                        if (!living.effectListComponent.ContainsEffectForEffectType(eEffect.Stun) &&
                            !living.effectListComponent.ContainsEffectForEffectType(eEffect.StunImmunity))
                        {
                            Body.Strength = npcTemplate.Strength;
                            Body.SwitchWeapon(eActiveWeaponSlot.Standard);
                            Body.VisibleActiveWeaponSlots = 16;
                            Body.styleComponent.NextCombatStyle =
                                ChieftainCaimheul.slam; //check if target has stun or immunity if not slam
                            Body.BlockChance = 50;
                            Body.ParryChance = 0;
                        ...
                        else if (!living.effectListComponent.ContainsEffectForEffectType(eEffect.Stun) &&
                                 living.effectListComponent.ContainsEffectForEffectType(eEffect.StunImmunity))
                        {
                            Body.Strength = npcTemplate.Strength;
                            Body.SwitchWeapon(eActiveWeaponSlot.Standard);
                    ...
                    if (Body.HealthPercent <= 50 && Phase2 == false) Phase2 = true;
