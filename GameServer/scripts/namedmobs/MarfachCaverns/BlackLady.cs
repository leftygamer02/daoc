...
namespace DOL.GS
{
    public class BlackLady : GameEpicBoss
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var blackladybrain = new BlackLadyBrain();
        ...
        public override void Die(GameObject killer)
        {
            ...
            foreach (var npc2 in WorldMgr.GetNPCsFromRegion(CurrentRegionID))
...
{
    internal class BlackLadyBrain : StandardMobBrain
    {
...
namespace DOL.GS
{
    public class Ogress : GameNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            Constitution = 100;
            Quickness = 125;
            var ogressbrain = new OgressBrain();
...
namespace DOL.AI.Brain
{
    internal class OgressBrain : StandardMobBrain
    {
        public OgressBrain()
            : base()
        {
            ...
            foreach (GamePlayer player in Body.GetPlayersInRadius(2000))
                if (player != null)
                {
                    if (player.IsAlive && player.IsVisibleTo(Body) && player.Client.Account.PrivLevel == 1 &&
                        (player.CharacterClass.ID == 6 || player.CharacterClass.ID == 10 ||
                         player.CharacterClass.ID == 48
                         || player.CharacterClass.ID == 46 || player.CharacterClass.ID == 47 ||
                         player.CharacterClass.ID == 42 || player.CharacterClass.ID == 28 ||
                         player.CharacterClass.ID == 26))
                    {
