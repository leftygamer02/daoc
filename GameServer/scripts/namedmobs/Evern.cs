...
namespace DOL.AI.Brain
{
    public class EvernBrain : EpicBossBrain
    {
            ...
            if (Body.IsAlive && HasAggro)
            {
                ...
                if (Body.TargetObject != null)
                    if (Body.HealthPercent < 100)
                        if (spawnfairy == false)
                        {
                            new ECSGameTimer(Body, new ECSGameTimer.ECSTimerCallback(DoSpawn),
        ...
        {
            for (var i = 0; i < Util.Random(2, 5); i++)
            {
                var Add = new EvernFairy();
...
namespace DOL.AI.Brain
{
    public class EvernFairyBrain : StandardMobBrain
    {
        ...
        public override void Think()
        {
            var point1 = new Point3D(430333, 379905, 2463);
            var point2 = new Point3D(429814, 379895, 2480);
            var point3 = new Point3D(429309, 379894, 2454);
            var point4 = new Point3D(430852, 380150, 2444);
            var point5 = new Point3D(428801, 380156, 2428);
            var point6 = new Point3D(430854, 380680, 2472);
            var point7 = new Point3D(429186, 380418, 2478);
            var point8 = new Point3D(430462, 380411, 2443);
            var point9 = new Point3D(430468, 430468, 2474);
            var point10 = new Point3D(429057, 380920, 2452);
            ...
            if (Body.IsAlive)
            {
                ...
                {
                    case 1:
                        if (!Body.IsMoving && !HasAggro) Body.WalkTo(point1, 80);
                        break;
                    case 2:
                        if (!Body.IsMoving && !HasAggro) Body.WalkTo(point2, 80);
                        break;
                    case 3:
                        if (!Body.IsMoving && !HasAggro) Body.WalkTo(point3, 80);
                        break;
                    case 4:
                        if (!Body.IsMoving && !HasAggro) Body.WalkTo(point4, 80);
                        break;
                    case 5:
                        if (!Body.IsMoving && !HasAggro) Body.WalkTo(point5, 80);
                        break;
                    case 6:
                        if (!Body.IsMoving && !HasAggro) Body.WalkTo(point6, 80);
                        break;
                    case 7:
                        if (!Body.IsMoving && !HasAggro) Body.WalkTo(point7, 80);
                        break;
                    case 8:
                        if (!Body.IsMoving && !HasAggro) Body.WalkTo(point8, 80);
                        break;
                    case 9:
                        if (!Body.IsMoving && !HasAggro) Body.WalkTo(point9, 80);
                        break;
                    case 10:
                        if (!Body.IsMoving && !HasAggro) Body.WalkTo(point10, 80);
                        break;
                }
