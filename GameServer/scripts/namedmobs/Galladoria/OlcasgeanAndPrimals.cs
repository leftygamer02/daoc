...
namespace DOL.GS
{
    public class OlcasgeanInitializator : GameNPC
    {
        ...
        public static GameNPC Olcasgean_Initializator = new();
...
namespace DOL.AI.Brain
{
    public class OIBrain : StandardMobBrain
    {
        ...
        public override void Think()
        {
            ...
            point1.X = 39652;
            point1.Y = 60831;
            point1.Z = 11893; //loc of waterfall bridge to start event and pop elementars
            if (Body.IsAlive)
        ...
        public int Message1(ECSGameTimer timer)
        {
            BroadcastMessage(string.Format(
                "A voice that seems to come from all around you says: 'Intruders have entered inner sanctum.'"));
        ...
        {
            BroadcastMessage(
                string.Format("'I am tired, and yet, there is much left for me to take care of this day'"));
        ...
        {
            var Add1 = new GuardianEarthmender();
            Add1.X = 40020;
            Add1.Y = 62401;
            Add1.Z = 11676;
        ...
        {
            var Add2 = new MagicalEarthmender();
            Add2.X = 39459;
            Add2.Y = 62412;
            Add2.Z = 11688;
        ...
        {
            var Add3 = new NaturalEarthmender();
            Add3.X = 39552;
            Add3.Y = 62929;
            Add3.Z = 11690;
        ...
        {
            var Add4 = new ShadowyEarthmender();
            Add4.X = 39965;
            Add4.Y = 62921;
            Add4.Z = 11662;
...
namespace DOL.GS
{
    public class Olcasgean : GameEpicBoss
    {
        ...
        public override int MaxHealth => 250000;
        ...
        public override void TakeDamage(GameObject source, eDamageType damageType, int damageAmount, int criticalAmount)
        {
            ...
            var damageDealt = damageAmount + criticalAmount;
        ...
        public override bool AddToWorld()
        {
            ...
            foreach (GameNPC npc in GetNPCsInRadius(5500))
                if (npc != null && npc.IsAlive)
                    if (npc.Brain is WaterPrimalBrain || npc.Brain is AirPrimalBrain || npc.Brain is FirePrimalBrain ||
                        npc.Brain is EarthPrimalBrain
                        || npc.Brain is GuardianEarthmenderBrain || npc.Brain is MagicalEarthmenderBrain ||
                        npc.Brain is NaturalEarthmenderBrain || npc.Brain is ShadowyEarthmenderBrain ||
                        npc.Brain is OlcasgeanBrain2)
                        npc.RemoveFromWorld();

            Faction = FactionMgr.GetFactionByID(96);
            Faction.AddFriendFaction(FactionMgr.GetFactionByID(96));
            var sBrain = new OlcasgeanBrain();
            SetOwnBrain(sBrain);
            base.AddToWorld();
            return true;
        }
...
namespace DOL.AI.Brain
{
    public class OlcasgeanBrain : StandardMobBrain
    {
        ...
        private List<GamePlayer> player_in_range;
        private List<GamePlayer> player_in_range2;
        private List<GamePlayer> player_to_port;
        private List<GamePlayer> ported_player;

        public void SpawnAntiPass()
        {
            var Add = new WaterfallAntipass();
            Add.X = 39670;
            Add.Y = 60649;
            Add.Z = 12013;
            Add.CurrentRegion = Body.CurrentRegion;
        ...
        public int SpawnEffects(ECSGameTimer timer)
        {
            ...
            {
                var spot = new Point3D(39526, 62755, 11690);
                for (var i = 0; i < Util.Random(8, 15); i++)
                {
                    var Add = new OlcasgeanEffect();
        ...
        public override void Think()
        {
            ...
            if (Body.IsAlive)
            {
                ...
                point1.X = 40170;
                point1.Y = 62600;
                point1.Z = 11681; //location where players need to stay to avoid Olcasgean spamming dd spell
                ...
                if (HasAggro && Body.TargetObject != null) //Boss in combat
                {
                    ...
                    if (spawn_antipass ==
                        false) //spawn anti pass near waterfall so players cant leave boss area until killed 
                    {
                        SpawnAntiPass();
                        spawn_antipass = true;
                    }
                    ...
                    {
                        var port = (GamePlayer) player_to_port[Util.Random(0, player_to_port.Count) - 1];
                        TeleportTarget = port;

                        if (TeleportTarget != null && TeleportTarget.IsAlive && !teleport_player &&
                            Body.HealthPercent <= 50)
                        {
                            new ECSGameTimer(Body, new ECSGameTimer.ECSTimerCallback(DoPort),
                                Util.Random(12000, 20000)); //do teleport every 12-20s
                    ...
                    switch (random)
                    {
                        case 1:
                        {
                            if (TeleportTarget.IsAlive && TeleportTarget != null &&
                                !ported_player.Contains(TeleportTarget))
                            {
                                TeleportTarget.MoveTo(Body.CurrentRegionID, 38399, 60893, 12242, 3548);
                                TeleportTarget.Client.Out.SendMessage(Body.Name + " throws you away...",
...
namespace DOL.GS
{
    public class Olcasgean2 : GameEpicBoss
    {
        ...
        public override bool AddToWorld()
        {
            ...
            Faction = FactionMgr.GetFactionByID(96);
            Faction.AddFriendFaction(FactionMgr.GetFactionByID(96));
            var sBrain = new OlcasgeanBrain2();
            SetOwnBrain(sBrain);
            base.AddToWorld();
            return true;
        }
    ...
    public class AirPrimal : GameEpicBoss
    {
        ...
        public override void TakeDamage(GameObject source, eDamageType damageType, int damageAmount, int criticalAmount)
        {
            if (source is GamePet || source is TurretPet)
                base.TakeDamage(source, damageType, 5,
                    5); //pets deal less dmg to this primal to avoid being killed to fast
            else //take dmg
                base.TakeDamage(source, damageType, damageAmount, criticalAmount);
        }
        ...
        public override int MaxHealth => 900; //low health, as source says 1 volcanic pillar 5 could one shot it
...
namespace DOL.AI.Brain
{
    public class AirPrimalBrain : StandardMobBrain
    {
        ...
        private List<GamePlayer> inRangeLiving;
        ...
        public void PickRandomTarget()
        {
            ...
            {
                var damage_enemies = new List<GameLiving>();
                for (var i = 0; i < enemies.Count; i++)
                {
                    ...
                    if (living.IsVisibleTo(Body) && Body.TargetInView) damage_enemies.Add(enemies[i] as GameLiving);
                }
        ...
        public override void Think()
        {
            ...
            if (Body.InCombatInLast(20 * 1000) == false && Body.InCombatInLast(25 * 1000)) Body.Health = Body.MaxHealth;
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            {
                                path4 = true;
                                if (!Body.IsWithinRadius(point5, 20) && path1 == true && path2 == true &&
                                ...
                                {
                                    path5 = true;
                                    if (!Body.IsWithinRadius(point6, 20) && path1 == true && path2 == true &&
                                        path3 == true && path4 == true && path5 == true
                                        && path6 == false)
                                    {
                                    ...
                                    {
                                        path6 = true;
                                        if (!Body.IsWithinRadius(point7, 20) && path1 == true && path2 == true &&
                                            path3 == true && path4 == true && path5 == true
                                        ...
                                        {
                                            path7 = true;
                                            if (!Body.IsWithinRadius(point8, 20) && path1 == true && path2 == true &&
                                                path3 == true && path4 == true && path5 == true
                                            ...
                                            {
                                                path8 = true;
                                                if (!Body.IsWithinRadius(point9, 20) && path1 == true &&
                                                    path2 == true && path3 == true && path4 == true && path5 == true
                                                    && path6 == true && path7 == true && path8 == true &&
                                                    path9 == false)
                                                {
                                                ...
                                                {
                                                    path9 = true;
                                                    if (!Body.IsWithinRadius(point10, 20) && path1 == true &&
                                                        path2 == true && path3 == true && path4 == true && path5 == true
                                                        && path6 == true && path7 == true && path8 == true &&
                                                        path9 == true && path10 == false)
                                                    {
                                                    ...
                                                    {
                                                        path10 = true;
                                                        if (!Body.IsWithinRadius(point11, 20) && path1 == true &&
                                                            path2 == true && path3 == true && path4 == true &&
                                                            path5 == true
                                                            && path6 == true && path7 == true && path8 == true &&
                                                            path9 == true && path10 == true && path11 == false)
                                                        {
...
namespace DOL.GS
{
    public class WaterPrimal : GameEpicBoss
    {
        ...
        public override int MaxHealth => 125000;
            ...
            if (source is GamePlayer || source is GamePet)
            {
                if (WaterPrimalBrain.dontattack) //dont take any dmg 
                {
                        ...
                        || damageType == eDamageType.Slash)
                    {
                        ...
                        if (truc != null)
                            truc.Out.SendMessage(Name + " is under waterfall effect!", eChatType.CT_System,
        ...
        public override bool AddToWorld()
        {
            ...
            Faction = FactionMgr.GetFactionByID(96);
            Faction.AddFriendFaction(FactionMgr.GetFactionByID(96));
            var sBrain = new WaterPrimalBrain();
            SetOwnBrain(sBrain);
            Brain.Start();
            base.AddToWorld();
            return true;
...
namespace DOL.AI.Brain
{
    public class WaterPrimalBrain : StandardMobBrain
    {
        ...
        public override void AttackMostWanted()
        {
            ...
            {
                if (ECS.Debug.Diagnostics.AggroDebugEnabled) PrintAggroTable();
        ...
        public int TeleportPlayer(ECSGameTimer timer)
        {
            if (TeleportTarget.IsAlive && TeleportTarget != null && HasAggro)
            {
                ...
                {
                    case 1:
                        TeleportTarget.MoveTo(Body.CurrentRegionID, 38626, 60891, 11771, 2881);
                        break;
                    case 2:
                        TeleportTarget.MoveTo(Body.CurrentRegionID, 40606, 60868, 11721, 1095);
                        break;
                }
...
namespace DOL.GS
{
    public class FirePrimal : GameEpicBoss
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var sBrain = new FirePrimalBrain();
            SetOwnBrain(sBrain);
            Brain.Start();
            base.AddToWorld();
            return true;
...
namespace DOL.AI.Brain
{
    public class FirePrimalBrain : StandardMobBrain
    {
        ...
        public override void Think()
        {
            ...
            if (Body.IsAlive)
            {
                Body.CastSpell(FireDS,
                    SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells)); //cast dmg shield every 60s
                foreach (GamePlayer player in Body.GetPlayersInRadius(2500))
                    if (player != null)
                        if (player.IsAlive && player.Client.Account.PrivLevel == 1)
                            if (!AggroTable.ContainsKey(player))
...
namespace DOL.GS
{
    public class TrailOfFire : GameNPC
    {
        ...
        private int Show_Effect(ECSGameTimer timer)
        {
            if (IsAlive)
            {
                Parallel.ForEach(GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE).OfType<GamePlayer>(),
                    player => { player?.Out.SendSpellEffectAnimation(this, this, 5906, 0, false, 0x01); });
        ...
        public override bool AddToWorld()
        {
            ...
            var sBrain = new TrailOfFireBrain();
            SetOwnBrain(sBrain);
            //Brain.Start();
    ...
    public class EarthPrimal : GameEpicBoss
    {
        ...
        public override void Die(GameObject killer)
        {
            ...
            foreach (GameNPC npc in GetNPCsInRadius(8000))
                if (npc != null)
                    if (npc.IsAlive)
                        if (npc.Brain is GuardianEarthmenderBrain || npc.Brain is MagicalEarthmenderBrain ||
                            npc.Brain is NaturalEarthmenderBrain || npc.Brain is ShadowyEarthmenderBrain)
                            npc.Die(null);
        ...
        public override bool AddToWorld()
        {
            ...
            var sBrain = new EarthPrimalBrain();
            SetOwnBrain(sBrain);
            Brain.Start();
            base.AddToWorld();
            return true;
...
namespace DOL.AI.Brain
{
    public class EarthPrimalBrain : StandardMobBrain
    {
        ...
        public int TargetIsOut(ECSGameTimer timer)
        {
            if (Body.IsAlive)
                if (HasAggro && Body.TargetObject != null)
                {
                    ...
                    if (!target.IsWithinRadius(spawn, 900) && AggroTable.ContainsKey(target) && target != null &&
                        target.IsAlive)
                    {
        ...
        public override void Think()
        {
            ...
            if (Body.IsOutOfTetherRange && !HasAggro) FSM.SetCurrentState(eFSMStateType.RETURN_TO_SPAWN);
    ...
    public class GuardianEarthmender : GameNPC
    {
        ...
        public override void TakeDamage(GameObject source, eDamageType damageType, int damageAmount, int criticalAmount)
        {
            if (source is GamePlayer)
            {
                ...
                if (truc.CharacterClass.ID == 43 || truc.CharacterClass.ID == 44 || truc.CharacterClass.ID == 45 ||
                    truc.CharacterClass.ID == 56 || truc.CharacterClass.ID == 55) // bm,hero,champ,vw,ani
                {
                    if (source is GamePlayer) base.TakeDamage(source, damageType, damageAmount, criticalAmount);
                }
                else
                {
            ...
            if (source is GamePet) base.TakeDamage(source, damageType, damageAmount, criticalAmount);
        }
        ...
        public override bool AddToWorld()
        {
            ...
            var sBrain = new GuardianEarthmenderBrain();
            SetOwnBrain(sBrain);
...
namespace DOL.AI.Brain
{
    public class GuardianEarthmenderBrain : StandardMobBrain
    {
        ...
        private List<GameNPC> inRangeLiving;
        ...
        public override void Think()
        {
            ...
            if (Body.IsAlive)
                foreach (GameNPC npc in Body.GetNPCsInRadius(1000))
                    if (npc != null)
                        if (npc.Brain is NaturalEarthmenderBrain || npc.Brain is MagicalEarthmenderBrain ||
                            npc.Brain is ShadowyEarthmenderBrain || npc.Brain is EarthPrimalBrain)
                            if (npc.IsAlive && npc.HealthPercent < 100)
                            {
                                ...
                                if (!Body.IsCasting)
                                    Body.CastSpell(EarthmenderHeal,
    ...
    public class MagicalEarthmender : GameNPC
    {
        ...
        public override void TakeDamage(GameObject source, eDamageType damageType, int damageAmount, int criticalAmount)
        {
            if (source is GamePlayer)
            {
                ...
                if (truc.CharacterClass.ID == 40 || truc.CharacterClass.ID == 41 || truc.CharacterClass.ID == 42 ||
                    truc.CharacterClass.ID == 56 || truc.CharacterClass.ID == 55) // eld,ench,menta,vw,ani
                {
                    if (source is GamePlayer) base.TakeDamage(source, damageType, damageAmount, criticalAmount);
                }
                else
        ...
        public override bool AddToWorld()
        {
            ...
            var sBrain = new MagicalEarthmenderBrain();
            SetOwnBrain(sBrain);
            sBrain.AggroLevel = 100;
            sBrain.AggroRange = 500;
            Brain.Start();
    ...
    public class MagicalEarthmenderBrain : StandardMobBrain
    {
        ...
        public override void Think()
        {
            ...
            if (Body.IsAlive)
                foreach (GameNPC npc in Body.GetNPCsInRadius(5000))
                    if (npc != null)
                        if (npc.Brain is GuardianEarthmenderBrain || npc.Brain is NaturalEarthmenderBrain ||
                            npc.Brain is ShadowyEarthmenderBrain || npc.Brain is EarthPrimalBrain)
                            if (npc.IsAlive && npc.HealthPercent < 100)
                            {
                                /* if (!inRangeLiving.Contains(npc))
    ...
    public class NaturalEarthmender : GameNPC
    {
        ...
        public override void TakeDamage(GameObject source, eDamageType damageType, int damageAmount, int criticalAmount)
        {
            if (source is GamePlayer)
            {
                ...
                if (truc.CharacterClass.ID == 48 || truc.CharacterClass.ID == 47 || truc.CharacterClass.ID == 46 ||
                    truc.CharacterClass.ID == 56 || truc.CharacterClass.ID == 55) // bard,druid,warden,ani,vw
                {
                    if (source is GamePlayer) base.TakeDamage(source, damageType, damageAmount, criticalAmount);
                }
                else
        ...
        public override bool AddToWorld()
        {
            ...
            var sBrain = new NaturalEarthmenderBrain();
            SetOwnBrain(sBrain);
            sBrain.AggroLevel = 100;
            sBrain.AggroRange = 500;
            Brain.Start();
    ...
    public class ShadowyEarthmender : GameNPC
    {
        ...
        public override void TakeDamage(GameObject source, eDamageType damageType, int damageAmount, int criticalAmount)
        {
            if (source is GamePlayer)
            {
                ...
                if (truc.CharacterClass.ID == 49 || truc.CharacterClass.ID == 50 || truc.CharacterClass.ID == 56 ||
                    truc.CharacterClass.ID == 55) // ns,ranger,vw,ani
                {
                    if (source is GamePlayer) base.TakeDamage(source, damageType, damageAmount, criticalAmount);
                }
                else
        ...
        public override bool AddToWorld()
        {
            ...
            var sBrain = new ShadowyEarthmenderBrain();
            SetOwnBrain(sBrain);
            sBrain.AggroLevel = 100;
            sBrain.AggroRange = 500;
            Brain.Start();
        ...
        public override void Think()
        {
            ...
            if (Body.IsAlive)
                foreach (GameNPC npc in Body.GetNPCsInRadius(5000))
                    if (npc != null)
                        if (npc.Brain is GuardianEarthmenderBrain || npc.Brain is MagicalEarthmenderBrain ||
                            npc.Brain is NaturalEarthmenderBrain || npc.Brain is EarthPrimalBrain)
                            if (npc.IsAlive && npc.HealthPercent < 100)
                            {
                                /* if (!inRangeLiving.Contains(npc))
                                 {
...
namespace DOL.GS
{
    public class Vortex : GameNPC
    {
        ...
        public override double AttackDamage(InventoryItem weapon)
        {
            return base.AttackDamage(weapon) * Strength / 250 * Properties.EPICS_DMG_MULTIPLIER;
        }
...
namespace DOL.GS
{
    public class WaterfallAntipass : GameNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            BodyType = 8;
            Realm = eRealm.None;
            var adds = new WaterfallAntipassBrain();
            LoadedFromScript = true;
            SetOwnBrain(adds);
            base.AddToWorld();
            return true;
