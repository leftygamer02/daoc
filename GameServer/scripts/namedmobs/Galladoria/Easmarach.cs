                        ...
                        || damageType == eDamageType.Slash)
                    {
                        ...
                        if (truc != null)
                            truc.Out.SendMessage(
                                Name + " is too far away from waterfall and is immune to your damage!",
            ...
            foreach (GamePlayer player in GetPlayersInRadius(10000))
                if (player != null)
                    player.Out.SendMessage(
                        "With the death of the Easmarach, the current of the falls reduces significantly.",
...
namespace DOL.AI.Brain
{
    public class EasmarachBrain : StandardMobBrain
    {
        ...
        {
            var point1 = new Point3D(37811, 50342, 10958);
            ...
            if (Body.IsAlive)
            {
                var nopass = new Point3D(37653, 52843, 10758); //you shall not pass!
                    ...
                    if (player != null)
                        if (player.IsAlive && player.Client.Account.PrivLevel == 1)
                            if (player.IsWithinRadius(nopass, 1000))
                            {
                                ...
                                player.Out.SendMessage("The strong current of the waterfall pushes you behind",
                                    eChatType.CT_Broadcast, eChatLoc.CL_ChatWindow);
                            }
            }

            if (Body.InCombatInLast(65 * 1000) == false && Body.InCombatInLast(70 * 1000))
            {
