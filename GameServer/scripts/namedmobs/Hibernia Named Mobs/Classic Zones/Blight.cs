...
namespace DOL.GS
{
    public class Blight : GameEpicBoss
    {
        ...
        public override bool AddToWorld()
        {
            ...
            TetherRange = 2600;
            BroadcastMessage(
                "Bloody bones fly from all directions into a swirling cloud of gore in the air before you. The bones begin to join together forming a single giant skeleton.");

            RespawnInterval = -1;
            var sbrain = new BlightBrain();
            SetOwnBrain(sbrain);
        ...
        protected int Show_Effect(ECSGameTimer timer)
        {
            if (IsAlive)
            {
                Parallel.ForEach(GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE).OfType<GamePlayer>(),
                    player => { player?.Out.SendSpellEffectAnimation(this, this, 5117, 0, false, 0x01); });
        ...
        {
            var respawnTime = ServerProperties.Properties.SET_EPIC_GAME_ENCOUNTER_RESPAWNINTERVAL * 60000;
...
namespace DOL.AI.Brain
{
    public class BlightBrain : StandardMobBrain
    {
        ...
        {
            if (Body.IsAlive && canGrowth && Body.Size < 200) Body.Size += 5;
...
namespace DOL.GS
{
    public class FireBlight : GameNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            LoadedFromScript = true;
            var sbrain = new FireBlightBrain();
            SetOwnBrain(sbrain);
            var success = base.AddToWorld();
            if (success) new ECSGameTimer(this, new ECSGameTimer.ECSTimerCallback(Show_Effect), 500);
        ...
        private protected int Show_Effect(ECSGameTimer timer)
        {
            ...
            {
                Parallel.ForEach(GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE).OfType<GamePlayer>(),
                    player => { player?.Out.SendSpellEffectAnimation(this, this, 4216, 0, false, 0x01); });
...
namespace DOL.GS
{
    public class LateBlight : GameNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            LoadedFromScript = true;
            var sbrain = new LateBlightBrain();
            SetOwnBrain(sbrain);
            var success = base.AddToWorld();
            if (success) new ECSGameTimer(this, new ECSGameTimer.ECSTimerCallback(Show_Effect), 500);
...
namespace DOL.GS
{
    public class FleshBlight : GameNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            LoadedFromScript = true;
            var sbrain = new FleshBlightBrain();
            SetOwnBrain(sbrain);
            var success = base.AddToWorld();
            if (success) new ECSGameTimer(this, new ECSGameTimer.ECSTimerCallback(Show_Effect), 500);
...
namespace DOL.GS
{
    public class BlightController : GameNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new BlightControllerBrain();
            SetOwnBrain(sbrain);
            base.AddToWorld();
            return true;
        }
...
namespace DOL.AI.Brain
{
    public class BlightControllerBrain : APlayerVicinityBrain
    {
        ...
        public void SpawnBlight()
        {
            ...
            if (FleshBlight.FleshBlightCount == 2 && LateBlight.LateBlightCount == 4 &&
                FireBlight.FireBlightCount == 8 && !CreateBlight)
            {
                var boss = new Blight();
                boss.X = Body.X + Util.Random(-500, 500);
                boss.Y = Body.Y + Util.Random(-500, 500);
