...
namespace DOL.AI.Brain
{
    public class SuttungBrain : StandardMobBrain
    {
        ...
        {
            var point = new Point3D(31088, 53870, 11886);
            if (Body.IsAlive)
            {
                foreach (GamePlayer player in Body.GetPlayersInRadius(8000))
                    if (player != null && player.IsAlive && player.Client.Account.PrivLevel == 1 && !message2 &&
                        player.IsWithinRadius(point, 400))
                        message2 = true;
            ...
            if (HasAggro)
            {
                ...
                {
                    new ECSGameTimer(Body, new ECSGameTimer.ECSTimerCallback(BerserkerPhase),
                        Util.Random(20000, 35000));
                    IsBerserker = true;
                }
            }
        ...
        {
            BroadcastMessage(
                "An otherworldly howling sound suddenly becomes perceptible. The sound quickly grows louder but it is not accompanied by a word. Moments after it begins, the howling sound is gone, replace by the familiar noises of the slowly shifting glacier.");
            return 0;
        }
    ...
    public class HjalmarBrain : StandardMobBrain
    {
        ...
        public override void Think()
        {
            ...
            if (HasAggro && Body.TargetObject != null)
            {
                ...
                if (message2 == false)
                {
                    BroadcastMessage(Body.Name +
                                     " bellows 'I am amazed that you have made it this far! I'm afraid that your journey ends here with all of your death, however, I will show you no mercy!'");
                    BroadcastMessage(string.Format(
                        Body.Name +
        ...
        protected int Show_Effect(ECSGameTimer timer)
        {
            if (IsAlive)
            {
                Parallel.ForEach(GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE).OfType<GamePlayer>(),
                    player => { player?.Out.SendSpellEffectAnimation(this, this, 4323, 0, false, 0x01); });
                return 3000;
            }
...
namespace DOL.GS
{
    public class HjalmarSuttungController : GameNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new HjalmarSuttungControllerBrain();
            SetOwnBrain(sbrain);
            base.AddToWorld();
            return true;
        }
...
namespace DOL.AI.Brain
{
    public class HjalmarSuttungControllerBrain : APlayerVicinityBrain
    {
        ...
        public override void Think()
        {
            var respawn = GS.ServerProperties.Properties.SET_SI_EPIC_ENCOUNTER_RESPAWNINTERVAL * 60000;
            if (Body.IsAlive)
            {
