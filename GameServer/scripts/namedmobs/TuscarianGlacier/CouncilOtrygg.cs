    ...
    public class Otrygg : GameEpicBoss
    {
        ...
        public override void Die(GameObject killer)
        {
            foreach (GameNPC npc in GetNPCsInRadius(8000))
            {
                ...
                if (npc.Brain is OtryggAddBrain) npc.Die(this);
            }
...
namespace DOL.AI.Brain
{
    public class OtryggBrain : StandardMobBrain
    {
        ...
        public override void Think()
        {
            if (!CheckProximityAggro())
            {
                ...
                if (!RemoveAdds)
                {
                    foreach (GameNPC npc in Body.GetNPCsInRadius(8000))
                    {
                        if (npc == null) continue;
                        if (!npc.IsAlive) continue;
                        if (npc.Brain is OtryggAddBrain) npc.Die(Body);
                    }
            ...
            {
                RemoveAdds = false;
                if (OtryggAdd.PetsCount is < 8 and >= 0) SpawnPetsMore();
            }
        ...
        {
            for (var i = 0; i < 8; i++) // Spawn 8 pets
            {
                var Add = new OtryggAdd();
            ...
            if (HasAggro && Body.TargetObject != null)
                foreach (GameNPC npc in Body.GetNPCsInRadius(4000))
                {
                    ...
                    if (npc.Brain is OtryggAddBrain brain)
                    {
                        ...
                        if (!brain.HasAggro && Body != npc && Body.Brain != npc.Brain && target != null &&
                            target.IsAlive)
