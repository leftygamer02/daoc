    ...
    public class Fornfrusenen : GameEpicBoss
    {
        ...
        public override void Die(GameObject killer) //on kill generate orbs
        {
            ...
            BroadcastMessage(string.Format(
                "The frosty glows in {0}'s eyes abruptly blinks out. {0}'s form slowly fades into the ice. The shard swiftly evaporate leaving no trace of their corporeal existence behind!",
                Name));
            base.Die(killer);
        }
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new FornfrusenenBrain();
            SetOwnBrain(sbrain);
...
namespace DOL.AI.Brain
{
    public class FornfrusenenBrain : StandardMobBrain
    {
        ...
        public override void OnAttackedByEnemy(AttackData ad)
        {
            ...
            {
                BroadcastMessage(string.Format(
                    "{0} awakens from its peaceful slumber and emerges from this ice walls and hisses \"I know your name {1}, take a good look at your surroundings! Within this ice is where you'll be entombed for all eternity! Hahahahaha\"",
                    Body.Name, ad.Attacker.Name));
                SpamMessage = true;
            }
        ...
        {
            for (var i = 0; i < Util.Random(2, 3); i++)
            {
                var Add = new FornfrusenenShard();
...
namespace DOL.GS
{
    public class FornfrusenenShard : GameNPC
    {
        ...
        public override void TakeDamage(GameObject source, eDamageType damageType, int damageAmount, int criticalAmount)
        {
            var point = new Point3D(49617, 32874, 10859);
            if (source is GamePlayer || source is GamePet)
            {
