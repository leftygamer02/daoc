            ...
            if (Body.IsAlive)
            {
                ...
                else
                {
                    ...
                    if (damage_enemies.Count > 0)
                    {
                        ...
                        if (RandomTarget.IsVisibleTo(Body) && Body.TargetInView && RandomTarget != null &&
            ...
            if (HasAggro && Body.TargetObject != null)
            {
                if (message1 == false)
                    if (Body.TargetObject is GamePlayer && Body.TargetObject != null)
                    {
                        ...
                        {
                            BroadcastMessage(string.Format(Body.Name + " Impossible! An ugly " +
                                                           player.CharacterClass.Name +
                                                           " there? How could this be? Guthlac, we must defend our Queen and King!"));
                            message1 = true;
                        }
                    }
                ...
                if (!Body.IsCasting)
                {
                    ...
                    if (Util.Chance(20))
                        if (target != null && target.IsAlive &&
                            !target.effectListComponent.ContainsEffectForEffectType(eEffect.StrConDebuff))
                            Body.CastSpell(Icelord_SC_Debuff, SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells));

                    if (Util.Chance(20))
                        if (target != null && target.IsAlive &&
                            !target.effectListComponent.ContainsEffectForEffectType(eEffect.MeleeHasteDebuff))
...
namespace DOL.AI.Brain
{
    public class GuthlacBrain : StandardMobBrain
    {
        ...
        private List<GamePlayer> PlayersToDD = new();
        ...
        private List<GamePlayer> Enemys_To_Root = new();
        ...
        public int PickRandomTarget(ECSGameTimer timer)
        {
            if (HasAggro)
            {
                ...
                if (Enemys_To_Root.Count > 0)
                    ...
                    {
                        var Target =
                            (GamePlayer) Enemys_To_Root[
                                Util.Random(0, Enemys_To_Root.Count - 1)]; //pick random target from list
        ...
        public override void Think()
        {
            ...
            if (HasAggro && Body.TargetObject != null)
            {
                ...
                if (IsBombUp == false && FrozenBomb.FrozenBombCount == 0)
                {
                    ...
                    {
                        var ptarget = PlayersToDD[Util.Random(0, PlayersToDD.Count - 1)];
        ...
        protected int Show_Effect(ECSGameTimer timer)
        {
            if (IsAlive)
            {
                Parallel.ForEach(GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE).OfType<GamePlayer>(),
                    player => { player?.Out.SendSpellEffectAnimation(this, this, 177, 0, false, 0x01); });
        ...
        public override bool AddToWorld()
        {
            ...
            if (success)
            {
                ...
                new ECSGameTimer(this, new ECSGameTimer.ECSTimerCallback(Explode),
                    25000); //25 seconds until this will explode and deal heavy cold dmg
            }
