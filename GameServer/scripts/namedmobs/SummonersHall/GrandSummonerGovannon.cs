    ...
    public class GrandSummonerGovannon : GameEpicBoss
    {
        ...
        public override void Die(GameObject killer)
        {
            foreach (var npc in WorldMgr.GetNPCsFromRegion(CurrentRegionID))
                if (npc != null)
                    if (npc.IsAlive && (npc.Brain is SummonedDemonBrain || npc.Brain is SummonedSacrificeBrain ||
                                        npc.Brain is ShadeOfAelfgarBrain))
                        npc.RemoveFromWorld();
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new GrandSummonerGovannonBrain();
            SetOwnBrain(sbrain);
        ...
        public static void ScriptLoaded(DOLEvent e, object sender, EventArgs args)
        {
            ...
            if (npcs.Length == 0)
            {
                ...
                var OF = new GrandSummonerGovannon();
                ...
                var ubrain = new GrandSummonerGovannonBrain();
            ...
            {
                log.Warn(
                    "Grand Summoner Govannon exist ingame, remove it and restart server if you want to add by script code.");
            }
        }
...
namespace DOL.AI.Brain
{
    public class GrandSummonerGovannonBrain : StandardMobBrain
    {
            ...
            if (!CheckProximityAggro())
            {
                ...
                if (!RemoveAdds)
                {
                    foreach (var npc in WorldMgr.GetNPCsFromRegion(Body.CurrentRegionID))
                        if (npc != null)
                            if (npc.IsAlive && (npc.Brain is SummonedDemonBrain ||
                                                npc.Brain is SummonedSacrificeBrain ||
                                                npc.Brain is ShadeOfAelfgarBrain))
                                npc.RemoveFromWorld();
        ...
        {
            var Add1 = new SummonedSacrifice();
            Add1.X = 31018;
            Add1.Y = 40889;
            Add1.Z = 15491;
        ...
        {
            if (SummonedSacrifice.SacrificeKilledCount == 1 &&
                SummonedDemon.SummonedDemonCount == 1) //both summoned demon and sacrifice must be killed
                if (ShadeOfAelfgar.ShadeOfAelfgarCount == 0) //make sure there is only 1 always
                {
                    var Add1 = new ShadeOfAelfgar();
                    Add1.X = 32128;
                    Add1.Y = 41667;
                    Add1.Z = 15491;
                    Add1.CurrentRegionID = 248;
...
namespace DOL.GS
{
    ...
    {
        public override int MaxHealth => 6000;
        ...
        public override bool AddToWorld()
        {
            ...
            Faction.AddFriendFaction(FactionMgr.GetFactionByID(187));
            var sacrifice = new SummonedSacrificeBrain();
...
namespace DOL.AI.Brain
{
    public class SummonedSacrificeBrain : StandardMobBrain
    {
        ...
        public override void Think()
        {
            var point1 = new Point3D(32063, 40896, 15468);
...
namespace DOL.GS
{
    public class SummonedDemon : GameNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            Faction = FactionMgr.GetFactionByID(187);
            Faction.AddFriendFaction(FactionMgr.GetFactionByID(187));
            var sacrifice = new SummonedDemonBrain();
            SetOwnBrain(sacrifice);
            base.AddToWorld();
            return true;
        }
...
namespace DOL.GS
{
    public class ShadeOfAelfgar : GameEpicNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            Faction = FactionMgr.GetFactionByID(187);
            Faction.AddFriendFaction(FactionMgr.GetFactionByID(187));
            var sacrifice = new ShadeOfAelfgarBrain();
            SetOwnBrain(sacrifice);
            base.AddToWorld();
            return true;
        }
    ...
    public class ShadeOfAelfgarBrain : StandardMobBrain
    {
        ...
        public void PickRandomTarget()
        {
            ...
            if (Enemys_To_Port.Count > 0)
                ...
                {
                    var Target =
                        (GamePlayer) Enemys_To_Port[
                            Util.Random(0, Enemys_To_Port.Count - 1)]; //pick random target from list
                    RandomTarget = Target; //set random target to static RandomTarget
                    RandomTarget.MoveTo(Body.CurrentRegionID, 32091, 39684, 16302, 4094);
                    new ECSGameTimer(Body, new ECSGameTimer.ECSTimerCallback(ResetPort),
                        Util.Random(10000, 20000)); //port every 10-20s
                    CanPort = true;
                }
        }
