...
namespace DOL.GS
{
    public class SummonerRoesia : GameEpicBoss
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new SummonerRoesiaBrain();
            SetOwnBrain(sbrain);
        ...
        public static void ScriptLoaded(DOLEvent e, object sender, EventArgs args)
        {
            ...
            if (npcs.Length == 0)
            {
                ...
                var ubrain = new SummonerRoesiaBrain();
            ...
            {
                log.Warn(
                    "Summoner Roesia exist ingame, remove it and restart server if you want to add by script code.");
            }
        }
    }
}
                    ...
                    if (Util.Chance(35))
                        foreach (Spell spells in Body.Spells)
                            if (spells != null)
                            {
                                ...
                                if (Body.IsMoving &&
                                    Body.TargetObject.IsWithinRadius(Body.TargetObject, spells.Range) && Body.IsCasting)
                                    Body.StopFollowing();
