...
namespace DOL.GS
{
    public class SummonerLossren : GameEpicBoss
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var sbrain = new SummonerLossrenBrain();
            SetOwnBrain(sbrain);
        ...
        public static void ScriptLoaded(DOLEvent e, object sender, EventArgs args)
        {
            ...
            if (npcs.Length == 0)
            {
                ...
                var ubrain = new SummonerLossrenBrain();
            ...
            {
                log.Warn(
                    "Summoner Lossren exist ingame, remove it and restart server if you want to add by script code.");
            }
        }
    }
}
...
namespace DOL.AI.Brain
{
    public class SummonerLossrenBrain : StandardMobBrain
    {
            ...
            if (!CheckProximityAggro())
            {
                ...
                if (!RemoveAdds)
                {
                    foreach (GameNPC souls in Body.GetNPCsInRadius(5000))
                        if (souls != null)
                            if (souls.IsAlive &&
                                (souls.Brain is TorturedSoulsBrain || souls.Brain is ExplodeUndeadBrain))
            ...
            if (Body.InCombat && Body.IsAlive && HasAggro && Body.TargetObject != null)
            {
                ...
                if (IsCreatingSouls == false)
                {
                    new ECSGameTimer(Body, new ECSGameTimer.ECSTimerCallback(DoSpawn),
                        Util.Random(5000, 8000)); //every 5-8s it will spawn tortured souls
        ...
        {
            var point1 = new Point3D(39189, 41889, 16000);
            var point2 = new Point3D(38505, 41211, 16001);
            var point3 = new Point3D(39180, 40583, 16000);
            var point4 = new Point3D(39745, 41176, 16001);

            for (var i = 0; i < Util.Random(18, 25); i++) //create 18-25 souls every time timer will launch
            {
                var add = new TorturedSouls();
...
namespace DOL.AI.Brain
{
    public class TorturedSoulsBrain : StandardMobBrain
    {
        ...
        public override void Think()
        {
            if (!CheckProximityAggro()) FSM.SetCurrentState(eFSMStateType.RETURN_TO_SPAWN);
...
namespace DOL.GS
{
    public class TorturedSouls : GameNPC
    {
        ...
        private List<string> soul_names = new()
        {
        ...
        public override bool AddToWorld()
        {
            ...
            Faction.AddFriendFaction(FactionMgr.GetFactionByID(187));
            var souls = new TorturedSoulsBrain();
            SetOwnBrain(souls);
            base.AddToWorld();
            return true;
        }
...
namespace DOL.GS
{
    ...
    {
        public override int MaxHealth => 4000;
        ...
        private List<GamePlayer> Zombie_Targets = new();
        ...
        public override bool AddToWorld()
        {
            ...
            Faction = FactionMgr.GetFactionByID(187);
            Faction.AddFriendFaction(FactionMgr.GetFactionByID(187));
            var souls = new ExplodeUndeadBrain();
            SetOwnBrain(souls);
            ...
            if (success)
            {
                ...
                {
                    var Target = (GamePlayer) Zombie_Targets[Util.Random(0, Zombie_Targets.Count - 1)];
                    RandomTarget = Target;
                    BroadcastMessage(string.Format(Name + " crawls toward " + RandomTarget.Name + "!"));
                }
            }
