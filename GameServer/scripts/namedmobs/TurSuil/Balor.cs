...
namespace DOL.AI.Brain
{
    public class BalorBrain : StandardMobBrain
    {
        ...
        public override void Think()
        {
            ...
            {
                var health = Body.HealthPercent / 10;
                if (health < Stage)
                {
                    switch (health)
                    {
    ...
    public class BalorEyeBrain : StandardMobBrain
    {
        ...
        public override void Think()
        {
            if (Body.IsAlive)
            {
                ...
                if (HasAggro && Cancast &&
                    Body.TargetObject != null) //&& RandomTarget != null && RandomTarget.IsAlive && !Body.IsCasting)
                {
        ...
        public void PickRandomTarget()
        {
            ...
            {
                var Target = Enemys_To_DD[Util.Random(0, Enemys_To_DD.Count - 1)]; //pick random target from list
...
namespace DOL.GS
{
    public class BalorEye : GameNPC
    {
        ...
        public override bool AddToWorld()
        {
            ...
            var eye = new BalorEyeBrain();
            SetOwnBrain(eye);
            eye.Start();
            var success = base.AddToWorld();
            if (success)
                new ECSGameTimer(this, new ECSGameTimer.ECSTimerCallback(RemoveEye),
                    18200); //mob will be removed after this time
