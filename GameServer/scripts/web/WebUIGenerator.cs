...
public class WebUIGenerator
{
    ...
    {
        var dirs = Directory.GetDirectories(parent.m_path);
        var iter = dirs.GetEnumerator();
        while (iter.MoveNext())
        {
            var dir = new WebUIDir();
    ...
    private static void CopyFromTemplate(WebUIDir dir)
    {
        var path =
            dir.m_path.Replace(
                "." + Path.DirectorySeparatorChar + "webui" + Path.DirectorySeparatorChar + "template",
                "." + Path.DirectorySeparatorChar + "webui" + Path.DirectorySeparatorChar + "generated");

        if (!Directory.Exists(path)) Directory.CreateDirectory(path);

        foreach (WebUIDir d in dir.m_dirs) CopyFromTemplate(d);

        foreach (var s in dir.m_files)
        {
            var fi = new FileInfo(s);
            var fpath =
                s.Replace("." + Path.DirectorySeparatorChar + "webui" + Path.DirectorySeparatorChar + "template",
                    "." + Path.DirectorySeparatorChar + "webui" + Path.DirectorySeparatorChar + "generated");
    ...
    {
        var path =
            fname.Replace(Path.DirectorySeparatorChar + "webui" + Path.DirectorySeparatorChar + "template",
                Path.DirectorySeparatorChar + "webui" + Path.DirectorySeparatorChar + "generated");
        var buf = "";

        using (var rdr = File.OpenText(fname))
        {
            buf = rdr.ReadToEnd();
    ...
    private static void InitJS()
    {
        ...
        var nl = new StreamWriter(new MemoryStream());
        ...
        m_js.AppendFormat("var aac = \"{0}\"",
            GameServer.Instance.Configuration.AutoAccountCreation ? "enabled" : "disabled");
        m_js.Append(nl.NewLine);
