...
public class OnlineStatusUpdate
{
    ...
    protected static string UpdateUrl =
        UrlEncode("http://portal.dolserver.net/serverlist.php?action=submit&username=" + UserName + "&password=" +
                  Password + "&totalclients=" + ClientCount + "&version=" + ScriptVersion);
    ...
    {
        if (ServerProperties.Properties.SERVERLISTUPDATE_ENABLED) Init();
    }
    ...
    public static bool Init()
    {
        ...
        if (args != null)
        {
            ...
            {
                log.Error(
                    "Login/URL Error: Validating Arguments Failed! The server's player count will not be added/updated on the list!");
    ...
    private static bool ListUpdater(string updateurl)
    {
        try
        {
            ...
            var contentBuffer = webclient.DownloadData(updateurl);
            var result = Encoding.ASCII.GetString(contentBuffer).ToLower();
