...
public class SummonSiegeRam : SpellHandler
{
    ...
    public override bool StartSpell(GameLiving target)
    {
        ...
        {
            player.Out.SendMessage("You are too far away to attack " + Caster.TargetObject.Name,
        ...
        {
            player.Out.SendMessage("You are too close to attack " + Caster.TargetObject.Name,
                eChatType.CT_SpellResisted, eChatLoc.CL_SystemWindow);
            return false;
        }

        //Limit 2 Rams in a certain radius
        var ramSummonRadius = 200;
        var ramsInRadius = 0;
        foreach (GameNPC npc in Caster.CurrentRegion.GetNPCsInRadius(Caster.X, Caster.Y, Caster.Z,
                     (ushort) ramSummonRadius, false, false))
            if (npc is GameSiegeRam ram && ram.Realm == Caster.Realm)
                ramsInRadius++;

        if (ramsInRadius >= 2)
        {
            MessageToCaster("Too many rams in this area and you cannot summon another ram here!",
