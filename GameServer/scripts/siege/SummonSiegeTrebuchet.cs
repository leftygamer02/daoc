...
public class SummonSiegeTrebuchet : SpellHandler
{
    ...
    public override bool StartSpell(GameLiving target)
    {
        ...
        foreach (var area1 in currentAreas)
        {
            ...
            {
                MessageToCaster("You cannot use siege weapons here (PK)!",
                    PacketHandler.eChatType.CT_SpellResisted);
                return false;
            }
        }

        //Only allow one treb/catapult in the radius
        var trebSummonRadius = 500;
        foreach (GameNPC npc in Caster.CurrentRegion.GetNPCsInRadius(Caster.X, Caster.Y, Caster.Z,
                     (ushort) trebSummonRadius, false, false))
            if (npc is GameSiegeCatapult)
            {
                MessageToCaster("You are too close to another trebuchet or catapult and cannot summon here!",
                    PacketHandler.eChatType.CT_SpellResisted);
                return false;
            }
    ...
    public override void ApplyEffectOnTarget(GameLiving target, double effectiveness)
    {
        ...
        var tre = new GameSiegeTrebuchet();
        tre.X = Caster.X;
        tre.Y = Caster.Y;
        tre.Z = Caster.Z;
