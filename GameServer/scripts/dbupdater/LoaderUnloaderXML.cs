    ...
    public static IList<Type> GetAllDataTableTypes()
    {
        ...
        try
        {
            ...
            foreach (var type in asm.GetTypes())
            {
                ...
                // Don't Keep Views...
                if (type.GetCustomAttributes<DataTable>(false).Any() &&
                    AttributesUtils.GetViewName(type) == null)
                    result.Add(type);
            }
        }
        catch (Exception ex)
    ...
    public static void UnloadXMLTable(Type t, string directory)
    {
        if (t == null || !typeof(DataObject).IsAssignableFrom(t))
            if (log.IsInfoEnabled)
                log.InfoFormat("Null Type or Incompatible Type in UnloadXMLTable Call: {0}",
            ...
            if (attrib.Any())
            {
                ...
                try
                {
                    ...
                    if (remarkables.Length > 0)
                    {
                        ...
                        objects = (IEnumerable) toArrayGeneric.Invoke(null,
                            new object[] {castGeneric.Invoke(null, new object[] {firstOrder})});
                    }
                }
                catch (Exception oe)
                {
                    if (log.IsWarnEnabled)
                        log.WarnFormat("Error while sorting Table {0} for XML Unload, Probably Unsorted... - {1}",
                            tableName, oe);
                }
