...
public class AutoXMLDatabaseUpdate : IDatabaseUpdater
{
    ...
    /// </summary>
    [ServerProperty("xmlautoload", "xml_autoload_update_enable",
        "Enable or disable Auto XML Dataase Update Packages (Should be enabled for first run...)", true)]
    public static bool XML_AUTOLOAD_UPDATE_ENABLE;

    /// <summary>
    /// Set Default Path for Loading "Insert" XML Package Directory
    /// </summary>
    [ServerProperty("xmlautoload", "xml_load_insert_directory",
        "Enforce directory path where the XML Insert Packages are Loaded To Database (Relative to Scripts or Absolute...)",
        "dbupdater/insert")]
    ...
    /// </summary>
    [ServerProperty("xmlautoload", "xml_load_replace_directory",
        "Enforce directory path to where the XML Replace Packages are Loaded To Database (Relative to Scripts or Absolute...)",
        "dbupdater/replace")]
    ...
    public void Update()
    {
        ...
        try
        {
            var insertdir = Path.IsPathRooted(XML_LOAD_INSERT_DIRECTORY)
                ? XML_LOAD_INSERT_DIRECTORY
                : string.Format("{0}{1}scripts{1}{2}", GameServer.Instance.Configuration.RootDirectory,
                    Path.DirectorySeparatorChar, XML_LOAD_INSERT_DIRECTORY);

            if (!Directory.Exists(insertdir))
                Directory.CreateDirectory(insertdir);

            var replacedir = Path.IsPathRooted(XML_LOAD_REPLACE_DIRECTORY)
                ? XML_LOAD_REPLACE_DIRECTORY
                : string.Format("{0}{1}scripts{1}{2}", GameServer.Instance.Configuration.RootDirectory,
                    Path.DirectorySeparatorChar, XML_LOAD_REPLACE_DIRECTORY);
    ...
    private void ApplyXMLChangeForDirectory(IEnumerable<AutoXMLUpdateRecord> records, string directory,
        bool replace)
    {
    ...
    private bool CheckXMLPackageAndApply(FileInfo xml, bool replace)
    {
        ...
        if (log.IsInfoEnabled)
            log.InfoFormat("Auto Loading XML File {0} into Database (Mode:{1})", packageName,
        ...
        try
        {
            ...
            if (xmlTable.Length > 0)
            {
                ...
                var existingObjects = ((IEnumerable) genericMethod.Invoke(GameServer.Database, new object[] { }))
                        ...
                        .FirstOrDefault(entry => uniqueMember
                            .Any(constraint => constraint
                                .All(bind => bind.ValueType == typeof(string)
                                    ? bind.GetValue(entry).ToString().Equals(bind.GetValue(obj).ToString(),
        ...
        {
            if (log.IsErrorEnabled)
                log.ErrorFormat("Error While Loading XML Package {0} into Database (Mode:{1}) - {2}", packageName,
                    replace ? "Replace" : "Insert", e);
            result = false;
        }
    ...
    /// <returns></returns>
    private AutoXMLUpdateRecord GetAutoXMLUpdateRecordFromCollection(IEnumerable<AutoXMLUpdateRecord> records,
        string relativeID, string hash)
    {
