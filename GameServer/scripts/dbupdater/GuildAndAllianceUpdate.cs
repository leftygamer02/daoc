...
public class GuildAndAllianceUpdate : IDatabaseUpdater
{
    ...
    public void Update()
    {
        ...
        var alliances = DOLDB<DBAlliance>.SelectObjects(DB.Column("LeaderGuildID").IsEqualTo(string.Empty)
            .Or(DB.Column("LeaderGuildID").IsNull()));

        if (alliances.Any())
        {
            var leadingGuilds = DOLDB<DBGuild>.MultipleSelectObjects(alliances.Select(al =>
                DB.Column("AllianceID").IsEqualTo(al.ObjectId)
                    .And(DB.Column("GuildName").IsEqualTo(al.AllianceName))));
            ...
            var alliancesWithLeader = leadingGuilds.Select((gd, i) =>
            {
                ...
                catch (Exception e)
                {
                    if (log.IsErrorEnabled)
                        log.ErrorFormat(
                            "Wrong records while trying to retrieve Guild Leader (AllianceID: {0}, AllianceName: {1})\n{2}",
                            al.ObjectId, al.AllianceName, e);
                }
            ...
            foreach (var pair in alliancesWithLeader)
                if (pair.Leader != null)
                    ...
                    log.WarnFormat("Alliance (ID:{0}, Name:{1}) can't resolve its Leading Guild !",
                        pair.Alliance.ObjectId, pair.Alliance.AllianceName);
