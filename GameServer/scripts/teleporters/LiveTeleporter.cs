...
public class LiveTeleporter : GameNPC
{
    ...
    /// </summary>
    protected virtual eRealm DestinationRealm => Realm;
    ...
    protected virtual bool GetTeleportLocation(GamePlayer player, string text)
    {
        switch (Realm) // Only offer locations based on what realm i am set at.
        {
            case eRealm.Albion:
                ...
                {
                    var reply = string.Format("The isles of Avalon are an excellent choice. {0} {1}",
            ...
            case eRealm.Midgard:
                ...
                if (text.ToLower() == "shrouded isles")
                {
                    var reply = string.Format("The isles of Aegir are an excellent choice. {0} {1}",
        ...
        var port = WorldMgr.GetTeleportLocation(DestinationRealm, string.Format("{0}:{1}", Type, text));
        if (port != null)
        {
    ...
    {
        var region = WorldMgr.GetRegion((ushort) destination.RegionID);
    ...
    protected virtual void OnTeleportSpell(GamePlayer player, Teleport destination)
    {
        ...
        var spell = SkillBase.GetSpellByID(5999); // UniPortal spell.

        if (spell != null)
        {
            TargetObject = player;
            var portalHandler = new UniPortal(this, spell, spellLine, destination);
