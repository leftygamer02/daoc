...
public class OFTeleporter : GameNPC
{
    ...
    public void StartTeleporting()
    {
        ...
        var cast = CastSpell(PortSpell, SkillBase.GetSpellLine(GlobalSpellsLines.Mob_Spells), false);
        if (GetSkillDisabledDuration(PortSpell) > 0)
            cast = false;

        if (Assistants == null) Assistants = new List<OFAssistant>();
        ...
        if (cast)
        {
            ...
            foreach (GamePlayer player in GetPlayersInRadius(500))
                player.Out.SendMessage(Name + " says, \"" + portMessage + "\"", eChatType.CT_Say,
            ...
            foreach (var assi in Assistants) assi.CastEffect();
        }
    }
    ...
    private int CastTimerCallback(ECSGameTimer selfRegenerationTimer)
    {
        if (selfRegenerationTimer == castTimer)
            foreach (var assi in Assistants)
                assi.CastEffect();
    ...
    public override void OnAfterSpellCastSequence(ISpellHandler handler)
    {
        ...
        foreach (var player in GetPlayersInRadius(500).OfType<GamePlayer>().ToList())
        {
                    ...
                    if (medallion != null)
                        switch (medallion.Id_nb)
                        {
                            ...
                            case CityID:
                                ...
                                break;
                            case KeepID:
                                if (!IsAllowedToKeepJump(player)) break;
                            ...
                            case BattlegroundsID:
                            {
                                ...
                                {
                                    if (player.RealmPoints >= 122500) break;
                            ...
                            case AlbionHousingEntID:
                                PortLocation = new GameLocation("Housing Entrance Alb", 2, 584736, 561341, 3576,
                                    2268);
                            ...
                            case PersonalHouseID:
                                ...
                                break;
                            case GuildHouseID:
                                var guildHouse = HouseMgr.GetGuildHouseByPlayer(player);
                                if (guildHouse == null)
                                    goto case AlbionHousingEntID; // Fall through, port to housing entrance.
                                else
                            ...
                            case HearthBindID:
                                ...
                                var houses = (ArrayList) HouseMgr.GetHousesCloseToSpot(
                    ...
                    if (medallion != null)
                        switch (medallion.Id_nb)
                        {
                            ...
                            case MidgardHousingEntID:
                                PortLocation = new GameLocation("Housing Entrance Mid", 102, 526776, 561737, 3634,
                                    3971);
                                break;
                            case PersonalHouseID:
                                var house = HouseMgr.GetHouseByPlayer(player);
                    ...
                    if (medallion != null)
                        switch (medallion.Id_nb)
                        {
                            ...
                            case HiberniaHousingEntID:
                                PortLocation = new GameLocation("Housing Entrance Hib", 202, 555117, 526463, 3012,
                                    1045);
                                break;
                            case PersonalHouseID:
                                var house = HouseMgr.GetHouseByPlayer(player);
...
public class MainTeleporterBrain : StandardMobBrain
{
    ...
    {
        var teleporter = Body as OFTeleporter;
