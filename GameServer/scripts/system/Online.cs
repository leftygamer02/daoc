...
public class OnlineCommandHandler : AbstractCommandHandler, ICommandHandler
{
    ...
    private List<ClassToCount> classcount = new();


    public void OnCommand(GameClient client, string[] args)
    {
        var textList = GetOnlineInfo(client.Account.PrivLevel >= (uint) ePrivLevel.GM);
    ...
    public IList<string> GetOnlineInfo(bool bGM)
    {
        ...
        var entering = connecting + enterworld + charscreen;
        var leaving = disconnecting + linkdeath;
        var albTotal = albTanks + albCasters + albSupport + albStealthers;
        var midTotal = midTanks + midCasters + midSupport + midStealthers;
        var hibTotal = hibTanks + hibCasters + hibSupport + hibStealthers;
        var total = entering + playing + leaving;
        ...
        if (zoneIDs.Length > 0 && showByZone)
            for (var r = 0; r < zoneIDs.Length; r++)
            {
                albsinregion = 0;
                midsinregion = 0;
                hibsinregion = 0;
                ...
                output.Add(string.Format("\nPlayers in {0}: {1} \n Albs: {2} | Mids: {3} | Hibs: {4}",
                    zone.Description, totalinregion, albsinregion, midsinregion, hibsinregion));
            }

        if (showZoneBreakzone)
        {
            var zoneXnumbers = new Dictionary<string, int>();
            foreach (var c in clients)
            {
                if (c == null || c.Player == null || c.Player.CurrentZone == null ||
                    c.Player.CurrentZone.Description == null || (c.Account.PrivLevel > 1 && c.Player.IsAnonymous))
                    continue;
            ...
            output.Add(string.Format("\n"));
            foreach (var kvp in sortedZones) output.Add(kvp.Value + " players in " + kvp.Key);
        }
        ...
        if (showDetailedClass)
        {
            ...
            lock (classcount)
            {
                classcount.Sort(delegate(ClassToCount ctc1, ClassToCount ctc2)
                {
                    return ctc1.count.CompareTo(ctc2.count);
                });
                classcount.Reverse();
                for (var c = 0; c < classcount.Count; c++)
                    if (classcount[c].count > 0)
                        output.Add(string.Format("{0}: {1} ({2}%)", classcount[c].name,
                            classcount[c].count.ToString(), (int) (classcount[c].count * 100 / total)));
            }
        }
