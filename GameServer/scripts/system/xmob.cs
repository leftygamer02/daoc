...
public class XCreateCommandHandler : AbstractCommandHandler, ICommandHandler
{
        ...
        switch (args[1].ToLower())
        {
            ...
            case "view":
            {
                ...
                if (GetClientSpot(client).Count != 0)
                {
                    ...
                    foreach (var npc in GetClientSpot(client))
            ...
            case "npcsetupcreate":
            {
                ...
                // spawn in  2 circle
                for (var i = 0; i < setupmobs.Count / 2; i++)
                {
                    var h = (ushort) (4096 / (setupmobs.Count / 2) * i);
                    var loc = client.Player.GetPointFromHeading(h, 150);
                    spawnsetupmob(client, setupmobs[i], realm, loc.X, loc.Y, h);
                }

                for (var i = setupmobs.Count / 2; i < setupmobs.Count; i++)
                {
                    var h = (ushort) (4096 / (setupmobs.Count - setupmobs.Count / 2) * i);
                    var loc = client.Player.GetPointFromHeading(h, 250);
                    spawnsetupmob(client, setupmobs[i], realm, loc.X, loc.Y, h);
                }

                client.Out.SendMessage(setupmobs.Count + " setup mob spawned!", eChatType.CT_Important,
            ...
            case "copy":
            {
                ...
                for (var i = 0; i < num; i++)
                {
                    ...
                    if (brain == null)
                    {
                        client.Out.SendMessage("Cannot create brain, standard brain being applied",
                            eChatType.CT_System, eChatLoc.CL_SystemWindow);
    ...
    {
        var X = Util.Random(client.Player.X - radius / 2, client.Player.X + radius / 2);
        var Y = Util.Random(client.Player.Y - radius / 2, client.Player.Y + radius / 2);
