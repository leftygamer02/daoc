...
public class LaunchQuestAlb : BaseQuest
{
    /// <summary>
    ...
    protected static void TalkToRey(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var quest = player.IsDoingQuest(typeof(LaunchQuestAlb)) as LaunchQuestAlb;
        ...
        if (e == GameObjectEvent.Interact)
        {
            if (quest != null)
                ...
                ReyAlb.SayTo(player, "Hello, " + player.Name +
                                     ". I need your help. Fen seems to be losing his mind, he keeps talking about his lunch... or was it a launch? " +
                ...
                switch (wArgs.Text)
                {
                    case "causing some mayhem":
                        player.Out.SendQuestSubscribeCommand(ReyAlb,
                            QuestMgr.GetIDForQuestType(typeof(LaunchQuestAlb)),
                    ...
                    case "mayhem managed":
                        if (quest.Step == 2)
                        {
                            player.Out.SendMessage(
                                "Nice, fixing all that damage should keep him busy for a few more weeks. Maybe I can finally get some sleep.",
        ...
        get
        {
            switch (Step)
            {
                case 1:
                    ...
                    sb.Append(
                        $"Kill players, captures keeps and relics, and generally cause destruction in the frontiers.");
                    ...
                    if (RealmPointsEarned < REALM_POINT_GOAL)
                        sb.Append("\nRealm Points Earned From Kills: (" + RealmPointsEarned.ToString("N0") + " | " +
                                  REALM_POINT_GOAL.ToString("N0") + ")");
    ...
    public override void Notify(DOLEvent e, object sender, EventArgs args)
    {
        ...
        if (e == GameLivingEvent.EnemyKilled)
        {
            ...
            player.Out.SendMessage("[Event] Enemy Killed: (" + PlayersKilled + " | " + PLAYER_KILL_GOAL + ")",
                eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
            player.Out.SendMessage(
                "[Event] Realm Points Gained: (" + RealmPointsEarned + " | " + REALM_POINT_GOAL.ToString("N0") +
                ")", eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
        ...
        if (e == GamePlayerEvent.CapturedKeepsChanged)
        {
            if (KeepsTaken < KEEP_TAKE_GOAL)
                KeepsTaken++;
            player.Out.SendMessage("[Event] Keeps Taken: (" + KeepsTaken + " | " + KEEP_TAKE_GOAL + ")",
                eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
            player.Out.SendQuestUpdate(this);
        }
        ...
        if (e == GamePlayerEvent.CapturedRelicsChanged)
        {
            if (RelicsTaken < RELIC_CAPTURE_GOAL)
                RelicsTaken++;
            player.Out.SendMessage("[Event] Relics Taken: (" + RelicsTaken + " | " + RELIC_CAPTURE_GOAL + ")",
                eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
            player.Out.SendQuestUpdate(this);
        }
    ...
    {
        PlayersKilled = GetCustomProperty(PlayersKilledKey) != null
            ? int.Parse(GetCustomProperty(PlayersKilledKey))
            : 0;
        ...
        RealmPointsEarned = GetCustomProperty(RPSPropertyKey) != null
            ? int.Parse(GetCustomProperty(RPSPropertyKey))
            : 0;
    }
