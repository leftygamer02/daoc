...
public class PlayTheLastSong : BaseQuest
{
    ...
    public static void ScriptLoaded(DOLEvent e, object sender, EventArgs args)
    {
        ...
        //Freeya
        var npcs = WorldMgr.GetNPCsByName("Freeya", eRealm.Midgard);
        ...
        if (Freeya == null)
        {
            ...
            Freeya.AddToWorld();
            if (SAVE_INTO_DATABASE) Freeya.SaveIntoDatabase();
        }
        ...
        if (VikingDextz == null)
        {
            ...
            if (SAVE_INTO_DATABASE) VikingDextz.SaveIntoDatabase();
        }
        ...
        GameEventMgr.AddHandler(VikingDextz, GameLivingEvent.WhisperReceive,
            new DOLEventHandler(TalkToVikingDextz));
    ...
    public static void ScriptUnloaded(DOLEvent e, object sender, EventArgs args)
    {
        ...
        GameEventMgr.RemoveHandler(VikingDextz, GameLivingEvent.WhisperReceive,
            new DOLEventHandler(TalkToVikingDextz));
    ...
    protected static void TalkToVikingDextz(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var quest = player.IsDoingQuest(typeof(PlayTheLastSong)) as PlayTheLastSong;
        ...
        if (e == GameObjectEvent.Interact)
        {
            if (quest != null)
                ...
                {
                    case 1:
                        VikingDextz.SayTo(player,
                            "God dag " + player.Name +
                            ", my mission is a difficult one! Last year we lost a wonderful and helpful Skald. " +
                    ...
                    case 2:
                        VikingDextz.SayTo(player,
                            player.Name +
                            ", you will find Freeya's grave on the hill northwest from Svasud Faste. Please check if everything is fine there.");
                        break;
                    case 3:
                        VikingDextz.SayTo(player,
                            "You are probably forsaken by all good spirits! You saw Freeya? " +
            ...
            if (quest == null)
                switch (wArgs.Text)
                {
                    case "support Thor Boyaux":
                        player.Out.SendQuestSubscribeCommand(VikingDextz,
                            QuestMgr.GetIDForQuestType(typeof(PlayTheLastSong)),
                            "Will you help Viking Dextz ([Memorial] Play the last Song)?");
                        break;
                }
            else
                switch (wArgs.Text)
                {
                    case "Freeya":
                        VikingDextz.SayTo(player,
                            "Freeya - a Master Enforcer and a good friend. I miss her every day. We could use her help now. " +
                            "Our borders need constant reinforcements, and we have heard of a growing threat in the north. ");
                        if (quest.Step == 1)
                        {
                            VikingDextz.Emote(eEmote.Cry);
                            VikingDextz.SayTo(player,
                                "We buried her in Uppland on the hill, north west of Svasud Faste. " +
                    ...
                    case "help me":
                        if (quest.Step == 1)
                        {
                            VikingDextz.SayTo(player,
                                ...
                                ", that's very kind of you! You do me a great service.");
                            quest.Step = 2;
        ...
        if (e == GameObjectEvent.Interact)
        {
            if (quest != null)
            {
                switch (quest.Step)
                {
                    case 1:
                        Freeya.SayTo(player,
                            "Hello Adventurer, do you know Dextz? He is a good friend, please visit him!");
                        break;
                    case 2:
                        player.Emote(eEmote.Shiver);
                        Freeya.TurnTo(player, 500);
                        if (player.Name.Contains("Dextz") ||
                            (player.Guild != null && player.Guild.Name.Contains("Thor Boyaux")))
                        {
                            Freeya.Emote(eEmote.Hug);
                            Freeya.SayTo(player,
                                player.CharacterClass.Name + "... It is good to see you here again, my friend. " +
                        ...
                        else
                        {
                            Freeya.Emote(eEmote.Hug);
                            Freeya.SayTo(player,
                                "God dag " + player.CharacterClass.Name +
                                ". I could sense you were coming. Please don't be scared, I promise I am friendly! " +
                    ...
                    case 3:
                        Freeya.SayTo(player,
                            "Alright " + player.Name +
                            ", all you have to do is say \"song\" to me and I will begin the ceremony!");
                        break;
                }
            }
            else
            {
                if (player.Realm != eRealm.Midgard)
                    Freeya.SayTo(player,
                        "Hello Adventurer, do you know Dextz? I had a wonderful time with him, once! " +
                    ...
                    Freeya.SayTo(player,
                        "Hello " + player.CharacterClass.Name +
                        ". Do not forget, nobody is useless in this world who makes someone else\'s burden easier.");
            }
        }
        // The player whispered to the NPC
                ...
                switch (wArgs.Text)
                {
                    ...
                    case "song":
                        ...
                        if (quest.Step == 3 && !IsSinging)
                        {
                            ...
                            new ECSGameTimer(Freeya,
                                new ECSGameTimer.ECSTimerCallback(timer => FinishSinging(timer, player)), 18000);

                            if (quest.Step == 4 && !IsSinging) quest.Step = 5;
    ...
    private static int FinishSinging(ECSGameTimer timer, GamePlayer player)
    {
        ...
        Freeya.Say("And this song is for you, " + player.Name +
                   ". You are very brave to come here in service of Midgard. " +
    ...
    //Set quest name
    public override string Name => "[Memorial] Play the Last Song";
    ...
    public override void Notify(DOLEvent e, object sender, EventArgs args)
    {
        ...
        if (e == GameObjectEvent.Interact && Step == 4 && !IsSinging)
        {
            ...
            {
                new ECSGameTimer(Freeya, new ECSGameTimer.ECSTimerCallback(timer => FinishSinging(timer, player)),
                    3000);

                if (Step == 4 && !IsSinging) Step = 5;
