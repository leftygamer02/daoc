...
public class MegalocerosKillQuestMid : Quests.DailyQuest
{
    ...
    private static void TalkToJarek(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var quest =
            player.IsDoingQuest(typeof(MegalocerosKillQuestMid)) as MegalocerosKillQuestMid;
                ...
                switch (wArgs.Text.ToLower())
                {
                    case "clear the megaloceros":
                        player.Out.SendQuestSubscribeCommand(Jarek,
                            QuestMgr.GetIDForQuestType(typeof(MegalocerosKillQuestMid)),
    ...
    {
        megalocerosKilled = GetCustomProperty(QuestPropertyKey) != null
    ...
    public override void Notify(DOLEvent e, object sender, EventArgs args)
    {
        ...
        megalocerosKilled++;
        player.Out.SendMessage("[Daily] Megaloceros Killed: (" + megalocerosKilled + " | " + MAX_KILLED + ")",
            eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
        player.Out.SendQuestUpdate(this);

        if (megalocerosKilled >= MAX_KILLED) Step = 2;
    }
