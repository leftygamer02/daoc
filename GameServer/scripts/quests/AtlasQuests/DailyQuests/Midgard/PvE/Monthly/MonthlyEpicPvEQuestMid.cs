...
public class MonthlyEpicPvEQuestMid : Quests.MonthlyQuest
{
    ...
    public static void ScriptLoaded(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var npcs = WorldMgr.GetNPCsByName("Jarek", eRealm.Midgard);
        ...
        if (Jarek == null)
        {
            ...
            Jarek.AddToWorld();
            if (SAVE_INTO_DATABASE) Jarek.SaveIntoDatabase();
        }
    ...
    private static void TalkToJarek(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var quest =
            player.IsDoingQuest(typeof(MonthlyEpicPvEQuestMid)) as MonthlyEpicPvEQuestMid;
            ...
            if (quest != null)
                switch (quest.Step)
                {
                    case 1:
                        Jarek.SayTo(player,
                            player.Name +
                            ", please find allies and kill the epic creatures in Trollheim and Iarnvidiur's Lair!");
                        break;
                    case 2:
                        Jarek.SayTo(player,
                            "Hello " + player.Name + ", did you [slay the creatures] and return for your reward?");
                        break;
                }
            else
                Jarek.SayTo(player, "Hello " + player.Name +
                                    ", I am Jarek. For several months the situation in Trollheim and Iarnvidur's Lair has changed. " +
                ...
                switch (wArgs.Text)
                {
                    case "kill Nosdoden and Iarnvidiur":
                        player.Out.SendQuestSubscribeCommand(Jarek,
                            QuestMgr.GetIDForQuestType(typeof(MonthlyEpicPvEQuestMid)),
    ...
    private static void CheckPlayerAcceptQuest(GamePlayer player, byte response)
    {
        ...
        else
        {
            ...
            Jarek.SayTo(player,
                "Please, find the epic monsters in Trollheim and Iarnvidiur's Lair and return for your reward.");
        }
    }
    ...
    public override string Description
    {
        get
        {
            switch (Step)
            {
                case 1:
                    return
                        "Make your way and defeat the epic creatures in Trollheim as well as in Iarnvidiur's Lair! \n" +
                        "Killed: " + Nosdoden_NAME + " (" + _nosdodenKilled + " | " + MAX_KILLED +
                        ") in Trollheim\n" +
                        "Killed: " + Iarnvidiur_NAME + " (" + _iarnvidiurKilled + " | " + MAX_KILLED +
                        ") in Iarnvidiur's Lair\n";
                case 2:
    ...
    public override void Notify(DOLEvent e, object sender, EventArgs args)
    {
        ...
        if (gArgs.Target.Name.ToLower() == Iarnvidiur_NAME.ToLower() && gArgs.Target is GameNPC &&
            _iarnvidiurKilled < MAX_KILLED)
        {
            _iarnvidiurKilled = 1;
            player.Out.SendMessage(
                "[Monthly] You killed " + Iarnvidiur_NAME + ": (" + _iarnvidiurKilled + " | " + MAX_KILLED + ")",
                eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
            player.Out.SendQuestUpdate(this);
        }
        else if (gArgs.Target.Name.ToLower() == Nosdoden_NAME.ToLower() && gArgs.Target is GameNPC &&
                 _nosdodenKilled < MAX_KILLED)
        {
            _nosdodenKilled = 1;
            player.Out.SendMessage(
                "[Monthly] You killed " + Nosdoden_NAME + ": (" + _nosdodenKilled + " | " + MAX_KILLED + ")",
                eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
            player.Out.SendQuestUpdate(this);
        }

        if (_iarnvidiurKilled >= MAX_KILLED && _nosdodenKilled >= MAX_KILLED) Step = 2;
    }
    ...
    {
        _iarnvidiurKilled = GetCustomProperty(Iarnvidiur_NAME) != null
            ? int.Parse(GetCustomProperty(Iarnvidiur_NAME))
            : 0;
        _nosdodenKilled = GetCustomProperty(Nosdoden_NAME) != null
            ? int.Parse(GetCustomProperty(Nosdoden_NAME))
            : 0;
    }
