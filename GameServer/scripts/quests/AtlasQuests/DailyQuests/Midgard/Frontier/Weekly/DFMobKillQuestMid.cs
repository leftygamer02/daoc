...
public class DFMobKillQuestMid : Quests.WeeklyQuest
{
    ...
    private static void TalkToPatrick(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var quest = player.IsDoingQuest(typeof(DFMobKillQuestMid)) as DFMobKillQuestMid;
        ...
        if (e == GameObjectEvent.Interact)
        {
            if (quest != null)
                ...
                Patrick.SayTo(player, "Hello " + player.Name +
                                      ", I am Patrick. I have received word from a hunter that forces are building in Darkness Falls. " +
                ...
                switch (wArgs.Text)
                {
                    case "stop the invasion":
                        player.Out.SendQuestSubscribeCommand(Patrick,
                            QuestMgr.GetIDForQuestType(typeof(DFMobKillQuestMid)),
