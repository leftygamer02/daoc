...
public class KillNPCInFrontiersMid : Quests.DailyQuest
{
    ...
    protected static void TalkToHerou(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var quest = player.IsDoingQuest(typeof(KillNPCInFrontiersMid)) as KillNPCInFrontiersMid;
        ...
        if (e == GameObjectEvent.Interact)
        {
            if (quest != null)
                ...
                Herou.SayTo(player, "Hello " + player.Name +
                                    ", I am Herou, Fen\'s friend. I serve the realm and ensure its borders are always protected. " +
                ...
                switch (wArgs.Text)
                {
                    case "clear the frontiers":
                        player.Out.SendQuestSubscribeCommand(Herou,
                            QuestMgr.GetIDForQuestType(typeof(KillNPCInFrontiersMid)),
