...
public class FrontiersMonthlyQuestMid : Quests.MonthlyQuest
{
    ...
    public static void ScriptLoaded(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var npcs = WorldMgr.GetNPCsByName("Kelteen", eRealm.Midgard);
        ...
        if (Kelteen == null)
        {
            ...
            Kelteen.AddToWorld();
            if (SAVE_INTO_DATABASE) Kelteen.SaveIntoDatabase();
        }
    ...
    private static void TalkToKelteen(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var quest =
            player.IsDoingQuest(typeof(FrontiersMonthlyQuestMid)) as FrontiersMonthlyQuestMid;
        ...
        if (e == GameObjectEvent.Interact)
        {
            if (quest != null)
                ...
                {
                    case 1:
                        Kelteen.SayTo(player,
                            $"Hello {player.CharacterClass.Name}, you will find enemies in Albion, Hibernia or in our lands. " +
                            $"Come back when you have killed enough enemies and taken keeps for our safety.");
                        break;
                    case 2:
                        Kelteen.SayTo(player,
                            "Hello " + player.Name + ", did you success [capturing keeps and killing enemies]?");
                        break;
                }
            else
            ...
            if (quest == null)
                switch (wArgs.Text)
                {
                    case "order and security":
                        player.Out.SendQuestSubscribeCommand(Kelteen,
                            QuestMgr.GetIDForQuestType(typeof(FrontiersMonthlyQuestMid)),
                            "Will you help " + Kelteen.Name + " to slay enemies and capture keeps? " + questTitle +
                            "?");
                        break;
                }
            else
                    ...
                    case "capturing keeps and killing enemies":
                        if (quest.Step == 2)
                        {
                            player.Out.SendMessage(
                                "Thank you for your help! Midgard will thank you for your contribution.",
                                eChatType.CT_Chat, eChatLoc.CL_PopupWindow);
                            quest.FinishQuest();
                        }
    ...
    public override void Notify(DOLEvent e, object sender, EventArgs args)
    {
        ...
        if (e == GameLivingEvent.EnemyKilled && Step == 1 && PlayersKilled < MAX_KILLING_GOAL)
        {
            ...
            if (gArgs.Target.CurrentRegionID != 1 && gArgs.Target.CurrentRegionID != 100 &&
                gArgs.Target.CurrentRegionID != 200)
            {
                player.Out.SendMessage("[Monthly] You need to find enemies in the old frontiers.",
                    eChatType.CT_System, eChatLoc.CL_SystemWindow);
                return;
            }

            PlayersKilled++;
            player.Out.SendMessage("[Monthly] Enemies Killed: (" + PlayersKilled + " | " + MAX_KILLING_GOAL + ")",
        ...
        else if (e == GamePlayerEvent.CapturedKeepsChanged && Step == 1 && CapturedKeeps < MAX_CAPTURED_KEEPS_GOAL)
        {
            CapturedKeeps++;
            player.Out.SendMessage(
                "[Monthly] Captured Keeps: (" + CapturedKeeps + " | " + MAX_CAPTURED_KEEPS_GOAL + ")",
                eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
            player.Out.SendQuestUpdate(this);
        }

        if (PlayersKilled >= MAX_KILLING_GOAL && CapturedKeeps >= MAX_CAPTURED_KEEPS_GOAL) Step = 2;
    }
    ...
    {
        PlayersKilled = GetCustomProperty("FrontiersMonthlyMidKill") != null
            ? int.Parse(GetCustomProperty("FrontiersMonthlyMidKill"))
            : 0;
        CapturedKeeps = GetCustomProperty("FrontiersMonthlyMidKeep") != null
            ? int.Parse(GetCustomProperty("FrontiersMonthlyMidKeep"))
            : 0;
    }
    ...
    {
        var reward = ServerProperties.Properties.MONTHLY_RVR_REWARD;
        ...
        if (m_questPlayer.Inventory.IsSlotsFree(3, eInventorySlot.FirstBackpack, eInventorySlot.LastBackpack))
        {
            ...
            if (reward > 0)
            {
                m_questPlayer.Out.SendMessage(
                    $"You have been rewarded {reward} Realmpoints for finishing Monthly Quest.",
        ...
        {
            m_questPlayer.Out.SendMessage("Clear three slots of your inventory for your reward",
                eChatType.CT_System, eChatLoc.CL_SystemWindow);
        }
    }
}