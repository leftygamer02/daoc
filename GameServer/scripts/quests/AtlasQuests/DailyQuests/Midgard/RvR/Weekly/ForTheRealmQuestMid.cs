...
public class ForTheRealmQuestMid : Quests.WeeklyQuest
{
    ...
    protected static void TalkToRey(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var quest = player.IsDoingQuest(typeof(ForTheRealmQuestMid)) as ForTheRealmQuestMid;
        ...
        if (e == GameObjectEvent.Interact)
        {
            if (quest != null)
                switch (quest.Step)
                {
                    case 1:
                        ReyMid.SayTo(player,
                            "Head into the enemy frontiers and slay their forces. There are many Hibernia and Albion enemies, you will find them.");
                        break;
                    case 2:
                        ReyMid.SayTo(player,
                ...
                ReyMid.SayTo(player, "You won't believe it, " + player.Name +
                ...
                switch (wArgs.Text)
                {
                    case "bunch of toeknuckles":
                        player.Out.SendQuestSubscribeCommand(ReyMid,
                            QuestMgr.GetIDForQuestType(typeof(ForTheRealmQuestMid)),
    ...
    public override void Notify(DOLEvent e, object sender, EventArgs args)
    {
        ...
        if (gArgs.Target.Realm == eRealm.Hibernia && gArgs.Target.Realm != player.Realm &&
            gArgs.Target is GamePlayer && player.GetConLevel(gArgs.Target) > MIN_PLAYER_CON &&
            _playersKilledHib < MAX_KILLGOAL)
        {
            _playersKilledHib++;
            player.Out.SendMessage(
                "[Daily] Killed Hibernia Enemy: (" + _playersKilledHib + " | " + MAX_KILLGOAL + ")",
                 ...
                 _playersKilledAlb < MAX_KILLGOAL)
        {
            _playersKilledAlb++;
            player.Out.SendMessage(
                "[Daily] Killed Albion Enemy: (" + _playersKilledAlb + " | " + MAX_KILLGOAL + ")",
                eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
            player.Out.SendQuestUpdate(this);
        }
