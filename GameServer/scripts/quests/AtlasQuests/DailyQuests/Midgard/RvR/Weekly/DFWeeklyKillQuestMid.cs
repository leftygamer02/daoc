...
public class DFWeeklyKillQuestMid : Quests.WeeklyQuest
{
    ...
    public static void ScriptLoaded(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var npcs = WorldMgr.GetNPCsByName("Patrick", eRealm.Midgard);
        ...
        if (Patrick == null)
        {
            ...
            Patrick.AddToWorld();
            if (SAVE_INTO_DATABASE) Patrick.SaveIntoDatabase();
        }
    ...
    private static void TalkToPatrick(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var quest = player.IsDoingQuest(typeof(DFWeeklyKillQuestMid)) as DFWeeklyKillQuestMid;
        ...
        if (e == GameObjectEvent.Interact)
        {
            if (quest != null)
                ...
                {
                    case 1:
                        Patrick.SayTo(player,
                            "Please head into Darkness Falls and harvest parts from Midgard's enemies!");
                        break;
                    case 2:
                ...
                Patrick.SayTo(player, "Oh, " + player.Name +
                                      ", glad you finally returned. Boss has a new recipe that requires bones that have been steeped in a [demonic aura]. \n" +
        ...
        else if (e == GameLivingEvent.WhisperReceive)
        {
            ...
            if (quest == null)
                ...
                {
                    case "demonic aura":
                        player.Out.SendQuestSubscribeCommand(Patrick,
                            QuestMgr.GetIDForQuestType(typeof(DFWeeklyKillQuestMid)),
                            "Will you help Patrick " + questTitle + "?");
                        break;
                }
            else
                switch (wArgs.Text)
    ...
    public override void Notify(DOLEvent e, object sender, EventArgs args)
    {
        ...
        EnemiesKilled++;
        player.Out.SendMessage("[Weekly] Enemy Killed: (" + EnemiesKilled + " | " + MAX_KILLED + ")",
            eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
        player.Out.SendQuestUpdate(this);

        if (EnemiesKilled >= MAX_KILLED) Step = 2;
    }
    ...
    {
        EnemiesKilled = GetCustomProperty(QuestPropertyKey) != null
