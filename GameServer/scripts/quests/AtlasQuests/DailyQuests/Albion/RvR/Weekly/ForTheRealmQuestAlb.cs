...
public class ForTheRealmQuestAlb : Quests.WeeklyQuest
{
    ...
    private static void TalkToRey(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var quest = player.IsDoingQuest(typeof(ForTheRealmQuestAlb)) as ForTheRealmQuestAlb;
        ...
        if (e == GameObjectEvent.Interact)
        {
            if (quest != null)
                switch (quest.Step)
                {
                    case 1:
                        ReyAlb.SayTo(player,
                            "Head into the enemy frontiers and slay their forces. There are many Midgard and Hibernia enemies, you will find them.");
                        break;
                    case 2:
                        ReyAlb.SayTo(player,
                            "Hello " + player.Name + ", did you [slay their forces] for your reward?");
                        break;
                }
            else
                ReyAlb.SayTo(player, "You won't believe it, " + player.Name +
                                     ". We got a bulk order for exotic bones too. " +
                ...
                switch (wArgs.Text)
                {
                    case "bunch of toeknuckles":
                        player.Out.SendQuestSubscribeCommand(ReyAlb,
                            QuestMgr.GetIDForQuestType(typeof(ForTheRealmQuestAlb)),
    ...
    public override void Notify(DOLEvent e, object sender, EventArgs args)
    {
            ...
            _playersKilledMid < MAX_KILLGOAL)
        {
            ...
            player.Out.SendMessage(
                "[Weekly] Midgard Enemy Killed: (" + _playersKilledMid + " | " + MAX_KILLGOAL + ")",
                 ...
                 _playersKilledHib < MAX_KILLGOAL)
        {
            _playersKilledHib++;
            player.Out.SendMessage(
                "[Weekly] Hibernia Enemy Killed: (" + _playersKilledHib + " | " + MAX_KILLGOAL + ")",
                eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
            player.Out.SendQuestUpdate(this);
        }
    ...
    {
        _playersKilledHib = GetCustomProperty("ForTheRealmKilledHib") != null
            ? int.Parse(GetCustomProperty("ForTheRealmKilledHib"))
            : 0;
        _playersKilledMid = GetCustomProperty("ForTheRealmKilledMid") != null
            ? int.Parse(GetCustomProperty("ForTheRealmKilledMid"))
            : 0;
    }
