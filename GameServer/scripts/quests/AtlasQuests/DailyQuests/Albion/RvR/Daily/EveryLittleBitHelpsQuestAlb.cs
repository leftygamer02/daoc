...
public class EveryLittleBitHelpsQuestAlb : Quests.DailyQuest
{
    ...
    public static void ScriptLoaded(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var npcs = WorldMgr.GetNPCsByName("Rey", eRealm.Albion);
        ...
        if (ReyAlb == null)
        {
            ...
            ReyAlb.AddToWorld();
            if (SAVE_INTO_DATABASE) ReyAlb.SaveIntoDatabase();
        }
    ...
    private static void TalkToRey(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var quest =
            player.IsDoingQuest(typeof(EveryLittleBitHelpsQuestAlb)) as EveryLittleBitHelpsQuestAlb;
        ...
        if (e == GameObjectEvent.Interact)
        {
            if (quest != null)
                switch (quest.Step)
                {
                    case 1:
                        ReyAlb.SayTo(player,
                            "Find and kill enemies of Midgard and Hibernia. You will find suitable players in the frontiers.");
                        break;
                    case 2:
                ...
                ReyAlb.SayTo(player, "Hello " + player.Name +
                                     ", I am Rey. My master, Fen, has a need for some... exotic bones. " +
            ...
            if (quest == null)
                switch (wArgs.Text)
                {
                    case "take the toeknuckle":
                        player.Out.SendQuestSubscribeCommand(ReyAlb,
                            QuestMgr.GetIDForQuestType(typeof(EveryLittleBitHelpsQuestAlb)),
    ...
    public override void Notify(DOLEvent e, object sender, EventArgs args)
    {
        ...
        if (gArgs.Target.Realm == eRealm.Midgard && gArgs.Target.Realm != player.Realm &&
            gArgs.Target is GamePlayer && player.GetConLevel(gArgs.Target) > MIN_PLAYER_CON &&
            _playersKilledMid < MAX_KILLGOAL)
        {
            _playersKilledMid++;
            player.Out.SendMessage(
                "[Daily] Midgard Enemy Killed: (" + _playersKilledMid + " | " + MAX_KILLGOAL + ")",
                eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
            player.Out.SendQuestUpdate(this);
        }
        else if (gArgs.Target.Realm == eRealm.Hibernia && gArgs.Target.Realm != player.Realm &&
                 gArgs.Target is GamePlayer && player.GetConLevel(gArgs.Target) > MIN_PLAYER_CON &&
                 _playersKilledHib < MAX_KILLGOAL)
        {
            _playersKilledHib++;
            player.Out.SendMessage(
                "[Daily] Hibernia Enemy Killed: (" + _playersKilledHib + " | " + MAX_KILLGOAL + ")",
                eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
            player.Out.SendQuestUpdate(this);
        }
    ...
    {
        _playersKilledHib = GetCustomProperty("PlayersKilledHib") != null
            ? int.Parse(GetCustomProperty("PlayersKilledHib"))
            : 0;
        _playersKilledMid = GetCustomProperty("PlayersKilledMid") != null
            ? int.Parse(GetCustomProperty("PlayersKilledMid"))
            : 0;
    }
    ...
    {
        var reward = ServerProperties.Properties.DAILY_RVR_REWARD;

        m_questPlayer.ForceGainExperience((m_questPlayer.ExperienceForNextLevel -
                                           m_questPlayer.ExperienceForCurrentLevel) / 5);
        m_questPlayer.AddMoney(Money.GetMoney(0, 0, m_questPlayer.Level, 32, Util.Random(50)),
        ...
        AtlasROGManager.GenerateJewel(m_questPlayer, (byte) (m_questPlayer.Level + 1),
            m_questPlayer.Level + Util.Random(5, 15));
        _playersKilledHib = 0;
        _playersKilledMid = 0;

        if (reward > 0)
        {
            m_questPlayer.Out.SendMessage($"You have been rewarded {reward} Realmpoints for finishing Daily Quest.",
