...
public class EpicRvRMobsWeeklyQuestAlb : Quests.WeeklyQuest
{
    ...
    public static void ScriptLoaded(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var npcs = WorldMgr.GetNPCsByName("Haszan", eRealm.Albion);
        ...
        if (Haszan == null)
        {
            ...
            Haszan.Heading = 3111;
            Haszan.AddToWorld();
            if (SAVE_INTO_DATABASE) Haszan.SaveIntoDatabase();
        }
    ...
    private static void TalkToHaszan(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var quest =
            player.IsDoingQuest(typeof(EpicRvRMobsWeeklyQuestAlb)) as EpicRvRMobsWeeklyQuestAlb;
            ...
            if (quest != null)
                switch (quest.Step)
                {
                    case 1:
                        Haszan.SayTo(player,
                            player.Name +
                            ", please find allys and kill the epic creatures in frontiers for Albion!");
                        break;
                    case 2:
                        Haszan.SayTo(player,
                ...
                Haszan.SayTo(player, "Hello " + player.Name +
                                     ", I am Haszan. Some large monsters have blocked the supply lines in our frontier, and I could use your help in getting rid of them.\n" +
                ...
                switch (wArgs.Text)
                {
                    case "kill the epic creatures":
                        player.Out.SendQuestSubscribeCommand(Haszan,
                            QuestMgr.GetIDForQuestType(typeof(EpicRvRMobsWeeklyQuestAlb)),
