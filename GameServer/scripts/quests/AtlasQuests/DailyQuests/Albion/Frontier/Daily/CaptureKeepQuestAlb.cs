...
public class CaptureKeepQuestAlb : Quests.DailyQuest
{
    ...
    private static void TalkToHaszan(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var quest = player.IsDoingQuest(typeof(CaptureKeepQuestAlb)) as CaptureKeepQuestAlb;
        ...
        if (e == GameObjectEvent.Interact)
        {
            if (quest != null)
                switch (quest.Step)
                {
                    case 1:
                        Haszan.SayTo(player,
                            "Find an enemy occupied keep and capture it. If you succeed come back for your reward.");
                        break;
                    case 2:
                ...
                Haszan.SayTo(player, "Hello " + player.Name +
                                     ", I am Haszan. I help the king with logistics, and he's tasked me with getting things done around here. " +
                ...
                switch (wArgs.Text)
                {
                    case "reclaim a keep":
                        player.Out.SendQuestSubscribeCommand(Haszan,
                            QuestMgr.GetIDForQuestType(typeof(CaptureKeepQuestAlb)),
    ...
    public override void Notify(DOLEvent e, object sender, EventArgs args)
    {
        ...
        _isCaptured = 1;
        player.Out.SendMessage("[Daily] Captured Keep: (" + _isCaptured + " | " + MAX_CAPTURED + ")",
