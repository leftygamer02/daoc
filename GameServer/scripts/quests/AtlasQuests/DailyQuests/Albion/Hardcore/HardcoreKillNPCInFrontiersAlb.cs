...
public class HardcoreKillNPCInFrontiersAlb : Quests.DailyQuest
{
    ...
    private static void TalkToSucci(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var oranges =
            player.IsDoingQuest(typeof(HardcoreKillNPCInFrontiersAlb)) as HardcoreKillNPCInFrontiersAlb;
            ...
            if (oranges == null)
                switch (wArgs.Text.ToLower())
                {
                    case "one can be brave":
                        player.Out.SendQuestSubscribeCommand(SucciAlb,
                            QuestMgr.GetIDForQuestType(typeof(HardcoreKillNPCInFrontiersAlb)),
                ...
                switch (wArgs.Text)
                {
                    case "another sunrise":
                        if (oranges.Step == 2)
                        {
                            player.Out.SendMessage("Enjoy your meal. With luck, it shall not be your last.",
    ...
    public override void Notify(DOLEvent e, object sender, EventArgs args)
    {
        ...
        if (e == GameLivingEvent.Dying && Step == 1) FailQuest();
        ...
        FrontierMobsKilled++;
        player.Out.SendMessage("[Hardcore] Monster Killed: (" + FrontierMobsKilled + " | " + MAX_KillGoal + ")",
    ...
    {
        FrontierMobsKilled = GetCustomProperty(QuestPropertyKey) != null
