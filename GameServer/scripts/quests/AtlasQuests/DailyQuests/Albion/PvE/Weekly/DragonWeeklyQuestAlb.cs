...
public class DragonWeeklyQuestAlb : Quests.WeeklyQuest
{
    ...
    private static void TalkToHector(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var quest = player.IsDoingQuest(typeof(DragonWeeklyQuestAlb)) as DragonWeeklyQuestAlb;
        ...
        if (e == GameObjectEvent.Interact)
        {
            if (quest != null)
                switch (quest.Step)
                {
                    case 1:
                        Hector.SayTo(player,
                            player.Name + ", please travel to Dartmoor and kill the dragon for Albion!");
                        break;
                    case 2:
                        Hector.SayTo(player,
                ...
                Hector.SayTo(player, "Hello " + player.Name + ", I am Hector. I bring sad news today. " +
                                     DRAGON_NAME + " razed a small settlement in Dartmoor last night. \n" +
                                     "Please, help the king avenge their deaths and keep Albion safe from " +
                ...
                switch (wArgs.Text)
                {
                    case "kill the dragon":
                        player.Out.SendQuestSubscribeCommand(Hector,
                            QuestMgr.GetIDForQuestType(typeof(DragonWeeklyQuestAlb)),
