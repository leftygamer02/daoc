...
public class DanaoinKillQuestAlb : Quests.DailyQuest
{
    ...
    private static void TalkToJames(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var quest = player.IsDoingQuest(typeof(DanaoinKillQuestAlb)) as DanaoinKillQuestAlb;
                ...
                switch (wArgs.Text)
                {
                    case "clear the Danaoin":
                        player.Out.SendQuestSubscribeCommand(James,
                            QuestMgr.GetIDForQuestType(typeof(DanaoinKillQuestAlb)),
    ...
    {
        danaoinKilled = GetCustomProperty(QuestPropertyKey) != null
    ...
    public override void Notify(DOLEvent e, object sender, EventArgs args)
    {
        ...
        danaoinKilled++;
        player.Out.SendMessage("[Daily] Danaoin Farmers Killed: (" + danaoinKilled + " | " + MAX_KILLED + ")",
            eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
        player.Out.SendQuestUpdate(this);

        if (danaoinKilled >= MAX_KILLED) Step = 2;
    }
