...
public class TeamBuildingAlb : Quests.DailyQuest
{
    ...
    public static void ScriptLoaded(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var npcs = WorldMgr.GetNPCsByName("Hector", eRealm.Albion);
        ...
        if (Hector == null)
        {
            ...
            Hector.Heading = 3111;
            Hector.AddToWorld();
            if (SAVE_INTO_DATABASE) Hector.SaveIntoDatabase();
        }
    ...
    protected static void TalkToHector(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var quest = player.IsDoingQuest(typeof(TeamBuildingAlb)) as TeamBuildingAlb;
        ...
        if (e == GameObjectEvent.Interact)
        {
            if (quest != null)
                switch (quest.Step)
                {
                    case 1:
                        Hector.SayTo(player,
                            "Kill creatures in any RvR zone to help us clear more room for the armies to maneuver around.");
                        break;
                    case 2:
                ...
                Hector.SayTo(player, "Hello " + player.Name +
                                     ", I am Hector. I help the king with logistics, and he's tasked me with getting things done around here. " +
            ...
            if (quest == null)
                switch (wArgs.Text)
                {
                    case "feeling social":
                        player.Out.SendQuestSubscribeCommand(Hector,
                            QuestMgr.GetIDForQuestType(typeof(TeamBuildingAlb)),
                            "Will you help Hector " + questTitle + "");
                        break;
                }
            else
                switch (wArgs.Text)
                {
                    case "forge the bonds of unity":
                        if (quest.Step == 2)
                        {
                            player.Out.SendMessage("I can feel our realm growing more cohesive every day!",
    ...
    {
        TeamBuildMobsKilled = GetCustomProperty(QuestPropertyKey) != null
    ...
    private static void CheckPlayerAcceptQuest(GamePlayer player, byte response)
    {
        ...
        if (response == 0x00)
        {
            player.Out.SendMessage("Thank you for helping our realm prosper.", eChatType.CT_Say,
                ...
                case 1:
                    ...
                    else
                    {
                        var output = new StringBuilder(
                            "Kill orange con or higher mobs while in a group containing the following base classes:\n");
    ...
    public override void FinishQuest()
    {
        ...
        m_questPlayer.AddMoney(Money.GetMoney(0, 0, m_questPlayer.Level, 50, Util.Random(50)),
