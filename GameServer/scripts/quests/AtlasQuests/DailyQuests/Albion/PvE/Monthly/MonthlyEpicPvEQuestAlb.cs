...
public class MonthlyEpicPvEQuestAlb : Quests.MonthlyQuest
{
    ...
    public static void ScriptLoaded(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var npcs = WorldMgr.GetNPCsByName("James", eRealm.Albion);
        ...
        if (James == null)
        {
            ...
            James.AddToWorld();
            if (SAVE_INTO_DATABASE) James.SaveIntoDatabase();
        }
    ...
    private static void TalkToJames(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var quest =
            player.IsDoingQuest(typeof(MonthlyEpicPvEQuestAlb)) as MonthlyEpicPvEQuestAlb;
            ...
            if (quest != null)
                switch (quest.Step)
                {
                    case 1:
                        James.SayTo(player,
                            player.Name +
                            ", please find allies and kill the epic creatures in Krondon and in the Crystal Cave of Avalon City!");
                        break;
                    case 2:
                        James.SayTo(player,
                ...
                James.SayTo(player, "Hello " + player.Name +
                                    ", I am James. For several months the situation in Krondon and in the Crystal Cave of Avalon City has changed. " +
                ...
                switch (wArgs.Text)
                {
                    case "kill Orylle and Xanxicar":
                        player.Out.SendQuestSubscribeCommand(James,
                            QuestMgr.GetIDForQuestType(typeof(MonthlyEpicPvEQuestAlb)),
    ...
    private static void CheckPlayerAcceptQuest(GamePlayer player, byte response)
    {
        ...
        else
        {
            ...
            James.SayTo(player,
                "Please, find the epic monsters in Krondon and in the Crystal Cave of Avalon City and return for your reward.");
        }
    }
    ...
    public override string Description
    {
        get
        {
            switch (Step)
            {
                case 1:
                    return
                        "Make your way and defeat the epic creatures in Krondon as well as in the Crystal Cave of Avalon City! \n" +
                        "Killed: " + Orylle_NAME + " (" + _orylleKilled + " | " + MAX_KILLED + ") in Krondon\n" +
                        "Killed: " + Xanxicar_NAME + " (" + _xanxicarKilled + " | " + MAX_KILLED +
                        ") in Crystal Cave\n";
                case 2:
    ...
    public override void Notify(DOLEvent e, object sender, EventArgs args)
    {
        ...
        if (gArgs.Target.Name.ToLower() == Orylle_NAME.ToLower() && gArgs.Target is GameNPC &&
            _orylleKilled < MAX_KILLED)
        {
            _orylleKilled = 1;
            player.Out.SendMessage(
                "[Monthly] You killed " + Orylle_NAME + ": (" + _orylleKilled + " | " + MAX_KILLED + ")",
                eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
            player.Out.SendQuestUpdate(this);
        }
        else if (gArgs.Target.Name.ToLower() == Xanxicar_NAME.ToLower() && gArgs.Target is GameNPC &&
                 _xanxicarKilled < MAX_KILLED)
        {
            _xanxicarKilled = 1;
            player.Out.SendMessage(
                "[Monthly] You killed " + Xanxicar_NAME + ": (" + _xanxicarKilled + " | " + MAX_KILLED + ")",
                eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
            player.Out.SendQuestUpdate(this);
        }

        if (_orylleKilled >= MAX_KILLED && _xanxicarKilled >= MAX_KILLED) Step = 2;
    }
    ...
    public override void LoadQuestParameters()
    {
        ...
        _xanxicarKilled = GetCustomProperty(Xanxicar_NAME) != null
            ? int.Parse(GetCustomProperty(Xanxicar_NAME))
            : 0;
    }
