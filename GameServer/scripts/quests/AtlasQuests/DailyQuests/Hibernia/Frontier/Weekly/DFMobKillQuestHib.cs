...
public class DFMobKillQuestHib : Quests.WeeklyQuest
{
    ...
    private static void TalkToStefano(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var quest = player.IsDoingQuest(typeof(DFMobKillQuestHib)) as DFMobKillQuestHib;
        ...
        if (e == GameObjectEvent.Interact)
        {
            if (quest != null)
                switch (quest.Step)
                {
                    ...
                    case 2:
                        Stefano.SayTo(player,
                            "Hello " + player.Name + ", did you [slay monsters] for your reward?");
                        break;
                }
            else
                Stefano.SayTo(player, "Hello " + player.Name +
                                      ", I am Stefano. I have received word from a ranger that forces are building in Darkness Falls. " +
                ...
                switch (wArgs.Text)
                {
                    case "stop the invasion":
                        player.Out.SendQuestSubscribeCommand(Stefano,
                            QuestMgr.GetIDForQuestType(typeof(DFMobKillQuestHib)),
                            "Will you help Stefano " + questTitle + "?");
                        break;
                }
            else
                switch (wArgs.Text)
    ...
    private static void CheckPlayerAcceptQuest(GamePlayer player, byte response)
    {
        ...
        if (response == 0x00)
        {
            player.Out.SendMessage(
                "Thank you for helping our realm. That should buy us some time to plot a counterattack.",
