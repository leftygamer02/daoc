...
public class EpicRvRMobsWeeklyQuestHib : Quests.WeeklyQuest
{
    ...
    public static void ScriptLoaded(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var npcs = WorldMgr.GetNPCsByName("Cola", eRealm.Hibernia);
        ...
        if (Cola == null)
        {
            ...
            Cola.Heading = 1586;
            Cola.AddToWorld();
            if (SAVE_INTO_DATABASE) Cola.SaveIntoDatabase();
        }
    ...
    private static void TalkToCola(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var quest =
            player.IsDoingQuest(typeof(EpicRvRMobsWeeklyQuestHib)) as EpicRvRMobsWeeklyQuestHib;
            ...
            if (quest != null)
                switch (quest.Step)
                {
                    case 1:
                        Cola.SayTo(player,
                            player.Name +
                            ", please find allies and kill the epic creatures in frontiers for Hibernia!");
                        break;
                    case 2:
                        Cola.SayTo(player,
                ...
                Cola.SayTo(player, "Hello " + player.Name +
                                   ", I am Cola. Some large monsters have blocked the supply lines in our frontier, and I could use your help in getting rid of them.\n" +
                ...
                switch (wArgs.Text)
                {
                    case "kill the epic creatures":
                        player.Out.SendQuestSubscribeCommand(Cola,
                            QuestMgr.GetIDForQuestType(typeof(EpicRvRMobsWeeklyQuestHib)),
                            "Will you help Cola " + questTitle + "?");
                        break;
                }
            else
                switch (wArgs.Text)
    ...
    public override void LoadQuestParameters()
    {
        ...
        _glacierGiantKilled = GetCustomProperty(GLACIERGIANT_NAME) != null
            ? int.Parse(GetCustomProperty(GLACIERGIANT_NAME))
            : 0;
        _greenKnightKilled = GetCustomProperty(GREENKNIGHT_NAME) != null
            ? int.Parse(GetCustomProperty(GREENKNIGHT_NAME))
            : 0;
    }
    ...
    public override void Notify(DOLEvent e, object sender, EventArgs args)
    {
        ...
        if (gArgs.Target.Name.ToLower() == EVERN_NAME.ToLower() && gArgs.Target is GameNPC &&
            _evernKilled < MAX_KILLED)
        {
            _evernKilled = 1;
            player.Out.SendMessage(
                "[Weekly] You killed " + EVERN_NAME + ": (" + _evernKilled + " | " + MAX_KILLED + ")",
            ...
            player.Out.SendQuestUpdate(this);
                 ...
                 _greenKnightKilled < MAX_KILLED)
        {
            _greenKnightKilled = 1;
            player.Out.SendMessage(
                "[Weekly] You killed " + GREENKNIGHT_NAME + ": (" + _greenKnightKilled + " | " + MAX_KILLED + ")",
                eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
            player.Out.SendQuestUpdate(this);
        }
        else if (gArgs.Target.Name.ToLower() == GLACIERGIANT_NAME.ToLower() && gArgs.Target is GameNPC &&
                 _glacierGiantKilled < MAX_KILLED)
        {
            _glacierGiantKilled = 1;
            player.Out.SendMessage(
                "[Weekly] You killed " + GLACIERGIANT_NAME + ": (" + _glacierGiantKilled + " | " + MAX_KILLED + ")",
                eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
            player.Out.SendQuestUpdate(this);
        }
    ...
    public override void FinishQuest()
    {
        ...
        AtlasROGManager.GenerateJewel(m_questPlayer, (byte) (m_questPlayer.Level + 1),
            m_questPlayer.Level + Util.Random(10, 11));
        _evernKilled = 0;
