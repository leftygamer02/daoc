...
public class EveryLittleBitHelpsQuestHib : Quests.DailyQuest
{
    ...
    public static void ScriptLoaded(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var npcs = WorldMgr.GetNPCsByName("Rey", eRealm.Hibernia);
        ...
        if (ReyHib == null)
        {
            ...
            ReyHib.AddToWorld();
            if (SAVE_INTO_DATABASE) ReyHib.SaveIntoDatabase();
        }
    ...
    protected static void TalkToRey(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var quest =
            player.IsDoingQuest(typeof(EveryLittleBitHelpsQuestHib)) as EveryLittleBitHelpsQuestHib;
        ...
        if (e == GameObjectEvent.Interact)
        {
            if (quest != null)
                switch (quest.Step)
                {
                    case 1:
                        ReyHib.SayTo(player,
                            "Find and kill enemies of Midgard and Albion. You will find suitable players in the frontiers.");
                        break;
                    case 2:
                ...
                ReyHib.SayTo(player, "Hello " + player.Name +
            ...
            if (quest == null)
                switch (wArgs.Text)
                {
                    case "take the toeknuckle":
                        player.Out.SendQuestSubscribeCommand(ReyHib,
                            QuestMgr.GetIDForQuestType(typeof(EveryLittleBitHelpsQuestHib)),
    ...
    public override void Notify(DOLEvent e, object sender, EventArgs args)
    {
        ...
        else if (gArgs.Target.Realm == eRealm.Albion && gArgs.Target.Realm != player.Realm &&
                 gArgs.Target is GamePlayer && player.GetConLevel(gArgs.Target) > MIN_PLAYER_CON &&
                 _playersKilledAlb < MAX_KILLGOAL)
        {
            _playersKilledAlb++;
            player.Out.SendMessage(
                "[Daily] Albion Enemy Killed: (" + _playersKilledAlb + " | " + MAX_KILLGOAL + ")",
                eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
            player.Out.SendQuestUpdate(this);
        }
    ...
    {
        _playersKilledAlb = GetCustomProperty("PlayersKilledAlb") != null
            ? int.Parse(GetCustomProperty("PlayersKilledAlb"))
            : 0;
