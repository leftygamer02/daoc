...
public class HardcoreKillAPlayerHib : Quests.DailyQuest
{
    ...
    protected static void TalkToSucci(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var oranges =
            player.IsDoingQuest(typeof(HardcoreKillAPlayerHib)) as HardcoreKillAPlayerHib;
                ...
                switch (wArgs.Text.ToLower())
                {
                    case "predator":
                        player.Out.SendQuestSubscribeCommand(SucciHib,
                            QuestMgr.GetIDForQuestType(typeof(HardcoreKillAPlayerHib)),
    ...
    public override void Notify(DOLEvent e, object sender, EventArgs args)
    {
        ...
        PlayerKilled = 1;
        player.Out.SendMessage("[Hardcore] Enemy Killed: (" + PlayerKilled + " | " + MAX_KillGoal + ")",
    ...
    {
        var reward = ServerProperties.Properties.HARDCORE_RVR_REWARD;
        ...
        if (reward > 0)
        {
            m_questPlayer.Out.SendMessage(
                $"You have been rewarded {reward} Realmpoints for finishing Hardcore Quest.",
