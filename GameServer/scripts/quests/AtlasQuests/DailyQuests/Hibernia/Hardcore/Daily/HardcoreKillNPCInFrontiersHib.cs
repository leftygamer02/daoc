...
public class HardcoreKillNPCInFrontiersHib : Quests.DailyQuest
{
    ...
    private static void TalkToSucci(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var oranges =
            player.IsDoingQuest(typeof(HardcoreKillNPCInFrontiersHib)) as HardcoreKillNPCInFrontiersHib;
            ...
            if (oranges == null)
                switch (wArgs.Text.ToLower())
                {
                    case "one can be brave":
                        player.Out.SendQuestSubscribeCommand(SucciHib,
                            QuestMgr.GetIDForQuestType(typeof(HardcoreKillNPCInFrontiersHib)),
                ...
                switch (wArgs.Text)
                {
                    case "another sunrise":
                        if (oranges.Step == 2)
                        {
                            player.Out.SendMessage("Tell me, did you face your fears?", eChatType.CT_Chat,
