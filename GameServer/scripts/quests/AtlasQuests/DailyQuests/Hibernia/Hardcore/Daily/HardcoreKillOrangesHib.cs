...
public class HardcoreKillOrangesHib : Quests.DailyQuest
{
    ...
    public static void ScriptLoaded(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var npcs = WorldMgr.GetNPCsByName("Succi", eRealm.Hibernia);
        ...
        if (SucciHib == null)
        {
            ...
            SucciHib.AddToWorld();
            if (SAVE_INTO_DATABASE) SucciHib.SaveIntoDatabase();
        }
    ...
    private static void TalkToSucci(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var oranges =
            player.IsDoingQuest(typeof(HardcoreKillOrangesHib)) as HardcoreKillOrangesHib;
        ...
        if (e == GameObjectEvent.Interact)
        {
            if (oranges != null)
            {
                ...
                {
                    case 1:
                        SucciHib.SayTo(player,
                            "Seek out creatures greater in strength than you and cast them into the abyss.");
                        break;
                    case 2:
            ...
            else
            {
                ...
                SucciHib.SayTo(player,
                    " NOTE: This is a HARDCORE quest. If you die or join a group while doing this quest, it will be aborted automatically.");
            }
        }
            ...
            if (oranges == null)
                switch (wArgs.Text)
                {
                    case "today is not the day":
                        player.Out.SendQuestSubscribeCommand(SucciHib,
                            QuestMgr.GetIDForQuestType(typeof(HardcoreKillOrangesHib)),
                ...
                switch (wArgs.Text)
                {
                    case "another sunrise":
                        if (oranges.Step == 2)
                        {
                            player.Out.SendMessage(
                                "From dust we are born, and to dust we return. Your time will come eventually.",
                    ...
                    case "abort":
                        player.Out.SendCustomDialog(
                            "To face one's own demise is not for the faint of heart. Death has turned its back on you for today.",
    ...
    public override string Description
    {
        get
        {
            ...
            {
                case 1:
                    return "Kill mobs orange con or higher. \n Orange Con Monsters Killed: (" + OrangeConKilled +
                           " | " + MAX_KillGoal + ")";
                case 2:
    ...
    public override void Notify(DOLEvent e, object sender, EventArgs args)
    {
        ...
        OrangeConKilled++;
        player.Out.SendMessage("[Hardcore] Monster Killed: (" + OrangeConKilled + " | " + MAX_KillGoal + ")",
    ...
    {
        OrangeConKilled = GetCustomProperty(QuestPropertyKey) != null
    ...
    public override void FinishQuest()
    {
        m_questPlayer.ForceGainExperience((m_questPlayer.ExperienceForNextLevel -
                                           m_questPlayer.ExperienceForCurrentLevel) / 2);
    ...
    {
        OrangeConKilled = 0;
        m_questPlayer.Out.SendMessage(questTitle + " failed.", eChatType.CT_ScreenCenter_And_CT_System,
