...
public class MonthlyEpicPvEQuestHib : Quests.MonthlyQuest
{
    ...
    public static void ScriptLoaded(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var npcs = WorldMgr.GetNPCsByName("Anthony", eRealm.Hibernia);
        ...
        if (Anthony == null)
        {
            ...
            Anthony.AddToWorld();
            if (SAVE_INTO_DATABASE) Anthony.SaveIntoDatabase();
        }
    ...
    private static void TalkToAnthony(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var quest =
            player.IsDoingQuest(typeof(MonthlyEpicPvEQuestHib)) as MonthlyEpicPvEQuestHib;
        ...
        if (e == GameObjectEvent.Interact)
        {
            if (quest != null)
                switch (quest.Step)
                {
                    case 1:
                        Anthony.SayTo(player,
                            player.Name +
                            ", please find allies and kill the epic creatures in Tur Suil and Fomor!");
                        break;
                    case 2:
                        Anthony.SayTo(player,
                ...
                Anthony.SayTo(player, "Hello " + player.Name +
                                      ", I am Anthony. For several months the situation in Tur Suil and Fomor has changed. " +
                ...
                switch (wArgs.Text)
                {
                    case "kill Balor and Myrddraxis":
                        player.Out.SendQuestSubscribeCommand(Anthony,
                            QuestMgr.GetIDForQuestType(typeof(MonthlyEpicPvEQuestHib)),
    ...
    private static void CheckPlayerAcceptQuest(GamePlayer player, byte response)
    {
        ...
        else
        {
            ...
            Anthony.SayTo(player,
                "Please, find the epic monsters in Tur Suil and Fomor and return for your reward.");
        }
    }
            ...
            switch (Step)
            {
                case 1:
                    return "Make your way and defeat the epic creatures in Tur Suil as well as in Fomor! \n" +
                           ...
                           "Killed: " + Myrddraxis_NAME + " (" + _myrddraxisKilled + " | " + MAX_KILLED +
                           ") in Fomor\n";
                case 2:
    ...
    public override void Notify(DOLEvent e, object sender, EventArgs args)
    {
        ...
        if (gArgs.Target.Name.ToLower() == Balor_NAME.ToLower() && gArgs.Target is GameNPC &&
            _balorKilled < MAX_KILLED)
        {
            _balorKilled = 1;
            player.Out.SendMessage(
                "[Monthly] You killed " + Balor_NAME + ": (" + _balorKilled + " | " + MAX_KILLED + ")",
                eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
            player.Out.SendQuestUpdate(this);
        }
        else if (gArgs.Target.Name.ToLower() == Myrddraxis_NAME.ToLower() && gArgs.Target is GameNPC &&
                 _myrddraxisKilled < MAX_KILLED)
        {
            _myrddraxisKilled = 1;
            player.Out.SendMessage(
                "[Monthly] You killed " + Myrddraxis_NAME + ": (" + _myrddraxisKilled + " | " + MAX_KILLED + ")",
                eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
            player.Out.SendQuestUpdate(this);
        }

        if (_balorKilled >= MAX_KILLED && _myrddraxisKilled >= MAX_KILLED) Step = 2;
    }
    ...
    public override void LoadQuestParameters()
    {
        ...
        _myrddraxisKilled = GetCustomProperty(Myrddraxis_NAME) != null
            ? int.Parse(GetCustomProperty(Myrddraxis_NAME))
            : 0;
    }
