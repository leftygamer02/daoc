...
public class OctonidKillQuestHib : Quests.DailyQuest
{
    ...
    private static void TalkToAnthony(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var quest = player.IsDoingQuest(typeof(OctonidKillQuestHib)) as OctonidKillQuestHib;
        ...
        if (e == GameObjectEvent.Interact)
        {
            if (quest != null)
                ...
                Anthony.SayTo(player, "Hello " + player.Name +
                                      ", I am Anthony. I help the king with logistics, and he's tasked me with getting things done around here. " +
                ...
                switch (wArgs.Text.ToLower())
                {
                    case "clear the octonids":
                        player.Out.SendQuestSubscribeCommand(Anthony,
                            QuestMgr.GetIDForQuestType(typeof(OctonidKillQuestHib)),
    ...
    {
        OctonidKilled = GetCustomProperty(QuestPropertyKey) != null
    ...
    public override void Notify(DOLEvent e, object sender, EventArgs args)
    {
        ...
        OctonidKilled++;
        player.Out.SendMessage("[Daily] Octonid Killed: (" + OctonidKilled + " | " + MAX_KILLED + ")",
            eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
        player.Out.SendQuestUpdate(this);

        if (OctonidKilled >= MAX_KILLED) Step = 2;
    }
