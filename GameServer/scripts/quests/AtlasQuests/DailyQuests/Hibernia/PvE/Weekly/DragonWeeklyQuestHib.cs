...
public class DragonWeeklyQuestHib : Quests.WeeklyQuest
{
    ...
    private static void TalkToDean(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var quest = player.IsDoingQuest(typeof(DragonWeeklyQuestHib)) as DragonWeeklyQuestHib;
        ...
        if (e == GameObjectEvent.Interact)
        {
            if (quest != null)
                switch (quest.Step)
                {
                    case 1:
                        Dean.SayTo(player,
                            player.Name + ", please travel to Sheeroe Hills and kill the dragon for Hibernia!");
                        break;
                    case 2:
                        Dean.SayTo(player,
                ...
                Dean.SayTo(player, "Hello " + player.Name + ", I am Dean. I bring sad news today. " + DRAGON_NAME +
                                   " razed a small settlement in Sheeroe Hills last night. \n" +
                                   "Please, help the king avenge their deaths and keep Hibernia safe from " +
                ...
                switch (wArgs.Text)
                {
                    case "kill the dragon":
                        player.Out.SendQuestSubscribeCommand(Dean,
                            QuestMgr.GetIDForQuestType(typeof(DragonWeeklyQuestHib)),
            ...
            switch (Step)
            {
                case 1:
                    return "Travel to Sheeroe Hills and slay " + DRAGON_NAME + " for Hibernia. \nKilled: " +
                           DRAGON_NAME + " (" + DragonKilled + " | " + MAX_KILLED + ")";
                case 2:
