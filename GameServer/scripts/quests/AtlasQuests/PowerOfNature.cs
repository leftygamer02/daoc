...
public class PowerOfNature : BaseQuest
{
    ...
    public static void ScriptLoaded(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var npcs = WorldMgr.GetNPCsByName("Theresa", eRealm.Hibernia);
        ...
        if (magical_theresas_doll == null)
        {
            ...
            magical_theresas_doll.Description =
                "This magical doll from Karl is a sign of love and is a present for Theresa.";
            if (SAVE_INTO_DATABASE)
                GameServer.Database.AddObject(theresas_doll);
        }
    ...
    protected static void TalkToTheresa(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var quest = player.IsDoingQuest(typeof(PowerOfNature)) as PowerOfNature;
                ...
                switch (wArgs.Text)
                {
                    ...
                    case "begin":
                        if (quest.Step == 3)
                        {
                            ...
                            new ECSGameTimer(Karl,
                                new ECSGameTimer.ECSTimerCallback(timer => StartEffectPlayer(timer, player)), 1000);
                            quest.Step = 4;
