...
public class BeetlePvEQuestHib : Quests.AtlasQuest
{
    ...
    private static void TalkToHarris(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var quest = player.IsDoingQuest(typeof(BeetlePvEQuestHib)) as BeetlePvEQuestHib;
            ...
            if (quest != null)
                switch (quest.Step)
                {
                    case 1:
                        Harris.SayTo(player,
                            player.Name +
                            ", please find the monstrous creatures in Sheeroe Hills, Darkness Falls and Summoner's Hall, erase them and return for your reward.");
                        break;
                    case 2:
                        Harris.SayTo(player,
                            "Hello " + player.Name +
                ...
                Harris.SayTo(player, "Hello " + player.Name +
                                     ", I am Harris. The mission is very dangerous and i'm not as strong as i used to be. " +
                ...
                switch (wArgs.Text)
                {
                    case "help the beetles":
                        player.Out.SendQuestSubscribeCommand(Harris,
                            QuestMgr.GetIDForQuestType(typeof(BeetlePvEQuestHib)),
    ...
    public override void Notify(DOLEvent e, object sender, EventArgs args)
    {
        ...
        if (gArgs.Target.Name.ToLower() == HIB_DRAGON_NAME.ToLower() && gArgs.Target is GameNPC &&
            _dragonKilled < MAX_KILLED)
        {
            _dragonKilled = 1;
            player.Out.SendMessage(
                "[Beetle] You killed " + HIB_DRAGON_NAME + ": (" + _dragonKilled + " | " + MAX_KILLED + ")",
    ...
    {
        _dragonKilled = GetCustomProperty(HIB_DRAGON_NAME) != null
            ? int.Parse(GetCustomProperty(HIB_DRAGON_NAME))
            : 0;
