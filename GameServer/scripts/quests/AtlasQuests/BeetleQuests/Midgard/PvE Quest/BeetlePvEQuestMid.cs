...
public class BeetlePvEQuestMid : Quests.AtlasQuest
{
    ...
    private static void TalkToRengahr(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var quest = player.IsDoingQuest(typeof(BeetlePvEQuestMid)) as BeetlePvEQuestMid;
        ...
        if (e == GameObjectEvent.Interact)
        {
            if (quest != null)
                switch (quest.Step)
                {
                    case 1:
                        Rengahr.SayTo(player,
                            player.Name +
                            ", please find the monstrous creatures in Malmohus, Darkness Falls and Summoner's Hall, erase them and return for your reward.");
                        break;
                    case 2:
                        Rengahr.SayTo(player,
                            "Hello " + player.Name +
                ...
                Rengahr.SayTo(player, "Hello " + player.Name +
                                      ", I am Rengahr. The mission is very dangerous and i'm not as strong as i used to be. " +
                ...
                switch (wArgs.Text)
                {
                    case "help the beetles":
                        player.Out.SendQuestSubscribeCommand(Rengahr,
                            QuestMgr.GetIDForQuestType(typeof(BeetlePvEQuestMid)),
    ...
    public override void Notify(DOLEvent e, object sender, EventArgs args)
    {
        ...
        if (gArgs.Target.Name.ToLower() == MID_DRAGON_NAME.ToLower() && gArgs.Target is GameNPC &&
            _dragonKilled < MAX_KILLED)
        {
            _dragonKilled = 1;
            player.Out.SendMessage(
                "[Beetle] You killed " + MID_DRAGON_NAME + ": (" + _dragonKilled + " | " + MAX_KILLED + ")",
                eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
            player.Out.SendQuestUpdate(this);
        }
        else if (gArgs.Target.Name.ToLower() == LEGION_NAME.ToLower() && gArgs.Target is GameNPC &&
                 _legionKilled < MAX_KILLED)
        {
            _legionKilled = 1;
            player.Out.SendMessage(
                "[Beetle] You killed " + LEGION_NAME + ": (" + _legionKilled + " | " + MAX_KILLED + ")",
                eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
            player.Out.SendQuestUpdate(this);
        }
        else if (gArgs.Target.Name.ToLower() == GRAND_SUMMONER_NAME.ToLower() && gArgs.Target is GameNPC &&
                 _grandSummonerKilled < MAX_KILLED)
        {
            _grandSummonerKilled = 1;
            player.Out.SendMessage(
                "[Beetle] You killed " + GRAND_SUMMONER_NAME + ": (" + _grandSummonerKilled + " | " + MAX_KILLED +
                ")", eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
            player.Out.SendQuestUpdate(this);
        }

        if (_dragonKilled >= MAX_KILLED && _legionKilled >= MAX_KILLED && _grandSummonerKilled >= MAX_KILLED) Step = 2;
    }
    ...
    {
        _dragonKilled = GetCustomProperty(MID_DRAGON_NAME) != null
            ? int.Parse(GetCustomProperty(MID_DRAGON_NAME))
            : 0;
        _legionKilled = GetCustomProperty(LEGION_NAME) != null ? int.Parse(GetCustomProperty(LEGION_NAME)) : 0;
        _grandSummonerKilled = GetCustomProperty(GRAND_SUMMONER_NAME) != null
            ? int.Parse(GetCustomProperty(GRAND_SUMMONER_NAME))
            : 0;
    }
