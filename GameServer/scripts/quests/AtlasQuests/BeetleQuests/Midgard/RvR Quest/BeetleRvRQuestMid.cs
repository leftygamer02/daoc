...
public class BeetleRvRQuestMid : Quests.AtlasQuest
{
    ...
    public static void ScriptLoaded(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var npcs = WorldMgr.GetNPCsByName("Rengahr", eRealm.Midgard);
        ...
        if (Rengahr == null)
        {
            ...
            var templateMid = new GameNpcInventoryTemplate();
            ...
            Rengahr.AddToWorld();
            if (SAVE_INTO_DATABASE) Rengahr.SaveIntoDatabase();
        }
    ...
    private static void TalkToHarris(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var quest = player.IsDoingQuest(typeof(BeetleRvRQuestMid)) as BeetleRvRQuestMid;
                ...
                switch (quest.Step)
                {
                    case 1:
                        Rengahr.SayTo(player,
                            player.Name +
                            ", please find enemies in their, aswell in our lands and kill them. Come back when you also captured keeps and a relic for your reward.");
                        break;
                    case 2:
                        Rengahr.SayTo(player,
                            "Hello " + player.Name +
                            ", I am glad that you are back, the [beetles] will be very happy about this news!");
                        break;
                    case 3:
                        Rengahr.SayTo(player,
                            "The friendly beetle gave me two rewards for you. You can [choose], which one you need the most!");
                        break;
                }
            else
                Rengahr.SayTo(player, "Hello " + player.Name +
                                      ", I am Rengahr. The mission is very dangerous and I hope you can help me and Midgard. " +
            ...
            if (quest == null)
                switch (wArgs.Text)
                {
                    case "help Midgard and the beetles":
                        player.Out.SendQuestSubscribeCommand(Rengahr,
                            QuestMgr.GetIDForQuestType(typeof(BeetleRvRQuestMid)),
                            "Will you help Rengahr " + questTitle + "?");
                        break;
                }
            else
                switch (wArgs.Text)
                {
                    ...
                    case "choose":
                        ...
                        break;
                    case "Beetle Egg":
                        player.Out.SendCustomDialog("Do you really want to have the Beetle Egg?",
                            new CustomDialogResponse(QuestRewardEgg));
                        break;
                    case "Beetle Bone":
                        player.Out.SendCustomDialog("Do you really want to have the Beetle Bone?",
                            new CustomDialogResponse(QuestRewardBone));
                        break;
                    case "abort":
    ...
    private static void QuestRewardEgg(GamePlayer player, byte response)
    {
        ...
        else
        {
            ...
            {
                player.Out.SendMessage("Clear two slots of your inventory for your reward!", eChatType.CT_System,
    ...
    public override string Description
    {
        get
        {
            switch (Step)
            {
                case 1:
                    return
                        "Invade the enemy realm for capturing a relic and slay enemies in the frontiers for Midgard." +
    ...
    public override void Notify(DOLEvent e, object sender, EventArgs args)
    {
        ...
        if (e == GameLivingEvent.EnemyKilled && Step == 1 && _enemiesKilled < MAX_KILLED)
        {
            ...
            _enemiesKilled++;
            player.Out.SendMessage("[Beetle] Enemies Killed: (" + _enemiesKilled + " | " + MAX_KILLED + ")",
        ...
        {
            _captured++;
            player.Out.SendMessage("[Beetle] Captured Keeps: (" + _captured + " | " + MAX_CAPTURED + ")",
                eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
            player.Out.SendQuestUpdate(this);
        }
        ...
        if (e == GamePlayerEvent.CapturedRelicsChanged && Step == 1 && _relicsCaptured < MAX_RELICS_CAPTURED)
        {
            ...
            player.Out.SendMessage(
                "[Beetle] Captured Relics: (" + _relicsCaptured + " | " + MAX_RELICS_CAPTURED + ")",
                eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
            player.Out.SendQuestUpdate(this);
        }

        if (_enemiesKilled >= MAX_KILLED && _captured >= MAX_CAPTURED &&
            _relicsCaptured >= MAX_RELICS_CAPTURED) Step = 2;
    }
    ...
    {
        _enemiesKilled = GetCustomProperty(QuestPropertyKey + "Enemy") != null
            ? int.Parse(GetCustomProperty(QuestPropertyKey + "Enemy"))
            : 0;
        _captured = GetCustomProperty(QuestPropertyKey + "Keep") != null
            ? int.Parse(GetCustomProperty(QuestPropertyKey + "Keep"))
            : 0;
        _relicsCaptured = GetCustomProperty(QuestPropertyKey + "Relic") != null
            ? int.Parse(GetCustomProperty(QuestPropertyKey + "Relic"))
            : 0;
    }
    ...
    {
        var reward = ServerProperties.Properties.BEETLE_RVR_REWARD;
        ...
        if (reward > 0)
        {
            m_questPlayer.Out.SendMessage(
                $"You have been rewarded {reward} Realmpoints for finishing Beetle Quest.", eChatType.CT_Important,
                eChatLoc.CL_SystemWindow);
