...
public class BeetlePvEQuestAlb : Quests.AtlasQuest
{
    ...
    private static void TalkToLaura(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var quest = player.IsDoingQuest(typeof(BeetlePvEQuestAlb)) as BeetlePvEQuestAlb;
            ...
            if (quest != null)
                switch (quest.Step)
                {
                    case 1:
                        Laura.SayTo(player,
                            player.Name +
                            ", please find the monstrous creatures in Dartmoor, Darkness Falls and Summoner's Hall, erase them and return for your reward.");
                        break;
                    case 2:
                        Laura.SayTo(player,
                            "Hello " + player.Name +
                ...
                Laura.SayTo(player, "Hello " + player.Name +
                                    ", I am Laura. The mission is very dangerous and i'm not as strong as i used to be. " +
                ...
                switch (wArgs.Text)
                {
                    case "help the beetles":
                        player.Out.SendQuestSubscribeCommand(Laura,
                            QuestMgr.GetIDForQuestType(typeof(BeetlePvEQuestAlb)),
    ...
    public override void Notify(DOLEvent e, object sender, EventArgs args)
    {
        ...
        if (gArgs.Target.Name.ToLower() == ALB_DRAGON_NAME.ToLower() && gArgs.Target is GameNPC &&
            _dragonKilled < MAX_KILLED)
        {
            _dragonKilled = 1;
            player.Out.SendMessage(
                "[Beetle] You killed " + ALB_DRAGON_NAME + ": (" + _dragonKilled + " | " + MAX_KILLED + ")",
    ...
    {
        _dragonKilled = GetCustomProperty(ALB_DRAGON_NAME) != null
            ? int.Parse(GetCustomProperty(ALB_DRAGON_NAME))
            : 0;
