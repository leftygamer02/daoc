...
public class ImmediateResolutionVB : BaseQuest
{
    ...
    public override void Notify(DOLEvent e, object sender, EventArgs args)
    {
        ...
        if (player.IsDoingQuest(typeof(ImmediateResolutionVB)) == null) return;

        if (Step == 2 && e == GamePlayerEvent.GiveItem)
        {
            var gArgs = (GiveItemEventArgs) args;
            if (gArgs.Target == _gaurdsman1 || gArgs.Target == _gaurdsman2 || gArgs.Target == _gaurdsman3 ||
                gArgs.Target == _gaurdsman4)
                if (gArgs.Item.Id_nb == waxSealedNoteInDb)
                {
                    var selectedGuard = gArgs.Target as GameNPC;
    ...
    public static void ScriptLoaded(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var gameNpcQuery = WorldMgr.GetNPCsByName(masterTorNpc, eRealm.Albion);
        ...
        GameEventMgr.AddHandler(_masterTor, GameObjectEvent.Interact, new DOLEventHandler(TalkToMasterTor));
    ...
    public static void ScriptUnloaded(DOLEvent e, object sender, EventArgs args)
    {
        ...
        GameEventMgr.RemoveHandler(_masterTor, GameObjectEvent.Interact, new DOLEventHandler(TalkToMasterTor));
        GameEventMgr.RemoveHandler(_masterTor, GameLivingEvent.WhisperReceive,
            new DOLEventHandler(TalkToMasterTor));
    ...
    protected static void TalkToStewardWillie(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var quest = player.IsDoingQuest(typeof(ImmediateResolutionVB)) as ImmediateResolutionVB;
        ...
        if (e == GameObjectEvent.Interact)
        {
            if (quest != null)
            {
                _stewardWillie.SayTo(player,
                    "You have been chosen to assist us in delivering [a message]. I'm sure it will be worth your time.");
                return;
            }
        }
        else if (e == GameLivingEvent.WhisperReceive)
        {
            ...
            if (quest != null)
                switch (wArgs.Text)
                {
                    case "a message":
                        _stewardWillie.SayTo(player,
                            "Here! There is a large bridge to the northeast of this keep. It leads to the northern regions and then on into the frontier lands. You will take this message to the guards at the opposite side of the bridge. They are about to be [attacked] so you must hurry!");
                        break;
                    case "attacked":
                        _stewardWillie.SayTo(player,
                            "Barbarians have long attacked our borders! So long as they live, we shall never rest! Now off with ye");
                        if (quest.Step == 1)
                        {
                            quest.Step = 2;
    ...
    protected static void TalkToMasterTor(DOLEvent e, object sender, EventArgs args)
    {
        ...
        if (e == GameObjectEvent.Interact)
        {
            if (quest == null)
            {
                _masterTor.SayTo(player,
                    "Welcome young warrior! It is good that you have decided to take up the fight in Albion. Our borders are in need of [adventurers] such as you");
                return;
            }
        }
        else if (e == GameLivingEvent.WhisperReceive)
        {
            ...
            if (quest == null)
                ...
                {
                    case "adventurers":
                        player.Out.SendQuestSubscribeCommand(_masterTor,
                            QuestMgr.GetIDForQuestType(typeof(ImmediateResolutionVB)), $"Do you accept this task?");
                        break;
                }
            else
                switch (wArgs.Text)
    ...
    public override string Description
    {
        get
        {
            switch (Step)
            {
                ...
                case 2:
                    return
                        "Locate the guards who defend a bridge just east of Humberton. Deliver the wax-sealed note to them. Do not tarry!";
            }
    ...
    public override void FinishQuest()
    {
        ...
        var money = Money.GetMoney(0, 0, 0, 0, 30 + Util.Random(50));
    ...
    {
        if (log.IsWarnEnabled) log.Warn($"Could not find {entity}, cannot load quest: {questTitle}");
    }
