...
public class Church_50 : BaseQuest
{
    ...
    public static void ScriptLoaded(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var npcs = WorldMgr.GetNPCsByName("Roben Fraomar", eRealm.Albion);
        ...
        if (statue_of_arawn == null)
        {
            ...
            if (SAVE_INTO_DATABASE) GameServer.Database.AddObject(statue_of_arawn);
        }
...
// end item
        ...
        if (ClericEpicBoots == null)
        {
            ...
            if (SAVE_INTO_DATABASE) GameServer.Database.AddObject(i);
...
//end item
    ...
    protected static void TalkToRoben(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var quest = player.IsDoingQuest(typeof(Church_50)) as Church_50;
        ...
        if (e == GameObjectEvent.Interact)
        {
            ...
            if (quest == null)
                Roben.SayTo(player,
                    "It appears that those present when the glyph was made whole received a [vision].");
            else
                switch (quest.Step)
                {
                    case 1:
                        Roben.SayTo(player,
                            "You must not let this occur " + player.GetName(0, false) +
                            "! I am familar with [Lyonesse]. I suggest that you gather a strong group of adventurers in order to succeed in this endeavor!");
                        break;
                    case 2:
        ...
        else if (e == GameLivingEvent.WhisperReceive)
        {
            ...
            if (quest == null)
                switch (wArgs.Text)
                {
                    case "vision":
                        Roben.SayTo(player,
                            "They speak of a broken cathedral located within the borders of Lyonesse. The glyph was able to show the new [occupants] of this cathedral.");
                        break;
                    case "occupants":
                        Roben.SayTo(player,
                            "Occupants that worship not the church of Albion, but the dark lord Arawn! Magess Axton requests that you gather a group and destroy the leader of these dark disciples. She believes these worshippers of Arawan strive to [break the light of camelot] and establish their own religion within our realm.");
                        break;
                    case "break the light of camelot":
                        player.Out.SendQuestSubscribeCommand(Roben, QuestMgr.GetIDForQuestType(typeof(Church_50)),
                            "Will you help Roben [Church Level 50 Epic]?");
                        break;
                }
            else
                switch (wArgs.Text)
                {
                    case "Lyonesse":
                        Roben.SayTo(player,
                            "The cathedral that Axton speaks of lies deep at the heart of that land, behind the Pikeman, across from the Trees. Its remaining walls can be seen at great distances during the day so you should not miss it. I would travel with thee, but my services are required elswhere. Fare thee well " +
    ...
    //Set quest name
    public override string Name => "Passage to Eternity (Level 50 Church Epic)";
    ...
    public override string Description
    {
        get
        {
            switch (Step)
            {
                case 1:
                    return
                        "Gather a strong group of adventures and travel to the ancient temple of Arawn. This temple can be found within Lyonesse, surrounded by the dark one's priests. Only by slaying their leader can this evil be stopped!";
                case 2:
            ...
            if (gArgs != null && gArgs.Target != null && Blythe != null)
                if (gArgs.Target.Name == Blythe.Name)
                {
                    m_questPlayer.Out.SendMessage(
                        "As you search the dead body of sister Blythe, you find a sacred " + statue_of_arawn.Name +
                        ", bring it to " + Roben.Name + " has proof of your success.", eChatType.CT_System,
