...
public class Academy_50 : BaseQuest
{
    ...
    private protected int _DemonicMinionsKilled = 0;
    private protected int _BechardKilled = 0;
    private protected int _SilchardeKilled = 0;
    ...
    public static void ScriptLoaded(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var npcs = WorldMgr.GetNPCsByName("Master Ferowl", eRealm.Albion);
...
//				Morgana.AddNPCEquipment((byte) eVisibleItems.BOOT, 133, 61, 0, 0);
    ...
    protected static void TalkToFerowl(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var quest = player.IsDoingQuest(typeof(Academy_50)) as Academy_50;
        ...
        if (e == GameObjectEvent.Interact)
        {
            ...
            if (quest != null)
                switch (quest.Step)
                {
                    case 1:
                        Ferowl.SayTo(player,
                            "Albions fate lies in you hands. Seek out [Morgana] at the fallen tower in Lyonesse!");
                        break;
                    case 2:
                        Ferowl.SayTo(player,
                            "Were you able to [fulfill] your given task? Albions fate lies in you hands. ");
                        break;
                }
            else
                Ferowl.SayTo(player,
                    "Ah good to see you, there are rumors about your tasks all over Albion, yet we are in need of your [services] once again!");
        ...
        else if (e == GameLivingEvent.WhisperReceive)
        {
            ...
            if (quest == null)
                ...
                {
                    case "services":
                        player.Out.SendQuestSubscribeCommand(Ferowl, QuestMgr.GetIDForQuestType(typeof(Academy_50)),
                            "Will you help Ferowl [Academy Level 50 Epic]");
                        break;
                }
            else
                switch (wArgs.Text)
                {
                    case "Morgana":
                        Ferowl.SayTo(player,
                            "You must have heard about her, she's the evil sister of King Arthur, tried to take his throne. She must be [stopped]!");
                        break;
                    case "stopped":
                        Ferowl.SayTo(player,
                            "Once Morgana has summoned her army everything is lost. So hurry and stop her unholy rituals. With the help of two mighty demons Silcharde and Bechard she can summon as many minions as she wants. Killing them should be enough to stop her [ritual].");
                        break;
                    case "ritual":
                        Ferowl.SayTo(player,
                            "Morgana is probably performing her rital at the fallen tower in Lyonesse. To get there follow the Telamon road past the majority of the Danaoian Farmers, until you see the [fallen tower].");
                        break;
                    case "fallen tower":
                        Ferowl.SayTo(player,
                            "Be wise and don't take any unneccessary risks by going directly on Morgana , you might be a strong " +
                            player.CharacterClass.Name +
                            ", but you are no match for Morgana herself. Kill her demons and return to me, we will then try to take care of the rest, once her time has come.");
                        break;

                    // once the deomns are dead:
                    case "fulfill":
                        Ferowl.SayTo(player,
                            "Did you find anything near the fallen tower? If yes [give it to me], we could need any hints we can get on our crusade against Morgana.");
                        break;
                    case "give it to me":
                        if (quest.Step == 2)
                        {
    ...
    private static void CheckPlayerAcceptQuest(GamePlayer player, byte response)
    {
        ...
        else
        {
            ...
            Ferowl.SayTo(player,
                "I have heard rumors about the witch [Morgana] trying to summon an army of demons to crush the mighty city of Camelot!");
        }
    }

    //Set quest name
    public override string Name => "Symbol of the Broken (Level 50 Academy Epic)";
    ...
    public override string Description
    {
        get
        {
            switch (Step)
            {
                case 1:
                    return
                        "Seek out Bechard and Silcharde at the fallen tower in Lyonesse and kill them with rest of summoned demons!\n" +
        ...
        if (Step == 1 && e == GameLivingEvent.EnemyKilled)
        {
            ...
            if (gArgs != null && gArgs.Target != null)
            {
                ...
                if (_BechardKilled >= 1 && _SilchardeKilled >= 1 && _DemonicMinionsKilled >= 20)
                {
                    ...
                    //DeleteMorgana();
                    player.Out.SendMessage("A sense of calm settles about you!", eChatType.CT_Broadcast,
                        eChatLoc.CL_ChatWindow);
                    GiveItem(player, sealed_pouch);
                    m_questPlayer.Out.SendMessage("Take the pouch to " + Ferowl.GetName(0, true),
