...
public class WolfPeltCloak : BaseQuest
{
    ...
    public static void ScriptLoaded(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var gameNpcQuery = WorldMgr.GetNPCsByName(stewardWillieNpcName, eRealm.Albion);
        ...
        GameEventMgr.AddHandler(_stewardWillie, GameObjectEvent.Interact, new DOLEventHandler(TalkToStewardWillie));
        GameEventMgr.AddHandler(_stewardWillie, GameLivingEvent.WhisperReceive,
            new DOLEventHandler(TalkToStewardWillie));
        GameEventMgr.AddHandler(_don, GameObjectEvent.Interact, new DOLEventHandler(TalkToBrotherDon));
        GameEventMgr.AddHandler(_don, GameLivingEvent.WhisperReceive, new DOLEventHandler(TalkToBrotherDon));
        GameEventMgr.AddHandler(_lynett, GameObjectEvent.Interact, new DOLEventHandler(TalkToSeamstressLynnet));
    ...
    public static void ScriptUnloaded(DOLEvent e, object sender, EventArgs args)
    {
        ...
        GameEventMgr.RemoveHandler(_stewardWillie, GameObjectEvent.Interact,
            new DOLEventHandler(TalkToStewardWillie));
        GameEventMgr.RemoveHandler(_stewardWillie, GameLivingEvent.WhisperReceive,
            new DOLEventHandler(TalkToStewardWillie));
        GameEventMgr.RemoveHandler(_don, GameObjectEvent.Interact, new DOLEventHandler(TalkToBrotherDon));
    ...
    protected static void TalkToStewardWillie(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var quest = player.IsDoingQuest(typeof(WolfPeltCloak)) as WolfPeltCloak;
        ...
        if (e == GameObjectEvent.Interact)
        {
            if (quest != null)
            {
                if (player.Inventory.GetFirstItemByID(_wolfFur.Id_nb, eInventorySlot.FirstBackpack,
                        eInventorySlot.LastBackpack) != null)
                    _stewardWillie.SayTo(player,
                        "Ah, well done! His Lordship will be pleased to know there is one less mongrel in the pack! Give me the fur so I can throw it with the others.");
                else if (player.Inventory.GetFirstItemByID(_wolfHeadToken.Id_nb, eInventorySlot.FirstBackpack,
                             eInventorySlot.LastBackpack) != null)
                    _stewardWillie.SayTo(player,
                        "Give the token to Seamstress Lynnet in Ludlow, she'll give ye your reward. Thank ye for your fine services to His Lordship.");
                else
                    _stewardWillie.SayTo(player,
                        "Good! I know we ca'count on ye. I will reward ye for the pelt ye bring me from one of those vile beasts!");
                return;
            }
            else
            {
        ...
        else if (e == GameLivingEvent.WhisperReceive)
        {
            ...
            if (quest == null)
                switch (wArgs.Text)
                {
                    case "problem":
                        _stewardWillie.SayTo(player,
                            "What? Ye haven't heard? Hhhmm, then I wonder if ye would [like to help]");
                        break;
                    case "pack of wolves":
                        _stewardWillie.SayTo(player,
                            "There should be some around the area of this village, take a look near the road to Camelot. Kill any wolf pups you can find, and bring me its fur.");
                        break;
                    case "like to help":
                        _stewardWillie.SayTo(player,
                            "That's wonderful! We've been havin' a serious problem with a [pack of wolves]. His Lordship wants'em eliminated because they have been a-bothering the people here about. His Lordship has authorized me to reward those who [serve him well].");
                        break;
                    case "serve him well":
                        player.Out.SendQuestSubscribeCommand(_stewardWillie,
                            QuestMgr.GetIDForQuestType(typeof(WolfPeltCloak)),
                            "Do you accept the Wolf Pelt Cloak quest?");
                        break;
                }
            else
                switch (wArgs.Text)
    ...
    protected static void TalkToSeamstressLynnet(DOLEvent e, object sender, EventArgs args)
    {
        ...
        if (e == GameObjectEvent.Interact)
            if (quest != null)
                _lynett.SayTo(player,
                    "I hear you have a token for me, as proof of your valuable work for his Lordship. Give it to me and I will reward you.");
    }
    ...
    protected static void TalkToBrotherDon(DOLEvent e, object sender, EventArgs args)
    {
        ...
        if (e == GameObjectEvent.Interact)
        {
            if (player.Inventory.GetFirstItemByID(_wolfPeltCloak.Id_nb, eInventorySlot.FirstBackpack,
                    eInventorySlot.LastBackpack) != null)
                _don.SayTo(player,
                    "Hail! You don't perhaps have one of those fine wolf pelt cloaks? If you no longer have need of it, we could greatly use it at the [orphanage].");
            return;
        }
        else if (e == GameLivingEvent.WhisperReceive)
        {
            ...
            switch (wArgs.Text)
            {
                case "orphanage":
                    _don.SayTo(player,
                        "Why yes, the little ones can get an awful chill during the long cold nights, so the orphanage could use a good [donation] of wolf cloaks. I would take any that you have.");
                    break;
                case "donation":
    ...
    public override void Notify(DOLEvent e, object sender, EventArgs args)
    {
        ...
        else if (Step == 2 && e == GamePlayerEvent.GiveItem)
        {
            ...
            if (gArgs.Target.Name == _stewardWillie.Name && gArgs.Item.Id_nb == _wolfFur.Id_nb)
            {
                ...
                _stewardWillie.SayTo(m_questPlayer,
                    "Take this token from His Lordship. If ye give it to Seamstress Lynnet in Ludlow, she'll give ye your reward. Thank ye for your fine services to His Lordship.");
        ...
        else if (Step == 3 && e == GamePlayerEvent.GiveItem)
        {
            ...
            if (gArgs.Target.Name == _lynett.Name && gArgs.Item.Id_nb == _wolfHeadToken.Id_nb)
            {
                ...
                _lynett.SayTo(player,
                    "Well done! Here's your fine wolf pelt cloak. Wear it with pride knowing you have helped his Lordship.");
                FinishQuest();
                return;
            }
        }
    ...
    public override void FinishQuest()
    {
        ...
        var money = Money.GetMoney(0, 0, 0, 0, 50);
        m_questPlayer.AddMoney(money, "You recieve {0} for your service.");
        InventoryLogging.LogInventoryAction("(QUEST;" + Name + ")", m_questPlayer, eInventoryActionType.Quest,
            money);
    }

    public static void _logReasonQuestCantBeImplemented(string entity)
    {
        if (log.IsWarnEnabled) log.Warn($"Could not find {entity}, cannot load quest: Wolf Pelt Quest");
    }
}