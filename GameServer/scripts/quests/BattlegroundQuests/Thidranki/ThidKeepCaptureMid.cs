...
public class ThidKeepCaptureMid : Quests.DailyQuest
{
    ...
    private static void TalkToHerou(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var quest = player.IsDoingQuest(typeof(ThidKeepCaptureMid)) as ThidKeepCaptureMid;
                ...
                switch (wArgs.Text)
                {
                    case "help a skeleton":
                        player.Out.SendQuestSubscribeCommand(PazzMid,
                            QuestMgr.GetIDForQuestType(typeof(ThidKeepCaptureMid)),
                            "Will you help Pazz with " + questTitle + "");
                        break;
                }
            else
                switch (wArgs.Text)
