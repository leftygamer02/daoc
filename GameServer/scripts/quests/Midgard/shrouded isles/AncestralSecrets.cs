...
public class AncestralSecrets : BaseQuest
{
    ...
    private void AncestralKeeperDying(DOLEvent e, object sender, EventArgs arguments)
    {
        ...
        if (args.Killer is GamePet pet)
            ...
            {
                var pet_owner = pet.Owner as GamePlayer;
                if (pet_owner != null && pet_owner.IsAlive)
                {
                    if (pet_owner.Group != null)
                        foreach (var gpl in
                                 pet_owner.Group.GetPlayersInTheGroup()) //gain credit for every one in petowner grp
    ...
    private static void AdvanceAfterKill(GamePlayer player)
    {
        ...
        SendMessage(player, "You feel the curse lift and the pendant turn into a powerful chain.", 0,
    ...
    private static void PlayerEnterKeeperArea(DOLEvent e, object sender, EventArgs args)
    {
        ...
        if (Monitor.TryEnter(spawnLock))
            try
            {
                ...
                player.Out.SendMessage("Ancestral Keeper ambushes you!", eChatType.CT_ScreenCenter,
    ...
    protected static void TalkToOtaYrling(DOLEvent e, object sender, EventArgs args)
    {
                ...
                switch (quest.Step)
                {
                    case 1:
                        OtaYrling.SayTo(player,
                            "Once densely populated by dwarves, this changed with the [impact] of a meteorite. " +
                            "Wide areas were devastated and forests were set on fire. Even today, the wounds of this event are still clearly visible.");
                        break;
                    case 2:
                        OtaYrling.SayTo(player,
                            "Hey " + player.Name +
                            ", don't listen to Longbeard and his friend Styr, they came from Dellingstad and fled.");

                        var random = Util.Random(0, 3);
                        var message = "";
                        switch (random)
                        {
                            case 0:
                            ...
                            case 1:
                                ...
                                Styr.Emote(eEmote.Laugh);
                                message =
                                    $"Longbeard yells, \"Haha, Styr look at this \"{player.CharacterClass.Name}\"";
                                break;
                            case 2:
                                Longbeard.Emote(eEmote.Laugh);
                                Styr.TurnTo(player);
                                ...
                                message =
                                    $"Styr yells, \"Haha, Longbeard look at this \"{player.CharacterClass.Name}\"";
                                break;
                            case 3:
                                Longbeard.Emote(eEmote.Laugh);
                                Styr.TurnTo(player);
                    ...
                    case 3:
                        OtaYrling.SayTo(player,
                            "Hey " + player.Name +
                            ", please visit Jaklyr in Bjarken and tell him that I sent you, he will understand.");
                        break;
                    case 4:
                        OtaYrling.SayTo(player,
                            "Greetings, thank you for your courage, I am with you mentally. Jaklyr might told you how you find the Delling Crater, right?");
                        break;
                    case 5:
                        OtaYrling.SayTo(player,
                            "God dag my friend, I am happy that you did it, please bring this magical pendant to Jaklyr in Bjarken.");
                        break;
                    case 6:
                        OtaYrling.SayTo(player,
                            "Many Years have passed and you made it, not only me but all of Midgard thanks you! You deserved your [reward]!");
                        break;
                }
            else
        ...
        else if (e == GameLivingEvent.WhisperReceive)
        {
            ...
            if (quest == null)
                ...
                {
                    case "Curse":
                        player.Out.SendQuestSubscribeCommand(OtaYrling,
                            QuestMgr.GetIDForQuestType(typeof(AncestralSecrets)),
                            "Will you help Ota Yrling find [Ancestral Secrets]?");
                        break;
                }
            else
                switch (wArgs.Text)
                {
                    case "impact":
                        OtaYrling.SayTo(player,
                            player.Name + ", we need your help finding [secrets] in the Delling Crater.");
                        break;
                    case "secrets":
                        if (quest.Step == 1)
                        {
                            OtaYrling.SayTo(player,
                                "Please visit Jaklyr in Bjarken and tell him that I se... Oh no Longbeard and his friend Styr...");
                            Longbeard.Yell("Haha, you need help from this " + player.CharacterClass.Name +
                                           " Ota Yrling?");
                            Longbeard.Emote(eEmote.Laugh);
                            Styr.Emote(eEmote.Laugh);
                            quest.Step = 2;
        ...
        else if (e == GameObjectEvent.ReceiveItem)
        {
            ...
            if (quest != null)
                if (rArgs.Item.Id_nb == stone_pendant.Id_nb)
                    if (quest.Step == 6)
                        OtaYrling.SayTo(player,
                            "Many Years have passed and you made it, not only me but all of Midgard thanks you! You have deserved your [reward]!");
        }
    }
        ...
        if (e == GameObjectEvent.Interact)
        {
            if (quest != null)
                switch (quest.Step)
                {
                    case 1:
                        Jaklyr.SayTo(player,
                            "Hello Adventurer, great to see more people in our town. Can I help you?");
                        break;
                    case 2:
                        Jaklyr.SayTo(player, "Hey " + player.CharacterClass.Name +
                                             ", did you hear about two dwarfs who fled from Dellingstad and living now in Aegirhamn? " +
                                             "Ota Yrling said that they are annoying.");
                        break;
                    case 3:
                        Jaklyr.SayTo(player,
                            "Hey " + player.CharacterClass.Name + ", how can I help you? Did someone [sent] you?");
                        break;
                    case 4:
                ...
                switch (wArgs.Text)
                {
                    ...
                    case "Delling Crater":
                        if (quest.Step == 3)
                        {
                            ...
                            {
                                Jaklyr.SayTo(player,
                                    "Head to the Caldera in Delling Crater. Follow the road west and at the crossroads go north towards Delling Crater. " +
                                    "Search for Ancestral Keeper in the crater and kill it. " +
                    ...
                    case "pendant":
                        ...
                        Jaklyr.SayTo(player,
                            "I knew it, the Ancestral Keeper has lost its magic and is now [trapped] in this pendant.");
                    ...
                    case "trapped":
                        ...
                        {
                            Jaklyr.SayTo(player,
                                "Take it back and return to Ota Yrling in Aegirhamn. Bring her the pendant as a gift!");
                            GiveItem(player, stone_pendant);
                            quest.Step = 6;
                        }
                ...
                switch (quest.Step)
                {
                    case 1:
                        Longbeard.SayTo(player,
                            "Yes, I am a Dwarf from Dellingstad, do you have a problem with that?");
                        break;
                    case 2:
                        Longbeard.SayTo(player,
                            "You are kidding me right? Nobody came back, just [don't try] it kid.");
                        break;
                    case 3:
                        Longbeard.SayTo(player,
                            "Hey " + player.CharacterClass.Name +
                            ", have you visited Jaklyr in Bjarken yet? I thought you want to go to Delling Crater.");
                        break;
                    case 4:
                    ...
                    case 5:
                        Longbeard.SayTo(player,
                            "Wow, I really never thought that you will do it. That's great my friend! Does Jaklyr know about it yet?");
                        break;
                    case 6:
                        Longbeard.SayTo(player,
                            "Congratulations " + player.Name + ", you will get your recognition!");
                        break;
                }
            else
        ...
        else if (e == GameLivingEvent.WhisperReceive)
        {
            ...
            if (quest == null)
                ...
                switch (wArgs.Text)
                {
                    ...
                    case "the crater":
                        Longbeard.SayTo(player,
                            "Yeah a crater, many years ago a meteorite fell from the sky. It's right next to Dellingstad, that's why its called Delling Crater." +
                    ...
                    case "challenge":
                        ...
                        {
                            Longbeard.SayTo(player,
                                "Okay Adventurer, I warned you, but if you need help, then visit Jaklyr in Bjarken, he knows as much as I do about this event.\nHa det!");
