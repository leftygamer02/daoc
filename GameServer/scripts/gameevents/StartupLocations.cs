...
public static class StartupLocations
{
    ...
    /// </summary>
    private static readonly List<StartupLocation> m_cachedLocations = new();
        ...
        try
        {
            ...
            if (dbStartupLocation == null)
            {
                log.WarnFormat(
                    ...
                    ch.AccountName, ch.Name, ch.Region, ch.Realm, ch.Class, (eCharacterClass) ch.Class, ch.Race,
                    (eRace) ch.Race, chArgs.GameClient.Version);
            }
            else
            {
                ...
                BindCharacter(ch);
                Console.WriteLine(
                    "startup location: account={0}; char name={1}; region={2}; realm={3}; class={4} ({5}); race={6} ({7}); version={8}",
                    ch.AccountName, ch.Name, ch.Region, ch.Realm, ch.Class, (eCharacterClass) ch.Class, ch.Race,
                    (eRace) ch.Race, chArgs.GameClient.Version);
            }
        }
        catch (Exception e)
        {
            if (log.IsErrorEnabled)
                log.ErrorFormat(
                    ...
                    ch.AccountName, ch.Name, ch.Region, ch.Realm, ch.Class, (eCharacterClass) ch.Class, ch.Race,
                    (eRace) ch.Race, chArgs.GameClient.Version, e);
        }
    }
    ...
    public static IList<StartupLocation> GetAllStartupLocationForCharacter(DOLCharacters ch,
        GameClient.eClientVersion cli)
    {
    ...
    public static StartupLocation GetNonTutorialLocation(GamePlayer player)
    {
        try
        {
            return GetAllStartupLocationForCharacter(
                    player.Client.Account.Characters[player.Client.ActiveCharIndex], player.Client.Version)
                .First(sl => sl.ClientRegionID != TUTORIAL_REGIONID);
        }
        catch
        {
            return null;
