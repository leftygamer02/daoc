...
public static class StartupGuilds
{
    ...
    /// </summary>
    [ServerProperty("startup", "starting_guild", "Starter Guild - Edit this to enable/disable the starter guilds",
        true)]
    ...
    public static void CheckStartupGuilds()
    {
        foreach (eRealm currentRealm in Enum.GetValues(typeof(eRealm)))
        {
            ...
            CheckGuild(currentRealm,
                LanguageMgr.GetTranslation(Properties.SERV_LANGUAGE,
                    string.Format("Guild.StartupGuild.{0}", GlobalConstants.RealmToName(currentRealm))));
        }
    }
    ...
    public static void AddNewbieToStarterGuild(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var account = chArgs.GameClient.Account;


        var guildname = LanguageMgr.GetTranslation(Properties.SERV_LANGUAGE,
            string.Format("Guild.StartupGuild.{0}", GlobalConstants.RealmToName((eRealm) ch.Realm)));
    ...
    private static void CheckGuild(eRealm currentRealm, string guildName)
    {
        ...
        {
            var newguild = GuildMgr.CreateGuild(currentRealm, guildName);
            newguild.Ranks[8].OcHear = true;
            newguild.Motd = LanguageMgr.GetTranslation(Properties.SERV_LANGUAGE,
                "Guild.StartupGuild.Motd");
            newguild.Omotd = LanguageMgr.GetTranslation(Properties.SERV_LANGUAGE,
                "Guild.StartupGuild.Omotd");
            newguild.BonusType = Guild.eBonusType.Experience;
            newguild.BonusStartTime = DateTime.Now;
            newguild.Ranks[8].Title = LanguageMgr.GetTranslation(Properties.SERV_LANGUAGE,
                "Guild.StartupGuild.Title");
