...
public static class LevelUpRespec
{
    ...
    /// </summary>
    [ServerProperty("startup", "give_dol_respec_at_level",
        "What levels does we give a DOL respec ? separated by a semi-colon or a range with a dash (ie 1-5;7;9)",
        "0")]
    ...
    public static void OnLevelUp(DOLEvent e, object sender, EventArgs args)
    {
        ...
        foreach (var str in Util.SplitCSV(GIVE_DOL_RESPEC_AT_LEVEL, true))
        {
            byte level_respec = 0;
            ...
            {
                var oldAmount = player.RespecAmountDOL;
                player.RespecAmountDOL++;

                if (oldAmount != player.RespecAmountDOL)
                    player.Out.SendMessage(
                        string.Format("As you reached level {0}, you are awarded an Atlas (full) respec!",
                            player.Level), eChatType.CT_Important, eChatLoc.CL_SystemWindow);
            }
        }
