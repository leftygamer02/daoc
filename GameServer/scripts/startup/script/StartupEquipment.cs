...
public static class CreationStartupEquipment
{
    ...
    /// </summary>
    [ServerProperty("startup", "enable_free_starter_equipment",
        "Enable Startup Free Equipment gifts imported from StarterEquipment Table", true)]
    ...
    /// Table Cache
    /// </summary>
    private static readonly Dictionary<eCharacterClass, List<ItemTemplate>> m_cachedClassEquipment = new();
    ...
    public static void InitStarterEquipment()
    {
        ...
        foreach (var equipclass in GameServer.Database.SelectAllObjects<StarterEquipment>())
            if (equipclass.Template != null)
            {
                foreach (var classID in Util.SplitCSV(equipclass.Class, true))
                {
                    ...
                    if (int.TryParse(classID, out cId))
                        try
                        {
                            var gameClass = (eCharacterClass) cId;
                        ...
                        catch (Exception e)
                        {
                            if (log.IsWarnEnabled)
                                log.WarnFormat(
                                    ...
                                    equipclass.StarterEquipmentID, equipclass.Class, equipclass.TemplateID, classID,
                                    e);
                        }
                }
            }
            else
            {
                if (log.IsWarnEnabled)
                    log.WarnFormat(
                        "Cannot Find Item Template for Record - ID: {0}, ClassID(s): {1}, Itemtemplate: {2}",
                        equipclass.StarterEquipmentID, equipclass.Class, equipclass.TemplateID);
            }
    }
    ...
    public static void OnCharacterCreation(DOLEvent e, object sender, EventArgs args)
    {
        ...
        try
        {
            ...
            if (m_cachedClassEquipment.ContainsKey((eCharacterClass) ch.Class))
                ...
                foreach (var item in m_cachedClassEquipment
                             .Where(k => k.Key == 0 || k.Key == (eCharacterClass) ch.Class)
                             .SelectMany(kv => kv.Value).OrderBy(it => it.Item_Type))
                {
                    ...
                    foreach (var currentSlot in GameLivingInventory.EQUIP_SLOTS)
                        if ((eInventorySlot) inventoryItem.Item_Type == currentSlot)
                        {
                            ...
                            if (currentSlot == eInventorySlot.LeftHandWeapon &&
                                (eObjectType) inventoryItem.Object_Type != eObjectType.Shield &&
                                !usedSlots.ContainsKey(eInventorySlot.RightHandWeapon))
                            ...
                            if (usedSlots.ContainsKey(chosenSlot))
                            {
                                if (log.IsWarnEnabled)
                                    log.WarnFormat(
                                        "Cannot add Starter Equipment item {0} to class {1} an item is already assigned to this slot! (Added to Backpack...)",
                                        item.Id_nb, ch.Class);
                                break;
                            }
                    ...
                    if (!itemChoosen)
                        ...
                        for (var i = (int) eInventorySlot.FirstBackpack; i < (int) eInventorySlot.LastBackpack; i++)
