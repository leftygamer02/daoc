...
public class PredatorCommandHandler : AbstractCommandHandler, ICommandHandler
{
    ...
    {
        if (IsSpammingCommand(client.Player, "Predator")) return;
        ...
        else if (args[1] == "join")
        {
            ...
            {
                client.Out.SendMessage("The mightiest predators hunt alone! Leave your group to join the hunt.",
                    eChatType.CT_Important,
                    eChatLoc.CL_SystemWindow);
                return;
            }

            var area = client.Player.CurrentZone
                .GetAreasOfSpot(client.Player.X, client.Player.Y, client.Player.Z)
            ...
            {
                client.Out.SendMessage("You must be in an Old Frontiers zone to join the hunt.",
                    eChatType.CT_Important,
                    eChatLoc.CL_SystemWindow);
                return;
            }
