...
public class BountyCommandHandler : AbstractCommandHandler, ICommandHandler
{
    ...
    {
        if (IsSpammingCommand(client.Player, "Bounty")) return;
        ...
        if (args[1] == "add")
        {
            ...
            if (playerLoyalty < minLoyalty)
            {
                client.Out.SendMessage(
                    $"You need to have at least {minLoyalty} days of Realm Loyalty to post a bounty.",
            ...
            if (args.Length == 3)
            {
                if (!int.TryParse(args[2], out amount)) amount = minBountyReward;
