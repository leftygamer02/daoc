...
internal static class ZoneBonusRotator
{
    // Region ID's
    ...
    // RvR Zone ID's
    private static List<int> albionRvRZones = new() {11, 12, 14, 15};
    private static List<int> midgardRvRZones = new() {111, 112, 113, 115};
    private static List<int> hiberniaRvRZones = new() {210, 211, 212, 214};

    // low level zone ids
    private static List<int> albionLowbieZones = new() {0, 1, 52, 8, 6};
    private static List<int> midgardLowbieZones = new() {102, 104, 106, 155};
    private static List<int> hiberniaLowbieZones = new() {201, 202, 203, 206, 182};

    // high level zone ids
    private static List<int> albionHighZones = new() {4, 7, 10, 53, 55, 56, 57};
    private static List<int> midgardHighZones = new() {107, 108, 116, 152, 153, 154, 156, 158};
    private static List<int> hiberniaHighZones = new() {204, 205, 216, 183, 184, 185, 186, 187};

    // PvE Zone ID's
    private static List<int> albionClassicZones = new();
    private static List<int> albionSIZones = new();
    private static List<int> midgardClassicZones = new();
    private static List<int> midgardSIZones = new();
    private static List<int> hiberniaClassicZones = new();
    private static List<int> hiberniaSIZones = new();
    ...
    private static SimpleScheduler scheduler = new();
    ...
    {
        // Get Albion ZoneID's
        foreach (var zone in DOLDB<Zones>.SelectObjects(DB.Column("RegionID").IsEqualTo(ALBION_CLASSIC_ID)))
            if (!albionRvRZones.Contains(zone.ZoneID))
                albionClassicZones.Add(zone.ZoneID);

        foreach (var zone in DOLDB<Zones>.SelectObjects(DB.Column("RegionID").IsEqualTo(ALBION_SI_ID)))
            albionSIZones.Add(zone.ZoneID);

        // Get Midgard ZoneID's
        foreach (var zone in DOLDB<Zones>.SelectObjects(DB.Column("RegionID").IsEqualTo(MIDGARD_CLASSIC_ID)))
            if (!midgardRvRZones.Contains(zone.ZoneID))
                midgardClassicZones.Add(zone.ZoneID);

        foreach (var zone in DOLDB<Zones>.SelectObjects(DB.Column("RegionID").IsEqualTo(MIDGARD_SI_ID)))
            midgardSIZones.Add(zone.ZoneID);

        // Get Hibernia ZoneID's
        foreach (var zone in DOLDB<Zones>.SelectObjects(DB.Column("RegionID").IsEqualTo(HIBERNIA_CLASSIC_ID)))
            if (!hiberniaRvRZones.Contains(zone.ZoneID))
                hiberniaClassicZones.Add(zone.ZoneID);

        foreach (var zone in DOLDB<Zones>.SelectObjects(DB.Column("RegionID").IsEqualTo(HIBERNIA_SI_ID)))
    ...
    internal static int UpdatePvEZones()
    {
        ...
        foreach (var client in WorldMgr.GetAllClients()) TellClient(client);
    ...
    internal static int UpdateRvRZones()
    {
        ...
        {
            case 1:
                foreach (var i in albionRvRZones)
                {
                    var zone = DOLDB<Zones>.SelectObject(DB.Column("ZoneID").IsEqualTo(i));
                ...
                break;
            case 2:
                foreach (var i in midgardRvRZones)
                {
                    var zone = DOLDB<Zones>.SelectObject(DB.Column("ZoneID").IsEqualTo(i));
                    zone.Experience = RvRExperienceBonusAmount;
                    zone.Realmpoints = RPBonusAmount;
                ...
                break;
            case 3:
                foreach (var i in hiberniaRvRZones)
                {
                    var zone = DOLDB<Zones>.SelectObject(DB.Column("ZoneID").IsEqualTo(i));
                    zone.Experience = RvRExperienceBonusAmount;
                    zone.Realmpoints = RPBonusAmount;
    ...
    private static void GetNextRvRZone()
    {
        ...
        */
        ...
        //set new realm
        var rand = Util.Random(2) + 1;
        while (rand == currentRvRRealm) rand = Util.Random(2) + 1;
    ...
    private static void GetNextPvEZones()
    {
        ...
        */
        ...
        var UseClassicMidHighZones = Util.Chance(50);
        ...
        var UseClassicHibHighZones = Util.Chance(50);
    ...
    public static List<string> GetTextList()
    {
        ...
        temp.Add("Classic Zone: " + albDBZone.Name + " " + GetLevelRange(albDBZone.ZoneID) + " (XP +" +
                 albDBZone.Experience + "%)");
        temp.Add("SI Zone: " + albDBZoneSI.Name + " " + GetLevelRange(albDBZoneSI.ZoneID) + " (XP +" +
                 albDBZoneSI.Experience + "%)");
        temp.Add("");
        temp.Add("Current Midgard Zones: ");
        temp.Add("Classic Zone: " + midDBZone.Name + " " + GetLevelRange(midDBZone.ZoneID) + " (XP +" +
                 midDBZone.Experience + "%)");
        temp.Add("SI Zone: " + midDBZoneSI.Name + " " + GetLevelRange(midDBZoneSI.ZoneID) + " (XP +" +
                 midDBZoneSI.Experience + "%)");
        temp.Add("");
        temp.Add("Current Hibernia Zones: ");
        temp.Add("Classic Zone: " + hibDBZone.Name + " " + GetLevelRange(hibDBZone.ZoneID) + " (XP +" +
                 hibDBZone.Experience + "%)");
        temp.Add("SI Zone: " + hibDBZoneSI.Name + " " + GetLevelRange(hibDBZoneSI.ZoneID) + " (XP +" +
                 hibDBZoneSI.Experience + "%)");

        temp.Add("");
        var rvr = _lastRvRChangeTick + RvRTimer - GameLoop.GameLoopTime;
        temp.Add("RvR Time Remaining: " + TimeSpan.FromMilliseconds(rvr).Hours + "h " +
                 TimeSpan.FromMilliseconds(rvr).Minutes + "m " + TimeSpan.FromMilliseconds(rvr).Seconds + "s");

        var pve = _lastPvEChangeTick + PvETimer - GameLoop.GameLoopTime;
        temp.Add("PvE Time Remaining: " + TimeSpan.FromMilliseconds(pve).Hours + "h " +
                 TimeSpan.FromMilliseconds(pve).Minutes + "m " + TimeSpan.FromMilliseconds(pve).Seconds + "s");
