...
public class LaunchRewardNPC : GameNPC
{
    ...
    public override bool Interact(GamePlayer player)
    {
        ...
        if (hasRealmCredit)
        {
            if (!HasCustomParam(player))
            {
                if (player.Level < 50)
                {
                    player.Out.SendMessage(
                        $"For your efforts, the realm of {RealmName(Realm)} has decided to award you with {OrbsReward} Atlas Orbs, congratulations!",
                        eChatType.CT_Say, eChatLoc.CL_PopupWindow);
                    player.Out.SendMessage(
                        "Unfortunately though, the Orbs are too powerful for you at this time. Come back when you reach level 50.\n\n" +
                        "My dear friend Cruella de Ville has something you can have immediately, go find here! She is around here.",
                        eChatType.CT_Say, eChatLoc.CL_PopupWindow);
                    return false;
                }
                ...
                player.Out.SendMessage(
                    ...
                    eChatType.CT_Say, eChatLoc.CL_PopupWindow);
                player.Out.SendMessage(
                    $"Would you like to [receive them] now? \n\n I suggest you storing them in your Account Vault to avoid losing them, I won't be able to give you more.",
                    eChatType.CT_Say, eChatLoc.CL_PopupWindow);
            ...
            player.Out.SendMessage(
                ...
                "I don't have additional rewards for you but don't forget to visit my friend Cruella de Ville.. she can be found around here.",
                eChatType.CT_Say, eChatLoc.CL_PopupWindow);
            return true;
        }
        ...
        {
            player.Out.SendMessage(
                $"As you haven't fought for {RealmName(Realm)}, I don't have any reward for you. \n\n" +
        ...
        player.Out.SendMessage($"You are not eligible for any reward on {RealmName(Realm)} at the moment.",
            eChatType.CT_Say, eChatLoc.CL_PopupWindow);
        ...
        switch (str)
        {
            case "receive them":
                ...
                player.Out.SendMessage($"Here, take {OrbsReward} Atlas Orbs! \n\n" +
                                       $"The whole realm of {RealmName(Realm)} thanks you for your help and wish you luck with your adventures! \n \n" +
                                       "Now, don't forget to visit my friend Cruella de Ville, I'm sure she will have more rewards for you.",
                    eChatType.CT_Say, eChatLoc.CL_PopupWindow);
    ...
    {
        var LaunchQuestParamKey = $"{customKey}{Realm}";
        var HasLaunchQuestParam = DOLDB<AccountXCustomParam>.SelectObject(DB.Column("Name")
            .IsEqualTo(player.Client.Account.Name).And(DB.Column("KeyName").IsEqualTo(LaunchQuestParamKey)));
    ...
    private bool HasCustomParam(GamePlayer player)
    {
        ...
        return DOLDB<AccountXCustomParam>.SelectObject(DB.Column("Name").IsEqualTo(player.Client.Account.Name)
            .And(DB.Column("KeyName").IsEqualTo(LaunchQuestParamKey))) != null;
    }
}