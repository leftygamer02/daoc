...
public static class AtlasEventSettings
{
    ...
    public static int EventRPCap = Properties.EVENT_RPCAP;
    public static int SoloPop = Properties.EVENT_SOLO_POP;
    ...
    public static void OnRPGain(DOLEvent e, object sender, EventArgs args)
    {
        ...
        if (Properties.EVENT_THIDRANKI && p.RealmPoints > EventRPCap)
            switch (p.Realm)
            {
                case eRealm.Albion:
                    p.MoveTo(330, 52759, 39528, 4677, 36);
        ...
        if (Properties.EVENT_TUTORIAL && p.RealmPoints > EventRPCap) p.MoveTo(27, 342521, 385230, 5410, 1756);
    }
    ...
    public static void OnPlayerLogin(DOLEvent e, object sender, EventArgs args)
    {
        ...
        if (p.Client.Account.PrivLevel > 1) return;

        // BG event login checks
        if (Properties.EVENT_THIDRANKI && p.CurrentRegionID != 252)
        {
            switch (p.Realm)
            {
                //case eRealm.Albion:
        ...
        if (Properties.EVENT_TUTORIAL)
        {
            ...
            if (p.Level != EventLvCap) p.Bind(true);
        }
    }
