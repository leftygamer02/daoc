...
public class EventLevelBracketNPC : GameNPC
{
    ...
    public override bool Interact(GamePlayer player)
    {
        ...
        var usedi30 = DOLDB<DOLCharactersXCustomParam>.SelectObject(DB.Column("DOLCharactersObjectId")
            .IsEqualTo(player.ObjectId).And(DB.Column("KeyName").IsEqualTo(customKey)));

        const string customKey2 = "usedi40";
        var usedi40 = DOLDB<DOLCharactersXCustomParam>.SelectObject(DB.Column("DOLCharactersObjectId")
            .IsEqualTo(player.ObjectId).And(DB.Column("KeyName").IsEqualTo(customKey)));
    ...
    public override bool WhisperReceive(GameLiving source, string str)
    {
        ...
        switch (str)
        {
            case "level 30":
                if (player.Level < 30)
                {
                    player.Out.SendMessage("I have given you enough experience to particpate, now go get grinding!",
                        eChatType.CT_Say, eChatLoc.CL_PopupWindow);
                    ...
                    if (usedi30 == null)
                    {
                        var i30char = new DOLCharactersXCustomParam();
