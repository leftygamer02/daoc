...
public class AtlasEventTeleporter : GameNPC
{
    ...
    public override bool Interact(GamePlayer player)
    {
        ...
        player.Out.SendMessage("Hello " + player.Name + "!\n\n" + "Are you ready to [fight]?", eChatType.CT_Say,
            eChatLoc.CL_PopupWindow);
        player.Out.SendMessage(
            "If you need so, I can port you back to your Realm's [testing lobby] or to the [battleground]",
    ...
    public override bool WhisperReceive(GameLiving source, string str)
    {
        ...
        switch (str)
        {
            case "fight":
                ...
                {
                    t.Out.SendMessage(
                        "You must be level " + ServerProperties.Properties.EVENT_LVCAP +
                        " to enter the event, speak with my colleague!", eChatType.CT_System,
                ...
                {
                    t.Out.SendMessage("You need to wait a little longer before porting again.", eChatType.CT_Say,
                        eChatLoc.CL_PopupWindow);
                    return false;
                }
                ...
                if (t.Group == null)
                {
                    ...
                    var randX = Util.Random(223000, 235000);
                    var randY = Util.Random(216000, 227000);
                    var z = 6000;
                ...
                else if (t.Group != null && t.Group.MemberCount > 1)
                {
                    ...
                    foreach (var groupMember in t.Group.GetPlayersInTheGroup())
                        ...
                        if (GetDistanceTo(groupMember) > 5000 && !groupMember.InCombat)
                        {
                            ...
                            t.StartInvulnerabilityTimer(ServerProperties.Properties.TIMER_PVP_TELEPORT * 1000,
                                null);
                            t.MoveTo(groupMember.CurrentRegionID, groupMember.X, groupMember.Y, groupMember.Z,
                                groupMember.Heading);
                            return true;
                        }
                    ...
                    var randX = Util.Random(205000, 253000);
                    var randY = Util.Random(204000, 216000);
                    var z = 9000;
