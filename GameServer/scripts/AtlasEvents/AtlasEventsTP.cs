    ...
    public override bool Interact(GamePlayer player)
    {
        ...
        if (base.CurrentRegionID == 252 || base.CurrentRegionID == 165)
        {
            player.Out.SendMessage(
                "Hello " + player.Name + "!\n\n" +
                "If you need so, I can port you back to your Realm's [event zone]", eChatType.CT_Say,
                eChatLoc.CL_PopupWindow);

            if (WorldMgr.GetAllClientsCount() >= SoloPop && player.Group == null)
                player.Out.SendMessage("\nAdditionally, I can port you to the [solo zone]", eChatType.CT_Say,
                    eChatLoc.CL_PopupWindow);
        ...
        if (player.Level != EventLVCap)
        {
            player.Out.SendMessage(
                "Hello " + player.Name + "!\n\n" +
                "Speak to my Event Level colleague to attain enough experience before joining the Battleground!",
                eChatType.CT_Say, eChatLoc.CL_PopupWindow);
            return true;
        }

        player.Out.SendMessage("Hello " + player.Name + "!\n\n" + "Are you ready to [fight] in Thidranki?",
            eChatType.CT_Say, eChatLoc.CL_PopupWindow);

        if (WorldMgr.GetAllClientsCount() >= SoloPop && player.Group == null)
            player.Out.SendMessage("Additionally, I can port you to the [solo zone]", eChatType.CT_Say,
                eChatLoc.CL_PopupWindow);
