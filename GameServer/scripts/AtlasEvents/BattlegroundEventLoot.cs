...
public class BattlegroundEventLoot : GameNPC
{
    ...
    public override bool Interact(GamePlayer player)
    {
        ...
        if (player.Level < 50)
            player.Out.SendMessage("Come back when you're older, kid.", eChatType.CT_Say, eChatLoc.CL_PopupWindow);

        var realmName = player.Realm.ToString();
        if (realmName.Equals("_FirstPlayerRealm"))
            realmName = "Albion";
        else if (realmName.Equals("_LastPlayerRealm")) realmName = "Hibernia";
        ...
        player.Out.SendMessage("Hello " + player.Name + "! We're happy to see you here, supporting your realm.\n" +
                               "For your efforts, " + realmName +
                               " has procured a [full suit] of equipment and some [gems] to adorn them with. " +
    ...
    public override bool WhisperReceive(GameLiving source, string str)
    {
        ...
        var armorType = GetArmorType(realm, charclass, (byte) player.Level);
        ...
        if (str.Equals("full suit"))
        {
            ...
            var hasFreeArmor = DOLDB<DOLCharactersXCustomParam>.SelectObject(DB.Column("DOLCharactersObjectId")
            ...
            if (hasFreeArmor != null)
            {
                player.Out.SendMessage(
                    "Sorry " + player.Name +
                    ", I don't have enough items left to give you another set.\n\n Go fight for your Realm to get more equipment!",
            ...
            var charFreeEventEquip = new DOLCharactersXCustomParam();
        ...
        else if (str.Equals("weapons"))
        {
            ...
            var hasFreeWeapons = DOLDB<DOLCharactersXCustomParam>.SelectObject(DB.Column("DOLCharactersObjectId")
                .IsEqualTo(player.ObjectId).And(DB.Column("KeyName").IsEqualTo(customKey)));
            ...
            if (hasFreeWeapons != null)
            {
                player.Out.SendMessage(
                    "Sorry " + player.Name +
                    ", I don't have enough weapons left to give you another set.\n\n Go fight for your Realm to get more equipment!",
                    eChatType.CT_Say, eChatLoc.CL_PopupWindow);
                return false;
            }
            ...
            else
            {
                var atlasGem =
                    new List<ItemTemplate>(
                        DOLDB<ItemTemplate>.SelectObjects(DB.Column("Id_nb").IsEqualTo("atlas_gem")));
                InventoryItem invitem = GameInventoryItem.Create<ItemUnique>(atlasGem.FirstOrDefault());
                player.Inventory.AddItem(eInventorySlot.FirstEmptyBackpack, invitem);

                var atlasCloak =
                    new List<ItemTemplate>(
                        DOLDB<ItemTemplate>.SelectObjects(DB.Column("Id_nb").IsEqualTo("atlas_cloak")));
                ...
                var atlasRing =
                    new List<ItemTemplate>(
                        DOLDB<ItemTemplate>.SelectObjects(DB.Column("Id_nb").IsEqualTo("atlas_ring")));
        ...
        else if (str.Equals("coin"))
        {
            ...
            var customKey = moneyKey + player.Realm;
            var hasAccountMoney = DOLDB<AccountXCustomParam>.SelectObject(DB.Column("Name")
                .IsEqualTo(player.Client.Account.Name).And(DB.Column("KeyName").IsEqualTo(customKey)));

            if (hasAccountMoney != null)
            {
                player.Out.SendMessage(
                    "Sorry " + player.Name +
                    ", I don't have enough money left to give you more.\n\n Go fight for your Realm to get some!",
                    eChatType.CT_Say, eChatLoc.CL_PopupWindow);
                return false;
            }

            var charFreeEventMoney = new AccountXCustomParam();
        ...
        else if (str.Equals("Atlas Orbs"))
        {
            ...
            var customKey = orbKey + player.Realm;
            var hasFreeOrbs = DOLDB<AccountXCustomParam>.SelectObject(DB.Column("Name")
                .IsEqualTo(player.Client.Account.Name).And(DB.Column("KeyName").IsEqualTo(customKey)));
            ...
            if (hasFreeOrbs != null)
            {
                player.Out.SendMessage(
                    "Sorry " + player.Name +
                    ", I don't have enough Atlas Orbs left to give you more.\n\n Go fight for your Realm to get some!",
                    eChatType.CT_Say, eChatLoc.CL_PopupWindow);
                return false;
            }
            ...
            if (orbs == null)
            {
                player.Out.SendMessage("Error: Currency ID not found!", eChatType.CT_System,
    ...
    public static void GenerateGems(GamePlayer player)
    {
        ...
        var gemSlots = new List<eInventorySlot>();
        gemSlots.Add(eInventorySlot.Cloak);
        gemSlots.Add(eInventorySlot.Neck);
        gemSlots.Add(eInventorySlot.Waist);
        gemSlots.Add(eInventorySlot.Jewellery);
        ...
        foreach (var islot in gemSlots)
        {
            GeneratedUniqueItem item = null;
            item = new GeneratedUniqueItem(realm, charclass, (byte) (player.Level + freeLootLevelOffset),
                eObjectType.Magical, islot);
            item.AllowAdd = true;
            item.Color = 0;
            item.IsTradable = false;
            item.Price = 1;
    ...
    public static void GenerateWeapon(GameLiving player, eCharacterClass charClass, eObjectType type,
        eInventorySlot invSlot)
    {
        ...
        {
            var shieldSize = GetShieldSizeFromClass(charClass);
            for (var i = 0; i < shieldSize; i++)
            {
                GeneratedUniqueItem item = null;
                item = new GeneratedUniqueItem(realm, charClass, (byte) (player.Level + freeLootLevelOffset), type,
                    invSlot, (eDamageType) i + 1);
                item.AllowAdd = true;
                item.Color = (int) color;
                item.IsTradable = false;
                item.Price = 1;
        ...
        {
            //slash flex
            var dmgTypeItem = new GeneratedUniqueItem(realm, charClass,
                (byte) (player.Level + freeLootLevelOffset), type, invSlot, eDamageType.Slash);
            ...
            //crush flex
            var dmgTypeItem2 = new GeneratedUniqueItem(realm, charClass,
                (byte) (player.Level + freeLootLevelOffset), type, invSlot, eDamageType.Crush);
        ...
        else if (type == eObjectType.TwoHandedWeapon || type == eObjectType.PolearmWeapon ||
                 type == eObjectType.LargeWeapons)
        {
            var endDmgType = 4; //default for all 3, slash/crush/thrust
            if (type == eObjectType.LargeWeapons || realm == eRealm.Midgard) endDmgType = 3; //only slash/crush

            //one for each damage type
            for (var i = 1; i < endDmgType; i++)
            {
                var dmgTypeItem = new GeneratedUniqueItem(realm, charClass,
                    (byte) (player.Level + freeLootLevelOffset), type, invSlot, (eDamageType) i);
                dmgTypeItem.AllowAdd = true;
                dmgTypeItem.Color = (int) color;
                dmgTypeItem.IsTradable = false;
                dmgTypeItem.Price = 1;
    ...
    {
        var weapons = new List<eObjectType>();
    ...
    public static void GenerateArmor(GamePlayer player)
    {
        ...
        var bodySlots = new List<eInventorySlot>();
        ...
        foreach (var islot in bodySlots)
        {
            GeneratedUniqueItem item = null;
            item = new GeneratedUniqueItem(realm, charclass, (byte) (player.Level + freeLootLevelOffset), armorType,
                islot);
            item.AllowAdd = true;
            item.Color = (int) color;
            item.IsTradable = false;
