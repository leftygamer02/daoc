...
public class LordOfBattle : GameTrainingDummy
{
    ...
    public override bool Interact(GamePlayer player)
    {
        ...
        player.Out.SendMessage(
            ...
            "If you desire, I can port you back to your realm's [event zone]", eChatType.CT_Say,
            eChatLoc.CL_PopupWindow);

        if (player.effectListComponent.ContainsEffectForEffectType(eEffect.ResurrectionIllness))
            EffectService.RequestCancelEffect(
                EffectListService.GetEffectOnTarget(player, eEffect.ResurrectionIllness));

        if (player.effectListComponent.ContainsEffectForEffectType(eEffect.RvrResurrectionIllness))
            EffectService.RequestCancelEffect(
                EffectListService.GetEffectOnTarget(player, eEffect.RvrResurrectionIllness));
...
public class LordOfBattleBrain : StandardMobBrain
{
    ...
    public override void Think()
    {
        ...
        foreach (GamePlayer player in Body.GetPlayersInRadius(7000)) playersToKill.Add(player);

        foreach (GamePlayer player in Body.GetPlayersInRadius(2500))
        {
            if (!player.IsAlive && !playersToRez.ContainsKey(player)) playersToRez.Add(player, GameLoop.GameLoopTime);
        ...
        foreach (var deadPlayer in playersToRez.Keys)
