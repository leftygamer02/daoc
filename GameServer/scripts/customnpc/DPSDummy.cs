...
public class DPSDummy : GameTrainingDummy
{
    ...
    public override bool Interact(GamePlayer player)
    {
        ...
        SendReply(player,
            "Hello, you can change my [armor] and [resistances] with ease, you need but ask. Right click me to reset them back to 0. \n\n" +
    ...
    public override bool WhisperReceive(GameLiving source, string text)
    {
        ...
        {
            if (!double.TryParse(splitText[1], out _) || double.Parse(splitText[1]) < 0 ||
                double.Parse(splitText[1]) > 70)
            {
            ...
            {
                case "slash":
                    ApplyBonus(this, eBuffBonusCategory.BaseBuff, (eProperty) eResist.Slash,
                        double.Parse(splitText[1]), 1, false);
                    break;
                case "thrust":
                    ApplyBonus(this, eBuffBonusCategory.BaseBuff, (eProperty) eResist.Thrust,
                        double.Parse(splitText[1]), 1, false);
                    break;
                case "crush":
                    ApplyBonus(this, eBuffBonusCategory.BaseBuff, (eProperty) eResist.Crush,
                        double.Parse(splitText[1]), 1, false);
                    break;
                case "body":
                    ApplyBonus(this, eBuffBonusCategory.BaseBuff, (eProperty) eResist.Body,
                        double.Parse(splitText[1]), 1, false);
                    break;
                case "cold":
                    ApplyBonus(this, eBuffBonusCategory.BaseBuff, (eProperty) eResist.Cold,
                        double.Parse(splitText[1]), 1, false);
                    break;
                case "energy":
                    ApplyBonus(this, eBuffBonusCategory.BaseBuff, (eProperty) eResist.Energy,
                        double.Parse(splitText[1]), 1, false);
                    break;
                case "heat":
                    ApplyBonus(this, eBuffBonusCategory.BaseBuff, (eProperty) eResist.Heat,
                        double.Parse(splitText[1]), 1, false);
                    break;
                case "matter":
                    ApplyBonus(this, eBuffBonusCategory.BaseBuff, (eProperty) eResist.Matter,
                        double.Parse(splitText[1]), 1, false);
                    break;
                case "spirit":
                    ApplyBonus(this, eBuffBonusCategory.BaseBuff, (eProperty) eResist.Spirit,
                        double.Parse(splitText[1]), 1, false);
                    break;
                case "block":
        ...
        else
        {
            switch (splitText[0].ToLower())
            {
                ...
                case "resistances":
                    SendReply(player,
                        "Whisper me the resist type and value you'd like. Example: '/whisper Body 10' will give me +10% Body resist. \n" +
    ...
    private void CreateArmorSetOfType(eObjectType armorType)
    {
        ...
        foreach (var slot in invSlots)
        {
            ...
            ItemBonus[eProperty.ArmorFactor] += invItem.DPS_AF;
        ...
        ItemBonus[eProperty.ArmorAbsorption] += GetAbsorb(armorType);
    ...
    {
        var type = (eObjectType) item.Object_Type;
    ...
    private void ClearAFAndABSBuffs()
    {
        ...
        if (GetModified(eProperty.ArmorFactor) > 0)
            ApplyBonus(this, eBuffBonusCategory.Other, eProperty.ArmorFactor, ItemBonus[eProperty.ArmorFactor], 1,
                true);

        if (GetModified(eProperty.ArmorAbsorption) > 0)
            ApplyBonus(this, eBuffBonusCategory.Other, eProperty.ArmorAbsorption,
                ItemBonus[eProperty.ArmorAbsorption], 1, true);

        if (ItemBonus[eProperty.ArmorFactor] > 0) ItemBonus[eProperty.ArmorFactor] = 0;

        if (ItemBonus[eProperty.ArmorAbsorption] > 0) ItemBonus[eProperty.ArmorAbsorption] = 0;
    }
    ...
    private void ResetResists()
    {
        if (GetResist(eDamageType.Slash) > 0)
            ApplyBonus(this, eBuffBonusCategory.BaseBuff, (eProperty) eResist.Slash, GetResist(eDamageType.Slash),
                1, true);

        if (GetResist(eDamageType.Crush) > 0)
            ApplyBonus(this, eBuffBonusCategory.BaseBuff, (eProperty) eResist.Crush, GetResist(eDamageType.Crush),
                1, true);

        if (GetResist(eDamageType.Thrust) > 0)
            ApplyBonus(this, eBuffBonusCategory.BaseBuff, (eProperty) eResist.Thrust, GetResist(eDamageType.Thrust),
                1, true);

        if (GetResist(eDamageType.Natural) > 0)
            ApplyBonus(this, eBuffBonusCategory.BaseBuff, (eProperty) eResist.Natural,
                GetResist(eDamageType.Natural), 1, true);

        if (GetResist(eDamageType.Body) > 0)
            ApplyBonus(this, eBuffBonusCategory.BaseBuff, (eProperty) eResist.Body, GetResist(eDamageType.Body), 1,
                true);

        if (GetResist(eDamageType.Cold) > 0)
            ApplyBonus(this, eBuffBonusCategory.BaseBuff, (eProperty) eResist.Cold, GetResist(eDamageType.Cold), 1,
                true);

        if (GetResist(eDamageType.Energy) > 0)
            ApplyBonus(this, eBuffBonusCategory.BaseBuff, (eProperty) eResist.Energy, GetResist(eDamageType.Energy),
                1, true);

        if (GetResist(eDamageType.Heat) > 0)
            ApplyBonus(this, eBuffBonusCategory.BaseBuff, (eProperty) eResist.Heat, GetResist(eDamageType.Heat), 1,
                true);

        if (GetResist(eDamageType.Matter) > 0)
            ApplyBonus(this, eBuffBonusCategory.BaseBuff, (eProperty) eResist.Matter, GetResist(eDamageType.Matter),
                1, true);

        if (GetResist(eDamageType.Spirit) > 0)
            ApplyBonus(this, eBuffBonusCategory.BaseBuff, (eProperty) eResist.Spirit, GetResist(eDamageType.Spirit),
                1, true);
    }
    ...
    {
        var slot = (eInventorySlot) item.Item_Type;
        var damage = (eDamageType) item.Type_Damage;
