...
public class LordOfBoss : GameTrainingDummy
{
    ...
    public override bool Interact(GamePlayer player)
    {
        ...
        foreach (var npc in WorldMgr.GetNPCsFromRegion(player.CurrentRegionID))
        {
            if (npc.Brain is LordOfBossBrain || npc.Name.Contains("Council") || npc.Name.Contains("isolationist") ||
                npc.Name.Contains("muryan"))
                continue;
            inFight = true;
        }
        ...
        if (player.Group == null && player.Client.Account.PrivLevel == 1)
        {
            player.Out.SendMessage(
                $"This challenge is too big for a lonely {player.CharacterClass.Name}, assemble a group and come back to me! \n I can port you [back] while you find some companions.",
        ...
        {
            player.Out.SendMessage(
                $"You are not the leader of your group, {player.CharacterClass.Name}. Ask them to come speak with me to start.",
                eChatType.CT_Say, eChatLoc.CL_PopupWindow);
            return false;
        }
        ...
        {
            player.Out.SendMessage(
                $"There's a battle already going on, {player.CharacterClass.Name}. You can't start a new fight yet. \n\n I can also [reset] the arena.",
                eChatType.CT_Say, eChatLoc.CL_PopupWindow);
            return false;
        }

        player.Out.SendMessage(
            "Greetings, " + player.CharacterClass.Name + ".\n\n" + "I hope you're up for a challenge.",
            eChatType.CT_Say, eChatLoc.CL_PopupWindow);

        switch (player.Realm)
        {
            case eRealm._FirstPlayerRealm:
                player.Out.SendMessage("I have some minions from [Caer Sidi] ready for you..", eChatType.CT_Say,
                    eChatLoc.CL_PopupWindow);
                break;
            case eRealm.Midgard:
                player.Out.SendMessage("I have some minions from [Tuscaren Glacier] ready for you..",
                    eChatType.CT_Say, eChatLoc.CL_PopupWindow);
                break;
            case eRealm.Hibernia:
                player.Out.SendMessage("I have some minions from [Galladoria] ready for you..", eChatType.CT_Say,
                    eChatLoc.CL_PopupWindow);
                break;
        }

        player.Out.SendMessage("..as well as many demons from [Darkness Falls].", eChatType.CT_Say,
            eChatLoc.CL_PopupWindow);
        player.Out.SendMessage("If you're not ready, I can also port you [back].", eChatType.CT_Say,
            eChatLoc.CL_PopupWindow);
    ...
    public override bool WhisperReceive(GameLiving source, string str)
    {
        ...
        switch (str.ToLower())
        {
            ...
            case "reset":
                ...
                foreach (var mob in WorldMgr.GetNPCsFromRegion(t.CurrentRegionID))
                {
                    if (mob.Brain is LordOfBossBrain || mob.Name.Contains("Council") ||
                        mob.Name.Contains("isolationist") || mob.Name.Contains("muryan"))
                        continue;
                    mob.Delete();
                }
...
public class LordOfBossBrain : StandardMobBrain
{
    ...
    private Dictionary<GamePlayer, long> playersToRez;
    private List<GamePlayer> playersToKill;
