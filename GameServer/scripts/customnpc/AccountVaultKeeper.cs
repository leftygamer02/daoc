...
public class AccountVaultKeeper : GameNPC
{
    ...
    public override bool Interact(GamePlayer player)
    {
        ...
        var message = $"Greetings {player.Name}, nice meeting you.\n";
        ...
        var vaultItem = GetDummyVaultItem(player);
        var vault = new AccountVault(player, this,
            player.Client.Account.Name + "_" + player.Realm.ToString(), 0, vaultItem);
    ...
    public override bool WhisperReceive(GameLiving source, string text)
    {
        ...
        {
            var vault = new AccountVault(player, this,
                player.Client.Account.Name + "_" + player.Realm.ToString(), 0, GetDummyVaultItem(player));
            player.ActiveInventoryObject = vault;
            player.Out.SendInventoryItemsUpdate(vault.GetClientInventory(player), eInventoryWindowType.HouseVault);
        }
        else if (text == "second")
        {
            var vault = new AccountVault(player, this,
                player.Client.Account.Name + "_" + player.Realm.ToString(), 1, GetDummyVaultItem(player));
            player.ActiveInventoryObject = vault;
            player.Out.SendInventoryItemsUpdate(vault.GetClientInventory(player), eInventoryWindowType.HouseVault);
        }
    ...
    {
        var vaultItem = new ItemTemplate();
...
public class AccountVault : GameHouseVault
{
    ...
    public override bool Interact(GamePlayer player)
    {
        ...
        lock (_vaultLock)
        {
            if (!_observers.ContainsKey(player.Name)) _observers.Add(player.Name, player);
        }
    ...
    public override bool MoveItem(GamePlayer player, ushort fromSlot, ushort toSlot)
    {
        ...
        var fromAccountVault = fromSlot >= (ushort) eInventorySlot.HousingInventory_First &&
                               fromSlot <= (ushort) eInventorySlot.HousingInventory_Last;
        var toAccountVault = toSlot >= (ushort) eInventorySlot.HousingInventory_First &&
                             toSlot <= (ushort) eInventorySlot.HousingInventory_Last;

        if (fromAccountVault == false && toAccountVault == false) return false;
        ...
        if (toAccountVault)
        {
            var item = player.Inventory.GetItem((eInventorySlot) toSlot);
            if (item != null)
        ...
        if (toAccountVault && itemInFromSlot != null && itemInFromSlot.IsTradable == false)
            ...
            {
                player.Out.SendMessage("You can not put this item into an Account Vault!", eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);
                player.Out.SendInventoryItemsUpdate(null);
                return false;
            }
    ...
    {
        return GameServer.Database.SelectObjects<InventoryItem>(DB.Column("OwnerID").IsEqualTo(GetOwner(player))
