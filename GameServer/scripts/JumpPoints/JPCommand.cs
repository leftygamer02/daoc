...
public class OnJumpPoint : AbstractCommandHandler, ICommandHandler
{
    public void OnCommand(GameClient client, string[] args)
    {
        ...
        {
            case "add":
                AddJumpPoint(client, args);
                break;
            case "list":
                ListJumpPoints(client);
                break;
            case "port":
                PortToJumpPoint(client, args);
                break;
            case "remove":
                RemoveJumpPoint(client, args);
                break;
            default:
                DisplaySyntax(client);
                break;
    ...
    private void AddJumpPoint(GameClient client, string[] args)
    {
        ...
        var p = GameServer.Database.SelectObjects<DBJumpPoint>(DB.Column("Name").IsEqualTo(args[2]))
            .FirstOrDefault();
    ...
    private void ListJumpPoints(GameClient client)
    {
        ...
        foreach (var p in col) SendSystemMessage(client, p.Name);
    }
    ...
    private void PortToJumpPoint(GameClient client, string[] args)
    {
        ...
        {
            var p = GameServer.Database.SelectObjects<DBJumpPoint>(DB.Column("Name").IsEqualTo(args[3]))
                .FirstOrDefault();
    ...
    private bool CheckExpansion(GameClient clientJumper, GameClient clientJumpee, ushort RegionID)
    {
        ...
        if (reg != null && reg.Expansion > (int) clientJumpee.ClientType)
        {
            clientJumper.Out.SendMessage(
                clientJumpee.Player.Name + " cannot jump to Destination region (" + reg.Description +
                ") because it is not supported by your client type.", eChatType.CT_System,
