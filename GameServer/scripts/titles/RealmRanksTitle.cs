...
public abstract class RealmGenericEventPlayerTitle : GenericEventPlayerTitle
{
    ...
    protected override Tuple<string, string, string, string> GenericNames
    {
        get
        {
            ...
            var male = string.Format("GamePlayer.RealmTitle.{0}.RR{1}.Male", realm, RRLevel);
            var female = string.Format("GamePlayer.RealmTitle.{0}.RR{1}.Female", realm, RRLevel);
            ;
    ...
    {
        get { return p => p.Realm == Realm && p.RealmLevel / 10 + 1 == RRLevel; }
    }

    /// <summary>
    /// Should this Title go through Translator
    /// </summary>
    protected override bool Translate => true;

    /// <summary>
    /// The event to hook.
    /// </summary>
    public override DOLEvent Event => GamePlayerEvent.RRLevelUp;
...
{
    protected override eRealm Realm => eRealm.Midgard;
}
...
{
    protected override eRealm Realm => eRealm.Hibernia;
}
