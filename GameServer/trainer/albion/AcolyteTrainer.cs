...
/// Acolyte Trainer
/// </summary>
[NPCGuildScript("Acolyte Trainer",
    eRealm.Albion)] // this attribute instructs DOL to use this script for all "Acolyte Trainer" NPC's in Albion (multiple guilds are possible for one script)
        ...
        if (player.CharacterClass.ID == (int) TrainedClass)
        {
            ...
            if (player.Level >= 5)
                player.Out.SendMessage(
                    Name +
                    " says, \"You must now seek your training elsewhere. Which path would you like to follow? [Cleric], [Heretic], or [Friar]?\"",
        ...
        switch (text)
        {
            case "Cleric":
                if (player.Race == (int) eRace.Avalonian || player.Race == (int) eRace.Briton ||
                    ...
                    SayTo(player,
                        "So, you wish to serve the Church as healer, defender and leader of our faith. The Church of Albion will welcome one of your skill. Perhaps in time, your commitment will lead others to join our order.");
                else
                    SayTo(player,
                        "The path of a Cleric is not available to your race. Please choose another.");

                return true;
            case "Friar":
                if (player.Race == (int) eRace.Briton)
                    SayTo(player,
                        "Members of a brotherhood, you will find more than a community should you join ranks with the Defenders of Albion. Deadly with a Quarterstaff, and proficient with the healing of wounds, the army is in constant need of new recruits such as you.");
                else
