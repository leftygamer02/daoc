...
[NPCGuildScript("Rogue Trainer",
    eRealm.Midgard)] // this attribute instructs DOL to use this script for all "Rogue Trainer" NPC's in Midgard (multiple guilds are possible for one script)
        ...
        if (player.CharacterClass.ID == (int) TrainedClass)
        {
            ...
            if (player.Level >= 5)
                player.Out.SendMessage(
                    Name +
                    " says, \"You must now seek your training elsewhere. Which path would you like to follow? [Hunter] or [Shadowblade]?\"",
        ...
        switch (text)
        {
            case "Hunter":
                if (player.Race == (int) eRace.Dwarf || player.Race == (int) eRace.Kobold ||
                    player.Race == (int) eRace.Frostalf || player.Race == (int) eRace.Norseman ||
                    ...
                    player.Out.SendMessage(
                        Name +
                        " says, \"The path of an Hunter is not available to your race. Please choose another.\"",
                        eChatType.CT_Say, eChatLoc.CL_PopupWindow);

                return true;
            case "Shadowblade":
                if (player.Race == (int) eRace.Kobold || player.Race == (int) eRace.Norseman ||
                    player.Race == (int) eRace.Valkyn)
                    player.Out.SendMessage(Name + " says, \"I can't tell you something about this class.\"",
                        eChatType.CT_Say, eChatLoc.CL_PopupWindow);
                else
                    player.Out.SendMessage(
                        Name +
                        " says, \"The path of a Shadowblade is not available to your race. Please choose another.\"",
                        eChatType.CT_Say, eChatLoc.CL_PopupWindow);
