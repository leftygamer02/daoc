...
internal class ApiHost
{
    public ApiHost()
    {
        ...
        api.MapGet("/player/{playerName}/specs",
            async c => await c.Response.WriteAsJsonAsync(
                _player.GetPlayerSpec(c.Request.RouteValues["playerName"].ToString())));

        api.MapGet("/player/{playerName}/tradeskills",
            async c => await c.Response.WriteAsJsonAsync(
                _player.GetPlayerTradeSkills(c.Request.RouteValues["playerName"].ToString())));
        ...
        api.MapGet("/guild/{guildName}", (string guildName) =>
        {
            ...
            if (guildInfo == null) return Results.NotFound($"Guild {guildName} not found");
        ...
        {
            if (realmName == null) return Results.NotFound();
            ...
            var realmInfo = _realm.GetKeepsByRealm(realm);
