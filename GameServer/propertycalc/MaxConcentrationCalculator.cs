...
public class MaxConcentrationCalculator : PropertyCalculator
{
    ...
    public override int CalcValue(GameLiving living, eProperty property)
    {
        if (living is GamePlayer)
        {
            ...
            var concBase = (int) (player.Level * 4 * 2.2);
            var stat = player.GetModified((eProperty) player.CharacterClass.ManaStat);
            var statConc = (stat - 50) * 2.8;
            //int factor = (stat > 50) ? (stat - 50) / 2 : (stat - 50);
            //int conc = (concBase + concBase * factor / 100) / 2;
            var conc = (concBase + (int) statConc) / 2;
            ...
            if (conc < 0)
            {
                if (log.IsWarnEnabled)
                    log.WarnFormat(
                        living.Name +
                        ": concentration is less than zerro (conc:{0} eff:{1:R} concBase:{2} stat:{3}})", conc,
                        player.Effectiveness, concBase, stat);
                conc = 0;
            }
