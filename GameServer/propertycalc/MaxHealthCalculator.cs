...
public class MaxHealthCalculator : PropertyCalculator
{
    ...
    public override int CalcValue(GameLiving living, eProperty property)
    {
        if (living is GamePlayer)
        {
            ...
            var hpBase = player.CalculateMaxHealth(player.Level, player.GetModified(eProperty.Constitution));
            ...
            var cap = Math.Max(player.Level * 4, 20) +
            ...
            {
                var levelbonus = Math.Min(player.Level - 40, 10);
                hpBase = (int) (hpBase * (100 + levelbonus) * 0.01);
            }

            var abilityBonus = living.AbilityBonus[(int) property];

            var toughness = player.GetAbility<AtlasOF_ToughnessAbility>();
            var toughnessMod = toughness != null ? 1 + toughness.GetAmountForLevel(toughness.Level) * 0.01 : 1;

            return Math.Max((int) (hpBase * toughnessMod) + itemBonus + buffBonus + abilityBonus, 1);
        }
        else if (living is GameKeepComponent)
        {
            var gameKeep = (living as GameKeepComponent)?.Keep;

            if (gameKeep != null)
            {
                var baseHealth = gameKeep.BaseLevel * Properties.KEEP_COMPONENTS_BASE_HEALTH;
                baseHealth += (int) (baseHealth * (gameKeep.Level - 1) *
                                     Properties.KEEP_COMPONENTS_HEALTH_UPGRADE_MODIFIER);
                return baseHealth;
            }
        ...
        {
            var gameKeep = (living as GameKeepDoor)?.Component?.Keep;
            ...
            if (gameKeep != null)
            {
                ...
                var baseHealth = gameKeep.BaseLevel * Properties.KEEP_DOORS_BASE_HEALTH;
                baseHealth += (int) (baseHealth * (gameKeep.Level - 1) *
                                     Properties.KEEP_DOORS_HEALTH_UPGRADE_MODIFIER);
                return baseHealth;
            }
        ...
        else if (living is TheurgistPet theu)
        {
            ...
            */
            ...
            if (theu.Name.Contains("air"))
                ...
                hp = 500;
            else if (theu.Name.Contains("earth")) hp = 350;
        ...
        else if (living is TurretPet ani)
        {
            ...
            */
            ...
            if (living.Level < 10)
            {
                hp = living.Level * 20 + 20 + ani.Constitution +
                     living.BaseBuffBonusCategory[(int) property]; // default
            }
            else
            {
                // approx to original formula, thx to mathematica :)
                hp = (int) (50 + 14 * living.Level + 0.548331 * living.Level) + ani.Constitution +
                     living.BaseBuffBonusCategory[(int) property];
                if (living.Level < 25)
                    hp += 20;
            }

            if (ani.Brain != null && ani.Brain is AI.Brain.TurretFNFBrain)
                hp = (int) (hp * .8);
        ...
        else if (living is GamePet pet)
        {
            ...
            if (living.Level < 10)
            {
                hp = living.Level * 20 + 20 + pet.Constitution +
                     living.BaseBuffBonusCategory[(int) property]; // default
            }
            else
            {
                // approx to original formula, thx to mathematica :)
                hp = (int) (50 + 15 * living.Level + 0.548331 * living.Level * living.Level) + pet.Constitution +
                     living.BaseBuffBonusCategory[(int) property];
                if (living.Level < 25)
                    hp += 20;
            }
            ...
            var conmod = 20; // at level 50 +75 con ~= +300 hit points

            // first adjust hitpoints based on base CON

            if (basecon != Properties.GAMENPC_BASE_CON)
                hp = Math.Max(1,
                    hp + (basecon - Properties.GAMENPC_BASE_CON) *
                    Properties.GAMENPC_HP_GAIN_PER_CON);
            ...
            var conhp = hp + conmod * living.Level * (living.GetModified(eProperty.Constitution) - basecon) / 250;
        ...
        else if (living is GameNPC)
        {
            ...
            if (living.Level < 20)
            {
                ...
                hp = (int) (living.Level * 11 + 20 +
                            Math.Floor((double) ((living as GameNPC).Constitution * living.Level) /
                                       (1 + (20 - living.Level)))) /*living.BaseBuffBonusCategory[(int)property]*/
                    ; // default
            }
            else
            {
                ...
                // approx to original formula, thx to mathematica :)
                hp = (int) (50 + 12 * living.Level + levelScalar * living.Level * living.Level) +
                     (living as GameNPC).Constitution;
                if (living.Level < 25)
                    hp += 20;
            }

            int basecon = (living as GameNPC).Constitution;
            var conmod = 25; // at level 50 +75 con ~= +300 hit points

            // first adjust hitpoints based on base CON

            if (basecon != Properties.GAMENPC_BASE_CON && living.Level >= 10)
                hp = Math.Max(1,
                    hp + (basecon - Properties.GAMENPC_BASE_CON) *
                    Properties.GAMENPC_HP_GAIN_PER_CON);
            ...
            if (living.Level > 10)
            {
                // adjust hit points based on constitution difference from base con
                // modified from http://www.btinternet.com/~challand/hp_calculator.htm
                var conhp = hp + conmod * living.Level * (living.GetModified(eProperty.Constitution) - basecon) /
                    250;
        ...
        else
        {
            if (living.Level < 10)
            {
                return living.Level * 20 + 20 +
                       living.GetBaseStat(eStat.CON) /*living.BaseBuffBonusCategory[(int)property]*/; // default
            }
            else
            {
                // approx to original formula, thx to mathematica :)
                var hp = (int) (50 + 11 * living.Level + 0.548331 * living.Level * living.Level) +
                         living.GetBaseStat(eStat.CON) /*living.BaseBuffBonusCategory[(int)property]*/;
                if (living.Level < 25) hp += 20;
    ...
    {
        if (living == null) return 0;
        var itemBonusCapIncreaseCap = GetItemBonusCapIncreaseCap(living);
        var itemBonusCapIncrease = living.ItemBonus[(int) eProperty.MaxHealthCapBonus];
