...
public class CriticalArcheryHitChanceCalculator : PropertyCalculator
{
    ...
    {
        var chance = living.BuffBonusCategory4[(int) property] + living.AbilityBonus[(int) property];

        //Volley effect apply crit chance during volley effect
        var volley = EffectListService.GetEffectOnTarget(living, eEffect.Volley);
        if (living is GamePlayer archer && volley != null)
        {
            chance += 10;
            if (archer.GetAbility<RealmAbilities.AtlasOF_FalconsEye>() is RealmAbilities.AtlasOF_FalconsEye
                falcon_eye)
        ...
        if (living is GamePet gamePet)
        {
            if (ServerProperties.Properties.EXPAND_WILD_MINION && gamePet.Brain is IControlledBrain playerBrain
                                                               ...
                                                               && player
                                                                       .GetAbility<
                                                                           RealmAbilities.
                                                                           AtlasOF_WildMinionAbility>()
                                                                   is RealmAbilities.AtlasOF_WildMinionAbility ab)
                chance += ab.Amount;
        }
        else // not a pet
        {
