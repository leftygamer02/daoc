...
//			DOLConsole.WriteSystem("calc skill prop "+property+":");
        if (living is GamePlayer)
        {
            ...
            var itemCap = player.Level / 5 + 1;

            var itemBonus = player.ItemBonus[(int) property];
            ...
            if (itemBonus > itemCap)
                itemBonus = itemCap;
            var buffs = player.BaseBuffBonusCategory[(int) property]; // one buff category just in case..
