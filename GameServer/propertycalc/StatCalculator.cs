...
public class StatCalculator : PropertyCalculator
{
    ...
    public override int CalcValue(GameLiving living, eProperty property)
    {
        ...
        var baseStat = living.GetBaseStat((eStat) property);
        var abilityBonus = living.AbilityBonus[propertyIndex];
        var debuff = living.DebuffCategory[propertyIndex];
        var deathConDebuff = 0;
        ...
        if (living is GamePlayer)
        {
            ...
            if (property == (eProperty) player.CharacterClass.ManaStat)
                if (player.CharacterClass.ID != (int) eCharacterClass.Scout &&
                    player.CharacterClass.ID != (int) eCharacterClass.Hunter &&
        ...
        var unbuffedBonus = baseStat + itemBonus;
        ...
        var stat = unbuffedBonus + buffBonus + abilityBonus;
    ...
    public override int CalcValueFromBuffs(GameLiving living, eProperty property)
    {
        ...
        var propertyIndex = (int) property;
        var baseBuffBonus = living.BaseBuffBonusCategory[propertyIndex];
        var specBuffBonus = living.SpecBuffBonusCategory[propertyIndex];
        ...
        var baseBuffBonusCap = living is GamePlayer ? (int) (living.Level * 1.25) : short.MaxValue;
        var specBuffBonusCap = living is GamePlayer ? (int) (living.Level * 1.5 * 1.25) : short.MaxValue;
    ...
    public override int CalcValueFromItems(GameLiving living, eProperty property)
    {
        ...
        var itemBonusCap = GetItemBonusCap(living, property);
        ...
        var itemBonusCapIncrease = GetItemBonusCapIncrease(living, property);
        var mythicalitemBonusCapIncrease = GetMythicalItemBonusCapIncrease(living, property);
    ...
    public static int GetItemBonusCapIncrease(GameLiving living, eProperty property)
    {
        ...
        var itemBonusCapIncrease =
            living.ItemBonus[(int) (eProperty.StatCapBonus_First - eProperty.Stat_First + property)];
        if (living is GamePlayer)
        {
            var player = living as GamePlayer;
    ...
    {
        if (living == null) return 0;
        var MythicalitemBonusCapIncreaseCap = GetMythicalItemBonusCapIncreaseCap(living);
        var MythicalitemBonusCapIncrease =
            living.ItemBonus[(int) (eProperty.MythicalStatCapBonus_First - eProperty.Stat_First + property)];
        var itemBonusCapIncrease = GetItemBonusCapIncrease(living, property);
        if (living is GamePlayer)
        {
            var player = living as GamePlayer;
