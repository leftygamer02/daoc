...
public class ParryChanceCalculator : PropertyCalculator
{
    public override int CalcValue(GameLiving living, eProperty property)
    {
        ...
        if (player != null)
        {
            var buff = player.BaseBuffBonusCategory[(int) property] * 10
            ...
            if (player.HasSpecialization(Specs.Parry))
                parrySpec = (player.Dexterity * 2 - 100) / 4 +
                            (player.GetModifiedSpecLevel(Specs.Parry) - 1) * (10 / 2) + 50;

            if (parrySpec > 500) parrySpec = 500;

            return parrySpec + buff;
        }

        var pet = living as NecromancerPet;
        if (pet != null)
        {
            var brain = pet.Brain as IControlledBrain;
            if (brain != null)
            {
                var buff = pet.BaseBuffBonusCategory[(int) property] * 10
        ...
        if (npc != null) return npc.ParryChance * 10;
