...
public class MaxManaCalculator : PropertyCalculator
{
    ...
    public override int CalcValue(GameLiving living, eProperty property)
    {
        if (living is GamePlayer)
        {
            ...
            var manaStat = player.CharacterClass.ManaStat;
            ...
            var manaBase = player.CalculateMaxMana(player.Level, player.GetModified((eProperty) manaStat));
            var itemBonus = living.ItemBonus[(int) property];
            var poolBonus = living.ItemBonus[(int) eProperty.PowerPool];
            var abilityBonus = living.AbilityBonus[(int) property];

            var itemCap = player.Level / 2 + 1;
            var poolCap = player.Level / 2;
            ...
            if (itemBonus > itemCap) itemBonus = itemCap;
            ...
            return (int) (manaBase + itemBonus + abilityBonus +
                          (manaBase + itemBonus + abilityBonus) * poolBonus * 0.01);
        }
        else
        {
