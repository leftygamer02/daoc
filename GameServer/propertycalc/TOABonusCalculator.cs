...
public class BuffEffectivenessPercentCalculator : PropertyCalculator
{
    public override int CalcValue(GameLiving living, eProperty property)
    {
        ...
        return Math.Min(25,
            livingToCheck.ItemBonus[(int) property] + livingToCheck.AbilityBonus[(int) property] -
            livingToCheck.DebuffCategory[(int) property]);
    }
}
...
public class HealingEffectivenessPercentCalculator : PropertyCalculator
{
    ...
    {
        // Hardcap at 25%
        var percent = Math.Min(25, living.BaseBuffBonusCategory[(int) property]
...
public class CriticalHealHitChanceCalculator : PropertyCalculator
{
    ...
    public override int CalcValue(GameLiving living, eProperty property)
    {
        var percent = living.AbilityBonus[(int) property];
...
public class SpellCastSpeedPercentCalculator : PropertyCalculator
{
    public override int CalcValue(GameLiving living, eProperty property)
    {
        ...
        return Math.Min(15,
            livingToCheck.AbilityBonus[(int) property] - livingToCheck.DebuffCategory[(int) property]);
...
public class SpellDamagePercentCalculator : PropertyCalculator
{
    ...
    {
        // Hardcap at 10%
        var percent = Math.Min(10, living.BaseBuffBonusCategory[(int) property]
                                   + living.ItemBonus[(int) property]
                                   - living.DebuffCategory[(int) property]);
