...
public class GameClient : BaseClient, ICustomParamsValuable
{
    ...
    /// </summary>
    protected Dictionary<string, List<string>> m_customParams = new();
    ...
    public bool CanSendTooltip(int type, int id)
    {
        ...
        // Queries cleanup
        foreach (var keys in m_tooltipRequestTimes.SelectMany(e =>
                     e.Value.Where(it => it.Value < GameTimer.GetTickCount())
                         .Select(el => new Tuple<int, int>(e.Key, el.Key))))
        {
            long dummy;
    ...
    public bool IsPlaying =>
        ...
        m_clientState == eClientState.Playing || m_clientState == eClientState.Linkdead;
    ...
    public Account Account
    {
        ...
        set
        {
            m_account = value;
            // Load Custom Params
            this.InitFromCollection<AccountXCustomParam>(value.CustomParams, param => param.KeyName,
                param => param.Value);
    ...
    public GamePlayer Player
    {
        ...
        set
        {
            var oldPlayer = Interlocked.Exchange(ref m_player, value);
            if (oldPlayer != null) oldPlayer.Delete();
    ...
    public Dictionary<string, List<string>> CustomParamsDictionary
    {
        ...
        set
        {
            Account.CustomParams =
                value.SelectMany(kv => kv.Value.Select(val => new AccountXCustomParam(Account.Name, kv.Key, val)))
                    .ToArray();
    ...
    /// </summary>
    public ReaderWriterDictionary<Tuple<ushort, ushort>, long> GameObjectUpdateArray => m_GameObjectUpdateArray;

    /// <summary>
    /// Get the House Update Array (Read/Write)
    /// </summary>
    public ReaderWriterDictionary<Tuple<ushort, ushort>, long> HouseUpdateArray => m_HouseUpdateArray;
    ...
    protected override void OnReceive(int numBytes)
    {
        ...
        if (Version == eClientVersion.VersionNotChecked)
        {
            ...
            eClientVersion ver;
            var lib = AbstractPacketLib.CreatePacketLibForVersion(version, this, out ver);
        ...
        if (Version != eClientVersion.VersionUnknown) m_packetProcessor.ReceiveBytes(numBytes);
    }
    ...
    public override void OnDisconnect()
    {
        ...
        try
        {
            ...
            if (ClientState == eClientState.WorldEnter && Player != null) Player.SaveIntoDatabase();
        }
        catch (Exception e)
        {
    ...
    public void LoadPlayer(int accountindex, string playerClass)
    {
        ...
        var dolChar = m_account.Characters[accountindex];
    ...
    public void LoadPlayer(DOLCharacters dolChar, string playerClass)
    {
        ...
        try
        {
            player = (GamePlayer) gasm.CreateInstance(playerClass, false, BindingFlags.CreateInstance, null,
                new object[] {this, dolChar}, null, null);
        }
        catch (Exception e)
        {
            if (log.IsErrorEnabled)
        ...
        if (player == null)
            foreach (var asm in ScriptMgr.Scripts)
            {
                try
                {
                    player = (GamePlayer) asm.CreateInstance(playerClass, false, BindingFlags.CreateInstance, null,
                        new object[] {this, dolChar}, null, null);
                }
                catch (Exception e)
                {
        ...
        try
        {
            if (m_player != null)
            {
                ...
                if (Properties.KICK_IDLE_PLAYER_STATUS)
                {
                    ...
                    int[] currentloc =
                        {m_player.X, m_player.Y, m_player.Z, m_player.Heading, m_player.CurrentRegionID};
                    //Compapre Old and Current.
                    var check = oldloc.SequenceEqual(currentloc);
                    //If match
                    if (check)
                    {
    ...
    protected internal void Quit()
    {
        lock (this)
        {
            try
            {
                var oldClientState = ClientState;
                if (m_sessionID != 0)
                {
                    if (oldClientState == eClientState.Playing || oldClientState == eClientState.WorldEnter ||
                        ...
                        try
                        {
                            if (Player != null) Player.Quit(true); //calls delete
