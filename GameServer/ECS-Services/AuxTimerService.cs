...
public class AuxTimerService
{
    ...
    public static void Tick(long tick)
    {
        ...
        while (TimerToRemove.Count > 0)
            lock (_removeTimerLockObject)
            {
                ...
                {
                    var callbackMethodName = TimerToRemove.Peek().Callback.Method.Name;
        ...
        while (TimerToAdd.Count > 0)
            lock (_addTimerLockObject)
            {
                ...
                {
                    var callbackMethodName = TimerToAdd.Peek().Callback.Method.Name;
        ...
        Parallel.ForEach(ActiveTimers, timer =>
        {
            if (timer != null && timer.NextTick < AuxGameLoop.GameLoopTime)
            {
                ...
                if (stopTick - startTick > 25)
                    log.Warn(
                        $"Long AuxTimerService.Tick for Timer Callback: {timer.Callback?.Method?.DeclaringType}:{timer.Callback?.Method?.Name}  Owner: {timer.TimerOwner?.Name} Time: {stopTick - startTick}ms");
            }
        });
        ...
        if (debugTimer && TimerToRemoveCallbacks != null && TimerToAddCallbacks != null)
        {
            ...
            log.Debug(
                $"==== AuxTimerService RemoveTimer Top 5 Callback Methods. Total TimerToRemove Count: {TimerToRemoveCount} ====");
            ...
            log.Debug(
                $"==== AuxTimerService AddTimer Top 5 Callback Methods. Total TimerToAdd Count: {TimerToAddCount} ====");
    ...
    public static void RemoveTimer(AuxECSGameTimer timerToRemove)
    {
        lock (_removeTimerLockObject)
        {
            if (ActiveTimers.Contains(timerToRemove)) TimerToRemove?.Push(timerToRemove);
