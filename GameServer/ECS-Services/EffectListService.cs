...
public static class EffectListService
{
    ...
    public static void Tick(long tick)
    {
        ...
        var arr = EntityManager.GetLivingByComponent(typeof(EffectListComponent));
    ...
    private static void HandleEffects(long tick, GameLiving living)
    {
        if (living?.effectListComponent?.Effects.Count > 0)
        {
            ...
            lock (living.effectListComponent.EffectsLock)
            {
                ...
                for (var i = 0; i < currentEffects.Count; i++) effects.AddRange(currentEffects[i]);
            }
            ...
            for (var j = 0; j < effects.Count; j++)
            {
                ...
                else if (e is ECSGameSpellEffect effect)
                {
                        ...
                        (!effect.IsConcentrationEffect() || effect.SpellHandler.Spell.IsFocus))
                    {
                        ...
                        {
                            if (effect.SpellHandler.Spell.IsPulsing && effect.SpellHandler.Caster.LastPulseCast ==
                                effect.SpellHandler.Spell &&
                                effect.ExpireTick >= effect.LastTick +
                                (effect.Duration > 0 ? effect.Duration : effect.PulseFreq))
                            {
                    ...
                    if (!(effect is ECSImmunityEffect) && effect.EffectType != eEffect.Pulse &&
                        ...
                        {
                            var factor = 2.0 - (effect.Duration - effect.GetRemainingTimeForClient()) /
                            ...
                            effect.Owner.BuffBonusMultCategory1.Set((int) eProperty.MaxSpeed, effect.EffectType,
                                1.0 - effect.SpellHandler.Spell.Value * factor * 0.01);
                    ...
                    if (effect.IsConcentrationEffect() && tick > effect.NextTick)
                    {
                            ...
                            && !effect.IsDisabled)
                        {
                            ...
                            if (effect.Owner.effectListComponent.GetSpellEffects(effect.EffectType).Count > 1)
                                disabled =
                                    effect.Owner.effectListComponent.GetBestDisabledSpellEffect(effect.EffectType);
                                 ...
                                 && effect.IsDisabled)
                        {
                            ...
                            if (isBest)
                            {
                                ...
                                if (enabled != null) EffectService.RequestDisableEffect(enabled);
                            }
                        }
    ...
    public static ECSGameEffect GetEffectOnTarget(GameLiving target, eEffect effectType,
        eSpellType spellType = eSpellType.Null)
    {
        ...
        lock (target.effectListComponent.EffectsLock)
        {
            ...
            if (effects != null && spellType == eSpellType.Null)
                ...
                return effects.OfType<ECSGameSpellEffect>()
                    .Where(e => e.SpellHandler.Spell.SpellType == (byte) spellType).FirstOrDefault();
            else
                return null;
        }
    }

    public static ECSGameSpellEffect GetSpellEffectOnTarget(GameLiving target, eEffect effectType,
        eSpellType spellType = eSpellType.Null)
    {
        ...
        lock (target.effectListComponent.EffectsLock)
        {
            ...
            if (effects != null)
                return effects.OfType<ECSGameSpellEffect>().Where(e =>
                        e is ECSGameSpellEffect && (spellType == eSpellType.Null ||
                                                    e.SpellHandler.Spell.SpellType == (byte) spellType))
                    .FirstOrDefault();
            else
                return null;
        }
