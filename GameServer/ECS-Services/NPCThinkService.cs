...
{
    private static int _segmentsize = 5000;
    private static List<Task> _tasks = new();
    private static int completed;
    private static int unthinking;
    private static long interval = 2000;
    private static long last_interval = 0;
    ...
    public static void Tick(long tick)
    {
        ...
        var arr = EntityManager.GetAllNpcsArrayRef();
        ...
        Parallel.ForEach(arr, npc =>
        {
            try
            {
                ...
                if (thinkTimer) Interlocked.Increment(ref NumOfNPCs);
                ...
                if (npc is GameNPC && (npc as GameNPC).Brain != null)
                {
                    ...
                    {
                        if (thinkTimer) Interlocked.Increment(ref ActiveThinkTimerTickCount);
                        ...
                        if (stopTick - startTick > 25 && brain != null)
                            log.Warn(
                                $"Long NPCThink for {brain.Body?.Name}({brain.Body?.ObjectID}) Interval: {brain.ThinkInterval} BrainType: {brain.GetType().ToString()} Time: {stopTick - startTick}ms");
                        ...
                        if (brain is ControlledNpcBrain)
                            ...
                            brain.LastThinkTick =
                                tick + Util.Random(10) * 50; //Offsets the LastThinkTick by 0-500ms
                    }
        ...
        if (thinkTimer && ActiveThinkTimerTickCount > 0)
        {
            ...
            log.Debug(
                $"==== Non-Null NPCs in EntityManager Array: {NumOfNPCs} | Null NPCs: {NumNullSlots} |  Total Size: {arr.Length}====");
