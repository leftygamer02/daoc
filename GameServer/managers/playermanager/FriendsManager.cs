...
public sealed class FriendsManager
{
    ...
    public void AddPlayerFriendsListToCache(GamePlayer Player)
    {
        ...
        if (!PlayersFriendsListsCache.AddIfNotExists(Player, friends))
            if (log.IsWarnEnabled)
                log.WarnFormat("Gameplayer ({0}) is already registered in Friends Manager Cache while adding!",
                    Player);
        ...
        if (!PlayersFriendsStatusCache.AddIfNotExists(Player, offlineFriends))
            if (log.IsWarnEnabled)
                log.WarnFormat(
                    "Gameplayer ({0}) is already registered in Friends Manager Status Cache while adding!", Player);
    }
    ...
    public void RemovePlayerFriendsListFromCache(GamePlayer Player)
    {
        ...
        if (!PlayersFriendsListsCache.TryRemove(Player, out friends))
            if (log.IsWarnEnabled)
                log.WarnFormat(
                    "Gameplayer ({0}) was not registered in Friends Manager Cache while trying to remove!", Player);

        FriendStatus[] offlineFriends;
        if (!PlayersFriendsStatusCache.TryRemove(Player, out offlineFriends))
            if (log.IsWarnEnabled)
                log.WarnFormat(
                    "Gameplayer ({0}) was not registered in Friends Manager Status Cache while trying to remove!",
                    Player);
    }
    ...
    public bool AddFriendToPlayerList(GamePlayer Player, string Friend)
    {
        ...
        if (!PlayersFriendsListsCache.TryUpdate(Player, list =>
            ...
            if (log.IsWarnEnabled)
                log.WarnFormat(
                    "Gameplayer ({0}) was not registered in Friends Manager Cache while trying to Add a new Friend ({1})",
                    Player, Friend);
        ...
        if (success)
        {
            ...
            var offlineFriend = Database.SelectObjects<DOLCharacters>(DB.Column("Name").IsEqualTo(Friend))
                .FirstOrDefault();
            ...
            if (offlineFriend != null)
                if (!PlayersFriendsStatusCache.TryUpdate(Player, list => list.Where(frd => frd.Name != Friend)
                        .Concat(new[]
                        {
                            new FriendStatus(offlineFriend.Name, offlineFriend.Level, offlineFriend.Class,
                                offlineFriend.LastPlayed)
                        })
                        .ToArray()))
                    if (log.IsWarnEnabled)
                        log.WarnFormat(
                            "Gameplayer ({0}) was not registered in Friends Manager Status Cache while trying to Add a new Friend ({1})",
                            Player, Friend);
        }
    ...
    public bool RemoveFriendFromPlayerList(GamePlayer Player, string Friend)
    {
        ...
        if (!PlayersFriendsListsCache.TryUpdate(Player, list =>
            ...
            if (log.IsWarnEnabled)
                log.WarnFormat(
                    "Gameplayer ({0}) was not registered in Friends Manager Cache while trying to Remove a Friend ({1})",
                    Player, Friend);
        ...
        if (success)
        {
            ...
            if (!PlayersFriendsStatusCache.TryUpdate(Player,
                    list => list.Where(frd => frd.Name != Friend).ToArray()))
                if (log.IsWarnEnabled)
                    log.WarnFormat(
                        "Gameplayer ({0}) was not registered in Friends Manager Status Cache while trying to Remove a Friend ({1})",
                        Player, Friend);
        }
    ...
    public void SendPlayerFriendsSocial(GamePlayer Player)
    {
        ...
        var index = 0;
        foreach (var friend in this[Player]
                     .Select(name => PlayersFriendsListsCache.FirstOrDefault(kv => kv.Key.Name == name))
                     .Where(kv => kv.Key != null && !kv.Key.IsAnonymous && kv.Key.Realm == Player.Realm)
                     .Select(kv => kv.Key))
        {
    ...
    private void NotifyPlayerFriendsEnteringGame(GamePlayer Player)
    {
        ...
        foreach (var friend in PlayersFriendsListsCache.Where(kv => kv.Value.Contains(playerName))
                     .Select(kv => kv.Key))
