...
public sealed class InvalidNamesManager
{
    ...
    public bool this[string firstPart, string secondPart] =>
        this[firstPart] || this[secondPart]
                        || this[string.Concat(firstPart, secondPart)] ||
                        this[string.Concat(firstPart, " ", secondPart)];
    ...
    public void LoadInvalidNamesFromFile()
    {
        ...
        if (!File.Exists(InvalidNamesFile))
        {
            if (log.IsWarnEnabled)
                log.WarnFormat("Invalid Names File does not exists, trying to create default file: {0}",
                    InvalidNamesFile);
        ...
        try
        {
            using (var file = File.OpenText(InvalidNamesFile))
            {
