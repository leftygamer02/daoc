...
public class ROGMobGenerator : LootGeneratorBase
{
    ...
    public override LootList GenerateLoot(GameNPC mob, GameObject killer)
    {
        ...
        try
        {
            ...
            var killedcon = (int) player.GetConLevel(mob);

            //grey con dont drop loot
            if (killedcon <= -3) return loot;

            var classForLoot = (eCharacterClass) player.CharacterClass.ID;
            // allow the leader to decide the loot realm
            if (player.Group != null) player = player.Group.Leader;

            // chance to get a RoG Item
            var chance = BASE_ROG_CHANCE + (killedcon < 0 ? killedcon + 1 : killedcon) * 3;
            ...
            else if (player.Group != null)
            {
                ...
                var guaranteedDrop = mob.Level > 67 ? 1 : 0; //guarantee a drop for very high level mobs
            ...
            {
                var tmpChance = player.OutOfClassROGPercent > 0 ? player.OutOfClassROGPercent : 0;
                if (player.Level == 50 && Util.Chance(tmpChance)) classForLoot = GetRandomClassFromRealm(player.Realm);
                ...
                {
                    var tmp = AtlasROGManager.GenerateMonsterLootROG(player.Realm, classForLoot,
                        (byte) (mob.Level + 1), player.CurrentZone?.IsOF ?? false);
                    item = tmp;
                    item.MaxCount = 1;
                    loot.AddFixed(item, 1);
                }
    ...
    private ItemTemplate GenerateItemTemplate(GamePlayer player, eCharacterClass classForLoot, byte lootLevel,
        int killedcon)
    {
        ItemTemplate item = null;


        var tmp = AtlasROGManager.GenerateMonsterLootROG(player.Realm, classForLoot, lootLevel,
    ...
    private eCharacterClass GetRandomClassFromGroup(Group group)
    {
        ...
        var ranClass = validClasses[Util.Random(validClasses.Count - 1)];
    ...
    {
        var classesForRealm = new List<eCharacterClass>();
        switch (realm)
        {
            case eRealm.Albion:
