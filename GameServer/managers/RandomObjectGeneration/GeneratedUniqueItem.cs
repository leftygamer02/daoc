...
public class GeneratedUniqueItem : ItemUnique
{
    ...
    private static Dictionary<int, Spell> ProcSpells = new();

    protected static Dictionary<eProperty, string> hPropertyToMagicPrefix = new();
    ...
    public GeneratedUniqueItem(eRealm realm, eCharacterClass charClass, byte level, eObjectType type,
        eInventorySlot slot, int minUtility = 15)
        : this(realm, charClass, level, type, slot, GenerateDamageType(type, charClass), minUtility)
    {
    }

    public GeneratedUniqueItem(eRealm realm, eCharacterClass charClass, byte level, eObjectType type,
        eInventorySlot slot, eDamageType dmg, int minUtility = 15)
    ...
    public GeneratedUniqueItem(bool toa, eRealm realm, eCharacterClass charClass, byte level, eObjectType type,
        eInventorySlot slot, eDamageType dmg, int utilityMinimum = 15)
        : base()
    {
        Realm = (int) realm;
        Level = level;
        ...
        //name item
        GenerateItemNameModel();

        //set item quality (this can be called again by any script with real mob values)
        GenerateItemQuality((double) Util.Random(0, 6) - 3);

        //item magical bonuses
        //if staff and magic..... focus
        GenerateMagicalBonuses(toa);

        Color = GetRandomColorForRealm(realm);

        IsDropable = true;
        IsPickable = true;
        IsTradable = true;
        CapUtility(Level, utilityMinimum);
        ...
        //constants
        var condition = Level * 2000;
        Condition = condition;
        MaxCondition = condition;
        Durability = condition;
        MaxDurability = condition;

        GenerateItemWeight();

        Description = "Atlas ROG | " + charClass.ToString();
    ...
    {
        // set base quality
        var minQuality = ROG_STARTING_QUAL + Math.Max(0, Level - 59);
        var maxQuality = (int) (1.310 * conlevel + 94.29 + 3);

        if (Level > 51 && minQuality < 97)
            minQuality = 97;
        ...
        Quality = Util.Random(minQuality, maxQuality);

        Price = Money.SetAutoPrice(Level, Quality);
        Price /= 8;
        if (Price <= 0)
            Price = 2; // 2c as sell price is 50%
    }
    ...
    private void GenerateItemStats()
    {
        ...
        switch (type)
        {
            ...
            {
                if ((eInventorySlot) Item_Type == eInventorySlot.LeftHandWeapon)
                    Hand = 2;
                break;
            }
            //left handed weapons
            ...
            {
                if ((eInventorySlot) Item_Type == eInventorySlot.TwoHandWeapon)
                    Hand = 1;
                break;
            }
        }
    ...
    private void GenerateProc()
    {
        ...
        if (Object_Type == (int) eObjectType.Magical)
            return;

        ProcChance = 10;

        if ((Object_Type >= (int) eObjectType._FirstWeapon &&
             Object_Type <= (int) eObjectType._LastWeapon) || Object_Type == (int) eObjectType.Shield)
        {
            if (Util.Chance(50))
            {
                //LT procs
        ...
        else if (Object_Type >= (int) eObjectType._FirstArmor &&
                 Object_Type <= (int) eObjectType._LastArmor && Item_Type == Slot.TORSO)
        {
            if (Util.Chance(50))
            {
                //Heal procs
    ...
    private int GetRandomColorForRealm(eRealm realm)
    {
        ...
        switch (realm)
        {
            case eRealm.Hibernia:
                if (Level > 10) validColors.Add(2); //old green

                if (Level > 20) validColors.Add(10); //leather green
                ...
                break;
            case eRealm.Albion:
                if (Level > 10) validColors.Add(1); //old red

                if (Level > 20) validColors.Add(9); //leather red
                ...
                break;
            case eRealm.Midgard:
                if (Level > 10) validColors.Add(3); //old red

                if (Level > 20) validColors.Add(14); //leather red
    ...
    private void GenerateMagicalBonuses(bool toa)
    {
        ...
        if (Level > 60 && Util.Chance(10)) number++;
        if (Level > 70 && Util.Chance(25)) number++;
        if (Level > 70 && Util.Chance(25)) number++;
        if (Level > 80 && Util.Chance(80)) number++;
        // END

        if (Util.Chance(ROG_100_MAGICAL_OFFSET + Level * 2) ||
            (eObjectType) Object_Type == eObjectType.Magical) // 100% magical starting at level 40
        {
            //1
            number++;

            if (Util.Chance(Level * 8 - 40)) // level 6 - 17 (100%)
            {
                //2
                number++;

                if (Util.Chance(Level * 6 - 60)) // level 11 - 27 (100%)
                {
                    //3
                    number++;

                    if (Util.Chance(Level * 4 - 80)) // level 21 - 45 (100%)
                    {
                        //4
                        number++;
        ...
        if (Object_Type == (int) eObjectType.Magical && number < 1)
            number = 1;


        var fMagicScaled = false;
        var fAddedBonus = false;

        var quality = (double) Quality * .01;

        var multiplier = quality * quality * quality + 0.20;

        if (toa) multiplier += 0.15;

        for (var i = 0; i < number; i++)
        {
            var type = GetPropertyType(toa);
            var property = GetProperty(type);
            var tmpMulti = multiplier;
            if (type == eBonusType.Stat)
                tmpMulti = 1;
            if (!BonusExists(property))
            {
                var amount = (int) Math.Ceiling((double) GetBonusAmount(type, property));
                WriteBonus(property, amount);
                fAddedBonus = true;
                if (!fMagicScaled)
                {
                    fMagicScaled = true;
    ...
    {
        if (Object_Type == (int) eObjectType.Staff)
        {
            if (Bonus1Type != 0)
                return false;

            if (Realm == (int) eRealm.Albion && Description == "friar")
                return false;
    ...
    private eProperty GetProperty(eBonusType type)
    {
        switch (type)
        {
            ...
            {
                // fill valid skills
                var validSkills = new ArrayList();

                var fIndividualSkill = false;
                ...
                if (!BonusExists(eProperty.AllSkills))
                {
                        ...
                        !BonusExists(eProperty.AllArcherySkills))
                    {
                        ...
                        if ((eRealm) Realm == eRealm.Albion)
                            foreach (var property in AlbSkillBonus)
                                if (!BonusExists(property) && SkillIsValidForClass(property) &&
                                    !IsCompetingSkillLine(property))
                                {
                        ...
                        else if ((eRealm) Realm == eRealm.Hibernia)
                            foreach (var property in HibSkillBonus)
                                if (!BonusExists(property) && SkillIsValidForClass(property) &&
                                    !IsCompetingSkillLine(property))
                                {
                                    if (SkillIsValidForObjectType(property))
                        ...
                        else if ((eRealm) Realm == eRealm.Midgard)
                            foreach (var property in MidSkillBonus)
                                if (!BonusExists(property) && SkillIsValidForClass(property) &&
                                    !IsCompetingSkillLine(property))
                                {
                                    if (SkillIsValidForObjectType(property))
                        ...
                        if (!fIndividualSkill)
                            ...
                            if (SkillIsValidForObjectType(eProperty.AllSkills) &&
                                Util.Chance(ROG_STAT_ALLSKILL_CHANCE))
                    ...
                    {
                        if (!BonusExists(eProperty.AllMagicSkills) &&
                            SkillIsValidForObjectType(eProperty.AllMagicSkills) &&
                            Util.Chance(ROG_STAT_ALLSKILL_CHANCE))
                            validSkills.Add(eProperty.AllMagicSkills);

                        if (!BonusExists(eProperty.AllMeleeWeaponSkills) &&
                            SkillIsValidForObjectType(eProperty.AllMeleeWeaponSkills) &&
                            Util.Chance(ROG_STAT_ALLSKILL_CHANCE))
                            validSkills.Add(eProperty.AllMeleeWeaponSkills);

                        if (!BonusExists(eProperty.AllDualWieldingSkills) &&
                            SkillIsValidForObjectType(eProperty.AllDualWieldingSkills) &&
                            Util.Chance(ROG_STAT_ALLSKILL_CHANCE))
                            validSkills.Add(eProperty.AllDualWieldingSkills);

                        if (!BonusExists(eProperty.AllArcherySkills) &&
                            SkillIsValidForObjectType(eProperty.AllArcherySkills) &&
                            Util.Chance(ROG_STAT_ALLSKILL_CHANCE))
            ...
            case eBonusType.Stat:
            {
                ...
                */
                return GetWeightedStatForClass(charClass);
            }
            case eBonusType.AdvancedStat:
            {
                // ToDo: this does not check for duplicates like INT and Acuity
                var validStats = new ArrayList();
                foreach (var property in AdvancedStats)
                    if (!BonusExists(property) && StatIsValidForObjectType(property) &&
                        StatIsValidForRealm(property))
                        validStats.Add(property);
    ...
    {
        var skillsToCheck = new List<eProperty>();
        if (prop == eProperty.Skill_Slashing || prop == eProperty.Skill_Thrusting ||
            prop == eProperty.Skill_Crushing)
        {
        ...
        if (prop == eProperty.Skill_Matter || prop == eProperty.Skill_Body || prop == eProperty.Skill_Spirit ||
            prop == eProperty.Skill_Mind)
        {
        ...
        if (prop == eProperty.Skill_Earth || prop == eProperty.Skill_Cold || prop == eProperty.Skill_Fire ||
            prop == eProperty.Skill_Wind)
        {
        ...
        if (prop == eProperty.Skill_DeathSight || prop == eProperty.Skill_Death_Servant ||
            prop == eProperty.Skill_Pain_working)
        {
        ...
        if (prop == eProperty.Skill_Light || prop == eProperty.Skill_Mana || prop == eProperty.Skill_Void ||
            prop == eProperty.Skill_Enchantments || prop == eProperty.Skill_Mentalism)
        {
        ...
        if (prop == eProperty.Skill_Darkness || prop == eProperty.Skill_Suppression ||
            prop == eProperty.Skill_Runecarving || prop == eProperty.Skill_Summoning ||
            prop == eProperty.Skill_BoneArmy)
        {
    ...
    {
        switch ((eObjectType) Object_Type)
        {
    ...
    private bool SkillIsValidForMagical(eProperty property)
    {
        ...
        switch (property)
        {
            ...
            {
                if (charClass != eCharacterClass.Paladin) return false;
    ...
    private bool SkillIsValidForArmor(eProperty property)
    {
        ...
        switch (property)
        {
            ...
            case eProperty.Skill_Witchcraft:
            {
                if (property == eProperty.Skill_Witchcraft) return false; //we don't want actual Witchcraft skills
            ...
            case eProperty.Skill_Parry:
            {
                ...
                if (type == eObjectType.Cloth && realm == eRealm.Hibernia && level >= 5)
                    ...
                    return true;
                else if (realm == eRealm.Albion && level >= 10 && (type == eObjectType.Leather ||
                                                                   type == eObjectType.Chain ||
                                                                   type == eObjectType.Plate))
                    return true;
            ...
            case eProperty.Skill_Shields:
            {
                ...
                if (type == eObjectType.Cloth && realm == eRealm.Albion)
                    return true;
                else if (type == eObjectType.Studded || type == eObjectType.Chain ||
                         type == eObjectType.Reinforced || type == eObjectType.Scale || type == eObjectType.Plate)
                    return true;
                break;
            }
            case eProperty.Skill_ShortBow:
            ...
            case eProperty.AllDualWieldingSkills:
            {
                ...
                else if (type == eObjectType.Leather || type == eObjectType.Reinforced ||
                         (type == eObjectType.Studded && realm == eRealm.Midgard))
                    return true;
                break;
            }
            case eProperty.AllMagicSkills:
    ...
    private bool SkillIsValidForWeapon(eProperty property)
    {
        ...
        switch (property)
        {
            ...
            case eProperty.Skill_Witchcraft:
            {
                ...
                if (type == eObjectType.Staff && Description != "friar")
                    return true;
                break;
            }
            //healer things
            case eProperty.Skill_Smiting:
            {
                if (((type == eObjectType.Shield && Type_Damage < 3) || type == eObjectType.CrushingWeapon)
                    && charClass == eCharacterClass.Cleric)
                    return true;
                break;
            }
            ...
            case eProperty.Skill_Rejuvenation:
            {
                if (realm != eRealm.Albion ||
                    (charClass != eCharacterClass.Cleric && charClass != eCharacterClass.Friar))
                    return false;

                if ((type == eObjectType.Staff && Description == "friar") ||
                    (type == eObjectType.Shield && Type_Damage < 3) || type == eObjectType.CrushingWeapon)
                    return true;
                break;
            }
            case eProperty.Skill_Augmentation:
            case eProperty.Skill_Mending:
            {
                if (realm != eRealm.Midgard ||
                    (charClass != eCharacterClass.Healer && charClass != eCharacterClass.Shaman))
                    return false;

                if ((type == eObjectType.Shield && Type_Damage < 2) || type == eObjectType.Hammer) return true;
            ...
            case eProperty.Skill_Regrowth:
            {
                ...
                if (type == eObjectType.Blunt || type == eObjectType.Blades ||
                    (type == eObjectType.Shield && Type_Damage < 2))
                    return true;
                break;
            }
            //archery things
            case eProperty.Skill_Archery:
                if (type == eObjectType.CompositeBow || type == eObjectType.RecurvedBow ||
                    type == eObjectType.Longbow)
                    return true;
                break;
            case eProperty.Skill_Composite:
            {
            ...
            case eProperty.Skill_Battlesongs:
            {
                ...
                if (type == eObjectType.Sword || type == eObjectType.Axe || type == eObjectType.Hammer ||
                    (type == eObjectType.Shield && Type_Damage < 3))
                    return true;
                break;
            }
            case eProperty.Skill_BeastCraft:
            ...
            case eProperty.Skill_Critical_Strike:
            {
                ...
                if (type == eObjectType.Piercing || type == eObjectType.SlashingWeapon ||
                    type == eObjectType.ThrustWeapon || type == eObjectType.Blades || type == eObjectType.Sword ||
                    type == eObjectType.Axe || type == eObjectType.LeftAxe)
                    return true;
                break;
            }
            case eProperty.Skill_Cross_Bows:
            ...
            case eProperty.Skill_Crushing:
            {
                ...
                if (type == eObjectType.CrushingWeapon ||
                    ((type == eObjectType.TwoHandedWeapon || type == eObjectType.PolearmWeapon) &&
                     Type_Damage == (int) eDamageType.Crush))
                    return true;
                break;
            }
            case eProperty.Skill_Dual_Wield:
            {
                ...
                if (type == eObjectType.SlashingWeapon || type == eObjectType.ThrustWeapon ||
                    type == eObjectType.CrushingWeapon)
                    return true;
                break;
            }
            case eProperty.Skill_Envenom:
            ...
            case eProperty.Skill_Left_Axe:
            {
                ...
                if (Item_Type == Slot.TWOHAND) return false;
                if (type == eObjectType.Axe || type == eObjectType.LeftAxe)
                    return true;
                break;
            }
            case eProperty.Skill_Music:
            {
                ...
                if (type == eObjectType.Blades || type == eObjectType.Blunt ||
                    (type == eObjectType.Shield && Type_Damage == 1) || type == eObjectType.Instrument)
                    return true;
                break;
            }
            case eProperty.Skill_Nightshade:
            {
                if (charClass != eCharacterClass.Nightshade) return false;
            ...
            case eProperty.Skill_Pathfinding:
            {
                if (charClass != eCharacterClass.Ranger) return false;
            ...
            case eProperty.Skill_SoulRending:
            {
                ...
                if (type == eObjectType.SlashingWeapon || type == eObjectType.CrushingWeapon ||
                    type == eObjectType.ThrustWeapon || type == eObjectType.Flexible || type == eObjectType.Shield)
                    return true;
                break;
            }
            case eProperty.Skill_Spear:
            ...
            case eProperty.Skill_Stealth:
            {
                ...
                if (type == eObjectType.Longbow || type == eObjectType.RecurvedBow ||
                    type == eObjectType.CompositeBow ||
                    (realm == eRealm.Albion && type == eObjectType.Shield && Type_Damage == 1) ||
                    type == eObjectType.Spear || type == eObjectType.Sword || type == eObjectType.Axe ||
                    type == eObjectType.LeftAxe || type == eObjectType.SlashingWeapon ||
                    type == eObjectType.ThrustWeapon || type == eObjectType.Piercing ||
                    type == eObjectType.Blades || (realm == eRealm.Albion && type == eObjectType.Instrument))
                    return true;
                break;
            }
            case eProperty.Skill_Stormcalling:
            ...
            case eProperty.Skill_Thrusting:
            {
                ...
                if (type == eObjectType.ThrustWeapon ||
                    ((type == eObjectType.TwoHandedWeapon || type == eObjectType.PolearmWeapon) &&
                     Type_Damage == (int) eDamageType.Thrust))
                    return true;
                break;
            }
            case eProperty.Skill_Two_Handed:
            ...
            case eProperty.Skill_Valor:
            {
                ...
                if (type == eObjectType.Blades || type == eObjectType.Piercing || type == eObjectType.Blunt ||
                    type == eObjectType.LargeWeapons || type == eObjectType.Shield)
                    return true;
                break;
            }
            ...
            case eProperty.AllArcherySkills:
            {
                ...
                if (type == eObjectType.CompositeBow || type == eObjectType.Longbow ||
                    type == eObjectType.RecurvedBow)
                    return true;
                break;
            }
            case eProperty.AllDualWieldingSkills:
            {
                ...
                if (type == eObjectType.Axe || type == eObjectType.Sword || type == eObjectType.Hammer ||
                    type == eObjectType.LeftAxe || type == eObjectType.SlashingWeapon ||
                    type == eObjectType.CrushingWeapon || type == eObjectType.ThrustWeapon ||
                    type == eObjectType.Piercing || type == eObjectType.Blades || type == eObjectType.Blunt)
                    return true;
                break;
            }
            case eProperty.AllMagicSkills:
            {
                ...
                if (type == eObjectType.Longbow || type == eObjectType.CelticSpear ||
                    type == eObjectType.PolearmWeapon || type == eObjectType.TwoHandedWeapon ||
                    type == eObjectType.Crossbow || (type == eObjectType.Shield && Type_Damage > 2))
                    return false;
                else
                    return true;
            }
            case eProperty.AllMeleeWeaponSkills:
            {
                ...
                if (type == eObjectType.Staff && realm != eRealm.Albion)
                    return false;
                else if (type == eObjectType.Staff && Description != "friar") // do not add if caster staff
                    return false;
                else if (type == eObjectType.Longbow || type == eObjectType.CompositeBow ||
                         type == eObjectType.RecurvedBow || type == eObjectType.Crossbow ||
                         type == eObjectType.Fired || type == eObjectType.Instrument)
                    return false;
                else
                    return true;
            }
    ...
    private bool StatIsValidForRealm(eProperty property)
    {
        switch (property)
        {
            ...
            {
                if (Realm == (int) eRealm.Midgard || Realm == (int) eRealm.Albion)
                    return false;
                break;
            }
    ...
    private bool StatIsValidForArmor(eProperty property)
    {
        ...
        switch (property)
        {
            ...
            case eProperty.IntCapBonus:
            {
                ...
                if (realm == eRealm.Hibernia && Level < 20 && type != eObjectType.Reinforced &&
                    type != eObjectType.Cloth)
                    return false;

                if (realm == eRealm.Hibernia && Level >= 20 && type != eObjectType.Scale &&
                    type != eObjectType.Cloth)
                    return false;
            ...
            {
                if (realm == eRealm.Albion && Level >= 20 && type == eObjectType.Studded)
                    return false;

                if (realm == eRealm.Midgard && Level >= 10 && type == eObjectType.Leather)
                    return false;

                if (realm == eRealm.Midgard && Level >= 20 && type == eObjectType.Studded)
                    return false;
            ...
            case eProperty.PieCapBonus:
            {
                if (realm == eRealm.Albion)
                {
                    ...
                    if (type == eObjectType.Studded && Level >= 20)
                        return false;

                    if (type == eObjectType.Chain && Level < 10)
                        return false;
                }
            ...
            case eProperty.ChaCapBonus:
            {
                if (realm == eRealm.Albion)
                {
                    ...
                    if (type == eObjectType.Chain && Level < 20)
                        return false;
                }
                ...
                {
                    if (type == eObjectType.Leather && Level >= 15)
                        return false;

                    if (type == eObjectType.Reinforced && Level < 15)
                        return false;
                }
            ...
            case eProperty.EmpCapBonus:
            {
                ...
                if (type == eObjectType.Scale && Level < 20)
                    return false;
        ...
        switch (type)
        {
            case eObjectType.Staff:
            {
                if ((property == eProperty.Piety || property == eProperty.PieCapBonus) && realm == eRealm.Hibernia)
                    return false;
                else if ((property == eProperty.Piety || property == eProperty.PieCapBonus) &&
                         realm == eRealm.Albion && Description != "friar")
                    return false; // caster staff
                else if (property == eProperty.Charisma || property == eProperty.Empathy ||
                         property == eProperty.ChaCapBonus || property == eProperty.EmpCapBonus)
                    return false;
                else if ((property == eProperty.Intelligence || property == eProperty.IntCapBonus ||
                          property == eProperty.AcuCapBonus) && Description == "friar")
                    return false;
                break;
            }
            ...
            case eObjectType.Shield:
            {
                if ((realm == eRealm.Albion || realm == eRealm.Midgard) && (property == eProperty.Intelligence ||
                                                                            property == eProperty.IntCapBonus ||
                                                                            property == eProperty.Empathy ||
                                                                            property == eProperty.EmpCapBonus))
                    return false;
                else if (realm == eRealm.Hibernia &&
                         (property == eProperty.Piety || property == eProperty.PieCapBonus))
                    return false;
                else if ((realm == eRealm.Albion || realm == eRealm.Hibernia) && Type_Damage > 1 &&
                         (property == eProperty.Charisma || property == eProperty.ChaCapBonus))
                    return false;
                else if (realm == eRealm.Midgard && Type_Damage > 2 &&
                         (property == eProperty.Charisma || property == eProperty.ChaCapBonus))
                    return false;
                else if (Type_Damage > 2 && property == eProperty.MaxMana)
                    return false;
            ...
            case eObjectType.CrushingWeapon:
            {
                if (property == eProperty.Intelligence || property == eProperty.IntCapBonus ||
                    property == eProperty.Empathy || property == eProperty.EmpCapBonus ||
                    property == eProperty.Charisma || property == eProperty.ChaCapBonus)
                    return false;
                break;
            }
            ...
            case eObjectType.Axe:
            {
                if (property == eProperty.Intelligence || property == eProperty.IntCapBonus ||
                    property == eProperty.Empathy || property == eProperty.EmpCapBonus ||
                    property == eProperty.AcuCapBonus || property == eProperty.Acuity)
                    return false;
                break;
            }
            case eObjectType.TwoHandedWeapon:
            ...
            case eObjectType.Fired:
            {
                if (property == eProperty.Intelligence || property == eProperty.IntCapBonus ||
                    property == eProperty.Empathy || property == eProperty.EmpCapBonus ||
                    property == eProperty.Charisma || property == eProperty.ChaCapBonus ||
                    property == eProperty.MaxMana || property == eProperty.PowerPool ||
                    property == eProperty.PowerPoolCapBonus || property == eProperty.AcuCapBonus ||
                    property == eProperty.Acuity || property == eProperty.Piety ||
                    property == eProperty.PieCapBonus)
                    return false;
                break;
            }
            case eObjectType.Spear:
    ...
    {
        if (property == eProperty.AllFocusLevels) amount = Math.Min(50, amount);

        if (Bonus1 == 0)
        {
            Bonus1 = amount;
            Bonus1Type = (int) property;
        ...
        {
            Bonus2 = amount;
            Bonus2Type = (int) property;
        }
        else if (Bonus3 == 0)
        {
            Bonus3 = amount;
            Bonus3Type = (int) property;
        }
        else if (Bonus4 == 0)
        {
            Bonus4 = amount;
            Bonus4Type = (int) property;
        }
        else if (Bonus5 == 0)
        {
            Bonus5 = amount;
            Bonus5Type = (int) property;
        }
    }

    private bool BonusExists(eProperty property)
    {
        if (Bonus1Type == (int) property ||
            Bonus2Type == (int) property ||
            Bonus3Type == (int) property ||
            Bonus4Type == (int) property ||
            Bonus5Type == (int) property)
    ...
    private int GetBonusAmount(eBonusType type, eProperty property)
    {
        switch (type)
        {
            ...
            case eBonusType.Resist:
            {
                var max = (int) Math.Ceiling((Level / 2.0 + 1) / 4);
                return Util.Random((int) Math.Ceiling((double) max / 2.0), max);
            }
            case eBonusType.Skill:
            {
                var max = (int) Util.Random(1, 4);
            ...
            case eBonusType.Stat:
            {
                ...
                {
                    var max = (int) Math.Ceiling((double) Level * 4.0 / 4);
                    return Util.Random((int) Math.Ceiling((double) max / 2.0), max);
                }
                else if (property == eProperty.MaxMana)
                {
                    var max = (int) Math.Ceiling(((double) Level / 2.0 + 1) / 4);
                    return Util.Random((int) Math.Ceiling((double) max / 2.0), max);
                }
                else
                {
                    var max = (int) Math.Ceiling((double) Level * 1 / 3);
                    return Util.Random((int) Math.Ceiling((double) max / 2.0), max);
                }
            }
            case eBonusType.AdvancedStat:
    ...
    private void CapUtility(int mobLevel, int utilityMinimum)
    {
        ...
        var utiLine = GetHighestUtilitySingleLine();
        var bonus = GetPropertyFromBonusLine(utiLine);
    ...
    {
        var highestUti = GetSingleUtility(Bonus1Type, Bonus1);
        var highestLine =
            highestUti > 0 ? 1 : 0; //if line1 had a bonus, set it as highest line, otherwise default to 0
        ...
        if (GetSingleUtility(ExtraBonusType, ExtraBonus) > highestUti) highestLine = 11;
    ...
    {
        var lowestUti = GetSingleUtility(Bonus1Type, Bonus1);
        var lowestLine =
            lowestUti > 0 ? 1 : 0; //if line1 had a bonus, set it as highest line, otherwise default to 0
    ...
    {
        var type = GetObjectTypeByWeight(level);

        switch ((eRealm) realm)
        {
            case eRealm.Albion:
            {
                var maxArmor = AlbionArmor.Length - 1;
                var maxWeapon = AlbionWeapons.Length - 1;
                ...
                switch (type)
                {
                    case eGenerateType.Armor:
                        return GetAlbionArmorType(charClass, level); //AlbionArmor[Util.Random(0, maxArmor)];
                    case eGenerateType.Weapon:
                        return GetAlbionWeapon(charClass); //AlbionWeapons[Util.Random(0, maxWeapon)];
            ...
            case eRealm.Midgard:
            {
                var maxArmor = MidgardArmor.Length - 1;
                var maxWeapon = MidgardWeapons.Length - 1;

                if (level < 10)
                    maxArmor--; // remove chain

                if (level < 5) maxWeapon = 4; // remove all but base weapons and shield

                switch (type)
                {
                    case eGenerateType.Armor:
                        return GetMidgardArmorType(charClass, level); //MidgardArmor[Util.Random(0, maxArmor)];
                    case eGenerateType.Weapon:
                        return GetMidgardWeapon(charClass); //MidgardWeapons[Util.Random(0, maxWeapon)];
                    case eGenerateType.Magical: return eObjectType.Magical;
                }
            ...
            case eRealm.Hibernia:
            {
                var maxArmor = HiberniaArmor.Length - 1;
                var maxWeapon = HiberniaWeapons.Length - 1;
                ...
                {
                    case eGenerateType.Armor:
                        return GetHiberniaArmorType(charClass, level); //HiberniaArmor[Util.Random(0, maxArmor)];
                    case eGenerateType.Weapon:
                        return GetHiberniaWeapon(charClass); //HiberniaWeapons[Util.Random(0, maxWeapon)];
                    case eGenerateType.Magical: return eObjectType.Magical;
                }
    ...
    {
        var genTypes = new List<eGenerateType>();
    ...
    {
        var weaponTypes = new List<eObjectType>();
        /*
         * Albion Weapons
        ...
        */
        ...
        //clone existing list
        var outputList = new List<eObjectType>(weaponTypes);

        //add duplicate values
        foreach (var type in weaponTypes) outputList.Add(type);

        //get our random value from the list
        var randomGrab = Util.Random(0, outputList.Count - 1);
    ...
    private void SetWeaponSpeed()
    {
        ...
        switch ((eObjectType) Object_Type)
        {
            case eObjectType.SlashingWeapon:
            {
                SPD_ABS = Util.Random(26, 39);
                return;
            }
            case eObjectType.CrushingWeapon:
            {
                SPD_ABS = Util.Random(30, 40);
                return;
            }
            case eObjectType.ThrustWeapon:
            {
                SPD_ABS = Util.Random(25, 37);
                return;
            }
            case eObjectType.Fired:
            {
                SPD_ABS = Util.Random(40, 46);
                return;
            }
            case eObjectType.TwoHandedWeapon:
            {
                SPD_ABS = Util.Random(43, 51);
                return;
            }
            case eObjectType.PolearmWeapon:
            {
                SPD_ABS = Util.Random(53, 56);
                return;
            }
            case eObjectType.Staff:
            {
                SPD_ABS = Util.Random(30, 50);
                return;
            }
            case eObjectType.MaulerStaff: //Maulers
            {
                SPD_ABS = Util.Random(34, 54);
                return;
            }
            case eObjectType.Longbow:
            {
                SPD_ABS = Util.Random(40, 52);
                return;
            }
            case eObjectType.Crossbow:
            {
                SPD_ABS = Util.Random(33, 54);
                return;
            }
            case eObjectType.Flexible:
            {
                SPD_ABS = Util.Random(33, 39);
                return;
            }
            case eObjectType.Sword:
                if (Hand == 1)
                {
                    SPD_ABS = Util.Random(46, 51); // two handed
                    return;
                }
                else
                {
                    SPD_ABS = Util.Random(25, 38); // one handed
                    return;
                }
            case eObjectType.Hammer:
            {
                if (Hand == 1)
                {
                    SPD_ABS = Util.Random(49, 52); // two handed
                    return;
                }
                else
                {
                    SPD_ABS = Util.Random(31, 39); // one handed
                    return;
                }
            }
            case eObjectType.Axe:
            {
                if (Hand == 1)
                {
                    SPD_ABS = Util.Random(49, 53); // two handed
                    return;
                }
                else
                {
                    SPD_ABS = Util.Random(37, 40); // one handed
                    return;
                }
            }
            case eObjectType.Spear:
            {
                SPD_ABS = Util.Random(43, 52);
                return;
            }
            case eObjectType.CompositeBow:
            {
                SPD_ABS = Util.Random(40, 47);
                return;
            }
            case eObjectType.LeftAxe:
            {
                SPD_ABS = Util.Random(27, 31);
                return;
            }
            case eObjectType.HandToHand:
            {
                SPD_ABS = Util.Random(27, 37);
                return;
            }
            case eObjectType.FistWraps:
            {
                SPD_ABS = Util.Random(28, 41);
                return;
            }
            case eObjectType.RecurvedBow:
            {
                SPD_ABS = Util.Random(45, 52);
                return;
            }
            case eObjectType.Blades:
            {
                SPD_ABS = Util.Random(27, 39);
                return;
            }
            case eObjectType.Blunt:
            {
            ...
            case eObjectType.Piercing:
            {
                SPD_ABS = Util.Random(25, 36);
                return;
            }
            case eObjectType.LargeWeapons:
            {
                SPD_ABS = Util.Random(47, 53);
                return;
            }
            case eObjectType.CelticSpear:
            {
                SPD_ABS = Util.Random(40, 56);
                return;
            }
            case eObjectType.Scythe:
            {
                SPD_ABS = Util.Random(40, 53);
                return;
            }
            case eObjectType.Shield:
            {
        ...
        if (Hand == 1)
        {
            SPD_ABS = 50; // two handed
            return;
        }
        else if (Hand == 2)
        {
            SPD_ABS = 30; // left hand
            return;
        }
        else
        {
            SPD_ABS = 40; // right hand
            return;
        }
    }
    ...
    {
        var type = (eObjectType) Object_Type;
        var slot = (eInventorySlot) Item_Type;
    ...
    public bool WriteMagicalName(eProperty property)
    {
        ...
        {
            var str = hPropertyToMagicPrefix[property];
    ...
    {
        var slot = (eInventorySlot) Item_Type;
        var damage = (eDamageType) Type_Damage;
        var realm = (eRealm) Realm;
        var type = (eObjectType) Object_Type;
        ...
        switch (type)
        {
            ...
            case eObjectType.Leather:
            {
                ...
                switch (realm)
                {
                    case eRealm.Albion:
                        ...
                        {
                            case eInventorySlot.ArmsArmor:
                                model = GetLeatherSleevesForLevel(Level, eRealm.Albion);
                                break;
                            case eInventorySlot.LegsArmor:
                                model = GetLeatherPantsForLevel(Level, eRealm.Albion);
                                break;
                            case eInventorySlot.FeetArmor:
                                model = GetLeatherBootsForLevel(Level, eRealm.Albion);
                                break;
                            case eInventorySlot.HeadArmor:
                                model = GetLeatherHelmForLevel(Level, eRealm.Albion);
                                break;
                            case eInventorySlot.TorsoArmor:
                                model = GetLeatherTorsoForLevel(Level, eRealm.Albion);
                                break;
                            case eInventorySlot.HandsArmor:
                                model = GetLeatherHandsForLevel(Level, eRealm.Albion);
                                break;
                        }
                    ...
                    case eRealm.Midgard:
                        ...
                        {
                            case eInventorySlot.ArmsArmor:
                                model = GetLeatherSleevesForLevel(Level, eRealm.Midgard);
                                break;
                            case eInventorySlot.LegsArmor:
                                model = GetLeatherPantsForLevel(Level, eRealm.Midgard);
                                break;
                            case eInventorySlot.FeetArmor:
                                model = GetLeatherBootsForLevel(Level, eRealm.Midgard);
                                break;
                            case eInventorySlot.HeadArmor:
                                model = GetLeatherHelmForLevel(Level, eRealm.Midgard);
                                break;
                            case eInventorySlot.TorsoArmor:
                                model = GetLeatherTorsoForLevel(Level, eRealm.Midgard);
                                break;
                            case eInventorySlot.HandsArmor:
                                model = GetLeatherHandsForLevel(Level, eRealm.Midgard);
                                break;
                        }
                    ...
                    case eRealm.Hibernia:
                        ...
                        {
                            case eInventorySlot.ArmsArmor:
                                model = GetLeatherSleevesForLevel(Level, eRealm.Hibernia);
                                break;
                            case eInventorySlot.LegsArmor:
                                model = GetLeatherPantsForLevel(Level, eRealm.Hibernia);
                                break;
                            case eInventorySlot.FeetArmor:
                                model = GetLeatherBootsForLevel(Level, eRealm.Hibernia);
                                break;
                            case eInventorySlot.HeadArmor:
                                model = GetLeatherHelmForLevel(Level, eRealm.Hibernia);
                                break;
                            case eInventorySlot.TorsoArmor:
                                model = GetLeatherTorsoForLevel(Level, eRealm.Hibernia);
                                break;
                            case eInventorySlot.HandsArmor:
                                model = GetLeatherHandsForLevel(Level, eRealm.Hibernia);
                                break;
                        }
            ...
            case eObjectType.Studded:
            {
                ...
                switch (realm)
                {
                    case eRealm.Albion:
                        ...
                        {
                            case eInventorySlot.ArmsArmor:
                                model = GetStuddedSleevesForLevel(Level, eRealm.Albion);
                                break;
                            case eInventorySlot.LegsArmor:
                                model = GetStuddedPantsForLevel(Level, eRealm.Albion);
                                break;
                            case eInventorySlot.FeetArmor:
                                model = GetStuddedBootsForLevel(Level, eRealm.Albion);
                                break;
                            case eInventorySlot.HeadArmor:
                                model = GetStuddedHelmForLevel(Level, eRealm.Albion);
                                break;
                            case eInventorySlot.TorsoArmor:
                                model = GetStuddedTorsoForLevel(Level, eRealm.Albion);
                                break;
                            case eInventorySlot.HandsArmor:
                                model = GetStuddedHandsForLevel(Level, eRealm.Albion);
                                break;
                        }
                    ...
                    case eRealm.Midgard:
                        ...
                        {
                            case eInventorySlot.ArmsArmor:
                                model = GetStuddedSleevesForLevel(Level, eRealm.Midgard);
                                break;
                            case eInventorySlot.LegsArmor:
                                model = GetStuddedPantsForLevel(Level, eRealm.Midgard);
                                break;
                            case eInventorySlot.FeetArmor:
                                model = GetStuddedBootsForLevel(Level, eRealm.Midgard);
                                break;
                            case eInventorySlot.HeadArmor:
                                model = GetStuddedHelmForLevel(Level, eRealm.Midgard);
                                break;
                            case eInventorySlot.TorsoArmor:
                                model = GetStuddedTorsoForLevel(Level, eRealm.Midgard);
                                break;
                            case eInventorySlot.HandsArmor:
                                model = GetStuddedHandsForLevel(Level, eRealm.Midgard);
                                break;
                        }
            ...
            case eObjectType.Plate:
            {
                ...
                {
                    case eInventorySlot.ArmsArmor:
                        model = GetPlateSleevesForLevel(Level, eRealm.Albion);
                        break;
                    case eInventorySlot.LegsArmor:
                        model = GetPlatePantsForLevel(Level, eRealm.Albion);
                        break;
                    case eInventorySlot.FeetArmor:
                        model = GetPlateBootsForLevel(Level, eRealm.Albion);
                        break;
                    case eInventorySlot.HeadArmor:
                        ...
                        break;
                    case eInventorySlot.TorsoArmor:
                        model = GetPlateTorsoForLevel(Level, eRealm.Albion);
                        break;
                    case eInventorySlot.HandsArmor:
                        model = GetPlateHandsForLevel(Level, eRealm.Albion);
                        break;
                }
            ...
            case eObjectType.Chain:
            {
                ...
                switch (realm)
                {
                    case eRealm.Albion:
                        ...
                        {
                            case eInventorySlot.ArmsArmor:
                                model = GetChainSleevesForLevel(Level, eRealm.Albion);
                                break;
                            case eInventorySlot.LegsArmor:
                                model = GetChainPantsForLevel(Level, eRealm.Albion);
                                break;
                            case eInventorySlot.FeetArmor:
                                model = GetChainBootsForLevel(Level, eRealm.Albion);
                                break;
                            case eInventorySlot.HeadArmor:
                                model = GetChainHelmForLevel(Level, eRealm.Albion);
                                break;
                            case eInventorySlot.TorsoArmor:
                                model = GetChainTorsoForLevel(Level, eRealm.Albion);
                                break;
                            case eInventorySlot.HandsArmor:
                                model = GetChainHandsForLevel(Level, eRealm.Albion);
                                break;
                        }
                    ...
                    case eRealm.Midgard:
                        ...
                        {
                            case eInventorySlot.ArmsArmor:
                                model = GetChainSleevesForLevel(Level, eRealm.Midgard);
                                break;
                            case eInventorySlot.LegsArmor:
                                model = GetChainPantsForLevel(Level, eRealm.Midgard);
                                break;
                            case eInventorySlot.FeetArmor:
                                model = GetChainBootsForLevel(Level, eRealm.Midgard);
                                break;
                            case eInventorySlot.HeadArmor:
                                model = GetChainHelmForLevel(Level, eRealm.Midgard);
                                break;
                            case eInventorySlot.TorsoArmor:
                                model = GetChainTorsoForLevel(Level, eRealm.Midgard);
                                break;
                            case eInventorySlot.HandsArmor:
                                model = GetChainHandsForLevel(Level, eRealm.Midgard);
                                break;
                        }
            ...
            case eObjectType.Reinforced:
            {
                ...
                {
                    case eInventorySlot.ArmsArmor:
                        model = GetReinforcedSleevesForLevel(Level, eRealm.Hibernia);
                        break;
                    case eInventorySlot.LegsArmor:
                        model = GetReinforcedPantsForLevel(Level, eRealm.Hibernia);
                        break;
                    case eInventorySlot.FeetArmor:
                        model = GetReinforcedBootsForLevel(Level, eRealm.Hibernia);
                        break;
                    case eInventorySlot.HeadArmor:
                        model = GetReinforcedHelmForLevel(Level, eRealm.Hibernia);
                        break;
                    case eInventorySlot.TorsoArmor:
                        model = GetReinforcedTorsoForLevel(Level, eRealm.Hibernia);
                        break;
                    case eInventorySlot.HandsArmor:
                        model = GetReinforcedHandsForLevel(Level, eRealm.Hibernia);
                        break;
                }
            ...
            case eObjectType.Scale:
            {
                ...
                {
                    case eInventorySlot.ArmsArmor:
                        model = GetScaleSleevesForLevel(Level, eRealm.Hibernia);
                        break;
                    case eInventorySlot.LegsArmor:
                        model = GetScalePantsForLevel(Level, eRealm.Hibernia);
                        break;
                    case eInventorySlot.FeetArmor:
                        model = GetScaleBootsForLevel(Level, eRealm.Hibernia);
                        break;
                    case eInventorySlot.HeadArmor:
                        model = GetScaleHelmForLevel(Level, eRealm.Hibernia);
                        break;
                    case eInventorySlot.TorsoArmor:
                        model = GetScaleTorsoForLevel(Level, eRealm.Hibernia);
                        break;
                    case eInventorySlot.HandsArmor:
                        model = GetScaleHandsForLevel(Level, eRealm.Hibernia);
                        break;
                }
    ...
    private static int Get2HAxeModelForLevel(int Level, eRealm realm)
    {
        ...
        switch (realm)
        {
            ...
            case eRealm.Midgard:
                validModels.Add(317);
                if (Level > 10) validModels.Add(318);
    ...
    private static int Get2HSwordForLevel(int Level, eRealm realm)
    {
        ...
        switch (realm)
        {
            ...
            case eRealm.Midgard:
                ...
                if (Level > 40) validModels.Add(957);

                if (Level > 50) validModels.Add(1032);
    ...
    private static int GetBladeModelForLevel(int Level, eRealm realm)
    {
        ...
        switch (realm)
        {
            case eRealm.Hibernia:
                validModels.Add(444);
                validModels.Add(445);
                if (Level > 10) validModels.Add(446);

                if (Level > 30) validModels.Add(447);

                if (Level > 40) validModels.Add(460);

                if (Level > 50) validModels.Add(473);
            ...
            case eRealm.Albion:
                ...
                if (Level > 40) validModels.Add(877);

                if (Level > 50) validModels.Add(879);
    ...
    private static int Get2HHammerForLevel(int Level, eRealm realm)
    {
        ...
        switch (realm)
        {
            case eRealm.Hibernia:
                validModels.Add(474);
                validModels.Add(463);
                if (Level > 10) validModels.Add(462);
    ...
    private static int Get2HThrustForLevel(int Level, eRealm realm)
    {
        ...
        switch (realm)
        {
            case eRealm.Albion:
                ...
                if (Level > 50) validModels.Add(2208);
    ...
    private static byte GetNonTorsoExtensionForLevel(int Level)
    {
        ...
        var appliedExtension = possibleExt[Util.Random(possibleExt.Count - 1)];
        return appliedExtension;
    }
    ...
    {
        //LT spells
        var Level5Lifetap = DOLDB<DBSpell>.SelectObject(DB.Column("Spell_ID").IsEqualTo(8010));
        var Level10Lifetap = DOLDB<DBSpell>.SelectObject(DB.Column("Spell_ID").IsEqualTo(8011));
        var Level15Lifetap = DOLDB<DBSpell>.SelectObject(DB.Column("Spell_ID").IsEqualTo(8012));
