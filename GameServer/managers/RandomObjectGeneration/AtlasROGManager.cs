...
public static class AtlasROGManager
{
    ...
    private static void GenerateBPs(GameLiving living, int amount)
    {
        ...
        if (living != null && living is GamePlayer)
        {
            ...
            double numCurrentLoyalDays = LoyaltyManager.GetPlayerRealmLoyalty(player) != null
                ? LoyaltyManager.GetPlayerRealmLoyalty(player).Days
                : 0;

            if (numCurrentLoyalDays >= 30) numCurrentLoyalDays = 30;

            var loyaltyBonus = amount * .2 * (numCurrentLoyalDays / 30);

            var relicBonus = amount * (0.025 * RelicMgr.GetRelicCount(player.Realm));
            ...
            if (loyaltyBonus > 0)
                player.Out.SendMessage(
                    $"You gained an additional {Convert.ToInt32(loyaltyBonus)} BPs due to your realm loyalty!",
                    eChatType.CT_Important, eChatLoc.CL_SystemWindow);
            if (relicBonus > 0)
                player.Out.SendMessage(
                    $"You gained an additional {Convert.ToInt32(relicBonus)} BPs due to your realm's relic ownership!",
                    eChatType.CT_Important, eChatLoc.CL_SystemWindow);
        }
    }
    ...
    public static void GenerateOrbAmount(GameLiving living, int amount)
    {
        ...
        if (living != null && living is GamePlayer)
        {
            ...
            var relicOrbBonus = amount * (0.025 * RelicMgr.GetRelicCount(player.Realm));
            ...
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language, "GamePlayer.PickupObject.YouGetAmount",
                    amount, item.Name), eChatType.CT_Loot, eChatLoc.CL_SystemWindow);

            if (loyaltyBonus > 0)
                player.Out.SendMessage(
                    $"You gained an additional {Convert.ToInt32(loyaltyBonus)} orb(s) due to your realm loyalty!",
                    eChatType.CT_Loot, eChatLoc.CL_SystemWindow);
            if (relicOrbBonus > 0)
                player.Out.SendMessage(
                    $"You gained an additional {Convert.ToInt32(relicOrbBonus)} orb(s) due to your realm's relic ownership!",
                    eChatType.CT_Loot, eChatLoc.CL_SystemWindow);

            if (!player.Inventory.AddCountToStack(item, totOrbs))
                if (!player.Inventory.AddTemplate(item, totOrbs, eInventorySlot.FirstBackpack,
                        eInventorySlot.LastBackpack))
                {
                    item.Count = totOrbs;
                    player.CreateItemOnTheGround(item);
                    player.Out.SendMessage(
                        $"Your inventory is full, your {item.Name}s have been placed on the ground.",
                        eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                }
    ...
    public static void GenerateBattlegroundToken(GameLiving living, int amount)
    {
        if (living != null && living is GamePlayer)
        {
            ...
            if (!player.Inventory.AddCountToStack(item, amount))
                if (!player.Inventory.AddTemplate(item, amount, eInventorySlot.FirstBackpack,
                        eInventorySlot.LastBackpack))
                {
                    item.Count = amount;
                    player.CreateItemOnTheGround(item);
    ...
    public static ItemUnique GenerateBeadOfRegeneration()
    {
        ...
        var item = new ItemUnique(beadTemplate);
