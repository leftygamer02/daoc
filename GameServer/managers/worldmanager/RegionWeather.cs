...
public sealed class RegionWeather
{
    ...
    /// </summary>
    public int Duration => (int) Math.Min(int.MaxValue, DueTime - StartTime);
    ...
    /// <param name="StartTime">StartTime of the Weather (StopWatch ms Timestamp)</param>
    public void CreateWeather(uint Position, uint Width, ushort Speed, ushort Intensity, ushort FogDiffusion,
        long StartTime)
    {
        ...
        DueTime = StartTime +
                  Convert.ToInt64(Math.Ceiling((WeatherMaxPosition + Width - this.Position) / (double) Speed *
                                               1000));
    }
    ...
    public override string ToString()
    {
        return string.Format(
            "[RegionWeather Region={0}, Position={1}, Width={2}, Speed={3}, Intensity={4}, Diffusion={5}, DurationTime={6}s, MinPosition={7}, MaxPosition={8}]",
            Region.ID, Position, Width, Speed, Intensity, FogDiffusion, Duration / 1000, WeatherMinPosition,
            WeatherMaxPosition + Width);
    }
}