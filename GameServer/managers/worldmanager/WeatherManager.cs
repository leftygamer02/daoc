...
public sealed class WeatherManager
{
    ...
    /// </summary>
    private int DefaultTimerInterval => Math.Max(1000, ServerProperties.Properties.WEATHER_CHECK_INTERVAL);

    /// <summary>
    /// Default Weather Chance
    /// </summary>
    private int DefaultWeatherChance => Math.Min(99, ServerProperties.Properties.WEATHER_CHANCE);

    /// <summary>
    /// Log Weather Change to Info Logger
    /// </summary>
    private bool EventLogWeather => ServerProperties.Properties.WEATHER_LOG_EVENTS;
    ...
    /// <returns>True if weather was changed</returns>
    public bool StartWeather(ushort regionId, uint position, uint width, ushort speed, ushort diffusion,
        ushort intensity)
    {
        return ChangeWeather(regionId,
            weather => weather.CreateWeather(position, width, speed, intensity, diffusion, SimpleScheduler.Ticks));
    }
    ...
    {
        return ChangeWeather(regionId,
            weather => weather.CreateWeather(weather.Position, weather.Width, weather.Speed, weather.Intensity,
                weather.FogDiffusion, SimpleScheduler.Ticks));
    }
    ...
    private void StopWeather(RegionWeather weather)
    {
        ...
        foreach (var player in weather.Region.Objects.OfType<GamePlayer>())
        {
            ...
            if (player.X > weatherCurrentPosition - weather.Width && player.X < weatherCurrentPosition)
                player.Out.SendMessage("The sky clears up again as the storm clouds disperse!",
    ...
    {
        if (EventLogWeather && log.IsInfoEnabled)
            log.InfoFormat("Weather Started in Region {0} (ID {1})\n{2}", weather.Region.Description,
                weather.Region.ID, weather);
    ...
    private void SendWeatherUpdate(RegionWeather weather, GamePlayer player)
    {
        ...
        if (weather.StartTime == 0)
            ...
            player.Out.SendWeather(weather.CurrentPosition(SimpleScheduler.Ticks), weather.Width, weather.Speed,
                weather.FogDiffusion, weather.Intensity);
    }
                ...
                catch (Exception ex)
                {
                    if (log.IsErrorEnabled)
                        log.ErrorFormat("Error While Registering Region's Weather : {0} (ID:{1})\n{2}",
                            region.Description, region.ID, ex);
                }
            }
            else
            {
                if (log.IsWarnEnabled)
                    log.WarnFormat("Trying to Add Region {0} (ID:{1}) to WeatherManager while already Registered!",
                        region.Description, region.ID);
            }
        }
    }
    ...
    public void UnRegisterRegion(Region region)
    {
        ...
        lock (LockObject)
        {
            ...
            else
            {
                if (log.IsWarnEnabled)
                    log.WarnFormat(
                        "Trying to Remove Region {0} (ID:{1}) from WeatherManager but was not registered!",
                        region.Description, region.ID);
            }
        }
    }
