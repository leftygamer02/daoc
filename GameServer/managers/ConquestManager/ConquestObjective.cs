...
public class ConquestObjective
{
    ...
    public bool ActiveFlags =>
        ObjectiveOne != null && ObjectiveTwo != null && ObjectiveThree != null && ObjectiveFour != null;
    ...
    {
        foreach (var player in ConquestService.ConquestManager.GetContributors())
        {
            if (player.CurrentRegion != Keep.CurrentRegion || player.Level < 40) continue;
            player.Out.SendMessage($"The realm thanks you for your efforts in the conquest.", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            var RPBase = _realmPointTickAward;
            var flagMod = 1 + 0.25 * GetNumFlagsOwnedByRealm(player.Realm);
