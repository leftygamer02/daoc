...
public class GameLotMarker : GameStaticItem
{
    ...
    public override IList GetExamineMessages(GamePlayer player)
    {
        ...
        else if (!string.IsNullOrEmpty(DatabaseItem.Name)) list.Add(" It is owned by " + DatabaseItem.Name + ".");
    ...
    public override bool Interact(GamePlayer player)
    {
        ...
        if (house != null)
            ...
            if (house.HouseNumber != DatabaseItem.HouseNumber &&
                player.Client.Account.PrivLevel != (int) ePrivLevel.Admin)
            {
        ...
        if (string.IsNullOrEmpty(DatabaseItem.OwnerID))
        {
            player.Out.SendCustomDialog(
                ...
                Money.GetString(HouseTemplateMgr.GetLotPrice(DatabaseItem)) + "!", BuyLot);
        }
        else
        {
    ...
    private void BuyLot(GamePlayer player, byte response)
    {
        ...
        lock (DatabaseItem) // Mannen 10:56 PM 10/30/2006 - Fixing every lock(this)
        {
            ...
            if (HouseMgr.GetHouseNumberByPlayer(player) != 0 &&
                player.Client.Account.PrivLevel != (int) ePrivLevel.Admin)
            {
                ChatUtil.SendMerchantMessage(player,
                    "You already own another lot or house (Number " + HouseMgr.GetHouseNumberByPlayer(player) +
                    ").");
                return;
            }

            var totalCost = HouseTemplateMgr.GetLotPrice(DatabaseItem);
    ...
    public override bool ReceiveItem(GameLiving source, InventoryItem item)
    {
        ...
        if (HouseMgr.IsOwner(DatabaseItem, player))
        {
            ...
            InventoryLogging.LogInventoryAction(player,
                "(HOUSE;" + (CurrentHouse == null ? DatabaseItem.HouseNumber : CurrentHouse.HouseNumber) + ")",
    ...
    private void CreateHouse(GamePlayer player, int model)
    {
        ...
        if (player.Guild != null) DatabaseItem.Emblem = player.Guild.Emblem;
        ...
        if (model != 0)
        {
            ...
            foreach (GamePlayer p in player.GetPlayersInRadius(500)) house.Exit(p, true);
