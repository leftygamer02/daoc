...
public class HouseMgr
{
    ...
    public static bool Start(GameClient client = null)
    {
        ...
        {
            var reg = WorldMgr.GetRegion(entry.id);
        ...
        if (log.IsInfoEnabled)
            log.Info("[Housing] Loaded " + houses + " houses and " + lotmarkers + " lotmarkers in " + regions +
                     " regions!");

        if (client != null)
            client.Out.SendMessage(
                "Loaded " + houses + " houses and " + lotmarkers + " lotmarkers in " + regions + " regions!",
    ...
    public static string LoadHousingForRegion(ushort regionID)
    {
        ...
        foreach (var house in regionHousing)
        {
            // if we already loaded this house, that's no bueno, but just skip
            if (housesForRegion.ContainsKey(house.HouseNumber))
                continue;
        ...
        var results = "[Housing] Loaded " + houses + " houses and " + lotmarkers + " lotmarkers for region " +
                      regionID + "!";
    ...
    {
        var spawnType = eLotSpawnType.Marker;
        ...
        if (string.IsNullOrEmpty(house.OwnerID) == false)
        {
            ...
            if (newHouse.Model > 0) spawnType = eLotSpawnType.House;
        }
    ...
    {
        if (_houseList.ContainsKey(regionID)) _houseList.Remove(regionID);

        if (_idList.ContainsKey(regionID)) _idList.Remove(regionID);
    }
    ...
    public static void AddHouse(House house)
    {
        ...
        else
        {
            ...
            for (var i = HousingConstants.MinPermissionLevel; i < HousingConstants.MaxPermissionLevel + 1; i++)
            {
                if (house.PermissionLevels.ContainsKey(i))
                {
                    var oldPermission = house.PermissionLevels[i];
                    if (oldPermission != null) GameServer.Database.DeleteObject(oldPermission);
                }
    ...
    public static bool UpgradeHouse(House house, InventoryItem deed)
    {
        ...
        foreach (var player in house.GetAllPlayersInHouse()) player.LeaveHouse();

        // if there is a consignment merchant, we have to re-initialize since we changed the house
        var merchant =
            DOLDB<HouseConsignmentMerchant>.SelectObject(DB.Column("HouseNumber").IsEqualTo(house.HouseNumber));
        long oldMerchantMoney = 0;
        if (merchant != null) oldMerchantMoney = merchant.Money;
        ...
        if (merchant != null) house.AddConsignment(oldMerchantMoney);
    ...
    public static void RemoveHouse(House house)
    {
        ...
        if (house.Model > 0)
            ...
            log.WarnFormat("Lotmarker #{0} owned by {1} had rent due, lot now for sale!", house.HouseNumber,
                house.DatabaseItem.Name);
    ...
    public static void RemoveHouseItems(House house)
    {
        ...
        var iobjs =
            DOLDB<DBHouseIndoorItem>.SelectObjects(DB.Column("HouseNumber").IsEqualTo(house.HouseNumber));
        GameServer.Database.DeleteObject(iobjs);
        house.IndoorItems.Clear();

        var oobjs =
            DOLDB<DBHouseOutdoorItem>.SelectObjects(DB.Column("HouseNumber").IsEqualTo(house.HouseNumber));
        GameServer.Database.DeleteObject(oobjs);
        house.OutdoorItems.Clear();

        var hpobjs =
            DOLDB<DBHouseHookpointItem>.SelectObjects(DB.Column("HouseNumber").IsEqualTo(house.HouseNumber));
        GameServer.Database.DeleteObject(hpobjs);

        foreach (var item in house.HousepointItems.Values)
    ...
    public static void RemoveHousePermissions(House house)
    {
        ...
        {
            var guild = GuildMgr.GetGuildByName(house.DatabaseItem.GuildName);
            if (guild != null) guild.GuildHouseNumber = 0;
        }
        ...
        var pobjs =
            DOLDB<DBHousePermissions>.SelectObjects(DB.Column("HouseNumber").IsEqualTo(house.HouseNumber));
        GameServer.Database.DeleteObject(pobjs);
        house.PermissionLevels.Clear();

        var cpobjs =
            DOLDB<DBHouseCharsXPerms>.SelectObjects(DB.Column("HouseNumber").IsEqualTo(house.HouseNumber));
    ...
    public static bool IsOwner(DBHouse house, GamePlayer player)
    {
        ...
        {
            foreach (var c in player.Client.Account.Characters)
    ...
    {
        var acctObjectIds = new List<string>();
    ...
    public static bool HouseTransferToGuild(GamePlayer player, House house)
    {
        ...
        if (consignmentMerchant != null &&
            (consignmentMerchant.DBItems().Count > 0 || consignmentMerchant.TotalMoney > 0))
        {
            ChatUtil.SendSystemMessage(player,
                "All items and money must be removed from your consigmment merchant in order to transfer this house!");
            return false;
        }

        // send house xfer prompt to player
        player.Out.SendCustomDialog(
            LanguageMgr.GetTranslation(player.Client.Account.Language, "Scripts.Player.Housing.TransferToGuild",
                player.Guild.Name), MakeGuildLot);
    ...
    private static void MakeGuildLot(GamePlayer player, byte response)
    {
        ...
        player.Guild.SendMessageToGuildMembers(
            LanguageMgr.GetTranslation(player.Client.Account.Language, "Scripts.Player.Housing.GuildNowOwns",
                player.Guild.Name, player.Name),
    ...
    public static int CheckRents(AuxECSGameTimer timer)
    {
        ...
        foreach (var entry in regs.Value)
        {
            ...
            var rent = GetRentByModel(house.Model);

            // Does this house need to pay rent?
            if (rent > 0L && diff.Days >= Properties.RENT_DUE_DAYS)
            {
                var lockboxAmount = house.KeptMoney;
                long consignmentAmount = 0;

                var consignmentMerchant = house.ConsignmentMerchant;
                if (consignmentMerchant != null) consignmentAmount = consignmentMerchant.TotalMoney;
                ...
                {
                    var remainingDifference = rent - lockboxAmount;
                    ...
                    else
                    {
                        ...
                        log.Warn(
                            $"[HOUSING] House {house.HouseNumber} owned by {house.Name} can't afford rent and is being repossesed! rentamount: {rent} lockboxAmount: {lockboxAmount} consignmentAmount: {consignmentAmount}");
        ...
        foreach (var h in houseRemovalList) RemoveHouse(h);
    ...
    public static IEnumerable GetHousesCloseToSpot(ushort regionid, int x, int y, int radius)
    {
        ...
        foreach (var house in GetHouses(regionid).Values)
        {
            var xdiff = house.X - x;
            var ydiff = house.Y - y;
            var range = xdiff * xdiff + ydiff * ydiff;
            if (range < 0)
                range *= -1;
            if (range > radiussqrt)
                continue;
