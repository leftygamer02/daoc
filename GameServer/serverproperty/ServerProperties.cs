...
public abstract class Properties
{
    ...
    public static void InitProperties()
    {
        ...
        foreach (var prop in propDict) Load(prop.Value.Item1, prop.Value.Item2, prop.Value.Item3);
    ...
    [ServerProperty("system", "tempproperties_to_register",
        "Serialized list of tempprop string, separated by semi-colon for be registered when a player disconnect",
        "LastPotionItemUsedTick;SpellAvailableTime;ItemUseDelay;LastChargedItemUsedTick")]
    public static string TEMPPROPERTIES_TO_REGISTER;

    /// <summary>
    /// Do we activate TempProperties manager Checkup on log in
    /// </summary>
    [ServerProperty("system", "activate_temp_properties_manager_checkup",
        "Do we activate TempProperties manager Checkup on log in?", true)]
    public static bool ACTIVATE_TEMP_PROPERTIES_MANAGER_CHECKUP;

    /// <summary>
    /// Do we activate TempProperties manager Checkup debug
    /// </summary>
    [ServerProperty("log", "activate_temp_properties_manager_checkup_debug",
        "Do we activate TempProperties manager Checkup debug?", false)]
    ...
    /// </summary>
    [ServerProperty("system", "ignore_too_long_outcoming_packet", "Shall we ignore too long outcoming packet ?",
        false)]
    ...
    /// </summary>
    [ServerProperty("system", "override_deck_rng", "Should we use raw RNG instead of Deck-Of-Cards normalization?",
        false)]
    ...
    /// </summary>
    [ServerProperty("system", "max_rewardquest_description_length",
        "Maximum length for reward quest description text to prevent client crashes.", 1000)]
    public static int MAX_REWARDQUEST_DESCRIPTION_LENGTH;

    /// <summary>
    /// If the server should only accept connections from staff
    /// </summary>
    [ServerProperty("system", "staff_login",
        "Staff Login Only - Edit this to set weather you wish staff to be the only ones allowed to Log in values True,False",
        false)]
    ...
    /// </summary>
    [ServerProperty("system", "client_version_min",
        "Minimum Client Version - Edit this to change which client version at the least have to be used: -1 = any, 1.80 = 180",
        -1)]
    ...
    /// </summary>
    [ServerProperty("system", "client_version_max",
        "Maximum Client Version - Edit this to change which client version at the most have to be used: -1 = any, 1.80 = 180",
        -1)]
    ...
    /// </summary>
    [ServerProperty("system", "client_enable_encryption_rc4",
        "Enable client RC4 encryption - Advanced user only, you need a special launcher to enable encryption",
        false)]
    ...
    /// </summary>
    [ServerProperty("system", "load_buff_tokens",
        "Should the server load buff tokens (npc and items), values True,False", true)]
    public static bool LOAD_BUFF_TOKENS;

    /// <summary>
    /// Should the server load Arrow Summoning items
    /// </summary>
    [ServerProperty("system", "load_arrow_summoning",
        "Should the server load Arrow Summoning items, values True,False", true)]
    public static bool LOAD_ARROW_SUMMONING;

    /// <summary>
    /// Should the server load Housing items
    /// </summary>
    [ServerProperty("system", "load_housing_items", "Should the server load Housing items, values True,False",
        true)]
    ...
    /// Disable Bug Reports
    /// </summary>
    [ServerProperty("system", "disable_bug_reports",
        "Set to true to disable bug reporting, and false to enable bug reporting", true)]
    public static bool DISABLE_BUG_REPORTS;

    /// <summary>
    /// Max bug repots in Queue
    /// </summary>
    [ServerProperty("system", "max_bugreport_queue",
        "Maximum number of bug reports allowed in queue.  0 to disabled max check", 100)]
    public static int MAX_BUGREPORT_QUEUE;

    /// <summary>
    /// The max number of players on the server
    /// </summary>
    [ServerProperty("system", "max_players",
        "Max Players - Edit this to set the maximum players allowed to connect at the same time", 4000)]
    ...
    /// </summary>
    [ServerProperty("system", "debug_load_regions",
        "Serialized list of region IDs that will load objects, separated by semi-colon (leave this blank to load all regions normally)",
        "")]
    ...
    /// </summary>
    [ServerProperty("system", "disabled_expansions",
        "Serialized list of disabled expansions IDs, expansion IDs are client type seperated by ;", "")]
    ...
    /// </summary>
    [ServerProperty("system", "allow_customize_face_after_creation",
        "Allow player to change their character face after creation through customizing ?", true)]
    ...
    /// </summary>
    [ServerProperty("system", "allow_customize_stats_after_creation",
        "Allow player to change their character starting stats after creation through customizing ?", true)]
    public static bool ALLOW_CUSTOMIZE_STATS_AFTER_CREATION;

    /// <summary>
    /// StatSave Interval
    /// </summary>
    [ServerProperty("system", "statsave_interval",
        "Interval between each DB Stats store in minutes. -1 for deactivated.", -1)]
    public static int STATSAVE_INTERVAL;

    /// <summary>
    /// Bug Report Email Addresses
    /// </summary>
    [ServerProperty("system", "bug_report_email_addresses",
        "set to the email addresses you want bug reports sent to (bug reports will only send if the user has set an email address for his account, multiple addresses seperate with ;",
        "")]
    ...
    /// Ban Hackers
    /// </summary>
    [ServerProperty("system", "ban_hackers",
        "Should we ban hackers, if set to true, bans will be done, if set to false, kicks will be done", false)]
    ...
    [ServerProperty("system", "statprint_frequency",
        "How often (milliseconds) should statistics be printed on the server console.", 30000)]
    public static int STATPRINT_FREQUENCY;

    /// <summary>
    /// Should users be able to create characters in all realms using the same account
    /// </summary>
    [ServerProperty("system", "allow_all_realms",
        "should we allow characters to be created on all realms using a single account", false)]
    public static bool ALLOW_ALL_REALMS;

    /// <summary>
    /// This will Allow/Disallow dual loggins
    /// </summary>
    [ServerProperty("system", "allow_dual_logins",
        "Disable to disallow players to connect with more than 1 account at a time.", true)]
    ...
    /// </summary>
    [ServerProperty("system", "command_spam_delay",
        "The spam delay, default 1500ms before the same command can be issued again.", 1500)]
    ...
    /// </summary>
    [ServerProperty("system", "speedhack_tolerance",
        "Number of times speed hack detected before banning.  Multiples of 5 (20, 25, 30, etc)", 20)]
    public static int SPEEDHACK_TOLERANCE;

    /// <summary>
    /// Turn on move detect
    /// </summary>
    [ServerProperty("system", "enable_movedetect",
        "Should the move detect code be enabled to kick possible movement hackers?", false)]
    public static bool ENABLE_MOVEDETECT;

    /// <summary>
    /// Coords per second tolerance before player is identified as a hacker?
    /// </summary>
    [ServerProperty("system", "cps_tolerance",
        "Coords per second tolerance before player is identified as a hacker?", 1000)]
    public static int CPS_TOLERANCE;

    /// <summary>
    /// Time tolerance before player is identified as move hacker
    /// </summary>
    [ServerProperty("system", "cps_time_tolerance",
        "Time tolerance for CPS before player is identified as a move hacker?", 200)]
    public static int CPS_TIME_TOLERANCE;

    /// <summary>
    /// Z distance tolerance before player is identified as a jump hacker
    /// </summary>
    [ServerProperty("system", "jump_tolerance",
        "Z distance tolerance before player is identified as a jump hacker?", 200)]
    public static int JUMP_TOLERANCE;

    /// <summary>
    /// Display centered screen messages if a player enters an area.
    /// </summary>
    [ServerProperty("system", "display_area_enter_screen_desc",
        "Display centered screen messages if a player enters an area.", false)]
    ...
    /// </summary>
    [ServerProperty("system", "hours_uptime_between_shutdown",
        "Hours between a scheduled server shutdown (-1 = no scheduled restart)", -1)]
    ...
    /// </summary>
    [ServerProperty("system", "log_trades", "Should the server Log all trades a player makes, values True,False",
        false)]
    ...
    /// Log Email Addresses
    /// </summary>
    [ServerProperty("system", "log_email_addresses",
        "set to the email addresses you want logs automatically emailed to, multiple addresses seperate with ;",
        "")]
    ...
    /// </summary>
    [ServerProperty("log", "log_inventory_trade",
        "Enable trade logging in inventory log (log_inventory must be enabled)", true)]
    public static bool LOG_INVENTORY_TRADE;

    /// <summary>
    /// Enable craft logging in inventory log (log_inventory must be enabled)
    /// </summary>
    [ServerProperty("log", "log_inventory_craft",
        "Enable craft logging in inventory log (log_inventory must be enabled)", true)]
    public static bool LOG_INVENTORY_CRAFT;

    /// <summary>
    /// Enable loot logging in inventory log (log_inventory must be enabled)
    /// </summary>
    [ServerProperty("log", "log_inventory_loot",
        "Enable loot logging in inventory log (log_inventory must be enabled)", true)]
    public static bool LOG_INVENTORY_LOOT;

    /// <summary>
    /// Enable quest logging in inventory log (log_inventory must be enabled)
    /// </summary>
    [ServerProperty("log", "log_inventory_quest",
        "Enable quest logging in inventory log (log_inventory must be enabled)", true)]
    public static bool LOG_INVENTORY_QUEST;

    /// <summary>
    /// Enable merchant logging in inventory log (log_inventory must be enabled)
    /// </summary>
    [ServerProperty("log", "log_inventory_merchant",
        "Enable merchant logging in inventory log (log_inventory must be enabled)", true)]
    public static bool LOG_INVENTORY_MERCHANT;

    /// <summary>
    /// Enable other logging in inventory log (log_inventory must be enabled)
    /// </summary>
    [ServerProperty("log", "log_inventory_other",
        "Enable other logging in inventory log (log_inventory must be enabled)", true)]
    ...
    /// </summary>
    [ServerProperty("server", "minutes to kick",
        "How many minutes before kicking inactive player to char screen <Default 1hr> ", 60)]
    ...
    /// </summary>
    [ServerProperty("atlas", "pve_mob_damage_f1",
        "The first factor in the PVE mob damage equation. Lower hits harder.", 3.2)]
    public static double PVE_MOB_DAMAGE_F1;

    /// <summary>
    /// The second factor in the PVE mob damage equation. Lower hits harder.
    /// </summary>
    [ServerProperty("atlas", "pve_mob_damage_f2",
        "The second factor in the PVE mob damage equation. Lower hits harder.", 150.0)]
    ...
    /// </summary>
    [ServerProperty("server", "use_new_passives_ras_scaling", "Use new passives realmabilities scaling (1.108+) ?",
        false)]
    ...
    /// </summary>
    [ServerProperty("server", "use_new_actives_ras_scaling",
        "Use new actives realmabilities (5-Tiers) scaling (1.108+) ?", false)]
    public static bool USE_NEW_ACTIVES_RAS_SCALING;

    /// <summary>
    /// Use pre 1.105 train or livelike
    /// </summary>
    [ServerProperty("server", "custom_train", "Train is custom pre-1.105 one ? (false set it to livelike 1.105+)",
        true)]
    ...
    /// </summary>
    [ServerProperty("server", "motd",
        "The Server Message of the Day - Edit this to set what is displayed when a level 2+ character enters the game for the first time, set to \"\" for nothing",
        "Welcome to a Dawn of Light server, please edit this MOTD")]
    public static string MOTD;

    /// <summary>
    /// The message players get when they enter the game past level 1
    /// </summary>
    [ServerProperty("server", "starting_msg",
        "The Starting Mesage - Edit this to set what is displayed when a level 1 character enters the game for the first time, set to \"\" for nothing",
        "Welcome for your first time to a Dawn of Light server, please edit this Starter Message")]
    public static string STARTING_MSG;

    /// <summary>
    /// The broadcast type
    /// </summary>
    [ServerProperty("server", "broadcast_type",
        "Broadcast Type - Edit this to change what /b does, values 0 = disabled, 1 = area, 2 = visibility distance, 3 = zone, 4 = region, 5 = realm, 6 = server",
        1)]
    ...
    /// Anon Modifier
    /// </summary>
    [ServerProperty("server", "anon_modifier",
        "Various modifying options for anon, 0 = default, 1 = /who shows player but as ANON, -1 = disabled", 0)]
    ...
    /// </summary>
    [ServerProperty("server", "death_messages_all_realms",
        "Set to true if you want all realms to see other realms death and kill messages", false)]
    ...
    /// </summary>
    [ServerProperty("server", "disabled_commands",
        "Serialized list of disabled commands separated by semi-colon, example /realm;/toon;/quit", "")]
    ...
    /// </summary>
    [ServerProperty("server", "use_dblanguage",
        "Use Database Language datas instead of files (if empty = build the table from files)", false)]
    public static bool USE_DBLANGUAGE;

    /// <summary>
    /// Update existing rows within the LanguageSystem table from language files.
    /// </summary>
    [ServerProperty("server", "update_existing_db_system_sentences_from_files",
        "Update existing rows within the LanguageSystem table from language files.", false)]
    ...
    /// </summary>
    [ServerProperty("server", "region_max_objects",
        "Set the maximum number of objects allowed in a region.  Smaller numbers offer better performance.  This can't be changed while the server is running. (256 - 65535)",
        (ushort) 30000)]
    ...
    /// Show logins channel
    /// </summary>
    [ServerProperty("server", "show_logins_channel",
        "What channel should be used for login messages? See eChatType, default is System.", (byte) 0)]
    public static byte SHOW_LOGINS_CHANNEL;

    /// <summary>
    /// Enable PvE Speed
    /// </summary>
    [ServerProperty("server", "enable_pve_speed",
        "Set to true if you wish to enable the extra 25% increase to speed when not in combat or an RvR zone",
        true)]
    ...
    /// </summary>
    [ServerProperty("server", "enable_encumberance_speed_loss",
        "Set to true if you wish to enable the encumberance speed loss", true)]
    ...
    /// </summary>
    [ServerProperty("server", "use_new_tooltip_forcedupdate",
        "Set to true if you wish to enable the new 1.110+ Tooltip Forced update each time the server send a skill to a new client.",
        true)]
    ...
    /// </summary>
    [ServerProperty("server", "enable_albion_advanced_weapon_spec",
        "Set to true to determine damage variance for polearms and 2H weapons on 1H crush/slash/thrust spec.",
        true)]
    ...
    /// </summary>
    [ServerProperty("world", "set_difficulty_on_epic_encounters",
        "Tune encounters taggued <Epic Encounter>. 0 means auto adaptative, others values are % of the initial difficulty (100%=initial difficulty)",
        100)]
    ...
    /// </summary>
    [ServerProperty("world", "disabled_regions",
        "Serialized list of disabled region IDs, separated by semi-colon or a range with a dash (ie 1-5;7;9)", "")]
    ...
    [ServerProperty("world", "world_item_decay_time",
        "How long (milliseconds) will an item dropped on the ground stay in the world.", (uint) 180000)]
    public static uint WORLD_ITEM_DECAY_TIME;

    [ServerProperty("world", "world_pickup_distance",
        "How far before you can no longer pick up an object (loot for example).", 256)]
    public static int WORLD_PICKUP_DISTANCE;

    [ServerProperty("world", "world_day_increment",
        "Day Increment (0 to 512, default is 24).  Larger increments make shorter days.", (uint) 24)]
    public static uint WORLD_DAY_INCREMENT;

    [ServerProperty("world", "world_npc_update_interval",
        "How often (milliseconds) will npc's broadcast updates to the clients. Minimum allowed = 1000 (1 second). 0 will disable this update.",
        (uint) 8000)]
    public static uint WORLD_NPC_UPDATE_INTERVAL;

    [ServerProperty("world", "world_object_update_interval",
        "How often (milliseconds) will objects (static, housing, doors, broadcast updates to the clients. Minimum allowed = 10000 (10 seconds). 0 will disable this update.",
        (uint) 30000)]
    public static uint WORLD_OBJECT_UPDATE_INTERVAL;

    [ServerProperty("world", "world_playertoplayer_update_interval",
        "How often (milliseconds) will other players packet be broadcasted again to the clients. Minimum allowed = 1000 (1 seconds). 0 will disable this update.",
        (uint) 1000)]
    public static uint WORLD_PLAYERTOPLAYER_UPDATE_INTERVAL;

    [ServerProperty("world", "world_player_update_interval",
        "How often (milliseconds) will players be checked for updates. Minimum allowed = 100 (100 milliseconds).",
        (uint) 300)]
    public static uint WORLD_PLAYER_UPDATE_INTERVAL;

    [ServerProperty("world", "weather_check_interval",
        "How often (milliseconds) will weather be checked for a chance to start a storm.", 5 * 60 * 1000)]
    ...
    [ServerProperty("world", "weather_log_events",
        "Should weather events be shown in the Log (and on the console).", true)]
    public static bool WEATHER_LOG_EVENTS;

    /// <summary>
    /// Perform a LoS check on client with each mob
    /// </summary>
    [ServerProperty("world", "always_check_los",
        "Perform a LoS check before aggroing. This can involve a huge lag, handle with care!", false)]
    ...
    /// </summary>
    [ServerProperty("world", "check_los_during_cast_minimum_interval",
        "The minimum interval (milliseconds) between two LOS checks performed during a spell cast.", 200)]
    public static int CHECK_LOS_DURING_CAST_MINIMUM_INTERVAL;

    /// <summary>
    /// Interrupt cast if a LoS check fails before end of cast
    /// </summary>
    [ServerProperty("world", "check_los_during_cast_interrupt",
        "Should the casting animation be interrupted if a during cast LOS checks fails.", false)]
    public static bool CHECK_LOS_DURING_CAST_INTERRUPT;

    /// <summary>
    /// Perform LOS check between controlled NPC's and players
    /// </summary>
    [ServerProperty("world", "always_check_pet_los",
        "Should we perform LOS checks between controlled NPC's and players?", false)]
    public static bool ALWAYS_CHECK_PET_LOS;

    /// HPs gained per champion's level
    /// </summary>
    [ServerProperty("world", "hps_per_championlevel",
        "The amount of extra HPs gained each time you reach a new Champion's Level", 40)]
    public static int HPS_PER_CHAMPIONLEVEL;

    /// <summary>
    /// Time player must wait after failed task
    /// </summary>
    [ServerProperty("world", "task_pause_ticks",
    ...
    /// </summary>
    [ServerProperty("world", "timer_player_init",
        "Immunity Timer length when a player logs into game or zones into a new region, in seconds", 15)]
    public static int TIMER_PLAYER_INIT;

    /// <summary>
    /// Display the zonepoint with a choosen model
    /// </summary>
    [ServerProperty("world", "zonepoint_npctemplate",
        "Display the zonepoint with the following npctemplate. 0 for no display", 0)]
    public static int ZONEPOINT_NPCTEMPLATE;

    /// <summary>
    /// Line of Sight Manager Enable
    /// </summary>
    [ServerProperty("world", "losmgr_enable",
        "Enable the Line of Sight Manager where overriden methods are implemented.", false)]
    public static bool LOSMGR_ENABLE;

    /// <summary>
    /// Line of Sight Manager Debug Level
    /// </summary>
    [ServerProperty("world", "losmgr_debug_level",
        "Set the Level of Debug for the Line of Sight (LoS) Manager (do not set level 3 in production), 0 = no debug, 1 = info, 2 = warn, 3 = debug.",
        0)]
    ...
    /// </summary>
    [ServerProperty("world", "losmgr_cleanup_frequency",
        "How fast should the Line of Sight (LoS) Manager clean up its data (in milliseconds), don't get under 30000 ms, raise cleanup count if you put high number here.",
        120000)]
    ...
    /// </summary>
    [ServerProperty("world", "losmgr_cleanup_entries",
        "Number of Entries cleaned from Line of Sight (LoS) Manager each Cleanup ticks, if you need to go above 10 000 entries consider using a shorter cleanup frequency.",
        1000)]
    ...
    /// </summary>
    [ServerProperty("world", "losmgr_query_timeout",
        "Timeout (in milliseconds) until Line of Sight (LoS) Manager tries to resend a LoS check, -1 to disable (don't get under 100ms except for Local Network).",
        300)]
    ...
    /// </summary>
    [ServerProperty("world", "losmgr_player_vs_player_cache_timeout",
        "Set Timeout (in milliseconds) for PvP Line of Sight (LoS) Manager cache data, 0 to disable.", 200)]
    public static int LOSMGR_PLAYER_VS_PLAYER_CACHE_TIMEOUT;

    /// <summary>
    /// Line of Sight Manager PvE Cache Timeout
    /// </summary>
    [ServerProperty("world", "losmgr_player_vs_environment_cache_timeout",
        "Set Timeout (in milliseconds) for PvE Line of Sight (LoS) Manager cache data, 0 to disable. (Should be slightly above Brain Think timer to have any effect)",
        1500)]
    ...
    /// </summary>
    [ServerProperty("world", "losmgr_environment_vs_environment_cache_timeout",
        "Set Timeout (in milliseconds) for EvE Line of Sight (LoS) Manager cache data, 0 to disable. (Should be at least 2 times PvE cache to lower LoS Queries)",
        5000)]
    ...
    /// </summary>
    [ServerProperty("world", "losmgr_player_check_frequency",
        "Line of Sight (LoS) Manager will try to reduce player queries to this frequency (in millisecond), raise this if player are experiencing lags.",
        100)]
    ...
    /// </summary>
    [ServerProperty("world", "losmgr_player_vs_player_range_threshold",
        "Line of Sight (LoS) Manager won't check LoS for players within this range. (Should be low to prevent abuses)",
        32)]
    ...
    /// </summary>
    [ServerProperty("world", "losmgr_player_vs_environment_range_threshold",
        "Line of Sight (LoS) Manager won't check LoS for mobs and NPC within this range. (Shouldn't be under default to prevent LoS flood)",
        125)]
    ...
    /// </summary>
    [ServerProperty("world", "losmgr_environment__vs_environment_range_threshold",
    ...
    /// </summary>
    [ServerProperty("world", "losmgr_max_contamination_radius",
        "Line of Sight (LoS) Manager will update LoS of Mobs/NPCs in this range of the checker if target is a Mob, and highest value used for contamination LoS updates.",
        350)]
    ...
    /// </summary>
    [ServerProperty("world", "losmgr_npc_contamination_radius",
        "Line of Sight (LoS) Manager will update LoS of Mobs/NPCs in this range of the checker, and highest value used for Player contamination LoS updates.",
        250)]
    ...
    /// </summary>
    [ServerProperty("world", "losmgr_pet_contamination_radius",
        "Line of Sight (LoS) Manager will update LoS of Player's Pet in this range of the checker, keep value low to prevent abuses.",
        50)]
    ...
    /// </summary>
    [ServerProperty("world", "losmgr_players_contamination_radius",
        "Line of Sight (LoS) Manager will update LoS of Players in this range of the checker, should be disabled to prevent PvP abuses.",
        0)]
    ...
    /// </summary>
    [ServerProperty("world", "losmgr_guard_contamination_radius",
        "Line of Sight (LoS) Manager will update LoS of Keep Guards in this range of the checker, raising this value will pack more Guards when Aggroing.",
        200)]
    ...
    /// </summary>
    [ServerProperty("world", "losmgr_contamination_zfactor",
        "Line of Sight (LoS) Manager Contamination will use this to lower or raise the Z checks when updating LoS checks. 0 = Z must be exact, 1 = Z range is radius.",
        0.5)]
    ...
    /// </summary>
    [ServerProperty("server", "autoselect_caster",
        "Set to True if you wish beneficial spells to target the caster if the current target isn't valid.  Allows self-healing without changing targets.",
        false)]
    ...
    /// </summary>
    [ServerProperty("rates", "XP_Cap_Percent",
        "Maximum XP a player can earn given in percent of their level. Default is 125%", 125)]
    public static int XP_CAP_PERCENT;

    /// <summary>
    /// Xp Cap for a player in a group.  Given in percent of level.  Default is 125%
    /// </summary>
    [ServerProperty("rates", "XP_Group_Cap_Percent",
        "Maximum XP a player can earn while in a group, given in percent of their level. Default is 125%", 125)]
    public static int XP_GROUP_CAP_PERCENT;

    /// <summary>
    /// Xp Cap for a player vs player kill.  Given in percent of level.  Default is 125%
    /// </summary>
    [ServerProperty("rates", "XP_PVP_Cap_Percent",
        "Maximum XP a player can earn killing another player, given in percent of their level. Default is 125%",
        125)]
    ...
    /// </summary>
    [ServerProperty("rates", "XP_HardCap_Percent",
        "Hardcap XP a player can earn after all other adjustments are applied. Given in percent of their level. Default is 500%",
        500)]
    ...
    /// The Experience Rate
    /// </summary>
    [ServerProperty("rates", "xp_rate",
        "The Experience Points Rate Modifier - Edit this to change the rate at which you gain experience points e.g 1.5 is 50% more 2.0 is twice the amount (100%) 0.5 is half the amount (50%)",
        1.0)]
    ...
    /// </summary>
    [ServerProperty("rates", "cl_xp_rate",
        "The Champion Level Experience Points Rate Modifier - Edit this to change the rate at which you gain CL experience points e.g 1.5 is 50% more 2.0 is twice the amount (100%) 0.5 is half the amount (50%)",
        1.0)]
    ...
    /// </summary>
    [ServerProperty("rates", "rp_rate",
        "The Realm Points Rate Modifier - Edit this to change the rate at which you gain realm points e.g 1.5 is 50% more 2.0 is twice the amount (100%) 0.5 is half the amount (50%)",
        1.0)]
    ...
    [ServerProperty("rates", "bp_rate",
        "The Bounty Points Rate Modifier - Edit this to change the rate at which you gain bounty points e.g 1.5 is 50% more 2.0 is twice the amount (100%) 0.5 is half the amount (50%)",
        1.0)]
    ...
    /// </summary>
    [ServerProperty("rates", "pve_melee_damage",
        "The PvE Melee Damage Modifier - Edit this to change the amount of melee damage done when fighting mobs e.g 1.5 is 50% more damage 2.0 is twice the damage (100%) 0.5 is half the damage (50%)",
        1.0)]
    ...
    /// </summary>
    [ServerProperty("rates", "pve_spell_damage",
        "The PvE Spell Damage Modifier - Edit this to change the amount of spell damage done when fighting mobs e.g 1.5 is 50% more damage 2.0 is twice the damage (100%) 0.5 is half the damage (50%)",
        1.0)]
    ...
    /// </summary>
    [ServerProperty("rates", "pve_spell_conhitpercent",
        "The percent per con (1 = OJ, 2 = red ...) subtracted to hitchance for spells in PVE  Must be >= 0.  0 is none, 5 is 5% per level, etc.  Default is 10%",
        (uint) 10)]
    ...
    /// </summary>
    [ServerProperty("rates", "pvp_melee_damage",
        "The PvP Melee Damage Modifier - Edit this to change the amount of melee damage done when fighting players e.g 1.5 is 50% more damage 2.0 is twice the damage (100%) 0.5 is half the damage (50%)",
        1.0)]
    ...
    /// </summary>
    [ServerProperty("rates", "pvp_spell_damage",
        "The PvP Spell Damage Modifier - Edit this to change the amount of spell damage done when fighting players e.g 1.5 is 50% more damage 2.0 is twice the damage (100%) 0.5 is half the damage (50%)",
        1.0)]
    ...
    /// </summary>
    [ServerProperty("rates", "block_cap",
        "Block Rate Cap Modifier - Edit this to change the highest possible block rate against an enemy (Hard Cap) in game e.g .60 = 60%",
        0.60)]
    ...
    /// </summary>
    [ServerProperty("rates", "evade_cap",
        "Evade Rate Cap Modifier - Edit this to change the highest possible evade rate against an enemy (Hard Cap) in game e.g .50 = 50%",
        0.50)]
    ...
    /// </summary>
    [ServerProperty("rates", "parry_cap",
        "Parry Rate Cap Modifier - Edit this to change the highest possible parry rate against an enemy (Hard Cap) in game e.g .50 = 50%",
        0.50)]
    ...
    /// </summary>
    [ServerProperty("rates", "cs_opening_effectiveness",
        "Critical strike opening style effectiveness.  Increase this to make CS styles BS, BSII and Perf Artery more effective",
        1.0)]
    ...
    /// </summary>
    [ServerProperty("rates", "money_drop",
        "Money Drop Modifier - Edit this to change the amount of money which is dropped e.g 1.5 is 50% more 2.0 is twice the amount (100%) 0.5 is half the amount (50%)",
        1.0)]
    ...
    /// </summary>
    [ServerProperty("rates", "base_smallchest_chance",
        "Percentage chance that a mob will drop a small chest of money", 10)]
    public static int BASE_SMALLCHEST_CHANCE;

    /// <summary>
    /// The math multiplier for small chest value
    /// </summary>
    [ServerProperty("rates", "smallchest_multiplier",
        "The math multiplier for small chest value. Increase for more gold", 10)]
    public static int SMALLCHEST_MULTIPLIER;

    /// <summary>
    /// The large chest drop chance
    /// </summary>
    [ServerProperty("rates", "base_largechest_chance",
        "Percentage chance that a mob will drop a large chest of money", 5)]
    public static int BASE_LARGECHEST_CHANCE;

    /// <summary>
    /// The math multiplier for small chest value
    /// </summary>
    [ServerProperty("rates", "largechest_multiplier",
        "The math multiplier for large chest value. Increase for more gold", 17)]
    public static int LARGECHEST_MULTIPLIER;

    /// <summary>
    /// The time until a player is worth rps again after death
    /// </summary>
    [ServerProperty("rates", "rp_worth_seconds",
        "Realm Points Worth Seconds - Edit this to change how many seconds until a player is worth RPs again after being killed ",
        300)]
    ...
    /// </summary>
    [ServerProperty("rates", "item_condition_loss_chance",
        "What chance does armor or weapon have to lose condition?", 5)]
    public static int ITEM_CONDITION_LOSS_CHANCE;

    /// <summary>
    /// Under level 35 mount speed, live like = 135
    /// </summary>
    [ServerProperty("rates", "mount_under_level_35_speed",
        "What is the speed of player controlled mounts under level 35?", (short) 135)]
    public static short MOUNT_UNDER_LEVEL_35_SPEED;

    /// <summary>
    /// Over level 35 mount speed, live like = 145
    /// </summary>
    [ServerProperty("rates", "mount_over_level_35_speed",
        "What is the speed of player controlled mounts over level 35?", (short) 145)]
    public static short MOUNT_OVER_LEVEL_35_SPEED;

    /// <summary>
    /// Relic Bonus Modifier
    /// </summary>
    [ServerProperty("rates", "relic_owning_bonus",
        "Relic Owning Bonus in percent per relic (default 10%) in effect when owning enemy relic", (short) 10)]
    ...
    /// </summary>
    [ServerProperty("npc", "mob_autoset_str_multiplier",
        "Multiplier to use when auto-setting STR stat. Multiplied by 10 when applied. ", 1.0)]
    ...
    /// </summary>
    [ServerProperty("npc", "pet_autoset_str_base", "Base Value to use when auto-setting Pet STR stat. ",
        (short) 30)]
    ...
    /// </summary> 
    [ServerProperty("npc", "pet_autoset_str_multiplier",
        "Multiplier to use when auto-setting Pet STR stat. Multiplied by 10 when applied. ", 1.0)]
    public static double PET_AUTOSET_STR_MULTIPLIER;

    /// Base Value to use when auto-setting pet CON stat.
    /// </summary>
    [ServerProperty("npc", "pet_autoset_con_base", "Base Value to use when auto-setting Pet CON stat. ",
        (short) 30)]
    ...
    /// </summary>
    [ServerProperty("npc", "pet_autoset_dex_base", "Base Value to use when auto-setting Pet DEX stat. ",
        (short) 30)]
    ...
    /// </summary>
    [ServerProperty("npc", "pet_autoset_qui_base", "Base Value to use when auto-setting Pet QUI stat. ",
        (short) 30)]
    ...
    /// </summary>
    [ServerProperty("npc", "pet_autoset_int_base", "Multiplier to use when auto-setting Pet INT stat. ",
        (short) 30)]
    ...
    /// </summary>
    [ServerProperty("npc", "necro_pet_str_base",
        "Base value to use when setting strength for most necromancer pets.", (short) 60)]
    public static short NECRO_PET_STR_BASE;

    /// <summary>
    /// Multiplier to use when setting strength for most necromancer pets.
    /// </summary>
    [ServerProperty("npc", "necro_pet_str_multiplier",
        "Multiplier to use when setting strength for most necromancer pets.", 1.0)]
    public static double NECRO_PET_STR_MULTIPLIER;

    /// <summary>
    /// Base value to use when setting constitution for most necromancer pets.
    /// </summary>
    [ServerProperty("npc", "necro_pet_con_base",
        "Base value to use when setting constitution for most necromancer pets.", (short) 60)]
    public static short NECRO_PET_CON_BASE;

    /// <summary>
    /// Multiplier to use when setting constitution for most necromancer pets.
    /// </summary>
    [ServerProperty("npc", "necro_pet_con_multiplier",
        "Multiplier to use when setting constitution for most necromancer pets.", 0.5)]
    public static double NECRO_PET_CON_MULTIPLIER;

    /// <summary>
    /// Base value to use when setting dexterity for most necromancer pets.
    /// </summary>
    [ServerProperty("npc", "necro_pet_dex_base",
        "Base value to use when setting dexterity for most necromancer pets.", (short) 60)]
    public static short NECRO_PET_DEX_BASE;

    /// <summary>
    /// Multiplier to use when setting dexterity for most necromancer pets.
    /// </summary>
    [ServerProperty("npc", "necro_pet_dex_multiplier",
        "Multiplier to use when setting dexterity for most necromancer pets.", 0.0)]
    public static double NECRO_PET_DEX_MULTIPLIER;

    /// <summary>
    /// Base value to use when setting quickness for most necromancer pets.
    /// </summary>
    [ServerProperty("npc", "necro_pet_qui_base",
        "Base value to use when setting quickness for most necromancer pets.", (short) 60)]
    public static short NECRO_PET_QUI_BASE;

    /// <summary>
    /// Multiplier to use when setting quickness for most necromancer pets.
    /// </summary>
    [ServerProperty("npc", "necro_pet_qui_multiplier",
        "Multiplier to use when setting quickness for most necromancer pets.", 0.3333)]
    public static double NECRO_PET_QUI_MULTIPLIER;

    /// <summary>
    /// Base value to use when setting intelligence for most necromancer pets.
    /// </summary>
    [ServerProperty("npc", "necro_pet_int_base",
        "Base value to use when setting intelligence for most necromancer pets.", (short) 60)]
    public static short NECRO_PET_INT_BASE;

    /// <summary>
    /// Multiplier to use when setting intelligence for most necromancer pets.
    /// </summary>
    [ServerProperty("npc", "necro_pet_int_multiplier",
        "Multiplier to use when setting intelligence for most necromancer pets.", 0.3333)]
    public static double NECRO_PET_INT_MULTIPLIER;

    /// <summary>
    /// Base value to use when setting strength for greater necroservant pets.
    /// </summary>
    [ServerProperty("npc", "necro_greater_pet_str_base",
        "Base value to use when setting strength for greater necroservant pets.", (short) 60)]
    public static short NECRO_GREATER_PET_STR_BASE;

    /// <summary>
    /// Multiplier to use when setting strength for greater necroservant pets.
    /// </summary>
    [ServerProperty("npc", "necro_greater_pet_str_multiplier",
        "Multiplier to use when setting strength for greater necroservant pets.", 0.0)]
    ...
    /// </summary>
    [ServerProperty("npc", "necro_greater_pet_con_base",
        "Base value to use when setting constitution for greater necroservant pets.", (short) 60)]
    public static short NECRO_GREATER_PET_CON_BASE;

    /// <summary>
    /// Multiplier to use when setting constitution for greater necroservant pets.
    /// </summary>
    [ServerProperty("npc", "necro_greater_pet_con_multiplier",
        "Multiplier to use when setting constitution for greater necroservant pets.", 0.3333)]
    ...
    /// </summary>
    [ServerProperty("npc", "necro_greater_pet_dex_base",
        "Base value to use when setting dexterity for greater necroservant pets.", (short) 60)]
    public static short NECRO_GREATER_PET_DEX_BASE;

    /// <summary>
    /// Multiplier to use when setting dexterity for greater necroservant pets.
    /// </summary>
    [ServerProperty("npc", "necro_greater_pet_dex_multiplier",
        "Multiplier to use when setting dexterity for greater necroservant pets.", 0.5)]
    ...
    /// </summary>
    [ServerProperty("npc", "necro_greater_pet_qui_base",
        "Base value to use when setting quickness for greater necroservant pets.", (short) 60)]
    public static short NECRO_GREATER_PET_QUI_BASE;

    /// <summary>
    /// Multiplier to use when setting quickness for greater necroservant pets.
    /// </summary>
    [ServerProperty("npc", "necro_greater_pet_qui_multiplier",
        "Multiplier to use when setting quickness for greater necroservant pets.", 1.0)]
    ...
    /// </summary>
    [ServerProperty("npc", "necro_greater_pet_int_base",
        "Base value to use when setting intelligence for greater necroservant pets.", (short) 60)]
    public static short NECRO_GREATER_PET_INT_BASE;

    /// <summary>
    /// Multiplier to use when setting intelligence for greater necroservant pets.
    /// </summary>
    [ServerProperty("npc", "necro_greater_pet_int_multiplier",
        "Multiplier to use when setting intelligence for greater necroservant pets.", 0.3333)]
    ...
    /// </summary>
    [ServerProperty("npc", "pet_scale_spell_max_level",
        "Disabled if 0 or less.  If greater than 0, this value is the level at which pets cast their spells at 100% effectivness, so choose spells for pets assuming they're at the level set here.  Live is max pet level, 44 or 50 depending on patch.",
        0)]
    ...
    /// Scale pet spell values according to their level?
    /// </summary>
    [ServerProperty("npc", "pet_bd_commander_taunt_multiplier",
        "Percentage of damage that BD commanders get as extra aggro when taunting, e.g. a taunting BD commander gets 150% normal aggro at 50, 200% at 100, 250% at 150 etc. ",
        150)]
    ...
    /// Scale pet spell values according to their level?
    /// </summary>
    [ServerProperty("npc", "pet_cap_bd_minion_spell_scaling_by_spec",
        "When scaling BD minion spells, do we cap the level they scale do by the BD's spec level?  This provides an incentive to spec darkness and suppression and use items that boost them.",
        false)]
    ...
    /// </summary>
    [ServerProperty("npc", "npc_min_respawn_interval",
        "Minimum respawn time, in minutes, for npc's without a set respawninterval", 5)]
    public static int NPC_MIN_RESPAWN_INTERVAL;

    /// <summary>
    /// Respawn Interval for Shrouded Isles Epic Encounter
    /// </summary>
    [ServerProperty("world", "set_si_epic_encounter_respawninterval",
        "Respawn Time, in minutes, for Epic Encounters in Shrouded Isles", 60)]
    public static int SET_SI_EPIC_ENCOUNTER_RESPAWNINTERVAL;

    /// <summary>
    /// Respawn Interval for Normal Epic Game Boss Encounter
    /// </summary>
    [ServerProperty("world", "set_epic_game_encounter_respawninterval",
        "Respawn Time, in minutes, for Normal Epic Game Encounters", 60)]
    public static int SET_EPIC_GAME_ENCOUNTER_RESPAWNINTERVAL;

    /// <summary>
    /// Respawn Interval for Epic Quest Mobs
    /// </summary>
    [ServerProperty("world", "set_epic_quest_encounter_respawninterval",
        "Respawn Time, in minutes, for Epic Quest Encounters", 30)]
    public static int SET_EPIC_QUEST_ENCOUNTER_RESPAWNINTERVAL;

    /// <summary>
    /// Weapon damage cap for epic encounters that use melee weapons
    /// </summary>
    [ServerProperty("npc", "set_epic_encounter_weapon_damage_cap",
        "Maximum damage cap multipler for epic encounters that use melee weapons", 1.5)]
    ...
    /// </summary>
    [ServerProperty("npc", "gamenpc_hp_gain_per_con",
        "How many hitpoints per point of CON above gamenpc_base_con should an NPC gain", 2)]
    ...
    /// </summary>
    [ServerProperty("npc", "gamenpc_followcheck_time",
        "How often, in milliseconds, to check follow distance. Lower numbers make NPC follow closer but increase load on server.",
        500)]
    ...
    /// </summary>
    [ServerProperty("npc", "gamenpc_default_classtype",
        "Change the classtype of any npc of classtype DOL.GS.GameNPC to this.", "DOL.GS.GameNPC")]
    ...
    /// </summary>
    [ServerProperty("npc", "npc_heal_threshold",
        "NPCs, including pets, heal targets whose health falls below this percentage.", 75)]
    public static int NPC_HEAL_THRESHOLD;

    /// <summary>
    /// Charmed NPC heal when a target is below what percentage of their health?
    /// </summary>
    [ServerProperty("npc", "charmed_npc_heal_threshold",
        "Charmed NPC, heal targets whose health falls below this percentage.", 50)]
    ...
    /// </summary>
    [ServerProperty("npc", "expand_wild_minion",
        "Expand the Wild Minion RA to also improve crit chance for ranged and spell attacks?", false)]
    ...
    /// </summary>
    [ServerProperty("pvp", "RvRLinkDeathRelogGracePeriod",
        "The Grace Period in minutes, to allow to relog near enemy structure after a link death.", "20")]
    public static string RVR_LINK_DEATH_RELOG_GRACE_PERIOD;

    /// <summary>
    /// PvP Immunity Timer - Killed by Mobs
    /// </summary>
    [ServerProperty("pvp", "Timer_Killed_By_Mob", "Immunity Timer When player killed in PvP, in seconds",
        30)]
    //30 seconds default
    ...
    /// </summary>
    [ServerProperty("pvp", "Timer_Killed_By_Player", "Immunity Timer When player killed in PvP, in seconds",
        120)]
    //2 min default
    ...
    /// </summary>
    [ServerProperty("pvp", "Timer_Region_Changed", "Immunity Timer when player changes regions, in seconds",
        30)]
    //30 seconds default
    ...
    /// </summary>
    [ServerProperty("pvp", "Timer_Game_Entered", "Immunity Timer when player enters the game, in seconds",
        10)]
    //10 seconds default
    ...
    /// </summary>
    [ServerProperty("pvp", "Timer_PvP_Teleport",
        "Immunity Timer when player teleports within the same region, in seconds", 30)]
    //30 seconds default
    ...
    /// </summary>
    [ServerProperty("pvp", "Relic_Return_Time",
        "A lost relic will automatically returns to its defined point, in seconds", 20 * 60)]
    //20 mins default
    ...
    /// </summary>
    [ServerProperty("pvp", "bg_zones_closed_message", "Message to display to player if BG zones are closed",
        "The battlegrounds are not open on this server.")]
    public static string BG_ZONES_CLOSED_MESSAGE;

    /// <summary>
    /// How many players are required on the relic pad to trigger the pillar?
    /// </summary>
    [ServerProperty("pvp", "relic_players_required_on_pad",
        "How many players are required on the relic pad to trigger the pillar?", 16)]
    ...
    /// </summary>
    [ServerProperty("pvp", "pvp_realm_timer_minutes",
        "# of minutes an account must wait to change realms after PvP combat. 0 disables the timer", 0)]
    ...
    [ServerProperty("conquest", "conquest_capture_award",
        "How many RPs/orbs awarded for capturing the conquest target?", 1000)]
    ...
    /// </summary>
    [ServerProperty("keeps", "keep_guard_los_check_time",
        "Number of seconds between allowed LOS checks for keep guards", 5)]
    public static int KEEP_GUARD_LOS_CHECK_TIME;

    /// <summary>
    /// The number of players needed for claiming
    /// </summary>
    [ServerProperty("keeps", "claim_num",
        "Players Needed For Claim - Edit this to change the amount of players required to claim a keep, towers are half this amount",
        8)]
    ...
    /// Use Keep Balancing
    /// </summary>
    [ServerProperty("keeps", "use_keep_balancing",
        "Set to true if you want keeps to be higher level in NF the less you have, and lower level the more you have",
        false)]
    ...
    /// </summary>
    [ServerProperty("keeps", "use_live_keep_bonuses",
        "Set to true if you want to use the live keeps bonuses, for example 3% extra xp", false)]
    public static bool USE_LIVE_KEEP_BONUSES;

    /// <summary>
    /// Use Supply Chain
    /// </summary>
    [ServerProperty("keeps", "use_supply_chain",
        "Set to true if you want to use the live supply chain for keep teleporting, set to false to allow teleporting to any keep that your realm controls (and towers)",
        false)]
    ...
    /// </summary>
    [ServerProperty("keeps", "enable_keep_upgrade_timer",
        "Enable the keep upgrade timer to slowly raise keep levels?", false)]
    ...
    /// </summary>
    [ServerProperty("keeps", "set_structures_toughness",
        "This value is % of total damages inflicted to walls. (100=full damages)", 100)]
    public static int SET_STRUCTURES_TOUGHNESS;

    /// <summary>
    /// Define toughness for keep doors: 100 is 100% player's damages inflicted.
    /// </summary>
    [ServerProperty("keeps", "set_keep_door_toughness",
        "This value is % of total damages inflicted to level 1 door. (100=full damages)", 100)]
    public static int SET_KEEP_DOOR_TOUGHNESS;

    /// <summary>
    /// Define toughness for tower doors: 100 is 100% player's damages inflicted.
    /// </summary>
    [ServerProperty("keeps", "set_tower_door_toughness",
        "This value is % of total damages inflicted to level 1 door. (100=full damages)", 100)]
    ...
    /// </summary>
    [ServerProperty("keeps", "structures_allowpetattack", "Allow player pets to attack keep and tower walls?",
        true)]
    ...
    /// </summary>
    [ServerProperty("keeps", "tower_rp_claim_multiplier",
        "Integer multiplier used in determining RP reward for claiming towers.", 100)]
    public static int TOWER_RP_CLAIM_MULTIPLIER;

    /// <summary>
    /// Multiplier used in determining RP reward for keeps.
    /// </summary>
    [ServerProperty("keeps", "keep_rp_claim_multiplier",
        "Integer multiplier used in determining RP reward for claiming keeps.", 1000)]
    ...
    /// </summary>
    [ServerProperty("keeps", "lord_rp_worth_seconds",
        "The number of seconds from last kill the this lord is worth no RP.", 300)]
    ...
    /// </summary>
    [ServerProperty("keeps", "keep_rp_multiplier",
        "Integer multiplier used to increase/decrease RP worth based on keep level difference from 50.", 50)]
    public static int KEEP_RP_MULTIPLIER;

    /// <summary>
    /// Multiplier used to add or subtract RP worth based on tower level difference from 50.
    /// </summary>
    [ServerProperty("keeps", "tower_rp_multiplier",
        "Integer multiplier used to increase/decrease RP worth based on tower level difference from 50.", 50)]
    public static int TOWER_RP_MULTIPLIER;

    /// <summary>
    /// Multiplier used to add or subtract RP worth based on upgrade level > 1
    /// </summary>
    [ServerProperty("keeps", "upgrade_multiplier",
        "Integer multiplier used to increase/decrease RP worth based on upgrade level (0..10)", 100)]
    ...
    /// </summary>
    [ServerProperty("keeps", "keep_balance_multiplier",
        "Multiplier used to determine keep level changes when balancing.  For each keep above or below normal multiply by this.",
        1.0)]
    ...
    /// Multiplier used to determine keep level changes when balancing.  For each keep above or below normal multiply by this.
    /// </summary>
    [ServerProperty("keeps", "tower_balance_multiplier",
        "Multiplier used to determine tower level changes when balancing.  For each keep above or below normal multiply by this.",
        0.15)]
    ...
    /// </summary>
    [ServerProperty("keeps", "keep_guard_level_multiplier",
        "Multiplier used to determine keep guard levels.  This is applied to the bonus level (usually 4) and added after balance adjustments.",
        1.6)]
    ...
    /// </summary>
    [ServerProperty("keeps", "pet_spam_damage_multiplier",
        "Modifier used to adjust damage for pet spam classes (currently animist and theurgist).", 1.0)]
    public static double PET_SPAM_DAMAGE_MULTIPLIER;

    /// <summary>
    /// Multiplier used to determine keep guard levels.  This is applied to the bonus level (usually 4) and added after balance adjustments.
    /// </summary>
    [ServerProperty("keeps", "tower_guard_level_multiplier",
        "Multiplier used to determine tower guard levels.  This is applied to the bonus level (usually 4) and added after balance adjustments.",
        1.0)]
    ...
    /// </summary>
    [ServerProperty("keeps", "use_new_keeps",
    ...
    /// </summary>
    [ServerProperty("keeps", "autoequip_guards_loaded_from_db",
        "Should guards loaded from db be equipped by Keepsystem? (false=load equipment from db)", true)]
    ...
    /// </summary>
    [ServerProperty("keeps", "automodel_guards_loaded_from_db",
        "Should guards loaded from db be modeled by Keepsystem? (false=load from db)", true)]
    public static bool AUTOMODEL_GUARDS_LOADED_FROM_DB;

    /// <summary>
    /// Are unclaimed keeps considered the enemy in PvP mode?
    /// </summary>
    [ServerProperty("keeps", "pvp_unclaimed_keeps_enemy", "Are unclaimed keeps considered the enemy in PvP mode?",
        false)]
    ...
    /// </summary>
    [ServerProperty("keeps", "teleport_login_near_enemy_keep",
        "Should players that log near enemy keeps be teleported to a safe area?", true)]
    ...
    /// </summary>
    [ServerProperty("keeps", "teleport_login_bg_level_exceeded",
        "Should players that exceed BG level cap be moved out of BG when logging in?", true)]
    public static bool TELEPORT_LOGIN_BG_LEVEL_EXCEEDED;

    /// <summary>
    /// Do you allowed player to climb towers?
    /// </summary>
    [ServerProperty("keeps", "allow_tower_climb",
        "Do you allowed player to climb towers? Set True for yes, False for not.", false)]
    public static bool ALLOW_TOWER_CLIMB;

    /// <summary>
    /// Keep guard heal when a target is below what percentage of their health?
    /// </summary>
    [ServerProperty("keeps", "keep_heal_threshold", "Keep guards heal targets whose health falls below this value.",
        60)]
    ...
    /// Base Value to use when auto-setting STR stat.
    /// </summary>
    [ServerProperty("keeps", "guard_autoset_str_base", "Base Value to use when auto-setting STR stat. ",
        (short) 20)]
    ...
    /// Multiplier to use when auto-setting STR stat.
    /// </summary>
    [ServerProperty("keeps", "guard_autoset_str_multiplier",
        "Multiplier to use when auto-setting STR stat.  Multiplied by 10 when used.", 0.7)]
    public static double GUARD_AUTOSET_STR_MULTIPLIER;

    /// <summary>
    /// Base Value to use when auto-setting CON stat.
    /// </summary>
    [ServerProperty("keeps", "guard_autoset_con_base", "Base Value to use when auto-setting CON stat. ",
        (short) 30)]
    ...
    /// Base Value to use when auto-setting QUI stat.
    /// </summary>
    [ServerProperty("keeps", "guard_autoset_qui_base", "Base Value to use when auto-setting qui stat. ",
        (short) 40)]
    ...
    /// Base Value to use when auto-setting INT stat.
    /// </summary>
    [ServerProperty("keeps", "guard_autoset_int_base", "Base Value to use when auto-setting INT stat. ",
        (short) 30)]
    ...
    /// Multiplier to use when auto-setting STR stat.
    /// </summary>
    [ServerProperty("keeps", "lord_autoset_str_multiplier",
        "Multiplier to use when auto-setting STR stat.  Multiplied by 10 when used.", 0.8)]
    ...
    /// </summary>
    [ServerProperty("keeps", "keep_doors_base_health",
        "Keep doors base health. Will be multiplied by the keep's base level.", 200)]
    public static int KEEP_DOORS_BASE_HEALTH;

    /// <summary>
    /// Doors health upgrade modifier.
    /// </summary>
    [ServerProperty("keeps", "keep_doors_health_upgrade_modifier",
        "The modifier used to calculate the extra amount of door health per upgrade.", 1.0)]
    public static double KEEP_DOORS_HEALTH_UPGRADE_MODIFIER;

    /// <summary>
    /// Components base health value.
    /// </summary>
    [ServerProperty("keeps", "keep_components_base_health",
        "Keep components base health. Will be multiplied by the keep's base level.", 200)]
    public static int KEEP_COMPONENTS_BASE_HEALTH;

    /// <summary>
    /// Components health upgrade modifier.
    /// </summary>
    [ServerProperty("keeps", "keep_components_health_upgrade_modifier",
        "The modifier used to calculate the extra amount of component health per upgrade.", 1.0)]
    ...
    /// </summary>
    [ServerProperty("pve", "currency_exchange_allow",
        "Allow players to exchange aurulite/blood seals/glass/scales by giving them to a merchant who takes the desired currency, i.e. you can give glass to dragon merchants to get scales?",
        true)]
    ...
    /// </summary>
    [ServerProperty("pve", "currency_exchange_values", "Value of special currencies for currency conversion.",
        "atlanteanglass|1;aurulite|4;dragonscales|40;BloodSeal|1000")]
    public static string CURRENCY_EXCHANGE_VALUES;

    /// <summary>
    /// Allow currencies to be exchanged for BPs?
    /// </summary>
    [ServerProperty("pve", "bp_exchange_allow",
        "Allow players to exchange special currencies for BPs by giving them to BP merchants?", true)]
    public static bool BP_EXCHANGE_ALLOW;

    /// <summary>
    /// BP exchange values.  Each item grants x BPs.
    /// </summary>
    [ServerProperty("pve", "bp_exchange_values", "Value of special currencies in BPs.",
        "atlanteanglass|1;aurulite|4;dragonscales|40;BloodSeal|1000")]
    ...
    /// </summary>
    [ServerProperty("pve", "baf_initial_chance",
        "Percent chance for a mob to bring a friend when attacked by a single attacker.  Each multiples of 100 guarantee an add, so a cumulative chance of 250% guarantees two adds with a 50% chance of a third.",
        0)]
    ...
    /// </summary>
    [ServerProperty("pve", "baf_additional_chance",
        "Percent chance for a mob to bring a friend for each additional attacker.  Each multiples of 100 guarantee an add, so a cumulative chance of 250% guarantees two adds with a 50% chance of a third.",
        50)]
    ...
    /// </summary>
    [ServerProperty("pve", "baf_mobs_attack_puller",
        "Do mobs brought by friends only attack the character who pulled them?  If false, mobs attack random players near the puller.",
        false)]
    ...
    /// </summary>
    [ServerProperty("pve", "baf_mobs_attack_bg_members",
        "Do mobs brought by friends attack random nearby players in the puller's battlegroup?  If false, mobs only attack characters in the pulling player's group.",
        false)]
    ...
    /// </summary>
    [ServerProperty("pve", "baf_mobs_count_bg_members",
        "Is the number of mobs brought by a friend based on the number of nearby players in the pulling player's battlegroup?  If false, the number of mobs brought is determined by the number of players in the pulling player's group.",
        false)]
    ...
    /// </summary>
    [ServerProperty("pve", "spell_hitchance_damage_reduction_multiplier",
        "Spell damage reduction multiplier based on hitchance if < 55%. Lower numbers reduce damage reduction.",
        4.3)]
    ...
    /// </summary>
    [ServerProperty("pve", "artifact_xp_rate",
        "Adjust the rate at which all artifacts gain xp.  Higher numbers mean slower XP gain. XP / this = result",
        350)]
    ...
    /// </summary>
    [ServerProperty("pve", "atlantis_teleport_plvl",
        "Set the minimum privilege level required to enter Atlantis zones.", 2)]
    public static int ATLANTIS_TELEPORT_PLVL;

    /// <summary>
    /// Time Before Adventure Wings Instances Destroy when Empty
    /// </summary>
    [ServerProperty("pve", "adventurewing_time_to_destroy",
        "Set the time before Instanced Adventure Wings (Catacombs) are destroy when empty (in minutes).", 5)]
    public static int ADVENTUREWING_TIME_TO_DESTROY;

    /// <summary>
    /// Aurulite Loot Generator Drop Base Chance
    /// </summary>
    [ServerProperty("pve", "lootgenerator_aurulite_base_chance",
        "Base chance for dropping Aurulite using Loot Generator.", 10)]
    public static int LOOTGENERATOR_AURULITE_BASE_CHANCE;

    /// <summary>
    /// Aurulite Loot Generator Amount Ratio
    /// </summary>
    [ServerProperty("pve", "lootgenerator_aurulite_amount_ratio",
        "Modify the final count of Aurulite Loot Generator drop. (TotalCount * lootgenerator_aurulite_amount_ratio)",
        0.5)]
    ...
    /// </summary>
    [ServerProperty("pve", "lootgenerator_aurulite_named_count",
        "Increase count of Aurulite Loot Generator drop for Named mobs. (count * lootgenerator_aurulite_named_count)",
        1.5)]
    ...
    /// </summary>
    [ServerProperty("pve", "lootgenerator_atlanteanglass_base_chance",
        "Base chance for dropping Atlantean Glass using Loot Generator.", 20)]
    public static int LOOTGENERATOR_ATLANTEANGLASS_BASE_CHANCE;

    /// <summary>
    /// Atlantean Glass Loot Generator Named Boost Count
    /// </summary>
    [ServerProperty("pve", "lootgenerator_atlanteanglass_named_count",
        "Increase count of Atlantean Glass Loot Generator drop for Named mobs. (count * lootgenerator_atlanteanglass_named_count)",
        1.5)]
    ...
    /// </summary>
    [ServerProperty("pve", "lootgenerator_dragonscales_base_chance",
        "Base chance for dropping Dragon Scales using Loot Generator.", 15)]
    public static int LOOTGENERATOR_DRAGONSCALES_BASE_CHANCE;

    /// <summary>
    /// Dragon Scales Loot Generator Named Boost Count
    /// </summary>
    [ServerProperty("pve", "lootgenerator_dragonscales_named_count",
        "Multiplier for number of scales dropped from named mobs, including named dragons.  Must be an integer.",
        2)]
    ...
    /// </summary>
    [ServerProperty("pve", "lootgenerator_dreadedseals_starting_level",
        "Mob level to start dropping Dreaded Glowing Seals", 25)]
    public static int LOOTGENERATOR_DREADEDSEALS_STARTING_LEVEL;

    /// <summary>
    /// Dreaded Seals Loot Generator Drop Chance Per Level
    /// </summary>
    [ServerProperty("pve", "lootgenerator_dreadedseals_drop_chance_per_level",
        "Increase in Dreaded Glowing Seal drop chance per level, in hundredths of a percent.", 25)]
    public static int LOOTGENERATOR_DREADEDSEALS_DROP_CHANCE_PER_LEVEL;

    /// <summary>
    /// Dreaded Seals Loot Generator Base Chance
    /// </summary>
    [ServerProperty("pve", "lootgenerator_dreadedseals_base_chance",
        "Base chance to drop a Dreaded Seal, in hundredths of a percent", 25)]
    public static int LOOTGENERATOR_DREADEDSEALS_BASE_CHANCE;

    /// <summary>
    /// Dreaded Seals Loot Generator Named Boost Chance
    /// </summary>
    [ServerProperty("pve", "lootgenerator_dreadedseals_named_chance",
        "Increase chance of Dreaded Seals Loot Generator drop for Named mobs. (count * lootgenerator_dreadedseals_named_chance)",
        1.5)]
    ...
    /// </summary>
    [ServerProperty("pve", "dreadedseals_level_multiplier",
        "Level based multipliers for RPs and BPs awarded when turning in dreaded seals.",
        "21|2;26|3;31|5;36|30;41|70;46|150;50|300")]
    public static string DREADEDSEALS_LEVEL_MULTIPLIER;

    /// <summary>
    /// Dreaded Seal RP values before level multiplier and BP rate
    /// </summary>
    [ServerProperty("pve", "dreadedseals_bp_values",
        "BP values of dreaded seal types before level multiplier and BP rate is applied.",
        "glowing_dreaded_seal|3.334;sanguine_dreaded_seal|3.334;lambent_dreaded_seal|33.334;lambent_dreaded_seal2|33.334;fulgent_dreaded_seal|166.667;effulgent_dreaded_seal|833.334")]
    public static string DREADEDSEALS_BP_VALUES;

    /// <summary>
    /// Dreaded Seal BP values before level multiplier and RP rate
    /// </summary>
    [ServerProperty("pve", "dreadedseals_rp_values",
        "RP values of dreaded seal types before level multiplier and RP rate is applied.",
        "glowing_dreaded_seal|10;sanguine_dreaded_seal|10;lambent_dreaded_seal|100;lambent_dreaded_seal2|100;fulgent_dreaded_seal|500;effulgent_dreaded_seal|2500")]
    public static string DREADEDSEALS_RP_VALUES;

    /// <summary>
    /// PvE Experience Loss Start Level
    /// </summary>
    [ServerProperty("pve", "pve_exp_loss_level",
        "Which level should players killed in PvE start losing experience?", (byte) 6)]
    public static byte PVE_EXP_LOSS_LEVEL;

    /// <summary>
    /// PvE Conn Loss Start Level
    /// </summary>
    [ServerProperty("pve", "pve_con_loss_level",
        "Which level should players killed in PvE start losing constitution?", (byte) 6)]
    ...
    /// </summary>
    [ServerProperty("housing", "housing_starting_npctemplate_id",
        "The starting NPCTemplate ID to use for housing NPC's", 500)]
    ...
    [ServerProperty("housing", "indoor_items_depend_on_size",
        "If true the max number of allowed House indoor items are set like live (40, 60, 80, 100)", true)]
    ...
    [ServerProperty("housing", "housing_lot_price_start", "Starting lot price before per hour reductions",
        95L * 1000L * 100L * 100L)]
    // 95p
    public static long HOUSING_LOT_PRICE_START;

    [ServerProperty("housing", "housing_lot_price_per_hour", "Lot price reduction per hour.",
        (long) (1.2 * 1000 * 100 * 100))]
    // 1.2p
    ...
    /// </summary>
    [ServerProperty("housing", "rent_due_days",
        "How often, in days, is rent due?  0 for never, negative for testing repossession.", 7)]
    ...
    /// </summary>
    [ServerProperty("housing", "rent_bounty_point_to_gold",
        "The worth of 1 (one) bounty point in gold (e.g. 1 bp = 1g -> 10000, 1bp = 10g -> 100000)", 10000)]
    public static long RENT_BOUNTY_POINT_TO_GOLD;

    /// <summary>
    /// Do housing consignment merchants use BP instead of money?
    /// </summary>
    [ServerProperty("housing", "consignment_use_bp",
        "If true the housing consignment merchants use BP instead of money.", false)]
    public static bool CONSIGNMENT_USE_BP;

    /// <summary>
    /// Enable consignment merchants and market cache
    /// </summary>
    [ServerProperty("housing", "market_enabled",
        "If true the market explorers are enabled and the cache is initialized on server start.", true)]
    public static bool MARKET_ENABLED;

    /// <summary>
    /// Enable consignment merchants and market cache
    /// </summary>
    [ServerProperty("housing", "market_enable",
        "If true the market explorers are enabled and the cache is initialized on server start.", true)]
    ...
    /// </summary>
    [ServerProperty("housing", "market_fee_percent",
        "What is the additional fee (%) charged to players using the market explorer?", 20)]
    ...
    /// </summary>
    [ServerProperty("classes", "allow_train_anywhere",
        "Allow players to use the /train command to open a trainer window anywhere in the world?", true)]
    public static bool ALLOW_TRAIN_ANYWHERE;

    /// <summary>
    /// Allow players to /train without having a trainer present
    /// </summary>
    [ServerProperty("classes", "allow_vault_command",
        "Allow players to use the /vault command to open the player's vault anywhere in the world?", false)]
    public static bool ALLOW_VAULT_COMMAND;

    /// <summary>
    /// Disable some classes from being created
    /// </summary>
    [ServerProperty("classes", "disabled_classes",
        "Serialized list of disabled classes, separated by semi-colon or a range with a dash (ie 1-5;7;9)", "")]
    public static string DISABLED_CLASSES;

    /// <summary>
    /// Disable some races from being created
    /// </summary>
    [ServerProperty("classes", "disabled_races",
        "Serialized list of disabled races, separated by semi-colon or a range with a dash (ie 1-5;7;9)", "")]
    public static string DISABLED_RACES;

    /// <summary>
    /// Days before your elligable for a free level in Albion
    /// </summary>
    [ServerProperty("classes", "freelevel_days_albion",
        "days before your elligable for a free level in Albion, use -1 to deactivate", 7)]
    public static int FREELEVEL_DAYS_ALBION;

    /// <summary>
    /// Days before your elligable for a free level in Midgard
    /// </summary>
    [ServerProperty("classes", "freelevel_days_midgard",
        "days before your elligable for a free level in Midgard, use -1 to deactivate", 7)]
    public static int FREELEVEL_DAYS_MIDGARD;

    /// <summary>
    /// Days before your elligable for a free level in Hibernia
    /// </summary>
    [ServerProperty("classes", "freelevel_days_hibernia",
        "days before your elligable for a free level in Hibernia, use -1 to deactivate", 7)]
    public static int FREELEVEL_DAYS_HIBERNIA;

    /// <summary>
    /// Buff Range, 0 for unlimited
    /// </summary>
    [ServerProperty("classes", "buff_range",
        "The range that concentration buffs can last from the owner before it expires.  0 for unlimited.", 0)]
    ...
    /// </summary>
    [ServerProperty("classes", "turret_area_cap_radius", "Sets the Radius which is checked for the turretareacap",
        1000)]
    ...
    /// </summary>
    [ServerProperty("classes", "allow_cross_realm_items",
        "Do we want to allow items to be equipped regardless of realm?", false)]
    ...
    /// </summary>
    [ServerProperty("classes", "slash_level_requirement",
        "What level should you have on your account be able to use /level?", 50)]
    public static int SLASH_LEVEL_REQUIREMENT;

    /// <summary>
    /// Should we allow archers to be able to use arrows from their quiver?
    /// </summary>
    [ServerProperty("classes", "allow_old_archery",
        "Should we allow archers to be able to use arrows from their quiver?", false)]
    ...
    [ServerProperty("spells", "spell_interrupt_maxstagelength", "Max length of stage 1 and 3, 1000 = 1 second",
        1500)]
    public static int SPELL_INTERRUPT_MAXSTAGELENGTH;

    [ServerProperty("spells", "spell_charm_named_check",
        "Prevents charm spell to work on Named Mobs, 0 = disable, 1 = enable", 1)]
    ...
    /// </summary>
    [ServerProperty("guild", "alliance_max",
        "Max Guilds In Alliance - Edit this to change the maximum number of guilds in an alliance -1 = unlimited, 0=disable alliances",
        -1)]
    ...
    /// </summary>
    [ServerProperty("guild", "guild_num",
        "Players Needed For Guild Form - Edit this to change the amount of players required to form a guild", 8)]
    public static int GUILD_NUM;

    /// <summary>
    /// This enables or disables new guild dues. Live standard is 2% dues
    /// </summary>
    [ServerProperty("guild", "new_guild_dues",
        "Guild dues can be set from 1-100% if enabled, or standard 2% if not", false)]
    ...
    /// </summary>
    [ServerProperty("guild", "allow_guild_invite_in_rvr", "Do we allow invite players to guild in rvr zone?",
        false)]
    ...
    /// </summary>
    [ServerProperty("guild", "guild_buff_artifact_xp",
        "Extra artifact XP gain percent for the guild artifact XP buff?", (ushort) 5)]
    public static ushort GUILD_BUFF_ARTIFACT_XP;

    /// <summary>
    /// Guild masterlevel XP Buff bonus amount
    /// </summary>
    [ServerProperty("guild", "guild_buff_masterlevel_xp",
        "Extra masterlevel XP gain percent for the guild masterlevel XP buff?", (ushort) 20)]
    public static ushort GUILD_BUFF_MASTERLEVEL_XP;

    /// <summary>
    /// How much merit to reward guild when dragon is killed, if any.
    /// </summary>
    [ServerProperty("guild", "guild_merit_on_dragon_kill",
        "How much merit to reward guild when dragon is killed, if any.", (ushort) 0)]
    public static ushort GUILD_MERIT_ON_DRAGON_KILL;

    /// <summary>
    /// How much merit to reward guild when legion is killed, if any.
    /// </summary>
    [ServerProperty("guild", "guild_merit_on_legion_kill",
        "How much merit to reward guild when legion is killed, if any.", (ushort) 0)]
    ...
    /// </summary>
    [ServerProperty("guild", "guild_banner_lost_time",
        "When a banner is lost to the enemy how many minutes is the wait before purchase is allowed?",
        (ushort) 1440)]
    ...
    /// </summary>
    [ServerProperty("craft", "crafting_adjust_product_price",
        "Change price to recommended value in database for product itemtemplate", false)]
    public static bool CRAFTING_ADJUST_PRODUCT_PRICE;

    /// <summary>
    /// The crafting price control for secondary craft (trinketing) modifier
    /// </summary>
    [ServerProperty("craft", "crafting_secondary_sellback_percent",
        "How many percent of raw materials cost is SellBack values", 98.572)]
    public static double CRAFTING_SECONDARYCRAFT_SELLBACK_PERCENT;

    /// <summary>
    /// The crafting price control for craft modifier
    /// </summary>
    [ServerProperty("craft", "crafting_sellback_percent",
        "How many percent of raw materials cost is SellBack values", 95)]
    public static int CRAFTING_SELLBACK_PERCENT;

    /// <summary>
    /// The crafting speed modifier
    /// </summary>
    [ServerProperty("craft", "crafting_speed",
        "Crafting Speed Modifier - Edit this to change the speed at which you craft e.g 1.5 is 50% faster 2.0 is twice as fast (100%) 0.5 is half the speed (50%)",
        1.0)]
    ...
    /// </summary>
    [ServerProperty("craft", "capital_city_crafting_skill_gain_bonus",
        "Crafting skill gain bonus % in capital cities; 5 = 5%", 5)]
    public static int CAPITAL_CITY_CRAFTING_SKILL_GAIN_BONUS;

    /// <summary>
    /// Crafting speed bonus in capital cities
    /// </summary>
    [ServerProperty("craft", "capital_city_crafting_speed_bonus",
        "Crafting speed bonus in capital cities; 2 = 2x, 3 = 3x, ..., 1 = standard", 1.0)]
    public static double CAPITAL_CITY_CRAFTING_SPEED_BONUS;

    /// <summary>
    /// Crafting speed bonus in capital cities
    /// </summary>
    [ServerProperty("craft", "keep_crafting_speed_bonus",
        "Crafting speed bonus in the keeps; 2 = 2x, 3 = 3x, ..., 1 = standard", 1.0)]
    public static double KEEP_CRAFTING_SPEED_BONUS;

    /// <summary>
    /// Allow any realm to craft items with a realm of 0 (no realm)
    /// </summary>
    [ServerProperty("craft", "allow_craft_norealm_items", "Allow any realm to craft items with 0 (no) realm.",
        false)]
    ...
    /// Max character crafting skill?
    /// </summary>
    [ServerProperty("craft", "crafting_max_skills_amount",
        "The amount to which set the crafting skills when using crafting_max_skills", 1)]
    public static int CRAFTING_MAX_SKILLS_AMOUNT;

    /// <summary>
    /// Use salvage per realm and get back material to use in chars realm
    /// </summary>
    [ServerProperty("salvage", "use_salvage_per_realm",
        "Enable to get back material to use in chars realm. Disable to get back the same material in all realms.",
        false)]
    ...
    /// Use salvage per realm and get back material to use in chars realm
    /// </summary>
    [ServerProperty("salvage", "use_new_salvage",
        "Enable to use a new system calcul of salvage count based on object_type.", false)]
    ...
    /// </summary>
    [ServerProperty("account", "allow_auto_account_creation",
        "Allow auto-account creation  This is also set in serverconfig.xml and must be enabled for this property to work.",
        true)]
    ...
    /// </summary>
    [ServerProperty("account", "time_between_account_creation",
        "The time in minutes between 2 accounts creation. This avoid account bombing with dynamic ip. 0 to disable",
        0)]
    ...
    /// </summary>
    [ServerProperty("account", "time_between_account_creation_sameip",
        "The time in minutes between accounts creation from the same ip after 2 creations.", 15)]
    public static int TIME_BETWEEN_ACCOUNT_CREATION_SAMEIP;

    /// <summary>
    /// Total number of account allowed for the same IP
    /// </summary>
    [ServerProperty("account", "total_accounts_allowed_sameip", "Total number of account allowed for the same IP",
        20)]
    ...
    /// </summary>
    [ServerProperty("account", "backup_deleted_characters",
        "Should we backup deleted characters and not delete associated content?", true)]
    ...
    /// </summary>
    [ServerProperty("conquest", "conquest_cycle_timer",
        "Time (in minutes) of the overall conquest window and cooldown", 90)]
    public static int CONQUEST_CYCLE_TIMER;

    /// <summary>
    /// Time (in seconds) of the duration between conquest objective point tallies
    /// </summary>
    [ServerProperty("conquest", "conquest_tally_interval",
        "Time (in seconds) of the duration between conquest objective point tallies", 300)]
    ...
    /// </summary>
    [ServerProperty("conquest", "max_subtask_rp_reward", "Max reward (in RP value) for any given subtask interval",
        5000)]
    ...
    /// </summary>
    [ServerProperty("conquest", "max_keep_conquest_rp_reward", "Max reward (in RP value) for a keep capture",
        25000)]
    ...
    /// </summary>
    [ServerProperty("bounty", "bounty_payout_rate",
        "Bounty Reward payout rate - 1 for 100% (no Realm Tax), default is 0.9 for 10% tax", 0.9)]
    ...
    /// </summary>
    [ServerProperty("atlas", "force_discord_link",
        "Enforces the check on the link between game account and Discord", false)]
    ...
    /// Bounty expire check interval in seconds
    /// </summary>
    [ServerProperty("predator", "queued_player_insert_interval",
        "How long to wait between trying to insert new players into system, in seconds", 10)]
    public static int QUEUED_PLAYER_INSERT_INTERVAL;

    [ServerProperty("predator", "predator_abuse_timeout",
        "Time a player is prevented from rejoining Predator after leaving RvR/joining group, in minutes", 10)]
    public static int PREDATOR_ABUSE_TIMEOUT;

    [ServerProperty("predator", "out_of_bounds_timeout",
        "Time a player is allowed to leave a valid hunting zone before disqualification, in seconds", 180)]
    ...
    [ServerProperty("atlas", "max_craft_time",
        "The maximum craft time allowed in seconds. All timers above this value will be normalised to the input value",
        0)]
    ...
    public static IDictionary<string, Tuple<ServerPropertyAttribute, FieldInfo, ServerProperty>> AllDomainProperties
    {
        get
        {
            ...
            foreach (var field in type.GetFields())
            {
                ...
                var attribs = field.GetCustomAttributes(typeof(ServerPropertyAttribute), false);
                if (attribs.Length == 0)
                    continue;

                var att = (ServerPropertyAttribute) attribs[0];

                var serverProp = allProperties
    ...
    public static void Load(ServerPropertyAttribute attrib, FieldInfo field, ServerProperty prop)
    {
        ...
        {
            if (field.IsInitOnly)
                log.WarnFormat("Property {0} is ReadOnly, Value won't be changed - {1} !", key,
                    field.GetValue(null));
