...
public abstract class AbstractCraftingSkill
{
    ...
    {
        if (!CanPlayerStartToCraftItem(player, recipe)) return;
        ...
        var craftingTime = GetCraftingTime(player, recipe);

        var chanceToMakeItem = CalculateChanceToMakeItem(player, recipe.Level);
        player.Out.SendMessage(
            LanguageMgr.GetTranslation(player.Client.Account.Language, "AbstractCraftingSkill.CraftItem.BeginWork",
                recipe.Product.Name, chanceToMakeItem.ToString()), eChatType.CT_Missed, eChatLoc.CL_SystemWindow);
        player.Out.SendTimerWindow(
            LanguageMgr.GetTranslation(player.Client.Account.Language,
                "AbstractCraftingSkill.CraftItem.CurrentlyMaking", recipe.Product.Name), craftingTime);
    ...
    protected virtual void StartCraftingTimerAndSetCallBackMethod(GamePlayer player, Recipe recipe,
        int craftingTime)
    {
    ...
    protected virtual void StopCraftingCurrentItem(GamePlayer player, ItemTemplate itemToCraft)
    {
        ...
        player.Out.SendMessage(
            LanguageMgr.GetTranslation(player.Client.Account.Language, "AbstractCraftingSkill.CraftItem.StopWork",
                itemToCraft.Name), eChatType.CT_System, eChatLoc.CL_SystemWindow);
    }
    ...
    protected virtual bool CanPlayerStartToCraftItem(GamePlayer player, Recipe recipe)
    {
        ...
        if (!GameServer.ServerRules.IsAllowedToCraft(player, recipe.Product)) return false;

        if (!CheckForTools(player, recipe)) return false;

        if (!CheckSecondCraftingSkillRequirement(player, recipe)) return false;

        if (!CheckRawMaterials(player, recipe)) return false;

        if (player.IsMoving || player.IsStrafing)
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language,
                    "AbstractCraftingSkill.CraftItem.MoveAndInterrupt"), eChatType.CT_System,
        ...
        if (player.InCombat)
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language,
                    "AbstractCraftingSkill.CraftItem.CantCraftInCombat"), eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            return false;
        }
        ...
        if (player.IsCasting)
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language,
                    "AbstractCraftingSkill.CraftItem.CantCraftCasting"), eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            return false;
        }
    ...
    {
        var minimumLevel = GetSecondaryCraftingSkillMinimumLevel(recipe);
            ...
            switch (material.Model)
            {
                ...
                case 537: //"heavy thread"
                {
                    if (player.GetCraftingSkillValue(eCraftingSkill.ClothWorking) < minimumLevel)
                    {
                        player.Out.SendMessage(
                            LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "AbstractCraftingSkill.CheckSecondCraftingSkillRequirement.NoClothworkingSkill",
                                minimumLevel, material.Name), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                        return false;
                    }
                ...
                case 521: //"leather square"
                {
                    if (player.GetCraftingSkillValue(eCraftingSkill.LeatherCrafting) < minimumLevel)
                    {
                        player.Out.SendMessage(
                            LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "AbstractCraftingSkill.CheckSecondCraftingSkillRequirement.NoLeathercraftingSkill",
                                minimumLevel, material.Name), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                        return false;
                    }
                ...
                case 519: //"metal bars"
                {
                    if (player.GetCraftingSkillValue(eCraftingSkill.MetalWorking) < minimumLevel)
                    {
                        player.Out.SendMessage(
                            LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "AbstractCraftingSkill.CheckSecondCraftingSkillRequirement.NoMetalworkingSkill",
                                minimumLevel, material.Name), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                        return false;
                    }
                ...
                case 520: //"wooden boards"
                {
                    if (player.GetCraftingSkillValue(eCraftingSkill.WoodWorking) < minimumLevel)
                    {
                        player.Out.SendMessage(
                            LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "AbstractCraftingSkill.CheckSecondCraftingSkillRequirement.NoWoodworkingSkill",
                                minimumLevel, material.Name), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                        return false;
                    }
    ...
    public virtual bool CheckRawMaterials(GamePlayer player, Recipe recipe)
    {
        ...
        lock (player.Inventory)
        {
            ...
            if (missingMaterials != null)
            {
                player.Out.SendMessage(
                    LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "AbstractCraftingSkill.CheckRawMaterial.NoIngredients", recipe.Product.Name),
                    eChatType.CT_System, eChatLoc.CL_SystemWindow);
                player.Out.SendMessage(
                    LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "AbstractCraftingSkill.CheckRawMaterial.YouAreMissing", recipe.Product.Name),
    ...
    public virtual void GainCraftingSkillPoints(GamePlayer player, Recipe recipe)
    {
        foreach (var ingredient in recipe.Ingredients)
        {
            var template = ingredient.Material;
    ...
    public virtual bool RemoveUsedMaterials(GamePlayer player, Recipe recipe)
    {
        ...
        while (enumerator.MoveNext())
        {
            ...
            if (item != null)
            {
                ...
                InventoryLogging.LogInventoryAction(player, "(craft)", eInventoryActionType.Craft, item.Template,
                    de.Value.HasValue ? de.Value.Value : item.Count);
            }
        }
    ...
    public virtual void BuildCraftedItem(GamePlayer player, Recipe recipe)
    {
        ...
        var
            changedSlots =
                new Dictionary<int, int>(
                    5); // key : > 0 inventory ; < 0 ground || value: < 0 = new item count; > 0 = add to old

        lock (player.Inventory)
        {
            var count = product.PackSize < 1 ? 1 : product.PackSize;
            foreach (var item in player.Inventory.GetItemRange(eInventorySlot.FirstBackpack,
                         eInventorySlot.LastBackpack))
            {
                if (item == null)
        ...
        while (enumerator.MoveNext())
        {
            ...
            if (countToAdd > 0) // Add to exiting item
            {
                ...
                {
                    InventoryLogging.LogInventoryAction("(craft)", player, eInventoryActionType.Other,
            ...
            {
                var unique = new ItemUnique(product);
            ...
            if (slot.Key > 0) // Create new item in the backpack
            {
                ...
                InventoryLogging.LogInventoryAction("(craft)", player, eInventoryActionType.Craft, newItem.Template,
                    newItem.Count);
            }
            else // Create new item on the ground
            {
                ...
                player.Out.SendDialogBox(eDialogCode.SimpleWarning, 0, 0, 0, 0, eDialogType.Ok, true,
                    LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "AbstractCraftingSkill.BuildCraftedItem.BackpackFull", product.Name));
            }
        }
        ...
        player.Out.SendMessage(
            LanguageMgr.GetTranslation(player.Client.Account.Language,
                "AbstractCraftingSkill.BuildCraftedItem.Successfully", product.Name, newItem.Quality),
        ...
        if (recipe.IsForUniqueProduct && newItem.Quality == 100)
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language,
                    "AbstractCraftingSkill.BuildCraftedItem.Masterpiece"), eChatType.CT_Important,
                eChatLoc.CL_SystemWindow);
    ...
    {
        var con = GetItemCon(player.GetCraftingSkillValue(m_eskill), craftingLevel);
        if (con < -3)
            con = -3;
        if (con > 3)
            con = 3;
    ...
    {
        var con = GetItemCon(player.GetCraftingSkillValue(m_eskill), recipeLevel);
        if (con < -3)
            con = -3;
        if (con > 3)
            con = 3;
    ...
    public virtual int GetCraftingTime(GamePlayer player, Recipe recipe)
    {
        ...
        var craftingTime = (int) (baseMultiplier * materialsCount / divisorMod);
        ...
        if (Keeps.KeepBonusMgr.RealmHasBonus(Keeps.eKeepBonusType.Craft_Timers_5, (eRealm) player.Realm))
            craftingTime = (int) (craftingTime / 1.05);
        else if (Keeps.KeepBonusMgr.RealmHasBonus(Keeps.eKeepBonusType.Craft_Timers_3,
                     (eRealm) player.Realm))
            craftingTime = (int) (craftingTime / 1.03);

        var con = GetItemCon(player.GetCraftingSkillValue(m_eskill), recipe.Level);
        var mod = 1.0;
        switch (con)
        {
            case -3:
        ...
        if (player.Client.Account.PrivLevel > 1)
            player.Out.SendMessage($"[DEBUG] Crafting time: {craftingTime}s", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
    ...
    private int GetQuality(GamePlayer player, int recipeLevel)
    {
        ...
        {
            if (Util.Chance(2)) return 100; // 2% chance for master piece

            return 96 + Util.Random(3);
        }

        var delta = GetItemCon(player.GetCraftingSkillValue(m_eskill), recipeLevel);
        if (delta < -2)
        {
            if (Util.Chance(2))
        ...
        var chancePart = new int[4]; // index ranges from 96%(0) to 99%(5)
        var sum = 0;
        for (var i = 0; i < 4; i++)
        {
    ...
    {
        var diff = itemCraftingLevel - crafterSkill;
        if (diff <= -50)
            return -3; // grey
        else if (diff <= -31)
            return -2; // green
