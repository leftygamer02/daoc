...
public class Repair
{
    ...
    {
        if (!IsAllowedToBeginWork(player, item, 50)) return 0;
        ...
        if (player.IsMoving || player.IsStrafing)
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language, "Repair.BeginWork.StopRepair1",
                    item.Name), eChatType.CT_System, eChatLoc.CL_SystemWindow);
            if (tradePartner != null)
                tradePartner.Out.SendMessage(
                    LanguageMgr.GetTranslation(player.Client.Account.Language, "Repair.BeginWork.StopRepair2",
                        player.Name, item.Name), eChatType.CT_System, eChatLoc.CL_SystemWindow);
            return 0;
        }

        player.Out.SendMessage(
            LanguageMgr.GetTranslation(player.Client.Account.Language, "Repair.BeginWork.BeginRepairing2",
                item.Name, CalculateSuccessChances(player, item).ToString()), eChatType.CT_System,
        ...
        var workDuration = GetRepairTime(player, item);
        player.Out.SendTimerWindow(
            LanguageMgr.GetTranslation(player.Client.Account.Language, "Repair.BeginWork.Repairing", item.Name),
            workDuration);
    ...
    {
        var player =
            (GamePlayer) timer.Properties.getProperty<object>(AbstractCraftingSkill.PLAYER_CRAFTER, null);
        var tradePartner = (GamePlayer) timer.Properties.getProperty<object>(PLAYER_PARTNER, null);
        var item =
            (InventoryItem) timer.Properties.getProperty<object>(AbstractCraftingSkill.RECIPE_BEING_CRAFTED, null);
        ...
        {
            var toRecoverCond = (int) ((item.MaxCondition - item.Condition) * 0.01 / item.MaxCondition) + 1;
            ...
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language, "Repair.Proceed.FullyRepaired1",
                    item.Name), eChatType.CT_System, eChatLoc.CL_SystemWindow);
            if (tradePartner != null)
                tradePartner.Out.SendMessage(
                    LanguageMgr.GetTranslation(player.Client.Account.Language, "Repair.Proceed.FullyRepaired2",
                        player.Name, item.Name), eChatType.CT_System, eChatLoc.CL_SystemWindow);
        }
        else
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language, "Repair.Proceed.FailImprove1",
                    item.Name), eChatType.CT_System, eChatLoc.CL_SystemWindow);
            if (tradePartner != null)
                tradePartner.Out.SendMessage(
                    LanguageMgr.GetTranslation(player.Client.Account.Language, "Repair.Proceed.FailImprove2",
                        player.Name, item.Name), eChatType.CT_System, eChatLoc.CL_SystemWindow);
        }
    ...
    public static bool IsAllowedToBeginWork(GamePlayer player, InventoryItem item, int percentNeeded)
    {
            ...
            item.SlotPosition > (int) eInventorySlot.LastBackpack)
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language,
                    "Repair.IsAllowedToBeginWork.BackpackItems"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
            return false;
        }
        ...
        if (player.IsCrafting || player.IsSalvagingOrRepairing)
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language,
                    "Repair.IsAllowedToBeginWork.EndCurrentAction"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
            return false;
        }
        ...
        if (item.Condition >= item.MaxCondition)
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language,
                    "Repair.IsAllowedToBeginWork.FullyRepaired", item.Name), eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            return false;
        }
        ...
        if (player.GetCraftingSkillValue(skill) < percentNeeded / 100 * CraftingMgr.GetItemCraftLevel(item))
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language,
                    "Repair.IsAllowedToBeginWork.NotEnoughSkill", item.Name), eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            return false;
        }
    ...
    protected static int CalculateSuccessChances(GamePlayer player, InventoryItem item)
    {
        ...
        var chancePercent =
            (int) (90 / (CraftingMgr.GetItemCraftLevel(item) * 0.5) * player.GetCraftingSkillValue(skill)) -
            80; // 50% = 10% chance, 100% = 100% chance
    ...
    {
        if (!IsAllowedToBeginWork(player, siegeWeapon, 50)) return 0;
        ...
        if (player.IsMoving || player.IsStrafing)
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language, "Repair.BeginWork.StopRepair1",
                    siegeWeapon.Name), eChatType.CT_System, eChatLoc.CL_SystemWindow);
            return 0;
        }

        player.Out.SendMessage(
            LanguageMgr.GetTranslation(player.Client.Account.Language, "Repair.BeginWork.BeginRepair",
                siegeWeapon.Name), eChatType.CT_System, eChatLoc.CL_SystemWindow);

        var workDuration = GetRepairTime(player, siegeWeapon);
        player.Out.SendTimerWindow(
            LanguageMgr.GetTranslation(player.Client.Account.Language, "Repair.BeginWork.Repairing",
                siegeWeapon.Name), workDuration);
        player.CraftTimer = new ECSGameTimer(player);
    ...
    protected static int ProceedSiegeWeapon(ECSGameTimer timer)
    {
        var player =
            (GamePlayer) timer.Properties.getProperty<object>(AbstractCraftingSkill.PLAYER_CRAFTER, null);
        var siegeWeapon =
            (GameSiegeWeapon) timer.Properties.getProperty<object>(AbstractCraftingSkill.RECIPE_BEING_CRAFTED,
                null);
        ...
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language, "Repair.ProceedSiegeWeapon.FailRepair",
                    siegeWeapon.Name), eChatType.CT_System, eChatLoc.CL_SystemWindow);
            return 0;
        }
    ...
    public static bool IsAllowedToBeginWork(GamePlayer player, GameSiegeWeapon siegeWeapon, int percentNeeded)
    {
        if (player.GetCraftingSkillValue(eCraftingSkill.WeaponCrafting) < 301)
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language,
                    "Repair.IsAllowedToBeginWork.WeaponCrafter"), eChatType.CT_Say, eChatLoc.CL_SystemWindow);
            return false;
        }
        ...
        if (siegeWeapon.Health >= siegeWeapon.MaxHealth)
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language,
                    "Repair.IsAllowedToBeginWork.FullyRepaired", siegeWeapon.Name), eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            return false;
        }
    ...
    protected static int CalculateSuccessChances(GamePlayer player, GameSiegeWeapon siegeWeapon)
    {
        ...
        var chancePercent = 90 - 50 / player.GetCraftingSkillValue(eCraftingSkill.WoodWorking);
