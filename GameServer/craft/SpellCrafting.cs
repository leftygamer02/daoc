...
{
    protected override string Profession => "CraftersProfession.Spellcrafter";
    ...
    public override bool IsAllowedToCombine(GamePlayer player, InventoryItem item)
    {
        ...
        if (((InventoryItem) player.TradeWindow.TradeItems[0]).Model ==
            525) // first ingredient to combine is Dust => echantement
        {
            if (item.Level < 15)
            {
        ...
        {
            var bonusToApply = new ArrayList(4);
                        ...
                        if (item.Bonus4Type != 0)
                        {
                            player.Out.SendMessage(
                                LanguageMgr.GetTranslation(player.Client.Account.Language,
                                    "SpellCrafting.IsAllowedToCombine.AlreadyImbued", item.Name),
                                eChatType.CT_System, eChatLoc.CL_SystemWindow);
                            return false;
                        }
                    }
            ...
            lock (player.TradeWindow.Sync)
            {
                for (var i = 0; i < player.TradeWindow.ItemsCount; i++)
                {
                    var currentItem = (InventoryItem) player.TradeWindow.TradeItems[i];

                    if (currentItem.Object_Type != (int) eObjectType.SpellcraftGem)
                    {
                        player.Out.SendMessage(
                            LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "SpellCrafting.IsAllowedToCombine.FalseItem"), eChatType.CT_System,
                            eChatLoc.CL_SystemWindow);
                        return false;
                    }
                    ...
                    if (bonusToApply.Contains(currentItem.Bonus1Type))
                    {
                        player.Out.SendMessage(
                            LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "SpellCrafting.IsAllowedToCombine.NoSameBonus"), eChatType.CT_System,
                            eChatLoc.CL_SystemWindow);
                        return false;
                    }
                    ...
                    if (bonusToApply.Count > 4)
                    {
                        player.Out.SendMessage(
                            LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "SpellCrafting.IsAllowedToCombine.DifferentTypes", item.Name), eChatType.CT_System,
                            eChatLoc.CL_SystemWindow);
                        return false;
                    }
            ...
            var bonusLevel = GetTotalImbuePoints(player, item);
            if (bonusLevel > player.GetCraftingSkillValue(eCraftingSkill.SpellCrafting) / 20)
            {
                player.Out.SendMessage(
                    LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "SpellCrafting.IsAllowedToCombine.NotEnoughSkill"), eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);
                return false;
            }
            ...
            if (bonusLevel - GetItemMaxImbuePoints(item) > 6)
            {
                player.Out.SendMessage(
                    LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "SpellCrafting.IsAllowedToCombine.NoMoreLevels", item.Name), eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);
                return false;
            }
        }
    ...
    protected override void ApplyMagicalEffect(GamePlayer player, InventoryItem item)
    {
        ...
        else if (((InventoryItem) player.TradeWindow.TradeItems[0]).Object_Type ==
                 (int) eObjectType.SpellcraftGem) // Spellcraft item
        ...
        else
        {
            ChatUtil.SendErrorMessage(player,
                "Spellcrafting error: Item was not an ItemUnique, crafting changes not saved to DB!");
            log.ErrorFormat(
                "Spellcrafting error: Item {0} was not an ItemUnique for player {1}, crafting changes not saved to DB!",
                item.Id_nb, player.Name);
        }
    }
    ...
    {
        var spellCrafterLevel = player.GetCraftingSkillValue(eCraftingSkill.SpellCrafting);
        ...
        lock (player.TradeWindow.Sync)
        {
            foreach (InventoryItem gemme in player.TradeWindow.TradeItems)
            {
                switch (gemme.Name)
                {
                    case "coppery imbued dust":
                        bonusMod += 2;
                        break;
                    case "coppery enchanted dust":
                        bonusMod += 4;
                        break;
                    case "coppery glowing dust":
                        bonusMod += 6;
                        break;
                    case "coppery ensorcelled dust":
                        bonusMod += 8;
                        break;

                    case "silvery imbued dust":
                        bonusMod += 10;
                        break;
                    case "silvery enchanted dust":
                        bonusMod += 12;
                        break;
                    case "silvery glowing dust":
                        bonusMod += 14;
                        break;
                    case "silvery ensorcelled dust":
                        bonusMod += 16;
                        break;

                    case "golden imbued dust":
                        bonusMod += 18;
                        break;
                    case "golden enchanted dust":
                        bonusMod += 20;
                        break;
                    case "golden glowing dust":
                        bonusMod += 22;
                        break;
                    case "golden ensorcelled dust":
                        bonusMod += 24;
                        break;

                    case "mithril imbued dust":
                        bonusMod += 26;
                        break;
                    case "mithril enchanted dust":
                        bonusMod += 28;
                        break;
                    case "mithril glowing dust":
                        bonusMod += 30;
                        break;
                    case "mithril ensorcelled dust":
                        bonusMod += 32;
                        break;

                    case "platinum imbued dust":
                        bonusMod += 34;
                        break;
                    case "platinum enchanted dust":
                        bonusMod += 36;
                        break;
                    case "platinum glowing dust":
                        bonusMod += 38;
                        break;
                    case "platinum ensorcelled dust":
                        bonusMod += 40;
                        break;
                }
    ...
    {
        var maxBonusLevel = GetItemMaxImbuePoints(item);
        var bonusLevel = GetTotalImbuePoints(player, item);

        var sucessChances = 100;
        if (bonusLevel > maxBonusLevel)
            sucessChances = CalculateChanceToOverchargeItem(player, item, maxBonusLevel, bonusLevel);
        var destroyChance = 100 - CalculateChanceToPreserveItem(player, item, maxBonusLevel, bonusLevel);


        var tradePartner = player.TradeWindow.Partner;
        ...
        if (Util.Chance(sucessChances))
        {
            lock (player.TradeWindow.Sync)
            {
                ...
                foreach (InventoryItem gem in (ArrayList) player.TradeWindow.TradeItems.Clone())
                {
                    ...
                    InventoryLogging.LogInventoryAction(player, "(craft)", eInventoryActionType.Craft,
                        gem.Template);
                }
            ...
            if (tradePartner != null)
                tradePartner.Out.SendMessage(
                    LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "SpellCrafting.ApplySpellcraftGems.ImbuedItem", player.Name), eChatType.CT_Important,
                    eChatLoc.CL_SystemWindow);
        ...
        else if (Util.Chance(destroyChance))
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language,
                    "SpellCrafting.ApplySpellcraftGems.PowerExplodes"), eChatType.CT_Important,
                eChatLoc.CL_SystemWindow);
            ...
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language,
                    "SpellCrafting.ApplySpellcraftGems.Failed"), eChatType.CT_Important, eChatLoc.CL_SystemWindow);
            ...
            if (tradePartner != null)
            {
                ...
                tradePartner.Out.SendMessage(
                    LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "SpellCrafting.ApplySpellcraftGems.Failed"), eChatType.CT_Important,
                    eChatLoc.CL_SystemWindow);
            }
        }
        else
    ...
    public virtual void ShowSpellCraftingInfos(GamePlayer player, InventoryItem item)
    {
        ...
        var spellcraftInfos = new ArrayList(10);

        var totalItemCharges = GetItemMaxImbuePoints(item);
        var totalGemmesCharges = GetTotalImbuePoints(player, item);

        spellcraftInfos.Add(LanguageMgr.GetTranslation(player.Client.Account.Language,
            "SpellCrafting.ShowSpellCraftingInfos.TotalImbue", item.Name, totalItemCharges));
        spellcraftInfos.Add(LanguageMgr.GetTranslation(player.Client.Account.Language,
            "SpellCrafting.ShowSpellCraftingInfos.CurrentBonus", item.Name));
        if (item.Bonus1Type != 0)
        {
            spellcraftInfos.Add("\t" + SkillBase.GetPropertyName((eProperty) item.Bonus1Type) + ": " + item.Bonus1 +
                                " " + (item.Bonus1Type >= (int) eProperty.Resist_First &&
                                       item.Bonus1Type <= (int) eProperty.Resist_Last
                                    ? "%"
                                    : " pts"));
            if (item.Bonus2Type != 0)
            {
                spellcraftInfos.Add("\t" + SkillBase.GetPropertyName((eProperty) item.Bonus2Type) + ": " +
                                    item.Bonus2 + " " +
                                    (item.Bonus2Type >= (int) eProperty.Resist_First &&
                                     item.Bonus2Type <= (int) eProperty.Resist_Last
                                        ? "%"
                                        : "pts"));
                if (item.Bonus3Type != 0)
                {
                    spellcraftInfos.Add("\t" + SkillBase.GetPropertyName((eProperty) item.Bonus3Type) + ": " +
                                        item.Bonus3 + " " +
                                        (item.Bonus3Type >= (int) eProperty.Resist_First &&
                                         item.Bonus3Type <= (int) eProperty.Resist_Last
                                            ? "%"
                                            : "pts"));
                    if (item.Bonus4Type != 0)
                        spellcraftInfos.Add("\t" + SkillBase.GetPropertyName((eProperty) item.Bonus4Type) + ": " +
                                            item.Bonus4 + " " +
                                            (item.Bonus4Type >= (int) eProperty.Resist_First &&
                                             item.Bonus4Type <= (int) eProperty.Resist_Last
                                                ? "%"
                                                : "pts"));
                }
            }
        ...
        spellcraftInfos.Add(LanguageMgr.GetTranslation(player.Client.Account.Language,
            "SpellCrafting.ShowSpellCraftingInfos.GemBonuses"));
        lock (player.TradeWindow.Sync)
        {
            for (var i = 0; i < player.TradeWindow.ItemsCount; i++)
            {
                var currentGem = (InventoryItem) player.TradeWindow.TradeItems[i];
                spellcraftInfos.Add("\t" + currentGem.Name + " - " +
                                    SkillBase.GetPropertyName((eProperty) currentGem.Bonus1Type) + ": (" +
                                    GetGemImbuePoints(currentGem.Bonus1Type, currentGem.Bonus1) + ") " +
                                    currentGem.Bonus1 + " " +
                                    (currentGem.Bonus1Type >= (int) eProperty.Resist_First &&
                                     currentGem.Bonus1Type <= (int) eProperty.Resist_Last
                                        ? "%"
                                        : "pts"));
            }
        }
        ...
        spellcraftInfos.Add(LanguageMgr.GetTranslation(player.Client.Account.Language,
            "SpellCrafting.ShowSpellCraftingInfos.ImbueCapacity", totalGemmesCharges, totalItemCharges));
        ...
        if (totalGemmesCharges > totalItemCharges)
        {
            ...
            spellcraftInfos.Add("\t");
            spellcraftInfos.Add(LanguageMgr.GetTranslation(player.Client.Account.Language,
                "SpellCrafting.ShowSpellCraftingInfos.Penality",
                GetOverchargePenality(totalItemCharges, totalGemmesCharges)));
            spellcraftInfos.Add("\t");
            spellcraftInfos.Add(LanguageMgr.GetTranslation(player.Client.Account.Language,
                "SpellCrafting.ShowSpellCraftingInfos.Success",
                CalculateChanceToOverchargeItem(player, item, maxBonusLevel, bonusLevel)));
        }

        var partner = player.TradeWindow.Partner;
        ...
        if (totalGemmesCharges > totalItemCharges)
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language,
                    "SpellCrafting.ShowSpellCraftingInfos.Modified"), eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
    }
    ...
    /// <returns></returns>
    protected int CalculateChanceToOverchargeItem(GamePlayer player, InventoryItem item, int maxBonusLevel,
        int bonusLevel)
    {
        // Luhz Crafting Update:
        ...
        try
        {
            var success = 34 + ItemQualOCModifiers[item.Quality - 94];
            success -= OCStartPercentages[bonusLevel - maxBonusLevel];
            var skillbonus = player.GetCraftingSkillValue(eCraftingSkill.SpellCrafting) / 10;
            if (skillbonus > 100)
                skillbonus = 100;
            success += skillbonus;
            var fudgefactor = (int) (100.0 * ((skillbonus / 100.0 - 1.0) *
    ...
    {
        var totalGemBonus = 0;
        var biggerGemCharge = 0;

        if (item.Bonus1Type != 0)
        {
            var currentBonusImbuePoint = GetGemImbuePoints(item.Bonus1Type, item.Bonus1);
        ...
        lock (player.TradeWindow.Sync)
        {
            for (var i = 0; i < player.TradeWindow.TradeItems.Count; i++)
            {
                var currentGem = (InventoryItem) player.TradeWindow.TradeItems[i];
                var currentGemCharge = GetGemImbuePoints(currentGem.Bonus1Type, currentGem.Bonus1);
    ...
    {
        var diff = bonusLevel - maxBonusLevel;
        if (diff == 1) return -10;
    ...
    /// <param name="bonusLevel"></param>
    /// <returns></returns>
    protected int CalculateChanceToPreserveItem(GamePlayer player, InventoryItem item, int maxBonusLevel,
        int bonusLevel)
    {
        var baseChance = 0;
        ...
        var intSkill = player.GetCraftingSkillValue(eCraftingSkill.SpellCrafting);
        int intSkillMod;
        if (intSkill < 50)
            intSkillMod = -50;
        else if (intSkill < 100)
            intSkillMod = -45;
        else if (intSkill < 150)
            intSkillMod = -40;
        else if (intSkill < 200)
            intSkillMod = -35;
        else if (intSkill < 250)
            intSkillMod = -30;
        else if (intSkill < 300)
            intSkillMod = -25;
        else if (intSkill < 350)
            intSkillMod = -20;
        else if (intSkill < 400)
            intSkillMod = -15;
        else if (intSkill < 450)
            intSkillMod = -10;
        else if (intSkill < 500)
            intSkillMod = -5;
        else if (intSkill < 550)
            intSkillMod = 0;
        else if (intSkill < 600)
            intSkillMod = 5;
        else if (intSkill < 650)
            intSkillMod = 10;
        else if (intSkill < 700)
            intSkillMod = 15;
        else if (intSkill < 750)
            intSkillMod = 20;
        else if (intSkill < 800)
            intSkillMod = 25;
        else if (intSkill < 850)
            intSkillMod = 30;
        else if (intSkill < 900)
            intSkillMod = 35;
        else if (intSkill < 950)
            intSkillMod = 40;
        else if (intSkill < 1000)
            intSkillMod = 45;
        else
            intSkillMod = 50;

        var finalChances = (int) (intSkillMod + gemModifier + baseChance);
