...
public class Salvage
{
    ...
    public static int BeginWork(GamePlayer player, InventoryItem item)
    {
        ...
        if (!IsAllowedToBeginWork(player, item)) return 0;
        ...
        if (item.SalvageYieldID > 0)
        {
            ...
            if (salvageYield != null && string.IsNullOrEmpty(salvageYield.MaterialId_nb) == false)
            {
                ...
                if (material == null)
                {
                    player.Out.SendMessage(
                        "Can't find material (" + material.Id_nb + ") needed to salvage this item!",
                        eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                    log.ErrorFormat("Salvage Error for ID: {0}:  Material not found: {1}", salvageYield.ID,
                        material.Id_nb);
                }
            }
            ...
            if (material == null)
            {
                ...
                {
                    player.Out.SendMessage(
                        "This items salvage recipe (" + item.SalvageYieldID + ") not implemented yet.",
                        eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                    log.ErrorFormat("SalvageYield ID {0} not found for item: {1}", item.SalvageYieldID, item.Name);
                }
                else if (salvageYield == null)
                {
                    player.Out.SendMessage("Salvage recipe not found for this item.", eChatType.CT_System,
                        eChatLoc.CL_SystemWindow);
                    log.ErrorFormat("Salvage Lookup Error: ObjectType: {0}, Item: {1}", item.Object_Type,
                        item.Name);
                }
            ...
            {
                player.Out.SendMessage("MaterialId_nb is null for (" + item.Name + ") salvageYield ID!",
                    eChatType.CT_Important, eChatLoc.CL_SystemWindow);
            ...
            {
                player.Out.SendMessage(
                    "Can't find material (" + salvageYield.MaterialId_nb + ") needed to salvage this item!",
                    eChatType.CT_Important, eChatLoc.CL_SystemWindow);
        ...
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language, "Salvage.BeginWork.InterruptSalvage"),
        ...
        player.Out.SendMessage(
            LanguageMgr.GetTranslation(player.Client.Account.Language, "Salvage.BeginWork.BeginSalvage",
                item.Template.Name), eChatType.CT_System, eChatLoc.CL_SystemWindow);
        ...
        player.Out.SendTimerWindow(
            LanguageMgr.GetTranslation(player.Client.Account.Language, "Salvage.BeginWork.Salvaging", item.Name),
            salvageYield.Count);
    ...
    public static int GetYieldPenalty(GamePlayer player, InventoryItem item, int SalvageCount)
    {
        ...
        var Percent = (int) player.GetCraftingSkillValue(CraftingMgr.GetSecondaryCraftingSkillToWorkOnItem(item)) *
            100 / CraftingMgr.GetItemCraftLevel(item);
        ...
        if (Percent < Multiplier) //Multiplier will never be below 0%
        {
            ...
            if (player.Client.Account.PrivLevel != 1)
                player.Out.SendDebugMessage("SkillBelow = true " + iType + Multiplier + " PlayerSkill= %" +
                                            Percent + " Returning " + ReturnCount + " of " + SalvageCount);
        ...
        if (player.Client.Account.PrivLevel != 1)
            player.Out.SendDebugMessage("SkillBelow = false " + iType + Multiplier + " PlayerSkill= %" + Percent +
                                        " Returning " + ReturnCount + " of " + SalvageCount);
    ...
    public static int BeginWork(GamePlayer player, GameSiegeWeapon siegeWeapon)
    {
        ...
        if (recipe == null)
        {
            player.Out.SendMessage("Error retrieving salvage data!", eChatType.CT_Important,
                eChatLoc.CL_SystemWindow);
        ...
        var rawMaterials =
            DOLDB<DBCraftedXItem>.SelectObjects(DB.Column("CraftedItemId_nb").IsEqualTo(recipe.Id_nb));

        if (rawMaterials == null || rawMaterials.Count == 0)
        {
            player.Out.SendMessage("No raw materials provided for this siege weapon!", eChatType.CT_Important,
                eChatLoc.CL_SystemWindow);
        ...
        if (player.IsCrafting || player.IsSalvagingOrRepairing)
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language,
                    "Salvage.IsAllowedToBeginWork.EndCurrentAction"), eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            return 0;
        }
        ...
        foreach (var material in rawMaterials)
        {
            ...
            if (template == null)
            {
                player.Out.SendMessage("Missing raw material " + material.IngredientId_nb + "!",
                    eChatType.CT_Important, eChatLoc.CL_SystemWindow);
            ...
            InventoryLogging.LogInventoryAction("(salvage)", player, eInventoryActionType.Craft, item.Template,
                item.Count);
        }

        if (error)
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language, "Salvage.BeginWork.NoRoom"),
                eChatType.CT_System, eChatLoc.CL_SystemWindow);
    ...
    {
        var player = timer.Properties.getProperty<GamePlayer>(AbstractCraftingSkill.PLAYER_CRAFTER, null);
        var itemToSalvage = timer.Properties.getProperty<InventoryItem>(SALVAGED_ITEM, null);
        var yield = timer.Properties.getProperty<SalvageYield>(SALVAGE_YIELD, null);
        var itemList =
            player.TempProperties.getProperty<IList<InventoryItem>>(SALVAGE_QUEUE, null);
        var materialCount = yield.Count;

        if (player == null || itemToSalvage == null || yield == null || materialCount == 0)
        {
            player.Out.SendMessage("Error retrieving salvage data for this item!", eChatType.CT_Important,
                eChatLoc.CL_SystemWindow);
        ...
        if (rawMaterial == null)
        {
            player.Out.SendMessage("Error finding the raw material needed to salvage this item!",
                eChatType.CT_Important, eChatLoc.CL_SystemWindow);
        ...
        {
            player.Out.SendMessage("Error finding the item to salvage!", eChatType.CT_Important,
                eChatLoc.CL_SystemWindow);
            return 0;
        }

        InventoryLogging.LogInventoryAction(player, "(salvage)", eInventoryActionType.Craft, itemToSalvage.Template,
            itemToSalvage.Count);

        var
            changedSlots = new Dictionary<int, int>(5); // value: < 0 = new item count; > 0 = add to old
        lock (player.Inventory)
        {
            var count = materialCount;
            foreach (var item in player.Inventory.GetItemRange(eInventorySlot.FirstBackpack,
                         eInventorySlot.LastBackpack))
            {
                ...
                var countFree = item.MaxCount - item.Count;
                if (count > countFree)
                {
            ...
            if (count > 0) // Add new object
            {
                ...
                changedSlots.Add((int) firstEmptySlot,
                    -count); // Create the item in the free slot (always at least one)
            }
        }
        ...
        var enumerator = changedSlots.GetEnumerator();
        while (enumerator.MoveNext())
        {
            var de = enumerator.Current;
            var countToAdd = de.Value;
            if (countToAdd > 0) // Add to exiting item
            {
                ...
                InventoryLogging.LogInventoryAction("(salvage)", player, eInventoryActionType.Craft,
                    newItem.Template, countToAdd);
            }
            else
            {
                ...
                InventoryLogging.LogInventoryAction("(salvage)", player, eInventoryActionType.Craft,
                    newItem.Template, newItem.Count);
            }
        }

        player.Inventory.CommitChanges();
        player.Out.SendMessage(
            LanguageMgr.GetTranslation(player.Client.Account.Language, "Salvage.Proceed.GetBackMaterial",
                materialCount, rawMaterial.Name, itemToSalvage.Name), eChatType.CT_Important,
            eChatLoc.CL_SystemWindow);
    ...
    public static bool IsAllowedToBeginWork(GamePlayer player, InventoryItem item, bool mute = false)
    {
        ...
        {
            if (!mute)
                player.Out.SendMessage("You can't salvage while in combat.", eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);
            return false;
        }
        ...
        if (item.IsNotLosingDur || item.IsIndestructible)
        {
            if (!mute)
                player.Out.SendMessage(
                    LanguageMgr.GetTranslation(player.Client.Account.Language, "Salvage.BeginWork.NoSalvage",
                        item.Name + ".  This item is indestructible"), eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);
            return false;
        }
        ...
        {
            if (!mute)
                player.Out.SendMessage("This item cannot be salvaged.", eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);
            return false;
        }
        ...
        if (item.SalvageYieldID < 0)
        {
            if (!mute)
                player.Out.SendMessage(
                    LanguageMgr.GetTranslation(player.Client, "Salvage.BeginWork.NoSalvage", item.Name),
                    eChatType.CT_System, eChatLoc.CL_SystemWindow);
            return false;
        }
            ...
            item.SlotPosition > (int) eInventorySlot.LastBackpack)
        {
            if (!mute)
                player.Out.SendMessage(
                    LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "Salvage.IsAllowedToBeginWork.BackpackItems"), eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);
            return false;
        }

        var skill = CraftingMgr.GetSecondaryCraftingSkillToWorkOnItem(item);
        if (skill == eCraftingSkill.NoCrafting)
        {
            if (!mute)
                player.Out.SendMessage(
                    LanguageMgr.GetTranslation(player.Client.Account.Language, "Salvage.BeginWork.NoSalvage",
                        item.Name + ".  You do not have the required secondary skill"), eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);
            return false;
        }
        ...
        if (player.GetCraftingSkillValue(skill) < 0.75 * CraftingMgr.GetItemCraftLevel(item))
        {
            if (!mute)
                player.Out.SendMessage(
                    LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "Salvage.IsAllowedToBeginWork.NotEnoughSkill", item.Name), eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);
            return false;
        }
    ...
    public static bool IsAllowedToBeginWorkSilent(GamePlayer player, InventoryItem item)
    {
        ...
        if (item.IsNotLosingDur || item.IsIndestructible)
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language, "Salvage.BeginWork.NoSalvage",
                    item.Name + ".  This item is indestructible"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
            return false;
        }
            ...
            item.SlotPosition > (int) eInventorySlot.LastBackpack)
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language,
                    "Salvage.IsAllowedToBeginWork.BackpackItems"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
            return false;
        }
    ...
    public static int GetCountForSalvage(InventoryItem item, ItemTemplate rawMaterial)
    {
        ...
        {
            var usureoverall =
                maxCount * (item.Condition / 5 / 1000) / 100; // assume that all items have 50000 base con
    ...
    /// </summary>
    public static int GetMaterialYield(GamePlayer player, InventoryItem item, SalvageYield salvageYield,
        ItemTemplate rawMaterial)
    {
        ...
        {
            maxCount = (int) (item.Price * 0.45 /
                              rawMaterial.Price); // crafted item return max 45% of the item value in material

            if (item.IsCrafted) maxCount = (int) Math.Ceiling((double) maxCount / 2);
        }

        var playerPercent = player.GetCraftingSkillValue(CraftingMgr.GetSecondaryCraftingSkillToWorkOnItem(item)) *
            100 / CraftingMgr.GetItemCraftLevel(item);
        ...
        var minCount =
            (int) ((maxCount - 1) / 25f * playerPercent) -
            (3 * maxCount - 4); //75% => min = 1; 100% => min = maxCount;
