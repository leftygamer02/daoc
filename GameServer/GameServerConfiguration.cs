...
//			m_rootDirectory = root["Server"]["RootDirectory"].GetString(m_rootDirectory);
        ...
        var serverType = root["Server"]["GameType"].GetString("Normal");
        ...
        var db = root["Server"]["DBType"].GetString("XML");
        switch (db.ToLower())
        {
            case "xml":
        ...
        var port = -1;
        var addressStr = root["Server"]["UDPOutIP"].GetString(string.Empty);
        var portStr = root["Server"]["UDPOutPort"].GetString(string.Empty);
...
//			root["Server"]["RootDirectory"].Set(m_rootDirectory);
        ...
        get
        {
            return new string[] {"System.dll", "System.Xml.dll"}
                .Union(new DirectoryInfo(Path.Combine(RootDirectory, "lib"))
                    ...
                    .Select(f => f.Name).Where(f => !f.Equals(new FileInfo(ScriptCompilationTarget).Name,
    ...
    public string[] AdditionalScriptAssemblies => string.IsNullOrEmpty(m_scriptAssemblies.Trim())
        ? new string[] { }
        : m_scriptAssemblies.Split(',');
