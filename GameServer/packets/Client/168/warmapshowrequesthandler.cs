...
[PacketHandlerAttribute(PacketHandlerType.TCP, eClientPackets.ShowWarmapRequest, "Show Warmap",
...
public class WarmapShowRequestHandler : IPacketHandler
{
    ...
    public void HandlePacket(GameClient client, GSPacketIn packet)
    {
        ...
        var RealmMap = packet.ReadByte();
        var keepId = packet.ReadByte();
        ...
        switch (code)
        {
            ...
            case 2:
            {
                ...
                if (client.Account.PrivLevel == (int) ePrivLevel.Player &&
                    (client.Player.InCombat || client.Player.CurrentRegionID != 163 ||
                     GameRelic.IsPlayerCarryingRelic(client.Player)))
                    return;

                AbstractGameKeep keep = null;

                if (keepId > 6) keep = GameServer.KeepManager.GetKeepByID(keepId);

                if (keep == null && keepId > 6) return;
                ...
                if (client.Account.PrivLevel == (int) ePrivLevel.Player)
                {
                    ...
                    if (keep != null)
                    {
                        ...
                        if (keep.Realm != client.Player.Realm) return;

                        if (keep is GameKeep && ((keep as GameKeep).OwnsAllTowers == false || keep.InCombat)) return;
                    ...
                    if (!found)
                    {
                        client.Player.Out.SendMessage(
                            "You cannot teleport unless you are near a valid portal stone.", eChatType.CT_System,
                ...
                if (x != 0) client.Player.MoveTo(163, x, y, z, heading);
