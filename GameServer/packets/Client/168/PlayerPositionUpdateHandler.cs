...
[PacketHandlerAttribute(PacketHandlerType.TCP, eClientPackets.PositionUpdate, "Handles player position updates",
...
public class PlayerPositionUpdateHandler : IPacketHandler
{
    ...
    public void HandlePacket(GameClient client, GSPacketIn packet)
    {
        ...
        var environmentTick = GameLoop.GameLoopTime;
        int oldSpeed = client.Player.CurrentSpeed;

        //read the state of the player
        packet.Skip(2); //PID
        var speedData = packet.ReadShort();
        var speed = speedData & 0x1FF;
        ...
        if ((client.Player.IsMezzed || client.Player.IsStunned) && client.Player.effectListComponent.GetAllEffects()
        ...
        var xOffsetInZone = packet.ReadShort();
        var yOffsetInZone = packet.ReadShort();
        var currentZoneID = packet.ReadShort();

        try
        {
            var grabZone = WorldMgr.GetZone(currentZoneID);
        }
        catch (Exception e)
        {
        ...
        var newZone = WorldMgr.GetZone(currentZoneID);
        if (newZone == null)
        {
        ...
        var realX = newZone.XOffset + xOffsetInZone;
        var realY = newZone.YOffset + yOffsetInZone;

        var zoneChange = newZone != client.Player.LastPositionUpdateZone;
        if (zoneChange)
        {
            ...
            var description = newZone.Description;
            var screenDescription = description;
            ...
            client.Out.SendMessage(
                LanguageMgr.GetTranslation(client.Account.Language, "PlayerPositionUpdateHandler.Entered",
                    description),
        ...
        var coordsPerSec = 0;
        var jumpDetect = 0;
        var timediff = (int) (GameLoop.GameLoopTime - client.Player.LastPositionUpdateTick);
        var distance = 0;

        if (timediff > 0)
        {
            var newPoint = new Point3D(realX, realY, realZ);
            distance = newPoint.GetDistanceTo(new Point3D((int) client.Player.LastPositionUpdatePoint.X,
                (int) client.Player.LastPositionUpdatePoint.Y,
        ...
        var tolerance = ServerProperties.Properties.CPS_TOLERANCE;
        ...
        if (client.Player.IsAllowedToFly == false &&
            (coordsPerSec > tolerance || jumpDetect > ServerProperties.Properties.JUMP_TOLERANCE))
        {
            var isHackDetected = true;

            if (coordsPerSec > tolerance)
            {
                // check to see if CPS time tolerance is exceeded
                var lastCPSTick = client.Player.TempProperties.getProperty<int>(LASTCPSTICK, 0);
                ...
                if (client.Account.PrivLevel == 1)
                {
                    ...
                    if (ServerProperties.Properties.ENABLE_MOVEDETECT)
                    {
                        if (ServerProperties.Properties.BAN_HACKERS &&
                            false) // banning disabled until this technique is proven accurate
                        {
                            ...
                            b.Type = "B";
                            b.Reason = string.Format("Autoban MOVEHACK:(CPS:{0}, JT:{1}) on player:{2}",
                                coordsPerSec, jumpDetect, client.Player.Name);
            ...
            if (oldAreas != null)
                foreach (var area in oldAreas)
                    if (!newAreas.Contains(area))
                    {
                        ...
                        var checkrvrarea = area as AbstractArea;
        ...
        var SHlastTick = client.Player.TempProperties.getProperty<long>(SHLASTUPDATETICK);
        var SHlastFly = client.Player.TempProperties.getProperty<int>(SHLASTFLY);
        var SHlastStatus = client.Player.TempProperties.getProperty<int>(SHLASTSTATUS);
        var SHcount = client.Player.TempProperties.getProperty<int>(SHSPEEDCOUNTER);
        var status = ((speedData & 0x1FF) ^ speedData) >> 8;
        var fly = ((flyingflag & 0x1FF) ^ flyingflag) >> 8;

        if (client.Player.IsJumping)
            SHcount = 0;
        if (SHlastTick != 0 && SHlastTick != environmentTick)
        {
            if ((SHlastStatus == status || (status & 0x8) == 0) && (fly & 0x80) != 0x80 && (SHlastFly == fly ||
                    (SHlastFly & 0x10) == (fly & 0x10) || !((SHlastFly & 0x10) == 0x10 &&
                                                            (fly & 0x10) == 0x0 && (flyingflag & 0x7FF) > 0)))
            {
                if (environmentTick - SHlastTick < 400)
                {
                    ...
                    if (SHcount > 1 && client.Account.PrivLevel > 1)
                        ...
                        client.Out.SendMessage(
                            string.Format("SH: ({0}) detected: {1}, count {2}",
                                500 / (environmentTick - SHlastTick), environmentTick - SHlastTick, SHcount),
                    ...
                    if (SHcount % 5 == 0)
                    {
                        ...
                        if (client.Account.PrivLevel > 1)
                        {
                            client.Out.SendMessage("SH: Logging SH cheat.", eChatType.CT_Damaged,
                                eChatLoc.CL_SystemWindow);

                            if (SHcount >= ServerProperties.Properties.SPEEDHACK_TOLERANCE)
                                client.Out.SendMessage("SH: Player would have been banned!", eChatType.CT_Damaged,
                                    eChatLoc.CL_SystemWindow);
                        }
                            ...
                            SHcount >= ServerProperties.Properties.SPEEDHACK_TOLERANCE)
                        {
                            if (ServerProperties.Properties.BAN_HACKERS)
                            {
                                ...
                                b.DateBan = DateTime.Now;
                                b.Type = "B";
                                b.Reason = string.Format("Autoban SH:({0},{1}) on player:{2}", SHcount,
                                    environmentTick - SHlastTick, client.Player.Name);
                                GameServer.Database.AddObject(b);
                                GameServer.Database.SaveObject(b);
        ...
        var state = (speedData >> 10) & 7;
        client.Player.IsClimbing = state == 7;
        client.Player.IsSwimming = state == 1;
        // debugFly on, but player not do /debug on (hack)
        if (state == 3 && !client.Player.TempProperties.getProperty(GamePlayer.DEBUG_MODE_PROPERTY, false) &&
            !client.Player.IsAllowedToFly)
        {
            var builder = new StringBuilder();
        ...
        {
            var locations = client.Player.LastUniqueLocations;
            var loc = locations[0];
        ...
        if (GameServer.ServerRules.CanTakeFallDamage(client.Player) && !client.Player.IsSwimming)
        {
            ...
            if (flyingflag >> 15 != 0)
            {
                var safeFallLevel = client.Player.GetAbilityLevel(Abilities.SafeFall);
                fallSpeed = (flyingflag & 0xFFF) -
                            100 * safeFallLevel; // 0x7FF fall speed and 0x800 bit = fall speed overcaped
                var fallMinSpeed = 400;
                var fallDivide = 6;
        ...
        else
        {
            ...
            var contenthead = (ushort) (client.Player.Heading + (true ? 0x1000 : 0));
        ...
        // Write health + Attack
        outpak.WriteByte((byte) (client.Player.HealthPercent +
                                 (client.Player.attackComponent.AttackState ? 0x80 : 0)));
        ...
        outpak190.WriteByte((byte) (client.Player.RPFlag
            ? 1
            : 0)); // roleplay flag, if == 1, show name (RP) with gray color
        ...
        var outpak1124 = new GSUDPPacketOut(client.Out.GetPacketCode(eServerPackets.PlayerPosition));
        ...
        outpak1124.WriteShort((ushort) (client.Player.Steed?.RiderSlot(client.Player) ??
                                        0)); // fall damage flag coming in, steed seat position going out
        ...
        outpak1124.WriteByte((byte) (client.Player.HealthPercent +
                                     (client.Player.attackComponent.AttackState ? 0x80 : 0)));
        ...
        foreach (GamePlayer player in client.Player.GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE))
        {
            ...
            if (player == client.Player)
            {
                ...
                player.Client.GameObjectUpdateArray[
                        new Tuple<ushort, ushort>(client.Player.CurrentRegionID, (ushort) client.Player.ObjectID)] =
            ...
            {
                var relic = client.Player.MinotaurRelic;
    ...
    private void _HandlePacket1124(GameClient client, GSPacketIn packet)
    {
        ...
        var currentZoneID = packet.ReadShort();
        var playerState = packet.ReadShort();
        var fallingDMG = packet.ReadShort();
        var newHeading = packet.ReadShort();
        var playerAction = (byte) packet.ReadByte();
        packet.Skip(2); // unknown bytes x2
        var playerHealth = (byte) packet.ReadByte();
        ...
        var coordsPerSec = 0;
        var jumpDetect = 0;
        var timediff = GameLoop.GameLoopTime - client.Player.LastPositionUpdateTick;
        var distance = 0;
        ...
        if (client.Player.Steed != null && client.Player.Steed.MaxSpeed > 0)
            tolerance += client.Player.Steed.MaxSpeed;
        else if (client.Player.MaxSpeed > 0) tolerance += client.Player.MaxSpeed;
        ...
        if (!client.Player.IsAllowedToFly &&
            (coordsPerSec > tolerance || jumpDetect > ServerProperties.Properties.JUMP_TOLERANCE))
        {
            var isHackDetected = true;
        ...
        if (state == 3 &&
            client.Player.TempProperties.getProperty<bool>(GamePlayer.DEBUG_MODE_PROPERTY, false) == false &&
            !client.Player.IsAllowedToFly) //debugFly on, but player not do /debug on (hack)
        {
            var builder = new StringBuilder();
            builder.Append("HACK_FLY");
            builder.Append(": CharName=");
        ...
        lock (client.Player.LastUniqueLocations)
        {
            var locations = client.Player.LastUniqueLocations;
            var loc = locations[0];
            if (loc.X != (int) newPlayerX || loc.Y != (int) newPlayerY || loc.Z != (int) newPlayerZ ||
                loc.RegionID != client.Player.CurrentRegionID)
            {
                loc = locations[locations.Length - 1];
                Array.Copy(locations, 0, locations, 1, locations.Length - 1);
        ...
        if (GameServer.ServerRules.CanTakeFallDamage(client.Player) && !client.Player.IsSwimming)
            try
            {
                ...
                if (fallingDMG >> 15 != 0)
                {
                    ...
                    if (fallSpeed > 500)
                        if (client.Player.CharacterClass.ID != (int) eCharacterClass.Necromancer ||
                            !client.Player.IsShade)
                            client.Player.CalcFallDamage(fallPercent);

                    client.Player.MaxLastZ = client.Player.Z;
                }
                else if (maxLastZ < client.Player.Z || client.Player.IsRiding ||
                         newPlayerZSpeed > -150) // is riding, for dragonflys
                {
                    client.Player.MaxLastZ = client.Player.Z;
                }
            }
        ...
        if (client.Player.Steed != null && client.Player.Steed.ObjectState == GameObject.eObjectState.Active)
        {
            ...
            (playerState >> 10 ==
             4) // patch 0062 fix bug on release preventing players from receiving res sickness
        {
        ...
        outpak1127.WriteByte((byte) (client.Player.HealthPercent +
                                     (client.Player.attackComponent.AttackState ? 0x80 : 0)));
        ...
        outpak190.WriteByte((byte) (client.Player.HealthPercent +
                                    (client.Player.attackComponent.AttackState ? 0x80 : 0)));
        ...
        //trade window
        if (client.Player.TradeWindow?.Partner != null &&
            !client.Player.IsWithinRadius(client.Player.TradeWindow.Partner, WorldMgr.GIVE_ITEM_DISTANCE))
            client.Player.TradeWindow.CloseTrade();
    }
}