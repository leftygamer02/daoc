...
/// </summary>
[PacketHandlerAttribute(PacketHandlerType.TCP, eClientPackets.PlayerPickupHouseItem,
    "Handle Housing Pick Up Request.", eClientStatus.PlayerInGame)]
    ...
    public void HandlePacket(GameClient client, GSPacketIn packet)
    {
        ...
        switch (method)
        {
            case 1: //garden item
                ...
                foreach (var entry in house.OutdoorItems)
                {
                    ...
                    if (client.Player.Inventory.AddItem(eInventorySlot.FirstEmptyBackpack, invitem))
                        InventoryLogging.LogInventoryAction("(HOUSE;" + house.HouseNumber + ")", client.Player,
                            eInventoryActionType.Other, invitem.Template, invitem.Count);
                    ...
                    ChatUtil.SendSystemMessage(client,
                        string.Format("You get {0} and put it in your backpack.", invitem.Name));
                    return;
                }
            ...
            case 3: //wall/floor mode
                ...
                if (iitem == null)
                {
                    client.Player.Out.SendMessage("error: id was null", eChatType.CT_Help,
                ...
                if (iitem.BaseItem != null)
                {
                    ...
                    if (GetItemBack(item))
                    {
                        ...
                        {
                            var removalMsg = string.Format("The {0} is cleared from the {1}.", item.Name,
                    ...
                    {
                        ChatUtil.SendSystemMessage(client,
                            "The " + item.Name + " is cleared from the wall surface.");
                    }
                }
                else if (iitem.DatabaseItem.BaseItemID.Contains("GuildBanner"))
                {
                    ...
                    var idnb = iitem.DatabaseItem.BaseItemID.Split('_');
                    ...
                    {
                        var invMsg = string.Format("The {0} is cleared from the {1}.", inv.Name,
                            method == 2 ? "wall surface" : "floor");

                        ChatUtil.SendSystemMessage(client, invMsg);
                        InventoryLogging.LogInventoryAction("(HOUSE;" + house.HouseNumber + ")", client.Player,
                            eInventoryActionType.Other, inv.Template, inv.Count);
                    }
                    else
                    {
                        ChatUtil.SendSystemMessage(client, "You need place in your inventory !");
                ...
                foreach (var plr in house.GetAllPlayersInHouse()) plr.Out.SendTCP(pak);
