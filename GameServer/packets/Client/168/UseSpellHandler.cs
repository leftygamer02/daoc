...
/// </summary>
[PacketHandlerAttribute(PacketHandlerType.TCP, eClientPackets.UseSpell, "Handles Player Use Spell Request.",
...
public class UseSpellHandler : AbstractCommandHandler, IPacketHandler
{
    ...
    public void HandlePacket(GameClient client, GSPacketIn packet)
    {
        ...
        else
        {
            ...
            if (client.Version > GameClient.eClientVersion.Version171)
            {
                ...
                var newZone = WorldMgr.GetZone((ushort) currentZoneID);
                if (newZone == null)
                {
        ...
        {
            var index = snap[spellLineIndex].Item2.FindIndex(s => s is Spell
                ? s.Level == spellLevel
                : s is Styles.Style
                    ? ((Styles.Style) s).SpecLevelRequirement == spellLevel
                    : s is Ability
                        ? ((Ability) s).SpecLevelRequirement == spellLevel
                        : false);

            if (index > -1) sk = snap[spellLineIndex].Item2[index];
        ...
        else if (sk is Ability)
        {
            ...
            if (handler != null) handler.Execute(ab, player);
        ...
        else
        {
            if (Log.IsWarnEnabled)
                Log.Warn("Client <" + player.Client.Account.Name + "> requested incorrect spell at level " +
                         spellLevel +
                         " in spell-line " + (sl == null || sl.Name == null ? "unkown" : sl.Name));

            player.Out.SendMessage(
                string.Format("Error : Spell (Line {0}, Level {1}) can't be resolved...", spellLineIndex,
                    spellLevel), eChatType.CT_SpellResisted, eChatLoc.CL_SystemWindow);
        }
    ...
    protected class UseSpellAction : RegionECSAction
    {
        ...
        protected override int OnTick(ECSGameTimer timer)
        {
            ...
            {
                var index = snap[m_spellLineIndex].Item2.FindIndex(s => s is Spell
                    ? s.Level == m_spellLevel
                    : s is Styles.Style
                        ? ((Styles.Style) s).SpecLevelRequirement == m_spellLevel
                        : s is Ability
                            ? ((Ability) s).SpecLevelRequirement == m_spellLevel
                            : false);

                if (index > -1) sk = snap[m_spellLineIndex].Item2[index];
            ...
            else
            {
                ...
                player.Out.SendMessage(
                    string.Format("Error : Spell (Line {0}, Level {1}) can't be resolved...", m_spellLineIndex,
                        m_spellLevel), eChatType.CT_SpellResisted, eChatLoc.CL_SystemWindow);
            }
