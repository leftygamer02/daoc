...
[PacketHandlerAttribute(PacketHandlerType.TCP, eClientPackets.WorldInitRequest, "Handles world init replies",
        ...
        protected override int OnTick(AuxECSGameTimer timer)
        {
            ...
            if (player.Inventory != null)
                lock (player.Inventory)
                {
                    ...
                    foreach (var myitem in player.Inventory.AllItems)
                        if (myitem != null && myitem.Emblem != 0)
                        {
                            if (playerGuild == null || myitem.Emblem != playerGuild.Emblem) myitem.Emblem = 0;
            ...
            player.Out.SendInventoryItemsUpdate(eInventoryWindowType.Inventory,
                player.Inventory.GetItemRange(eInventorySlot.FirstBackpack, eInventorySlot.LastBagHorse));
            player.Out
                .SendUpdatePlayerSkills(); //TODO Insert 0xBE - 08 Various in SendUpdatePlayerSkills() before send spells
            ...
            player.Out.SendDebugMode(
                player.TempProperties.getProperty<object>(GamePlayer.DEBUG_MODE_PROPERTY, null) != null);
            ...
            var isStealthed = player.IsStealthed;
            ...
            if (log.IsDebugEnabled)
                log.DebugFormat("Client {0}({1} PID:{2} OID:{3}) entering Region {4}(ID:{5})",
                    player.Client.Account.Name, player.Name, player.Client.SessionID, player.ObjectID,
                    player.CurrentRegion.Description, player.CurrentRegionID);
            return 0;
        }
    }
}