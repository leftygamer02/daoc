...
[PacketHandlerAttribute(PacketHandlerType.TCP, eClientPackets.PlayerMoveItem, "Handle Moving Items Request",
...
public class PlayerMoveItemRequestHandler : IPacketHandler
{
    ...
    public void HandlePacket(GameClient client, GSPacketIn packet)
    {
        ...
        var toClientSlot = packet.ReadShort();
        var fromClientSlot = packet.ReadShort();
        ...
        {
            var objectID = (ushort) (toClientSlot - 1000);
            var obj = WorldMgr.GetObjectByIDFromRegion(client.Player.CurrentRegionID, objectID);
            if (obj == null || obj.ObjectState != GameObject.eObjectState.Active)
            {
                client.Out.SendInventorySlotsUpdate(new int[] {fromClientSlot});
                client.Out.SendMessage("Invalid trade target. (" + objectID + ")", eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);
                return;
            }

            var tradeTarget = obj as GamePlayer;
            ...
            if (tradeTarget != null)
            {
                ...
                {
                    client.Out.SendInventorySlotsUpdate(new int[] {fromClientSlot});
                    client.Out.SendMessage("Can't trade with inactive players.", eChatType.CT_System,
                        eChatLoc.CL_SystemWindow);
                    return;
                }
                ...
                {
                    client.Out.SendInventorySlotsUpdate(new int[] {fromClientSlot});
                    client.Out.SendMessage("You can't trade with yourself, silly!", eChatType.CT_System,
                        eChatLoc.CL_SystemWindow);
                    return;
                }
                 ...
                 fromClientSlot <= (ushort) eInventorySlot.MaxEquipable))
            {
                    ...
                    if (obj is GamePlayer)
                        client.Out.SendMessage(
                            LanguageMgr.GetTranslation(client.Account.Language,
                                "PlayerMoveItemRequestHandler.TooFarAway", client.Player.GetName((GamePlayer) obj)),
                            eChatType.CT_System, eChatLoc.CL_SystemWindow);
                    else
                        client.Out.SendMessage(
                            LanguageMgr.GetTranslation(client.Account.Language,
                                "PlayerMoveItemRequestHandler.TooFarAway", obj.GetName(0, false)),
                            eChatType.CT_System, eChatLoc.CL_SystemWindow);
                ...
                var item = client.Player.Inventory.GetItem((eInventorySlot) fromClientSlot);
                if (item == null)
                {
                    client.Out.SendInventorySlotsUpdate(new int[] {fromClientSlot});
                    client.Out.SendMessage("Null item (client slot# " + fromClientSlot + ").", eChatType.CT_System,
                        eChatLoc.CL_SystemWindow);
                    return;
                }
                ...
                if (obj is GameNPC == false || item.Count == 1)
                    ...
                    client.Player.Notify(GamePlayerEvent.GiveItem, client.Player,
                        new GiveItemEventArgs(client.Player, obj, item));
                ...
                if (client.Account.PrivLevel == (uint) ePrivLevel.Player && tradeTarget != null &&
                    tradeTarget.Client.Account.PrivLevel != (uint) ePrivLevel.Player)
                    if (!item.IsDropable && !(obj is GameNPC &&
                                              (obj is Blacksmith || obj is Recharger ||
                                               (obj as GameNPC).CanTradeAnyItem)))
                    {
                        client.Out.SendInventorySlotsUpdate(new int[] {fromClientSlot});
                        client.Out.SendMessage("You can not remove this item!", eChatType.CT_System,
                            eChatLoc.CL_SystemWindow);
                        return;
                    }
            ...
            if (fromClientSlot >= (ushort) eInventorySlot.Mithril &&
                fromClientSlot <= (ushort) eInventorySlot.Copper)
            {
                var money = new int[5];
                money[fromClientSlot - (ushort) eInventorySlot.Mithril] = itemCount;
                var flatMoney = Money.GetMoney(money[0], money[1], money[2], money[3], money[4]);
                ...
                client.Player.Notify(GamePlayerEvent.GiveMoney, client.Player,
                    new GiveMoneyEventArgs(client.Player, obj, flatMoney));
        ...
        if (client.Player.ActiveInventoryObject != null &&
            client.Player.ActiveInventoryObject.MoveItem(client.Player, fromClientSlot, toClientSlot))
        ...
        if (((fromClientSlot >= (ushort) eInventorySlot.Ground &&
              fromClientSlot <= (ushort) eInventorySlot.LastBackpack)
             || (fromClientSlot >= (ushort) eInventorySlot.FirstVault &&
                 fromClientSlot <= (ushort) eInventorySlot.LastVault)
             || (fromClientSlot >= (ushort) eInventorySlot.FirstBagHorse &&
                 fromClientSlot <= (ushort) eInventorySlot.LastBagHorse))
            && ((toClientSlot >= (ushort) eInventorySlot.Ground &&
                 toClientSlot <= (ushort) eInventorySlot.LastBackpack)
                || (toClientSlot >= (ushort) eInventorySlot.FirstVault &&
                    toClientSlot <= (ushort) eInventorySlot.LastVault)
                || (toClientSlot >= (ushort) eInventorySlot.FirstBagHorse &&
                    toClientSlot <= (ushort) eInventorySlot.LastBagHorse)))
        {
            ...
            if (toClientSlot == (ushort) eInventorySlot.Ground)
            {
                ...
                {
                    client.Out.SendInventorySlotsUpdate(new int[] {fromClientSlot});
                    client.Out.SendMessage("Invalid item (slot# " + fromClientSlot + ").", eChatType.CT_System,
                        eChatLoc.CL_SystemWindow);
                    return;
                }
                ...
                {
                    client.Out.SendInventorySlotsUpdate(new int[] {fromClientSlot});
                    client.Out.SendMessage("You can not drop this item!", eChatType.CT_System,
                        eChatLoc.CL_SystemWindow);
                    return;
                }
                ...
                {
                    client.Out.SendMessage("You drop " + item.GetName(0, false) + " on the ground!",
                        eChatType.CT_System, eChatLoc.CL_SystemWindow);
                    return;
                }
            ...
            client.Player.Inventory.MoveItem((eInventorySlot) fromClientSlot, (eInventorySlot) toClientSlot,
                itemCount);
        ...
        if (((fromClientSlot >= (ushort) eInventorySlot.Ground &&
             ...
             || (fromClientSlot >= (ushort) eInventorySlot.FirstBagHorse &&
                 fromClientSlot <= (ushort) eInventorySlot.LastBagHorse))
            && (toClientSlot == (ushort) eInventorySlot.PlayerPaperDoll ||
                toClientSlot == (ushort) eInventorySlot.NewPlayerPaperDoll
                || (toClientSlot >= (ushort) eInventorySlot.Ground &&
                    toClientSlot <= (ushort) eInventorySlot.LastBackpack)
                || (toClientSlot >= (ushort) eInventorySlot.FirstVault &&
                    toClientSlot <= (ushort) eInventorySlot.LastVault)
