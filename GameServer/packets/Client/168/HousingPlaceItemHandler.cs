...
[PacketHandlerAttribute(PacketHandlerType.TCP, eClientPackets.HousePlaceItem,
    "Handles things like placing indoor/outdoor items.", eClientStatus.PlayerInGame)]
public class HousingPlaceItemHandler : IPacketHandler
{
    ...
    public void HandlePacket(GameClient client, GSPacketIn packet)
    {
        try
        {
            var unknow1 = packet.ReadByte(); // 1=Money 0=Item (?)
            var slot = packet.ReadByte(); // Item/money slot
            var housenumber = packet.ReadShort(); // N° of house
            int unknow2 = (byte) packet.ReadByte();
            _position = (byte) packet.ReadByte();
            var method = packet.ReadByte(); // 2=Wall 3=Floor
            var rotation = packet.ReadByte(); // garden items only
            ...
            ChatUtil.SendDebugMessage(client,
                string.Format("HousingPlaceItem: slot: {0}, position: {1}, method: {2}, xpos: {3}, ypos: {4}", slot,
                    _position, method, xpos, ypos));
            ...
            var orgitem = client.Player.Inventory.GetItem((eInventorySlot) slot);
            if (orgitem == null)
            {
                client.Player.Out.SendInventorySlotsUpdate(null);
                return;
            ...
            if (orgitem.Id_nb == "house_removal_deed")
            {
                ...
                client.Player.Out.SendCustomDialog(
                    LanguageMgr.GetTranslation(client.Account.Language,
                        "WARNING: You are about to delete this house and all indoor and outdoor items attached to it!"),
                    HouseRemovalDialog);
                ...
                orgitem.Id_nb.Contains("villa_deed") || orgitem.Id_nb.Contains("mansion_deed"))
            {
                ...
                client.Player.TempProperties.setProperty(DeedWeak, new WeakRef(orgitem));
                client.Player.TempProperties.setProperty(TargetHouse, house);
                client.Player.Out.SendMessage("Warning:\n This will remove *all* items from your current house!",
                    eChatType.CT_System, eChatLoc.CL_PopupWindow);
                client.Player.Out.SendCustomDialog("Are you sure you want to upgrade your House?",
                    HouseUpgradeDialog);
            ...
            if (orgitem.Name == "deed of guild transfer")
            {
                ...
                if (HouseMgr.HouseTransferToGuild(client.Player, house))
                {
                    ...
                    InventoryLogging.LogInventoryAction(client.Player, "(HOUSE;" + housenumber + ")",
                        eInventoryActionType.Other, orgitem.Template, orgitem.Count);
            ...
            if (house.CanChangeInterior(client.Player, DecorationPermissions.Remove))
            {
                if (orgitem.Name == "interior banner removal")
                {
                    ...
                    ChatUtil.SendSystemMessage(client.Player, "Scripts.Player.Housing.InteriorBannersRemoved",
                        null);
                    return;
                }
                ...
                if (orgitem.Name == "interior shield removal")
                {
                    ...
                    ChatUtil.SendSystemMessage(client.Player, "Scripts.Player.Housing.InteriorShieldsRemoved",
                        null);
                    return;
                }
            ...
            var objType = orgitem.Object_Type;
            if (objType == 49) // Garden items 
            {
                method = 1;
            }
            else if (orgitem.Id_nb == "housing_porch_deed" || orgitem.Id_nb == "housing_porch_remove_deed" ||
                     orgitem.Id_nb == "housing_consignment_deed")
            {
                method = 4;
            }
            else if
                (objType >= 59 &&
                 objType <= 64) // Outdoor Roof/Wall/Door/Porch/Wood/Shutter/awning Material item type
            {
            ...
            switch (method)
            {
                case 1: // GARDEN OBJECT
                {
                    ...
                    foreach (GamePlayer player in WorldMgr.GetPlayersCloseToSpot(house.RegionID, house,
                ...
                case 3: // FLOOR OBJECT
                {
                    ...
                    {
                        ChatUtil.SendSystemMessage(client, "Scripts.Player.Housing.IndoorMaxItems",
                            GetMaxIndoorItemsForHouse(house.Model));
                        client.Out.SendInventorySlotsUpdate(new[] {slot});
                        return;
                    }
                    ...
                    var properRotation = client.Player.Heading / 10;
                    properRotation = properRotation.Clamp(0, 360);

                    if (method == 2 && IsSuitableForWall(orgitem))
                    {
                        properRotation = 360;
                        if (objType != 50) client.Out.SendInventorySlotsUpdate(new[] {slot});
                    }
                    ...
                    ChatUtil.SendSystemMessage(client, "Scripts.Player.Housing.IndoorItemPlaced",
                        GetMaxIndoorItemsForHouse(house.Model) - house.IndoorItems.Count);

                    switch (method)
                    {
                        case 2:
                            ChatUtil.SendSystemMessage(client, "Scripts.Player.Housing.IndoorWallPlaced",
                                orgitem.Name);
                            break;
                        case 3:
                            ChatUtil.SendSystemMessage(client, "Scripts.Player.Housing.IndoorFloorPlaced",
                                orgitem.Name);
                            break;
                    }

                    // update furniture for all players in the house
                    foreach (var plr in house.GetAllPlayersInHouse()) plr.Out.SendFurniture(house, pos);
                        ...
                        case "housing_porch_remove_deed":
                            if (house.ConsignmentMerchant != null)
                            {
                                ChatUtil.SendSystemMessage(client,
                                    "You must first remove the consignment merchant.");
                                client.Out.SendInventorySlotsUpdate(new[] {slot});
                                return;
                            }
                ...
                case 5: // HOOKPOINT
                {
                    ...
                    if (house.GetHookpointLocation((uint) _position) == null)
                    {
                        ...
                        {
                            if (client.Player.TempProperties.getProperty<bool>(
                                    HousingConstants.AllowAddHouseHookpoint, false))
                            {
                                ChatUtil.SendSystemMessage(client, "Scripts.Player.Housing.HookPointID",
                                    +_position);
                                ChatUtil.SendSystemMessage(client, "Scripts.Player.Housing.HookPointCloser", null);

                                client.Player.Out.SendCustomDialog(
                                    LanguageMgr.GetTranslation(client.Account.Language,
                                        "Scripts.Player.Housing.HookPointLogLoc"), LogLocation);
                            }
                            else
                            {
                                ChatUtil.SendDebugMessage(client,
                                    "use '/house addhookpoints' to allow addition of new housing hookpoints.");
                            }
                        }
                    }
                    else if (house.GetHookpointLocation((uint) _position) != null)
                    {
                        ...
                        foreach (var hpitem in DOLDB<DBHouseHookpointItem>.SelectObjects(DB.Column("HouseNumber")
                                     .IsEqualTo(house.HouseNumber)))
                        ...
                        else
                        {
                            var error =
                                string.Format(
                                    "Hookpoint already has item on attempt to attach {0} to hookpoint {1} for house {2}!",
                                    orgitem.Id_nb, _position, house.HouseNumber);
                ...
                case 7: // House vault.
                {
                    ...
                    if (_position > HousingConstants.MaxHookpointLocations)
                    {
                        ...
                        log.Error("HOUSING: " + client.Player.Name + " working with invalid position " + _position +
                                  " in house " +
                    ...
                    var vaultIndex = house.GetFreeVaultNumber();
                    if (vaultIndex < 0)
                    {
        ...
        catch (Exception ex)
        {
            ...
            client.Out.SendMessage("Error processing housing action; the error has been logged!",
    ...
    {
        var maxitems = Properties.MAX_INDOOR_HOUSE_ITEMS;
    ...
    private void LogLocation(GamePlayer player, byte response)
    {
        ...
        else
        {
            ...
            ChatUtil.SendSystemMessage(player,
                "Error adding position " + _position + ", error recorded in server error Log.");
        }
    }
    ...
    private static void HouseRemovalDialog(GamePlayer player, byte response)
    {
        ...
        if (consignmentMerchant != null &&
            (consignmentMerchant.DBItems(player).Count > 0 || consignmentMerchant.TotalMoney > 0))
        {
            ChatUtil.SendSystemMessage(player,
                "All items and money must be removed from your consignment merchant in order to remove this house!");
            return;
        }

        player.Inventory.RemoveItem(item);
        log.WarnFormat("HOUSING: {0}:{1} is removing house from lot {2} owned by {3}", player.Name,
            player.Client.Account.Name, house.HouseNumber, house.OwnerID);
        InventoryLogging.LogInventoryAction(player, "(HOUSE;" + house.HouseNumber + ")", eInventoryActionType.Other,
