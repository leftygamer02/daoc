...
[PacketHandlerAttribute(PacketHandlerType.TCP, eClientPackets.HouseDecorationRotate,
    "Handles housing decoration rotation", eClientStatus.PlayerInGame)]
public class HousingDecorationRotateHandler : IPacketHandler
{
    public void HandlePacket(GameClient client, GSPacketIn packet)
    {
        ...
        var housenumber = packet.ReadShort();
        ...
        var old = iitem.Rotation;
        iitem.Rotation = (iitem.Rotation + angle) % 360;

        if (iitem.Rotation < 0) iitem.Rotation = 360 + iitem.Rotation;
        ...
        foreach (var plr in house.GetAllPlayersInHouse()) plr.Client.Out.SendFurniture(house, position);
    }
}