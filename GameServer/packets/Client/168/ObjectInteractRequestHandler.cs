...
[PacketHandlerAttribute(PacketHandlerType.TCP, eClientPackets.ObjectInteractRequest,
    "Handles Client Interact Request", eClientStatus.PlayerInGame)]
public class ObjectInteractRequestHandler : IPacketHandler
{
    public void HandlePacket(GameClient client, GSPacketIn packet)
    {
        // packet.Skip(10);
        var playerX = packet.ReadInt();
        var playerY = packet.ReadInt();
    ...
    protected class InteractActionHandler : AuxRegionECSAction
    {
        ...
        protected override int OnTick(AuxECSGameTimer timer)
        {
            ...
            var region = player.CurrentRegion;
            if (region == null)
                return 0;

            var obj = region.GetObject(m_targetOid);
