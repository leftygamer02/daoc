...
/// D4 is up to 1.104
/// </summary>
[PacketHandlerAttribute(PacketHandlerType.TCP, eClientPackets.TrainRequest, "Handles Player Train Requests",
...
public class PlayerTrainRequestHandler : IPacketHandler
{
    ...
    public void HandlePacket(GameClient client, GSPacketIn packet)
    {
            ...
            client.Account.PrivLevel == (int) ePrivLevel.Player)
        {
            ...
            if (trainer == null || (trainer.CanTrain(client.Player) == false &&
                                    trainer.CanTrainChampionLevels(client.Player) == false))
            {
                client.Out.SendMessage("You must select a valid trainer for your class.", eChatType.CT_Important,
        ...
        var idLine = packet.ReadByte();
        var unk = packet.ReadByte();
        var row = packet.ReadByte();
        var skillIndex = packet.ReadByte();
        ...
        if (idLine > 0)
        {
            if (row > 0 && skillIndex > 0)
            {
                ...
                {
                    var skillstatus =
                        clspec.GetSkillStatus(tree, row - 1, skillIndex - 1);

                    if (skillstatus.Item1 == 1)
                    {
                        client.Out.SendMessage("You already have that ability!", eChatType.CT_System,
                    ...
                    {
                        client.Out.SendMessage("You do not meet the requirements for that ability!",
                    ...
                    {
                        client.Out.SendMessage("You do not have enough champion specialty points for that ability!",
                            eChatType.CT_System, eChatLoc.CL_SystemWindow);
                        return;
                    }
                ...
                {
                    client.Out.SendMessage("Could not find Champion Spec!", eChatType.CT_System,
                        eChatLoc.CL_SystemWindow);
                    log.ErrorFormat("Could not find Champion Spec idline {0}, row {1}, skillindex {2}", idLine, row,
                        skillIndex);
                }
            }
        }
        else
        {
            ...
            {
                var spec = (Specialization) speclist[skillIndex];
                ...
                var temp = client.Player.SkillSpecialtyPoints + client.Player.GetAutoTrainPoints(spec, 2);
                ...
                else
                {
                    client.Out.SendMessage(
                        "That specialization costs " + (spec.Level + 1) + " specialization points!",
                        eChatType.CT_System, eChatLoc.CL_SystemWindow);
                    client.Out.SendMessage("You don't have that many specialization points left for this level.",
                        eChatType.CT_System, eChatLoc.CL_SystemWindow);
                    return;
                }
            }
            ...
            {
                // Realm Abilities
                var raList = SkillBase.GetClassRealmAbilities(client.Player.CharacterClass.ID)
                    .Where(ra => !(ra is RR5RealmAbility));
                if (skillIndex < raList.Count() + 100)
                {
                    var ra = raList.ElementAtOrDefault(skillIndex - 100);
                    if (ra != null)
                    {
                        ra.Level = client.Player.GetAbilityLevel(ra.KeyName);
                        var cost = ra.CostForUpgrade(ra.Level);
                        ra.Level++;

                        if (client.Player.RealmSpecialtyPoints < cost)
                        {
                            client.Out.SendMessage(ra.Name + " costs " + cost + " realm ability points!",
                                eChatType.CT_System, eChatLoc.CL_SystemWindow);
                            client.Out.SendMessage(
                                "You don't have that many realm ability points left to get this.",
                                eChatType.CT_System, eChatLoc.CL_SystemWindow);
                            return;
                        }
                        ...
                        {
                            client.Out.SendMessage(
                                "You do not have required pre-requisites to train " + ra.Name +
                                " now. Come back later.", eChatType.CT_System, eChatLoc.CL_SystemWindow);
                            return;
                        }
                    ...
                    {
                        client.Out.SendMessage(
                            "Unfortunately your training failed. Please report that to admins or game master. Thank you.",
                            eChatType.CT_System, eChatLoc.CL_SystemWindow);
...
/// Handles Train clicks from Trainer Window
/// </summary>
[PacketHandlerAttribute(PacketHandlerType.TCP, eClientPackets.TrainHandler, "Handles Player Train",
    eClientStatus.PlayerInGame)]
public class PlayerTrainHandler : IPacketHandler
{
    public void HandlePacket(GameClient client, GSPacketIn packet)
    {
        ...
        var amounts = new Dictionary<uint, uint>();
        var stop = false;
        ...
        foreach (var spec in specs)
        {
            if (amounts.ContainsKey(skillcount))
                ...
                {
                    var train = new TrainCommandHandler(true);
                    train.OnCommand(client,
                        new string[] {"&trainline", spec.KeyName, amounts[skillcount].ToString()});
                    trained = true;
                }
        ...
        for (uint i = 0; i < size; i++)
        {
            ...
            if (val > 0 && !amounts.ContainsKey(i)) amounts.Add(i, val);
        }
        ...
        if (amounts != null && amounts.Count > 0)
        {
            ...
            {
                var ra = raList.ElementAtOrDefault((int) kv.Key);
                if (ra != null)
                {
                    var playerRA = (RealmAbility) client.Player.GetAbility(ra.KeyName);

                    if (playerRA != null && (playerRA.Level >= ra.MaxLevel || playerRA.Level >= kv.Value))
                        continue;

                    var cost = 0;
                    for (var i = playerRA != null ? playerRA.Level : 0; i < kv.Value; i++)
                    ...
                    if (client.Player.RealmSpecialtyPoints < cost)
                    {
                        client.Out.SendMessage(ra.Name + " costs " + cost + " realm ability points!",
                            eChatType.CT_System, eChatLoc.CL_SystemWindow);
                        client.Out.SendMessage("You don't have that many realm ability points left to get this.",
                            eChatType.CT_System, eChatLoc.CL_SystemWindow);
                        continue;
                    }
    ...
    /// </summary>
    [PacketHandlerAttribute(PacketHandlerType.TCP, eClientPackets.TrainWindowHandler, "Call Player Train Window",
        eClientStatus.PlayerInGame)]
        ...
        public void HandlePacket(GameClient client, GSPacketIn packet)
        {
                ...
                client.Account.PrivLevel == (int) ePrivLevel.Player)
            {
                ...
                if (trainer == null || (trainer.CanTrain(client.Player) == false &&
                                        trainer.CanTrainChampionLevels(client.Player) == false))
                {
                    client.Out.SendMessage("You must select a valid trainer for your class.",
