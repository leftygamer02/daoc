...
[PacketHandlerAttribute(PacketHandlerType.TCP, eClientPackets.PlayerGroundTarget,
    "Handles Player Ground Target Settings", eClientStatus.PlayerInGame)]
...
//			ushort unk2 = packet.ReadShort();
    ...
    protected class ChangeGroundTargetHandler : RegionECSAction
    {
        ...
        public ChangeGroundTargetHandler(GamePlayer actionSource, int x, int y, int z, ushort flag) : base(
            actionSource)
        {
            m_x = x;
            m_y = y;
        ...
        protected override int OnTick(ECSGameTimer timer)
        {
            ...
            if (!player.GroundTargetInView)
                player.Out.SendMessage("Your ground target is not visible!", eChatType.CT_System,
            ...
            if (player.Steed != null && player.Steed.MAX_PASSENGERS >= 1 &&
                player.Steed.OwnerID == player.InternalID)
            {
                ...
                if (player.Steed is GameBoat)
                    ...
                    {
                        player.Out.SendMessage("You usher your boat forward.", eChatType.CT_System,
                            eChatLoc.CL_SystemWindow);
                        player.Steed.WalkTo(player.GroundTarget, player.Steed.MaxSpeed);
                        return 0;
                    }

                if (player.Steed.MAX_PASSENGERS > 8 &&
                    player.Steed.CurrentRiders.Length < player.Steed.REQUIRED_PASSENGERS)
                {
