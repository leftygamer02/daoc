...
[PacketHandlerAttribute(PacketHandlerType.TCP, eClientPackets.ModifyTrade, "Player Accepts Trade",
...
public class PlayerModifyTradeHandler : IPacketHandler
{
    public void HandlePacket(GameClient client, GSPacketIn packet)
    {
        ...
        var trade = client.Player.TradeWindow;
        if (trade == null)
            return;
        if (isok == 0)
        {
        ...
        else if (isok == 1)
        {
            ...
            for (var i = 0; i < 10; i++)
            {
                var slotPosition = packet.ReadByte();
                var item = client.Player.Inventory.GetItem((eInventorySlot) slotPosition);
                if (item != null
                    && ((item.IsDropable && item.IsTradable) || client.Player.CanTradeAnyItem
                                                             || trade is SelfCraftWindow
                                                             || (trade.Partner != null &&
                                                                 trade.Partner.CanTradeAnyItem)))
                    tradeSlots.Add(item);
            }
            ...
            var money = Money.GetMoney(tradeMoney[0], tradeMoney[1], tradeMoney[2], tradeMoney[3], tradeMoney[4]);
