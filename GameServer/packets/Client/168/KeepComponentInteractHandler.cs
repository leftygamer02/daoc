...
[PacketHandlerAttribute(PacketHandlerType.TCP, eClientPackets.KeepComponentInteract, "Keep component interact",
    eClientStatus.PlayerInGame)]
public class KeepComponentInteractHandler : IPacketHandler
{
    public void HandlePacket(GameClient client, GSPacketIn packet)
    {
        var keepId = packet.ReadShort();
        var wallId = packet.ReadShort();
        var responce = packet.ReadShort();
        int HPindex = packet.ReadShort();

        var keep = GameServer.KeepManager.GetKeepByID(keepId);

        if (keep == null ||
            !(GameServer.ServerRules.IsSameRealm(client.Player, (GameKeepComponent) keep.KeepComponents[wallId],
                true) || client.Account.PrivLevel > 1))
            return;
        ...
        else if (responce == 0x02) //select an hookpoint
        {
            if (client.Account.PrivLevel > 1)
                client.Out.SendMessage("DEBUG : selected hookpoint id " + HPindex, eChatType.CT_Say,
