...
[PacketHandler(PacketHandlerType.TCP, eClientPackets.CharacterSelectRequest,
    "Handles setting SessionID and the active character", eClientStatus.LoggedIn)]
public class CharacterSelectRequestHandler : IPacketHandler
{
    public void HandlePacket(GameClient client, GSPacketIn packet)
    {
        if (client.Version >= GameClient.eClientVersion.Version1125 &&
            client.MinorRev ==
            "d") // 1125d support TODO this needs to be changed when a version greater than 1125d comes out
        {
        ...
        var charName = packet.ReadString(28);
        ...
        else
        {
            ...
            {
                var charFound = false;
                for (var i = 0; i < client.Account.Characters.Length; i++)
                ...
                {
                    // Log character play
                    AuditMgr.AddAuditEntry(client, AuditType.Character, AuditSubtype.CharacterLogin, string.Empty,
                        charName);
                }
            }
...
/// </summary>
public class
    CharacterSelectRequestHandler1125d : IPacketHandler // 1125d support TODO duplicate code from above, can be reused
{
    public void HandlePacket(GameClient client, GSPacketIn packet)
    {
        var type = (byte) packet.ReadByte(); // changed from ushort low end

        packet.Skip(1); // unknown

        var charName = packet.ReadString(24); // down from 28, both need checking
...
public class CharacterSelectRequestHandler1126 : IPacketHandler
{
    public void HandlePacket(GameClient client, GSPacketIn packet)
    {
        var charIndex = (byte) packet.ReadByte(); // character account location
            ...
            client.ClientState == GameClient.eClientState.CharScreen)
        {
            var charFound = false;
            var selectedChar = "";
            var realmOffset = charIndex - (client.Account.Realm * 10 - 10);
            var charSlot = client.Account.Realm * 100 + realmOffset;
            for (var i = 0; i < client.Account.Characters.Length; i++)
