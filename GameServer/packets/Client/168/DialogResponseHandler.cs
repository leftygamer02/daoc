...
[PacketHandlerAttribute(PacketHandlerType.TCP, eClientPackets.DialogResponse,
    "Response Packet from a Question Dialog", eClientStatus.PlayerInGame)]
public class DialogResponseHandler : IPacketHandler
{
    ...
    {
        var data1 = packet.ReadShort();
        var data2 = packet.ReadShort();
        var data3 = packet.ReadShort();
    ...
    protected class DialogBoxResponseAction : AuxRegionECSAction
    {
        ...
        public DialogBoxResponseAction(GamePlayer actionSource, uint data1, int data2, int data3, int messageType,
            byte response)
            : base(actionSource)
        {
            m_data1 = data1;
            ...
            switch ((eDialogCode) m_messageType)
            {
                ...
                case eDialogCode.GuildInvite:
                {
                    ...
                    if (m_response == 0x01) //accept
                    {
                        ...
                        if (!ServerProperties.Properties.ALLOW_GUILD_INVITE_IN_RVR &&
                            (guildLeader.CurrentZone.IsRvR || player.CurrentZone.IsRvR))
                        {
                        ...
                        {
                            player.Out.SendMessage("You are still in a guild, you'll have to leave it first.",
                                eChatType.CT_System,
                ...
                case eDialogCode.GuildLeave:
                {
                    if (m_response == 0x01) //accepte
                    {
                        if (player.Guild == null)
                        {
                            player.Out.SendMessage("You are not in a guild.", eChatType.CT_System,
                                eChatLoc.CL_SystemWindow);
                            return 0;
                        }
                    ...
                    {
                        player.Out.SendMessage("You decline to quit your guild.", eChatType.CT_System,
                            eChatLoc.CL_SystemWindow);
                        return 0;
                    }
                ...
                {
                    var questNPC =
                        (GameLiving) WorldMgr.GetObjectByIDFromRegion(player.CurrentRegionID, (ushort) m_data2);
                    if (questNPC == null)
                        return 0;
                    ...
                    if (m_response == 0x01) // accept
                    {
                        ...
                        player.Notify(m_data3 == 0x01 ? GamePlayerEvent.AbortQuest : GamePlayerEvent.AcceptQuest,
                            player, args);
                        return 0;
                    }

                    player.Notify(m_data3 == 0x01 ? GamePlayerEvent.ContinueQuest : GamePlayerEvent.DeclineQuest,
                        player, args);
                    return 0;
                }
                case eDialogCode.GroupInvite:
                {
                    if (m_response == 0x01)
                    {
                        var cln = WorldMgr.GetClientFromID(m_data1);
                        if (cln == null)
                            return 0;

                        var groupLeader = cln.Player;
                        if (groupLeader == null)
                            return 0;

                        if (player.Group != null)
                        {
                            player.Out.SendMessage("You are still in a group.", eChatType.CT_System,
                                eChatLoc.CL_SystemWindow);
                            return 0;
                        }

                        if (!GameServer.ServerRules.IsAllowedToGroup(groupLeader, player, false)) return 0;

                        if (player.InCombatPvE)
                        {
                            player.Out.SendMessage("You can't join a group while in combat!", eChatType.CT_System,
                                eChatLoc.CL_SystemWindow);
                            return 0;
                        }
                        ...
                        if (groupLeader.Group != null)
                        {
                            ...
                            {
                                player.Out.SendMessage("The group is full.", eChatType.CT_System,
                                    eChatLoc.CL_SystemWindow);
                                return 0;
                            }
                ...
                case eDialogCode.KeepClaim:
                {
                    if (m_response == 0x01)
                    {
                        ...
                        var keep = GameServer.KeepManager.GetKeepCloseToSpot(player.CurrentRegionID,
                            player, WorldMgr.VISIBILITY_DISTANCE);
                        if (keep == null)
                        {
                        ...
                        if (keep.CheckForClaim(player)) keep.Claim(player);
                ...
                case eDialogCode.HousePayRent:
                {
                    ...
                    var moneyToAdd =
                        player.TempProperties.getProperty<long>(HousingConstants.MoneyForHouseRent, -1);
                    ...
                    if (moneyToAdd != -1)
                    {
                        ...
                        if (moneyToAdd + house.KeptMoney > HouseMgr.GetRentByModel(house.Model) *
                            ServerProperties.Properties.RENT_LOCKBOX_PAYMENTS)
                            moneyToAdd =
                                HouseMgr.GetRentByModel(house.Model) *
                                ServerProperties.Properties.RENT_LOCKBOX_PAYMENTS - house.KeptMoney;

                        // take the money from the player
                        if (!player.RemoveMoney(moneyToAdd))
                            return 0;
                        InventoryLogging.LogInventoryAction(player, "(HOUSE;" + house.HouseNumber + ")",
                            eInventoryActionType.Other, moneyToAdd);
                        ...
                        player.Out.SendMessage("You deposit " + Money.GetString(moneyToAdd) + " in the lockbox.",
                            eChatType.CT_System, eChatLoc.CL_SystemWindow);
                        player.Out.SendMessage("The lockbox now has " + Money.GetString(house.KeptMoney) +
                                               " in it.  The weekly payment is " +
                                               Money.GetString(HouseMgr.GetRentByModel(house.Model)) + ".",
                            eChatType.CT_System, eChatLoc.CL_SystemWindow);
                        player.Out.SendMessage("The house is now prepaid for the next " +
                    ...
                    {
                        if (bpsToMoney + house.KeptMoney > HouseMgr.GetRentByModel(house.Model) *
                            ServerProperties.Properties.RENT_LOCKBOX_PAYMENTS)
                            bpsToMoney =
                                HouseMgr.GetRentByModel(house.Model) *
                                ServerProperties.Properties.RENT_LOCKBOX_PAYMENTS - house.KeptMoney;
                        ...
                        // notify the player of what we took and how long they are prepaid for
                        player.Out.SendMessage("You deposit " + Money.GetString(bpsToMoney) + " in the lockbox.",
                            eChatType.CT_System, eChatLoc.CL_SystemWindow);
                        player.Out.SendMessage("The lockbox now has " + Money.GetString(house.KeptMoney) +
                                               " in it.  The weekly payment is " +
                                               Money.GetString(HouseMgr.GetRentByModel(house.Model)) + ".",
