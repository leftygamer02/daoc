...
[PacketHandlerAttribute(PacketHandlerType.TCP, eClientPackets.PlayerInitRequest, "Region Entering Init Request",
...
public class PlayerInitRequestHandler : IPacketHandler
{
    ...
    protected class PlayerInitRequestAction : AuxRegionECSAction
    {
        ...
        protected override int OnTick(AuxECSGameTimer timer)
        {
            ...
            {
                player.CurrentRegion.Notify(RegionEvent.PlayerEnter, player.CurrentRegion,
                    new RegionPlayerEventArgs(player));
                player.Out.SendPlayerRevive(player);
            }

            var mobs = SendMobsAndMobEquipmentToPlayer(player);
            player.Out.SendTime();

            var checkInstanceLogin = false;
            var updateTempProperties = false;
            ...
            if (player.Guild != null) SendGuildMessagesToPlayer(player);

            SendHouseRentRemindersToPlayer(player);
            if (player.Level > 1 && ServerProperties.Properties.MOTD != "")
                player.Out.SendMessage(ServerProperties.Properties.MOTD, eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);
            else if (player.Level == 1) player.Out.SendStarterHelp();
            ...
            if (checkInstanceLogin)
                if (WorldMgr.Regions[player.CurrentRegionID] == null || player.CurrentRegion == null ||
                    player.CurrentRegion.IsInstance)
                {
                    Log.WarnFormat("{0}:{1} logging into instance or CurrentRegion is null, moving to bind!",
                        player.Name, player.Client.Account.Name);
                    player.MoveToBind();
                }

            if (player.IsUnderwater) player.IsDiving = true;
            ...
            if (updateTempProperties)
                if (ServerProperties.Properties.ACTIVATE_TEMP_PROPERTIES_MANAGER_CHECKUP)
                    try
                    {
                        IList<TempPropertiesManager.TempPropContainer> TempPropContainerList =
                            TempPropertiesManager.TempPropContainerList
                                .Where(item => item.OwnerID == player.DBCharacter.ObjectId).ToList();

                        foreach (var container in TempPropContainerList)
                        {
                            long longresult = 0;
                            if (long.TryParse(container.Value, out longresult))
                            {
                                ...
                                if (ServerProperties.Properties.ACTIVATE_TEMP_PROPERTIES_MANAGER_CHECKUP_DEBUG)
                                    Log.Debug("Container " + container.TempPropString + " with value " +
                                              container.Value + " for player " + player.Name +
                                              " was removed from container list, tempproperties added");
                            }
                    ...
                    {
                        Log.Debug("Error in TempProproperties Manager when searching TempProperties to apply: " +
                                  e.ToString());
                    }
        ...
        private static void CheckBGLevelCapForPlayerAndMoveIfNecessary(GamePlayer player)
        {
            ...
            {
                var
                    list = GameServer.KeepManager.GetKeepsOfRegion(player.CurrentRegionID);
                    ...
                    if (player.Level > k.BaseLevel)
                    {
                        player.Out.SendMessage(
                            LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "PlayerInitRequestHandler.LevelCap"),
        ...
        private static void CheckIfPlayerLogsNearEnemyKeepAndMoveIfNecessary(GamePlayer player)
        {
            ...
            var gracePeriodInMinutes = 0;
            int.TryParse(ServerProperties.Properties.RVR_LINK_DEATH_RELOG_GRACE_PERIOD, out gracePeriodInMinutes);
            var keep = GameServer.KeepManager.GetKeepCloseToSpot(player.CurrentRegionID, player,
                WorldMgr.VISIBILITY_DISTANCE);
            if (keep != null && player.Client.Account.PrivLevel == 1 &&
                GameServer.KeepManager.IsEnemy(keep, player))
            {
                if (WorldMgr.RvRLinkDeadPlayers.ContainsKey(player.InternalID))
                {
            ...
            foreach (var playerId in linkDeadPlayerIds)
                if (playerId != null &&
                    DateTime.Now.Subtract(new TimeSpan(0, gracePeriodInMinutes, 0)) >
                    WorldMgr.RvRLinkDeadPlayers[playerId])
        ...
        private static void CheckIfPlayerLogsNearKeepUnderAttackAndMoveIfNecessary(GamePlayer player)
        {
            ...
            var keep = GameServer.KeepManager.GetKeepCloseToSpot(player.CurrentRegionID, player,
                WorldMgr.VISIBILITY_DISTANCE);
            if (keep != null && keep.InCombat && player.Client.Account.PrivLevel == 1 &&
                !GameServer.KeepManager.IsEnemy(keep, player))
            {
                if (WorldMgr.RvRLinkDeadPlayers.ContainsKey(player.InternalID))
                {
        ...
        private static void SendMessageAndMoveToSafeLocation(GamePlayer player)
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language,
                    "PlayerInitRequestHandler.SaferLocation"),
        ...
        private static void SendHouseRentRemindersToPlayer(GamePlayer player)
        {
            ...
            if (house != null)
            {
                var due = house.LastPaid.AddDays(ServerProperties.Properties.RENT_DUE_DAYS).AddHours(1) -
                          DateTime.Now;
                if ((due.Days <= 0 || due.Days < ServerProperties.Properties.RENT_DUE_DAYS) &&
                    house.KeptMoney < HouseMgr.GetRentByModel(house.Model))
                {
            ...
            {
                var ghouse = HouseMgr.GetGuildHouseByPlayer(player);
                if (ghouse != null)
                {
                    var due = ghouse.LastPaid.AddDays(ServerProperties.Properties.RENT_DUE_DAYS).AddHours(1) -
                              DateTime.Now;
                    if ((due.Days <= 0 || due.Days < ServerProperties.Properties.RENT_DUE_DAYS) &&
                        ghouse.KeptMoney < HouseMgr.GetRentByModel(ghouse.Model))
                    {
                        //Sending reminder as Text Window as the Help window wasnt properly popping up on the client
                        var guildmessage = new List<string>();
                        guildmessage.Add("Rent for guild house " + ghouse.HouseNumber + " due in " + due.Days +
                                         " days!");
        ...
        private static void SendGuildMessagesToPlayer(GamePlayer player)
        {
            try
            {
                if (player.GuildRank.GcHear && player.Guild.Motd != "")
                {
                    player.Out.SendMessage(
                        LanguageMgr.GetTranslation(player.Client.Account.Language,
                            "PlayerInitRequestHandler.GuildMessage"),
                        eChatType.CT_System, eChatLoc.CL_SystemWindow);
                ...
                if (player.GuildRank.OcHear && player.Guild.Omotd != "")
                    player.Out.SendMessage(
                        LanguageMgr.GetTranslation(player.Client.Account.Language,
                            "PlayerInitRequestHandler.OfficerMessage", player.Guild.Omotd),
                        eChatType.CT_System, eChatLoc.CL_SystemWindow);

                if (player.Guild.alliance != null && player.GuildRank.AcHear &&
                    player.Guild.alliance.Dballiance.Motd != "")
                    player.Out.SendMessage(
                        LanguageMgr.GetTranslation(player.Client.Account.Language,
                            "PlayerInitRequestHandler.AllianceMessage",
                            player.Guild.alliance.Dballiance.Motd), eChatType.CT_System, eChatLoc.CL_SystemWindow);
            }
            catch (Exception ex)
            {
                Log.Error(
                    "SendGuildMessageToPlayer exception, missing guild ranks for guild: " + player.Guild.Name + "?",
                    ex);
                if (player != null)
                    player.Out.SendMessage(
