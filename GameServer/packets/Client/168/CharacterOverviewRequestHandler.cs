...
[PacketHandlerAttribute(PacketHandlerType.TCP, eClientPackets.CharacterOverviewRequest,
    "Handles account realm info and sending char overview", eClientStatus.LoggedIn)]
public class CharacterOverviewRequestHandler : IPacketHandler
{
    ...
    public void HandlePacket(GameClient client, GSPacketIn packet)
    {
        ...
        if (client.Player != null)
            try
            {
                ...
                if (cachedCharacter != null) cachedCharacter = client.Player.DBCharacter;
            }
        ...
        if (accountName.EndsWith("-X"))
        {
            if (GameServer.ServerRules.IsAllowedCharsInAllRealms(client))
                ...
                {
                    case 1:
                        client.Out.SendRealm(eRealm.Albion);
                        break;
                    case 2:
                        client.Out.SendRealm(eRealm.Midgard);
                        break;
                    case 3:
                        client.Out.SendRealm(eRealm.Hibernia);
                        break;
                    default:
                        client.Out.SendRealm(eRealm.None);
                        break;
        ...
        else
        {
            ...
            if (client.Account.Realm == (int) eRealm.None ||
                GameServer.ServerRules.IsAllowedCharsInAllRealms(client))
            {
                ...
                else
                {
                    if (log.IsErrorEnabled)
                        log.Error("User has chosen unknown realm: " + accountName + "; account=" +
                                  client.Account.Name);
                    client.Out.SendRealm(eRealm.None);
                    return;
                }
    ...
    private void _HandlePacket1124(GameClient client, GSPacketIn packet)
    {
        ...
        var realm = (byte) packet.ReadByte();
        if (realm == 0)
        {
            if (GameServer.ServerRules.IsAllowedCharsInAllRealms(client))
                client.Out.SendRealm(eRealm.None);
