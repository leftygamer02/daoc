...
[PacketHandlerAttribute(PacketHandlerType.TCP, eClientPackets.PlayerRegionChangeRequest,
...
public class PlayerRegionChangeRequestHandler : IPacketHandler
{
    ...
    {
        var jumpSpotId = packet.ReadShort();

        var targetRealm = client.Player.Realm;

        if (client.Player.CurrentRegion.Expansion == (int) eClientExpansion.TrialsOfAtlantis &&
            client.Player.CurrentZone.Realm != eRealm.None)
        ...
        var filterRealm = DB.Column("Realm").IsEqualTo((byte) targetRealm).Or(DB.Column("Realm").IsEqualTo(0))
            .Or(DB.Column("Realm").IsNull());
        ...
        if (zonePoint == null || zonePoint.TargetRegion == 0)
        {
            ChatUtil.SendDebugMessage(client,
                $"Invalid Jump (ZonePoint table): [{jumpSpotId}]{(zonePoint == null ? ". Entry missing!" : ". TargetRegion is 0!")}");
        ...
        if (client.Account.PrivLevel > 1)
        {
            ...
            client.Out.SendMessage(
                $"ZonePoint Target: Region = {zonePoint.TargetRegion}, ClassType = \'{zonePoint.ClassType}\'",
        ...
        {
            var reg = WorldMgr.GetRegion(zonePoint.TargetRegion);
            if (reg != null)
                ...
                if (client.Player.CurrentRegion.IsCustom == false && reg.IsDisabled)
                    ...
                    {
                        client.Out.SendMessage("This region has been disabled!", eChatType.CT_System,
        ...
        if (client.Player.CurrentRegion.OnZonePoint(client.Player, zonePoint) == false) return;

        //check caps for battleground
        var bg = GameServer.KeepManager.GetBattleground(zonePoint.TargetRegion);
        if (bg != null)
        ...
        if (string.IsNullOrEmpty(zonePoint.ClassType) == false)
        {
            ...
            if (customHandler != null && customHandler.GetType().FullName != zonePoint.ClassType) customHandler = null;
            ...
            if (customHandler == null)
            {
                ...
                var type = client.Player.CurrentRegion.IsInstance
                    ? "DOL.GS.ServerRules.InstanceDoorJumpPoint"
                    : zonePoint.ClassType;
                var t = ScriptMgr.GetType(type);
                ...
                if (t == null)
                    ...
                    Log.ErrorFormat("jump point {0}: class {1} must implement IJumpPointHandler interface!",
                        zonePoint.Id,
                        zonePoint.ClassType);
                else
                    try
                    ...
                    catch (Exception e)
                    {
                        customHandler = null;
                        Log.Error(
                            string.Format("jump point {0}: error creating a new instance of jump point handler {1}",
                                zonePoint.Id,
                                zonePoint.ClassType), e);
                    }
            }

            if (customHandler != null) m_customJumpPointHandlers[zonePoint.ClassType] = customHandler;
        }
    ...
    protected class RegionChangeRequestHandler : RegionECSAction
    {
        ...
        public RegionChangeRequestHandler(GamePlayer actionSource, ZonePoint zonePoint,
            IJumpPointHandler checkHandler)
            : base(actionSource)
        {
            if (zonePoint == null)
        ...
        protected override int OnTick(ECSGameTimer timer)
        {
            ...
            var reg = WorldMgr.GetRegion(m_zonePoint.TargetRegion);
            ...
            if (m_checkHandler != null)
                ...
                catch (Exception e)
                {
                    ...
                    player.Out.SendMessage("exception in jump point (" + m_zonePoint.Id + ") handler...",
                        eChatType.CT_System,
                        eChatLoc.CL_SystemWindow);
                    return 0;
                }
            ...
            //move the player
            player?.MoveTo(m_zonePoint.TargetRegion, m_zonePoint.TargetX, m_zonePoint.TargetY, m_zonePoint.TargetZ,
                m_zonePoint.TargetHeading);
            return 0;
        }
    }
}