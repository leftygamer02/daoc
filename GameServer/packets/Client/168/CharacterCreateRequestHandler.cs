...
/// </summary>
[PacketHandler(PacketHandlerType.TCP, eClientPackets.CharacterCreateRequest, "Handles character creation requests",
...
public class CharacterCreateRequestHandler : IPacketHandler
{
    ...
    public void HandlePacket(GameClient client, GSPacketIn packet)
    {
        ...
        if (client.Version > GameClient.eClientVersion.Version1124) // 1125 support
        {
            ...
            if (!string.IsNullOrEmpty(pakdata.CharName) &&
                (pakdata.CharName.Length < 3 || !nameCheck.IsMatch(pakdata.CharName)))
            ...
            switch (pakdata.Operation)
            {
                ...
                case 2: // Customize face or stats
                    if (!string.IsNullOrEmpty(pakdata.CharName))
                    {
                        ...
                        var character = client.Account.Characters != null
                            ? client.Account.Characters.FirstOrDefault(ch =>
                                ch.Name.Equals(pakdata.CharName, StringComparison.OrdinalIgnoreCase))
                            : null;
                        if (character != null)
                            needRefresh |= CheckCharacterForUpdates1125(pakdata, client, character,
                                pakdata.CustomizeType);
                    }
                    ...
                    if (!string.IsNullOrEmpty(pakdata.CharName))
                    {
                        ...
                        var character = client.Account.Characters != null
                            ? client.Account.Characters.FirstOrDefault(ch =>
                                ch.Name.Equals(pakdata.CharName, StringComparison.OrdinalIgnoreCase))
                            : null;
                        if (character == null) needRefresh |= CreateCharacter(pakdata, client, pakdata.CharacterSlot);
                    }
            ...
            if (needRefresh) client.Out.SendLoginGranted();
        ...
        if (!accountName.StartsWith(client.Account
                .Name)) // TODO more correctly check, client send accountName as account-S, -N, -H (if it not fit in 20, then only account)
        {
            if (Properties.BAN_HACKERS) client.BanAccount($"Autoban wrong Account '{accountName}'");
        ...
        for (var i = 0; i < charsCount; i++)
        {
            ...
            if (!string.IsNullOrEmpty(pakdata.CharName) &&
                ...
                if ((ePrivLevel) client.Account.PrivLevel == ePrivLevel.Player)
                {
                    if (Properties.BAN_HACKERS) client.BanAccount($"Autoban bad CharName '{pakdata.CharName}'");
            ...
            switch ((eOperation) pakdata.Operation)
            {
                ...
                case eOperation.Customize:
                    if (!string.IsNullOrEmpty(pakdata.CharName))
                    {
                        ...
                        var character = client.Account.Characters?.FirstOrDefault(ch =>
                            ch.Name.Equals(pakdata.CharName, StringComparison.OrdinalIgnoreCase));
                        if (character != null) needRefresh |= CheckCharacterForUpdates(pakdata, client, character);
                    }
                ...
                case eOperation.Create:
                    if (!string.IsNullOrEmpty(pakdata.CharName))
                    {
                        ...
                        var character = client.Account.Characters?.FirstOrDefault(ch =>
                            ch.Name.Equals(pakdata.CharName, StringComparison.OrdinalIgnoreCase));
                        if (character == null) needRefresh |= CreateCharacter(pakdata, client, i);
                    }
        ...
        if (needRefresh) client.Out.SendCharacterOverview(currentRealm);
    }

    private class CreationCharacterData
    {
        ...
        public CreationCharacterData(GSPacketIn packet, GameClient client)
        {
            if (client.Version > GameClient.eClientVersion.Version1124) // 1125+ support
            {
                ...
                if (Realm > 0) // put inside this, as when a char is deleted, there is no realm sent TODO redo how account slot is stored in DB perhaps
                    CharacterSlot -=
                        (Realm - 1) * 10; // calc to get character slot into same format used in database.

                var startRaceGender1 = (byte) packet.ReadByte();
            ...
            // 4bit=race
            var startRaceGender = (byte) packet.ReadByte();
    ...
    private bool CreateCharacter(CreationCharacterData pdata, GameClient client, int accountSlot)
    {
        ...
        if (pdata.CustomMode == 0x01)
        {
            ...
            if (log.IsDebugEnabled) log.Debug("Disable Config Button");
        }
        ...
        if (log.IsDebugEnabled) log.Debug($"Creation {client.Version} character, class:{ch.Class}, realm:{ch.Realm}");

        // Is class disabled ?
        var disabledClasses = Util.SplitCSV(Properties.DISABLED_CLASSES);
        var occurences =
        ...
        if (occurences > 0 && (ePrivLevel) client.Account.PrivLevel == ePrivLevel.Player)
        {
            if (log.IsDebugEnabled)
                log.Debug(
                    $"Client {client.Account.Name} tried to create a disabled classe: {(eCharacterClass) ch.Class}");

            return true;
        }

        // check if race disabled
        var disabledRaces = Util.SplitCSV(Properties.DISABLED_RACES);
        occurences =
        ...
        if (!Enum.IsDefined(typeof(eCharacterClass), (eCharacterClass) ch.Class))
        {
            if (log.IsErrorEnabled)
                log.Error(
                    $"{client.Account.Name} tried to create a character with wrong class ID: {ch.Class}, realm:{ch.Realm}");

            if (Properties.BAN_HACKERS)
            {
                client.BanAccount(
                    $"Autoban character create class: id:{ch.Class} realm:{ch.Realm} name:{ch.Name} account:{account.Name}");
                client.Disconnect();
                return false;
            }
        ...
        // Log creation
        AuditMgr.AddAuditEntry(client, AuditType.Account, AuditSubtype.CharacterCreate, string.Empty,
            pdata.CharName);

        client.Account.Characters = null;

        if (log.IsInfoEnabled) log.Info($"Character {pdata.CharName} created on Account {account}!");
    ...
    {
        var newModel = character.CurrentModel;
        ...
        if (pdata.CustomMode == 1 || pdata.CustomMode == 2 || pdata.CustomMode == 3)
        {
            ...
            if (pdata.CustomMode != 3)
            {
                ...
                if (flagChangedStats)
                {
                    var charClass = ScriptMgr.FindCharacterClass(character.Class);

                    if (charClass != null)
                    {
                        var valid = IsCustomPointsDistributionValid(character, stats, out var points);
                        ...
                        if (Properties.ALLOW_CUSTOMIZE_STATS_AFTER_CREATION)
                        {
                            ...
                            if (log.IsInfoEnabled) log.Info($"Character {character.Name} Stats updated in cache!");

                            if (client.Player != null)
                            {
                                foreach (var stat in stats.Keys)
                                    client.Player.ChangeBaseStat(stat,
                                        (short) (stats[stat] - client.Player.GetBaseStat(stat)));
            ...
            if (pdata.CustomMode == 2) // change player customization
            {
                ...
                if (Properties.ALLOW_CUSTOMIZE_FACE_AFTER_CREATION)
                {
                    if (pdata.CreationModel != character.CreationModel) character.CurrentModel = newModel;

                    if (log.IsInfoEnabled)
                        log.Info(
                            $"Character {character.Name} face properties configured by account {client.Account.Name}!");
                }
            }
    ...
    // 1125 support
    private bool CheckCharacterForUpdates1125(CreationCharacterData pdata, GameClient client,
        DOLCharacters character, int type)
    {
        var newModel = character.CurrentModel;
        ...
        if (pdata.CustomMode == 1 || pdata.CustomMode == 2 || pdata.CustomMode == 3)
        {
                    ...
                    if (flagChangedStats)
                    {
                        ...
                        if (charClass != null)
                        {
                            ...
                            if (points > MaxStartingBonusPoints)
                            {
                                ...
                                {
                                    if (Properties.BAN_HACKERS)
                                        client.BanAccount(string.Format(
                                            "Autoban Hack char update : Wrong allowed points:{0}", points));

                                    log.InfoFormat("Disconnecting {0} because the stats  are above expected",
                                        character.Name);
                                    client.Disconnect();
                                    return false;
                                }
                            ...
                            if (Properties.ALLOW_CUSTOMIZE_STATS_AFTER_CREATION || character.CustomisationStep == 3)
                            {
                                ...
                                if (client.Player != null)
                                {
                                    ...
                                    if (log.IsInfoEnabled)
                                        log.InfoFormat("Character {0} Player Stats updated in cache!",
                                            character.Name);
                                }
            ...
            if (pdata.CustomMode ==
                2) // change player customization // is this changing starting race? im not sure TODO check
            {
                if (client.Account.PrivLevel == 1 && ((pdata.CreationModel >> 11) & 3) == 0)
                {
                    if (Properties.BAN_HACKERS) // Player size must be > 0 (from 1 to 3)
                    {
                        client.BanAccount(
                            string.Format("Autoban Hack char update : zero character size in model:{0}", newModel));
                        client.Disconnect();
                        return false;
                    }
                ...
                if (Properties.ALLOW_CUSTOMIZE_FACE_AFTER_CREATION)
                {
                    ...
                    if (log.IsInfoEnabled)
                        log.InfoFormat("Character {0} face properties configured by account {1}!", character.Name,
                            client.Account.Name);
                }
            }
            else if (pdata.CustomMode == 3) //auto config -- seems someone thinks this is not possible?
    ...
    public static bool CheckForDeletedCharacter(string accountName, GameClient client, int slot)
    {
        ...
        var allChars = client.Account.Characters;

        if (allChars != null)
            foreach (var character in allChars.ToArray())
                if (character.AccountSlot == charSlot && client.ClientState == GameClient.eClientState.CharScreen)
                {
                    if (log.IsWarnEnabled)
                        log.Warn(
                            $"DB Character Delete:  Account {accountName}, Character: {character.Name}, slot position: {character.AccountSlot}, client slot {slot}");

                    if (allChars.Length < client.ActiveCharIndex && client.ActiveCharIndex > -1 &&
                        allChars[client.ActiveCharIndex] == character)
                        client.ActiveCharIndex = -1;

                    GameEventMgr.Notify(DatabaseEvent.CharacterDeleted, null,
                        new CharacterEventArgs(character, client));
                    ...
                    if (Properties.BACKUP_DELETED_CHARACTERS)
                    {
                        ...
                        if (log.IsWarnEnabled)
                            log.Warn(
                                $"DB Character {character.ObjectId} backed up to DOLCharactersBackup and no associated content deleted.");
                    }
                    else
                    {
                        ...
                        try
                        {
                            var objs = GameServer.Database.SelectObjects<InventoryItem>(DB.Column("OwnerID")
                                .IsEqualTo(character.ObjectId));
                        ...
                        catch (Exception e)
                        {
                            if (log.IsErrorEnabled)
                                log.Error(
                                    $"Error deleting char items, char OID={character.ObjectId}, Exception:{e}");
                        }

                        // delete quests
                        try
                        {
                            var objs = GameServer.Database.SelectObjects<DBQuest>(DB.Column("Character_ID")
                                .IsEqualTo(character.ObjectId));
                            GameServer.Database.DeleteObject(objs);
                        }
                        catch (Exception e)
                        {
                            if (log.IsErrorEnabled)
                                log.Error(
                                    $"Error deleting char quests, char OID={character.ObjectId}, Exception:{e}");
                        }

                        // delete ML steps
                        try
                        {
                            var objs = GameServer.Database.SelectObjects<DBCharacterXMasterLevel>(
                                DB.Column("Character_ID").IsEqualTo(character.ObjectId));
                            GameServer.Database.DeleteObject(objs);
                        }
                        catch (Exception e)
                        {
                            if (log.IsErrorEnabled)
                                log.Error(
                                    $"Error deleting char ml steps, char OID={character.ObjectId}, Exception:{e}");
                        }
                    }

                    var deletedChar = character.Name;
                    ...
                    // Log deletion
                    AuditMgr.AddAuditEntry(client, AuditType.Character, AuditSubtype.CharacterDelete, string.Empty,
                        deletedChar);
    ...
    /// <returns></returns>
    public static bool IsCustomPointsDistributionValid(DOLCharacters character, IDictionary<eStat, int> stats,
        out int points)
    {
        var charClass = ScriptMgr.FindCharacterClass(character.Class);
        ...
        if (charClass != null)
        {
            ...
            foreach (var stat in stats.Keys)
            {
                var raceAmount = GlobalConstants.STARTING_STATS_DICT[(eRace) character.Race][stat];

                var classAmount = 0;

                for (var level = character.Level; level > 5; level--)
                {
                    if (charClass.PrimaryStat != eStat.UNDEFINED && charClass.PrimaryStat == stat) classAmount++;
                ...
                var above = stats[stat] - raceAmount - classAmount;

                // Miss Some points...
                if (above < 0 && character.Level == 1) return false;
    ...
    public static bool IsCharacterValid(DOLCharacters ch)
    {
        ...
        try
        {
            ...
            if (!GlobalConstants.STARTING_CLASSES_DICT.ContainsKey((eRealm) ch.Realm) || !GlobalConstants
                    .STARTING_CLASSES_DICT[(eRealm) ch.Realm].Contains((eCharacterClass) ch.Class))
            {
                if (log.IsWarnEnabled)
                    log.Warn(
                        $"Wrong class: {ch.Class}, realm:{ch.Realm} on character creation from Account: {ch.AccountName}");

                valid = false;
            }

            var charClass = ScriptMgr.FindCharacterClass(ch.Class);

            if (!charClass.EligibleRaces.Exists(s => (int) s.ID == ch.Race))
            {
                if (log.IsWarnEnabled)
                    log.WarnFormat("Wrong race: {0}, class:{1} on character creation from Account: {2}", ch.Race,
                        ch.Class, ch.AccountName);
                valid = false;
            }
            ...
            var stats = new Dictionary<eStat, int>
            {
                {eStat.STR, ch.Strength}, {eStat.CON, ch.Constitution}, {eStat.DEX, ch.Dexterity},
                {eStat.QUI, ch.Quickness},
                {eStat.INT, ch.Intelligence}, {eStat.PIE, ch.Piety}, {eStat.EMP, ch.Empathy},
                {eStat.CHR, ch.Charisma}
            };
            ...
            var gender = ch.Gender == 0 ? eGender.Male : eGender.Female;

            if (GlobalConstants.RACE_GENDER_CONSTRAINTS_DICT.ContainsKey((eRace) ch.Race) &&
                GlobalConstants.RACE_GENDER_CONSTRAINTS_DICT[(eRace) ch.Race] != gender)
            {
                if (log.IsWarnEnabled)
                    log.Warn(
                        $"Wrong Race gender: {ch.Gender}, race: {ch.Race} on character creation from Account: {ch.AccountName}");

                valid = false;
            }

            if (GlobalConstants.CLASS_GENDER_CONSTRAINTS_DICT.ContainsKey((eCharacterClass) ch.Class) &&
                GlobalConstants.CLASS_GENDER_CONSTRAINTS_DICT[(eCharacterClass) ch.Class] != gender)
            {
                if (log.IsWarnEnabled)
                    log.Warn(
                        $"Wrong class gender: {ch.Gender}, class:{ch.Class} on character creation from Account: {ch.AccountName}");
        ...
        catch (Exception e)
        {
            if (log.IsErrorEnabled)
                log.Error(
                    $"CharacterCreation error on account {ch.AccountName}, slot {ch.AccountSlot}. Exception:{e}");
