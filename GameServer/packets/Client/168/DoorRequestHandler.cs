...
[PacketHandlerAttribute(PacketHandlerType.TCP, eClientPackets.DoorRequest, "Door Interact Request Handler",
...
public class DoorRequestHandler : IPacketHandler
{
    ...
    public void HandlePacket(GameClient client, GSPacketIn packet)
    {
        ...
        var doorType = doorID / 100000000;

        var radius = Properties.WORLD_PICKUP_DISTANCE * 4;
        var zoneDoor = doorID / 1000000;
        ...
        if (client.Account.PrivLevel > 1 || Properties.ENABLE_DEBUG)
        {
            if (doorType == 7)
            {
                ...
                client.Out.SendDebugMessage(
                    $"Keep Door ID:{doorID} state:{doorState} (Owner Keep:{ownerKeepId} KeepID:{keepID} ComponentID:{componentID} DoorIndex:{doorIndex} TowerNumber:{towerNum})");

                if (keepID > 255 && ownerKeepId < 10)
                    ChatUtil.SendDebugMessage(client,
                        "Warning: Towers with an Owner Keep ID < 10 will have untargetable doors!");
            }
            else if (doorType == 9)
            {
                var doorIndex = doorID - doorType * 10000000;
                client.Out.SendDebugMessage(
                    $"House DoorID:{doorID} state:{doorState} (doorType:{doorType} doorIndex:{doorIndex})");
            }
            else
            {
                var fixture = doorID - zoneDoor * 1000000;
                var fixturePiece = fixture;
                fixture /= 100;
                fixturePiece = fixturePiece - fixture * 100;

                client.Out.SendDebugMessage(
                    $"{debugText}DoorID:{doorID} state:{doorState} zone:{zoneDoor} fixture:{fixture} fixturePiece:{fixturePiece} Type:{doorType}");
            }
        }

        if (client.Player.TargetObject is GameDoor &&
            !client.Player.IsWithinRadius(client.Player.TargetObject, radius))
        {
            client.Player.Out.SendMessage("You are too far to open this door", eChatType.CT_Important,
            ...
            if (doorType != 9 && client.Account.PrivLevel > 1 && client.Player.CurrentRegion.IsInstance == false)
            {
                if (client.Player.TempProperties.getProperty(DoorMgr.WANT_TO_ADD_DOORS, false))
                    client.Player.Out.SendCustomDialog(
                        "This door is not in the database. Place yourself nearest to this door and click Accept to add it.",
                        AddingDoor);
                else
    ...
    public void AddingDoor(GamePlayer player, byte response)
    {
        ...
        var doorType = m_handlerDoorID / 100000000;
        if (doorType == 7)
        {
    ...
    protected class ChangeDoorAction : RegionECSAction
    {
        ...
        protected override int OnTick(ECSGameTimer timer)
        {
            ...
            var doorList = DoorMgr.getDoorByID(m_doorId);

            if (doorList.Count > 0)
            {
                var success = false;
                foreach (var mydoor in doorList)
                {
                    if (success)
                        break;
                ...
                if (!success)
                    player.Out.SendMessage(
                        LanguageMgr.GetTranslation(player.Client.Account.Language,
                            "DoorRequestHandler.OnTick.TooFarAway", doorList[0].Name),
