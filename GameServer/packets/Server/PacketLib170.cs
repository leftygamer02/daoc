...
public class PacketLib170 : PacketLib169
{
            ...
            else
            {
                ...
                if (name.Length > byte.MaxValue)
                {
                    if (log.IsWarnEnabled)
                        log.Warn(quest.GetType().ToString() + ": name is too long for 1.68+ clients (" +
                                 name.Length + ") '" + name + "'");
                ...
                {
                    if (log.IsWarnEnabled)
                        log.Warn(quest.GetType().ToString() + ": description is too long for 1.68+ clients (" +
                                 desc.Length + ") '" + desc + "'");
    ...
    public override void SendWarmapBonuses()
    {
        ...
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.WarmapBonuses)))
        {
            ...
            var magic = RelicMgr.GetRelicCount(m_gameClient.Player.Realm, eRelicType.Magic);
            var strength = RelicMgr.GetRelicCount(m_gameClient.Player.Realm, eRelicType.Strength);
            var relics = (byte) ((magic << 4) | strength);
