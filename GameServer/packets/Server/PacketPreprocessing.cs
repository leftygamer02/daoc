...
public class PacketPreprocessing
{
    ...
    public PacketPreprocessing()
    {
        ...
        RegisterPreprocessors((int) eClientStatus.LoggedIn,
            (client, packet) => client.Account != null); // player must be logged into an account
        RegisterPreprocessors((int) eClientStatus.PlayerInGame,
            (client, player) => client.Player != null); // player must be logged into a character
    }
    ...
    public void RegisterPacketDefinition(int packetId, int preprocessorId)
    {
        ...
        {
            log.InfoFormat("Replacing Packet Processor for packet ID {0} with preprocessorId {1}", packetId,
                preprocessorId);
