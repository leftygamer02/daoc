...
public class PacketLib173 : PacketLib172
{
    ...
    public override void SendRegions(ushort regionId)
    {
        if (m_gameClient.Player != null)
        {
            if (!m_gameClient.Socket.Connected)
                return;
            var region = WorldMgr.GetRegion((ushort) m_gameClient.Player.CurrentRegionID);
            if (region == null)
                return;
            using (var pak = new GSTCPPacketOut(0xB1))
            {
        ...
        else
        {
            ...
            while (entries != null && count > index)
                using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.ClientRegions)))
                {
                    for (var i = 0; i < 4; i++)
                    {
                        while (index < count && (int) m_gameClient.ClientType <= entries[index].expansion) index++;
    ...
    public override void SendHexEffect(GamePlayer player, byte effect1, byte effect2, byte effect3, byte effect4,
        byte effect5)
    {
        if (player == null)
            return;
            ...
            else
            {
                ...
                if (name.Length > byte.MaxValue)
                {
                    if (log.IsWarnEnabled)
                        log.Warn(quest.GetType().ToString() + ": name is too long for 1.71 clients (" +
                                 name.Length + ") '" + name + "'");
                ...
                {
                    if (log.IsWarnEnabled)
                        log.Warn(quest.GetType().ToString() + ": description is too long for 1.71 clients (" +
                                 desc.Length + ") '" + desc + "'");
                ...
                {
                    if (log.IsWarnEnabled)
                        log.Warn(quest.GetType().ToString() +
