...
public class PacketLib1126 : PacketLib1125
{
    ...
    public override void SendCharacterOverview(eRealm realm)
    {
        ...
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.CharacterOverview1126)))
        {
            ...
            foreach (var c in m_gameClient.Account.Characters)
                ...
                {
                    log.Error(
                        $"SendCharacterOverview - Duplicate char in slot? Slot: {c.AccountSlot}, Account: {c.AccountName}",
                        ex);
                }
            ...
            if (charsBySlot.Any())
            {
                ...
                foreach (var item in allItems)
                    ...
                    {
                        log.Error(
                            $"SendCharacterOverview - Duplicate item on character? OwnerID: {item.OwnerID}, SlotPosition: {item.SlotPosition}, Account: {m_gameClient.Account.Name}",
                            ex);
                    }
            }
            ...
            for (var i = firstSlot; i < firstSlot + 10; i++)
            {
                ...
                if (leftHandWeapon != null)
                    leftHandWeaponColor =
                        (ushort) (leftHandWeapon.Emblem != 0 ? leftHandWeapon.Emblem : leftHandWeapon.Color);
                ushort torsoColor = 0;
                if (torso != null)
