...
public class PacketEncoding1110 : IPacketEncoding
{
    ...
    public byte[] DecryptPacket(byte[] buf, int offset, bool udpPacket)
    {
        ...
        if (_sbox == null || EncryptionState == eEncryptionState.NotEncrypted)
            return buf;
        var tmpsbox = new byte[_sbox.Length];
        Array.Copy(_sbox, 0, tmpsbox, 0, _sbox.Length);
        byte i = 0;
        byte j = 0;
        var len =
            (ushort) ((buf[offset] << 8) | (buf[offset + 1] + 10)); //+10 byte for packet#,session,param,code,checksum
        offset += 2;
        int k;
        for (k = len / 2 + offset; k < len + offset; k++)
        {
            ...
            tmpsbox[i] = tmpsbox[j];
            tmpsbox[j] = tmp;
            var xorKey = tmpsbox[(byte) (tmpsbox[i] + tmpsbox[j])];
            buf[k] ^= xorKey;
            j += buf[k];
        }
    ...
    public byte[] EncryptPacket(byte[] buf, int offset, bool udpPacket)
    {
        ...
        byte i = 0;
        byte j = 0;
        var len = (ushort) ((buf[offset] << 8) | buf[offset + 1]);
        offset += 2;
    ...
    private static byte[] GenerateSBox(byte[] commonKey)
    {
        ...
        int x, y;
        for (x = 0; x < sbox.Length; x++) sbox[x] = (byte) x;
