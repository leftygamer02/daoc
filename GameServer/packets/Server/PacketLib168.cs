    ...
    public virtual void SendVersionAndCryptKey()
    {
        ...
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.CryptKey)))
        {
            ...
            pak.WriteByte(0x00);
				...
				((PacketEncoding168) m_gameClient.PacketProcessor.Encoding).EncryptionState =
 PacketEncoding168.eEncryptionState.RSAEncrypted;
#endif
    ...
    public virtual void SendCharacterOverview(eRealm realm)
    {
        ...
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.CharacterOverview)))
        {
            ...
            var characters = m_gameClient.Account.Characters;
            if (characters == null)
                pak.Fill(0x0, 1848);
            else
                for (var i = firstAccountSlot; i < firstAccountSlot + 8; i++)
                {
                    var written = false;
                    for (var j = 0; j < characters.Length && written == false; j++)
                        if (characters[j].AccountSlot == i)
                        {
                            ...
                            var reg = WorldMgr.GetRegion((ushort) characters[j].Region);
                            if (reg != null)
                            {
                                var description = m_gameClient.GetTranslatedSpotDescription(reg, characters[j].Xpos,
                                    characters[j].Ypos, characters[j].Zpos);
                            ...
                            pak.FillString(
                                m_gameClient.RaceToTranslatedName(characters[j].Race, characters[j].Gender), 24);
                            pak.WriteByte((byte) characters[j].Level);
                            pak.WriteByte((byte) characters[j].Class);
                            pak.WriteByte((byte) characters[j].Realm);
                            pak.WriteByte(
                                (byte) ((((characters[j].Race & 0x10) << 2) + (characters[j].Race & 0x0F)) |
                                        (characters[j].Gender << 4)));
                            ...
                            var items = DOLDB<InventoryItem>.SelectObjects(DB.Column("OwnerID")
                                .IsEqualTo(characters[j].ObjectId)
                                .And(DB.Column("SlotPosition").IsGreaterOrEqualTo(10)
                                    .And(DB.Column("SlotPosition").IsLessOrEqualTo(29))));
                            var found = 0;
                            //16 bytes: armor model
                            for (var k = 0x15; k < 0x1D; k++)
                            {
                                found = 0;
    ...
    public virtual void SendCharStatsUpdate()
    {
        ...
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.StatsUpdate), 36))
        {
            ...
            pak.WriteShort(
                (ushort) (m_gameClient.Player.GetModified(eProperty.Strength) -
                          m_gameClient.Player.GetBaseStat(eStat.STR)));
            pak.WriteShort(
                (ushort) (m_gameClient.Player.GetModified(eProperty.Dexterity) -
                          m_gameClient.Player.GetBaseStat(eStat.DEX)));
            pak.WriteShort(
                (ushort) (m_gameClient.Player.GetModified(eProperty.Constitution) -
                          m_gameClient.Player.GetBaseStat(eStat.CON)));
            pak.WriteShort(
                (ushort) (m_gameClient.Player.GetModified(eProperty.Quickness) -
                          m_gameClient.Player.GetBaseStat(eStat.QUI)));
            pak.WriteShort(
                (ushort) (m_gameClient.Player.GetModified(eProperty.Intelligence) -
                          m_gameClient.Player.GetBaseStat(eStat.INT)));
            pak.WriteShort(
                (ushort) (m_gameClient.Player.GetModified(eProperty.Piety) -
                          m_gameClient.Player.GetBaseStat(eStat.PIE)));
            pak.WriteShort(
                (ushort) (m_gameClient.Player.GetModified(eProperty.Empathy) -
                          m_gameClient.Player.GetBaseStat(eStat.EMP)));
            pak.WriteShort(
                (ushort) (m_gameClient.Player.GetModified(eProperty.Charisma) -
                          m_gameClient.Player.GetBaseStat(eStat.CHR)));
    ...
    {
        var entries = WorldMgr.GetRegionList();
        ...
        while (count > index)
            using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.ClientRegions)))
            {
                ...
                {
                    while (index < count &&
                           (entries[index].id > byte.MaxValue ||
                            (int) m_gameClient.ClientType <= entries[index].expansion))
    ...
    public virtual void SendNPCCreate(GameNPC npc)
    {
        ...
        // Update Cache
        m_gameClient.GameObjectUpdateArray[new Tuple<ushort, ushort>(npc.CurrentRegionID, (ushort) npc.ObjectID)] =
            0;
    }
    ...
    public virtual void SendLivingEquipmentUpdate(GameLiving living)
    {
        ...
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.EquipmentUpdate)))
        {
            ...
            pak.WriteByte((byte) ((living.IsCloakHoodUp ? 0x01 : 0x00) |
                                  (int) living.rangeAttackComponent.ActiveQuiverSlot));
    ...
    public virtual void SendGroupWindowUpdate()
    {
        ...
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.VariousUpdate)))
        {
            ...
            if (group != null)
                foreach (var updateLiving in group.GetMembersInTheGroup())
                {
                    var sameRegion = updateLiving.CurrentRegion == m_gameClient.Player.CurrentRegion;
                    ...
                    pak.WritePascalString(updateLiving is GamePlayer
                        ? ((GamePlayer) updateLiving).CharacterClass.Name
                        : "NPC");
                    //classname
                }
    ...
    public virtual void SendInventorySlotsUpdate(ICollection<int> slots)
    {
        ...
        if (slots == null || slots.Count <= Properties.MAX_ITEMS_PER_PACKET)
        {
            SendInventorySlotsUpdateRange(slots, 0);
        }
        else
        {
            var updateSlots = new List<int>(Properties.MAX_ITEMS_PER_PACKET);
            foreach (var slot in slots)
            {
                updateSlots.Add(slot);
                if (updateSlots.Count >= Properties.MAX_ITEMS_PER_PACKET)
                {
        ...
        ICollection<InventoryItem> itemsToUpdate)
    {
        ...
        // so we send big updates in parts
        var slotsToUpdate =
            new List<int>(Math.Min(Properties.MAX_ITEMS_PER_PACKET, itemsToUpdate.Count));
        foreach (var item in itemsToUpdate)
        {
            ...
            if (slotsToUpdate.Count >= Properties.MAX_ITEMS_PER_PACKET)
            {
    ...
    public virtual void SendDoorState(Region region, IDoor door)
    {
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.DoorState)))
        {
            ...
            if (flag == 0 && doorType != 7 && region != null && region.IsDungeon == false &&
            ...
            lock (m_gameClient.Player.TradeWindow.Sync)
            {
                ...
                if (items != null)
                    foreach (InventoryItem item in items)
                    {
                        ...
                        pak.WriteByte((byte) (item.Hand << 6));
                        pak.WriteByte(
                            (byte) ((item.Type_Damage > 3 ? 0 : item.Type_Damage << 6) | item.Object_Type));
                ...
                if (m_gameClient.Player.TradeWindow is SelfCraftWindow)
                    ...
                    pak.WritePascalString("Trading with " +
                                          m_gameClient.Player.TradeWindow.Partner.Name); // transaction with ...
                SendTCP(pak);
            }
        }
    }
    ...
    public virtual void SendPlayerForgedPosition(GamePlayer player)
    {
        using (var pak = new GSUDPPacketOut(GetPacketCode(eServerPackets.PlayerPosition)))
        {
            ...
            // Get Off Corrd
            var offX = player.X - player.CurrentZone.XOffset;
            var offY = player.Y - player.CurrentZone.YOffset;
            ...
            else
            {
                ...
                var contenthead = (ushort) (player.Heading + (true ? 0x1000 : 0));
            ...
            // Write health + Attack
            var healthcontent = (byte) (player.HealthPercent + (player.attackComponent.AttackState ? 0x80 : 0));
            ...
            using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.VariousUpdate)))
            {
                ...
                while (index < total)
                {
                    ...
                    if (index >= byte.MaxValue ||
                        pak.Length + 8 + usableSkills[index].Item1.Name.Length > 1000)
                        break;
    ...
    {
        var text = m_gameClient.Player.FormatStatistics();

        text.Add(" ");
        text.Add("Titles:");

        foreach (var title in m_gameClient.Player.Titles)
    ...
    public virtual void SendTrainerWindow()
    {
        ...
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.TrainerWindow)))
        {
            ...
            {
                var level = m_gameClient.Player.GetAbilityLevel(ra.KeyName);
                pak.WriteByte((byte) i++);
                pak.WriteByte((byte) level);
                pak.WriteByte((byte) ra.CostForUpgrade(level));
                var canBeUsed = ra.CheckRequirement(m_gameClient.Player);
    ...
    public virtual void SendUpdateIcons(IList changedEffects, ref int lastUpdateEffectsCount)
    {
        ...
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.UpdateIcons)))
        {
            ...
            {
                foreach (var effect in m_gameClient.Player.EffectList)
                    if (effect.Icon != 0)
                        fxcount++;
    ...
    {
        if (m_gameClient.Account.PrivLevel > (int) ePrivLevel.Player || Properties.ENABLE_DEBUG)
    ...
    public virtual void SendConcentrationList()
    {
        ...
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.ConcentrationList)))
        {
            lock (m_gameClient.Player.effectListComponent.ConcentrationEffectsLock)
            {
                ...
                for (var i = 0; i < m_gameClient.Player.effectListComponent.ConcentrationEffects.Count; i++)
                {
        ...
        eWalkState walkState)
    {
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.PetWindow)))
        {
            ...
            if (pet != null)
                ...
                {
                    var count = 0;
                    foreach (var effect in pet.EffectList)
                    {
                        pak.WriteShort(effect
                            .Icon); // 0x08 - null terminated - (byte) list of shorts - spell icons on pet
                        if (++count > 8) break;
                    }
                }
    ...
    public virtual void SendMasterLevelWindow(byte ml)
    {
        ...
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.MasterLevelWindow)))
        {
            ...
            if (mlRequired < 10)
                ...
                for (var i = 1; i < 11; i++)
                {
                    var description = m_gameClient.Player.GetMLStepDescription(mlRequired, i);
    ...
    protected virtual void SendQuestPacket(AbstractQuest quest, int index)
    {
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.QuestEntry)))
        {
            ...
            else
            {
                ...
                {
                    if (log.IsWarnEnabled)
                        log.Warn(quest.GetType() + ": name is too long for 1.68+ clients (" + name.Length + ") '" +
                                 name + "'");
                ...
                {
                    if (log.IsWarnEnabled)
                        log.Warn(quest.GetType() + ": description is too long for 1.68+ clients (" + desc.Length +
                                 ") '" + desc + "'");
    ...
    protected virtual void WriteGroupMemberUpdate(GSTCPPacketOut pak, bool updateIcons, GameLiving living)
    {
        ...
        if (sameRegion)
        {
            ...
            if (SpellHelper.FindEffectOnTarget(living, "DamageOverTime") != null)
                playerStatus |= 0x08;
            if (living is GamePlayer &&
                ((GamePlayer) living).Client.ClientState == GameClient.eClientState.Linkdead)
                playerStatus |= 0x10;
    ...
    protected virtual void SendInventorySlotsUpdateRange(ICollection<int> slots, eInventoryWindowType windowType)
    {
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.InventoryUpdate)))
        {
            ...
            pak.WriteByte(
                (byte) ((m_gameClient.Player.IsCloakHoodUp ? 0x01 : 0x00) |
                        (int) m_gameClient.Player.rangeAttackComponent.ActiveQuiverSlot));
            //bit0 is hood up bit4 to 7 is active quiver
            ...
            if (slots != null)
                foreach (var updatedSlot in slots)
                {
                    if (updatedSlot >= (int) eInventorySlot.Consignment_First &&
                        ...
                        pak.WriteByte(
                            (byte) (updatedSlot - (int) eInventorySlot.Consignment_First +
                                    (int) eInventorySlot.HousingInventory_First));
                    else
                        pak.WriteByte((byte) updatedSlot);
                    var item = m_gameClient.Player.Inventory.GetItem((eInventorySlot) updatedSlot);
                    ...
                    switch (item.Object_Type)
                    {
                        ...
                        case (int) eObjectType.Thrown:
                            ...
                            break;
                        case (int) eObjectType.Instrument:
                            value1 = item.DPS_AF == 2
                                ? 0
                                : item.DPS_AF; // 0x00 = Lute ; 0x01 = Drum ; 0x03 = Flute
                            value2 = 0;
                            break; // unused
                        case (int) eObjectType.Shield:
                            value1 = item.Type_Damage;
