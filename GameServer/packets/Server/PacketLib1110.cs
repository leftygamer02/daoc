...
public class PacketLib1110 : PacketLib1109
{
        ...
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.UpdateIcons)))
        {
            ...
            foreach (var effect in m_gameClient.Player.EffectList)
                if (effect.Icon != 0)
                {
                    ...
                    pak.WriteByte(effect is GameSpellEffect || effect.Icon > 5000
                        ? (byte) (fxcount - 1)
                        : (byte) 0xff);
                    ...
                    if (effect is GameSpellEffect)
                        pak.WriteShort((ushort) ((GameSpellEffect) effect).Spell
                            .InternalID); //v1.110+ send the spell ID for delve info in active icon
                    else
                    ...
                    {
                        if (((StaticEffect) effect).HasNegativeEffect) flagNegativeEffect = 1;
                    }
                    else if (effect is GameSpellEffect)
                    {
                        if (!((GameSpellEffect) effect).SpellHandler.HasPositiveEffect) flagNegativeEffect = 1;
                    }
    ...
    public override void SendSiegeWeaponAnimation(GameSiegeWeapon siegeWeapon)
    {
        ...
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.SiegeWeaponAnimation)))
        {
            ...
            pak.WriteShort((ushort) siegeWeapon.SiegeWeaponTimer
                .TimeUntilElapsed); // timer is no longer ( value / 100 )
