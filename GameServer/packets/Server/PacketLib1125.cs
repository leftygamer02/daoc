...
public class PacketLib1125 : PacketLib1124
{
    ...
    public override void SendVersionAndCryptKey()
    {
        ...
        {
            pak.WritePascalStringIntLE((int) m_gameClient.Version / 1000 + "." +
                                       ((int) m_gameClient.Version - 1000) + m_gameClient.MinorRev);
        ...
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.CharacterOverview)))
        {
            ...
            else
            {
                ...
                for (var i = firstSlot; i < firstSlot + 10; i++)
                    ...
                    {
                        if (!itemsByOwnerID.TryGetValue(c.ObjectId,
                                out var charItems))
                        ...
                        if (charItems.TryGetValue(eInventorySlot.FeetArmor, out item)) extensionBoots = item.Extension;
                        ...
                        if (region != null)
                            locationDescription =
                                region.GetTranslatedSpotDescription(m_gameClient, c.Xpos, c.Ypos, c.Zpos);

                        if (locationDescription.Length >
                            23) // location name over 23 chars has to be truncated eg. "The Great Pyramid of Stygia"
                            locationDescription = locationDescription.Substring(0, 20) + "...";

                        pak.WritePascalStringIntLE(locationDescription);

                        var classname = "";
                        if (c.Class != 0) classname = ((eCharacterClass) c.Class).ToString();
                        ...
                        else
                        {
                            ...
                            if (rightHandWeapon != null) righthand = 0x00;

                            if (leftHandWeapon != null) lefthand = 0x01;
    ...
    public override void SendUDPInitReply()
    {
        using (var pak = new GSUDPPacketOut(GetPacketCode(eServerPackets.UDPInitReply)))
        {
            ...
            var datetimenow = (ulong) DateTime.Now.Ticks >> 24; //shift 24 bits to match live value
    ...
    protected override void WriteGroupMemberUpdate(GSTCPPacketOut pak, bool updateIcons, bool updateMap,
        GameLiving living)
    {
    ...
    public override void SendMarketExplorerWindow(IList<InventoryItem> items, byte page, byte maxpage)
    {
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.MarketExplorerWindow)))
        {
            ...
            foreach (var item in items)
            {
                ...
                pak.WriteShortLowEndian((ushort) (item.PackSize > 1
                ...
                {
                    var bpPrice = "";
                    if (item.SellPrice > 0) bpPrice = "[" + item.SellPrice.ToString() + " BP";
    ...
    public override void SendMerchantWindow(MerchantTradeItems tradeItemsList, eMerchantWindowType windowType)
    {
        if (tradeItemsList != null)
            for (byte page = 0; page < MerchantTradeItems.MAX_PAGES_IN_TRADEWINDOWS; page++)
            {
                ...
                if (itemsInPage == null || itemsInPage.Count == 0) continue;
                ...
                using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.MerchantWindow)))
                {
                    ...
                    {
                        if (!itemsInPage.Contains((int) i)) continue;
    ...
    protected override void WriteHouseFurniture(GSTCPPacketOut pak, IndoorItem item, int index)
    {
        ...
        byte type = 0;
        if (item.Emblem > 0) item.Color = item.Emblem;
        ...
        if (item.Size != 0) type |= 8; // have size
