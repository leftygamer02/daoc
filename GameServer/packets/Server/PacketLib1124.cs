...
public class PacketLib1124 : AbstractPacketLib, IPacketLib
{
    ...
    protected string BuildTaskString()
    {
        ...
        var task = m_gameClient.Player.Task;
        var pMission = m_gameClient.Player.Mission;
        ...
        var name = taskStr + personalMission + groupMission + realmMission;
        ...
        if (name.Length > 2048 - 10) name = name.Substring(0, 2048 - 10 - name.Length);
    ...
    {
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.VisualEffect)))
        {
    ...
    public virtual void SendCharacterOverview(eRealm realm)
    {
        ...
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.CharacterOverview)))
        {
            ...
            {
                var charsBySlot = new Dictionary<int, DOLCharacters>();
                foreach (var c in m_gameClient.Account.Characters)
                    try
                    {
                    ...
                    {
                        log.Error(
                            "SendCharacterOverview - Duplicate char in slot? Slot: " + c.AccountSlot +
                            ", Account: " + c.AccountName, ex);
                    }
                ...
                if (charsBySlot.Any())
                {
                    var filterBySlotPosition = DB.Column("SlotPosition")
                    ...
                    var allItems = DOLDB<InventoryItem>.SelectObjects(DB.Column("OwnerID")
                        .IsIn(charsBySlot.Values.Select(c => c.ObjectId)).And(filterBySlotPosition));
                    ...
                    foreach (var item in allItems)
                        ...
                        {
                            log.Error(
                                "SendCharacterOverview - Duplicate item on character? OwnerID: " + item.OwnerID +
                                ", SlotPosition: " + item.SlotPosition + ", Account: " + m_gameClient.Account.Name,
                                ex);
                        }
                }

                for (var i = firstSlot; i < firstSlot + 10; i++)
                {
                    ...
                    else
                    {
                        ...
                        pak.WriteByte(
                            (byte) c
                                .CustomisationStep); //1 = auto generate config, 2= config ended by player, 3= enable config to player
                        pak.WriteByte((byte) c.MoodType);
                        pak.Fill(0x0, 13); //0 String

                        var locationDescription = string.Empty;
                        var region = WorldMgr.GetRegion((ushort) c.Region);
                        if (region != null)
                            locationDescription =
                                m_gameClient.GetTranslatedSpotDescription(region, c.Xpos, c.Ypos, c.Zpos);
                        ...
                        var racename = m_gameClient.RaceToTranslatedName(c.Race, c.Gender);
                        pak.FillString(racename, 24);

                        pak.WriteByte((byte) c.Level);
                        pak.WriteByte((byte) c.Class);
                        pak.WriteByte((byte) c.Realm);
                        pak.WriteByte((byte) ((((c.Race & 0x10) << 2) + (c.Race & 0x0F)) |
                                              (c.Gender << 4))); // race max value can be 0x1F
                        ...
                        if (rightHandWeapon != null)
                            rightHandColor = (ushort) (rightHandWeapon.Emblem != 0
                                ? rightHandWeapon.Emblem
                                : rightHandWeapon.Color);
                        ...
                        if (helmet != null) helmetColor = (ushort) (helmet.Emblem != 0 ? helmet.Emblem : helmet.Color);

                        pak.WriteShortLowEndian(helmetColor);

                        ushort glovesColor = 0;
                        if (gloves != null) glovesColor = (ushort) (gloves.Emblem != 0 ? gloves.Emblem : gloves.Color);

                        pak.WriteShortLowEndian(glovesColor);

                        ushort bootsColor = 0;
                        if (boots != null) bootsColor = (ushort) (boots.Emblem != 0 ? boots.Emblem : boots.Color);

                        pak.WriteShortLowEndian(bootsColor);

                        ushort leftHandWeaponColor = 0;
                        if (leftHandWeapon != null)
                            leftHandWeaponColor = (ushort) (leftHandWeapon.Emblem != 0
                                ? leftHandWeapon.Emblem
                                : leftHandWeapon.Color);

                        pak.WriteShortLowEndian(leftHandWeaponColor);

                        ushort torsoColor = 0;
                        if (torso != null) torsoColor = (ushort) (torso.Emblem != 0 ? torso.Emblem : torso.Color);

                        pak.WriteShortLowEndian(torsoColor);

                        ushort cloakColor = 0;
                        if (cloak != null) cloakColor = (ushort) (cloak.Emblem != 0 ? cloak.Emblem : cloak.Color);

                        pak.WriteShortLowEndian(cloakColor);

                        ushort legsColor = 0;
                        if (legs != null) legsColor = (ushort) (legs.Emblem != 0 ? legs.Emblem : legs.Color);

                        pak.WriteShortLowEndian(legsColor);

                        ushort armsColor = 0;
                        if (arms != null) armsColor = (ushort) (arms.Emblem != 0 ? arms.Emblem : arms.Color);
    ...
    public virtual void SendCharResistsUpdate()
    {
        ...
        var cap = (int) (m_gameClient?.Player != null ? (m_gameClient.Player.Level >> 1) + 1 : 1);
        for (var i = 0; i < updateResists.Length; i++) caps[i] = cap;


        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.StatsUpdate)))
        {
            // racial resists
            for (var i = 0; i < updateResists.Length; i++)
            {
            ...
            for (var i = 0; i < updateResists.Length; i++)
            {
                var mod = m_gameClient.Player.GetModified((eProperty) updateResists[i]);
                var buff = mod - racial[i] - m_gameClient.Player.AbilityBonus[(int) updateResists[i]] -
                           Math.Min(caps[i], m_gameClient.Player.ItemBonus[(int) updateResists[i]]);
            ...
            // item caps
            for (var i = 0; i < updateResists.Length; i++) pak.WriteByte((byte) caps[i]);
    ...
    public virtual void SendCharStatsUpdate()
    {
        ...
        try
        {
            ...
            var modStats = new int[updateStats.Length];
            var itemCaps = new int[updateStats.Length];

            var itemCap = (int) (m_gameClient.Player.Level * 1.5);
            var bonusCap = (int) (m_gameClient.Player.Level / 2 + 1);
            for (var i = 0; i < updateStats.Length; i++)
            {
                var cap = itemCap;
            ...
            using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.StatsUpdate)))
            {
                ...
                for (var i = 0; i < updateStats.Length; i++)
                {
                    ...
                    var abilityBonus = m_gameClient.Player.AbilityBonus[(int) updateStats[i]];

                    var acuityItemBonus = 0;
                    if (updateStats[i] == m_gameClient.Player.CharacterClass.ManaStat)
                        if (m_gameClient.Player.CharacterClass.ID != (int) eCharacterClass.Scout &&
                            m_gameClient.Player.CharacterClass.ID != (int) eCharacterClass.Hunter &&
                            m_gameClient.Player.CharacterClass.ID != (int) eCharacterClass.Ranger
                    ...
                    var buff = modStats[i] - baseStats[i];
                    buff -= abilityBonus;
                    buff -= Math.Min(itemCaps[i],
                        m_gameClient.Player.ItemBonus[(int) updateStats[i]] + acuityItemBonus);
                ...
                for (var i = 0; i < updateStats.Length; i++)
                {
                    ...
                    pak.WriteShort((ushort) (m_gameClient.Player.ItemBonus[(int) updateStats[i]] +
                                             acuityItemBonus));
                }
                ...
                // item caps
                for (var i = 0; i < updateStats.Length; i++) pak.WriteByte((byte) itemCaps[i]);
                ...
                for (var i = 0; i < updateStats.Length; i++)
                {
                    var acuityItemBonus = 0;
                    if (m_gameClient.Player.CharacterClass.ClassType != eClassType.PureTank &&
                        (int) updateStats[i] == (int) m_gameClient.Player.CharacterClass.ManaStat)
                        if (m_gameClient.Player.CharacterClass.ID != (int) eCharacterClass.Scout &&
                            m_gameClient.Player.CharacterClass.ID != (int) eCharacterClass.Hunter &&
                            m_gameClient.Player.CharacterClass.ID != (int) eCharacterClass.Ranger
                            && m_gameClient.Player.CharacterClass.ID != (int) eCharacterClass.Nightshade)
                    ...
                    pak.WriteByte((byte) (m_gameClient.Player.AbilityBonus[(int) updateStats[i]] +
                                          acuityItemBonus));
                }
    ...
    public virtual void SendCombatAnimation(GameObject attacker, GameObject defender, ushort weaponID,
    ...
    public virtual void SendControlledHorse(GamePlayer player, bool flag)
    {
        if (player == null || player.ObjectState != GameObject.eObjectState.Active)
            return;
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.ControlledHorse)))
        {
            ...
            else
            {
                ...
                {
                    var newGuildBitMask = (player.Guild.Emblem & 0x010000) >> 9;
    ...
    public virtual void SendEnterHouse(House house)
    {
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.HouseEnter)))
        {
            ...
            pak.WriteByte(0x00);
            pak.WriteByte((byte) (house.GetGuildEmblemFlags() |
                                  ((house.Emblem & 0x010000) >> 14))); //new Guild Emblem
    ...
    public virtual void SendGarden(House house)
    {
        ...
        // Update cache
        m_gameClient.HouseUpdateArray.UpdateIfExists(
            new Tuple<ushort, ushort>(house.RegionID, (ushort) house.HouseNumber), GameTimer.GetTickCount());
    }
    ...
    public virtual void SendGroupWindowUpdate()
    {
        ...
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.VariousUpdate)))
        {
            ...
            if (group != null)
                ...
                {
                    var sameRegion = living.CurrentRegion == m_gameClient.Player.CurrentRegion;
                    ...
                    if (sameRegion)
                    {
                        ...
                        if (SpellHelper.FindEffectOnTarget(living, "DamageOverTime") != null)
                            playerStatus |= 0x08;
                        if (living is GamePlayer && ((GamePlayer) living).Client.ClientState ==
                            GameClient.eClientState.Linkdead)
                            playerStatus |= 0x10;
                    ...
                    pak.WritePascalString(living is GamePlayer
                        ? ((GamePlayer) living).CharacterClass.Name
                        : "NPC"); //classname
                }
    ...
    {
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.KeepComponentHookpointStore)))
        {
            ...
            foreach (HookPointItem item in inventory.GetAllItems())
            {
                ...
                pak.WriteByte((byte) i);
                i++;
                if (item.GameObjectType ==
                    "GameKeepGuard") //TODO: hack wrong must think how design thing to have merchante of gameobject(living or item)
                    pak.WriteShort(0);
                else
    ...
    public virtual void SendHouse(House house)
    {
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.HouseCreate)))
        {
            ...
            var flagPorchAndGuildEmblem = (house.Emblem & 0x010000) >> 13; //new Guild Emblem
            if (house.Porch)
        ...
        // Update cache
        m_gameClient.HouseUpdateArray[new Tuple<ushort, ushort>(house.RegionID, (ushort) house.HouseNumber)] =
    ...
    public virtual void SendInventoryItemsUpdate(IDictionary<int, InventoryItem> updateItems,
        eInventoryWindowType windowType)
    {
        if (m_gameClient.Player == null)
            return;
        if (updateItems == null)
    ...
    /// <param name="windowType"></param>
    protected virtual void SendInventoryItemsPartialUpdate(IDictionary<int, InventoryItem> items,
        eInventoryWindowType windowType)
    {
        ...
        {
            var houseVault = m_gameClient.Player.ActiveInventoryObject as GameVault;
            pak.WriteByte((byte) items.Count);
            pak.WriteByte(0x00); // new in 189b+, show shield in left hand
            pak.WriteByte((byte) ((m_gameClient.Player.IsCloakInvisible ? 0x01 : 0x00) |
                                  (m_gameClient.Player.IsHelmInvisible
                                      ? 0x02
                                      : 0x00))); // new in 189b+, cloack/helm visibility
            if (windowType == eInventoryWindowType.HouseVault && houseVault != null)
                ...
                pak.WriteByte((byte) ((m_gameClient.Player.IsCloakHoodUp ? 0x01 : 0x00) |
                                      (int) m_gameClient.Player.rangeAttackComponent
                                          .ActiveQuiverSlot)); //bit0 is hood up bit4 to 7 is active quiver
    ...
    protected virtual void SendInventorySlotsUpdateRange(ICollection<int> slots, eInventoryWindowType windowType)
    {
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.InventoryUpdate)))
        {
            ...
            if (slots != null)
                foreach (var updatedSlot in slots)
                {
                    if (updatedSlot >= (int) eInventorySlot.Consignment_First &&
                        updatedSlot <= (int) eInventorySlot.Consignment_Last)
                    {
                        log.Error(
                            "PacketLib198:SendInventorySlotsUpdateBase - GameConsignmentMerchant inventory is no longer cached with player.  Use a Dictionary<int, InventoryItem> instead.");
                        pak.WriteByte((byte) (updatedSlot - (int) eInventorySlot.Consignment_First +
    ...
    public virtual void SendKeepClaim(IGameKeep keep, byte flag)
    {
        ...
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.KeepClaim)))
        {
    ...
    public virtual void SendKeepInfo(IGameKeep keep)
    {
        ...
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.KeepInfo)))
        {
            pak.WriteShort((ushort) keep.KeepID);
            pak.WriteShort(0);
    ...
    {
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.KeepRealmUpdate)))
        {
            pak.WriteShort((ushort) keep.KeepID);
            pak.WriteByte((byte) keep.Realm);
            pak.WriteByte((byte) keep.Level);
    ...
    {
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.KeepComponentHookpointUpdate)))
        {
    ...
    {
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.KeepComponentInfo)))
        {
    ...
    {
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.KeepComponentDetailUpdate)))
        {
            pak.WriteShort((ushort) keepComponent.Keep.KeepID);
            pak.WriteShort((ushort) keepComponent.ID);
            pak.WriteByte((byte) keepComponent.Height);
    ...
    {
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.KeepComponentRemove)))
        {
            pak.WriteShort((ushort) keepComponent.Keep.KeepID);
            pak.WriteShort((ushort) keepComponent.ID);
            SendTCP(pak);
    ...
    {
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.KeepComponentUpdate)))
        {
            pak.WriteShort((ushort) keep.KeepID);
            pak.WriteByte((byte) keep.Realm);
            pak.WriteByte((byte) keep.Level);
            pak.WriteByte((byte) keep.SentKeepComponents.Count);
            foreach (var component in keep.SentKeepComponents)
            {
                var m_flag = (byte) component.Height;
    ...
    {
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.KeepComponentInteractResponse)))
        {
            pak.WriteShort((ushort) component.Keep.KeepID);
            ...
            if (component.Keep.Guild != null) pak.WriteString(component.Keep.Guild.Name);
    ...
    public virtual void SendKeepComponentHookPoint(IGameKeepComponent component, int selectedHookPointIndex)
    {
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.KeepComponentHookpointUpdate)))
        {
            pak.WriteShort((ushort) component.Keep.KeepID);
            pak.WriteShort((ushort) component.ID);
            var freeHookpoints = new ArrayList();
            foreach (var hookPt in component.HookPoints.Values)
                if (hookPt.IsFree)
    ...
    {
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.KeepRemove)))
        {
            pak.WriteShort((ushort) keep.KeepID);
            SendTCP(pak);
        }
    ...
    public virtual void SendLivingEquipmentUpdate(GameLiving living)
    {
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.EquipmentUpdate)))
        {
            ...
            pak.WriteByte((byte) ((living.IsCloakInvisible ? 0x01 : 0x00) |
                                  (living.IsHelmInvisible ? 0x02 : 0x00))); // new in 189b+, cloack/helm visibility
            pak.WriteByte((byte) ((living.IsCloakHoodUp ? 0x01 : 0x00) |
                                  (int) living.rangeAttackComponent
                                      .ActiveQuiverSlot)); //bit0 is hood up bit4 to 7 is active quiver
            ...
            if (items != null)
            {
                ...
                foreach (var item in items)
                {
                    ...
                    var slot = item.SlotPosition;
                    //model = GetModifiedModel(model);
                    var texture = item.Emblem != 0 ? item.Emblem : item.Color;
                    if (item.SlotPosition == Slot.LEFTHAND ||
                        item.SlotPosition == Slot.CLOAK) // for test only cloack and shield
    ...
    {
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.ObjectDataUpdate)))
        {
            pak.WriteShort((ushort) living.ObjectID);
            pak.WriteByte(0);
            pak.WriteByte(living.Level);
    ...
    public virtual void SendLoginGranted()
    {
        ...
        if (ServerProperties.Properties.ALLOW_ALL_REALMS ||
            m_gameClient.Account.PrivLevel > (int) ePrivLevel.Player)
            SendLoginGranted(1);
        else
    ...
    public virtual void SendMasterLevelWindow(byte ml)
    {
        ...
        var mlToSend =
            (byte) (ml == 0 ? m_gameClient.Player.MLLevel == 0 ? 1 : m_gameClient.Player.MLLevel : ml);
        ...
        if (m_gameClient.Player.MLLevel < 10)
        {
            ...
            if (m_gameClient.Player.GetStepCountForML((byte) (m_gameClient.Player.MLLevel + 1)) > 0)
                mlStepPercent = 100.0 *
                                m_gameClient.Player.GetCountMLStepsCompleted(
                                    (byte) (m_gameClient.Player.MLLevel + 1)) /
                                m_gameClient.Player.GetStepCountForML((byte) (m_gameClient.Player.MLLevel + 1));
        }
        else
        {
        ...
        using (var pak = new GSTCPPacketOut((byte) eServerPackets.MasterLevelWindow))
        {
            ...
            for (var i = 1; i < 11; i++)
            {
                var description = m_gameClient.Player.GetMLStepDescription(mlToSend, i);
    ...
    {
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.MinotaurRelicMapRemove)))
        {
    ...
    {
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.MinotaurRelicMapUpdate)))
        {
            pak.WriteIntLowEndian((uint) id);
    ...
    public virtual void SendNonHybridSpellLines()
    {
        ...
        // Footer packet
        using (var pak3 = new GSTCPPacketOut(GetPacketCode(eServerPackets.VariousUpdate)))
        {
    ...
    public virtual void SendNPCCreate(GameNPC npc)
    {
        ...
        if (npc is GameMovingObject)
            ...
            using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.NPCCreate)))
            {
                ...
                var level = npc.GetDisplayLevel(m_gameClient.Player);
                if ((npc.Flags & GameNPC.eFlags.STATUE) != 0) level |= 0x80;
                ...
                var add = "";
                byte flags2 = 0x00;
                var brain = npc.Brain as IControlledBrain;

                if (brain != null) flags2 |= 0x80; // have Owner
                ...
                var questIndicator = npc.GetQuestIndicator(m_gameClient.Player);
                ...
                var name = npc.Name;
                var guildName = npc.GuildName;

                var translation = LanguageMgr.GetTranslation(m_gameClient, npc);
                if (translation != null)
                {
    ...
    public virtual void SendPetFakeFriendlyGuildID(GameNPC pet)
    {
        ...
        var playerOwner = npcBrain.GetPlayerOwner();
        ...
        if (playerOwner != m_gameClient.Player)
            if (playerOwner.Guild == null || m_gameClient.Player.Guild == null ||
                playerOwner.Guild != m_gameClient.Player.Guild)
                if (playerOwner.Group == null ||
                    !playerOwner.Group.GetMembersInTheGroup().Contains(m_gameClient.Player))
                    return;
    ...
    private static Guild m_dummyGuild = GuildMgr.CreateGuild(0, "DummyGuildToMakePetsUntargetable") ??
                                        GuildMgr.GetGuildByName("DummyGuildToMakePetsUntargetable");
    ...
    public virtual void SendObjectCreate(GameObject obj)
    {
        ...
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.ObjectCreate)))
        {
            ...
            if (obj is GameStaticItemTimed && m_gameClient.Player != null &&
                ((GameStaticItemTimed) obj).IsOwner(m_gameClient.Player))
                flag |= 0x04;
            pak.WriteShort((ushort) flag);
            if (obj is GameStaticItem)
            {
                var newEmblemBitMask = ((obj as GameStaticItem).Emblem & 0x010000) << 9;
        ...
        // Update Object Cache
        m_gameClient.GameObjectUpdateArray[new Tuple<ushort, ushort>(obj.CurrentRegionID, (ushort) obj.ObjectID)] =
            GameTimer.GetTickCount();
    }
    ...
    {
        // Remove from cache
        if (m_gameClient.GameObjectUpdateArray.ContainsKey(new Tuple<ushort, ushort>(obj.CurrentRegionID,
                (ushort) obj.ObjectID)))
        {
            long dummy;
            m_gameClient.GameObjectUpdateArray.TryRemove(
                new Tuple<ushort, ushort>(obj.CurrentRegionID, (ushort) obj.ObjectID), out dummy);
        }
    ...
    public virtual void SendObjectUpdate(GameObject obj)
    {
        ...
        if (obj is GameNPC)
        {
            ...
            if ((npc.Flags & GameNPC.eFlags.STATUE) != 0) flags |= 0x01;

            if (npc.IsUnderwater) flags |= 0x10;
            ...
            if (npc.IsMoving && !npc.IsAtTargetPosition)
            {
                ...
                {
                    var tz = npc.CurrentRegion.GetZone(npc.TargetPosition.X, npc.TargetPosition.Y);
                    if (tz != null)
                    {
            ...
            var target = npc.TargetObject;
            if (npc.attackComponent.AttackState && target != null &&
                target.ObjectState == GameObject.eObjectState.Active && !npc.IsTurningDisabled)
                targetOID = (ushort) target.ObjectID;
        }

        using (var pak = new GSUDPPacketOut(GetPacketCode(eServerPackets.ObjectUpdate)))
        {
            pak.WriteShort((ushort) speed);
        ...
        if (obj is GameNPC) (obj as GameNPC).NPCUpdatedCallback();
    }
        ...
        eWalkState walkState)
    {
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.PetWindow)))
        {
            ...
            pak.WriteByte(0x00); //unused
            switch
                (windowAction) //0-released, 1-normal, 2-just charmed? | Roach: 0-close window, 1-update window, 2-create window
            {
            ...
            if (pet != null)
                ...
                lock (pet.effectListComponent.EffectsLock)
                {
                    ...
                    pak.WriteByte((byte) icons.Count); // effect count
                    // 0x08 - null terminated - (byte) list of shorts - spell icons on pet
                    foreach (ushort icon in icons) pak.WriteShort(icon);
                }
            else
    ...
    public virtual void SendPlayerCreate(GamePlayer playerToCreate)
    {
        ...
        var playerRegion = playerToCreate.CurrentRegion;
        ...
        var playerZone = playerToCreate.CurrentZone;
        if (playerZone == null)
        {
            if (log.IsWarnEnabled)
        ...
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.PlayerCreate172)))
        {
            ...
            var flags = (GameServer.ServerRules.GetLivingRealm(m_gameClient.Player, playerToCreate) & 0x03) << 2;
            ...
            pak.WriteByte(
                playerToCreate.GetFaceAttribute(eCharFacePart.EyeSize)); //1-4 = Eye Size / 5-8 = Nose Size
            pak.WriteByte(playerToCreate.GetFaceAttribute(eCharFacePart.LipSize)); //1-4 = Ear size / 5-8 = Kin size
            pak.WriteByte(
                playerToCreate.GetFaceAttribute(eCharFacePart.MoodType)); //1-4 = Ear size / 5-8 = Kin size
            pak.WriteByte(
                playerToCreate.GetFaceAttribute(eCharFacePart
                    .EyeColor)); //1-4 = Skin Color / 5-8 = Eye Color                
            pak.WriteByte(
                playerToCreate.GetFaceAttribute(eCharFacePart.HairColor)); //Hair: 1-4 = Color / 5-8 = unknown
            pak.WriteByte(
                playerToCreate.GetFaceAttribute(eCharFacePart.FaceType)); //1-4 = Unknown / 5-8 = Face type
            pak.WriteByte(
                playerToCreate.GetFaceAttribute(eCharFacePart.HairStyle)); //1-4 = Unknown / 5-8 = Hair Style
            ...
            pak.WritePascalString(
                GameServer.ServerRules.GetPlayerTitle(m_gameClient.Player,
                    playerToCreate)); // new in 1.74, NewTitle
            if (playerToCreate.IsOnHorse)
            {
                pak.WriteByte(playerToCreate.ActiveHorse.ID);
                if (playerToCreate.ActiveHorse.BardingColor == 0 && playerToCreate.ActiveHorse.Barding != 0 &&
                    playerToCreate.Guild != null)
                {
                    var newGuildBitMask = (playerToCreate.Guild.Emblem & 0x010000) >> 9;
        ...
        m_gameClient.GameObjectUpdateArray[
                new Tuple<ushort, ushort>(playerToCreate.CurrentRegionID, (ushort) playerToCreate.ObjectID)] =
            GameTimer.GetTickCount();

        SendObjectGuildID(playerToCreate,
            playerToCreate.Guild); //used for nearest friendly/enemy object buttons and name colors on PvP server

        if (playerToCreate.GuildBanner != null) SendRvRGuildBanner(playerToCreate, true);
    }
    ...
    public virtual void SendPlayerForgedPosition(GamePlayer player)
    {
        ...
        {
            var heading = 4096 + player.Heading;
            ...
            {
                var rSpeed = player.CurrentSpeed;
            ...
            if (player.IsDiving) flagcontent += 0x04;

            if (player.IsWireframe) flagcontent += 0x01;

            if (player.IsStealthed) flagcontent += 0x02;

            if (player.IsTorchLighted) flagcontent += 0x80;
    ...
    public virtual void SendPlayerFreeLevelUpdate()
    {
        ...
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.VisualEffect)))
        {
            ...
            var flag = player.FreeLevelState;

            var t = new TimeSpan((long) (DateTime.Now.Ticks - player.LastFreeLeveled.Ticks));
            ...
            {
                case eRealm.Albion:
                    time = (ushort) (ServerProperties.Properties.FREELEVEL_DAYS_ALBION * 24 * 60 -
                                     t.TotalMinutes);
                    break;
                case eRealm.Midgard:
                    time = (ushort) (ServerProperties.Properties.FREELEVEL_DAYS_MIDGARD * 24 * 60 -
                                     t.TotalMinutes);
                    break;
                case eRealm.Hibernia:
                    time = (ushort) (ServerProperties.Properties.FREELEVEL_DAYS_HIBERNIA * 24 * 60 -
                                     t.TotalMinutes);
                    break;
            }
    ...
    public virtual void SendPlayerPositionAndObjectID()
    {
        ...
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.PositionAndObjectID)))
        {
            ...
            var flags = 0;
            var zone = m_gameClient.Player.CurrentZone;
            if (zone == null) return;
    ...
    public virtual void SendPlayerTitles()
    {
        ...
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.DetailWindow)))
        {
            ...
            byte line = 1;
            foreach (var str in m_gameClient.Player.FormatStatistics())
            {
                pak.WriteByte(line++);
                pak.WritePascalString(str);
            }
            ...
            pak.WriteByte(200);
            var titlesCountPos = pak.Position;
            ...
            var titlesLen = pak.Position - titlesCountPos - 1; // include titles count
    ...
    public virtual void SendPlayerTitleUpdate(GamePlayer player)
    {
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.VisualEffect)))
        {
            ...
            else
            {
                ...
                var val = GameServer.ServerRules.GetPlayerTitle(m_gameClient.Player, player);
    ...
    public virtual void SendQuestListUpdate()
    {
        ...
        lock (m_gameClient.Player.QuestList)
        {
            foreach (var quest in m_gameClient.Player.QuestList.ToList())
                SendQuestPacket(quest.Step == 0 || quest.Step == -1 || quest.Step == -2 ? null : quest,
                    questIndex++);
        }
    }
    ...
    protected virtual void SendQuestWindow(GameNPC questNPC, GamePlayer player, DataQuest quest, bool offer)
    {
        ...
        {
            var QuestID = quest.ClientQuestID;
            ...
            var personalizedSummary = BehaviourUtils.GetPersonalizedMessage(quest.Description, player);
            if (personalizedSummary.Length > 255)
                pak.WritePascalString(personalizedSummary.Substring(0,
                    255)); // Summary is max 255 bytes or client will crash !
            else
                pak.WritePascalString(personalizedSummary);

            if (offer)
            {
                var personalizedStory = BehaviourUtils.GetPersonalizedMessage(quest.Story, player);
            ...
            foreach (var reward in quest.FinalRewards) WriteItemData(pak, GameInventoryItem.Create(reward));

            pak.WriteByte((byte) quest.NumOptionalRewardsChoice);
            pak.WriteByte((byte) quest.OptionalRewards.Count);
            foreach (var reward in quest.OptionalRewards) WriteItemData(pak, GameInventoryItem.Create(reward));
    ...
    protected virtual void SendQuestWindow(GameNPC questNPC, GamePlayer player, RewardQuest quest, bool offer)
    {
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.Dialog)))
        {
            var QuestID = QuestMgr.GetIDForQuestType(quest.GetType());
            pak.WriteShort(offer ? (byte) 0x22 : (byte) 0x21); // Dialog
            pak.WriteShort(QuestID);
            pak.WriteShort((ushort) questNPC.ObjectID);
            pak.WriteByte(0x00); // unknown
            ...
            var personalizedSummary = BehaviourUtils.GetPersonalizedMessage(quest.Summary, player);
            if (personalizedSummary.Length > 255)
            ...
            if (offer)
            {
                ...
                if (personalizedStory.Length > ServerProperties.Properties.MAX_REWARDQUEST_DESCRIPTION_LENGTH)
                {
                    ...
                    pak.WriteStringBytes(personalizedStory.Substring(0,
                        ServerProperties.Properties.MAX_REWARDQUEST_DESCRIPTION_LENGTH));
                }
                else
                {
                    pak.WriteShort((ushort) personalizedStory.Length);
            ...
            {
                if (quest.Conclusion.Length >
                    (ushort) ServerProperties.Properties.MAX_REWARDQUEST_DESCRIPTION_LENGTH)
                {
                    pak.WriteShort((ushort) ServerProperties.Properties.MAX_REWARDQUEST_DESCRIPTION_LENGTH);
                    pak.WriteStringBytes(quest.Conclusion.Substring(0,
                        (ushort) ServerProperties.Properties.MAX_REWARDQUEST_DESCRIPTION_LENGTH));
                }
                else
                {
            ...
            foreach (var goal in quest.Goals) pak.WritePascalString(string.Format("{0}\r", goal.Description));
            ...
            foreach (var reward in quest.Rewards.BasicItems) WriteItemData(pak, GameInventoryItem.Create(reward));

            pak.WriteByte((byte) quest.Rewards.ChoiceOf);
            pak.WriteByte((byte) quest.Rewards.OptionalItems.Count);
            foreach (var reward in quest.Rewards.OptionalItems) WriteItemData(pak, GameInventoryItem.Create(reward));
    ...
    protected virtual void SendQuestPacket(AbstractQuest q, int index)
    {
        ...
        else if (q is RewardQuest)
        {
            ...
            using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.QuestEntry)))
            {
                ...
                {
                    goalindex++;
                    var goalDesc = string.Format("{0}\r", goal.Description);
        ...
        else
        {
            using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.QuestEntry)))
            {
                ...
                var name = string.Format("{0} (Level {1})", q.Name, q.Level);
                var desc = string.Format("[Step #{0}]: {1}", q.Step, q.Description);
                if (name.Length > byte.MaxValue)
                {
                    if (log.IsWarnEnabled)
                        log.Warn(q.GetType().ToString() + ": name is too long for 1.68+ clients (" + name.Length +
                                 ") '" + name + "'");
                ...
                {
                    if (log.IsWarnEnabled)
                        log.Warn(q.GetType().ToString() + ": description is too long for 1.68+ clients (" +
                                 desc.Length + ") '" + desc + "'");
    ...
    public virtual void SendRegions(ushort regionId)
    {
        ...
        if (region == null)
            return;
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.ClientRegion)))
        {
            //				pak.WriteByte((byte)((region.Expansion + 1) << 4)); // Must be expansion
            pak.WriteByte(
                0); // but this packet sended when client in old region. but this field must show expanstion for jump destanation region
            ...
            if (ip == "any" || ip == "0.0.0.0" || ip == "127.0.0.1" || ip.StartsWith("10.") ||
    ...
    public virtual void SendRvRGuildBanner(GamePlayer player, bool show)
    {
        ...
        if (show && player.Guild == null)
            return;
        var pak = new GSTCPPacketOut((byte) eServerPackets.VisualEffect);
        pak.WriteShort((ushort) player.ObjectID);
        pak.WriteByte(0xC); // show Banner
        pak.WriteByte((byte) (show ? 0 : 1)); // 0-enable, 1-disable
        var newEmblemBitMask = ((player.Guild.Emblem & 0x010000) << 8) | (player.Guild.Emblem & 0xFFFF);
    ...
    public virtual void SendSiegeWeaponAnimation(GameSiegeWeapon siegeWeapon)
    {
        if (siegeWeapon == null)
            return;
        var siegeID = new byte[siegeWeapon.ObjectID]; // test
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.SiegeWeaponAnimation)))
        {
            ...
            switch ((byte) siegeWeapon.SiegeWeaponTimer.CurrentAction)
            {
                case 0x01: //aiming
                {
                    pak.WriteByte((byte) (siegeWeapon.TargetObject == null
                        ? 0
                        : siegeWeapon.TargetObject.HealthPercent)); //Send target health percent
                    //pak.WriteByte(siegeID[1]); // lowest value of siegeweapon.ObjectID
                    pak.WriteShort((ushort) (siegeWeapon.TargetObject == null
                        ? 0x0000
                        : siegeWeapon.TargetObject.ObjectID));
                    break;
                }
                case 0x02: //arming
                {
    ...
    public virtual void SendSiegeWeaponFireAnimation(GameSiegeWeapon siegeWeapon, int timer)
    {
        ...
        {
            pak.WriteInt((uint) siegeWeapon.ObjectID);
            pak.WriteInt((uint) (siegeWeapon.TargetObject == null
                ? siegeWeapon.GroundTarget.X
                : siegeWeapon.TargetObject.X));
            pak.WriteInt((uint) (siegeWeapon.TargetObject == null
                ? siegeWeapon.GroundTarget.Y
                : siegeWeapon.TargetObject.Y));
            pak.WriteInt((uint) (siegeWeapon.TargetObject == null
                ? siegeWeapon.GroundTarget.Z + 50
                : siegeWeapon.TargetObject.Z + 50));
            pak.WriteInt((uint) (siegeWeapon.TargetObject == null ? 0 : siegeWeapon.TargetObject.ObjectID));
            pak.WriteShort(siegeWeapon.Effect);
    ...
    public virtual void SendSiegeWeaponInterface(GameSiegeWeapon siegeWeapon, int time)
    {
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.SiegeWeaponInterface)))
        {
            ...
            var name = siegeWeapon.Name;

            var translation = LanguageMgr.GetTranslation(m_gameClient, siegeWeapon);
            if (translation != null)
                if (!Util.IsEmpty(((DBLanguageNPC) translation).Name))
                    name = ((DBLanguageNPC) translation).Name;
    ...
    public virtual void SendTradeWindow()
    {
        ...
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.TradeWindow)))
        {
            lock (m_gameClient.Player.TradeWindow.Sync)
            {
                ...
                pak.WriteShort(0x0000);
                var items = m_gameClient.Player.TradeWindow.PartnerTradeItems;
                if (items != null)
                {
                    pak.WriteByte((byte) items.Count);
                    pak.WriteByte(0x01);
                ...
                if (m_gameClient.Player.TradeWindow.Partner != null)
                    pak.WritePascalString("Trading with " +
                                          m_gameClient.Player.GetName(m_gameClient.Player.TradeWindow
                                              .Partner)); // transaction with ...
                else
    ...
    public virtual void SendTrainerWindow()
    {
        ...
        var specs = m_gameClient.Player.GetSpecList().Where(it => it.Trainable).ToList();
        var autotrains = player.CharacterClass.GetAutotrainableSkills();
        ...
        var raList = SkillBase.GetClassRealmAbilities(m_gameClient.Player.CharacterClass.ID)
        ...
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.TrainerWindow)))
        {
            ...
            {
                var level = player.GetAbilityLevel(ra.KeyName);
                pak.WriteByte((byte) i++);
                pak.WriteByte((byte) level);
                pak.WriteByte((byte) ra.CostForUpgrade(level));
                var canBeUsed = ra.CheckRequirement(player);
        ...
        if (m_gameClient.TrainerSkillCache == null)
        {
            ...
            foreach (var spec in specs)
            {
                ...
                foreach (var ab in spec.PretendAbilitiesForLiving(player, player.MaxLevel))
                    toAdd.Add(new Tuple<int, int, Skill>(5, ab.InternalID, ab));

                foreach (var ls in spec
                             .PretendLinesSpellsForLiving(player, player.MaxLevel).Where(k => !k.Key.IsBaseLine))
                foreach (var sk in ls.Value)
                    toAdd.Add(new Tuple<int, int, Skill>((int) sk.SkillType, sk.InternalID, sk));

                foreach (var st in spec.PretendStylesForLiving(player, player.MaxLevel))
                    toAdd.Add(new Tuple<int, int, Skill>((int) st.SkillType, st.InternalID, st));

                skillDictCache.Add(new Tuple<Specialization, List<Tuple<int, int, Skill>>>(spec,
                    toAdd.OrderBy(e =>
                            e.Item3 is Ability
                                ? ((Ability) e.Item3).SpecLevelRequirement
                                : e.Item3 is Style
                                    ? ((Style) e.Item3).SpecLevelRequirement
                                    : e.Item3.Level)
                        .ToList()));
            }
        ...
        // Send Names first
        var index = 0;
        for (var skindex = 0; skindex < skillDictCache.Count; skindex++)
            using (var pakindex = new GSTCPPacketOut(GetPacketCode(eServerPackets.TrainerWindow)))
            {
                ...
                foreach (var sk in skillDictCache[skindex].Item2.Select(e => e.Item3))
                {
                    // send name
        ...
        // Send Skill Secondly
        using (var pakskill = new GSTCPPacketOut(GetPacketCode(eServerPackets.TrainerWindow)))
        {
            ...
            for (var skindex = 0; skindex < skillDictCache.Count; skindex++)
            {
                ...
                foreach (var sk in skillDictCache[skindex].Item2)
                {
                ...
                if (m_gameClient.CanSendTooltip(24, spell.InternalID))
                {
                    ...
                    if (spell.HasSubSpell)
                        if (m_gameClient.CanSendTooltip(24, SkillBase.GetSpellByID(spell.SubSpellID).InternalID))
                            SendDelveInfo(DetailDisplayHandler.DelveSpell(m_gameClient,
                                SkillBase.GetSpellByID(spell.SubSpellID)));

                    if (spell.SpellType == (byte) eSpellType.DefensiveProc ||
                        spell.SpellType == (byte) eSpellType.OffensiveProc)
                        SendDelveInfo(DetailDisplayHandler.DelveSpell(m_gameClient,
                            SkillBase.GetSpellByID((int) spell.Value)));
                }
            }
        }
    }
    ...
    public virtual void SendUpdateIcons(IList changedEffects, ref int lastUpdateEffectsCount)
    {
        ...
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.UpdateIcons)))
        {
            ...
            foreach (var effect in m_gameClient.Player.effectListComponent.GetAllEffects()
                         .Where(e => e.EffectType != eEffect.Pulse))
                if (effect.Icon != 0)
                {
                    fxcount++;
                    if (changedEffects != null && !changedEffects.Contains(effect)) continue;
                    ...
                    pak.WriteByte(effect is ECSGameAbilityEffect && effect.Icon <= 5000
                        ? (byte) 0xff
                        : (byte) (fxcount - 1));
                    ...
                    if (!effect.HasPositiveEffect) flagNegativeEffect = 1;
            ...
            var oldCount = lastUpdateEffectsCount;
            ...
            if (changedEffects != null) changedEffects.Clear();

            if (entriesCount == 0) return; // nothing changed - no update is needed
    ...
    public virtual void SendUpdatePlayer()
    {
        ...
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.VariousUpdate)))
        {
            ...
            var skill = CraftingMgr.getSkillbyEnum(player.CraftingPrimarySkill);
            if (skill != null)
    ...
    public virtual void SendUpdatePlayerSkills()
    {
        ...
        var usableSkills = m_gameClient.Player.GetAllUsableSkills(true);

        var sent = false; // set to true once we can't send packet anymore !
        var index = 0; // index of our position in the list !
        var total = usableSkills.Count; // cache List count.
        var packetCount = 0; // Number of packet sent for the entire list
        while (!sent)
        {
            var packetEntry = 0; // needed to tell client how much skill we send
            // using pak
            using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.VariousUpdate)))
            {
                // Write header
                ...
                while (index < total)
                {
                    ...
                    if (index >= byte.MaxValue ||
                        pak.Length + 8 + usableSkills[index].Item1.Name.Length > 1400)
                        break;

                    // Enter Packet Values !! Format Level - Type - SpecialField - Bonus - Icon - Name
                    var skill = usableSkills[index].Item1;
                    var skillrelated = usableSkills[index].Item2;

                    if (skill is Specialization)
                    {
                        var spec = (Specialization) skill;
                        pak.WriteByte((byte) spec.Level);
                        pak.WriteShort((ushort) spec.InternalID); //new 1.112
                        pak.WriteByte((byte) spec.SkillType);
                        pak.WriteShort(0);
                        pak.WriteByte((byte) (m_gameClient.Player.GetModifiedSpecLevel(spec.KeyName) -
                                              spec.Level)); // bonus
                    ...
                    else if (skill is Spell)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                spin = usableSkills.FindIndex(sk =>
                                    sk.Item1 is Specialization && ((Specialization) sk.Item1).KeyName ==
                                    ((SpellLine) skillrelated).Spec);
                    ...
                    else if (skill is Style)
                    {
                        ...
                        switch (style.OpeningRequirementType)
                        {
                            case Style.eOpening.Offensive:
                                pre = (int) style
                                    .AttackResultRequirement; // last result of our attack against enemy hit, miss, target blocked, target parried, ...
                                if (style.AttackResultRequirement == Style.eAttackResultRequirement.Style)
                                {
                                    ...
                                    var styleindex = Math.Max(0,
                                        usableSkills.FindIndex(it =>
                                            it.Item1 is Style && it.Item1.ID == style.OpeningRequirementValue));
                                    var speccount = Math.Max(0,
                                        usableSkills.FindIndex(it => it.Item1 is Specialization == false));
                            ...
                            case Style.eOpening.Defensive:
                                pre = 100 + (int) style
                                    .AttackResultRequirement; // last result of enemies attack against us hit, miss, you block, you parry, ...
                                break;
                        ...
                        pak.WriteByte(
                            GlobalConstants.GetSpecToInternalIndex(style.Spec)); // index specialization in bonus...
    ...
    public virtual void SendUDPInitReply()
    {
        using (var pak = new GSUDPPacketOut(GetPacketCode(eServerPackets.UDPInitReply)))
        {
            ...
            if (!m_gameClient.Socket
                    .Connected) // || !_gameClient.UsingRC4) // not using RC4, wont accept UDP packets anyway.)
                return;
            ...
            else
            {
                ...
                var ip = playerRegion.ServerIP;
                if (ip == "any" || ip == "0.0.0.0" || ip == "127.0.0.1" || ip.StartsWith("10.13.") ||
                    ip.StartsWith("192.168."))
                    ip = ((IPEndPoint) m_gameClient.Socket.LocalEndPoint).Address.ToString();
    ...
    public virtual void SendWarlockChamberEffect(GamePlayer player)
    {
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.VisualEffect)))
        {
            ...
            {
                foreach (var fx in player.EffectList)
                    if (fx is GameSpellEffect)
                    {
                        var effect = (GameSpellEffect) fx;
                        if (effect.SpellHandler.Spell != null &&
                            effect.SpellHandler.Spell.SpellType == (byte) eSpellType.Chamber)
                        {
    ...
    public virtual void SendWarmapBonuses()
    {
        ...
        var HibKeeps = 0;
        var OwnerDFTowers = 0;
        var OwnerDF = eRealm.None;
        foreach (var keep in GameServer.KeepManager.GetFrontierKeeps())
        ...
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.WarmapBonuses)))
        {
            var RealmKeeps = 0;
            var RealmTowers = 0;
            ...
            pak.WriteByte(
                (byte) (((byte) RelicMgr.GetRelicCount(m_gameClient.Player.Realm, eRelicType.Magic) << 4) |
                        (byte) RelicMgr.GetRelicCount(m_gameClient.Player.Realm, eRelicType.Strength)));
    ...
    public virtual void SendWarmapUpdate(ICollection<IGameKeep> list)
    {
        ...
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.WarMapClaimedKeeps)))
        {
            var KeepCount = 0;
            var TowerCount = 0;
            ...
            foreach (GameRelic relic in RelicMgr.getNFRelics())
                ...
                {
                    case eRealm.Albion:
                        if (relic.RelicType == eRelicType.Strength) albStr = (byte) relic.Realm;

                        if (relic.RelicType == eRelicType.Magic) albMagic = (byte) relic.Realm;

                        break;
                    case eRealm.Hibernia:
                        if (relic.RelicType == eRelicType.Strength) hibStr = (byte) relic.Realm;

                        if (relic.RelicType == eRelicType.Magic) hibMagic = (byte) relic.Realm;

                        break;
                    case eRealm.Midgard:
                        if (relic.RelicType == eRelicType.Strength) midStr = (byte) relic.Realm;

                        if (relic.RelicType == eRelicType.Magic) midMagic = (byte) relic.Realm;
            ...
            foreach (AbstractGameKeep keep in list)
            {
                ...
                var id = keepId & 0xFF;
                var tower = keep.KeepID >> 8;
                var map = id / 25 - 1;

                var index = id - (map * 25 + 25);
                ...
                {
                    if (GameServer.KeepManager.FrontierRegionsList.Contains(m_gameClient.Player.CurrentRegionID) &&
                        m_gameClient.Player.Realm == keep.Realm)
                    {
                        var theKeep = keep as GameKeep;
                        if (theKeep != null)
                ...
                if (keep.InCombat) flag |= (byte) eRealmWarmapKeepFlags.UnderSiege;
    ...
    {
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.WarMapDetailUpdate)))
        {
            ...
            foreach (var obj in fights)
            {
                ...
                pak.WriteByte(
                    obj[3]); // color - ( Fights:  0x00 - Grey , 0x01 - RedBlue , 0x02 - RedGreen , 0x03 - GreenBlue )
                pak.WriteByte(
                    obj[4]); // type  - ( Fights:  Size 0x00 - small  0x01 - medium  0x02 - big 0x03 - huge )
            }
            ...
            foreach (var obj in groups)
            {
                ...
                pak.WriteByte((byte) (realm == 3 ? 0x04 :
                    realm == 2 ? 0x02 : 0x01)); //	color   ( Groups:  0x01 - Alb  , 0x02 - Mid , 0x04 - Hib
                switch ((eRealm) obj[3])
                {
    ...
    protected void WriteCustomTextWindowData(GSTCPPacketOut pak, IList<string> text)
    {
        ...
        foreach (var listStr in text)
        {
            ...
            if (str != null)
            {
                ...
                {
                    var s = str.Substring(0, byte.MaxValue);
                ...
                {
                    str = str.Substring(0,
                        (int) Math.Max(Math.Min(1, str.Length), MaxPacketLength - pak.Position - 2));
                    needBreak = true;
                }
    ...
    protected virtual void WriteGroupMemberUpdate(GSTCPPacketOut pak, bool updateIcons, bool updateMap,
        GameLiving living)
    {
        ...
        if (player?.Client?.ClientState == GameClient.eClientState.Linkdead)
            playerStatus |= 0x10;
        if (living.DebuffCategory[(int) eProperty.SpellRange] != 0 ||
            living.DebuffCategory[(int) eProperty.ArcheryRange] != 0)
            playerStatus |= 0x40;
        pak.WriteByte(playerStatus);
        // 0x00 = Normal , 0x01 = Dead , 0x02 = Mezzed , 0x04 = Diseased ,
        ...
        if (updateIcons)
        {
            ...
            lock (living.effectListComponent.EffectsLock)
            {
                ...
                if (living is GamePlayer necro && necro.CharacterClass.ID == (int) eCharacterClass.Necromancer &&
                    necro.IsShade)
                    effects.AddRange(necro.ControlledBrain.Body.effectListComponent.GetAllEffects()
                        .Where(e => e.TriggersImmunity));
    ...
    protected virtual void WriteItemData(GSTCPPacketOut pak, InventoryItem item)
    {
        ...
        if (skill != null &&
            skill is AdvancedCraftingSkill /* && ((AdvancedCraftingSkill)skill).IsAllowedToCombine(_gameClient.Player, item)*/
           )
        ...
        if (item.Object_Type != (int) eObjectType.AlchemyTincture)
        {
            ...
            {
                var chargeEffectsLine = SkillBase.GetSpellLine(GlobalSpellsLines.Item_Effects);
                if (chargeEffectsLine != null)
                {
                    var spells = SkillBase.GetSpellList(chargeEffectsLine.KeyName);
    ...
    protected virtual void WriteItemData(GSTCPPacketOut pak, InventoryItem item, int questID)
    {
        ...
        var skill = CraftingMgr.GetCraftingSkill(item);
        switch (skill)
        {
    ...
    public virtual void SendCustomDialog(string msg, CustomDialogResponse callback, bool customFormat)
    {
        ...
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.Dialog)))
        {
            ...
            pak.WriteShort(0x00);
            pak.WriteByte((byte) (callback == null ? 0x00 : 0x01)); //ok or yes/no response
            pak.WriteByte((byte) (customFormat
                ? 0x00
                : 0x01)); // autowrap text? false if customFormatted == true. Allows use of \n etc in string
            if (msg.Length > 0)
                pak.WriteString(msg, msg.Length);
            pak.WriteByte(0x00);
            SendTCP(pak);
    ...
    public virtual void SendInventoryItemsUpdate(eInventoryWindowType windowType,
        ICollection<InventoryItem> itemsToUpdate)
    {
        if (m_gameClient.Player == null)
            return;
        ...
        // so we send big updates in parts
        var slotsToUpdate =
            new List<int>(Math.Min(ServerProperties.Properties.MAX_ITEMS_PER_PACKET, itemsToUpdate.Count));
        ...
        if (slotsToUpdate.Count > 0) SendInventorySlotsUpdateRange(slotsToUpdate, windowType);
    }
    ...
    public virtual void SendDoorState(Region region, IDoor door)
    {
        ...
        {
            var zone = (ushort) (door.DoorID / 1000000);
            var doorType = door.DoorID / 100000000;
            var flag = door.Flag;

            // by default give all unflagged above ground non keep doors a default sound (excluding TrialsOfAtlantis zones)
            if (flag == 0 && doorType != 7 && region != null && !region.IsDungeon &&
                region.Expansion != (int) eClientExpansion.TrialsOfAtlantis)
                flag = 1;
    ...
    public virtual void SendMerchantWindow(MerchantTradeItems tradeItemsList, eMerchantWindowType windowType)
    {
        if (tradeItemsList != null)
            ...
            {
                var itemsInPage = tradeItemsList.GetItemsInPage((int) page);
                if (itemsInPage == null || itemsInPage.Count == 0)
                    continue;

                using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.MerchantWindow)))
                {
                        ...
                        else
                        {
                            if (log.IsErrorEnabled)
                                log.Error("Merchant item template '" +
                                          ((MerchantItem) itemsInPage[
                                              page * MerchantTradeItems.MAX_ITEM_IN_TRADEWINDOWS + i])
                                          .ItemTemplateID +
                                          "' not found, abort!!!");
                            return;
                        }
    ...
    public virtual void SendUpdateCraftingSkills()
    {
        ...
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.VariousUpdate)))
        {
            ...
            foreach (var de in m_gameClient.Player.CraftingSkills)
            {
                var curentCraftingSkill = CraftingMgr.getSkillbyEnum((eCraftingSkill) de.Key);
                var value = de.Value;
                if (value < 0) value = 0;
    ...
    public virtual void SendUpdateWeaponAndArmorStats()
    {
        ...
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.VariousUpdate)))
        {
            ...
            pak.WritePascalString(" ");
            // weaponskill
            var ws = m_gameClient.Player?.DisplayedWeaponSkill;
            if (ws is null) ws = 0;
            ...
            pak.WritePascalString(" ");
            // overall EAF
            var eaf = m_gameClient.Player?.EffectiveOverallAF;
    ...
    public virtual void SendEncumberance()
    {
        ...
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.Encumberance)))
        {
            ...
            var maxencumberance = m_gameClient.Player?.MaxEncumberance;
            var encumberance = m_gameClient.Player?.Encumberance;
    ...
    public virtual void SendCustomTrainerWindow(int type,
        List<Tuple<Specialization, List<Tuple<Skill, byte>>>> tree)
    {
        if (m_gameClient.Player == null)
            return;
        ...
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.TrainerWindow)))
        {
            if (tree != null && tree.Count > 0)
            {
                ...
                for (var skillIndex = 0; skillIndex < tree.Count; skillIndex++)
                {
                    pak.WriteByte((byte) (skillIndex + 1));
                    pak.WriteByte((byte) tree[skillIndex].Item2.Where(t => t.Item1 != null)
                        .Count()); // Count of item for this line

                    for (var itemIndex = 0; itemIndex < tree[skillIndex].Item2.Count; itemIndex++)
                    {
                        var sk = tree[skillIndex].Item2[itemIndex].Item1;
    ...
    public virtual void SendChampionTrainerWindow(int type)
    {
        ...
        {
            // Get Player CL Spec
            var clspec = player.GetSpecList().Where(sp => sp is LiveChampionsSpecialization)
                .Cast<LiveChampionsSpecialization>().FirstOrDefault();

            // check if the tree can be used
            List<Tuple<MiniLineSpecialization, List<Tuple<Skill, byte>>>> tree = null;
            if (clspec != null) tree = clspec.GetTrainerTreeDisplay(player, clspec.RetrieveTypeForIndex(type));
                ...
                for (var skillIndex = 0; skillIndex < tree.Count; skillIndex++)
                {
                    ...
                    for (var itemIndex = 0; itemIndex < tree[skillIndex].Item2.Count; itemIndex++)
                    {
                        ...
                        if (sk != null)
                        {
                            ...
                            // Skill Status
                            pak.WriteByte(clspec.GetSkillStatus(tree, skillIndex, itemIndex)
                                .Item1); // 0 = disable, 1 = trained, 2 = can train
    ...
    public virtual void SendDisableSkill(ICollection<Tuple<Skill, int>> skills)
    {
        ...
        var specCount = listskills.Where(sk => sk.Item1 is Specialization).Count();

        // Get through all disabled skills
        foreach (var disabled in skills)
        {
            // Check if spell
            byte lsIndex = 0;
            foreach (var ls in listspells)
            {
                var index = ls.Item2.FindIndex(sk =>
                    sk.SkillType == disabled.Item1.SkillType && sk.ID == disabled.Item1.ID);

                if (index > -1)
                {
                    disabledSpells.Add(new Tuple<byte, byte, ushort>(lsIndex, (byte) index,
                        (ushort) (disabled.Item2 > 0 ? disabled.Item2 / 1000 + 1 : 0)));
                    break;
                }
            ...
            var skIndex = listskills.FindIndex(skt =>
                disabled.Item1.SkillType == skt.Item1.SkillType && disabled.Item1.ID == skt.Item1.ID) - specCount;

            if (skIndex > -1)
                disabledSkills.Add(new Tuple<ushort, ushort>((ushort) skIndex,
                    (ushort) (disabled.Item2 > 0 ? disabled.Item2 / 1000 + 1 : 0)));
        }
        ...
        if (disabledSkills.Count > 0)
            ...
            {
                var countskill = (byte) Math.Min(disabledSkills.Count, 255);
                if (countskill > 0)
                {
        ...
        if (disabledSpells.Count > 0)
        {
            ...
            foreach (var groups in groupedDuration)
                ...
                using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.DisableSkills)))
                {
                    var total = (byte) Math.Min(groups.Count(), 255);
                    if (total > 0)
                    {
    ...
    public virtual void SendDebugMessage(string format, params object[] parameters)
    {
        if (m_gameClient.Account.PrivLevel > (int) ePrivLevel.Player || ServerProperties.Properties.ENABLE_DEBUG)
            SendMessage(string.Format("[DEBUG] " + format, parameters), eChatType.CT_System,
    ...
    {
        // Remove from cache
        if (m_gameClient.HouseUpdateArray.ContainsKey(new Tuple<ushort, ushort>(house.RegionID,
                (ushort) house.HouseNumber)))
        {
            long dummy;
            m_gameClient.HouseUpdateArray.TryRemove(
                new Tuple<ushort, ushort>(house.RegionID, (ushort) house.HouseNumber), out dummy);
        }
    ...
    public virtual void SendExitHouse(House house, ushort unknown = 0)
    {
        ...
        if (m_gameClient != null && m_gameClient.Player != null &&
            m_gameClient.ClientState != GameClient.eClientState.Linkdead)
    ...
    public virtual void SendMovingObjectCreate(GameMovingObject obj)
    {
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.MovingObjectCreate)))
        {
            ...
            var flag = obj.Type() | ((byte) obj.Realm == 3 ? 0x40 : (byte) obj.Realm << 4) |
                       (obj.GetDisplayLevel(m_gameClient.Player) << 9);
            ...
            var translation = LanguageMgr.GetTranslation(m_gameClient, obj);
            if (translation != null)
                if (!Util.IsEmpty(((DBLanguageNPC) translation).Name))
                    name = ((DBLanguageNPC) translation).Name;
