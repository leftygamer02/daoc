...
public class PacketProcessor
{
    ...
    /// </summary>
    protected readonly Queue<IPacket> m_lastPackets = new(MAX_LAST_PACKETS);
    ...
    /// </summary>
    public IPacketEncoding Encoding => m_encoding;

    /// <summary>
    /// Caches packet handlers loaded for a given client version (in string format, used for namespace search).
    /// </summary>
    private static Dictionary<string, IPacketHandler[]> m_cachedPacketHandlerSearchResults = new();

    /// <summary>
    /// Stores packet handler attributes for each version, required to load preprocessors.
    /// </summary>
    private static Dictionary<string, List<PacketHandlerAttribute>> m_cachedPreprocessorSearchResults = new();

    private static object m_packetHandlerCacheLock = new();

    public virtual void LoadPacketHandlers(GameClient client)
    {
        var baseVersion = "v168";
        ...
        var attributes = new List<PacketHandlerAttribute>();
        ...
        foreach (var pha in attributes) m_packetPreprocessor.RegisterPacketDefinition(pha.Code, pha.PreprocessorID);
    }
    ...
    public virtual void LoadPacketHandlers(string version, out IPacketHandler[] packetHandlers,
        out List<PacketHandlerAttribute> attributes)
    {
        ...
        lock (m_packetHandlerCacheLock)
        {
            ...
            {
                var count = SearchAndAddPacketHandlers(version, Assembly.GetAssembly(typeof(GameServer)),
                    packetHandlers);
            ...
            log.Info("PacketProcessor: Loaded " + attributes.Count + " preprocessors from cache for version=" +
                     version + "!");
        }
    }
    ...
    public void RegisterPacketHandler(int packetCode, IPacketHandler handler, IPacketHandler[] packetHandlers)
    {
        if (packetHandlers[packetCode] != null)
            log.InfoFormat("Overwriting Client Packet Code {0}, with handler {1} in PacketProcessor", packetCode,
    ...
    {
        var tcp = m_tcpSendBuffer;
        var udp = m_udpSendBuffer;
    ...
    /// </summary>
    protected readonly Queue<byte[]> m_tcpQueue = new(256);
    ...
    public void SendTCP(GSTCPPacketOut packet)
    {
        ...
        var buf = packet.GetBuffer(); //packet.WritePacketLength sets the Capacity
    ...
    public void SendTCP(byte[] buf)
    {
        ...
        if (m_client.Socket.Connected)
        {
                ...
                if (log.IsErrorEnabled)
                {
                    var desc =
                        string.Format(
                            ...
                            buf[2], m_client.Account != null ? m_client.Account.Name : m_client.TcpEndpoint,
                            buf.Length);
                    ...
                    if (Properties.IGNORE_TOO_LONG_OUTCOMING_PACKET)
                    {
                        ...
                        m_client.Out.SendMessage(
                            "ALERT: Error sending an update to your client. Oversize packet detected and discarded. Please /report this issue!",
            ...
            try
            {
                ...
                m_client.Socket.BeginSend(m_tcpSendBuffer, 0, buf.Length, SocketFlags.None, m_asyncTcpCallback,
                    m_client);
    ...
    protected static void AsyncTcpSendCallback(IAsyncResult ar)
    {
        ...
        try
        {
            var pakProc = client.PacketProcessor;
            var q = pakProc.m_tcpQueue;
            ...
            var count = 0;
            var data = pakProc.m_tcpSendBuffer;
...
//						Log.WarnFormat("async sent {0} bytes", sent);
            ...
            var took = GameTimer.GetTickCount() - start;
            if (took > 100 && log.IsWarnEnabled)
                log.WarnFormat("AsyncTcpSendCallback.BeginSend took {0}ms! (TCP to client: {1})", took,
                    client.ToString());
        }
    ...
    /// </summary>
    protected readonly Queue<byte[]> m_udpQueue = new(256);
...
//				Log.FatalFormat("UDP sent over TCP");
    ...
    private void AsyncUdpSendCallback(IAsyncResult ar)
    {
        try
        {
            ...
            var sent = s.EndSendTo(ar);

            var count = 0;
            var data = m_udpSendBuffer;
...
//						Log.WarnFormat("async UDP sent {0} bytes", sent);
            ...
            var took = GameTimer.GetTickCount() - start;
            if (took > 100 && log.IsWarnEnabled)
                log.WarnFormat("AsyncUdpSendCallback.BeginSend took {0}ms! (TCP to client: {1})", took,
                    m_client.ToString());
        }
        catch (Exception e)
        {
    ...
    public void ReceiveBytes(int numBytes)
    {
        lock (m_SyncLock)
        {
            var buffer = m_client.ReceiveBuffer;

            //End Offset of buffer
            var bufferSize = m_client.ReceiveBufferOffset + numBytes;
            ...
            do
            {
                var packetLength = (buffer[curOffset] << 8) + buffer[curOffset + 1] + GSPacketIn.HDR_SIZE;
                var dataLeft = bufferSize - curOffset;
                ...
                var packetEnd = curOffset + packetLength;

                int calcCheck = CalculateChecksum(buffer, curOffset, packetLength - 2);
                var pakCheck = (buffer[packetEnd - 2] << 8) | buffer[packetEnd - 1];
                ...
                if (pakCheck != calcCheck)
                {
                    ...
                    if (log.IsInfoEnabled)
                    {
                        ...
                        foreach (var prevPak in GetLastPackets()) log.Info(prevPak.ToHumanReadable());
    ...
    {
        var source = m_client.Account != null ? m_client.Account.Name : m_client.TcpEndpoint;
        if (log.IsErrorEnabled)
    ...
    public void HandlePacket(GSPacketIn packet)
    {
        ...
        if (packetHandler != null)
        {
            ...
            catch (Exception e)
            {
                if (log.IsErrorEnabled)
                {
                    var client = m_client == null ? "null" : m_client.ToString();
                    log.Error(
                        "Error while processing packet (handler=" + packetHandler.GetType().FullName +
                        "  client: " + client + ")", e);
                }
            }
#if LOGACTIVESTACKS
            ...
            if (timeUsed > 1000)
            {
                var source = m_client.Account != null ? m_client.Account.Name : m_client.TcpEndpoint;
                if (log.IsWarnEnabled)
                    log.Warn("(" + source + ") Handle packet Thread " + Thread.CurrentThread.ManagedThreadId + " " +
                             packetHandler +
                             " took " + timeUsed + "ms!");
            }
        }
