...
public class PacketLib174 : PacketLib173
{
    ...
    public override void SendCharacterOverview(eRealm realm)
    {
        ...
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.CharacterOverview)))
        {
            ...
            if (characters == null)
                ...
                for (var i = firstAccountSlot; i < firstAccountSlot + 10; i++)
                {
                    var written = false;
                    for (var j = 0; j < characters.Length && written == false; j++)
                        if (characters[j].AccountSlot == i)
                        {
                            ...
                            items = DOLDB<InventoryItem>.SelectObjects(DB.Column("OwnerID")
                                .IsEqualTo(characters[j].ObjectId)
                                .And(DB.Column("SlotPosition").IsGreaterOrEqualTo(10))
                                .And(DB.Column("SlotPosition").IsLessOrEqualTo(37)));
                            ...
                            pak.WriteByte(
                                (byte) ((ExtensionTorso << 4) | (characters[j].IsCloakHoodUp ? 0x1 : 0x0)));
                            pak.WriteByte((byte) characters[j]
                            ...
                            pak.WriteByte(
                                (byte) ((((characters[j].Race & 0x10) << 2) + (characters[j].Race & 0x0F)) |
                                        (characters[j].Gender << 4))); // race max value can be 0x1F
    ...
    public override void SendPlayerCreate(GamePlayer playerToCreate)
    {
        ...
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.PlayerCreate172)))
        {
            ...
            pak.WriteByte(
                playerToCreate.GetFaceAttribute(eCharFacePart.EyeColor)); //1-4 = Skin Color / 5-8 = Eye Color
    ...
    public override void SendPlayerPositionAndObjectID()
    {
        ...
        using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.PositionAndObjectID)))
        {
            ...
            pak.WritePascalString(GameServer.Instance.Configuration
                .ServerNameShort); // new in 1.74, same as in SendLoginGranted
