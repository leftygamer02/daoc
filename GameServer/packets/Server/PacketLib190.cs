...
public class PacketLib190 : PacketLib189
{
    ...
    public override void SendMasterLevelWindow(byte ml)
    {
        ...
        {
            mlXPPercent = 100.0 * (double) m_gameClient.Player.MLExperience /
                          (double) m_gameClient.Player.GetMLExperienceForLevel(
                              (int) (m_gameClient.Player.MLLevel + 1));
            if (m_gameClient.Player.GetStepCountForML((byte) (m_gameClient.Player.MLLevel + 1)) > 0)
                mlStepPercent = 100.0 *
                                (double) m_gameClient.Player.GetCountMLStepsCompleted(
                                    (byte) (m_gameClient.Player.MLLevel + 1)) /
                                (double) m_gameClient.Player.GetStepCountForML(
                                    (byte) (m_gameClient.Player.MLLevel + 1));
        }
        else
        {
