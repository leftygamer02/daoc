...
public abstract class AbstractPacketLib
{
    ...
    public static IPacketLib CreatePacketLibForVersion(int rawVersion, GameClient client,
        out GameClient.eClientVersion version)
    {
        foreach (var t in ScriptMgr.GetDerivedClasses(typeof(IPacketLib)))
        foreach (PacketLibAttribute attr in t.GetCustomAttributes(typeof(PacketLibAttribute), false))
            if (attr.RawVersion == rawVersion)
                try
                {
                    var lib = (IPacketLib) Activator.CreateInstance(t, new object[] {client});
                ...
                catch (Exception e)
                {
                    if (log.IsErrorEnabled)
                        log.Error("error creating packetlib (" + t.FullName + ") for raw version " + rawVersion,
                            e);
                }
