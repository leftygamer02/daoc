...
namespace DOL.GS.Quests.Atlantis.Artifacts
{
	...
	public class Battler : ArtifactQuest
	{
		...
		/// </summary>
		private static int m_artifactID = 1006;
		...
		public Battler(GamePlayer questingPlayer, Atlas.DataLayer.Models.Quest dbQuest)
		...
		public override bool WhisperReceive(GameLiving source, GameLiving target, string text)
		{
			...
			else if (Step == 4)
			{
				switch (text.ToLower())
				{
					...
					case "two handed":
						{
							String versionID = String.Format("{0};{1};", GetCustomProperty("DamageType"), text.ToLower());
