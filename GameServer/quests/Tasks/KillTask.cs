...
public class KillTask : AbstractTask
{
    ...
    private static readonly int[] MoneyReward = new int[20]
        {28, 57, 77, 105, 140, 190, 257, 347, 470, 632, 735, 852, 987, 1147, 1330, 1542, 1790, 2077, 2407, 2801};

    private static readonly string[] TaskObjects =
    {
        "Skin", "Meat", "Bones", "Tooth", "Claw", "Skin", "Legs", "Collar", "Bone", "Ear", "Head", "Hair",
        "Carapace", "Skull", "Pile of dirt", "dust", "Slice", "Wings", "egg", "heart", "mandible"
    };

    private static readonly int[] ObjectModels =
        {629, 102, 105, 106, 106, 629, 108, 109, 497, 501, 503, 506, 517, 540, 541, 541, 548, 551, 587, 595, 614};
    ...
    public override long RewardMoney
    {
        get
        {
            ...
            var ValueScarto = MoneyReward[m_taskPlayer.Level - 1] / 100 * Scarto;
            return Util.Random(MoneyReward[m_taskPlayer.Level - 1] - ValueScarto,
                MoneyReward[m_taskPlayer.Level - 1] + ValueScarto);
        }
    }
    ...
    {
        get => string.Format(StrFormat[0], MobName, TaskObjects[ItemIndex]);
        set { }
    }
    ...
    public override string Description =>
        ((KillTask) m_taskPlayer.Task).MobKilled == false
            ? "Find a " + MobName + " and kill it then return to me for your reward."
            : "Return to " + ReceiverName + " for your reward!";
    ...
    public override void Notify(DOLEvent e, object sender, EventArgs args)
    {
        ...
        if (e == GameLivingEvent.EnemyKilled)
        {
            ...
            if (ServerProperties.Properties.TASK_GIVE_RANDOM_ITEM == false)
            {
                if (((KillTask) player.Task).MobName == target.Name)
                {
                    ...
                    player.Out.SendMessage(
                        "You must now return to " + player.Task.ReceiverName + " to receive your reward!",
            ...
            else
            {
                ...
                var lowestCon = int.MaxValue;
                lock (target.XPGainers.SyncRoot)
                {
                    if (target.XPGainers.Keys.Count == 0) return;
                    ...
                    foreach (GameObject gainer in target.XPGainers.Keys)
                        if (gainer is GamePlayer)
                        {
                            ...
                            {
                                if (target.Level <= GameLiving.NoXPForLevel[gainer.Level]) lowestCon = -3;
                            }
                            else
                            {
                                var con = (int) gainer.GetConLevel(target);
                                if (con < lowestCon) lowestCon = con;
                            }
                        }
                }
                ...
                if (lowestCon >= -2)
                {
                    ...
                    {
                        var group = m_taskPlayer.Group.GetPlayersInTheGroup();
                    ...
                    if (Owners.Count > 0)
                    {
                        var dropMessages = new ArrayList();
                        var itemdrop = GenerateItem(ItemName, 1, ObjectModels[ItemIndex]);
                        var droppeditem = new WorldInventoryItem(itemdrop);
                        for (var a = 0; a < Owners.Count; a++) droppeditem.AddOwner((GameObject) Owners[a]);
        ...
        else if (e == GameObjectEvent.InteractWith)
        {
            ...
            {
                var myargs = (InteractWithEventArgs) args;
                if (myargs.Target.Name == ((KillTask) player.Task).ReceiverName)
                {
                    if (player.Level > 20) return;
                    player.Out.SendMessage(
                        myargs.Target.Name + " says, *Good work " + player.Name +
                        ". Here is your reward as promised.*", eChatType.CT_System, eChatLoc.CL_SystemWindow);
                    FinishTask();
                }
            }
        }
        else if (e == GamePlayerEvent.GiveItem && ServerProperties.Properties.TASK_GIVE_RANDOM_ITEM == true)
        {
            ...
            var target = gArgs.Target as GameLiving;
            var item = gArgs.Item;
            ...
            if (player.Task.ReceiverName == target.Name && item.Name == player.Task.ItemName)
            {
                ...
                InventoryLogging.LogInventoryAction(player, target, eInventoryActionType.Quest, item.Template,
                    item.Count);
                player.Out.SendMessage(
                    target.Name + " says, *Good work " + player.Name + ". Here is your reward as promised.*",
                    eChatType.CT_System, eChatLoc.CL_SystemWindow);
                FinishTask();
            }
        }
    ...
    public static bool BuildTask(GamePlayer player, GameLiving source)
    {
        ...
        var Mob = GetRandomMob(player);

        if (Mob == null)
        {
            player.Out.SendMessage("Sorry, I couldn't find any mob kill order. Come back later!", eChatType.CT_Say,
        ...
        {
            player.Out.SendMessage("I have no task for you, come back later", eChatType.CT_Say,
                eChatLoc.CL_PopupWindow);
            return false;
        }
        else
        {
            ...
            player.Out.SendMessage(
                source.Name + " says, *Very well " + player.Name +
                ", it's good to see adventurers willing to help out the realm in such times. Search to the " +
                GetDirectionFromHeading(Mob.Heading) + " and kill a " + Mob.Name +
                " and return to me for your reward. Good luck!*", eChatType.CT_System, eChatLoc.CL_SystemWindow);
            player.Out.SendDialogBox(eDialogCode.SimpleWarning, 1, 1, 1, 1, eDialogType.Ok, false,
    ...
    {
        var minLevel = GameLiving.NoXPForLevel[Player.Level] + 1;
        int maxLevel = Player.Level;
        var npc = Player.CurrentZone.GetRandomNPC(eRealm.None, minLevel, maxLevel);
    ...
    public static bool CheckNamedGuard(GameLiving living)
    {
        ...
        if (name.StartsWith("Sir ") && string.IsNullOrEmpty(living.GuildName)) return true;

        if (name.StartsWith("Captain ") && string.IsNullOrEmpty(living.GuildName)) return true;

        if (name.StartsWith("Jarl ")) return true;

        if (name.StartsWith("Lady ") && string.IsNullOrEmpty(living.GuildName)) return true;
        ...
        if (name.StartsWith("Huntress ")) return true;

        if (name.StartsWith("Bork ")) return true;
