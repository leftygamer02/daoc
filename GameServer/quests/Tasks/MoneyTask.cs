...
public class MoneyTask : AbstractTask
{
    ...
    private static readonly int[] MoneyReward = new int[20]
        {25, 34, 46, 63, 84, 114, 154, 208, 282, 379, 441, 511, 592, 688, 798, 925, 1074, 1246, 1444, 1830};
    ...
    {
        "Potion", "Flask", "Necklace", "Ring", "Dyes", "Green Gem", "Note", "Scroll", "Book", "Hammer", "Crown",
        "Blue orb", "Red orb", "Silver orb", "Wand", "Bottle", "Gold key", "Silver key", "Mirror", "Scroll tube",
        "Four leaf clover", "Vine", "Silver earring", "Gold bracelet", "Gold necklace", "Amulet", "Letter"
    };

    private static readonly int[] ObjectModels =
    {
        99, 99, 101, 103, 229, 113, 498, 499, 500, 510, 511, 523, 524, 606, 552, 554, 583, 584, 592, 603, 607, 611,
        621, 622, 623, 101, 498
    };
    ...
    /// </summary>
    public override string Description => "Bring the " + ItemName + " to " + ReceiverName + " in " + RecieverZone;
    ...
    {
        var Id = Util.Random(0, TaskObjects.Length - 1);
        var format = Util.Random(0, StrFormat.Length - 1);
        var middle = Util.Random(0, Middle.Length - 1);
        return GenerateItem(string.Format(StrFormat[format], Name, Middle[middle], TaskObjects[Id]), Level,
            ObjectModels[Id]);
    }
    ...
    public static bool BuildTask(GamePlayer player, GameLiving source)
    {
        ...
        var NPC = GetRandomNPC(player);
        if (NPC == null)
        {
            player.Out.SendMessage("I have no task for you, come back some time later.", eChatType.CT_System,
                eChatLoc.CL_PopupWindow);
            return false;
        }
        else
        {
            var TaskItems = GenerateNPCItem(NPC.Name, player.Level);
            ...
            player.Out.SendMessage(
                "Bring " + TaskItems.GetName(0, false) + " to " + NPC.Name + " in " + NPC.CurrentZone.Description,
