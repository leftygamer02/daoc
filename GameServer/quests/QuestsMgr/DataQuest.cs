...
public class DataQuest : AbstractQuest
{
    ...
    public enum eStepType : byte
    {
        ...
        KillFinish =
            1, // Killing the target finishes the quest and gives the reward.  Can set chance to drop on StepItemTemplate.
        ...
        SearchFinish =
            9, // Search in a specified location to finish the quest. Can set chance to drop on StepItemTemplate.
    ...
    /// </summary>
    protected static List<KeyValuePair<int, QuestSearchArea>> m_allQuestSearchAreas = new();
    ...
    protected List<string> m_sourceTexts = new();
    protected List<string> m_targetNames = new();
    protected List<ushort> m_targetRegions = new();
    protected List<string> m_targetTexts = new();
    protected List<eStepType> m_stepTypes = new();
    protected List<string> m_stepTexts = new();
    protected List<string> m_stepItemTemplates = new();
    protected List<string> m_advanceTexts = new();
    protected List<string> m_collectItems = new();

    protected List<long> m_rewardXPs = new();

    // CLXP added
    protected List<long> m_rewardCLXPs = new();

    // RP added
    protected List<long> m_rewardRPs = new();

    // BP added
    protected List<long> m_rewardBPs = new();
    protected List<long> m_rewardMoneys = new();
    private byte m_numOptionalRewardsChoice = 0;
    protected List<ItemTemplate> m_optionalRewards = new();
    protected List<ItemTemplate> m_optionalRewardChoice = new();
    protected int[] m_rewardItemsChosen = null;
    protected List<ItemTemplate> m_finalRewards = new();
    protected List<string> m_questDependencies = new();
    protected List<byte> m_allowedClasses = new();
    ...
    /// <param name="dbQuest"></param>
    /// <param name="charQuest"></param>
    public DataQuest(GamePlayer questingPlayer, GameObject sourceObject, DBDataQuest dataQuest,
        CharacterXDataQuest charQuest)
    {
        ...
        {
            if (sourceObject is GameNPC) m_startNPC = sourceObject as GameNPC;
    ...
    protected void ParseQuestData()
    {
        ...
        try
        {
            foreach (var entry in m_allQuestSearchAreas)
                if (entry.Key == ID)
                    m_numSearchAreas++;
            ...
            if (!string.IsNullOrEmpty(lastParse))
            {
                ...
                foreach (var str in parse1) m_sourceTexts.Add(str);
            }
            ...
            {
                parse1 = lastParse.Split('|');
                foreach (var str in parse1) m_targetTexts.Add(str);
            }
            ...
            {
                parse1 = lastParse.Split('|');
                foreach (var str in parse1) m_stepTypes.Add((eStepType) Convert.ToByte(str));
            }
            ...
            {
                parse1 = lastParse.Split('|');
                foreach (var str in parse1) m_stepTexts.Add(str);
            }
            ...
            {
                parse1 = lastParse.Split('|');
                foreach (var str in parse1) m_stepItemTemplates.Add(str);
            }
            ...
            {
                parse1 = lastParse.Split('|');
                foreach (var str in parse1) m_advanceTexts.Add(str);
            }
            ...
            {
                parse1 = lastParse.Split('|');
                foreach (var str in parse1) m_collectItems.Add(str);
            }
            ...
            {
                parse1 = lastParse.Split('|');
                foreach (var str in parse1) m_rewardMoneys.Add(Convert.ToInt64(str));
            }
            ...
            {
                parse1 = lastParse.Split('|');
                foreach (var str in parse1) m_rewardXPs.Add(Convert.ToInt64(str));
            }
            ...
            {
                parse1 = lastParse.Split('|');
                foreach (var str in parse1) m_rewardCLXPs.Add(Convert.ToInt64(str));
            }
            ...
            {
                parse1 = lastParse.Split('|');
                foreach (var str in parse1) m_rewardRPs.Add(Convert.ToInt64(str));
            }
            ...
            {
                parse1 = lastParse.Split('|');
                foreach (var str in parse1) m_rewardBPs.Add(Convert.ToInt64(str));
            }
            ...
            if (!string.IsNullOrEmpty(lastParse))
            {
                ...
                foreach (var str in parse1)
                    ...
                    {
                        var item = GameServer.Database.FindObjectByKey<ItemTemplate>(str);
                        if (item != null)
                        {
                        ...
                        {
                            var errorText =
                                string.Format("DataQuest: Optional reward ItemTemplate not found: {0}", str);
                            log.Error(errorText);
            ...
            if (!string.IsNullOrEmpty(lastParse))
            {
                ...
                foreach (var str in parse1)
                {
                    ...
                    {
                        var errorText = string.Format("DataQuest: Final reward ItemTemplate not found: {0}",
                            str);
                        log.Error(errorText);
                        m_lastErrorText += " " + errorText;
                    }
            ...
            {
                parse1 = lastParse.Split('|');
                foreach (var str in parse1) m_allowedClasses.Add(Convert.ToByte(str));
            }
        ...
        {
            var errorText = "Error parsing quest data for " + m_dataQuest.Name + " (" + m_dataQuest.ID +
                            "), last string to parse = '" + lastParse + "'.";
    ...
    protected void ParseSearchAreas()
    {
        ...
        try
        {
            ...
            var areasToDelete = new List<KeyValuePair<int, QuestSearchArea>>();

            foreach (var entry in m_allQuestSearchAreas)
                if (entry.Key == ID)
                    areasToDelete.Add(entry);

            foreach (var entry in areasToDelete)
            {
                m_lastErrorText += " ::Removing QuestSearchArea for DataQuest ID:" + ID + ", Step " +
                                   entry.Value.Step;
        ...
        {
            log.Error(
                "Error parsing quest data for " + m_dataQuest.Name + " (" + m_dataQuest.ID +
                "), last string to parse = '" + lastParse + "'.", ex);
    ...
    protected void CreateSearchArea(string areaStr)
    {
        try
        {
            ...
            var questArea = new QuestSearchArea(this, requiredStep, parse[2],
                Convert.ToUInt16(parse[3]), Convert.ToInt32(parse[4]), Convert.ToInt32(parse[5]),
                Convert.ToInt32(parse[6]), Convert.ToInt32(parse[7]));
            m_allQuestSearchAreas.Add(new KeyValuePair<int, QuestSearchArea>(ID, questArea));

            m_lastErrorText +=
                string.Format(
                    " ::Created Search Area for quest {0}, step {1} in region {2} at X:{3}, Y:{4}, Radius:{5}, Text:{6}, Seconds:{7}.",
                    Name, requiredStep, parse[3], parse[4], parse[5], parse[6], parse[2], parse[7]);
        }
        catch
        {
            var error = "Error creating search area for " + m_dataQuest.Name + " (" + m_dataQuest.ID +
                        "), area str = '" + areaStr + "'";
            log.Error(error);
    ...
    {
        get => m_numOptionalRewardsChoice;
    ...
    /// </summary>
    public virtual string FinishText => BehaviourUtils.GetPersonalizedMessage(m_dataQuest.FinishText, m_questPlayer);
    ...
    /// </summary>
    public virtual List<string> StepTexts => m_stepTexts;
    ...
    public virtual short Count
    {
        get
        {
            if (m_charQuest != null) return m_charQuest.Count;

            return 0;
        }
        set
        {
            var oldCount = m_charQuest.Count;
            m_charQuest.Count = value;
            if (m_charQuest.Count != oldCount) GameServer.Database.SaveObject(m_charQuest);
        }
    }
    ...
    public override int Step
    {
        ...
        set
        {
            if (m_charQuest != null)
            {
                ...
                if (m_charQuest.Step != oldStep) GameServer.Database.SaveObject(m_charQuest);
            }
        }
    }
    ...
    /// </summary>
    public string AdditionalData => m_additionalData;
    ...
    {
        var charQuest = DOLDB<CharacterXDataQuest>.SelectObject(DB.Column("Character_ID")
            .IsEqualTo(player.QuestPlayerID).And(DB.Column("DataQuestID").IsEqualTo(ID)));
    ...
    public override bool CheckQuestQualification(GamePlayer player)
    {
        ...
        if (StartType == eStartType.Collection)
        {
            ...
            if (charQuest != null && charQuest.Count >= MaxQuestCount) return false;
        ...
        {
            foreach (var q in player.QuestListFinished)
                if (q is DataQuest && (q as DataQuest).ID == ID)
                    if (q.IsDoingQuest(q) == true ||
                        ((q as DataQuest).Count >= MaxQuestCount && MaxQuestCount >= 0))
            ...
            if (m_questDependencies.Count > 0)
            {
                ...
                foreach (var str in m_questDependencies)
                foreach (var q in player.QuestListFinished)
                ...
                if (numFound < m_questDependencies.Count) return false;
            }
        }
    ...
    {
        if (checkQuest is DataQuest && (checkQuest as DataQuest).ID == ID) return Step > 0;
    ...
    public string TargetName
    {
        get
        {
            try
            {
                if (m_targetNames.Count > 0) return m_targetNames[Step - 1];
            }
            catch (Exception ex)
            {
    ...
    public ushort TargetRegion
    {
        get
        {
            try
            {
                if (m_targetRegions.Count > 0) return m_targetRegions[Step - 1];
            }
            catch (Exception ex)
            {
    ...
    protected string TargetText
    {
        get
        {
            try
            {
                ...
                {
                    if (Step < 1) return m_targetTexts[0];
    ...
    protected string StepItemTemplate
    {
        get
        {
            try
            {
                if (m_stepItemTemplates.Count > 0) return m_stepItemTemplates[Step - 1].Trim();
            }
            catch (Exception ex)
            {
                log.Error("DataQuest [" + ID + "] StepItemTemplate error for Step " + Step, ex);
                if (QuestPlayer != null)
                    ChatUtil.SendDebugMessage(QuestPlayer,
                        "DataQuest [" + ID + "] StepItemTemplate error for Step " + Step);
            }
    ...
    protected string CollectItemTemplate
    {
        get
        {
            try
            {
                if (m_collectItems.Count > 0) return m_collectItems[Step - 1];
            }
            catch (Exception ex)
            {
                ...
                if (QuestPlayer != null)
                    ChatUtil.SendDebugMessage(QuestPlayer,
                        "DataQuest [" + ID + "] CollectItemTemplate error for Step " + Step);
            }
    ...
    protected string AdvanceText
    {
        get
        {
            try
            {
                if (m_advanceTexts.Count > 0) return m_advanceTexts[Step - 1];
            }
            catch (Exception ex)
            {
                ...
                if (QuestPlayer != null)
                    ChatUtil.SendDebugMessage(QuestPlayer,
                        "DataQuest [" + ID + "] AdvanceText error for Step " + Step);
            }
    ...
    protected long RewardMoney
    {
        get
        {
            try
            {
                if (m_rewardMoneys.Count == 0) return 0;
            ...
            catch (Exception ex)
            {
                ...
                if (QuestPlayer != null)
                    ChatUtil.SendDebugMessage(QuestPlayer,
                        "DataQuest [" + ID + "] RewardMoney error for Step " + Step);
            }
    ...
    protected long RewardXP
    {
        get
        {
            try
            {
                if (m_rewardXPs.Count == 0) return 0;
            ...
            catch (Exception ex)
            {
                ...
                if (QuestPlayer != null)
                    ChatUtil.SendDebugMessage(QuestPlayer,
                        "DataQuest [" + ID + "] RewardXP error for Step " + Step);
            }
    ...
    protected long RewardCLXP
    {
        get
        {
            try
            {
                if (m_rewardCLXPs.Count == 0) return 0;
            ...
            catch (Exception ex)
            {
                ...
                if (QuestPlayer != null)
                    ChatUtil.SendDebugMessage(QuestPlayer,
                        "DataQuest [" + ID + "] RewardCLXP error for Step " + Step);
            }
    ...
    protected long RewardRP
    {
        get
        {
            try
            {
                if (m_rewardRPs.Count == 0) return 0;
            ...
            catch (Exception ex)
            {
                ...
                if (QuestPlayer != null)
                    ChatUtil.SendDebugMessage(QuestPlayer,
                        "DataQuest [" + ID + "] RewardRP error for Step " + Step);
            }
    ...
    protected long RewardBP
    {
        get
        {
            try
            {
                if (m_rewardBPs.Count == 0) return 0;
            ...
            catch (Exception ex)
            {
                ...
                if (QuestPlayer != null)
                    ChatUtil.SendDebugMessage(QuestPlayer,
                        "DataQuest [" + ID + "] RewardBP error for Step " + Step);
            }
    ...
    {
        if (m_rewardMoneys.Count < 1) return 0;
    ...
    {
        if (m_rewardXPs.Count < 1) return 0;

        int currentLevel = player.Level;
        if (currentLevel > player.MaxLevel)
            return 0;
        var experienceToLevel = player.GetExperienceNeededForLevel(currentLevel + 1) -
    ...
    protected virtual bool ExecuteCustomQuestStep(GamePlayer player, int step, eStepCheckType stepCheckType)
    {
        ...
        if (!string.IsNullOrEmpty(m_classType))
        {
            if (m_customQuestStep == null)
            {
                foreach (var assembly in AppDomain.CurrentDomain.GetAssemblies())
                    if (assembly.GetType(m_classType) != null)
                    {
                        try
                        {
                            m_customQuestStep = assembly.CreateInstance(m_classType, false,
                                BindingFlags.CreateInstance, null, new object[] { }, null, null) as IDataQuestStep;
                        }
                        catch (Exception)
                        {
                        }
            ...
            {
                log.ErrorFormat(
                    "Failed to construct custom DataQuest step of ClassType {0}!  Quest will continue anyway.",
                    m_classType);
                if (QuestPlayer != null)
                    ChatUtil.SendDebugMessage(QuestPlayer,
                        string.Format(
        ...
        if (m_customQuestStep != null) canContinue = m_customQuestStep.Execute(this, player, step, stepCheckType);
    ...
    protected virtual bool AdvanceQuestStep(GameObject obj = null)
    {
        try
        {
            var nextStepType = m_stepTypes[Step];
            var advance = false;
            ...
            if (ExecuteCustomQuestStep(QuestPlayer, Step, eStepCheckType.Step))
            {
                ...
                {
                    QuestPlayer.Out.SendMessage(
                        "Your XP is turned off, you must turn it on to complete this quest step!",
                ...
                {
                    QuestPlayer.Out.SendMessage(
                        "Your RP is turned off, you must turn it on to complete this quest step!",
                        eChatType.CT_Staff, eChatLoc.CL_SystemWindow);
                    return false;
                }
                ...
                if (!string.IsNullOrEmpty(StepItemTemplate)) stepTemplates.Add(StepItemTemplate);
                ...
                if (stepTemplates.Count == 1 && (StepType == eStepType.Kill || StepType == eStepType.KillFinish ||
                                                 StepType == eStepType.Search ||
                                                 StepType == eStepType.SearchFinish))
                {
                    var template = stepTemplates[0].Split(';');
                    ...
                    if (template.Length > 1)
                    {
                        ...
                        {
                            ChatUtil.SendDebugMessage(QuestPlayer,
                                "[DEBUG] AdvanceQuestStep error; chance to drop StepTemplate is 0 when advancing from Step " +
                                Step);
                            return false;
                        }
                    }
                ...
                if (stepTemplates.Count > 0)
                    ...
                    {
                        if (QuestPlayer.Inventory.IsSlotsFree(stepTemplates.Count, eInventorySlot.FirstBackpack,
                                eInventorySlot.LastBackpack))
                        {
                            foreach (var template in stepTemplates)
                            {
                                var item = GameServer.Database.FindObjectByKey<ItemTemplate>(template);
                                if (item == null)
                                {
                                    var errorMsg = string.Format("StepItemTemplate {0} not found in DB!",
                                        template);
                                    QuestPlayer.Out.SendMessage(errorMsg, eChatType.CT_Staff,
                        ...
                        {
                            QuestPlayer.Out.SendMessage(
                                "You don't have enough inventory space to advance this quest.  You need " +
                                stepTemplates.Count + " free slot(s)!", eChatType.CT_System,
                                eChatLoc.CL_SystemWindow);
                            advance = false;
                        }
                    }
            ...
            if (advance)
            {
                ...
                {
                    if (RewardXP > 0) m_questPlayer.ForceGainExperience(RewardXP);

                    if (RewardRP > 0) m_questPlayer.GainRealmPoints(RewardRP);

                    if (RewardMoney > 0)
                    {
                        m_questPlayer.AddMoney(RewardMoney, "You are awarded {0}!");
                        InventoryLogging.LogInventoryAction("(QUEST;" + Name + ")", m_questPlayer,
                            eInventoryActionType.Quest, RewardMoney);
                    }

                    if (RewardCLXP > 0) m_questPlayer.GainChampionExperience(RewardCLXP, eXPSource.Quest);

                    if (RewardBP > 0) m_questPlayer.GainBountyPoints(RewardBP);
                }
                // Then advance step
                ...
                switch (StepType)
                {
                    ...
                    case eStepType.CollectFinish:
                        ...
                        {
                            var npc = n;
                            if (npc != null && TargetName == npc.Name &&
                                (TargetRegion == 0 || TargetRegion == npc.CurrentRegionID))
        ...
        catch (Exception ex)
        {
            ...
            if (QuestPlayer != null)
                ChatUtil.SendDebugMessage(QuestPlayer,
                    "[DEBUG] AdvanceQuestStep error when advancing from Step " + Step + ": " + ex.Message);
        }
    ...
    public override void Notify(DOLEvent e, object sender, EventArgs args)
    {
        ...
        try
        {
            ...
            {
                var a = args as GiveItemEventArgs;
            ...
            {
                var a = args as ReceiveItemEventArgs;
                var p = a.Source as GamePlayer;
            ...
            {
                var a = args as WhisperReceiveEventArgs;
                var p = a.Source as GamePlayer;
            ...
            {
                var a = args as WhisperEventArgs;
                var p = sender as GamePlayer;

                if (p != null) OnPlayerWhisper(p, a.Target, a.Text);
            }
            ...
            if (e == GameLivingEvent.Dying)
            {
                ...
                var dying = sender as GameLiving;
                var killer = a.Killer;
                var playerKillers = a.PlayerKillers;
            ...
            {
                var rewardArgs = args as QuestRewardChosenEventArgs;
                if (rewardArgs == null)
                    return;
                ...
                if (ExecuteCustomQuestStep(QuestPlayer, 0, eStepCheckType.RewardsChosen))
                {
                    ...
                    {
                        for (var reward = 0; reward < rewardArgs.CountChosen; ++reward)
                        ...
                        {
                            QuestPlayer.Out.SendMessage(
                                LanguageMgr.GetTranslation(QuestPlayer.Client, "RewardQuest.Notify"),
    ...
    public static void RewardQuestNotify(DOLEvent e, object sender, EventArgs args)
    {
        ...
        if (e == GamePlayerEvent.AcceptQuest)
        {
            ...
            var player = qargs.Player;
            var giver = qargs.Source;

            foreach (var quest in GameObject.DataQuestCache)
                if (quest.ID + DATAQUEST_CLIENTOFFSET == qargs.QuestID)
                {
                    var charQuest = GetCharacterQuest(player, quest.ID, true);
                    var dq = new DataQuest(player, giver, quest, charQuest);
                    dq.Step = 1;
                    player.AddQuest(dq);
                    if (giver is GameNPC)
                    {
    ...
    protected virtual void CheckOfferQuest(GamePlayer player, GameObject obj)
    {
        ...
        if (CheckQuestQualification(player))
        {
                ...
                if (charQuest.Count < MaxQuestCount)
                {
                    ...
                    if (ExecuteCustomQuestStep(player, 0, eStepCheckType.Finish))
                    {
                        ...
                        if (m_finalRewards.Count > 0)
                            ...
                            {
                                if (player.Inventory.IsSlotsFree(m_finalRewards.Count, eInventorySlot.FirstBackpack,
                                        eInventorySlot.LastBackpack))
                                {
                                    foreach (var item in m_finalRewards)
                                        if (item != null)
                                            GiveItem(obj is GameLiving ? obj as GameLiving : null, player, item,
                                                false);
                                }
                                else
                                {
                                    SendMessage(player,
                                        "Your inventory does not have enough space to finish this quest!", 0,
                                        eChatType.CT_System, eChatLoc.CL_PopupWindow);
                                    return;
                                }
                            }

                        if (m_rewardXPs.Count > 0 && m_rewardXPs[0] > 0) player.ForceGainExperience(m_rewardXPs[0]);

                        if (m_rewardCLXPs.Count > 0 && m_rewardCLXPs[0] > 0)
                            player.GainChampionExperience(m_rewardCLXPs[0], eXPSource.Quest);

                        if (m_rewardRPs.Count > 0 && m_rewardRPs[0] > 0) player.GainRealmPoints(m_rewardRPs[0]);

                        if (m_rewardBPs.Count > 0 && m_rewardBPs[0] > 0) player.GainBountyPoints(m_rewardBPs[0]);

                        if (m_rewardMoneys.Count > 0 && m_rewardMoneys[0] > 0)
                        {
                            player.AddMoney(m_rewardMoneys[0], "You are awarded {0}!");
                            InventoryLogging.LogInventoryAction("(QUEST;" + Name + ")", player,
                                eInventoryActionType.Quest, m_rewardMoneys[0]);
                        }
                        ...
                        if (add) player.QuestListFinished.Add(this);

                        player.Out.SendQuestListUpdate();

                        player.Out.SendMessage(
                            string.Format(LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "AbstractQuest.FinishQuest.Completed", Name)), eChatType.CT_ScreenCenter,
                            eChatLoc.CL_SystemWindow);
                        player.Out.SendMessage(
                            string.Format(LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "AbstractQuest.FinishQuest.Completed", Name)), eChatType.CT_Important,
                            eChatLoc.CL_SystemWindow);
                    }
                }
            ...
            {
                var charQuest = GetCharacterQuest(player, ID, true);
                var dq = new DataQuest(player, obj, DBDataQuest, charQuest);
                dq.Step = 1;
                player.AddQuest(dq);
                if (m_sourceTexts.Count > 0)
                {
                ...
                if (obj is GameNPC) UpdateQuestIndicator(obj as GameNPC, player);
            ...
            if (StartType == eStartType.SearchStart)
            {
                if (m_searchStartItemTemplate != "")
                    lock (player.Inventory)
                    {
                        ...
                        {
                            var item =
                                GameServer.Database.FindObjectByKey<ItemTemplate>(m_searchStartItemTemplate.Trim());
                            if (item == null)
                            {
                                var errorMsg = string.Format("SearchStart Item Template {0} not found in DB!",
                                    m_searchStartItemTemplate);
                        ...
                        {
                            player.Out.SendMessage("Your backpack is full, you can't start this quest!",
    ...
    protected virtual void TryTurnTo(GameObject obj, GamePlayer player)
    {
        ...
        if (npc != null) npc.TurnTo(player, 10000);
    }
    ...
    protected virtual void CheckOfferedQuestReceiveItem(GamePlayer player, GameObject obj, InventoryItem item)
    {
        ...
        if (StartType == eStartType.Collection && item.Id_nb.Equals(DBDataQuest.CollectItemTemplate))
        {
            ...
            {
                TryTurnTo(obj, player);
                var lvlXP = (player.ExperienceForNextLevel - player.ExperienceForCurrentLevel) / player.Level;
                if (item.Count == 1)
                {
                ...
                {
                    var RemainingTurnIns = MaxQuestCount - charQuest.Count;
                    var TotalInputCount = item.Count;
                    long rewardXP = 0;
                    //if we're turning in more items than are required
                    if (RemainingTurnIns < item.Count)
                    {
                        for (var i = 0; i < RemainingTurnIns; i++)
                        {
                            if (long.TryParse(DBDataQuest.RewardXP, out rewardXP))
                            {
                                if (rewardXP == 0)
                ...
                {
                    SendMessage(player, "You need to unstack these first.", 0, eChatType.CT_System,
    ...
    protected virtual void CheckOfferedQuestWhisper(GamePlayer player, GameLiving living, string text)
    {
        ...
        if (CheckQuestQualification(player) && DBDataQuest.StartType == (byte) eStartType.Standard &&
            DBDataQuest.AcceptText == text)
        {
            TryTurnTo(living, player);

            var charQuest = GetCharacterQuest(player, ID, true);
            var dq = new DataQuest(player, living, DBDataQuest, charQuest);
            dq.Step = 1;
            player.AddQuest(dq);
            if (m_sourceTexts.Count > 0)
                SendMessage(player, m_sourceTexts[0], 0, eChatType.CT_System, eChatLoc.CL_PopupWindow);
            ...
            if (living is GameNPC) UpdateQuestIndicator(living as GameNPC, player);
        }
    }
            ...
            switch (StepType)
            {
                ...
                case eStepType.InteractFinish:
                {
                    if (StartType == eStartType.RewardQuest)
                    {
                        ...
                        {
                            log.ErrorFormat("DataQuest Finish is RewardQuest but object {0} is not an NPC!",
                                obj.Name);
                        }
                    }
                    else
                                   ...
                                   && player.Level >= Level && player.Level <= MaxLevel)
        {
                ...
                ExecuteCustomQuestStep(player, Step, eStepCheckType.GiveItem))
            {
                switch (StepType)
                {
                    ...
                    case eStepType.Collect:
                    {
                        ...
                        if (!string.IsNullOrEmpty(TargetText))
                        {
                            if (obj.Realm == eRealm.None)
                                ...
                                SendMessage(m_questPlayer, TargetText, 0, eChatType.CT_System,
                                    eChatLoc.CL_PopupWindow);
                        }

                        if (AdvanceQuestStep(obj)) RemoveItem(obj, player, item, true);
                    }
                        break;
                    ...
                    case eStepType.CollectFinish:
                    {
                        if (FinishQuest(obj, true)) RemoveItem(obj, player, item, true);
                    }
                        break;
                }
            }
                     ...
                     ExecuteCustomQuestStep(player, Step, eStepCheckType.GiveItem))
            {
                ...
                {
                    ChatUtil.SendDebugMessage(player,
                        "Received item in StepItemTemplates but current step is not deliver or deliver finish.");
                }
            }
            else
            {
    ...
    protected virtual void OnLivingIsDying(GameLiving dying, GameObject killer, List<GamePlayer> playerKillers)
    {
        if (StartType == eStartType.KillComplete)
        {
            ...
            foreach (var player in playerKillers)
                if (CheckQuestQualification(player))
                {
                    var charQuest = GetCharacterQuest(player, ID, true);
                    ...
                    if (charQuest.Count < MaxQuestCount)
                        if (ExecuteCustomQuestStep(player, 0, eStepCheckType.Finish))
                        {
                            ...
                            if (m_finalRewards.Count > 0)
                                lock (player.Inventory)
                                {
                                    if (player.Inventory.IsSlotsFree(m_finalRewards.Count,
        ...
        if (command == eQuestCommand.Search)
            ...
            if (m_numSearchAreas > 0 && player == QuestPlayer)
                ...
                foreach (AbstractArea playerArea in player.CurrentAreas)
                    if (playerArea is QuestSearchArea && (playerArea as QuestSearchArea).DataQuest != null &&
                        (playerArea as QuestSearchArea).DataQuest.ID == ID)
                        if ((playerArea as QuestSearchArea).Step == Step)
                        {
                            StartQuestActionTimer(player, command, (playerArea as QuestSearchArea).SearchSeconds,
                                "Searching ...");
    ...
    protected override void QuestCommandCompleted(eQuestCommand command, GamePlayer player)
    {
        if (command == eQuestCommand.Search && QuestPlayer == player)
        {
            if (StepType == eStepType.Search)
            {
                if (AdvanceQuestStep() == false)
                    SendMessage(QuestPlayer, "You fail to find anything!", 0, eChatType.CT_System,
                        eChatLoc.CL_SystemWindow);
            }
            else if (StepType == eStepType.SearchFinish)
            {
        ...
        if (command == eQuestCommand.SearchStart) CheckOfferQuest(player, null);
    }
    ...
    public virtual bool FinishQuest(GameObject obj, bool checkCustomStep)
    {
        ...
        {
            if (m_questPlayer.Inventory.IsSlotsFree(m_finalRewards.Count + m_optionalRewardChoice.Count,
                    eInventorySlot.FirstBackpack, eInventorySlot.LastBackpack))
            {
                long rewardXP = 0;
                long rewardRP = 0;
                ...
                if (StartType != eStartType.RewardQuest)
                {
                    if (m_rewardXPs.Count > 0) rewardXP = m_rewardXPs[lastStep - 1];

                    if (m_rewardRPs.Count > 0) rewardRP = m_rewardRPs[lastStep - 1];
                    ...
                    foreach (var item in m_optionalRewardChoice)
                        if (item != null)
                            GiveItem(m_questPlayer, item);
                ...
                if (StartType == eStartType.RewardQuest)
                {
                    if (m_rewardXPs.Count > 0) rewardXP = m_rewardXPs[0];

                    if (m_rewardRPs.Count > 0) rewardRP = m_rewardRPs[0];
            ...
            {
                SendMessage(m_questPlayer, "Your inventory does not have enough space to finish this quest!", 0,
                    eChatType.CT_System, eChatLoc.CL_PopupWindow);
                return false;
            }
        }
        ...
        m_questPlayer.Out.SendMessage(
            string.Format(LanguageMgr.GetTranslation(m_questPlayer.Client, "AbstractQuest.FinishQuest.Completed",
        ...
        {
            foreach (var q in m_questPlayer.QuestListFinished)
                if (q is DataQuest && (q as DataQuest).ID == ID)
                {
        ...
        if (add) m_questPlayer.QuestListFinished.Add(this);

        m_questPlayer.Out.SendQuestListUpdate();

        if (StartType == eStartType.RewardQuest) m_questPlayer.Out.SendSoundEffect(11, 0, 0, 0, 0, 0);

        if (!string.IsNullOrEmpty(m_dataQuest
                .FinishText)) // Give users option to have 'finish' text with rewardquest too
        {
        ...
        if (obj != null && obj is GameNPC) UpdateQuestIndicator(obj as GameNPC, m_questPlayer);

        if (m_startNPC != null) UpdateQuestIndicator(m_startNPC, m_questPlayer);
    ...
    public override void AbortQuest()
    {
        ...
        if (m_questPlayer.QuestList.Contains(this)) m_questPlayer.QuestList.Remove(this);

        if (m_charQuest.Count == 0)
        {
            if (m_questPlayer.QuestListFinished.Contains(this)) m_questPlayer.QuestListFinished.Remove(this);

            DeleteFromDatabase();
        }

        m_questPlayer.Out.SendQuestListUpdate();
        m_questPlayer.Out.SendMessage(LanguageMgr.GetTranslation(m_questPlayer.Client, "AbstractQuest.AbortQuest"),
            eChatType.CT_System, eChatLoc.CL_SystemWindow);
    ...
    public override void DeleteFromDatabase()
    {
        ...
        var charQuest = GameServer.Database.FindObjectByKey<CharacterXDataQuest>(m_charQuest.ID);
        if (charQuest != null) GameServer.Database.DeleteObject(charQuest);
    }
}