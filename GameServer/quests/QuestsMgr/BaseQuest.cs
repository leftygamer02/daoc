...
public abstract class BaseQuest : AbstractQuest
{
    ...
    public Queue m_animEmoteTeleportTimerQueue = new();
    public Queue m_animEmoteObjectQueue = new();

    public Queue m_animSpellTeleportTimerQueue = new();
    public Queue m_animSpellObjectQueue = new();

    public Queue m_portTeleportTimerQueue = new();
    public Queue m_portObjectQueue = new();
    public Queue m_portDestinationQueue = new();
    ...
    {
        if (questParts != null)
            for (var i = questParts.Count - 1; i >= 0; i--)
    ...
    protected static void UnRegisterBehaviour(QuestBehaviour questPart)
    {
        ...
        foreach (var trigger in questPart.Triggers) trigger.Unregister();
    }
    ...
    public static void AddBehaviour(QuestBehaviour questPart)
    {
        ...
        questPart.ID =
            questParts.Count; // fake id but ids only have to be unique quest wide its enough to use the number in the list as id.
    }
    ...
    public override void Notify(DOLEvent e, object sender, EventArgs args)
    {
        ...
        {
            var iArgs = args as InteractWithEventArgs;
        ...
        foreach (QuestBehaviour questPart in questParts) questPart.Notify(e, sender, args);
    }
    ...
    protected virtual int MakeAnimSpellSequence(ECSGameTimer callingTimer)
    {
        if (m_animSpellTeleportTimerQueue.Count > 0)
        {
            ...
            var animObject = (GameLiving) m_animSpellObjectQueue.Dequeue();
    ...
    protected virtual int MakeAnimEmoteSequence(ECSGameTimer callingTimer)
    {
        if (m_animEmoteTeleportTimerQueue.Count > 0)
        {
            ...
            var animObject = (GameLiving) m_animEmoteObjectQueue.Dequeue();
            foreach (GamePlayer player in animObject.GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE))
        ...
        int fuzzyLocation)
    {
        ...
        m_animSpellTeleportTimerQueue.Enqueue(new ECSGameTimer(caster,
            new ECSGameTimer.ECSTimerCallback(MakeAnimSpellSequence), (int) delay));

        m_animEmoteObjectQueue.Enqueue(target);
        m_animEmoteTeleportTimerQueue.Enqueue(new ECSGameTimer(target,
            new ECSGameTimer.ECSTimerCallback(MakeAnimEmoteSequence), (int) delay + 2000));
        ...
        m_portTeleportTimerQueue.Enqueue(new ECSGameTimer(target,
            new ECSGameTimer.ECSTimerCallback(MakePortSequence), (int) delay + 3000));

        if (location.Name != null)
            m_questPlayer.Out.SendMessage(
                LanguageMgr.GetTranslation(m_questPlayer.Client, "BaseQuest.TeleportTo.Text1", target.Name,
                    location.Name), eChatType.CT_System, eChatLoc.CL_SystemWindow);
    }
    ...
    protected virtual int MakePortSequence(ECSGameTimer callingTimer)
    {
        if (m_portTeleportTimerQueue.Count > 0)
        {
            ...
            var gameObject = (GameObject) m_portObjectQueue.Dequeue();
            var location = (GameLocation) m_portDestinationQueue.Dequeue();
    ...
    private Dictionary<int, QuestStepInteraction> m_interactions = new();
    private const int INTERACT_ITEM_RESPAWN_SECONDS = 120;
    ...
    protected void AddInteractStep(int step, string objectName, int numRequired, ItemTemplate itemResult,
        string interactText)
    {
        try
        {
            var info = new QuestStepInteraction();
