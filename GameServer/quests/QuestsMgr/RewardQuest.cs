...
public class RewardQuest : BaseQuest
{
    ...
    {
        var goal = new QuestGoal("none", this, description, type, m_goals.Count + 1, targetNumber, questItem);
        m_goals.Add(goal);
        return goal;
    }
    ...
    /// <param name="questItem"></param>
    /// <returns></returns>
    protected QuestGoal AddGoal(string id, string description, QuestGoal.GoalType type, int targetNumber,
        ItemTemplate questItem)
    {
        var goal = new QuestGoal(id, this, description, type, m_goals.Count + 1, targetNumber, questItem);
        m_goals.Add(goal);
        return goal;
    }
    ...
    /// </summary>
    public List<QuestGoal> Goals => m_goals;
    ...
    public override void Notify(DOLEvent e, object sender, EventArgs args)
    {
        ...
        if (e == GamePlayerEvent.QuestRewardChosen)
        {
            ...
            {
                QuestPlayer.Out.SendMessage(LanguageMgr.GetTranslation(QuestPlayer.Client, "RewardQuest.Notify"),
    ...
    public override void OnQuestAssigned(GamePlayer player)
    {
        player.Out.SendMessage(
            string.Format(LanguageMgr.GetTranslation(player.Client.Account.Language, "RewardQuest.OnQuestAssigned",
    ...
    {
        var inventorySpaceRequired = Rewards.BasicItems.Count + Rewards.ChosenItems.Count;

        if (QuestPlayer.Inventory.IsSlotsFree(inventorySpaceRequired, eInventorySlot.FirstBackpack,
                eInventorySlot.LastBackpack))
        {
            ...
            InventoryLogging.LogInventoryAction("(QUEST;" + Name + ")", QuestPlayer, eInventoryActionType.Quest,
                Rewards.Money);
            ...
            foreach (var basicReward in Rewards.BasicItems) GiveItem(QuestPlayer, basicReward);

            foreach (var optionalReward in Rewards.ChosenItems) GiveItem(QuestPlayer, optionalReward);
        ...
        else
        {
            QuestPlayer.Out.SendMessage(
                string.Format("Your inventory is full, you need {0} free slot(s) to complete this quest.",
                    inventorySpaceRequired), eChatType.CT_System, eChatLoc.CL_SystemWindow);
    ...
    public class QuestGoal
    {
        ...
        public enum GoalType
        {
            KillTask = 3,
            ScoutMission = 5
        }; // These are just a hunch for now.
        ...
        public QuestGoal(string id, RewardQuest quest, string description, GoalType type, int index, int target,
            ItemTemplate questItem)
        {
            m_id = id;
            m_quest = quest;
        ...
        public string Description
        {
            get
            {
                if (m_quest.QuestPlayer != null)
                    return string.Format(LanguageMgr.GetTranslation((m_quest.QuestPlayer as GamePlayer).Client,
                        "RewardQuest.Description", m_description, Current, Target));
        ...
        /// </summary>
        public GoalType Type => m_goalType;
        ...
        {
            get => Current > 0 ? m_questItem : null;
        ...
        protected int Current
        {
            get
            {
                if (m_quest.QuestPlayer == null)
                    return m_current;
                var propertyValue = m_quest.GetCustomProperty(string.Format("goal{0}Current", m_index));
        ...
        protected int Target
        {
            get
            {
                if (m_quest.QuestPlayer == null)
                    return m_current;
                var propertyValue = m_quest.GetCustomProperty(string.Format("goal{0}Target", m_index));
                if (propertyValue == null)
                {
                    Target = 0;
                    return Target;
        ...
        public void Advance()
        {
            if (Current < Target)
            {
                ...
                if (IsAchieved)
                {
                    ...
                    var done = true;
                    foreach (var goal in m_quest.Goals)
                        done &= goal.IsAchieved;

                    if (done && m_quest.QuestGiver.IsWithinRadius(m_quest.QuestPlayer,
                            WorldMgr.VISIBILITY_DISTANCE))
                        m_quest.QuestPlayer.Out.SendNPCsQuestEffect(m_quest.QuestGiver,
                            m_quest.QuestGiver.GetQuestIndicator(m_quest.QuestPlayer));
                }
            }
        }
        ...
        public int ZoneID1 => m_zoneID1;

        public int XOffset1 => m_xOffset1;

        public int YOffset1 => m_yOffset1;

        public int ZoneID2 => m_zoneID2;

        public int XOffset2 => m_xOffset2;

        public int YOffset2 => m_yOffset2;
    }
    ...
    public class QuestRewards
    {
        ...
        public int MoneyPercent
        {
            ...
            set
            {
                if (value >= 0 && value <= 100) m_moneyPercent = value;
            }
        }
        ...
        /// </summary>
        public long Money => (long) (m_maxCopperForLevel[m_quest.Level] * MoneyPercent / 100 + GiveGold * 10000);
        ...
        /// </summary>
        public List<ItemTemplate> BasicItems => m_basicItems;

        /// <summary>
        /// List of optional item rewards.
        /// </summary>
        public List<ItemTemplate> OptionalItems => m_optionalItems;

        /// <summary>
        /// List of optional rewards the player actually picked.
        /// </summary>
        public List<ItemTemplate> ChosenItems => m_chosenItems;
