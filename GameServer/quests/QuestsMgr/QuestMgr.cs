...
public sealed class QuestMgr
{
    ...
    public static bool Init()
    {
        ...
        foreach (var assembly in ScriptMgr.GameServerScripts)
    ...
    public static GameLiving ResolveLiving(object identifier, GameLiving defaultLiving, bool lookupDB)
    {
        ...
        {
            var tempID = Convert.ToString(identifier);

            // TODO: Dirty Hack this should be done better
            var mob = DOLDB<Mob>.SelectObject(DB.Column("Mob_ID").IsEqualTo(tempID)
                .Or(DB.Column("Name").IsEqualTo(tempID)));

            var livings = WorldMgr.GetNPCsByName(mob.Name, (eRealm) mob.Realm);
            ...
            else if (livings.Length > 1)
            {
                if (log.IsWarnEnabled)
                    log.Warn("Found more than one living with name :" + tempID + " in " +
                             (lookupDB ? "Database" : "WorldMgr"));
            }
            else
            {
                if (log.IsWarnEnabled)
                    log.Warn("Couldn't find GameLiving with id or name:" + tempID + " in " +
                             (lookupDB ? "Database" : "WorldMgr"));
            }
        }
    ...
    public static GameNPC ResolveNPC(object identifier, GameNPC defaultNPC, bool lookupDB)
    {
        ...
        if (identifier is string || identifier is int)
        {
            ...
            var npcs = WorldMgr.GetNPCsByName(mob.Name, (eRealm) mob.Realm);
...
//            ushort typeId =(ushort) type.GetHashCode();
        var typeId = (ushort) (m_questTypeMap.Count + 1);
...
//        }
    public static ushort GetIDForQuestType(Type questType)
    {
        var questEnumerator = m_questTypeMap.GetEnumerator();
        ...
        if (CanGiveQuest(questType, player, source) > 0)
        {
            if (questType.IsSubclassOf(typeof(RewardQuest)))
            {
                ...
                foreach (var asm in ScriptMgr.Scripts)
                {
                    try
                    {
                        rquest = (RewardQuest) asm.CreateInstance(questType.FullName, false,
                            BindingFlags.CreateInstance, null, new object[] { }, null, null);
                    }
                    catch (Exception e)
                    {
                        if (log.IsErrorEnabled)
    ...
    {
        if (source != null && CanGiveQuest(questType, player, source as GameNPC) <= 0)
            return false;
