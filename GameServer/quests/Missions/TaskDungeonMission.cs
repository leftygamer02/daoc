...
public class TaskDungeonMission : AbstractMission
{
    ...
    public eDungeonType DungeonType => m_dungeonType;

    private eTDMissionType m_missionType;

    public eTDMissionType TDMissionType => m_missionType;

    private TaskDungeonInstance m_taskRegion;

    public TaskDungeonInstance TaskRegion => m_taskRegion;
        ...
        : base(owner)
    {
        ...
        var rid = GetRegionFromLevel(player.Level, player.Realm, dungeonType);

        var instance =
            (TaskDungeonInstance) WorldMgr.CreateInstance(rid, typeof(TaskDungeonInstance));
        ...
        var keyname = "TaskDungeon" + rid + ".1"; //TODO; variations, eg .2, .3 etc.
    ...
    public override string Description
    {
        get
        {
            ...
            {
                case eTDMissionType.Boss:
                    return "You have been asked to kill " + m_bossName + " in the nearby caves.";
                case eTDMissionType.Specific:
                    return "You have been asked to kill " + m_total + " " + m_targetName + " in the nearby caves.";
                case eTDMissionType.Clear:
                {
                    ...
                    {
                        var test = m_total - m_current == 1;
                        return "You have been asked to clear the nearby caves. " + (m_total - m_current) +
                               " creature" + (test == true ? "" : "s") + " left!";
                    }
                }
    ...
    {
        if (m_owner is GamePlayer)
            (m_owner as GamePlayer).Out.SendMessage("Mission Complete", eChatType.CT_ScreenCenter,
                eChatLoc.CL_ChatWindow);
        else if (m_owner is Group)
            foreach (var player in (m_owner as Group).GetPlayersInTheGroup())
...
public class TaskDungeonInstance : Instance
{
    ...
    public override void OnPlayerEnterInstance(GamePlayer player)
    {
        ...
        player.Out.SendMessage("You have entered " + Description + ".", eChatType.CT_Important,
            eChatLoc.CL_SystemWindow);
        player.Out.SendMessage("This instance is currently level " + m_level + ".", eChatType.CT_Important,
            eChatLoc.CL_SystemWindow);
    }
    ...
    public void UpdateInstanceLevel()
    {
        ...
        if (m_level > 0)
        {
            ...
            foreach (var client in WorldMgr.GetClientsOfRegion(ID))
                if (client != null)
                    client.Out.SendMessage("This instance is now level " + m_level, eChatType.CT_Important,
                        eChatLoc.CL_SystemWindow);
        }
    }
