...
public class WildWilderness : BaseQuest
{
    ...
    public override string Name => QUEST_TITLE;
    ...
    public static void ScriptLoaded(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var npcs = WorldMgr.GetNPCsByName(QUEST_GIVER_NAME, eRealm.Hibernia);
        ...
        if (_lungerTail == null)
        {
            ...
            if (log.IsWarnEnabled) log.Warn("Could not find " + _lungerTail.Name + ", creating it ...");
        ...
        if (log.IsInfoEnabled) log.Info($"Quest {QUEST_TITLE} initialized");
    }
    ...
    protected static void SubscribeQuest(DOLEvent e, object sender, EventArgs args)
    {
        ...
        if (qargs.QuestID != QuestMgr.GetIDForQuestType(typeof(WildWilderness))) return;

        if (e == GamePlayerEvent.AcceptQuest)
            CheckPlayerAcceptQuest(qargs.Player, 0x01);
        else if (e == GamePlayerEvent.DeclineQuest) CheckPlayerAcceptQuest(qargs.Player, 0x00);
    }
    ...
    private static void TalkToMiach(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var quest = player.IsDoingQuest(typeof(WildWilderness)) as WildWilderness;
        ...
        if (e == GameObjectEvent.Interact)
        {
            if (quest != null)
                switch (quest.Step)
                {
                    case 1:
                        _miach.SayTo(player,
                            $"Great! While you are at it, bring one of the lunger tails to Resalg. He will pay you well for it. He is trying to create some potions and " +
                            $"I told him I'd acquire a [tail] for him.");
                        break;
                    case 2:
                        _miach.SayTo(player,
                            $"Hello {player.CharacterClass.Name}, find those lungers and bring a tail to Resalg!");
                        break;
                    case 3:
                ...
                _miach.SayTo(player,
                    "There was once a time when all of Hy Brasil was tame. There was once a time when nature's beasts were tame. Now, though, the creatures we once had no reason to fear have become aggressive. They kill not just for food, but for pleasure. They no longer only kill their normal prey. Now, they [hunt] us.");
        }
    ...
    private static void TalkToResalg(DOLEvent e, object sender, EventArgs args)
    {
            ...
            if (quest != null)
            {
                switch (quest.Step)
                {
                    ...
                    case 3:
                        ...
                        _resalg.SayTo(player,
                            "Ah! You've brought me a lunger tail! Now, I'm not sure if this is going to work, " +
        ...
        else if (e == GameObjectEvent.ReceiveItem)
        {
            ...
            _resalg.SayTo(player,
                "Ah! You've brought me a lunger tail! Now, I'm not sure if this is going to work, but I think it just might, yes, it just might. You see, I'm working on a special [formula].");
            RemoveItem(player, _lungerTail);
        }
    }
    ...
    private static void CheckPlayerAcceptQuest(GamePlayer player, byte response)
    {
        ...
        else
        {
            ...
            SendReply(player,
                "Great! While you are at it, bring one of the lunger tails to Resalg. He will pay you well for it. He is trying to create some potion. I told him I'd acquire a [tail] for him.");
        }
    }

    public override bool CheckQuestQualification(GamePlayer player)
    {
        if (player.IsDoingQuest(typeof(WildWilderness)) != null) return true;

        if (player.Level < MIN_LEVEL || player.Level > MAX_LEVEL) return false;
