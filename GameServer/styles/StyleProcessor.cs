...
public class StyleProcessor
{
    ...
    public static bool CanUseStyle(GameLiving living, Style style, InventoryItem weapon)
    {
        ...
        lock (living)
        {
            var target = living.TargetObject as GameLiving;
            if (target == null) return false;

            //Required attack result
            var requiredAttackResult = eAttackResult.Any;
            switch (style.AttackResultRequirement)
            {
                case Style.eAttackResultRequirement.Any:
                    requiredAttackResult = eAttackResult.Any;
                    break;
                case Style.eAttackResultRequirement.Block:
                    requiredAttackResult = eAttackResult.Blocked;
                    break;
                case Style.eAttackResultRequirement.Evade:
                    requiredAttackResult = eAttackResult.Evaded;
                    break;
                ...
                case Style.eAttackResultRequirement.Hit:
                    requiredAttackResult = eAttackResult.HitUnstyled;
                    break;
                case Style.eAttackResultRequirement.Style:
                    requiredAttackResult = eAttackResult.HitStyle;
                    break;
                case Style.eAttackResultRequirement.Miss:
                    requiredAttackResult = eAttackResult.Missed;
                    break;
                case Style.eAttackResultRequirement.Parry:
                    requiredAttackResult = eAttackResult.Parried;
                    break;
            }

            var lastAD =
                (AttackData) living.TempProperties.getProperty<object>(GameLiving.LAST_ATTACK_DATA, null);
            ...
            switch (style.OpeningRequirementType)
            {
                case Style.eOpening.Offensive:
                    ...
                    if (style.OpeningRequirementValue != 0
                        && (lastAD == null
                            ...
                            /*|| lastAD.Target != target*/
                        )) // style chains are *NOT* possible only on the same target
                        //DOLConsole.WriteLine("Offensive: Opening Requirement style needed failed!("+style.OpeningRequirementValue+")");
                        return false;

                    //Last attack result
                    var lastRes = lastAD != null ? lastAD.AttackResult : eAttackResult.Any;
                ...
                case Style.eOpening.Defensive:
                    var targetsLastAD =
                        (AttackData) target.TempProperties.getProperty<object>(GameLiving.LAST_ATTACK_DATA, null);

                    //Last attack result
                    if (requiredAttackResult != eAttackResult.Any)
                    {
                        if (targetsLastAD == null || targetsLastAD.Target != living) return false;

                        if (requiredAttackResult != eAttackResult.HitStyle &&
                            targetsLastAD.AttackResult != requiredAttackResult)
                ...
                case Style.eOpening.Positional:
                    ...
                    if ((target is GameKeepComponent || target is GameKeepDoor) &&
                        (Style.eOpeningPosition) style.OpeningRequirementValue != Style.eOpeningPosition.Front)
                        return false;

                    // get players angle on target
                    var angle = target.GetAngle(living);
        ...
        lock (living)
        {
                ...
                if (player != null)
                    player.Out.SendMessage(
                        LanguageMgr.GetTranslation(player.Client.Account.Language,
                            "StyleProcessor.TryToUseStyle.CantCombatMode"), eChatType.CT_YouHit,
            ...
            if (living.IsDisarmed)
            {
                if (living is GamePlayer)
                    (living as GamePlayer).Out.SendMessage("You are disarmed and cannot attack!",
            ...
            if (living.ActiveWeaponSlot == eActiveWeaponSlot.Distance)
            {
                if (player != null)
                    player.Out.SendMessage(
                        LanguageMgr.GetTranslation(player.Client.Account.Language,
                            "StyleProcessor.TryToUseStyle.CantMeleeCombat"), eChatType.CT_YouHit,
                        eChatLoc.CL_SystemWindow);
                return;
            }
            ...
            if (living.attackComponent.AttackState == false ||
                EffectListService.GetEffectOnTarget(living, eEffect.Engage) != null)
            ...
            if (living.TargetObject == null)
            {
                if (player != null)
                    player.Out.SendMessage(
                        LanguageMgr.GetTranslation(player.Client.Account.Language,
                            "StyleProcessor.TryToUseStyle.MustHaveTarget"), eChatType.CT_System,
                        eChatLoc.CL_SystemWindow);
                return;
            }

            var weapon = style.WeaponTypeRequirement == (int) eObjectType.Shield
                ? living.Inventory.GetItem(eInventorySlot.LeftHandWeapon)
                : living.ActiveWeapon;
            ...
            if (!CheckWeaponType(style, living, weapon))
            {
                if (player != null)
                {
                    if (style.WeaponTypeRequirement == Style.SpecialWeaponType.DualWield)
                        player.Out.SendMessage(
                            LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "StyleProcessor.TryToUseStyle.DualWielding"), eChatType.CT_System,
                            eChatLoc.CL_SystemWindow);
                    else
                        player.Out.SendMessage(
                            LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "StyleProcessor.TryToUseStyle.StyleRequires", style.GetRequiredWeaponName()),
            ...
            {
                var fatCost = CalculateEnduranceCost(player, style, weapon.SPD_ABS);
                if (player.Endurance < fatCost)
                {
                    player.Out.SendMessage(
                        LanguageMgr.GetTranslation(player.Client.Account.Language,
                            "StyleProcessor.TryToUseStyle.Fatigued"), eChatType.CT_System,
                        eChatLoc.CL_SystemWindow);
                    return;
                }
            }
            ...
            if (player != null)
            {
                ...
                if (style.OpeningRequirementType == Style.eOpening.Offensive &&
                    ...
                    preRequireStyle =
                        SkillBase.GetStyleByID(style.OpeningRequirementValue, player.CharacterClass.ID);
                ...
                if (player.styleComponent.NextCombatStyle == null)
                {
                    ...
                    {
                        var lastAD =
                            (AttackData) living.TempProperties.getProperty<object>(GameLiving.LAST_ATTACK_DATA,
                                null);
                        if (lastAD == null
                            || lastAD.AttackResult != eAttackResult.HitStyle
                            || lastAD.Style == null
                    ...
                    player.Out.SendMessage(
                        LanguageMgr.GetTranslation(player.Client.Account.Language,
                            "StyleProcessor.TryToUseStyle.PreparePerform", style.Name), eChatType.CT_System,
                        eChatLoc.CL_SystemWindow);
                    ...
                    if (living.IsEngaging)
                    {
                        ...
                        var effect =
                            (EngageECSGameEffect) EffectListService.GetEffectOnTarget(living, eEffect.Engage);
                        if (effect != null)
                            effect.Cancel(false);
                    }
                ...
                else
                {
                    ...
                    if (player.styleComponent.NextCombatBackupStyle != null)
                    {
                        ...
                        player.Out.SendMessage(
                            LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "StyleProcessor.TryToUseStyle.AlreadySelectedStyles"), eChatType.CT_System,
                            eChatLoc.CL_SystemWindow);
                    }
                    else
                    {
                        ...
                        if (player.styleComponent.NextCombatStyle.ID == style.ID)
                        {
                            if (player.styleComponent.CancelStyle)
                            {
                                ...
                                player.Out.SendMessage(
                                    LanguageMgr.GetTranslation(player.Client.Account.Language,
                                        "StyleProcessor.TryToUseStyle.NoLongerPreparing",
                                        player.styleComponent.NextCombatStyle.Name), eChatType.CT_System,
                                    eChatLoc.CL_SystemWindow);
                            ...
                            else
                            {
                                player.Out.SendMessage(
                                    LanguageMgr.GetTranslation(player.Client.Account.Language,
                                        "StyleProcessor.TryToUseStyle.AlreadyPreparing"), eChatType.CT_System,
                                    eChatLoc.CL_SystemWindow);
                            }
                        }
                        else
                        {
                            ...
                            {
                                var lastAD =
                                    (AttackData) living.TempProperties.getProperty<object>(
                                        GameLiving.LAST_ATTACK_DATA, null);
                                if (lastAD == null
                                    || lastAD.AttackResult != eAttackResult.HitStyle
                                    || lastAD.Style == null
                                    || lastAD.Style.ID != style.OpeningRequirementValue)
                            ...
                            player.Out.SendMessage(
                                LanguageMgr.GetTranslation(player.Client.Account.Language,
                                    "StyleProcessor.TryToUseStyle.BackupStyle", style.Name,
                                    player.styleComponent.NextCombatStyle.Name), eChatType.CT_System,
                                eChatLoc.CL_SystemWindow);
                        }
                    }
        ...
        lock (living)
        {
            ...
            if (weapon != null && weapon.Object_Type == (int) eObjectType.Shield)
                attackData.AnimationId =
                    weapon.Hand != 1 ? attackData.Style.Icon : attackData.Style.TwoHandAnimation; // 2h shield?
            ...
            if (!CanUseStyle(living, attackData.Style, weapon))
            {
                if (player != null)
                {
                    ...
                    player.Out.SendMessage(
                        LanguageMgr.GetTranslation(player.Client.Account.Language,
                            "StyleProcessor.ExecuteStyle.ExecuteFail", attackData.Style.Name), eChatType.CT_YouHit,
                        eChatLoc.CL_SystemWindow);
                }
            ...
            else
            {
                ...
                var staticGrowth =
                    attackData.Style
                        .StealthRequirement; //static growth is not a function of (effective) weapon speed
                var absorbRatio =
                    attackData.Damage /
                    living.attackComponent.UnstyledDamageCap(weapon); //scaling factor for style damage
                var effectiveWeaponSpeed = living.attackComponent.AttackSpeed(weapon) * 0.001;
                var styleGrowth = Math.Max(0,
                    attackData.Style.GrowthOffset + attackData.Style.GrowthRate *
                    living.GetModifiedSpecLevel(attackData.Style.Spec));
                var styleDamageBonus = living.GetModified(eProperty.StyleDamage) * 0.01 - 1;

                var talyGrowth = attackData.Style.GrowthRate;
                double talySpec = living.GetModifiedSpecLevel(attackData.Style.Spec);
                var talySpeed = living.attackComponent.AttackSpeed(weapon) * 0.001;
                var talyCap = living.attackComponent.UnstyledDamageCap(weapon);
                ...
                if (staticGrowth)
                {
                    ...
                    var initialDamage = attackData.StyleDamage;
                    ...
                    attackData.StyleDamage = (int) (attackData.StyleDamage * (1.0 - Math.Min(0.85,
                        attackData.Target.GetArmorAbsorb(attackData.ArmorHitLocation))));
                    attackData.StyleDamage -= (int) (attackData.StyleDamage *
                                                     (attackData.Target.GetResist(attackData.DamageType) +
                                                      SkillBase.GetArmorResist(armor, attackData.DamageType)) *
                                                     0.01);
                    attackData.StyleDamage -= (int) (attackData.StyleDamage *
                                                     attackData.Target.GetDamageResist(
                                                         attackData.Target.GetResistTypeForDamage(attackData
                                                             .DamageType)) * 0.01);
                }
                else
                {
                    attackData.StyleDamage =
                        (int) (talyGrowth * talySpec * talySpeed / talyCap * attackData.Damage);
                }
                ...
                {
                    absorb = (int) Math.Floor((double) attackData.StyleDamage *
                                              ((double) attackData.Target.GetModified(eProperty.StyleAbsorb) /
                                               100));
                ...
                attackData.Modifier = (int) (attackData.Damage * attackData.Modifier /
                                             (double) (attackData.Damage - attackData.StyleDamage +
                                                       attackData.Modifier));
                ...
                if (player != null)
                {
                    ...
                    if (absorb > 0)
                    {
                        player.Out.SendMessage("A barrier absorbs " + absorb + " damage!", eChatType.CT_YouHit,
                            eChatLoc.CL_SystemWindow);
                        if (living is GamePlayer)
                            (living as GamePlayer).Out.SendMessage("A barrier absorbs " + absorb + " damage!",
                                eChatType.CT_YouHit, eChatLoc.CL_SystemWindow);
                    }
                }
                ...
                if (attackData.Style.Procs.Count > 0)
                {
                    ...
                    {
                        var procsToExecute = new List<Tuple<Spell, int, int>>();
                        var onlyExecuteClassSpecific = false;

                        foreach (var proc in attackData.Style.Procs)
                        ...
                        foreach (var procToExecute in procsToExecute)
                        {
                            ...
                            if (Util.Chance(procToExecute.Item3))
                            {
                                ...
                                if (effect != null)
                                {
                                    ...
                                    if ((attackData.Style.OpeningRequirementType == Style.eOpening.Offensive &&
                                         attackData.Style.OpeningRequirementValue > 0)
                    ...
                    else
                    {
                        ...
                        var random = Util.Random(attackData.Style.Procs.Count - 1);
                        //effect could be null if the SpellID is bigger than ushort
                        effect = CreateMagicEffect(living, attackData.Target,
                            attackData.Style.Procs[random].Item1.ID);
                        if (effect != null)
                        {
                            attackData.StyleEffects.Add(effect);
                            if ((attackData.Style.OpeningRequirementType == Style.eOpening.Offensive &&
                ...
                if (weapon != null)
                    attackData.AnimationId =
                        weapon.Hand != 1
                            ...
                            : attackData.Style.TwoHandAnimation; // special animation for two-hand
                else if (living.Inventory != null)
                    attackData.AnimationId = living.Inventory.GetItem(eInventorySlot.RightHandWeapon) != null
                        ? attackData.Style.Icon
                        : attackData.Style.TwoHandAnimation; // special animation for two-hand
                else
    ...
    public static int CalculateEnduranceCost(GameLiving living, Style style, int weaponSpd)
    {
        ...
        var ValhallasBlessing = living.EffectList.GetOfType<ValhallasBlessingEffect>();
        ...
        if (style.Spec == Specs.Battlemaster)
            return Math.Max(1,
                (int) Math.Ceiling(30 * style.EnduranceCost / 40 *
                                   living.GetModified(eProperty.FatigueConsumption) * 0.01));

        var fatCost = weaponSpd * style.EnduranceCost / 40;
        if (weaponSpd < 40)
            fatCost++;
    ...
    protected static bool CheckWeaponType(Style style, GameLiving living, InventoryItem weapon)
    {
        ...
        switch (style.WeaponTypeRequirement)
        {
            case Style.SpecialWeaponType.DualWield:
                ...
                var rightHand = player.ActiveWeapon;
                var leftHand = player.Inventory.GetItem(eInventorySlot.LeftHandWeapon);

                if (rightHand == null || leftHand == null ||
                    (rightHand.Item_Type != Slot.RIGHTHAND && rightHand.Item_Type != Slot.LEFTHAND))
                    return false;

                if (style.Spec == Specs.HandToHand && (rightHand.Object_Type != (int) eObjectType.HandToHand ||
                                                       leftHand.Object_Type != (int) eObjectType.HandToHand))
                    return false;
                else if (style.Spec == Specs.Fist_Wraps && (rightHand.Object_Type != (int) eObjectType.FistWraps ||
                                                            leftHand.Object_Type != (int) eObjectType.FistWraps))
                    return false;
            ...
            default:
                ...
                if (style.WeaponTypeRequirement == (int) eObjectType.Shield
                    && (player.ActiveWeapon == null || (player.ActiveWeapon.Item_Type != Slot.RIGHTHAND &&
    ...
    {
        var styleLine = SkillBase.GetSpellLine(GlobalSpellsLines.Combat_Styles_Effect);
        if (styleLine == null || target == null) return null;

        var spells = SkillBase.GetSpellList(styleLine.KeyName);
        ...
        var spellHandler = ScriptMgr.CreateSpellHandler(caster, styleSpell, styleLine);
        if (spellHandler == null && styleSpell != null && caster is GamePlayer)
        {
            ((GamePlayer) caster).Out.SendMessage(
                styleSpell.Name + " not implemented yet (" + styleSpell.SpellType + ")", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            return null;
        }
        ...
        if ((target is GameKeepComponent || target is GameKeepDoor) &&
            spellHandler?.HasPositiveEffect == false) return null;
    ...
    {
        delveInfo.Add(LanguageMgr.GetTranslation(player.Client.Account.Language,
            "DetailDisplayHandler.HandlePacket.WeaponType", style.GetRequiredWeaponName()));
        var temp = LanguageMgr.GetTranslation(player.Client.Account.Language,
            "DetailDisplayHandler.HandlePacket.Opening") + " ";
        if (Style.eOpening.Offensive == style.OpeningRequirementType)
        {
            ...
            {
                case Style.eAttackResultRequirement.Hit:
                    temp += LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "DetailDisplayHandler.HandlePacket.YouHit");
                    break;
                case Style.eAttackResultRequirement.Miss:
                    temp += LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "DetailDisplayHandler.HandlePacket.YouMiss");
                    break;
                case Style.eAttackResultRequirement.Parry:
                    temp += LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "DetailDisplayHandler.HandlePacket.TargetParrys");
                    break;
                case Style.eAttackResultRequirement.Block:
                    temp += LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "DetailDisplayHandler.HandlePacket.TargetBlocks");
                    break;
                case Style.eAttackResultRequirement.Evade:
                    temp += LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "DetailDisplayHandler.HandlePacket.TargetEvades");
                    break;
                case Style.eAttackResultRequirement.Fumble:
                    temp += LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "DetailDisplayHandler.HandlePacket.YouFumble");
                    break;
                case Style.eAttackResultRequirement.Style:
                    var reqStyle =
                        SkillBase.GetStyleByID(style.OpeningRequirementValue, player.CharacterClass.ID);
                    if (reqStyle == null) reqStyle = SkillBase.GetStyleByID(style.OpeningRequirementValue, 0);

                    temp = LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "DetailDisplayHandler.HandlePacket.OpeningStyle") + " ";
                    if (reqStyle == null)
                ...
                default:
                    temp += LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "DetailDisplayHandler.HandlePacket.Any");
                    break;
            }
        }
        else if (Style.eOpening.Defensive == style.OpeningRequirementType)
        {
            ...
            switch (style.AttackResultRequirement)
            {
                case Style.eAttackResultRequirement.Miss:
                    temp += LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "DetailDisplayHandler.HandlePacket.TargetMisses");
                    break;
                case Style.eAttackResultRequirement.Hit:
                    temp += LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "DetailDisplayHandler.HandlePacket.TargetHits");
                    break;
                case Style.eAttackResultRequirement.Parry:
                    temp += LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "DetailDisplayHandler.HandlePacket.YouParry");
                    break;
                case Style.eAttackResultRequirement.Block:
                    temp += LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "DetailDisplayHandler.HandlePacket.YouBlock");
                    break;
                case Style.eAttackResultRequirement.Evade:
                    temp += LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "DetailDisplayHandler.HandlePacket.YouEvade");
                    break;
                case Style.eAttackResultRequirement.Fumble:
                    temp += LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "DetailDisplayHandler.HandlePacket.TargetFumbles");
                    break;
                case Style.eAttackResultRequirement.Style:
                    temp += LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "DetailDisplayHandler.HandlePacket.TargetStyle");
                    break;
                default:
                    temp += LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "DetailDisplayHandler.HandlePacket.Any");
        ...
        else if (Style.eOpening.Positional == style.OpeningRequirementType)
        {
            ...
            temp += LanguageMgr.GetTranslation(player.Client.Account.Language,
                "DetailDisplayHandler.HandlePacket.Positional");
            switch (style.OpeningRequirementValue)
            {
                case (int) Style.eOpeningPosition.Front:
                    temp += LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "DetailDisplayHandler.HandlePacket.Front");
                    break;
                case (int) Style.eOpeningPosition.Back:
                    temp += LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "DetailDisplayHandler.HandlePacket.Back");
                    break;
                case (int) Style.eOpeningPosition.Side:
                    temp += LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "DetailDisplayHandler.HandlePacket.Side");
                    break;
            }
        }
        ...
        if (style.OpeningRequirementValue != 0 && style.AttackResultRequirement == 0 &&
            style.OpeningRequirementType == 0)
            delveInfo.Add(string.Format("- Error: Opening Requirement '{0}' but requirement type is Any!",
                style.OpeningRequirementValue));

        temp = "";

        foreach (var st in SkillBase.GetStyleList(style.Spec, player.CharacterClass.ID))
            if (st.AttackResultRequirement == Style.eAttackResultRequirement.Style &&
                st.OpeningRequirementValue == style.ID)
                temp = temp == ""
                    ? st.Name
                    : temp + LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "DetailDisplayHandler.HandlePacket.Or", st.Name);

        if (temp != "")
            delveInfo.Add(LanguageMgr.GetTranslation(player.Client.Account.Language,
                "DetailDisplayHandler.HandlePacket.FollowupStyle", temp));

        temp = LanguageMgr.GetTranslation(player.Client.Account.Language,
            "DetailDisplayHandler.HandlePacket.FatigueCost") + " ";

        if (style.EnduranceCost < 5)
            temp += LanguageMgr.GetTranslation(player.Client.Account.Language,
                "DetailDisplayHandler.HandlePacket.VeryLow");
        else if (style.EnduranceCost < 10)
            temp += LanguageMgr.GetTranslation(player.Client.Account.Language,
                "DetailDisplayHandler.HandlePacket.Low");
        else if (style.EnduranceCost < 15)
            temp += LanguageMgr.GetTranslation(player.Client.Account.Language,
                "DetailDisplayHandler.HandlePacket.Medium");
        else if (style.EnduranceCost < 20)
            temp += LanguageMgr.GetTranslation(player.Client.Account.Language,
                "DetailDisplayHandler.HandlePacket.High");
        else
            temp += LanguageMgr.GetTranslation(player.Client.Account.Language,
                "DetailDisplayHandler.HandlePacket.VeryHigh");

        delveInfo.Add(temp);

        temp = LanguageMgr.GetTranslation(player.Client.Account.Language,
            "DetailDisplayHandler.HandlePacket.Damage") + " ";

        var tempGrowth =
            (style.GrowthRate * 50 + style.GrowthOffset) /
            0.295; //0.295 is the rounded down style quantum that is used on Live

        if (style.GrowthRate == 0 && style.GrowthOffset == 0)
            temp += LanguageMgr.GetTranslation(player.Client.Account.Language,
                "DetailDisplayHandler.HandlePacket.None");
            ...
            temp += LanguageMgr.GetTranslation(player.Client.Account.Language,
                "DetailDisplayHandler.HandlePacket.Devastating");

        delveInfo.Add(temp);

        temp =
            LanguageMgr.GetTranslation(player.Client.Account.Language, "DetailDisplayHandler.HandlePacket.ToHit") +
            " ";

        if (style.BonusToHit <= -20)
            temp += LanguageMgr.GetTranslation(player.Client.Account.Language,
                "DetailDisplayHandler.HandlePacket.VeryHighPenalty");
        else if (style.BonusToHit <= -15)
            temp += LanguageMgr.GetTranslation(player.Client.Account.Language,
                "DetailDisplayHandler.HandlePacket.HighPenalty");
        else if (style.BonusToHit <= -10)
            temp += LanguageMgr.GetTranslation(player.Client.Account.Language,
                "DetailDisplayHandler.HandlePacket.MediumPenalty");
        else if (style.BonusToHit <= -5)
            temp += LanguageMgr.GetTranslation(player.Client.Account.Language,
                "DetailDisplayHandler.HandlePacket.LowPenalty");
        else if (style.BonusToHit < 0)
            temp += LanguageMgr.GetTranslation(player.Client.Account.Language,
                "DetailDisplayHandler.HandlePacket.VeryLowPenalty");
        else if (style.BonusToHit == 0)
            temp += LanguageMgr.GetTranslation(player.Client.Account.Language,
                "DetailDisplayHandler.HandlePacket.NoBonus");
        else if (style.BonusToHit < 5)
            temp += LanguageMgr.GetTranslation(player.Client.Account.Language,
                "DetailDisplayHandler.HandlePacket.VeryLowBonus");
        else if (style.BonusToHit < 10)
            temp += LanguageMgr.GetTranslation(player.Client.Account.Language,
                "DetailDisplayHandler.HandlePacket.LowBonus");
        else if (style.BonusToHit < 15)
            temp += LanguageMgr.GetTranslation(player.Client.Account.Language,
                "DetailDisplayHandler.HandlePacket.MediumBonus");
        else if (style.BonusToHit < 20)
            temp += LanguageMgr.GetTranslation(player.Client.Account.Language,
                "DetailDisplayHandler.HandlePacket.HighBonus");
        else
            temp += LanguageMgr.GetTranslation(player.Client.Account.Language,
                "DetailDisplayHandler.HandlePacket.VeryHighBonus");

        delveInfo.Add(temp);

        temp = LanguageMgr.GetTranslation(player.Client.Account.Language,
            "DetailDisplayHandler.HandlePacket.Defense") + " ";
        ...
        {
            temp = LanguageMgr.GetTranslation(player.Client.Account.Language,
                "DetailDisplayHandler.HandlePacket.TargetEffect") + " ";

            var styleLine = SkillBase.GetSpellLine(GlobalSpellsLines.Combat_Styles_Effect);
            if (styleLine != null)
            {
                /*check if there is a class specific style proc*/
                var hasClassSpecificProc = false;
                ...
                foreach (var proc in style.Procs)
                {
                    ...
                    var spell = proc.Item1;
                    if (spell != null)
                    {
                        var spellHandler =
                            ScriptMgr.CreateSpellHandler(player.Client.Player, spell, styleLine);
        ...
        if (player.Client.Account.PrivLevel > 1)
        {
            ...
            delveInfo.Add(string.Format("AttackResultRequirement: {0}({1}) {2}", style.AttackResultRequirement,
                (int) style.AttackResultRequirement, indicator));
            delveInfo.Add(string.Format("OpeningRequirementType: {0}({1}) {2}", style.OpeningRequirementType,
                (int) style.OpeningRequirementType, indicator));
            delveInfo.Add(string.Format("OpeningRequirementValue: {0}", style.OpeningRequirementValue));
            delveInfo.Add(string.Format("ArmorHitLocation: {0}({1})", style.ArmorHitLocation,
                (int) style.ArmorHitLocation));
            ...
            if (style.Procs != null && style.Procs.Count > 0)
            {
                ...
                var procs = "";
                foreach (var spell in style.Procs)
                {
                    if (procs != "")
                        procs += ", ";
