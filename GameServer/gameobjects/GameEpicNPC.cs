...
public class GameEpicNPC : GameNPC
{
    ...
    public override void Die(GameObject killer)
    {
        try
        {
            ...
            if (numCurrentLoyalDays >= 1) realmLoyalty = (int) Math.Round(20 * (numCurrentLoyalDays / 30.0));
                ...
                if (bgPlayers != null)
                    foreach (GamePlayer bgPlayer in bgPlayers)
                        if (bgPlayer.IsWithinRadius(this, WorldMgr.MAX_EXPFORKILL_DISTANCE))
                        {
                            ...
                            if (Util.ChanceDouble(carapaceChance)) AtlasROGManager.GenerateBeetleCarapace(bgPlayer);
            ...
            else if (playerKiller?.Group != null)
            {
                foreach (var groupPlayer in playerKiller.Group.GetPlayersInTheGroup())
                    if (groupPlayer.IsWithinRadius(this, WorldMgr.MAX_EXPFORKILL_DISTANCE))
                    {
                        ...
                        if (Util.Chance(baseChance + realmLoyalty)) AtlasROGManager.GenerateReward(groupPlayer, amount);

                        if (Util.ChanceDouble(carapaceChance)) AtlasROGManager.GenerateBeetleCarapace(groupPlayer);
            ...
            else if (playerKiller != null)
            {
                ...
                {
                    if (Util.Chance(baseChance + realmLoyalty)) AtlasROGManager.GenerateReward(playerKiller, amount);

                    if (Util.ChanceDouble(carapaceChance)) AtlasROGManager.GenerateBeetleCarapace(playerKiller);
