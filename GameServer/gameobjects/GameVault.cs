...
public class GameVault : GameStaticItem, IGameInventoryObject
{
    ...
    /// </summary>
    protected object m_vaultSync = new();
    ...
    /// </summary>
    public int LastClientSlot => (int) eInventorySlot.HousingInventory_Last;
    ...
    public virtual Dictionary<int, InventoryItem> GetClientInventory(GamePlayer player)
    {
        ...
        var slotOffset = -FirstDBSlot + (int) eInventorySlot.HousingInventory_First;
        foreach (var item in DBItems(player))
            if (item != null)
            {
                if (!inventory.ContainsKey(item.SlotPosition + slotOffset))
                    inventory.Add(item.SlotPosition + slotOffset, item);
                else
                    log.ErrorFormat("GAMEVAULT: Duplicate item {0}, owner {1}, position {2}", item.Name,
                        item.OwnerID, item.SlotPosition + slotOffset);
            }
    ...
    public override bool Interact(GamePlayer player)
    {
        ...
        if (player.ActiveInventoryObject != null) player.ActiveInventoryObject.RemoveObserver(player);
    ...
    {
        var filterBySlot = DB.Column("SlotPosition").IsGreaterOrEqualTo(FirstDBSlot)
            .And(DB.Column("SlotPosition").IsLessOrEqualTo(LastDBSlot));
        return DOLDB<InventoryItem>.SelectObjects(
            DB.Column("OwnerID").IsEqualTo(GetOwner(player)).And(filterBySlot));
    }
    ...
    public virtual bool MoveItem(GamePlayer player, ushort fromSlot, ushort toSlot)
    {
        ...
        var fromHousing = fromSlot >= (ushort) eInventorySlot.HousingInventory_First &&
                          fromSlot <= (ushort) eInventorySlot.HousingInventory_Last;
        var toHousing = toSlot >= (ushort) eInventorySlot.HousingInventory_First &&
                        toSlot <= (ushort) eInventorySlot.HousingInventory_Last;

        if (fromHousing == false && toHousing == false) return false;

        var gameVault = player.ActiveInventoryObject as GameVault;
        if (gameVault == null)
        {
            player.Out.SendMessage("You are not actively viewing a vault!", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
        ...
        {
            player.Out.SendMessage("You don't have permission to add items!", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            return false;
        }
        ...
        {
            player.Out.SendMessage("You don't have permission to remove items!", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            return false;
        }

        var itemInFromSlot = player.Inventory.GetItem((eInventorySlot) fromSlot);
        var itemInToSlot = player.Inventory.GetItem((eInventorySlot) toSlot);
        ...
        {
            player.Out.SendMessage("You cannot swap with an untradable item!", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            log.DebugFormat("GameVault: {0} attempted to swap untradable item {2} with {1}", player.Name,
                itemInFromSlot.Name, itemInToSlot.Name);
            player.Out.SendInventoryItemsUpdate(null);
            return false;
        }
        ...
        {
            player.Out.SendMessage("You can not put this item into a House Vault!", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            player.Out.SendInventoryItemsUpdate(null);
            return false;
        }
            ...
            if (fromHousing)
            {
                if (toHousing)
                    NotifyObservers(player,
                        this.MoveItemInsideObject(player, (eInventorySlot) fromSlot, (eInventorySlot) toSlot));
                else
                    NotifyObservers(player,
                        this.MoveItemFromObject(player, (eInventorySlot) fromSlot, (eInventorySlot) toSlot));
            }
            else if (toHousing)
            {
                NotifyObservers(player,
                    this.MoveItemToObject(player, (eInventorySlot) fromSlot, (eInventorySlot) toSlot));
            }
        }
