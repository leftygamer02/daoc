    ...
    public class BuffMerchant : GameMerchant
    {
        ...
        public void CastBuffs()
        {
            ...
            while (m_buffs.Count > 0)
            {
                ...
                var spellHandler = ScriptMgr.CreateSpellHandler(con.Target, con.Spell, con.SpellLine);
        ...
        public override bool Interact(GamePlayer player)
        {
            ...
            player.Out.SendMessage(
                ...
                ". The King has instructed me to strengthen you so that you may defend the lands with valor. Simply hand me the token for the enhancement you desire, and I will empower you accordingly. Do you wish to purchase tokens with [Gold] or [Bounty Points]?",
        ...
        public override void OnPlayerBuy(GamePlayer player, int item_slot, int number)
        {
            if (isBounty == true) //...pay with Bounty Points.
            {
                ...
                lock (player.Inventory)
                {
                    if (player.BountyPoints < totalValue)
                    {
                        player.Out.SendMessage(
                            LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "GameMerchant.OnPlayerBuy.YouNeedBP", totalValue), eChatType.CT_System,
                    ...
                    InventoryLogging.LogInventoryAction(this, player, eInventoryActionType.Merchant, template,
                        amountToBuy);
            ...
            if (isBounty == false) //...pay with Money.
            {
                ...
                lock (player.Inventory)
                {
                    if (player.GetCurrentMoney() < totalValue)
                    {
                        player.Out.SendMessage(
                            LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "GameMerchant.OnPlayerBuy.YouNeed", Money.GetString(totalValue)), eChatType.CT_System,
                            eChatLoc.CL_SystemWindow);
                        return;
                    }
        ...
        public override bool ReceiveItem(GameLiving source, InventoryItem item)
        {
            ...
            if (GetDistanceTo(t) > WorldMgr.INTERACT_DISTANCE)
            {
                ((GamePlayer) source).Out.SendMessage(
                    "You are too far away to give anything to " + GetName(0, false) + ".", eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);
                return false;
            }
...
public class BuffTokensList
{
    ...
    public static void OnServerStartup(DOLEvent e, object sender, EventArgs args)
    {
        var buffMerch = DOLDB<ItemTemplate>.SelectObjects(DB.Column("PackageID").IsLike("BuffTokens"))
            .OrderBy(it => it.Item_Type).ToArray();
        MerchantItem m_item = null;
        var pagenumber = 0;
        var slotposition = 0;
        m_item = DOLDB<MerchantItem>.SelectObject(DB.Column("ItemListID").IsEqualTo("BuffTokens"));
        if (m_item == null)
            foreach (var item in buffMerch)
            {
...
public class BPBuffTokensList
{
    ...
    public static void OnServerStartup(DOLEvent e, object sender, EventArgs args)
    {
        var buffMerch = DOLDB<ItemTemplate>.SelectObjects(DB.Column("PackageID").IsLike("BPBuffTokens"))
            .OrderBy(it => it.Item_Type).ToArray();
        MerchantItem m_item = null;
        var pagenumber = 0;
        var slotposition = 0;
