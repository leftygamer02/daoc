...
{
    protected override string Type => GuildName;

    private List<Teleport> m_destinations = new();
    ...
    public override bool Interact(GamePlayer player)
    {
        ...
        if (GameServer.ServerRules.IsSameRealm(this, player, true) == false &&
            player.Client.Account.PrivLevel == (int) ePrivLevel.Player)
            return false;
            ...
            player.Client.Account.PrivLevel > (int) ePrivLevel.Player)
        {
            ...
            SayTo(player,
                "You can set what I say to players by setting the packageid with /mob package \"Some Text\"");
            return true;
        }
        ...
        {
            player.Out.SendMessage(string.Format("[{0}]", destination.TeleportID), eChatType.CT_Say,
        ...
        if (numDestinations == 0 && player.Client.Account.PrivLevel > (int) ePrivLevel.Player)
            SayTo(player,
                "I have not been set up properly, I need teleport locations.  Do /teleport add \"Destination Name\" \"" +
                GuildName + "\"");
    ...
    public override bool WhisperReceive(GameLiving source, string text)
    {
        ...
        foreach (var t in m_destinations)
        ...
        if (destination != null) OnDestinationPicked(player, destination);
