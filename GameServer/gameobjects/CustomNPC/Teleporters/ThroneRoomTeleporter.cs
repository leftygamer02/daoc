...
public class ThroneRoomTeleporter : GameNPC
{
    ...
    public override bool Interact(GamePlayer player)
    {
        ...
        {
            var reply = "I am afraid, but the King is busy right now.";

            if (player.Inventory.CountItemTemplate("Personal_Bind_Recall_Stone", eInventorySlot.Min_Inv,
                    eInventorySlot.Max_Inv) == 0)
                reply +=
                    " If you're only here to get your Personal Bind Recall Stone then I'll see what I can [do].";
    ...
    public override bool WhisperReceive(GameLiving source, string text)
    {
        ...
        if ((text.ToLower() == "king" || text.ToLower() == "exit") &&
            GlobalConstants.IsExpansionEnabled((int) eClientExpansion.DarknessRising))
        {
            uint throneRegionID = 0;
            var teleportThroneID = "error";
            var teleportExitID = "error";
            ...
            if (throneRegionID == 0)
            {
                log.ErrorFormat("Can't find King for player {0} speaking to {1} of realm {2}!", player.Name, Name,
                    Realm);
                player.Out.SendMessage("Server error, can't find throne room.",
                    PacketHandler.eChatType.CT_Staff, PacketHandler.eChatLoc.CL_SystemWindow);
                return false;
            }
            ...
            if (player.CurrentRegionID == throneRegionID)
            {
                ...
                if (teleport == null)
                {
                    ...
                    player.Out.SendMessage(
                        "Server error, can't find exit to this throne room.  Moving you to your last bind point.",
                        PacketHandler.eChatType.CT_Staff, PacketHandler.eChatLoc.CL_SystemWindow);
                    player.MoveToBind();
                }
            }
            else
            {
                ...
                if (teleport == null)
                {
                    ...
                    player.Out.SendMessage("Server error, can't find throne room teleport location.",
                        PacketHandler.eChatType.CT_Staff, PacketHandler.eChatLoc.CL_SystemWindow);
                }
            }
            ...
            if (teleport != null)
            {
                ...
                player.MoveTo((ushort) teleport.RegionID, teleport.X, teleport.Y, teleport.Z,
                    (ushort) teleport.Heading);
            }
