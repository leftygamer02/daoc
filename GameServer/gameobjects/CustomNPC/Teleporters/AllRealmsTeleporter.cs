...
public class AllRealmsTeleporter : GameTeleporter
{
    ...
    public string DisplayTeleportDestinations(eRealm destRealm)
    {
        ...
        switch (destRealm)
        {
            ...
            case eRealm.Hibernia:
                ...
                break;
            default:
                log.Warn(string.Format("DisplayTeleportDestinations does not handle player realm [{0}]",
                    destRealm.ToString()));
                break;
        }
    ...
    {
        var intro = DisplayTeleportDestinations(player.Realm);
        if (intro != null)
            SayTo(player, intro);
    ...
    public override bool WhisperReceive(GameLiving source, string text)
    {
        ...
        switch (text.ToUpper())
        {
            ...
            case "ALBION FRONTIERS":
                sRet.Append(
                    "Where in the frontiers would you like to go?\n[Forest Sauvage]\n[Castle Sauvage]\n[Snowdonia Fortress]\n");
            ...
            // All realms
            case "BATTLEGROUNDS":
                if (!ServerProperties.Properties.BG_ZONES_OPENED &&
                ...
                SayTo(player,
                    "I will teleport you to the appropriate battleground for your level and Realm Rank. If you exceed the Realm Rank for a battleground, you will not teleport. Please gain more experience to go to the next battleground.");
                break;
            case "ENTRANCE":
            case "PERSONAL":
            case "HEARTH":
        ...
        var port = GetTeleportLocation(player, text, realmTarget);
        if (port != null)
    ...
    protected Teleport GetTeleportLocation(GamePlayer player, string text, eRealm realm)
    {
        ...
        if (text.ToLower() == "battlegrounds")
        {
            ...
            {
                var portalKeep = GameServer.KeepManager.GetBGPK(player);
                if (portalKeep != null)
                {
                ...
                else
                {
                    if (player.Client.Account.PrivLevel > (uint) ePrivLevel.Player)
                        player.Out.SendMessage("No portal keep found.", eChatType.CT_Skill,
        ...
        if (text.ToLower() == "hearth")
        {
            ...
            var houses = (ArrayList) HouseMgr.GetHousesCloseToSpot((ushort) player.BindHouseRegion,
            ...
            var targetHouse = (House) houses[0];
            var hookpointItems = targetHouse.HousepointItems;
            var hasBindstone = false;

            foreach (var targetHouseItem in hookpointItems)
        ...
        {
            var house = HouseMgr.GetGuildHouseByPlayer(player);
