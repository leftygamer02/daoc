...
public class GameBoatStableMaster : GameMerchant
{
    ...
    public override void OnPlayerBuy(GamePlayer player, int item_slot, int number)
    {
        ...
        lock (player.Inventory)
        {
            ...
            if (!player.Inventory.AddTemplate(item, amountToBuy, eInventorySlot.FirstBackpack,
        ...
        if (item.Name.ToUpper().Contains("TICKET TO") || item.Description.ToUpper() == "TICKET")
        {
            ...
            var ticket =
                player.Inventory.GetFirstItemByName(item.Name, eInventorySlot.FirstBackpack,
                    eInventorySlot.LastBackpack) as InventoryItem;
            if (ticket != null)
    ...
    public override bool ReceiveItem(GameLiving source, InventoryItem item)
    {
        ...
        if (source is GamePlayer)
        {
            ...
            if (item.Name.ToLower().StartsWith(LanguageMgr.GetTranslation(ServerProperties.Properties.DB_LANGUAGE,
                    "GameStableMaster.ReceiveItem.TicketTo")) && item.Item_Type == 40)
            {
                foreach (GameNPC npc in GetNPCsInRadius(1500))
                    if (npc is GameTaxiBoat)
                    {
                        player.Out.SendMessage(
                            LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "GameBoatStableMaster.ReceiveItem.Departed", Name), eChatType.CT_System,
                            eChatLoc.CL_PopupWindow);
                        return false;
                    }

                var destination = item.Name.Substring(LanguageMgr
                    .GetTranslation(ServerProperties.Properties.DB_LANGUAGE,
                        "GameStableMaster.ReceiveItem.TicketTo").Length);
                var path = MovementMgr.LoadPath(item.Id_nb);
                ...
                if (path != null && Math.Abs(path.X - X) < 500 && Math.Abs(path.Y - Y) < 500)
                {
                    ...
                    player.Out.SendMessage(
                        LanguageMgr.GetTranslation(player.Client.Account.Language,
                            "GameBoatStableMaster.ReceiveItem.SummonedBoat", Name, destination),
                ...
                else
                {
                    player.Out.SendMessage(
                        LanguageMgr.GetTranslation(player.Client.Account.Language,
                            "GameBoatStableMaster.ReceiveItem.UnknownWay", Name, destination),
                        eChatType.CT_System, eChatLoc.CL_SystemWindow);
                }
            }
        }
    ...
    protected class HorseRideAction : RegionECSAction
    {
        ...
        protected override int OnTick(ECSGameTimer timer)
        {
            var horse = (GameNPC) m_actionSource;
