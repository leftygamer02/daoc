...
public class GameStableMaster : GameMerchant
{
    ...
    public override bool ReceiveItem(GameLiving source, InventoryItem item)
    {
        ...
        if (source is GamePlayer)
        {
                ...
                if (path != null && Math.Abs(path.X - X) < 500 && Math.Abs(path.Y - Y) < 500)
                {
                    ...
                    else
                    {
                        ...
                        foreach (GameNPC npc in GetNPCsInRadius(400))
                            if (npc.Name == LanguageMgr.GetTranslation(ServerProperties.Properties.DB_LANGUAGE,
                                    "GameStableMaster.ReceiveItem.HorseName"))
                            {
            ...
            else
            {
                player.Out.SendMessage(
                    LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "GameStableMaster.ReceiveItem.UnknownWay"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
            }
        }
    ...
    private bool isItemInMerchantList(InventoryItem item)
    {
        if (m_tradeItems != null)
            ...
            {
                var compareItem = de.Value as ItemTemplate;
                if (compareItem != null)
