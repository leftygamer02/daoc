...
public class GameTrainer : GameNPC
{
    ...
    public override bool Interact(GamePlayer player)
    {
        ...
        if (CanTrain(player))
        {
            ...
            if (player.FreeLevelState == 2)
            {
                ...
                var xp = player.GetExperienceNeededForLevel(player.LastFreeLevel + 1) -
                         player.GetExperienceNeededForLevel(player.LastFreeLevel);
        ...
        if (CanTrainChampionLevels(player)) player.Out.SendChampionTrainerWindow((int) m_championTrainerType);
    ...
    {
        return player.Level >= player.MaxLevel && player.Champion &&
    ...
    public override bool WhisperReceive(GameLiving source, string text)
    {
        ...
        if (CanTrain(player) && text == LanguageMgr.GetTranslation(player.Client.Account.Language,
                "GameTrainer.Interact.CaseRespecialize"))
            if (player.Level == 5 && !player.IsLevelRespecUsed)
            {
                var specPoints = player.SkillSpecialtyPoints;
                ...
                if (player.SkillSpecialtyPoints > specPoints)
                {
                    ...
                    player.Out.SendMessage(
                        LanguageMgr.GetTranslation(player.Client.Account.Language,
                            "GameTrainer.Interact.RegainPoints", player.SkillSpecialtyPoints - specPoints),
        ...
        lock (player.Inventory)
        {
            foreach (var item in player.Inventory.EquippedItems)
                if (!player.HasAbilityToUseItem(item.Template))
                    if (player.Inventory.IsSlotsFree(item.Count, eInventorySlot.FirstBackpack,
                            eInventorySlot.LastBackpack) == true)
                    {
                        player.Inventory.MoveItem((eInventorySlot) item.SlotPosition,
                            player.Inventory.FindFirstEmptySlot(eInventorySlot.FirstBackpack,
                                eInventorySlot.LastBackpack), item.Count);
                        player.Out.SendMessage(
                            LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "GameTrainer.CheckAbilityToUseItem.Text1", item.GetName(0, false)),
                            eChatType.CT_System, eChatLoc.CL_SystemWindow);
                    }
                    else
                    {
                        player.Inventory.MoveItem((eInventorySlot) item.SlotPosition, eInventorySlot.Ground,
                            item.Count);
                        player.Out.SendMessage(
                            LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "GameTrainer.CheckAbilityToUseItem.Text1", item.GetName(0, false)),
        ...
        if (player != null)
            switch (item.Id_nb)
            {
                case "respec_single":
                {
                    ...
                    player.Out.SendMessage(
                        LanguageMgr.GetTranslation(player.Client.Account.Language,
                            "GameTrainer.ReceiveItem.RespecSingle"), eChatType.CT_System, eChatLoc.CL_PopupWindow);
                    return true;
                }
                case "respec_full":
                {
                    ...
                    player.Out.SendMessage(
                        LanguageMgr.GetTranslation(player.Client.Account.Language,
                            "GameTrainer.ReceiveItem.RespecFull", item.Name), eChatType.CT_System,
                ...
                case "respec_realm":
                {
                    ...
                    player.Out.SendMessage(
                        LanguageMgr.GetTranslation(player.Client.Account.Language,
                            "GameTrainer.ReceiveItem.RespecRealm"), eChatType.CT_System, eChatLoc.CL_PopupWindow);
                    return true;
                }
            }
    ...
    public virtual bool CanPromotePlayer(GamePlayer player)
    {
        ...
        var pickedClass = ScriptMgr.FindCharacterClass((int) TrainedClass);
        ...
        if (GlobalConstants.CLASS_GENDER_CONSTRAINTS_DICT.ContainsKey(TrainedClass) &&
            GlobalConstants.CLASS_GENDER_CONSTRAINTS_DICT[TrainedClass] != player.Gender)
            return false;
    ...
    public bool PromotePlayer(GamePlayer player, int classid, string messageToPlayer, InventoryItem[] gifts)
    {
        ...
        if (player.SetCharacterClass(classid))
        {
            ...
            if (messageToPlayer != "")
                player.Out.SendMessage(
                    LanguageMgr.GetTranslation(player.Client.Account.Language, "GameTrainer.PromotePlayer.Says",
                        Name, messageToPlayer), eChatType.CT_System, eChatLoc.CL_PopupWindow);
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language, "GameTrainer.PromotePlayer.Upgraded",
                    player.CharacterClass.Name), eChatType.CT_Important, eChatLoc.CL_SystemWindow);
            ...
            // after gifts
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language, "GameTrainer.PromotePlayer.Accepted",
                    player.CharacterClass.Profession), eChatType.CT_Important, eChatLoc.CL_SystemWindow);
    ...
    {
        var temp = GameServer.Database.FindObjectByKey<ItemTemplate>(template);
        if (temp != null)
        {
            if (!player.Inventory.AddTemplate(GameInventoryItem.Create(temp), 1, eInventorySlot.FirstBackpack,
                    eInventorySlot.LastBackpack))
            {
                player.Out.SendMessage(
                    LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "GameTrainer.AddGift.NotEnoughSpace"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                return false;
            }
        ...
        if (CanTrainChampionLevels(player) == false)
        {
            ...
            if (triggers == null || triggers.Length == 0)
                SayTo(player, eChatLoc.CL_ChatWindow,
                    LanguageMgr.GetTranslation(player.Client.Account.Language, "GameTrainer.Train.SeekElsewhere"));
        }
    }
    ...
    {
        SayTo(player, eChatLoc.CL_ChatWindow,
            LanguageMgr.GetTranslation(player.Client.Account.Language, "GameTrainer.Train.WouldYouLikeTo"));
    }
