...
public class EmblemNPC : GameNPC
{
    ...
    public override bool Interact(GamePlayer player)
    {
        ...
        if (triggers == null || triggers.Length == 0)
            SayTo(player, eChatLoc.CL_ChatWindow,
                "For 5 gold, I can put the emblem of your guild on the item. Just hand me the item.");
    ...
    public override bool ReceiveItem(GameLiving source, InventoryItem item)
    {
        ...
        if (item.Emblem != 0)
        {
            t.Out.SendMessage("This item already has an emblem on it.", eChatType.CT_System,
            ...
            || item.Item_Type == Slot.CLOAK)
        {
            ...
            if (t.Level < 20) //if level of player < 20 so can not put emblem
            {
                ...
                {
                    t.Out.SendMessage(
                        "You have to be at least level 20 or have 400 in a tradeskill to be able to wear an emblem.",
            ...
            {
                t.Out.SendMessage("You do not have enough privileges for that.", eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);
                return false;
            }

            t.TempProperties.setProperty(EMBLEMIZE_ITEM_WEAK, new WeakRef(item));
            t.Out.SendCustomDialog("Do you agree to put an emblem on this object?",
                new CustomDialogResponse(EmblemerDialogResponse));
        }
        else
        {
            t.Out.SendMessage("I can not put an emblem on this item.", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
        }
