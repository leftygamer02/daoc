...
public class GameHealer : GameNPC
{
    ...
    public override IList GetExamineMessages(GamePlayer player)
    {
        ...
        list.Add(LanguageMgr.GetTranslation(player.Client.Account.Language, "GameObject.Target.YouTarget.Object",
            GetName(0, false, player.Client.Account.Language, this)));
        // Message: You examine {0}. {1} is {2} and is a healer.
        list.Add(LanguageMgr.GetTranslation(player.Client.Account.Language, "GameNPC.Examine.YouExamine.Healer",
            GetName(0, false, player.Client.Account.Language, this),
        ...
        list.Add(LanguageMgr.GetTranslation(player.Client.Account.Language, "GameNPC.Interact.GiveDonation.Healer",
            null));
        return list;
    }
    ...
    public override bool Interact(GamePlayer player)
    {
        ...
        if (triggers == null || triggers.Length == 0)
            ...
            ChatUtil.SendSayMessage(player, "GameNPC.Dialogue.Greetings.Healer", GetName(0, true),
                player.CharacterClass.Name);

        //GameSpellEffect effect = SpellHandler.FindEffectOnTarget(player, CURED_SPELL_TYPE);
        var
            effect = EffectListService.GetEffectOnTarget(player,
                eEffect.ResurrectionIllness); // Identify effect to remove
        if (effect != null) // If PvE sickness is active
        {
            ...
            ChatUtil.SendSystemMessage(player, "GameNPC.Interact.CuresRS.Healer",
                GetName(0, true, player.Client.Account.Language, this));
        }

        var
            rvrEffect = EffectListService.GetEffectOnTarget(player,
                eEffect.RvrResurrectionIllness); // Identify effect to remove
        ...
        {
            var oneConCost =
                GamePlayer.prcRestore[
                    player.Level < GamePlayer.prcRestore.Length ? player.Level : GamePlayer.prcRestore.Length - 1];
            ...
            player.Out.SendCustomDialog(
                LanguageMgr.GetTranslation(player.Client.Account.Language, "GameNPC.Dialog.AcceptDecline.Healer",
                    Money.GetString(player.TotalConstitutionLostAtDeath * (long) oneConCost)),
                new CustomDialogResponse(HealerDialogResponse));
        }
    ...
    protected void HealerDialogResponse(GamePlayer player, byte response)
    {
        ...
        if (!IsWithinRadius(player, WorldMgr.INTERACT_DISTANCE))
        {
            ...
            ChatUtil.SendSystemMessage(player, "GameNPC.Interact.TooFarAway",
                GetName(0, false, player.Client.Account.Language, this));
            return;
        }
        ...
        var cost = player.TempProperties.getProperty<long>(COST_BY_PTS);
        player.TempProperties.removeProperty(COST_BY_PTS);
        if (cost <= 0) cost = 1;
        var restorePoints = (int) Math.Min(player.TotalConstitutionLostAtDeath, player.GetCurrentMoney() / cost);
        if (restorePoints < 1)
            restorePoints = 1; // Constitution reduced by 1 at minimum
        var totalCost = restorePoints * cost; // Total cost to restore full Con lost
        ...
        if (player.RemoveMoney(totalCost))
        {
            InventoryLogging.LogInventoryAction(player, this, eInventoryActionType.Merchant,
                totalCost); // Deduct the cost from the player
            if (restorePoints == 1)
            ...
            ChatUtil.SendSystemMessage(player, "GameNPC.Interact.YouGiveDonation.Healer",
                GetPronoun(2, false, player.Client.Account.Language), Money.GetString(totalCost));
        ...
        else // If insufficient funds available, throw "error"
        {
            ...
            ChatUtil.SendSayMessage(player, "GameNPC.Interact.NeedMoney.Healer",
                GetName(0, true, player.Client.Account.Language, this), Money.GetString(totalCost), restorePoints);
        }
