...
public abstract class GameSummoner : GameNPC
{
    ...
    /// </summary>
    public abstract int PetSummonThreshold { get; }

    /// <summary>
    /// Template to base summon on
    /// </summary>
    public abstract NpcTemplate PetTemplate { get; }

    /// <summary>
    /// Level to summon pet at; positive for absolute value, negative for percentage of summoner's level
    /// </summary>
    public abstract byte PetLevel { get; }

    // Optional modifiers.
    public virtual int PetTetherRange => 0;

    public virtual int PetMaxDistance => 1500;

    public virtual byte PetSize => 0;

    public virtual int PetSummonDistance => 100;

    public virtual long PetResummonTime => 60000;
    ...
    protected virtual void SummonPet()
    {
        if (PetTemplate != null && PetLevel != 0 && m_pet == null)
        {
            ...
            if (m_pet != null)
            {
                ...
                var sideHeading = (ushort) (Heading + 900);
                if (Util.Random(1) < 1)
                    sideHeading += 1800;
                var point = GetPointFromHeading(sideHeading, PetSummonDistance);
                m_pet.X = point.X;
                m_pet.Y = point.Y;
                m_pet.Z = Z;
                ...
                if (m_pet.Brain is StandardMobBrain petBrain && Brain is StandardMobBrain brain &&
                    TargetObject is GameLiving living)
                {
