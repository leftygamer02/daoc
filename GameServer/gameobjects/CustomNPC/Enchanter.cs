...
public class Enchanter : GameNPC
{
    ...
    public override IList GetExamineMessages(GamePlayer player)
    {
        ...
        list.Add(LanguageMgr.GetTranslation(player.Client.Account.Language,
            "GameNPC.Enchanter.GetExamineMessages.GiveItem",
    ...
    public override bool ReceiveItem(GameLiving source, InventoryItem item)
    {
        ...
        if (DataQuestList.Count > 0)
            foreach (var quest in DataQuestList)
                quest.Notify(GameObjectEvent.ReceiveItem, this, new ReceiveItemEventArgs(player, this, item));

        if (item.Level >= 10 && item.IsCrafted)
        {
            if (item.Object_Type != (int) eObjectType.Magical && item.Object_Type != (int) eObjectType.Bolt &&
                item.Object_Type != (int) eObjectType.Poison)
            {
                if (item.Bonus == 0)
                {
                    ...
                    player.Client.Out.SendCustomDialog(
                        LanguageMgr.GetTranslation(player.Client, "GameNPC.Enchanter.ReceiveItem.Cost",
                            Money.GetString(CalculEnchantPrice(item))),
                        new CustomDialogResponse(EnchanterDialogResponse));
                }
                else
                    // Message: {0} says, "That is already enchanted."
                {
                    ChatUtil.SendSayMessage(player, "GameNPC.Enchanter.ReceiveItem.AlreadyEnchanted",
                        GetName(0, true));
                }
    ...
    protected void EnchanterDialogResponse(GamePlayer player, byte response)
    {
        ...
        var Fee = CalculEnchantPrice(item);
        ...
        // bright
        item.Name =
            LanguageMgr.GetTranslation(player.Client.Account.Language, "GameNPC.Enchanter.ItemLevel.Bright") + " " +
            item.Name;
