...
public class MarketExplorer : GameNPC, IGameInventoryObject
{
    ...
    public override bool Interact(GamePlayer player)
    {
        ...
        {
            player.Out.SendMessage("Sorry, the market is not available at this time.", eChatType.CT_Staff,
    ...
    /// </summary>
    public virtual int FirstClientSlot => (int) eInventorySlot.MarketExplorerFirst;

    /// <summary>
    /// Last slot of the client window that shows this inventory
    /// </summary>
    public virtual int LastClientSlot => (int) eInventorySlot.MarketExplorerFirst + 39; // not really sure

    /// <summary>
    /// First slot in the DB.
    /// </summary>
    public virtual int FirstDBSlot => (int) eInventorySlot.Consignment_First; // not used

    /// <summary>
    /// Last slot in the DB.
    /// </summary>
    public virtual int LastDBSlot => (int) eInventorySlot.Consignment_Last; // not used

    public eRealm GetRealmOfLot(ushort houseNumber)
    {
        if (houseNumber <= 1382) return eRealm.Albion;

        if (houseNumber <= 2573) return eRealm.Midgard;

        if (houseNumber <= 4398) return eRealm.Hibernia;
    ...
    public virtual bool SearchInventory(GamePlayer player, MarketSearch.SearchData searchData)
    {
        ...
        {
            var maxPerPage = 20;
            var maxPages = (byte) (Math.Ceiling((double) items.Count / (double) maxPerPage) - 1);
            var first = searchData.page * maxPerPage;
            var last = first + maxPerPage;
                ...
                if (index >= first && index <= last)
                {
                    if (GetRealmOfLot(item.OwnerLot) != player.Realm)
                    {
                        if (ServerProperties.Properties.MARKET_ENABLE_LOG)
                            log.DebugFormat(
                                "Not adding item '{0}' to the return search since its from different realm.",
                                item.Name);
                    }
                    else
                    {
            ...
            if ((int) searchData.page == 0)
                player.Out.SendMessage("Items returned: " + items.Count + ".", eChatType.CT_Important,
                    eChatLoc.CL_SystemWindow);
            ...
            if (items.Count == 0) // No items returned, let the client know
            {
                ...
                ((int) searchData.page <=
                 (int) maxPages) //Don't let us tell the client about any more than the max pages
            {
                player.Out.SendMessage("Moving to page " + (int) (searchData.page + 1) + ".",
    ...
    public virtual bool CanHandleMove(GamePlayer player, ushort fromClientSlot, ushort toClientSlot)
    {
        ...
        if (fromClientSlot >= FirstClientSlot && toClientSlot >= (int) eInventorySlot.FirstBackpack &&
    ...
    public virtual bool MoveItem(GamePlayer player, ushort fromClientSlot, ushort toClientSlot)
    {
            ...
            player.ActiveInventoryObject == this)
        {
            ...
            var itemSlot = fromClientSlot - (int) eInventorySlot.MarketExplorerFirst;

            var item = list[itemSlot];
    ...
    {
        var cm = HouseMgr.GetConsignmentByHouseNumber((int) item.OwnerLot);

        if (cm == null)
        {
            player.Out.SendMessage("I can't find the consigmnent merchant for this item!", eChatType.CT_Merchant,
                eChatLoc.CL_ChatWindow);
            log.ErrorFormat("ME: Error finding consignment merchant for lot {0}; {1}:{2} trying to buy {3}",
                item.OwnerLot, player.Name, player.Client.Account.Name, item.Name);
            return;
        }
