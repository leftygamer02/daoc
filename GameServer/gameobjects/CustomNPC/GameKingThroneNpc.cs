...
public class KingNPC : GameNPC
{
    ...
    public override bool Interact(GamePlayer player)
    {
        ...
        if (!player.Champion && player.Level == 50)
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language, "KingNPC.WhisperReceive.AskForChampion"),
        ...
        if (player.Champion)
        {
            ...
            if (player.ChampionLevel >= 5)
                player.Out.SendMessage("I can [respecialize] your champion skills if you so desire.",
                    eChatType.CT_System, eChatLoc.CL_PopupWindow);
        }
    ...
    public override bool WhisperReceive(GameLiving source, string str)
    {
        ...
        if (str == "Champions" && player.Level == 50)
        {
            if (player.Champion == true)
            {
                player.Out.SendMessage(
                    LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "KingNPC.WhisperReceive.AlreadyChampion"), eChatType.CT_System, eChatLoc.CL_PopupWindow);
                return false;
            }
            ...
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language, "KingNPC.WhisperReceive.IsNowChampion"),
                eChatType.CT_System, eChatLoc.CL_PopupWindow);
            return true;
        }
        ...
        if (str.ToLower() == "respecialize" && player.Champion && player.ChampionLevel >= 5)
        {
            ...
            player.SaveIntoDatabase();
            player.Out.SendMessage("I have reset your champion skills!", eChatType.CT_Important,
        ...
        if (player != null)
            switch (item.Id_nb)
            {
                case "respec_cl":
                {
                    ...
                    player.Out.SendMessage(
                        LanguageMgr.GetTranslation(player.Client.Account.Language,
                            "CLWeaponNPC.ReceiveItem.RespecCL"), eChatType.CT_System, eChatLoc.CL_PopupWindow);
                    return true;
                }
            }
