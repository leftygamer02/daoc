...
public class DreadedSealCollector : GameNPC
{
    ...
    protected static Dictionary<string, float> m_BPMultipliers =
        ParseValues(ServerProperties.Properties.DREADEDSEALS_BP_VALUES);

    protected static Dictionary<string, float> m_RPMultipliers =
        ParseValues(ServerProperties.Properties.DREADEDSEALS_RP_VALUES);
    ...
    protected static List<Tuple<int, int>> ParseMultipliers()
    {
        ...
        foreach (var entry in ServerProperties.Properties.DREADEDSEALS_LEVEL_MULTIPLIER.Split(';'))
        {
            var asVal = entry.Split('|');

            if (asVal.Length > 1 && int.TryParse(asVal[0], out var level) &&
                int.TryParse(asVal[1], out var multiplier))
    ...
    protected static Dictionary<string, float> ParseValues(string serverProperty)
    {
        ...
        foreach (var entry in serverProperty.Split(';'))
        {
            var asVal = entry.Split('|');

            if (asVal.Length > 1 && float.TryParse(asVal[1], out var value))
                dict[asVal[0]] = value;
        } // foreach
        ...
        if (source is GamePlayer player && item != null)
        {
            if (GetDistanceTo(player) > WorldMgr.INTERACT_DISTANCE)
            {
                ((GamePlayer) source).Out.SendMessage("You are too far away to give anything to me "
                                                      + player.Name + ". Come a little closer.", eChatType.CT_Say,
            ...
            if (m_levelMultipliers.Count < 1)
            {
                ((GamePlayer) source).Out.SendMessage(
                    "Sorry, no level multipliers are defined so I cannot accept seals at this time.",
            ...
            else
            {
                ...
                foreach (var tup in m_levelMultipliers)
                ...
                if (levelMultiplier <= 0)
                {
                    ((GamePlayer) source).Out.SendMessage("You are too young yet to make use of these items "
                                                          + player.Name + ". Come back in " +
                                                          (nextLevel - player.Level) + " levels.", eChatType.CT_Say,
                        eChatLoc.CL_ChatWindow);
