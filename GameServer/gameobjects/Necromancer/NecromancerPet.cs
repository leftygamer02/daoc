...
public class NecromancerPet : GamePet
{
    ...
    public override int GetModified(eProperty property)
    {
        ...
        var livingOwner = (Brain as IControlledBrain).GetLivingOwner();
        var playerOwner = livingOwner as GamePlayer;
        ...
        switch (property)
        {
            ...
            case eProperty.Intelligence:
            {
                ...
                var itemBonus = livingOwner.GetModifiedFromItems(property);
                var buffBonus = GetModifiedFromBuffs(property);
                var debuff = DebuffCategory[(int) property];
                ...
                switch (property)
                {
                    case eProperty.Strength:
                        baseBonus = Strength;
                        augRaBonus = AtlasRAHelpers.GetStatEnhancerAmountForLevel(playerOwner != null
                            ? AtlasRAHelpers.GetAugStrLevel(playerOwner)
                            : 0);
                        break;
                    case eProperty.Dexterity:
                        baseBonus = Dexterity;
                        augRaBonus = AtlasRAHelpers.GetStatEnhancerAmountForLevel(playerOwner != null
                            ? AtlasRAHelpers.GetAugDexLevel(playerOwner)
                            : 0);
                        break;
                    case eProperty.Quickness:
                        baseBonus = Quickness;
                        augRaBonus = AtlasRAHelpers.GetStatEnhancerAmountForLevel(playerOwner != null
                            ? AtlasRAHelpers.GetAugQuiLevel(playerOwner)
                            : 0);
                        break;
                    case eProperty.Intelligence:
                        baseBonus = Intelligence;
                        augRaBonus = AtlasRAHelpers.GetStatEnhancerAmountForLevel(playerOwner != null
                            ? AtlasRAHelpers.GetAugAcuityLevel(playerOwner)
                            : 0);
                        break;
                }
            ...
            case eProperty.Constitution:
            {
                ...
                var buffBonus = GetModifiedFromBuffs(eProperty.Constitution);
                var debuff = DebuffCategory[(int) property];
            ...
            {
                var conBonus = (int) (3.1 * m_summonConBonus);
                var hitsBonus = 30 * Level + m_summonHitsBonus;
                var debuff = DebuffCategory[(int) property];
                ...
                var totalBonus = conBonus + hitsBonus;

                var toughness = playerOwner?.GetAbility<AtlasOF_ToughnessAbility>();
                var toughnessMod =
                    toughness != null ? 1 + toughness.GetAmountForLevel(toughness.Level) * 0.01 : 1;
    ...
    public override int Health
    {
        ...
        set
        {
            ...
            if (oldPercent != HealthPercent)
            {
                ...
                var owner = (Brain as IControlledBrain).Owner as GamePlayer;
                if (owner.Group != null)
    ...
    public override void AutoSetStats()
    {
        if (Name.ToUpper() == "GREATER NECROSERVANT")
        {
            ...
            if (Level > 1)
            {
                Strength += (short) Math.Round(ServerProperties.Properties.NECRO_GREATER_PET_STR_MULTIPLIER *
                                               Level);
                Constitution +=
                ...
                Intelligence +=
                    (short) Math.Round(ServerProperties.Properties.NECRO_GREATER_PET_INT_MULTIPLIER * Level);
            }
        }
        else
        {
    ...
    {
        var tauntEffect = EffectList.GetOfType<TauntEffect>();
        var owner = (Brain as IControlledBrain).Owner as GamePlayer;
        ...
        if (tauntEffect != null)
        {
            ...
            if (owner != null)
                owner.Out.SendMessage(
                    string.Format("{0} seems to be less aggressive than before.", GetName(0, true)),
        ...
        else
        {
            ...
            if (owner != null)
                owner.Out.SendMessage(string.Format("{0} enters an aggressive stance.", GetName(0, true)),
                    eChatType.CT_System, eChatLoc.CL_SystemWindow);
            ...
            player.Out.SendMessage(
                string.Format(
                    LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "GameLiving.AttackData.HitsForDamage"), ad.Attacker.GetName(0, true), ad.Target.Name,
                    ad.Damage, modmessage), eChatType.CT_Damaged, eChatLoc.CL_SystemWindow);

            if (ad.CriticalDamage > 0)
                player.Out.SendMessage(
                    string.Format(
                        LanguageMgr.GetTranslation(player.Client.Account.Language,
                            "GameLiving.AttackData.CriticallyHitsForDamage"), ad.Attacker.GetName(0, true),
                        ad.Target.Name, ad.CriticalDamage), eChatType.CT_Damaged, eChatLoc.CL_SystemWindow);
        }
        ...
        int interruptDuration, bool dualWield, bool ignoreLOS)
    {
        ...
        return attackComponent.LivingMakeAttack(target, weapon, style, effectiveness, interruptDuration, dualWield,
            ignoreLOS);
    }
    ...
    public override bool CastSpell(Spell spell, SpellLine line)
    {
        ...
        {
            Notify(GameLivingEvent.CastFailed, this,
                new CastFailedEventArgs(null, CastFailedEventArgs.Reasons.AlreadyCasting));
            return false;
        }
        ...
        if (handler != null)
        {
            var power = handler.PowerCost(Owner);

            if (Owner.Mana < power)
            {
                Notify(GameLivingEvent.CastFailed, this,
                    new CastFailedEventArgs(null, CastFailedEventArgs.Reasons.NotEnoughPower));
                return false;
            }
    ...
    public override void OnAfterSpellCastSequence(ISpellHandler handler)
    {
        ...
        if (CurrentSpellHandler != null)
            CurrentSpellHandler.CastingCompleteEvent -=
                new CastingCompleteCallback(OnAfterSpellCastSequence); // Prevent from relaunch
    ...
    public void Taunt()
    {
        ...
        var chantsLine = SkillBase.GetSpellLine("Chants");
        if (chantsLine == null)
            return;

        var chantsList = SkillBase.GetSpellList("Chants");
        if (chantsList.Count == 0)
            return;

        // Find the best paladin taunt for this level.
        Spell tauntSpell = null;
        foreach (var spell in chantsList)
    ...
    private bool LoadEquipmentTemplate(string templateID)
    {
        ...
        var inventoryTemplate = new GameNpcInventoryTemplate();
