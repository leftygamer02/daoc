...
public class GameHouseVault : GameVault, IHouseHookpointItem
{
    ...
    public bool Attach(House house, uint hookpointID, ushort heading)
    {
        ...
        var hpitem = DOLDB<DBHouseHookpointItem>.SelectObjects(DB.Column("HouseNumber").IsEqualTo(house.HouseNumber)
            .And(DB.Column("HookpointID").IsEqualTo(hookpointID)));

        // if there isn't anything already on this hookpoint then add it to the DB
        if (hpitem.Count == 0) GameServer.Database.AddObject(hookedItem);
    ...
    {
        if (_hookedItem == null || CurrentHouse != player.CurrentHouse ||
            CurrentHouse.CanEmptyHookpoint(player) == false)
            return false;

        lock (m_vaultSync)
        {
            foreach (var observer in _observers.Values) observer.ActiveInventoryObject = null;
    ...
    protected override void NotifyObservers(GamePlayer player, IDictionary<int, InventoryItem> updateItems)
    {
        ...
        lock (_vaultLock)
        {
            foreach (var observer in _observers.Values)
            {
                ...
                observer.Client.Out.SendInventoryItemsUpdate(updateItems,
                    PacketHandler.eInventoryWindowType.Update);
