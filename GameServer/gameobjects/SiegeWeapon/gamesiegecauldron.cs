    ...
    public class SiegeDirectDamageSpellHandler : DirectDamageSpellHandler
    {
        ...
        public override AttackData CalculateDamageToTarget(GameLiving target, double effectiveness)
        {
            ...
            if (target is GamePlayer)
            {
                ...
                var id = player.CharacterClass.ID;
                //50% reduction for tanks
                if (id == (int) eCharacterClass.Armsman || id == (int) eCharacterClass.Warrior ||
                ...
                if (player.IsRiding && player.Steed is GameSiegeRam)
                    ad.Damage = (int) ((double) ad.Damage *
                                       (1.0 - (50.0 + (double) player.Steed.Level * 10.0) / 100.0));
            }
        ...
        public override void SendDamageMessages(AttackData ad)
        {
            ...
            {
                var siege = Caster as GameSiegeWeapon;
                if (siege.Owner != null)
                    siege.Owner.Out.SendMessage(
                        string.Format("You hit {0} for {1}{2} damage!", ad.Target.GetName(0, false), ad.Damage,
                            modmessage), eChatType.CT_YouHit, eChatLoc.CL_SystemWindow);
            }
        ...
        public override void DamageTarget(AttackData ad, bool showEffectAnimation, int attackResult)
        {
            ...
            {
                var siege = Caster as GameSiegeWeapon;
                if (siege.Owner != null) ad.Attacker = siege.Owner;
            }
