...
public class GameSiegeRam : GameSiegeWeapon
{
    ...
    public override void Aim()
    {
        ...
        //Range Check
        if (!IsWithinRadius(Owner.TargetObject, AttackRange))
        {
            if (Owner != null)
                Owner.Out.SendMessage("You are too far away to attack " + Owner.TargetObject.Name,
        ...
        var ramsAimedAtTarget = 0;
        foreach (GameNPC npc in Owner.CurrentRegion.GetNPCsInRadius(Owner.TargetObject.X, Owner.TargetObject.Y,
                     Owner.TargetObject.Z, (ushort) 600, false, false))
        ...
        {
            if (Owner != null)
                Owner.Out.SendMessage("Too many rams already attacking   " + TargetObject?.Name,
                    eChatType.CT_System, eChatLoc.CL_SystemWindow);
            return;
        }
    ...
    public override void Fire()
    {
        ...
        {
            if (Owner != null)
                Owner.Out.SendMessage(target.Name + " is already destroyed!", eChatType.CT_System,
    ...
    public override void DoDamage()
    {
        ...
        var damageAmount = CalcDamageToTarget(target);
        ...
        if (Owner != null)
        {
            ...
            Owner.Out.SendMessage("The " + Name + " hits " + target.Name + " for " + damageAmount + " damage!",
                eChatType.CT_YouHit, eChatLoc.CL_SystemWindow);
            Message.SystemToArea(this, GetName(0, false) + " hits " + target.GetName(0, true),
                eChatType.CT_OthersCombat, Owner);
        }
    ...
    {
        TargetObject =
            null; //reset aimed object when released. Prevent empty/bugged rams from taking space on the ram limit per door.
    ...
    private int GetReloadDelay =>
        //custom formula
        10000 + (Level + 1) * 2000 -
        (int) (10000 * ((double) CurrentRiders.Length / (double) MAX_PASSENGERS));
    ...
    public override short MaxSpeed
    {
        ...
        {
            //custom formula
            var speed = 10.0 + 5.0 * Level + 50.0 * CurrentRiders.Length / MAX_PASSENGERS;
            foreach (var player in CurrentRiders)
            {
