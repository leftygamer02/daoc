...
public class BDSubPet : BDPet
{
    ...
    public string PetSpecLine
    {
        get
        {
            ...
            if (m_PetSpecLine == null && Brain is IControlledBrain brain &&
                brain.GetPlayerOwner() is GamePlayer player)
            {
                // Get the spell that summoned this pet
                var dbSummoningSpell =
                    DOLDB<DBSpell>.SelectObject(DB.Column("LifeDrainReturn").IsEqualTo(NPCTemplate.TemplateId));
                if (dbSummoningSpell != null)
                {
                    // Figure out which spell line the summoning spell is from
                    var dbLineSpell =
                        DOLDB<DBLineXSpell>.SelectObject(DB.Column("SpellID").IsEqualTo(dbSummoningSpell.SpellID));
                    if (dbLineSpell != null)
                    {
                        // Now figure out what the spec name is
                        var line = player.GetSpellLine(dbLineSpell.LineName);
                        if (line != null)
    ...
    {
        var itemTemp = CommanderPet.GetWeaponTemplate(weaponType);
    ...
    public override void SortSpells()
    {
        ...
        if (ServerProperties.Properties.PET_SCALE_SPELL_MAX_LEVEL < 1)
        {
            base.SortSpells();
        }
        else
        {
            ...
            if (ServerProperties.Properties.PET_CAP_BD_MINION_SPELL_SCALING_BY_SPEC
                && Brain is IControlledBrain brain && brain.GetPlayerOwner() is GamePlayer BD)
            {
                var spec = BD.GetModifiedSpecLevel(PetSpecLine);
    ...
    {
        var commander = (Brain as IControlledBrain).Owner as CommanderPet;
        commander.RemoveControlledNpc(Brain as IControlledBrain);
