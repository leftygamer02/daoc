...
public class CommanderPet : BDPet
{
        ...
        : base(npcTemplate)
    {
        ...
        if (upperName == LanguageMgr
                .GetTranslation(LanguageMgr.DefaultLanguage, "GameObjects.CommanderPet.ReturnedCommander")
                .ToUpper())
        {
            CommanderType = eCommanderType.ReturnedCommander;
        }
        else if (upperName == LanguageMgr
                     .GetTranslation(LanguageMgr.DefaultLanguage, "GameObjects.CommanderPet.DecayedCommander")
                     .ToUpper())
        {
            CommanderType = eCommanderType.DecayedCommander;
        }
        else if (upperName == LanguageMgr
                     .GetTranslation(LanguageMgr.DefaultLanguage, "GameObjects.CommanderPet.SkeletalCommander")
                     .ToUpper())
        {
            CommanderType = eCommanderType.SkeletalCommander;
        }
        else if (upperName == LanguageMgr
                     .GetTranslation(LanguageMgr.DefaultLanguage, "GameObjects.CommanderPet.BoneCommander")
                     .ToUpper())
        {
            CommanderType = eCommanderType.BoneCommander;
        }
        else if (upperName == LanguageMgr
                     .GetTranslation(LanguageMgr.DefaultLanguage, "GameObjects.CommanderPet.DreadCommander")
                     .ToUpper())
        {
            CommanderType = eCommanderType.DreadCommander;
        }
        else if (upperName == LanguageMgr
                     .GetTranslation(LanguageMgr.DefaultLanguage, "GameObjects.CommanderPet.DreadArcher").ToUpper())
        {
            CommanderType = eCommanderType.DreadArcher;
        }
        else if (upperName == LanguageMgr
                     .GetTranslation(LanguageMgr.DefaultLanguage, "GameObjects.CommanderPet.DreadLich").ToUpper())
        {
            CommanderType = eCommanderType.DreadLich;
        }
        else if (upperName == LanguageMgr
                     .GetTranslation(LanguageMgr.DefaultLanguage, "GameObjects.CommanderPet.DreadGuardian")
                     .ToUpper())
        {
            CommanderType = eCommanderType.DreadGuardian;
        }
        else if (upperName == LanguageMgr
                     .GetTranslation(LanguageMgr.DefaultLanguage, "GameObjects.CommanderPet.DreadLord").ToUpper())
        {
        ...
        else
        {
            ...
            log.Warn(
                $"CommanderPet(): unrecognized commander name {Name} in npcTemplate {npcTemplate.TemplateId}, is Name in server default language?");
        }
        ...
        if (npcTemplate.TetherRange > 0)
            ...
            switch (CommanderType)
            {
                ...
                default:
                    log.Warn(
                        $"CommanderPet(): npcTemplate for unknown commander {Name} does not specify number of subpets, defaulting to 0");
        ...
        switch (CommanderType)
        {
            ...
            case eCommanderType.DreadLich:
                ...
                break;
            default:
                var oneHand = CanUseWeaponSlot(eActiveWeaponSlot.Standard);
                var twoHand = CanUseWeaponSlot(eActiveWeaponSlot.TwoHanded);
                if (oneHand && twoHand)
                    CommanderSwitchWeapon((eWeaponType) Util.Random((int) eWeaponType.OneHandAxe,
                        (int) eWeaponType.TwoHandSword));
                else if (oneHand)
                    CommanderSwitchWeapon((eWeaponType) Util.Random((int) eWeaponType.OneHandAxe,
                        (int) eWeaponType.OneHandSword));
                else if (twoHand)
                    CommanderSwitchWeapon((eWeaponType) Util.Random((int) eWeaponType.TwoHandAxe,
                        (int) eWeaponType.TwoHandSword));
                break;
        }
    ...
    static CommanderPet()
    {
        ...
        for (var i = (int) eWeaponType.MIN; i <= (int) eWeaponType.MAX; i++)
        {
            var itemTemp = GameServer.Database.FindObjectByKey<ItemTemplate>(WEAPON_KEYS[i]);
    ...
    {
        var ad =
            attackComponent.LivingMakeAttack(target, weapon, style, effectiveness, interruptDuration, dualWield);
        ...
        if (Taunting && ServerProperties.Properties.PET_BD_COMMANDER_TAUNT_VALUE > 100
                     ...
                     && ActiveWeaponSlot != eActiveWeaponSlot.Distance && target is GameNPC npc &&
                     npc.Brain is IOldAggressiveBrain aggroBrain)
        {
            var aggro = ad.Damage * ServerProperties.Properties.PET_BD_COMMANDER_TAUNT_VALUE / 100;
    ...
    {
        return
            false; // everything below this comment is added in 1.83, and should not exist in a strict 1.65 level. feel free to add it back in if desired
        if (!(source is GamePlayer player) || player != Owner)
            return false;

        var strargs = str.ToUpper().Split(' ');

        for (var i = 0; i < strargs.Length; i++)
        {
            var curStr = strargs[i];

            if (curStr == LanguageMgr.GetTranslation(player.Client.Account.Language,
                    "GameObjects.CommanderPet.WR.Const.Commander").ToUpper())
            {
                switch (CommanderType)
                {
                    case eCommanderType.DreadGuardian:
                        player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "GameObjects.CommanderPet.WR.DreadGuardian",
                                Name,
                                LanguageMgr.GetTranslation(player.Client.Account.Language,
                                    "GameObjects.CommanderPet.WR.Const.Harm"),
                                LanguageMgr.GetTranslation(player.Client.Account.Language,
                                    "GameObjects.CommanderPet.WR.Const.Empower"),
                                LanguageMgr.GetTranslation(player.Client.Account.Language,
                                    "GameObjects.CommanderPet.WR.Const.Combat")),
                            eChatType.CT_Say, eChatLoc.CL_PopupWindow);
                        break;
                    case eCommanderType.DreadLich:
                        player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "GameObjects.CommanderPet.WR.DreadLich",
                                Name,
                                LanguageMgr.GetTranslation(player.Client.Account.Language,
                                    "GameObjects.CommanderPet.WR.Const.Spells"),
                                LanguageMgr.GetTranslation(player.Client.Account.Language,
                                    "GameObjects.CommanderPet.WR.Const.Empower"),
                                LanguageMgr.GetTranslation(player.Client.Account.Language,
                                    "GameObjects.CommanderPet.WR.Const.Combat")),
                    ...
                    case eCommanderType.DreadArcher:
                        player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "GameObjects.CommanderPet.WR.DreadArcher",
                                Name,
                                LanguageMgr.GetTranslation(player.Client.Account.Language,
                                    "GameObjects.CommanderPet.WR.Const.Empower"),
                                LanguageMgr.GetTranslation(player.Client.Account.Language,
                    ...
                    default:
                        player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "GameObjects.CommanderPet.WR.XCommander",
                                Name,
                                LanguageMgr.GetTranslation(player.Client.Account.Language,
                                    "GameObjects.CommanderPet.WR.Const.Weapons"),
                                LanguageMgr.GetTranslation(player.Client.Account.Language,
                                    "GameObjects.CommanderPet.WR.Const.Combat")),
                            eChatType.CT_Say, eChatLoc.CL_PopupWindow);
                        break;
            ...
            else if (curStr == LanguageMgr
                         .GetTranslation(player.Client.Account.Language, "GameObjects.CommanderPet.WR.Const.Combat")
                         .ToUpper())
            {
                if (ControlledNpcList == null || ControlledNpcList.Length < 1)
                    player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language,
                            "GameObjects.CommanderPet.WR.CombatNoMinions",
                            Name,
                            LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "GameObjects.CommanderPet.WR.Const.Taunt")),
                        eChatType.CT_Say, eChatLoc.CL_PopupWindow);
                else
                    player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language,
                            "GameObjects.CommanderPet.WR.Combat",
                            Name,
                            LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "GameObjects.CommanderPet.WR.Const.Assist"),
                            LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "GameObjects.CommanderPet.WR.Const.Taunt")),
                        eChatType.CT_Say, eChatLoc.CL_PopupWindow);
            }
                         ...
                         .GetTranslation(player.Client.Account.Language, "GameObjects.CommanderPet.WR.Const.Assist")
                         .ToUpper())
            {
                if (ControlledNpcList != null && ControlledNpcList.Length > 0)
                {
                    ...
                    if (MinionsAssisting)
                        player.Out.SendMessage(
                            LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "GameObjects.CommanderPet.WR.Assist.On"), eChatType.CT_Say,
                            eChatLoc.CL_SystemWindow);
                    else
                        player.Out.SendMessage(
                            LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "GameObjects.CommanderPet.WR.Assist.Off"), eChatType.CT_Say,
                            eChatLoc.CL_SystemWindow);

                    // Refresh minion aggression state
                    if (Brain is IControlledBrain commBrain)
                        foreach (var minBrain in ControlledNpcList)
                            if (minBrain != null)
                                minBrain.SetAggressionState(commBrain.AggressionState);
                }
            }
            else if (curStr == LanguageMgr
                         .GetTranslation(player.Client.Account.Language, "GameObjects.CommanderPet.WR.Const.Taunt")
                         .ToUpper())
            {
                Taunting = !Taunting;
                ...
                if (Taunting)
                    player.Out.SendMessage(
                        LanguageMgr.GetTranslation(player.Client.Account.Language,
                            "GameObjects.CommanderPet.WR.CommStartTaunt"), eChatType.CT_Say,
                        eChatLoc.CL_SystemWindow);
                else
                    player.Out.SendMessage(
                        LanguageMgr.GetTranslation(player.Client.Account.Language,
                            "GameObjects.CommanderPet.WR.CommNoTaunt"), eChatType.CT_Say, eChatLoc.CL_SystemWindow);
            }

            else if (curStr == LanguageMgr
                         .GetTranslation(player.Client.Account.Language,
                             "GameObjects.CommanderPet.WR.Const.Weapons").ToUpper())
            {
                var oneHand = CanUseWeaponSlot(eActiveWeaponSlot.Standard);
                var twoHand = CanUseWeaponSlot(eActiveWeaponSlot.TwoHanded);
                if (oneHand && twoHand)
                    player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language,
                            "GameObjects.CommanderPet.WR.Weapons.All",
                            Name,
                            LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "GameObjects.CommanderPet.WR.Const.1HandedAxe"),
                            LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "GameObjects.CommanderPet.WR.Const.1HandedHammer"),
                            LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "GameObjects.CommanderPet.WR.Const.1HandedSword"),
                            LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "GameObjects.CommanderPet.WR.Const.2HandedAxe"),
                            LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "GameObjects.CommanderPet.WR.Const.2HandedHammer"),
                            LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "GameObjects.CommanderPet.WR.Const.2HandedSword")),
                        eChatType.CT_Say, eChatLoc.CL_PopupWindow);
                else if (oneHand)
                    player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language,
                            "GameObjects.CommanderPet.WR.Weapons.Limited",
                            Name,
                            LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "GameObjects.CommanderPet.WR.Const.1HandedAxe"),
                            LanguageMgr.GetTranslation(player.Client.Account.Language,
                    ...
                    player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language,
                        ...
                        eChatType.CT_Say, eChatLoc.CL_PopupWindow);
                         ...
                         .GetTranslation(player.Client.Account.Language, "GameObjects.CommanderPet.WR.Const.Spells")
                         .ToUpper())
            {
                if (CommanderType == eCommanderType.DreadLich)
                    player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language,
                            "GameObjects.CommanderPet.WR.DreadLich2",
                            Name,
                            LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "GameObjects.CommanderPet.WR.Const.Snares"),
                            LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "GameObjects.CommanderPet.WR.Const.Debilitating"),
                            LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "GameObjects.CommanderPet.WR.Const.Damage")),
                        eChatType.CT_Say, eChatLoc.CL_PopupWindow);
            }

            else if (curStr == LanguageMgr
                         .GetTranslation(player.Client.Account.Language,
                             "GameObjects.CommanderPet.WR.Const.Empower").ToUpper())
            {
                var found = false;
                switch (CommanderType)
                {
                    ...
                    case eCommanderType.DreadArcher:
                        ...
                        foreach (Spell spell in Spells)
                            if (spell.Name.ToUpper() == LanguageMgr.GetTranslation(LanguageMgr.DefaultLanguage,
                                    "GameObjects.CommanderPet.WR.Const.Empower").ToUpper())
                            {
                        ...
                        break;
                         ...
                         .GetTranslation(player.Client.Account.Language, "GameObjects.CommanderPet.WR.Const.Snares")
                         .ToUpper())
            {
                if (CommanderType == eCommanderType.DreadLich)
                {
                    if (CommSpellOther == null)
                    {
                        player.Out.SendMessage(
                            LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "GameObjects.CommanderPet.WR.Const.Spell.NotAvailable", Name), eChatType.CT_Say,
                    ...
                    else
                    {
                        ...
                        player.Out.SendMessage(
                            LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "GameObjects.CommanderPet.WR.DreadLich.Snare", Name), eChatType.CT_Say,
                            eChatLoc.CL_PopupWindow);
                    }
                }
            }

            else if (curStr == LanguageMgr.GetTranslation(player.Client.Account.Language,
                         "GameObjects.CommanderPet.WR.Const.Debilitating").ToUpper())
            {
                if (CommanderType == eCommanderType.DreadLich)
                {
                    ...
                    else
                    {
                        ...
                        player.Out.SendMessage(
                            LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "GameObjects.CommanderPet.WR.DreadLich.Debilitating", Name), eChatType.CT_Say,
                            eChatLoc.CL_PopupWindow);
                    }
                }
            }
                         ...
                         .GetTranslation(player.Client.Account.Language, "GameObjects.CommanderPet.WR.Const.Damage")
                         .ToUpper())
            {
                if (CommanderType == eCommanderType.DreadLich)
                {
                    if (CommSpellDamageDebuff != null || CommSpellDamage != null)
                    {
                        ...
                        player.Out.SendMessage(
                            LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "GameObjects.CommanderPet.WR.DreadLich.Damage", Name), eChatType.CT_Say,
                            eChatLoc.CL_PopupWindow);
                    }
                    else
                    {
                        player.Out.SendMessage(
                         ...
                         .GetTranslation(player.Client.Account.Language, "GameObjects.CommanderPet.WR.Const.One")
                         .ToUpper())
            {
                i++;
                ...
                CommanderSwitchWeapon(eActiveWeaponSlot.Standard, strargs[++i]);
                         ...
                         .GetTranslation(player.Client.Account.Language, "GameObjects.CommanderPet.WR.Const.Two")
                         .ToUpper())
            {
                i++;
                if (i + 1 >= strargs.Length)
            ...
            // German and possibly other languages use a single word for weapons, so we have to check for that
                         ...
                         "GameObjects.CommanderPet.WR.Const.1HandedAxe").ToUpper())
            {
                CommanderSwitchWeapon(eWeaponType.OneHandAxe);
            }
            else if (curStr == LanguageMgr.GetTranslation(player.Client.Account.Language,
                         "GameObjects.CommanderPet.WR.Const.1HandedHammer").ToUpper())
            {
                CommanderSwitchWeapon(eWeaponType.OneHandHammer);
            }
            else if (curStr == LanguageMgr.GetTranslation(player.Client.Account.Language,
                         "GameObjects.CommanderPet.WR.Const.1HandedSword").ToUpper())
            {
                CommanderSwitchWeapon(eWeaponType.OneHandSword);
            }
            else if (curStr == LanguageMgr.GetTranslation(player.Client.Account.Language,
                         "GameObjects.CommanderPet.WR.Const.2HandedAxe").ToUpper())
            {
                CommanderSwitchWeapon(eWeaponType.TwoHandAxe);
            }
            else if (curStr == LanguageMgr.GetTranslation(player.Client.Account.Language,
                         "GameObjects.CommanderPet.WR.Const.2HandedHammer").ToUpper())
            {
                CommanderSwitchWeapon(eWeaponType.TwoHandHammer);
            }
            else if (curStr == LanguageMgr.GetTranslation(player.Client.Account.Language,
                         "GameObjects.CommanderPet.WR.Const.2HandedSword").ToUpper())
            {
                CommanderSwitchWeapon(eWeaponType.TwoHandSword);
            }

            else if (curStr == LanguageMgr
                         .GetTranslation(player.Client.Account.Language, "GameObjects.CommanderPet.WR.Const.Harm")
                         .ToUpper())
            {
                if (CommanderType == eCommanderType.DreadGuardian)
                    player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language,
                            "GameObjects.CommanderPet.WR.DreadGuardian2",
                            Name,
                            LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "GameObjects.CommanderPet.WR.Const.Drain"),
                            LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "GameObjects.CommanderPet.WR.Const.Suppress")),
                        eChatType.CT_Say, eChatLoc.CL_PopupWindow);
            }

            else if (curStr == LanguageMgr
                         .GetTranslation(player.Client.Account.Language, "GameObjects.CommanderPet.WR.Const.Drain")
                         .ToUpper())
            {
                if (CommanderType == eCommanderType.DreadGuardian)
                {
                    ...
                    else
                    {
                        ...
                        player.Out.SendMessage(
                            LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "GameObjects.CommanderPet.WR.DreadGuardian.Drain", Name), eChatType.CT_Say,
                            eChatLoc.CL_PopupWindow);
                    }
                }
            }
                         ...
                         .GetTranslation(player.Client.Account.Language,
                             "GameObjects.CommanderPet.WR.Const.Suppress").ToUpper())
            {
                if (CommanderType == eCommanderType.DreadGuardian)
                {
                    if (CommSpellDamageDebuff != null || CommSpellDamage != null)
                    {
                        ...
                        player.Out.SendMessage(
                            LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "GameObjects.CommanderPet.WR.DreadGuardian.Suppress", Name), eChatType.CT_Say,
                            eChatLoc.CL_PopupWindow);
                    }
                    else
                    {
    ...
    private static readonly List<ItemTemplate> WEAPON_TEMPLATES = new((int) eWeaponType.MAX);
    ...
    public static ItemTemplate CreateBdPetWeapon(eWeaponType weaponType)
    {
        ...
        {
            weaponName = LanguageMgr.GetTranslation(LanguageMgr.DefaultLanguage,
        ...
        var bonedancer = LanguageMgr.GetTranslation(LanguageMgr.DefaultLanguage, "PlayerClass.Name.Bonedancer");
    ...
    public void CommanderSwitchWeapon(eActiveWeaponSlot slot, string weaponSpec)
    {
        ...
        {
            var upperWeapon = weaponSpec.ToUpper();

            if (slot == eActiveWeaponSlot.Standard)
            {
                if (upperWeapon == LanguageMgr
                        .GetTranslation(player.Client.Account.Language, "SkillBase.RegisterPropertyNames.Axe")
                        .ToUpper())
                    CommanderSwitchWeapon(eWeaponType.OneHandAxe);

                if (upperWeapon == LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "SkillBase.RegisterPropertyNames.Hammer").ToUpper())
                    CommanderSwitchWeapon(eWeaponType.OneHandHammer);

                if (upperWeapon == LanguageMgr
                        .GetTranslation(player.Client.Account.Language, "SkillBase.RegisterPropertyNames.Sword")
                        .ToUpper())
                    CommanderSwitchWeapon(eWeaponType.OneHandSword);
            }
            else
    ...
    {
        var itemTemp = GetWeaponTemplate(weaponType);
    ...
    public override void SortSpells()
    {
        ...
        foreach (var spell in HarmfulSpells)
            if (spell != null)
    ...
    {
        var brainlist = ControlledNpcList;
        if (brainlist == null) return false;
        foreach (var icb in brainlist)
            if (icb == controlledNpc)
                return false;

        if (controlledNpc.Owner != this)
            throw new ArgumentException(
                "ControlledNpc with wrong owner is set (player=" + Name + ", owner=" + controlledNpc.Owner.Name +
                ")", "controlledNpc");
    ...
    public override bool RemoveControlledNpc(IControlledBrain controlledNpc)
    {
        ...
        lock (ControlledNpcList)
        {
            ...
            if (found)
            {
                ...
                var tempBrain = m_controlledBrain[i];
