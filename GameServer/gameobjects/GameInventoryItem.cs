...
public class GameInventoryItem : InventoryItem, IGameInventoryItem, ITranslatableObject
{
    ...
    public virtual LanguageDataObject.eTranslationIdentifier TranslationIdentifier =>
        LanguageDataObject.eTranslationIdentifier.eItem;
    ...
    public static GameInventoryItem Create(ItemTemplate item)
    {
        ...
        if (!string.IsNullOrEmpty(classType))
        {
            ...
            if (itemUnique != null)
                gameItem = ScriptMgr.CreateObjectFromClassType<GameInventoryItem, ItemUnique>(classType,
                    itemUnique);
            else
    ...
    {
        var classType = item.Template.ClassType;

        if (!string.IsNullOrEmpty(classType))
        {
            var gameItem =
                ScriptMgr.CreateObjectFromClassType<GameInventoryItem, InventoryItem>(classType, item);
    ...
    {
        var worldItem = new WorldInventoryItem(this);

        var itemloc = player.GetPointFromHeading(player.Heading, 30);
    ...
    public virtual void OnStrikeTarget(GameLiving owner, GameObject target)
    {
        if (owner is GamePlayer)
        {
            ...
            if (ConditionPercent > 70 && Util.Chance(ServerProperties.Properties.ITEM_CONDITION_LOSS_CHANCE))
            {
                ...
                var con = GameObject.GetConLevel(player.Level, Level);
                if (con < -3.0)
                    con = -3.0;
                var sub = (int) (con + 4);
                if (oldPercent < 91) sub *= 2;
                    ...
                    if (ConditionPercent == 90)
                        player.Out.SendMessage(
                            LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "GamePlayer.Attack.CouldRepair", Name), eChatType.CT_System,
                            eChatLoc.CL_SystemWindow);
                    else if (ConditionPercent == 80)
                        player.Out.SendMessage(
                            LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "GamePlayer.Attack.NeedRepair", Name), eChatType.CT_System,
                            eChatLoc.CL_SystemWindow);
                    else if (ConditionPercent == 70)
                        player.Out.SendMessage(
                            LanguageMgr.GetTranslation(player.Client.Account.Language,
                                "GamePlayer.Attack.NeedRepairDire", Name), eChatType.CT_System,
                            eChatLoc.CL_SystemWindow);
    ...
    public virtual void Delve(List<string> delve, GamePlayer player)
    {
        ...
        if (IsCrafted)
        {
            delve.Add(LanguageMgr.GetTranslation(player.Client.Account.Language,
                "DetailDisplayHandler.HandlePacket.CrafterName", Creator));
            delve.Add(" ");
        }
        ...
        if (Object_Type == (int) eObjectType.Magical || Object_Type == (int) eObjectType.AlchemyTincture ||
            Object_Type == (int) eObjectType.SpellcraftGem)
        {
            WriteUsableClasses(delve, player.Client);
            WriteMagicalBonuses(delve, player.Client, false);
        }
        ...
        //***********************************
        if (Object_Type == (int) eObjectType.Poison) WritePoisonInfo(delve, player.Client);
        ...
        else if (CanUseEvery > 0)
        {
            ...
            var minutes = CanUseEvery / 60;
            var seconds = CanUseEvery % 60;
        ...
        if (BonusLevel > 0)
        {
            delve.Add(" ");
            delve.Add(LanguageMgr.GetTranslation(player.Client.Account.Language,
                "DetailDisplayHandler.HandlePacket.BonusLevel", BonusLevel));
        }

        //Add admin info
        if (player.Client.Account.PrivLevel > 1) WriteTechnicalInfo(delve, player.Client);
    }
    ...
    protected virtual void WriteUsableClasses(IList<string> output, GameClient client)
    {
        ...
        foreach (var allowed in Util.SplitCSV(AllowedClasses, true))
        {
            var classID = -1;
    ...
    protected virtual void WriteMagicalBonuses(IList<string> output, GameClient client, bool shortInfo)
    {
        ...
        if (!shortInfo)
        {
            ...
            {
                var requiredLevel = LevelRequirement > 0 ? LevelRequirement : Math.Min(50, Level);
                if (requiredLevel > 1)
                {
                    output.Add(LanguageMgr.GetTranslation(client.Account.Language,
            ...
            if (Object_Type == (int) eObjectType.Magical &&
                Item_Type == (int) eInventorySlot.FirstBackpack) // potion
            ...
            if (ProcSpellID != 0)
            {
                ...
                if (line != null)
                {
                    var procSpell = SkillBase.FindSpell(ProcSpellID, line);
            ...
            if (ProcSpellID1 != 0)
            {
                ...
                if (line != null)
                {
                    var procSpell = SkillBase.FindSpell(ProcSpellID1, line);
            ...
            if (SpellID != 0)
            {
                ...
                {
                    var spell = SkillBase.FindSpell(SpellID, chargeEffectsLine);
                    if (spell != null)
                    {
                        var spellHandler =
                        ...
                        if (spellHandler != null)
                        {
                            if (MaxCharges > 0)
                            {
                                ...
                                output.Add(LanguageMgr.GetTranslation(client.Account.Language,
                                    "DetailDisplayHandler.WriteMagicalBonuses.Charges", Charges));
                                output.Add(LanguageMgr.GetTranslation(client.Account.Language,
                                    "DetailDisplayHandler.WriteMagicalBonuses.MaxCharges", MaxCharges));
                                output.Add(" ");
                            }
            ...
            if (SpellID1 != 0)
            {
                ...
                if (chargeEffectsLine != null)
                {
                    var spell = SkillBase.FindSpell(SpellID1, chargeEffectsLine);
                    if (spell != null)
                    {
                        var spellHandler =
                            ScriptMgr.CreateSpellHandler(client.Player, spell, chargeEffectsLine);
                        ...
                        if (spellHandler != null)
                        {
                            if (MaxCharges > 0)
                            {
                                output.Add(LanguageMgr.GetTranslation(client.Account.Language,
                                    "DetailDisplayHandler.WriteMagicalBonuses.ChargedMagic"));
                                output.Add(LanguageMgr.GetTranslation(client.Account.Language,
                                    "DetailDisplayHandler.WriteMagicalBonuses.Charges", Charges1));
                                output.Add(LanguageMgr.GetTranslation(client.Account.Language,
                                    "DetailDisplayHandler.WriteMagicalBonuses.MaxCharges", MaxCharges1));
                                output.Add(" ");
                            }
                        ...
                        foreach (var spl in spells)
                            if (spl.ID == PoisonSpellID)
                            {
                                ...
                                output.Add(LanguageMgr.GetTranslation(client.Account.Language,
                                    "DetailDisplayHandler.WriteMagicalBonuses.ChargedMagic"));
                                output.Add(LanguageMgr.GetTranslation(client.Account.Language,
                                    "DetailDisplayHandler.WriteMagicalBonuses.Charges", PoisonCharges));
                                output.Add(LanguageMgr.GetTranslation(client.Account.Language,
                                    "DetailDisplayHandler.WriteMagicalBonuses.MaxCharges", PoisonMaxCharges));
                                output.Add(" ");
    ...
    protected virtual void WriteBonusLine(IList<string> list, GameClient client, int bonusCat, int bonusValue)
    {
            ...
            !SkillBase.CheckPropertyType((eProperty) bonusCat, ePropertyType.Focus))
        {
            ...
            {
                var singleUti = string.Format("{0:0.00}", GetSingleUtility(bonusCat, bonusValue));
                //- Axe: 5 pts
                //- Strength: 15 pts
                //- Constitution: 15 pts
                //- Hits: 40 pts
            ...
            if (poisonLine != null)
            {
                ...
                foreach (var spl in spells)
                    if (spl.ID == PoisonSpellID)
                    {
                        ...
                        list.Add(LanguageMgr.GetTranslation(client.Account.Language,
                            "DetailDisplayHandler.WritePoisonInfo.Charges", PoisonCharges));
                        list.Add(LanguageMgr.GetTranslation(client.Account.Language,
                            "DetailDisplayHandler.WritePoisonInfo.MaxCharges", PoisonMaxCharges));
                        list.Add(" ");
    ...
    {
        var mSpell = SkillBase.GetSpellByID(SpellID);
        list.Add(LanguageMgr.GetTranslation(client.Account.Language,
            "DetailDisplayHandler.WritePotionInfo.ChargedMagic"));
        list.Add(LanguageMgr.GetTranslation(client.Account.Language, "DetailDisplayHandler.WritePotionInfo.Charges",
            Charges));
        list.Add(LanguageMgr.GetTranslation(client.Account.Language,
            "DetailDisplayHandler.WritePotionInfo.MaxCharges", MaxCharges));
        list.Add(" ");
        ...
        var nextPotionAvailTime =
            client.Player.TempProperties.getProperty<long>("LastPotionItemUsedTick_Type" + mSpell.SharedTimerGroup);
        // Satyr Update: Individual Reuse-Timers for Pots need a Time looking forward
        // into Future, set with value of "itemtemplate.CanUseEvery" and no longer back into past
        if (nextPotionAvailTime > client.Player.CurrentRegion.Time)
        {
    ...
    protected static void WritePotionSpellsInfos(IList<string> list, GameClient client, Spell spl, NamedSkill line)
    {
        if (spl != null)
        {
            list.Add(LanguageMgr.GetTranslation(client.Account.Language,
                "DetailDisplayHandler.WriteMagicalBonuses.MagicAbility"));
            list.Add(LanguageMgr.GetTranslation(client.Account.Language,
                "DetailDisplayHandler.WritePotionInfo.Type", ((eSpellType) spl.SpellType).ToString()));
            list.Add(" ");
    ...
    {
        var itemDPS = DPS_AF / 10.0;
        var clampedDPS = Math.Min(itemDPS, 1.2 + 0.3 * player.Level);
        if (player.RealmLevel > 39)
            clampedDPS += 0.3;
        var itemSPD = SPD_ABS / 10.0;
        var effectiveDPS = clampedDPS * Quality / 100.0 * Condition / MaxCondition;
        ...
        if (itemDPS != 0) delve.Add("- " + effectiveDPS.ToString("0.0") + " DPS");
    }
    ...
    protected virtual void DelveArmorStats(List<string> delve, GamePlayer player)
    {
        ...
        double af = 0;
        var afCap = player.Level + (player.RealmLevel > 39 ? 1 : 0);
        ...
        if (DPS_AF != 0)
        {
            ...
            if (Object_Type != (int) eObjectType.Cloth) afCap *= 2;
    ...
    public virtual void WriteTechnicalInfo(List<string> delve, GameClient client)
    {
        ...
        delve.Add("   Value/Price: " + Money.GetShortString(Price) + " / " +
                  Money.GetShortString((long) (Price * (long) ServerProperties.Properties.ITEM_SELL_RATIO * .01)));
