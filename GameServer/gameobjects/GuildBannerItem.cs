...
public class GuildBannerItem : GameInventoryItem
{
    ...
    public eStatus Status => m_status;
    ...
    public override void OnReceive(GamePlayer player)
    {
        ...
        {
            var template = new ItemUnique(Template);
            template.ClassType = "";
            ...
            var trophy = new GameInventoryItem(template);
            player.Inventory.AddItem(eInventorySlot.FirstEmptyBackpack, trophy);
            OwnerGuild.SendMessageToGuildMembers(
                player.Name + " of " + GlobalConstants.RealmToName(player.Realm) +
                " has captured your guild banner!", eChatType.CT_Guild, eChatLoc.CL_SystemWindow);
        ...
        else
        {
            ...
            if (OwnerGuild != null)
                OwnerGuild.SendMessageToGuildMembers(player.Name + " has recovered your guild banner!",
                    eChatType.CT_Guild, eChatLoc.CL_SystemWindow);

            if (SummonPlayer != null) SummonPlayer.GuildBanner = null;
        }
    }
    ...
    public override void OnRemoveFromWorld()
    {
        if (Status == eStatus.Dropped)
        {
            ...
            if (OwnerGuild != null)
            {
                ...
                OwnerGuild.SendMessageToGuildMembers("Your guild banner has been lost!", eChatType.CT_Guild,
