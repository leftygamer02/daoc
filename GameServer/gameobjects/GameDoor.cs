...
public class GameDoor : GameLiving, IDoor
{
    ...
    public override void LoadFromDatabase(DataObject obj)
    {
        ...
        var m_dbdoor = obj as DBDoor;
        if (m_dbdoor == null) return;
        var curZone = WorldMgr.GetZone((ushort) (m_dbdoor.InternalID / 1000000));
        ...
        if (CurrentRegion.IsFrontier && (GameServer.Instance.Configuration.ServerType == eGameServerType.GST_PvE
                                         || GameServer.Instance.Configuration.ServerType ==
                                         eGameServerType.GST_PvP))
    ...
    /// </summary>
    public virtual ushort ZoneID => (ushort) (DoorID / 1000000);
    ...
    public virtual void Open(GameLiving opener = null)
    {
        ...
        if (HealthPercent > 40 || !m_openDead)
            lock (m_LockObject)
            {
                if (m_closeDoorAction == null) m_closeDoorAction = new CloseDoorAction(this);
    ...
    public override void TakeDamage(GameObject source, eDamageType damageType, int damageAmount, int criticalAmount)
    {
        ...
        {
            if (!m_openDead && Realm != eRealm.Door)
                attackerPlayer.Out.SendMessage(
                    LanguageMgr.GetTranslation(attackerPlayer.Client.Account.Language, "GameDoor.NowOpen", Name),
            ...
            if (!m_openDead && Realm != eRealm.Door)
            {
                ...
                {
                    attackerPlayer.Out.SendMessage(
                        LanguageMgr.GetTranslation(attackerPlayer.Client.Account.Language, "GameDoor.NowOpen",
                    ...
                    if (attackerGroup != null)
                        foreach (var living in attackerGroup.GetMembersInTheGroup())
                            ((GamePlayer) living).Out.SendMessage(
                                LanguageMgr.GetTranslation(attackerPlayer.Client.Account.Language,
                                    "GameDoor.NowOpen", Name), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                }
            }
        }
    }
    ...
    protected class CloseDoorAction : RegionECSAction
    {
        ...
        {
            var door = (GameDoor) m_actionSource;
            door.Close();
            return 0;
        }
    }
