...
public class GameMythirian : GameInventoryItem
{
    ...
    public override bool CanEquip(GamePlayer player)
    {
        ...
        {
            if (Type_Damage <= player.ChampionLevel) return true;

            player.Out.SendMessage("You do not meet the Champion Level requirement to equip this item.",
    ...
    public override void OnEquipped(GamePlayer player)
    {
        if (Name.ToLower().Contains("ektaktos"))
        {
            ...
            player.Out.SendMessage("You find yourself able to breathe water like air!", eChatType.CT_System,
    ...
    {
        if (Name.ToLower().Contains("ektaktos") &&
            SpellHelper.FindEffectOnTarget(player, typeof(WaterBreathingSpellHandler)) == null)
        {
