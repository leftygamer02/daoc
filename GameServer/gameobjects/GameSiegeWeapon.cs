...
public class GameSiegeWeapon : GameMovingObject
{
    ...
    /// </summary>
    protected readonly object m_decayTimerLock = new();
    ...
    /// </summary>
    protected readonly object m_controlRangeTimerLock = new();
    ...
    public int DecayedHp => 3 * (MaxHealth / 10);

    //Amount of HP to deduct from Siege Weapon on each Decay Tick
    public int DecayDeductHp => MaxHealth / 10;
    ...
    public void TakeControl(GamePlayer player)
    {
        ...
        {
            player.Out.SendMessage(GetName(0, true) + " is already under control.", eChatType.CT_Say,
        ...
        {
            player.Out.SendMessage("You already have a siege weapon under your control.", eChatType.CT_Say,
                eChatLoc.CL_SystemWindow);
            return;
        }
        ...
        if (IsMoving)
        {
            player.Out.SendMessage("You can't take control of a siege weapon while it is moving.", eChatType.CT_Say,
                eChatLoc.CL_SystemWindow);
            return;
        }
        ...
        player.Out.SendMessage("You take control of " + GetName(0, false) + ".", eChatType.CT_Say,
            eChatLoc.CL_SystemWindow);
        StartControlRangeCheck();
        if ((CurrentState & eState.Armed) != eState.Armed)
            Arm();
    }
    ...
    public virtual void ReleaseControl()
    {
        ...
        Owner.Out.SendMessage("You are no longer controlling " + GetName(0, false) + ".", eChatType.CT_Say,
            eChatLoc.CL_SystemWindow);
    ...
    {
        if (!CanUse()) return;
        if (SiegeWeaponTimer.IsAlive || IsMoving)
        {
            Owner.Out.SendMessage(GetName(0, true) + " isn't ready to be aimed yet!", eChatType.CT_Say,
                eChatLoc.CL_SystemWindow);
            return;
        }
        ...
        //Range Checks
        if (MinAttackRange != -1 && GetDistanceTo(Owner.TargetObject) < MinAttackRange)
        {
            Owner.Out.SendMessage("The " + GetName(0, false) + "'s target location is too close!", eChatType.CT_Say,
                eChatLoc.CL_SystemWindow);
            return;
        }

        if (MaxAttackRange != -1 && GetDistanceTo(Owner.TargetObject) > MaxAttackRange)
        {
            Owner.Out.SendMessage("The " + GetName(0, false) + "'s target is too far away to reach!",
        ...
        if (Owner != null)
            Owner.Out.SendMessage(
                GetName(0, true) + " is turning to your target. (" +
                (GetActionDelay(SiegeTimer.eAction.Aiming) / 1000).ToString("N") + "s)", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
    }
    ...
    public void Arm()
    {
        ...
        if (Owner != null)
            ...
            Owner.Out.SendMessage(
                "You prepare " + GetName(0, false) + " for firing. (" +
                (GetActionDelay(SiegeTimer.eAction.Arming) / 1000).ToString("N") + "s until armed)",
    ...
    public void Move()
    {
        ...
        foreach (IDoor door in Owner.CurrentRegion.GetDoorsInRadius(Owner.GroundTarget.X, Owner.GroundTarget.Y,
                     Owner.GroundTarget.Z, (ushort) (AttackRange - 50), false))
            if (door is GameKeepDoor)
            {
                Owner.Out.SendMessage("You can't move a ram that close to a door!", eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);
                return;
            }
    ...
    public virtual void Fire()
    {
        ...
        {
            if (Owner != null)
                Owner.Out.SendMessage("The " + Name + " is not ready to fire yet!", eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);
        ...
        //Range Checks
        if (MinAttackRange != -1 && GetDistanceTo(GroundTarget) < MinAttackRange)
        {
            Owner.Out.SendMessage("Your target is too close to this siege weapon!", eChatType.CT_Say,
                eChatLoc.CL_SystemWindow);
            return;
        }

        if (MaxAttackRange != -1 && GetDistanceTo(GroundTarget) > MaxAttackRange)
        {
            Owner.Out.SendMessage("Your target is too far away to this siege weapon!", eChatType.CT_Say,
                eChatLoc.CL_SystemWindow);
            return;
        }

        new ECSGameTimer(this, new ECSGameTimer.ECSTimerCallback(MakeDelayedDamage),
            GetActionDelay(SiegeTimer.eAction.Fire));
        BroadcastFireAnimation(GetActionDelay(SiegeTimer.eAction.Fire));
        if (Owner != null)
            Owner.Out.SendMessage("You fire " + GetName(0, false) + "!", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
        Arm();
    }
    ...
    */
    ...
    public void TryRepair()
    {
        ...
        if (TimesRepaired <= 3)
        {
            ...
            {
                Owner.Out.SendMessage("You must have woodworking skill to repair a siege weapon.", eChatType.CT_Say,
                    eChatLoc.CL_SystemWindow);
                return;
            }
        }
        ...
        {
            Owner.Out.SendMessage("The siegeweapon has decayed beyond repairs!", eChatType.CT_Say,
                eChatLoc.CL_SystemWindow);
            return;
        }

        var repairCommand = new RepairCommandHandler();
    ...
    public void salvage()
    {
        ...
        {
            Owner.Out.SendMessage("You must be a Siege weapon crafter to salvage it.", eChatType.CT_Say,
                eChatLoc.CL_SystemWindow);
            return;
        }
    ...
    public int GetActionDelay(SiegeTimer.eAction action)
    {
        if (action == SiegeTimer.eAction.Fire && GroundTarget != null)
            return (int) (ActionDelay[(int) action] + GetDistanceTo(GroundTarget));

        var delay = ActionDelay[(int) action];
        ...
        {
            var eff = Owner.EffectList.GetOfType<BannerOfBesiegingEffect>();
            if (eff != null)
    ...
    public bool CanUse()
    {
        ...
        {
            Owner.Out.SendMessage("You can't use this siegeweapon now!", eChatType.CT_Say,
                eChatLoc.CL_SystemWindow);
            return false;
        }
        ...
        if (Health <= DecayedHp)
        {
            Owner.Out.SendMessage("The siegeweapon needs to be repaired!", eChatType.CT_Say,
                eChatLoc.CL_SystemWindow);
            return false;
        }
        ...
        {
            Owner.Out.SendMessage("You are too far from your siege equipment to control it any longer!",
                eChatType.CT_System, eChatLoc.CL_SystemWindow);
            return false;
        }
        ...
        {
            Owner.Out.SendMessage($"This siege equipment is owned by an enemy realm!", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            return false;
        }
        ...
        if (Owner != null)
        {
            if (this is GameSiegeRam) //Ram Siege Interface is 2 seconds fast for some reason. adding 2 seconds to the action delay for rams
                Owner.Out.SendSiegeWeaponInterface(this,
                    (GetActionDelay(SiegeWeaponTimer.CurrentAction) + 2000) / 100);
            else
    ...
    private int ControlRangeTimerCallback(ECSGameTimer callingTimer)
    {
        ...
        if (GetDistanceTo(Owner) > SIEGE_WEAPON_CONTROLE_DISTANCE)
        {
            ReleaseControl();
            return 0;
        }
...
public class SiegeTimer : RegionECSAction
{
    ...
    protected override int OnTick(ECSGameTimer timer)
    {
        ...
        if (SiegeWeapon.Owner != null) SiegeWeapon.Owner.Out.SendSiegeWeaponInterface(SiegeWeapon, 0);
