...
public class GamePet : GameNPC
{
    ...
    public override double Effectiveness
    {
        get
        {
            var gl = (Brain as IControlledBrain).GetLivingOwner();
            if (gl != null)
                return m_effectiveness; //gl.Effectiveness;

            return 1.0;
        }
        set => m_effectiveness = value;
    }
    ...
    public virtual void SortSpells(int scaleLevel)
    {
        ...
        else
        {
            ...
            if (Properties.PET_LEVELS_WITH_OWNER ||
                ...
                Name.Contains("underhill") || Name.Contains("simulacrum") ||
                Name.Contains("spirit") || this is TheurgistPet)
            {
                ...
                if (CanCastHarmfulSpells)
                    for (var i = 0; i < HarmfulSpells.Count; i++)
                    {
                ...
                if (CanCastInstantHarmfulSpells)
                    for (var i = 0; i < InstantHarmfulSpells.Count; i++)
                    {
                ...
                if (CanCastHealSpells)
                    for (var i = 0; i < HealSpells.Count; i++)
                    {
                ...
                if (CanCastInstantHealSpells)
                    for (var i = 0; i < InstantHealSpells.Count; i++)
                    {
                ...
                if (CanCastInstantMiscSpells)
                    for (var i = 0; i < InstantMiscSpells.Count; i++)
                    {
                ...
                if (CanCastMiscSpells)
                    for (var i = 0; i < MiscSpells.Count; i++)
                    {
    ...
    public virtual void ScalePetSpell(Spell spell, int casterLevel = 0)
    {
        ...
        switch ((eSpellType) spell.SpellType)
        {
            ...
            case eSpellType.StyleBleeding: // Style bleed effect
                var scaleFactor = (casterLevel + 1.0) / Properties.PET_SCALE_SPELL_MAX_LEVEL;
            ...
            case eSpellType.SavageCombatSpeedBuff:
                ...
                spell.Value *= (double) casterLevel / Properties.PET_SCALE_SPELL_MAX_LEVEL;
                spell.ScaledToPetLevel = true;
                break;
            // Scale Duration
            ...
            case eSpellType.StyleSpeedDecrease: // Style hinder effet
                spell.Duration = (int) Math.Ceiling(spell.Duration * (double) casterLevel /
                                                    Properties.PET_SCALE_SPELL_MAX_LEVEL);
                spell.ScaledToPetLevel = true;
                break;
            ...
            case eSpellType.DirectDamageWithDebuff:
                ...
                spell.Value *= (double) casterLevel / Properties.PET_SCALE_SPELL_MAX_LEVEL;
                spell.Damage *= (double) casterLevel / Properties.PET_SCALE_SPELL_MAX_LEVEL;
                spell.Duration = (int) Math.Ceiling(spell.Duration * (double) casterLevel /
                                                    Properties.PET_SCALE_SPELL_MAX_LEVEL);
                spell.ScaledToPetLevel = true;
                break;
    ...
    public int AttackSpeed(params InventoryItem[] weapons)
    {
        ...
        var speed = 100 * weaponSpeed * (1.0 - (GetModified(eProperty.Quickness) - 60) / 500.0);
        return (int) Math.Max(500.0,
            speed * (double) GetModified(eProperty.MeleeSpeed) *
            0.01); // no bonus is 100%, opposite how players work
    }
    ...
    private object _buffedTargetsLock = new();
    ...
    public virtual void StripBuffs()
    {
        ...
        {
            if (m_buffedTargets != null)
                foreach (var living in m_buffedTargets)
                    if (living != this && living.EffectList != null)
                        foreach (var effect in living.EffectList)
                            if (effect is GameSpellEffect spellEffect && spellEffect.SpellHandler != null
                                                                      && spellEffect.SpellHandler.Caster != null &&
    ...
    public override bool IsObjectGreyCon(GameObject obj)
    {
        ...
        if (Brain is IControlledBrain)
        {
            var player = (Brain as IControlledBrain).GetLivingOwner();
            if (player != null)
                tempobj = player;
        }
