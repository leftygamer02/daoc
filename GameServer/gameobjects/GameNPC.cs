...
public class GameNPC : GameLiving, ITranslatableObject
{
    ...
    public virtual LanguageDataObject.eTranslationIdentifier TranslationIdentifier =>
        LanguageDataObject.eTranslationIdentifier.eNPC;
    ...
    public override byte Level
    {
        ...
        set
        {
            var bMaxHealth = m_health == MaxHealth;
    ...
    public override eRealm Realm
    {
        get
        {
            var brain = Brain as IControlledBrain;
            if (brain != null)
    ...
    public override void ChangeBaseStat(eStat stat, short val)
    {
        ...
        if (this != null && oldstat != newstat)
            ...
            {
                case eStat.STR:
                    npc.Strength = (short) newstat;
                    break;
                case eStat.DEX:
                    npc.Dexterity = (short) newstat;
                    break;
                case eStat.CON:
                    npc.Constitution = (short) newstat;
                    break;
                case eStat.QUI:
                    npc.Quickness = (short) newstat;
                    break;
                case eStat.INT:
                    npc.Intelligence = (short) newstat;
                    break;
                case eStat.PIE:
                    npc.Piety = (short) newstat;
                    break;
                case eStat.EMP:
                    npc.Empathy = (short) newstat;
                    break;
                case eStat.CHR:
                    npc.Charisma = (short) newstat;
                    break;
            }
    }
    ...
    public override bool IsUnderwater => (m_flags & eFlags.SWIMMING) == eFlags.SWIMMING || base.IsUnderwater;
    ...
    /// </summary>
    public virtual bool IsVisibleToPlayers => (uint) GameLoop.GameLoopTime - m_lastVisibleToPlayerTick < 60000;
    ...
    public override int X
    {
        get
        {
            ...
            {
                var expectedDistance = FastMath.Abs((long) TargetPosition.X - m_x);

                if (expectedDistance == 0)
                    return TargetPosition.X;

                var actualDistance = FastMath.Abs((long) (MovementElapsedTicks * TickSpeedX));
    ...
    public override int Y
    {
        get
        {
            ...
            if (TargetPosition.X != 0 || TargetPosition.Y != 0 || TargetPosition.Z != 0)
            {
                var expectedDistance = FastMath.Abs((long) TargetPosition.Y - m_y);

                if (expectedDistance == 0)
                    return TargetPosition.Y;

                var actualDistance = FastMath.Abs((long) (MovementElapsedTicks * TickSpeedY));
    ...
    public override int Z
    {
        get
        {
            ...
            if (TargetPosition.X != 0 || TargetPosition.Y != 0 || TargetPosition.Z != 0)
            {
                var expectedDistance = FastMath.Abs((long) TargetPosition.Z - m_z);

                if (expectedDistance == 0)
                    return TargetPosition.Z;

                var actualDistance = FastMath.Abs((long) (MovementElapsedTicks * TickSpeedZ));
    ...
    private bool m_wasStealthed = false;
    ...
    protected override void UpdateTickSpeed()
    {
        ...
        if (TargetPosition.X != 0 || TargetPosition.Y != 0 || TargetPosition.Z != 0)
        {
            ...
            var dx = (double) (TargetPosition.X - m_x) / dist;
            var dy = (double) (TargetPosition.Y - m_y) / dist;
            var dz = (double) (TargetPosition.Z - m_z) / dist;
    ...
    /// </summary>
    public bool IsAtTargetPosition => X == TargetPosition.X && Y == TargetPosition.Y && Z == TargetPosition.Z;
    ...
    public virtual void TurnTo(ushort heading, bool sendUpdate)
    {
        ...
        if (sendUpdate)
            ...
            else if (base.Heading != heading) base.Heading = heading;
    }
    ...
    public virtual void TurnTo(GameObject target, int duration)
    {
        ...
        var restore =
            (RestoreHeadingAction) TempProperties.getProperty<object>(RESTORE_HEADING_ACTION_PROP, null);
    ...
    /// </summary>
    public uint LastUpdateTickCount => m_lastUpdateTickCount;
    ...
    protected class ArriveAtTargetAction : RegionECSAction
    {
        ...
        protected override int OnTick(ECSGameTimer timer)
        {
            ...
            var arriveAtSpawnPoint = npc.IsReturningToSpawnPoint;
            ...
            {
                var waitTimer = new WaypointDelayAction(npc);
    ...
    public virtual void WalkTo(IPoint3D target, short speed)
    {
        ...
        MovementStartTick =
            GameLoop.GameLoopTime; // Adding this to prevent pets from warping when using GoTo and Here on the same target twice.
        UpdateTickSpeed();
        var ticksToArrive = GetTicksToArriveAt(TargetPosition, speed);
    ...
    public virtual void WalkToSpawn(short speed)
    {
        ...
        var brain = Brain as StandardMobBrain;

        if (brain != null && brain.HasAggro) brain.ClearAggroList();
    ...
    /// </summary>
    public GameObject CurrentFollowTarget => m_followTarget.Target as GameObject;
    ...
    public virtual void Follow(GameObject target, int minDistance, int maxDistance)
    {
        ...
        if (m_followTimer != null && m_followTimer.IsAlive && m_followTarget?.Target == target &&
            m_followMinDist == minDistance && m_followMaxDist == maxDistance)
        {
            return;
        }
    ...
    public virtual void StopFollowing()
    {
        lock (m_followTimer)
        {
            if (m_followTimer.IsAlive) m_followTimer.Stop();
    ...
    public virtual void FollowTargetInRange()
    {
        if (attackComponent.AttackState)
        {
            ...
            var ad = (AttackData) TempProperties.getProperty<object>(LAST_ATTACK_DATA, null);
        ...
        //sirru
        else if (attackComponent.Attackers.Count == 0 && Spells.Count > 0 && TargetObject != null &&
                 GameServer.ServerRules.IsAllowedToAttack(this, TargetObject as GameLiving, true))
        {
            ...
            if (CurrentRegion.Time - LastAttackedByEnemyTick > 10 * 1000)
            {
                ...
                var stanBrain = (StandardMobBrain) Brain;
                if (stanBrain != null)
    ...
    {
        var followSpeedScaler = 2.5; //This is used to scale the follow speed based on the distance from target

        if (IsCasting)
            return Properties.GAMENPC_FOLLOWCHECK_TIME;

        var wasInRange = m_followTimer.Properties.getProperty(FOLLOW_TARGET_IN_RANGE, false);
        m_followTimer.Properties.removeProperty(FOLLOW_TARGET_IN_RANGE);

        var followTarget = (GameObject) m_followTarget.Target;
        var followLiving = followTarget as GameLiving;
        ...
        if (followTarget == null || followTarget.ObjectState != eObjectState.Active ||
            CurrentRegionID != followTarget.CurrentRegionID)
        {
            StopFollowing();
            //Notify(GameNPCEvent.FollowLostTarget, this, new FollowLostTargetEventArgs(followTarget));
            return 0;
        ...
        if (Brain is StandardMobBrain)
        {
            ...
            if (!(Brain is IControlledBrain))
                ...
                {
                    var seconds = 25 + brain.GetAggroAmountForLiving(followLiving) / (MaxHealth + 1) * 100;
                    var lastattacked = LastAttackTick;
                    var lasthit = LastAttackedByEnemyTick;
                    if (GameLoop.GameLoopTime - lastattacked > seconds * 1000 &&
                        GameLoop.GameLoopTime - lasthit > seconds * 1000
                        && lasthit != 0)
                    {
                        //StopFollow();
                        //Notify(GameNPCEvent.FollowLostTarget, this, new FollowLostTargetEventArgs(followTarget));
            ...
            {
                var followspeed =
                    (short) Math.Max(Math.Min(MaxSpeed, GetDistance(new Point2D(newX, newY)) * followSpeedScaler),
                        50);
        ...
        var minAllowedFollowDistance = MIN_ALLOWED_FOLLOW_DISTANCE;
        ...
        //Are we in range yet?
        if ((int) distance <=
            (m_followMinDist < minAllowedFollowDistance ? minAllowedFollowDistance : m_followMinDist))
        {
            StopMoving();
            TurnTo(followTarget);
            if (!wasInRange)
    ...
    /// </summary>
    public bool IsMovingOnPath => m_IsMovingOnPath;
    ...
    protected void OnArriveAtWaypoint(DOLEvent e, object n, EventArgs args)
    {
        ...
        if (CurrentWayPoint != null)
        {
            var waitTimer = new WaypointDelayAction(this);
    ...
    protected class WaypointDelayAction : RegionECSAction
    {
        ...
        protected override int OnTick(ECSGameTimer timer)
        {
            ...
            if (!npc.IsMovingOnPath)
                return 0;
            var oldPathPoint = npc.CurrentWayPoint;
            var nextPathPoint = npc.CurrentWayPoint.Next;
            ...
            if (npc.CurrentWayPoint != null)
                npc.WalkTo(npc.CurrentWayPoint,
                    (short) Math.Min(npc.PathingNormalSpeed, npc.CurrentWayPoint.MaxSpeed));
            else
                npc.StopMovingOnPath();
    ...
    public override void LoadFromDatabase(DataObject obj)
    {
        ...
        if (MeleeDamageType == 0) MeleeDamageType = eDamageType.Slash;
        ...
        if (dbMob.RespawnInterval == -1) dbMob.RespawnInterval = 0;
        ...
        var aggroBrain = Brain as IOldAggressiveBrain;
        if (aggroBrain != null)
        {
            ...
            if (aggroBrain.AggroRange == Constants.USE_AUTOVALUES)
            {
                if (Realm == eRealm.None)
                {
                    ...
                    if (Name != Name.ToLower()) aggroBrain.AggroRange = 500;

                    if (CurrentRegion.IsDungeon) aggroBrain.AggroRange = 300;
                }
                else
                {
            ...
            if (aggroBrain.AggroLevel == Constants.USE_AUTOVALUES)
            {
                ...
                if (Level > 5) aggroBrain.AggroLevel = 30;

                if (Name != Name.ToLower()) aggroBrain.AggroLevel = 30;

                if (Realm != eRealm.None) aggroBrain.AggroLevel = 60;
            }
        }
    ...
    {
        if (Brain != null && Brain is IControlledBrain) return;
    ...
    public override void SaveIntoDatabase()
    {
        ...
        Mob mob = null;
        if (InternalID != null) mob = GameServer.Database.FindObjectByKey<Mob>(InternalID);
    ...
    public virtual void LoadTemplate(INpcTemplate template)
    {
        ...
        if (template.ReplaceMobValues)
        {
            ...
            Level = choosenLevel; // Also calls AutosetStats()
        }
        ...
        var choosenGender = template.Gender > 2 ? Util.Random(0, 2) : template.Gender;
        ...
        {
            var equipHasItems = false;
            var equip = new GameNpcInventoryTemplate();
            ...
            {
                foreach (var str in equipIDs) m_templatedInventory.Add(str);
                    ...
                    if (slotXModels.Length == 2)
                    {
                        ...
                        if (int.TryParse(slotXModels[0], out slot))
                        {
                            ...
                            if (tempModels.Count > 0)
                                equipHasItems |= equip.AddNPCEquipment((eInventorySlot) slot,
                                    tempModels[Util.Random(tempModels.Count - 1)]);
                        }
                    }
                }
            }
    ...
    /// </summary>
    protected readonly ArrayList m_questListToGive = new();

    /// <summary>
    /// Gets the questlist of this player
    /// </summary>
    public IList QuestListToGive => m_questListToGive;
    ...
    public void AddQuestToGive(Type questType)
    {
        lock (m_questListToGive.SyncRoot)
        {
            ...
            {
                var newQuest = (AbstractQuest) Activator.CreateInstance(questType);
    ...
    public int CanGiveQuest(Type questType, GamePlayer player)
    {
        ...
        {
            foreach (AbstractQuest q in m_questListToGive)
                if (q.GetType().Equals(questType) && q.CheckQuestQualification(player) &&
                    player.HasFinishedQuest(questType) < q.MaxQuestCount)
    ...
    /// </summary>
    public virtual bool ShowTeleporterIndicator => false;
    ...
    public bool CanShowOneQuest(GamePlayer player)
    {
        ...
        lock (m_questListToGive.SyncRoot)
        {
            foreach (AbstractQuest q in m_questListToGive)
            {
                var questType = q.GetType();
                var doingQuest = player.IsDoingQuest(questType) != null ? 1 : 0;
                if (q.CheckQuestQualification(player) &&
                    player.HasFinishedQuest(questType) + doingQuest < q.MaxQuestCount)
                    return true;
            }
        }
    ...
    public bool CanFinishOneQuest(GamePlayer player)
    {
        ...
        foreach (var q in dqs)
        {
            // Handle Data Quest here.

            DataQuest quest = null;
            if (q is DataQuest) quest = (DataQuest) q;

            if (quest != null && quest.TargetName == Name &&
                (quest.TargetRegion == 0 || quest.TargetRegion == CurrentRegionID))
            ...
            if (q is RewardQuest) rwQuest = (RewardQuest) q;

            if (rwQuest != null && rwQuest.QuestGiver == this)
            {
                var done = true;
                foreach (var goal in rwQuest.Goals) done &= goal.IsAchieved;
    ...
    {
        var quest = HasQuest(questType);
        if (quest != null)
        {
            var newQuest =
                (AbstractQuest) Activator.CreateInstance(questType, new object[] {player, startStep});
    ...
    {
        var exists = RiderArrayLocation(rider);
        if (exists != -1)
            return false;
    ...
    public virtual bool RiderDismount(bool forced, GamePlayer player)
    {
        ...
        var slot = RiderArrayLocation(player);
    ...
    {
        var location = RiderArrayLocation(player);
        if (location == -1)
            return location;
    ...
    /// </summary>
    public virtual int MAX_PASSENGERS => 1;

    /// <summary>
    /// The minimum number of passengers required to move
    /// </summary>
    public virtual int REQUIRED_PASSENGERS => 1;

    /// <summary>
    /// The slot offset for this NPC
    /// </summary>
    public virtual int SLOT_OFFSET => 0;
    ...
    public GamePlayer[] CurrentRiders
    {
        get
        {
            var list = new List<GamePlayer>(MAX_PASSENGERS);
            for (var i = 0; i < MAX_PASSENGERS; i++)
            {
    ...
    public override bool AddToWorld()
    {
        ...
        {
            var path = MovementMgr.LoadPath(PathID);
            if (path != null)
            {
        ...
        if (!InCombat && IsAlive && base.Health < MaxHealth) base.Health = MaxHealth;
    ...
    public override bool RemoveFromWorld()
    {
        if (IsMovingOnPath)
            StopMovingOnPath();
        if (MAX_PASSENGERS > 0)
            foreach (var player in CurrentRiders)
    ...
    public virtual bool MoveInRegion(ushort regionID, int x, int y, int z, ushort heading, bool forceMove)
    {
        ...
        if (forceMove == false)
        {
            ...
            var controlledBrain = Brain as ControlledNpcBrain;
        ...
        foreach (GamePlayer player in GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE))
        {
            ...
            if (m_inventory != null) player.Out.SendLivingEquipmentUpdate(this);
        }
    ...
    /// </summary>
    private ArrayList m_brains = new(1);

    /// <summary>
    /// The sync object for brain changes
    /// </summary>
    private readonly object m_brainSync = new();

    /// <summary>
    /// Gets the brain sync object
    /// </summary>
    public object BrainSync => m_brainSync;
    ...
    public virtual ABrain SetOwnBrain(ABrain brain)
    {
        ...
        lock (BrainSync)
        {
            var oldBrain = m_ownBrain;
            var activate = oldBrain.IsActive;
            if (activate)
                oldBrain.Stop();
            m_ownBrain = brain;
    ...
    public virtual void AddBrain(ABrain newBrain)
    {
        ...
        {
            Brain.Stop();
            var brains = new ArrayList(m_brains);
            brains.Add(newBrain);
    ...
    public virtual bool RemoveBrain(ABrain removeBrain)
    {
        ...
        {
            var brains = new ArrayList(m_brains);
            var index = brains.IndexOf(removeBrain);
            if (index < 0)
                //Console.WriteLine("Brain index < 0");
                return false;

            var active = brains[index] == Brain;
            if (active)
                removeBrain.Stop();
    ...
    public virtual string GetAggroLevelString(GamePlayer player, bool firstLetterUppercase)
    {
        ...
        if (Faction != null && aggroBrain != null && aggroBrain.AggroLevel > 0 && aggroBrain.AggroRange > 0)
        {
            ...
            if (GameServer.ServerRules.IsSameRealm(this, player, true))
            {
                if (firstLetterUppercase)
                    aggroLevelString = LanguageMgr.GetTranslation(player.Client.Account.Language,
            ...
            {
                aggroLevelString = LanguageMgr.GetTranslation(player.Client.Account.Language,
                    "GameNPC.GetAggroLevelString.Hostile1");
            }
        ...
        return LanguageMgr.GetTranslation(player.Client.Account.Language, "GameNPC.GetAggroLevelString.TowardsYou",
            aggroLevelString);
    }
        ...
        switch (Gender)
        {
                ...
                switch (form)
                {
                    case 1:
                        return Capitalize(capitalize,
                            LanguageMgr.GetTranslation(lang, "GameLiving.Pronoun.Male.Possessive"));
                    case 2:
                        return Capitalize(capitalize,
                            LanguageMgr.GetTranslation(lang, "GameLiving.Pronoun.Male.Objective"));
                    default:
                        return Capitalize(capitalize,
                            LanguageMgr.GetTranslation(lang, "GameLiving.Pronoun.Male.Subjective"));
                }
            ...
            case eGender.Female:
                switch (form)
                {
                    case 1:
                        return Capitalize(capitalize,
                            LanguageMgr.GetTranslation(lang, "GameLiving.Pronoun.Female.Possessive"));
                    case 2:
                        return Capitalize(capitalize,
                            LanguageMgr.GetTranslation(lang, "GameLiving.Pronoun.Female.Objective"));
                    default:
                        return Capitalize(capitalize,
                            LanguageMgr.GetTranslation(lang, "GameLiving.Pronoun.Female.Subjective"));
                }
            default:
                switch (form)
                {
                    case 1:
                        return Capitalize(capitalize,
                            LanguageMgr.GetTranslation(lang, "GameLiving.Pronoun.Neutral.Possessive"));
                    case 2:
                        return Capitalize(capitalize,
                            LanguageMgr.GetTranslation(lang, "GameLiving.Pronoun.Neutral.Objective"));
                    default:
                        return Capitalize(capitalize,
                            LanguageMgr.GetTranslation(lang, "GameLiving.Pronoun.Neutral.Subjective"));
                }
        }
    }
    ...
    {
        var language = Properties.DB_LANGUAGE;
        ...
        switch (player.Client.Account.Language)
        {
            case "EN":
            {
                ...
                list.Add(LanguageMgr.GetTranslation(player.Client.Account.Language,
                    "GameNPC.GetExamineMessages.YouExamine", GetName(0, false), GetPronoun(0, true),
    ...
    public override bool Interact(GamePlayer player)
    {
        ...
        if (!GameServer.ServerRules.IsSameRealm(this, player, true) && Faction != null &&
            Faction.GetAggroToFaction(player) > 50)
        {
            player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language,
                    "GameNPC.Interact.DirtyLook",
                    GetName(0, true, player.Client.Account.Language, this)), eChatType.CT_System,
        ...
        if (MAX_PASSENGERS > 1)
        {
            ...
            {
                player.Out.SendMessage($"This siege equipment is owned by an enemy realm!", eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);
                return false;
            }
            ...
            if (RiderSlot(player) != -1)
            {
                player.Out.SendMessage(
                    LanguageMgr.GetTranslation(player.Client.Account.Language, "GameNPC.Interact.AlreadyRiding",
            ...
            {
                player.Out.SendMessage(
                    LanguageMgr.GetTranslation(player.Client.Account.Language, "GameNPC.Interact.IsFull", name),
                    eChatType.CT_System, eChatLoc.CL_SystemWindow);
                return false;
            }

            if (player.IsRiding) player.DismountSteed(true);
    ...
    public virtual void SayTo(GamePlayer target, eChatLoc loc, string message, bool announce = true)
    {
        ...
        TurnTo(target);
        var resultText = LanguageMgr.GetTranslation(target.Client.Account.Language, "GameNPC.SayTo.Says",
            GetName(0, true, target.Client.Account.Language, this), message);
        switch (loc)
        {
            case eChatLoc.CL_PopupWindow:
                target.Out.SendMessage(resultText, eChatType.CT_System, eChatLoc.CL_PopupWindow);
                if (announce)
                    Message.ChatToArea(this,
                        LanguageMgr.GetTranslation(target.Client.Account.Language, "GameNPC.SayTo.SpeaksTo",
                            GetName(0, true, target.Client.Account.Language, this), target.GetName(0, false)),
                        eChatType.CT_System, WorldMgr.SAY_DISTANCE, target);
    ...
    public virtual bool IsWorthReward
    {
        get
        {
            if (CurrentRegion == null || CurrentRegion.Time - CHARMED_NOEXP_TIMEOUT <
                TempProperties.getProperty<long>(CHARMED_TICK_PROP))
                return false;
            if (Brain is IControlledBrain)
                return false;

            var XPGainerList = new HybridDictionary();
            lock (m_xpGainers.SyncRoot)
            {
                foreach (DictionaryEntry gainer in m_xpGainers) XPGainerList.Add(gainer.Key, gainer.Value);
            }

            if (XPGainerList.Keys.Count == 0) return false;
            foreach (DictionaryEntry de in XPGainerList)
            {
                var obj = (GameObject) de.Key;
        ...
        try
        {
            ...
            if (killer != null)
            {
                ...
                if (killer is GamePlayer player)
                    player.Out.SendMessage(GetName(0, true) + " dies!", eChatType.CT_PlayerDied,
                        eChatLoc.CL_SystemWindow);
            ...
            if (killer != null)
            {
                ...
                if (Faction != null && killer is GamePlayer)
                    lock (m_xpGainers.SyncRoot)
                    {
                        ...
                        {
                            var playerXpGainer = xpGainer as GamePlayer;
    ...
    protected override bool CheckRangedAttackInterrupt(GameLiving attacker, AttackData.eAttackType attackType)
    {
        ...
        if (MaxSpeedBase == 0 && attackType is AttackData.eAttackType.Ranged or AttackData.eAttackType.Spell &&
            !IsWithinRadius(attacker, 150))
            return false;

        var interrupted = base.CheckRangedAttackInterrupt(attacker, attackType);
    ...
    public virtual int RespawnInterval
    {
        get
        {
            ...
            var minutes = Util.Random(Properties.NPC_MIN_RESPAWN_INTERVAL,
                Properties.NPC_MIN_RESPAWN_INTERVAL + 5);

            if (Name != Name.ToLower()) minutes += 5;
            ...
            {
                var add = Level - 65 + Properties.NPC_MIN_RESPAWN_INTERVAL;
    ...
    public virtual void StartRespawn()
    {
        ...
        var respawnInt = RespawnInterval;
        var minBound = (int) Math.Floor(respawnInt * .95);
        var maxBound = (int) Math.Floor(respawnInt * 1.05);
    ...
    {
        var period = base.HealthRegenerationTimerCallback(selfRegenerationTimer);
        if (!InCombat)
        {
    ...
    public virtual void DropLoot(GameObject killer)
    {
        ...
        var droplist = new ArrayList();
        var autolootlist = new ArrayList();
        var aplayer = new ArrayList();
        ...
        var lootTemplates = LootMgr.GetLoot(this, killer);
        ...
        foreach (var lootTemplate in lootTemplates)
        {
            ...
            if (GameMoney.IsItemMoney(lootTemplate.Name))
            {
                ...
                if (Properties.ENABLE_ZONE_BONUSES)
                {
                    ...
                    var zoneBonus = (int) value * ZoneBonus.GetCoinBonus(killerPlayer) / 100;
                    if (zoneBonus > 0)
                    {
                        var amount = (long) (zoneBonus * Properties.MONEY_DROP);
                        killerPlayer.AddMoney(amount,
                            ZoneBonus.GetBonusMessage(killerPlayer,
                                (int) (zoneBonus * Properties.MONEY_DROP),
                                ZoneBonus.eZoneBonusType.COIN),
                            eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                        InventoryLogging.LogInventoryAction(this, killerPlayer, eInventoryActionType.Loot, amount);
                    }
                }

                if (Keeps.KeepBonusMgr.RealmHasBonus(Keeps.eKeepBonusType.Coin_Drop_5,
                        (eRealm) killer.Realm))
                    value += value / 100 * 5;
                else if (Keeps.KeepBonusMgr.RealmHasBonus(Keeps.eKeepBonusType.Coin_Drop_3,
                             (eRealm) killer.Realm))
                    value += value / 100 * 3;
                ...
                if (value != lootTemplate.Price)
                {
                    ...
                    if (killerPlayer != null)
                        killerPlayer.Out.SendMessage(
                            LanguageMgr.GetTranslation(killerPlayer.Client, "GameNPC.DropLoot.AdditionalMoney",
                                Money.GetString(value - lootTemplate.Price)), eChatType.CT_Loot,
                            eChatLoc.CL_SystemWindow);
                }
                ...
                if (killer is GamePlayer)
                {
                    ...
                    if (killerPlayer.GetModified(eProperty.MythicalCoin) > 0)
                    {
                        ...
                        killerPlayer.Out.SendMessage(LanguageMgr.GetTranslation(killerPlayer.Client,
                                "GameNPC.DropLoot.ItemAdditionalMoney",
                                Money.GetString(value - lootTemplate.Price)),
            ...
            else
            {
                ...
                if (lootTemplate is GeneratedUniqueItem) invitem.IsROG = true;
                ...
                if (((WorldInventoryItem) loot).Item.PackSize > 1 && ((WorldInventoryItem) loot).Item.MaxCount >=
                    ((WorldInventoryItem) loot).Item.PackSize)
            ...
            foreach (GameObject gainer in XPGainerList.Keys)
            {
                ...
                {
                    var brain = ((GameNPC) gainer).Brain as IControlledBrain;
                    if (brain != null)
                    {
            ...
            foreach (GameObject gainer in XPGainerList.Keys)
                if (gainer is GamePlayer)
                {
                    var player = gainer as GamePlayer;
                    if (player.Autoloot &&
                        loot.IsWithinRadius(player, 2400)) // should be large enough for most casters to autoloot
                    {
        ...
        Diagnostics.StartPerfCounter("ReaperService-NPC-DropLoot-BroadcastLoot-npc(" + GetHashCode() + ")");
        BroadcastLoot(droplist);
        Diagnostics.StopPerfCounter("ReaperService-NPC-DropLoot-BroadcastLoot-npc(" + GetHashCode() + ")");

        Diagnostics.StartPerfCounter("ReaperService-NPC-DropLoot-PickupLoot-npc(" + GetHashCode() + ")");
        ...
        Diagnostics.StopPerfCounter("ReaperService-NPC-DropLoot-PickupLoot-npc(" + GetHashCode() + ")");
    }
        ...
        int healAmount)
    {
        ...
        var attackerGroup = attackerLiving.Group;
        if (attackerGroup != null)
        {
            // collect "helping" group players in range
            var xpGainers = attackerGroup.GetMembersInTheGroup()
                .Where(l => IsWithinRadius(l, WorldMgr.MAX_EXPFORKILL_DISTANCE) && l.IsAlive &&
                            l.ObjectState == eObjectState.Active).ToArray();

            var damageAmount = (float) healAmount / xpGainers.Length;
        ...
        if (Brain is StandardMobBrain mobBrain)
            ...
            if (mobBrain.AggroTable.ContainsKey(healSource as GameLiving))
                if (healSource is GamePlayer ||
                    (healSource is GameNPC && (((GameNPC) healSource).Flags & eFlags.PEACE) == 0))
    ...
    public override void OnAfterSpellCastSequence(ISpellHandler handler)
    {
        ...
        */
        ...
        if (TargetObject == null) TargetObject = CachedTarget;
    ...
    public class SpellAction : RegionECSAction
    {
        ...
        protected override int OnTick(ECSGameTimer timer)
        {
            ...
            if (owner.Brain is StandardMobBrain &&
                ((StandardMobBrain) owner.Brain).CheckSpells(StandardMobBrain.eCheckSpellType.Offensive))
            {
                Stop();
                return 0;
            }
            else
            {
                ...
                if (owner.ActiveWeaponSlot != eActiveWeaponSlot.Distance &&
                    !owner.IsWithinRadius(owner.TargetObject, STICKMINIMUMRANGE))
    ...
    public override bool CastSpell(Spell spell, SpellLine line)
    {
        ...
        for (var i = m_spellTargetLosChecks.Count - 1; i >= 0; i--)
        {
        ...
        var LosChecker = TargetObject as GamePlayer;
        ...
        {
            if (m_spellTargetLosChecks.TryAdd(TargetObject,
                    new Tuple<Spell, SpellLine, long>(spellToCast, line, GameLoop.GameLoopTime)))
            {
    ...
    public void StartSpellAttackCheckLOS(GamePlayer player, ushort response, ushort targetOID)
    {
        ...
        if (m_spellTargetLosChecks.TryRemove(target, out var value))
        {
            var spell = value.Item1;
            var line = value.Item2;

            if ((response & 0x100) == 0x100 && line != null && spell != null)
            {
                var lasttarget = TargetObject;
                TargetObject = target;

                if (TargetObject is GameLiving living &&
            ...
            else
            {
                Notify(GameLivingEvent.CastFailed, this,
                    new CastFailedEventArgs(null, CastFailedEventArgs.Reasons.TargetNotInView));
            }
        }
    }
            ...
            if (s == null)
            {
                if (log.IsWarnEnabled)
                {
                    ...
                    if (m_InternalID != null)
                        sError += $", InternalID {m_InternalID}";
            ...
            switch (s.OpeningRequirementType)
            {
                ...
                case Style.eOpening.Positional:
                    switch ((Style.eOpeningPosition) s.OpeningRequirementValue)
                    {
                        ...
                        default:
                            log.Warn(
                                $"GameNPC.SortStyles(): Invalid OpeningRequirementValue for positional style {s.Name}, ID {s.ID}, ClassId {s.ClassID}");
                            break;
                    }
    ...
    public Dictionary<string, Ability> Abilities
    {
        get
        {
            var tmp = new Dictionary<string, Ability>();
    ...
    public virtual void FireAmbientSentence(eAmbientTrigger trigger, GameObject living)
    {
        ...
        if (trigger == eAmbientTrigger.interact && living == null)
            return; // Do not trigger interact messages with a corpse
        var mxa = (from i in ambientTexts where i.Trigger == trigger.ToString() select i)
            .ToList();
        ...
        {
            var playerOwner = ((IControlledBrain) Brain).GetPlayerOwner();
            if (playerOwner != null)
        ...
        {
            text = chosen.Text
                .Replace("{sourcename}", Brain?.Body?.Name) // '{sourcename}' returns the mob or NPC name
                .Replace("{targetname}", living?.Name) // '{targetname}' returns the mob/NPC target's name
                .Replace("{controller}",
                    controller); // '{controller}' returns the result of the controller var (use this when pets have dialogue)

            // Replace trigger keywords
            if (living is GamePlayer)
                text = text.Replace("{class}", ((GamePlayer) living).CharacterClass.Name)
                    .Replace("{race}", ((GamePlayer) living).RaceName);
        ...
        else
        {
            text = chosen.Text
                ...
                .Replace("{targetname}",
                    TargetObject == null
                        ? string.Empty
                        : TargetObject.Name) // '{targetname}' returns the mob/NPC target's name
                .Replace("{controller}",
                    controller); // '{controller}' returns the result of the controller var (use this when pets have dialogue)

            // Replace trigger keywords
            if (TargetObject is GamePlayer)
                text = text.Replace("{class}", ((GamePlayer) TargetObject).CharacterClass.Name)
                    .Replace("{race}", ((GamePlayer) TargetObject).RaceName);
            if (TargetObject is GameNPC)
                text = text.Replace("{class}", "NPC").Replace("{race}", "NPC");
        }
        // Replace trigger keywords

        if (chosen.Emote != 0) Emote((eEmote) chosen.Emote);
        ...
        switch (chosen.Voice)
        {
            ...
            case "c"
                : // Return custom Say message in Chat window to all players within range, without "{0} says," string start
            {
            ...
            default
                : // Return Say message with "{0} says," string start included (contrary to parameter description)
            {
                Say(text);
                return;
            }
    ...
    /// </summary>
    public IControlledBrain[] ControlledNpcList => m_controlledBrain;
    ...
    /// </summary>
    public virtual bool IsAvailable => !(Brain is IControlledBrain) && !InCombat;
                ...
                foreach (string str in droplist)
                    ...
                    if (str != lastloot)
                    {
                        player.Out.SendMessage(string.Format(LanguageMgr.GetTranslation(
                                player.Client.Account.Language, "GameNPC.DropLoot.Drops",
                                GetName(0, true, player.Client.Account.Language, this), str)), eChatType.CT_Loot,
                            eChatLoc.CL_SystemWindow);
                        lastloot = str;
                    }
            }
    ...
    public GameNPC Copy(GameNPC copyTarget)
    {
        ...
        if (Abilities != null && Abilities.Count > 0)
            foreach (var targetAbility in Abilities.Values)
        ...
        if (brain == null)
        {
            log.Warn("GameNPC.Copy():  Unable to create brain:  " + Brain.GetType().FullName +
                     ", using StandardMobBrain.");
            brain = new StandardMobBrain();
        }

        var newBrainSMB = brain as StandardMobBrain;
        var thisBrainSMB = Brain as StandardMobBrain;
        ...
        {
            var inventoryTemplate = Inventory as GameNpcInventoryTemplate;
