...
public abstract class GameObject : Point3D
{
    ...
    {
        get => m_CurrentRegion == null ? (ushort) 0 : m_CurrentRegion.ID;
    ...
    public Zone CurrentZone
    {
        get
        {
            if (m_CurrentRegion != null) return m_CurrentRegion.GetZone(X, Y);
    ...
    public virtual IList<IArea> CurrentAreas
    {
        get
        {
            ...
            {
                log.Error($"Error encountered when querying current zone of {Name}: {e}");
            }
        ...
        if (article == 0)
        {
            if (firstLetterUppercase)
                return LanguageMgr.GetTranslation(ServerProperties.Properties.DB_LANGUAGE,
                    "GameObject.GetName.Article1", Name);
            else
                return LanguageMgr.GetTranslation(ServerProperties.Properties.DB_LANGUAGE,
                    "GameObject.GetName.Article2", Name);
        }
        else
        {
            ...
            if (m_vowels.IndexOf(Name[0]) != -1)
            {
                if (firstLetterUppercase)
                    return LanguageMgr.GetTranslation(ServerProperties.Properties.DB_LANGUAGE,
                        "GameObject.GetName.Article3", Name);
                else
                    return LanguageMgr.GetTranslation(ServerProperties.Properties.DB_LANGUAGE,
                        "GameObject.GetName.Article4", Name);
            }
            else
            {
                if (firstLetterUppercase)
                    return LanguageMgr.GetTranslation(ServerProperties.Properties.DB_LANGUAGE,
                        "GameObject.GetName.Article5", Name);
                else
                    return LanguageMgr.GetTranslation(ServerProperties.Properties.DB_LANGUAGE,
                        "GameObject.GetName.Article6", Name);
            }
        }
    }
    ...
    public virtual string GetPronoun(int form, bool firstLetterUppercase)
    {
        switch (form)
        {
            default: // Subjective
                if (firstLetterUppercase)
                    return LanguageMgr.GetTranslation(ServerProperties.Properties.DB_LANGUAGE,
                        "GameObject.GetPronoun.Pronoun1");
                else
                    return LanguageMgr.GetTranslation(ServerProperties.Properties.DB_LANGUAGE,
                        "GameObject.GetPronoun.Pronoun2");
            case 1: // Possessive
                if (firstLetterUppercase)
                    return LanguageMgr.GetTranslation(ServerProperties.Properties.DB_LANGUAGE,
                        "GameObject.GetPronoun.Pronoun3");
                else
                    return LanguageMgr.GetTranslation(ServerProperties.Properties.DB_LANGUAGE,
                        "GameObject.GetPronoun.Pronoun4");
            case 2: // Objective
                if (firstLetterUppercase)
                    return LanguageMgr.GetTranslation(ServerProperties.Properties.DB_LANGUAGE,
                        "GameObject.GetPronoun.Pronoun5");
                else
                    return LanguageMgr.GetTranslation(ServerProperties.Properties.DB_LANGUAGE,
                        "GameObject.GetPronoun.Pronoun6");
        }
    }
    ...
    public virtual IList GetExamineMessages(GamePlayer player)
    {
        ...
        list.Add(LanguageMgr.GetTranslation(player.Client.Account.Language,
            "GameObject.GetExamineMessages.YouTarget", GetName(0, false)));
        return list;
    }
    ...
    {
        var boat = BoatMgr.GetBoatByOwner(InternalID);
        if (boat != null)
    ...
    /// </summary>
    protected List<DataQuest> m_dataQuests = new();
    ...
    {
        if (m_dataQuestCache != null) m_dataQuestCache = null;
    ...
    {
        if (m_dataQuestCache == null) FillDataQuestCache();

        m_dataQuests.Clear();

        try
        {
            foreach (var quest in m_dataQuestCache[CurrentRegionID])
                if (quest.StartName == Name)
                {
                    var dq = new DataQuest(quest, this);
                    AddDataQuest(dq);

                    // if a player forced the reload report any errors
                    if (player != null && dq.LastErrorText != "") ChatUtil.SendErrorMessage(player, dq.LastErrorText);
                }
        }
        catch
        {
        ...
        try
        {
            foreach (var quest in m_dataQuestCache[0])
                if (quest.StartName == Name)
                {
                    var dq = new DataQuest(quest, this);
                    AddDataQuest(dq);
    ...
    /// </summary>
    public List<DataQuest> DataQuestList => m_dataQuests;
    ...
    /// </summary>
    public virtual int InteractDistance => WorldMgr.INTERACT_DISTANCE;
    ...
    {
        var constep = Math.Max(1, (level + 9) / 10);
        var stepping = 1.0 / constep;
        var leveldiff = level - compareLevel;
    ...
    {
        var constep = Math.Max(1, (level + 10) / 10);
    ...
    public IEnumerable GetPlayersInRadius(bool useCache, ushort radiusToCheck, bool withDistance, bool ignoreZ)
    {
        if (CurrentRegion != null)
        {
            ...
            {
                if (this is GamePlayer && (this as GamePlayer).Client.Account.PrivLevel < 3 &&
    ...
    public IEnumerable GetNPCsInRadius(bool useCache, ushort radiusToCheck, bool withDistance, bool ignoreZ)
    {
        if (CurrentRegion != null)
        {
            ...
            {
                var result = CurrentRegion.GetNPCsInRadius(X, Y, Z, radiusToCheck, withDistance, ignoreZ);
                return result;
            }
        }
    ...
    {
        var template = GameServer.Database.FindObjectByKey<ItemTemplate>(templateID);
        if (template == null)
        {
    ...
    {
        var item = player.Inventory.GetFirstItemByID(str, eInventorySlot.Min_Inv, eInventorySlot.Max_Inv);
        if (item != null)
            return true;
        return false;
    }
