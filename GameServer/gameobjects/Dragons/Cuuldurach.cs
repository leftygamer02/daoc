...
public class Cuuldurach : GameDragon
{
    ...
    private ArrayList m_messengerList = new();
    ...
    public override bool CheckAddSpawns()
    {
        ...
        for (var glimmer = 1; glimmer <= 10; ++glimmer)
        {
            ...
            if (isMessenger)
                if (glimmerSpawn != null && glimmerSpawn.Brain != null && glimmerSpawn.Brain is RetrieverMobBrain)
                {
                    ...
                    glimmerSpawn.WalkTo(GetExitCoordinates(Util.Random(1, 4)),
                        200); // Pick 1 out of 4 possible exits.
                }
        }
    ...
    {
        GameNPC glimmer;
        for (var add = 0; add < numAdds; ++add)
        {
            glimmer = SpawnTimedAdd(624 + Util.Random(2), Util.Random(62, 68), x + Util.Random(250),
                y + Util.Random(250), 120, false);
