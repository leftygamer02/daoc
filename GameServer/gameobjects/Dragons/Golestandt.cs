...
public class Golestandt : GameDragon
{
    ...
    public override bool CheckAddSpawns()
    {
        ...
        var numAdds = Math.Max(1, PlayersInLair / 2);
        for (var add = 1; add <= numAdds; ++add)
            SpawnTimedAdd(600, Util.Random(57, 60), X + Util.Random(300, 600), Y + Util.Random(300, 600), 30,
                false); // granite giant pounder lvl 57-60
