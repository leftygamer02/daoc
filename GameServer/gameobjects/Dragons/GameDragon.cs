...
public abstract class GameDragon : GameNPC
{
    ...
    /// 100 = full strength
    /// </summary>
    public virtual int DragonDifficulty => Properties.SET_DIFFICULTY_ON_EPIC_ENCOUNTERS;
    ...
    {
        var AF = 400;
        if (attackComponent.Attackers.Count > 1) AF -= 5 * attackComponent.Attackers.Count;
    ...
    public override int GetResist(eDamageType damageType)
    {
        ...
        switch (damageType)
        {
            ...
            default:
                var resistPercent = 99;
                if (attackComponent.Attackers.Count > 1) resistPercent -= attackComponent.Attackers.Count;
    ...
    public override int MaxHealth => 40000 * DragonDifficulty / 100;
    ...
    public override short Strength => (short) (base.Strength * DragonDifficulty / 100);

    public override int RespawnInterval
    {
        get
        {
            var highmod = Level + 50;
            var lowmod = Level / 3;
            var result = Util.Random(lowmod, highmod);
    ...
    public override void Die(GameObject killer)
    {
        try
        {
            ...
            var killerBG =
                (BattleGroup) playerKiller?.TempProperties.getProperty<object>(BattleGroup.BATTLEGROUP_PROPERTY,
                    null);
    ...
    protected GameNPC SpawnTimedAdd(int templateID, int level, int x, int y, int uptime, bool isRetriever)
    {
        ...
        try
        {
            ...
            if (m_addTemplate != null)
            {
                ...
                if (isRetriever) add.SetOwnBrain(new RetrieverMobBrain());
    ...
    /// </summary>
    protected abstract Spell Breath { get; }
    ...
    /// </summary>
    protected abstract Spell ResistDebuff { get; }
    ...
    private void PrepareToBreathe()
    {
        ...
        HealthPercentOld = HealthPercent;
        var messageNo = Util.Random(1, m_breathAnnounce.Length) - 1;
    ...
    /// </summary>
    protected abstract Spell Glare { get; }
    ...
    /// </summary>
    protected int GlareChance => m_glareChance;
    ...
    public bool CheckGlare(GameLiving target)
    {
        ...
        var success = Util.Chance(GlareChance) && Brain is DragonBrain {GlareAvailable: true};
        if (success)
            GlareTarget = target;
        return success;
    }
    ...
    /// </summary>
    protected abstract Spell MeleeDebuff { get; }
    ...
    /// </summary>
    protected int MeleeDebuffChance => m_meleeDebuffChance;
    ...
    public bool CheckMeleeDebuff(GamePlayer target)
    {
        ...
        var success = Util.Chance(MeleeDebuffChance);
        if (success)
    ...
    /// </summary>
    protected abstract Spell RangedDebuff { get; }
    ...
    /// </summary>
    protected int RangedDebuffChance => m_rangedDebuffChance;
    ...
    {
        if (target == null) return false;
        var success = Util.Chance(RangedDebuffChance);
        if (success)
    ...
    /// </summary>
    protected int ThrowChance => m_throwChance;
    ...
    public bool CheckThrow(GameLiving target)
    {
        ...
        var success = Util.Chance(ThrowChance);
    ...
    private void ThrowLiving(GameLiving target)
    {
        ...
        var targetPosition = PositionOfTarget(target, 700, Heading, Util.Random(300, 500));

        if (target is GamePlayer)
        {
            target.MoveTo(target.CurrentRegionID, targetPosition.X, targetPosition.Y, targetPosition.Z,
                target.Heading);
        }
        else if (target is GameNPC)
        {
            (target as GameNPC).MoveInRegion(target.CurrentRegionID, targetPosition.X, targetPosition.Y,
                targetPosition.Z, target.Heading, true);
    ...
    /// Chance to cast stun.
    /// </summary>
    protected int StunChance => m_stunChance;
