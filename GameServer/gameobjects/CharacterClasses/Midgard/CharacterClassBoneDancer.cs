...
public class CharacterClassBoneDancer : ClassMystic
{
    ...
    {
        var subpet = Player.TargetObject as BDPet;
        if (subpet != null && subpet.Brain is BDPetBrain && Player.ControlledBrain is CommanderBrain &&
            (Player.ControlledBrain as CommanderBrain).FindPet(subpet.Brain as IControlledBrain))
        {
            Player.Notify(GameLivingEvent.PetReleased, subpet);
            var commander = (subpet.Brain as IControlledBrain).Owner as CommanderPet;
