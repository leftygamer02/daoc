...
public abstract class CharacterClassBase : ICharacterClass
{
    ...
    public CharacterClassBase()
    {
        ...
        var attrs = Attribute.GetCustomAttributes(GetType(), typeof(CharacterClassAttribute));
    ...
    public virtual void Init(GamePlayer player)
    {
        ...
        if (Player != null && log.IsWarnEnabled)
            log.WarnFormat(
                "Character Class initializing Player when it was already initialized ! Old Player : {0} New Player : {1}",
                Player, player);
    ...
    public string FemaleName => m_femaleName;
    ...
    public string Name =>
        Player != null && Player.Gender == eGender.Female && !Util.IsEmpty(m_femaleName)
            ? m_femaleName
            : m_name;
    ...
    /// </summary>
    public string Profession => LanguageMgr.TryTranslateOrDefault(Player, m_profession, m_profession);

    public int SpecPointsMultiplier => m_specializationMultiplier;
    ...
    /// </summary>
    public int AdjustedSpecPointsMultiplier => m_specializationMultiplier;

    public eStat PrimaryStat => m_primaryStat;

    public eStat SecondaryStat => m_secondaryStat;

    public eStat TertiaryStat => m_tertiaryStat;

    public eStat ManaStat => m_manaStat;

    public int WeaponSkillBase => m_wsbase;

    public int WeaponSkillRangedBase => m_wsbaseRanged;
    ...
    public virtual string GetTitle(GamePlayer player, int level)
    {
        ...
        var clamplevel = Math.Min(50, level / 5 * 5);

        var none = LanguageMgr.TryTranslateOrDefault(player, "!None!", "PlayerClass.GetTitle.none");

        if (clamplevel > 0)
            return LanguageMgr.TryTranslateOrDefault(player, string.Format("!{0}!", m_name),
                string.Format("PlayerClass.{0}.GetTitle.{1}", m_name, clamplevel));
        ...
        if (controlledBrain == null)
        {
            ...
            Player.Out.SendMessage(
                LanguageMgr.GetTranslation(Player.Client.Account.Language,
                    "GamePlayer.GamePet.SpellEnd.YouLoseControl", Player.ControlledBrain.Body.GetName(0, false)),
                eChatType.CT_System, eChatLoc.CL_SystemWindow);
            // Message: You release control of your controlled target.
            Player.Out.SendMessage(
                LanguageMgr.GetTranslation(Player.Client.Account.Language,
                    "GamePlayer.GamePet.SpellEnd.YouReleaseControl"), eChatType.CT_System,
        ...
        else
        {
            if (controlledBrain.Owner != Player)
                throw new ArgumentException(
                    "ControlledNpc with wrong owner is set (player=" + Player.Name + ", owner=" +
                    controlledBrain.Owner.Name + ")", "controlledNpc");
            if (Player.ControlledBrain == null)
                Player.InitControlledBrainArray(1);
            Player.Out.SendPetWindow(controlledBrain.Body, ePetWindowAction.Open, controlledBrain.AggressionState,
                controlledBrain.WalkState);
            if (controlledBrain.Body != null)
            {
                Player.Out.SendNPCCreate(controlledBrain
                    .Body); // after open pet window again send creation NPC packet
    ...
    {
        var controlledBrain = Player.ControlledBrain;
        if (controlledBrain == null)
            return;

        var npc = controlledBrain.Body;
        if (npc == null)
            return;
    ...
    public virtual void Shade(bool makeShade)
    {
        if (Player.IsShade == makeShade)
        {
            if (makeShade && Player.ObjectState == GameObject.eObjectState.Active)
                Player.Out.SendMessage(
                    LanguageMgr.GetTranslation(Player.Client.Account.Language, "GamePlayer.Shade.AlreadyShade"),
                    eChatType.CT_System, eChatLoc.CL_SystemWindow);
            return;
        }
        ...
        else
        {
            ...
            Player.Out.SendMessage(
                LanguageMgr.GetTranslation(Player.Client.Account.Language, "GamePlayer.Shade.NoLongerShade"),
                eChatType.CT_System, eChatLoc.CL_SystemWindow);
        }
    }
