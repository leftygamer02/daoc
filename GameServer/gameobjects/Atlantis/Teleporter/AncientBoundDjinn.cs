...
public abstract class AncientBoundDjinn : GameTeleporter
{
    ...
    {
        var npcTemplate = NpcTemplateMgr.GetTemplate(NpcTemplateId);
        ...
        Realm = eRealm.None;
        Flags ^= eFlags.FLYING | eFlags.PEACE;
        X = djinnStone.X;
        Y = djinnStone.Y;
    ...
    /// </summary>
    protected new virtual byte Size => base.Size;

    /// <summary>
    /// Gets the height at which the djinn is hovering.
    /// </summary>
    protected virtual int HoverHeight => 63;

    /// <summary>
    /// Teleporter type, needed to pick the right TeleportID.
    /// </summary>
    protected override string Type => "Djinn";

    /// <summary>
    /// The destination realm.
    /// </summary>
    protected override eRealm DestinationRealm => CurrentZone.Realm;
    ...
    /// </summary>
    public virtual bool IsSummoned => false;
    ...
    public override bool Interact(GamePlayer player)
    {
        ...
        var intro = string.Format("According to the rules set down by the Atlantean [masters], {0} {1} ",
    ...
    public override bool WhisperReceive(GameLiving source, string text)
    {
        ...
        switch (text.ToLower())
        {
            case "masters":
                var reply =
                    string.Format("The Atlantean masters are a great and powerful people to whom [we] are bound.");
                SayTo(player, reply);
                return true;
            case "we":
    ...
    protected override bool GetTeleportLocation(GamePlayer player, string text)
    {
        ...
        var port = WorldMgr.GetTeleportLocation(player.Realm, string.Format("{0}:{1}", Type, text));
    ...
    protected override void OnSubSelectionPicked(GamePlayer player, Teleport subSelection)
    {
        switch (subSelection.TeleportID.ToLower())
        {
            case "oceanus":
            {
                var reply = string.Format("I can transport you to the Haven of Oceanus in {0} {1}",
            ...
            case "stygia":
            {
                var reply = string.Format("Do you seek the sandy Haven of Stygia in the Stygian {0}",
            ...
            case "volcanus":
            {
                var reply = string.Format("Do you wish to approach [Typhon's Reach] from the Haven {0} {1}",
            ...
            case "aerus":
            {
                var reply = string.Format("Do you seek the Haven of Aerus outside [Green Glades] or {0}",
            ...
            {
                var reply = string.Format("I can provide access to [Sobekite Eternal], the {0} {1}",
            ...
            case "twilight":
            {
                var reply = string.Format("Do you seek an audience with one of the great ladies {0} {1}",
            ...
            {
                var reply = string.Format("Which interests you, the [entrance], the [Anubite] side, {0} {1}",
            ...
            case "deep":
            {
                var reply = string.Format("Do you wish to meet with the Mediators of the [southwest] {0} {1}",
    ...
    protected override void OnDestinationPicked(GamePlayer player, Teleport destination)
    {
        ...
        var teleportInfo = "The magic of the {0} delivers you to the Haven of {1}.";
    ...
    public override bool Say(string message)
    {
        foreach (GamePlayer player in GetPlayersInRadius(WorldMgr.SAY_DISTANCE))
            player.Out.SendMessage(string.Format("The {0} says, \"{1}\"", Name, message), eChatType.CT_System,
