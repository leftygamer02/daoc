...
public class TheurgistPet : GamePet
{
    ...
    {
        if (npcTemplate.Name.ToLower().Contains("earth")) ScalingFactor = 17;

        if (npcTemplate.Name.ToLower().Contains("air")) ScalingFactor = 11;
    }
    ...
    public override void OnAttackedByEnemy(AttackData ad)
    {
        ...
        {
            if (castingComponent != null && castingComponent.IsCasting &&
                castingComponent.spellHandler.CastStartTick +
                (CurrentSpellHandler as SpellHandler).CalculateCastingTime() / 2 < GameLoop.GameLoopTime)
                InterruptTime = 0;
            else
