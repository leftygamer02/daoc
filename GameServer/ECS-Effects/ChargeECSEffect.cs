...
public class ChargeECSGameEffect : ECSGameAbilityEffect
{
    ...
    public override void OnStartEffect()
    {
        ...
        if (OwnerPlayer != null)
        {
            ...
            OwnerPlayer.Out.SendMessage($"You are now charging {OwnerPlayer.TargetObject?.Name}!",
                eChatType.CT_System, eChatLoc.CL_SystemWindow);
            // "{0} begins charging wildly!"
            Message.SystemToArea(OwnerPlayer,
                LanguageMgr.GetTranslation(OwnerPlayer.Client, "Effects.ChargeEffect.AreaStartCharge",
                    OwnerPlayer.GetName(0, true)), eChatType.CT_System, OwnerPlayer);
        }
        else if (Owner is GameNPC)
        {
            var icb = ((GameNPC) Owner).Brain as IControlledBrain;
            if (icb != null && icb.Body != null)
            {
                var playerowner = icb.GetPlayerOwner();

                if (playerowner != null)
                    playerowner.Out.SendMessage("The " + icb.Body.Name + " charges its prey!", eChatType.CT_Say,
    ...
    public void Cancel(bool playerCancel)
    {
        ...
        if (OwnerPlayer != null)
        {
            ...
            OwnerPlayer.Out.SendMessage(
                LanguageMgr.GetTranslation(OwnerPlayer.Client, "Effects.ChargeEffect.EndCharge"),
                eChatType.CT_System, eChatLoc.CL_SystemWindow);
            // "{0} ceases their charge!"
            Message.SystemToArea(OwnerPlayer,
                LanguageMgr.GetTranslation(OwnerPlayer.Client, "Effects.ChargeEffect.AreaEndCharge",
                    OwnerPlayer.GetName(0, true)), eChatType.CT_System, OwnerPlayer);
        }
        else if (Owner is GameNPC)
        {
            ...
            if (icb != null && icb.Body != null)
            {
                ...
                if (playerowner != null)
                    playerowner.Out.SendMessage("The " + icb.Body.Name + " ceases its charge!", eChatType.CT_Say,
                        eChatLoc.CL_SystemWindow);
            }
        }
    }
