    ...
    public override void OnStartEffect()
    {
        if (Owner is GamePlayer)
        {
            ...
            if (SpellHandler.Spell.Value < 0 || Util.Chance(Convert.ToInt32(Math.Abs(SpellHandler.Spell.Value))))
            {
                ...
                gPlayer.StartInterruptTimer(gPlayer.SpellInterruptDuration, AttackData.eAttackType.Spell,
                    SpellHandler.Caster);
        ...
        else if (Owner is GameNPC)
        {
            ...
            var doConfuse =
                SpellHandler.Spell.Value < 0 || Util.Chance(Convert.ToInt32(SpellHandler.Spell.Value));

            if (!doConfuse)
                return;

            var doAttackFriend = SpellHandler.Spell.Value < 0 &&
                                 Util.Chance(Convert.ToInt32(Math.Abs(SpellHandler.Spell.Value)));
            ...
            {
                //it's a pet.
                var playerowner = (npc.Brain as IControlledBrain).GetPlayerOwner();
                if (playerowner != null && (playerowner.CharacterClass.ID == (int) eCharacterClass.Theurgist ||
                                            (playerowner.CharacterClass.ID == (int) eCharacterClass.Animist &&
                                             npc.Brain is TurretFNFBrain)))
                {
                    //Theurgist pets die.
            ...
            foreach (GameNPC target in npc.GetNPCsInRadius(1000))
            {
                ...
                else
                {
                    //this should prevent mobs from attacking friends.
                    if (GameServer.ServerRules.IsAllowedToAttack(npc, target, true) &&
                        !GameServer.ServerRules.IsSameRealm(npc, target, true))
                        (SpellHandler as ConfusionSpellHandler).targetList.Add(target);
                }
            }
            ...
            if ((SpellHandler as ConfusionSpellHandler).targetList.Count > 0)
            {
                ...
                var target =
                    (SpellHandler as ConfusionSpellHandler).targetList[
                        Util.Random((SpellHandler as ConfusionSpellHandler).targetList.Count - 1)] as GameLiving;
