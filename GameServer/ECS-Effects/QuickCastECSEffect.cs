...
public class QuickCastECSGameEffect : ECSGameAbilityEffect
{
    ...
    public override string Name =>
        LanguageMgr.GetTranslation(((GamePlayer) Owner).Client, "Effects.QuickCastEffect.Name");
    ...
    public override void OnStartEffect()
    {
        if (Owner is GamePlayer)
            (Owner as GamePlayer).Out.SendMessage(
                LanguageMgr.GetTranslation((Owner as GamePlayer).Client, "Effects.QuickCastEffect.YouActivatedQC"),
            ...
            if (Owner is GamePlayer)
                (Owner as GamePlayer).Out.SendMessage(
                    LanguageMgr.GetTranslation((Owner as GamePlayer).Client,
                        "Effects.QuickCastEffect.YourNextSpellNoQCed"), eChatType.CT_System,
