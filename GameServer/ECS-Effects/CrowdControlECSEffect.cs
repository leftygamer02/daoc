...
public abstract class AbstractCrowdControlECSEffect : ECSGameSpellEffect
{
    ...
    protected void OnHardCCStart()
    {
        ...
        if (Owner is GameNPC npc)
            npc.StopMoving();
        if (Owner.effectListComponent.GetAllEffects()
    ...
    protected void SendMessages()
    {
        ...
        ((SpellHandler) SpellHandler).MessageToCaster(
            Util.MakeSentence(SpellHandler.Spell.Message2, Owner.GetName(0, true)), eChatType.CT_Spell);
        Message.SystemToArea(Owner, Util.MakeSentence(SpellHandler.Spell.Message2, Owner.GetName(0, true)),
            eChatType.CT_Spell, Owner, SpellHandler.Caster);
    }
}
