...
{
    public InterceptECSGameEffect(ECSGameEffectInitParams initParams, GameLiving interceptSource,
                ...
                if (InterceptSource is GamePlayer)
                    ((GamePlayer) InterceptSource).Out.SendMessage(
                        LanguageMgr.GetTranslation(((GamePlayer) InterceptSource).Client,
                            "Effects.InterceptEffect.YouAttemtInterceptYBut", InterceptTarget.GetName(0, false)),
                ...
                if (InterceptTarget is GamePlayer && InterceptSource is GamePlayer)
                    ((GamePlayer) InterceptTarget).Out.SendMessage(
                        LanguageMgr.GetTranslation(((GamePlayer) InterceptTarget).Client,
                            "Effects.InterceptEffect.XAttemtInterceptYouBut", InterceptSource.GetName(0, true)),
                        eChatType.CT_System, eChatLoc.CL_SystemWindow);
            }
            else
            {
                if (InterceptSource is GamePlayer)
                    ((GamePlayer) InterceptSource).Out.SendMessage(
                        LanguageMgr.GetTranslation(((GamePlayer) InterceptSource).Client,
                            "Effects.InterceptEffect.YouAttemtInterceptY", InterceptTarget.GetName(0, false)),
                        eChatType.CT_System, eChatLoc.CL_SystemWindow);
                if (InterceptTarget is GamePlayer && InterceptSource is GamePlayer)
                    ((GamePlayer) InterceptTarget).Out.SendMessage(
                        LanguageMgr.GetTranslation(((GamePlayer) InterceptTarget).Client,
                            "Effects.InterceptEffect.XAttemptInterceptYou", InterceptSource.GetName(0, true)),
                        eChatType.CT_System, eChatLoc.CL_SystemWindow);
            }

            new InterceptECSGameEffect(new ECSGameEffectInitParams(InterceptTarget, 0, 1), InterceptSource,
                InterceptTarget);
        }
    }
