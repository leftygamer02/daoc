...
public class DamageOverTimeECSGameEffect : ECSGameSpellEffect
{
    ...
    {
        if (Owner.IsAlive == false) EffectService.RequestImmediateCancelEffect(this);
        ...
        if (Owner.IsAlive)
        {
            ...
            else if (SpellHandler is StyleBleeding bleedHandler)
            {
                if (Owner.effectListComponent.ContainsEffectForEffectType(eEffect.Bleed)
                    && Owner.TempProperties.getProperty<int>(StyleBleeding.BLEED_VALUE_PROPERTY) >
                    bleedHandler.Spell.Damage)
                {
                    if (OwnerPlayer != null)
                        bleedHandler.MessageToCaster("A stronger bleed effect already exists on your target.",
                            eChatType.CT_SpellResisted);
                    EffectService.RequestCancelEffect(this);
                    return;
                }

                if (StartTick + PulseFreq > GameLoop.GameLoopTime &&
                    Owner.TempProperties.getProperty<int>(StyleBleeding.BLEED_VALUE_PROPERTY) <
                    bleedHandler.Spell.Damage)
                    Owner.TempProperties.setProperty(StyleBleeding.BLEED_VALUE_PROPERTY,
                        (int) bleedHandler.Spell.Damage);

                if (OwnerPlayer != null)
                {
                    bleedHandler.MessageToLiving(Owner, bleedHandler.Spell.Message1, eChatType.CT_YouWereHit);
                    Message.SystemToArea(Owner,
                        Util.MakeSentence(bleedHandler.Spell.Message2, Owner.GetName(0, false)),
                        eChatType.CT_YouHit, Owner);
                }

                var bleedValue = Owner.TempProperties.getProperty<int>(StyleBleeding.BLEED_VALUE_PROPERTY);

                var ad = bleedHandler.CalculateDamageToTarget(Owner, 1.0);
        ...
        if (SpellHandler.Caster is GamePet)
            Owner.StartInterruptTimer(SpellHandler.Caster.SpellInterruptDuration, AttackData.eAttackType.Spell,
                SpellHandler.Caster);
    }
}