...
public class StatDebuffECSEffect : ECSGameSpellEffect
{
    ...
    public override void OnStartEffect()
    {
        ...
        {
            var effects = Owner.effectListComponent.GetSpellEffects(EffectType);
        ...
        else
        {
            if (EffectType == eEffect.MovementSpeedDebuff)
            {
                ...
                var speedDebuffs = Owner.effectListComponent.GetSpellEffects(eEffect.MovementSpeedDebuff)
                    .Where(x => x.SpellHandler.Spell.ID != SpellHandler.Spell.ID);

                if (speedDebuffs.Any(x => x.SpellHandler.Spell.Value > SpellHandler.Spell.Value)) return;

                foreach (var effect in speedDebuffs) EffectService.RequestDisableEffect(effect);
    ...
    public override void OnStopEffect()
    {
        ...
        else
        {
            if (EffectType == eEffect.MovementSpeedDebuff)
            {
                ...
                if (speedDebuff != null) EffectService.RequestEnableEffect(speedDebuff);
