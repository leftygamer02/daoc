...
public sealed class BehaviourMgr
{
    ...
    public static bool Init()
    {
        ...
        foreach (var type in assembly.GetTypes())
        {
            ...
            {
                var attr = GetActionAttribute(type);
                if (attr != null)
                {
            ...
            {
                var attr = getTriggerAttribute(type);
                if (attr != null)
                {
                    if (log.IsInfoEnabled)
            ...
            {
                var attr = getRequirementAttribute(type);
                if (attr != null)
                {
                    if (log.IsInfoEnabled)
        ...
        if (m_behaviourActionMap.Contains(actionType))
        {
            if (log.IsErrorEnabled)
                log.Error(actionType +
                          " is already registered, only one Type can be declared for each ActionType. Duplicate declaration found in :" +
                          m_behaviourActionMap[actionType] + " and " + type);
            return;
        }
    ...
    public static void RegisterBehaviourTrigger(eTriggerType triggerType, Type type)
    {
        ...
        {
            if (log.IsErrorEnabled)
                log.Error(triggerType +
                          " is already registered, only one Type can be declared for each TriggerType. Duplicate declaration found in :" +
                          m_behaviourTriggerMap[triggerType] + " and " + type);
            return;
        }
    ...
    public static void RegisterBehaviourRequirement(eRequirementType requirementType, Type type)
    {
        ...
        {
            if (log.IsErrorEnabled)
                log.Error(requirementType +
                          " is already registered, only one Type can be declared for each Requirement. Duplicate declaration found in :" +
                          m_behaviourRequirementMap[requirementType] + " and " + type);
            return;
        }
