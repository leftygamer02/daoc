...
public class TimerAction : AbstractAction<string, int>
{
    ...
    /// </summary>
    private const string TIMER_ID = "timerid";

    /// <summary>
    /// Constant used to store GameLiving Source in RegionTimer.Properties
    /// </summary>
    private const string TIMER_SOURCE = "timersource";
    ...
    public override void Perform(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var timer = new ECSGameTimer(player, new ECSGameTimer.ECSTimerCallback(QuestTimerCallBack));
    ...
    {
        var timerid = callingTimer.Properties.getProperty<object>(TIMER_ID, null) as string;
        if (timerid == null)
            throw new ArgumentNullException("TimerId out of Range", "timerid");

        var source = callingTimer.Properties.getProperty<object>(TIMER_SOURCE, null) as GameLiving;
        if (source == null)
            throw new ArgumentNullException("TimerSource null", "timersource");


        var args = new TimerEventArgs(source, timerid);
