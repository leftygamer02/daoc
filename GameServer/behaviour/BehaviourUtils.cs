...
internal class BehaviourUtils
{
    ...
    /// @deprecated
    /// </summary>
    private const string PLAYER_OLD = "{Player}";
    ...
    public static object ConvertObject(object obj, object defaultValue, Type destinationType)
    {
        ...
        if (destinationType == typeof(Type))
        {
            if (obj is Type)
                result = obj;
            else if (!string.IsNullOrEmpty(Convert.ToString(obj))) result = ScriptMgr.GetType(Convert.ToString(obj));
        }
    ...
    public static string GetPersonalizedMessage(string message, GamePlayer player)
    {
        ...
        var playerIndex = message.IndexOf(PLAYER);
        if (playerIndex == 0)
            message = message.Replace(PLAYER, player.GetName(0, true));
        else if (playerIndex > 0) message = message.Replace(PLAYER, player.GetName(0, false));
        ...
        else if (playerIndex > 0) message = message.Replace(PLAYER_OLD, player.GetName(0, false));
    ...
    public static GamePlayer GuessGamePlayerFromNotify(DOLEvent e, object sender, EventArgs args)
    {
        ...
        else if (e == GameObjectEvent.ReceiveItem || e == GameObjectEvent.ReceiveMoney)
        {
            player = ((SourceEventArgs) args).Source as GamePlayer;
        }
        else if (e == AreaEvent.PlayerEnter || e == AreaEvent.PlayerLeave)
        {
            var aArgs = (AreaEventArgs) args;
