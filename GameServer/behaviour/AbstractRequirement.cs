...
public abstract class AbstractRequirement<TypeN, TypeV> : IBehaviourRequirement
{
    ...
    {
        var attr = BehaviourMgr.getRequirementAttribute(GetType());
        // handle parameter N
        var defaultValueN = GetDefaultValue(attr.DefaultValueN);
        N = (TypeN) BehaviourUtils.ConvertObject(n, defaultValueN, typeof(TypeN));
        CheckParameter(N, attr.IsNullableN, typeof(TypeN));

        // handle parameter V
        var defaultValueV = GetDefaultValue(attr.DefaultValueV);
