...
public abstract class AbstractTrigger<TypeK, TypeI> : IBehaviourTrigger
{
    ...
    public AbstractTrigger(GameLiving defaultNPC, DOLEventHandler notifyHandler, eTriggerType type, object k,
        object i) : this(defaultNPC, notifyHandler, type)
    {
        var attr = BehaviourMgr.getTriggerAttribute(GetType());

        // handle parameter K
        var defaultValueK = GetDefaultValue(attr.DefaultValueK);
        this.k = (TypeK) BehaviourUtils.ConvertObject(k, defaultValueK, typeof(TypeK));
        CheckParameter(K, attr.IsNullableK, typeof(TypeK));

        // handle parameter I
        var defaultValueI = GetDefaultValue(attr.DefaultValueI);
    ...
    protected virtual bool CheckParameter(object value, bool isNullable, Type destinationType)
    {
        if (destinationType == typeof(Unused))
        {
            if (value != null)
                ...
                {
                    log.Warn("Parameter is not used for =" + GetType().Name + ".\n The recieved parameter " +
                             value +
                             " will not be used for anthing. Check your quest code for inproper usage of parameters!");
                    return false;
                }
        }
        else
        {
            if (!isNullable && value == null)
                ...
                {
                    log.Error("Not nullable parameter was null, expected type is " + destinationType.Name +
            ...
            if (value != null && !destinationType.IsInstanceOfType(value))
                if (log.IsErrorEnabled)
                {
                    log.Error("Parameter was not of expected type, expected type is " + destinationType.Name +
                              "for " + GetType().Name + ".\nRecived parameter was " + value);
                    return false;
                }
        }
