...
public class QuickCastAbilityHandler : IAbilityActionHandler
{
    ...
    public void Execute(Ability ab, GamePlayer player)
    {
        ...
        if (player.CurrentSpellHandler != null)
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language,
                    "Skill.Ability.QuickCast.CannotUseIsCasting"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
            return;
        }

        var quickcast =
            (QuickCastECSGameEffect) EffectListService.GetAbilityEffectOnTarget(player, eEffect.QuickCast);
        if (quickcast != null)
        {
        ...
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language, "Skill.Ability.QuickCast.CannotUseDead"),
        ...
        if (player.attackComponent.AttackState)
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language,
                    "Skill.Ability.QuickCast.CannotUseInMeleeCombat"), eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            return;
        }

        var quickcastChangeTick = player.TempProperties.getProperty<long>(GamePlayer.QUICK_CAST_CHANGE_TICK);
        var changeTime = player.CurrentRegion.Time - quickcastChangeTick;
        if (changeTime < DISABLE_DURATION)
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language,
                    "Skill.Ability.QuickCast.CannotUseChangeTick", (DISABLE_DURATION - changeTime) / 1000),
                eChatType.CT_System, eChatLoc.CL_SystemWindow);
