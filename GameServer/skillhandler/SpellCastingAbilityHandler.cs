...
public class SpellCastingAbilityHandler : IAbilityActionHandler, ISpellCastingAbilityHandler
{
    ...
    public void Execute(Ability ab, GamePlayer player)
    {
        ...
        if (SpellLine != null && Spell != null) player.CastSpell(this);
    }
    ...
    public virtual bool CheckPreconditions(GameLiving living, long bitmask)
    {
        ...
        if ((bitmask & INCOMBAT) != 0 && living.InCombat)
        {
            if (player != null)
                player.Out.SendMessage(
                    LanguageMgr.GetTranslation(player.Client.Account.Language, "Skill.Ability.CannotUseInCombat"),
                    eChatType.CT_System, eChatLoc.CL_SystemWindow);
            ...
            if (player != null)
                player.Out.SendMessage(
                    LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "Skill.Ability.CannotUseWhileStealthed"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
        ...
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language, "Skill.Ability.CannotUseMustBeInGroup"),
                eChatType.CT_System, eChatLoc.CL_SystemWindow);
            return true;
        }
        ...
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language, "Skill.Ability.CannotUseTargetNull"),
                eChatType.CT_System, eChatLoc.CL_SystemWindow);
            return true;
        }
