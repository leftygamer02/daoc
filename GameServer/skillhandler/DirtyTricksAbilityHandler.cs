...
namespace DOL.GS.Effects
{
    ...
    public class DirtyTricksEffect : TimedEffect
    {
        ...
        protected void EventHandler(DOLEvent e, object sender, EventArgs arguments)
        {
            ...
            var dt = target.EffectList.GetOfType<DTdetrimentalEffect>();
            if (dt == null) new DTdetrimentalEffect().Start(target);
            // Log.Debug("Effect Started from dirty tricks handler on " + target.Name);
        }

        public override string Name =>
            LanguageMgr.GetTranslation(((GamePlayer) Owner).Client, "Skill.Ability.DirtyTricks.Name");
        ...
        public override IList<string> DelveInfo
        {
            get
            {
                ...
                list.Add(LanguageMgr.GetTranslation(((GamePlayer) Owner).Client,
                    "Skill.Ability.DirtyTricks.Description"));
...
namespace DOL.GS.Effects
{
    ...
    public class DTdetrimentalEffect : StaticEffect, IGameEffect
    {
        ...
        public override void Start(GameLiving living)
        {
            ...
            if (player != null)
                player.Out.SendMessage(
                    LanguageMgr.GetTranslation(player.Client.Account.Language, "Skill.Ability.DirtyTricks.EffectStart"),
                    eChatType.CT_System, eChatLoc.CL_SystemWindow);
        }
        ...
        public override void Cancel(bool playerCancel)
        {
            ...
            m_player.DebuffCategory[(int) eProperty.FumbleChance] -= 50;
            var player = m_player as GamePlayer;
            if (player != null)
                player.Out.SendMessage(
                    LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "Skill.Ability.DirtyTricks.EffectCancel"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
        }
                    ...
                    if (seconds > 60)
                        delveInfoList.Add("- " + seconds / 60 + ":" + (seconds % 60).ToString("00") +
                                          LanguageMgr.GetTranslation(((GamePlayer) Owner).Client,
                                              "Skill.Ability.MinRemaining"));
                    else
                        delveInfoList.Add("- " + seconds + LanguageMgr.GetTranslation(((GamePlayer) Owner).Client,
                            "Skill.Ability.SecRemaining"));
                }
