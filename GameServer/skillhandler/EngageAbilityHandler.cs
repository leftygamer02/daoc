...
public class EngageAbilityHandler : IAbilityActionHandler
{
    ...
    public void Execute(Ability ab, GamePlayer player)
    {
        ...
        {
            var engage =
                EffectListService.GetEffectOnTarget(player, eEffect.Engage) as EngageECSGameEffect;
            if (engage != null)
            {
                engage.Cancel(true);
                return;
        ...
        if (!player.IsAlive)
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language, "Skill.Ability.Engage.CannotUseDead"),
        ...
        if (player.IsSitting)
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language,
                    "Skill.Ability.Engage.CannotUseStanding"), eChatType.CT_YouHit, eChatLoc.CL_SystemWindow);
            return;
        }
        ...
        if (player.ActiveWeaponSlot == eActiveWeaponSlot.Distance)
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language,
                    "Skill.Ability.Engage.CannotUseNoCaCWeapons"), eChatType.CT_YouHit, eChatLoc.CL_SystemWindow);
            return;
        }
        ...
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language, "Skill.Ability.Engage.NoTarget"),
        ...
        if (target.LastAttackedByEnemyTick > GameLoop.GameLoopTime - ENGAGE_ATTACK_DELAY_TICK)
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language,
                    "Skill.Ability.Engage.TargetAttackedRecently", target.GetName(0, true)), eChatType.CT_System,
        ...
        if (!GameServer.ServerRules.IsAllowedToAttack(player, target, true))
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language,
                    "Skill.Ability.Engage.NotAllowedToEngageTarget", target.GetName(0, false)), eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            return;
        }
