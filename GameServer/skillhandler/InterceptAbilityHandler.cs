...
public class InterceptAbilityHandler : IAbilityActionHandler
{
    ...
    public void Execute(Ability ab, GamePlayer player)
    {
        ...
        if (targetObject == null)
        {
            ...
            foreach (InterceptECSGameEffect intercept in player.effectListComponent.GetAbilityEffects()
                         .Where(e => e.EffectType == eEffect.Intercept))
            {
            ...
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language,
                    "Skill.Ability.Intercept.CancelTargetNull"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
            return;
        }
        ...
        var interceptTarget = targetObject as GamePlayer;
        if (interceptTarget == null || group == null || !group.IsInTheGroup(interceptTarget) ||
            interceptTarget == player)
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language,
                    "Skill.Ability.Intercept.CannotUse.NotInGroup"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
            return;
        }
        ...
        foreach (InterceptECSGameEffect intercept in interceptTarget.effectListComponent.GetAbilityEffects()
                     .Where(e => e.EffectType == eEffect.Intercept))
        {
            ...
            if (intercept.InterceptSource != player && !(intercept.InterceptSource is GameNPC))
            {
                player.Out.SendMessage(
                    LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "Skill.Ability.Intercept.CannotUse.InterceptTargetAlreadyInterceptedEffect",
                        intercept.InterceptSource.GetName(0, true), intercept.InterceptTarget.GetName(0, false)),
        ...
        new InterceptECSGameEffect(new ECSGameEffectInitParams(player, 0, 1), player,
            (GameLiving) player.TargetObject);
    }
}