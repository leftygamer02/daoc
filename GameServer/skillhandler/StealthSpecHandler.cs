...
public class StealthSpecHandler : ISpecActionHandler
{
    ...
    public void Execute(Specialization spec, GamePlayer player)
    {
        ...
        if (player.InCombat && !player.IsStealthed && player.Client.Account.PrivLevel == (int) ePrivLevel.Player)
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language,
                    "Skill.Ability.Stealth.CannotUseInCombat"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
            return;
        }
        ...
        if (volley != null)
        {
            player.Out.SendMessage("You can't stealth while you have active Volley!", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            return;
        }

        var stealthChangeTick = player.TempProperties.getProperty<long>(GamePlayer.STEALTH_CHANGE_TICK);
        var changeTime = player.CurrentRegion.Time - stealthChangeTick;
        if (changeTime < 2000)
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language,
                    "Skill.Ability.Stealth.CannotUseStealthChangeTick", ((2000 - changeTime) / 1000).ToString()),
        ...
        if (!player.IsStealthed)
        {
            ...
            if (!player.IsAlive)
            {
                player.Out.SendMessage(
                    LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "Skill.Ability.Stealth.CannotUseDead"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                return;
            }

            // Can't stealth if in attack mode
            if (player.attackComponent.AttackState ||
                (player.CurrentSpellHandler != null && player.CurrentSpellHandler.IsCasting))
            {
                player.Out.SendMessage(
                    LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "Skill.Ability.Stealth.CannotUseCombatState"), eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);
                return;
            }
            ...
            if (player.effectListComponent.GetAllPulseEffects().Count > 0)
            {
                player.Out.SendMessage(
                    LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "Skill.Ability.Stealth.CannotUseActivePulsingSpell"), eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);
                return;
            }
            ...
            if (player.IsMezzed)
            {
                player.Out.SendMessage(
                    LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "Skill.Ability.Stealth.CannotUseMezzed"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                return;
            }
            ...
            if (player.IsStunned)
            {
                player.Out.SendMessage(
                    LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "Skill.Ability.Stealth.CannotUseStunned"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                return;
            }
            ...
            foreach (GamePlayer ply in player.GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE))
            {
                ...
                player.Out.SendMessage(
                    LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "Skill.Ability.Stealth.CannotUseToCloseAnEnemy"), eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);
                return;
            }
        ...
        foreach (GameLiving attacker in attackers)
            if (attacker.TargetObject == (GameLiving) player)
            {
                ...
                if (attacker is GamePlayer) ((GamePlayer) attacker).Out.SendChangeTarget(attacker.TargetObject);

                if (attacker is GameNPC)
                {
                    var npc = (GameNPC) attacker;
                    if (npc.Brain is IOldAggressiveBrain) ((IOldAggressiveBrain) npc.Brain).RemoveFromAggroList(player);
    ...
    {
        var enemyLevel = Math.Max(1f, obj.Level);
