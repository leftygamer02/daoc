...
public class DistractionAbilityHandler : IAbilityActionHandler
{
    ...
    private List<GameNPC> m_distractedNPCs = new();
    ...
    public void Execute(Ability ab, GamePlayer player)
    {
        ...
        if (GameLoc == null)
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language,
                    "SummonAnimistPet.CheckBeginCast.GroundTargetNull"), eChatType.CT_System,
        ...
        if (!player.GroundTargetInView)
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language,
                    "SummonAnimistPet.CheckBeginCast.GroundTargetNotInView"), eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            return;
        }
        ...
        if (GameLoc.GetDistance(player) > 750)
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language,
                    "SummonAnimistPet.CheckBeginCast.GroundTargetNotInSpellRange"), eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            return;
        }
        ...
        foreach (GameNPC npc in player.GetNPCsInRadius(1500))
            if (npc.GetDistanceTo(GameLoc) < 400 && GameServer.ServerRules.IsAllowedToAttack(player, npc, true) &&
                !(npc is GameTrainingDummy))
                m_distractedNPCs.Add(npc);

        foreach (var distractedNpC in m_distractedNPCs) distractedNpC.TurnTo(GameLoc.X, GameLoc.Y);
    ...
    {
        foreach (var mDistractedNpC in m_distractedNPCs) mDistractedNpC.TurnTo(mDistractedNpC.SpawnHeading);
