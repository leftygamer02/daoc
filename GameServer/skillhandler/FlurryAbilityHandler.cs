    ...
    public void Execute(Ability ab, GamePlayer player)
    {
        ...
        if (!player.IsAlive)
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language, "Skill.Ability.CannotUseDead"),
        ...
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language, "Skill.Ability.CannotUseMezzed"),
                eChatType.CT_System, eChatLoc.CL_SystemWindow);
            return;
        }
        ...
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language, "Skill.Ability.CannotUseStunned"),
                eChatType.CT_System, eChatLoc.CL_SystemWindow);
            return;
        }
        ...
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language, "Skill.Ability.CannotUseStanding"),
                eChatType.CT_System, eChatLoc.CL_SystemWindow);
            return;
        }
        ...
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language, "Skill.Ability.CannotUseNoTarget"),
                eChatType.CT_System, eChatLoc.CL_SystemWindow);
            return;
        }
        ...
        if (!(player.TargetObject is GamePlayer || player.TargetObject is GameKeepGuard))
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language,
                    "Skill.Ability.Flurry.TargetNotPlayerOrGuards"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
            return;
        }
        ...
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language, "Skill.Ability.CannotAttackTarget",
                    player.TargetObject.Name), eChatType.CT_System, eChatLoc.CL_SystemWindow);
            return;
        }
        ...
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language, "Skill.Ability.CannotSeeTarget",
                    player.TargetObject.Name), eChatType.CT_System, eChatLoc.CL_SystemWindow);
            return;
        }

        if (!player.IsWithinRadius(player.TargetObject,
                135)) //Didn't use AttackRange cause of the fact that player could use a Bow
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language, "Skill.Ability.TargetIsWithinRadius"),
                eChatType.CT_System, eChatLoc.CL_SystemWindow);
            return;
        }
        ...
        {
            var SputinLegacy =
                (player.TargetObject as GamePlayer).EffectList.GetOfType<SputinsLegacyEffect>();
            if (SputinLegacy != null)
            {
                player.Out.SendMessage(
                    LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "Skill.Ability.TargetIsUnderSputinLegacy", player.TargetObject.Name), eChatType.CT_Missed,
        ...
        var target = (GameLiving) player.TargetObject;
        var damage = 0;
        var specc = player.CharacterClass is PlayerClass.ClassBlademaster
        ...
        var primaryResistModifier = target.GetResist(eDamageType.Slash);

        //Using the resist BuffBonusCategory2 - its unused in ResistCalculator
        var secondaryResistModifier = target.SpecBuffBonusCategory[(int) eProperty.Resist_Slash];
        ...
        */
        ...
        player.Out.SendMessage(
            LanguageMgr.GetTranslation(player.Client.Account.Language, "Skill.Ability.Flurry.YouHit",
                target.GetName(0, false), damage), eChatType.CT_YouHit, eChatLoc.CL_SystemWindow);

        if (target is GamePlayer)
            (target as GamePlayer).Out.SendMessage(
                LanguageMgr.GetTranslation((target as GamePlayer).Client, "Skill.Ability.Flurry.HitYou",
                    player.Name, damage), eChatType.CT_Damaged, eChatLoc.CL_SystemWindow);
