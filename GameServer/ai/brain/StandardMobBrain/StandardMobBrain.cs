...
public class StandardMobBrain : APlayerVicinityBrain, IOldAggressiveBrain
{
    ...
    protected virtual void CheckNPCAggro()
    {
        foreach (GameNPC npc in Body.GetNPCsInRadius((ushort) AggroRange, !Body.CurrentRegion.IsDungeon))
        {
            ...
            if (GS.ServerProperties.Properties.ALWAYS_CHECK_LOS)
            {
                ...
                if (npc.Brain is ControlledNpcBrain theirControlledNpcBrain &&
                    theirControlledNpcBrain.GetPlayerOwner() is GamePlayer theirOwner)
                {
                    theirOwner.Out.SendCheckLOS(Body, npc, new CheckLOSResponse(LosCheckForAggroCallback));
                    continue;
                }
                else if (this is ControlledNpcBrain ourControlledNpcBrain &&
                         ourControlledNpcBrain.GetPlayerOwner() is GamePlayer ourOwner)
                {
    ...
    /// </summary>
    public Dictionary<GameLiving, long> AggroTable { get; private set; } = new();
    ...
    public void AddAggroListTo(StandardMobBrain brain)
    {
        ...
        var aggroTable = Array.Empty<KeyValuePair<GameLiving, long>>();
    ...
    public virtual void AddToAggroList(GameLiving living, int aggroAmount)
    {
        ...
        if (living is GamePlayer player && aggroAmount > 0)
        {
                         ...
                         .Where(e => e.EffectType == eEffect.Protect))
            {
                ...
                var protectSource = protect.ProtectSource;
                ...
                var abilityLevel = protectSource.GetAbilityLevel(Abilities.Protect);
                var protectAmount = (int) (abilityLevel * 0.10 * aggroAmount);

                if (protectAmount > 0)
                {
                    aggroAmount -= protectAmount;
                    protectSource.Out.SendMessage(LanguageMgr.GetTranslation(protectSource.Client.Account.Language,
                            "AI.Brain.StandardMobBrain.YouProtDist", player.GetName(0, false),
                            Body.GetName(0, false, protectSource.Client.Account.Language, Body)),
                        eChatType.CT_System,
    ...
    public void PrintAggroTable()
    {
        ...
        foreach (var gl in AggroTable.Keys)
    ...
    protected virtual void LosCheckForAggroCallback(GamePlayer player, ushort response, ushort targetOID)
    {
        ...
        {
            var gameObject = Body.CurrentRegion.GetObject(targetOID);
    ...
    protected virtual GameLiving CalculateNextAttackTarget()
    {
        ...
        var aggroList = new List<KeyValuePair<GameLiving, long>>();
        ...
        foreach (var currentKey in aggroList)
        {
            ...
            if (living.IsAlive
                ...
                && living.ObjectState == GameObject.eObjectState.Active)
            {
                var aggro = amount * Math.Min(500.0 / Body.GetDistanceTo(living), 1);
                if (aggro > maxAggro)
                {
    ...
    public virtual void OnAttackedByEnemy(AttackData ad)
    {
        ...
        var damage = ad.Damage + ad.CriticalDamage + Math.Abs(ad.Modifier);
    ...
    private void ConvertDamageToAggroAmount(GameLiving attacker, int damage)
    {
        if (attacker is GameNPC NpcAttacker && NpcAttacker.Brain is IControlledBrain controlledBrain)
        {
            ...
            var aggroForOwner = (int) (damage * 0.25);
    ...
    protected virtual void BringFriends(GameLiving attacker)
    {
        ...
        else if (attacker is BDSubPet bdSubPet && bdSubPet.Owner is GamePet bdPet &&
                 bdPet.Owner is GamePlayer bdOwner)
        {
            puller = bdOwner;
            actualPuller = bdPet;
        }
        ...
        var bg =
            puller.TempProperties.getProperty<object>(BattleGroup.BATTLEGROUP_PROPERTY, null) as BattleGroup;

        // Check group first to minimize the number of HashSet.Add() calls
        if (puller.Group is Group group)
        {
            if (GS.ServerProperties.Properties.BAF_MOBS_COUNT_BG_MEMBERS && bg != null)
                countedAttackers = new HashSet<string>(); // We have to check for duplicates when counting attackers

            if (!GS.ServerProperties.Properties.BAF_MOBS_ATTACK_PULLER)
            {
                if (GS.ServerProperties.Properties.BAF_MOBS_ATTACK_BG_MEMBERS && bg != null)
                {
        ...
        if (bg != null && (GS.ServerProperties.Properties.BAF_MOBS_COUNT_BG_MEMBERS
                           || (GS.ServerProperties.Properties.BAF_MOBS_ATTACK_BG_MEMBERS &&
                               !GS.ServerProperties.Properties.BAF_MOBS_ATTACK_PULLER)))
        {
            if (victims == null && GS.ServerProperties.Properties.BAF_MOBS_ATTACK_BG_MEMBERS &&
                !GS.ServerProperties.Properties.BAF_MOBS_ATTACK_PULLER)
            ...
            foreach (GamePlayer player in bg.Members.Keys)
                if (player != null && (player.InternalID == puller.InternalID ||
                                       player.IsWithinRadius(puller, BAFPlayerRange, true)))
                {
                    if (GS.ServerProperties.Properties.BAF_MOBS_COUNT_BG_MEMBERS
        ...
        var percentBAF = GS.ServerProperties.Properties.BAF_INITIAL_CHANCE
                         + (numAttackers - 1) * GS.ServerProperties.Properties.BAF_ADDITIONAL_CHANCE;
        ...
        if (Body != null && Body.Spells != null && Body.Spells.Count > 0)
        {
                ...
                foreach (Spell spell in Body.Spells)
                {
                    ...
                    if (Body.ControlledBrain != null && Body.ControlledBrain.Body != null)
                    {
                        if (Util.Chance(30) && Body.ControlledBrain != null &&
                            ...
                            GS.ServerProperties.Properties.NPC_HEAL_THRESHOLD
                ...
                if (spell_rec.Count > 0)
                {
                    ...
                    if (!Body.IsReturningToSpawnPoint)
                        if ((spellToCast.Uninterruptible || !Body.IsBeingInterrupted) &&
                            CheckDefensiveSpells(spellToCast))
                            casted = true;
                }
            }
            else if (type == eCheckSpellType.Offensive)
            {
                ...
                if (spell_rec.Count > 0)
                {
                    ...
                    if ((spellToCast.Uninterruptible || !Body.IsBeingInterrupted) &&
                        CheckOffensiveSpells(spellToCast))
                        casted = true;
                }
            }
        }
        ...
        switch (spell.SpellType)
        {
            ...
            case (byte) eSpellType.SpreadHeal:
                if (spell.Target.ToLower() == "self")
                {
                    ...
                    if (Body.HealthPercent < GS.ServerProperties.Properties.NPC_HEAL_THRESHOLD)
                        Body.TargetObject = Body;
                ...
                if (Body.HealthPercent < GS.ServerProperties.Properties.NPC_HEAL_THRESHOLD / 2.0
    ...
    public static bool LivingHasEffect(GameLiving target, Spell spell)
    {
        ...
        {
            if (target.effectListComponent.Effects.TryGetValue(
                    EffectService.GetEffectFromSpell(spell, m_mobSpellLine.IsBaseLine),
                    out var existingEffects))
                if (existingEffects.FirstOrDefault(e =>
                        e.SpellHandler.Spell.ID == spell.ID || (spell.EffectGroup > 0 &&
                                                                e.SpellHandler.Spell.EffectGroup ==
                                                                spell.EffectGroup)) != null)
                    return true;
        }
        else if (EffectListService.GetEffectOnTarget(target,
                     EffectService.GetEffectFromSpell(spell, m_mobSpellLine.IsBaseLine)) != null)
        {
            return true;
        }
    ...
    public virtual bool CanRandomWalk
    {
        get
        {
             ...
             */
            if (!GS.ServerProperties.Properties.ALLOW_ROAM)
                return false;
