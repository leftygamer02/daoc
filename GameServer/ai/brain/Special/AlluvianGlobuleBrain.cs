    ...
    public override void Think()
    {
        ...
        if (!Body.IsReturningHome)
        {
            if (!Body.attackComponent.AttackState && AggroRange > 0)
            {
                ...
                {
                    if (!PlayersSeen.Contains(player)) PlayersSeen.Add(player);
            ...
            if (!HasAggro)
            {
                if (Body.attackComponent.AttackState) Body.StopAttack();
        ...
        if (!Body.attackComponent.AttackState && !Body.IsMoving && !Body.InCombat)
            ...
            Body.WalkTo(544196 + Util.Random(1, 3919), 514980 + Util.Random(1, 3200), 3140 + Util.Random(1, 540),
                80);
    }
