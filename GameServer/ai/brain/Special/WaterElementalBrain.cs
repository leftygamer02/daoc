...
public class WaterElementalBrain : StandardMobBrain
{
    ...
    public override void Think()
    {
        ...
        if (Body.IsAlive)
        {
            ...
            if (_isInStorm(regionWeather))
            {
                ...
                var index = _determinIndex(regionWeather.Intensity);
                var targetBodySize =
                    (byte) (_originalSize + (byte) (_originalSize * _modifiers[index].sizeModifier));
                if (Body.Size != targetBodySize) Body.Size = targetBodySize;

                var targetStrength = (short) (_originalStrength +
                                              (short) (_originalStrength * _modifiers[index].strengthModifier));
                if (Body.Strength != targetStrength) Body.Strength = targetStrength;
            }
            else
            {
                if (Body.Size != _originalSize) Body.Size = _originalSize;

                if (Body.Strength != _originalStrength) Body.Strength = _originalStrength;
            }
        }

        //This should never happen, but, in a run-away instance, let's not let these become giant
        if (Body.Size >= _maxSize) Body.Size = _originalSize;
    ...
    {
        if (regionWeather is null) return false;

        var weatherCurrentPosition = regionWeather.CurrentPosition(SimpleScheduler.Ticks);
        if (Body.X > weatherCurrentPosition - regionWeather.Width && Body.X < weatherCurrentPosition) return true;
