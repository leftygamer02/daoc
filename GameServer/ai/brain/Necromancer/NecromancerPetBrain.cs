...
public class NecromancerPetBrain : ControlledNpcBrain
{
    ...
    public override void Notify(DOLEvent e, object sender, EventArgs args)
    {
        ...
        {
            var petSpell = (PetSpellEventArgs) args;
            var hadQueuedSpells = false;

            if (SpellsQueued)
            {
                MessageToOwner(
                    LanguageMgr.GetTranslation((Owner as GamePlayer).Client.Account.Language,
        ...
        else if (e == GameLivingEvent.Dying)
        {
            ...
            if (playerowner != null && Body.DamageRvRMemory > 0) playerowner.DamageRvRMemory = Body.DamageRvRMemory;
        ...
        else if (e == GameLivingEvent.CastSucceeded)
        {
            ...
            var spellArgs = args as PetSpellEventArgs;
            var target = spellArgs.Target;
            var spellLine = spellArgs.SpellLine;
    ...
    {
        var shadeEffect = Owner.EffectList.GetOfType<NecromancerShadeEffect>();
        ...
        if (shadeEffect != null)
        {
            ...
            var effectList = new ArrayList(1);
            effectList.Add(shadeEffect);
            var effectsCount = 1;
            if (Owner is GamePlayer) (Owner as GamePlayer).Out.SendUpdateIcons(effectList, ref effectsCount);
        }
    }
    ...
    {
        var spellsQueued = m_attackSpellQueue.Count;
        for (var i = 0; i < spellsQueued; i++)
        {
            var entry = GetSpellFromAttackQueue();
            if (entry != null)
                if (!CastSpell(entry.Spell, entry.SpellLine, entry.Target))
                    // If the spell can't be cast, remove it from the queue
                    RemoveSpellFromAttackQueue();
    ...
    {
        var spellTarget = target as GameLiving;
    ...
    private SpellQueueEntry GetSpellFromQueue()
    {
        lock (m_spellQueue)
        {
            ...
            {
                DebugMessageToOwner(string.Format(
                    "Grabbing spell '{0}' from the start of the queue in order to cast it",
    ...
    private SpellQueueEntry GetSpellFromAttackQueue()
    {
        lock (m_attackSpellQueue)
        {
            if (m_attackSpellQueue.Count > 0)
            {
                DebugMessageToOwner(string.Format(
                    "Grabbing spell '{0}' from the start of the queue in order to cast it",
                    m_attackSpellQueue.Peek().Spell.Name));
    ...
    private void DebugMessageToOwner(string message)
    {
        if (GS.ServerProperties.Properties.ENABLE_DEBUG)
        {
            ...
            MessageToOwner(
                string.Format("[{0:00}:{1:00}.{2:000}] {3}", minutes % 60, seconds % 60, tick % 1000, message),
                eChatType.CT_Staff, Owner as GamePlayer);
        }
    }
