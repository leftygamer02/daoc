...
public class ControlledNpcBrain : StandardMobBrain, IControlledBrain
{
    ...
    public const int
        MAX_PET_AGGRO_DISTANCE =
            512; // Tolakram - Live test with caby pet - I was extremely close before auto aggro
    ...
    /// </summary>
    protected override int NoPlayersStopDelay => int.MaxValue;

    /// <summary>
    /// The interval for thinking, set via server property, default is 1500 or every 1.5 seconds
    /// </summary>
    public override int ThinkInterval => GS.ServerProperties.Properties.PET_THINK_INTERVAL;
    ...
    public override bool CheckSpells(eCheckSpellType type)
    {
        ...
        if (type == eCheckSpellType.Defensive)
        {
            ...
            if (Body.CanCastInstantHealSpells)
                foreach (var spell in Body.InstantHealSpells)
            ...
            if (Body.CanCastHealSpells)
                foreach (var spell in Body.HealSpells)
                    if (CheckDefensiveSpells(spell))
                    {
                        casted = true;
                        break;
        ...
        else if (Body.TargetObject is GameLiving living && living.IsAlive)
        {
            ...
            if (Body.CanCastHarmfulSpells)
                foreach (var spell in Body.HarmfulSpells)
                    if (CheckOffensiveSpells(spell))
                    {
                        casted = true;
                        break;
    ...
    protected override bool CheckDefensiveSpells(Spell spell)
    {
        ...
        switch (spell.SpellType)
        {
            ...
            {
                var target = spell.Target.ToUpper();
                ...
                if (target is "REALM" or "GROUP")
                {
                    ...
                    if (owner is GameNPC npc)
                        //Buff other minions
                        foreach (var icb in npc.ControlledNpcList)
            ...
            case (byte) eSpellType.SpreadHeal:
                var spellTarget = spell.Target.ToUpper();
                ...
                var healThreshold = Body.Name.Contains("underhill")
                    ? GS.ServerProperties.Properties.NPC_HEAL_THRESHOLD / 2
                    : GS.ServerProperties.Properties.NPC_HEAL_THRESHOLD;

                if (Body.Name.Contains("empyrean"))
                    healThreshold = Body.Name.Contains("empyrean")
                        ? GS.ServerProperties.Properties.CHARMED_NPC_HEAL_THRESHOLD
                        : GS.ServerProperties.Properties.NPC_HEAL_THRESHOLD;
                ...
                var emergencyThreshold = healThreshold / 2;

                //Heal owner
                owner = (this as IControlledBrain).Owner;
                int ownerPercent = owner.HealthPercent;
                if (ownerPercent < emergencyThreshold && !spell.TargetHasEffect(owner) &&
                    Body.IsWithinRadius(owner, spell.Range))
                {
                    Body.TargetObject = owner;
                    break;
                }
            ...
            default:
                log.Warn(
                    $"CheckDefensiveSpells() encountered an unknown spell type [{spell.SpellType}], calling base method");
    ...
    protected override GameLiving CalculateNextAttackTarget()
    {
        ...
        lock ((AggroTable as ICollection).SyncRoot)
        {
            ...
            {
                var living = (GameLiving) aggros.Key;
                ...
                if (living != null)
                {
                    ...
                    else
                    {
                        ...
                        var root =
                            EffectListService.GetSpellEffectOnTarget(living, eEffect.MovementSpeedDebuff);
                        if (root != null && root.SpellHandler.Spell.Value == 99) removable.Add(living);
                    }
                }
            }

            foreach (var living in removable)
            {
    ...
    public override void AttackMostWanted()
    {
        ...
        if (owner_npc != null && owner_npc.Brain is StandardMobBrain)
            ...
            {
                if (!CheckSpells(eCheckSpellType.Offensive)) Body.StartAttack(owner_npc.TargetObject);

                return;
            }

        var target = CalculateNextAttackTarget();
        ...
        if (target != null)
        {
            if (!Body.IsAttacking || target != Body.TargetObject)
            {
                ...
                var effects = new List<GameSpellEffect>();

                lock (Body.EffectList)
                {
                    foreach (var effect in Body.EffectList)
                        if (effect is GameSpellEffect &&
                            (effect as GameSpellEffect).SpellHandler is SpeedEnhancementSpellHandler)
                            effects.Add(effect as GameSpellEffect);
                }

                lock (Owner.EffectList)
                {
                    foreach (var effect in Owner.EffectList)
                        if (effect is GameSpellEffect &&
                            (effect as GameSpellEffect).SpellHandler is SpeedEnhancementSpellHandler)
                            effects.Add(effect as GameSpellEffect);
                }

                foreach (var effect in effects) effect.Cancel(false);
            }
        ...
        else
        {
            ...
            if (WalkState == eWalkState.Follow)
                FollowOwner();
            else if (m_tempX > 0 && m_tempY > 0 && m_tempZ > 0) Body.WalkTo(m_tempX, m_tempY, m_tempZ, Body.MaxSpeed);
        }
    }
    ...
    public virtual void OnOwnerAttacked(AttackData ad)
    {
        ...
        if (ad.Target is GamePlayer && ((ad.Target as GamePlayer).ControlledBrain != this ||
                                        (ad.Target as GamePlayer).ControlledBrain.Body == Owner))
            return;
