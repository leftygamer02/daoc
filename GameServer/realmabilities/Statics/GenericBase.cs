...
public abstract class GenericBase : GameStaticItem
{
    ...
    public override bool AddToWorld()
    {
        ...
        GameEventMgr.AddHandler(m_caster, GameObjectEvent.RemoveFromWorld, new DOLEventHandler(PlayerLeftWorld));
    ...
    protected virtual int PulseTimer(ECSGameTimer timer)
    {
        ...
        {
            currentPulse++;
            foreach (GamePlayer target in GetPlayersInRadius(m_radius)) CastSpell(target);

            foreach (GameNPC npc in GetNPCsInRadius(m_radius)) CastSpell(npc);
        }
    ...
    protected virtual void PlayerLeftWorld(DOLEvent e, object sender, EventArgs args)
    {
        ...
        if (m_caster == player) currentTick = (int) m_lifeTime;
    }
