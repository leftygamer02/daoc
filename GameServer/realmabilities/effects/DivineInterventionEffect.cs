...
public class DivineInterventionEffect : TimedEffect, IGameEffect
{
    ...
    private ArrayList m_affected = new();
    ...
    public override void Start(GameLiving living)
    {
        ...
        m_playerOwner.Out.SendMessage("You group is protected by a pool of healing!", eChatType.CT_Spell,
        ...
        foreach (var gp in m_group.GetPlayersInTheGroup())
        {
            ...
            gp.Out.SendMessage("You are protected by a pool of healing!", eChatType.CT_Spell,
                eChatLoc.CL_SystemWindow);
            m_affected.Add(gp);
            ...
            if (gp.CharacterClass.ID == (int) eCharacterClass.Necromancer)
                if (gp.ControlledBrain != null)
                {
                    ...
                    GameEventMgr.AddHandler(gp.ControlledBrain.Body, GameObjectEvent.TakeDamage,
                        new DOLEventHandler(TakeDamageNPC));
                }
        }
    }
    ...
    {
        var pjargs = args as MemberJoinedEventArgs;
        ...
        {
            ((GamePlayer) pjargs.Member).Out.SendMessage("You are protected by a pool of healing!",
            ...
            if (((GamePlayer) pjargs.Member).CharacterClass.ID == (int) eCharacterClass.Necromancer)
                if (((GamePlayer) pjargs.Member).ControlledBrain != null)
                {
                    ...
                    GameEventMgr.AddHandler(((GamePlayer) pjargs.Member).ControlledBrain.Body,
                        GameObjectEvent.TakeDamage, new DOLEventHandler(TakeDamageNPC));
                }
        }
    }
    ...
    {
        var pdargs = args as MemberDisbandedEventArgs;
        ...
        {
            ((GamePlayer) pdargs.Member).Out.SendMessage("You are no longer protected by a pool of healing!",
            ...
            if (((GamePlayer) pdargs.Member).CharacterClass.ID == (int) eCharacterClass.Necromancer)
                if (((GamePlayer) pdargs.Member).ControlledBrain != null)
                {
                    ...
                    GameEventMgr.RemoveHandler(((GamePlayer) pdargs.Member).ControlledBrain.Body,
                        GameObjectEvent.TakeDamage, new DOLEventHandler(TakeDamageNPC));
                }
        }
    ...
    {
        var targs = args as TakeDamageEventArgs;
        ...
        var dmgamount = npc.MaxHealth - npc.Health;
        ...
        if (petOwner != null)
            petOwner.Out.SendMessage(
                "Your " + npc.Name + " was healed by the pool of healing for " + healamount + "!",
                eChatType.CT_Spell, eChatLoc.CL_SystemWindow);

        m_playerOwner.Out.SendMessage(
            "Your pool of healing heals the " + npc.Name + " of " + petOwner.Name + " for " + healamount + "!",
            eChatType.CT_Spell, eChatLoc.CL_SystemWindow);
    ...
    protected void TakeDamage(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var dmgamount = player.MaxHealth - player.Health;
        ...
        player.Out.SendMessage("You are healed by the pool of healing for " + healamount + "!", eChatType.CT_Spell,
            eChatLoc.CL_SystemWindow);
        m_playerOwner.Out.SendMessage("Your pool of healing heals " + player.Name + " for " + healamount + "!",
            eChatType.CT_Spell, eChatLoc.CL_SystemWindow);
    ...
    public override void Stop()
    {
        ...
        if (m_group != null)
        {
            GameEventMgr.RemoveHandler(m_group, GroupEvent.MemberDisbanded,
                new DOLEventHandler(PlayerDisbandedGroup));
        ...
        foreach (GameLiving l in m_affected)
            if (l is GamePlayer)
            {
                (l as GamePlayer).Out.SendMessage("You are no longer protected by a pool of healing!",
                    eChatType.CT_SpellExpires, eChatLoc.CL_SystemWindow);
    ...
    public override IList<string> DelveInfo
    {
        get
        {
            ...
            if (seconds > 0)
            {
                delveInfoList.Add(" ");
                if (seconds > 60)
                    delveInfoList.Add("- " + seconds / 60 + ":" + (seconds % 60).ToString("00") +
                                      " minutes remaining.");
                else
