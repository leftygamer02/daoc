...
public class VanishEffect : TimedEffect
{
    ...
    public int CountDown(ECSGameTimer timer)
    {
        if (m_countdown > 0)
        {
            ((GamePlayer) Owner).Out.SendMessage("You are hidden for " + m_countdown + " more seconds!",
                eChatType.CT_SpellPulse, eChatLoc.CL_SystemWindow);
            m_countdown--;
            return 1000;
        }
    ...
    public double SpeedBonus => m_speedBonus;
