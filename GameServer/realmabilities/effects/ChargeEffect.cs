...
public class ChargeEffect : StaticEffect, IGameEffect
{
    ...
    public override void Start(GameLiving living)
    {
        ...
        {
            ((GamePlayer) m_living).Out.SendMessage("You begin to charge wildly!", eChatType.CT_Spell,
                eChatLoc.CL_SystemWindow);
        }
        else if (m_living is GameNPC)
        {
            var icb = ((GameNPC) m_living).Brain as IControlledBrain;
    ...
    public override void Cancel(bool playerCancel)
    {
        ...
        if (m_living is GamePlayer)
        {
            var player = m_living as GamePlayer;
    ...
    {
        StopTimers();
        m_expireTimer = new ECSGameTimer(m_living, new ECSGameTimer.ECSTimerCallback(ExpiredCallback),
            RealmAbilities.ChargeAbility.DURATION * 1000);
    }
