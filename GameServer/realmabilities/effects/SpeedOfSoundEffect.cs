...
public class SpeedOfSoundEffect : TimedEffect, IGameEffect
{
    ...
    private DOLEventHandler m_attackFinished = new(AttackFinished);
    ...
    private static void AttackFinished(DOLEvent e, object sender, EventArgs args)
    {
        ...
        {
            var cfea = args as CastingEventArgs;
            ...
            {
                var effect = player.EffectList.GetOfType<SpeedOfSoundEffect>();
                if (effect != null)
                    effect.Cancel(false);
            }
        }
        ...
        {
            var afargs = args as AttackFinishedEventArgs;
            if (afargs == null)
                return;
