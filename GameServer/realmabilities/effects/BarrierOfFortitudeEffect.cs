...
public class XBarrierOfFortitudeEffect : StaticEffect, IGameEffect
{
    private const string m_delveString =
        "Grants the group a melee absorption bonus (Does not stack with Soldier's Barricade or Bedazzling Aura).";
    ...
    private static void PlayerLeftWorld(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var BoFEffect = player.EffectList.GetOfType<XBarrierOfFortitudeEffect>();
        if (BoFEffect != null) BoFEffect.Cancel(false);
    }
