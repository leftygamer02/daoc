...
public class ShadowShroudEffect : TimedEffect
{
    ...
    public override void Start(GameLiving target)
    {
        ...
        if (target is GamePlayer)
        {
            ...
            GameEventMgr.AddHandler(EffectOwner, GamePlayerEvent.RegionChanged,
                new DOLEventHandler(PlayerLeftWorld));
        }
    }
    ...
    public override void Stop()
    {
        if (EffectOwner != null)
        {
            ...
            GameEventMgr.RemoveHandler(EffectOwner, GamePlayerEvent.Linkdeath,
                new DOLEventHandler(PlayerLeftWorld));
            GameEventMgr.RemoveHandler(EffectOwner, GamePlayerEvent.RegionChanged,
                new DOLEventHandler(PlayerLeftWorld));
        }
    ...
    protected void PlayerLeftWorld(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var ShadowShroud = (ShadowShroudEffect) player.EffectList.GetOfType<ShadowShroudEffect>();
    ...
    protected void OnAttack(DOLEvent e, object sender, EventArgs arguments)
    {
        ...
        var absorbPercent = ShadowShroudAbility.ABSPERCENT;
        var damageAbsorbed = (int) (0.01 * absorbPercent * (ad.Damage + ad.CriticalDamage));
        if (damageAbsorbed > 0)
        {
            ad.Damage -= damageAbsorbed;
            //TODO correct messages
            MessageToLiving(ad.Target, string.Format("Shadow Shroud Ability absorbs {0} damage!", damageAbsorbed),
                eChatType.CT_Spell);
            MessageToLiving(ad.Attacker,
                string.Format("A barrier absorbs {0} damage of your attack!", damageAbsorbed), eChatType.CT_Spell);
        }
    }
    ...
    {
        if (living is GamePlayer && message != null && message.Length > 0) living.MessageToSelf(message, type);
    }
