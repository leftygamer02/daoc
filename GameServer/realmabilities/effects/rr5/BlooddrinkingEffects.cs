...
public class BloodDrinkingEffect : TimedEffect
{
    ...
    protected void PlayerLeftWorld(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var BloodDrinking =
            (BloodDrinkingEffect) player.EffectList.GetOfType<BloodDrinkingEffect>();
    ...
    protected void OnAttack(DOLEvent e, object sender, EventArgs arguments)
    {
        ...
        var healPercent = BloodDrinkingAbility.HEALPERCENT;
        var healAbsorbed = (int) (0.01 * healPercent * (ad.Damage + ad.CriticalDamage));
        if (healAbsorbed > 0)
        {
            if (living.Health < living.MaxHealth)
            {
                //TODO correct messages
                MessageToLiving(living,
                    string.Format("Blooddrinking ability is healing you for {0} health points!", healAbsorbed),
                    eChatType.CT_Spell);
                foreach (GamePlayer p in EffectOwner.GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE))
                    //heal effect
