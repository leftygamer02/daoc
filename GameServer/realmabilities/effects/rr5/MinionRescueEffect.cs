...
public class MinionRescueEffect : TimedEffect
{
    ...
    public override void Start(GameLiving target)
    {
        ...
        if (target is GamePlayer)
        {
            ...
            var targetCount = 0;
            foreach (GamePlayer targetPlayer in EffectOwner.GetPlayersInRadius(
                         (ushort) MinionRescueAbility.SpellRadius))
            {
                if (targetCount == spiritCount) return;
                if (targetPlayer.IsAlive &&
                    GameServer.ServerRules.IsAllowedToAttack(EffectOwner, targetPlayer, true))
                {
    ...
    {
        for (var index = 0; index < spiritCount; index++)
        {
            if (spiritTimer[index] != null)
            {
                spiritTimer[index].Stop();
                spiritTimer[index] = null;
            }

            if (spirits[index] != null)
            {
                spirits[index].Delete();
                spirits[index] = null;
            }
        }
    ...
    private void SummonSpirit(int spiritId, GamePlayer targetPlayer)
    {
        ...
        spiritTimer[spiritId] =
            new ECSGameTimer(spirits[spiritId], new ECSGameTimer.ECSTimerCallback(spiritCallBack), 200);
    }
    ...
    private int spiritCallBack(ECSGameTimer timer)
    {
        ...
        var spirit = timer.TimerOwner as GameNPC;
        var targetPlayer = spirit.TargetObject as GamePlayer;
