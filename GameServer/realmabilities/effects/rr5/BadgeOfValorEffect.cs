...
public class BadgeOfValorEffect : TimedEffect
{
    ...
    {
        var afea = (AttackFinishedEventArgs) args;
        ...
        if (!(afea.AttackData.Target is GamePlayer))
            return;
        var target = afea.AttackData.Target as GamePlayer;

        var armor =
            target.Inventory.GetItem((eInventorySlot) (int) afea.AttackData.ArmorHitLocation);

        if (armor == null || armor.SPD_ABS == 0)
            return;
        //cap at 50%
        var bonusPercent = Math.Min(armor.SPD_ABS, 50);
