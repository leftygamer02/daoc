...
public class SearingPetEffect : TimedEffect
{
    ...
    protected virtual int PulseTimer(ECSGameTimer timer)
    {
        ...
        {
            foreach (GamePlayer target in pet.GetPlayersInRadius(spellRadius)) pbaoe.StartSpell(target);

            foreach (GameNPC npc in pet.GetNPCsInRadius(spellRadius)) pbaoe.StartSpell(npc);
        }
    ...
    private static void PlayerLeftWorld(DOLEvent e, object sender, EventArgs args)
    {
        ...
        {
            var pet = player.ControlledBrain.Body as GameNPC;
            var SearingPet = pet.EffectList.GetOfType<SearingPetEffect>();
            if (SearingPet != null)
