...
public class TestudoEffect : TimedEffect
{
    ...
    private void OnAttack(DOLEvent e, object sender, EventArgs arguments)
    {
        ...
        var shield = living.Inventory.GetItem(eInventorySlot.LeftHandWeapon);
        if (shield == null)
            return;
        ...
        if (living is GamePlayer)
            ((GamePlayer) living).Out.SendMessage(
                "Your Testudo Stance reduces the damage by " + (absorb + critic) + " points", eChatType.CT_Spell,
    ...
    {
        if (args == null) return;
        var ag = args as AttackFinishedEventArgs;
        if (ag == null) return;
