    ...
    private void OnAttack(DOLEvent e, object sender, EventArgs arguments)
    {
        ...
        if (extra > 0)
            player.Out.SendMessage(
                ...
                " additional points of damage", eChatType.CT_Spell, eChatLoc.CL_SystemWindow);
        var injuredTargets = new Hashtable();
        ...
        var groupHealCap = args.AttackData.Damage;
        var targetHealCap = args.AttackData.Damage;
        if (player.Group.MemberCount > 2)
        {
            ...
            foreach (var p in player.Group.GetPlayersInTheGroup())
            {
                ...
                {
                    var playerHealthPercent = p.Health / (double) p.MaxHealth;
        ...
        var bestHealPercent = targetHealCap / (double) mostInjuredLiving.MaxHealth;
        double totalHealed = 0;
        var healAmount = new Hashtable();

        var iter = injuredTargets.GetEnumerator();
        //calculate heal for all targets
        while (iter.MoveNext())
        {
            var healTarget = iter.Key as GameLiving;
            var targetHealthPercent = (double) iter.Value;
            //targets hp percent after heal is same as mostInjuredLiving
            var targetHealPercent = bestHealPercent + mostInjuredPercent - targetHealthPercent;
            var targetHeal = (int) (healTarget.MaxHealth * targetHealPercent);
        ...
        while (iter.MoveNext())
        {
            ...
            var reducedHeal = (int) Math.Min(targetHealCap, uncappedHeal * (groupHealCap / totalHealed));

            //heal target
            var baseheal = healTarget.MaxHealth - healTarget.Health;
            if (reducedHeal < baseheal)
                baseheal = reducedHeal;
            healTarget.ChangeHealth(player, eHealthChangeType.Spell, baseheal);
            player.Out.SendMessage("You heal " + healTarget.Name + " for " + baseheal + "!", eChatType.CT_Spell,
                eChatLoc.CL_SystemWindow);
            if (healTarget is GamePlayer)
                ((GamePlayer) healTarget).Out.SendMessage(player.Name + " heals you for " + baseheal + "!",
