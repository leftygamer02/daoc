...
public class RetributionOfTheFaithfulEffect : TimedEffect
{
    ...
    private void OnAttack(DOLEvent e, object sender, EventArgs arguments)
    {
        ...
        if (args.AttackData.Attacker.EffectList.GetOfType<ChargeEffect>() != null ||
            args.AttackData.Attacker.TempProperties.getProperty("Charging", false))
            return;
        if (!owner.IsWithinRadius(args.AttackData.Attacker, 300)) return;
        if (Util.Chance(50))
        {
            var effect = new RetributionOfTheFaithfulStunEffect();
