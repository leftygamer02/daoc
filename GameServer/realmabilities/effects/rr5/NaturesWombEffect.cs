...
public class NaturesWombEffect : TimedEffect
{
    ...
    private void OnAttack(DOLEvent e, object sender, EventArgs arguments)
    {
        ...
        var heal = ad.Damage + ad.CriticalDamage;
        ad.Damage = 0;
        ad.CriticalDamage = 0;
        var player = living as GamePlayer;
        var attackplayer = ad.Attacker as GamePlayer;
        if (attackplayer != null)
            attackplayer.Out.SendMessage(living.Name + "'s druidic powers absorb your attack!",
                eChatType.CT_SpellResisted, eChatLoc.CL_SystemWindow);
        var modheal = living.MaxHealth - living.Health;
        if (modheal > heal)
            modheal = heal;
        living.Health += modheal;
        if (player != null)
            player.Out.SendMessage(
                "Your druidic powers convert your enemies attack and heal you for " + modheal + "!",
