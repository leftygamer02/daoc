...
public class DashingDefenseEffect : StaticEffect, IGameEffect
{
    ...
    // <//summary>
    protected const string delveString =
        "Ability that if successful will guard an attack meant for the ability's target. You will block in the target's place.";
    ...
    public void Start(GamePlayer guardSource, GamePlayer guardTarget, int duration)
    {
        ...
        if (!guardSource.IsWithinRadius(guardTarget, GUARD_DISTANCE))
        {
            guardSource.Out.SendMessage(
                string.Format("You are now guarding {0}, but you must stand closer.",
                    guardTarget.GetName(0, false)), eChatType.CT_System, eChatLoc.CL_SystemWindow);
            guardTarget.Out.SendMessage(
                string.Format("{0} is now guarding you, but you must stand closer.", guardSource.GetName(0, true)),
                eChatType.CT_System, eChatLoc.CL_SystemWindow);
        }
        else
        {
            guardSource.Out.SendMessage(string.Format("You are now guarding {0}.", guardTarget.GetName(0, false)),
                eChatType.CT_System, eChatLoc.CL_SystemWindow);
            guardTarget.Out.SendMessage(string.Format("{0} is now guarding you.", guardSource.GetName(0, true)),
                eChatType.CT_System, eChatLoc.CL_SystemWindow);
        }
    ...
    public override void Cancel(bool playerCancel)
    {
        ...
        m_guardSource.Out.SendMessage(
            string.Format("You are no longer guarding {0}.", m_guardTarget.GetName(0, false)), eChatType.CT_System,
            eChatLoc.CL_SystemWindow);
        m_guardTarget.Out.SendMessage(
            string.Format("{0} is no longer guarding you.", m_guardSource.GetName(0, true)), eChatType.CT_System,
            eChatLoc.CL_SystemWindow);
    ...
    {
        StopTimers();
        m_expireTimer = new ECSGameTimer(GuardSource, new ECSGameTimer.ECSTimerCallback(ExpireCallback),
            m_effectDuration * 1000);
    }
