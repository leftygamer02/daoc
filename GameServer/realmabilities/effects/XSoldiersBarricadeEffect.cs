...
{
    private const string m_delveString =
        "Grants the group an absorption bonus to all forms of damage (Does not stack with Barrier of Fortitude or Bedazzling Aura).";
    ...
    private static void PlayerLeftWorld(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var SBEffect = player.EffectList.GetOfType<XSoldiersBarricadeEffect>();
        if (SBEffect != null) SBEffect.Cancel(false);
    }
    ...
    {
        StopTimers();
        m_expireTimer = new ECSGameTimer(m_player, new ECSGameTimer.ECSTimerCallback(ExpireCallback),
