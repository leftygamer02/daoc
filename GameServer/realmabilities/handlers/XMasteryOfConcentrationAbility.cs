...
public class MasteryofConcentrationAbility : TimedRealmAbility
{
    ...
    public override void Execute(GameLiving living)
    {
        ...
        var MoCEffect = caster.EffectList.GetOfType<XMasteryofConcentrationEffect>();
        if (MoCEffect != null)
        {
        ...
        var ra5l = caster.EffectList.GetOfType<ShieldOfImmunityEffect>();
        if (ra5l != null)
        {
            caster.Out.SendMessage("You cannot currently use this ability", eChatType.CT_SpellResisted,
        ...
        foreach (GamePlayer player in caster.GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE))
            if (caster.IsWithinRadius(player, WorldMgr.INFO_DISTANCE))
            {
                ...
                {
                    player.MessageFromArea(caster, caster.Name + " casts a spell!", eChatType.CT_Spell,
                        eChatLoc.CL_SystemWindow);
                    player.Out.SendMessage(caster.Name + "'s castings have perfect poise!", eChatType.CT_System,
                        eChatLoc.CL_SystemWindow);
                }
            }
