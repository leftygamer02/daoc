    ...
    public override bool CheckPreconditions(GameLiving living, long bitmask)
    {
        lock (living.EffectList)
        {
            ...
            if (effect != null && effect.SpellHandler.Spell.Value != 99)
            {
                ...
                if (player != null)
                    player.Out.SendMessage("You may not use this ability while snared!", eChatType.CT_System,
    ...
    public override void Execute(GameLiving living)
    {
            ...
            (living.TargetObject is GamePlayer enemy && enemy.Realm == living.Realm))
        {
            if (living is GamePlayer p)
                p.Out.SendMessage($"You can only charge enemy players.", eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);
            return;
        }
        ...
        //}
        var charge =
            (ChargeECSGameEffect) EffectListService.GetEffectOnTarget(living, eEffect.Charge);
