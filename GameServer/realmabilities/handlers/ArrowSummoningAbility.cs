...
public class ArrowSummoningAbility : TimedRealmAbility
{
    ...
    public override void Execute(GameLiving living)
    {
        ...
        var arrow_summoning_1 = GameServer.Database.FindObjectByKey<ItemTemplate>("arrow_summoning1");
        var arrow_summoning_2 = GameServer.Database.FindObjectByKey<ItemTemplate>("arrow_summoning2");
        var arrow_summoning_3 = GameServer.Database.FindObjectByKey<ItemTemplate>("arrow_summoning3");

        // what are these used for? - tolakram
        var as1 = WorldInventoryItem.CreateFromTemplate(arrow_summoning_1);
        var as2 = WorldInventoryItem.CreateFromTemplate(arrow_summoning_2);
        var as3 = WorldInventoryItem.CreateFromTemplate(arrow_summoning_3);

        if (!player.Inventory.AddTemplate(GameInventoryItem.Create(arrow_summoning_1), 10,
                eInventorySlot.FirstBackpack, eInventorySlot.LastBackpack))
            player.Out.SendMessage("You do not have enough inventory space to place this item!",
                eChatType.CT_System, eChatLoc.CL_SystemWindow);
        else if (!player.Inventory.AddTemplate(GameInventoryItem.Create(arrow_summoning_2), 10,
                     eInventorySlot.FirstBackpack, eInventorySlot.LastBackpack))
            player.Out.SendMessage("You do not have enough inventory space to place this item!",
                eChatType.CT_System, eChatLoc.CL_SystemWindow);
        else if (!player.Inventory.AddTemplate(GameInventoryItem.Create(arrow_summoning_3), 10,
                     eInventorySlot.FirstBackpack, eInventorySlot.LastBackpack))
            player.Out.SendMessage("You do not have enough inventory space to place this item!",
                eChatType.CT_System, eChatLoc.CL_SystemWindow);
    ...
    public void PlayerQuit(DOLEvent e, object sender, EventArgs arguments)
    {
        ...
        {
            var item = player.Inventory.GetFirstItemByID("arrow_summoning1", eInventorySlot.FirstBackpack,
                eInventorySlot.LastBackpack);
            while (item != null)
            {
                player.Inventory.RemoveItem(item);
                item = player.Inventory.GetFirstItemByID("arrow_summoning1", eInventorySlot.FirstBackpack,
                    eInventorySlot.LastBackpack);
            }

            item = player.Inventory.GetFirstItemByID("arrow_summoning2", eInventorySlot.FirstBackpack,
                eInventorySlot.LastBackpack);
            while (item != null)
            {
                player.Inventory.RemoveItem(item);
            ...
            item = player.Inventory.GetFirstItemByID("arrow_summoning3", eInventorySlot.FirstBackpack,
                eInventorySlot.LastBackpack);
            while (item != null)
            {
                player.Inventory.RemoveItem(item);
    ...
    public static void OnScriptLoaded(DOLEvent e, object sender, EventArgs args)
    {
        ...
        var arrow_summoning1 = GameServer.Database.FindObjectByKey<ItemTemplate>("arrow_summoning1");
        ...
        var arrow_summoning2 = GameServer.Database.FindObjectByKey<ItemTemplate>("arrow_summoning2");
        ...
        var arrow_summoning3 = GameServer.Database.FindObjectByKey<ItemTemplate>("arrow_summoning3");
