...
public class TheEmptyMindAbility : TimedRealmAbility
{
        ...
        foreach (GamePlayer t_player in living.GetPlayersInRadius(WorldMgr.INFO_DISTANCE))
            if (t_player == living && living is GamePlayer)
                (living as GamePlayer).Out.SendMessage(
                    "You clear your mind and become more resistant to magic damage!", eChatType.CT_Spell,
                    eChatLoc.CL_SystemWindow);
            else
                t_player.Out.SendMessage(living.Name + " casts a spell!", eChatType.CT_Spell,
                    eChatLoc.CL_SystemWindow);

        var effectiveness = GetEffectiveness();
    ...
    private SpellHandler CreateSpell(GameLiving caster)
    {
        ...
        dbspell.Frequency = 0;
        dbspell.Range = 1500;
        var spell = new Spell(dbspell, 0); // make spell level 0 so it bypasses the spec level adjustment code
        var line = new SpellLine("RAs", "RealmAbilities", "RealmAbilities", true);
