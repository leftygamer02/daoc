...
public class DecimationTrapAbility : TimedRealmAbility
{
    ...
    {
        var args = arguments as AreaEventArgs;
        if (args == null)
            return;
        var living = args.GameObject as GameLiving;
        if (living == null)
            return;
    ...
    {
        foreach (GamePlayer target in WorldMgr.GetPlayersCloseToSpot(region, traparea.X, traparea.Y, traparea.Z,
                     350))
    ...
    private void DamageTarget(GameLiving target)
    {
        ...
        var dist = target.GetDistanceTo(new Point3D(traparea.X, traparea.Y, traparea.Z));
        double mod = 1;
        if (dist > 0)
            mod = 1 - (double) dist / 350;

        var basedamage = (int) (effectiveness * mod);
        var resist = (int) (basedamage * target.GetModified(eProperty.Resist_Energy) * -0.01);
