...
public abstract class RAPropertyEnhancer : L5RealmAbility
{
        ...
        get
        {
            ...
            for (var i = 1; i <= MaxLevel; i++)
                list.Add(LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE,
                    "RAPropertyEnhancer.DelveInfo.Info1", i, GetAmountForLevel(i), ValueUnit));
    ...
    /// </summary>
    protected virtual string ValueUnit => "";

    public override void Activate(GameLiving living, bool sendUpdates)
    {
        if (m_activeLiving == null)
        {
            foreach (var property in m_property) living.AbilityBonus[(int) property] += GetAmountForLevel(Level);
    ...
    public override void OnLevelChange(int oldLevel, int newLevel = 0)
    {
        ...
        foreach (var property in m_property)
            m_activeLiving.AbilityBonus[(int) property] +=
                GetAmountForLevel(newLevel) - GetAmountForLevel(oldLevel);
