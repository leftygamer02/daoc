...
public class NegativeMaelstromAbility : TimedRealmAbility
{
    ...
    public override void Execute(GameLiving living)
    {
        ...
        if (caster.IsMoving)
        {
            caster.Out.SendMessage("You must be standing still to use this ability!", eChatType.CT_System,
        ...
        {
            caster.Out.SendMessage("You groundtarget is too far away to use this ability!", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            return;
        }
        ...
        {
            caster.Out.SendMessage("You are already casting an ability.", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            return;
        }

        player = caster;
        if (caster.attackComponent.AttackState) caster.attackComponent.StopAttack();
        ...
        foreach (GamePlayer i_player in caster.GetPlayersInRadius(WorldMgr.INFO_DISTANCE))
        {
            if (i_player == caster)
                ...
                i_player.MessageFromArea(caster, caster.Name + " casts a spell!", eChatType.CT_Spell,
                    eChatLoc.CL_SystemWindow);
        ...
        if (caster != null) new ECSGameTimer(caster, new ECSGameTimer.ECSTimerCallback(EndCast), 2000);
    }

    protected virtual int EndCast(ECSGameTimer timer)
    {
        var castWasSuccess = player.TempProperties.getProperty(NM_CAST_SUCCESS, false);
        ...
        GameEventMgr.RemoveHandler(player, GameLivingEvent.AttackFinished, new DOLEventHandler(CastInterrupted));
        GameEventMgr.RemoveHandler(player, GameLivingEvent.Dying, new DOLEventHandler(CastInterrupted));
        if (player.IsMezzed || player.IsStunned || player.IsSitting)
            return 0;
        if (!castWasSuccess)
            return 0;
        var nm = new Statics.NegativeMaelstromBase(dmgValue);
