...
public class SoulQuenchAbility : RR5RealmAbility
{
    ...
    {
        var modifier = 0.5 + caster.GetModifiedSpecLevel("Soulrending") * 0.01;
        var basedamage = (int) (250 * modifier);
        var resist = basedamage * target.GetResist(eDamageType.Spirit) / -100;
        var damage = basedamage + resist;
        var heal = (int) (damage * 0.75);
        var modheal = caster.MaxHealth - caster.Health;
        if (modheal > heal)
            modheal = heal;
        ...
        if (caster is GamePlayer && modheal > 0)
            ((GamePlayer) caster).Out.SendMessage("Your Soul Quench returns " + modheal + " lifepoints to you",
