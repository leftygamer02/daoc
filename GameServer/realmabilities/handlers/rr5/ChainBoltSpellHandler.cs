...
{
    public ChainBoltSpellHandler(GameLiving caster, Spell spell, SpellLine spellLine) : base(caster, spell,
        spellLine)
    {
    }
    ...
    public override bool StartSpell(GameLiving target)
    {
        ...
        var ticksToTarget = m_currentSource.GetDistanceTo(target) * 100 / 85; // 85 units per 1/10s
        var delay = 1 + ticksToTarget / 100;
        foreach (GamePlayer player in target.GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE))
            player.Out.SendSpellEffectAnimation(m_currentSource, target, m_spell.ClientEffect, (ushort) delay,
                false, 1);
