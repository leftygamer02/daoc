...
public class ChainLightningAbility : RR5RealmAbility
{
    ...
    public override void Execute(GameLiving living)
    {
        ...
        var rules = GameServer.ServerRules as AbstractServerRules;
        var player = living as GamePlayer;
        var target = living.TargetObject as GamePlayer;
        if (player.TargetObject == null || target == null)
        {
            player.Out.SendMessage("You must target a player to launch this spell!", eChatType.CT_SpellResisted,
        ...
        {
            player.Out.SendMessage("You must select an enemy target!", eChatType.CT_SpellResisted,
                eChatLoc.CL_SystemWindow);
            return;
        }
        ...
        {
            Message.ChatToOthers(living, "You are too far away from your target to use this ability!",
    ...
    {
        var level = caster.GetModifiedSpecLevel("Stormcalling");
        if (level > 50)
            level = 50;
        ...
        if (player != null)
            player.Out.SendMessage(
                "You hit " + target.Name + " for " + damage + "(" + resist + ") points of damage!",
    ...
    {
        list.Add(
            "Casts a lightning bolt at the enemy target and hits up to 5 targets. If there is only one target available, the spell will hit once. If there are multiple targets, the spell has a chance to jump from target to target and back to the prior target. With each jump, the damage of the spell is reduced by 25%.");
        list.Add("");
        list.Add("Range: 1500");
