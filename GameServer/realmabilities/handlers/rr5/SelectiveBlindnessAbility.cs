...
public class SelectiveBlindnessAbility : RR5RealmAbility
{
    ...
    public override void Execute(GameLiving living)
    {
        ...
        if (living is GamePlayer)
        {
            ...
            {
                m_player.Out.SendMessage("You need a target for this ability!", eChatType.CT_System,
            ...
            {
                m_player.Out.SendMessage("This work only on players!", eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);
                m_player.DisableSkill(this, 3 * 1000);
                return;
            }
            ...
            {
                m_player.Out.SendMessage("This work only on enemies!", eChatType.CT_Spell,
                    eChatLoc.CL_SystemWindow);
                m_player.DisableSkill(this, 3 * 1000);
                return;
            }
            ...
            {
                m_player.Out.SendMessage(m_player.TargetObject + " is too far away!", eChatType.CT_Spell,
                    eChatLoc.CL_SystemWindow);
                m_player.DisableSkill(this, 3 * 1000);
                return;
            }
            ...
            foreach (GamePlayer radiusPlayer in m_player.GetPlayersInRadius(WorldMgr.INFO_DISTANCE))
            {
                if (radiusPlayer == m_player)
                    ...
                    radiusPlayer.MessageFromArea(m_player, m_player.Name + " casts a spell!", eChatType.CT_Spell,
                        eChatLoc.CL_SystemWindow);
            ...
            if (m_player.RealmAbilityCastTimer != null)
            {
                ...
                m_player.Out.SendMessage("You cancel your Spell!", eChatType.CT_SpellResisted,
                    eChatLoc.CL_SystemWindow);
            }
    ...
    private void EndCast()
    {
        ...
        {
            m_player.Out.SendMessage(m_targetPlayer + " is too far away.", eChatType.CT_Spell,
                eChatLoc.CL_SystemWindow);
            m_player.DisableSkill(this, 3 * 1000);
            return;
        }
        ...
        foreach (GamePlayer radiusPlayer in m_targetPlayer.GetPlayersInRadius(SpellRadius))
        {
            ...
            var SelectiveBlindness =
                radiusPlayer.EffectList.GetOfType<SelectiveBlindnessEffect>();
    ...
    {
        list.Add(
            "AE target 150 unit radius, 1500 unit range, blind enemies to user. 20s duration or attack by user, 5min RUT.");
        list.Add("");
