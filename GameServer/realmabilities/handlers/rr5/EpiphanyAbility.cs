...
public class EpiphanyAbility : RR5RealmAbility
{
    ...
    {
        var mana = (int) (target.MaxMana * 0.25);
        var modheal = target.MaxMana - target.Mana;
        if (modheal < 1)
            return false;
        if (modheal > mana)
            modheal = mana;
        if (target is GamePlayer && target != owner)
            ((GamePlayer) target).Out.SendMessage(
                owner.Name + " restores you " + modheal + " points of mana, and 50% of your endurance.",
                eChatType.CT_Spell, eChatLoc.CL_SystemWindow);
        if (target != owner)
            owner.Out.SendMessage(
                "You restore" + target.Name + " " + modheal + " points of mana, and 50% of their endurance.",
                eChatType.CT_Spell, eChatLoc.CL_SystemWindow);
        if (target == owner)
            owner.Out.SendMessage("You restore yourself " + modheal + " points of mana, and 50% of your endurance.",
                eChatType.CT_Spell, eChatLoc.CL_SystemWindow);
