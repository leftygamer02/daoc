...
public class PurgeAbility : TimedRealmAbility
{
    ...
    public override void Execute(GameLiving living)
    {
        ...
        if (ServerProperties.Properties.USE_NEW_ACTIVES_RAS_SCALING)
        {
            ...
            if (seconds > 0)
            {
                var timer = new PurgeTimer(living, this, seconds);
                timer.Interval = 1000;
                timer.Start(1);
                DisableSkill(living);
            }
        ...
        else
        {
            if (Level < 2)
            {
                var timer = new PurgeTimer(living, this, 5);
                timer.Interval = 1000;
                timer.Start(1);
                DisableSkill(living);
            }
    ...
    protected static bool RemoveNegativeEffects(GameLiving living, PurgeAbility purge,
        bool isFromGroupPurge = false)
    {
        var removed = false;
        var effectsToRemove = new ArrayList();
        ...
        {
            var necroPet = (NecromancerPet) player.ControlledBrain.Body;
        ...
        foreach (var e in effectListComponent.GetAllEffects())
        {
        ...
        foreach (GamePlayer rangePlayer in player.GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE))
            rangePlayer.Out.SendSpellEffectAnimation(effectListComponent.Owner, effectListComponent.Owner, 7011, 0,
        ...
        if (removed)
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language,
                    "PurgeAbility.RemoveNegativeEffects.FallFromYou"), eChatType.CT_Advise,
    ...
    protected class PurgeTimer : GameTimer
    {
        ...
        protected override void OnTick()
        {
            ...
            {
                Stop();
                if (m_caster is GamePlayer) ((GamePlayer) m_caster).DisableSkill(m_purge, 0);
            ...
            if (counter > 0)
            {
                ...
                if (player != null)
                    player.Out.SendMessage(
                        LanguageMgr.GetTranslation(player.Client.Account.Language,
                            "PurgeAbility.OnTick.PurgeActivate", counter), eChatType.CT_System,
                        eChatLoc.CL_SystemWindow);
    ...
    public override void AddEffectsInfo(IList<string> list)
    {
        list.Add(LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE,
            "PurgeAbility.AddEffectsInfo.Info1"));
        list.Add("");
        list.Add(LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE,
            "PurgeAbility.AddEffectsInfo.Info2"));
        list.Add(LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE,
            "PurgeAbility.AddEffectsInfo.Info3"));
    }
}