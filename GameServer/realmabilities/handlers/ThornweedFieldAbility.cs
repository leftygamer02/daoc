...
public class ThornweedFieldAbility : TimedRealmAbility
{
    ...
    public override void Execute(GameLiving living)
    {
        ...
        {
            caster.Out.SendMessage("You must set a ground target to use this ability!", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            return;
        }
        else if (!caster.IsWithinRadius(caster.GroundTarget, 1500))
        {
            caster.Out.SendMessage("Your ground target is too far away to use this ability!", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            return;
        }
    ...
    protected virtual int EndCast(ECSGameTimer timer)
    {
        if (m_player.IsMezzed || m_player.IsStunned || m_player.IsSitting)
            return 0;
        var twf = new Statics.ThornweedFieldBase(m_dmgValue);
