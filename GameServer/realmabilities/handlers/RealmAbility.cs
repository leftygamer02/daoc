...
public class TimedRealmAbility : RealmAbility
{
    ...
    public virtual void AddReUseDelayInfo(IList<string> list)
    {
        ...
        {
            var reUseTime = GetReUseDelay(i);
            list.Add(LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE,
                "RealmAbility.AddReUseDelayInfo.Every", i,
                reUseTime == 0 ? "always" : FormatTimespan(reUseTime)));
        }
    }
    ...
    public override void AddDelve(ref MiniDelveWriter w)
    {
        ...
        for (var i = 1; i <= MaxLevel; i++) w.AddKeyValuePair(string.Format("ReuseTimer_{0}", i), GetReUseDelay(i));
    }
    ...
    public virtual void SendCasterSpellEffectAndCastMessage(GameLiving caster, ushort spellEffect, bool success)
    {
        ...
        {
            player.Out.SendSpellEffectAnimation(caster, caster, spellEffect, 0, false,
                ...
                if (player == caster)
                    player.Out.SendMessage(
                        LanguageMgr.GetTranslation(player.Client.Account.Language,
                            "RealmAbility.SendCasterSpellEffectAndCastMessage.You", m_name), eChatType.CT_Spell,
                        eChatLoc.CL_SystemWindow);
                else
                    player.Out.SendMessage(
                        LanguageMgr.GetTranslation(player.Client.Account.Language,
                            "RealmAbility.SendCasterSpellEffectAndCastMessage.Caster", caster.Name),
        ...
        foreach (GamePlayer player in caster.GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE))
            if (caster.IsWithinRadius(player, WorldMgr.INFO_DISTANCE))
            {
                if (player == caster)
                    player.Out.SendMessage(
                        LanguageMgr.GetTranslation(player.Client.Account.Language,
                            "RealmAbility.SendCastMessage.YouCast", m_name), eChatType.CT_Spell,
                        eChatLoc.CL_SystemWindow);
                else
                    player.Out.SendMessage(
                        LanguageMgr.GetTranslation(player.Client.Account.Language,
                            "RealmAbility.SendCastMessage.PlayerCasts", player.Name), eChatType.CT_Spell,
                        eChatLoc.CL_SystemWindow);
            }
    }
    ...
    public virtual bool CheckPreconditions(GameLiving living, long bitmask)
    {
        ...
        if ((bitmask & DEAD) != 0 && !living.IsAlive)
        {
            if (player != null)
                player.Out.SendMessage(
                    LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "RealmAbility..CheckPreconditions.Dead"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
        ...
        if ((bitmask & MEZZED) != 0 && living.IsMezzed)
        {
            if (player != null)
                player.Out.SendMessage(
                    LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "RealmAbility..CheckPreconditions.Mesmerized"), eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);
        ...
        if ((bitmask & STUNNED) != 0 && living.IsStunned)
        {
            if (player != null)
                player.Out.SendMessage(
                    LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "RealmAbility..CheckPreconditions.Stunned"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
        ...
        if ((bitmask & SITTING) != 0 && living.IsSitting)
        {
            if (player != null)
                player.Out.SendMessage(
                    LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "RealmAbility..CheckPreconditions.Sitting"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
        ...
        if ((bitmask & INCOMBAT) != 0 && living.InCombat)
        {
            if (player != null)
                player.Out.SendMessage(
                    LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "RealmAbility..CheckPreconditions.Combat"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
        ...
        if ((bitmask & NOTINCOMBAT) != 0 && !living.InCombat)
        {
            if (player != null)
                player.Out.SendMessage(
                    LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "RealmAbility..CheckPreconditions.BeInCombat"), eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);
        ...
        if ((bitmask & STEALTHED) != 0 && living.IsStealthed)
        {
            if (player != null)
                player.Out.SendMessage(
                    LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "RealmAbility..CheckPreconditions.Stealthed"), eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);
        ...
        if (player != null && (bitmask & NOTINGROUP) != 0 && player.Group == null)
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language,
                    "RealmAbility..CheckPreconditions.BeInGroup"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
            return true;
        }
...
public abstract class StyleRealmAbility : TimedRealmAbility
{
    ...
    public override void Execute(GameLiving living)
    {
        if (StyleToUse != null)
        {
            ...
            var lastAD = living.TempProperties.getProperty<AttackData>("LastAttackData", null);
