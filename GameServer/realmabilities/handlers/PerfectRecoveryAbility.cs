...
public class PerfectRecoveryAbility : TimedRealmAbility
{
    ...
    protected readonly ListDictionary m_resTimersByLiving = new();
    ...
    public override void Execute(GameLiving living)
    {
        ...
        {
            player.Out.SendMessage("You have to target a dead member of your realm!", eChatType.CT_SpellResisted,
                eChatLoc.CL_SystemWindow);
            return;
        }

        var resurrectionCaster =
            targetPlayer.TempProperties.getProperty<object>(RESURRECT_CASTER_PROPERTY, null) as GameLiving;
        if (resurrectionCaster != null)
        {
            player.Out.SendMessage("Your target is already considering a resurrection!", eChatType.CT_SpellResisted,
                eChatLoc.CL_SystemWindow);
            return;
        }
        ...
        {
            player.Out.SendMessage("You are too far away from your target to use this ability!",
        ...
        if (targetPlayer != null)
        {
            ...
            var resurrectExpiredTimer = new ECSGameTimer(targetPlayer);
            ...
            targetPlayer.Out.SendCustomDialog(
                "Do you allow " + living.GetName(0, true) + " to resurrect you\n with " + m_resurrectValue +
                " percent hits/power (PR)?", new CustomDialogResponse(ResurrectResponceHandler));
        }
    }
    ...
    protected virtual void ResurrectResponceHandler(GamePlayer player, byte response)
    {
        ...
        if (resurrectExpiredTimer != null) resurrectExpiredTimer.Stop();

        var rezzer = (GameLiving) player.TempProperties.getProperty<object>(RESURRECT_CASTER_PROPERTY, null);
        if (!player.IsAlive)
        {
            if (rezzer == null)
            {
                player.Out.SendMessage("No one is currently trying to resurrect you.", eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);
            }
            else
            {
                ...
                {
                    player.Out.SendMessage("You decline to be resurrected.", eChatType.CT_System,
                        eChatLoc.CL_SystemWindow);
    ...
    {
        var player = (GamePlayer) callingTimer.Properties.getProperty<object>("targetPlayer", null);
        if (player == null) return 0;
        player.TempProperties.removeProperty(RESURRECT_CASTER_PROPERTY);
        player.Out.SendMessage("Your resurrection spell has expired.", eChatType.CT_System,
            eChatLoc.CL_SystemWindow);
        return 0;
    }
    ...
    public void ResurrectLiving(GamePlayer resurrectedPlayer, GameLiving rezzer)
    {
        ...
        resurrectedPlayer.Endurance =
            (int) (resurrectedPlayer.MaxEndurance * m_resurrectValue / 100); //no endurance after any rez
        resurrectedPlayer.MoveTo(rezzer.CurrentRegionID, rezzer.X, rezzer.Y, rezzer.Z, rezzer.Heading);

        var living = resurrectedPlayer as GameLiving;
        GameTimer resurrectExpiredTimer = null;
        lock (m_resTimersByLiving.SyncRoot)
        {
        ...
        var effect = SpellHandler.FindEffectOnTarget(resurrectedPlayer, "PveResurrectionIllness");
        if (effect != null)
            effect.Cancel(false);
        var effecttwo = SpellHandler.FindEffectOnTarget(resurrectedPlayer, "RvrResurrectionIllness");
        if (effecttwo != null)
            effecttwo.Cancel(false);
        resurrectedPlayer.Out.SendMessage("You have been resurrected by " + rezzer.GetName(0, false) + "!",
            eChatType.CT_System, eChatLoc.CL_SystemWindow);
        //Lifeflight: this should make it so players who have been ressurected don't take damage for 5 seconds
        var rezImmune = new RezDmgImmunityEffect();
        rezImmune.Start(resurrectedPlayer);

        //Lifeflight: We need to reward rez RPs
        var casterPlayer = rezzer as GamePlayer;
        if (casterPlayer != null)
        {
            var rezRps = resurrectedPlayer.LastDeathRealmPoints * (m_resurrectValue + 50) / 1000;
            if (rezRps > 0)
            {
            ...
            {
                casterPlayer.Out.SendMessage("The player you resurrected was not worth realm points on death.",
                    eChatType.CT_Important, eChatLoc.CL_SystemWindow);
                casterPlayer.Out.SendMessage("You thus get no realm points for the resurrect.",
                    eChatType.CT_Important, eChatLoc.CL_SystemWindow);
            }
        }
    }
    ...
    protected virtual int GetResurrectValue()
    {
