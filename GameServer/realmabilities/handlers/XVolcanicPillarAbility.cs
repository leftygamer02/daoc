...
public class XVolcanicPillarAbility : TimedRealmAbility
{
    ...
    public override void Execute(GameLiving living)
    {
        ...
        {
            m_caster.Out.SendMessage("You need a target for this ability!", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            m_caster.DisableSkill(this, 3 * 1000);
            return;
        }

        if (!m_caster.IsWithinRadius(m_caster.TargetObject,
                (int) (1500 * m_caster.GetModified(eProperty.SpellRange) * 0.01)))
        {
            m_caster.Out.SendMessage(m_caster.TargetObject + " is too far away.", eChatType.CT_Spell,
                eChatLoc.CL_SystemWindow);
            return;
        }
        ...
        foreach (GamePlayer i_player in m_caster.GetPlayersInRadius(WorldMgr.INFO_DISTANCE))
        {
            if (i_player == m_caster)
                ...
                i_player.MessageFromArea(m_caster, m_caster.Name + " casts a spell!", eChatType.CT_Spell,
                    eChatLoc.CL_SystemWindow);
        ...
        if (m_caster.RealmAbilityCastTimer != null)
        {
            ...
            m_caster.Out.SendMessage("You cancel your Spell!", eChatType.CT_SpellResisted,
                eChatLoc.CL_SystemWindow);
        }
    ...
    protected virtual int EndCast(ECSGameTimer timer)
    {
        ...
        foreach (GameNPC mob in m_caster.TargetObject.GetNPCsInRadius(500))
        {
            ...
            m_caster.Out.SendMessage("You hit the " + mob.Name + " for " + m_dmgValue + " damage.",
        ...
        foreach (GamePlayer aeplayer in m_caster.TargetObject.GetPlayersInRadius(500))
        {
            ...
            m_caster.Out.SendMessage("You hit " + aeplayer.Name + " for " + m_dmgValue + " damage.",
                eChatType.CT_YouHit, eChatLoc.CL_SystemWindow);
            aeplayer.Out.SendMessage(m_caster.Name + " hits you for " + m_dmgValue + " damage.",
