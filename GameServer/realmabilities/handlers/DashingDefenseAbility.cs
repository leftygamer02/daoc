...
public class DashingDefenseAbility : TimedRealmAbility
{
    ...
    public override void Execute(GameLiving living)
    {
        ...
        if (player.TempProperties.getProperty(Dashing, false))
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language,
                    "DashingDefenseAbility.Execute.AlreadyEffect"), eChatType.CT_SpellResisted,
        ...
        if (player.Group == null)
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language,
                    "DashingDefenseAbility.Execute.MustInGroup"), eChatType.CT_SpellResisted,
                eChatLoc.CL_SystemWindow);
            return;
        }
        else
        {
            foreach (var grpMate in player.Group.GetPlayersInTheGroup())
    ...
    public override void AddEffectsInfo(IList<string> list)
    {
        ...
        if (ServerProperties.Properties.USE_NEW_ACTIVES_RAS_SCALING)
        {
            ...
            list.Add("");
            list.Add(LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE,
                "DashingDefenseAbility.AddEffectsInfo.Info2"));
            list.Add(LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE,
                "DashingDefenseAbility.AddEffectsInfo.Info3"));
        }
        else
        {
            list.Add(LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE,
