...
public class IchorOfTheDeepAbility : TimedRealmAbility
{
    ...
    public override void Execute(GameLiving living)
    {
        ...
        {
            caster.Out.SendMessage("You must select a target for this ability!", eChatType.CT_System,
        ...
        {
            caster.Out.SendMessage("You have an invalid target!", eChatType.CT_SpellResisted,
                eChatLoc.CL_SystemWindow);
            caster.DisableSkill(this, 3 * 1000);
            return;
        }
        ...
        if (caster == target)
        {
            caster.Out.SendMessage("You can't attack yourself!", eChatType.CT_SpellResisted,
                eChatLoc.CL_SystemWindow);
            caster.DisableSkill(this, 3 * 1000);
            return;
        }
        ...
        {
            caster.Out.SendMessage(target.Name + " is not in view!", eChatType.CT_SpellResisted,
                eChatLoc.CL_SystemWindow);
            caster.DisableSkill(this, 3 * 1000);
            return;
        }
        ...
        {
            caster.Out.SendMessage(caster.TargetObject.Name + " is too far away!", eChatType.CT_SpellResisted,
                eChatLoc.CL_SystemWindow);
            caster.DisableSkill(this, 3 * 1000);
            return;
        }
        ...
        {
            caster.Out.SendMessage("You can't attack a member of your realm!", eChatType.CT_SpellResisted,
                eChatLoc.CL_SystemWindow);
            caster.DisableSkill(this, 3 * 1000);
            return;
        }
        ...
        if (m_expireTimerID != null && m_expireTimerID.IsAlive)
        {
            caster.Out.SendMessage(
                "You must wait" + m_expireTimerID.TimeUntilElapsed / 1000 +
                " seconds to recast this type of ability!", eChatType.CT_Spell, eChatLoc.CL_SystemWindow);
            caster.DisableSkill(this, 3 * 1000);
            return;
        }
    ...
    {
        var living = caster.TargetObject as GameLiving;
        ...
        foreach (GamePlayer i_player in caster.GetPlayersInRadius(WorldMgr.INFO_DISTANCE))
            if (i_player == caster)
                ...
                i_player.Out.SendMessage(caster.Name + " casts a spell!", eChatType.CT_Spell,
                    eChatLoc.CL_SystemWindow);
        ...
        foreach (GameNPC mob in living.GetNPCsInRadius(500)) IchorEffect(living, mob);

        // Do everything for GamePlayer now
        foreach (GamePlayer aeplayer in living.GetPlayersInRadius(500)) IchorEffect(living, aeplayer);
    ...
    private int CalculateDamageWithFalloff(int initialDamage, GameLiving initTarget, GameLiving aetarget)
    {
        ...
        var modDamage = (int) Math.Round((decimal) (initialDamage *
                                                    ((500 - initTarget.GetDistance(new Point2D(aetarget.X,
                                                        aetarget.Y))) / 500.0)));
    ...
    {
        var living = timer.TimerOwner as GameLiving;
        if (living != null)
        {
    ...
    protected void IchorEffect(GameLiving centerTarget, GameLiving aoeTarget)
    {
        ...
        rootmodifier +=
            (int) ((duration + (double) primaryResistModifier) * (double) secondaryResistModifier * -0.01);
        ...
        var mez = EffectListService.GetEffectOnTarget(target, eEffect.Mez);
        if (mez != null)
            EffectService.RequestCancelEffect(mez);
        //mez.Cancel(false);

        // Falloff damage
        var dmgWithFalloff = CalculateDamageWithFalloff(dmgValue, living, target);
        ...
        caster.Out.SendMessage("You hit " + target.GetName(0, false) + " for " + dmgWithFalloff + " damage!",
        ...
        if (dmgWithFalloff > 0 && target is GamePlayer gpTarget)
            gpTarget.Out.SendMessage(caster.Name + " hits you for " + dmgWithFalloff + " damage!",
