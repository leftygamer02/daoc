...
public class BarrierOfFortitudeAbility : TimedRealmAbility
{
    ...
    private int m_duration = 30000; // 30s
    ...
    public override void Execute(GameLiving living)
    {
        ...
        if (player.Group != null)
            playersInGroup = player.Group.GetPlayersInTheGroup()
                .Where(x => x.IsAlive && player.IsWithinRadius(player, m_range));
        else
            playersInGroup = new List<GamePlayer>() {player};

        foreach (var playerInGroup in playersInGroup)
        {
            ...
            if (success)
                new BunkerOfFaithECSEffect(new ECSGameEffectInitParams(playerInGroup, m_duration, GetAbsorbAmount(),
                    CreateSpell(player)));
        }
