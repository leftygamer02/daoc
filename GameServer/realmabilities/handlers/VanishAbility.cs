...
public class VanishAbility : TimedRealmAbility
{
    ...
    public override void Execute(GameLiving living)
    {
        ...
        if (player != null)
        {
            var vanish = new VanishEffect(duration, speedBonus);
            vanish.Start(player);

            foreach (var effect in living.EffectList.GetAllOfType<GameSpellEffect>())
        ...
        foreach (GameLiving attacker in attackers)
            if (attacker.TargetObject == living)
            {
                ...
                if (attacker is GameNPC)
                {
                    ...
                    if (npc.Brain is IOldAggressiveBrain) ((IOldAggressiveBrain) npc.Brain).RemoveFromAggroList(living);
