...
public class ECSGameSpellEffect : ECSGameEffect, IConcentrationEffect
{
        ...
        if (TriggersImmunity)
        {
            if (OwnerPlayer != null)
            {
                if ((EffectType == eEffect.Stun && SpellHandler.Caster is GamePet) ||
                    SpellHandler is UnresistableStunSpellHandler)
                    return;
            ...
            else if (Owner is GameNPC)
            {
                ...
                {
                    var npcImmune =
                        (NPCECSStunImmunityEffect) EffectListService.GetEffectOnTarget(Owner,
                            eEffect.NPCStunImmunity);
                    if (npcImmune is null)
                        new NPCECSStunImmunityEffect(new ECSGameEffectInitParams(Owner, ImmunityDuration,
                            Effectiveness));
                }
                else if (EffectType == eEffect.Mez)
                {
                    var npcImmune =
                        (NPCECSMezImmunityEffect) EffectListService.GetEffectOnTarget(Owner,
                            eEffect.NPCMezImmunity);
                    if (npcImmune is null)
                        new NPCECSMezImmunityEffect(new ECSGameEffectInitParams(Owner, ImmunityDuration,
                            Effectiveness));
                }
            }
        }
    ...
    public void OnEffectStartsMsg(GameLiving target, bool msgTarget, bool msgSelf, bool msgArea)
    {
        ...
        if (msgTarget && target is GamePlayer playerTarget)
            ...
            ((SpellHandler) SpellHandler).MessageToLiving(playerTarget, SpellHandler.Spell.Message1,
                eChatType.CT_Spell);
        ...
        if (msgArea)
        {
            if (Caster is GamePlayer caster && caster == target)
                // "{0} looks more agile!"
                Message.SystemToArea(target,
                    Util.MakeSentence(SpellHandler.Spell.Message2, target.GetName(0, upperCase)),
                    eChatType.CT_Spell, target, Caster);
        ...
        // Sends a third-person message directly to the caster to indicate the spell has ended
            ...
            // "{0} looks more agile!"
        {
            ((SpellHandler) SpellHandler).MessageToCaster(
                Util.MakeSentence(SpellHandler.Spell.Message2, target.GetName(0, true)), eChatType.CT_Spell);
        }
    }
    ...
    public void OnEffectExpiresMsg(GameLiving target, bool msgTarget, bool msgSelf, bool msgArea)
    {
        ...
        if (msgTarget && target is GamePlayer playerTarget)
            ...
            ((SpellHandler) SpellHandler).MessageToLiving(playerTarget, SpellHandler.Spell.Message3,
                eChatType.CT_Spell);
        ...
        if (msgSelf && Caster is GamePlayer selfCaster)
            ...
            ((SpellHandler) SpellHandler).MessageToCaster(
                Util.MakeSentence(SpellHandler.Spell.Message4, target.GetName(0, true)), eChatType.CT_Spell);
        ...
        if (msgArea)
        {
            if (Caster is GamePlayer areaTarget && areaTarget == target)
                // "{0}'s enhanced agility fades."
                Message.SystemToArea(target,
                    Util.MakeSentence(SpellHandler.Spell.Message4, target.GetName(0, upperCase)),
                    eChatType.CT_Spell, target, Caster);
            else if (Caster is GamePet || target is GamePet or GamePlayer)
                // "{0}'s enhanced agility fades."
                Message.SystemToArea(target,
