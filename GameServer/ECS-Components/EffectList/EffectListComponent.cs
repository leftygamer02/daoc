...
public class EffectListComponent
{
    ...
    public Dictionary<eEffect, List<ECSGameEffect>> Effects { get; private set; } = new();

    public object EffectsLock { get; private set; } = new();
    public List<ECSGameSpellEffect> ConcentrationEffects { get; private set; } = new(20);
    ...
    public bool AddEffect(ECSGameEffect effect)
    {
        lock (EffectsLock)
        {
            try
            {
                ...
                if (effect is ECSGameSpellEffect newSpellEffect && Effects.TryGetValue(effect.EffectType,
                        out var existingGameEffects))
                {
                    var newSpellHandler = newSpellEffect.SpellHandler;
                    var newSpell = newSpellHandler.Spell;
                    var existingEffects =
                        existingGameEffects.Cast<ECSGameSpellEffect>().ToList();

                    // Effects contains this effect already so refresh it
                    if (existingEffects.FirstOrDefault(e =>
                            e.SpellHandler.Spell.ID == newSpell.ID || (newSpell.EffectGroup > 0 &&
                                                                       e.SpellHandler.Spell.EffectGroup ==
                                                                       newSpell.EffectGroup &&
                                                                       newSpell.IsPoisonEffect)) != null)
                    {
                        if (newSpellEffect.IsConcentrationEffect() && !newSpellEffect.RenewEffect)
                            return false;
                        for (var i = 0; i < existingEffects.Count; i++)
                        {
                            var existingEffect = existingEffects[i];
                            var existingSpellHandler = existingEffect.SpellHandler;
                            var existingSpell = existingSpellHandler.Spell;
                            ...
                            if ((existingSpell.IsPulsing && newSpellHandler.Caster.LastPulseCast == newSpell &&
                                ...
                                || existingSpell.ID == newSpell.ID)
                            {
                                ...
                                else
                                {
                                    ...
                                    if (existingEffect.Effectiveness != newSpellEffect.Effectiveness
                                        || existingSpell.SpellType is (byte) eSpellType.SpeedDecrease
                                            or (byte) eSpellType.UnbreakableSpeedDecrease)
                    ...
                    else if (effect.EffectType is eEffect.SavageBuff or eEffect.ArmorAbsorptionBuff)
                    {
                        ...
                        else if (!existingEffects.Where(e =>
                                         e.SpellHandler.Spell.SpellType ==
                                         newSpellEffect.SpellHandler.Spell.SpellType)
                                     .Any())
                        {
                            Effects[newSpellEffect.EffectType].Add(newSpellEffect);
                            EffectIdToEffect.TryAdd(newSpellEffect.Icon, newSpellEffect);
                    ...
                    else
                    {
                        ...
                        var foundIsOverwriteableEffect = false;
                        ...
                        for (var i = 0; i < existingEffects.Count; i++)
                        {
                            ...
                            if (existingSpellHandler.IsOverwritable(newSpellEffect) ||
                                newSpellEffect.EffectType == eEffect.MovementSpeedDebuff)
                            {
                                ...
                                if (effect.EffectType != eEffect.Bladeturn)
                                {
                                    ...
                                    if (newSpell.Value > existingSpell.Value ||
                                        newSpell.Damage > existingSpell.Damage)
                                    {
                                        if (newSpell.IsHelpful &&
                                            (newSpellHandler.Caster != existingSpellHandler.Caster
                                             || newSpellHandler.SpellLine.KeyName ==
                                             GlobalSpellsLines.Potions_Effects
                                             || existingSpellHandler.SpellLine.KeyName ==
                                             GlobalSpellsLines.Potions_Effects))
    ...
    public ECSGameSpellEffect GetBestDisabledSpellEffect(eEffect effectType = eEffect.Unknown)
    {
        lock (EffectsLock)
        {
            return GetSpellEffects(effectType)?.OrderByDescending(e => e.IsDisabled)
                .ThenByDescending(e => e.SpellHandler.Spell.Value).FirstOrDefault();
        }
    }
        ...
        lock (EffectsLock)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        var existingEffects = Effects[effect.EffectType];
                        ...
                        var effectToRemove = existingEffects.FirstOrDefault(e => e.Name == effect.Name);
