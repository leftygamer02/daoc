...
public class RangeAttackComponent
{
    ...
    public const int
        PROJECTILE_FLIGHT_SPEED =
            1800; // 1800 units per second. Live value is unknown, but DoL had 1500. Also affects throwing weapons.
    ...
    private InventoryItem GetAmmoFromInventory(eObjectType ammoType)
    {
        switch (ActiveQuiverSlot)
        {
            ...
            case eActiveQuiverSlot.None:
                return m_owner.Inventory.GetFirstItemByObjectType((int) ammoType, eInventorySlot.FirstBackpack,
    ...
    public GameObject Target
    {
        get
        {
            ...
            {
                var target = (GameObject) m_rangeAttackTarget.Target;
    ...
    public eCheckRangeAttackStateResult CheckRangeAttackState(GameObject target)
    {
        ...
        {
            var attackStart = m_owner.TempProperties.getProperty<long>(RANGED_ATTACK_START);
            ...
            if (GameLoop.GameLoopTime - attackStart > 15000 &&
                playerOwner.ActiveWeapon.Object_Type != (int) eObjectType.Crossbow)
            {
                playerOwner.Out.SendMessage(
                    LanguageMgr.GetTranslation(playerOwner.Client.Account.Language, "GamePlayer.Attack.TooTired"),
                    eChatType.CT_System, eChatLoc.CL_SystemWindow);
                return eCheckRangeAttackStateResult.Stop;
            }

            // This state is set when the player wants to fire.
            if (RangedAttackState is eRangedAttackState.Fire or eRangedAttackState.AimFire
                or eRangedAttackState.AimFireReload)
            {
                ...
                if (target is null or not GameLiving)
                {
                    ...
                    var volley = EffectListService.GetEffectOnTarget(playerOwner, eEffect.Volley);

                    if (volley == null)
                        playerOwner.Out.SendMessage(
                            LanguageMgr.GetTranslation(playerOwner.Client.Account.Language,
                                "System.MustSelectTarget"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                }
                else if (!playerOwner.IsWithinRadius(target, playerOwner.attackComponent.AttackRange))
                {
                    playerOwner.Out.SendMessage(
                        LanguageMgr.GetTranslation(playerOwner.Client.Account.Language,
                            "GamePlayer.Attack.TooFarAway", target.GetName(0, true)), eChatType.CT_System,
                        eChatLoc.CL_SystemWindow);
                }
                else if
                    (!playerOwner
                        .TargetInView) // TODO: Wrong, must be checked with the target parameter and not with the targetObject.
                {
                    playerOwner.Out.SendMessage(
                        LanguageMgr.GetTranslation(playerOwner.Client.Account.Language,
                            "GamePlayer.Attack.CantSeeTarget"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                }
                else if (!playerOwner.IsObjectInFront(target, 90))
                {
                    playerOwner.Out.SendMessage(
                        LanguageMgr.GetTranslation(playerOwner.Client.Account.Language,
                            "GamePlayer.Attack.NotInView", target.GetName(0, true)), eChatType.CT_System,
                        eChatLoc.CL_SystemWindow);
                }
                    ...
                    // Another check for ammo just before firing.
                {
                    playerOwner.Out.SendMessage(
                        LanguageMgr.GetTranslation(playerOwner.Client.Account.Language,
                            "GamePlayer.Attack.MustSelectQuiver"), eChatType.CT_YouHit, eChatLoc.CL_SystemWindow);
                }
                else if (!m_isAmmoCompatible)
                {
                    playerOwner.Out.SendMessage(
                        LanguageMgr.GetTranslation(playerOwner.Client.Account.Language,
                            "GamePlayer.Attack.CantUseQuiver"), eChatType.CT_YouHit, eChatLoc.CL_SystemWindow);
                }
                else if (GameServer.ServerRules.IsAllowedToAttack(playerOwner, (GameLiving) target, false))
                {
                         ...
                         EffectListService.GetEffectOnTarget(living, eEffect.Mez) != null))
                    {
                        ...
                        playerOwner.Out.SendMessage(
                            LanguageMgr.GetTranslation(playerOwner.Client.Account.Language,
                                "GamePlayer.Attack.CantCritical"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
            ...
            {
                var
                    volley = EffectListService.GetEffectOnTarget(playerOwner,
                        eEffect.Volley); //volley check to avoid spam
                if (volley == null)
                {
                    playerOwner.Out.SendMessage(
                        LanguageMgr.GetTranslation(playerOwner.Client.Account.Language,
                            "GamePlayer.Attack.ReadyToFire"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                    RangedAttackState = eRangedAttackState.ReadyToFire;
                    return eCheckRangeAttackStateResult.Hold;
                }
            }
    ...
    {
        var arrowRecoveryChance = m_owner.GetModified(eProperty.ArrowRecovery);
        ...
        else if (RangedAttackType == eRangedAttackType.RapidFire &&
                 m_owner.GetAbilityLevel(Abilities.RapidFire) == 2)
