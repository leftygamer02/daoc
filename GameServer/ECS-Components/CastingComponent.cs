...
public class CastingComponent
{
    ...
    public bool StartCastSpell(Spell spell, SpellLine line,
        ISpellCastingAbilityHandler spellCastingAbilityHandler = null)
    {
        ...
        {
            if (!CanCastSpell(p)) return false;
        ...
        var m_newSpellHandler = ScriptMgr.CreateSpellHandler(owner, spell, line);
        ...
        if (spellHandler != null)
        {
            ...
            else
            {
                ...
                {
                    if (spell.CastTime > 0 && !(spellHandler is ChamberSpellHandler) &&
                        spell.SpellType != (byte) eSpellType.Chamber)
                        if (spellHandler.Spell.InstrumentRequirement != 0)
                        {
                            if (spell.InstrumentRequirement != 0)
                                pl.Out.SendMessage(
                                    LanguageMgr.GetTranslation(pl.Client.Account.Language,
                                        "GamePlayer.CastSpell.AlreadyPlaySong"), eChatType.CT_SpellResisted,
                                    eChatLoc.CL_SystemWindow);
                            else
                                pl.Out.SendMessage(
                                    "You must wait " +
                                    ((spellHandler.CastStartTick + spellHandler.Spell.CastTime -
                                      GameLoop.GameLoopTime) / 1000 + 1).ToString() + " seconds to cast a spell!",
                    ...
                    {
                        pl.Out.SendMessage(
                            "You are already casting a spell! You prepare this spell as a follow up!",
                            eChatType.CT_SpellResisted, eChatLoc.CL_SystemWindow);
                        queuedSpellHandler = m_newSpellHandler;
                    }
                    else
                    {
                        pl.Out.SendMessage("You are already casting a spell!", eChatType.CT_SpellResisted,
                            eChatLoc.CL_SystemWindow);
                    }
                }
    ...
    private bool CanCastSpell(GameLiving living)
    {
        ...
        if (p.effectListComponent
            .ContainsEffectForEffectType(eEffect
                .Volley)) //Volley check, players can't cast spells under volley effect
        {
            p.Out.SendMessage("You can't cast spells while Volley is active!", eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
            return false;
        }
        ...
        if (living != null)
        {
            ...
            {
                p?.Out.SendMessage(
                    LanguageMgr.GetTranslation(p.Client.Account.Language, "GamePlayer.CastSpell.CantCastStunned"),
                    eChatType.CT_SpellResisted, eChatLoc.CL_SystemWindow);
                return false;
            }
            ...
            {
                p?.Out.SendMessage(
                    LanguageMgr.GetTranslation(p.Client.Account.Language, "GamePlayer.CastSpell.CantCastMezzed"),
                    eChatType.CT_SpellResisted, eChatLoc.CL_SystemWindow);
                return false;
            }
    ...
    private static void TickThenReplaceSpellHandler(ref ISpellHandler oldSpellHandler,
        ISpellHandler newSpellHandler)
    {
