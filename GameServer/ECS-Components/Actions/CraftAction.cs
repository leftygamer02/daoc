...
public class CraftAction
{
    ...
    public long EndTime => m_startTick + CraftTime;
    ...
    protected virtual void MakeItem()
    {
        ...
        {
            if (player != null)
                player.Out.SendMessage("Could not find recipe or item to craft!", eChatType.CT_Important,
                    eChatLoc.CL_SystemWindow);
            log.Error(
                "Crafting.MakeItem: Could not retrieve player, recipe, or raw materials to craft from CraftTimer.");
            return;
        }

        if (queue > 1 && remainingToCraft == 0) remainingToCraft = queue;
            ...
            if (!skill.RemoveUsedMaterials(player, recipe))
            {
                player.Out.SendMessage(
                    LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "AbstractCraftingSkill.MakeItem.NotAllMaterials"), eChatType.CT_System,
                    eChatLoc.CL_SystemWindow);
        ...
        else
        {
            player.Out.SendMessage(
                LanguageMgr.GetTranslation(player.Client.Account.Language,
                    "AbstractCraftingSkill.MakeItem.LoseNoMaterials", recipe.Product.Name), eChatType.CT_System,
                eChatLoc.CL_SystemWindow);
        ...
        if (remainingToCraft > 1)
        {
            if (skill.CheckRawMaterials(player, recipe))
            {
                ...
                m_startTick = GameLoop.GameLoopTime + 1;
                player.Out.SendTimerWindow(
                    LanguageMgr.GetTranslation(player.Client.Account.Language,
                        "AbstractCraftingSkill.CraftItem.CurrentlyMaking", recipe.Product.Name),
                    skill.GetCraftingTime(player, recipe));
                finishedCraft = false;
