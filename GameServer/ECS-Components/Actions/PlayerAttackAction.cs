...
public class PlayerAttackAction : AttackAction
{
    ...
    public override bool CheckInterruptTimer()
    {
        ...
        var attacker = _playerOwner.attackComponent.Attackers.Last();
    ...
    public override void OnAimInterrupt(GameObject attacker)
    {
        ...
        if (_playerOwner.ActiveWeapon != null && _playerOwner.ActiveWeapon.Object_Type == (int) eObjectType.Thrown)
            attackTypeMsg = LanguageMgr.GetTranslation(_playerOwner.Client.Account.Language,
                "GamePlayer.Attack.Type.Throw");
        else
            attackTypeMsg =
                LanguageMgr.GetTranslation(_playerOwner.Client.Account.Language, "GamePlayer.Attack.Type.Shot");

        if (attacker is GameNPC npcAttacker)
            _playerOwner.Out.SendMessage(
                LanguageMgr.GetTranslation(_playerOwner.Client.Account.Language, "GamePlayer.Attack.Interrupted",
                    attacker.GetName(0, true, _playerOwner.Client.Account.Language, npcAttacker), attackTypeMsg),
                eChatType.CT_YouHit, eChatLoc.CL_SystemWindow);
        else
            _playerOwner.Out.SendMessage(
                LanguageMgr.GetTranslation(_playerOwner.Client.Account.Language, "GamePlayer.Attack.Interrupted",
                    attacker.GetName(0, true), attackTypeMsg), eChatType.CT_YouHit, eChatLoc.CL_SystemWindow);
    }
    ...
    protected override bool FinalizeMeleeAttack()
    {
        if (base.FinalizeMeleeAttack())
        {
            if (_playerOwner.UseDetailedCombatLog)
                _playerOwner.Out.SendMessage($"Attack Speed: {_interval / 1000.0}s", eChatType.CT_DamageAdd,
    ...
    protected override bool FinalizeRangedAttack()
    {
        ...
        {
            _playerOwner.TempProperties.setProperty(RangeAttackComponent.RANGED_ATTACK_START,
