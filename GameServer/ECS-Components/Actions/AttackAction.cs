...
public abstract class AttackAction
{
    ...
    protected virtual bool PrepareMeleeAttack()
    {
        ...
        var mainHandAttackSpeed = _attackComponent.AttackSpeed(_weapon);
        ...
        var proc = _combatStyle?.Procs?.FirstOrDefault()?.Item1;

        if (proc != null)
            _rangeBonus = proc.SpellType == (byte) eSpellType.StyleRange
                ? (int) proc.Value - _attackComponent.AttackRange
                : 0;
    ...
    {
        var rangeCheckresult = _owner.rangeAttackComponent.CheckRangeAttackState(_target);
        ...
        var model = _weapon == null ? 0 : _weapon.Model;
        var flightDuration = (byte) (_ticksToTarget > 350 ? 1 + (_ticksToTarget - 350) / 75 : 1);
        var cancelPrepareAnimation = _owner.ActiveWeapon.Object_Type == (int) eObjectType.Thrown;
        ...
        Parallel.ForEach(_owner.GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE).OfType<GamePlayer>(), player =>
        {
            ...
            player.Out.SendCombatAnimation(_owner, _target, (ushort) model, 0x00, player.Out.BowShoot,
                flightDuration, 0x00, ((GameLiving) _target).HealthPercent);
        });
        ...
        switch (_owner.rangeAttackComponent.RangedAttackType)
        {
            ...
            {
                var tmpEffectiveness = 2 - 0.3 * _owner.GetConLevel(_target);
            ...
            case eRangedAttackType.RapidFire:
            {
                ...
                var elapsedTime = GameLoop.GameLoopTime -
                                  _owner.TempProperties.getProperty<long>(RangeAttackComponent
                                      .RANGED_ATTACK_START); // elapsed time before ready to fire
        ...
        {
            var PALevel = _owner.GetAbilityLevel(Abilities.PenetratingArrow);

            if (PALevel > 0 && _owner.rangeAttackComponent.RangedAttackType != eRangedAttackType.Long)
            {
                var bladeturn = livingTarget.effectListComponent.GetSpellEffects(eEffect.Bladeturn)
                    ?.FirstOrDefault();
    ...
    {
        _attackComponent.weaponAction = new WeaponAction(_owner, _target, _weapon, _leftWeapon, _effectiveness,
            _interruptDuration, _combatStyle);
    ...
    {
        _attackComponent.weaponAction =
            new WeaponAction(_owner, _target, _weapon, null, _effectiveness, _interruptDuration, null);
        ...
        if (_ticksToTarget > 0)
        {
            new ECSGameTimer(_owner, new ECSGameTimer.ECSTimerCallback(_attackComponent.weaponAction.Execute),
                _ticksToTarget);
    ...
    protected virtual bool FinalizeRangedAttack()
    {
        ...
        if (!_owner.effectListComponent.ContainsEffectForEffectType(eEffect.Volley))
            Parallel.ForEach(_owner.GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE).OfType<GamePlayer>(), player =>
            {
                ...
                player.Out.SendCombatAnimation(_owner, null, (ushort) (_weapon != null ? _weapon.Model : 0), 0x00,
                    player.Out.BowPrepare, 0x1A, 0x00, 0x00);
            });
