...
public class WeaponAction
{
    ...
    public WeaponAction(GameLiving owner, GameObject target, InventoryItem attackWeapon, InventoryItem leftWeapon,
        double effectiveness, int interruptDuration, Style combatStyle)
    {
        m_owner = owner;
        m_target = target;
    ...
    public void Execute()
    {
        ...
        var style = m_combatStyle;
        var leftHandSwingCount = 0;
        AttackData mainHandAD = null;
        AttackData leftHandAD = null;
        var mainWeapon = m_attackWeapon;
        var leftWeapon = m_leftWeapon;
        var leftHandEffectiveness = m_effectiveness;
        var mainHandEffectiveness = m_effectiveness;
        ...
        if (leftHandSwingCount > 0)
        {
            ...
            mainHandAD = m_owner.attackComponent.MakeAttack(m_target, mainWeapon, style, mainHandEffectiveness,
                m_interruptDuration, usingOH);
        ...
        var targetHasReflexAttackRA = false;

        if (mainHandAD.Target is GamePlayer targetPlayer && targetPlayer.effectListComponent != null &&
            targetPlayer.effectListComponent.ContainsEffectForEffectType(eEffect.ReflexAttack))
        ...
        if (mainHandAD.Attacker.IsStealthed
            ...
            && (mainHandAD.AttackResult == eAttackResult.HitUnstyled ||
                mainHandAD.AttackResult == eAttackResult.HitStyle))
            if (mainHandAD.Target.TargetObject == null)
                if (mainHandAD.Target is GamePlayer)
                {
                    var targetClient =
                        WorldMgr.GetClientByPlayerID(mainHandAD.Target.InternalID, false, false);
        ...
        if (leftHandSwingCount > 0 && mainWeapon.SlotPosition != Slot.RANGED)
            switch (mainHandAD.AttackResult)
            {
                ...
                case eAttackResult.Fumbled
                    : // Takii - Fumble should not prevent Offhand attack. https://www.atlasfreeshard.com/tickets/fumble-mainhand-lets-offhand-not-swing.1012/
                case eAttackResult.Parried:
                    for (var i = 0; i < leftHandSwingCount; i++)
                    {
                        if (m_target is GameLiving living &&
                            (living.IsAlive == false || living.ObjectState != eObjectState.Active))
                            break;

                        // Savage swings - main, left, main, left.
                        if (i % 2 == 0)
                            leftHandAD = m_owner.attackComponent.MakeAttack(m_target, leftWeapon, null,
                                leftHandEffectiveness, m_interruptDuration, usingOH);
                        else
                            leftHandAD = m_owner.attackComponent.MakeAttack(m_target, mainWeapon, null,
                                leftHandEffectiveness, m_interruptDuration, usingOH);
                        ...
                        if (targetHasReflexAttackRA)
                            HandleReflexAttack(m_owner, leftHandAD.Target, leftHandAD.AttackResult,
                                m_interruptDuration);
                    }
        ...
        // Show the animation.
        if (mainHandAD.AttackResult != eAttackResult.HitUnstyled &&
            mainHandAD.AttackResult != eAttackResult.HitStyle && leftHandAD != null)
        ...
        if (mainHandAD.StyleEffects.Count > 0 && mainHandAD.AttackResult == eAttackResult.HitStyle)
            foreach (var proc in mainHandAD.StyleEffects)
                proc.StartSpell(mainHandAD.Target);

        if (leftHandAD != null && leftHandAD.StyleEffects.Count > 0 &&
            leftHandAD.AttackResult == eAttackResult.HitStyle)
            foreach (var proc in leftHandAD.StyleEffects)
    ...
    {
        var dmgAddEffects = owner.effectListComponent.GetSpellEffects(eEffect.DamageAdd);
        ...
        if (dmgAddEffects != null)
        {
            ...
            foreach (var effect in dmgAddEffects)
                if (effect.SpellHandler.Spell.EffectGroup == 99999)
                {
                    dmgAddsUnaffectedByStacking.Add(effect);
                    ((DamageAddSpellHandler) effect.SpellHandler).EventHandler(null, owner,
                        new AttackFinishedEventArgs(ad), 1);
                }
            ...
            var numRegularDmgAddsApplied = 0;

            foreach (var effect in dmgAddEffects.Except(dmgAddsUnaffectedByStacking)
                         .OrderByDescending(e => e.SpellHandler.Spell.Damage))
            {
                var effectiveness =
                    1 + effect.SpellHandler.Caster.GetModified(eProperty.BuffEffectiveness) * 0.01;
                if (effect.IsBuffActive)
                {
                    ((DamageAddSpellHandler) effect.SpellHandler).EventHandler(null, owner,
                        new AttackFinishedEventArgs(ad),
                        numRegularDmgAddsApplied > 0 ? effectiveness * 0.5 : effectiveness);
        ...
        int interruptDuration)
    {
        ...
        switch (attackResult)
        {
            ...
            case eAttackResult.Evaded:
            case eAttackResult.Parried:
                var ReflexAttackAD = target.attackComponent.LivingMakeAttack(attacker, target.ActiveWeapon,
                    null, 1, interruptDuration, false, true);
                ...
                if (ReflexAttackAD.AttackResult == eAttackResult.HitUnstyled)
                {
                    ...
                    playerAttacker?.Out.SendMessage(
                        target.Name + " counter-attacks you for " + ReflexAttackAD.Damage + " damage.",
