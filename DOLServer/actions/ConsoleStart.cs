...
public class ConsoleStart : IAction
{
    ...
    private static bool StartServer()
    {
        ...
        var start = GameServer.Instance.Start();
        return start;
    }
    ...
    public void OnAction(Hashtable parameters)
    {
        ...
        else
        {
            ...
            configFile = new FileInfo(currentAssembly.DirectoryName + Path.DirectorySeparatorChar + "config" +
                                      Path.DirectorySeparatorChar + "serverconfig.xml");
        }
        ...
        else
        {
            ...
            if (File.Exists(currentAssembly.DirectoryName + Path.DirectorySeparatorChar + "DOLConfig.exe"))
                Console.WriteLine(
                    "No config file found, launching with default config and embedded database... (SQLite)");
        }
            ...
            if (line != null)
                switch (line.ToLower())
                {
                    ...
                    default:
                        ...
                        try
                        {
                            var res = ScriptMgr.HandleCommandNoPlvl(client, line);
                            if (!res) Console.WriteLine("Unknown command: " + line);
                        }
                        catch (Exception e)
                        {
