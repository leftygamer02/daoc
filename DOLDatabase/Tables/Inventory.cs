...
public class InventoryItem : DataObject
{
    ...
    public virtual int CanUseAgainIn
    {
        get
        {
            try
            {
                var elapsed = DateTime.Now.Subtract(m_lastUsedDateTime);
                var reuse = new TimeSpan(0, 0, Template.CanUseEvery);
    ...
    {
        get => Template.GetType() == typeof(ItemTemplate) ? Template as ItemTemplate : null;
        set
        {
            if (value != null) Template = value as ItemTemplate;
        }
    }
    ...
    {
        get => Template.GetType() == typeof(ItemUnique) ? Template as ItemUnique : null;
        set
        {
    ...
    public virtual ItemTemplate Template
    {
        get
        {
            if (m_item == null)
            {
                ...
                {
                    log.ErrorFormat(
                        "Template null for player {0} Inventory.  Can't find template {1}{2} for item in slot {3}!",
                        OwnerID, ITemplate_Id == null ? string.Empty : ITemplate_Id,
                        UTemplate_Id == null ? string.Empty : UTemplate_Id, m_slot_pos);
    ...
    /// </summary>
    public virtual long RepairCost => (Template.MaxCondition - Condition) * Template.Price / Template.MaxCondition;
    ...
    public virtual byte DurabilityPercent =>
        (byte) (Template.MaxDurability > 0 ? Durability * 100 / Template.MaxDurability : 0);

    public virtual byte ConditionPercent =>
        (byte) Math.Round(Template.MaxCondition > 0
            ? (double) Condition / Template.MaxCondition * 100
            : 0);

    public virtual bool IsMagical => Template.IsMagical;

    public virtual bool IsStackable => Template.IsStackable;
