...
public sealed class DataTableHandler
{
    ...
    /// </summary>
    public ElementBinding PrimaryKey => PrimaryKeys.FirstOrDefault();

    /// <summary>
    /// Retrieve Multiple Primary Key Binding (Future Support)
    /// </summary>
    public ElementBinding[] PrimaryKeys
    {
        get
        {
            return Table.PrimaryKey.Select(col => ElementBindings.FirstOrDefault(bind =>
                bind.ColumnName.Equals(col.ColumnName, StringComparison.OrdinalIgnoreCase))).ToArray();
        }
    }
    ...
    public DataTableHandler(Type ObjectType)
    {
        ...
        // Parse Table Type
        ElementBindings = ObjectType.GetMembers().Select(member => new ElementBinding(member))
            .Where(bind => bind.IsDataElementBinding).ToArray();
            ...
            if (bind.DataElement != null)
            {
                ...
                if (!string.IsNullOrEmpty(bind.DataElement.UniqueColumns))
                    ...
                    Table.Constraints.Add(
                        new UniqueConstraint(string.Format("U_{0}_{1}", TableName, bind.ColumnName), column));

                // Store Indexes for definition after table
                if (!string.IsNullOrEmpty(bind.DataElement.IndexColumns))
                    indexes.Add(string.Format("I_{0}_{1}", TableName, bind.ColumnName), bind.DataElement
                        .IndexColumns.Split(',')
                        .Select(col => FieldElementBindings.FirstOrDefault(ind =>
                            ind.ColumnName.Equals(col.Trim(), StringComparison.OrdinalIgnoreCase)))
        ...
        {
            var columns = bind.DataElement.UniqueColumns.Split(',').Select(column => column.Trim())
                .Concat(new[] {bind.ColumnName});
