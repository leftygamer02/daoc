...
public static class DatabaseUtils
{
    ...
    public static ElementBinding[][] GetUniqueMembers(Type objectType)
    {
        ...
        var uniques = tableHandler.Table.Constraints.OfType<UniqueConstraint>()
            .Where(constraint => !constraint.IsPrimaryKey)
            .Select(constraint => constraint.Columns
                .Select(col => tableHandler.FieldElementBindings.Single(bind =>
                    bind.ColumnName.Equals(col.ColumnName, StringComparison.OrdinalIgnoreCase)))
                .ToArray());

        var primary = tableHandler.FieldElementBindings.Where(bind => bind.PrimaryKey != null
                                                                      && !bind.PrimaryKey.AutoIncrement
                                                                      && !bind.ColumnName.Equals(
                                                                          string.Format("{0}_ID",
