...
public abstract class ObjectDatabase : IObjectDatabase
{
            ...
            foreach (var allowed in grp.GroupBy(item => item.AllowAdd))
                if (allowed.Key)
                {
                    ...
                    var resultsByObjs = results
                        .Select((result, index) => new {Success = result, DataObject = objs[index]})
                    ...
                    foreach (var resultGrp in resultsByObjs)
                        if (resultGrp.Key)
                        {
                            ...
                            if (tableHandler.HasRelations)
                                success &= SaveObjectRelations(tableHandler,
                                    resultGrp.Select(obj => obj.DataObject));
                        }
                        else
                        {
                            if (log.IsErrorEnabled)
                                foreach (var obj in resultGrp)
                                    log.ErrorFormat(
                                        "AddObjects: DataObject ({0}) could not be inserted into database...",
                                        obj.DataObject);
        ...
        foreach (var grp in dataObjects.GroupBy(obj => obj.GetType()))
        {
            ...
            foreach (var resultGrp in resultsByObjs)
                ...
                else
                {
                    if (log.IsErrorEnabled)
                        foreach (var obj in resultGrp)
                            log.ErrorFormat("SaveObject: DataObject ({0}) could not be saved into database...",
                                obj.DataObject);
        ...
        foreach (var grp in dataObjects.GroupBy(obj => obj.GetType()))
        {
            ...
            foreach (var allowed in grp.GroupBy(item => item.AllowDelete))
                if (allowed.Key)
                {
                    ...
                    foreach (var resultGrp in resultsByObjs)
                        if (resultGrp.Key)
                        {
                            ...
                            if (tableHandler.HasRelations)
                                success &= DeleteObjectRelations(tableHandler,
                                    resultGrp.Select(obj => obj.DataObject));
                        }
                        else
                        {
                            if (log.IsErrorEnabled)
                                foreach (var obj in resultGrp)
                                    log.ErrorFormat(
                                        "DeleteObject: DataObject ({0}) could not be deleted from database...",
                                        obj.DataObject);
                ...
                else
                {
                    if (log.IsWarnEnabled)
                        foreach (var obj in allowed)
                            log.WarnFormat("DeleteObject: DataObject ({0}) not allowed to be deleted from Database",
                                obj);
    ...
    protected bool SaveObjectRelations(DataTableHandler tableHandler, IEnumerable<DataObject> dataObjects)
    {
        ...
        foreach (var relation in tableHandler.ElementBindings.Where(bind => bind.Relation != null))
        {
            ...
            {
                if (log.IsErrorEnabled)
                    log.ErrorFormat("SaveObjectRelations: Remote Table for Type ({0}) is not registered !",
                        relation.ValueType.FullName);
                success = false;
                continue;
            }
            ...
            var groups = relation.ValueType.HasElementType
                ? dataObjects.Select(obj => new
                        {Source = obj, Enumerable = (IEnumerable<DataObject>) relation.GetValue(obj)})
                    .Where(obj => obj.Enumerable != null).Select(obj =>
                        obj.Enumerable.Select(rel => new {Local = obj.Source, Remote = rel}))
                    .SelectMany(obj => obj).Where(obj => obj.Remote != null).GroupBy(obj => obj.Remote.IsPersisted)
                : dataObjects.Select(obj => new {Local = obj, Remote = (DataObject) relation.GetValue(obj)})
                    .Where(obj => obj.Remote != null).GroupBy(obj => obj.Remote.IsPersisted);
            ...
            foreach (var allowed in grp.GroupBy(obj => grp.Key ? obj.Remote.Dirty : obj.Remote.AllowAdd))
                if (allowed.Key)
                {
                    var objs = allowed.ToArray();
                    var results = grp.Key
                        ? SaveObjectImpl(remoteHandler, objs.Select(obj => obj.Remote))
                        : AddObjectImpl(remoteHandler, objs.Select(obj => obj.Remote));

                    var resultsByObjs = results.Select((result, index) =>
                        new {Success = result, RelObject = objs[index]});
                            ...
                            if (remoteHandler.UsesPreCaching)
                            {
                                ...
                                if (primary != null)
                                    foreach (var successObj in resultGrp.Select(obj => obj.RelObject.Remote))
                                        remoteHandler.SetPreCachedObject(primary.GetValue(successObj),
                                            successObj);
                            }
                        }
                        else
                        {
                            if (log.IsErrorEnabled)
                                foreach (var result in resultGrp)
                                    log.ErrorFormat(
                                        "SaveObjectRelations: {0} Relation ({1}) of DataObject ({2}) failed for Object ({3})",
                                        grp.Key ? "Saving" : "Adding",
                ...
                else
                {
                    ...
                    if (!grp.Key)
                    {
                        if (log.IsWarnEnabled)
                            foreach (var obj in allowed)
                                log.WarnFormat(
                                    "SaveObjectRelations: DataObject ({0}) not allowed to be added to Database",
                                    obj);
    ...
    {
        var success = true;
        foreach (var relation in tableHandler.ElementBindings.Where(bind =>
                     bind.Relation != null && bind.Relation.AutoDelete))
        {
            // Relation Check
            var remoteHandler = GetTableHandler(relation.ValueType);
            if (remoteHandler == null)
            {
                if (log.IsErrorEnabled)
                    log.ErrorFormat("DeleteObjectRelations: Remote Table for Type ({0}) is not registered !",
                        relation.ValueType.FullName);
                success = false;
                continue;
            }
                ...
                else
                {
                    ...
                    if (log.IsWarnEnabled)
                        foreach (var obj in grp)
                            log.WarnFormat(
                                "DeleteObjectRelations: DataObject ({0}) not allowed to be deleted from Database",
                                obj);
            ...
            try
            {
                ...
                if (!TableDatasets.TryGetValue(tableName, out tableHandler))
                    throw new DatabaseException(
                        string.Format("Table {0} is not registered for Database Connection...", tableName));
                ...
                foreach (var relation in relations)
                {
                    ...
                    try
                    {
                        ...
                        if (!TableDatasets.TryGetValue(remoteName, out remoteHandler))
                            throw new DatabaseException(
                                string.Format("Table {0} is not registered for Database Connection...",
                                    remoteName));

                        // Select Object On Relation Constraint
                        var localBind = tableHandler.FieldElementBindings.Single(bind =>
                            bind.ColumnName.Equals(relation.Relation.LocalField,
                                StringComparison.OrdinalIgnoreCase));
                        var remoteBind = remoteHandler.FieldElementBindings.Single(bind =>
                            bind.ColumnName.Equals(relation.Relation.RemoteField,
                                StringComparison.OrdinalIgnoreCase));
                    ...
                    catch (Exception re)
                    {
                        if (log.IsErrorEnabled)
                            log.ErrorFormat(
                                "Could not Retrieve Objects from Relation (Table {0}, Local {1}, Remote Table {2}, Remote {3})\n{4}",
                                tableName,
                                relation.Relation.LocalField,
                                AttributesUtils.GetTableOrViewName(relation.ValueType),
                                relation.Relation.RemoteField, re);
                    }
                }
            }
            catch (Exception e)
    ...
    protected virtual void FillObjectRelationsImpl(ElementBinding relationBind, ElementBinding localBind,
        ElementBinding remoteBind, DataTableHandler remoteHandler, IEnumerable<DataObject> dataObjects)
    {
        ...
        if (remoteHandler.UsesPreCaching)
        {
            ...
            if (remoteHandler.PrimaryKeys.All(pk =>
                    pk.ColumnName.Equals(remoteBind.ColumnName, StringComparison.OrdinalIgnoreCase)))
        ...
        {
            var whereClauses =
                objects.Select(obj => DB.Column(remoteBind.ColumnName).IsEqualTo(localBind.GetValue(obj)));
        ...
        foreach (var result in resultByObjs)
            if (isElementType)
            {
                ...
                {
                    var castMethod = typeof(Enumerable).GetMethod("OfType").MakeGenericMethod(type);
                    var methodToArray = typeof(Enumerable).GetMethod("ToArray").MakeGenericMethod(type);
                    relationBind.SetValue(result.DataObject,
                        methodToArray.Invoke(null,
                            new object[] {castMethod.Invoke(null, new object[] {result.Results})}));
                }
                else
                {
        ...
        where TObject : DataObject
    {
        ...
        {
            if (log.IsErrorEnabled)
                log.ErrorFormat("FindObjectByKey: DataObject Type ({0}) not registered !",
    ...
    /// <param name="keys">Collection of Primary Key Values</param>
    /// <returns>Collection of DataObject with primary key matching values</returns>
    protected abstract IEnumerable<DataObject> FindObjectByKeyImpl(DataTableHandler tableHandler,
    ...
    public IList<IList<TObject>> MultipleSelectObjects<TObject>(IEnumerable<WhereClause> whereClauseBatch)
        where TObject : DataObject
    {
        if (whereClauseBatch == null)
            throw new ArgumentNullException("Parameter whereClauseBatch may not be null.");
        ...
        var objs = MultipleSelectObjectsImpl(tableHandler, whereClauseBatch)
            .Select(res => res.OfType<TObject>().ToArray()).ToArray();
        ...
        where TObject : DataObject
    {
        ...
        {
            if (log.IsErrorEnabled)
                log.ErrorFormat("SelectAllObjects: DataObject Type ({0}) not registered !",
                    typeof(TObject).FullName);
        ...
        var dataObjects = MultipleSelectObjectsImpl(tableHandler, new[] {WhereClause.Empty}).Single()
            .OfType<TObject>().ToArray();
    ...
    protected abstract IEnumerable<bool> AddObjectImpl(DataTableHandler tableHandler,
    ...
    protected abstract IEnumerable<bool> SaveObjectImpl(DataTableHandler tableHandler,
        IEnumerable<DataObject> dataObjects);
    ...
    protected abstract IEnumerable<bool> DeleteObjectImpl(DataTableHandler tableHandler,
        IEnumerable<DataObject> dataObjects);
    ...
    protected abstract IList<IList<DataObject>> SelectObjectsImpl(DataTableHandler tableHandler,
        string whereExpression, IEnumerable<IEnumerable<QueryParameter>> parameters,
        Transaction.IsolationLevel isolation);

    protected abstract IList<IList<DataObject>> MultipleSelectObjectsImpl(DataTableHandler tableHandler,
