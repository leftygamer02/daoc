...
public class MySQLObjectDatabase : SQLObjectDatabase
{
        ...
        : base(ConnectionString)
    {
        ...
        if (!this.ConnectionString.Contains("Treat Tiny As Boolean") &&
            !this.ConnectionString.Contains("TreatTinyAsBoolean"))
            this.ConnectionString += ";Treat Tiny As Boolean=False";

        if (!this.ConnectionString.Contains("Allow User Variables") &&
            !this.ConnectionString.Contains("AllowUserVariables"))
            this.ConnectionString += ";Allow User Variables=True";

        if (!this.ConnectionString.Contains("Convert Zero Datetime") &&
            !this.ConnectionString.Contains("ConvertZeroDateTime"))
    ...
    protected virtual string GetDatabaseType(ElementBinding bind, DataTableHandler table)
    {
        ...
        else if (bind.ValueType == typeof(string))
        {
            ...
            else if (table.Table.PrimaryKey.Any(key =>
                         key.ColumnName.Equals(bind.ColumnName, StringComparison.OrdinalIgnoreCase))
                     || table.Table.Constraints.OfType<UniqueConstraint>().Any(cstrnt =>
                         cstrnt.Columns.Any(col =>
                             col.ColumnName.Equals(bind.ColumnName, StringComparison.OrdinalIgnoreCase)))
                     || (table.Table.ExtendedProperties["INDEXES"] != null &&
                         (table.Table.ExtendedProperties["INDEXES"] as Dictionary<string, DataColumn[]>)
                         .Any(kv => kv.Value.Any(col =>
                             col.ColumnName.Equals(bind.ColumnName, StringComparison.OrdinalIgnoreCase)))))
    ...
    {
        var type = GetDatabaseType(bind, table);
                ...
                reader =>
                {
                    while (reader.Read())
                    {
                        ...
                        if (log.IsDebugEnabled)
                            log.DebugFormat("CheckOrCreateTable: Found Column {0} in existing table {1}", column,
                                table.TableName);
                    }

                    if (log.IsDebugEnabled)
                        log.DebugFormat("CheckOrCreateTable: {0} columns existing in table {1}",
                            currentTableColumns.Count, table.TableName);
                });
        }
        catch (Exception e)
        {
    ...
    protected void CreateTable(DataTableHandler table)
    {
        ...
        var indexesFields = indexes == null
            ? new string[] { }
    ...
    protected void AlterTable(IEnumerable<TableRowBindind> currentColumns, DataTableHandler table)
    {
        ...
        {
            var column = currentColumns.FirstOrDefault(col =>
                col.ColumnName.Equals(binding.ColumnName, StringComparison.OrdinalIgnoreCase));

            if (column != null)
            {
                // Check Null && Type
                if ((binding.DataElement != null && binding.DataElement.AllowDbNull) != column.AllowDbNull
                    || !GetDatabaseType(binding, table)
                        .Equals(column.ColumnType, StringComparison.OrdinalIgnoreCase))
                {
                    columnDefs.Add(string.Format("CHANGE `{1}` {0}", GetColumnDefinition(binding, table),
                        binding.ColumnName));
            ...
            ExecuteSelectImpl(string.Format("SHOW INDEX FROM `{0}`", table.TableName),
                ...
                reader =>
                {
                    while (reader.Read())
                    {
                        ...
                        if (log.IsDebugEnabled)
                            log.DebugFormat(
                                "AlterTable: Found Index `{0}` (Unique:{1}) on `{2}` in existing table {3}",
                                indexname, unique, column, table.TableName);
                    }

                    if (log.IsDebugEnabled)
                        log.DebugFormat("AlterTable: {0} Indexes existing in table {1}", indexes.Count,
                            table.TableName);
                });
        }
        catch (Exception e)
        ...
        var existingIndexes = indexes.GroupBy(ind => new {KeyName = ind.Item2, Unique = ind.Item1})
            .Select(grp => new {grp.Key.KeyName, grp.Key.Unique, Columns = grp.Select(i => i.Item3).ToArray()})
            .ToArray();
        ...
        if (table.Table.PrimaryKey.Length != currentPrimaryColumn.Length
            ...
            {
                var column = currentPrimaryColumn.FirstOrDefault(col =>
                    col.Equals(pk.ColumnName, StringComparison.OrdinalIgnoreCase));

                return column == null;
            }))
        {
            // Allow to edit Auto increment key if not previously modified
            foreach (var oldkeys in currentColumns.Where(col =>
                         col.Primary && !alteredColumn.Any(c =>
                             c.Equals(col.ColumnName, StringComparison.OrdinalIgnoreCase))))
                columnDefs.Add(string.Format("MODIFY `{0}` {1} {2}", oldkeys.ColumnName, oldkeys.ColumnType,
                    oldkeys.AllowDbNull ? "DEFAULT NULL" : "NOT NULL"));

            if (currentPrimaryColumn.Any())
                columnDefs.Add("DROP PRIMARY KEY");

            columnDefs.Add(string.Format("ADD PRIMARY KEY ({0})",
                string.Join(", ", table.Table.PrimaryKey.Select(pk => string.Format("`{0}`", pk.ColumnName)))));
        }
        ...
        foreach (var existing in existingIndexes.Where(ind => !ind.KeyName.Equals("PRIMARY")))
        {
            ...
            if (tableIndexes.TryGetValue(existing.KeyName, out realindex))
            {
                ...
                if (realindex.Length != existing.Columns.Length
                    || !realindex.All(col =>
                        existing.Columns.Any(c => c.Equals(col.ColumnName, StringComparison.OrdinalIgnoreCase))))
                {
                    columnDefs.Add(string.Format("DROP KEY `{0}`", existing.KeyName));
                    columnDefs.Add(string.Format("ADD KEY `{0}` ({1})", existing.KeyName,
                        string.Join(", ", realindex.Select(col => string.Format("`{0}`", col)))));
                }
            }
            else
            {
                // Check for Unique
                var realunique = table.Table.Constraints.OfType<UniqueConstraint>().FirstOrDefault(cstrnt =>
                    !cstrnt.IsPrimaryKey &&
                    cstrnt.ConstraintName.Equals(existing.KeyName, StringComparison.OrdinalIgnoreCase));
                if (realunique == null)
                {
                    columnDefs.Add(string.Format("DROP KEY `{0}`", existing.KeyName));
                }
                else if (realunique.Columns.Length != existing.Columns.Length
                         || !realunique.Columns.All(col =>
                             existing.Columns.Any(c =>
                                 c.Equals(col.ColumnName, StringComparison.OrdinalIgnoreCase))))
                {
                    columnDefs.Add(string.Format("DROP KEY `{0}`", existing.KeyName));
                    columnDefs.Add(string.Format("ADD UNIQUE KEY `{0}` ({1})", existing.KeyName,
                        string.Join(", ", realunique.Columns.Select(col => string.Format("`{0}`", col)))));
                }
            }
        }

        // Missing Indexes
        foreach (var missing in tableIndexes.Where(kv =>
                     existingIndexes.All(c => !c.KeyName.Equals(kv.Key, StringComparison.OrdinalIgnoreCase))))
            columnDefs.Add(string.Format("ADD KEY `{0}` ({1})", missing.Key,
                string.Join(", ", missing.Value.Select(col => string.Format("`{0}`", col)))));

        foreach (var missing in table.Table.Constraints.OfType<UniqueConstraint>().Where(cstrnt =>
                     !cstrnt.IsPrimaryKey && existingIndexes.All(c =>
                         !c.KeyName.Equals(cstrnt.ConstraintName, StringComparison.OrdinalIgnoreCase))))
            columnDefs.Add(string.Format("ADD UNIQUE KEY `{0}` ({1})", missing.ConstraintName,
                string.Join(", ", missing.Columns.Select(col => string.Format("`{0}`", col)))));
            ...
            using (var cmd = conn.CreateCommand())
            {
                ...
                using (var tran = conn.BeginTransaction(System.Data.IsolationLevel.Serializable))
                {
                    try
                    {
                        ...
                        // Update Null Values
                        var nullToNonNull = table.FieldElementBindings.Join(currentColumns, bind => bind.ColumnName,
                                col => col.ColumnName, (bind, col) => new {bind, col},
                                StringComparer.OrdinalIgnoreCase)
                            .Where(match =>
                                match.bind.DataElement != null && match.bind.DataElement.AllowDbNull == false &&
                                match.col.AllowDbNull == true)
                        ...
                        if (nullToNonNull.Any())
                        {
                            cmd.CommandText = string.Format("UPDATE `{0}` SET {1} WHERE {2}"
                                , table.TableName
                                , string.Join(", ", nullToNonNull.Select(bind =>
                                    string.Format("`{0}` = IFNULL(`{0}`, {1})", bind.ColumnName,
                                        bind.ValueType == typeof(DateTime)
                                            ? "'2000-01-01 00:00:00'"
                        ...
                        // Alter Table
                        cmd.CommandText = string.Format("ALTER TABLE `{0}` {1}", table.TableName,
                            string.Join(", \n", columnDefs));
                        cmd.ExecuteNonQuery();
    ...
    /// </summary>
    public override ConnectionType ConnectionType => ConnectionType.DATABASE_MYSQL;
    ...
    protected override object[] ExecuteScalarImpl(string SQLCommand,
            ...
            using (var conn = new MySqlConnection(ConnectionString))
            {
                using (var cmd = conn.CreateCommand())
                {
                    try
                    {
                            ...
                            if (retrieveLastInsertID)
                            {
                                using (var tran = conn.BeginTransaction(System.Data.IsolationLevel.ReadUncommitted))
                                {
                                        ...
                                        catch (Exception ex)
                                        {
                                            if (HandleSQLException(ex))
                                            {
                                                ...
                                                if (log.IsErrorEnabled)
                                                    log.ErrorFormat(
                                                        "ExecuteScalarImpl: Constraint Violation for command \"{0}\"\n{1}\n{2}",
                                    ...
                                    catch (Exception te)
                                    {
                                        ...
                                        if (log.IsErrorEnabled)
                                            log.ErrorFormat(
                                                "ExecuteScalarImpl: Error in Transaction (Rollback) for command : {0}\n{1}",
                                                SQLCommand, te);
                                    }
                                }
                            }
                        ...
                        if (log.IsDebugEnabled)
                            log.DebugFormat("ExecuteScalarImpl: SQL ScalarQuery exec time {0}ms",
