...
public class SQLiteObjectDatabase : SQLObjectDatabase
{
    ...
    protected IEnumerable<string> GetIndexesDefinition(DataTableHandler table)
    {
        ...
        var uniqueFields = table.Table.Constraints.OfType<UniqueConstraint>().Where(cstrnt => !cstrnt.IsPrimaryKey)
            .Select(cstrnt => string.Format("CREATE UNIQUE INDEX IF NOT EXISTS `{0}` ON `{2}` ({1})",
                cstrnt.ConstraintName,
    ...
    protected bool CheckTableAlteration(IEnumerable<TableRowBindind> currentColumns, DataTableHandler table)
    {
        ...
        if (table.Table.PrimaryKey.Length != currentColumns.Count(col => col.Primary)
            ...
            {
                var column = currentColumns.FirstOrDefault(col =>
                    col.ColumnName.Equals(pk.ColumnName, StringComparison.OrdinalIgnoreCase));
    ...
    protected void CheckTableIndexAlteration(DataTableHandler table)
    {
        ...
        var sortedIndexes = currentIndexes.Select(ind =>
        {
            ...
            var columns = ind.Item2.Substring(ind.Item2.IndexOf('(')).Split(',')
                .Select(sp => sp.Trim('`', '(', ')', ' '));
            return new {KeyName = ind.Item1, Unique = unique, Columns = columns.ToArray()};
        }).ToArray();
        if (log.IsDebugEnabled)
            log.DebugFormat("CheckTableIndexAlteration: {0} Indexes existing in table {1}", sortedIndexes.Length,
                table.TableName);
        ...
        foreach (var existing in sortedIndexes)
        {
            if (log.IsDebugEnabled)
                log.DebugFormat(
                    "CheckTableIndexAlteration: Found Index `{0}` (Unique:{1}) on ({2}) in existing table {3}",
                    existing.KeyName, existing.Unique, string.Join(", ", existing.Columns), table.TableName);
            ...
            if (tableIndexes.TryGetValue(existing.KeyName, out realindex))
            {
                        ...
                        existing.Columns.Any(c => c.Equals(col.ColumnName, StringComparison.OrdinalIgnoreCase))))
                {
                    ...
                    alterQueries.Add(string.Format("CREATE INDEX IF NOT EXISTS `{0}` ON `{2}` ({1})",
                        existing.KeyName, string.Join(", ", realindex.Select(col => string.Format("`{0}`", col))),
                        table.TableName));
                }
            }
            else
            {
                ...
                {
                    alterQueries.Add(string.Format("DROP INDEX `{0}`", existing.KeyName));
                    alterQueries.Add(string.Format("CREATE UNIQUE INDEX IF NOT EXISTS `{0}` ON `{2}` ({1})",
                        existing.KeyName,
        ...
        foreach (var missing in tableIndexes.Where(kv =>
                     sortedIndexes.All(c => !c.KeyName.Equals(kv.Key, StringComparison.OrdinalIgnoreCase))))
            alterQueries.Add(string.Format("CREATE INDEX IF NOT EXISTS `{0}` ON `{2}` ({1})", missing.Key,
                string.Join(", ", missing.Value.Select(col => string.Format("`{0}`", col))), table.TableName));

        foreach (var missing in table.Table.Constraints.OfType<UniqueConstraint>().Where(cstrnt =>
                     !cstrnt.IsPrimaryKey && sortedIndexes.All(c =>
                         !c.KeyName.Equals(cstrnt.ConstraintName, StringComparison.OrdinalIgnoreCase))))
            alterQueries.Add(string.Format("CREATE UNIQUE INDEX IF NOT EXISTS `{0}` ON `{2}` ({1})",
                missing.ConstraintName,
                string.Join(", ", missing.Columns.Select(col => string.Format("`{0}`", col))), table.TableName));
    ...
    protected void AlterTable(IEnumerable<TableRowBindind> currentColumns, DataTableHandler table)
    {
        ...
        catch (Exception e)
        {
            ...
            if (log.IsWarnEnabled)
                log.WarnFormat("AlterTableImpl: Error While Altering Table {0}, no modifications...",
                    table.TableName);
        ...
        using (var conn = new SQLiteConnection(ConnectionString))
        {
            ...
            using (var tran = conn.BeginTransaction(System.Data.IsolationLevel.ReadCommitted))
            {
                try
                {
                    ...
                    using (var command =
                           new SQLiteCommand(
                               string.Format("ALTER TABLE `{0}` RENAME TO `{0}_bkp`", table.TableName), conn))
                    {
                        command.Transaction = tran;
                        command.ExecuteNonQuery();
                    }
                    ...
                    var matchingColumns = table.FieldElementBindings.Join(currentColumns, bind => bind.ColumnName,
                        col => col.ColumnName, (bind, col) => new {bind, col}, StringComparer.OrdinalIgnoreCase);
                    var columns = matchingColumns.Select(match =>
                    {
                        if (match.bind.DataElement != null && match.bind.DataElement.AllowDbNull == false &&
                            match.col.AllowDbNull == true)
                        {
                            if (match.bind.ValueType == typeof(DateTime))
                                return new
                                {
                                    Target = match.bind.ColumnName,
                                    Source = string.Format("IFNULL(`{0}`, {1})", match.bind.ColumnName,
                                        "'2000-01-01 00:00:00'")
                                };
                            ...
                            {
                                Target = match.bind.ColumnName,
                                Source = string.Format("IFNULL(`{0}`, {1})", match.bind.ColumnName, "0")
                            };
                        }

                        return new
                        {
                            Target = match.bind.ColumnName, Source = string.Format("`{0}`", match.bind.ColumnName)
                        };
                    });

                    using (var command = new SQLiteCommand(
                               string.Format("INSERT INTO `{0}` ({1}) SELECT {2} FROM `{0}_bkp`", table.TableName,
                                   string.Join(", ", columns.Select(c => c.Target)),
                                   string.Join(", ", columns.Select(c => c.Source))), conn))
                    {
                        if (log.IsDebugEnabled)
                    ...
                    // Drop Renamed Table
                    using (var command = new SQLiteCommand(string.Format("DROP TABLE `{0}_bkp`", table.TableName),
                               conn))
                    {
                        command.Transaction = tran;
                        command.ExecuteNonQuery();
    ...
    /// </summary>
    public override ConnectionType ConnectionType => ConnectionType.DATABASE_SQLITE;
