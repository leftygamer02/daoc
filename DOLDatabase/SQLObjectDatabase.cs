...
public abstract class SQLObjectDatabase : ObjectDatabase
{
        ...
        if (TableDatasets.TryGetValue(tableName, out existingHandler))
        {
            if (dataObjectType != existingHandler.ObjectType)
                throw new DatabaseException(string.Format(
                    "Table Handler Duplicate for Type: {2}, Table Name '{0}' Already Registered with Type : {1}",
                    tableName, existingHandler.ObjectType, dataObjectType));
        ...
        try
        {
            if (isView)
            {
                if (!string.IsNullOrEmpty(viewAs))
                {
                    ...
                    ExecuteNonQueryImpl(string.Format("CREATE VIEW `{0}` AS {1}", tableName,
                        string.Format(viewAs,
                            string.Format("`{0}`", AttributesUtils.GetTableName(dataObjectType)))));
                }
            }
            else
            {
    ...
    protected override IEnumerable<bool> AddObjectImpl(DataTableHandler tableHandler,
        IEnumerable<DataObject> dataObjects)
    {
        ...
        {
            // Check Primary Keys
            var usePrimaryAutoInc =
                tableHandler.FieldElementBindings.Any(bind =>
                    bind.PrimaryKey != null && bind.PrimaryKey.AutoIncrement);

            // Columns
            var columns = tableHandler.FieldElementBindings
                .Where(bind => bind.PrimaryKey == null || !bind.PrimaryKey.AutoIncrement)
                .Select(bind => new
                {
                    Binding = bind, ColumnName = string.Format("`{0}`", bind.ColumnName),
                    ParamName = string.Format("@{0}", bind.ColumnName)
                }).ToArray();
            ...
            // Build Parameters
            var parameters = objs.Select(obj => columns.Select(col =>
                new QueryParameter(col.ParamName, col.Binding.GetValue(obj), col.Binding.ValueType)));
            ...
            if (usePrimaryAutoInc)
            {
                ...
                var binding = tableHandler.FieldElementBindings.First(bind =>
                    bind.PrimaryKey != null && bind.PrimaryKey.AutoIncrement);
                var resultByObjects = lastId.Select((result, index) =>
                    new {Result = Convert.ToInt64(result), DataObject = objs[index]});
                ...
                foreach (var result in resultByObjects)
                    ...
                    else
                    {
                        if (log.IsErrorEnabled)
                            log.ErrorFormat(
                                "Error adding data object into {0} Object = {1}, UsePrimaryAutoInc, Query = {2}",
            ...
            else
            {
                ...
                var resultByObjects =
                    affected.Select((result, index) => new {Result = result, DataObject = objs[index]});
    ...
    protected override IEnumerable<bool> SaveObjectImpl(DataTableHandler tableHandler,
        IEnumerable<DataObject> dataObjects)
    {
        var success = new List<bool>();
        if (!dataObjects.Any())
        ...
        try
        {
            ...
            var columns = tableHandler.FieldElementBindings
                .Where(bind => bind.PrimaryKey == null && bind.ReadOnly == null)
                .Select(bind => new
                {
                    Binding = bind, ColumnName = string.Format("`{0}`", bind.ColumnName),
                    ParamName = string.Format("@{0}", bind.ColumnName)
            ...
            if (!primary.Any())
                throw new DatabaseException(string.Format("Table {0} has no primary key for saving...",
            ...
            var command = string.Format("UPDATE `{0}` SET {1} WHERE {2}", tableHandler.TableName,
                ...
                string.Join(" AND ",
                    primary.Select(col => string.Format("{0} = {1}", col.ColumnName, col.ParamName))));

            var objs = dataObjects.ToArray();
            var parameters = objs.Select(obj => columns.Concat(primary).Select(col =>
                new QueryParameter(col.ParamName, col.Binding.GetValue(obj), col.Binding.ValueType)));
            ...
            foreach (var result in resultByObjects)
                ...
                else
                {
                    if (log.IsErrorEnabled)
                    {
                        if (result.Result < 0)
                            log.ErrorFormat(
                                "Error saving data object in table {0} Object = {1} --- constraint failed? {2}",
                                tableHandler.TableName, result.DataObject, command);
                        else
                            log.ErrorFormat(
                                "Error saving data object in table {0} Object = {1} --- keyvalue changed? {2}\n{3}",
                                tableHandler.TableName, result.DataObject, command, Environment.StackTrace);
                    }
    ...
    protected override IEnumerable<bool> DeleteObjectImpl(DataTableHandler tableHandler,
        IEnumerable<DataObject> dataObjects)
    {
        var success = new List<bool>();
        if (!dataObjects.Any())
        ...
        try
        {
            ...
            if (!primary.Any())
                throw new DatabaseException(string.Format("Table {0} has no primary key for deletion...",
                    tableHandler.TableName));
            ...
            var objs = dataObjects.ToArray();
            var parameters = objs.Select(obj => primary.Select(pk =>
                new QueryParameter(string.Format("@{0}", pk.ColumnName), pk.GetValue(obj), pk.ValueType)));
            ...
            foreach (var result in resultByObjects)
                ...
                else
                {
                    if (log.IsErrorEnabled)
                        log.ErrorFormat(
                            "Error deleting data object from table {0} Object = {1} --- keyvalue changed? {2}\n{3}",
                            tableHandler.TableName, result.DataObject, command, Environment.StackTrace);
                    success.Add(false);
                }
        }
    ...
    protected override IEnumerable<DataObject> FindObjectByKeyImpl(DataTableHandler tableHandler,
        IEnumerable<object> keys)
    {
        ...
        var primary = tableHandler.FieldElementBindings.Where(bind => bind.PrimaryKey != null)
            .Select(bind => new
            {
                ColumnName = string.Format("`{0}`", bind.ColumnName),
                ParamName = string.Format("@{0}", bind.ColumnName), ParamType = bind.ValueType
            }).ToArray();

        if (!primary.Any())
            throw new DatabaseException(string.Format("Table {0} has no primary key for finding by key...",
                tableHandler.TableName));
        ...
        foreach (var key in keys)
        {
            ...
            foreach (var column in primary) whereClause = whereClause.And(DB.Column(column.ColumnName).IsEqualTo(key));

            whereClauses.Add(whereClause);
        }

        var resultByKeys = MultipleSelectObjectsImpl(tableHandler, whereClauses)
            .Select(results => results.SingleOrDefault());
    ...
    {
        var tableName = AttributesUtils.GetTableOrViewName(typeof(TObject));
        DataTableHandler tableHandler;
        if (!TableDatasets.TryGetValue(tableName, out tableHandler))
            throw new DatabaseException(string.Format("Table {0} is not registered for Database Connection...",
                tableName));
    ...
    protected override IList<IList<DataObject>> SelectObjectsImpl(DataTableHandler tableHandler,
        string whereExpression, IEnumerable<IEnumerable<QueryParameter>> parameters,
        Transaction.IsolationLevel isolation)
    {
        ...
        ExecuteSelectImpl(command, parameters,
            reader => FillQueryResultList(reader, tableHandler, columns, primary, dataObjects));

        return dataObjects.ToArray();
    }

    protected override IList<IList<DataObject>> MultipleSelectObjectsImpl(DataTableHandler tableHandler,
        IEnumerable<WhereClause> whereClauseBatch)
    {
        var columns = tableHandler.FieldElementBindings.ToArray();

        var selectFromExpression = string.Format("SELECT {0} FROM `{1}` ",
            string.Join(", ", columns.Select(col => string.Format("`{0}`", col.ColumnName))),
            tableHandler.TableName);
        ...
        ExecuteSelectImpl(selectFromExpression, whereClauseBatch,
            reader => FillQueryResultList(reader, tableHandler, columns, primary, dataObjects));

        return dataObjects.ToArray();
    }

    private void FillQueryResultList(IDataReader reader, DataTableHandler tableHandler, ElementBinding[] columns,
        ElementBinding primary, List<IList<DataObject>> resultList)
    {
    ...
    protected virtual void DatabaseSetValue(DataObject obj, ElementBinding bind, object value)
    {
        ...
        {
            if (log.IsErrorEnabled)
                log.ErrorFormat("{0}: {1} = {2} doesnt fit to {3}\n{4}", obj.TableName, bind.ColumnName,
                    value.GetType().FullName, bind.ValueType, e);
        }
    }
    ...
    {
        dbParams.Clear();
        foreach (var param in parameter) dbParams.Add(ConvertToDBParameter(param));
    }
    ...
    protected virtual void ExecuteSelectImpl(string SQLCommand, IEnumerable<IEnumerable<QueryParameter>> parameters,
        ...
        do
        {
            ...
            using (var conn = CreateConnection(ConnectionString))
            {
                using (var cmd = conn.CreateCommand())
                {
                    ...
                    {
                        conn.Open();
                        var start = DateTime.UtcNow.Ticks / 10000;
                            ...
                            using (var reader = cmd.ExecuteReader())
                            {
                                ...
                                catch (Exception es)
                                {
                                    if (log.IsWarnEnabled)
                                        log.WarnFormat(
                                            "ExecuteSelectImpl: Exception in Select Callback : {2}{0}{2}{1}", es,
                        ...
                        if (log.IsDebugEnabled)
                            log.DebugFormat("ExecuteSelectImpl: SQL Select exec time {0}ms",
                                DateTime.UtcNow.Ticks / 10000 - start);
                    ...
                    catch (Exception e)
                    {
                        if (!HandleException(e))
                        {
                            if (log.IsErrorEnabled)
                                log.ErrorFormat(
                                    "ExecuteSelectImpl: UnHandled Exception for Select Query \"{0}\"\n{1}",
                                    SQLCommand, e);
    ...
    protected virtual void ExecuteSelectImpl(string selectFromExpression, IEnumerable<WhereClause> whereClauseBatch,
        Action<IDataReader> Reader)
    {
                ...
                using (var cmd = conn.CreateCommand())
                {
                    ...
                    catch (Exception e)
                    {
                        if (!HandleException(e))
                        {
                            if (log.IsErrorEnabled)
                                log.ErrorFormat(
                                    "ExecuteSelectImpl: UnHandled Exception in Select Query \"{0}\"\n{1}",
                                    selectFromExpression, e);
    ...
    protected virtual IEnumerable<int> ExecuteNonQueryImpl(string SQLCommand,
        IEnumerable<IEnumerable<QueryParameter>> parameters)
    {
        if (log.IsDebugEnabled)
            ...
            using (var conn = CreateConnection(ConnectionString))
            {
                using (var cmd = conn.CreateCommand())
                {
                    try
                    {
                        ...
                        foreach (var parameter in parameters.Skip(current))
                        {
                            ...
                            catch (Exception ex)
                            {
                                if (HandleSQLException(ex))
                                {
                                    ...
                                    if (log.IsErrorEnabled)
                                        log.ErrorFormat(
                                            "ExecuteNonQueryImpl: Constraint Violation for raw query \"{0}\"\n{1}\n{2}",
                                            SQLCommand, ex, Environment.StackTrace);
                                }
                                else
                                {
                                    throw;
                        ...
                        if (log.IsDebugEnabled)
                            log.DebugFormat("ExecuteNonQueryImpl: SQL NonQuery exec time {0}ms",
                                DateTime.UtcNow.Ticks / 10000 - start);
                        else if (log.IsWarnEnabled && DateTime.UtcNow.Ticks / 10000 - start > 500)
                    ...
                    catch (Exception e)
                    {
                        if (!HandleException(e))
                        {
                            if (log.IsErrorEnabled)
                                log.ErrorFormat(
                                    "ExecuteNonQueryImpl: UnHandled Exception for raw query \"{0}\"\n{1}",
                                    SQLCommand, e);
    ...
    /// <param name="retrieveLastInsertID">Return Last Insert ID of each Command instead of Scalar</param>
    /// <returns>Object Returned by Scalar</returns>
    protected object ExecuteScalarImpl(string SQLCommand, IEnumerable<QueryParameter> parameter,
        bool retrieveLastInsertID = false)
    {
    ...
    protected abstract object[] ExecuteScalarImpl(string SQLCommand,
        IEnumerable<IEnumerable<QueryParameter>> parameters, bool retrieveLastInsertID);
    ...
    protected virtual bool HandleException(Exception e)
    {
        ...
        if (socketException != null)
        {
            ...
            if (log.IsWarnEnabled)
                log.WarnFormat("Socket exception: ({0}) {1}; repeat: {2}", socketException.ErrorCode,
                    socketException.Message, ret);
        }
